...
title: Abonnements
description: Proposez une facturation récurrente pour les adhésions, SaaS ou
  services avec des cycles flexibles, des essais, des prorations, des options
  supplémentaires et des frais à la demande.
icon: repeat
---
...
- **Caisse sans couture** : Caisse hébergée et portail client
- **Orienté développeur** : API claires pour la création, les changements et le suivi d'utilisation

## Création d'abonnements

Créez des produits d'abonnement dans votre tableau de bord Dodo Payments, puis vendez-les via la caisse ou votre API. Séparer les produits des abonnements actifs vous permet de versionner les prix, d'attacher des options supplémentaires et de suivre les performances de manière indépendante.
...
- **Nom du produit** (obligatoire) : Le nom affiché dans la caisse, le portail client et les factures.
- **Description du produit** (obligatoire) : Une déclaration de valeur claire qui apparaît dans la caisse et les factures.
- **Image du produit** (obligatoire) : PNG/JPG/WebP jusqu'à 3 Mo. Utilisé dans la caisse et les factures.
...
- **Remise applicable (%)** : Pourcentage de remise optionnel appliqué au prix de base ; reflété dans la caisse et les factures.
...
<Warning>
Actuellement, il n'existe pas de champ direct pour détecter l'état d'essai. Ce qui suit est une solution de contournement qui nécessite de requérir des paiements, ce qui est inefficace. Nous travaillons sur une solution plus efficace.
</Warning>
...
Facture le montant proratisé basé sur le temps restant dans le cycle de facturation actuel. Idéal pour une facturation équitable qui prend en compte le temps non utilisé.
...
Facture la différence de prix immédiatement (mise à niveau) ou ajoute un crédit pour les renouvellements futurs (rétrogradation). Idéal pour des scénarios simples de mise à niveau/rétrogradation.
...
<Info>
Les crédits provenant des rétrogradations utilisant `difference_immediately` sont limités à l'abonnement et s'appliquent automatiquement aux renouvellements futurs. Ils sont distincts des <a href="/features/customer-credit">Crédits client</a>.
</Info>
...
### Changer de plans avec des options supplémentaires
...
Avant de vous engager dans un changement de plan, prévisualisez le montant exact et l'abonnement résultant :
...
- **`on_hold`** : L'abonnement est en pause en raison d'un échec de paiement. Une mise à jour du mode de paiement est requise pour le réactiver
...
- Un échec de paiement pour un changement de plan
...
<Info>
Après avoir mis à jour avec succès le mode de paiement pour un abonnement `on_hold`, vous recevrez `payment.succeeded` suivi des événements webhook `subscription.active`.
</Info>
...
<Accordion title="Changer de plan (prorata)">
...
Récupérez l'utilisation enregistrée pour les modèles de tarification mesurée ou hybride.
...
Améliorez ou dégradez un abonnement et contrôlez le comportement de prorata :
...
### Réactiver un abonnement en attente
Réactivez un abonnement qui a été mis en attente en raison d'un échec de paiement :
...
## Abonnements avec mandats conformes à la RBI
  ...
  Les abonnements UPI et par carte indienne fonctionnent sous les réglementations de la RBI (Banque de réserve de l'Inde) avec des exigences de mandat spécifiques :

  ### Limites de mandat

  Le type et le montant du mandat dépendent du montant récurrent de votre abonnement :
  ...
  ### Considérations pour les mises à niveau et les rétrogradations
  ...
  **Important :** Lors de la mise à niveau ou de la rétrogradation des abonnements, considérez attentivement les limites de mandat :
  ...
  ### Autorisation pour les prélèvements de grande valeur
  ...
  **Calendrier de traitement :** Les prélèvements récurrents sur les cartes indiennes et les abonnements UPI suivent un modèle de traitement unique :
...
<Info>
Les abonnements sont une base flexible pour les revenus récurrents. Commencez simplement, testez en profondeur et itérez en fonction des métriques d'adoption, de désabonnement et d'expansion.
</Info>