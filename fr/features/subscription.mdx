...
<Warning>
La valeur de `trial_period_days` doit être comprise entre 0 et 10 000 jours.
</Warning>
...
Les frais sont calculés au prorata en fonction du temps restant dans le cycle de facturation actuel. Idéal pour une facturation équitable qui prend en compte le temps non utilisé.
...
Facture immédiatement la différence de prix (mise à niveau) ou ajoute un crédit pour les renouvellements futurs (baisse de niveau). Idéal pour des scénarios de mise à niveau/baisse simples.
...
<Info>
Les crédits provenant des baisses de niveau utilisant `difference_immediately` sont limités à l'abonnement et sont automatiquement appliqués aux renouvellements futurs. Ils sont distincts des <a href="/features/customer-credit">Crédits clients</a>.
</Info>
...
<Info>
Les changements de plan déclenchent des frais immédiats. Les frais échoués peuvent déplacer l'abonnement vers le statut `on_hold`. Suivez les changements via les événements de webhook `subscription.plan_changed`.
</Info>
...
<Warning>
Lorsqu'un abonnement est dans l'état `on_hold`, il ne se renouvellera pas automatiquement. Vous devez mettre à jour le mode de paiement pour réactiver l'abonnement.
</Warning>

### Réactivation depuis l'état en attente

Pour réactiver un abonnement de l'état `on_hold`, mettez à jour le mode de paiement. Cela crée automatiquement :

1. Un frais pour les montants dus restants
2. Une facture
3. Un traitement du paiement en utilisant le nouveau mode de paiement
...
<Info>
Après avoir mis à jour avec succès le mode de paiement pour un abonnement {/* INLINE_CODE_PLACEHOLDER_833e01167b2e3da70d83bdfb406adc7f */, vous recevrez `payment.succeeded` suivi des événements de webhook `subscription.active`.
</Info>
...
Mettez à jour le mode de paiement pour un abonnement. Pour les abonnements actifs, cela met à jour le mode de paiement pour les renouvellements futurs. Pour les abonnements dans l'état {/* INLINE_CODE_PLACEHOLDER_833e01167b2e3da70d83bdfb406adc7f */, cela réactive l'abonnement en créant un frais pour les montants dus restants.
