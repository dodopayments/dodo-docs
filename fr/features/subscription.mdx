...
| Échec du renouvellement → en pause | `subscription.on_hold` |
| Abonnement mis en pause | `subscription.paused` |
| Reprise de l’abonnement en pause | `subscription.unpaused` |
| Échec de la création | `subscription.failed` |
| Plan mis à niveau/rétrogradé | `subscription.plan_changed` |
| Annulé | `subscription.cancelled` |
| Terme arrivé à échéance | `subscription.expired` |
| Modification d’un champ quelconque | `subscription.updated` |
...
Utilisez `PATCH /subscriptions/{subscription_id}` pour annuler à la prochaine date de facturation, prolonger la période d’abonnement, mettre à jour les informations de facturation ou modifier les métadonnées. Pour modifier la quantité, utilisez plutôt la [Change Plan API](/api-reference/subscriptions/change-plan) — `PATCH` n’accepte pas `quantity`.
