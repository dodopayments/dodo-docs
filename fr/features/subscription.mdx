...
Utilisez `PATCH /subscriptions/{id}` pour annuler à la prochaine date de facturation, prolonger la période d’abonnement, mettre à jour les informations de facturation ou modifier les métadonnées. Pour modifier la quantité, utilisez plutôt l’[API Change Plan](/api-reference/subscriptions/change-plan) — `PATCH` n’accepte pas `quantity`.
