...
| `on_hold` | Un paiement de renouvellement (ou une facturation liée à un changement de plan) a échoué ; les renouvellements sont arrêtés | **Oui** | Mettre à jour le moyen de paiement pour réactiver l'abonnement — automatiquement via [Payment Retries](/features/recovery/payment-retries) et [Dunning](/features/recovery/subscription-dunning), ou manuellement via l'[API Update Payment Method](/api-reference/subscriptions/update-payment-method) |
| `paused` | L'abonnement a été volontairement mis en pause par vous ou le client ; la facturation est gelée et l'accès est révoqué | **Oui** | Le reprendre avec `pause: false`, ou depuis le Customer Portal lorsque la mise en pause en libre-service est activée — consultez [Mettre en pause et reprendre les abonnements](#pausing-and-resuming-subscriptions) |
| `cancelled` | L'abonnement a été résilié et ne sera pas renouvelé | Rachat uniquement | Le client doit démarrer un nouvel abonnement ; le [Dunning](/features/recovery/subscription-dunning) peut l'inviter à effectuer un rachat |
| `failed` | La **création** de l'abonnement a échoué (le mandat initial ou le paiement n'a pas abouti) | **Non — terminal** | Le client doit créer un nouvel abonnement avec un moyen de paiement fonctionnel |
| `expired` | L'abonnement est arrivé au terme de sa durée | — | Le client doit démarrer un nouvel abonnement s'il le souhaite |
