...
sidebarTitle: Europe
description: Acceptez iDEAL, Bancontact, EPS, Multibanco et Przelewy24 pour
  augmenter les taux de conversion sur les marchés européens. Découvrez la
  disponibilité régionale, la couverture bancaire et la configuration.
icon: euro-sign
keywords:
  - Dodo Payments
  - paiements européens
  ...
  - méthodes de paiement européennes
---
...
Pour les entreprises desservant plusieurs pays européens, incluez toutes les méthodes régionales :
...
Utilisez vos clés API test Dodo Payments.
</Step>
...
- `AT` pour EPS
- `PT` pour Multibanco
- `PL` pour Przelewy24 (avec devise de facturation PLN)
</Step>
...
Si vous vendez à des clients néerlandais, incluez iDEAL. Ne pas le faire est comme ne pas accepter Visa aux États-Unis — vous perdrez des ventes importantes.
</Accordion>

<Accordion title="Match currency to region">
La plupart des méthodes de paiement européennes nécessitent l'EUR — assurez-vous que vos prix prennent en charge les transactions en Euro. La seule exception est Przelewy24, qui est proposé uniquement avec une facturation en **PLN** (Złoty polonais).
</Accordion>

<Accordion title="Handle redirects gracefully">
Toutes les méthodes européennes impliquent des redirections vers des sites bancaires. Assurez-vous que la gestion de votre URL de retour soit robuste et tienne compte des utilisateurs qui abandonnent en cours de flux.
</Accordion>

<Accordion title="Provide card fallbacks">
Tous les clients européens n'ont pas accès à ces méthodes régionales (touristes, expatriés, etc.). Incluez toujours `credit` et `debit` en tant que solutions de secours.
</Accordion>

<Accordion title="Consider Multibanco timing">
Les paiements aux guichets automatiques Multibanco peuvent prendre des heures pour être finalisés. Ne bloquez pas l'exécution sur un paiement immédiat — utilisez des webhooks pour la confirmation asynchrone.
</Accordion>
</AccordionGroup>
...
2. Devise définie sur EUR ?
3. Méthode incluse dans `allowed_payment_method_types` ?

**Solution :** Les méthodes européennes sont strictement régionales. Un client avec un pays de facturation `DE` (Allemagne) ne verra pas iDEAL, qui est réservé aux Pays-Bas.
</Accordion>
...
- Le client a entré des informations incorrectes

**Solution :** Le client doit réessayer. Si le problème persiste, suggérez une autre méthode de paiement.
</Accordion>
...
**Solution :** Cela est attendu pour les paiements basés sur les guichets automatiques. Attendez la confirmation du webhook. La référence expire généralement en 24 à 72 heures.
</Accordion>
</AccordionGroup>

## Conformité PSD2

Toutes les méthodes de paiement européennes se conforment aux réglementations PSD2 (Directive sur les services de paiement 2) :

- **Authentification Forte du Client (SCA)** — Intégrée dans le flux d'authentification bancaire
- **Communication Sécurisée** — Toutes les données transmises via des canaux sécurisés
- **Protection du Consommateur** — Plein respect des droits des consommateurs de l'UE
