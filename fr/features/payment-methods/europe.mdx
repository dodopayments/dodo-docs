...
<Step title="Configurer l'adresse de facturation appropriée">
Mettez le pays de l'adresse de facturation en correspondance avec le mode de paiement :
- `NL` pour iDEAL
- `BE` pour Bancontact
- `AT` pour EPS
- `PT` pour Multibanco
...
<Accordion title="Fournir des alternatives de carte">
Tous les clients européens n'ont pas accès à ces méthodes régionales (touristes, expatriés, etc.). Incluez toujours `credit` et `debit` comme alternatives.
</Accordion>
...
**Solution :** Les méthodes européennes sont strictement régionales. Un client avec le pays de facturation `DE` (Allemagne) ne verra pas iDEAL, qui est réservé aux Pays-Bas.
</Accordion>
