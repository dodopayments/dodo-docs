...
Pour que UPI apparaisse au checkout :
1. **Pays de facturation** doit être l’Inde (`IN`)
2. **Devise** doit être INR
...
`payment.succeeded` webhook est envoyé après le débit réel, pas à l'initiation.
</Step>
</Steps>

<Warning>
**Ne pas accorder de bénéfices à l'initiation du débit.** Attendez le webhook `payment.succeeded`, qui arrive ~48-51 heures après la date de prélèvement prévue.
</Warning>
...
| `payment.succeeded` | ~48h après la date de prélèvement | Accordez/continuez l'accès |
| `payment.failed` | Débit échoué | Informez le client, mettez l'accès en pause |
| `subscription.on_hold` | Paiement échoué | Incitez à mettre à jour le mode de paiement |
...
Les clients peuvent annuler des mandats via leurs applications bancaires à tout moment. Surveillez les webhooks `subscription.on_hold` et incitez les clients à se réabonner ou à mettre à jour les modes de paiement.
</Accordion>
...
<AccordionGroup>
<Accordion title="UPI n'apparaît pas au checkout">
**Vérifiez :**
1. Pays de facturation réglé sur `IN` ?
2. Devise réglée sur `INR` ?
3. Si marchand non indien : Monnaie adaptative activée ?
...
**Solution :** Le prochain prélèvement échouera et l'abonnement sera déplacé vers `on_hold`. Surveillez les webhooks pour `payment.failed`.
</Accordion>
</AccordionGroup>
