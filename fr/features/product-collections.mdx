...
| **Autoriser l’annulation d’abonnement** | Les clients peuvent annuler eux-mêmes leurs abonnements depuis le Customer Portal | Activé |
...
<Info>
Pour plus d’informations sur la manière dont les utilisations d’essai sont associées et enregistrées, consultez [Empêcher l’utilisation abusive des essais](/features/subscription#preventing-trial-misuse).
</Info>
...
Les collections de produits peuvent être gérées depuis le tableau de bord Dodo Payments ou par programmation via l’API. L’API offre un contrôle complet sur la création, la mise à jour, l’importation d’images et l’archivage des collections, ainsi que sur la gestion des groupes imbriqués et des produits.
...
- **Réorganiser** : Faites glisser-déposer les éléments pour modifier l’ordre d’affichage des produits
- **Activer/Désactiver des produits** : Contrôlez les produits qui apparaissent lors du checkout
- **Archiver** : Masquez une collection sans la supprimer définitivement (elle pourra être désarchivée ultérieurement)

<Frame>
  <img 
    src="/images/product-collection/collection-dashboard.png" 
    alt="Capture d’écran du tableau de bord des collections de produits présentant les opérations de gestion des collections" 
  style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>

### Gestion via l’API

Les endpoints suivants vous permettent de créer, mettre à jour, récupérer, archiver et organiser des collections de produits par programmation, notamment de gérer les groupes imbriqués et les produits qu’ils contiennent.

<AccordionGroup>
<Accordion title="Listing Product Collections">
Récupérez toutes les collections de produits associées à votre compte à l’aide d’une requête `GET` vers l’endpoint `/product-collections`. La pagination est prise en charge, ainsi que le filtrage par marque et l’inclusion des collections archivées.
...
Créez une nouvelle collection de produits en envoyant une requête `POST` vers l’endpoint `/product-collections`, avec des informations telles que le nom, la description et la marque.
...
Obtenez des informations détaillées sur une collection de produits spécifique, notamment ses groupes et ses produits, à l’aide d’une requête `GET` vers l’endpoint `/product-collections/{id}`.
...
Modifiez les détails d’une collection de produits (nom, description, marque, etc.) en envoyant une requête `PATCH` vers l’endpoint `/product-collections/{id}`.
...
Associez une image à une collection en la téléversant via une URL pré-signée. Demandez une URL de téléversement à l’endpoint `/product-collections/{id}/images`, puis `PUT` l’image vers l’URL retournée dans un délai de 60 secondes.
...
Archivez une collection en envoyant une requête `DELETE` vers l’endpoint `/product-collections/{id}`. La collection n’est alors plus disponible pour une nouvelle utilisation, mais n’est pas supprimée définitivement.
...
Les groupes vous permettent d’organiser les produits au sein d’une collection (par exemple, « Formules mensuelles » et « Formules annuelles »). Utilisez les endpoints des groupes pour ajouter, mettre à jour ou supprimer des groupes au sein d’une collection.
...
<Card title="Create Group" icon="code" href="/api-reference/product-collections/create-group">
  Ajoutez un nouveau groupe à une collection de produits.
</Card>
...
- **Regrouper logiquement** : Organisez les produits par intervalle de facturation (mensuel/annuel) ou par niveau de fonctionnalités (débutant/professionnel/entreprise)
- **Définir un ordre stratégique** : Placez votre plan le plus populaire ou recommandé en premier, car il sera présélectionné lors du checkout
- **Utiliser des noms clairs** : Les noms des produits doivent communiquer clairement les différences de valeur
- **Autoriser les deux directions** : Permettez à la fois les mises à niveau et les rétrogradations afin d’offrir plus de flexibilité aux clients
- **Tenir compte de la proratisation** : Choisissez un mode de proratisation adapté à votre modèle économique
- **Tester minutieusement** : Vérifiez les parcours de checkout et de changement de plan en mode test avant la mise en production

<Check>
Vous êtes maintenant prêt à créer des collections de produits et à offrir à vos clients une expérience unifiée de sélection de plans.
</Check>

<CardGroup cols={2}>
<Card title="Products" icon="box" href="/features/products">
  Créez des produits ponctuels, par abonnement ou facturés à l’usage à ajouter aux collections.
</Card>
