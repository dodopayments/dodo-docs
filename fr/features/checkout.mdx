...
<Info>
Cette section couvre la configuration du thème **côté serveur** avec `customization.theme_config`. Si vous utilisez le **SDK de Paiement** (paiement superposé ou intégré), voir les sections de personnalisation de thème dans [Paiement Superposé](/developer-resources/overlay-checkout#theme-customization) ou [Paiement Intégré](/developer-resources/inline-checkout#theme-customization) qui utilisent des propriétés camelCase (par exemple, `bgPrimary` au lieu de `bg_primary`).
</Info>
