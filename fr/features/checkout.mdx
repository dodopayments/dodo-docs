...
<Frame>
  <img src="/images/checkout/theme-example.png" alt="Page de paiement personnalisée" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Info>
Cette section présente la configuration du thème via l’**API côté serveur** en utilisant `customization.theme_config`. Si vous utilisez le **Checkout SDK** (paiement superposé ou intégré), consultez les sections de personnalisation des thèmes dans [Overlay Checkout](/developer-resources/overlay-checkout#theme-customization) ou [Inline Checkout](/developer-resources/inline-checkout#theme-customization) qui utilisent des propriétés en camelCase (par exemple, `bgPrimary` au lieu de `bg_primary`).
</Info>
