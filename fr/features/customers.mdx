...
- Solde actuel du portefeuille dans chaque devise (USD, EUR, GBP)
- Motifs et horodatages des transactions
- Historique détaillé des transactions pour le rapprochement
Consultez le registre des crédits du portefeuille du client, affichant :
...
Les portefeuilles clients sont des comptes de solde monétaire prépayés pour vos clients. Chaque client reçoit automatiquement un portefeuille pouvant contenir des fonds réels dans plusieurs devises (USD, EUR et GBP), qui peuvent être appliqués aux factures d'abonnement, utilisés pour les soldes de remboursement ou approvisionnés avec des bonus promotionnels.

### Principales fonctionnalités

- **Prise en charge multidevise** : suivez séparément les soldes monétaires en USD, EUR et GBP.
- **Soldes prépayés** : les clients peuvent détenir des fonds qui seront appliqués aux futures factures et aux paiements d'abonnement.
- **Historique des transactions** : consultez le registre complet de tous les ajouts et déductions de fonds.
- **Conception API-first** : ajoutez, déduisez et vérifiez les soldes des portefeuilles par programmation.
- **Facturation basée sur des crédits** : permettez aux clients d'acheter des crédits à l'avance et de les dépenser pour vos services.
- **Historique des transactions** : consultez le registre complet de tous les crédits et débits.
- **Conception API-first** : ajoutez, supprimez et vérifiez les soldes des portefeuilles par programmation.
...
<Note>
Tous les points de terminaison de l'API nécessitent une authentification avec votre clé API. Utilisez toujours votre clé secrète côté serveur et ne l'exposez jamais dans le code côté client.
</Note>