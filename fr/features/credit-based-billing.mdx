...
Attribuez des crédits à un produit d'abonnement lors du passage à la caisse :
...
Envoyez des événements d'utilisation qui déduisent automatiquement les crédits :
...
**Configuration :**
- Type de crédit : Unité personnalisée (« AI Tokens »)
- Précision : 0 (jetons entiers)
- Report : 25 % max, période de 1 mois
- Dépassement : Activé, facturer le dépassement lors de la facturation
...
**Configuration :**
- Type de crédit : Unité personnalisée (« API Calls »)
- Précision : 0 (appels entiers)
- Report : Désactivé
- Dépassement : Les plans Developer+ autorisent le dépassement (remis à zéro à la réinitialisation), le plan Free désactive le dépassement
...
| Plan | Prix | Crédits/Mois | Dépassement |
|------|-------|---------------|---------|
| Personal | 9 $/mois | 100 Go-heures | 0,05 $/Go-heure |
| Team | 49 $/mois | 1 000 Go-heures | 0,03 $/Go-heure |

**Configuration :**
- Type de crédit : Unité personnalisée (« GB-hours »)
- Précision : 2 (deux décimales)
- Report : 50 % max, report possible une fois
...
- **Commencez simplement** : Débutez avec un seul type de crédit et sans report. Ajoutez de la complexité en fonction des retours clients et des modèles d'utilisation.
- **Fixez des attentes claires** : Affichez les allocations de crédits, les soldes restants et les tarifs de dépassement de manière visible sur vos pages produits et le portail client.
- **Utilisez des unités significatives** : Nommez les crédits en fonction de ce qu'ils représentent (par exemple, « API Calls », « AI Tokens ») plutôt que des termes génériques. Cela aide les clients à saisir la valeur.
- **Configurez la date d'expiration avec soin** : Des fenêtres d'expiration courtes (7 jours) créent un sentiment d'urgence mais peuvent frustrer les clients. Des fenêtres plus longues (30 à 90 jours) sont plus conviviales pour la plupart des produits SaaS.
- **Surveillez les soldes faibles** : Définissez des seuils de solde faible pour alerter les clients avant qu'ils ne soient à court de crédits, réduisant ainsi les frais de dépassement surprises.
- **Testez en mode test** : Créez des crédits, associez-les à des produits de test et simulez le cycle complet achat → utilisation → déduction → expiration avant de passer en production.

<Info>
Le facturation basée sur les crédits fonctionne parfaitement avec toutes les autres fonctionnalités de Dodo Payments - abonnements avec essais, changements de plan avec prorata, et le portail client. Commencez par une configuration basique et développez-la à mesure que votre modèle tarifaire évolue.
</Info>
