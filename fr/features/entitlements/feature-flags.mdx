...
| Déclencheur | Effet |
|---|---|
| Le paiement ponctuel est réussi / l’abonnement devient actif | Grant créé avec `status: Delivered` et `delivered_at` définis. |
| L’abonnement est suspendu, annulé ou a expiré | Grant révoqué avec le `revocation_reason` correspondant. |
| Remboursement d’un paiement ponctuel | Grant révoqué avec `revocation_reason: refund`. |
| L’abonnement est rétabli (par exemple, dunning réussit) | Le grant révoqué est restauré avec `Delivered` — même grant `id`, champs de révocation effacés. |
