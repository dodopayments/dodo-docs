...
Voir [événements webhook `entitlement_grant.*`](/developer-resources/webhooks/intents/entitlement-grant). Les droits Telegram arrivent `pending` et passent à `delivered` une fois que le bot a émis le lien d'invitation.
