...
1. Le client finalise son paiement.
2. Dodo Payments crée un grant avec le statut `Pending` et un `oauth_url` pointant vers Discord.
3. Le client reçoit un e-mail contenant un bouton « Rejoindre Discord » (le lien OAuth). Le lien est également visible dans son Customer Portal.
4. Après l’autorisation du client, le bot l’ajoute au serveur et lui attribue le rôle configuré. Le grant passe à `Delivered`.
5. Si l’abonnement est annulé, mis en pause ou arrive à expiration, ou si le marchand révoque manuellement l’accès, le bot retire le rôle et le grant passe à `Revoked`.
...
- `entitlement_grant.created` se déclenche avec `status: "Pending"` et un `oauth_url` permettant au client de s’authentifier.
