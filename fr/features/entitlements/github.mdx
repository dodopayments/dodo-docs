...
1. Le client finalise le processus de paiement.
2. Dodo Payments crée une autorisation avec le statut `Pending`. Sa valeur `oauth_url` est `null` jusqu'à ce que le client commence le processus d'acceptation, auquel moment elle est renseignée avec une URL d'autorisation GitHub.
3. Le client s'autorise via le lien présent dans son e-mail ou dans le Customer Portal.
4. Dodo Payments invite le compte GitHub du client à rejoindre le dépôt avec le niveau d'autorisation configuré. L'autorisation passe à `Delivered` une fois l'invitation créée.
5. Si l'abonnement est annulé, remboursé ou révoqué, le client est supprimé en tant que collaborateur.
...
- `entitlement_grant.created` se déclenche avec `status: "Pending"` et `oauth_url` toujours défini sur `null` ; l'URL d'autorisation GitHub est renseignée lorsque le client commence le processus d'acceptation depuis son e-mail de livraison ou le Customer Portal.
- `entitlement_grant.delivered` se déclenche une fois l'invitation du collaborateur créée.
- `entitlement_grant.failed` se déclenche avec `error_code: "github_permission_denied"` (ou une valeur similaire) si la GitHub App a perdu l'accès au dépôt.
