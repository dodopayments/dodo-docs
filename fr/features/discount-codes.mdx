...
- **Remises flexibles** : Réductions basées sur des pourcentages
- **Codes cumulables** : Appliquez jusqu'à 20 codes par commande, paiement ou abonnement — combinez les campagnes (par exemple, `WELCOME10` + `BLACKFRIDAY20`) sans créer de codes personnalisés
- **Contrôle ciblé** : Restriction par produit et cycles d'abonnement
- **Gouvernance de campagne** : Dates d'expiration et limites d'utilisation
- **Paiement transparent** : Champ UI et prise en charge de l'API via les sessions de paiement
...
- **Nom de la remise** (obligatoire) : Nom affiché en interne et sur le tableau de bord
- **Code** (obligatoire) : La chaîne que les clients saisissent lors du paiement
- **Type & Montant** (obligatoire) : Définissez une valeur en pourcentage (seules les réductions basées sur des pourcentages sont actuellement prises en charge), ou générez un code aléatoire à l'aide du bouton fourni
- **Date d'expiration** (optionnelle) : Date après laquelle le code devient invalide
- **Limite d'utilisation** (optionnelle) : Nombre total maximal de réclamations pour tous les clients
- **Restriction de produit** (optionnelle) : Limitez l'applicabilité à certains produits
- **Limite de cycle d’abonnement** (optionnelle) : Nombre de cycles de facturation auxquels la remise s'applique
- **Métadonnées** (optionnelles) : Attachez des paires clé-valeur personnalisées pour le suivi interne ou les intégrations
