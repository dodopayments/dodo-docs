...
|---|---|
| `payment.succeeded` (ponctuel) | Génère une clé par `quantity` acheté. L’expiration de la clé respecte la durée de l’ayant droit. |
| `subscription.active` | Génère une clé par `quantity` d’abonnement (siège). La clé n’expire pas ; sa validité dépend de l’état de l’abonnement. |
| `subscription.renewed` | Aucune opération. Les clés existantes sont conservées. |
| `subscription.on_hold` | Désactive les clés. Elles sont réactivées lorsque l’abonnement n’est plus suspendu. |
| `subscription.paused` | Désactive les clés. Elles sont réactivées lorsque l’abonnement reprend. |
| `subscription.cancelled` / `expired` | Désactive définitivement les clés. |
| `subscription.plan_changed` | Désactive les anciennes clés et en émet de nouvelles pour le nouveau plan. |
| `refund.succeeded` (ponctuel) | Désactive les clés. |
| Révocation manuelle via l’API/le tableau de bord | Désactive les clés avec `revocation_reason: manual`. Elles ne sont pas automatiquement réattribuées lors du renouvellement de l’abonnement. |
| Clé de licence désactivée directement | Révoque l’octroi avec `revocation_reason: license_key_disabled`. La réactivation de la clé réactive automatiquement l’octroi. |
...
Utilisez le champ `source` des enregistrements de clés de licence pour distinguer, lors du rapprochement ou de l’audit, les stocks migrés et les clés attribuées manuellement des clés émises de manière organique. Lisez-le depuis l’objet `license_key` des octrois renvoyés par [List Grants](/api-reference/entitlements/list-grants) ; l’endpoint hérité `GET /license_keys` est obsolète.
...
<Warning>
`GET /license_keys`, `GET /license_keys/{id}` et `PATCH /license_keys/{id}` sont obsolètes. Utilisez les endpoints d’octroi d’entitlement ([List Grants](/api-reference/entitlements/list-grants), [List Customer Grants](/api-reference/entitlements/list-customer-grants)) pour les lectures. `POST /license_keys` reste pris en charge pour importer des clés existantes.
</Warning>
...
Gérez l’entitlement License Key lui-même : sa limite d’activation, sa durée et ses instructions.
...
La distribution et la révocation des clés de licence déclenchent les quatre [événements webhook `entitlement_grant.*`](/developer-resources/webhooks/intents/entitlement-grant). La charge utile de l’octroi contient un objet `license_key` renseigné avec la clé, l’expiration, le nombre d’activations utilisées et la limite.

Les événements hérités `license_key.*` (`license_key.created`) continuent de se déclencher pour le cycle de vie de l’enregistrement de clé de licence sous-jacent ; consultez la [page de la charge utile webhook des clés de licence](/developer-resources/webhooks/intents/license-key).

<Tip>
Pour les nouvelles intégrations, écoutez `entitlement_grant.delivered` plutôt que `license_key.created`. L’événement d’entitlement vous indique que la distribution est terminée pour toutes les intégrations du produit, et pas uniquement pour la clé de licence.
</Tip>
...
<Note>
Les produits créés avec l’ancien indicateur `license_key_enabled` ont été **automatiquement migrés** vers un entitlement License Key. La migration est transparente : les clés des clients existants continuent de fonctionner sans modification, les endpoints publics `/licenses/activate`, `/licenses/validate` et `/licenses/deactivate` continuent de fonctionner, et les endpoints API `/license_keys/*` continuent de lire et d’écrire dans le même stockage de clés.

La section autonome **License Keys** du tableau de bord reste disponible sous forme de liste de toutes les clés émises, ce qui est utile pour l’audit et la recherche. Toute nouvelle configuration (modification des limites d’activation, des durées ou des instructions) doit être effectuée en modifiant l’entitlement License Key migré sous **Entitlements**.
</Note>
...
- **Définissez clairement les limites d’activation** : choisissez des valeurs par défaut adaptées (1 pour les applications mono-utilisateur, 3 à 5 pour les licences d’équipe) et documentez-les.
- **Fournissez des instructions d’activation précises** : les clients les copient depuis leur e-mail ; des chemins et des commandes exacts réduisent les demandes d’assistance.
- **Validez les clés côté serveur** : pour les produits connectés au réseau, validez-les via `/licenses/validate` plutôt que de mettre l’activation en cache localement.
- **Utilisez les webhooks pour la révocation** : écoutez `entitlement_grant.revoked` afin de désactiver immédiatement les fonctionnalités de l’application lorsqu’un client annule ou demande un remboursement.
- **Testez avec des abonnements et des achats ponctuels** : le comportement des clés de licence diffère subtilement entre ces deux cas ; testez-les donc tous les deux avant la mise en production.
