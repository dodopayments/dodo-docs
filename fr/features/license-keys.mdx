...
Lorsqu'un client achète un produit en mode manuel, la subvention est créée avec le statut `pending` sans clé et un webhook [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) s'exécute avec `integration_type: "license_key"` et `status: "pending"`. Vous pouvez réagir à ce webhook ou interroger le point de terminaison [List Grants](/api-reference/entitlements/list-grants) avec les filtres `integration_type` et `status` :
