...
1. **Capturez l'ID du visiteur de DataFast** depuis le cookie `datafast_visitor_id` lors de la création des sessions de paiement
2. **Stockez l'ID du visiteur** dans vos métadonnées de paiement
...
<Steps>
<Step title="Installez le script DataFast">
...
- Assurez-vous que l'`datafast_visitor_id` est capturé et stocké dans les métadonnées de paiement
...
- Consultez le tableau de bord DataFast pour toute erreur ou log API
</Accordion>
...
- Vérifiez que le cookie `datafast_visitor_id` est correctement créé
- Assurez-vous que les ID des visiteurs correspondent entre la création du checkout et la finalisation du paiement
- Assurez-vous de capturer l'ID du visiteur avant de créer la session de checkout
...
- Vérifiez que l'URL du point de terminaison API est correcte : `https://datafa.st/api/v1/payments`
- Testez la transformation avec des payloads de webhook d'exemple
</Accordion>
