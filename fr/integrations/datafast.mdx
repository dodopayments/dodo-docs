...
<Step title="Installer le script DataFast">
  Tout d'abord, installez le script de suivi DataFast sur votre site web. Cela crée le `datafast_visitor_id` cookie qui suit vos visiteurs.
...
<Step title="Obtenez votre clé API">
  ...
  <Warning>
  Gardez votre clé API en sécurité et ne l'exposez jamais dans le code côté client.
  </Warning>
</Step>

<Step title="Capturer l'identifiant de visiteur dans le paiement">
...
- Assurez-vous que les identifiants de visiteur correspondent entre la création de la session de paiement et l'achèvement du paiement
