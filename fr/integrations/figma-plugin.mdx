...
Créez votre propre projet de plugin Figma en utilisant le [guide de démarrage rapide des plugins Figma](https://www.figma.com/plugin-docs/plugin-quickstart-guide/). Cela générera un squelette avec un fichier `manifest.json`.
...
<Step title="Cloner l'Implémentation Exemple">
Clonez le [dodo-paiements-figma repository](https://github.com/dodopayments/dodopayments-figma) et mettez à jour son `manifest.json` avec le nom et l'ID de votre dossier de plugin généré.
...
<Warning>
Utilisez toujours `test_mode` lors du développement pour éviter de traiter des paiements réels.
</Warning>
</Step>
...
- Mettez à jour `Authenticated.tsx` pour exposer des fonctionnalités aux utilisateurs avec des clés de licence valides
...
Le plugin nécessite un accès réseau pour communiquer avec le proxy CORS. Ajoutez ce qui suit à votre `manifest.json` :
...
<Tip>
Basculez vers `live_mode` uniquement lorsque vous êtes prêt pour la production et avez testé en profondeur en mode test.
</Tip>
</Step>
</Steps>
...
| `npm run lint` | Vérifier le style du code et les erreurs |
...
2. Tester en profondeur aussi bien en mode test qu'en mode live
