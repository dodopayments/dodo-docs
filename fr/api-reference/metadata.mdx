...
- Les métadonnées ne sont pas recherchables via notre API mais sont retournées dans les réponses API et les webhooks
...
### À ne pas faire :
- Ne stockez pas de données sensibles dans les métadonnées
- N'utilisez pas les métadonnées pour des valeurs changeantes fréquemment
- Ne comptez pas sur les métadonnées pour une logique commerciale critique
- Ne stockez pas d'informations en double qui sont disponibles ailleurs dans l'objet
- N'utilisez pas de caractères spéciaux dans les clés de métadonnées
