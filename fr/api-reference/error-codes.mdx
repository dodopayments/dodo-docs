...
- `ADDONS_IN_USAGE_BASED_BILLING_NOT_SUPPORTED`
  - **Déclencheur :** Tentative d'ajouter des addons aux abonnements de facturation à l'utilisation
...
- `BRAND_MISMATCH`
  - **Déclencheur :** Les articles du panier appartiennent à des marques différentes
  - **Message :** Tous les articles dans le panier de produits doivent appartenir à la même marque
...
- `BRAND_SUBMISSION_NOT_ENABLED`
  - **Déclencheur :** La fonctionnalité de resoumission de vérification de marque n'est pas activée
...
- `DISCOUNT_CODE_EXPIRED`
  - **Déclencheur :** Le code de réduction est passé sa date `expires_at`
  - **Message :** Le code de réduction a expiré

- `DISCOUNT_CODE_USAGE_LIMIT_EXCEEDED`
  - **Déclencheur :** Code de réduction réutilisé après avoir atteint `usage_limit`
  - **Message :** La limite d'utilisation ne peut pas être inférieure à times_used / Le code de réduction a atteint la limite d'utilisation
...
- `DISCOUNT_NOT_AVAILABLE_FOR_PRODUCT`
  - **Déclencheur :** Code appliqué à un produit non pertinent
...
- `DUPLICATE_LINE_ITEMS_IN_REQUEST`
  ...
  - **Message :** Identifiants d'article en double spécifiés dans le tableau d'articles
...
- `EXCHANGE_RATE_NOT_FOUND`
  - **Déclencheur :** Pas de taux de change pour `from → to` paire de devises
  - **Message :** Taux de change non trouvé pour convertir de la devise à la devise
...
- `INACTIVE_SUBSCRIPTION_PLAN_CHANGE_NOT_SUPPORTED`
  ...
  - **Message :** Le changement de plans n'est pas pris en charge pour les abonnements inactifs
...
- `INTEGER_CONVERSION_FAILURE`
  - **Déclencheur :** Toute conversion de entier ↔ chaîne/décimal qui échoue côté serveur
...
- `INVALID_QUANTITY`
  ...
  - **Message :** Une seule quantité est autorisée dans les produits à prix basés sur l'utilisation

- `INVALID_QUERY_PARAMS`
  - **Déclencheur :** Paramètres de requête mutuellement exclusifs / malformés
  - **Message :** Les paramètres de requête ne doivent contenir que soit un intervalle de temps, soit (début, fin)
...
- `INVALID_SUGGESTED_PRICE`
  - **Déclencheur :** PWYW prix < prix minimum autorisé
  - **Message :** Le prix suggéré ne peut pas être inférieur au prix minimum. En cas de paiement au montant souhaité, le prix est considéré comme le montant minimum accepté

- `INVALID_TAX_ID`
  - **Déclencheur :** Validation de la TVA/GST/TIN échouée
...
- `LINE_ITEM_FULLY_REFUNDED`
  ...
  - **Message :** L'article de ligne {id} a été entièrement remboursé et ne peut pas être remboursé davantage.

- `LINE_ITEM_NOT_FOUND`
  - **Déclencheur :** L'ID de l'article ne fait pas partie du paiement référencé
  - **Message :** L'article de ligne {id} non trouvé dans le paiement

- `LINE_ITEM_PRORATED`
  - **Déclencheur :** Tentative de remboursement ou de mise à jour d'une ligne proratisée
  - **Message :** L'article de ligne {id} ne peut pas être remboursé car il est proratisé

- `LINE_ITEM_REFUND_AMOUNT_TOO_HIGH`
  - **Déclencheur :** Montant du remboursement > montant payé (taxes incluses)
  - **Message :** L'article de ligne {id} demandé montant de remboursement incluant la taxe est {amount} qui est supérieur au montant payé {amount}

- `LINE_ITEM_REFUND_AMOUNT_TOO_LOW`
  - **Déclencheur :** Montant du remboursement en dessous du seuil minimum
  - **Message :** Montant de remboursement demandé pour l'article de ligne {id} est {amount} qui est trop bas

- `MAXIMUM_KEYS_REACHED`
  - **Déclencheur :** Métadonnées / champs personnalisés dépassent 50 paires
  - **Message :** Dépasse 50 paires clé-valeur

- `MERCHANT_NOT_LIVE`
  - **Déclencheur :** L'entreprise est toujours en mode test/sandbox
  - **Message :** Le marchand n'est pas en ligne
...
- `MISSING_ADDON_IDS`
  - **Déclencheur :** Liste `addon_id` vide ou IDs inconnus
  - **Message :** Un ou plusieurs IDs de produit n'existent pas : {id}

- `MISSING_METER_IDS`
  - **Déclencheur :** Liste d'ID de compteur vide ou contenant des ID invalides
  - **Message :** Un ou plusieurs IDs de compteur n'existent pas : {id}
...
- `NEGATIVE_BALANCE_ADJUSTMENT`
  ...
  - **Message :** Le solde du portefeuille ne doit pas devenir négatif
...
- `NO_EXPIRY_ON_SUBSCRIPTION_LICENSE_KEYS`
  - **Déclencheur :** Tentative de définir une expiration sur une clé basée sur un abonnement
...
- `ON_DEMAND_PLAN_CHANGE_NOT_SUPPORTED`
  - **Déclencheur :** Changement de plan prohibé pour à la demande
  - **Message :** Changer de plans n'est pas pris en charge pour les abonnements à la demande

- `ON_DEMAND_SUBSCRIPTIONS_NOT_ENABLED`
  - **Déclencheur :** L'entreprise a désactivé la fonctionnalité
...
- `PAY_AS_YOU_WANT_AMOUNT_REQUIRED`
  ...
  - **Message :** Le montant est obligatoire pour le produit à prix libre
...
- `PAYMENT_NOT_SUCCEEDED`
  - **Déclencheur :** Tentative de remboursement/de traitement d'un paiement infructueux
  - **Message :** Le paiement fourni n'a pas abouti

- `PLAN_CHANGE_NOT_ALLOWED_FOR_SCHEDULED_CANCELLATION`
  - **Déclencheur :** Changement de plan tenté sur un abonnement programmé pour annulation
  - **Message :** Abonnement programmé pour annulation
...
- `PRODUCT_IS_DELETED`
  - **Déclencheur :** Produit en suppression douce
  - **Message :** *Aucun message*

- `REFUND_AMOUNT_EXCEEDS_PAID_AMOUNT`
  - **Déclencheur :** Montant total agrégé du remboursement > montant payé
...
- `REQUEST_AMOUNT_BELOW_MINIMUM`
  - **Déclencheur :** Montant < minimum de produit
...
- `SUBSCRIPTION_EXPIRED`
  ...
  - **Message :** Abonnement expiré création de nouvelles charges impossible
...
- `SUBSCRIPTION_PAYMENT_RETRY_LIMIT_EXCEEDED`
  - **Déclencheur :** Les tentatives de paiement par abonnement ont dépassé le nombre maximum de tentatives
...
- `TOTAL_PAYMENT_AMOUNT_BELOW_MINIMUM_AMOUNT`
  - **Déclencheur :** Le total combiné du panier < minimum de la passerelle
...
- `UNAUTHORIZED`
  - **Déclencheur :** Pas de clé API ou jeton/scopes invalides
  - **Message :** Vous n'êtes pas autorisé à effectuer cette action

- `UNSUPPORTED_ACTION`
  - **Déclencheur :** Action non prise en charge pour le type de ressource
  - **Message :** Changer de plans pour les abonnements basés sur l'utilisation n'est pas pris en charge

- `UNSUPPORTED_BILLING_CURRENCY`
  - **Déclencheur :** Abonnements restreints à l'USD
  - **Message :** La devise de facturation non USD n'est pas prise en charge pour les abonnements

- `UNSUPPORTED_COUNTRY`
  - **Déclencheur :** Zone non encore supportée
  - **Message :** Pays {country_name} actuellement non pris en charge

- `UNSUPPORTED_CURRENCY`
  - **Déclencheur :** Devise de produit ou d'addon invalide
  - **Message :** La devise n'est actuellement pas prise en charge / Actuellement, seuls les produits en USD et INR sont pris en charge / Seuls USD et INR pris en charge pour les prix des addons / Peut demander USD ou INR pour billing_currency / Devise non prise en charge / Devise inattendue pour les abonnements de carte indienne

- `UNSUPPORTED_DISCOUNT_TYPE`
  - **Déclencheur :** Remises en montant forfaitaire, etc., pas encore en ligne
...
- `UNSUPPORTED_PAYMENT_CURRENCY`
  ...
  - **Message :** Transaction INR n'est pas prise en charge pour cette transaction

- `UNSUPPORTED_TAX_CATEGORY`
  - **Déclencheur :** Chaîne de catégorie de taxe non dans l'énumération
  - **Message :** Catégorie {category} actuellement non prise en charge

- `UNSUCCESSFUL_PAYMENT_ID`
  - **Déclencheur :** L'ID de paiement fait référence à un paiement infructueux
  - **Message :** L'ID de paiement a un statut infructueux.

- `ZERO_AMOUNT_PAYMENT_REFUND_NOT_ALLOWED`
  - **Déclencheur :** Tentative de rembourser un paiement à zéro montant
  - **Message :** Impossible de rembourser un paiement d'un montant de devise zéro
