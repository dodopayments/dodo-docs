...
| `INVALID_ACCOUNT` | Oui | Les détails du compte fournis ne sont pas valides |
...
| `INVALID_CARD_OWNER` | Oui | Les informations sur le propriétaire de la carte ne sont pas valides |
...
| `LIMIT_EXCEEDED` | Oui | La transaction dépasse la limite de carte ou de compte |
| `LIVE_MODE_TEST_CARD` | Oui | Une carte de test a été utilisée en mode direct |
...
| `PICKUP_CARD` | Oui | La carte a été signalée comme volée et doit être saisie |
...
<Note>
  **Erreur utilisateur** indique si le refus de paiement peut être résolu par le client. Lorsque `true`, le client peut agir pour résoudre le problème (par exemple, entrer les détails de la carte corrects). Lorsque `false`, le refus est dû à des problèmes au niveau du système ou à des restrictions bancaires qui ne peuvent pas être résolues par le client.
</Note>
