...
1. **Paiement en ligne - Expériences de paiement entièrement intégrées (BETA)**  
   ...
   - **Événements de décomposition en temps réel** : Recevez des événements `checkout.breakdown` lorsque les clients saisissent leurs informations
...
2. **Créer des paiements, des abonnements et des paiements en utilisant des méthodes de paiement existantes**  
    Rationalisez les flux de paiement en utilisant des méthodes de paiement enregistrées de clients existants. Le nouveau paramètre `payment_method_id` vous permet de créer des paiements, des abonnements ou des sessions de paiement en utilisant la méthode de paiement précédemment enregistrée d'un client.
   ...
   <Warning>
   Lors de l'utilisation de `payment_method_id` dans les sessions de paiement, `confirm` doit être défini sur `true`, et une méthode de paiement existante `customer_id` doit être fournie. La méthode de paiement sera validée pour son éligibilité avec la devise du paiement.
   </Warning>
...
5. **`redirect_immediately` Drapeau - Passer la page de succès de paiement**  
   Contrôlez le flux de paiement avec le nouveau drapeau `redirect_immediately`. Lorsqu'il est activé, les clients sont redirigés immédiatement après la finalisation du paiement, contournant la page de succès par défaut pour une expérience plus rapide et rationalisée.
