...
1. **Paiement en ligne - Expériences de paiement entièrement intégrées (BETA)**  
   ...
   Initialisez le SDK avec `displayType: 'inline'` et intégrez le checkout dans un élément de conteneur :
   ...
   - **Événements de décomposition en temps réel** : Recevez `checkout.breakdown` événements lorsque les clients saisissent leurs détails
   - **Sommaires de commande personnalisés** : Créez votre propre affichage de prix qui se met à jour en temps réel
   - **Intégration transparente** : Le cadre de checkout gère la collecte des paiements pendant que vous contrôlez la mise en page
   - **Méthodes SDK** : Utilisez `open()`, `close()`, et `isOpen()` pour contrôler le checkout par programme
...
2. **Créez des paiements, des abonnements et des checkouts en utilisant les méthodes de paiement existantes**  
    Rationalisez les flux de checkout en utilisant les méthodes de paiement sauvegardées des clients existants. Le nouveau paramètre `payment_method_id` vous permet de créer des paiements, des abonnements ou des sessions de checkout en utilisant une méthode de paiement précédemment sauvegardée d'un client.
   ...
   <Warning>
   Lors de l'utilisation de `payment_method_id` dans les sessions de checkout, `confirm` doit être défini sur `true`, et une méthode de paiement existante `customer_id` doit être fournie. La méthode de paiement sera validée pour son éligibilité avec la devise du paiement.
   </Warning>
...
5. **`redirect_immediately` Flag - Ignorer la page de succès de paiement**  
   Contrôlez le flux de checkout avec le nouveau drapeau `redirect_immediately`. Lorsqu'il est activé, les clients sont redirigés immédiatement après l'achèvement du paiement, contournant la page de succès par défaut pour une expérience plus rapide et rationalisée.
  ...
  ```
   ...
   <Note>
   Lorsque `redirect_immediately` est activé, les clients sont redirigés vers votre `return_url` immédiatement après l'achèvement du paiement, sautant entièrement la page de succès par défaut.
   </Note>
