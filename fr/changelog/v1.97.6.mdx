...
- **Objet d’abonnement** : Nouveau champ `cancellation_feedback` (l’une des valeurs ci-dessus) et `cancellation_comment` (texte libre facultatif), renseignés lorsque le client annule
- **Webhook `subscription.cancelled`** : Les deux champs sont inclus dans la charge utile
- **API** : Transmettez `cancellation_feedback` et `cancellation_comment` à `PATCH /subscriptions/{subscription_id}` lors de la planification ou de l’exécution d’une annulation par programmation
...
Vous pouvez également remplacer la valeur par défaut du marchand pour les paiements individuels, les frais d’abonnement à la demande et les changements de plan à l’aide de la valeur booléenne `adaptive_currency_fees_inclusive` :
...
Passez `crypto_currency` dans `allowed_payment_method_types` lors de la création d'une session de paiement :
