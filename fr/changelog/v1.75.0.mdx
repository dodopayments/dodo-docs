...
### 5. **`invoice_url` Champ dans la réponse de paiement et la réponse de lien de paiement**  
   ...
   La réponse de paiement et la réponse de lien de paiement incluent maintenant un champ `invoice_url`, fournissant un accès direct à la facture associée à chaque paiement.
...
### 7. **Capacités de Filtrage Améliorées pour les Clients, Remises et Clés de Licence**  
   ...
   **Filtres API Clients**
   ...
   | Filtrer | Description |
   |--------|-------------|
   | `email` | Filtrer par email du client |
   | `name` | Filtrer par nom du client (correspondance partielle, insensible à la casse) |
   | `created_at_gte` | Filtrer les clients créés à cette date ou après |
   | `created_at_lte` | Filtrer les clients créés à cette date ou avant |
   ...
   **Filtres API Clés de Licence**
   ...
   | Filtrer | Description |
   |--------|-------------|
   | `customer_id` | Filtrer par ID de client |
   | `status` | Filtrer par statut de clé de licence (`active`, `expired`, `disabled`) |
   | `product_id` | Filtrer par ID de produit |
   | `created_at_gte` | Filtrer les clés de licence créées à cette date ou après |
   | `created_at_lte` | Filtrer les clés de licence créées à cette date ou avant |
