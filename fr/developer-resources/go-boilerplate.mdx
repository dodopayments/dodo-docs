...
<Info>
Ce modèle utilise Go 1.21+ avec des motifs d'architecture propre (`cmd`, `internal`, `templates`), des modèles HTML et le SDK `dodopayments-go` pour une intégration API transparente.
</Info>

### Fonctionnalités

- **Configuration rapide** - Commencez en moins de 5 minutes
- **Intégration de paiement** - Flux de validation pré-configuré utilisant le SDK `dodopayments-go`
- **UI moderne** - Page tarifaire épurée avec thème sombre et modèles HTML
- **Gestion des webhooks** - Vérifiez et traitez en toute sécurité les événements de paiement
- **Portail client** - Gestion d'abonnement en libre-service
- **Meilleures pratiques Go** - Architecture propre avec `cmd`, `internal`, et `templates`
- **Validation de paiement pré-remplie** - Démontre le passage des données client pour améliorer l'UX
...
Mettez à jour `internal/lib/products.go` avec vos véritables ID de produit provenant de Dodo Payments :
...
|---------------------|---------|-------------|
| `/` | GET | Page tarifaire avec liste de produits |
...
- ID des produits (de votre tableau de bord Dodo)
- Tarification
- Fonctionnalités
- Descriptions
...
Dans `templates/index.html`, remplacez les données client en dur par vos véritables données utilisateur :
...
- Utilisez des clés API de production provenant du tableau de bord
...
<Accordion title="La création de session de paiement échoue">
**Causes courantes :**
- ID de produit invalide - vérifiez qu'il existe dans votre tableau de bord Dodo
- Clé API ou paramètre d'environnement incorrect dans `.env`
