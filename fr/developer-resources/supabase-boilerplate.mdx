...
Définissez l'URI de redirection sur : `https://[your-project-ref].supabase.co/auth/v1/callback` dans Google Cloud, puis activez le fournisseur Google dans Supabase Auth en utilisant votre Client ID et Secret.
</Step>

<Step title="Configurer Dodo Payments">
Générez une clé API et un secret Webhook depuis le tableau de bord Dodo. Définissez l'environnement sur `test_mode` pendant le développement.
</Step>
...
L'interface tarifaire lit ce tableau `features` et le rend dynamiquement.
</Step>
...
- Schéma Drizzle ORM et migrations
...
<AccordionGroup>
<Accordion title="Signature Webhook invalide (401)">
...
<Accordion title="Échec de la poussée vers la base de données">
- Vérifiez la syntaxe d'`DATABASE_URL` et la sortie réseau Supabase
- Attendez ~2–3 minutes après la création du projet avant la première poussée
</Accordion>
<Accordion title="Mismatch de redirection OAuth">
- L'URI de redirection doit être `https://[ref].supabase.co/auth/v1/callback`
- Assurez-vous de la même chose dans Google Cloud et le fournisseur d'authentification Supabase
</Accordion>
</AccordionGroup>
