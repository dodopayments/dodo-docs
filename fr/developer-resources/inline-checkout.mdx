...
| Champ | Description |
|-------|-------------|
| `subTotal` | La somme de tous les éléments de ligne de la session avant l’application des remises ou des taxes. |
| `discount` | La valeur totale de toutes les remises appliquées. |
| `tax` | Le montant de taxe calculé. En mode `inline`, ce montant est mis à jour dynamiquement lorsque l’utilisateur interagit avec les champs d’adresse. |
| `total` | Le résultat mathématique de `subTotal - discount + tax` dans la devise de base de la session. |
| `currency` | Le code de devise ISO (par exemple, `"USD"`) utilisé pour les valeurs standard du sous-total, de la remise et de la taxe. |
| `finalTotal` | Le montant réel facturé au client. Il peut inclure des ajustements supplémentaires liés aux taux de change ou des frais associés aux moyens de paiement locaux qui ne font pas partie de la ventilation de base du prix. |
| `finalTotalCurrency` | La devise dans laquelle le client paie réellement. Elle peut différer de `currency` si la [parité de pouvoir d’achat](/features/purchasing-power-parity) ou la conversion en devise locale est active. |
...
<Info>
La vérification du domaine est uniquement requise pour le **checkout inline (intégré)**. Elle n’est pas requise pour le checkout hébergé.
</Info>
...
Apple Pay est vérifié par domaine depuis le tableau de bord.

<Steps>
<Step title="Open Wallet domains">
Accédez à **Settings → Payment Methods** et, sur la ligne **Apple Pay**, cliquez sur **Manage domains**.
...
Depuis le panneau Wallet domains, téléchargez le fichier d’association.

<Frame caption="Download the Apple Pay domain association file">
  <img src="/images/apple-pay-domains/wallet-domains-panel.png" alt="Panneau Wallet domains avec le bouton Download file" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Register your domain">
Cliquez sur **Register domain** et saisissez le domaine sur lequel vous intégrez le checkout inline (par exemple `shop.example.com`), puis cliquez sur **Continue**.
...
Il doit être accessible via HTTPS, accessible sans redirections et diffusé avec `Content-Type: application/octet-stream` ou `text/plain`.
</Step>
...
Lorsque le statut indique **Active**, Apple Pay est activé pour ce domaine. Utilisez le bouton bascule **Enabled** pour l’activer ou le désactiver pour chaque domaine.

<Frame caption="Verified domains show an Active status">
  <img src="/images/apple-pay-domains/domains-active.png" alt="Liste des domaines Wallet affichant des domaines avec le statut Apple Pay Active et des boutons bascule Enabled" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
## Checkout inline et overlay

Choisissez le type de checkout adapté à votre cas d’utilisation :

| Fonctionnalité | Checkout inline | Checkout overlay |
|---------|-----------------|------------------|
| Niveau d’intégration | Entièrement intégré à la page | Fenêtre modale superposée à la page |
| Contrôle de la mise en page | Contrôle total | Limité |
| Branding | Transparent | Séparé de la page |
| Effort d’implémentation | Plus important | Moins important |
| Idéal pour | Pages de checkout personnalisées, parcours à fort taux de conversion | Intégration rapide, pages existantes |

<Tip>
Utilisez le **checkout inline** lorsque vous souhaitez un contrôle maximal de l’expérience de checkout et un branding homogène. Utilisez le **checkout overlay** pour une intégration plus rapide avec un minimum de modifications sur vos pages existantes.
</Tip>
...
Pour obtenir davantage d’aide, consultez notre [communauté Discord](https://discord.gg/bYqAp4ayYh) ou contactez notre équipe d’assistance aux développeurs.
