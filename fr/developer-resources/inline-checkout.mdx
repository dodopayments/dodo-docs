...
Allez dans **Paramètres → Méthodes de paiement** et, sur la ligne **Apple Pay**, cliquez sur **Gérer les domaines**.

<img src="/images/apple-pay-domains/manage-domains.png" alt="Bouton Gérer les domaines sur la ligne Apple Pay dans les paramètres des méthodes de paiement" style={{ maxHeight: '500px', width: 'auto' }} />

Depuis le panneau des domaines du Wallet, téléchargez le fichier d'association.

<img src="/images/apple-pay-domains/wallet-domains-panel.png" alt="Panneau des domaines du Wallet avec le bouton Télécharger le fichier" style={{ maxHeight: '500px', width: 'auto' }} />

Cliquez sur **Enregistrer le domaine** et entrez le domaine où vous intégrez le paiement en ligne (ex. `shop.example.com`), puis **Continuer**.

<img src="/images/apple-pay-domains/register-domain.png" alt="Formulaire d'enregistrement d'un domaine avec un domaine entré" style={{ maxHeight: '500px', width: 'auto' }} />

Hébergez-le à l'adresse :

```
https://shop.example.com/.well-known/apple-developer-merchantid-domain-association
```

Il doit être servi via HTTPS, accessible sans redirections, et servi avec `Content-Type: application/octet-stream` ou `text/plain`.

Cliquez sur **Vérifier le domaine**. Dodo Payments confirme que le fichier est en ligne et soumet votre domaine à Apple.

<img src="/images/apple-pay-domains/verify-domain.png" alt="Écran de vérification de votre domaine avec le chemin d'hébergement du fichier d'association et le bouton Vérifier le domaine" style={{ maxHeight: '500px', width: 'auto' }} />

Quand le statut affiche **Actif**, Apple Pay est activé pour ce domaine. Utilisez le bouton **Activé** pour l'activer ou le désactiver par domaine.

<img src="/images/apple-pay-domains/domains-active.png" alt="Liste des domaines du Wallet montrant les domaines avec un statut Apple Pay Actif et des boutons Activés" style={{ maxHeight: '500px', width: 'auto' }} />

1. Ouvrez le paiement sur un appareil Apple
2. Vérifiez que le bouton Apple Pay apparaît
3. Effectuez une transaction test

## Prise en charge des navigateurs

Le Dodo Payments Checkout SDK prend en charge les navigateurs suivants :
...
Choisissez le type de paiement adapté à votre cas d'utilisation :

| Fonctionnalité | Paiement intégré | Paiement superposé |
|----------------|----------------|-------------------|
| Profondeur d'intégration | Complètement intégré dans la page | Modal au-dessus de la page |
| Contrôle de la disposition | Contrôle complet | Limité |
| Image de marque | Intégrée | Séparée de la page |
...
Utilisez **le paiement intégré** lorsque vous souhaitez un contrôle maximal sur l'expérience de paiement et une image de marque intégrée. Utilisez **le paiement superposé** pour une intégration plus rapide avec des changements minimes à vos pages existantes.

## Ressources connexes

Utilisez le paiement superposé pour une intégration rapide basée sur un modal.
...
Pour plus d'aide, visitez notre [communauté Discord](https://discord.gg/bYqAp4ayYh) ou contactez notre équipe de support pour développeurs.
