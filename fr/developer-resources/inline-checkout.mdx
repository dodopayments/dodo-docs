...
Initialisez le SDK et spécifiez `displayType: 'inline'`. Vous devez également écouter l'événement `checkout.breakdown` pour mettre à jour votre interface utilisateur avec des calculs d'impôts et de totaux en temps réel.
...
<Check>
Vous devriez voir des événements `checkout.breakdown` enregistrés dans la console de votre navigateur si vous avez ajouté un journal dans le callback `onEvent`.
</Check>
...
Cet exemple montre comment mettre en œuvre un résumé de commande personnalisé aux côtés du paiement en ligne, en les synchronisant à l'aide de l'événement `checkout.breakdown`.
...
| `options.showTimer` | `boolean` | Non | Afficher ou masquer le minuteur de paiement. Par défaut, il est `true`. Lorsqu'il est désactivé, vous recevrez l'événement `checkout.link_expired` lorsque la session expire. |
| `options.showSecurityBadge` | `boolean` | Non | Afficher ou masquer le badge de sécurité. Par défaut, il est `true`. |
| `options.manualRedirect` | `boolean` | Non | Lorsqu'il est activé, le paiement ne redirigera pas automatiquement après l'achèvement. Au lieu de cela, vous recevrez les événements `checkout.status` et `checkout.redirect_requested` pour gérer la redirection vous-même. |
| `options.themeConfig` | `ThemeConfig` | Non | Configuration de thème personnalisé. |
...
| `checkout.form_ready` | Le formulaire de paiement est prêt à recevoir des entrées utilisateur. Utile pour masquer les états de chargement et afficher l'interface de paiement. |
| `checkout.breakdown` | Déclenché lorsque les prix, les taxes ou les remises sont mis à jour. |
| `checkout.customer_details_submitted` | Les informations du client ont été soumises. |
| `checkout.pay_button_clicked` | Déclenché lorsque le client clique sur le bouton de paiement. Utile pour l'analyse et le suivi des parcours de conversion. |
...
| `checkout.link_expired` | Déclenché lorsque la session de paiement expire. Ne reçu que lorsque `showTimer` est défini sur `false`. |
...
L'événement `checkout.breakdown` est le moyen principal de garder l'interface utilisateur de votre application synchronisée avec l'état du paiement Dodo.
...
| Champ | Description |
|------|-------------|
| `subTotal` | Le total de tous les articles de la session avant l'application de remises ou de taxes. |
| `discount` | La valeur totale de toutes les remises appliquées. |
| `tax` | Le montant de la taxe calculée. En mode `inline`, cela se met à jour dynamiquement lorsque l'utilisateur interagit avec les champs d'adresse. |
| `total` | Le résultat mathématique de `subTotal - discount + tax` dans la devise de base de la session. |
| `currency` | Le code de devise ISO (par exemple, `"USD"`) pour le total standard, les remises et les valeurs fiscales. |
| `finalTotal` | Le montant réel facturé au client. Cela peut inclure des ajustements de change supplémentaires ou des frais de méthode de paiement locaux qui ne font pas partie de la ventilation des prix de base. |
...
1.  **Formatage des devises** : Les prix sont toujours retournés sous forme d'entiers dans la plus petite unité monétaire (par exemple, cents pour USD, yen pour JPY). Pour les afficher, divisez par 100 (ou la puissance de 10 appropriée) ou utilisez une bibliothèque de formatage comme `Intl.NumberFormat`.
...
3.  **Le "Total Final" vs "Total"** : Bien que `total` vous donne le calcul des prix standard, `finalTotal` est la source de vérité pour la transaction. Si `finalTotal` est présent, cela reflète exactement ce qui sera facturé à la carte du client, y compris tout ajustement dynamique.
4.  **Retour d'information en temps réel** : Utilisez le champ `tax` pour montrer aux utilisateurs que les taxes sont calculées en temps réel. Cela donne un aspect "live" à votre page de paiement et réduit les frictions lors de l'étape de saisie d'adresse.
...
<Info>
Cette section couvre la configuration de thème **côté client** utilisant le SDK de paiement. Vous pouvez également configurer des thèmes **côté serveur** lors de la création d'une session de paiement via l'API en utilisant le paramètre `theme_config`. Voir [Personnalisation du Thème de Paiement](/features/checkout#checkout-theme-customization) pour la configuration au niveau de l'API.
</Info>
...
Le paiement en ligne prend en charge les **mises à jour de méthode de paiement** pour les abonnements. Lorsqu'un client doit mettre à jour sa méthode de paiement - que ce soit pour un abonnement actif ou pour réactiver un abonnement en attente - vous pouvez afficher le flux de mise à jour directement au sein de votre mise en page.
...
2. Passez le `payment_link` retourné comme le `checkoutUrl` pour ouvrir le paiement en ligne :
...
L'iframe de paiement ne rend que le formulaire de collecte de méthode de paiement. Les clients peuvent saisir de nouveaux détails de carte ou sélectionner une méthode de paiement enregistrée sans quitter votre page.

### Pour les Abonnements en Attente

Lors de la mise à jour de la méthode de paiement pour un abonnement en statut `on_hold`, Dodo Payments crée automatiquement un prélèvement pour les dues restantes. Surveillez les webhooks `payment.succeeded` et `subscription.active` pour confirmer la réactivation.
...
<Tip>
Vous pouvez également utiliser une méthode de paiement enregistrée existante au lieu de collecter de nouveaux détails en passant `type: 'existing'` avec un `payment_method_id` à l'API de Mise à Jour de Méthode de Paiement.
</Tip>

## Gestion des Erreurs

Le SDK fournit des informations détaillées sur les erreurs via le système d'événements. Implémentez toujours une gestion des erreurs appropriée dans votre callback `onEvent` :
...
1. **Design Réactif** : Assurez-vous que votre élément conteneur a suffisamment de largeur et de hauteur. L'iframe s'agrandira normalement pour remplir son conteneur.
...
3. **États de Chargement** : Montrez un indicateur de chargement dans votre conteneur jusqu'à ce que l'événement `checkout.opened` se déclenche.
...
<Info>
Pour les implémentations en mode sombre, il est recommandé d'utiliser `#0d0d0d` comme couleur de fond pour une meilleure intégration visuelle avec le cadre de paiement en ligne.
</Info>

## Validation du Statut de Paiement

<Warning>
Ne comptez pas seulement sur les événements de paiement en ligne pour déterminer le succès ou l'échec du paiement. Implémentez toujours une validation côté serveur à l'aide de webhooks et/ou de polling.
</Warning>

### Pourquoi la Validation Côté Serveur est Essentielle

Bien que des événements de paiement en ligne comme `checkout.status` fournissent un retour d'information en temps réel, ils **ne doivent pas** être votre seule source de vérité pour le statut de paiement. Des problèmes de réseau, des plantages de navigateur ou des utilisateurs fermant la page peuvent entraîner la perte d'événements. Pour assurer une validation de paiement fiable :
...
2. **Implémentez un mécanisme de polling** - Votre frontend doit interroger votre serveur pour des mises à jour de statut.
...
**1. Écoutez les événements de paiement** - Lorsque l'utilisateur clique sur le paiement, commencez à vous préparer à vérifier le statut :
...
**3. Gérez les webhooks côté serveur** - Mettez à jour votre base de données lorsque Dodo envoie les webhooks `payment.succeeded` ou `payment.failed`. Voir notre [documentation sur les webhooks](/developer-resources/webhooks) pour plus de détails.
...
2. Commencez à interroger votre serveur pour le statut de paiement confirmé
3. Affichez un état "Vérification du paiement..." pendant le polling
4. Affichez l'interface utilisateur de succès/échec en fonction du statut confirmé par le serveur
...
- Vérifiez que `elementId` correspond à celui de `id` d'un `div` qui existe réellement dans le DOM.
- Assurez-vous que `displayType: 'inline'` a été passé à `Initialize`.
- Vérifiez que le `checkoutUrl` est valide.
</Accordion>

<Accordion title="Les taxes ne se mettent pas à jour dans mon interface">
- Assurez-vous que vous écoutez l'événement `checkout.breakdown`.
- Les taxes ne sont calculées qu'après que l'utilisateur a saisi un pays valide et un code postal dans le cadre de paiement.
</Accordion>
</AccordionGroup>

## Activation des Portefeuilles Numériques

Pour des informations détaillées sur la configuration de Apple Pay, Google Pay et d'autres portefeuilles numériques, consultez la page <a href="/features/payment-methods/digital-wallets">Portefeuilles Numériques</a>.

### Configuration Rapide pour Apple Pay

<Steps>
<Step title="Télécharger le fichier d'association de domaine">
...
Envoyez un e-mail à **support@dodopayments.com** avec l'URL de votre domaine de production et demandez l'activation de Apple Pay.
</Step>

<Step title="Tester après confirmation">
Une fois confirmé, vérifiez qu'Apple Pay apparaît dans le paiement et testez le flux complet.
</Step>
</Steps>

<Warning>
Apple Pay nécessite une vérification de domaine avant d'apparaître en production. Contactez le support avant de passer en ligne si vous prévoyez d'offrir Apple Pay.
</Warning>
...
## Paiement en Ligne vs Superposition de Paiement

Choisissez le bon type de paiement pour votre cas d'utilisation :

| Fonctionnalité | Paiement en Ligne | Superposition de Paiement |
|---------------|------------------|-------------------------|
...
| Effort de mise en œuvre | Plus élevé | Moins élevé |
| Meilleur pour | Pages de paiement personnalisées, flux à forte conversion | Intégration rapide, pages existantes |

<Tip>
Utilisez **le paiement en ligne** lorsque vous souhaitez un contrôle maximum sur l'expérience de paiement et un branding transparent. Utilisez **la superposition de paiement** pour une intégration plus rapide avec des modifications minimales de vos pages existantes.
</Tip>

## Ressources Associées

<CardGroup cols={2}>
<Card title="Superposition de Paiement" icon="layer-group" href="/developer-resources/overlay-checkout">
    Utilisez la superposition de paiement pour une intégration rapide basée sur un modal.
</Card>

<Card title="API de Sessions de Paiement" icon="code" href="/api-reference/checkout-sessions/create">
...
<Card title="Guide d'Intégration" icon="book" href="/developer-resources/integration-guide">
    Guide complet pour intégrer Dodo Paiements.
</Card>
</CardGroup>
