...
description: Apprenez à intégrer Dodo Payments avec votre projet Tanstack App
  Router en utilisant notre adaptateur Tanstack. Couvre le processus de
...
{/* LOCKED_PATTERN_848a353666469e7a047ef05af5a45770 */}
    Intégrez le paiement Dodo Payments dans votre application Tanstack.
</Card>
{/* LOCKED_PATTERN_df89dec21e12f4bf30b6acf75ba025e1 */}
    Permettez aux clients de gérer leurs abonnements et informations.
</Card>
...
<Warning>
Ne committez jamais votre <code>.env</code> ni vos secrets dans le contrôle de version.
</Warning>
</Step>
</Steps>
...
<Info>
Tous les exemples supposent que vous utilisez le routeur d'applications Tanstack.
</Info>

<Tabs>
{/* LOCKED_PATTERN_28fd81a4703a84d931eeff69f1520d54 */}
  <Info>
  Utilisez ce handler pour intégrer le paiement Dodo Payments dans votre application Tanstack. Il prend en charge les flux de paiement statiques (GET), dynamiques (POST) et de session (POST).
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_0026b036379d81a9d03c36b3b6e83ddd */}
  <Info>
  Utilisez ce handler pour recevoir et traiter en toute sécurité les événements webhook Dodo Payments dans votre application Tanstack.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments prend en charge trois types de flux de paiement pour intégrer des paiements sur votre site web ; cet adaptateur prend en charge tous les types de flux.
</Info>
...
<Warning>
Si <code>productId</code> est absent, le handler renvoie une réponse 400. Les paramètres de requête invalides entraînent également une réponse 400.
</Warning>
...
- Envoyez les paramètres en tant que corps JSON dans une requête POST.
- Prend en charge les paiements ponctuels et récurrents.
- Pour une liste complète des champs du corps POST pris en charge, consultez :
...
Les sessions de checkout fournissent une expérience de paiement hébergée plus sécurisée qui gère le flux de paiement complet pour les achats ponctuels et les abonnements avec un contrôle total de la personnalisation.
...
Le Gestionnaire de Route du Portail Client vous permet d'intégrer de manière transparente le portail client Dodo Payments dans votre application Tanstack.
...
<Warning>
Renvoie une erreur 400 si <code>customer_id</code> est manquant.
</Warning>
