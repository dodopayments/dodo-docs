...
Fixe le montant facturé, en unités monétaires principales (par exemple, <code>12.5</code> pour 12,50 $). Uniquement pour les produits Pay What You Want, et ignoré s’il est inférieur au prix minimal du produit.
</ParamField>
...
Checkout sessions fournit une expérience de paiement hébergée plus sécurisée qui gère l’intégralité du processus de paiement pour les achats ponctuels et les abonnements, avec un contrôle complet de la personnalisation.

Consultez le [Guide d’intégration de Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session) pour plus de détails et la liste complète des champs pris en charge.

### Format de la réponse
Checkout sessions renvoie une réponse JSON contenant l’URL de paiement :
...
Le gestionnaire de route Customer Portal vous permet d’intégrer facilement le portail client Dodo Payments à votre application Tanstack.
...
Si ce paramètre est défini sur <code>true</code>, un e-mail contenant le lien vers le portail est envoyé au client.
</ParamField>
...
- **Validation de la payload :** Validée avec Zod. Renvoie 400 si la payload n’est pas valide.
- **Gestion des erreurs :**
  - 401 : Signature non valide
  - 400 : Payload non valide
  - 500 : Erreur interne lors de la vérification
- **Routage des événements :** Appelle le gestionnaire d’événement approprié en fonction du type de payload.
