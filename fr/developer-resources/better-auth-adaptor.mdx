...
- Points de terminaison d'ingestion et de reporting d'utilisation mesurée pour la facturation basée sur l'utilisation
...
<Check>
Tous les packages requis sont maintenant installés.
</Check>
</Step>
</Steps>
...
Ajoutez ceci à votre fichier <code>.env</code> :
...
<Warning>
Ne jamais commettre de clés API ou de secrets dans le contrôle de version.
</Warning>
</Step>

<Step title="Configurer l'intégration côté serveur">
...
<Step title="Configurer l'intégration côté client">
...
<Info>
  ...
  intégrations. La méthode héritée <code>checkout</code> est obsolète et est conservée
...
<Info>
  ...
  ne nécessite pas d'informations de facturation à l'avance, car elles seront prises auprès de l'utilisateur
...
<Info>
  ...
  <code>checkoutSession</code> prend l'email et le nom du client de leur
  session better-auth, cependant, vous pouvez les remplacer en passant le
  objet <code>customer</code> avec l'email et le nom.
</Info>

<Info>
  Vous pouvez passer les mêmes options dans l'argument que le corps de la requête pour [Créer
  une session de paiement](https://docs.dodopayments.com/api-reference/checkout-sessions/create)
  point de terminaison.
</Info>

<Note>
  L'URL de retour est prise à partir de <code>successUrl</code> configuré dans le
  plugin serveur. Vous n'avez pas besoin d'inclure <code>return_url</code> dans le
  payload client.
</Note>
...
- <code>authClient.dodopayments.usage.ingest</code> ingère les événements pour l'utilisateur connecté et vérifié par e-mail.
...
<Tip>
  Les horodatages plus anciens qu'une heure ou plus de cinq minutes dans le futur sont
  rejetés lors de l'ingestion de l'utilisation.
</Tip>
<Tip>
  Si vous omettez <code>meter_id</code> lors de la liste des compteurs d'utilisation, tous les compteurs 
...
<Info>
  Le plugin webhooks traite les événements de paiement en temps réel de Dodo Payments avec
  vérification de signature sécurisée. Le point de terminaison par défaut est{" "}
  <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
<Step title="Générer et définir le secret du webhook">
Générez un secret de webhook pour votre URL de point de terminaison (par exemple, <code>https://&lt;votre-domaine&gt;/api/auth/dodopayments/webhooks</code>) dans le tableau de bord Dodo Payments et définissez-le dans votre <code>.env</code> :
...
<AccordionGroup>
  <Accordion title="Options du plugin">
    - <b>client</b> (obligatoire) : instance du client DodoPayments -{" "}
    <b>createCustomerOnSignUp</b> (facultatif) : Création automatique de clients lors de l'inscription de l'utilisateur - <b>use</b> (obligatoire) : Tableau de plugins à activer (checkout,
    portal, usage, webhooks)
  </Accordion>
  <Accordion title="Options du plugin de paiement">
...
<AccordionGroup>
  <Accordion title="Problèmes courants">
    ...
    <b>Mismatch de signature de webhook</b> : Assurez-vous que le secret du webhook correspond à celui
  ...
  <Accordion title="Meilleures pratiques">
    - Utilisez des variables d'environnement pour tous les secrets et clés. - Testez en{" "}
    <code>test_mode</code> avant de passer à <code>live_mode</code>. - Enregistrez
