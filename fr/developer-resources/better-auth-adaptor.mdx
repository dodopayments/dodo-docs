---
title: Adaptateur Better-Auth
description: Ce guide vous montre comment intégrer Dodo Payments dans votre flux
  d'authentification en utilisant l'adaptateur Better-Auth.
icon: code-merge
---

# Vue d'ensemble

L'<b>Adaptateur Better Auth</b> pour Dodo Payments fournit :

- Création automatique de clients lors de l'inscription
- Sessions de paiement (préférées) avec mappage de slug de produit
- Portail client en libre-service
- Points de terminaison d'ingestion et de reporting d'utilisation pour la facturation basée sur l'utilisation
- Traitement des événements webhook en temps réel avec vérification de signature
...
- Projet existant utilisant [better-auth](https://www.npmjs.com/package/better-auth)
...
<Check>
Tous les paquets requis sont maintenant installés.
</Check>
</Step>
</Steps>
...
Ajoutez-les à votre <code>.env</code> :
...
Créez ou mettez à jour <code>src/lib/auth.ts</code> :
...
<Tip>
Définissez <code>environment</code> sur <code>live_mode</code> pour la production.
</Tip>
</Step>

{/* LOCKED_PATTERN_673a79a3fa2d75e23b52c6942c035dd2 */}
Créez ou mettez à jour <code>src/lib/auth-client.ts</code> :
...
<Info>
  Préférez <code>authClient.dodopayments.checkoutSession</code> pour les nouvelles
  intégrations. L’ancienne méthode <code>checkout</code> est obsolète et conservée
  uniquement pour la rétrocompatibilité.
</Info>

## Création d'une session de paiement (préférée)
...
<Info>
  Contrairement à l’ancienne méthode <code>checkout</code>, <code>checkoutSession</code>
  ne nécessite pas d’informations de facturation à l’avance car elles seront prises
  auprès de l’utilisateur sur la page de paiement. Vous pouvez toujours les
  remplacer en passant la clé <code>billing</code> dans l’argument.
</Info>

<Info>
  Comme pour l’ancienne méthode <code>checkout</code>, <code>checkoutSession</code>
  prend l’e-mail et le nom du client à partir de leur session better-auth, mais
  vous pouvez les remplacer en passant l’objet <code>customer</code> contenant
  l’e-mail et le nom.
</Info>

<Info>
  Vous pouvez passer les mêmes options dans l’argument que dans le corps de la
  requête vers l’endpoint [Create Checkout Session](https://docs.dodopayments.com/api-reference/checkout-sessions/create).
</Info>

<Note>
  L’URL de retour est récupérée depuis le <code>successUrl</code> configuré dans le
  plugin serveur. Vous n’avez pas besoin d’inclure <code>return_url</code> dans la
  charge utile du client.
</Note>

## Paiement hérité (obsolète)

<Warning>
  La méthode <code>authClient.dodopayments.checkout</code> est obsolète. Utilisez
  <code>checkoutSession</code> à la place pour les nouvelles implémentations.
</Warning>
...
Activez le plugin <code>usage()</code> sur le serveur pour capturer les événements mesurés et permettre aux clients d'inspecter leur facturation basée sur l'utilisation.

- <code>authClient.dodopayments.usage.ingest</code> ingère des événements pour l'utilisateur connecté, vérifié par email.
- <code>authClient.dodopayments.usage.meters.list</code> liste l'utilisation récente pour les compteurs liés aux abonnements de ce client.
...
<Tip>
  Les horodatages vieux de plus d’une heure ou situés plus de cinq minutes dans
  le futur sont rejetés lors de l’ingestion d’usage.
</Tip>
<Tip>
  Si vous omettez <code>meter_id</code> lors de la liste des compteurs d’usage,
  tous les compteurs liés aux abonnements du client sont renvoyés.
</Tip>

# Webhooks

<Info>
  Le plugin webhooks traite les événements de paiement en temps réel provenant de
  Dodo Payments avec une vérification sécurisée des signatures. L’endpoint par
  défaut est <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
{/* LOCKED_PATTERN_1f3f4c5fbf255932f2c0aedf7d16c279 */}
Générez un secret webhook pour votre URL d’endpoint (par ex. <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) dans le tableau de bord Dodo Payments et définissez-le dans votre fichier <code>.env</code> :
...
<AccordionGroup>
  {/* LOCKED_PATTERN_9586fa213a07317c452129bb9c8365ad */}
    - <b>client</b> (requis) : instance client DodoPayments -
    <b>createCustomerOnSignUp</b> (optionnel) : création automatique de clients lors de l’inscription des utilisateurs -
    <b>use</b> (requis) : tableau de plugins à activer (checkout, portal, usage, webhooks)
  </Accordion>
  {/* LOCKED_PATTERN_a313db3f1395a5521aceb89c3251fabd */}
    - <b>products</b> : tableau de produits ou fonction asynchrone renvoyant des produits -
    <b>successUrl</b> : URL de redirection après un paiement réussi -
    <b>authenticatedUsersOnly</b> : exiger l’authentification de l’utilisateur (par défaut : false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  {/* LOCKED_PATTERN_8cb977e6bc7a95fb0f5c0e3c34015b67 */}
    - <b>Invalid API key</b> : vérifiez à nouveau votre <code>DODO_PAYMENTS_API_KEY</code> dans <code>.env</code>. -
    <b>Webhook signature mismatch</b> : assurez-vous que le secret webhook correspond à celui défini dans le tableau de bord Dodo Payments. - <b>Customer not created</b> : confirmez que <code>createCustomerOnSignUp</code> est défini sur <code>true</code>.
  </Accordion>
  {/* LOCKED_PATTERN_328949dc9617c5646d2623ce7f82eb78 */}
    - Utilisez des variables d’environnement pour tous les secrets et clés. - Testez en <code>test_mode</code> avant de passer en <code>live_mode</code>. - Enregistrez les événements webhook pour le débogage et l’audit.
  </Accordion>
</AccordionGroup>
