...
<Steps>
<Step title="Générer et définir le secret webhook">
Générez un secret webhook pour votre URL de point de terminaison (par exemple, <code>https://&lt;votre-domaine&gt;/api/auth/dodopayments/webhooks</code>) dans le tableau de bord Dodo Payments et définissez-le dans votre fichier <code>.env</code> :
...
<AccordionGroup>
  <Accordion title="Options du Plugin">
    ...
    <b>createCustomerOnSignUp</b> (optionnel) : Création automatique de clients lors de l'inscription de l'utilisateur - <b>use</b> (obligatoire) : Tableau de plugins à activer (paiement,
    portail, webhooks)
  </Accordion>
  <Accordion title="Options du Plugin de Paiement">
    ...
    <b>authenticatedUsersOnly</b> : Exiger l'authentification de l'utilisateur (par défaut : faux)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Problèmes Courants">
    - <b>Clé API invalide</b> : Vérifiez votre <code>DODO_PAYMENTS_API_KEY</code> dans <code>.env</code>. -{" "}
    <b>Mismatch de signature webhook</b> : Assurez-vous que le secret webhook correspond à celui
    défini dans le tableau de bord Dodo Payments. - <b>Client non créé</b> : Confirmez que <code>createCustomerOnSignUp</code> est défini sur <code>true</code>.
  </Accordion>
  <Accordion title="Meilleures Pratiques">
    - Utilisez des variables d'environnement pour tous les secrets et clés. - Testez en <code>test_mode</code> avant de passer à <code>live_mode</code>. - Enregistrez
