...
<Info>
  Contrairement à la méthode héritée <code>checkout</code>, <code>checkoutSession</code>
  ne nécessite pas d'informations de facturation à l'avance car elles seront fournies par l'utilisateur
  sur la page de paiement. Vous pouvez toujours les remplacer en passant la
  clé <code>billing</code> dans l'argument.
</Info>

<Info>
  Semblable à la méthode héritée <code>checkout</code>,
  <code>checkoutSession</code> prend l'email et le nom du client à partir de leur
  session better-auth, cependant, vous pouvez les remplacer en passant l'objet
  <code>customer</code> avec l'email et le nom.
</Info>

<Info>
  Vous pouvez passer les mêmes options dans l'argument que le corps de la requête pour le point de terminaison [Créer
  une session de paiement](https://docs.dodopayments.com/api-reference/checkout-sessions/create).
</Info>

<Note>
  L'URL de retour est prise à partir de <code>successUrl</code> configurée dans le
...
<Warning>
  ...
  <code>checkoutSession</code> pour de nouvelles implémentations.
</Warning>
...
Activez le plugin <code>usage()</code> sur le serveur pour capturer les événements mesurés et permettre aux clients de vérifier leur facturation basée sur l'usage.

- <code>authClient.dodopayments.usage.ingest</code> ingère des événements pour l'utilisateur connecté, dont l'email est vérifié.
...
<Tip>
  Les horodatages antérieurs à une heure ou plus de cinq minutes dans le futur sont
  rejetés lors de l'ingestion de l'utilisation.
</Tip>
<Tip>
  Si vous omettez <code>meter_id</code> en listant les compteurs d'utilisation, tous les compteurs
  liés aux abonnements du client sont retournés.
</Tip>

# Webhooks

<Info>
  Le plugin de webhooks traite les événements de paiement en temps réel de Dodo Payments avec
  une vérification de signature sécurisée. Le point de terminaison par défaut est
  <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
<Step title="Generate and set webhook secret">
Générez un secret de webhook pour votre URL de point de terminaison (par exemple, <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) dans le tableau de bord Dodo Payments et définissez-le dans votre fichier <code>.env</code>:
...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (obligatoire) : Instance client DodoPayments
    - <b>createCustomerOnSignUp</b> (optionnel) : Création automatique de clients lors de l'inscription de l'utilisateur
    - <b>use</b> (obligatoire) : Tableau de plugins à activer (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (optionnel) : Fonction qui reçoit l'authentification BetterAuth `User` et retourne des champs supplémentaires à attacher au client DodoPayments lors de la création et mise à jour (par exemple `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b> : Tableau de produits ou fonction asynchrone retournant des produits -{" "}
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>Clé API invalide</b> : Vérifiez votre
    <code>DODO_PAYMENTS_API_KEY</code> dans <code>.env</code>. -
    <b>Discordance de signature de webhook</b> : Assurez-vous que le secret du webhook correspond à celui
    défini dans le tableau de bord Dodo Payments. - <b>Client non créé</b> : Confirmez que
    <code>createCustomerOnSignUp</code> est défini sur <code>true</code>.
  </Accordion>
  <Accordion title="Best Practices">
    - Utilisez des variables d'environnement pour tous les secrets et clés. - Testez en
    <code>test_mode</code> avant de passer en <code>live_mode</code>. - Enregistrez
    les événements webhook pour le débogage et l'audit.
  </Accordion>
</AccordionGroup>

# Invite pour les LLMs
