...
<Info>
Les changements de plan peuvent déclencher un frais immédiat en fonction du mode de proratisation que vous choisissez.
</Info>
...
- Migration des utilisateurs vers un nouveau produit ou prix sans annuler leur abonnement
...
- `payment.succeeded` : Facture immédiate pour le changement de plan réussie
...
<Note>
Des champs comme <code>invoice_id</code> et <code>payment_id</code> ne sont retournés que lorsqu'une facturation immédiate et/ou une facture est créée lors du changement de plan. Comptez toujours sur les événements de webhook (par exemple, <code>payment.succeeded</code>, <code>subscription.plan_changed</code>) pour confirmer les résultats.
</Note>
...
<Tip>
Choisissez `prorated_immediately` pour une comptabilité équitable dans le temps ; choisissez `full_immediately` pour redémarrer la facturation ; utilisez `difference_immediately` pour des mises à niveau simples et un crédit automatique sur les rétrogradations.
</Tip>
...
<Note>
Pour des schémas de charge utile détaillés, consultez les <a href="/developer-resources/webhooks/intents/subscription">charges utiles de webhook d'abonnement</a> et les <a href="/developer-resources/webhooks/intents/payment">charges utiles de webhook de paiement</a>.
</Note>
...
<AccordionGroup>
<Accordion title="Frais créés mais abonnement non mis à jour">
**Symptômes** : L'appel API réussit mais l'abonnement reste sur l'ancien plan

**Causes courantes** :
- Traitement du webhook échoué ou retardé
- État de l'application non mis à jour après réception des webhooks
...
**Causes courantes** :
- Attentes du mode de proratisation : les rétrogradations créditent la différence de prix du plan complet avec `difference_immediately`, tandis que `prorated_immediately` crée un crédit proratisé basé sur le temps restant dans le cycle
...
<Accordion title="Échec du changement de plan avec erreur 422">
...
<Accordion title="Échec de la facturation immédiate lors du changement de plan">
**Symptômes** : Changement de plan initié mais échec de la facturation immédiate
...
- Envisagez de permettre des changements de plan avec des échecs de facturation immédiate
</Accordion>
...
**Que se passe-t-il** :
Lorsque le frais de changement de plan échoue, l'abonnement est automatiquement placé dans l'état `on_hold`. L'abonnement ne se renouvellera pas automatiquement tant que le moyen de paiement n'est pas mis à jour.
...
2. **Création automatique de frais** : L'API crée automatiquement un frais pour les montants dus
3. **Génération de facture** : Une facture est générée pour le frais
...
- `subscription.on_hold` : Abonnement placé en attente (reçu lorsque le frais de changement de plan échoue)
...
**Meilleures pratiques** :
- Informez immédiatement les clients lorsque le frais de changement de plan échoue
...
<Step title="Tester différents modes de proratisation">
