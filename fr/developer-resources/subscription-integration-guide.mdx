...
Pour les charges suivantes, utilisez l'endpoint [POST /subscriptions/{subscription_id}/charge](/api-reference/subscriptions/create-charge) et spécifiez le montant à facturer au client pour cette transaction.
...
Pour les charges ultérieures, utilisez le point de terminaison [POST /subscriptions/{subscription_id}/charge](/api-reference/subscriptions/create-charge) et spécifiez le montant à facturer au client pour cette transaction.

<Note>
Pour un guide complet, étape par étape—y compris des exemples de requêtes/réponses, des politiques de nouvelle tentative sûres, et la gestion des webhooks—voir le <a href="/developer-resources/ondemand-subscriptions">Guide des Abonnements à la Demande</a>.
</Note>
