...
3. **`subscription.on_hold`** - L'abonnement est suspendu en raison d'un échec de renouvellement.
...
2. `payment.succeeded` - Confirme le paiement initial :
    - Pour une facturation immédiate (0 jours d'essai) : Attendez-vous à ce que cela prenne entre 2 et 10 minutes
...
Pour réactiver un abonnement de l'état {/* INLINE_CODE_PLACEHOLDER_833e01167b2e3da70d83bdfb406adc7f */, utilisez l'API de mise à jour du mode de paiement. Cela fait automatiquement :
...
Lorsque vous recevez un webhook {/* INLINE_CODE_PLACEHOLDER_5e9a0875003c752d72379e80bcc55356 */, mettez à jour l'état de votre application et informez le client :
...
| `timestamp` | string | Oui | L'horodatage de quand l'événement s'est produit (pas nécessairement le même que celui de la livraison) |
...
- Charge le client uniquement pour la différence entre l'ancien et le nouveau plan
- Pendant une période d'essai, cela passera immédiatement l'utilisateur au nouveau plan, facturant le client immédiatement

#### 2. `full_immediately`
- Charge le client du montant total de l'abonnement pour le nouveau plan
...
- Lors d'une mise à niveau, le client est immédiatement facturé de la différence entre les deux montants de plan.
- Par exemple, si le plan actuel est de 30 dollars et que le client passe à un plan de 80 dollars, il est facturé 50 dollars instantanément. 
- Lors d'une rétrogradation, le montant inutilisé du plan actuel est ajouté en tant que crédit interne et appliqué automatiquement aux renouvellements d'abonnement futurs.
...
Pour créer un abonnement à la demande, utilisez le [POST /subscriptions](/api-reference/subscriptions/post-subscriptions) point de terminaison de l'API et incluez le champ `on_demand` dans le corps de votre requête. Cela vous permet d'autoriser un mode de paiement sans frais immédiats, ou de définir un prix initial personnalisé.
