---
title: Tarification par siège avec options supplémentaires et liens de paiement
description: Apprenez à créer des options supplémentaires pour la tarification
  par siège et à générer des liens de paiement avec des quantités d'options
  personnalisées en utilisant Dodo Payments
icon: users
---

Dans ce tutoriel, vous apprendrez à mettre en œuvre la tarification par siège en utilisant les options supplémentaires de Dodo Payments. Nous allons créer un produit d'abonnement avec des options supplémentaires pour des sièges additionnels et vous montrer comment générer des liens de paiement avec des quantités d'options personnalisées.
...
- Créer des produits d'abonnement avec une tarification par siège
...
Créons un modèle de tarification par siège :
...
<Step title="Entrez les détails de l'option supplémentaire">
Remplissez ces valeurs pour notre option supplémentaire pour siège :

**Nom de l'option supplémentaire** : `Additional Team Seat`
...
Nous allons commencer par créer un produit d'abonnement de base qui inclut 5 membres d'équipe. Cela sera la base de notre modèle de tarification par siège.
...
<Step title="Remplissez les détails de l'abonnement">
...
## Étape 3 : Connectez l'option supplémentaire à l'abonnement
...
<Check>
**Tarification par siège configurée !** Les clients peuvent maintenant acheter votre plan de base et ajouter des sièges supplémentaires selon leurs besoins.
</Check>
</Step>
</Steps>
...
Créez un fichier `.env` avec votre clé API Dodo Payments :
...
Testons notre implémentation de la tarification par siège pour nous assurer que tout fonctionne correctement.

<Steps>
<Step title="Démarrez votre serveur">
1. Assurez-vous d'avoir votre fichier `.env` avec la clé API correcte
2. Mettez à jour les identifiants de produit et d'option dans votre code avec les valeurs réelles de votre tableau de bord Dodo Payments
...
2. Vous devriez voir l'interface de démonstration de la tarification par siège
...
4. Complétez l'achat test

<Check>
Le paiement doit afficher la répartition des prix correcte et vous permettre de compléter l'achat.
</Check>
</Step>
...
## Félicitations ! Vous avez mis en œuvre la tarification par siège

Vous avez réussi à créer un système de tarification par siège avec Dodo Payments ! Voici ce que vous avez accompli :
...
<Info>
Cet exemple démontre seulement une implémentation minimale de la tarification par siège. Pour une utilisation en production, vous devriez ajouter une gestion des erreurs robuste, une authentification, une validation des données, des mesures de sécurité et adapter la logique pour répondre aux exigences de votre application.
</Info>
