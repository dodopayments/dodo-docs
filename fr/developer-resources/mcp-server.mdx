...
<Tab title="Planche à voile">
Ajouter à `~/.codeium/windsurf/mcp_config.json`:
...
Après avoir cliqué, définissez vos variables d'environnement dans le `mcp.json` de Cursor via **Paramètres de Cursor** > **Outils & MCP** > **Nouveau serveur MCP**.
...
Après avoir cliqué, définissez vos variables d'environnement dans le `mcp.json` de VS Code via **Palette de commandes** > **MCP : Ouvrir la configuration utilisateur**.
