...
<Info>
**Construit avec [ContextMCP.ai](https://contextmcp.ai)** : Dodo Knowledge MCP est alimenté par ContextMCP, permettant une recherche sémantique rapide à travers notre documentation en utilisant des embeddings vectoriels.
</Info>
...
<Note>
Nécessite Node.js 18 ou version supérieure. Le package `mcp-remote` gère la connexion au serveur MCP distant.
</Note>
...
<Frame>
  <iframe 
      ...
      title="Utilisation du Serveur MCP Distant de Dodo Payment"
      frameBorder="0"
...
<Tip>
Utilisez `@latest` pour toujours tirer la version la plus récente, ou fixez-vous à une version spécifique comme `@1.0.0` pour la stabilité.
</Tip>
...
<Tabs>
<Tab title="Cursor">
Configurez le serveur Dodo Payments MCP dans Cursor pour permettre un accès conversationnel à vos données de paiement.
...
Après avoir cliqué, définissez vos variables d'environnement dans le fichier `mcp.json` de Cursor via **Paramètres de Cursor** > **Outils & MCP** > **Nouveau serveur MCP**.
...
<Step title="Éditer la configuration">
    Cliquez sur **Modifier Config** pour ouvrir le fichier `mcp_config.json`.
</Step>
...
<Step title="Enregistrer et redémarrer">
...
<Tab title="Claude Code">
Configurez le serveur Dodo Payments MCP dans Claude Code CLI.
...
La configuration sera stockée dans le fichier `.claude.json` dans votre répertoire personnel.
...
<Tab title="VS Code">
Configurez le serveur Dodo Payments MCP dans VS Code.
...
Après avoir cliqué, définissez vos variables d'environnement dans le fichier `mcp.json` de VS Code via **Palette de commandes** > **MCP : Ouvrir la configuration utilisateur**.

**Configuration Manuelle**

<Steps>
<Step title="Ouvrir la configuration MCP">
...
<Step title="Ajouter la configuration du serveur">
    Ajoutez le serveur Dodo Payments MCP :
...
<Step title="Recharger la fenêtre">
...
<Step title="Configurer le serveur MCP">
    Ajoutez la configuration du serveur Dodo Payments MCP :
...
<Tab title="Zed">
Configurez le serveur Dodo Payments MCP dans Zed pour permettre un accès conversationnel à vos données de paiement.
<Steps>
<Step title="Ouvrir les paramètres de Zed">
    Naviguez vers **Paramètres** > **Assistant** > **Modèle de Protocole de Contexte**.
</Step>
...
Configurez le serveur Dodo Payments MCP dans d'autres clients compatibles MCP.
...
| `DODO_PAYMENTS_ENVIRONMENT` | À régler sur `live_mode` pour la production | Non |
...
- `top-level-unions` - Support pour les types d'union au niveau supérieur
- `valid-json` - Correction de l'analyse des arguments JSON en chaîne
...
3. **`invoke_api_endpoint`** - Exécute toute méthode avec les paramètres appropriés

<Info>
Les outils dynamiques sont automatiquement activés pour les API avec plus de 50 méthodes. Remplacez avec `--tools=all` pour inclure tous les outils.
</Info>
...
Lors du déploiement à distance, demandez toujours une authentification via l'en-tête `Authorization` ou l'en-tête `x-dodo-payments-api-key`.
...
Vérifiez que `DODO_PAYMENTS_ENVIRONMENT` est réglé correctement (`live_mode` pour la production).
...
<Accordion title="Problèmes de compatibilité du client">
**Essayez un autre préréglage de client**
...
<Card title="Protocole MCP" icon="link" href="https://modelcontextprotocol.io">
  En savoir plus sur la norme du Protocole de Contexte Modèle
</Card>

<Card title="Dépôt GitHub" icon="github" href="https://github.com/dodopayments/dodopayments-typescript/tree/main/packages/mcp-server">
  Voir le code source du serveur MCP et contribuer
</Card>

<Card title="Package NPM" icon="npm" href="https://www.npmjs.com/package/dodopayments-mcp">
