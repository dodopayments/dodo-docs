...
Configurez le serveur MCP de Dodo Payments dans votre client IA préféré.

<Tabs>
<Tab title="Cursor">
Configurez le serveur MCP de Dodo Payments dans Cursor pour permettre un accès conversationnel à vos données de paiement.
...
Après avoir cliqué, définissez vos variables d'environnement dans `mcp.json` via **Paramètres de Cursor** > **Outils & MCP** > **Nouveau serveur MCP**.
...
Configurez le serveur MCP de Dodo Payments dans Claude Desktop pour permettre un accès conversationnel à vos données de paiement.
<Steps>
...
Configurez le serveur MCP de Dodo Payments dans Claude Code CLI.
...
<Tab title="VS Code">
Configurez le serveur MCP de Dodo Payments dans VS Code.
...
Après avoir cliqué, définissez vos variables d'environnement dans `mcp.json` via **Palette de commandes** > **MCP : Ouvrir la configuration utilisateur**.
...
<Step title="Ajouter la configuration du serveur">
    Ajoutez le serveur MCP de Dodo Payments :
...
Configurez le serveur MCP de Dodo Payments dans Cline (VS Code) pour permettre un accès conversationnel à vos données de paiement.
<Steps>
...
<Step title="Configurer le serveur MCP">
    Ajoutez la configuration du serveur MCP de Dodo Payments :
...
<Step title="Recharger VS Code">
    Rechargez la fenêtre de VS Code (`Cmd/Ctrl + Shift + P` > "Recharger la fenêtre") pour activer la connexion au serveur MCP.
</Step>
</Steps>
</Tab>

<Tab title="Zed">
Configurez le serveur MCP de Dodo Payments dans Zed pour permettre un accès conversationnel à vos données de paiement.
<Steps>
...
Configurez le serveur MCP de Dodo Payments dans d'autres clients compatibles MCP.
...
### Préréglages de clients disponibles
...
- `top-level-unions` - Support pour les types d'union de niveau supérieur
...
<Tip>
Utilisez `--operation=read` pendant le développement pour éviter des modifications accidentelles de vos données.
</Tip>
...
## Mode d'outils dynamiques

Pour les grandes API, charger tous les outils en même temps peut submerger la fenêtre de contexte de l'IA. Le mode d'outils dynamiques offre une alternative plus efficace.
...
Construisez des serveurs MCP personnalisés ou étendez celui existant de manière programmatique.
...
**Mettre en œuvre l'authentification pour les serveurs distants**

Lors du déploiement à distance, exigez toujours une authentification via l'en-tête `Authorization` ou l'en-tête `x-dodo-payments-api-key`.
...
**Mettre en œuvre des limites de taux**
...
**Vérifiez les journaux du client**
...
Vérifiez que l'opération fonctionne lors de l'appel de l'API Dodo Payments directement via curl ou Postman.
</Accordion>

<Accordion title="Problèmes de compatibilité des clients">
**Essayez un autre préréglage de client**

Si les outils ne fonctionnent pas correctement, essayez de spécifier explicitement votre client :
