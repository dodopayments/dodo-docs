...
2. **Logique de filtrage** : Sélectionnez **OU** (cela signifie "comptez si L'UN de ces critères est vrai")
3. **Ajoutez la première condition** :
   - Clé de propriété : `quality`
   - Comparateur : `equals`  
   - Valeur : `standard`
4. **Cliquez sur "Ajouter une condition"** pour la seconde :
   - Clé de propriété : `quality`
   - Comparateur : `equals`
   - Valeur : `hd`
...
**Prix par unité** : Entrez → `0.05` (cela correspond à 0,05 $ par image)

**Seuil gratuit** : Entrez → `10` (les clients reçoivent 10 images gratuites par mois)
...
2. Accédez à **Clients** dans la barre latérale gauche
