...
Si les nouvelles tentatives continuent d'échouer parce que le moyen de paiement lui-même est défectueux (carte expirée, compte fermé, etc.), utilisez [`POST /subscriptions/{subscription_id}/update-payment-method`](/api-reference/subscriptions/update-payment-method) pour en obtenir un nouveau du client. En cas de succès, l'abonnement revient à `active` et `payment.succeeded` suivi par `subscription.active` webhooks sont émis.
</Step>
</Steps>
...
Utilisez votre clé API de test pour créer l'abonnement, puis ouvrez le `checkout_url` retourné et complétez le mandat.
</Step>
<Step title="Trigger a charge">
Appelez le point de terminaison de facturation avec un petit `product_price` (par exemple, `100`) et vérifiez que vous recevez `payment.succeeded`.
</Step>
<Step title="Go live">
Passez à votre clé API en direct une fois que vous avez validé les événements et les mises à jour de l'état interne.
</Step>
</Steps>
