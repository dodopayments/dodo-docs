...
1. Vous créez un abonnement avec l'objet `on_demand` pour autoriser une méthode de paiement et éventuellement collecter un premier paiement.
...
<AccordionGroup>
  <Accordion title="Paramètres du corps de la demande de charge">
    ...
    <ParamField body="product_currency" type="string">
      Remplacement de la devise facultatif pour la charge.
    </ParamField>

    <ParamField body="product_description" type="string">
      Remplacement de la description facultatif pour cette charge.
    </ParamField>
...
<Warning>
Les modèles de réessai en rafale peuvent être signalés comme frauduleux ou soupçonnés de tests de carte par nos systèmes de risque et processeurs. Évitez les réessais groupés ; suivez le calendrier de retour en arrière et les conseils d'alignement temporel ci-dessous.
</Warning>
...
- **Varier les métadonnées (facultatif)** : Si vous maintenez votre propre système de réessai, différenciez les réessais via des métadonnées (par exemple, `retry_attempt`).
...
- **422 Demande invalide** : Assurez-vous que `on_demand.mandate_only` est fourni lors de la création et que `product_price` est fourni pour les charges.
