...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Intégrez le checkout Dodo Payments dans votre application SvelteKit.
</Card>
...
<Warning>
Ne commettez jamais votre fichier <code>.env</code> ni vos secrets dans le contrôle de version.
</Warning>
</Step>
</Steps>
...
<Tab title="Checkout Handler">
  <Info>
  Utilisez ce gestionnaire pour intégrer le checkout Dodo Payments dans votre application SvelteKit. Il prend en charge les flux de paiement statiques (GET), dynamiques (POST) et basés sur la session (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Utilisez ce gestionnaire pour recevoir et traiter en toute sécurité les événements webhook Dodo Payments dans votre application SvelteKit.
  </Info>
  <CodeGroup>
...
Fixe le montant facturé, exprimé dans les unités principales de la devise (par ex., <code>12.5</code> pour 12,50 $). Uniquement pour les produits Pay What You Want, et ignoré s’il est inférieur au prix minimum du produit.
</ParamField>
...
- Pour la liste complète des champs acceptés dans le corps POST, référez-vous à :
...
<Warning>
Dynamic Checkout utilise les endpoints `POST /payments` et `POST /subscriptions`, désormais obsolètes. Il continue de fonctionner pour les intégrations existantes, mais les nouvelles intégrations doivent utiliser les **Checkout Sessions** ci-dessous.
</Warning>
...
Les Checkout Sessions offrent une expérience de paiement hébergée plus sécurisée, qui gère l’ensemble du processus de paiement pour les achats ponctuels et les abonnements, avec un contrôle complet de la personnalisation.

Consultez le [guide d’intégration des Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session) pour plus de détails et la liste complète des champs pris en charge.

### Format de la réponse
Les Checkout Sessions renvoient une réponse JSON contenant l’URL de paiement :
...
Le gestionnaire de route Customer Portal vous permet d’intégrer facilement le portail client Dodo Payments à votre application SvelteKit.

### Paramètres de requête
<ParamField query="customer_id" type="string" required>
L’identifiant client pour la session du portail (par exemple, <code>?customer_id=cus_123</code>).
</ParamField>
<ParamField query="send_email" type="boolean">
Si cette valeur est définie sur <code>true</code>, un e-mail contenant le lien vers le portail est envoyé au client.
</ParamField>
...
- **Vérification de la signature :** la signature du webhook est vérifiée à l’aide de <code>webhookKey</code>. Renvoie 401 si la vérification échoue.
...
- **Routage des événements :** appelle le gestionnaire d’événement approprié en fonction du type de charge utile.
