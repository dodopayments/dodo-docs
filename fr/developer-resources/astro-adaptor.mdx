---
title: Adaptateur Astro
description: Découvrez comment intégrer <CardGroup cols={2}> à votre projet Astro App
  Router à l’aide de notre adaptateur Astro. Couvre le checkout, le Customer
  Portal, les webhooks et la configuration sécurisée de l’environnement.
icon: code-merge
keywords:
  - Dodo Payments
  - Astro
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Intégrez le checkout de Dodo Payments à votre application Astro.
</Card>
...
<Info>
Tous les exemples supposent que vous utilisez Astro App Router.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Utilisez ce gestionnaire pour intégrer le checkout de Dodo Payments à votre application Astro. Il prend en charge les flux de paiement statiques (GET), dynamiques (POST) et de session (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Utilisez ce gestionnaire pour recevoir et traiter de manière sécurisée les événements webhook de Dodo Payments dans votre application Astro.
  </Info>
  <CodeGroup>
...
Identifiant du produit (p. ex. <code>?productId=pdt_nZuwz45WAs64n3l07zpQR</code>).
</ParamField>
...
Spécifier la devise du paiement (p. ex. <code>USD</code>).
</ParamField>
...
Fixe le montant facturé, exprimé dans les unités monétaires principales (p. ex. <code>12.5</code> pour 12,50 $). Uniquement pour les produits Pay What You Want, et ignoré s’il est inférieur au prix minimum du produit.
</ParamField>
...
- Pour obtenir la liste complète des champs pris en charge dans le corps POST, consultez :
  - [Corps de la requête pour un produit de paiement unique](https://docs.dodopayments.com/api-reference/payments/post-payments)
  - [Corps de la requête pour un produit d’abonnement](https://docs.dodopayments.com/api-reference/subscriptions/post-subscriptions)
...
Les sessions Checkout offrent une expérience de checkout hébergée et plus sécurisée qui gère l’intégralité du flux de paiement pour les achats uniques et les abonnements, avec un contrôle complet de la personnalisation.
...
Le gestionnaire de routes Customer Portal vous permet d’intégrer facilement le Customer Portal de Dodo Payments à votre application Astro.
...
S’il vaut <code>true</code>, envoie un e-mail au client contenant le lien vers le portail.
</ParamField>
