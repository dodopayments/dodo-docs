...
Désactiver le champ du nom.
</ParamField>
<ParamField query="disableEmail" type="boolean">
Désactiver le champ d’adresse e-mail.
</ParamField>
...
Fixe le montant facturé, exprimé dans les unités principales de la devise (p. ex. <code>12.5</code> pour 12,50 $). Uniquement pour les produits Pay What You Want, et ignoré s’il est inférieur au prix minimum du produit.
</ParamField>
...
<Warning>
Dynamic Checkout proxyfie les endpoints `POST /payments` et `POST /subscriptions` dépréciés. Il continue de fonctionner pour les intégrations existantes, mais les nouvelles intégrations doivent utiliser **Checkout Sessions** ci-dessous.
</Warning>

### Format de la réponse
Dynamic checkout renvoie une réponse JSON contenant l’URL de checkout :
...
Checkout sessions offrent une expérience de checkout hébergée plus sécurisée qui gère l’intégralité du processus de paiement, aussi bien pour les achats ponctuels que pour les abonnements, avec un contrôle complet de la personnalisation.
...
### Format de la réponse
Checkout sessions renvoient une réponse JSON contenant l’URL de checkout :
...
Le gestionnaire de route Customer Portal vous permet d’intégrer facilement le portail client Dodo Payments à votre application Remix.
...
- **Validation du payload :** effectuée avec Zod. Renvoie 400 si le payload est invalide.
- **Gestion des erreurs :**
  - 401 : signature invalide
  - 400 : payload invalide
  - 500 : erreur interne lors de la vérification
- **Routage des événements :** appelle le gestionnaire d’événements approprié en fonction du type de payload.
