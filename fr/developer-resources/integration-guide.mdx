...
2. Créez un produit (paiement unique ou abonnement). Les produits d'abonnement doivent être tarifés à au moins **1 $** (ou l'équivalent dans la devise choisie) ; les montants inférieurs à ce minimum ne sont pas pris en charge.
...
<Card title="Create Checkout Session" icon="code" href="/api-reference/checkout-sessions/create">
  Référence API pour créer des sessions de paiement sécurisé et hébergé pour des paiements uniques et des abonnements
</Card>
<Card title="Create Payment Link" icon="link" href="/api-reference/payments/post-payments">
  Référence API pour créer des liens de paiement dynamiques de manière programmatique
</Card>
</CardGroup>