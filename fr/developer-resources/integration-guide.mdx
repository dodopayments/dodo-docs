...
<Step title="Add advanced controls (optional)">
  ...
  - <ParamField query="paymentAmount" type="integer">Montant en centimes (uniquement pour la tarification Pay What You Want).</ParamField>
...
<Step title="Share the link">
  Envoyez le lien de paiement finalisé à votre client. Lorsqu'il le consulte, tous les paramètres de requête sont collectés et enregistrés avec un identifiant de session. L'URL est ensuite simplifiée pour ne contenir que le paramètre de session (par exemple, <code>?session=sess_1a2b3c4d</code>). Les informations enregistrées persistent après les actualisations de la page et sont accessibles pendant tout le processus de paiement.
  <Check>
...
Créé via un appel d'API ou notre SDK avec les informations du client. Voici un exemple :
...
<Info>Après la création du lien de paiement, redirigez vos clients pour qu'ils finalisent leur paiement.</Info>
...
Notre implémentation des Webhooks suit la spécification [Standard Webhooks](https://standardwebhooks.com/). Pour les définitions des types de Webhooks, consultez notre [guide des événements Webhook](/developer-resources/webhooks/intents/webhook-events-guide).
...
| `payment.processing` | Le paiement est accepté, mais son traitement est toujours en cours (méthodes asynchrones). | Attendez le `payment.succeeded`/`payment.failed` final — n'exécutez pas encore la commande. |
| `payment.cancelled`  | Le paiement est annulé avant sa finalisation.                   | Libérez tout état réservé et marquez la commande comme abandonnée.                       |
...
Si vous vendez des produits numériques avec des clés de licence, gérez également `license_key.created`. Pour consulter la liste complète des événements — notamment les événements liés aux abonnements, aux droits, aux crédits, à la récupération et au recouvrement — consultez le [guide des événements Webhook](/developer-resources/webhooks/intents/webhook-events-guide).

Vous pouvez consulter ce projet, qui contient une implémentation de démonstration, sur [GitHub](https://github.com/dodopayments/dodo-checkout-demo), avec Next.js et TypeScript.
