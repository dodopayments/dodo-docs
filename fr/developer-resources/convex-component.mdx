...
<CardGroup cols={2}>
<Card title="Fonction de Paiement" icon="cart-shopping" href="#checkout-function">
    Intégrez le paiement Dodo avec un flux basé sur la session.
</Card>
<Card title="Portail Client" icon="user" href="#customer-portal-function">
...
<Step title="Ajouter le Composant à la Configuration Convex">
...
Configurez les variables d'environnement dans votre tableau de bord Convex (**Paramètres** → **Variables d'environnement**). Vous pouvez accéder au tableau de bord en exécutant :
...
<Steps>
<Step title="Créer une Requête Interne">
...
<Warning>
Avant d'utiliser cette requête, assurez-vous de définir le schéma approprié dans votre fichier `convex/schema.ts` ou de modifier la requête pour correspondre à votre schéma existant.
</Warning>
...
<Step title="Configurer le Composant DodoPayments">
 <CodeGroup>
...
<Tabs>
<Tab title="Configuration de la Fonction de Paiement">
  <Info>
...
<Tab title="Configuration du portail client">
  <Info>
  Utilisez cette fonction pour permettre aux clients de gérer leurs abonnements et leurs détails via le portail client Dodo Payments. Le client est automatiquement identifié via la fonction `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="Configuration du gestionnaire de webhook">
  <Info>
  Utilisez ce gestionnaire pour recevoir et traiter les événements de webhook Dodo Payments en toute sécurité dans votre application Convex. Tous les gestionnaires de webhook reçoivent le `ActionCtx` de Convex comme premier paramètre, vous permettant d'utiliser `ctx.runQuery()` et `ctx.runMutation()` pour interagir avec votre base de données.
  </Info>
  <CodeGroup>
...
<Info>
Le client est automatiquement identifié à l'aide de la fonction `identify` configurée dans votre configuration DodoPayments. Cette fonction doit retourner l'`dodoCustomerId` du client.
</Info>
