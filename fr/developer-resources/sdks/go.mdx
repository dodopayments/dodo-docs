...
Lorsque l'API renvoie un code de statut non réussi, le SDK renvoie une erreur de type
`*dodopayments.Error`. Elle expose le `StatusCode`, le sous-jacent `*http.Request` et
`*http.Response`, ainsi que le JSON du corps de l'erreur. Utilisez le modèle `errors.As` pour
l'examiner, et branchez sur `StatusCode` pour gérer des cas spécifiques :
...
- **GitHub** : Ouvrez un problème sur le [répertoire](https://github.com/dodopayments/dodopayments-go)
