...
<Info>
`billing` nécessite au minimum le code pays ISO à deux lettres. `customer` accepte soit `{"customer_id": ...}` pour associer un client existant, soit `{"email": ..., "name": ...}` pour en créer un nouveau. `product_price` est exprimé dans la plus petite dénomination de la devise.
</Info>
...
Suivez les événements personnalisés pour la facturation à l'usage :
...
Parcourez automatiquement tous les éléments :
...
Utilisez aiohttp pour améliorer les performances asynchrones :
...
Ou pour activer la journalisation au niveau debug :
...
Vous avez besoin d'aide avec le SDK Python ?
...
- **GitHub** : Ouvrez un ticket dans le [dépôt](https://github.com/dodopayments/dodopayments-python)

## Contribuer

Les contributions sont les bienvenues ! Consultez les [directives de contribution](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md) pour commencer.
