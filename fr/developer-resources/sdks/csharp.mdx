...
<Info>
`Billing` nécessite au minimum le code ISO à deux lettres `Country`. Utilisez `AttachExistingCustomer` pour attacher un client existant, ou `NewCustomer` pour en créer un. `ProductPrice` est dans la plus basse dénomination de monnaie.
</Info>

## Gestion des erreurs

Le SDK déclenche des exceptions spécifiques en fonction du code d'état HTTP. Toutes les erreurs 4xx héritent de `DodoPayments4xxException`.
...
- `DodoPaymentsIOException` : Erreurs de réseau I/O
...
Itérez à travers tous les résultats sur toutes les pages en utilisant la méthode `Paginate`, qui retourne un `IAsyncEnumerable` :
...
- **GitHub** : Ouvrez un problème sur le [répertoire](https://github.com/dodopayments/dodopayments-csharp)
