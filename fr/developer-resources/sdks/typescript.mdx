...
<Info>
`billing` nécessite au minimum le code pays ISO à deux lettres. `customer` est une union de `{ customer_id }` (pour associer un client existant) ou de `{ email, name? }` (pour en créer un nouveau). `product_price` est exprimé dans la plus petite unité monétaire.
</Info>
...
<Info>
Les événements doivent avoir des valeurs `event_id` uniques pour garantir l’idempotence. Les ID en double dans une même requête sont rejetés, et les requêtes suivantes contenant des ID existants sont ignorées.
</Info>

### Récupérer les événements d’utilisation

Récupérez les informations détaillées sur les événements d’utilisation :
...
Configurez les paramètres du proxy pour différents runtimes :
...
Contrôlez le niveau de verbosité des journaux à l’aide de variables d’environnement ou des options du client :
...
- `'debug'` - Afficher les messages de débogage, les informations, les avertissements et les erreurs
- `'info'` - Afficher les informations, les avertissements et les erreurs  
- `'warn'` - Afficher les avertissements et les erreurs (par défaut)
- `'error'` - Afficher uniquement les erreurs
- `'off'` - Désactiver toute la journalisation

<Warning>
Au niveau de débogage, toutes les requêtes et réponses HTTP sont journalisées, y compris les en-têtes et les corps. Certains en-têtes d’authentification sont masqués, mais les données sensibles présentes dans les corps peuvent rester visibles.
</Warning>

## Migration depuis le SDK Node.js

Si vous effectuez une mise à niveau depuis le SDK Node.js hérité, le SDK TypeScript offre une meilleure sécurité de typage ainsi que davantage de fonctionnalités :
...
Les runtimes suivants sont pris en charge :

- Navigateurs web (versions à jour de Chrome, Firefox, Safari, Edge, etc.)
...
- Cloudflare Workers
- Vercel Edge Runtime
- Jest 28 ou version ultérieure avec l’environnement `"node"`
...
- **GitHub** : Ouvrez une issue sur le [dépôt](https://github.com/dodopayments/dodopayments-typescript)

## Contribution

Les contributions sont les bienvenues ! Consultez les [directives de contribution](https://github.com/dodopayments/dodopayments-typescript/blob/main/CONTRIBUTING.md) pour commencer.
