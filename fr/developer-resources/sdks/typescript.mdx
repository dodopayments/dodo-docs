...
<Info>
Les événements doivent avoir des valeurs ID uniques `event_id` pour l'idempotence. Les ID dupliqués dans la même demande sont rejetés, et les demandes ultérieures avec des ID existants sont ignorées.
</Info>
...
- `'debug'` - Afficher les messages de débogage, d'info, d'avertissements et d'erreurs
- `'info'` - Afficher les messages d'info, d'avertissements et d'erreurs  
- `'warn'` - Afficher les avertissements et erreurs (par défaut)
...
Les méthodes de liste dans l'API DodoPayments sont paginées. Vous pouvez utiliser la syntaxe `for await … of` pour itérer à travers les éléments de toutes les pages :
...
- Navigateurs web (Chrome, Firefox, Safari, Edge à jour, et plus)
- Versions Node.js 20 LTS ou ultérieures ([non-EOL](https://endoflife.date/nodejs))
- Deno v1.28.0 ou supérieur
- Bun 1.0 ou ultérieur
- Cloudflare Workers
- Vercel Edge Runtime
- Jest 28 ou plus avec l'environnement `"node"`
