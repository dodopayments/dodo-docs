...
`dodo wh trigger` peut envoyer des payloads simulés pour les 47 types d’événements que Dodo Payments fournit :
...
| **Clé de licence** | `created` |
| **Versement** | `created`, `in_progress`, `on_hold`, `failed`, `success` |
| **Crédit** | `added`, `deducted`, `expired`, `rolled_over`, `rollover_forfeited`, `overage_charged`, `overage_reset`, `manual_adjustment`, `balance_low` |
| **Checkout abandonné** | `detected`, `recovered` |
| **Relance de paiement** | `started`, `recovered` |
| **Attribution de droits** | `created`, `delivered`, `failed`, `revoked` |

<Note>
Les formes des payloads simulés suivent les schémas correspondants dans la référence API. Consultez [Événements Webhook](/developer-resources/webhooks/intents/webhook-events-guide) pour comprendre la signification de chaque événement et savoir quand Dodo Payments l’émet en production.
</Note>

<Tip>
`payout.created` est émis tant que le versement affiche encore un statut `not_initiated`, le payload simulé en tient donc également compte. Consultez [Événements de versement](/developer-resources/webhooks/intents/payout) pour connaître l’intégralité du cycle de vie du versement.
</Tip>
...
La CLI recherche une nouvelle version au démarrage et affiche une notification dans la barre d’état lorsqu’une version est disponible. Pour effectuer la mise à niveau :

```text
/update
```

Vous pouvez également réexécuter l’installateur pour effectuer la mise à niveau sur place :
...
- **GitHub** : Ouvrez un ticket dans le [dépôt](https://github.com/dodopayments/dodopayments-cli/issues)
