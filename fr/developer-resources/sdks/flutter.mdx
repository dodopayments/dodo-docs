...
Personnalisez la barre d'outils, les boutons et la palette de couleurs du navigateur de checkout via
`customization` sur `CheckoutParams`. Les options sont regroupées par plateforme, car
...
Permet à la barre d'outils de se réduire lors du défilement. Visible uniquement lorsque `presentationStyle` est `fullScreen` — `pageSheet` maintient les barres épinglées, quel que soit ce réglage.
</ParamField>
