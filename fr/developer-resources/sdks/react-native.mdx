...
Personnalisez la barre d'outils, les boutons et le jeu de couleurs du navigateur de checkout via
`customization` sur `start(...)`. Les options sont regroupées par plateforme, car
l'onglet personnalisé d'Android et `SFSafariViewController` d'iOS exposent des
contrôles natifs différents. Tous les champs sont facultatifs ; si vous omettez `customization`, l'apparence
...
Permet à la barre d'outils de se réduire lors du défilement. Visible uniquement lorsque `presentationStyle` est `fullScreen` — `pageSheet` maintient les barres fixes, quel que soit ce réglage.
</ParamField>
