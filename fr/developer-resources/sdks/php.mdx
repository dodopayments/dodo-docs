...
Le SDK utilise des paramètres nommés pour spécifier les arguments optionnels. Vous pouvez initialiser des objets valeur en utilisant le constructeur statique `with` :
...
Lorsque la bibliothèque ne peut pas se connecter à l'API ou reçoit un code d'état non réussi (4xx ou 5xx), une sous-classe de `APIException` est levée :
...
<Note>
  Les paramètres `extra*` avec le même nom remplacent les paramètres documentés.
</Note>
