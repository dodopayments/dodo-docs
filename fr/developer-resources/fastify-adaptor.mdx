---
title: Adaptateur Fastify
description: Apprenez à intégrer Dodo Payments avec votre projet Fastify App
  Router en utilisant notre adaptateur NextJS. Couvre le processus de paiement,
...
<Card title="Gestionnaire de Paiement" icon="cart-shopping" href="#checkout-route-handler">
    Intégrez le processus de paiement Dodo Payments dans votre application Fastify.
</Card>
...
<Tab title="Gestionnaire de Webhook">
  <Info>
  Utilisez ce gestionnaire pour recevoir et traiter les événements de webhook de Dodo Payments de manière sécurisée dans votre application Fastify.
  </Info>
  <CodeGroup>
...
Tout paramètre de requête commençant par <code>metadata_</code> sera passé comme métadonnées.
</ParamField>
...
### Gestionnaires d'Événements de Webhook Supportés
<CodeGroup>
...
L'adaptateur @dodopayments/fastify fournit des gestionnaires de routes pour les fonctionnalités de Paiement, Portail Client et Webhook de Dodo Payments, conçues pour s'intégrer directement dans une application Fastify.
...
**But** : Ce gestionnaire redirige les utilisateurs vers la page de paiement Dodo Payments.
...
But : Cette route permet aux clients de gérer leurs abonnements via le portail Dodo Payments.
...
But : Traite les événements de webhook entrants de Dodo Payments pour déclencher des événements dans votre application.
