...
{/* LOCKED_PATTERN_8dc9e7ab6a6bc27accdab8f9d7e4a851 */}
    Intégrez le processus de paiement Dodo Payments dans votre application Nuxt à l'aide d'une route serveur.
</Card>
...
{/* LOCKED_PATTERN_6ee2fb9f853d24077e79222b982063ef */}
    Recevez et traitez en toute sécurité les événements webhook de Dodo Payments dans Nuxt.
</Card>
</CardGroup>
...
<Info>
Ce guide explique comment intégrer Dodo Payments à votre application Nuxt à l'aide du module Nuxt officiel. Vous apprendrez à configurer les routes de paiement, le portail client et les webhooks API, ainsi qu'à gérer en toute sécurité les variables d'environnement.
</Info>
...
<Warning>
N'ajoutez jamais votre fichier <code>.env</code> ou vos secrets dans le contrôle de version.
</Warning>
...
<Info>
Utilisez ce gestionnaire pour intégrer la caisse Dodo Payments dans votre application Nuxt. Il prend en charge les flux de paiement statiques (GET), dynamiques (POST) et de session (POST).
</Info>
<CodeGroup>
...
<Info>
Créez une route GET pour permettre aux clients d'accéder à leur portail. Accepte <code>customer_id</code> en paramètre de requête.
</Info>
<CodeGroup>
...
<Info>
Créez une route POST pour recevoir et traiter en toute sécurité les événements webhook de Dodo Payments.
</Info>
<CodeGroup>
...
<Info>
Dodo Payments prend en charge trois types de flux de paiement pour intégrer les paiements sur votre site ; cet adaptateur prend en charge tous les types de flux.
</Info>
...
- Pour la liste complète des champs acceptés dans le corps de la requête POST, consultez :
...
Les sessions de paiement offrent une expérience de caisse hébergée plus sécurisée qui gère l'intégralité du flux de paiement pour les achats uniques et les abonnements, avec un contrôle total de la personnalisation.
...
### Paramètres de requête
<ParamField query="customer_id" type="string" required>
L'ID client pour la session de portail (par exemple <code>?customer_id=cus_123</code>).
</ParamField>
<ParamField query="send_email" type="boolean">
