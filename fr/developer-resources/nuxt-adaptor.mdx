...
<Warning>
Dynamic Checkout utilise les endpoints obsolètes `POST /payments` et `POST /subscriptions` comme proxies. Il continue de fonctionner pour les intégrations existantes, mais les nouvelles intégrations doivent utiliser **Checkout Sessions** ci-dessous.
</Warning>

### Format de réponse
Dynamic Checkout renvoie une réponse JSON contenant l’URL de paiement :
...
Checkout Sessions offrent une expérience de paiement hébergée plus sécurisée qui gère l’intégralité du processus de paiement pour les achats ponctuels et les abonnements, avec un contrôle complet de la personnalisation.

Consultez le [Guide d’intégration de Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session) pour plus d’informations et la liste complète des champs pris en charge.

### Format de réponse
Checkout Sessions renvoient une réponse JSON contenant l’URL de paiement :
...
Le gestionnaire de route Customer Portal vous permet d’intégrer facilement le portail client Dodo Payments dans votre application Nuxt.

### Paramètres de requête
<ParamField query="customer_id" type="string" required>
L’identifiant client de la session du portail (par exemple, <code>?customer_id=cus_123</code>).
</ParamField>
<ParamField query="send_email" type="boolean">
S’il est défini sur <code>true</code>, envoie un e-mail au client contenant le lien vers le portail.
</ParamField>
