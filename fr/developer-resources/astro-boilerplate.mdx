...
- **Intégration des paiements** - Flux de paiement préconfiguré utilisant `@dodopayments/astro`
- **UI moderne** - Page de tarification propre et sombre avec Tailwind CSS
- **Gestionnaire de Webhooks** - Point de terminaison webhook prêt à l'emploi pour les événements de paiement
- **Portail client** - Gestion des abonnements en un clic
- **TypeScript** - Entièrement typé avec des types minimaux et ciblés
- **Checkout pré-rempli** - Montre comment transmettre des données client pour améliorer l'expérience utilisateur
...
Mettez à jour `src/lib/products.ts` avec vos véritables IDs de produits de Dodo Payments :
...
Ouvrez [http://localhost:4321](http://localhost:4321) pour voir votre page de tarification !
</Step>
</Steps>
...
- IDs de produits (depuis votre tableau de bord Dodo)
- Tarification
- Caractéristiques
- Descriptions
...
Dans `src/components/Header.astro`, remplacez l'ID client codé en dur par l'ID client réel de votre système d'authentification ou de votre base de données :
...
Le modèle démontre la gestion des événements webhook dans `src/pages/api/webhook.ts` :

- `onSubscriptionActive` - Déclenché lorsque l'abonnement devient actif
- `onSubscriptionCancelled` - Déclenché lorsque l'abonnement est annulé
...
N'oubliez pas de mettre à jour la variable d'environnement `DODO_PAYMENTS_WEBHOOK_KEY` dans votre environnement de production pour correspondre à la clé de signature webhook pour votre domaine déployé.

## Dépannage

<AccordionGroup>
<Accordion title="Module introuvable ou erreurs de build">
...
- Vérifiez la console du navigateur et le terminal pour des erreurs
</Accordion>

<Accordion title="Webhooks ne reçoivent pas d'événements">
...
Mettez à jour l'URL du webhook dans votre [tableau de bord Dodo](https://app.dodopayments.com/developer/webhooks) vers l'URL ngrok. N'oubliez pas de mettre à jour votre fichier .env avec la clé de vérification du webhook correcte.
</Accordion>

<Accordion title="Le lien du portail client ne fonctionne pas">
Remplacez le `CUSTOMER_ID` codé en dur dans `src/components/Header.astro` par un véritable ID client de votre tableau de bord Dodo.

Ou intégrez votre système d'authentification et votre base de données pour récupérer dynamiquement l'identifiant client.
</Accordion>

<Accordion title="La construction échoue avec une erreur d'adaptateur">
Ce boilerplate utilise une sortie statique avec des routes API à la demande. Vous devez installer un adaptateur pour le déploiement :
