...
2. **Passer des montants dynamiques** : Incluez un champ `amount` dans le panier de produits lors de la création de sessions de paiement
3. **Laissez les clients choisir** : Si aucun montant n'est fourni, les clients peuvent entrer leur propre prix (dans vos limites)

<Info>
Lorsque vous passez un `amount` dans le panier de produits, ce montant est utilisé pour le paiement. Si vous omettez le champ `amount`, les clients peuvent choisir leur propre prix lors du paiement (sous réserve de vos paramètres minimum/maximum).
</Info>
...
Cliquez sur **Ajouter un produit** pour enregistrer. Notez l'ID de votre produit (par exemple, `pdt_123abc456def`) à utiliser dans les sessions de paiement.
</Step>
</Steps>
...
Une fois que votre produit est configuré avec Pay What You Want, vous pouvez créer des sessions de paiement avec des montants dynamiques. Le champ `amount` dans le panier de produits vous permet de définir le prix de manière programmatique pour chaque session de paiement.

### Comprendre le champ Montant

Lors de la création d'une session de paiement, vous pouvez inclure un champ `amount` dans chaque élément du panier de produits :
...
<Warning>
Le champ `amount` ne s'applique qu'aux produits Pay What You Want. Pour les produits réguliers, ce champ est ignoré.
</Warning>
...
<Note>
**Format de montant** : Le champ `amount` doit être dans la plus petite unité de la devise. Pour l'USD, cela signifie des cents (par exemple, 25,00 $ = `2500`). Pour d'autres devises, utilisez la plus petite unité (par exemple, paise pour INR).
</Note>

## Étape 3 : Laisser les clients choisir leur prix

Si vous souhaitez que les clients sélectionnent leur propre prix lors du paiement, il vous suffit d'omettre le champ `amount` du panier de produits. La page de paiement affichera un champ de saisie où les clients peuvent entrer n'importe quel montant dans vos limites minimum et maximum.
...
Utilisez les métadonnées pour suivre pourquoi des montants spécifiques ont été choisis (par exemple, `pricing_tier`, `discount_code`, `user_segment`).
</Card>
...
- Le produit est un produit **de paiement unique** (one-time), pas un abonnement
- Le montant est dans le bon format (plus petite unité de devise, par exemple, cents pour USD)
</Accordion>

<Accordion title="Le montant dépasse le maximum ou est inférieur au minimum">
L'API rejettera les sessions de paiement où le montant enfreint les limites de prix de votre produit. Validez toujours les montants avant de créer des sessions de paiement, ou laissez les clients choisir leur prix en omettant le champ `amount`.
</Accordion>

<Accordion title="Le client ne peut pas entrer son propre prix">
Si les clients ne voient pas le champ de saisie de prix, assurez-vous d'avoir omis le champ `amount` du panier de produits. Lorsque `amount` est fourni, le paiement utilise ce montant exact.
</Accordion>
</AccordionGroup>