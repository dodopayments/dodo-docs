...
Fixe le montant facturé, en unités monétaires principales (par exemple, <code>12.5</code> pour 12,50 $). Uniquement pour les produits Pay What You Want, et ignoré s’il est inférieur au prix minimum du produit.
</ParamField>
<ParamField query="showDiscounts" type="boolean">
Afficher les champs de remise.
</ParamField>
<ParamField query="metadata_*" type="string">
Tout paramètre de requête commençant par <code>metadata_</code> sera transmis en tant que metadata.
</ParamField>
...
<Warning>
Dynamic Checkout utilise les endpoints `POST /payments` et `POST /subscriptions`, désormais obsolètes. Il continue de fonctionner pour les intégrations existantes, mais les nouvelles intégrations doivent utiliser **Checkout Sessions** ci-dessous.
</Warning>
...
Checkout Sessions offre une expérience de paiement hébergée plus sécurisée, qui gère l’intégralité du processus de paiement pour les achats uniques comme pour les abonnements, avec un contrôle total de la personnalisation.
...
Le gestionnaire de route Customer Portal vous permet d’intégrer facilement le portail client Dodo Payments à votre application Next.js.
