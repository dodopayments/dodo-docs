...
Horodatage ISO 8601 auquel l’événement s’est produit. Utilise par défaut l’horodatage UTC actuel s’il n’est pas fourni. Doit se situer dans une fenêtre allant d’une heure dans le passé à 5 minutes dans le futur — les horodatages en dehors de cette fenêtre sont rejetés.
</ParamField>
...
<Info>
**Définissez les horodatages intentionnellement.** Omettez `timestamp` pour les événements en temps réel : l’horodatage UTC actuel sera alors utilisé par défaut. Définissez-le explicitement (au format ISO 8601) pour les événements différés ou groupés afin que l’utilisation soit affectée à la bonne période de facturation, mais notez que la fenêtre acceptée est étroite : les événements dont l’horodatage remonte à plus d’**une heure** ou se situent à plus de **5 minutes dans le futur** sont rejetés. Le rétrosynchronisation des données historiques n’est pas prise en charge — envoyez les événements mis en mémoire tampon dans l’heure.
</Info>
