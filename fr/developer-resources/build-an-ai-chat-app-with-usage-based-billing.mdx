...
<Steps>
<Step title="Obtenez la clé API Dodo Payments">
...
De plus, définissez le **Prix Fixe** à `0` pour garantir que les clients ne sont facturés que sur la base de leur utilisation, sans frais de base.
...
<Step title="Créez le serveur backend">
...
Ajoutons maintenant une belle interface de chat avec l'historique complet des conversations ! Créez `public/index.html` :
...
1. Vérifiez que votre fichier `.env` a toutes les clés API de l'Étape 2
...
<Step title="Ouvrez l'interface de chat">
1. Ouvrez votre navigateur
2. Accédez à `http://localhost:3000`
3. Vous devriez voir la belle interface de chat

<Note>
Assurez-vous de mettre à jour le `CUSTOMER_ID` dans `server.ts` avec votre véritable ID client test de l'Étape 4.
</Note>
</Step>
...
**Solutions :**
- Assurez-vous que la clé test commence par `test_`, la clé live commence par `live_`
