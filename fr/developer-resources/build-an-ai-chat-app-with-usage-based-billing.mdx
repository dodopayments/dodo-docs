---
title: Construire une application de chat AI avec AI SDK & LLM Blueprint
...
Dans ce tutoriel, vous allez construire une **application de chat AI** avec une facturation automatique basée sur l'utilisation. Nous allons créer tout à partir de zéro : le compteur de facturation, la configuration du produit, et le code de l'application qui alimente les conversations et suit l'utilisation des tokens en temps réel.

<Note>
Ce tutoriel fournit une application fonctionnelle complète avec à la fois le backend et le frontend. L'application de chat utilise l'AI de Google Gemini et suit automatiquement l'utilisation des tokens sans aucun comptage manuel requis.
</Note>
...
- **Modèle de tarification** : Paiement par token (0,01 $ par 1 000 tokens)
...
**Nom du compteur** → `AI Token Usage Meter`
...
<Step title="Obtenir la clé API Google AI">
...
## Étape 3 : Créer votre produit de facturation

Maintenant, nous devons créer un produit qui définit notre tarification (0,01 $ par 1 000 tokens avec 10 000 tokens gratuits). Cela connecte notre compteur à une facturation réelle.
...
**Prix par unité** : Entrez → `0.00001` (c'est 0,01 $ par 1 000 tokens ou 0,00001 $ par token)

**Seuil gratuit** : Entrez → `10000` (les clients obtiennent 10 000 tokens gratuits par mois)
...
<Step title="Trouver votre ID client">
...
## Étape 5 : Construire l'application de chat
...
<Step title="Créer la structure du projet">
...
## Étape 6 : Ajouter l'interface de chat
...
## Étape 7 : Tester votre application de chat
...
<Step title="Avoir votre première conversation">
...
Maintenant, vérifions que les événements sont bien reçus :
...
<Step title="Vérifier le comptage des tokens">
...
<Step title="Tester le niveau gratuit">
...
2. Mettez à jour vers la dernière version du SDK Blueprints : `npm install @dodopayments/ingestion-blueprints@latest`
</Accordion>
