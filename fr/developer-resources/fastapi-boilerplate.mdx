...
- Transmettez les trois en-têtes `webhook-id`, `webhook-timestamp` et `webhook-signature` à `webhook.verify()` — la signature Standard Webhooks couvre `id.timestamp.body`, et non le corps seul
</Accordion>
</AccordionGroup>
