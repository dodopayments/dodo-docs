...
Vous pouvez synchroniser n'importe quelle combinaison de ces entités en les spécifiant dans le paramètre `scopes`. Toutes les opérations de synchronisation sont incrémentielles et ne transférent que les enregistrements nouveaux ou mis à jour pour une performance optimale.
...
Liste d'entités de données à synchroniser, séparées par des virgules. Portées disponibles : `licences`, `payments`, `customers`, `subscriptions`. Exemple : `"payments,customers"`.
</ParamField>

<ParamField path="--api-key" type="string" required>
Votre clé API Dodo Payments. Doit commencer par `dp_live_` pour le mode production ou `dp_test_` pour le mode test.
</ParamField>

<ParamField path="--env" type="string" required>
Cible d'environnement. Doit être soit `"live_mode"` soit `"test_mode"`. Cela détermine quel environnement Dodo Payments synchroniser.
</ParamField>
...
<Tip>
L'option `interval` est requise lors de l'utilisation d'`.start()` pour la synchronisation automatique. La synchronisation s'exécutera en continu à l'intervalle spécifié jusqu'à ce que le processus soit arrêté.
</Tip>
...
Temps en secondes entre les synchronisations automatiques. Requis lors de l'utilisation d'`.start()` pour la synchronisation automatique. Optionnel lors de l'utilisation d'`.run()` pour la synchronisation manuelle.
</ParamField>
...
**ClickHouse** : Les tables (`Subscriptions`, `Payments`, `Licenses`, `Customers`) seront créées en utilisant le moteur ReplacingMergeTree. Lors de la requête, utilisez le mot-clé `FINAL` pour garantir des résultats dédupliqués.
</Warning>
