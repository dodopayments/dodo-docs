...
<Info>
**Mise en œuvre** : Disponible via [npm package](https://www.npmjs.com/package/dodo-sync) | **Code Source** : [GitHub](https://github.com/dodopayments/dodo-sync)
</Info>
...
L'outil CLI fournit un moyen rapide de configurer et d'exécuter la synchronisation de base de données. Installez-le globalement pour l'utiliser depuis n'importe où dans votre terminal :
...
Intervalle de synchronisation en secondes. Détermine la fréquence à laquelle l'opération de synchronisation s'exécute. Si non fourni, la synchronisation s'exécutera une fois et sortira.
</ParamField>
...
**ClickHouse** : Des tables (`Subscriptions`, `Payments`, `Licenses`, `Customers`) seront créées en utilisant le moteur ReplacingMergeTree. Lors de l'interrogation, utilisez le mot-clé `FINAL` pour garantir des résultats dédupliqués.
</Warning>
