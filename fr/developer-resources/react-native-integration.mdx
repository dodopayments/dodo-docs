...
- 🎥 Regardez notre [vidéo de démonstration](https://youtube.com/shorts/uy4vikrKMPI) pour voir le SDK Dodo Payments en action
...
Vous pouvez personnaliser le Checkout Unifié React Native pour qu'il corresponde au design de votre application en modifiant les couleurs, les polices, et plus encore via le paramètre d'apparence lors de l'appel à `initPaymentSession()`.
...
| `componentBorder` | Couleur de bord externe pour les entrées, les onglets et d'autres composants |
...
| `componentText` | Couleur du texte d'entrée (ex. : numéro de carte, code postal) |
| `placeholderText` | Couleur du texte d'espace réservé pour les champs de saisie |
| `icon` | Couleur pour les icônes (ex. : bouton de fermeture) |
| `error` | Couleur pour les messages d'erreur et les actions destructrices |

Exemple de configuration avec support pour les modes clair et sombre :
