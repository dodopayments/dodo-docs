...
|--------|------|--------|-------------|
| `mode` | `"test" \| "live"` | Oui | Mode environnement : `'test'` pour le développement, `'live'` pour la production |
...
| `options.showTimer` | `boolean` | Non | Afficher ou masquer le minuteur de paiement. Par défaut `true`. Lorsqu'il est désactivé, vous recevrez l'événement `checkout.link_expired` lorsque la session expire. |
| `options.showSecurityBadge` | `boolean` | Non | Afficher ou masquer le badge de sécurité. Par défaut `true`. |
| `options.manualRedirect` | `boolean` | Non | Lorsqu'il est activé, le paiement ne redirigera pas automatiquement après l'achèvement. À la place, vous recevrez les événements `checkout.status` et `checkout.redirect_requested` pour gérer la redirection vous-même. |
...
Lors de l'utilisation de `manualRedirect`, gérez l'achèvement de la commande dans votre fonction de rappel `onEvent` :
...
Le SDK fournit des événements en temps réel que vous pouvez écouter via la fonction de rappel `onEvent` :
...
| `checkout.form_ready` | Le formulaire de paiement est prêt à recevoir des entrées utilisateur. Utile pour masquer les états de chargement. |
...
| `checkout.status` | Déclenché lorsque `manualRedirect` est activé. Contient le statut de paiement (`succeeded`, `failed`, ou `processing`). |
...
<Info>
Cette section couvre la configuration de thème **côté client** utilisant le SDK de paiement. Vous pouvez également configurer des thèmes **côté serveur** lors de la création d'une session de paiement via l'API à l'aide du paramètre `theme_config`. Voir [Personnalisation du Thème de Paiement](/features/checkout#checkout-theme-customization) pour la configuration au niveau de l'API.
</Info>
...
Si vous souhaitez seulement personnaliser le thème clair :
...
Si vous souhaitez seulement personnaliser le thème sombre :
...
#### Configuration de Thème avec d'Autres Options

Vous pouvez combiner la configuration de thème avec d'autres options de paiement :
...
Le SDK fournit des informations d'erreur détaillées via le système d'événements. Implémentez toujours une gestion des erreurs appropriée dans votre fonction de rappel `onEvent` :
...
<Warning>
Geérez toujours l'événement `checkout.error` pour fournir une bonne expérience utilisateur lorsque des erreurs se produisent.
</Warning>

## Meilleures Pratiques

1. **Initialiser une seule fois** : Initiez le SDK une seule fois lorsque votre application se charge, et non à chaque tentative de paiement.
2. **Gestion des erreurs** : Implémentez toujours une gestion des erreurs appropriée dans votre fonction de rappel d'événements.
3. **Mode Test** : Utilisez le mode `test` pendant le développement et passez à `live` seulement lorsque vous êtes prêt pour la production.
...
8. **Redirections Manuelles** : Utilisez `manualRedirect` lorsque vous avez besoin de contrôle personnalisé sur la navigation après le paiement.
9. **Gestion du Minuteur** : Désactivez le minuteur (`showTimer: false`) si vous souhaitez gérer l'expiration de session manuellement.
...
- Vérifiez que l'initialisation du SDK a été effectuée avec succès.
...
**Causes possibles :**
- Conflits CSS avec le style de votre application.
- Paramètres de thème non appliqués correctement.
- Problèmes de design réactif.

**Solutions :**
- Vérifiez les conflits CSS dans les outils de développement du navigateur.
- Vérifiez que les paramètres de thème sont corrects.
- Testez sur différentes tailles d'écran.
- Assurez-vous qu'il n'y a pas de conflits de z-index avec la superposition.
...
Pour des informations détaillées sur la configuration de Google Pay et d'autres portefeuilles numériques, consultez la page <a href="/features/payment-methods/digital-wallets">Portefeuilles Numériques</a>.

<Note>
Apple Pay n'est pas encore supporté dans le paiement en superposition. La prise en charge d'Apple Pay arrive bientôt.
</Note>
...
<Tip>
Utilisez le **paiement en superposition** pour une intégration plus rapide avec des modifications minimales à vos pages existantes. Utilisez le **paiement intégré** lorsque vous souhaitez un contrôle maximal sur l'expérience de paiement et un branding harmonieux.
</Tip>

## Ressources Connexes

<CardGroup cols={2}>
<Card title="Paiement Intégré" icon="credit-card" href="/developer-resources/inline-checkout">
    Intégrez le paiement directement dans votre page pour des expériences complètement intégrées.
</Card>
