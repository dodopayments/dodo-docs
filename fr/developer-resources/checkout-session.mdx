...
Activer le mode de collecte d'adresse minimale. Lorsqu'il est activé, le processus de paiement ne collecte que :
...
Cela réduit considérablement les frictions lors du paiement en éliminant les champs de formulaire inutiles.

<Tip>
Activez l'adresse minimale pour un achèvement plus rapide du paiement. La collecte d'adresse complète reste disponible pour les entreprises qui nécessitent des détails de facturation complets.
</Tip>
</ParamField>
...
Forcer l'interface de paiement à s'afficher dans une langue spécifique.
...
Configurer des fonctionnalités et comportements spécifiques pour la session de paiement.
...
Si défini sur vrai, ne réalise aucun prélèvement et autorise uniquement les détails du moyen de paiement pour une utilisation future.
</ParamField>
...
Remplacement optionnel de la description du produit pour la facturation et les lignes d'articles. Si non spécifié, la description stockée du produit sera utilisée.
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
Indique si les frais de devise adaptatifs doivent être inclus dans le prix du produit (vrai) ou ajoutés en plus (faux). Ignoré si la tarification adaptative n'est pas activée.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
Voici 10 exemples complets montrant différentes configurations de sessions de paiement pour divers scénarios commerciaux :
...
<Note>
Le remplacement `billing_currency` n'a d'effet que lorsque la devise adaptative est activée dans les paramètres de votre compte. Si la devise adaptative est désactivée, ce paramètre n'aura aucun effet.
</Note>
