...
<Tip>
**Options d'intégration alternatives** : au lieu d'effectuer une redirection, vous pouvez intégrer directement le paiement dans votre page avec [Overlay Checkout](/developer-resources/overlay-checkout) (superposition modale) ou [Inline Checkout](/developer-resources/inline-checkout) (entièrement intégré). Dans une application mobile native, transmettez la même URL aux [SDK Mobile Checkout](/developer-resources/mobile-integration) pour Android, iOS, React Native ou Flutter. Toutes ces options utilisent la même URL de session de paiement.
</Tip>
</Step>

<Step title="Handle the return">
Après le paiement, les clients sont redirigés vers votre `return_url` avec des paramètres de requête comprenant l'ID du paiement ou de l'abonnement, le statut, l'adresse e-mail du client et les éventuelles clés de licence. Consultez la [documentation des paramètres `return_url`](#request-body) pour obtenir la liste complète.
</Step>
</Steps>
...
**Exemple :** `1` pour un seul article, `3` pour plusieurs quantités
</ParamField>
...
Montant payé par le client lorsque pay_what_you_want est activé. Si cette option est désactivée, ce champ est ignoré.
...
Remplacements par session de paiement pour les droits de crédit déjà associés à ce produit. Utilisez ce champ pour accorder un nombre différent de crédits pour cette session uniquement — par exemple, une offre promotionnelle accordant 1 000 crédits API au lieu des 500 crédits par défaut du produit — sans créer de produit distinct.

<Info>
Chaque `credit_entitlement_id` doit déjà être associé au produit. Ce champ remplace uniquement le `credits_amount` accordé lors de la finalisation de cette session ; les paramètres des droits de crédit au niveau du produit (expiration, report, etc.) continuent de s'appliquer.
</Info>
...
Nombre de crédits à accorder pour cette session de paiement, en remplacement du `credits_amount` défini au niveau du produit. Doit être supérieur à zéro.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
<Tip>
**Trouver les ID de vos produits** : vous trouverez les ID des produits dans votre tableau de bord Dodo Payments, sous Produits → Afficher les détails, ou en utilisant l'[API List Products](/api-reference/products/get-products).
</Tip>
...
Créez un nouvel enregistrement client pendant le paiement.
...
Numéro de téléphone du client au format international. Obligatoire pour certains moyens de paiement et la prévention de la fraude.
...
Adresse complète, avec numéro et nom de rue, ainsi que numéro d'appartement ou d'unité, le cas échéant.
...
Code pays ISO à deux lettres (ISO 3166-1 alpha-2). Ce champ est toujours obligatoire lorsqu'une billing_address est fournie.
...
Contrôlez les moyens de paiement disponibles pour les clients lors du paiement. Cela permet d'optimiser l'expérience pour certains marchés ou exigences commerciales.
...
**Devises prises en charge** : `USD`, `EUR`, `GBP`, `CAD`, `AUD`, `INR`, et bien d'autres
...
| `license_key` | `string` | Présent si le produit dispose de clés de licence activées. Séparé par des virgules si plusieurs clés sont présentes |
...
Affichez le tag « à la demande » pour les produits concernés.
</ParamField>

<ParamField body="force_language" type="string">
Forcez l'affichage de l'interface de paiement dans une langue spécifique. Par défaut, le paiement détecte automatiquement la langue du navigateur du client.
...
Permettez aux clients d'ajouter ou de supprimer des addons d'un produit par abonnement lors du paiement. S'applique uniquement aux produits par abonnement.
</ParamField>
...
Permettez aux clients de saisir leurs numéros d'identification fiscale.
</ParamField>

<ParamField body="always_create_new_customer" type="boolean" default="false">
Forcez la création d'un nouvel enregistrement client au lieu de mettre à jour les enregistrements existants.
</ParamField>
...
Permettez aux clients de modifier leur adresse lors du paiement.
</ParamField>

<ParamField body="allow_customer_editing_city" type="boolean">
Permettez aux clients de modifier leur ville lors du paiement.
</ParamField>

<ParamField body="allow_customer_editing_state" type="boolean">
Permettez aux clients de modifier leur État lors du paiement.
</ParamField>

<ParamField body="allow_customer_editing_country" type="boolean">
Permettez aux clients de modifier leur pays lors du paiement.
</ParamField>

<ParamField body="allow_customer_editing_zipcode" type="boolean">
Permettez aux clients de modifier leur code postal lors du paiement.
</ParamField>
...
<Tip>
Utilisez cette option si vous disposez d'une page de réussite personnalisée offrant une meilleure expérience utilisateur, ou pour les [applications mobiles](/developer-resources/mobile-integration) et les flux de paiement intégrés.
</Tip>
</ParamField>
</Expandable>
</ParamField>
...
Identifiant unique de ce champ. Utilisé comme clé dans les payloads webhook et les réponses de l'API.
...
Indique si ce champ doit être rempli avant la finalisation du paiement.
</ParamField>
...
Nombre de jours de la période d'essai avant le premier prélèvement. Doit être compris entre 0 et 10 000 jours.

Un montant d'essai payant ne peut pas être défini par session — configurez `trial_amount` sur le prix récurrent du produit. Consultez [Essais payants](/features/subscription#paid-trials).
</ParamField>

<ParamField body="on_demand" type="object">
Configuration d'abonnement à la demande pour la facturation basée sur l'utilisation ou au compteur.

<Expandable title="On-demand object properties">
<ParamField body="mandate_only" type="boolean" required>
Si la valeur est true, aucun prélèvement n'est effectué et seuls les détails du moyen de paiement sont autorisés pour une utilisation ultérieure.
</ParamField>

<ParamField body="product_price" type="integer">
Prix du produit pour le prélèvement initial du client. Si aucune valeur n'est indiquée, le prix enregistré du produit est utilisé.

**Format** : représenté dans la plus petite unité de la devise (par exemple, les centimes pour USD). Pour facturer 1,00 $, transmettez `100`.
</ParamField>

<ParamField body="product_currency" type="string">
Devise facultative du prix du produit. Si aucune valeur n'est indiquée, la devise du produit est utilisée par défaut.
</ParamField>

<ParamField body="product_description" type="string">
Remplacement facultatif de la description du produit pour la facturation et les lignes de commande. Si aucune valeur n'est indiquée, la description enregistrée du produit est utilisée.
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
Indique si les frais de devise adaptive currency doivent être inclus dans le prix du produit (true) ou ajoutés en supplément (false). Ignoré si la tarification adaptive currency n'est pas activée.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
Pour plus d'informations sur la configuration et les tests de BNPL, consultez la page <a href="/features/payment-methods/bnpl">Achetez maintenant, payez plus tard (BNPL)</a>.
...
<Tip>
Utilisez `redirect_immediately: true` lorsque vous disposez d'une page de réussite personnalisée offrant une meilleure expérience utilisateur que la page de réussite par défaut. Cette option est particulièrement utile pour les [applications mobiles](/developer-resources/mobile-integration) et les flux de paiement intégrés.
</Tip>
...
Avec les sessions de paiement, cela n'est plus nécessaire. Vous pouvez simplement transmettre les informations dont vous disposez et nous nous chargeons du reste. Par exemple :
