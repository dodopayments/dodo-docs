...
<Card title="Dépôt GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/supabase">
  Code source complet et guide d'installation
</Card>

<Tip>
  <strong>Découvrez le kit de démarrage complet pour supabase</strong><br />
  Un kit de démarrage d'abonnement minimal construit avec Next.js, Supabase et Dodo Payments. Ce modèle vous aide à configurer rapidement un SaaS basé sur un abonnement avec authentification, paiements et webhooks.
...
La fonction est déjà configurée dans `functions/webhook/index.ts` - déployez-la simplement :
...
1. Allez sur [DodoPayments Dashboard](https://app.dodopayments.com) → Développeur → Webhooks
2. Créez un nouveau point de terminaison webhook
3. Configurez votre URL webhook comme point de terminaison
4. Activez ces événements de souscription :
   - `subscription.active`
...
✅ **Idempotence** - Prévention des traitements en double avec des ID de webhook  
✅ **Journalisation des événements** - Complète traçabilité dans la table `webhook_events`  
✅ **Gestion des erreurs** - Enregistrées et réessayables

> **Remarque :** Cette implémentation démontre la gestion de trois événements de souscription principaux (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) avec des champs minimaux. Vous pouvez facilement l'étendre pour prendre en charge des types d'événements et des champs supplémentaires en fonction de vos besoins.
...
2. **Vérifie l'idempotence** - Recherche l'ID de webhook pour éviter le traitement en double
3. **Journalise l'événement** - Stocke les données brutes du webhook dans la table `webhook_events`
4. **Traite les mises à jour** - Crée ou met à jour des clients et des souscriptions via le client Supabase
5. **Gère les erreurs** - Enregistre les pannes et marque l'événement pour un réessai
...
<Note>
Le drapeau `--no-verify-jwt` est requis car les webhooks n'incluent pas de jetons JWT. La sécurité est assurée par la vérification de signature du webhook.
</Note>
...
| Problème | Solution |
|-------|----------|
| Vérification échouée | Vérifiez que la clé du webhook est correcte dans le tableau de bord DodoPayments |
| Erreur de permission de base de données | Assurez-vous d'utiliser la clé de rôle de service |
