...
1. Allez sur le [tableau de bord DodoPayments](https://app.dodopayments.com) → Développeur → Webhooks
...
<Note>
Le `--no-verify-jwt` drapeau est requis car les webhooks n'incluent pas de jetons JWT. La sécurité est assurée par la vérification de la signature du webhook.
</Note>
...
2. Ajoutez un endpoint avec votre URL Supabase
...
| Problème | Solution |
|-------|----------|
| Vérification échouée | Vérifiez que la clé de webhook est correcte depuis le tableau de bord DodoPayments |
| Erreur de permission de base de données | Assurez-vous d'utiliser la clé de rôle de service |
| Erreur de vérification JWT | Déployez avec le `--no-verify-jwt` drapeau |
| Fonction introuvable | Vérifiez que la référence du projet est correcte et que la fonction est déployée |

## Ressources

- [Docs des Fonctions Edge Supabase](https://supabase.com/docs/guides/functions)
