---
title: Fonctions Netlify
description: Déployez les webhooks DodoPayments sur les Fonctions Netlify
icon: diamond
---

{/* LOCKED_PATTERN_168b18b8d8742ed481ee0ff6b3f8bcb1 */}
  Code source complet et guide de configuration
</Card>

## Configuration rapide

### 1. Prérequis
- [Compte Netlify](https://app.netlify.com/signup)
...
4. Copiez et collez le contenu de [`schema.sql`](#database-schema)
5. Exécutez la requête
6. Récupérez votre chaîne de connexion depuis Neon → Détails de connexion
...
> **Note :** Nous définirons `DODO_PAYMENTS_WEBHOOK_KEY` après le déploiement une fois que vous aurez votre URL de webhook.
...
1. Accédez au [DodoPayments Dashboard](https://app.dodopayments.com) → Développeur → Webhooks
...
✅ **Gestion des erreurs** - Journalisées et reprogrammables

> **Remarque :** Cette implémentation illustre la gestion de trois événements d'abonnement principaux (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) avec un nombre minimal de champs. Vous pouvez facilement l’étendre pour prendre en charge d’autres types d’événements et champs selon vos besoins.
...
1. **Vérifie la signature** - S'assure que la requête provient de DodoPayments en utilisant HMAC-SHA256
2. **Vérifie les doublons** - Utilise l'ID du webhook pour éviter de traiter deux fois le même événement
3. **Journalise l'événement** - Stocke le webhook brut dans la table `webhook_events` pour l'audit
4. **Traite l'événement** - Crée ou met à jour les clients et abonnements dans Neon
5. **Gère les erreurs** - Enregistre les échecs et marque l'événement comme non traité pour une nouvelle tentative
...
**Consultez les journaux dans le tableau de bord Netlify :**
1. Sélectionnez votre site → onglet **Functions**
2. Cliquez sur la fonction `webhook`
3. Consultez les journaux en temps réel et l'historique des invocations

**Configurer dans le tableau de bord DodoPayments :**
1. Allez dans Développeurs → Webhooks
2. Ajoutez un point de terminaison avec votre URL de Fonctions Netlify
...
| Problème | Solution |
|----------|----------|
| Échec de la vérification | Vérifiez que la clé webhook est correcte depuis le tableau de bord DodoPayments |
| Erreur de connexion à la base de données | Vérifiez la chaîne de connexion Neon et utilisez une connexion poolée |
| Fonction introuvable (404) | Exécutez `netlify init` et `npm run deploy` à nouveau |
| Variables d'environnement indisponibles | Définissez les variables dans l'interface CLI ou le tableau de bord, puis redéployez |

## Ressources

- [Documentation des Fonctions Netlify](https://docs.netlify.com/functions/overview/)
...
- [Dépôt GitHub](https://github.com/dodopayments/cloud-functions/tree/main/netlify)
