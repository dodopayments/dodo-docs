...
<Card title="Dépôt GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/netlify">
...
1. **Vérifie la signature** - S'assure que la demande provient de DodoPayments en utilisant HMAC-SHA256
2. **Vérifie les doublons** - Utilise l'ID du webhook pour éviter le traitement du même événement deux fois
3. **Journalise l'événement** - Stocke le webhook brut dans la table `webhook_events` pour la traçabilité
4. **Traite l'événement** - Crée ou met à jour des clients et des abonnements dans Neon
5. **Gère les erreurs** - Journalise les échecs et marque l'événement comme non traité pour réessai
...
**Voir les journaux dans le tableau de bord Netlify :**
1. Sélectionnez votre site → onglet **Fonctions**
...
| Problème | Solution |
|-------|----------|
| Vérification échouée | Vérifiez que la clé webhook est correcte sur le tableau de bord DodoPayments |
| Erreur de connexion à la base de données | Vérifiez la chaîne de connexion Neon et utilisez une connexion regroupée |
| Fonction non trouvée (404) | Exécutez à nouveau `netlify init` et `npm run deploy` |
| Variables d'environnement non disponibles | Définissez les variables dans l'CLI ou le tableau de bord, puis redéployez |
