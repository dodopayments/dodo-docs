...
1. Accédez au [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
...
✅ **Idempotence** - Empêche les traitements en double grâce aux identifiants de webhook  
...
3. **Vérifie les doublons** - Utilise l'ID du webhook pour empêcher le traitement du même événement deux fois
4. **Consigne l'événement** - Stocke le webhook brut dans la table `webhook_events` pour la piste d'audit
5. **Traite l'événement** - Crée ou met à jour les clients et abonnements dans Neon
6. **Gère les erreurs** - Enregistre les échecs et marque l'événement comme non traité pour réessayer
