...
description: Déployez les webhooks DodoPayments sur Vercel
icon: triangle
---

<Card title="Dépôt GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/vercel">
...
6. Obtenez votre chaîne de connexion depuis Neon → Détails de la connexion
...
✅ **Idempotence** - Empêche le traitement en double avec les identifiants de webhook  
✅ **Journalisation des événements** - Trace d'audit complète dans la table `webhook_events`  
✅ **Gestion des erreurs** - Journalisée et réessayable

> **Remarque :** Cette implémentation démontre la gestion de trois événements d'abonnement principaux (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) avec des champs minimaux. Vous pouvez facilement l'étendre pour supporter d'autres types d'événements et champs selon vos besoins.
...
1. **Désactive le parsing du corps** - Pour accéder au corps brut pour la vérification de la signature
2. **Vérifie la signature** - S'assure que la requête provient de DodoPayments en utilisant HMAC-SHA256
3. **Vérifie les doublons** - Utilise l'identifiant du webhook pour éviter de traiter le même événement deux fois
4. **Journalise l'événement** - Stocke le webhook brut dans la table `webhook_events` pour la trace d'audit
5. **Traite l'événement** - Crée ou met à jour les clients et abonnements dans Neon
6. **Gère les erreurs** - Journalise les échecs et marque l'événement comme non traité pour un réessai
...
3. Cliquez sur **Fonctions** → **Journaux**
...
- [Docs des Fonctions Vercel](https://vercel.com/docs/functions)
- [Vercel CLI](https://vercel.com/docs/cli)
- [Docs Neon](https://neon.com/docs/)
- [Guide des événements de webhook](/developer-resources/webhooks/intents/webhook-events-guide)
- [Dépôt GitHub](https://github.com/dodopayments/cloud-functions/tree/main/vercel)
