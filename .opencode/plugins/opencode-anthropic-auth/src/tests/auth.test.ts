...
describe("exchange", () => {
  test("accepts code#state format", async () => {
    ...
    const result = await exchange("mycode#mystate", "myverifier", CODE_CALLBACK_URL, "mystate");
  ...
  test("returns failed on invalid callback input", async () => {
    ...
    const result = await exchange("not-a-callback", "verifier", CODE_CALLBACK_URL);
  ...
  test("returns failed on state mismatch", async () => {
    ...
    const result = await exchange("code#wrong", "verifier", CODE_CALLBACK_URL, "expected");
    expect(result.type).toBe("failed");
    expect(fetchSpy).not.toHaveBeenCalled();
  });
});
