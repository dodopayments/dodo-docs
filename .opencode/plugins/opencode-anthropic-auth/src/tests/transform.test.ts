...
import { CLAUDE_CODE_IDENTITY, OPENCODE_IDENTITY_PREFIX, REQUIRED_BETAS } from "../constants";
import {
...
describe("rewriteUrl", () => {
  ...
  test("adds beta=true to /v1/messages URL object", () => {
    const { input } = rewriteUrl(new URL("https://api.anthropic.com/v1/messages"));
  ...
  test("preserves existing query params when ANTHROPIC_BASE_URL is set", () => {
    process.env.ANTHROPIC_BASE_URL = "http://localhost:8080";
    const { input } = rewriteUrl("https://api.anthropic.com/v1/messages?foo=bar");
    const url = new URL(input.toString());
    expect(url.origin).toBe("http://localhost:8080");
