...
function fireConcurrentFetches(result: { fetch: typeof fetch }) {
  return Promise.all(Array.from({ length: 5 }, () => result.fetch(MESSAGES_URL, EMPTY_POST)));
}
...
describe("auth.loader", () => {
  ...
  test("returns empty object for non-oauth auth", async () => {
    const plugin = await getPlugin();
    const result = await plugin.auth.loader(() => Promise.resolve({ type: "api" }), { models: {} });
  ...
  test("fetch wrapper strips tool prefix from streaming response", async () => {
    ...
    const responseStream = new ReadableStream({
      start(controller) {
        controller.enqueue(
          encoder.encode('data: {"content_block":{"type":"tool_use","name":"mcp_bash"}}\n\n')
        );
        controller.close();
      },
    });
