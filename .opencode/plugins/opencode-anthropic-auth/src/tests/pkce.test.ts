...
describe("generatePKCE", () => {
  ...
  test("challenge is SHA-256 of verifier (independent verification)", async () => {
    const result = await generatePKCE();
    const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(result.verifier));
  ...
  test("deterministic output for known random bytes", async () => {
    ...
    crypto.getRandomValues = (<T extends ArrayBufferView>(array: T): T => {
      new Uint8Array(array.buffer, array.byteOffset, array.byteLength).set(knownBytes);
      return array;
    ...
    try {
      ...
      const expectedVerifier = btoa(bin).replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
