...
The bank account form adapts to what you are submitting, so you only see fields that apply to you:

- **Field labels follow your business type.** An individual or sole proprietor sees wording written for a personal account, while a registered company sees entity wording — so **account holder name** and **beneficiary name** no longer read as duplicates. Where a field needs more explanation than fits on one line, a tooltip sits next to the label.
- **Labels also follow your currency and transfer method.** The same field is named the way your bank names it, for example the China domestic bank code is labeled **CNAPS**.
- **Choose Other for an unlisted bank.** If your bank is not in the dropdown, select **Other** and type the name in yourself.

<Tip>
The **Payouts** page is available in test mode as well, so you can reach **Linked Bank Accounts** and add or edit an account from either mode. Balances, the growth chart, and payout history stay empty in test mode, because payouts only ever run against live sales.
</Tip>
