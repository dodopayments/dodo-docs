...
Every status transition also emits a webhook, so you can reconcile payouts in your own systems without polling. See the [Webhook Event Guide](/developer-resources/webhooks/intents/webhook-events-guide) for the payload and delivery caveats. To receive email or in-app notifications for the same transitions instead, see [Communication Preferences](/features/communication-preferences).
