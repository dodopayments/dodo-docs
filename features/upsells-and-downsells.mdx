...
async function upgradeSubscription(
  ...
  // change-plan returns 200 with a body whose fields are all null for a standard change — resolving without an error means it was accepted.
...
def upgrade_subscription(
    ...
    # change_plan returns 200 with a body whose fields are all null for a standard change — returning without an error means it was accepted.
...
async function downgradeSubscription(
  ...
  // Execute the downgrade. change-plan returns 200 with a body whose fields are all null for a standard change.
  await client.subscriptions.changePlan(subscriptionId, {
    product_id: newProductId,
    quantity: 1,
    proration_billing_mode: 'difference_immediately'
...
def downgrade_subscription(subscription_id: str, new_product_id: str):
    ...
    # Execute the downgrade. change_plan returns 200 with a body whose fields are all null for a standard change.
    client.subscriptions.change_plan(
        subscription_id=subscription_id,
        product_id=new_product_id,
        quantity=1,
