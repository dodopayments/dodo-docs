...
Each cart line is resolved on its own, so you can localize one product and leave others on base pricing in the same checkout. If any line matches a localized rule, the whole transaction is forced fees-inclusive (see [Fees are inclusive when a rule matches](#fees-are-inclusive-when-a-rule-matches)).
