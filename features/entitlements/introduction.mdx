...
| `subscription.past_due` | No-op. Grants stay delivered for the whole [grace period](/features/subscription#grace-period). |
| `subscription.on_hold` | Revoke all delivered and pending grants. `revocation_reason: subscription_on_hold`. |
| `subscription.paused` | Revoke all delivered and pending grants. `revocation_reason: SubscriptionPaused`. |
...
- **Do not revoke access on `subscription.past_due`.** That event opens a [grace period](/features/subscription#grace-period), and the customer is meant to keep access until the window ends. Wait for `subscription.on_hold` or `subscription.cancelled`.
