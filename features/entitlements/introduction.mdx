...
| `subscription.paused` | Revoke all delivered and pending grants. `revocation_reason: SubscriptionPaused` — this value may change to match the lower-cased style of the others, so branch on the `subscription.paused` event rather than matching the string. |
