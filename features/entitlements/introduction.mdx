...
<Warning>
**Do not hardcode the pause `revocation_reason`.** Unlike every other reason in this table, the value emitted for `subscription.paused` is currently `SubscriptionPaused` rather than the lower-case, underscored `subscription_paused` you would expect. Treat it as unstable and branch on the `subscription.paused` webhook instead. See [Pausing and Resuming Subscriptions](/features/subscription#pausing-and-resuming-subscriptions).
</Warning>
