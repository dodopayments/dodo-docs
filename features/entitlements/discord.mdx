...
2. Dodo Payments creates a grant in `Pending` status. Its `oauth_url` is `null` until the customer begins the accept flow, at which point it is populated with a Discord authorization URL.
...
- `entitlement_grant.created` fires with `status: "Pending"`. `oauth_url` is `null` at this point — it is populated once the customer starts the accept flow from their delivery email or customer portal.
