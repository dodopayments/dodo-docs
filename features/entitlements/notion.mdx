...
2. Dodo Payments creates a grant in `Pending` status. Its `oauth_url` is `null` until the customer begins the accept flow, at which point it is populated with a Notion authorization URL.
