...
|-------|---------|-------------|
| **Payment Succeeded** | Enabled | Automatically sends a confirmation email to customers when their payment is successfully processed. Includes receipt details and order information. |
| **Subscription Plan Changed** | Enabled | Sends an email to customers when their subscription plan changes, whether through an upgrade, downgrade, or modification. Confirms the new plan details and pricing. |
| **License Key Created** | Enabled | Sends an email to customers when a license key is generated for their purchase. Includes the license key and activation instructions. |
| **Refund Successful** | Enabled | Notifies customers when a refund has been successfully processed and funds are on their way back to them. |
| **Subscription Trial Ending** | Enabled | Reminds customers that their free trial is ending soon (sent approximately 2 days before the trial period expires and the first billing charge occurs). |
| **Subscription Cancelled Immediately** | Enabled | Notifies customers when their subscription has been cancelled with immediate effect. |
| **Subscription Set to Cancel at Next Billing Date** | Enabled | Informs customers when their subscription has been scheduled to cancel at the end of the current billing period, rather than immediately. |
| **Payment Failed** | Disabled | Notifies customers when a payment attempt on their account has failed. Disabled by default — enable if you want Dodo Payments to send these notifications directly to customers rather than handling them yourself. |
| **Subscription Renewal Failed** | Disabled | Notifies customers specifically when a subscription renewal payment fails. Fires for failed payments that are tied to a subscription renewal; complements the general Payment Failed email. Disabled by default. |
| **Upcoming Renewal Reminder** | Disabled | Sends customers a reminder approximately 2 days before their subscription renews. Disabled by default — useful if you want proactive renewal notice sent to customers. |

<Tip>
Customer emails that are **enabled by default** are sent automatically unless you turn them off. Emails that are **disabled by default** will only be sent if you explicitly enable them — these are typically events where you may prefer to handle customer communication through your own systems.
</Tip>
