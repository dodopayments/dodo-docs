...
3. **Apply PPP**: If the product has [Purchasing Power Parity](/features/purchasing-power-parity) enabled, the country percentage reduces the base price
4. **Convert**: The price (or PPP-adjusted price) converts to the customer's currency at live FX rates
5. **Round**: If [Charm Pricing](/features/charm-pricing) is enabled, the converted amount rounds to a clean ending like 49.99
6. **Display**: Final payable amount is shown transparently before payment
...
The checkout page supports 22 languages:
...
When [Purchasing Power Parity](/features/purchasing-power-parity) is enabled on a product, the discount applies to the PPP-adjusted price, not the base price.
...
<Info>
This section covers the **server-side API** theme configuration using `customization.theme_config`. If you're using the **Checkout SDK** (overlay or inline checkout), see the theme customization section in [Overlay Checkout](/developer-resources/overlay-checkout#theme-customization) which uses camelCase properties (e.g., `bgPrimary` instead of `bg_primary`).
</Info>
