...
Beyond the core fields, the discounts API accepts the following optional controls on create and update:

- **`starts_at`** (optional): Schedule when a code becomes active. Before this time the code exists but cannot be applied — attempts return `DISCOUNT_NOT_YET_ACTIVE`. When set, `starts_at` must be earlier than `expires_at`.
- **`customer_eligibility`** (optional): Restrict who can redeem the code. Supported values are `any` (default — no restriction), `first_time` (only customers with no prior successful payment), `existing` (only returning customers), and `specific` (only customers explicitly attached to the discount). Ineligible customers receive `DISCOUNT_CUSTOMER_NOT_ELIGIBLE`.
- **`per_customer_usage_limit`** (optional): Cap how many times a single customer can redeem the code, independent of the global `usage_limit`. Must be less than or equal to `usage_limit`. Exceeding it returns `DISCOUNT_PER_CUSTOMER_USAGE_LIMIT_EXCEEDED`.

<Info>
`customer_eligibility` and `per_customer_usage_limit` are evaluated across all apply paths — direct API, hosted checkout confirm, and payment links.
</Info>
