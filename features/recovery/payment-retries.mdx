...
<Note>
If a subscription is cancelled while retries are still scheduled, the retry chain ends immediately and no further attempts are made. Other non-active statuses (`paused`, `expired`, `pending`, `failed`) keep retrying, because their open renewal invoices represent debt for periods the customer has already consumed.
</Note>
