...
You do not have to wait for the next scheduled attempt. Open a failed renewal payment under **Sales → Transactions** and use **Retry Payment Manually** to charge the subscription's saved payment method straight away.

A manual send is independent of the automatic chain. It does not consume one of the 8 scheduled attempts, it does not re-anchor the back-off cadence, and turning **Enable Payment Retries** off does not remove the button — you can still collect a failed invoice by hand.

| Guardrail | Limit |
|-----------|-------|
| Sends per invoice | 3 |
| Cooldown before the second send | 1 hour |
| Cooldown before the third send | 3 hours |

The action is offered only where it can succeed: the payment must be a failed **renewal** on an `on_hold` subscription that still has a saved payment method, with no other charge already in flight and no hard decline on record. When one of those conditions fails, the button is either hidden or disabled with the reason shown beneath it.

<Note>
A manual retry creates a **new** payment record rather than changing the failed one, which keeps its original error. Manual sends carry a `retry_attempt` above `0`, so they appear in retry analytics and are billed the same retry fee as an automatic attempt. The same guardrails apply in test mode as in live mode.
</Note>
...
|-------|---------------------|
| Renewal payment fails | `active` → `on_hold`, or `active` → `past_due` when a [grace period](/features/subscription#grace-period) is configured |
| Retry attempt fails | stays `on_hold` or `past_due` (next retry scheduled if window allows) |
| Retry attempt succeeds | `on_hold` or `past_due` → `active`, next billing date advanced |
...
<Note>
If a subscription is cancelled while retries are still scheduled, the retry chain ends immediately and no further attempts are made. Other non-active statuses (`on_hold`, `past_due`, `expired`, `pending`, `failed`) keep retrying, because their open renewal invoices represent debt for periods the customer has already consumed.
</Note>
...
| `subscription.past_due` | A renewal fails and a [grace period](/features/subscription#grace-period) opens instead |
