...
If you set a [grace period](/features/subscription#grace-period), a failed renewal moves the subscription to `past_due` instead of on hold. The on-hold email sequence runs during that window, so the customer is asked to pay while they still have access. One failed payment starts one sequence. The end of the window starts no second sequence, and any email still queued is sent after the window ends. The dunning attempt records a `past_due` trigger state, so filter on that as well as on hold when you read dunning analytics.
</Step>
