...
| **Upgrade Plan** | Move to a higher-tier product in the same collection | Configurable (default: disabled) |
| **Downgrade Plan** | Move to a lower-tier product in the same collection | Configurable (default: disabled) |
...
| **Allow Subscription Cancellation** | Customers can cancel their own subscriptions from the Customer Portal. Master switch for both cancellation options | Enabled |
| **Allow Cancellation at Next Billing Date** | Customers can schedule a cancellation for the end of the current billing period. Turn it off to offer immediate cancellation only | Enabled |
| **Allow Subscription Pause** | Customers can pause and resume their own subscriptions from the Customer Portal | Disabled |
| **Bill Usage at Pause** | A paused usage-based subscription is invoiced for its owed usage at the moment it is paused, instead of carrying the usage to the next regular invoice | Enabled |
...
<Info>
Turning off "Allow Subscription Cancellation" hides self-service cancellation from customers and rejects cancellation requests made through the Customer Portal API. "Allow Cancellation at Next Billing Date" narrows that further, rejecting only the scheduled path while immediate cancellation keeps working. Your own merchant-side cancellations are unaffected by either setting. See [Cancelling a Subscription](/features/customer-portal#cancelling-a-subscription).
</Info>

<Info>
"Allow Subscription Pause" governs the Customer Portal only — you can pause and resume from the dashboard or the API whichever way it is set. "Bill Usage at Pause" applies to usage-based subscriptions only, and is recorded per billing cycle, so changing it mid-cycle does not change how the cycle in progress settles. See [Pausing and Resuming Subscriptions](/features/subscription#pausing-and-resuming-subscriptions).
</Info>
