...
description: Accept WeChat Pay for Chinese customers. Learn about USD and CNY billing, QR code payments, and configuration via Stripe US.
icon: "comment-dollar"
...
sequenceDiagram
    ...
    participant Dodo
    participant StripeUS
    participant WeChat

    Customer->>Checkout: Select WeChat Pay
    Checkout->>Dodo: Create payment
    Dodo->>StripeUS: Initiate WeChat Pay
    StripeUS->>Customer: Display QR code
    Customer->>WeChat: Scan QR in WeChat app
    WeChat->>StripeUS: Payment confirmed
    StripeUS->>Dodo: Success callback
...
<Info>
WeChat Pay is processed via Stripe US. Customers pay in USD or CNY, and you receive settlement in USD.
</Info>
...
Follow the simulated WeChat Pay flow in the test environment. A test QR code will be generated.
</Step>
</Steps>
