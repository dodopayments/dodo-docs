...
In your dashboard, go to **Settings → Payment Methods** and on the **Apple Pay** row, click **Manage domains**. In the Wallet domains panel, click **Register domain** and enter the domain where you embed checkout (e.g., `shop.example.com`).
</Step>

<Step title="Download the domain association file">
Download the association file from the Wallet domains screen.
</Step>
...
Click **Verify domain**. Dodo Payments checks the hosted file and submits your domain to Apple. When its status shows **Active**, Apple Pay is live for that domain. Use the **Enabled** toggle to turn it on or off per domain.
</Step>
