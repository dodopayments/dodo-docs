...
sidebarTitle: BNPL
description: Offer Klarna, Afterpay, Billie, and Sunbit installment payments to increase average order value and conversion. Learn about eligibility, minimum amounts, and testing.
icon: "calendar-days"
keywords: ["Dodo Payments", "buy now pay later", "BNPL", "Klarna", "Afterpay", "Sunbit", "payment methods", "buy now pay later (bnpl)"]
---
...
| **Subscriptions** | Yes (Stripe US & UK connectors) |
...
**Payment Options:**
- **Installment Financing** — Split purchases into manageable monthly payments
...
<Warning>
Always include `credit` and `debit` as fallbacks. Not all customers are eligible for BNPL, and transactions below the provider's minimum won't qualify.
</Warning>

## Transaction Amount Limits

Each BNPL provider has its own minimum (and sometimes maximum) transaction amount:
...
Transactions outside these thresholds:
- BNPL options won't appear at checkout
- No error is thrown — options simply don't show
- Card payments remain available

This is expected behavior. Don't include a BNPL method in `allowed_payment_method_types` if your product price is unlikely to fall within that provider's supported range.
...
Ensure the checkout session has `billing_address.country` set to `US` and `billing_currency` set to `USD`.
</Step>

<Step title="Use a qualifying amount">
Set the transaction amount between **$60.00 and $19,999.00**. Sunbit will not appear outside this range.
</Step>

<Step title="Select Sunbit">
Choose Sunbit at checkout and complete the financing application flow in the Sunbit modal.
</Step>

<Step title="Test failure">
To simulate a declined application, close the Sunbit modal before completing the flow. The payment transitions to `requires_payment_method`.
</Step>
</Steps>

<Note>
Sunbit is only available for US customers paying in USD with a transaction amount between $60.00 and $19,999.00.
</Note>
...
**Check:**
1. Transaction amount within the provider's supported range? (Klarna/Afterpay: min $50.01; Sunbit: $60.00–$19,999.00)
...
**Solution:** Most commonly, the transaction is below the minimum or above the maximum. Verify the amount falls within the provider's supported range.
</Accordion>
