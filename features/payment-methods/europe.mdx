...
sidebarTitle: Europe
description: Accept iDEAL, Bancontact, EPS, Multibanco, Przelewy24, BLIK, and Satispay to increase conversion rates across European markets. Learn about regional availability, bank coverage, and configuration.
icon: "euro-sign"
...
SEPA Direct Debit enables recurring payments across the Eurozone, letting customers authorize you to debit their bank account on a schedule. It is widely used for subscriptions and offers lower processing costs than card payments.

### How It Works

Customers authorize a one-time mandate that grants you permission to debit their account. Once authorized, you can charge them on the agreed schedule without requiring re-authentication for each payment.

### Availability

- **Billing currency:** EUR
- **Transaction type:** Subscriptions and recurring payments
- **Coverage:** All Eurozone countries
...
const session = await client.checkoutSessions.create({
  product_cart: [{ product_id: 'prod_123', quantity: 1 }],
  allowed_payment_method_types: ['sepa_debit', 'credit', 'debit'],
  billing_currency: 'EUR',
  billing_address: {
    country: 'DE'
  },
...
<Warning>
SEPA Direct Debit is not instant. A payment can take up to **5 working days** to confirm, so do not treat authorization as settlement — fulfill only once the payment reaches a succeeded state.
</Warning>
