...
<Note>
SEPA Direct Debit sets up a reusable mandate, so it can drive recurring billing. Because settlement is not immediate, a renewal is not confirmed on the renewal date itself — treat a cycle as paid only once its payment reaches a succeeded state. The other Eurozone methods on this page set up no reusable mandate and stay one-time only.
</Note>
...
const session = await client.checkoutSessions.create({
  ...
  allowed_payment_method_types: [
    ...
    'sepa',            // Eurozone (one-time payments and subscriptions)
