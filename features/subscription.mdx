...
Use `PATCH /subscriptions/{id}` to cancel at the next billing date, extend the subscription period, update billing details, or modify metadata. To change the quantity, use the [Change Plan API](/api-reference/subscriptions/change-plan) instead — `PATCH` does not accept `quantity`.
