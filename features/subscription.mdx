...
| `past_due` | A renewal payment failed while a [grace period](#grace-period) is configured; the customer keeps access until the window ends | **Yes** | Collect the open invoice before the window closes — see [Grace Period](#grace-period) |
...
stateDiagram-v2
    ...
    active --> past_due: Renewal fails, grace period enabled
    past_due --> active: Payment recovers inside the window
    past_due --> on_hold: Window ends (default)
    past_due --> cancelled: Window ends, expiry action is cancel
...
A grace period is an optional window between a failed renewal and the loss of access. Instead of moving straight to `on_hold`, the subscription moves to `past_due` and the customer keeps everything they bought until the window closes.

<Info>
Grace periods are configured per business and are **disabled by default**. While the feature is off, a failed renewal behaves exactly as it always has and no subscription ever reports `past_due`.
</Info>

The window is configurable between **1 and 30 days** (default **3**), and you choose what happens when it ends:

| Expiry action | Result at the deadline |
|---|---|
| `on_hold` (default) | The subscription moves to `on_hold`. Retries and dunning continue from there. |
| `cancel_subscription` | The subscription is cancelled and the open invoice is voided. |

**Inside the window**, a `past_due` subscription answers every access and billing question the same way `active` does — entitlements stay granted, [license keys](/features/license-keys) stay enabled, and metered usage keeps accruing. It differs in four ways: it carries a deadline, it does not renew, it cannot be paused, and it emits `subscription.past_due` on entry.

[Payment Retries](/features/recovery/payment-retries) keep firing on their normal schedule throughout the window, and the [dunning](/features/recovery/subscription-dunning) sequence runs alongside them. If a payment succeeds before the deadline, the subscription returns to `active` and the window closes.

<Note>
The window length and the expiry action are fixed at the moment the subscription enters `past_due`. Changing your grace period settings later does not alter the outcome for a subscription already inside a window.
</Note>

<Warning>
`past_due` is a new value of the `status` field, and `subscription.past_due` is a new webhook event type. If your integration decodes `status` into a fixed set of values, or switches on it without a default branch, add `past_due` before the feature is enabled for your account. Code that treats `status` as a plain string needs no change.
</Warning>
...
- A [grace period](#grace-period) ends without the renewal being paid, and the configured expiry action is `on_hold`
...
| Renewal fails → grace period | `subscription.past_due` |
