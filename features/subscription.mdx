...
- **The billing clock freezes.** `next_billing_date` moves forward by the exact length of the pause, as does `expires_at` when the subscription has an expiry, so the customer keeps the time they already paid for.
...
Turning it off stops new customer pauses, but it does not trap a customer who is already paused — they can still resume a pause they started themselves. A pause you started is generally not resumable from the portal while the setting is off, so merchant-initiated pauses stay under your control.
