...
| `past_due` | A renewal payment failed and the subscription entered its grace period; the `subscription.past_due` webhook carries `past_due_ends_at`, when it moves to `on_hold` or `cancelled` | **Yes** | Retries and [Dunning](/features/recovery/subscription-dunning) run during the grace period; the customer can also update the payment method directly |
...
```typescript
// Note: POST /subscriptions is deprecated — new integrations should create on-demand
// subscriptions with checkout sessions (`subscription_data.on_demand`).
