...
| `paused` | The subscription was explicitly paused and will not renew until it is resumed | **Yes** | Unpause via [Update Subscription](/api-reference/subscriptions/patch-subscriptions) with `pause: false` |
...
stateDiagram-v2
    ...
    active --> paused: Paused via API
    paused --> active: Unpaused via API
