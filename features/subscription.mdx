...
- **Access is revoked immediately.** Entitlements, license keys, and digital product delivery tied to the subscription stop working until it is resumed.
...
A [usage-based](/features/usage-based-billing/introduction) subscription can have unbilled usage at the moment it is paused. **Bill Usage at Pause** under **Settings → Subscriptions** decides what happens to it:
...
<Warning>
The settlement invoice is collected like any other invoice, so it can fail. If it goes unpaid past the [dunning](/features/recovery/subscription-dunning) grace period, the subscription moves to `on_hold` while remaining flagged as paused. Paying that invoice returns it to `paused` rather than `active`, so you still need to resume it explicitly.
</Warning>
...
One rule is worth knowing before you turn it off: a customer can always resume a pause they started themselves, but a customer can never resume a pause **you** started while the setting is off. That keeps merchant-initiated pauses under your control.
...
Pause and resume are a single `pause` field on the update subscription endpoint. There is no separate pause endpoint.
...
<Warning>
`pause` is exclusive of every other field — sending it alongside anything else is rejected with `422`. Setting `status` to `paused` does **not** pause a subscription; use the `pause` field instead.
</Warning>
...
- **Scheduled plan changes are delayed, not dropped.** A [plan change scheduled](#changing-plans-with-add-ons) for the next billing date sits untouched while the subscription is paused, then applies at the shifted billing date once it resumes. Its `scheduled_change.effective_at` is a snapshot from when it was scheduled and is not adjusted for the pause, so it can show a date in the past — read it as "was scheduled for", not as a guaranteed date.
...
| `on_hold` | A renewal payment (or plan-change charge) failed; renewals have stopped | **Yes** | Update the payment method to reactivate — automatically via [Payment Retries](/features/recovery/payment-retries) and [Dunning](/features/recovery/subscription-dunning), or manually via the [Update Payment Method API](/api-reference/subscriptions/update-payment-method) |
| `paused` | The subscription was deliberately paused by you or the customer; billing is frozen and access is revoked | **Yes** | Resume it with `pause: false`, or from the Customer Portal when self-service pause is enabled — see [Pausing and Resuming Subscriptions](#pausing-and-resuming-subscriptions) |
...
<Info>
`on_hold` and `paused` are also distinct. `on_hold` is involuntary — a payment failed. `paused` is deliberate — you or the customer chose to freeze the subscription, and no payment is owed.
</Info>
...
Pause and resume run through the same `PATCH /subscriptions/{subscription_id}` endpoint using the `pause` field: `pause: true` pauses an active subscription and `pause: false` resumes it. The field cannot be combined with any other field in the same request.

<Card title="Pausing and Resuming Subscriptions" icon="circle-pause" href="#pausing-and-resuming-subscriptions">
Review pause behavior, billing effects, and the related business settings.
</Card>
</Accordion>
