...
Starting a pause and resuming one are not gated the same way. **Allow Subscription Pause** decides who may *start* a pause from the portal. Resuming follows a looser rule, so that turning the setting off never traps a customer in a pause they chose themselves.

| The current pause was started by | Resuming from the portal while the setting is off |
|---|---|
| The customer | Allowed. A customer can always undo their own pause. |
| You, from the dashboard or the API | Rejected with `403` and the error code `SUBSCRIPTION_CUSTOMER_PAUSE_DISABLED`. Merchant-initiated pauses stay under your control. |

Turning the setting on lets customers resume either kind. Your own resume from the dashboard or the API is never restricted.

<Warning>
**Which row applies is not something you can read from the API.** There is no field on the subscription recording who paused it, so your own code cannot tell a customer-initiated pause from one you started — this is behavior of the hosted Customer Portal, not a rule to branch on. Two consequences worth planning around:

- If you call the portal API directly, handle the `403` when it arrives rather than trying to predict it.
- A subscription paused before this feature shipped, or imported without pause history, counts as merchant-initiated.

Where the initiator cannot be determined at all, the resume is allowed rather than blocked — the behavior errs toward letting a customer out of a pause.
</Warning>
...
Pause and resume run through the same `PATCH /subscriptions/{subscription_id}` endpoint using the `pause` field: `pause: true` pauses an active subscription and `pause: false` resumes it. The field cannot be combined with any other field in the same request. For the full behavior, billing effects, and related business settings, see [Pausing and Resuming Subscriptions](#pausing-and-resuming-subscriptions).

<Card title="API Reference" icon="code" href="/api-reference/subscriptions/patch-subscriptions">
View the update subscription API, including the `pause` field.
</Card>
</Accordion>
