...
To list brands including the archived ones, set `include_archived` on the list endpoint. Archived brands are excluded by default, and every brand now carries an `archived_at` field that is `null` while the brand is active.
