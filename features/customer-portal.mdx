...
Customers open the pause dialog from the subscription details page. The dialog states the current plan, warns that access ends immediately, and explains how the billing dates shift before the customer confirms.
...
<Info>
Self-service pause is controlled by the **Allow Subscription Pause** setting under **Settings → Subscriptions**, and it is **disabled by default**. While it is off, customers see no pause option and portal pause requests are rejected. Pauses you perform yourself from the dashboard are never affected.
</Info>
