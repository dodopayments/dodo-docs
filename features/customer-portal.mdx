...
<Info>
Self-service pause is controlled by the **Allow Subscription Pause** setting under **Settings → Subscriptions**, and it is **disabled by default**. While it is off, customers cannot start a new pause and portal pause requests are rejected — but a customer who is already paused keeps the ability to resume. Pauses you perform yourself from the dashboard are never affected.
</Info>

<Warning>
Turning the setting off does not strand a customer who already paused. A customer can always resume a pause they started themselves. What they cannot do is resume a pause **you** started while the setting is off — those stay under your control.
</Warning>
