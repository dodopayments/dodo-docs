...
### Reactivating Subscriptions in a Grace Period

If you set a [grace period](/features/subscription#grace-period), a failed renewal moves the subscription to `past_due` instead of on hold. The customer keeps access until the deadline.

The recovery flow is the same: the customer updates the payment method, and the subscription returns to `active`. The difference is what happens if they do nothing. At the deadline the subscription moves to `on_hold` or to `cancelled`, as you configured, and access ends there.
