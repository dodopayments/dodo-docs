...
<Info>
Self-service pause is controlled by the **Allow Subscription Pause** setting under **Settings → Subscriptions**, and it is **disabled by default**. While it is off, customers cannot start a new pause and portal pause requests are rejected. Pauses you perform yourself from the dashboard are never affected.

Turning it off does not strand a customer who is already paused — resuming is gated separately. See [Who Can Resume a Pause](/features/subscription#who-can-resume-a-pause).
</Info>
