...
1. Customer clicks **Change Plan** on the subscription details page.
2. Available upgrade/downgrade options are shown based on the product collection.
3. Customer selects the new plan. A **Confirm Plan Change** panel shows the current vs. updated plan, a line-item breakdown (credit for the old plan, charge for the new one), tax, and the amount due now. Customers can also add a discount code here.
4. Customer confirms with **Pay $&#123;amount&#125;**.
5. Proration is calculated and the charge is processed — either against the saved payment method, or via a hosted checkout page if the business collects plan-change payments by payment link (see below).
6. Subscription is updated to the new plan. Whether that happens right away or only after the charge clears depends on your [payment failure handling](/developer-resources/subscription-upgrade-downgrade#handling-payment-failures); a plan change paid by payment link always waits for the payment.
...
By default, an immediate plan change charges the customer's saved payment method directly. Turning on **Collect Plan Change Payments by Payment Link** changes that: instead of charging the saved card, the customer is sent to a Dodo Checkout page to pay. This applies to both upgrades and downgrades that result in a charge.
...
<Info>
The subscription stays on its **current plan** until that checkout payment succeeds — a failed or abandoned payment leaves the subscription untouched, so the customer can retry on the same checkout session. Scheduled downgrades (`effective_at: next_billing_date`) are unaffected and continue to apply on the next billing date without going through checkout.
</Info>

This only applies to **immediate** plan changes. If the customer already has a pending plan-change payment link, a further change-plan request is rejected until that link is paid or expires.
...
<Card title="Plan Change via Payment Link (API)" icon="code" href="/developer-resources/subscription-upgrade-downgrade#collecting-payment-via-a-checkout-link">
  How to request a payment link with `collect_via_payment_link` and handle the response.
</Card>
...
- **Allow Subscription Cancellation**: Enable or disable the ability for customers to cancel their own subscriptions. This is the master switch — turning it off blocks both cancellation options below.
- **Allow Cancellation at Next Billing Date**: Enable or disable the scheduled cancellation option specifically. Turn it off to offer immediate cancellation only, while leaving **Allow Subscription Cancellation** on. Enabled by default.
- **Allow Subscription Pause**: Enable or disable the ability for customers to pause and resume their own subscriptions
- **Collect Plan Change Payments by Payment Link**: Send customers to a checkout page to pay for an immediate plan change, instead of charging their saved payment method. Off by default.
...
<Info>
The two options can be controlled separately. **Allow Cancellation at Next Billing Date**, also under **Settings → Subscriptions** and enabled by default, gates only the scheduled option. Turn it off to leave **Cancel now** as the customer's only self-service route. Revoking an existing scheduled cancellation stays available either way, so a customer who wants to keep paying is never stranded.
</Info>
