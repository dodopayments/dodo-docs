...
| `subscription.past_due` | No-op. Keys stay enabled for the whole [grace period](/features/subscription#grace-period), and are disabled only if the window ends unpaid. |
...
- **One-time products** issue one key per unit bought — a cart line with `product_cart.quantity` of 3 issues 3 keys, not 1.
- **Manual API grants** issue exactly one key.

<Note>
Each key records the product it was bought against. When one License Key entitlement is shared by several products, the key is filed against the product actually in the cart.
</Note>
