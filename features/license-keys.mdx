...
- **`manual`**: The purchase creates a `pending` grant with no key, and you supply each key value yourself. Use this when keys come from your own system, a third-party vendor, or a finite pool.

See [Self-Served License Keys](/features/self-served-license-keys) for the full manual flow.
...
| Origin | Generated by Dodo Payments on payment | [Supplied by you](/features/self-served-license-keys) against a pending grant | Created/migrated via `POST /license_keys` |
