...
<Info>
Customers can also do this themselves. The [Customer Portal](/features/customer-portal) lists each license key's activations, newest first, alongside the activation count and limit, and lets the customer deactivate one to free up a slot. Keys the API reports as disabled or expired are marked as such, so the customer sees the state before an activation fails.
</Info>
