...
| `subscription.past_due` | No-op. Downloads stay available for the whole [grace period](/features/subscription#grace-period). |
| `subscription.on_hold` / `cancelled` / `expired` | Revoke the grant. New presigned URLs are no longer issued. |
| `subscription.paused` | Revoke the grant. New presigned URLs are no longer issued until the subscription is resumed. |
