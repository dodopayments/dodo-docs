...
icon: "layer-group"
keywords: ["Dodo Payments", "payment methods", "payment platform", "SaaS billing"]
---
...
| **ACH Direct Debit**     | US                   | USD                  | Yes | $0.50 |
| **SEPA Direct Debit**    | Eurozone             | EUR                  | Yes | €0.50 |
...
<Info>
**ACH and SEPA Direct Debit support subscriptions.** Both set up a reusable mandate, so they can drive recurring billing as well as one-time payments. Settlement is asynchronous — a debit is not confirmed instantly the way a card is — so treat a renewal as paid only once it reaches a succeeded state rather than at authorization. The other Eurozone methods (iDEAL, Bancontact, EPS, Multibanco) set up no reusable mandate and stay one-time only.
</Info>
...
Bank account debits for US customers paying in USD, available on one-time payments and subscriptions.
</Card>
...
iDEAL (Netherlands), Bancontact (Belgium), EPS (Austria), Multibanco (Portugal), and BLIK (Poland).
</Card>
...
<CardGroup cols={3}>
  ...
  <Card title="Preview Checkout" icon="eye" href="/api-reference/checkout-sessions/preview">
    Preview pricing and taxes before creating a session.
  </Card>
  <Card title="Update Payment Method" icon="credit-card" href="/api-reference/subscriptions/update-payment-method">
    Restrict methods on a subscription's payment-method update page.
  </Card>
</CardGroup>
