---
title: "Affiliates"
description: "Launch and manage your affiliate program while processing transactions through Dodo Payments."
icon: "users-line"
keywords: ["Dodo Payments", "affiliates", "payment platform", "SaaS billing"]
---
...
- **[Rekomi](https://rekomi.com)**: An affiliate tracking and management platform with a one-paste Dodo Payments connection that also handles affiliate payouts and tax forms for you.
...
<Steps>
  ...
  <Step title="Pass Referral Data to Dodo Payments">
    ...
    // integrations. `metadata` is passed the same way.
    async function main() {
    ...
    // Note: POST /subscriptions is deprecated — prefer checkout sessions for new
    // integrations. `metadata` is passed the same way.
    async function main() {
...
You can also use [Rekomi](https://rekomi.com) to run your affiliate program end to end. Rekomi tracks clicks and conversions, calculates commissions on the pre-tax sale amount, and pays your affiliates for you in 150+ countries, tax forms included.

<Info>
For detailed setup instructions, see our [Rekomi integration guide](/integrations/rekomi). The connection is a single pasted API key: Rekomi creates the webhook endpoint in your Dodo Payments account itself, so there is no manual webhook setup.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="Rekomi Support" icon="headset">
    - Visit the [Rekomi documentation](https://rekomi.com/docs) for setup and troubleshooting guides
    - Email us at **support@rekomi.com** or use the live chat in the [Rekomi dashboard](https://app.rekomi.com)
  </Card>
