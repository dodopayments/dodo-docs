...
| **Add-on** | Credits issued with an add-on attached to a subscription |
...
| **Overage Reset** | Tracked overage cleared at the billing-cycle reset |
...
| `credit.overage_reset` | Tracked overage was reset at the end of the billing cycle |
...
All ledger events (every `credit.*` event except `credit.balance_low`) include the full `CreditLedgerEntry` payload with balance before/after, overage before/after, source reference, and the `metadata` of the grant's source subscription or payment (empty for grants created directly via the API). The `credit.balance_low` event includes the threshold configuration and current balance.
