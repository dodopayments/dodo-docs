...
<Card title="Add-ons" icon="puzzle" href="/features/addons">
  Let customers purchase additional credits through subscription add-ons.
</Card>
...
Credits are granted when a customer purchases a product (subscription, one-time, or add-on) with attached credit entitlements. For subscriptions, credits are re-issued each billing cycle.
</Step>
...
If credits run out mid-cycle, you can allow overage (continued usage beyond the balance), auto top-up, or block further usage — all configurable per credit.
</Step>
</Steps>
...
| **Add-on** | Credits included with an add-on purchase |
...
- **7 days**, **30 days** (default), **60 days**, **90 days**, or **Never**
</ParamField>
</Step>
...
<ParamField path="Auto Top-Up" type="boolean">
Automatically add credits when the balance reaches a configured threshold.
</ParamField>
...
- **Price Per Unit** — Cost per additional credit when overage is enabled (with currency)
- **Charge Overage at Billing** — Add overage charges to the next invoice
- **Credits Reduce Overage** — New credit grants first offset any existing overage
- **Preserve Overage at Reset** — Carry forward overage into the next billing cycle
</ParamField>

<ParamField path="Block or Disable Subscription" type="boolean">
Stop service entirely when credits are exhausted or the subscription expires.
</ParamField>
</Step>
...
<Tip>
Start with simple settings — no rollover, no overage — and add complexity as you learn how customers use credits. You can update settings at any time without affecting existing grants.
</Tip>
...
<img src="/images/CBB/Desktop - Attach Credit - UBB-5.jpg" alt="Meter configuration with credit selection and credits per unit" />
</Frame>
</Step>
...
## Add-ons with Credits

Add-ons can include credit entitlements, allowing customers to purchase additional credits alongside their base subscription.

<Steps>
<Step title="Create an Add-on with Credits">
When creating an add-on, configure the **base price** and **free threshold**. Click **Attach credit** to link a credit entitlement to the add-on.

<Frame caption="Add-on configuration with base price, threshold, and credit attachment.">
<img src="/images/CBB/Desktop - Attach Addons-6.jpg" alt="Add-on configuration panel with credit selection" />
</Frame>

<ParamField path="Base Price" type="number">
The price of the add-on. Toggle **Override base price** to use a different price than the default.
</ParamField>

<ParamField path="Free Threshold" type="number">
Usage units included free with the add-on before credits are consumed.
</ParamField>

<ParamField path="Number of credits issued" type="number">
The amount of credits granted when the customer purchases this add-on.
</ParamField>
</Step>

<Step title="Attach Add-on to a Subscription Product">
In your subscription product's Pricing section, click the **+** button in the **Select add-ons** area. A subscription can have up to **3 add-ons**.

<Frame caption="Add-on selected and attached to the subscription product.">
<img src="/images/CBB/Desktop - Attach Addons-1.jpg" alt="Subscription product showing attached add-on with credit" />
</Frame>
</Step>
</Steps>

<Tip>
Use credit-based add-ons to create natural upsell paths. For example, offer a "500 Extra API Calls" add-on alongside your base subscription plan.
</Tip>
...
| Setting | Description |
|---------|-------------|
| **Allow Overage** | Let customers continue using the service beyond their credit balance |
| **Overage Limit** | Maximum overage units allowed before blocking |
| **Price Per Unit** | Cost per additional credit consumed as overage (with currency) |
| **Charge at Billing** | Bill accumulated overage charges on the next invoice |
| **Preserve Overage at Reset** | Keep overage balance when credits reset (vs. clearing it) |
| **Credits Reduce Overage** | New credit grants automatically offset existing overage balance |

<Warning>
If both **Allow Overage** and **Block or Disable Subscription** are turned off, customers can continue using the service without any credit cost tracking. Choose one behavior to enforce credit limits.
</Warning>
...
| Setting | Description |
|---------|-------------|
| **Credit Expiry** | Duration after issuance before credits expire (7, 30, 60, 90 days, or never) |
...
- **Wallet Selector** — Switch between different credit entitlements
...
|-----------------|-------------|
| **Credit Added** | Credits granted (subscription, one-time, addon, or API) |
...
View the complete API documentation for credit entitlement operations.
</Card>
</Accordion>

<Accordion title="Attach Credits to Products">
Attach credit entitlements to products when creating or updating them via the API. Specify the credit amount, proration behavior, and trial credits.
</Accordion>

<Accordion title="Grant Credits via API">
Grant credits directly to a customer's balance without requiring a purchase. Use this for promotional credits, service compensation, or custom integrations.
</Accordion>
...
<Accordion title="Manual Ledger Entries">
Create manual credit or debit entries via the API for billing adjustments, refunds, or custom integrations. Each entry supports idempotency keys for safe retries.
</Accordion>
</AccordionGroup>
  ...
  product_cart: [
    {
      ...
      addons: [
        {
          addon_id: 'addon_extra_tokens',
          quantity: 1  // Extra token pack add-on with credits
        }
      ]
    }
  ],
...
- Overage: Enabled, charge at billing
...
- Rollover: Disabled
- Overage: Developer+ plans allow overage, Free plan blocks
...
<Info>
Credit-Based Billing works seamlessly with all other Dodo Payments features — subscriptions with trials, plan changes with proration, add-ons, and the customer portal. Start with a basic setup and expand as your pricing model evolves.
</Info>
