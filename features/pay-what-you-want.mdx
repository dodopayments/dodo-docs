...
icon: "dollar-sign"
keywords: ["Dodo Payments", "pay what you want pricing", "payment platform", "SaaS billing"]
---
...
#### Sharing A Link With A Fixed Amount

When you share a checkout link for a PWYW product from **Products**, open **Advanced Settings** and set a **Payment amount** to charge exactly that value instead of letting the buyer choose. The amount must be at or above the product's Minimum Price, and it is carried on the link as the `paymentAmount` query parameter in major currency units (e.g. `12.5` for $12.50). Leave the field empty to keep the buyer-chooses-amount behaviour.
