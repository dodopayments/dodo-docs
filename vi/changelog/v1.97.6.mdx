...
- **Đối tượng Subscription**: Trường `cancellation_feedback` mới (một trong các giá trị ở trên) và `cancellation_comment` (văn bản tự do tùy chọn), được điền khi khách hàng hủy
- **Webhook `subscription.cancelled`**: Cả hai trường đều được bao gồm trong payload
- **API**: Truyền `cancellation_feedback` và `cancellation_comment` vào `PATCH /subscriptions/{subscription_id}` khi lên lịch hoặc thực thi việc hủy theo cách lập trình
...
Bạn cũng có thể ghi đè giá trị mặc định của merchant cho từng khoản thanh toán, các khoản phí subscription on-demand và các thay đổi plan bằng boolean `adaptive_currency_fees_inclusive`:
