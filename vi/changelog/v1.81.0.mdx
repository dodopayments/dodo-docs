...
Đã thêm trường `refund_status` vào phản hồi `GET /payments/{payment_id}` để nhanh chóng kiểm tra xem một khoản thanh toán đã được hoàn tiền hay chưa mà không cần truy vấn riêng endpoint refunds.
