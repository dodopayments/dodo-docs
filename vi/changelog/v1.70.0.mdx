...
1. **Thanh Toán Trực Tiếp - Trải Nghiệm Thanh Toán Hoàn Toàn Nhúng (BETA)**  
   ...
   - **Sự kiện phân tích theo thời gian thực**: Nhận `checkout.breakdown` sự kiện khi khách hàng nhập thông tin của họ
   - **Tóm tắt đơn hàng tùy chỉnh**: Xây dựng hiển thị giá của riêng bạn mà cập nhật theo thời gian thực
...
2. **Tạo Thanh Toán, Đăng Ký và Thanh Toán Sử Dụng Các Phương Thức Thanh Toán Hiện Có**  
    Đơn giản hóa quy trình thanh toán bằng cách sử dụng các phương thức thanh toán đã lưu từ khách hàng hiện có. Tham số mới `payment_method_id` cho phép bạn tạo thanh toán, đăng ký hoặc phiên thanh toán sử dụng phương thức thanh toán đã lưu trước đó của khách hàng.
...
5. **`redirect_immediately` Cờ - Bỏ Qua Trang Thành Công Thanh Toán**  
  ...
  ```
   ...
   <Note>
   Khi `redirect_immediately` được kích hoạt, khách hàng sẽ được chuyển hướng đến `return_url` của bạn ngay lập tức sau khi hoàn tất thanh toán, hoàn toàn bỏ qua trang thành công mặc định.
   </Note>
