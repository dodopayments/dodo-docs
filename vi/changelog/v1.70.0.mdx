...
1. **Thanh Toán Trực Tiếp - Trải Nghiệm Thanh Toán Hoàn Toàn Nhúng (BETA)**  
   ...
   - **Sự kiện phân tích theo thời gian thực**: Nhận các sự kiện `checkout.breakdown` khi khách hàng nhập thông tin của họ
   - **Tóm tắt đơn hàng tùy chỉnh**: Xây dựng hiển thị giá cả của riêng bạn mà cập nhật theo thời gian thực
   - **Tích hợp liền mạch**: Khung thanh toán xử lý việc thu thập thanh toán trong khi bạn kiểm soát bố cục
   - **Phương thức SDK**: Sử dụng `open()`, `close()`, và `isOpen()` để điều khiển thanh toán một cách lập trình
...
2. **Tạo Thanh Toán, Đăng Ký và Thanh Toán Sử Dụng Phương Thức Thanh Toán Hiện Có**  
    Tối ưu hóa quy trình thanh toán bằng cách sử dụng các phương thức thanh toán đã lưu từ khách hàng hiện tại. Tham số `payment_method_id` mới cho phép bạn tạo thanh toán, đăng ký hoặc phiên thanh toán bằng cách sử dụng phương thức thanh toán đã lưu trước đó của khách hàng.
   ...
   <Warning>
   Khi sử dụng `payment_method_id` trong các phiên thanh toán, `confirm` phải được thiết lập thành `true`, và một `customer_id` hiện có phải được cung cấp. Phương thức thanh toán sẽ được xác minh tính đủ điều kiện với loại tiền tệ của thanh toán.
   </Warning>
...
5. **`redirect_immediately` Cờ - Bỏ Qua Trang Thành Công Thanh Toán**  
   Kiểm soát quy trình thanh toán với cờ `redirect_immediately` mới. Khi được kích hoạt, khách hàng sẽ được chuyển hướng ngay lập tức sau khi hoàn tất thanh toán, bỏ qua trang thành công mặc định để có trải nghiệm nhanh hơn, mượt mà hơn.
  ...
  ```
   ...
   <Note>
   Khi `redirect_immediately` được kích hoạt, khách hàng sẽ được chuyển hướng đến `return_url` ngay lập tức sau khi hoàn tất thanh toán, hoàn toàn bỏ qua trang thành công mặc định.
   </Note>
