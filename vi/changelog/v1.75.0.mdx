...
### 5. **`invoice_url` Trường trong Phản hồi Thanh toán và Phản hồi Liên kết Thanh toán**  
   ...
   Phản hồi Thanh toán và Phản hồi Liên kết Thanh toán hiện bao gồm một trường `invoice_url`, cung cấp quyền truy cập trực tiếp vào hóa đơn liên quan đến mỗi khoản thanh toán.
...
### 7. **Cải thiện khả năng Lọc cho Khách hàng, Giảm giá, và Khóa bản quyền**  
   ...
   **Bộ lọc API Giảm giá**
   ...
   | `active` | Lọc theo trạng thái hoạt động (true = không hết hạn, false = đã hết hạn) |
   | `product_id` | Lọc theo hạn chế sản phẩm (chỉ những giảm giá áp dụng cho sản phẩm này) |
   ...
   **Bộ lọc API Khóa bản quyền**
   ...
   | `status` | Lọc theo trạng thái khóa cấp phép (`active`, `expired`, `disabled`) |
   | `product_id` | Lọc theo ID sản phẩm |
   | `created_at_gte` | Lọc các khóa cấp phép được tạo vào hoặc sau dấu thời gian này |
   | `created_at_lte` | Lọc các khóa cấp phép được tạo vào hoặc trước dấu thời gian này |
