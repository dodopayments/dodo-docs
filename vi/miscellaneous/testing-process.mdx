...
Các phương thức thanh toán châu Âu (iDEAL, Bancontact, EPS, Multibanco) có thể được kiểm thử trong sandbox mode.
...
Đặt quốc gia của billing address khớp với phương thức thanh toán:
- `NL` cho iDEAL
- `BE` cho Bancontact
- `AT` cho EPS
- `PT` cho Multibanco
...
Các phương thức châu Âu yêu cầu currency EUR.
</Step>

<Step title="Complete test flow">
Làm theo quy trình xác thực ngân hàng mô phỏng trong môi trường kiểm thử.
</Step>
</Steps>
...
Đừng chỉ kiểm thử các khoản thanh toán thành công. Hãy kiểm thử cả các trường hợp bị từ chối, bị hủy và các trường hợp đặc biệt như không đủ tiền.
</Accordion>

<Accordion title="Verify webhook handling">
Đảm bảo các webhook endpoints của bạn xử lý chính xác tất cả event types, đặc biệt là `payment.succeeded`, `payment.failed` và các subscription events.

Sử dụng [Dodo Payments CLI](/developer-resources/sdks/cli#webhooks) để kiểm thử webhooks trên máy cục bộ:
- `dodo wh listen` chuyển tiếp các webhook kiểm thử trực tiếp đến máy chủ cục bộ của bạn
- `dodo wh trigger` gửi payload giả lập cho tất cả các loại sự kiện webhook được hỗ trợ
</Accordion>

<Accordion title="Test on real devices">
Đối với Apple Pay và Google Pay, hãy kiểm thử trên các thiết bị iOS và Android thực tế. Simulator không mô phỏng đầy đủ hành vi của wallet.
</Accordion>

<Accordion title="Test regional methods with correct addresses">
Các phương thức thanh toán theo khu vực (UPI, iDEAL, v.v.) yêu cầu billing address tương ứng. Billing address tại Hoa Kỳ sẽ không hiển thị iDEAL.
</Accordion>

<Accordion title="Verify minimum amounts">
Các phương thức BNPL yêu cầu mức tối thiểu $50.01. Hãy kiểm thử để đảm bảo chúng được hiển thị/ẩn chính xác dựa trên tổng giá trị giỏ hàng.
</Accordion>
</AccordionGroup>
...
Kiểm thử webhooks trên máy cục bộ bằng Dodo Payments CLI.
</Card>
</CardGroup>
