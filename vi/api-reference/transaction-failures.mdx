...
| `CARD_VELOCITY_EXCEEDED` | Có | Đã cố gắng quá nhiều giao dịch trong thời gian ngắn |
...
| `FRAUDULENT` | Có | Giao dịch đã bị đánh dấu là có thể gian lận |
| `GENERIC_DECLINE` | Không | Giao dịch đã bị từ chối do lý do không xác định |
...
| `INVALID_PIN` | Có | PIN được cung cấp không chính xác |
...
| `MANDATE_INVALID` | Có | Giấy ủy quyền không hợp lệ |
| `MANDATE_REQUIRED` | Có | Một giấy ủy quyền là cần thiết cho giao dịch này |
| `MANDATE_REQUIRED_SYSTEM` | Không | Hệ thống yêu cầu giấy ủy quyền cho loại giao dịch này |
...
| `REVOCATION_OF_AUTHORIZATION` | Có | Quyền truy cập cho giao dịch đã bị thu hồi |
...
| `TRANSACTION_NOT_APPROVED` | Có | Giao dịch đã không được chấp thuận |
| `TRY_AGAIN_LATER` | Không | Giao dịch nên được thử lại sau |
| `UNKNOWN_ERROR` | Không | Đã xảy ra lỗi không xác định |

<Note>
  **Lỗi người dùng** chỉ ra liệu việc từ chối thanh toán có thể được giải quyết bởi khách hàng hay không. Khi `true`, khách hàng có thể hành động để khắc phục vấn đề (ví dụ: nhập đúng thông tin thẻ). Khi `false`, việc từ chối do các vấn đề cấp hệ thống hoặc các hạn chế của ngân hàng mà khách hàng không thể khắc phục được.
</Note>
