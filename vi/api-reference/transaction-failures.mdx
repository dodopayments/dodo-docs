...
Đọc `error_code` từ `payment.failed` webhook hoặc đối tượng thanh toán, ánh xạ nó đến hành động đề xuất ở trên, và quyết định xem có nên thử lại hay không. Đối với gia hạn đăng ký, các từ chối mềm được thử lại tự động — xem [Subscription Payment Retries](/features/recovery/payment-retries).

Đối với lỗi ở cấp độ API và logic kinh doanh (như `PAYMENT_NOT_SUCCEEDED` hoặc `REFUND_WINDOW_EXPIRED`) không phải là từ chối thẻ, hãy xem tham khảo [Error Codes](/api-reference/error-codes).
