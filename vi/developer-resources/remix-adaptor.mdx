...
Cố định số tiền được tính, theo đơn vị tiền tệ chính (ví dụ: <code>12.5</code> tương đương với $12.50). Chỉ áp dụng cho các sản phẩm Pay What You Want và sẽ bị bỏ qua nếu thấp hơn giá tối thiểu của sản phẩm.
</ParamField>
...
### Định dạng phản hồi
Dynamic checkout trả về phản hồi JSON chứa URL checkout:
...
Checkout Sessions cung cấp trải nghiệm checkout được lưu trữ, an toàn hơn và xử lý toàn bộ quy trình thanh toán cho cả giao dịch mua một lần và subscription, đồng thời cho phép kiểm soát đầy đủ việc tùy chỉnh.
...
### Định dạng phản hồi
Checkout Sessions trả về phản hồi JSON chứa URL checkout:
...
## Trình xử lý route Customer Portal

Trình xử lý route Customer Portal cho phép bạn tích hợp liền mạch customer portal của Dodo Payments vào ứng dụng Remix.
...
## Trình xử lý route Webhook

- **Method:** Chỉ hỗ trợ các request POST. Các method khác trả về 405.
- **Xác minh chữ ký:** Xác minh chữ ký webhook bằng <code>webhookKey</code>. Trả về 401 nếu xác minh không thành công.
...
- **Định tuyến event:** Gọi event handler phù hợp dựa trên type của payload.

### Các event handler của Webhook được hỗ trợ
<CodeGroup>
