...
**Tùy chọn có sẵn**: `credit`, `debit`, `upi_collect`, `apple_pay`, `google_pay`, `amazon_pay`, `klarna`, `affirm`, `afterpay_clearpay`, `cashapp`, `multibanco`, `bancontact_card`, `eps`, `ideal`, `przelewy24`, `paypal`
...
ID duy nhất cho trường này. Sử dụng như khóa trong các tải trọng webhook và phản hồi API.

**Ví dụ**: `"company_name"`, `"referral_source"`, `"team_size"`
</ParamField>

<ParamField body="label" type="string" required>
Nhãn hiển thị cho thấy khách hàng trên mẫu thanh toán.

**Ví dụ**: `"Company Name"`, `"How did you hear about us?"`, `"Team Size"`
</ParamField>

<ParamField body="field_type" type="string" required>
Loại trường xác định quy tắc xác nhận đầu vào.

**Các loại có sẵn**:
- `text` - Đầu vào văn bản dòng đơn
- `number` - Đầu vào số
- `email` - Địa chỉ email có xác nhận
- `url` - URL có xác nhận
- `date` - Chọn ngày
...
Liệu trường này có cần phải được điền trước khi thanh toán có thể hoàn thành.
</ParamField>

<ParamField body="placeholder" type="string">
Văn bản giữ chỗ hiển thị trong trường nhập liệu.
</ParamField>
...
Số ngày cho thời gian thử nghiệm trước khi tính phí đầu tiên. Phải nằm trong khoảng từ 0 đến 10.000 ngày.
</ParamField>

<ParamField body="on_demand" type="object">
Cấu hình đăng ký theo yêu cầu cho thanh toán dựa trên sử dụng hoặc đo đạc.

<Expandable title="On-demand object properties">
<ParamField body="mandate_only" type="boolean" required>
Nếu được đặt là đúng, không thực hiện bất kỳ khoản phí nào và chỉ xác thực chi tiết phương thức thanh toán để sử dụng trong tương lai.
</ParamField>

<ParamField body="product_price" type="integer">
Giá sản phẩm cho khoản phí ban đầu đối với khách hàng. Nếu không chỉ định, giá lưu trữ của sản phẩm sẽ được sử dụng.

**Định dạng**: Được biểu thị bằng đơn vị nhỏ nhất của tiền tệ (ví dụ, xu đối với USD). Ví dụ, để tính phí $1.00, hãy truyền `100`.
</ParamField>

<ParamField body="product_currency" type="string">
Tiền tệ tùy chọn của giá sản phẩm. Nếu không chỉ định, mặc định là tiền tệ của sản phẩm.
</ParamField>

<ParamField body="product_description" type="string">
Mô tả sản phẩm tùy chọn được ghi đè cho hóa đơn và mục hàng. Nếu không chỉ định, mô tả lưu trữ của sản phẩm sẽ được sử dụng.
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
Liệu phí tiền tệ đa dạng có nên được bao gồm trong giá sản phẩm (true) hay thêm bên ngoài (false). Bỏ qua nếu giá chỉ định không được bật.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
Để biết thêm thông tin chi tiết về cấu hình và thử nghiệm UPI, hãy xem trang <a href="/features/payment-methods/india">Phương Thức Thanh Toán Ấn Độ</a>.
...
Để biết thêm thông tin chi tiết về cấu hình và thử nghiệm BNPL, hãy xem trang <a href="/features/payment-methods/bnpl">Mua Trước Trả Sau (BNPL)</a>.
...
### Sự Khác Biệt Chính
Trước đây, khi tạo một liên kết thanh toán với Liên Kết Động, bạn cần cung cấp địa chỉ thanh toán đầy đủ của khách hàng.

Với Phiên Thanh Toán, điều này không còn cần thiết nữa. Bạn chỉ cần chuyển qua bất kỳ thông tin nào bạn có, và chúng tôi sẽ xử lý phần còn lại. Ví dụ:
