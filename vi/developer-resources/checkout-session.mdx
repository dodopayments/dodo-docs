...
Cho phép khách hàng thêm hoặc xóa addons trên sản phẩm đăng ký trong quá trình thanh toán. Chỉ áp dụng cho các sản phẩm đăng ký.
</ParamField>
...
Buộc tạo một hồ sơ khách hàng mới thay vì cập nhật các hồ sơ hiện có.
</ParamField>

<ParamField body="allow_customer_editing_email" type="boolean">
Cho phép khách hàng chỉnh sửa địa chỉ email trong quá trình thanh toán.
</ParamField>
...
Bỏ qua trang thành công thanh toán mặc định và chuyển hướng khách hàng ngay lập tức tới `return_url` sau khi hoàn tất thanh toán.

<Tip>
Sử dụng điều này khi bạn có một trang thành công tùy chỉnh mang lại trải nghiệm người dùng tốt hơn, hoặc cho các ứng dụng di động và các quy trình thanh toán tích hợp.
</Tip>
</ParamField>
</Expandable>
</ParamField>
...
Điểm định danh duy nhất cho trường này. Được sử dụng làm khóa trong payload của webhook và phản hồi API.
...
Trường này có phải được điền trước khi hoàn tất thanh toán không.
</ParamField>
...
Danh sách các tùy chọn cho loại trường `dropdown`. Yêu cầu khi `field_type` là `dropdown`, bị bỏ qua cho các loại khác.
...
Số ngày cho thời gian dùng thử trước khi tính phí lần đầu. Phải từ 0 đến 10000 ngày.
</ParamField>

<ParamField body="on_demand" type="object">
Cấu hình đăng ký theo nhu cầu cho thanh toán theo mức sử dụng hoặc đo lường.

<Expandable title="On-demand object properties">
<ParamField body="mandate_only" type="boolean" required>
Nếu đặt thành true, sẽ không thực hiện bất kỳ khoản phí nào và chỉ ủy quyền chi tiết phương thức thanh toán để sử dụng trong tương lai.
</ParamField>

<ParamField body="product_price" type="integer">
Giá sản phẩm cho lần tính phí ban đầu đối với khách hàng. Nếu không chỉ định, giá đã lưu trữ của sản phẩm sẽ được sử dụng.

**Format**: Được biểu diễn dưới đơn vị thấp nhất của tiền tệ (ví dụ: xu đối với USD). Ví dụ, để tính phí $1.00, hãy truyền `100`.
</ParamField>

<ParamField body="product_currency" type="string">
Đơn vị tiền tệ tùy chọn của giá sản phẩm. Nếu không chỉ định, mặc định là đơn vị tiền tệ của sản phẩm.
</ParamField>

<ParamField body="product_description" type="string">
Ghi đè mô tả sản phẩm tùy chọn cho hóa đơn và các mặt hàng. Nếu không chỉ định, mô tả đã lưu trữ của sản phẩm sẽ được sử dụng.
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
Có bao gồm phí tiền tệ thích ứng trong giá sản phẩm (true) hay thêm lên trên (false). Bỏ qua nếu định giá thích ứng không được bật.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
Để biết thông tin chi tiết về cấu hình và kiểm tra UPI, hãy xem trang <a href="/features/payment-methods/india">Phương thức Thanh toán Ấn Độ</a>.
...
Để biết thông tin chi tiết về cấu hình và kiểm tra BNPL, hãy xem trang <a href="/features/payment-methods/bnpl">Mua Trước Trả Sau (BNPL)</a>.
...
Với Phiên Thanh toán, điều này không còn cần thiết nữa. Bạn chỉ cần truyền thông tin bạn có, và chúng tôi sẽ xử lý phần còn lại. Ví dụ:
