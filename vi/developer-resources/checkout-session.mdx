...
<Info>
**Thời gian hiệu lực phiên**: Các phiên thanh toán có hiệu lực trong 24 giờ theo mặc định. Nếu bạn truyền `confirm=true` trong yêu cầu của bạn, phiên sẽ chỉ có hiệu lực trong 15 phút.
</Info>
...
**Định dạng**: Được biểu diễn bằng đơn vị thấp nhất của tiền tệ (ví dụ: xu cho USD). Ví dụ, để tính phí $1.00, hãy truyền `100`.
</ParamField>
</Expandable>
</ParamField>
...
**Định dạng**: Bao gồm mã quốc gia, ví dụ: `"+1234567890"` cho các số của Mỹ
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
<Warning>
**Quan trọng**: Luôn bao gồm `credit` và `debit` như các tùy chọn dự phòng để ngăn chặn thất bại thanh toán khi các phương thức thanh toán ưu tiên không khả dụng.
</Warning>
...
Giao diện cho thanh toán. Các tùy chọn: `light`, `dark`, hoặc `system`.
</ParamField>
...
<Note>
Tham số `billing_currency` chỉ có hiệu lực khi tiền tệ thích ứng được kích hoạt trong cài đặt tài khoản của bạn. Nếu tiền tệ thích ứng bị vô hiệu hóa, tham số này sẽ không có tác dụng.
</Note>
...
<Warning>
Khi sử dụng `payment_method_id`, `confirm` phải được đặt thành `true` và một `customer_id` hiện có phải được cung cấp. Phương thức thanh toán sẽ được xác thực để kiểm tra tính đủ điều kiện với tiền tệ của thanh toán.
</Warning>
...
<Note>
Khi `redirect_immediately` được kích hoạt, khách hàng sẽ được chuyển hướng đến `return_url` của bạn ngay sau khi hoàn tất thanh toán, bỏ qua hoàn toàn trang thành công mặc định.
</Note>
