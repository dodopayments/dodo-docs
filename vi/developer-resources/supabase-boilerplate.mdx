...
Tạo một khóa API và bí mật Webhook từ bảng điều khiển Dodo. Đặt môi trường thành `test_mode` khi phát triển.
</Step>
...
Giao diện giá đọc mảng `features` này và hiển thị một cách động.
</Step>
...
- Chức năng Edge của Supabase cho webhooks (`dodo-webhook`)
- Mô hình và di chuyển của Drizzle ORM
- Bảng điều khiển với hóa đơn, trạng thái đăng ký và các tính năng gói
...
- Kiểm tra cú pháp `DATABASE_URL` và truy cập mạng Supabase
- Đợi ~2–3 phút sau khi tạo dự án trước lần đẩy đầu tiên
</Accordion>
<Accordion title="Mismatch chuyển hướng OAuth">
- URI chuyển hướng phải là `https://[ref].supabase.co/auth/v1/callback`
- Đảm bảo điều này giống nhau trong Google Cloud và nhà cung cấp Supabase Auth
</Accordion>
</AccordionGroup>
