...
**Tên đồng hồ đo** → `AI Token Usage Meter`
...
<Warning>
Tên sự kiện `ai_chat_usage` phải khớp chính xác với những gì chúng tôi sẽ gửi từ mã ứng dụng sau này. Tên sự kiện phân biệt chữ hoa chữ thường!
</Warning>
</Step>
...
**Tính toán trên**: Loại → `totalTokens`

**Đơn vị đo lường**: Loại → `tokens`

<Info>
Chúng tôi sử dụng "Tổng" vì chúng tôi muốn cộng tất cả các token đã tiêu thụ qua nhiều tin nhắn trò chuyện. SDK sẽ tự động gửi `totalTokens` trong mỗi sự kiện.
</Info>
</Step>
...
<Note>
Lưu khóa API này - chúng tôi sẽ thêm nó vào tệp `.env` của chúng tôi sau.
</Note>
</Step>
...
<Note>
Giữ khóa này an toàn - chúng tôi cũng sẽ thêm nó vào tệp `.env` của chúng tôi.
</Note>
</Step>
</Steps>
...
Ngoài ra, đặt **Giá Cố Định** thành `0` để đảm bảo khách hàng chỉ bị tính phí theo mức sử dụng của họ, không có phí cơ bản.
...
**Giá mỗi đơn vị**: Nhập → `0.00001` (điều này là $0.01 cho 1,000 token hoặc $0.00001 cho mỗi token)

**Ngưỡng miễn phí**: Nhập → `10000` (khách hàng sẽ nhận 10,000 token miễn phí mỗi tháng)
...
Tạo tệp `.env` trong thư mục gốc của dự án của bạn:
...
Bây giờ chúng ta hãy thêm một giao diện trò chuyện đẹp với toàn bộ lịch sử cuộc trò chuyện! Tạo `public/index.html`:
...
3. Bạn sẽ thấy giao diện trò chuyện đẹp đẽ
...
<Info>
Đồng hồ đo tự động cộng tất cả các giá trị `totalTokens`!
</Info>
</Step>
...
4. Kiểm tra nhật ký máy chủ để tìm thông điệp lỗi
</Accordion>
...
- Khoảng trắng hoặc dấu nháy thừa trong tệp `.env`

**Giải pháp:**
- Đảm bảo khóa thử nghiệm bắt đầu bằng `test_`, khóa sống bắt đầu bằng `live_`
- Xóa bất kỳ dấu nháy nào xung quanh các khóa trong tệp `.env`
