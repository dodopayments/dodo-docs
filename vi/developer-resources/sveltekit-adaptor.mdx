...
Cố định số tiền được tính, theo đơn vị tiền tệ chính (ví dụ: <code>12.5</code> tương đương 12,50 USD). Chỉ áp dụng cho các sản phẩm Pay What You Want và sẽ bị bỏ qua nếu thấp hơn giá tối thiểu của sản phẩm.
</ParamField>
...
<Warning>
Dynamic Checkout proxy các endpoint `POST /payments` và `POST /subscriptions` đã deprecated. Tính năng này vẫn hoạt động cho các integration hiện có, nhưng các integration mới nên sử dụng **Checkout Sessions** bên dưới.
</Warning>

### Định dạng phản hồi
Dynamic checkout trả về một JSON response với checkout URL:
...
Checkout sessions cung cấp trải nghiệm checkout được host an toàn hơn, xử lý toàn bộ payment flow cho cả giao dịch mua một lần và subscription, đồng thời cho phép kiểm soát đầy đủ việc tùy chỉnh.
...
### Định dạng phản hồi
Checkout sessions trả về một JSON response với checkout URL:
...
Route Handler của Customer Portal cho phép bạn tích hợp liền mạch customer portal của Dodo Payments vào ứng dụng SvelteKit của mình.
...
Nếu được đặt thành <code>true</code>, hệ thống sẽ gửi email chứa portal link cho khách hàng.
</ParamField>
...
- **Signature Verification:** Xác minh webhook signature bằng <code>webhookKey</code>. Trả về 401 nếu xác minh thất bại.
- **Payload Validation:** Được validate bằng Zod. Trả về 400 nếu payload không hợp lệ.
- **Error Handling:**
  - 401: Signature không hợp lệ
  - 400: Payload không hợp lệ
  - 500: Lỗi nội bộ trong quá trình xác minh
- **Event Routing:** Gọi event handler tương ứng dựa trên loại payload.
