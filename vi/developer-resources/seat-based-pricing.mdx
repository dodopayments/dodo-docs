...
<Step title="Tạo file môi trường của bạn">
Tạo một file `.env` với khóa API Dodo Payments của bạn:
...
<Warning>
Không bao giờ cam kết khóa API của bạn vào phiên bản kiểm soát. Thêm `.env` vào file `.gitignore` của bạn.
</Warning>
</Step>
...
Tạo một file `public/index.html` để dễ dàng thử nghiệm:
...
<Step title="Phục vụ các file tĩnh">
Thêm điều này vào `src/server.ts` của bạn để phục vụ file HTML:
...
<Check>
**Các file tĩnh đã được cấu hình!** Truy cập `http://localhost:3000` để xem giao diện demo của bạn.
</Check>
</Step>
</Steps>
...
1. Đảm bảo bạn đã có file `.env` với khóa API đúng
2. Cập nhật ID sản phẩm và ID add-on trong mã của bạn với các giá trị thực tế từ bảng điều khiển Dodo Payments của bạn
...
1. Mở trình duyệt của bạn và đi tới `http://localhost:3000`
2. Bạn sẽ thấy giao diện demo dựa trên ghế giá
3. Thử với các số lượng ghế khác nhau (0, 3, 10, v.v.)
