...
<Note>
Để xem hướng dẫn đầy đủ từng bước (bao gồm các ví dụ request/response, chính sách thử lại an toàn và cách xử lý webhook), hãy xem <a href="/developer-resources/ondemand-subscriptions">Hướng dẫn gói đăng ký theo yêu cầu</a>.
</Note>
...
<Warning>
**Tiền tệ được cố định sau khoản phí thành công đầu tiên.** Luôn truyền rõ ràng `billing_currency` **và** `billing_address.country` khi tạo checkout. Nếu bỏ qua, tiền tệ sẽ được phát hiện từ IP của khách hàng (Adaptive Currency), và một khi gói đăng ký phát sinh khoản phí đầu tiên, tiền tệ sẽ được cố định trong suốt thời gian tồn tại của gói. Khách hàng không thể chuyển đổi tiền tệ khi đi du lịch sau đó.
</Warning>
