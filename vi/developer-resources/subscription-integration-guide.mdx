...
Sử dụng Checkout Sessions để bán các sản phẩm đăng ký với một quy trình thanh toán an toàn, được lưu trữ. Truyền sản phẩm đăng ký của bạn trong `product_cart` và chuyển hướng khách hàng đến `checkout_url` đã trả về.
...
2. **`subscription.updated`** - Đối tượng đăng ký đã được cập nhật (kích hoạt khi có bất kỳ thay đổi nào về trường).
...
5. **`subscription.renewed`** - Đăng ký đã được gia hạn cho kỳ thanh toán tiếp theo.
...
2. `payment.succeeded` - Xác nhận thanh toán ban đầu:
    - Đối với thanh toán ngay lập tức (0 ngày dùng thử): Mong đợi trong vòng 2-10 phút
    - Đối với ngày dùng thử: bất cứ khi nào kết thúc
3. `subscription.renewed` - Chỉ ra việc khấu trừ thanh toán và gia hạn cho chu kỳ tiếp theo. (Về cơ bản, bất cứ khi nào thanh toán được khấu trừ cho các sản phẩm đăng ký, bạn sẽ nhận được webhook `subscription.renewed` cùng với `payment.succeeded`)
...
Khi bạn nhận được một webhook {/* INLINE_CODE_PLACEHOLDER_5e9a0875003c752d72379e80bcc55356 */, hãy cập nhật trạng thái ứng dụng của bạn và thông báo cho khách hàng:
...
|----------|------|----------|-------------|
| `business_id` | chuỗi | Có | Định danh duy nhất cho doanh nghiệp |
| `timestamp` | chuỗi | Có | Dấu thời gian của khi sự kiện xảy ra (không nhất thiết phải giống như khi nó được gửi) |
| `type` | chuỗi | Có | Loại sự kiện. Xem [Loại Sự Kiện](#event-types) |
| `data` | đối tượng | Có | Tải trọng dữ liệu chính. Xem [Đối Tượng Dữ Liệu](#data-object) |
...
- Tính toán số tiền khấu trừ dựa trên thời gian còn lại trong chu kỳ thanh toán hiện tại
- Tính phí khách hàng chỉ cho sự khác biệt giữa kế hoạch cũ và mới
- Trong thời gian dùng thử, điều này sẽ ngay lập tức chuyển người dùng sang kế hoạch mới, tính phí khách hàng ngay lập tức

#### 2. `full_immediately`
- Tính phí khách hàng số tiền đăng ký đầy đủ cho kế hoạch mới
- Bỏ qua bất kỳ thời gian còn lại hoặc tín dụng từ kế hoạch trước
- Hữu ích khi bạn muốn đặt lại chu kỳ thanh toán hoặc tính phí toàn bộ số tiền bất kể khấu trừ
...
- Khi bạn gọi API này, Dodo Payments ngay lập tức khởi động một khoản phí dựa trên tùy chọn khấu trừ mà bạn đã chọn
- Nếu việc thay đổi kế hoạch là một sự hạ cấp và bạn sử dụng `prorated_immediately`, tín dụng sẽ được tự động tính toán và thêm vào số dư tín dụng của đăng ký. Những tín dụng này là cụ thể cho đăng ký đó và sẽ chỉ được sử dụng để bù đắp cho các khoản thanh toán định kỳ trong tương lai của cùng một đăng ký
- Tùy chọn `full_immediately` bỏ qua việc tính toán tín dụng và tính phí toàn bộ số tiền của kế hoạch mới

<Tip>
**Chọn tùy chọn khấu trừ của bạn một cách cẩn thận**: Sử dụng `prorated_immediately` cho việc thanh toán công bằng mà tính đến thời gian chưa sử dụng, hoặc `full_immediately` khi bạn muốn tính phí toàn bộ số tiền của kế hoạch mới bất kể chu kỳ thanh toán hiện tại.
</Tip>
