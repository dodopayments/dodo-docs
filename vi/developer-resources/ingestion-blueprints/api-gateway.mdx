...
<Card title="Giám Sát Hiệu Suất" icon="chart-line">
Theo dõi thời gian phản hồi và tỷ lệ lỗi cùng với dữ liệu thanh toán.
</Card>
<Card title="SaaS Đa Thuê" icon="users">
...
Tạo một đồng hồ trong [Bảng điều khiển Dodo Payments](https://app.dodopayments.com/):
- **Tên Sự kiện**: `api_call` (hoặc tên tùy chọn của bạn)
- **Loại Tập hợp**: `count` để theo dõi số lần gọi
- Cấu hình thêm các thuộc tính nếu theo dõi siêu dữ liệu như thời gian phản hồi, mã trạng thái, v.v.
</Step>

<Step title="Theo Dõi Các Cuộc Gọi API">
<CodeGroup>
...
Số lượng sự kiện tối đa trước khi tự động xóa. Mặc định: `100`.
</ParamField>

<ParamField path="flushInterval" type="number">
Thời gian tự động xóa trong mili giây. Mặc định: `5000` (5 giây).
</ParamField>

## Thực Hành Tốt Nhất

<Tip>
**Sử dụng Tập hợp cho Khối lượng Cao**: Đối với các ứng dụng xử lý hơn 10 yêu cầu mỗi giây, sử dụng `createBatch()` để giảm tải và cải thiện hiệu suất.
</Tip>

<Warning>
**Luôn Dọn Dẹp Các Tập hợp**: Gọi `batch.cleanup()` khi ứng dụng tắt để xóa các sự kiện đang chờ và ngăn ngừa mất dữ liệu.
</Warning>
