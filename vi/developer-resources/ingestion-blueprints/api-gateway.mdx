...
<Tip>
**Sử Dụng Batching cho Khối Lượng Cao**: Đối với các ứng dụng xử lý hơn 10 yêu cầu mỗi giây, hãy sử dụng `createBatch()` để giảm tải và cải thiện hiệu suất.
</Tip>

<Warning>
**Luôn Dọn Dẹp Các Batches**: Gọi `batch.cleanup()` khi ứng dụng tắt để xả các sự kiện đang chờ và ngăn ngừa mất dữ liệu.
</Warning>
