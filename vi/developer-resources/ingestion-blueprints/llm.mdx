...
2. **Đi tới** Sản phẩm → Công tơ
3. **Nhấp** vào "Tạo Công tơ"
4. **Cấu hình công tơ của bạn**:
   - **Tên Công tơ**: Chọn một tên mô tả (ví dụ: "LLM Token Usage")
   - **Tên Sự kiện**: Đặt một định danh sự kiện duy nhất (ví dụ: `llm.chat_completion`)
   - **Loại tổng hợp**: Chọn `sum` để cộng dồn số token
   - **Trên Tài sản**: Chọn những gì để theo dõi:
     - `inputTokens` - Theo dõi token đầu vào/nhắc
...
<Warning>
Luôn sử dụng `test_mode` trong quá trình phát triển để tránh ảnh hưởng đến chỉ số sản xuất.
</Warning>
</ParamField>
...
<Note>
Khi sử dụng các mô hình có khả năng lý do thông qua AI SDK (như Google Gemini 2.5 Flash với chế độ suy nghĩ), token lý do sẽ được tự động đưa vào số lượng `outputTokens` để tính toán hóa đơn chính xác.
</Note>
</Accordion>
...
<Note>
**Chế độ suy nghĩ của Gemini:** Khi sử dụng các mô hình Gemini với khả năng suy nghĩ/lý do (như Gemini 2.5 Pro), SDK tự động bao gồm `thoughtsTokenCount` (token lý do) trong `outputTokens` để phản ánh chính xác toàn bộ chi phí tính toán.
</Note>
</Accordion>
</AccordionGroup>
...
- `inputTokens` - Số lượng token đầu vào/nhắc đã sử dụng
...
- `provider` - Nhà cung cấp LLM (nếu có trong metadata bao bọc)
...
<Info>
Công tơ Dodo Payments của bạn sử dụng các trường `metadata` (đặc biệt là `inputTokens`, `outputTokens` hoặc `totalTokens`) để tính toán việc sử dụng và hóa đơn.
</Info>
