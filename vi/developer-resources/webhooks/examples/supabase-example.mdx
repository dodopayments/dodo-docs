...
Supabase tự động cung cấp `SUPABASE_URL` và `SUPABASE_SERVICE_ROLE_KEY` trong thời gian chạy.
...
✅ **Ghi lại sự kiện** - Lưu vết kiểm toán hoàn chỉnh trong bảng `webhook_events`  
...
Chức năng Edge dựa trên Deno:
1. **Xác minh chữ ký** - Sử dụng thư viện dodopayments để xác minh HMAC-SHA256
2. **Kiểm tra idempotency** - Tìm kiếm ID webhook để ngăn chặn xử lý trùng lặp
3. **Ghi lại sự kiện** - Lưu trữ dữ liệu webhook thô trong bảng `webhook_events`
4. **Xử lý cập nhật** - Tạo hoặc cập nhật khách hàng và đăng ký thông qua khách hàng Supabase
5. **Xử lý lỗi** - Ghi lại các lỗi và đánh dấu sự kiện để thử lại
...
<Note>
Cờ `--no-verify-jwt` là bắt buộc vì webhooks không bao gồm mã thông báo JWT. Bảo mật được cung cấp bởi xác minh chữ ký webhook.
</Note>
...
| Lỗi quyền cơ sở dữ liệu | Đảm bảo sử dụng Khóa vai trò dịch vụ |
| Lỗi xác minh JWT | Triển khai với cờ `--no-verify-jwt` |
| Không tìm thấy hàm | Xác minh tham chiếu dự án là chính xác và hàm đã được triển khai |
