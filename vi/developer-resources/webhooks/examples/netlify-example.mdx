...
<Card title="Kho lưu trữ GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/netlify">
...
1. Đăng ký trên [Neon](https://neon.com)
2. Tạo một dự án mới
3. Mở SQL Editor
...
> **Lưu ý:** Chúng tôi sẽ đặt `DODO_PAYMENTS_WEBHOOK_KEY` sau khi triển khai khi bạn có URL webhook của mình.
...
1. Đi đến [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
...
✅ **Ghi lại sự kiện** - Hồ sơ kiểm toán hoàn chỉnh trong bảng `webhook_events`  
...
2. **Kiểm tra trùng lặp** - Sử dụng ID webhook để ngăn chặn xử lý cùng một sự kiện hai lần
3. **Ghi lại sự kiện** - Lưu trữ webhook thô trong bảng `webhook_events` để kiểm toán
...
**Xem nhật ký trong Netlify Dashboard:**
1. Chọn trang của bạn → tab **Functions**
2. Nhấp vào chức năng `webhook`
3. Xem nhật ký thời gian thực và lịch sử gọi lại
...
| Vấn đề | Giải pháp |
|-------|----------|
| Xác minh không thành công | Kiểm tra xem khóa webhook từ bảng điều khiển DodoPayments có chính xác không |
| Lỗi kết nối cơ sở dữ liệu | Xác minh chuỗi kết nối Neon và sử dụng kết nối có tổ chức |
| Không tìm thấy chức năng (404) | Chạy lại `netlify init` và `npm run deploy` |
