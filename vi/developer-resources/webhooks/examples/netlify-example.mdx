...
description: Triển khai webhooks DodoPayments lên Netlify Functions
icon: diamond
---

{/* LOCKED_PATTERN_168b18b8d8742ed481ee0ff6b3f8bcb1 */}
  Hoàn thành mã nguồn và hướng dẫn cài đặt
</Card>
...
> **Ghi chú:** Chúng tôi sẽ thiết lập `DODO_PAYMENTS_WEBHOOK_KEY` sau khi triển khai khi bạn có URL webhook.

### 5. Khởi tạo & Triển khai
...
✅ **Tính không phụ thuộc** - Ngăn chặn xử lý trùng với ID webhook  
✅ **Ghi nhật ký sự kiện** - Dòng điều tra đầy đủ trong bảng `webhook_events`  
✅ **Xử lý lỗi** - Được ghi lại và có thể thử lại

> **Ghi chú:** Cách triển khai này trình bày xử lý ba sự kiện đăng ký cốt lõi (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) với các trường tối thiểu. Bạn có thể dễ dàng mở rộng để hỗ trợ thêm loại sự kiện và trường theo yêu cầu.
...
2. **Kiểm tra trùng lặp** - Sử dụng ID webhook để ngăn xử lý cùng sự kiện hai lần
3. **Ghi lại sự kiện** - Lưu webhook thô vào bảng `webhook_events` để phục vụ kiểm tra
4. **Xử lý sự kiện** - Tạo hoặc cập nhật khách hàng và đăng ký trong Neon
5. **Xử lý lỗi** - Ghi lại thất bại và đánh dấu sự kiện chưa xử lý để thử lại
...
**Xem nhật ký trong Bảng điều khiển Netlify:**
1. Chọn trang của bạn → tab **Functions**
2. Nhấp vào hàm `webhook`
3. Xem nhật ký thời gian thực và lịch sử gọi hàm

**Cấu hình trong Bảng điều khiển DodoPayments:**
1. Đi đến Nhà phát triển → Webhooks
2. Thêm điểm cuối với URL Netlify Functions của bạn
...
| Sự cố | Giải pháp |
|-------|----------|
| Xác thực thất bại | Kiểm tra khóa webhook có đúng từ bảng điều khiển DodoPayments |
| Lỗi kết nối cơ sở dữ liệu | Xác minh chuỗi kết nối Neon và sử dụng kết nối pooling |
| Không tìm thấy hàm (404) | Chạy lại `netlify init` và `npm run deploy` |
| Biến môi trường không khả dụng | Đặt biến trong CLI hoặc bảng điều khiển, sau đó triển khai lại |

## Tài nguyên

- [Tài liệu Netlify Functions](https://docs.netlify.com/functions/overview/)
...
- [Kho lưu trữ GitHub](https://github.com/dodopayments/cloud-functions/tree/main/netlify)
