...
<Card title="Kho lưu trữ GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/cloudflare">
...
✅ **Ghi nhật ký sự kiện** - Đầy đủ dấu vết kiểm toán trong bảng `webhook_events`  
✅ **Xử lý lỗi** - Được ghi lại và có thể thử lại

> **Lưu ý:** Triển khai này minh họa việc xử lý ba sự kiện đăng ký cốt lõi (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) với các trường tối thiểu. Bạn có thể dễ dàng mở rộng nó để hỗ trợ thêm các loại sự kiện và trường dựa trên yêu cầu của bạn.
...
Trình xử lý webhook:
1. **Xác minh chữ ký** - Đảm bảo yêu cầu đến từ DodoPayments sử dụng HMAC-SHA256
2. **Kiểm tra trùng lặp** - Sử dụng ID webhook để ngăn chặn việc xử lý sự kiện giống nhau hai lần
3. **Ghi lại sự kiện** - Lưu webhook thô trong bảng `webhook_events` cho dấu vết kiểm toán
4. **Xử lý sự kiện** - Tạo hoặc cập nhật khách hàng và đăng ký
5. **Xử lý lỗi** - Ghi lại các thất bại và đánh dấu sự kiện là chưa được xử lý để thử lại
...
| Vấn đề | Giải pháp |
|-------|----------|
| Xác minh không thành công | Kiểm tra khóa webhook có đúng không từ bảng điều khiển DodoPayments |
