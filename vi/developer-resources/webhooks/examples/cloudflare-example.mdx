...
description: Triển khai webhooks DodoPayments lên Cloudflare Workers
icon: cloud
---

{/* LOCKED_PATTERN_2b313346674bb180793c904f406c4c27 */}
  Hoàn chỉnh mã nguồn và hướng dẫn thiết lập
</Card>
...
- Tài khoản [Cơ sở dữ liệu Neon](https://neon.com)
...
5. Chạy truy vấn
6. Lấy chuỗi kết nối của bạn từ Neon → Connection Details
...
> **Note:** Chúng tôi sẽ thiết lập `DODO_PAYMENTS_WEBHOOK_KEY` sau khi triển khai khi bạn có URL webhook.
...
2. Tạo một endpoint webhook mới
3. Cấu hình URL webhook của bạn là endpoint
...
✅ **Tính bất biến** - Ngăn xử lý trùng lặp bằng ID webhook  
✅ **Ghi nhật ký sự kiện** - Dấu vết kiểm toán đầy đủ trong bảng `webhook_events`  
✅ **Xử lý lỗi** - Được ghi nhật ký và có thể thử lại

> **Note:** Cài đặt này minh họa cách xử lý ba sự kiện đăng ký cốt lõi (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) với các trường tối thiểu. Bạn có thể dễ dàng mở rộng để hỗ trợ các loại sự kiện và trường bổ sung dựa trên yêu cầu của mình.
...
Bộ xử lý webhook:
1. **Xác thực chữ ký** - Đảm bảo yêu cầu đến từ DodoPayments bằng HMAC-SHA256
2. **Kiểm tra trùng lặp** - Sử dụng ID webhook để ngăn xử lý sự kiện giống nhau hai lần
3. **Ghi nhật ký sự kiện** - Lưu webhook nguyên bản vào bảng `webhook_events` để phục vụ kiểm tra
4. **Xử lý sự kiện** - Tạo hoặc cập nhật khách hàng và đăng ký
5. **Xử lý lỗi** - Ghi lại thất bại và đánh dấu sự kiện là chưa xử lý để thử lại
...
| Issue | Solution |
|-------|----------|
| Xác thực thất bại | Kiểm tra khóa webhook đúng từ bảng điều khiển DodoPayments |
| Lỗi kết nối cơ sở dữ liệu | Xác minh chuỗi kết nối Neon |
| Worker không triển khai | Chạy `npm install`, kiểm tra cú pháp `wrangler.toml` |
...
- [Kho lưu trữ GitHub](https://github.com/dodopayments/cloud-functions/tree/main/cloudflare)
