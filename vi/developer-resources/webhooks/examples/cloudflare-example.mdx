...
1. Đăng ký tài khoản [Neon](https://neon.com)
2. Tạo một dự án mới
3. Mở SQL Editor
...
✅ **Ghi log sự kiện** - Lưu trữ toàn bộ lịch sử trong bảng `webhook_events`  
✅ **Xử lý lỗi** - Đã ghi log và có thể thử lại
...
Trình xử lý webhook:
1. **Xác minh chữ ký** - Đảm bảo yêu cầu đến từ DodoPayments bằng cách sử dụng HMAC-SHA256
2. **Kiểm tra trùng lặp** - Sử dụng ID webhook để ngăn chặn việc xử lý cùng một sự kiện hai lần
3. **Ghi log sự kiện** - Lưu trữ webhook thô trong bảng `webhook_events` để theo dõi lịch sử
4. **Xử lý sự kiện** - Tạo hoặc cập nhật khách hàng và đăng ký
5. **Xử lý lỗi** - Ghi log các lỗi và đánh dấu sự kiện là chưa được xử lý để thử lại
