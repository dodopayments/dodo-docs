...
1. Truy cập [DodoPayments Dashboard](https://app.dodopayments.com) → Nhà phát triển → Webhooks
...
✅ **Ghi lại sự kiện** - Lưu trữ toàn bộ lịch sử trong bảng `webhook_events`  
...
Trình xử lý webhook:
1. **Vô hiệu hóa phân tích cú pháp body** - Để truy cập body thô cho xác minh chữ ký
2. **Xác minh chữ ký** - Đảm bảo yêu cầu đến từ DodoPayments bằng HMAC-SHA256
3. **Kiểm tra trùng lặp** - Sử dụng ID webhook để ngăn chặn việc xử lý cùng một sự kiện hai lần
4. **Ghi lại sự kiện** - Lưu trữ webhook thô trong bảng `webhook_events` để theo dõi lịch sử
