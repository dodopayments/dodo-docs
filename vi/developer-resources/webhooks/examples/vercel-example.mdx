...
<Card title="Kho lưu trữ GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/vercel">
...
1. Đăng ký tại [Neon](https://neon.com)
2. Tạo một dự án mới
3. Mở SQL Editor
4. Sao chép và dán nội dung từ [`schema.sql`](#database-schema)
5. Chạy truy vấn
...
4. Bật các sự kiện đăng ký này:
   - `subscription.active`
   - `subscription.cancelled`
   - `subscription.renewed`
5. Sao chép **Mã bí mật ký**
...
✅ **Xác thực chữ ký** - Sử dụng thư viện dodopayments  
✅ **Idempotency** - Ngăn chặn xử lý trùng lặp với ID webhook  
✅ **Ghi lại sự kiện** - Dấu vết kiểm toán hoàn chỉnh trong bảng `webhook_events`  
✅ **Xử lý lỗi** - Đã ghi lại và có thể thử lại

> **Lưu ý:** Triển khai này chứng minh việc xử lý ba sự kiện đăng ký chính (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) với các trường tối thiểu. Bạn có thể dễ dàng mở rộng nó để hỗ trợ các loại sự kiện và trường bổ sung dựa trên yêu cầu của bạn.
...
Bộ xử lý webhook:
1. **Vô hiệu hóa phân tích thân** - Để truy cập cơ thể thô cho xác minh chữ ký
2. **Xác minh chữ ký** - Đảm bảo yêu cầu đến từ DodoPayments bằng HMAC-SHA256
3. **Kiểm tra trùng lặp** - Sử dụng ID webhook để ngăn chặn xử lý cùng một sự kiện hai lần
4. **Ghi lại sự kiện** - Lưu trữ webhook thô trong bảng `webhook_events` để lấy dấu vết kiểm toán
5. **Xử lý sự kiện** - Tạo hoặc cập nhật khách hàng và các đăng ký trong Neon
6. **Xử lý lỗi** - Ghi lại các lỗi và đánh dấu sự kiện là chưa được xử lý để thử lại
