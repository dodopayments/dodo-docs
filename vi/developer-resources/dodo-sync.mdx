...
Bạn có thể đồng bộ bất kỳ kết hợp nào của các thực thể này bằng cách chỉ định chúng trong tham số `scopes`. Tất cả các hoạt động đồng bộ là gia tăng và chỉ chuyển giao các bản ghi mới hoặc đã cập nhật để có hiệu suất tối ưu.
...
Danh sách các thực thể dữ liệu để đồng bộ, phân cách bằng dấu phẩy. Các phạm vi có sẵn: `licences`, `payments`, `customers`, `subscriptions`. Ví dụ: `"payments,customers"`.
</ParamField>

<ParamField path="--api-key" type="string" required>
API key của bạn cho Dodo Payments. Nên bắt đầu bằng `dp_live_` cho chế độ trực tiếp hoặc `dp_test_` cho chế độ thử nghiệm.
</ParamField>

<ParamField path="--env" type="string" required>
Mục tiêu môi trường. Phải là `"live_mode"` hoặc `"test_mode"`. Điều này xác định môi trường Dodo Payments mà bạn sẽ đồng bộ.
</ParamField>
...
<Tip>
Tuỳ chọn `interval` là bắt buộc khi sử dụng `.start()` để đồng bộ tự động. Việc đồng bộ sẽ tiếp tục diễn ra liên tục tại khoảng thời gian quy định cho đến khi quá trình bị dừng lại.
</Tip>
...
<Tip>
Khi sử dụng đồng bộ thủ công, tuỳ chọn `interval` không bắt buộc. Bạn có thể gọi `.run()` bất cứ khi nào bạn cần thực hiện một hoạt động đồng bộ.
</Tip>
...
Mảng các thực thể để đồng bộ. Các tuỳ chọn có sẵn: `"licences"`, `"payments"`, `"customers"`, `"subscriptions"`. Bạn có thể bao gồm bất kỳ kết hợp nào của những điều này.
</ParamField>
...
**Thuộc tính bắt buộc:**
- `bearerToken`: API key của bạn cho Dodo Payments
- `environment`: Either `"test_mode"` hoặc `"live_mode"`
</ParamField>

<ParamField body="interval" type="number">
Thời gian tính bằng giây giữa các lần đồng bộ tự động. Bắt buộc khi sử dụng `.start()` cho việc đồng bộ tự động. Tùy chọn khi sử dụng `.run()` cho việc đồng bộ thủ công.
</ParamField>
...
<Warning>
**MongoDB**: Một cơ sở dữ liệu có tên `dodopayments_sync` sẽ được tạo tự động trên máy chủ cơ sở dữ liệu của bạn. Tất cả dữ liệu đồng bộ sẽ được lưu trữ ở đó. Tên cơ sở dữ liệu này hiện tại là cố định và không thể thay đổi.
...
**ClickHouse**: Các bảng (`Subscriptions`, `Payments`, `Licenses`, `Customers`) sẽ được tạo bằng cách sử dụng engine ReplacingMergeTree. Khi truy vấn, hãy sử dụng từ khóa `FINAL` để đảm bảo kết quả không trùng lặp.
</Warning>
