...
Hiển thị bộ chọn loại tiền tệ.
</ParamField>
<ParamField query="paymentAmount" type="number">
Cố định số tiền được tính, theo đơn vị tiền tệ chính (ví dụ: <code>12.5</code> cho 12,50 USD). Chỉ áp dụng cho các sản phẩm Pay What You Want và bị bỏ qua nếu thấp hơn giá tối thiểu của sản phẩm.
</ParamField>
...
<Warning>
Dynamic Checkout proxy các endpoint `POST /payments` và `POST /subscriptions` đã deprecated. Tính năng này vẫn hoạt động cho các tích hợp hiện có, nhưng các tích hợp mới nên sử dụng **Checkout Sessions** bên dưới.
</Warning>
...
Checkout sessions cung cấp trải nghiệm checkout được host an toàn hơn, xử lý toàn bộ quy trình thanh toán cho cả giao dịch mua một lần và subscription, đồng thời cho phép kiểm soát đầy đủ việc tùy chỉnh.
...
## Route Handler cho Customer Portal

Route Handler cho Customer Portal cho phép bạn tích hợp liền mạch customer portal của Dodo Payments vào ứng dụng Nuxt.
...
Nếu được đặt thành <code>true</code>, hệ thống sẽ gửi email chứa liên kết portal cho khách hàng.
</ParamField>
...
- **Định tuyến Event:** Gọi event handler phù hợp dựa trên loại payload.
