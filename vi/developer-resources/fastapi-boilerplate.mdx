...
<Info>
Boilerplate này sử dụng FastAPI với các mẫu async/await, Pydantic để xác thực, và `dodopayments` SDK Python để tích hợp API một cách liền mạch.
</Info>
...
<Warning>
Không bao giờ cam kết file `.env` của bạn vào hệ thống kiểm soát phiên bản. Nó đã được bao gồm trong `.gitignore`.
</Warning>
</Step>
...
| Sự kiện | Mô tả |
|-------|-------------|
| `payment.succeeded` | Thanh toán đã hoàn tất thành công |
...
Cập nhật URL webhook trong [bảng điều khiển Dodo](https://app.dodopayments.com/developer/webhooks) đến URL ngrok. Đảm bảo cập nhật file `.env` của bạn với khóa xác minh webhook chính xác.
</Accordion>
