...
<Info>
Boilerplate này sử dụng FastAPI với các mẫu async/await, Pydantic để xác thực và SDK Python `dodopayments` để tích hợp API liền mạch.
</Info>
...
| `/webhook` | POST | Xử lý webhook thanh toán Dodo |
| `/customer-portal` | POST | Tạo URL cổng thông tin khách hàng |
...
| `payment.failed` | Cố gắng thanh toán thất bại |
...
- Cập nhật URL webhook thành miền sản xuất của bạn
...
<Accordion title="Lỗi khi tạo phiên thanh toán">
...
- Kiểm tra log của FastAPI để có thông điệp lỗi chi tiết
</Accordion>
...
Cập nhật URL webhook trong [bảng điều khiển Dodo](https://app.dodopayments.com/developer/webhooks) thành URL ngrok. Đảm bảo cập nhật file `.env` của bạn với khóa xác minh webhook chính xác.
</Accordion>

<Accordion title="Lỗi xác minh chữ ký webhook">
- Đảm bảo `DODO_PAYMENTS_WEBHOOK_KEY` được đặt chính xác trong file `.env` của bạn
- Xác minh rằng bạn đang sử dụng thân request thô cho xác minh chữ ký
- Kiểm tra rằng bạn đang đọc header `webhook-signature` một cách chính xác
</Accordion>
</AccordionGroup>
