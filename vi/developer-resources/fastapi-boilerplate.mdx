...
- Xác minh rằng bạn đang sử dụng raw request body để xác minh chữ ký
- Truyền cả ba header `webhook-id`, `webhook-timestamp` và `webhook-signature` đến `webhook.verify()` — chữ ký Standard Webhooks bao phủ `id.timestamp.body`, không chỉ riêng body
</Accordion>
</AccordionGroup>
