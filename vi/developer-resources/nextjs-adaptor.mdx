...
Cố định số tiền được tính phí theo đơn vị currency chính (ví dụ: <code>12.5</code> tương đương $12.50). Chỉ áp dụng cho các sản phẩm Pay What You Want và bị bỏ qua nếu thấp hơn giá tối thiểu của sản phẩm.
</ParamField>
...
<Warning>
Dynamic Checkout proxy cho các endpoint `POST /payments` và `POST /subscriptions` đã deprecated. Tính năng này vẫn hoạt động với các integration hiện có, nhưng các integration mới nên sử dụng **Checkout Sessions** bên dưới.
</Warning>
...
Checkout sessions cung cấp trải nghiệm checkout được lưu trữ an toàn hơn, xử lý toàn bộ quy trình thanh toán cho cả giao dịch mua một lần và subscription, đồng thời cho phép kiểm soát hoàn toàn việc tùy chỉnh.
...
Route Handler của Customer Portal cho phép bạn tích hợp liền mạch cổng thông tin khách hàng Dodo Payments vào ứng dụng Next.js của mình.
