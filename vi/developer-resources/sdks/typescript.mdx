...
<Info>
Các sự kiện phải có các giá trị `event_id` idempotency duy nhất. Các ID trùng lặp trong cùng một yêu cầu sẽ bị từ chối, và các yêu cầu tiếp theo với các ID tồn tại sẽ bị bỏ qua.
</Info>
...
**Các mức log có sẵn:**
- `'debug'` - Hiển thị thông báo gỡ lỗi, thông tin, cảnh báo và lỗi
- `'info'` - Hiển thị thông báo thông tin, cảnh báo và lỗi  
- `'warn'` - Hiển thị cảnh báo và lỗi (mặc định)
- `'error'` - Chỉ hiển thị lỗi
- `'off'` - Vô hiệu hóa tất cả các log
...
Các phương thức danh sách trong API DodoPayments được phân trang. Bạn có thể sử dụng cú pháp `for await … of` để lặp qua các mục trên tất cả các trang:
...
- Trình duyệt web (Chrome, Firefox, Safari, Edge và hơn thế nữa - mới nhất)
- Node.js 20 LTS hoặc phiên bản mới hơn ([non-EOL](https://endoflife.date/nodejs))
...
<CardGroup cols={2}>
<Card title="Kho lưu trữ GitHub" icon="github" href="https://github.com/dodopayments/dodopayments-typescript">
...
<Card title="Báo cáo sự cố" icon="bug" href="https://github.com/dodopayments/dodopayments-typescript/issues">
