...
<Warning>
`POST /subscriptions` (the SDK's `subscriptions.create` method) đã **lỗi thời**. Phương thức này vẫn hoạt động đối với các tích hợp hiện có, nhưng các tích hợp mới nên tạo subscription thông qua [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`billing` yêu cầu tối thiểu mã quốc gia ISO gồm hai chữ cái. `customer` chấp nhận `{"customer_id": ...}` để gắn với customer hiện có hoặc `{"email": ..., "name": ...}` để tạo customer mới. `product_price` được tính theo mệnh giá tiền tệ nhỏ nhất.
</Info>
...
Theo dõi các sự kiện tùy chỉnh để tính phí dựa trên mức sử dụng:
...
Tự động lặp qua tất cả các mục:
...
Để kiểm soát tốt hơn việc phân trang:
...
Tùy chỉnh client `httpx` nền tảng:
...
Bật tính năng ghi log bằng cách thiết lập biến môi trường:

```bash
export DODO_PAYMENTS_LOG=info
```

Hoặc để ghi log ở cấp độ debug:
...
Bạn cần trợ giúp về Python SDK?
...
- **GitHub**: Tạo issue trên [repository](https://github.com/dodopayments/dodopayments-python)

## Đóng góp

Chúng tôi hoan nghênh mọi đóng góp! Hãy xem [hướng dẫn đóng góp](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md) để bắt đầu.
