...
<Card title="Context Support" icon="clock">
  Hỗ trợ đầy đủ cho `context.Context` đối với việc hủy và hết thời gian chờ
</Card>
...
<Warning>
`POST /subscriptions` (phương thức `subscriptions.create` của SDK) đã **không còn được dùng**. Phương thức này vẫn hoạt động với các integration hiện có, nhưng integration mới nên tạo subscription thông qua [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`Billing` yêu cầu tối thiểu mã ISO gồm hai chữ cái `Country`. `Customer` là một `CustomerRequestUnionParam` — truyền `AttachExistingCustomerParam{CustomerID: ...}` cho customer hiện có hoặc `NewCustomerParam{Email: ..., Name: ...}` cho customer mới. `ProductPrice` được tính theo đơn vị tiền tệ nhỏ nhất.
</Info>
...
Khi API trả về mã trạng thái không thành công, SDK trả về một lỗi thuộc kiểu
`*dodopayments.Error`. Lỗi này cung cấp `StatusCode`, `*http.Request` bên dưới và
`*http.Response`, cùng với JSON của nội dung lỗi. Sử dụng mẫu `errors.As` để
kiểm tra lỗi, và rẽ nhánh dựa trên `StatusCode` để xử lý các trường hợp cụ thể:
...
Thêm middleware tùy chỉnh để ghi log hoặc thu thập metrics:
...
Client an toàn khi được sử dụng đồng thời:
...
- **GitHub**: Mở issue trên [repository](https://github.com/dodopayments/dodopayments-go)

## Đóng góp

Chúng tôi hoan nghênh các đóng góp! Hãy xem [hướng dẫn đóng góp](https://github.com/dodopayments/dodopayments-go/blob/main/CONTRIBUTING.md) để bắt đầu.
