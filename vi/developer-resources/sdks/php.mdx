...
<Note>
  Thư viện API PHP Dodo Payments hiện đang ở **beta**. Chúng tôi rất hào hứng cho
  bạn thử nghiệm với nó! Vui lòng chia sẻ bất kỳ đề xuất, báo cáo lỗi, hoặc
  yêu cầu tính năng bằng cách [gửi một
  vấn đề](https://github.com/dodopayments/dodopayments-php/issues).
</Note>
...
<Warning>
  Lưu trữ các khóa API của bạn một cách an toàn bằng cách sử dụng biến môi trường. Không bao giờ để lộ chúng trong
  mã nguồn của bạn hoặc cam kết chúng vào hệ thống kiểm soát phiên bản.
</Warning>

## Tính năng chính

<CardGroup cols={2}>
<Card title="Tuân thủ PSR-4" icon="check">
  Tuân theo các Khuyến nghị về Tiêu chuẩn PHP cho phát triển PHP hiện đại
</Card>

<Card title="PHP hiện đại" icon="php">
  Được xây dựng cho PHP 8.1+ với khai báo kiểu và kiểu nghiêm ngặt
</Card>

<Card title="Kiểm tra toàn diện" icon="vial">
  Độ phủ kiểm tra toàn diện cho độ tin cậy và ổn định
</Card>

<Card title="Xử lý ngoại lệ" icon="shield-check">
...
SDK sử dụng các tham số có tên để chỉ định các đối số tùy chọn. Bạn có thể khởi tạo các đối tượng giá trị bằng cách sử dụng constructor tĩnh `with`:
...
| Hết thời gian      | `APITimeoutException`          |
| Lỗi mạng          | `APIConnectionException`       |

<Tip>
  Luôn bọc các cuộc gọi API trong các khối try-catch để xử lý các lỗi tiềm ẩn
  một cách nhẹ nhàng và cung cấp phản hồi có ý nghĩa cho người dùng.
</Tip>
...
<Note>
  Các tham số `extra*` có cùng tên sẽ ghi đè lên các tham số đã được tài liệu hóa.
</Note>
...
<CardGroup cols={2}>
<Card title="Kho lưu trữ GitHub" icon="github" href="https://github.com/dodopayments/dodopayments-php">
...
<Card title="Báo cáo sự cố" icon="bug" href="https://github.com/dodopayments/dodopayments-php/issues">
