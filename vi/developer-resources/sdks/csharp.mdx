...
<Warning>
`POST /subscriptions` (phương thức `subscriptions.create` của SDK) đã **deprecated**. Phương thức này vẫn hoạt động cho các tích hợp hiện có, nhưng các tích hợp mới nên tạo subscription thông qua [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`Billing` yêu cầu tối thiểu mã `Country` ISO gồm hai chữ cái. Sử dụng `AttachExistingCustomer` để liên kết một customer hiện có hoặc `NewCustomer` để tạo customer mới. `ProductPrice` được tính theo mệnh giá thấp nhất của currency.
</Info>

## Xử lý lỗi

SDK sẽ throw các exception cụ thể dựa trên HTTP status code. Tất cả lỗi 4xx đều kế thừa từ `DodoPayments4xxException`.
...
Các loại exception khác:
- `DodoPaymentsIOException`: Lỗi networking I/O
- `DodoPaymentsInvalidDataException`: Không thể diễn giải dữ liệu đã được parsed
- `DodoPaymentsException`: Base class cho tất cả exception

## Phân trang

### Tự động phân trang

Lặp qua tất cả kết quả trên mọi trang bằng phương thức `Paginate`, phương thức này trả về một `IAsyncEnumerable`:
...
Đăng ký client trong DI container của bạn:
...
<Tip>
Trong quá trình phát triển, hãy sử dụng [user secrets](https://learn.microsoft.com/en-us/aspnet/core/security/app-secrets) thay vì lưu trữ các key trong `appsettings.json`:
...
<Card title="NuGet Package" icon="box" href="https://www.nuget.org/packages/DodoPayments.Client">
  Xem package trên NuGet Gallery
</Card>
...
- **GitHub**: Mở issue trên [repository](https://github.com/dodopayments/dodopayments-csharp)
