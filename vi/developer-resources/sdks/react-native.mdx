...
`start` chỉ reject với một `CheckoutError` khi bị sử dụng sai hoặc nền tảng gặp lỗi. Khoản thanh toán bị hủy hoặc bị từ chối luôn được trả về dưới dạng kết quả, không phải exception.

- `INVALID_CHECKOUT_URL`: không phải session URL `checkout.dodopayments.com`.
- `INVALID_RETURN_URL`: không phải absolute URL hợp lệ.
- `ALREADY_IN_PROGRESS`: một checkout đang chạy.
- `PLATFORM_ERROR`: lỗi nền tảng không mong muốn.

## Session bị bỏ dở

Nếu app hoặc JS bundle bị dừng giữa chừng trong quá trình checkout, promise sẽ bị mất nhưng native layer vẫn giữ session. Khôi phục session ở lần mount tiếp theo và đối soát với backend của bạn.
...
<Card title="Expo Boilerplate" icon="layer-group" href="/developer-resources/expo-boilerplate">
  Ví dụ Expo hoàn chỉnh có tích hợp checkout.
</Card>
</CardGroup>
