...
Tùy chỉnh nút đóng của sheet, kiểu hiển thị và bảng màu
thông qua `customization` trên `start(...)`. Tất cả các trường đều không bắt buộc; nếu bỏ qua
`customization`, giao diện `SFSafariViewController` mặc định của iOS sẽ được sử dụng.
