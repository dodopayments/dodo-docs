...
{/* LOCKED_PATTERN_848a353666469e7a047ef05af5a45770 */}
    Tích hợp trang thanh toán Dodo Payments vào ứng dụng Express của bạn.
</Card>
...
{/* LOCKED_PATTERN_727e199da3d921456f615ec3f0aa16ca */}
    Nhận và xử lý sự kiện webhook từ Dodo Payments.
</Card>
</CardGroup>
...
<Warning>
Không bao giờ đẩy tệp <code>.env</code> hoặc các bí mật của bạn vào hệ thống kiểm soát phiên bản.
</Warning>
</Step>
</Steps>
...
{/* LOCKED_PATTERN_28fd81a4703a84d931eeff69f1520d54 */}
  <Info>
  Sử dụng bộ xử lý này để tích hợp trang thanh toán Dodo Payments vào ứng dụng Express của bạn. Hỗ trợ các luồng thanh toán tĩnh (GET), động (POST) và phiên (POST).
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_085b207030169d6eead800f9835ef4fb */}
  <Info>
  Sử dụng bộ xử lý này để cho phép khách hàng quản lý đăng ký và thông tin chi tiết của họ thông qua cổng khách hàng Dodo Payments.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_0026b036379d81a9d03c36b3b6e83ddd */}
  <Info>
  Sử dụng bộ xử lý này để nhận và xử lý sự kiện webhook từ Dodo Payments một cách an toàn trong ứng dụng Express của bạn.
  </Info>
  <CodeGroup> 
...
<Info>
Dodo Payments hỗ trợ ba loại luồng thanh toán để tích hợp thanh toán vào trang web của bạn, bộ chuyển đổi này hỗ trợ tất cả các loại luồng thanh toán.
</Info>
...
Xác định sản phẩm (ví dụ, <code>?productId=pdt_nZuwz45WAs64n3l07zpQR</code>).
</ParamField>
...
Chỉ định tiền tệ thanh toán (ví dụ, <code>USD</code>).
</ParamField>
...
Hiện các trường giảm giá.
</ParamField>
<ParamField query="metadata_*" type="string">
Bất kỳ tham số truy vấn nào bắt đầu bằng <code>metadata_</code> sẽ được chuyển thành metadata.
</ParamField>
...
- Gửi các tham số dưới dạng JSON trong body của yêu cầu POST.
- Hỗ trợ cả thanh toán một lần và định kỳ.
- Để xem danh sách đầy đủ các trường thân POST được hỗ trợ, hãy tham khảo:
...
Nếu đặt thành <code>true</code>, gửi một email cho khách hàng kèm liên kết cổng.
</ParamField>
