...
| Field | Description |
|-------|-------------|
| `subTotal` | Tổng của tất cả các mục hàng trong phiên trước khi áp dụng bất kỳ khoản giảm giá hoặc thuế nào. |
| `discount` | Tổng giá trị của tất cả các khoản giảm giá đã áp dụng. |
| `tax` | Số tiền thuế được tính. Trong chế độ `inline`, giá trị này được cập nhật linh động khi người dùng tương tác với các trường địa chỉ. |
| `total` | Kết quả tính toán của `subTotal - discount + tax` theo đơn vị tiền tệ cơ sở của phiên. |
| `currency` | Mã tiền tệ ISO (ví dụ: `"USD"`) cho các giá trị tiêu chuẩn của tổng phụ, khoản giảm giá và thuế. |
| `finalTotal` | Số tiền thực tế mà khách hàng bị tính phí. Số tiền này có thể bao gồm các điều chỉnh bổ sung do chênh lệch tỷ giá hoặc phí của phương thức thanh toán địa phương, vốn không nằm trong bảng phân tích giá cơ bản. |
| `finalTotalCurrency` | Loại tiền tệ mà khách hàng thực sự sử dụng để thanh toán. Giá trị này có thể khác với `currency` nếu [ngang giá sức mua](/features/purchasing-power-parity) hoặc tính năng chuyển đổi sang tiền tệ địa phương đang được kích hoạt. |
