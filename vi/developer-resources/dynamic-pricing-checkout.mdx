...
1. **Đặt giới hạn giá**: Định nghĩa một mức giá tối thiểu (bắt buộc) và tùy chọn một mức giá tối đa
2. **Truyền các khoản tiền động**: Bao gồm một trường `amount` trong giỏ hàng sản phẩm khi tạo phiên thanh toán
3. **Để khách hàng chọn**: Nếu không có khoản tiền nào được cung cấp, khách hàng có thể nhập giá của riêng họ (trong giới hạn của bạn)

<Info>
Khi bạn truyền một `amount` trong giỏ hàng sản phẩm, khoản tiền đó sẽ được sử dụng cho việc thanh toán. Nếu bạn bỏ qua trường {/* INLINE_CODE_PLACEHOLDER_ed202d24820b800650e5cfc3bb366e40 */, khách hàng có thể chọn giá của riêng họ trong quá trình thanh toán (theo cài đặt tối thiểu/tối đa của bạn).
</Info>
...
**Ví dụ**: Nếu mức tối thiểu của bạn là $5.00, nhập `5.00` (hoặc `500` xu).
</Step>
...
Nhấn **Thêm Sản Phẩm** để lưu. Lưu ý ID sản phẩm của bạn (ví dụ: `pdt_123abc456def`) để sử dụng trong các phiên thanh toán.
</Step>
</Steps>
...
Khi sản phẩm của bạn được cấu hình với Pay What You Want, bạn có thể tạo các phiên thanh toán với các khoản tiền động. Trường `amount` trong giỏ hàng sản phẩm cho phép bạn thiết lập giá một cách lập trình cho mỗi phiên thanh toán.

### Hiểu Về Trường Số Tiền

Khi tạo một phiên thanh toán, bạn có thể bao gồm một trường `amount` trong mỗi mục giỏ hàng sản phẩm:

- **Nếu `amount` được cung cấp**: Phiên thanh toán sẽ sử dụng khoản tiền chính xác này (phải nằm trong giới hạn tối thiểu/tối đa của bạn)
...
<Note>
**Định dạng Khoản Tiền**: Trường `amount` phải ở đơn vị thấp nhất của tiền tệ. Đối với USD, điều này có nghĩa là xu (ví dụ: $25.00 = `2500`). Đối với các loại tiền tệ khác, sử dụng đơn vị nhỏ nhất (ví dụ: paise cho INR).
</Note>

## Bước 3: Cho Khách Hàng Chọn Giá Của Họ

Nếu bạn muốn khách hàng chọn giá của riêng họ trong quá trình thanh toán, chỉ cần bỏ qua trường `amount` từ giỏ hàng sản phẩm. Trang thanh toán sẽ hiển thị một trường nhập liệu nơi khách hàng có thể nhập bất kỳ khoản tiền nào trong giới hạn tối thiểu và tối đa của bạn.
...
Sử dụng metadata để theo dõi lý do tại sao các khoản tiền cụ thể được chọn (ví dụ: `pricing_tier`, `discount_code`, `user_segment`).
</Card>
...
<AccordionGroup>
<Accordion title="Khoản tiền bị bỏ qua">
Nếu trường `amount` của bạn bị bỏ qua, hãy xác minh rằng:
- Sản phẩm đã được bật **Pay What You Want** trong bảng điều khiển
- Sản phẩm là một sản phẩm **Thanh toán Đơn** (một lần), không phải là một sản phẩm đăng ký
- Khoản tiền ở định dạng chính xác (đơn vị tiền tệ thấp nhất, ví dụ: xu cho USD)
</Accordion>

<Accordion title="Khoản tiền vượt quá tối đa hoặc dưới tối thiểu">
API sẽ từ chối các phiên thanh toán mà khoản tiền vi phạm giới hạn giá của sản phẩm của bạn. Luôn xác thực các khoản tiền trước khi tạo các phiên thanh toán, hoặc để khách hàng chọn giá của họ bằng cách bỏ qua trường `amount`.
</Accordion>

<Accordion title="Khách hàng không thể nhập giá của riêng họ">
Nếu khách hàng không thấy trường nhập giá, hãy đảm bảo bạn đã bỏ qua trường `amount` từ giỏ hàng sản phẩm. Khi `amount` được cung cấp, phiên thanh toán sẽ sử dụng khoản tiền chính xác đó.
</Accordion>
</AccordionGroup>