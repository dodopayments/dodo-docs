...
<Info>
  Không giống như phương thức <code>checkout</code> cũ, <code>checkoutSession</code>
  không yêu cầu thông tin thanh toán trước vì sẽ được lấy từ người dùng
  ở trang thanh toán. Bạn vẫn có thể ghi đè bằng cách truyền khóa
  <code>billing</code> trong đối số.
</Info>

<Info>
  Tương tự như phương thức <code>checkout</code> cũ,
  <code>checkoutSession</code> lấy email và tên của khách hàng từ phiên
  better-auth của họ, tuy nhiên, bạn có thể ghi đè bằng cách truyền đối tượng
  <code>customer</code> với email và tên.
</Info>

<Info>
  Bạn có thể truyền các tùy chọn tương tự trong đối số như phần thân yêu cầu cho [Create
  Checkout
...
<Note>
  URL trả về được lấy từ <code>successUrl</code> được cấu hình trong
  server plugin. Bạn không cần phải bao gồm <code>return_url</code> trong
  payload của client.
</Note>

## Thanh toán Cũ (Đã Ngưng Sử Dụng)

<Warning>
  Phương thức <code>authClient.dodopayments.checkout</code> đã ngưng sử dụng. Sử dụng
  <code>checkoutSession</code> thay thế cho những triển khai mới.
</Warning>
...
## Theo Dõi Việc Sử Dụng Tính Theo Lượng

Kích hoạt plugin <code>usage()</code> trên server để thu thập các sự kiện tính theo lượng và cho phép khách hàng kiểm tra hóa đơn dựa trên việc sử dụng của họ.

- <code>authClient.dodopayments.usage.ingest</code> nạp các sự kiện cho người dùng đã đăng nhập và xác minh email.
- <code>authClient.dodopayments.usage.meters.list</code> liệt kê việc sử dụng gần đây cho các thiết bị đo gắn liền với gói đăng ký của khách hàng.
...
<Tip>
  Dấu thời gian cũ hơn một giờ hoặc hơn năm phút trong tương lai sẽ
  bị từ chối khi nạp việc sử dụng.
</Tip>
<Tip>
  Nếu bạn bỏ qua <code>meter_id</code> khi liệt kê các thiết bị đo sử dụng, tất cả các thiết bị đo
  gắn liền với gói đăng ký của khách hàng sẽ được trả về.
</Tip>

# Webhooks

<Info>
  Plugin webhooks xử lý sự kiện thanh toán thời gian thực từ Dodo Payments kèm
  với xác minh chữ ký an toàn. Endpoint mặc định là
  <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
<Step title="Generate and set webhook secret">
Tạo một bí mật webhook cho URL endpoint của bạn (ví dụ, <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) trong Dodo Payments Dashboard và đặt nó trong tập tin <code>.env</code> của bạn:
...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (bắt buộc): Phiên bản khách hàng DodoPayments
    - <b>createCustomerOnSignUp</b> (tùy chọn): Tự động tạo khách hàng khi đăng ký
    - <b>use</b> (bắt buộc): Mảng các plugin để kích hoạt (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (tùy chọn): Hàm nhận BetterAuth `User` và trả về các trường bổ sung để gắn vào khách hàng DodoPayments khi tạo và cập nhật (ví dụ `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b>: Mảng các sản phẩm hoặc hàm async trả về các sản phẩm -
    <b>successUrl</b>: URL để chuyển hướng sau khi thanh toán thành công -
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>Key API không hợp lệ</b>: Kiểm tra lại
    <code>DODO_PAYMENTS_API_KEY</code> trong <code>.env</code>. -
    <b>Webhook không khớp chữ ký</b>: Đảm bảo bí mật webhook khớp với bí mật đã đặt
    trong Bảng Điều Khiển Dodo Payments. - <b>Khách hàng không được tạo</b>: Xác nhận
    <code>createCustomerOnSignUp</code> được đặt thành <code>true</code>.
  </Accordion>
  <Accordion title="Best Practices">
    - Sử dụng biến môi trường cho tất cả các bí mật và key. - Kiểm tra ở chế độ
    <code>test_mode</code> trước khi chuyển sang <code>live_mode</code>. - Ghi lại
    các sự kiện webhook để gỡ lỗi và kiểm toán.
  </Accordion>
</AccordionGroup>
