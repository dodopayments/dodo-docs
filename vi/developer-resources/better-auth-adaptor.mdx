...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (bắt buộc): Client DodoPayments instance
    - <b>createCustomerOnSignUp</b> (tùy chọn): Tự động tạo khách hàng khi người dùng đăng ký
    - <b>use</b> (bắt buộc): Mảng các plugin để kích hoạt (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (tùy chọn): Hàm nhận `User` của BetterAuth và trả về các trường bổ sung để đính kèm vào khách hàng của DodoPayments khi tạo và cập nhật (e.g. `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b>: Mảng sản phẩm hoặc hàm async trả về sản phẩm -{" "}
    <b>successUrl</b>: URL chuyển hướng sau khi thanh toán thành công -{" "}
    <b>authenticatedUsersOnly</b>: Yêu cầu xác thực người dùng (mặc định: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>API key không hợp lệ</b>: Kiểm tra lại kỹ lưỡng
    <code>DODO_PAYMENTS_API_KEY</code> trong <code>.env</code>. -{" "}
    <b>Chữ ký webhook không khớp</b>: Đảm bảo bí mật webhook trùng khớp với cái đã thiết lập trong Dodo Payments Dashboard. - <b>Khách hàng chưa được tạo</b>: Xác nhận
    <code>createCustomerOnSignUp</code> được đặt là <code>true</code>.
  </Accordion>
  <Accordion title="Best Practices">
    - Sử dụng biến môi trường cho tất cả bí mật và key. - Kiểm tra trong
    <code>test_mode</code> trước khi chuyển sang <code>live_mode</code>. - Ghi
    lại sự kiện webhook để gỡ lỗi và kiểm toán.
  </Accordion>
</AccordionGroup>
