...
Các đăng ký theo yêu cầu cho phép bạn ủy quyền phương thức thanh toán của khách hàng một lần và sau đó tính phí các khoản biến đổi bất cứ khi nào bạn cần, thay vì theo lịch trình cố định. Tính năng này có sẵn cho tất cả các tài khoản—không cần phê duyệt.
...
2. Sau đó, bạn tạo các khoản phí đối với đăng ký đó với số tiền tùy chỉnh bằng cách sử dụng điểm cuối tính phí chuyên dụng.
...
<AccordionGroup>
  <Accordion title="Tham số nội dung yêu cầu tính phí">
    <ParamField body="product_price" type="integer" required>
      Số tiền để tính phí (trong đơn vị tiền tệ nhỏ nhất). Ví dụ: để tính phí $25.00, hãy truyền <code>2500</code>.
    </ParamField>
...
<Warning>
Tính phí một đăng ký không phải theo yêu cầu có thể thất bại. Đảm bảo rằng đăng ký có `on_demand: true` trong chi tiết của nó trước khi tính phí.
</Warning>

## Thử lại thanh toán

Hệ thống phát hiện gian lận của chúng tôi có thể chặn các mẫu thử lại mạnh mẽ (và có thể đánh dấu chúng là thử nghiệm thẻ tiềm năng). Tuân theo chính sách thử lại an toàn.

<Warning>
Các mẫu thử lại bùng nổ có thể bị đánh dấu là gian lận hoặc thử nghiệm thẻ nghi ngờ bởi các hệ thống và nhà xử lý rủi ro của chúng tôi. Tránh các lần thử lại cụm; tuân theo lịch trình lùi và hướng dẫn căn chỉnh thời gian bên dưới.
</Warning>
...
- **Thay đổi thông tin bổ sung (tùy chọn)**: Nếu bạn duy trì hệ thống thử lại của riêng mình, phân biệt các lần thử lại qua thông tin bổ sung (ví dụ: `retry_attempt`).
...
<Tip>
Chỉ thử lại trên các vấn đề mềm/tạm thời (ví dụ: `insufficient_funds`, `issuer_unavailable`, `processing_error`, thời gian mạng). Nếu cùng một từ chối lặp lại, tạm dừng các lần thử lại tiếp theo.
</Tip>
...
- Sử dụng thông tin bổ sung chỉ để quan sát (ví dụ: `retry_attempt`); không bao giờ cố gắng "tránh" các hệ thống gian lận/rủi ro bằng cách xoay vòng các trường không quan trọng.
...
<Tip>
Đối với các luồng theo yêu cầu, tập trung vào `payment.succeeded` và `payment.failed` để đối chiếu các khoản phí dựa trên sử dụng.
</Tip>
...
Sử dụng khóa API thử nghiệm của bạn để tạo đăng ký với `payment_link: true`, sau đó mở liên kết và hoàn thành mệnh lệnh.
</Step>
...
### Các mã từ chối bạn không nên thử lại
...
<Note>
Để biết danh sách đầy đủ các lý do từ chối và liệu chúng có thể được người dùng sửa chữa hay không, hãy xem tài liệu [Transaction Failures](/api-reference/transaction-failures).
</Note>

<Tip>
Chỉ thử lại trong các trường hợp mềm/tạm thời (ví dụ: `insufficient_funds`, `issuer_unavailable`, `processing_error`, hết thời gian mạng). Nếu cùng một lỗi từ chối lặp lại, tạm dừng không thử lại nữa.
</Tip>

### Hướng dẫn triển khai (không có mã)

- Sử dụng bộ lập lịch/hàng đợi lưu trữ chính xác dấu thời gian; tính toán lần thử tiếp theo tại đúng thời điểm trong ngày (ví dụ: `T + 3 days` vào cùng giờ HH:MM).
- Duy trì và tham khảo dấu thời gian thanh toán cuối cùng thành công `T` để tính toán lần thử tiếp theo; không gộp nhiều đăng ký vào cùng một thời điểm.
- Luôn đánh giá lý do từ chối cuối cùng; dừng thử lại đối với các từ chối cứng trong danh sách bỏ qua ở trên.
- Giới hạn thử lại đồng thời cho mỗi khách hàng và mỗi tài khoản để ngăn chặn các sự tăng đột ngột ngoài dự kiến.
- Giao tiếp một cách chủ động: gửi email/SMS cho khách hàng để cập nhật phương thức thanh toán của họ trước lần thử tiếp theo theo lịch trình.
- Sử dụng siêu dữ liệu chỉ để quan sát (ví dụ: `retry_attempt`); không bao giờ cố "né tránh" hệ thống gian lận/rủi ro bằng cách xoay vòng các trường không quan trọng.
...
- **subscription.active**: Yêu cầu ủy quyền và đăng ký đã được kích hoạt
...
<Tip>
Đối với các luồng theo yêu cầu, tập trung vào `payment.succeeded` và `payment.failed` để đối chiếu các chi phí dựa trên sử dụng. Khi `payment.failed` được theo sau bởi `subscription.on_hold`, hãy xem [Handling failed charges](#handling-failed-charges) để khôi phục đăng ký.
</Tip>
...
Sử dụng khóa API thử nghiệm của bạn để tạo đăng ký với `payment_link: true`, sau đó mở liên kết và hoàn thành ủy quyền.
</Step>
{/* LOCKED_PATTERN_bbd99e8cb553e62a729a22de4094d970 */}
Gọi endpoint thanh toán với `product_price` nhỏ (ví dụ: `100`) và xác minh rằng bạn nhận được `payment.succeeded`.
</Step>
{/* LOCKED_PATTERN_8339399a605a0a43aad806057be77fed */}
Chuyển sang khóa API trực tiếp của bạn khi bạn đã xác nhận các sự kiện và cập nhật trạng thái nội bộ.
</Step>
</Steps>
...
- **Không nhận được webhook**: Xác minh URL webhook và cấu hình bí mật chữ ký của bạn.
