...
Nếu các lần thử lại không thành công vì phương thức thanh toán bị lỗi (thẻ hết hạn, tài khoản đóng, v.v.), sử dụng [`POST /subscriptions/{subscription_id}/update-payment-method`](/api-reference/subscriptions/update-payment-method) để thu thập phương thức mới từ khách hàng. Khi thành công, đăng ký sẽ trở về `active` và `payment.succeeded` tiếp theo sẽ gửi các webhook `subscription.active`.
</Step>
</Steps>
...
Bạn có thể hủy đăng ký theo yêu cầu qua API bất kỳ lúc nào. Bạn kiểm soát việc hủy bỏ ngay lập tức hay theo lịch.
...
Giữ hành vi cổng khách hàng — duy trì đăng ký hoạt động cho đến ngày hủy theo lịch, rồi kết thúc.
...
Thực hiện xử lý webhook để theo dõi hành trình khách hàng. Xem [Thực hiện Webhooks](/developer-resources/integration-guide#implementing-webhooks).
...
- **subscription.cancelled**: Đăng ký bị hủy hoàn toàn (xem [Hủy Bỏ](#cancellation))
- **payment.succeeded**: Tính phí thành công
- **payment.failed**: Tính phí không thành công

<Tip>
Đối với luồng theo yêu cầu, hãy tập trung vào `payment.succeeded` và `payment.failed` để đối chiếu các khoản phí dựa trên sử dụng. Khi `payment.failed` được theo sau bởi `subscription.on_hold`, hãy xem [Xử lý các khoản phí không thành công](#handling-failed-charges) để khôi phục đăng ký.
</Tip>
...
<Steps>
<Step title="Create in test mode">
Sử dụng khóa API kiểm tra của bạn để tạo đăng ký, sau đó mở `checkout_url` trả về và hoàn tất yêu cầu.
</Step>
<Step title="Trigger a charge">
Gọi endpoint tính phí với một `product_price` nhỏ (ví dụ, `100`) và xác minh bạn nhận được `payment.succeeded`.
</Step>
<Step title="Go live">
Chuyển sang khóa API trực tiếp của bạn sau khi bạn đã xác nhận các sự kiện và cập nhật trạng thái nội bộ.
</Step>
</Steps>
