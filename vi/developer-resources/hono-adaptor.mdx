...
### Các Query Parameter được hỗ trợ
...
Cố định số tiền được tính phí theo đơn vị tiền tệ cơ bản (ví dụ: <code>12.5</code> tương đương $12.50). Chỉ áp dụng cho sản phẩm Pay What You Want và sẽ bị bỏ qua nếu thấp hơn giá tối thiểu của sản phẩm.
</ParamField>
...
Sử dụng những điều này bên trong mã của bạn như sau:

process.env.DODO_PAYMENTS_API_KEY
process.env.DODO_PAYMENTS_WEBHOOK_KEY

Lưu ý Bảo mật: Đừng commit thông tin bí mật vào hệ thống quản lý phiên bản. Sử dụng tệp .env cục bộ và trình quản lý bí mật trong các môi trường triển khai (ví dụ: AWS, Vercel, Heroku, v.v.).