...
Sau khi chỉnh sửa `convex.config.ts`, chạy `npx convex dev` một lần để tạo ra các loại cần thiết.
</Step>
<Step title="Cài đặt biến môi trường">
Cài đặt biến môi trường trong bảng điều khiển Convex của bạn (**Cài đặt** → **Biến Môi Trường**). Bạn có thể truy cập bảng điều khiển bằng cách chạy:
...
<Warning>
Trước khi sử dụng truy vấn này, hãy chắc chắn định nghĩa lược đồ thích hợp trong tệp `convex/schema.ts` của bạn hoặc thay đổi truy vấn để phù hợp với lược đồ hiện có của bạn.
</Warning>
...
<Tab title="Thiết lập Cổng thông tin Khách hàng">
  <Info>
  Sử dụng chức năng này để cho phép khách hàng quản lý các đăng ký và thông tin của họ qua cổng thông tin khách hàng của Dodo Payments. Khách hàng sẽ được xác định tự động qua hàm `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="Thiết lập Bộ xử lý Webhook">
  <Info>
  Sử dụng bộ xử lý này để nhận và xử lý các sự kiện webhook Dodo Payments một cách an toàn trong ứng dụng Convex của bạn. Tất cả các bộ xử lý webhook nhận `ActionCtx` làm tham số đầu tiên, cho phép bạn sử dụng `ctx.runQuery()` và `ctx.runMutation()` để tương tác với cơ sở dữ liệu của bạn.
  </Info>
  <CodeGroup>
...
<Info>
Khách hàng sẽ được xác định tự động bằng cách sử dụng hàm `identify` được cấu hình trong thiết lập DodoPayments của bạn. Hàm này nên trả về `dodoCustomerId` của khách hàng.
</Info>
