...
{/* LOCKED_PATTERN_076a045a43785e7154357214ce1a1fe2 */}
    Tích hợp quy trình thanh toán Dodo Payments theo phiên.
</Card>
{/* LOCKED_PATTERN_dbb6954e19619bb187d501d5ccba29a3 */}
    Cho phép khách hàng quản lý các đăng ký và thông tin của họ.
</Card>
...
Sau khi chỉnh sửa `convex.config.ts`, hãy chạy `npx convex dev` một lần để tạo các kiểu cần thiết.
</Step>
...
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Bí mật webhook của bạn (bắt buộc để xử lý webhook)

<Warning>
Luôn sử dụng biến môi trường Convex cho thông tin nhạy cảm. Không bao giờ commit bí mật vào hệ thống kiểm soát phiên bản.
</Warning>
</Step>
</Steps>
...
Đầu tiên, tạo một truy vấn nội bộ để lấy khách hàng từ cơ sở dữ liệu của bạn. Truy vấn này sẽ được sử dụng trong các hàm thanh toán để xác định khách hàng.

<Warning>
Trước khi sử dụng truy vấn này, hãy đảm bảo định nghĩa sơ đồ phù hợp trong tệp `convex/schema.ts` của bạn hoặc thay đổi truy vấn cho phù hợp với sơ đồ hiện có.
</Warning>
...
{/* LOCKED_PATTERN_cfdbe95b4d26ef82d25c4a1bfd5d5aeb */}
  <Info>
  Sử dụng hàm này để tích hợp quy trình thanh toán Dodo Payments vào ứng dụng Convex của bạn. Áp dụng quy trình checkout dựa trên phiên với hỗ trợ đầy đủ các tính năng.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_de2bafe43db636b1864521f4f7e5aa25 */}
  <Info>
  Sử dụng hàm này để cho phép khách hàng quản lý các đăng ký và thông tin của họ qua cổng khách hàng Dodo Payments. Khách hàng tự động được xác định thông qua hàm `identify`.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_eeaa14c4f31af768a3b7e376ee69b626 */}
  <Info>
  Sử dụng trình xử lý này để nhận và xử lý các sự kiện webhook của Dodo Payments một cách an toàn trong ứng dụng Convex của bạn. Tất cả trình xử lý webhook đều nhận Convex `ActionCtx` làm tham số đầu tiên, cho phép bạn sử dụng `ctx.runQuery()` và `ctx.runMutation()` để tương tác với cơ sở dữ liệu.
  </Info>
  <CodeGroup>
  ...
  <Warning>
  Hãy đảm bảo định nghĩa các phép biến đổi cơ sở dữ liệu tương ứng trong backend Convex của bạn cho từng sự kiện webhook bạn muốn xử lý. Chẳng hạn, tạo một phép biến đổi <code>createPayment</code> để ghi lại các khoản thanh toán thành công hoặc <code>createSubscription</code> để quản lý trạng thái đăng ký.
  </Warning>
</Tab>
</Tabs>
...
<Info>
Thành phần Convex của Dodo Payments sử dụng quy trình thanh toán dựa trên phiên, cung cấp trải nghiệm thanh toán an toàn, có thể tùy chỉnh với giỏ hàng sản phẩm và thông tin khách hàng được cấu hình sẵn. Đây là phương pháp được đề xuất cho tất cả các luồng thanh toán.
</Info>
...
Nếu được đặt thành <code>true</code>, gửi email tới khách hàng với đường dẫn cổng.
</ParamField>

<Info>
Khách hàng được tự động xác định bằng hàm `identify` được cấu hình trong thiết lập DodoPayments của bạn. Hàm này nên trả về `dodoCustomerId` của khách hàng.
</Info>
...
<Info>
Sử dụng hàm checkout từ các thành phần React của bạn với các hook Convex.
</Info>
