...
Khi một khách hàng mua sản phẩm ở chế độ thủ công, quyền cấp được tạo ở trạng thái `pending` không có mã và một webhook [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) được kích hoạt với `integration_type: "license_key"` và `status: "pending"`. Bạn có thể phản ứng với webhook đó, hoặc kiểm tra endpoint [List Grants](/api-reference/entitlements/list-grants) với các bộ lọc `integration_type` và `status`:
