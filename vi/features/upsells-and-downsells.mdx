...
description: Tăng doanh thu với các giao dịch mua một cú nhấp chuột bằng cách sử
  dụng phương thức thanh toán đã lưu cho các ưu đãi sau khi mua, nâng cấp đăng
  ký và bán chéo.
icon: arrow-up-right-dots
---

<Info>
Upsells và downsells cho phép bạn cung cấp các sản phẩm bổ sung hoặc thay đổi kế hoạch cho khách hàng bằng cách sử dụng phương thức thanh toán đã lưu của họ. Điều này cho phép giao dịch mua một cú nhấp chuột mà không cần thu thập thanh toán, nâng cao đáng kể tỷ lệ chuyển đổi.
</Info>

<CardGroup cols={3}>
<Card title="Upsells Sau Khi Mua" icon="cart-plus">
  Cung cấp các sản phẩm bổ sung ngay lập tức sau khi thanh toán với giao dịch mua một cú nhấp chuột.
</Card>

<Card title="Nâng Cấp Đăng Ký" icon="arrow-up">
  Di chuyển khách hàng lên các cấp cao hơn với tỷ lệ tự động và thanh toán ngay lập tức.
</Card>

<Card title="Bán Chéo" icon="grid-2-plus">
  Thêm sản phẩm liên quan cho khách hàng hiện có mà không cần nhập lại chi tiết thanh toán.
</Card>
</CardGroup>
...
- **Upsells**: Cung cấp một sản phẩm giá trị cao hơn hoặc nâng cấp (ví dụ: kế hoạch Pro thay vì Basic)
- **Downsells**: Cung cấp một lựa chọn có giá thấp hơn khi khách hàng từ chối hoặc hạ cấp
- **Cross-sells**: Đề xuất các sản phẩm bổ sung (ví dụ: phụ kiện, sản phẩm liên quan)

Dodo Payments cho phép những luồng này thông qua tham số `payment_method_id`, cho phép bạn thu phí từ phương thức thanh toán đã lưu của khách mà không cần họ nhập lại thông tin thẻ.

### Lợi Ích Chính

| Lợi Ích | Ảnh Hưởng |
|---------|--------|
| **Giao dịch mua một cú nhấp chuột** | Bỏ qua hoàn toàn mẫu thanh toán cho khách hàng quay lại |
| **Tỷ lệ chuyển đổi cao hơn** | Giảm ma sát tại thời điểm quyết định |
| **Xử lý ngay lập tức** | Các khoản phí được xử lý ngay lập tức với `confirm: true` |
| **Trải nghiệm người dùng liền mạch** | Khách hàng ở lại ứng dụng của bạn trong suốt quy trình |
...
Trước khi triển khai upsells và downsells, hãy đảm bảo bạn đã có:

<Steps>
<Step title="Khách hàng với Phương Thức Thanh Toán Đã Lưu">
  Khách hàng phải đã hoàn tất ít nhất một giao dịch mua. Các phương thức thanh toán sẽ được tự động lưu khi khách hàng hoàn tất checkout.
</Step>

<Step title="Sản Phẩm Được Cấu Hình">
  Tạo các sản phẩm upsell của bạn trong bảng điều khiển Dodo Payments. Chúng có thể là giao dịch thanh toán một lần, đăng ký hoặc phụ kiện.
</Step>

<Step title="Webhook Endpoint">
  Thiết lập webhooks để xử lý các sự kiện `payment.succeeded`, `payment.failed`, và `subscription.plan_changed`.
</Step>
</Steps>

## Nhận Phương Thức Thanh Toán của Khách Hàng

Trước khi cung cấp một upsell, hãy lấy các phương thức thanh toán đã lưu của khách hàng:
...
<Info>
Các phương thức thanh toán sẽ được tự động lưu khi khách hàng hoàn tất checkout. Bạn không cần phải lưu chúng một cách rõ ràng.
</Info>

## Upsells Một Cú Nhấp Chuột Sau Khi Mua

Cung cấp các sản phẩm bổ sung ngay lập tức sau khi giao dịch mua thành công. Khách hàng có thể chấp nhận chỉ với một cú nhấp chuột vì phương thức thanh toán của họ đã được lưu.
...
<Warning>
Khi sử dụng `payment_method_id`, bạn phải thiết lập `confirm: true` và cung cấp một `customer_id` hiện có. Phương thức thanh toán phải thuộc về khách hàng đó.
</Warning>

## Nâng Cấp Đăng Ký

Di chuyển khách hàng đến các gói đăng ký cấp cao hơn với việc xử lý tự động.
...
Luôn xem trước các thay đổi kế hoạch để cho khách hàng thấy chính xác những gì họ sẽ phải thanh toán:
...
### Các Chế Độ Tính Tỷ Lệ

Chọn cách khách hàng được tính phí khi nâng cấp:

| Chế Độ | Hành Vi | Tốt Nhất |
|------|----------|----------|
| `difference_immediately` | Tính phí chênh lệch giá ngay lập tức ($30→$80 = $50) | Nâng cấp đơn giản |
| `prorated_immediately` | Tính phí dựa trên thời gian còn lại trong chu kỳ thanh toán | Tính phí công bằng theo thời gian |
| `full_immediately` | Tính phí toàn bộ giá kế hoạch mới, bỏ qua thời gian còn lại | Chu kỳ thanh toán được đặt lại |

<Tip>
Sử dụng `difference_immediately` cho các quy trình nâng cấp đơn giản. Sử dụng `prorated_immediately` khi bạn muốn tính đến thời gian không sử dụng trên kế hoạch hiện tại.
</Tip>

## Bán Chéo

Thêm các sản phẩm bổ sung cho khách hàng hiện có mà không cần họ nhập lại thông tin thanh toán.
...
Khi khách hàng muốn chuyển sang kế hoạch cấp thấp hơn, hãy xử lý sự chuyển tiếp một cách tốt đẹp với tín dụng tự động.

### Cách Giảm Cấp Hoạt Động

1. Khách hàng yêu cầu giảm cấp (Pro → Basic)
2. Hệ thống tính toán giá trị còn lại trên kế hoạch hiện tại
3. Tín dụng được thêm vào đăng ký cho các lần gia hạn trong tương lai
4. Khách hàng chuyển sang kế hoạch mới ngay lập tức
...
<Info>
Tín dụng từ việc giảm cấp sử dụng `difference_immediately` có phạm vi đăng ký và tự động được áp dụng cho các lần gia hạn trong tương lai. Chúng khác với [Tín Dụng Khách Hàng](/features/customer-credit).
</Info>

## Ví Dụ Hoàn Chỉnh: Quy Trình Upsell Sau Khi Mua

Dưới đây là một triển khai hoàn chỉnh cho thấy cách cung cấp một upsell sau một giao dịch mua thành công:
...
<AccordionGroup>
<Accordion title="Thời Gian Upsells Chiến Lược">
Thời điểm tốt nhất để cung cấp một upsell là ngay sau khi giao dịch mua thành công khi khách hàng đang trong tâm lý mua sắm. Một số thời điểm hiệu quả khác:
- Sau khi sử dụng tính năng đạt mốc
- Khi tiếp cận giới hạn kế hoạch
- Trong suốt quá trình hoàn thành onboarding
</Accordion>

<Accordion title="Xác Thực Tính Duyệt Phương Thức Thanh Toán">
Trước khi cố gắng thu phí một cú nhấp chuột, hãy xác minh phương thức thanh toán:
- Có tương thích với loại tiền tệ của sản phẩm
- Chưa hết hạn
- Thuộc về khách hàng

API sẽ xác thực những điều này, nhưng việc kiểm tra chủ động sẽ nâng cao trải nghiệm người dùng.
</Accordion>

<Accordion title="Xử Lý Lỗi Một Cách Khéo Léo">
Khi các khoản thu phí một cú nhấp chuột bị lỗi:
1. Quay trở lại luồng thanh toán tiêu chuẩn
2. Thông báo cho khách hàng với thông điệp rõ ràng
3. Cung cấp tùy chọn cập nhật phương thức thanh toán
4. Đừng cố gắng lặp lại các khoản thu phí đã thất bại
</Accordion>

<Accordion title="Cung Cấp Giá Trị Rõ Ràng">
Upsells chuyển đổi tốt hơn khi khách hàng hiểu giá trị:
- Hiển thị những gì họ nhận được so với kế hoạch hiện tại
- Nhấn mạnh vào sự khác biệt về giá, không phải tổng số tiền
- Sử dụng chứng minh xã hội (ví dụ: "Nâng cấp phổ biến nhất")
</Accordion>

<Accordion title="Tôn Trọng Lựa Chọn của Khách Hàng">
- Luôn cung cấp cách dễ dàng để từ chối
- Đừng hiển thị cùng một upsell nhiều lần sau khi từ chối
- Theo dõi và phân tích các upsells chuyển đổi để tối ưu hóa đề nghị
</Accordion>
</AccordionGroup>

## Webhooks để Theo Dõi

Theo dõi các sự kiện webhook này cho các luồng upsell và downgrades:

| Sự Kiện | Kích Hoạt | Hành Động |
|-------|---------|--------|
| `payment.succeeded` | Đã hoàn thành thanh toán upsell / cross-sell | Giao sản phẩm, cập nhật quyền truy cập |
| `payment.failed` | Thu phí một cú nhấp chuột bị lỗi | Hiện lỗi, cung cấp thử lại hoặc quay lại |
| `subscription.plan_changed` | Nâng cấp / giảm cấp đã hoàn thành | Cập nhật tính năng, gửi xác nhận |
| `subscription.active` | Đăng ký được kích hoạt lại sau khi thay đổi gói | Cấp quyền truy cập đến cấp mới |

<Card title="Hướng Dẫn Tích Hợp Webhook" icon="webhook" href="/developer-resources/webhooks">
...
<CardGroup cols={2}>
<Card title="Hướng Dẫn Nâng Cấp Đăng Ký" icon="arrows-rotate" href="/developer-resources/subscription-upgrade-downgrade">
  Hướng dẫn chi tiết về thay đổi kế hoạch, chế độ tỷ lệ và xử lý lỗi.
</Card>

<Card title="Phiên Thanh Toán" icon="cart-shopping" href="/developer-resources/checkout-session">
  Tài liệu tham khảo hoàn chỉnh cho việc tạo các phiên thanh toán với tất cả các tùy chọn.
</Card>

<Card title="API Phương Thức Thanh Toán của Khách Hàng" icon="credit-card" href="/api-reference/customers/get-customer-payment-methods">
  Tài liệu tham khảo API để liệt kê các phương thức thanh toán của khách hàng.
</Card>

<Card title="Phụ Kiện" icon="puzzle-piece" href="/features/addons">
  Nâng cao các gói đăng ký bằng các phụ kiện linh hoạt để tăng doanh thu.
</Card>
</CardGroup>
