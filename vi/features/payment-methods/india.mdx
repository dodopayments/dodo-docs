...
sidebarTitle: Ấn Độ
description: Chấp nhận UPI và tất cả các thẻ phát hành tại Ấn Độ ở Ấn Độ với các
  quy định đăng ký tuân thủ RBI. Hiểu về thời gian xử lý chậm 48 giờ, giới hạn
  đăng ký, và xử lý webhook.
...
keywords:
  ...
  - phương thức thanh toán Ấn Độ
---

Ấn Độ có cơ sở hạ tầng thanh toán độc đáo do UPI (hơn 60% giao dịch kỹ thuật số) và các thẻ phát hành tại Ấn Độ (Visa, Mastercard, Rupay, v.v.) chi phối. Dodo Payments hỗ trợ tất cả các phương thức này với việc tuân thủ đầy đủ RBI cho các quy định đăng ký.
...
Không giống như hầu hết các phương thức thanh toán thay thế, UPI và tất cả các thẻ phát hành tại Ấn Độ (Visa, Mastercard, Rupay, v.v.) hỗ trợ các thanh toán định kỳ thông qua các quy định của RBI.
</Card>
</CardGroup>
...
*Các đăng ký yêu cầu quy định tuân thủ RBI với các quy tắc xử lý đặc biệt. Thời gian xử lý chậm 48 giờ áp dụng cho tất cả thẻ phát hành tại Ấn Độ và UPI.
...
| :---- | :--- | :----- |
| `subscription.active` | Ủy quyền được chấp thuận | Ghi nhận bắt đầu đăng ký |
| `payment.succeeded` | ~48 giờ sau ngày tính phí | Cấp tiếp tục truy cập |
| `payment.failed` | Trừ tiền thất bại | Thông báo khách hàng, tạm dừng truy cập |
| `subscription.on_hold` | Thanh toán thất bại | Yêu cầu cập nhật phương thức thanh toán |
| `subscription.active` | Kích hoạt lại sau khi thanh toán | Khôi phục truy cập |
...
<Accordion title="Payment deduction delayed beyond 48 hours">
**Nguyên nhân:** Sự chậm trễ của API ngân hàng có thể kéo dài quá trình xử lý thêm 2-3 giờ nữa.

**Giải pháp:** Đây là điều bình thường. Xây dựng hệ thống của bạn để xử lý các sự chậm trễ có thể thay đổi lên đến tổng cộng ~51 giờ.
</Accordion>

<Accordion title="Mandate cancelled but subscription still active">
**Nguyên nhân:** Trường hợp đặc biệt trong quy định RBI — việc hủy lệnh trong khoảng thời gian xử lý không ngay lập tức hủy đăng ký.

**Giải pháp:** Lần tính phí tiếp theo sẽ thất bại và đăng ký sẽ chuyển đến `on_hold`. Giám sát webhooks cho `payment.failed`.
</Accordion>
</AccordionGroup>
...
<Card title="Subscriptions" icon="repeat" href="/features/subscription">
Hoàn thành tài liệu đăng ký bao gồm các mệnh lệnh RBI.
</Card>
