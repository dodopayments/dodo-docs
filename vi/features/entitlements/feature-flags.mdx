...
- Khi mua hàng, grant được tạo và chuyển thẳng sang `Delivered`. Không có giai đoạn `Pending`, không cần khách hàng thực hiện thao tác nào và cũng không có khả năng delivery thất bại.
- Grant chứa payload có kiểu `feature`: `{ "feature_type": "boolean", "feature_id": "advanced_reports" }`. Ứng dụng của bạn đọc `feature_id` để quyết định cần mở khóa tính năng nào.
- Khi hủy, hoàn tiền hoặc revoke thủ công, grant được chuyển sang `Revoked` và ứng dụng của bạn nhận thấy flag biến mất.
...
Grant của feature flag tuân theo [vòng đời grant tiêu chuẩn](/features/entitlements/introduction#how-grants-work) với một điểm đơn giản hóa: không có bước delivery, vì vậy grant không bao giờ ở trạng thái `Pending` và cũng không bao giờ chuyển sang `Failed`.

| Trigger | Effect |
|---|---|
| Thanh toán một lần thành công / subscription chuyển sang trạng thái active | Grant được tạo với `status: delivered` và `delivered_at` được thiết lập. |
| Subscription bị tạm dừng, hủy hoặc hết hạn | Grant bị revoke với `revocation_reason` tương ứng. |
| Hoàn tiền cho một thanh toán một lần | Grant bị revoke với `revocation_reason: refund`. |
| Subscription được khôi phục (ví dụ: dunning thành công) | Grant đã bị revoke được khôi phục về `Delivered` — cùng `id` của grant, các trường revocation được xóa. |
| Revoke thủ công qua API | Grant bị revoke với `revocation_reason: manual`. Không tự động khôi phục khi gia hạn. |
...
- `entitlement_grant.created` — đã ở trạng thái `Delivered` và đi kèm payload `feature`. Bật tính năng.
- `entitlement_grant.delivered` — được kích hoạt khi một grant trước đó đã bị revoke được khôi phục. Bật lại tính năng.
- `entitlement_grant.revoked` — quyền truy cập bị thu hồi. Tắt tính năng và kiểm tra `revocation_reason` để quyết định nội dung thông báo.
...
2. **Gắn flag** vào sản phẩm subscription Pro Plan.
3. **Khách hàng đăng ký subscription.** Dodo Payments tạo grant `Delivered` và kích hoạt `entitlement_grant.created`; webhook handler của bạn bật `advanced_reports` cho khách hàng với giới hạn là 100.
4. **Ứng dụng kiểm soát quyền truy cập tính năng.** Khi tải dashboard, kiểm tra tập tính năng đã lưu trong cache (hoặc gọi `listEntitlementGrants`) và chỉ render tab báo cáo khi `advanced_reports` hiện diện.
5. **Khách hàng hủy subscription.** Dodo Payments revoke grant và kích hoạt `entitlement_grant.revoked`; handler của bạn tắt tính năng. Nếu sau đó khách hàng khôi phục subscription thông qua dunning, `entitlement_grant.delivered` sẽ khôi phục tính năng — không cần thay đổi code.

---

## Thực hành tốt nhất

- **Sử dụng feature id ổn định, theo snake_case.** Code ứng dụng của bạn kiểm tra các chuỗi này; việc đổi tên một feature id là breaking change ở cả hai phía.
- **Mỗi capability một flag.** Nên dùng `advanced_reports` + `api_access` làm hai entitlement thay vì một `pro_bundle` duy nhất — việc revoke và kết hợp các plan sẽ rõ ràng hơn.
- **Điều khiển state từ webhook, xác minh bằng API.** Webhook giúp database của bạn luôn cập nhật; endpoint list là nguồn dữ liệu chính xác để các job reconciliation và xử lý cache miss sử dụng.
- **Xử lý `Revoked` ngay lập tức.** Một flag đã bị revoke nghĩa là khách hàng không còn trả phí cho tính năng đó. Kiểm soát quyền truy cập ở request tiếp theo, không phải session tiếp theo.
- **Đặt giới hạn trong metadata, không đặt trong code.** Khi thay đổi quota, bạn chỉ cần chỉnh sửa entitlement — khách hàng mới sẽ tự động nhận giá trị này, trong khi các grant hiện có vẫn giữ snapshot đã mua.
