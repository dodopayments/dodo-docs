...
| Sự kiện | Tác động |
|---|---|
| Thanh toán một lần thành công / gói đăng ký trở thành active | Grant được tạo với `status: Delivered` và `delivered_at` được thiết lập. |
| Gói đăng ký bị tạm dừng, hủy hoặc hết hạn | Grant bị thu hồi với `revocation_reason` tương ứng. |
| Hoàn tiền cho một thanh toán một lần | Grant bị thu hồi với `revocation_reason: refund`. |
| Gói đăng ký được khôi phục (ví dụ: dunning thành công) | Grant đã bị thu hồi được khôi phục về `Delivered` — cùng grant `id`, các trường thu hồi được xóa. |
