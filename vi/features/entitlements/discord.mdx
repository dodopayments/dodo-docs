...
2. Dodo Payments tạo một grant ở trạng thái `Pending` với một `oauth_url` trỏ đến Discord.
3. Khách hàng nhận được email có nút "Tham gia Discord" (liên kết OAuth). Liên kết này cũng hiển thị trong Customer Portal của họ.
4. Sau khi khách hàng cấp quyền, bot sẽ thêm họ vào server và gán vai trò đã cấu hình. Grant chuyển sang `Delivered`.
5. Nếu subscription bị hủy, tạm dừng hoặc hết hạn, hoặc merchant thu hồi thủ công, bot sẽ xóa vai trò và grant chuyển sang `Revoked`.
...
- `entitlement_grant.created` được kích hoạt với `status: "Pending"` và một `oauth_url` để khách hàng cấp quyền.
- `entitlement_grant.delivered` được kích hoạt sau khi vai trò được gán.
- `entitlement_grant.revoked` được kích hoạt khi vai trò bị xóa.
