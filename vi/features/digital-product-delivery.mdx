...
Tải tệp (PDF, ZIP, hình ảnh, video, v.v.) lên tới 500 MiB. Các tệp được truyền và lưu trữ hiệu quả.
</Card>
...
| Sự kiện | Hành vi |
|---|---|
| `payment.succeeded` (một lần) | Cấp một grant. Grant này chứa các URL tải xuống presigned có hiệu lực trong khoảng 15 phút; khách hàng có thể làm mới chúng bằng cách mở lại liên kết trong email hoặc trang Customer Portal. |
| `subscription.active` | Cấp một grant. Các tệp vẫn có thể truy cập trong khi subscription còn hoạt động. |
| `subscription.renewed` | Không thực hiện thao tác nào. Grant hiện tại tiếp tục được sử dụng; các URL presigned mới được tạo ở mỗi lần fetch. |
| `subscription.on_hold` / `cancelled` / `expired` | Thu hồi grant. Không cấp URL presigned mới nữa. |
| `subscription.paused` | Thu hồi grant. Không cấp URL presigned mới cho đến khi subscription được tiếp tục. |
| `subscription.plan_changed` | Thu hồi grant cũ; cấp grant mới cho entitlement của plan mới. |
| `refund.succeeded` (một lần) | Thu hồi grant. |
| Thu hồi thủ công | Thu hồi bằng `revocation_reason: manual`. |
...
Thu hồi quyền truy cập của khách hàng một cách thủ công.
</Card>
</CardGroup>
...
<Note>
Các sản phẩm được cấu hình với khối `digital_product_delivery` cũ ngay trên sản phẩm đã được **tự động di chuyển** sang một entitlement Digital Files. Các tệp hiện có được tải lên thông qua product file API cũ vẫn được bảo toàn; chúng tiếp tục có thể được tải xuống và xuất hiện trong grant payload cùng với các tệp mới được tải lên. Trong cấu hình `digital_files` của entitlement, chúng được theo dõi trong danh sách `legacy_file_ids`. Các bản cập nhật trong tương lai (thêm tệp, thay đổi hướng dẫn, thay thế URL bên ngoài) nên được thực hiện bằng cách chỉnh sửa entitlement Digital Files đã được di chuyển trong **Entitlements**.
...
- **Xem xét tải xuống là một lần duy nhất.** Khách hàng sẽ chia sẻ hoặc mất liên kết, vì vậy hãy thiết kế sản phẩm của bạn dựa trên giả định rằng bất cứ thứ gì họ tải xuống là của họ.
- **Sử dụng hướng dẫn để đặt kỳ vọng.** Đối với các gói tệp nhiều, thêm một dòng `instructions` để giải thích cài đặt gì trước hoặc cách kết hợp các tệp.
- **Chú ý giới hạn 500 MiB.** Các vật phẩm lớn hơn (bộ dữ liệu nhiều GB, khóa học video) nên được lưu trữ bên ngoài và liên kết thông qua `external_url` thay vì tải lên.
- **Kết hợp với Khóa Giấy Phép để thu hồi quyền truy cập.** Nếu bạn cần thu hồi quyền truy cập vào các tính năng trong sản phẩm sau khi hoàn tiền, kết hợp quyền truy cập Tệp Kỹ thuật số với quyền Khóa Giấy Phép và xác thực khóa khi chạy.
- **Kiểm tra luồng làm mới cổng khách hàng.** Xác nhận khách hàng có thể trở lại cổng một tuần sau đó và vẫn nhận được liên kết tải xuống hoạt động. Đây là con đường khôi phục chính khi các liên kết email hết hạn.
