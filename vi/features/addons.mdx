---
sidebarTitle: Add-ons
title: Add-ons for Subscriptions
description: Enhance your subscription products with flexible add-ons for
  seat-based billing, feature upgrades, and creative pricing models
icon: puzzle
keywords:
  - Dodo Payments
  - tiện ích bổ sung cho đăng ký
  - nền tảng thanh toán
  - hóa đơn SaaS
---

<Info>
Add-ons are additional products that can be attached to your main subscription products, enabling flexible pricing models and enhanced customer experiences. Whether you need seat-based billing, feature upgrades, or custom pricing structures, add-ons give you the power to create sophisticated subscription offerings.
</Info>

<CardGroup cols={2}>
<Card title="Seat-Based Billing" icon="users" href="/developer-resources/seat-based-pricing">
Create add-ons for additional team seats, user licenses, or capacity upgrades with per-seat pricing.
</Card>

<Card title="Usage Extensions" icon="bolt" href="/features/usage-based-billing/introduction">
Mở rộng giới hạn sử dụng, cuộc gọi API hoặc mức dữ liệu với giá bổ sung linh hoạt.
</Card>

</CardGroup>

## Add-ons là gì?

Add-ons là các sản phẩm bổ sung mà khách hàng có thể mua cùng với gói đăng ký chính của họ. Chúng hoàn hảo cho:

- **Seat-based billing**: Additional team members, user licenses, or concurrent users
- **Feature upgrades**: Premium features, advanced analytics, or priority support
- **Usage extensions**: Extra storage, API calls, or bandwidth allowances
- **Service add-ons**: Professional services, training, or consultation hours

<Frame>
<img src="/images/cookbooks/seat-based/seat-based-addons.png" alt="Các tiện ích bổ sung gắn với sản phẩm đăng ký trong bảng điều khiển" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>

## Lợi ích chính

- **Flexible Pricing Models**: Cung cấp các gói cơ bản với các add-on tùy chọn để tạo ra các cấu trúc định giá tinh vi. Bạn có thể phục vụ các phân khúc khách hàng đa dạng với những nâng cấp phát triển theo nhu cầu của họ.
- **Revenue Optimization**: Tăng doanh thu trung bình trên mỗi người dùng (ARPU) bằng cách trình bày các add-on phù hợp. Điều này tạo cơ hội upsell tự nhiên khi khách hàng bổ sung tính năng theo thời gian.
- **Simplified Management**: Quản lý tất cả các thành phần định giá từ một bảng điều khiển.Các add-on được tự động bao gồm trong cả phiên thanh toán và quản lý đăng ký.
- **Customer Choice**: Cho phép khách hàng tùy chỉnh đăng ký bằng cách chọn chỉ những add-on họ cần, điều này nâng cao sự hài lòng và giảm tỷ lệ churn.

## Tạo Add-on

Add-on được tạo dưới dạng các sản phẩm riêng biệt trong bảng điều khiển Dodo Payments của bạn và sau đó được gắn vào các sản phẩm đăng ký chính. Sự tách biệt này cho phép bạn:

- Tái sử dụng add-on cho nhiều sản phẩm đăng ký khác nhau
- Quản lý giá độc lập
- Theo dõi hiệu suất add-on riêng biệt
- Cập nhật add-on mà không ảnh hưởng đến các đăng ký cơ bản

<Frame>
<img src="/images/cookbooks/seat-based/seat-based-addons-creation.png" alt="Tạo các tiện ích bổ sung trong giao diện bảng điều khiển" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>

### Cấu hình Add-on

Khi tạo add-on, bạn có thể cấu hình:

- **Định giá**: Thiết lập mức giá một lần hoặc định kỳ cho tiện ích bổ sung
- **Tiền tệ**: Thiết lập giá cơ sở bằng bất kỳ loại tiền tệ nào mà Dodo Payments có thể tính phí. Trình chọn tiền tệ có thể tìm kiếm ghim **USD, GBP, EUR, và INR** lên đầu; khách hàng sử dụng loại tiền tệ khác với tiền tệ cơ sở của bạn sẽ được tính phí thông qua [Adaptive Currency](/features/adaptive-currency)
- **Chu kỳ thanh toán**: Phải khớp với chu kỳ thanh toán của gói đăng ký
- **Giới hạn số lượng**: Thiết lập số lượng tối thiểu và tối đa cho mỗi khách hàng
- **Tính khả dụng**: Kiểm soát những sản phẩm gói đăng ký nào có thể sử dụng tiện ích bổ sung
- **Cài đặt thuế**: Cấu hình các danh mục thuế phù hợp

### Bắt đầu

Sẵn sàng triển khai add-on trong doanh nghiệp đăng ký của bạn? Dưới đây là cách bắt đầu:

<Steps>
<Step title="Plan Your Add-ons">
Xác định các tính năng, dịch vụ hoặc khả năng bổ sung sẽ mang lại lợi ích cho khách hàng của bạn khi được cung cấp dưới dạng add-on.

Cân nhắc:
- Điều gì khách hàng thường yêu cầu?
- Những tính năng nào có thể được kiếm tiền riêng biệt?
- Điều gì sẽ tạo ra con đường nâng cấp tự nhiên?
</Step>

<Step title="Create Your First Add-on">
Sử dụng bảng điều khiển hoặc API Dodo Payments để tạo sản phẩm add-on đầu tiên của bạn.

<Card title="Dashboard Guide" icon="box" href="/developer-resources/seat-based-pricing">
Làm theo hướng dẫn từng bước của chúng tôi để tạo add-on trong bảng điều khiển.
</Card>
</Step>

<Step title="Attach to Subscriptions">
Kết nối add-on của bạn với các sản phẩm đăng ký phù hợp, nơi chúng nên khả dụng.
</Step>

<Step title="Test Integration">
Tạo các phiên thanh toán thử nghiệm với các tổ hợp add-on khác nhau để đảm bảo mọi thứ hoạt động đúng.
</Step>

<Step title="Monitor Performance">
Theo dõi tỷ lệ chấp nhận và tác động doanh thu của add-on để tối ưu hóa chiến lược định giá của bạn.
</Step>
</Steps>

## Quản lý API

Dodo Payments cung cấp một API toàn diện để quản lý add-on theo lập trình:

<AccordionGroup>
<Accordion title="Create Add-ons">
Sử dụng điểm cuối `POST /addons` để tạo add-on mới với giá tùy chỉnh, mô tả và tùy chọn cấu hình.

<Card title="API Reference" icon="code" href="/api-reference/addons/create-addon">
Xem tài liệu API đầy đủ để tạo add-on.
</Card>
</Accordion>

<Accordion title="Update Add-ons">
Sửa đổi add-on hiện tại bằng cách sử dụng điểm cuối `PATCH /addons/{id}` để cập nhật giá, mô tả hoặc khả năng sẵn có.

<Card title="API Reference" icon="code" href="/api-reference/addons/update-addon">
Tìm hiểu cách cập nhật chi tiết add-on theo lập trình.
</Card>
</Accordion>

<Accordion title="List and Retrieve">
Sử dụng `GET /addons` để liệt kê tất cả add-on hoặc `GET /addons/{id}` để truy xuất chi tiết add-on cụ thể.

<Card title="API Reference" icon="code" href="/api-reference/addons/list-addons">
Truy cập tài liệu API đầy đủ về danh sách và truy xuất.
</Card>
</Accordion>

<Accordion title="Image Management">
Cập nhật hình ảnh add-on bằng điểm cuối `PUT /addons/{id}/images` để trình bày sản phẩm tốt hơn.

<Card title="API Reference" icon="code" href="/api-reference/addons/update-addon-images">
Tìm hiểu cách quản lý hình ảnh add-on thông qua API.
</Card>
</Accordion>
</AccordionGroup>

## Các trường hợp sử dụng phổ biến

- **Seat-Based Billing**: Additional team members, user licenses, or concurrent users
- **Feature Upgrades**: Premium features, advanced analytics, or priority support  
- **Usage Extensions**: Extra storage, API calls, or bandwidth allowances
- **Service Add-ons**: Professional services, training, or consultation hours

## Ví dụ tích hợp

### Phiên thanh toán có Add-on
Khi tạo các phiên thanh toán, bạn có thể bao gồm add-on với số lượng tùy chỉnh:

```typescript
const session = await client.checkoutSessions.create({
  product_cart: [
    {
      product_id: 'your_subscription_id',
      quantity: 1,
      addons: [
        {
          addon_id: 'your_addon_id',
          quantity: 3 // 3 additional seats
        }
      ]
    }
  ],
  // ... other checkout options
});
```

<Tip>
Để cho phép khách hàng tự thêm hoặc loại bỏ tiện ích bổ sung trên trang thanh toán, đặt cờ tính năng [`allow_editing_addons`](/developer-resources/checkout-session#optional-fields) thành `true` khi tạo phiên. Mặc định là `false` và chỉ áp dụng cho các sản phẩm đăng ký.

```typescript
const session = await client.checkoutSessions.create({
  product_cart: [{ product_id: 'your_subscription_id', quantity: 1 }],
  feature_flags: {
    allow_editing_addons: true // customers can edit add-ons at checkout
  },
});
```

</Tip>

### Thay đổi Gói với Tiện ích Bổ sung
Chỉnh sửa các đăng ký hiện có để thêm, xóa hoặc cập nhật tiện ích bổ sung:

```typescript
// Add add-ons to existing subscription
await client.subscriptions.changePlan('sub_123', {
  product_id: 'prod_new',
  quantity: 1,
  proration_billing_mode: 'difference_immediately',
  addons: [
    { addon_id: 'addon_123', quantity: 2 }
  ]
});

// Remove all existing add-ons
await client.subscriptions.changePlan('sub_123', {
  product_id: 'prod_new',
  quantity: 1,
  proration_billing_mode: 'difference_immediately',
  addons: [] // Empty array removes all existing add-ons
});
```

### Định giá Động
Tính toán tổng chi phí một cách động dựa trên lựa chọn tiện ích bổ sung:

```typescript
function calculateTotalCost(basePrice: number, addons: AddonSelection[]) {
  const addonTotal = addons.reduce((sum, addon) => 
    sum + (addon.price * addon.quantity), 0
  );
  return basePrice + addonTotal;
}
```

## Thực Tiễn Tốt Nhất

- **Bắt đầu đơn giản**: Ra mắt với 2-3 tiện ích bổ sung cơ bản và mở rộng tùy chọn dựa trên phản hồi và mức sử dụng của khách hàng.
- **Duy trì tính rõ ràng về giá**: Truyền đạt rõ ràng giá cả và giá trị của tiện ích bổ sung để khách hàng hiểu họ sẽ nhận được gì với chi phí bổ sung.
- **Thử nghiệm kỹ lưỡng**: Xác nhận các tổ hợp tiện ích bổ sung để đảm bảo rằng tính toán giá vẫn chính xác và luồng thanh toán hoạt động trơn tru.

### Cân nhắc Thiết kế

- **Giá Trị Rõ Ràng**: Mỗi tiện ích bổ sung cần có lợi ích rõ ràng mà khách hàng dễ dàng hiểu
- **Nhóm Hợp Lý**: Nhóm các tiện ích bổ sung liên quan lại với nhau trong luồng thanh toán của bạn
- **Số Lượng Linh Hoạt**: Cho phép khách hàng điều chỉnh số lượng tiện ích bổ sung khi cần
- **Giá cả Minh Bạch**: Hiển thị tổng chi phí rõ ràng trong suốt quá trình thanh toán

<Info>
Tiện ích bổ sung là cách mạnh mẽ để tạo ra mô hình định giá linh hoạt, có thể mở rộng cùng với khách hàng của bạn. Bắt đầu với các trường hợp đơn giản và mở rộng khi bạn học được điều gì hoạt động tốt nhất cho doanh nghiệp và khách hàng của bạn.
</Info>
