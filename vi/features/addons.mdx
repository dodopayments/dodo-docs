...
Sử dụng endpoint `POST /addons` để tạo các add-ons mới với giá tùy chỉnh, mô tả và tùy chọn cấu hình.
...
Chỉnh sửa các add-ons hiện có bằng cách sử dụng endpoint `PATCH /addons/{id}` để cập nhật giá, mô tả hoặc tính khả dụng.
...
Sử dụng `GET /addons` để liệt kê tất cả các add-ons hoặc `GET /addons/{id}` để lấy thông tin chi tiết về add-on cụ thể.
...
Cập nhật hình ảnh add-on bằng cách sử dụng endpoint `PUT /addons/{id}/images` để cải thiện trình bày sản phẩm.
