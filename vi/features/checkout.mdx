...
<Info>
Phần này đề cập đến cấu hình chủ đề **API phía máy chủ** sử dụng `customization.theme_config`. Nếu bạn đang sử dụng **Checkout SDK** (overlay hoặc inline checkout), hãy xem phần tùy chỉnh chủ đề trong [Overlay Checkout](/developer-resources/overlay-checkout#theme-customization) sử dụng các thuộc tính camelCase (ví dụ: `bgPrimary` thay vì `bg_primary`).
</Info>
