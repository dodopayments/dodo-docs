...
Định nghĩa credits bằng đơn vị riêng của bạn - token, API calls, giờ compute hoặc bất kỳ chỉ số nào có ý nghĩa đối với sản phẩm của bạn. Các đơn vị tùy chỉnh sử dụng độ chính xác mà bạn thiết lập (từ 0 đến 10 chữ số thập phân).
...
| Nguồn | Mô tả |
|--------|-------------|
| **Gói đăng ký** | Tín dụng được cấp khi mua gói đăng ký và được cấp lại trong mỗi chu kỳ thanh toán |
| **Một lần** | Tín dụng được cấp với sản phẩm thanh toán một lần |
| **Add-on** | Tín dụng được cấp với add-on được gắn vào gói đăng ký |
| **API** | Tín dụng được cấp thủ công qua API hoặc dashboard |
| **Chuyển tiếp** | Tín dụng được chuyển tiếp từ chu kỳ thanh toán trước |
...
Chỉ áp dụng cho credits của Custom Unit. Số chữ số thập phân được phép, từ `0` đến `10`:

- `0` - Số nguyên (phù hợp nhất cho các mục có thể đếm như API calls)
- `1` - Một chữ số thập phân (0.0)
- `2` - Hai chữ số thập phân (0.00) - **mặc định**
- `3` - Ba chữ số thập phân (0.000)
- Tối đa `10` - Dành cho các đơn vị yêu cầu độ chính xác cao (ví dụ: token phân số hoặc mức sử dụng siêu nhỏ)
...
Tín dụng được gắn vào sản phẩm như **các quyền lợi** trong quá trình tạo hoặc chỉnh sửa sản phẩm. Bạn có thể gắn tối đa **5 tín dụng cho mỗi sản phẩm**. Tín dụng hoạt động với cả ba loại giá.
...
<Info>
Bạn có thể gắn tối đa 5 tín dụng cho mỗi sản phẩm. Mỗi tín dụng có thể có cấu hình riêng.
</Info>
</Step>
...
1. **Ứng dụng của bạn gửi các sự kiện sử dụng** - Mỗi sự kiện bao gồm ID khách hàng, tên sự kiện và metadata
2. **Các bộ đếm tổng hợp sự kiện** - Sử dụng Count, Sum, Max hoặc Last aggregation
3. **Tín dụng được khấu trừ tự động** - Một công việc nền xử lý các sự kiện mỗi phút, chuyển đổi đơn vị bộ đếm thành tín dụng theo tỷ lệ bạn đã cấu hình, và khấu trừ từ số dư khách hàng theo thứ tự FIFO (ưu tiên các khoản cấp phát cũ nhất trước)
4. **Theo dõi chi tiêu vượt mức** - Nếu số dư tín dụng bằng không và chi tiêu vượt mức được kích hoạt, hệ thống sẽ theo dõi việc sử dụng vượt mức cho việc lập hóa đơn cuối kỳ
...
| **Sum** | Tổng của trường giá trị | Tổng số bytes được truyền tải |
| **Max** | Giá trị cao nhất ghi lại | Số người dùng đồng thời cao nhất |
| **Last** | Giá trị mới nhất | Dung lượng lưu trữ hiện tại |
...
|-----------------|-------------|
| **Tín dụng được thêm** | Tín dụng được cấp (từ gói đăng ký, giao dịch một lần hoặc API) |
| **Tín dụng bị trừ** | Tín dụng được sử dụng thông qua việc sử dụng hoặc ghi nợ thủ công |
| **Tín dụng hết hạn** | Tín dụng hết hạn mà không được chuyển tiếp |
| **Tín dụng được chuyển tiếp** | Tín dụng được chuyển sang kỳ tiếp theo |
| **Tín dụng chuyển tiếp bị mất** | Tín dụng được chuyển tiếp bị mất sau khi đạt số lần chuyển tiếp tối đa |
| **Tính phí vượt mức** | Mức sử dụng vượt quá số dư tín dụng khi đã bật tính năng vượt mức |
| **Đặt lại mức vượt mức** | Mức vượt mức được theo dõi bị xóa khi đặt lại chu kỳ thanh toán |
| **Tự động nạp thêm** | Tự động bổ sung tín dụng khi số dư thấp |
| **Điều chỉnh thủ công** | Khoản tín dụng hoặc ghi nợ được merchant áp dụng thủ công |
| **Hoàn tiền** | Tín dụng được hoàn tiền |
...
| `credit.deducted` | Tín dụng được sử dụng thông qua việc sử dụng hoặc ghi nợ thủ công |
| `credit.expired` | Tín dụng chưa sử dụng đã hết hạn |
| `credit.rolled_over` | Tín dụng được chuyển sang một khoản cấp mới |
| `credit.rollover_forfeited` | Tín dụng bị mất khi đạt số lần chuyển tiếp tối đa |
| `credit.overage_charged` | Phí vượt mức được áp dụng |
| `credit.overage_reset` | Mức vượt mức được theo dõi đã được đặt lại vào cuối chu kỳ thanh toán |
| `credit.manual_adjustment` | Đã thực hiện điều chỉnh tín dụng/ghi nợ thủ công |
| `credit.balance_low` | Số dư đã giảm xuống dưới ngưỡng được cấu hình |

Tất cả các sự kiện sổ cái (mọi sự kiện `credit.*` ngoại trừ `credit.balance_low`) đều bao gồm payload `CreditLedgerEntry` đầy đủ với số dư trước/sau, mức vượt mức trước/sau, tham chiếu nguồn và `metadata` của gói đăng ký hoặc khoản thanh toán nguồn của khoản cấp; trường này để trống đối với các khoản cấp được tạo trực tiếp qua API. Sự kiện `credit.balance_low` bao gồm cấu hình ngưỡng và số dư hiện tại.
