...
| Tiếp tục Subscription đã tạm dừng | `subscription.unpaused` |
| Tạo thất bại | `subscription.failed` |
| Nâng cấp/hạ cấp plan | `subscription.plan_changed` |
| Đã hủy | `subscription.cancelled` |
| Kết thúc kỳ hạn | `subscription.expired` |
| Bất kỳ trường nào thay đổi | `subscription.updated` |
...
Sử dụng `PATCH /subscriptions/{subscription_id}` để hủy vào ngày thanh toán tiếp theo, kéo dài thời hạn Subscription, cập nhật thông tin thanh toán hoặc chỉnh sửa metadata. Để thay đổi số lượng, hãy sử dụng [API Thay đổi Plan](/api-reference/subscriptions/change-plan) — `PATCH` không chấp nhận `quantity`.
...
Kéo dài thời gian hoạt động của Subscription bằng cách truyền `subscription_period_count` và `subscription_period_interval` mới vào `PATCH /subscriptions/{subscription_id}`. Thời điểm hết hạn của Subscription được tính lại dựa trên số lượng và khoảng thời gian mới — ví dụ: để cấp thêm thời gian cho khách hàng trong plan hiện tại của họ:
