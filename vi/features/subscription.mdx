...
| `active` | Subscription đang hoạt động và sẽ tự động gia hạn | — | Không cần thực hiện hành động nào |
| `on_hold` | Khoản thanh toán gia hạn (hoặc khoản phí do thay đổi plan) không thành công; các lần gia hạn đã dừng | **Có** | Cập nhật phương thức thanh toán để kích hoạt lại — tự động thông qua [Payment Retries](/features/recovery/payment-retries) và [Dunning](/features/recovery/subscription-dunning), hoặc thủ công thông qua [Update Payment Method API](/api-reference/subscriptions/update-payment-method) |
| `paused` | Subscription đã được bạn hoặc customer chủ động tạm dừng; hoạt động lập hóa đơn bị đóng băng và quyền truy cập bị thu hồi | **Có** | Tiếp tục subscription bằng `status: active` hoặc từ Customer Portal khi tính năng tự phục vụ tạm dừng được bật — xem [Pausing and Resuming Subscriptions](#pausing-and-resuming-subscriptions) |
| `cancelled` | Subscription đã bị hủy và sẽ không gia hạn | Chỉ mua lại | Customer phải bắt đầu một subscription mới; [Dunning](/features/recovery/subscription-dunning) có thể nhắc customer mua lại |
| `failed` | **Việc tạo** subscription không thành công (mandate ban đầu hoặc khoản thanh toán không thành công) | **Không — trạng thái kết thúc** | Customer phải tạo một subscription mới bằng phương thức thanh toán đang hoạt động |
| `expired` | Subscription đã đến cuối kỳ hạn | — | Customer phải bắt đầu một subscription mới nếu có nhu cầu |
