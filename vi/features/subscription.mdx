...
Sử dụng `PATCH /subscriptions/{id}` để hủy vào ngày thanh toán tiếp theo, kéo dài kỳ subscription, cập nhật thông tin thanh toán hoặc chỉnh sửa metadata. Để thay đổi số lượng, hãy sử dụng [Change Plan API](/api-reference/subscriptions/change-plan) thay thế — `PATCH` không chấp nhận `quantity`.
