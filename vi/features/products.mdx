...
Đo lường tiêu thụ và hóa đơn theo sử dụng thực tế.
</Card>
</CardGroup>
...
<Frame>
  <img src="/images/products.png" alt="Sản phẩm" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
- **Giá**: Số tiền cơ bản và tiền tệ.
- **Giảm giá (%)**: Giảm giá tùy chọn được hiển thị trong thanh toán và hóa đơn.
- Đối với đăng ký, thiết lập **Lặp lại mỗi** (ví dụ: 1 tháng hoặc 1 năm) và **Số ngày dùng thử** nếu cần. Giá đăng ký phải ít nhất là **1 đô la** (hoặc tương đương trong tiền tệ bạn chọn); các khoản tiền dưới mức tối thiểu này không được hỗ trợ và đăng ký sẽ không hoạt động.
...
<Frame>
  <img src="/images/product-collection/checkout-page.png" alt="Bộ sưu tập sản phẩm" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
Đối với sản phẩm định kỳ, đặt đối tượng `price` thành giá định kỳ (`type: recurring_price`) và chỉ định `payment_frequency_interval` và `subscription_period_interval` (`Day`, `Week`, `Month`, hoặc `Year`) cùng với số lượng tương ứng.
...
Nếu bạn không muốn hiển thị hoặc sử dụng sản phẩm nữa, bạn có thể lưu trữ nó bằng một yêu cầu `DELETE` tới endpoint `/products/{id}`. Hành động này sẽ ẩn sản phẩm nhưng không xóa vĩnh viễn.
