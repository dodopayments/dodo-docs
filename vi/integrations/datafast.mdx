...
1. **Lấy ID khách truy cập của DataFast** từ cookie `datafast_visitor_id` khi tạo phiên giao dịch thanh toán
...
<Step title="Cài đặt Script DataFast">
  Đầu tiên, cài đặt script theo dõi DataFast trên trang web của bạn. Điều này tạo ra cookie `datafast_visitor_id` mà theo dõi khách truy cập của bạn.
  
  Truy cập [tài liệu của DataFast](https://datafa.st/docs) để biết hướng dẫn cài đặt cụ thể cho nền tảng của bạn.
</Step>
...
<AccordionGroup>
<Accordion title="Các khoản thanh toán không xuất hiện trong DataFast">
- Xác minh rằng khóa API của DataFast là chính xác và đang hoạt động
- Kiểm tra xem `datafast_visitor_id` có đang được ghi lại và lưu trữ trong siêu dữ liệu thanh toán hay không
- Đảm bảo rằng chuyển đổi webhook đang định dạng đúng dữ liệu đầu vào
- Xác minh rằng webhook đang kích hoạt trên các sự kiện `payment.succeeded`
- Kiểm tra bảng điều khiển DataFast để xem có thông báo lỗi hay nhật ký API nào không
</Accordion>

<Accordion title="Phân bổ doanh thu không hoạt động">
- Xác nhận rằng script theo dõi DataFast đã được cài đặt và đang hoạt động trên trang web của bạn
- Xác minh rằng cookie `datafast_visitor_id` đang được thiết lập chính xác
- Kiểm tra xem ID khách truy cập có khớp giữa việc tạo giao dịch thanh toán và hoàn tất thanh toán hay không
- Đảm bảo bạn đang ghi lại ID khách truy cập trước khi tạo phiên giao dịch thanh toán
- Xem xét [tài liệu API thanh toán của DataFast](https://datafa.st/docs/payments-api) để có thêm hướng dẫn
</Accordion>

<Accordion title="Lỗi chuyển đổi">
- Xác nhận rằng cấu trúc JSON khớp với định dạng API thanh toán của DataFast
- Kiểm tra rằng tất cả các trường bắt buộc (`amount`, `currency`, `transaction_id`, `datafast_visitor_id`) đều có mặt
- Đảm bảo rằng số tiền được chuyển đổi đúng cách (chia cho 100 cho hầu hết các loại tiền tệ, ngoại trừ các loại tiền tệ không thập phân)
- Xác minh rằng URL điểm cuối API là chính xác: `https://datafa.st/api/v1/payments`
- Kiểm tra chuyển đổi với các dữ liệu mẫu của webhook
</Accordion>
