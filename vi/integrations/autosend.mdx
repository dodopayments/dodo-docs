...
- Xác minh rằng API Key là chính xác và đang hoạt động
- Kiểm tra rằng miền người gửi đã được xác minh trong AutoSend
- Đảm bảo rằng địa chỉ email của người nhận hợp lệ
- Xem xét các giới hạn và chỉ tiêu gửi của AutoSend
- Xác minh rằng URL điểm cuối API là chính xác: `https://api.autosend.com/v1/mails/send`
- Kiểm tra rằng các tham số cần thiết có trong payload
</Accordion>

<Accordion title="Lỗi chuyển đổi">
- Xác thực cấu trúc JSON khớp với định dạng API AutoSend
- Kiểm tra rằng tất cả các trường yêu cầu đều có mặt (`to`, `from`, `templateId` hoặc `html`/`text`)
- Đảm bảo các địa chỉ email được định dạng chính xác
