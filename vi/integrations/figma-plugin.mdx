...
Tạo dự án plugin Figma của riêng bạn bằng cách sử dụng [hướng dẫn khởi động nhanh plugin của Figma](https://www.figma.com/plugin-docs/plugin-quickstart-guide/). Điều này sẽ tạo ra một khung với tệp `manifest.json`.
...
Nhân bản kho [dodopayments-figma](https://github.com/dodopayments/dodopayments-figma) và cập nhật `manifest.json` của nó với tên và ID từ thư mục plugin đã tạo của bạn.
...
- Cập nhật `Authenticated.tsx` để mở rộng chức năng cho người dùng có khóa giấy phép hợp lệ
- Tùy chỉnh `LicenseKeyInput.tsx` để phù hợp với sở thích thiết kế của bạn

<Note>
Component `Authenticated.tsx` là nơi bạn sẽ thêm các tính năng premium của plugin yêu cầu giấy phép hợp lệ.
</Note>
</Step>
</Steps>
...
<Steps>
<Step title="Cấu Hình Quyền Truy Cập Mạng">
Plugin cần quyền truy cập mạng để giao tiếp với proxy CORS. Thêm vào `manifest.json` như sau:
...
<Tip>
Chuyển sang `live_mode` chỉ khi bạn sẵn sàng cho sản xuất và đã thử nghiệm kỹ lưỡng trong chế độ thử nghiệm.
</Tip>
</Step>
</Steps>
...
2. Thử nghiệm kỹ lưỡng trong cả chế độ thử nghiệm và trực tiếp
