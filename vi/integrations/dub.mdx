...
Dub theo dõi khách truy cập qua một ID nhấp chuột độc đáo (`dub_id`) được lưu trữ trong cookie khi người dùng nhấp vào các liên kết ngắn của bạn. Để gán doanh số bán hàng cho các liên kết của bạn, bạn cần:

1. **Ghi lại ID nhấp chuột của Dub** từ cookie `dub_id` khi tạo phiên giao dịch thanh toán
2. **Lưu ID nhấp chuột** trong siêu dữ liệu thanh toán của bạn cùng với ID bên ngoài của khách hàng
3. **Gửi dữ liệu bán hàng cho Dub** khi thanh toán thành công bằng cách sử dụng Track API của họ
...
<Step title="Nhận Khoá API Dub của bạn">
  Điều hướng đến [bảng điều khiển Dub của bạn](https://app.dub.co) → Cài đặt → Khoá API và tạo một khoá API mới với phạm vi `conversions.write`.
...
- **Luôn bao gồm ID nhấp chuột trong siêu dữ liệu**: Nếu không có ID nhấp chuột, Dub không thể gán doanh thu cho các liên kết của bạn
- **Sử dụng ID bên ngoài một cách nhất quán**: Truyền ID khách hàng mà bạn sử dụng trong hệ thống của bạn để phân tích ở cấp độ khách hàng chính xác
- **Xử lý lưu lượng truy cập tự nhiên một cách duyên dáng**: Đặt `webhook.cancel = true` khi không có ID nhấp chuột để tránh các cuộc gọi API không cần thiết
- **Kiểm tra với các khoản thanh toán mẫu**: Xác minh rằng tích hợp hoạt động đúng trước khi chính thức ra mắt
- **Giám sát bảng điều khiển Dub của bạn**: Kiểm tra rằng doanh số bán hàng xuất hiện chính xác với việc gán đúng
...
- Xác minh rằng khoá API Dub của bạn là chính xác và có phạm vi `conversions.write`
- Kiểm tra rằng `dub_click_id` đang được ghi lại và lưu trữ trong siêu dữ liệu thanh toán
- Đảm bảo rằng quá trình chuyển đổi webhook đang định dạng tải trọng một cách đúng đắn
- Xác minh rằng webhook đang kích hoạt trên các sự kiện `payment.succeeded`
- Xác nhận rằng theo dõi chuyển đổi đã được bật cho các liên kết Dub của bạn
</Accordion>

<Accordion title="Gán doanh thu không hoạt động">
- Xác nhận rằng người dùng đang nhấp vào các liên kết ngắn Dub của bạn trước khi thanh toán
- Xác minh rằng cookie `dub_id` đang được thiết lập một cách chính xác trên miền của bạn
- Kiểm tra rằng các ID nhấp chuột khớp giữa việc tạo phiên giao dịch và hoàn tất thanh toán
- Đảm bảo rằng bạn đang ghi lại ID nhấp chuột trước khi tạo phiên giao dịch
</Accordion>

<Accordion title="Lỗi chuyển đổi">
- Xác thực rằng cấu trúc JSON khớp với định dạng Track Sale API của Dub
- Kiểm tra rằng tất cả các trường cần thiết (`clickId`, `externalId`, `amount`) đều có mặt
- Đảm bảo số tiền ở dạng xu (số nguyên, không phải số thập phân)
- Xác minh rằng URL điểm cuối API là chính xác: `https://api.dub.co/track/sale`
- Kiểm tra quá trình chuyển đổi với các tải trọng webhook mẫu
</Accordion>

<Accordion title="Doanh số được theo dõi trùng lặp">
- Đảm bảo rằng bạn chỉ theo dõi trên các sự kiện `payment.succeeded`, không phải `payment.processing`
- Sử dụng các giá trị `invoiceId` duy nhất cho mỗi giao dịch bán hàng
- Đối với các đăng ký, thêm dấu thời gian hoặc kỳ thanh toán để tránh trùng lặp khi gia hạn
</Accordion>
</AccordionGroup>
...
<Info>
Cần giúp đỡ? Liên hệ với bộ phận hỗ trợ Dodo Payments tại [support@dodopayments.com](mailto:support@dodopayments.com) để được hỗ trợ về việc tích hợp.
</Info>
