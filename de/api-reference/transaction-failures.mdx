...
Ein Karte kann auch abgelehnt werden, wenn die Risikomotor der ausstellenden Bank den Karteninhaber als Hochrisikokunden einstuft – unabhängig vom Händler oder den Transaktionsdetails. Diese Ablehnungen erscheinen typischerweise als allgemeine Codes wie `DO_NOT_HONOR`, `GENERIC_DECLINE`, `CARD_DECLINED`, `TRANSACTION_NOT_APPROVED`, oder `FRAUDULENT`. In diesen Fällen teilt die Bank keinen spezifischen Grund mit, und weder Dodo Payments noch der Händler können die Entscheidung übersteuern. Bitten Sie den Kunden, seine Bank zu kontaktieren, um das Problem zu klären, oder eine andere Karte oder Zahlungsmethode zu verwenden.

## Fehler programmatisch behandeln

Lesen Sie `error_code` aus dem `payment.failed` Webhook oder dem Zahlungsobjekt, ordnen Sie es der empfohlenen Aktion oben zu und entscheiden Sie, ob ein erneuter Versuch unternommen werden soll. Bei Abonnementverlängerungen werden weiche Ablehnungen automatisch für Sie erneut versucht – siehe [Abonnement Zahlungswiederholungen](/features/recovery/payment-retries).

Für API-Ebene und Geschäftslogikfehler (wie `PAYMENT_NOT_SUCCEEDED` oder `REFUND_WINDOW_EXPIRED`), die keine Kartenablehnungen sind, siehe die [Fehlercodes](/api-reference/error-codes) Referenz.

## Verwandt

End-to-End-Leitfaden zum Erkennen, Anzeigen und Wiederholen fehlgeschlagener Zahlungen.

API- und Geschäftslogik-Fehlercodes für Fehler außerhalb von Ablehnungen.

Automatische Wiederholungen, die weiche Ablehnungen bei Abonnementverlängerungen beheben.

Email-Sequenzen, die harte Ablehnungen beheben, indem sie zur Aktualisierung der Zahlungsmethode auffordern.

## Support

Für zusätzliche Hilfe zu Transaktionsfehlern oder Integrationsproblemen, kontaktieren Sie bitte unser Support-Team unter support@dodopayments.com.