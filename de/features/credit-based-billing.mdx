...
Initialisieren Sie den Dodo Payments-Client:
...
Gutschriften während des Checkouts an ein Abonnementprodukt anhängen:
...
Senden Sie Nutzungsereignisse, die Gutschriften automatisch abbuchen:
...
| Plan | Preis | Credits/Monat | Überziehung |
|------|-------|---------------|-------------|
...
**Konfiguration:**
- Gutschriftart: benutzerdefinierte Einheit ("AI Tokens")
- Genauigkeit: 0 (ganze Tokens)
- Übertrag: max. 25 %, 1-monatiger Zeitraum
- Überziehung: aktiviert, Überziehungen werden zur Abrechnung hinzugefügt
- Meter: `ai.generation` mit Summenaggregation auf dem `tokens`-Feld
</Accordion>
...
**Konfiguration:**
- Gutschriftart: benutzerdefinierte Einheit ("API Calls")
- Genauigkeit: 0 (ganze Calls)
- Übertrag: deaktiviert
- Überziehung: Developer+-Pläne erlauben Überziehungen (werden beim Zurücksetzen erlassen), Kostenloser Plan deaktiviert Überziehungen
- Meter: `api.request` mit Zählaggregation
</Accordion>
...
**Konfiguration:**
- Gutschriftart: benutzerdefinierte Einheit ("GB-hours")
- Genauigkeit: 2 (zwei Dezimalstellen)
- Übertrag: max. 50 %, wird einmal übertragen
- Überziehung: aktiviert mit 200 %-Limit
- Meter: `storage.usage` mit Summenaggregation
</Accordion>
</AccordionGroup>
...
- **Einfach anfangen**: Beginnen Sie mit einem einzigen Gutschrifttyp und ohne Übertrag. Fügen Sie Komplexität basierend auf Kundenfeedback und Nutzungsmustern hinzu.
- **Klare Erwartungen setzen**: Zeigen Sie Kreditzuweisungen, verbleibende Guthaben und Überziehungspreise deutlich auf Ihren Produktseiten und im Kundenportal an.
- **Sinnvolle Einheiten verwenden**: Benennen Sie Gutschriften nach dem, was sie darstellen (z. B. „API Calls“, „AI Tokens“) statt generischer Begriffe. Dies hilft Kunden, den Wert zu verstehen.
- **Ablauf bedacht konfigurieren**: Kurze Ablaufzeiten (7 Tage) erzeugen Dringlichkeit, können Kunden aber frustrieren. Längere Zeiträume (30–90 Tage) sind für die meisten SaaS-Produkte kundenfreundlicher.
- **Niedrige Guthaben überwachen**: Legen Sie Schwellen für niedrige Guthaben fest, um Kunden zu warnen, bevor ihnen die Mittel ausgehen, und Überraschungsüberziehungen zu reduzieren.
- **Im Testmodus prüfen**: Erstellen Sie Gutschriften, hängen Sie sie an Testprodukte und simulieren Sie den vollständigen Kauf → Nutzung → Abzug → Ablauf-Zyklus, bevor Sie live gehen.

<Info>
Credit-Based Billing funktioniert nahtlos mit allen anderen Dodo Payments-Funktionen – Abonnements mit Testphasen, Planwechsel mit anteiliger Abrechnung und das Kundenportal. Beginnen Sie mit einer grundlegenden Konfiguration und erweitern Sie diese, wenn sich Ihr Preismodell weiterentwickelt.
</Info>
