...
<Info>
Dieser Abschnitt behandelt die **serverseitige API**-Themenkonfiguration mithilfe von `customization.theme_config`. Wenn Sie das **Checkout SDK** (Overlay oder Inline Checkout) einsetzen, siehe die Abschnitte zur Themenanpassung in [Overlay Checkout](/developer-resources/overlay-checkout#theme-customization) oder [Inline Checkout](/developer-resources/inline-checkout#theme-customization), die camelCase-Eigenschaften verwenden (z. B. `bgPrimary` statt `bg_primary`).
</Info>
...
|----------|-------------|
| `light` | Farbkonfiguration für den Hellmodus |
| `dark` | Farbkonfiguration für den Dunkelmodus |
...
| `pay_button_text` | Eigener Text für den Zahlungsbutton (z. B. "Complete Purchase", "Subscribe Now") |
...
| `text_error` | Fehlerfarbe für Text |
...
| `input_focus_border` | Rahmenfarbe bei Fokussierung des Eingabefeldes |

<Info>
Alle Farboptionen akzeptieren gängige CSS-Farbformate:
...
<Tip>
Sie müssen nicht alle Farbeigenschaften angeben. Alle nicht spezifizierten Eigenschaften verwenden die Standardthemenwerte.
</Tip>
