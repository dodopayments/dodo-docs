...
2. Dodo Payments erstellt eine Berechtigung im Status `Pending` mit einem `oauth_url` für den Kunden, um sein GitHub-Konto zu verknüpfen.
3. Der Kunde autorisiert die Verknüpfung über den Link in seiner E-Mail oder im Customer Portal.
4. Dodo Payments lädt das GitHub-Konto des Kunden mit der konfigurierten Berechtigung in das Repository ein. Die Berechtigung wechselt zu `Delivered`, sobald die Einladung erstellt wurde.
5. Wenn das Abonnement gekündigt, erstattet oder widerrufen wird, wird der Kunde als Mitarbeiter aus dem Repository entfernt.
