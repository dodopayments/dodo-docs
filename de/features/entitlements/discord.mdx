...
2. Dodo Payments erstellt einen Grant im Status `Pending` mit einem `oauth_url`, der auf Discord verweist.
3. Der Kunde erhält eine E-Mail mit einer Schaltfläche „Join the Discord“ (dem OAuth-Link). Der Link ist auch in seinem Customer Portal sichtbar.
4. Nachdem der Kunde die Autorisierung erteilt hat, fügt der Bot ihn zum Server hinzu und weist ihm die konfigurierte Rolle zu. Der Grant wechselt zu `Delivered`.
5. Wenn das Abonnement gekündigt, pausiert oder abgelaufen ist oder der Händler den Grant manuell widerruft, entfernt der Bot die Rolle und der Grant wechselt zu `Revoked`.
...
- `entitlement_grant.created` wird mit `status: "Pending"` und einem `oauth_url` ausgelöst, über den der Kunde die Autorisierung erteilen kann.
