...
| Auslöser | Auswirkung |
|---|---|
| Einmalzahlung erfolgreich / Abonnement wird aktiv | Grant erstellt mit gesetztem `status: Delivered` und `delivered_at`. |
| Abonnement pausiert, gekündigt oder abgelaufen | Grant mit dem entsprechenden `revocation_reason` widerrufen. |
| Rückerstattung für eine Einmalzahlung | Grant mit `revocation_reason: refund` widerrufen. |
| Abonnement wird wiederhergestellt (z. B. Dunning erfolgreich) | Der widerrufene Grant wird auf `Delivered` wiederhergestellt — derselbe Grant `id`, Widerrufsfelder gelöscht. |
| Manueller API-Widerruf | Grant mit `revocation_reason: manual` widerrufen. Wird bei der Verlängerung nicht automatisch wiederhergestellt. |
