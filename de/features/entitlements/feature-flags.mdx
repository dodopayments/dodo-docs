...
Feature-Flag-Grants folgen dem standardmäßigen [Grant-Lebenszyklus](/features/entitlements/introduction#how-grants-work) mit einer Vereinfachung: Es gibt keinen Bereitstellungsschritt, daher verbleiben Grants nie in `Pending` und wechseln nie zu `Failed`.

| Auslöser | Effekt |
|---|---|
| Einmalzahlung erfolgreich / Abonnement wird aktiv | Grant mit gesetzten Werten `status: delivered` und `delivered_at` erstellt. |
| Abonnement pausiert, gekündigt oder abgelaufen | Grant mit dem passenden `revocation_reason` aufgehoben. |
| Rückerstattung einer Einmalzahlung | Grant mit `revocation_reason: refund` aufgehoben. |
| Abonnement wird wiederhergestellt (z. B. durch erfolgreiches Dunning) | Der aufgehobene Grant wird zu `Delivered` wiederhergestellt – derselbe Grant `id`, Aufhebungsfelder gelöscht. |
| Manuelle Aufhebung über die API | Grant mit `revocation_reason: manual` aufgehoben. Bei der Verlängerung nicht automatisch wiederhergestellt. |
