...
|--------|---------------|--------------|---------------------------|
| `pending` | Das Abonnement wird erstellt oder verarbeitet | — | Warte auf `subscription.active` (oder `subscription.failed`) |
| `active` | Das Abonnement ist aktiv und wird automatisch verlängert | — | Keine Aktion erforderlich |
| `on_hold` | Eine Verlängerungszahlung (oder Gebühr für eine Tarifänderung) ist fehlgeschlagen; Verlängerungen wurden angehalten | **Ja** | Aktualisiere die Zahlungsmethode, um das Abonnement zu reaktivieren – automatisch über [Payment Retries](/features/recovery/payment-retries) und [Dunning](/features/recovery/subscription-dunning) oder manuell über die [Update Payment Method API](/api-reference/subscriptions/update-payment-method) |
| `paused` | Das Abonnement wurde von dir oder dem Kunden absichtlich pausiert; die Abrechnung ist eingefroren und der Zugriff wurde entzogen | **Ja** | Setze es mit `pause: false` oder über das Customer Portal fort, wenn das Selbstbedienungs-Pausieren aktiviert ist – siehe [Abonnements pausieren und fortsetzen](#pausing-and-resuming-subscriptions) |
| `cancelled` | Das Abonnement wurde gekündigt und wird nicht verlängert | Nur erneuter Kauf | Der Kunde muss ein neues Abonnement starten; [Dunning](/features/recovery/subscription-dunning) kann einen erneuten Kauf anstoßen |
