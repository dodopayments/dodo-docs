...
| Erneuerung erfolgreich | `subscription.renewed` |
| Erneuerung fehlgeschlagen → pausiert | `subscription.on_hold` |
| Subscription pausiert | `subscription.paused` |
| Pausierte Subscription fortgesetzt | `subscription.unpaused` |
| Erstellung fehlgeschlagen | `subscription.failed` |
| Plan hochgestuft/herabgestuft | `subscription.plan_changed` |
...
Verwende `PATCH /subscriptions/{subscription_id}`, um zum nächsten Abrechnungsdatum zu kündigen, den Subscription-Zeitraum zu verlängern, Abrechnungsdetails zu aktualisieren oder Metadaten zu ändern. Um die Menge zu ändern, verwende stattdessen die [API zum Planwechsel](/api-reference/subscriptions/change-plan) — `PATCH` akzeptiert `quantity` nicht.
