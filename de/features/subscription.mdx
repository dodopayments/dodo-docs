...
Planänderungen ermöglichen das Upgraden oder Downgraden von Abonnements, das Anpassen von Mengen oder den Wechsel zu verschiedenen Produkten. Abhängig vom gewählten Prorationsmodus kann eine Änderung eine sofortige Belastung auslösen, Guthaben erzeugen oder keine Rechnungsanpassung vornehmen.
...
<Info>
```
**Schneller Vergleich der vier Prorationsmodi:**

| | `prorated_immediately` | `difference_immediately` | `full_immediately` | `do_not_bill` |
|---|---|---|---|---|
| **Upgrade** | Prorierte Gebühren für verbleibende Tage | Vollständiger Preisunterschied in Rechnung gestellt | Voller Preis des neuen Plans in Rechnung gestellt | Keine Gebühr — sofortiger Wechsel |
| **Downgrade** | Proriertes Guthaben für verbleibende Tage | Vollständiger Preisunterschied als Guthaben | Kein Guthaben, volle Gebühr | Kein Guthaben — sofortiger Wechsel |
| **Abrechnungszyklus** | Bleibt unverändert | Bleibt unverändert | Setzt sich auf heute zurück | Bleibt unverändert |
| **Am besten geeignet für** | Faire zeitbasierte Abrechnung | Einfache Tarifwechsel | Abrechnungszyklus setzt sich zurück | Kostenlose Migrationen oder kulante Wechsel |
...
#### `do_not_bill`
Wechsel zum neuen Plan ohne Abrechnungsanpassung. Keine Prorationsgebühren, keine Guthaben — der Kunde wechselt einfach zum neuen Plan. Am besten geeignet für kulante Migrationen, kostenlose Planwechsel oder Szenarien, in denen Sie die Kostenunterschiede absorbieren möchten.
...
**Szenario**: Kunde im Basistarif ($30/Monat) wechselt am Tag 16 eines 30-tägigen Zyklus mit `prorated_immediately` zum Pro-Tarif ($80/Monat).
...
Nächste Verlängerung am ursprünglichen Abrechnungsdatum: **$80.00/Monat**.

```json
<Tip>
```
Für detailliertere Berechnungsbeispiele und Grenzfälle siehe unseren vollständigen [Upgrade & Downgrade Guide](/developer-resources/subscription-upgrade-downgrade).
```json
</Tip>
```
...
**Szenario**: Kunde im Pro-Tarif ($80/Monat) wechselt zum Starter-Tarif ($20/Monat) mit `difference_immediately`.

```
Credit = Old plan − New plan = $80 − $20 = $60.00
```

Das $60-Guthaben wird automatisch auf zukünftige Verlängerungen angewendet:
...
- Verlängerung 4: **$20.00** (Vollpreis)

```json
<Info>
```
Erfahren Sie mehr darüber, wie Guthaben im [Upgrade & Downgrade Guide](/developer-resources/subscription-upgrade-downgrade) verwaltet werden.
```json
</Info>
```
...
Add-ons bei Planänderungen anpassen. Add-ons werden in die Prorationsberechnungen einbezogen:
...
<Info>
```
Planänderungen lösen sofortige Gebühren aus. Fehlgeschlagene Gebühren können das Abonnement in den `on_hold`-Status versetzen. Verfolgen Sie Änderungen über `subscription.plan_changed` Webhook-Ereignisse.
```json
</Info>
```

### Planänderungen vorschauen

Vor dem Bestätigen einer Planänderung die genaue Gebühr und das resultierende Abonnement vorschauen:
...
```
Planänderungen vorschauen, bevor Sie sie bestätigen.
```json
</Card>
```
...
Abonnements können während ihres Lebenszyklus unterschiedliche Zustände annehmen:

- **`active`**: Das Abonnement ist aktiv und wird automatisch verlängert
- **`on_hold`**: Das Abonnement ist aufgrund fehlgeschlagener Zahlung pausiert. Aktualisierung der Zahlungsmethode erforderlich, um es wieder zu aktivieren
- **`cancelled`**: Das Abonnement ist gekündigt und wird nicht verlängert
- **`expired`**: Das Abonnement hat sein Enddatum erreicht
- **`pending`**: Das Abonnement wird erstellt oder verarbeitet

### Pausierter Zustand

Ein Abonnement wechselt in den `on_hold`-Zustand, wenn:

- Eine Verlängerungszahlung scheitert (unzureichende Mittel, abgelaufene Karte, etc.)
- Eine Planänderungsgebühr scheitert
- Die Autorisierung der Zahlungsmethode scheitert

```json
<Warning>
```
Wenn ein Abonnement im `on_hold`-Zustand ist, wird es nicht automatisch verlängert. Sie müssen die Zahlungsmethode aktualisieren, um das Abonnement zu reaktivieren.
```json
</Warning>
```

### Reaktivierung aus dem pausierten Zustand

Um ein Abonnement aus dem `on_hold`-Zustand zu reaktivieren, müssen Sie die Zahlungsmethode aktualisieren. Dies bewirkt automatisch:

1. Erstellung einer Gebühr für verbleibende Forderungen
2. Erstellung einer Rechnung
3. Durchführung der Zahlung mit der neuen Zahlungsmethode
...
```
Sehen Sie sich die API zur Erstellung von Abonnements an.
```json
</Card>
```
```json
...
```
Ändern Sie das aktive Produkt und die Mengen mit Prorationssteuerungen.
```json
{"LOCKED_PATTERN_afa3d1700c97ae5510a3b95972626011" : ""}
```
Überprüfen Sie die Optionen für Planänderungen.
```json
{"LOCKED_PATTERN_4ceaf3811b39bde3b7bfedbcf0487a0b" : ""}
```
```json
...
"
Für bedarfsabhängige Abonnements spezifische Beträge auf Abruf berechnen.
...
```
Durchsuchen Sie Listing- und Abruf-APIs.
"{"LOCKED_PATTERN_4ceaf3811b39bde3b7bfedbcf0487a0b":""}
```
...
Nutzung für gemessene oder hybride Preismodelle erfassen.
```
