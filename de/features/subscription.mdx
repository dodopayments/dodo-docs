...
Verwenden Sie `PATCH /subscriptions/{id}`, um zum nächsten Abrechnungsdatum zu kündigen, den Abonnementzeitraum zu verlängern, Rechnungsdetails zu aktualisieren oder Metadaten zu ändern. Verwenden Sie zum Ändern der Menge stattdessen die [Change Plan API](/api-reference/subscriptions/change-plan) — `PATCH` akzeptiert `quantity` nicht.
