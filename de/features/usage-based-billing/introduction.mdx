---
sidebarTitle: Einführung
title: Nutzungsbasierte Abrechnung
description: Berechnen Sie Kunden basierend auf dem, was sie tatsächlich nutzen,
  mit flexibler Messung und automatisierter Abrechnung.
icon: box
---

Die nutzungsbasierte Abrechnung ermöglicht es Ihnen, Kunden für das zu berechnen, was sie tatsächlich nutzen – API-Aufrufe, Speicher, KI-Token oder jede Metrik, die Sie definieren. Kunden zahlen für den Verbrauch, nicht für feste Gebühren.

<Info>
**Perfekt für**: APIs, KI-Dienste, Infrastrukturplattformen und jedes Produkt, bei dem der Wert mit der Nutzung skaliert.
</Info>

## So funktioniert es

<Steps>
<Step title="Nutzungsereignisse senden">
Ihre Anwendung sendet Ereignisse, wenn abrechnungsfähige Aktionen auftreten.

```javascript
{
  "event_id": "evt_123",
  "customer_id": "cus_abc",
  "event_name": "api.call",
  "metadata": { "endpoint": "/v1/users" }
}
```

</Step>

<Step title="Zähler aggregieren Nutzung">
Zähler aggregieren Ereignisse in abrechnungsfähige Mengen unter Verwendung von Count, Sum, Max oder Last Aggregation.
</Step>

<Step title="Automatische Abrechnung">
Legen Sie Preise pro Einheit und kostenlose Schwellenwerte fest. Kunden werden automatisch in jedem Zyklus abgerechnet.
</Step>
</Steps>

## Kernkonzepte

### Ereignisse

Ereignisse repräsentieren Nutzungsaktionen in Ihrer Anwendung. Jedes Ereignis umfasst:
- **Ereignisname**: Art der Nutzung (z. B. `api.call`)
- **Kunden-ID**: Wer es verwendet hat
- **Metadaten**: Zusätzliche Eigenschaften zur Aggregation

### Zähler

Zähler aggregieren Ereignisse in abrechnungsfähige Mengen:
- **Count**: Gesamte Ereignisse (API-Aufrufe)
- **Sum**: Werte addieren (insgesamt Bytes)
- **Max**: Höchster Wert (Spitzenbenutzer)
- **Last**: Jüngster Wert

<Frame>
<img src="/images/usage-based/UBB-2.png" alt="Erstellen Sie die Zähleroberfläche" />
</Frame>

### Produkte mit nutzungsbasierter Preisgestaltung

Legen Sie Preise pro Einheit und optionale kostenlose Schwellenwerte fest:

<Frame>
<img src="/images/usage-based/UBB-4.png" alt="Preisgestaltungskonfiguration" />
</Frame>

**Beispiel**: 2.500 Aufrufe - 1.000 kostenlos = 1.500 × $0.02 = $30.00

### Kundenkredite

Geben Sie Kunden enthaltene Nutzungsgutschriften, die sich bei jedem Abrechnungszyklus erneuern. Gutschriften gelten vor der bezahlten Nutzung.

## Schnellstart

<Steps>
<Step title="Erstellen Sie einen Zähler">
In Ihrem Dashboard: **Zähler** → **Zähler erstellen**

1. Ereignisnamen festlegen (z. B. `api.call`)
2. Aggregation wählen (Anzahl, Summe, Max, Letzte)
3. Optionale Filter hinzufügen
</Step>

<Step title="Zum Produkt hinzufügen">
Verknüpfen Sie den Zähler mit einem Produkt mit Preisgestaltung:

<Frame>
<img src="/images/usage-based/UBB-5.png" alt="Zähler zum Produkt hinzufügen" />
</Frame>

1. Wählen Sie **Nutzungsbasierte Abrechnung**
2. Wählen Sie Ihren Zähler aus
3. Legen Sie den Preis pro Einheit und die kostenlose Schwelle fest
</Step>

<Step title="Ereignisse senden">

```javascript
await fetch('https://test.dodopayments.com/events/ingest', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${process.env.DODO_API_KEY}`,
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    events: [{
      event_id: "unique_event_id",
      customer_id: "cus_abc123",
      event_name: "api.call",
      metadata: { endpoint: "/v1/users" }
    }]
  })
});
```

</Step>

<Step title="Nutzung überwachen">
<Frame>
<img src="/images/guides/usage-based-billing/meter-quantities-chart.png" alt="Zähler-Dashboard" />
</Frame>

Überprüfen Sie das Dashboard Ihres Zählers, um Ereignisse und Nutzung Aggregation zu sehen. Kunden werden automatisch in jedem Zyklus abgerechnet.
</Step>
</Steps>

## Anwendungsfälle

<CardGroup cols={2}>
<Card title="APIs & Infrastruktur" icon="cloud">
Verfolgen Sie API-Aufrufe, Rechenstunden, Bandbreite, Speicher. Beispiel: $0.001 pro Aufruf nach 10.000 kostenlos.
</Card>

<Card title="KI-Nutzungsverfolgung" icon="brain">
Verfolgen Sie Token, Generierungen, Verarbeitungszeit. Beispiel: \$0.04 pro Standardbild, \$0.08 pro HD.
</Card>

<Card title="Daten & Analytik" icon="chart-line">
Berechnen Sie nach verarbeiteten Daten, ausgeführten Abfragen, analysierten Datensätzen. Beispiel: $5 pro GB nach 10 GB kostenlos.
</Card>

<Card title="Kommunikation" icon="message">
Zählen Sie Nachrichten, Minuten, übertragene Daten. Beispiel: $0.01 pro gesendeter SMS.
</Card>
</CardGroup>

## Nächste Schritte

<CardGroup cols={2}>
<Card title="Ereignisaufnahme" icon="bolt" href="/features/usage-based-billing/event-ingestion">
Erfahren Sie, wie Sie Nutzungsereignisse aus Ihrer Anwendung strukturieren und senden.
</Card>

<Card title="Zählerleitfaden" icon="sliders" href="/features/usage-based-billing/meters">
Meistern Sie die Zählerkonfiguration, Aggregationstypen und Ereignisfilterung.
</Card>

<Card title="Vollständiges Tutorial" icon="code" href="/developer-resources/usage-based-billing-build-ai-image-generator">
Erstellen Sie einen vollständigen KI-Bildgenerator mit nutzungsbasierter Abrechnung von Grund auf.
</Card>

<Card title="Integrationsleitfaden" icon="book" href="/developer-resources/usage-based-billing-guide">
Umfassender Leitfaden zur Implementierung der nutzungsbasierten Abrechnung in Ihrer Anwendung.
</Card>
</CardGroup>
