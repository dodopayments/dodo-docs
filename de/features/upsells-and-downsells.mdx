...
Dodo Payments ermöglicht diese Abläufe über den `payment_method_id` Parameter, der es Ihnen erlaubt, die hinterlegte Zahlungsmethode eines Kunden zu belasten, ohne dass dieser die Kartendaten erneut eingeben muss.

### Wesentliche Vorteile

| Vorteil | Auswirkung |
|---------|--------|
| **One-Click-Käufe** | Überspringen des Zahlungsformulars für zurückkehrende Kunden |
| **Höhere Konversion** | Reibung im Entscheidungsprozess verringern |
| **Sofortige Verarbeitung** | Belastungen werden sofort mit `confirm: true` bearbeitet |
...
<Steps>
<Step title="Kunde mit gespeicherter Zahlungsmethode">
...
<Step title="Produkte konfiguriert">
...
<Step title="Webhook Endpoint">
  Richten Sie Webhooks ein, um die `payment.succeeded`, `payment.failed` und `subscription.plan_changed` Ereignisse zu behandeln.
</Step>
</Steps>
...
<Warning>
Wenn Sie `payment_method_id` verwenden, müssen Sie `confirm: true` setzen und eine bestehende `customer_id` bereitstellen. Die Zahlungsmethode muss zu diesem Kunden gehören.
</Warning>
...
|------|----------|----------|
| `difference_immediately` | Belastet Preisunterschied sofort ($30→$80 = $50) | Einfache Upgrades |
| `prorated_immediately` | Belastet basierend auf verbleibender Zeit im Abrechnungszyklus | Faire zeitbasierte Abrechnung |
| `full_immediately` | Belastet den vollen Preis des neuen Plans, ignoriert verbleibende Zeit | Abrechnungszyklus wird zurückgesetzt |
...
<Info>
Gutschriften aus Downgrades unter Verwendung von `difference_immediately` sind abonnementsgebunden und werden automatisch auf zukünftige Erneuerungen angewendet. Sie sind verschieden von [Kunden-Gutschriften](/features/customer-credit).
</Info>
...
<AccordionGroup>
<Accordion title="Zeitpunkt Ihrer Upsells strategisch planen">
...
<Accordion title="Zahlungsmethodenberechtigung überprüfen">
...
<Accordion title="Fehlermeldungen elegant behandeln">
...
<Accordion title="Klaren Wertvorschlag bieten">
...
<Accordion title="Kundenwahl respektieren">
...
|-------|---------|--------|
| `payment.succeeded` | Upsell/Cross-Sell-Zahlung abgeschlossen | Produkt liefern, Zugang aktualisieren |
| `payment.failed` | One-Click-Belastung fehlgeschlagen | Fehler anzeigen, Wiederholung oder Rückfall anbieten |
| `subscription.plan_changed` | Upgrade/Downgrade abgeschlossen | Funktionen aktualisieren, Bestätigung senden |
| `subscription.active` | Abonnement nach Planänderung reaktiviert | Zugang zur neuen Stufe gewähren |

<Card title="Webhook-Integrationsleitfaden" icon="webhook" href="/developer-resources/webhooks">
...
<CardGroup cols={2}>
<Card title="Leitfaden zum Abonnement-Upgrade" icon="arrows-rotate" href="/developer-resources/subscription-upgrade-downgrade">
  Detaillierter Leitfaden zu Planänderungen, Prorationsmodi und dem Umgang mit Fehlern.
</Card>

<Card title="Checkout-Sitzungen" icon="cart-shopping" href="/developer-resources/checkout-session">
  Vollständige Referenz für das Erstellen von Checkout-Sitzungen mit allen Optionen.
</Card>

<Card title="API für Zahlungsmethoden von Kunden" icon="credit-card" href="/api-reference/customers/get-customer-payment-methods">
  API-Referenz für die Auflistung von Zahlungsmethoden der Kunden.
</Card>

<Card title="Add-ons" icon="puzzle-piece" href="/features/addons">
  Verbessern Sie Abonnements mit flexiblen Add-ons für zusätzliche Einnahmen.
</Card>
</CardGroup>
