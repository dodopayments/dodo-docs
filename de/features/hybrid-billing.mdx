...
**Lösungen**:
1. Überprüfen, ob das Ereignis-Engagement funktioniert (API-Antworten prüfen)
2. Bestätigen, dass `customer_id` mit dem Abonnementkunden übereinstimmt
3. Überprüfen, dass `event_name` mit der Zählerkonfiguration übereinstimmt
4. Verifizieren, dass die Ereignisse die richtigen Zeitstempel haben (nicht datiert in der Zukunft)
</Accordion>
...
**Lösungen**:
1. Verwenden Sie die `previewChangePlan` API, um genaue Gebühren vor der Bestätigung anzuzeigen
2. Kommunizieren, dass die Proratisierung sowohl für Abonnements als auch für Add-ons gilt
3. Ziehen Sie in Betracht, `difference_immediately` für einfachere Upgrade-Abrechnungen zu verwenden
</Accordion>
