...
| Ereignis | Verhalten |
|---|---|
| `payment.succeeded` (einmalig) | Einen Grant ausstellen. Der Grant enthält presigned download URLs, die etwa 15 Minuten gültig sind; Kunden können sie aktualisieren, indem sie den E-Mail-Link oder die Customer Portal-Seite erneut öffnen. |
| `subscription.active` | Einen Grant ausstellen. Dateien bleiben zugänglich, solange das Abonnement aktiv ist. |
| `subscription.renewed` | No-op. Derselbe Grant bleibt bestehen; bei jedem Abruf werden neue presigned download URLs erstellt. |
| `subscription.on_hold` / `cancelled` / `expired` | Den Grant widerrufen. Es werden keine neuen presigned download URLs mehr ausgestellt. |
| `subscription.paused` | Den Grant widerrufen. Es werden keine neuen presigned download URLs mehr ausgestellt, bis das Abonnement fortgesetzt wird. |
| `subscription.plan_changed` | Den alten Grant widerrufen und einen neuen für die Berechtigung des neuen Plans ausstellen. |
| `refund.succeeded` (einmalig) | Den Grant widerrufen. |
| Manuelles Widerrufen | Mit `revocation_reason: manual` widerrufen. |
...
<Note>
Produkte, die den älteren `digital_product_delivery`-Block direkt am Produkt konfiguriert hatten, wurden **automatisch** zu einer Digital-Files-Berechtigung migriert. Vorhandene Dateien, die über die Legacy-Produktdatei-API hochgeladen wurden, bleiben erhalten; sie können weiterhin heruntergeladen werden und erscheinen neben neu hochgeladenen Dateien in den Grant-Payloads. In der `digital_files`-Konfiguration der Berechtigung werden sie in der Liste `legacy_file_ids` erfasst. Künftige Aktualisierungen (Hinzufügen von Dateien, Ändern von Anweisungen, Ersetzen der externen URL) sollten durch Bearbeiten der migrierten Digital-Files-Berechtigung unter **Entitlements** vorgenommen werden.
