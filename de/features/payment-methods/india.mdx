...
Damit UPI an der Kasse angezeigt wird:
...
`payment.succeeded` Webhook wird nach dem tatsächlichen Betrag gesendet, nicht beim Start.
</Step>
</Steps>

<Warning>
**Gewähren Sie keine Vorteile beim Start der Abbuchung.** Warten Sie auf den `payment.succeeded` Webhook, der ~48-51 Stunden nach dem geplanten Abbuchungsdatum eintrifft.
</Warning>
...
| :---- | :--- | :----- |
| `subscription.created` | Mandat genehmigt | Abonnierungsbeginn aufzeichnen |
| `payment.succeeded` | ~48h nach dem Abbuchungsdatum | Zugriff gewähren/fortsetzen |
| `payment.failed` | Abbuchung fehlgeschlagen | Kunde benachrichtigen, Zugriff pausieren |
| `subscription.on_hold` | Zahlung fehlgeschlagen | Austausch der Zahlungsmethode anfordern |
...
<Accordion title="Mandatenstornierungen verwalten">
Kunden können Mandate jederzeit über ihre Bank-Apps stornieren. Überwachen Sie `subscription.on_hold` Webhooks und fordern Sie die Kunden auf, sich wieder anzumelden oder die Zahlungsmethoden zu aktualisieren.
</Accordion>
...
<AccordionGroup>
<Accordion title="UPI wird an der Kasse nicht angezeigt">
**Überprüfen:**
1. Rechnungsland auf `IN` gesetzt?
2. Währung auf `INR` gesetzt?
3. Wenn nicht-indischer Händler: Adaptive Currency aktiviert?
4. `upi_collect` in `allowed_payment_method_types` enthalten?

**Lösung:** Verifizieren Sie, dass die Rechnungsadresse `country: "IN"` und `billing_currency: "INR"` enthält.
</Accordion>
...
**Lösung:** Die nächste Abbuchung wird fehlschlagen und das Abonnement wird in `on_hold` verschoben. Überwachen Sie Webhooks für `payment.failed`.
</Accordion>
</AccordionGroup>
