...
<Step title="Passende Rechnungsadresse festlegen">
Legen Sie das Land der Rechnungsadresse so fest, dass es mit der Zahlungsmethode übereinstimmt:
- `NL` für iDEAL
- `BE` für Bancontact
- `AT` für EPS
- `PT` für Multibanco
...
<Accordion title="Kartenalternativen bereitstellen">
Nicht alle europäischen Kunden haben Zugang zu diesen regionalen Methoden (Touristen, Expats usw.). Always include `credit` und `debit` als Alternativen.
</Accordion>
...
2. Währung auf EUR eingestellt?
3. Methode in `allowed_payment_method_types` enthalten?

**Lösung:** Europäische Methoden sind streng regional. Ein Kunde mit dem Rechnungsland `DE` (Deutschland) sieht iDEAL nicht, da es nur in den Niederlanden verfügbar ist.
</Accordion>
