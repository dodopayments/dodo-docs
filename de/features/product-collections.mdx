...
|---------|-------------|---------|
| **Mehrere Abonnements erlauben** | Kunden können gleichzeitig mehr als ein aktives Abonnement haben | Aktiviert |
| **Abonnementaktualisierungen erlauben** | Kunden können ihre bestehenden Abonnements jederzeit über das Customer Portal upgraden oder downgraden | Deaktiviert |
| **Missbrauch von Testzeiträumen verhindern** | Kunden, die bereits einen Testzeitraum eingelöst haben, erhalten statt eines neuen Testzeitraums einen kostenpflichtigen Kauf ohne Testzeitraum | Deaktiviert |
...
<Info>
Details dazu, wie Einlösungen von Testzeiträumen abgeglichen und erfasst werden, finden Sie unter [Missbrauch von Testzeiträumen verhindern](/features/subscription#preventing-trial-misuse).
</Info>
...
Product Collections können entweder über das Dodo Payments-Dashboard oder programmgesteuert über die API verwaltet werden. Die API bietet vollständige Kontrolle über die Erstellung, Aktualisierung, das Hochladen von Bildern und die Archivierung von Collections sowie über die Verwaltung verschachtelter Gruppen und Produkte.

### Dashboard-Operationen

- **Erstellen**: Neue Collections mit Produkten und Gruppen einrichten
...
- **Archivieren**: Eine Collection ausblenden, ohne sie dauerhaft zu löschen (sie kann später wieder aus dem Archiv entfernt werden)

<Frame>
  <img 
    src="/images/product-collection/collection-dashboard.png" 
    alt="Screenshot des Product-Collection-Dashboards mit den Verwaltungsoperationen für Collections" 
  style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
Rufen Sie alle mit Ihrem Konto verknüpften Product Collections mit einer `GET`-Anfrage an den `/product-collections`-Endpoint ab. Unterstützt Pagination, die Filterung nach Marke und das Einschließen archivierter Collections.
...
Erstellen Sie eine neue Product Collection, indem Sie eine `POST`-Anfrage an den `/product-collections`-Endpoint senden und dabei Details wie Name, Beschreibung und Marke angeben.
...
Rufen Sie detaillierte Informationen zu einer bestimmten Product Collection ab – einschließlich ihrer Gruppen und Produktelemente – indem Sie eine `GET`-Anfrage an den `/product-collections/{id}`-Endpoint senden.
...
Verknüpfen Sie ein Bild mit einer Collection, indem Sie es über eine vorab signierte URL hochladen. Fordern Sie beim `/product-collections/{id}/images`-Endpoint eine Upload-URL an und `PUT` Sie anschließend das Bild innerhalb von 60 Sekunden an die zurückgegebene URL.
...
Archivieren Sie eine Collection, indem Sie eine `DELETE`-Anfrage an den `/product-collections/{id}`-Endpoint senden. Dadurch wird die Collection für eine neue Verwendung ausgeblendet, aber nicht dauerhaft entfernt.
...
Stellen Sie eine archivierte Collection wieder her, indem Sie eine `POST`-Anfrage an den `/product-collections/{id}/unarchive`-Endpoint senden.
...
Mit Gruppen können Sie Produkte innerhalb einer Collection organisieren (z. B. „Monatliche Pläne“ im Vergleich zu „Jährliche Pläne“). Verwenden Sie die Gruppen-Endpoints, um Gruppen innerhalb einer Collection hinzuzufügen, zu aktualisieren oder zu entfernen.
...
- **Strategisch anordnen**: Ihren beliebtesten oder empfohlenen Plan an die erste Stelle setzen, da er beim Checkout vorausgewählt wird
- **Klar benennen**: Produktnamen sollten die Unterschiede beim Mehrwert klar vermitteln
- **Beide Richtungen aktivieren**: Upgrades und Downgrades erlauben, damit Kunden flexibel bleiben
- **Proration berücksichtigen**: Einen Proration-Modus auswählen, der zu Ihrem Geschäftsmodell passt
- **Gründlich testen**: Checkout- und Planänderungsabläufe im Testmodus überprüfen, bevor Sie live gehen

<Check>
Sie können jetzt Product Collections erstellen und Kunden eine einheitliche Erfahrung bei der Planauswahl bieten.
</Check>

<CardGroup cols={2}>
<Card title="Products" icon="box" href="/features/products">
  Erstellen Sie einmalige, abonnementsbasierte oder nutzungsbasierte Produkte, die Sie Collections hinzufügen können.
</Card>

<Card title="Checkout" icon="cart-shopping" href="/features/checkout">
  Zeigen Sie Collection-Produkte in einer einheitlichen Checkout-Erfahrung an.
</Card>

<Card title="Customer Portal" icon="id-card" href="/features/customer-portal">
  Ermöglichen Sie Kunden, innerhalb derselben Collection Upgrades oder Downgrades durchzuführen.
</Card>
