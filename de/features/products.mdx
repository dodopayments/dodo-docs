...
Für wiederkehrende Produkte setzen Sie das `price`-Objekt auf einen wiederkehrenden Preis (`type: recurring_price`) und spezifizieren Sie `payment_frequency_interval` und `subscription_period_interval` (`Day`, `Week`, `Month` oder `Year`) zusammen mit den entsprechenden Anzahlen.
...
Wenn Sie ein Produkt nicht mehr anzeigen oder verwenden möchten, können Sie es mit einer `DELETE`-Anforderung an den `/products/{id}`-Endpunkt archivieren. Diese Aktion verbirgt das Produkt, löscht es jedoch nicht dauerhaft.
