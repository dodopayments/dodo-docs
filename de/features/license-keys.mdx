...
Wenn ein Kunde den Kauf eines Produkts mit aktivierten Lizenzschlüsseln abschließt, wird der Lizenzschlüssel automatisch als Abfrageparameter zu Ihrem `return_url` hinzugefügt. Dadurch können Sie den Schlüssel sofort auf Ihrer Erfolgsseite anzeigen, ohne einen zusätzlichen API-Aufruf zu tätigen.

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001&email=customer%40example.com
```

Wenn der Kauf mehrere Lizenzschlüssel generiert (z. B. Menge > 1), werden diese durch Kommas getrennt:
...
<Tip>
Analysieren Sie den `license_key`-Parameter auf Ihrer Rückgabeseite, um den Schlüssel sofort den Kunden anzuzeigen und das Erlebnis nach dem Kauf zu verbessern.
</Tip>

## Best Practices

- **Grenzen klar halten**: Wählen Sie sinnvolle Standardwerte für Ablauf und Aktivierungen
- **Benutzer anleiten**: Geben Sie präzise Aktivierungsanweisungen und Selbsthilfe-Dokumentation
- **Überprüfungen automatisieren**: Validieren Sie Schlüssel serverseitig, bevor Sie Zugriff gewähren
- **Ereignisse überwachen**: Verwenden Sie Webhooks, um Missbrauch zu erkennen und automatische Widerrufe vorzunehmen
