...
Wenn ein Kunde ein Produkt im manuellen Modus kauft, wird die Berechtigung im `pending`-Status ohne Schlüssel erstellt und ein [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) Webhook wird mit `integration_type: "license_key"` und `status: "pending"` ausgelöst. Sie können auf diesen Webhook reagieren oder den Endpunkt [List Grants](/api-reference/entitlements/list-grants) mit den Filtern `integration_type` und `status` abfragen:
