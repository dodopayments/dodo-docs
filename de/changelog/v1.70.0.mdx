...
Initialisieren Sie das SDK mit `displayType: 'inline'` und betten Sie den Checkout in ein Containerelement ein:
...
- **Benutzerdefinierte Bestellübersichten**: Erstellen Sie Ihre eigene Preisübersicht, die sich in Echtzeit aktualisiert
- **Nahtlose Integration**: Der Checkout-Rahmen kümmert sich um die Zahlungsabwicklung, während Sie das Layout steuern
...
2. **Zahlungen, Abonnements und Checkouts mit bestehenden Zahlungsmethoden erstellen**  
   Optimieren Sie den Checkout-Prozess, indem Sie gespeicherte Zahlungsmethoden von bestehenden Kunden verwenden. Der neue `payment_method_id`-Parameter ermöglicht es Ihnen, Zahlungen, Abonnements oder Checkout-Sitzungen mit der zuvor gespeicherten Zahlungsmethode eines Kunden zu erstellen.
...
<Warning>
   Wenn Sie `payment_method_id` in Checkout-Sitzungen verwenden, muss `confirm` auf `true` gesetzt werden, und eine bestehende `customer_id` muss bereitgestellt werden. Die Zahlungsmethode wird auf ihre Berechtigung mit der Währung der Zahlung überprüft.
</Warning>
...
5. **`redirect_immediately`-Flag - Zahlungsbestätigungsseite überspringen**  
   Steuern Sie den Checkout-Prozess mit dem neuen `redirect_immediately`-Flag. Wenn aktiviert, werden die Kunden sofort nach Abschluss der Zahlung umgeleitet, wodurch die Standard-Zahlungsbestätigungsseite umgangen wird, was zu einer schnelleren, nahtloseren Erfahrung führt.
...
<Tip>
   Verwenden Sie `redirect_immediately: true`, wenn Sie eine benutzerdefinierte Bestätigungsseite haben, die eine bessere Benutzererfahrung als die standardmäßige Zahlungsbestätigungsseite bietet.
</Tip>

<Note>
   Wenn `redirect_immediately` aktiviert ist, werden die Kunden sofort nach Abschluss der Zahlung zu Ihrem `return_url` umgeleitet, wodurch die Standardbestätigungsseite vollständig übersprungen wird.
</Note>
