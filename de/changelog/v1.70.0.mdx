...
1. **Inline-Checkout - Vollständig eingebettete Checkout-Erlebnisse (BETA)**  
   ...
   - **Ereignisse zur Echtzeitaufgliederung**: Erhalten Sie `checkout.breakdown` Ereignisse, während Kunden ihre Daten eingeben
   - **Benutzerdefinierte Bestellübersichten**: Erstellen Sie Ihre eigene Preisübersicht, die in Echtzeit aktualisiert wird
   - **Nahtlose Integration**: Der Checkout-Rahmen übernimmt die Zahlungsabwicklung, während Sie das Layout steuern
...
2. **Zahlungen, Abonnements und Checkouts mit vorhandenen Zahlungsmethoden erstellen**  
    Optimieren Sie die Checkout-Prozesse, indem Sie gespeicherte Zahlungsmethoden von bestehenden Kunden verwenden. Der neue `payment_method_id` Parameter ermöglicht es Ihnen, Zahlungen, Abonnements oder Checkout-Sitzungen mit der zuvor gespeicherten Zahlungsmethode eines Kunden zu erstellen.
...
5. **`redirect_immediately` Flag - Zahlungsbestätigungsseite überspringen**  
   Steuern Sie den Checkout-Prozess mit dem neuen `redirect_immediately` Flag. Wenn aktiviert, werden Kunden sofort nach Abschluss der Zahlung umgeleitet, wodurch die Standardbestätigungsseite umgangen wird, um ein schnelleres, reibungsloseres Erlebnis zu bieten.
  ...
  ```
   ...
   <Tip>
   Verwenden Sie `redirect_immediately: true`, wenn Sie eine benutzerdefinierte Erfolgsseite haben, die eine bessere Benutzererfahrung bietet als die Standard-Zahlungsbestätigungsseite.
   </Tip>

   <Note>
   Wenn `redirect_immediately` aktiviert ist, werden Kunden sofort nach Abschluss der Zahlung zu Ihrem `return_url` umgeleitet, wodurch die Standardbestätigungsseite vollständig übersprungen wird.
   </Note>
