...
- **Abonnementobjekt**: Neues `cancellation_feedback`-Feld (einer der oben genannten Werte) und `cancellation_comment` (optionaler freier Text), gefüllt, wenn der Kunde kündigt
- **`subscription.cancelled`-Webhook**: Beide Felder sind in der Nutzlast enthalten
- **API**: Übergeben Sie `cancellation_feedback` und `cancellation_comment` an `PATCH /subscriptions/{id}`, wenn Sie eine Kündigung programmatisch planen oder ausführen
...
Übergeben Sie `crypto` in `allowed_payment_method_types` bei der Erstellung einer Checkout-Sitzung:
