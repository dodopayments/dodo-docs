...
Erstellen Sie Ihr eigenes Figma-Plugin-Projekt mithilfe des [Figma Plugin-Quickstart-Guides](https://www.figma.com/plugin-docs/plugin-quickstart-guide/). Dies generiert ein Gerüst mit einer `manifest.json`-Datei.
...
Klonen Sie das [dodopayments-figma-Repository](https://github.com/dodopayments/dodopayments-figma) und aktualisieren Sie die `manifest.json` mit dem Namen und der ID aus Ihrem generierten Plugin-Ordner.
...
<Warning>
Verwenden Sie immer die `test_mode` während der Entwicklung, um echte Zahlungen zu vermeiden.
</Warning>
</Step>
...
Das Plugin benötigt Netzwerkzugriff, um mit dem CORS-Proxy zu kommunizieren. Fügen Sie Folgendes zu Ihrer `manifest.json` hinzu:
...
Konfigurieren Sie den API-Modus in `src/ui/api.ts`:
...
<Tip>
Wechseln Sie nur zu `live_mode`, wenn Sie bereit für die Produktion sind und gründlich im Testmodus getestet haben.
</Tip>
</Step>
</Steps>
...
| Befehl | Beschreibung |
|---------|-------------|
| `npm run build` | Bau das Plugin für die Produktion |
| `npm run dev` | Starte den Entwicklungsserver mit Dateibeobachtung |
...
2. Gründlich sowohl im Test- als auch im Live-Modus testen
3. Befolgen Sie die [Richtlinien für die Plugin-Veröffentlichung von Figma](https://www.figma.com/plugin-docs/publishing-plugins/)
4. Zur Überprüfung durch die Figma-Community einreichen
