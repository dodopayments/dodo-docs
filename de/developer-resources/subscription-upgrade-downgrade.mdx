...
<Card title="API zur Vorschau von Planänderungen" icon="eye" href="/api-reference/subscriptions/preview-change-plan">
...
So gehen Sie mit sofortiger Abrechnung um: `prorated_immediately`, `full_immediately` oder `difference_immediately`.
</ParamField>
...
- Gebühren für die partielle Differenz im aktuellen Zyklus
- Wenn im Testzeitraum, sofortige Gebühren und Wechsel zum neuen Plan
- Downgrade: kann eine anteilige Gutschrift für zukünftige Erneuerungen erzeugen

#### `full_immediately`
- Erhebt sofort den vollen Betrag des neuen Plans
- Ignoriert die verbleibende Zeit des alten Plans
...
- Upgrade: sofortige Gebühr für die Preisunterschiede zwischen alten und neuen Plänen
...
<Tip>
Wählen Sie `prorated_immediately` für eine faire Zeitabrechnung; wählen Sie `full_immediately` um die Abrechnung neu zu starten; verwenden Sie `difference_immediately` für einfache Upgrades und automatische Gutschriften bei Downgrades.
</Tip>
...
**Häufige Ursachen**:
- Erwartungen an den Prorationsmodus: Downgrades gutschreiben die volle Preisunterschied des Plans mit `difference_immediately`, während `prorated_immediately` eine anteilige Gutschrift basierend auf der verbleibenden Zeit im Zyklus erstellt
- Gutschriften sind abonnementspezifisch und übertragen sich nicht zwischen Abonnements
- Guthabenstand nicht im Kundendashboard sichtbar
...
- Implementieren Sie ein Kundenportal, um Guthabenstände anzuzeigen
...
- Lesen Sie den Rohdatenkörper der Anfrage vor jeder JSON-Parsing-Middleware
...
- Benachrichtigen Sie den Kunden, die Zahlungsmethode zu aktualisieren
- Implementieren Sie eine Wiederholungslogik für temporäre Fehler
- Erwägen Sie, Planänderungen mit fehlgeschlagenen sofortigen Gebühren zuzulassen
</Accordion>

<Accordion title="Abonnement pausiert nach Planänderung">
...
1. **Zahlungsmethode aktualisieren** mit der Update Payment Method API
...
<Step title="Testen Sie verschiedene Prorationsmodi">
...
<Accordion title="401 Unauthorisiert">
...
- Implementieren Sie Benachrichtigungen für `subscription.on_hold`
