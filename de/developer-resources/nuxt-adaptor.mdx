---
title: Nuxt-Adapter
description: Integrieren Sie <CardGroup cols={2}> mit dem offiziellen Nuxt-Modul in Ihr Nuxt-Projekt. Behandelt Checkout,
  Customer Portal, Webhooks und die sichere Einrichtung der Umgebung.
icon: code-merge
keywords:
  - Dodo Payments
  - Nuxt
...
<Card title="Checkout API Route" icon="cart-shopping" href="#checkout-api-route">
    Integrieren Sie den Checkout von Dodo Payments über eine Server-Route in Ihre Nuxt-App.
</Card>
<Card title="Customer Portal API Route" icon="user" href="#customer-portal-api-route">
    Ermöglichen Sie Kunden, Abonnements und Details über eine Nuxt-Server-Route zu verwalten.
</Card>
...
<Info>
Diese Anleitung erklärt, wie Sie Dodo Payments mithilfe des offiziellen Nuxt-Moduls in Ihre Nuxt-Anwendung integrieren. Sie lernen, wie Sie Checkout-, Customer-Portal- und Webhook-API-Routen einrichten und Umgebungsvariablen sicher verwalten.
</Info>
...
CODE_PLACEHOLDER_2ab8cd524713e4e4e1_END

</Step>
</Steps>

<Warning>
Committen Sie Ihre <code>.env</code>-Datei oder Secrets niemals in die Versionsverwaltung.
</Warning>

## Beispiele für API-Route-Handler

<Info>
Alle Dodo Payments-Integrationen in Nuxt werden über Server-Routen im Verzeichnis <code>server/routes/api/</code> verarbeitet.
</Info>

<Tabs>
<Tab title="Checkout API Route">
<Info>
Verwenden Sie diesen Handler, um den Checkout von Dodo Payments in Ihre Nuxt-App zu integrieren. Unterstützt statische (GET), dynamische (POST) und Session-basierte (POST) Zahlungsabläufe.
</Info>
<CodeGroup>
...
<Info>
Erstellen Sie eine GET-Route, über die Kunden auf ihr Portal zugreifen können. Akzeptiert <code>customer_id</code> als Query-Parameter.
</Info>
<CodeGroup>
...
**Query-Parameter:**
- <code>customer_id</code> (erforderlich): Die Kunden-ID für die Portal-Session (z. B. <code>?customer_id=cus_123</code>)
- <code>send_email</code> (optional, boolean): Wenn <code>true</code>, wird dem Kunden eine E-Mail mit dem Portal-Link gesendet.
...
- **Statische Payment Links:** Sofort teilbare URLs für eine schnelle Zahlungserfassung ohne Code.
- **Dynamische Payment Links:** Erzeugen Sie Payment Links programmgesteuert mit benutzerdefinierten Details über die API oder SDKs.
- **Checkout-Sessions:** Erstellen Sie sichere, anpassbare Checkout-Erlebnisse mit vorkonfigurierten Produktwarenkörben und Kundendetails.
...
Legt den berechneten Betrag in Haupteinheiten der Währung fest (z. B. <code>12.5</code> für 12,50 $). Nur für Pay What You Want-Produkte und wird ignoriert, wenn der Betrag unter dem Mindestpreis des Produkts liegt.
</ParamField>
...
<Warning>
Dynamic Checkout stellt die veralteten `POST /payments`- und `POST /subscriptions`-Endpunkte als Proxy bereit. Es funktioniert weiterhin für bestehende Integrationen, aber neue Integrationen sollten die unten aufgeführten **Checkout Sessions** verwenden.
</Warning>

### Antwortformat
Dynamic Checkout gibt eine JSON-Antwort mit der Checkout-URL zurück:
...
Checkout Sessions bieten eine sicherere, gehostete Checkout-Erfahrung, die den vollständigen Zahlungsablauf sowohl für einmalige Käufe als auch für Abonnements abwickelt und dabei umfassende Anpassungsmöglichkeiten bietet.

Weitere Informationen und eine vollständige Liste der unterstützten Felder finden Sie im [Integrationsleitfaden für Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session).
...
## Route Handler für Customer Portal

Der Route Handler für Customer Portal ermöglicht Ihnen die nahtlose Integration des Customer Portal von Dodo Payments in Ihre Nuxt-Anwendung.
...
Wenn dieser Wert auf <code>true</code> gesetzt ist, wird dem Kunden eine E-Mail mit dem Portal-Link gesendet.
</ParamField>
...
- **Signaturüberprüfung:** Die Webhook-Signatur wird mithilfe von <code>webhookKey</code> überprüft. Bei fehlgeschlagener Überprüfung wird 401 zurückgegeben.
- **Payload-Validierung:** Die Validierung erfolgt mit Zod. Bei einem ungültigen Payload wird 400 zurückgegeben.
- **Fehlerbehandlung:**
  - 401: Ungültige Signatur
  - 400: Ungültiger Payload
  - 500: Interner Fehler während der Überprüfung
- **Event-Routing:** Der entsprechende Event-Handler wird basierend auf dem Payload-Typ aufgerufen.
