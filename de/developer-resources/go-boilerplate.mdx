...
<Info>
Dieses Boilerplate verwendet Go 1.21+ mit sauberen Architekturmustern (`cmd`, `internal`, `templates`), HTML-Vorlagen und das `dodopayments-go` SDK für nahtlose API-Integration.
</Info>
...
- **Moderne UI** - Saubere, dunkel gestaltete Preisseite mit HTML-Vorlagen
...
<Warning>
Verpflichten Sie niemals Ihre `.env` Datei in die Versionskontrolle. Sie ist bereits in `.gitignore` enthalten.
</Warning>
</Step>
...
- Wechseln Sie von `DODO_PAYMENTS_ENVIRONMENT` zu `live_mode`
