...
Standorte von Konfigurationsdateien:
...
1. **MCP-Authentifizierungscache löschen**: `rm -rf ~/.mcp-auth`
...
<Tip>
Verwenden Sie `@latest`, um immer die aktuellste Version zu ziehen, oder fixieren Sie eine bestimmte Version wie `@1.0.0` für Stabilität.
</Tip>
...
<Step title="Cursor-Einstellungen öffnen">
    Navigieren Sie zu **Cursor-Einstellungen** > **Funktionen** > **Modellkontextprotokoll** oder drücken Sie `Cmd/Ctrl + Shift + P` und suchen Sie nach "MCP-Einstellungen".
</Step>

<Step title="Konfiguration bearbeiten">
    Klicken Sie auf **Konfig bearbeiten**, um die `mcp_config.json`-Datei zu öffnen.
</Step>
...
Nach dem Klicken, setzen Sie Ihre Umgebungsvariablen in VS Codes `mcp.json` über **Befehls-Palette** > **MCP: Benutzereinstellung öffnen**.

**Manuelle Konfiguration**

<Steps>
<Step title="MCP-Konfiguration öffnen">
    Öffnen Sie die Befehls-Palette (`Cmd/Ctrl + Shift + P`) und suchen Sie nach "MCP: Benutzereinstellung öffnen".
</Step>
...
| `DODO_PAYMENTS_WEBHOOK_KEY` | Ihr Webhook-Signaturschlüssel | Nein |
...
- `valid-json` - Korrekte JSON-Zeichenfolgenargumente
- `refs` - Unterstützung für $ref-Pointer in Schemata
- `unions` - Unterstützung für Unionstypen (anyOf)
- `formats` - Unterstützung für Formatvalidierungen
- `tool-name-length=N` - Maximale Toolnamenlänge
...
<Tip>
Verwenden Sie `--operation=read` während der Entwicklung, um versehentliche Modifikationen Ihrer Daten zu verhindern.
</Tip>
...
3. **`invoke_api_endpoint`** - Führt eine beliebige Methode mit geeigneten Parametern aus
...
Bei der Bereitstellung im Remote-Betrieb immer eine Authentifizierung über den `Authorization`-Header oder `x-dodo-payments-api-key`-Header verlangen.
...
Einige Clients unterstützen nicht alle JSON-Schema-Funktionen. Verwenden Sie `--describe-capabilities`, um zu verstehen, was unterstützt wird.
</Accordion>
</AccordionGroup>
