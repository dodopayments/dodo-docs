...
- **Keine API-Schlüssel in Parametern**: API-Schlüssel werden serverseitig injiziert und niemals in den an das LLM gesendeten Tool-Parametern exponiert.
- **Isolierte Sandbox**: Code wird in einer sicheren Umgebung ausgeführt, ohne Zugriff auf das Netzwerk oder das Host-Dateisystem.
- **Kontrolliertes SDK**: Dem Agenten stehen nur autorisierte SDK-Methoden zur Verfügung.

### Skalierbar auf jede API-Größe
Wenn eine API wächst, verschlechtert sich die Leistung traditioneller MCPs, weil mehr Tools in den Kontext geladen werden müssen. Code Mode bleibt mit 2 Tools konstant, unabhängig von der Oberfläche der API. Cloudflare hat erfolgreich über 2.500 API-Endpunkte in nur 2 Tools und etwa 1.000 Token des Kontexts reduziert.

<Info>
Für weitere Details zu den Vorteilen dieser Architektur, siehe die Engineering-Blogs von [Anthropic](https://www.anthropic.com/engineering/advanced-tool-use) und [Cloudflare](https://blog.cloudflare.com/code-mode/), und die [Programmatic Tool Calling](https://platform.claude.com/docs/en/agents-and-tools/tool-use/programmatic-tool-calling) Dokumentation von Claude.
</Info>
...
Erkunden Sie die komplette Dodo Payments API-Dokumentation
</Card>
