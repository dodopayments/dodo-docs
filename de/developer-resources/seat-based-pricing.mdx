...
Füllen Sie diese Werte für unser Sitz-Add-On aus:
...
**Wiederkehrender Preis**: Eingeben → `49.00`

**Abrechnungszyklus**: Auswählen → `Monthly`
...
<Step title="Fügen Sie das Sitz-Add-On hinzu">
...
<Warning>
Verpflichten Sie niemals Ihren API-Schlüssel zur Versionskontrolle. Fügen Sie `.env` zu Ihrer `.gitignore` Datei hinzu.
</Warning>
</Step>
...
Fügen Sie dies zu Ihrem `src/server.ts` hinzu, um die HTML-Datei bereitzustellen:
...
2. Sie sollten die Demooberfläche für die sitzplatzbasierte Preisgestaltung sehen
3. Probieren Sie verschiedene Sitzplatzmengen (0, 3, 10 usw.) aus
