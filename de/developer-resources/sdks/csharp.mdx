...
<Warning>
Die Methode `POST /subscriptions` (die `subscriptions.create`-Methode des SDK) ist **veraltet**. Sie funktioniert weiterhin für bestehende Integrationen, aber neue Integrationen sollten Abonnements über eine [Checkout Session](/developer-resources/checkout-session) erstellen.
</Warning>
...
<Info>
`Billing` erfordert mindestens den zweistelligen ISO-Code `Country`. Verwenden Sie `AttachExistingCustomer`, um einen bestehenden Kunden zuzuordnen, oder `NewCustomer`, um einen neuen Kunden zu erstellen. `ProductPrice` wird in der kleinsten Währungseinheit angegeben.
</Info>

## Fehlerbehandlung

Das SDK löst abhängig vom HTTP-Statuscode spezifische Ausnahmen aus. Alle 4xx-Fehler erben von `DodoPayments4xxException`.
...
Andere Ausnahmetypen:
- `DodoPaymentsIOException`: Netzwerkfehler bei der Ein-/Ausgabe
- `DodoPaymentsInvalidDataException`: Fehler bei der Interpretation geparster Daten
...
Durchlaufen Sie alle Ergebnisse über alle Seiten hinweg mit der Methode `Paginate`, die ein `IAsyncEnumerable` zurückgibt:
...
<Tip>
Verwenden Sie für die Entwicklung [Benutzergeheimnisse](https://learn.microsoft.com/en-us/aspnet/core/security/app-secrets), anstatt Schlüssel in `appsettings.json` zu speichern:
...
<Card title="NuGet Package" icon="box" href="https://www.nuget.org/packages/DodoPayments.Client">
  Paket in der NuGet Gallery anzeigen
</Card>
...
Benötigen Sie Hilfe mit dem C# SDK?
...
- **GitHub**: Erstellen Sie ein Issue im [Repository](https://github.com/dodopayments/dodopayments-csharp)
