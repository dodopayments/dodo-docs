...
Passe den Schließen-Button, den Präsentationsstil und das Farbschema des Sheets an
über `customization` auf `start(...)`. Alle Felder sind optional. Wenn
`customization` weggelassen wird, wird die standardmäßige Darstellung von iOS `SFSafariViewController` verwendet.
