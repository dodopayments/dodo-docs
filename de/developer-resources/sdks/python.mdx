...
<Info>
`billing` erfordert mindestens den zweistelligen ISO-Ländercode. `customer` akzeptiert entweder `{"customer_id": ...}`, um einen vorhandenen Kunden anzuhängen oder `{"email": ..., "name": ...}`, um einen neuen zu erstellen. `product_price` ist in der niedrigsten Währungsdenomination.
</Info>

## Nutzungsbasierte Abrechnung

### Verarbeitungsereignisse einlesen

Benutzerdefinierte Ereignisse für die nutzungsbasierte Abrechnung verfolgen:
...
Automatisch durch alle Elemente iterieren:
...
Protokollierung aktivieren, indem die Umgebungsvariable festgelegt wird:

```bash
export DODO_PAYMENTS_LOG=info
```

Oder für Debug-Level-Protokollierung:
...
- **GitHub**: Öffnen Sie ein Issue auf dem [Repository](https://github.com/dodopayments/dodopayments-python)

## Mitarbeit

Wir freuen uns über Beiträge! Lesen Sie die [Richtlinien für Beiträge](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md), um loszulegen.
