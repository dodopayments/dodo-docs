...
Das Python SDK bietet eine pythonische Schnittstelle zur Dodo Payments API und stellt sowohl synchrone als auch asynchrone Clients mit Typdefinitionen für Anfragen und Antworten bereit. Es unterstützt Python 3.9+ und umfasst umfassende Testabdeckung.
...
<Info>
Das SDK erfordert Python 3.9 oder höher. Wir empfehlen die Verwendung der aktuellsten stabilen Python-Version für das beste Erlebnis und Sicherheitsupdates.
</Info>
...
<Warning>
Speichere deine API-Schlüssel stets sicher über Umgebungsvariablen. Committe sie niemals in ein Versionskontrollsystem.
</Warning>
...
<Card title="Async/Await" icon="bolt">
  Volle Unterstützung für asynchrone Operationen mit asyncio und optionaler aiohttp-Integration
</Card>

<Card title="Type Hints" icon="check">
  Umfassende Type-Hints für bessere IDE-Unterstützung und Typüberprüfung mit mypy
</Card>
...
<Warning>
`POST /subscriptions` (the SDK's `subscriptions.create` method) ist **veraltet**. Es funktioniert weiterhin für bestehende Integrationen, aber neue Integrationen sollten Abonnements über eine [Checkout Session](/developer-resources/checkout-session) erstellen.
</Warning>
...
<Info>
`billing` erfordert mindestens den zweistelligen ISO-Ländercode. `customer` akzeptiert entweder `{"customer_id": ...}`, um einen bestehenden Kunden zu verknüpfen, oder `{"email": ..., "name": ...}`, um einen neuen Kunden zu erstellen. `product_price` wird in der kleinsten Währungseinheit angegeben.
</Info>
...
Alle Elemente automatisch durchlaufen:
...
Den zugrunde liegenden `httpx`-Client anpassen:
...
aiohttp für verbesserte asynchrone Leistung verwenden:
...
Die Protokollierung durch Setzen der Umgebungsvariable aktivieren:

```bash
export DODO_PAYMENTS_LOG=info
```

Oder für die Protokollierung auf Debug-Ebene:
...
Benötigst du Hilfe mit dem Python SDK?
...
- **GitHub**: Erstelle ein Issue im [Repository](https://github.com/dodopayments/dodopayments-python)

## Mitwirken

Wir freuen uns über Beiträge! Lies die [Richtlinien zum Mitwirken](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md), um loszulegen.
