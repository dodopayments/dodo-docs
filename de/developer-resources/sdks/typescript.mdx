...
<Info>
Ereignisse müssen eindeutige `event_id` Werte für die Idempotenz haben. Doppelte IDs innerhalb derselben Anfrage werden abgelehnt, und nachfolgende Anfragen mit vorhandenen IDs werden ignoriert.
</Info>
...
- `'debug'` - Debug-Nachrichten, Informationen, Warnungen und Fehler anzeigen
- `'info'` - Informationen, Warnungen und Fehler anzeigen  
- `'warn'` - Warnungen und Fehler anzeigen (Standard)
- `'error'` - Nur Fehler anzeigen
- `'off'` - Alle Protokollierungen deaktivieren
...
List-Methoden in der DodoPayments API sind paginiert. Sie können die `for await … of` Syntax verwenden, um durch Elemente über alle Seiten zu iterieren:
...
- Node.js 20 LTS oder spätere ([non-EOL](https://endoflife.date/nodejs)) Versionen
