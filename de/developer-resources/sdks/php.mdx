...
<Warning>
`POST /subscriptions` (die Methode `subscriptions.create` des SDK) ist **veraltet**. Sie funktioniert weiterhin für bestehende Integrationen, aber neue Integrationen sollten Abonnements über eine [Checkout Session](/developer-resources/checkout-session) erstellen.
</Warning>
...
<Info>
`billing` erfordert mindestens den zweistelligen ISO-Standardcode `country`. Übergeben Sie `AttachExistingCustomer::with(customerID: '...')`, um einen bestehenden Kunden zu verknüpfen, oder `NewCustomer::with(email: '...', name: '...')`, um einen Kunden zu erstellen. `productPrice` wird in der kleinsten Währungseinheit angegeben.
</Info>
...
Wenn die Bibliothek keine Verbindung zur API herstellen kann oder einen nicht erfolgreichen Statuscode (4xx oder 5xx) empfängt, wird eine Unterklasse von `APIException` ausgelöst:
...
| Zeitüberschreitung          | `APITimeoutException`          |
| Netzwerkfehler    | `APIConnectionException`       |

<Tip>
  Umschließen Sie API-Aufrufe immer mit try-catch-Blöcken, um potenzielle Fehler
  ordnungsgemäß zu behandeln und Nutzern aussagekräftiges Feedback zu geben.
</Tip>
...
Stellen Sie Anfragen an nicht dokumentierte Endpunkte:
...
Senden Sie nicht dokumentierte Parameter an beliebige Endpunkte oder lesen Sie nicht dokumentierte Eigenschaften von Antworten aus:
...
<Note>
  Die Parameter `extra*` mit demselben Namen überschreiben dokumentierte Parameter.
</Note>
...
Registrieren Sie ihn in `config/services.yaml`:
...
- **GitHub**: Erstellen Sie ein Issue im [Repository](https://github.com/dodopayments/dodopayments-php)

## Mitwirken

Wir freuen uns über Beiträge! Lesen Sie die [Richtlinien zum Mitwirken](https://github.com/dodopayments/dodopayments-php/blob/main/CONTRIBUTING.md), um loszulegen.
