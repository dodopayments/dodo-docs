...
description: Verstehen Sie die verschiedenen Webhook-Ereignisse, die durch Dodo
  Payments während Zahlungs-, Erstattungs-, Abonnement-, Streitfall-,
  Lizenzschlüssel-, Wiederherstellungs- und Mahnprozesse ausgelöst werden.
icon: bell
keywords:
  - Dodo Payments
  - Webhook-Ereignisse
  ...
  - Mahnverfahren-Ereignisse
  - verlassener Checkout
---
...
| **Ereignisname**                    | **Ereignistyp**                       | **Beschreibung**                                                                  |
|-------------------------------------|---------------------------------------|-----------------------------------------------------------------------------------|
| **Verlassener Checkout erkannt**     | `abandoned_checkout.detected` | Ausgelöst, wenn ein unvollständiger oder fehlgeschlagener Checkout nach 60 Minuten erkannt wird. |
| **Verlassener Checkout wiederhergestellt** | `abandoned_checkout.recovered` | Tritt auf, wenn ein Kunde die Zahlung über einen Wiederherstellungslink abschließt.              |
...
| **Ereignisname**       | **Ereignistyp**              | **Beschreibung**                                                                  |
|------------------------|------------------------------|-----------------------------------------------------------------------------------|
| **Mahnverfahren gestartet** | `dunning.started`         | Ausgelöst, wenn ein Mahnversuch für ein pausiertes oder gekündigtes Abonnement beginnt. |
| **Mahnverfahren wiederhergestellt** | `dunning.recovered`       | Tritt auf, wenn ein Kunde seine Zahlungsmethode aktualisiert und die entsprechende Belastung erfolgreich ist. |

Für detaillierte Payload-Schemata, Feldbeschreibungen und Handler-Beispiele siehe [Recovery Webhooks](/developer-resources/webhooks/intents/recovery).

Diese Webhook-Ereignisse sind darauf ausgelegt, Händlern kritische Updates bereitzustellen, um ihre Vorgänge effektiv zu verwalten. Implementieren Sie robuste Handler für jedes relevante Ereignis in Ihrer Webhook-Integration, um eine reibungslose und zuverlässige Funktionalität sicherzustellen.
