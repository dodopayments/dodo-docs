...
5. Kopieren Sie das **Signaturgeheimnis**
...
✅ **Ereignisprotokollierung** - Vollständige Prüfungskette in `webhook_events`-Tabelle  
✅ **Fehlerbehandlung** - Protokolliert und wiederholbar

> **Hinweis:** Diese Implementierung zeigt die Verarbeitung von drei Kern-Abonnementereignissen (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) mit minimalen Feldern. Sie können sie leicht erweitern, um zusätzliche Ereignistypen und Felder gemäß Ihren Anforderungen zu unterstützen.
...
Der Webhook-Handler:
1. **Deaktiviert die Körperanalyse** - Um den rohen Körper für die Signaturüberprüfung zuzugreifen
2. **Überprüft die Signatur** - Stellt sicher, dass die Anfrage von DodoPayments kommt, indem HMAC-SHA256 verwendet wird
3. **Überprüft auf Duplikate** - Verwendet die Webhook-ID, um zu verhindern, dass das gleiche Ereignis zweimal verarbeitet wird
4. **Protokolliert das Ereignis** - Speichert den rohen Webhook in der `webhook_events`-Tabelle für die Prüfung
5. **Verarbeitet das Ereignis** - Erstellt oder aktualisiert Kunden und Abonnements in Neon
6. **Behandelt Fehler** - Protokolliert Fehlermeldungen und markiert das Ereignis als nicht verarbeitet für einen Wiederholungsversuch
