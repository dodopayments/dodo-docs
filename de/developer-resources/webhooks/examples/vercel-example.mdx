...
1. Registrieren Sie sich bei [Neon](https://neon.com)
2. Erstellen Sie ein neues Projekt
3. Öffnen Sie den SQL-Editor
4. Kopieren und fügen Sie den Inhalt von [`schema.sql`](#database-schema) ein
5. Führen Sie die Abfrage aus
6. Rufen Sie Ihre Verbindungszeichenfolge bei Neon → Connection Details ab
...
1. Gehen Sie zum [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
...
✅ **Ereignisprotokollierung** - Vollständiger Audit-Trail in der Tabelle `webhook_events`  
...
Der Webhook-Handler:
1. **Deaktiviert die Body-Verarbeitung** - Damit auf den Roh-Body zur Signaturprüfung zugegriffen werden kann
2. **Überprüft die Signatur** - Stellt sicher, dass die Anfrage von DodoPayments stammt, indem HMAC-SHA256 verwendet wird
3. **Prüft auf Duplikate** - Nutzt die Webhook-ID, um die doppelte Verarbeitung desselben Ereignisses zu verhindern
4. **Protokolliert das Ereignis** - Speichert den Roh-Webhook in der Tabelle `webhook_events` für den Audit-Trail
5. **Verarbeitet das Ereignis** - Erstellt oder aktualisiert Kunden und Abonnements in Neon
6. **Behandelt Fehler** - Protokolliert Fehler und markiert das Ereignis als nicht verarbeitet zur Wiederholung
