...
{/* LOCKED_PATTERN_2b313346674bb180793c904f406c4c27 */}
  Vollständiger Quellcode und Einrichtungshandbuch
</Card>
...
6. Hole deine Verbindungszeichenfolge von Neon → Connection Details
...
Bearbeite `wrangler.toml` und setze deinen Worker-Namen:
...
1. Gehe zu [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
2. Erstelle einen neuen Webhook-Endpunkt
3. Konfiguriere deine Webhook-URL als Endpunkt
4. Aktiviere diese Abonnement-Ereignisse:
   - `subscription.active`
...
✅ Signaturprüfung - Verwendung der dodopayments-Bibliothek  
✅ Idempotenz - Verhindert doppelte Verarbeitung mit Webhook-IDs  
✅ Ereignisprotokollierung - Vollständige Prüfnachweis in `webhook_events` Tabelle  
✅ Fehlerbehandlung - Protokolliert und erneut versuchbar

> **Hinweis:** Diese Implementierung zeigt die Verarbeitung von drei Kern-Abonnement-Ereignissen (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) mit minimalen Feldern. Du kannst sie einfach erweitern, um weitere Ereignistypen und Felder gemäß deinen Anforderungen zu unterstützen.
...
Der Webhook-Handler:
1. **Prüft die Signatur** - Stellt sicher, dass die Anfrage von DodoPayments stammt, mithilfe von HMAC-SHA256
2. **Überprüft auf Duplikate** - Verwendet die Webhook-ID, um die Verarbeitung desselben Ereignisses zweimal zu verhindern
3. **Protokolliert das Ereignis** - Speichert den rohen Webhook in der `webhook_events` Tabelle für den Prüfnachweis
4. **Verarbeitet das Ereignis** - Erstellt oder aktualisiert Kunden und Abonnements
5. **Behandelt Fehler** - Protokolliert Ausfälle und markiert das Ereignis als nicht verarbeitet für einen erneuten Versuch
...
| Problem | Lösung |
|-------|----------|
| Verifikation fehlgeschlagen | Überprüfe, ob der Webhook-Schlüssel korrekt aus dem DodoPayments-Dashboard stammt |
| Datenbankverbindungsfehler | Verifiziere die Neon-Verbindungszeichenfolge |
| Worker wird nicht bereitgestellt | Führe `npm install` aus, prüfe `wrangler.toml` auf Syntax |
