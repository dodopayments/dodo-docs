...
> **Hinweis:** Wir setzen `DODO_PAYMENTS_WEBHOOK_KEY` nach der Bereitstellung, wenn Sie Ihre Webhook-URL haben.
...
✅ **Signaturüberprüfung** - Mit der dodopayments-Bibliothek  
...
> **Hinweis:** Diese Implementierung demonstriert die Handhabung von drei Kern-Abonnementereignissen (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) mit minimalen Feldern. Sie können sie einfach erweitern, um zusätzliche Ereignistypen und Felder basierend auf Ihren Anforderungen zu unterstützen.
...
Der Webhook-Handler:
1. **Überprüft die Signatur** - Stellt sicher, dass die Anfrage von DodoPayments stammt, unter Verwendung von HMAC-SHA256
2. **Überprüft auf Duplikate** - Verwendet die Webhook-ID, um die Verarbeitung desselben Ereignisses zweimal zu verhindern
3. **Protokolliert das Ereignis** - Speichert das rohe Webhook in der `webhook_events`-Tabelle zur Prüfspur
4. **Verarbeitet das Ereignis** - Erstellt oder aktualisiert Kunden und Abonnements
5. **Behandelt Fehler** - Protokolliert Fehler und kennzeichnet das Ereignis als nicht verarbeitet zur Wiederholung
