...
4. Kopieren Sie den Inhalt von [`schema.sql`](#database-schema) und fügen Sie ihn ein
5. Führen Sie die Abfrage aus
6. Holen Sie sich Ihren Verbindungsstring von Neon → Verbindungsdetails
...
✅ **Signaturverifizierung** - Verwendung der dodopayments-Bibliothek  
✅ **Idempotenz** - Verhindert die doppelte Verarbeitung mit Webhook-IDs  
✅ **Ereignisprotokollierung** - Vollständige Überprüfung in der `webhook_events`-Tabelle  
✅ **Fehlerbehandlung** - Protokolliert und wiederholbar

> **Hinweis:** Diese Implementierung demonstriert die Verarbeitung von drei zentralen Abonnementereignissen (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) mit minimalen Feldern. Sie können sie leicht erweitern, um zusätzliche Ereignisarten und Felder basierend auf Ihren Anforderungen zu unterstützen.
...
Der Webhook-Handler:
1. **Überprüft die Signatur** - Stellt sicher, dass die Anfrage von DodoPayments kommt, unter Verwendung von HMAC-SHA256
2. **Überprüft auf Duplikate** - Verwendet die Webhook-ID, um die Verarbeitung desselben Ereignisses zweimal zu verhindern
3. **Protokolliert das Ereignis** - Speichert den Roh-Webhook in der `webhook_events`-Tabelle für die Überprüfung
...
3. Sehen Sie sich die Echtzeitprotokolle und Aufrufhistorie an
...
| Datenbankverbindungsfehler | Überprüfen Sie den Neon-Verbindungsstring und verwenden Sie eine gepoolte Verbindung |
| Funktion nicht gefunden (404) | Führen Sie `netlify init` und `npm run deploy` erneut aus |
| Env-Variablen nicht verfügbar | Setzen Sie Variablen in der CLI oder im Dashboard und stellen Sie dann erneut bereit |
