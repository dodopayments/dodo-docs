...
1. Melde dich bei [Neon](https://neon.com) an
2. Erstelle ein neues Projekt
3. Öffne den SQL-Editor
4. Kopiere und füge den Inhalt von [`schema.sql`](#database-schema) ein
5. Führe die Abfrage aus
6. Hole deinen Verbindungsstring von Neon → Connection Details
...
> **Hinweis:** Wir setzen `DODO_PAYMENTS_WEBHOOK_KEY` nach der Bereitstellung, sobald du deine Webhook-URL hast.
...
1. Gehe zum [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
...
✅ **Signaturprüfung** – Mit der dodopayments-Bibliothek  
✅ **Idempotenz** – Verhindert doppelte Verarbeitung durch Webhook-IDs  
✅ **Ereignisprotokollierung** – Vollständige Prüfspur in `webhook_events` Tabelle  
✅ **Fehlerbehandlung** – Protokolliert und erneut ausführbar

> **Hinweis:** Diese Implementierung zeigt die Handhabung von drei Kern-Abonnementereignissen (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) mit minimalen Feldern. Du kannst sie problemlos erweitern, um zusätzliche Ereignistypen und Felder gemäß deinen Anforderungen zu unterstützen.
...
Der Webhook-Handler:
1. **Überprüft die Signatur** – Stellt sicher, dass die Anfrage von DodoPayments stammt, mithilfe von HMAC-SHA256
2. **Prüft auf Duplikate** – Verwendet die Webhook-ID, um zu verhindern, dass dasselbe Ereignis zweimal verarbeitet wird
3. **Protokolliert das Ereignis** – Speichert den rohen Webhook in `webhook_events` Tabelle für die Prüfspur
4. **Verarbeitet das Ereignis** – Erstellt oder aktualisiert Kunden und Abonnements in Neon
5. **Behandelt Fehler** – Protokolliert Fehler und markiert das Ereignis als nicht verarbeitet zur erneuten Ausführung
...
**Protokolle im Netlify Dashboard ansehen:**
1. Wähle deine Website aus → Reiter **Functions**
2. Klicke auf die `webhook` Funktion
3. Sieh dir Echtzeitprotokolle und Aufrufhistorie an
...
| Datenbankverbindungsfehler | Überprüfe den Neon-Verbindungsstring und verwende eine gepoolte Verbindung |
| Funktion nicht gefunden (404) | Führe `netlify init` und `npm run deploy` erneut aus |
| Umgebungsvariablen nicht verfügbar | Setze die Variablen in der CLI oder im Dashboard und deploye erneut |
