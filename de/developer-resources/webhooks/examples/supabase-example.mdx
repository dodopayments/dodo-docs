...
keywords:
  ...
  - PostgreSQL
  - Webhook-Handler
  - Webhook-Bereitstellung
---

<Card title="GitHub Repository" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/supabase">
  Vollständiger Quellcode und Installationsanleitung
</Card>

<Tip>
  <strong>Sehen Sie sich das komplette Starter-Kit für Supabase an</strong><br />
  Ein minimales Subscription-Starter-Kit, das mit Next.js, Supabase und Dodo Payments erstellt wurde. Dieses Boilerplate hilft Ihnen, schnell ein abonnementbasiertes SaaS mit Authentifizierung, Zahlungen und Webhooks einzurichten.
...
4. Kopieren und fügen Sie den gesamten Inhalt von [`schema.sql`](#database-schema) ein
5. Führen Sie die Abfrage aus

### 4. Setze Anfangsgeheimnisse

Supabase stellt `SUPABASE_URL` und `SUPABASE_SERVICE_ROLE_KEY` zur Laufzeit automatisch bereit.
...
1. Gehen Sie zu [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
2. Erstellen Sie einen neuen Webhook-Endpunkt
3. Konfigurieren Sie Ihre Webhook-URL als Endpunkt
4. Aktivieren Sie diese Subscription-Ereignisse:
   - `subscription.active`
...
✅ **Signaturverifikation** – Verwendet die dodopayments-Bibliothek  
✅ **Idempotenz** – Verhindert doppelte Verarbeitung mit Webhook-IDs  
✅ **Ereignisprotokollierung** – Vollständige Audit-Trail in der `webhook_events`-Tabelle  
✅ **Fehlerbehandlung** – Protokolliert und erneut versuchbar

> **Hinweis:** Diese Implementierung demonstriert die Verarbeitung von drei zentralen Subscription-Ereignissen (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) mit minimalen Feldern. Sie können sie leicht erweitern, um zusätzliche Ereignistypen und Felder entsprechend Ihren Anforderungen zu unterstützen.
...
Die auf Deno basierende Edge Function:
1. **Überprüft die Signatur** – Verwendet die dodopayments-Bibliothek für HMAC-SHA256-Verifikation
2. **Prüft Idempotenz** – Sucht nach der Webhook-ID, um doppelte Verarbeitung zu verhindern
3. **Protokolliert das Ereignis** – Speichert rohe Webhook-Daten in der `webhook_events`-Tabelle
4. **Verarbeitet Aktualisierungen** – Erstellt oder aktualisiert Kunden und Abonnements über den Supabase-Client
5. **Handhabt Fehler** – Protokolliert Ausfälle und markiert das Ereignis zur Wiederholung
...
<Note>
Das `--no-verify-jwt`-Flag ist erforderlich, da Webhooks keine JWT-Tokens enthalten. Die Sicherheit wird durch die Überprüfung der Webhook-Signatur bereitgestellt.
</Note>
...
Oder im [Supabase Dashboard](https://supabase.com/dashboard) → Edge Functions → webhook → Protokoll-Tab
...
| Problem | Lösung |
|---------|--------|
| Verifikation fehlgeschlagen | Überprüfen Sie, ob der Webhook-Schlüssel aus dem DodoPayments-Dashboard korrekt ist |
| Datenbank-Berechtigungsfehler | Stellen Sie sicher, dass Sie den Service Role Key verwenden |
| JWT-Verifikationsfehler | Bereitstellen mit dem `--no-verify-jwt`-Flag |
| Funktion nicht gefunden | Überprüfen Sie, ob der Projekt-Ref korrekt ist und die Funktion bereitgestellt wurde |
