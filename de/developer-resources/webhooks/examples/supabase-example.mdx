...
4. Kopieren Sie den gesamten Inhalt von [`schema.sql`](#database-schema)
...
1. Gehen Sie zum [DodoPayments-Dashboard](https://app.dodopayments.com) → Entwickler → Webhooks
...
✅ **Signaturüberprüfung** - Verwendung der dodopayments-Bibliothek  
✅ **Idempotenz** - Verhindert doppelte Verarbeitung mit Webhook-IDs  
✅ **Ereignisprotokollierung** - Vollständiger Audit-Trail in `webhook_events`-Tabelle  
✅ **Fehlerbehandlung** - Protokolliert und wiederholbar

> **Hinweis:** Diese Implementierung demonstriert die Handhabung von drei Kernabonnementereignissen (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) mit minimalen Feldern. Sie können sie leicht erweitern, um zusätzliche Ereignistypen und Felder basierend auf Ihren Anforderungen zu unterstützen.
...
Die Deno-basierte Edge-Funktion:
1. **Verifiziert die Signatur** - Verwendet die dodopayments-Bibliothek für HMAC-SHA256-Überprüfung
...
5. **Behandelt Fehler** - Protokolliert fehlerhafte Vorgänge und kennzeichnet Ereignisse zur Wiederholungen
...
<Hinweis>
Das `--no-verify-jwt`-Flag ist erforderlich, da Webhooks keine JWT-Token enthalten. Die Sicherheit wird durch die Überprüfung der Webhook-Signatur gewährleistet.
</Hinweis>
...
| Problem | Lösung |
|-------|----------|
| Überprüfung fehlgeschlagen | Überprüfen Sie, ob der Webhook-Schlüssel im DodoPayments-Dashboard korrekt ist |
| Datenbankberechtigungsfehler | Stellen Sie sicher, dass der Service Role Key verwendet wird |
| JWT-Überprüfungsfehler | Bereitstellung mit `--no-verify-jwt`-Flag |
| Funktion nicht gefunden | Überprüfen Sie, ob der Projektref korrekt und die Funktion bereitgestellt ist |
