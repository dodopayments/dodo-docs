...
2. Erstellen Sie ein Produkt (Einmalzahlung oder Abonnement). Abonnementprodukte müssen mindestens **1 $** (oder den Gegenwert in Ihrer gewählten Währung) kosten; Beträge unter diesem Minimum werden nicht unterstützt.
...
<Card title="Create Checkout Session" icon="code" href="/api-reference/checkout-sessions/create">
  API-Referenz zur Erstellung sicherer, gehosteter Checkout-Sitzungen für Einmalzahlungen und Abonnements
</Card>
<Card title="Create Payment Link" icon="link" href="/api-reference/payments/post-payments">
  API-Referenz zur programmgesteuerten Erstellung dynamischer Zahlungslinks
</Card>
</CardGroup>