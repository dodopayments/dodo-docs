...
<Step title="Add advanced controls (optional)">
  - <ParamField query="paymentCurrency" type="string">Gibt die Zahlungswährung an. Standardmäßig wird die Währung des Abrechnungslandes verwendet.</ParamField>
  - <ParamField query="showCurrencySelector" type="boolean" default="true">Währungswähler ein- oder ausblenden.</ParamField>
...
Wird über einen API-Aufruf oder unser SDK mit Kundendaten erstellt. Hier ist ein Beispiel:

Es gibt zwei APIs zum Erstellen dynamischer Payment Links:

- API für Einmalzahlungs-Payment-Links [API reference](/api-reference/payments/post-payments)
- API für Abonnement-Payment-Links [API reference](/api-reference/subscriptions/post-subscriptions)

Der folgende Leitfaden beschreibt die Erstellung eines Payment Links für eine Einmalzahlung.

Ausführliche Anweisungen zur Integration von Abonnements finden Sie in diesem [Leitfaden zur Abonnementintegration](/developer-resources/subscription-integration-guide).

<Info>Stellen Sie sicher, dass Sie `payment_link = true` übergeben, um den Payment Link zu erhalten </Info>
...
<Info>Leiten Sie Ihre Kunden nach der Erstellung des Payment Links weiter, damit sie ihre Zahlung abschließen können.</Info>
...
Unsere Webhook-Implementierung folgt der Spezifikation [Standard Webhooks](https://standardwebhooks.com/). Definitionen der Webhook-Typen finden Sie in unserem [Leitfaden zu Webhook-Ereignissen](/developer-resources/webhooks/intents/webhook-events-guide).
...
| Ereignistyp           | Zeitpunkt des Auslösens                                              | Vorgehensweise                                                                 |
| -------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------- |
| `payment.succeeded`  | Die Zahlung wurde erfolgreich verarbeitet.                        | Erfüllen Sie die Bestellung – Zugriff gewähren, Produkt bereitstellen, Beleg senden. |
| `payment.failed`     | Der Zahlungsversuch schlägt fehl (abgelehnte Karte, Fehler usw.).       | Benachrichtigen Sie den Kunden und fordern Sie einen erneuten Versuch an; erfüllen Sie die Bestellung nicht.                    |
| `payment.processing` | Die Zahlung wurde akzeptiert, wird aber noch verarbeitet (asynchrone Methoden). | Warten Sie auf `payment.succeeded`/`payment.failed` als endgültigen Status – erfüllen Sie die Bestellung noch nicht. |
| `payment.cancelled`  | Die Zahlung wird vor Abschluss abgebrochen.                   | Geben Sie reservierte Zustände frei und markieren Sie die Bestellung als abgebrochen.                       |
...
Wenn Sie digitale Produkte mit Lizenzschlüsseln verkaufen, verarbeiten Sie außerdem `license_key.created`. Die vollständige Liste der Ereignisse – einschließlich Abonnement-, Berechtigungs-, Guthaben-, Wiederherstellungs- und Mahnverfahren-Ereignissen – finden Sie im [Leitfaden zu Webhook-Ereignissen](/developer-resources/webhooks/intents/webhook-events-guide).

Sie können dieses Projekt mit einer Demo-Implementierung auf [GitHub](https://github.com/dodopayments/dodo-checkout-demo) unter Verwendung von Next.js und TypeScript verwenden.

Die Live-Implementierung können Sie [hier](https://atlas.dodopayments.com/) aufrufen.
