...
Produkt-ID (z. B. <code>?productId=pdt_nZuwz45WAs64n3l07zpQR</code>).
</ParamField>
...
<Warning>
Dynamic Checkout nutzt die veralteten `POST /payments`- und `POST /subscriptions`-Endpunkte. Es funktioniert weiterhin für bestehende Integrationen, aber neue Integrationen sollten unten **Checkout Sessions** verwenden.
</Warning>
...
Der Customer Portal-Routen-Handler ermöglicht Ihnen die nahtlose Integration des Dodo Payments-Kundenportals in Ihre Remix-Anwendung.
...
- **Payload-Validierung:** Die Validierung erfolgt mit Zod. Für ungültige Payloads wird 400 zurückgegeben.
- **Fehlerbehandlung:**
