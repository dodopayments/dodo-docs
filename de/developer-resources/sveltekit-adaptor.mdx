...
description: Erfahren Sie, wie Sie Dodo Payments mit Ihrem SvelteKit App
  Router-Projekt unter Verwendung unseres SvelteKit Adaptors integrieren.
  Behandelt Checkout, Kundenportal, Webhooks und die Einrichtung einer sicheren
  Umgebung.
icon: code-merge
keywords:
  - Dodo Payments
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Integrieren Sie Dodo Payments Checkout in Ihre SvelteKit-App.
</Card>
<Card title="Customer Portal" icon="user" href="#customer-portal-route-handler">
    Ermöglichen Sie Kunden die Verwaltung von Abonnements und Details.
</Card>
...
<Info>
Alle Beispiele gehen davon aus, dass Sie den SvelteKit App Router verwenden.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Verwenden Sie diesen Handler, um Dodo Payments Checkout in Ihre SvelteKit-App zu integrieren. Unterstützt statische (GET), dynamische (POST) und Sitzungs- (POST) Zahlungsabläufe.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Verwenden Sie diesen Handler, um Dodo Payments Webhook-Ereignisse sicher in Ihrer SvelteKit-App zu empfangen und zu verarbeiten.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments unterstützt drei Arten von Zahlungsabläufen für die Integration von Zahlungen auf Ihrer Website, dieser Adapter unterstützt alle Typen von Zahlungsabläufen.
</Info>
...
- Unterstützt sowohl Einmal- als auch wiederkehrende Zahlungen.
- Für eine vollständige Liste der unterstützten POST-Body-Felder verweisen Sie auf:
...
<Warning>
Dynamic Checkout proxyt die veralteten `POST /payments`- und `POST /subscriptions`-Endpunkte. Es funktioniert weiterhin für bestehende Integrationen, aber neue Integrationen sollten unten **Checkout Sessions** verwenden.
</Warning>
...
Checkout Sessions bieten eine sicherere, gehostete Checkout-Erfahrung, die den vollständigen Zahlungsablauf sowohl für einmalige Käufe als auch für Abonnements mit vollständiger Kontrolle über die Anpassungsmöglichkeiten abwickelt.
...
Der Customer Portal Route Handler ermöglicht Ihnen die nahtlose Integration des Dodo Payments-Kundenportals in Ihre SvelteKit-Anwendung.
...
Wenn der Wert auf <code>true</code> gesetzt ist, wird dem Kunden eine E-Mail mit dem Portal-Link gesendet.
</ParamField>
...
- **Payload-Validierung:** Die Validierung erfolgt mit Zod. Bei ungültigen Payloads wird 400 zurückgegeben.
- **Fehlerbehandlung:**
