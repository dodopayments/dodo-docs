...
description: Erfahren Sie, wie Sie <CardGroup cols={2}> mit unserem Tanstack
  Adaptor in Ihr Tanstack App Router-Projekt integrieren. Behandelt Checkout,
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Integrieren Sie den Dodo Payments Checkout in Ihre Tanstack-App.
</Card>
...
<Info>
Alle Beispiele setzen voraus, dass Sie den Tanstack App Router verwenden.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Verwenden Sie diesen Handler, um den Dodo Payments Checkout in Ihre Tanstack-App zu integrieren. Unterstützt statische (GET), dynamische (POST) und sitzungsbasierte (POST) Payment-Flows.
  </Info>
  <CodeGroup>
...
<Tab title="Customer Portal Handler">
  <Info>
  Verwenden Sie diesen Handler, damit Kunden ihre Abonnements und persönlichen Daten über das Dodo Payments Customer Portal verwalten können.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Verwenden Sie diesen Handler, um Dodo Payments Webhook-Ereignisse sicher in Ihrer Tanstack-App zu empfangen und zu verarbeiten.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments unterstützt drei Arten von Payment-Flows zur Integration von Zahlungen in Ihre Website; dieser Adaptor unterstützt alle Arten von Payment-Flows.
</Info>
...
Bundesstaat/Provinz des Kunden.
</ParamField>
...
Legt den berechneten Betrag in großen Währungseinheiten fest (z. B. <code>12.5</code> für 12,50 $). Nur für Pay What You Want-Produkte und wird ignoriert, wenn der Betrag unter dem Mindestpreis des Produkts liegt.
</ParamField>
...
<Warning>
Dynamic Checkout verwendet die veralteten `POST /payments`- und `POST /subscriptions`-Endpunkte als Proxy. Es funktioniert weiterhin für bestehende Integrationen, aber neue Integrationen sollten die unten aufgeführten **Checkout Sessions** verwenden.
</Warning>
...
Checkout Sessions bieten ein sichereres, gehostetes Checkout-Erlebnis, das den vollständigen Zahlungsablauf sowohl für einmalige Käufe als auch für Abonnements mit vollständiger Kontrolle über die Anpassungsmöglichkeiten abwickelt.

Weitere Informationen und eine vollständige Liste der unterstützten Felder findest du im [Integrationsleitfaden für Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session).
...
Der Customer Portal Route Handler ermöglicht dir die nahtlose Integration des Dodo Payments-Kundenportals in deine Tanstack-Anwendung.
...
- **Payload-Validierung:** Die Validierung erfolgt mit Zod. Bei einer ungültigen Payload wird 400 zurückgegeben.
- **Fehlerbehandlung:**
