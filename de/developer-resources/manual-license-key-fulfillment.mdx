...
Wenn Sie sich nicht auf Webhooks verlassen möchten, listen Sie die Berechtigungen für Ihre Lizenzschlüssel-Zuweisung auf und filtern Sie nach `status`. Jede Berechtigung auf einer Lizenzschlüssel-Zuweisung ist bereits eine Lizenzschlüssel-Berechtigung, daher ist kein `integration_type` Filter erforderlich:
...
Der Endpunkt validiert die Berechtigung, bevor er etwas liefert. Gehen Sie wie folgt mit diesen Antworten um:

| Status | Bedeutung | Was zu tun ist |
|---|---|---|
| `200` | Schlüssel geliefert, Berechtigung ist jetzt `delivered`. | Erledigt. |
| `400` | Keine Lizenzschlüssel-Berechtigung oder der Schlüssel ist leer/Leerzeichen. | Anfrage korrigieren; nicht unverändert wiederholen. |
| `404` | Keine Berechtigung mit dieser ID für Ihr Unternehmen. | `grant_id` überprüfen. |
| `409` | Berechtigung erwartet keine Erfüllung (bereits geliefert oder hat bereits einen Schlüssel), **oder** der Schlüsselwert existiert bereits. | Wenn bereits geliefert, als Erfolg behandeln. Bei einem doppelten Schlüssel einen anderen Schlüssel angeben. |
| `422` | Anforderungstext hat die Validierung nicht bestanden (z.B. `activations_limit < 1`). | Feld korrigieren und erneut versuchen. |

<Info>
Erfüllung kann bei vorübergehenden Fehlern (Timeouts, `5xx`) sicher wiederholt werden. Jede Berechtigung kann nur einmal erfüllt werden, daher gibt eine Wiederholung nach einem erfolgreichen, aber nicht bestätigten Anruf `409` zurück, anstatt einen zweiten Schlüssel auszugeben oder eine doppelte E-Mail zu senden. Verwenden Sie die Berechtigung `id` als Ihren Idempotenz-Schlüssel.
</Info>

## Überprüfung des Ablaufs

1. Kaufen Sie das Produkt im Testmodus (siehe die [Checkout-Anleitungen](/developer-resources/integration-guide)).
2. Bestätigen Sie, dass Ihr Webhook `entitlement_grant.created` mit `status: "pending"` und `integration_type: "license_key"` empfangen hat, oder dass die Berechtigung in der Listenkriterienantwort mit diesen Filtern erscheint.
3. Rufen Sie den Erfüllungsendpunkt mit einem Testschlüssel auf.
4. Bestätigen Sie, dass die Antwort `status: "delivered"` mit einem gefüllten `license_key` anzeigt, der Kunde die Schlüssel-E-Mail erhält und `entitlement_grant.delivered` ausgeführt wird.

<Check>
Sobald geliefert, kann der Kunde den Schlüssel gegen die öffentlichen Lizenzendpunkte [aktivieren und validieren](/features/license-keys#activation-validation-deactivation) genau wie einen automatisch generierten Schlüssel.
</Check>
...
<Card title="List Grants" icon="users" href="/api-reference/entitlements/list-grants">
Filtern Sie nach `integration_type` und `status`, um ausstehende Berechtigungen zu finden.
</Card>
<Card title="Fulfill License Key Grant" icon="code" href="/api-reference/entitlements/fulfill-license-key">
Liefern Sie den Schlüsselwert und ändern Sie die Berechtigung in geliefert.
</Card>
<Card title="Entitlement Grant Webhooks" icon="webhook" href="/developer-resources/webhooks/intents/entitlement-grant">
Die `entitlement_grant.*` Ereignisse, die auf ausstehende und gelieferte Berechtigungen hinweisen.
</Card>
</CardGroup>
