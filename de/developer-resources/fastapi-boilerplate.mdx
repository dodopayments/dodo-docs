...
<Info>
Dieses Boilerplate verwendet FastAPI mit async/await-Mustern, Pydantic zur Validierung und das `dodopayments` Python SDK für nahtlose API-Integration.
</Info>
...
<Warning>
Kommen Sie niemals Ihre `.env` Datei in die Versionskontrolle ein. Es ist bereits in `.gitignore` enthalten.
</Warning>
</Step>
...
|----------|--------|-------------|
| `/checkout` | POST | Erstellt eine neue Checkout-Sitzung |
| `/webhook` | POST | Verarbeitet Dodo Payments Webhooks |
| `/customer-portal` | POST | Generiert die URL für das Kundenportal |
...
| `refund.succeeded` | Rückerstattung erfolgreich bearbeitet |
...
- Verwenden Sie Produktions-API-Schlüssel vom Dashboard
...
- Falscher API-Schlüssel oder Umgebungsdatei in `.env`
...
Aktualisieren Sie die Webhook-URL in Ihrem [Dodo-Dashboard](https://app.dodopayments.com/developer/webhooks) auf die ngrok-URL. Stellen Sie sicher, dass Sie Ihre `.env` Datei mit dem richtigen Webhook-Bestätigungsschlüssel aktualisieren.
</Accordion>

<Accordion title="Fehler bei der Überprüfung der Webhook-Signatur">
- Stellen Sie sicher, dass `DODO_PAYMENTS_WEBHOOK_KEY` korrekt in Ihrer `.env` gesetzt ist
- Überprüfen Sie, ob Sie den Rohanforderungstext für die Signaturüberprüfung verwenden
- Überprüfen Sie, ob Sie den `webhook-signature`-Header korrekt lesen
</Accordion>
</AccordionGroup>
