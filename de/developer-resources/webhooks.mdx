...
<Step title="Geheime Schlüssel abrufen">
    ...
    <Warning>
    Halten Sie Ihren geheimen Webhook-Schlüssel sicher und geben Sie ihn niemals in clientseitigem Code oder öffentlichen Repositories preis.
    </Warning>
</Step>
...
<Step title="Ereignisse auswählen">
    ...
    - Bestimmte Ereignisse basierend auf Ihren Bedürfnissen kombinieren
</Step>
...
Jede Webhook-Anfrage enthält einen `webhook-signature` Header, eine HMAC SHA256 Signatur des Webhook-Payloads und des Zeitstempels, signiert mit Ihrem geheimen Schlüssel.
...
* `unwrap()`: Überprüft Signaturen mit Ihrem geheimen Webhook-Schlüssel
* `unsafe_unwrap()`: Analysiert Payloads ohne Überprüfung

<Tip>
  Geben Sie Ihren geheimen Webhook über `DODO_PAYMENTS_WEBHOOK_KEY` an, wenn Sie den Dodo Payments-Client initialisieren.
</Tip>
...
1. Erstellen Sie die signierte Nachricht, indem Sie `webhook-id`, `webhook-timestamp` und den genauen rohen stringifizierten `payload` aneinanderreihen, getrennt durch Punkte (`.`).
2. Berechnen Sie die HMAC SHA256 dieser Zeichenkette mit Ihrem geheimen Webhook-Schlüssel aus dem Dashboard.
...
<Accordion title="Sofortige Antwort" icon="bolt">
  Geben Sie sofort nach dem Empfang des Webhooks einen `200` Statuscode zurück. Verarbeiten Sie das Ereignis asynchron, um Zeitüberschreitungen zu vermeiden.
...
<Accordion title="Doppelte Ereignisse behandeln" icon="rotate">
  Implementieren Sie Idempotenz mit dem `webhook-id` Header, um dasselbe Ereignis mehrfach ohne Nebenwirkungen sicher zu verarbeiten.
</Accordion>
...
<Steps>
<Step title="Zu Webhooks navigieren">
    Gehen Sie zu Ihrem Dodo Payments-Dashboard und navigieren Sie zu `Settings > Webhooks`.
</Step>
...
<Steps>
<Step title="Navigieren Sie zu den Benachrichtigungseinstellungen">
