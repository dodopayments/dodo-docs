...
<Step title="Create Webhook Endpoint">
    ...
    <Frame>
        <img src="/images/webhooks/create-endpoint.png" alt="Webhook hinzufügen" style={{ maxHeight: '500px', width: 'auto' }} />
    </Frame>
</Step>
...
<Expandable title="Data object properties">
  ...
  Typ der Ressource. Einer von: `Payment`, `Subscription`, `Refund`, `Dispute`, `LicenseKey`, `CreditLedgerEntry`, `CreditBalanceLow`, `AbandonedCheckout`, `DunningAttempt`, `EntitlementGrant`, oder `Payout`.
  </ResponseField>
  
  Zusätzliche Felder variieren je nach Ereignistyp. Die vollständigen Schemas finden Sie in der Dokumentation zum jeweiligen Ereignis.
</Expandable>
</ResponseField>
...
<CardGroup cols={2}>
  ...
  <Card title="Event Payloads" icon="code" href="/developer-resources/webhooks/intents/payment">
    Detaillierte Payload-Schemata für jedes Ereignis anzeigen
  </Card>
  <Card title="Handle Payment Failures" icon="screwdriver-wrench" href="/developer-resources/handle-payment-failures">
    Reagieren Sie auf `payment.failed` und beheben Sie abgelehnte Zahlungen
  </Card>
</CardGroup>
...
Mit diesem interaktiven Tool können Sie einen Ereignistyp auswählen und eine realistische Mock-Nutzlast an Ihren Endpunkt senden. Der Vorgang wird wiederholt, sodass Sie mehrere Ereignisse in einer Sitzung testen können.

Der Trigger-Befehl deckt die Ereignisse für Abonnements, Zahlungen, Rückerstattungen, Disputes und Lizenzschlüssel ab — die genaue Liste finden Sie unter [Unterstützte Webhook-Ereignisse](/developer-resources/sdks/cli#supported-webhook-events). Ereignisse für Auszahlungen, Gutschriften, abgebrochene Checkouts, Mahnverfahren und die Gewährung von Berechtigungen sind derzeit nicht als Mock-Trigger verfügbar. Testen Sie diese anhand echter Transaktionen im Testmodus mit `dodo wh listen`.

<Warning>
Mock-Webhook-Nutzlasten von `dodo wh trigger` sind nicht signiert. Verwenden Sie während des Testens ausschließlich `unsafe_unwrap()` anstelle von `unwrap()` in Ihrem Webhook-Handler.
</Warning>

<Card title="CLI Webhook Testing Docs" icon="terminal" href="/developer-resources/sdks/cli#webhook-testing">
  Siehe die vollständige Dokumentation zum Testen von CLI-Webhooks
</Card>
...
<Step title="Access Rate Limit Settings">
    Suchen Sie im Tab **Advanced** den Abschnitt „Rate Limit (throttling)“.
</Step>

<Step title="Configure Rate Limit">
    Klicken Sie auf die Schaltfläche **Edit**, um die Rate-Limit-Einstellungen zu ändern.
    
    <Info>
    Standardmäßig gilt für Webhooks „No rate limit“, d. h. Ereignisse werden zugestellt, sobald sie auftreten.
    </Info>
</Step>

<Step title="Set Limits">
    Konfigurieren Sie das gewünschte Rate Limit, um die Zustellhäufigkeit von Webhooks zu steuern und eine Systemüberlastung zu verhindern.
</Step>
</Steps>

<Tip>
Verwenden Sie Rate Limiting, wenn Ihr Webhook-Handler Zeit zur Verarbeitung von Ereignissen benötigt oder Sie mehrere Ereignisse gemeinsam bündeln möchten.
</Tip>

### Benutzerdefinierte Header

Fügen Sie allen an Ihren Endpunkt gesendeten Webhook-Anfragen benutzerdefinierte HTTP-Header hinzu. Dies ist nützlich für die Authentifizierung, das Routing oder das Hinzufügen von Metadaten zu Ihren Webhook-Anfragen.

<Steps>
<Step title="Add Custom Header">
    Geben Sie im Abschnitt „Custom Headers“ einen **Key** und **Value** für Ihren benutzerdefinierten Header ein.
</Step>
...
<Step title="Save Configuration">
    Ihre benutzerdefinierten Header werden in alle Webhook-Anfragen an diesen Endpunkt aufgenommen.
</Step>
</Steps>

### Transformationen

Mit Transformationen können Sie die Nutzlast eines Webhooks ändern und an eine andere URL weiterleiten. Diese leistungsstarke Funktion ermöglicht Ihnen Folgendes:

- Die Struktur der Nutzlast vor der Verarbeitung ändern
- Webhooks abhängig vom Inhalt an verschiedene Endpunkte weiterleiten
...
<Step title="Test Transformation">
    Verwenden Sie die Testoberfläche, um zu überprüfen, ob Ihre Transformation vor dem Produktivgang ordnungsgemäß funktioniert.
</Step>
</Steps>

<Warning>
Transformationen können die Zustellleistung von Webhooks erheblich beeinflussen. Testen Sie gründlich und halten Sie die Transformationslogik einfach und effizient.
</Warning>
...
Der Tab „Logs“ bietet umfassende Einblicke in den Zustellstatus Ihrer Webhooks, sodass Sie Webhook-Ereignisse effektiv überwachen, debuggen und verwalten können.
...
Der Tab „Activity“ bietet Echtzeit-Einblicke in die Zustellleistung Ihrer Webhooks mithilfe visueller Analysen.
...
Bleiben Sie mit automatischen E-Mail-Benachrichtigungen über den Zustand Ihrer Webhooks informiert. Wenn die Zustellung von Webhooks fehlschlägt oder Ihr Endpunkt nicht mehr antwortet, erhalten Sie E-Mail-Benachrichtigungen, damit Sie Probleme schnell beheben und Ihre Integrationen zuverlässig am Laufen halten können.

<Frame>
  <img src="/images/webhooks/alerting.png" alt="Einstellungen für Webhook-Benachrichtigungen mit Konfiguration von E-Mail-Benachrichtigungen" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Step title="Navigate to Alerting Settings">
  Öffnen Sie Ihr Dodo Payments Dashboard und navigieren Sie zu **Dashboard → Webhooks → Alerting**.
</Step>

<Step title="Enable Email Notifications">
  Aktivieren Sie **Email notifications**, um Benachrichtigungen über Probleme bei der Webhook-Zustellung zu erhalten.
</Step>

<Step title="Configure Email Address">
  Geben Sie die E-Mail-Adresse ein, unter der Sie Webhook-Benachrichtigungen erhalten möchten. Wir senden Benachrichtigungen an diese Adresse, wenn bestimmte Ereignisse bei Ihrer Webhook-Einrichtung auftreten, beispielsweise Zustellungsprobleme, die Ihre Integrationen beeinträchtigen könnten.
</Step>
</Steps>

<Tip>
Aktivieren Sie E-Mail-Benachrichtigungen, um Probleme bei der Webhook-Zustellung frühzeitig zu erkennen und zuverlässige Integrationen aufrechtzuerhalten. Sie werden benachrichtigt, wenn Zustellungen fehlschlagen oder Ihr Endpunkt nicht mehr reagiert.
</Tip>

## Auf Cloud-Plattformen bereitstellen

Bereit, Ihren Webhook-Handler in der Produktionsumgebung bereitzustellen? Wir bieten plattformspezifische Anleitungen, die Sie bei der Bereitstellung von Webhooks bei beliebten Cloud-Anbietern unterstützen und Best Practices für jede Plattform enthalten.
...
<Info>
Jede Plattformanleitung umfasst die Umgebungseinrichtung, die Signaturüberprüfung und die für den jeweiligen Anbieter spezifischen Bereitstellungsschritte.
</Info>
...
<Card title="List Webhooks" icon="list" href="/api-reference/webhooks/list-webhooks">
  API-Referenz zum Abrufen und Verwalten Ihrer Webhook-Endpunkte
</Card>
</CardGroup>