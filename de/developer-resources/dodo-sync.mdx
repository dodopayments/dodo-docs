...
- **Pipelines**: ETL-Pipelines, Echtzeitsynchronisierung auszubauen.
...
Dodo Payments API-Konfiguration für Authentifizierung und Auswahl der Umgebung. Siehe die [TypeScript SDK-Typen](https://github.com/dodopayments/dodopayments-typescript/blob/main/src/client.ts) für vollständige Optionen.
...
**ClickHouse**: Tabellen (`Subscriptions`, `Payments`, `Licenses`, `Customers`) werden mit der ReplacingMergeTree-Engine erstellt. Verwenden Sie beim Abfragen das `FINAL` Schlüsselwort, um sicherzustellen, dass die Ergebnisse dedupliziert sind.
</Warning>
