...
<Note>
Bewahren Sie diesen Schlüssel sicher auf - wir werden ihn auch zu unserer `.env` Datei hinzufügen.
</Note>
</Step>
</Steps>
...
Zusätzlich setzen Sie den **Festpreis** auf `0`, um sicherzustellen, dass Kunden nur basierend auf ihrem Verbrauch, ohne Grundgebühr, belastet werden.
...
**Preis pro Einheit**: Eingeben → `0.00001` (das sind $0.01 pro 1.000 Tokens oder $0.00001 pro Token)

**Freigrenze**: Eingeben → `10000` (Kunden erhalten 10.000 kostenlose Tokens pro Monat)
...
3. Finden Sie den Kunden, den Sie gerade erstellt haben (mit der Test-E-Mail)
...
<Step title="Backend-Server erstellen">
...
Jetzt fügen wir eine schöne Chat-Oberfläche mit vollständiger Gesprächshistorie hinzu! Erstellen Sie `public/index.html`:
...
<Note>
Stellen Sie sicher, dass Sie die `CUSTOMER_ID` in `server.ts` mit Ihrer tatsächlichen Testkunden-ID aus Schritt 4 aktualisieren.
</Note>
</Step>
...
<Info>
Das Meter summiert automatisch alle `totalTokens` Werte!
</Info>
</Step>
...
**Lösungen:**
1. Überprüfen Sie, ob der Ereignisname genau mit der Meter-Konfiguration übereinstimmt (groß- und kleinschreibungsempfindlich: `ai_chat_usage`)
...
<Accordion title="API-Schlüssel ungültig Fehler">
**Mögliche Ursachen:**
