...
<Warning>
Der Ereignisname `ai_chat_usage` muss genau dem entsprechen, was wir später aus unserem Anwendungs-Code senden. Ereignisnamen sind groß-/kleinschreibungsempfindlich!
</Warning>
</Step>
...
**Aggregat über**: Typ → `totalTokens`

**Maßeinheit**: Typ → `tokens`

<Info>
Wir verwenden "Summe", weil wir alle verbrauchten Token über mehrere Chatnachrichten hinweg addieren möchten. Das SDK sendet automatisch `totalTokens` in jedem Ereignis.
</Info>
</Step>
...
<Steps>
<Step title="Dodo Payments API-Schlüssel abrufen">
...
<Note>
Speichern Sie diesen API-Schlüssel - wir werden ihn später in unsere `.env` Datei einfügen.
</Note>
</Step>
...
<Note>
Bewahren Sie diesen Schlüssel sicher auf - wir werden ihn auch in unsere `.env` Datei einfügen.
</Note>
</Step>
</Steps>
...
Stellen Sie außerdem den **Festpreis** auf `0` ein, um sicherzustellen, dass Kunden nur basierend auf ihrer Nutzung berechnet werden, ohne Grundgebühr.
...
**Preis pro Einheit**: Geben Sie ein → `0.00001` (das sind $0.01 pro 1.000 Token oder $0.00001 pro Token)

**Kostenfreier Schwellenwert**: Geben Sie ein → `10000` (Kunden erhalten 10.000 kostenlose Token pro Monat)
...
<Tip>
**So funktioniert die Abrechnung**: Wenn ein Kunde 25.000 Tokens in einem Monat verwendet, wird er für 15.000 Tokens belastet (25.000 - 10.000 kostenlose) = 15.000 × 0,00001 $ = 0,15 $
</Tip>
</Step>
...
2. Navigieren Sie zu **Verkäufe** -> **Kunden** in der linken Seitenleiste
3. Finden Sie den Kunden, den Sie gerade (mit der Test-E-Mail) erstellt haben
...
Aktualisieren Sie `package.json`, um den Modul-Typ und die Skripte hinzuzufügen:
...
Erstellen Sie `src/server.ts` und kopieren Sie diesen vollständigen Server-Code:
...
Jetzt fügen wir eine schöne Chat-Oberfläche mit vollständiger Gesprächs-Historie hinzu! Erstellen Sie `public/index.html`:
...
<Note>
Stellen Sie sicher, dass Sie das `CUSTOMER_ID` in `server.ts` mit Ihrer tatsächlichen Test-Kunden-ID aus Schritt 4 aktualisieren.
</Note>
</Step>
...
<Info>
Das Zähler summiert automatisch alle `totalTokens` Werte!
</Info>
</Step>
...
<Frame>
  <img src="/images/cookbooks/ai-chat-app/free-tier-test.png" alt="Kostenloses Kontingent Test"/>
</Frame>
...
**Lösungen:**
1. Überprüfen Sie, ob der Ereignisname genau mit der Zählerkonfiguration übereinstimmt (groß-/kleinschreibungsempfindlich: `ai_chat_usage`)
...
**Lösungen:**
- Stellen Sie sicher, dass der Testschlüssel mit `test_` beginnt, lebender Schlüssel mit `live_` beginnt
- Entfernen Sie alle Anführungszeichen um die Schlüssel in `.env` Datei
- Generieren Sie einen neuen Schlüssel, falls erforderlich
</Accordion>
</AccordionGroup>
