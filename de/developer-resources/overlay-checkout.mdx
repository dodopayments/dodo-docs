...
| Option | Typ | Erforderlich | Beschreibung |
|--------|------|----------|-------------|
| `checkoutUrl` | `string` | Ja | Checkout-Session-URL aus der [API zum Erstellen von Checkout-Sitzungen](/api-reference/checkout-sessions/create) |
| `options.showTimer` | `boolean` | Nein | Den Checkout-Timer anzeigen oder ausblenden. Standardmäßig auf `true` gesetzt. Wenn deaktiviert, erhalten Sie das `checkout.link_expired`-Ereignis, wenn die Sitzung abläuft. |
| `options.showSecurityBadge` | `boolean` | Nein | Den Sicherheitsbadge anzeigen oder ausblenden. Standardmäßig auf `true` gesetzt. |
| `options.manualRedirect` | `boolean` | Nein | Wenn aktiviert, wird nach Abschluss des Checkouts nicht automatisch umgeleitet. Stattdessen erhalten Sie die `checkout.status`- und `checkout.redirect_requested`-Ereignisse, um die Umleitung selbst zu steuern. |
...
Bei der Verwendung von `manualRedirect` behandeln Sie den Abschluss des Checkouts in Ihrer `onEvent`-Rückruffunktion:
...
Das SDK bietet Echtzeitereignisse, auf die Sie über die `onEvent`-Rückruffunktion hören können:

#### Daten zum Kassenstatus-Ereignis

Wenn `manualRedirect` aktiviert ist, empfangen Sie das `checkout.status`-Ereignis mit folgenden Daten:
...
Wenn `manualRedirect` aktiviert ist, empfangen Sie das `checkout.redirect_requested`-Ereignis mit folgenden Daten:
...
| `checkout.form_ready` | Checkout-Formular ist bereit, Benutzereingaben zu erhalten. Nützlich zum Ausblenden von Ladezuständen. |
| `checkout.payment_page_opened` | Zahlungsseite wurde angezeigt |
| `checkout.customer_details_submitted` | Kunden- und Rechnungsinformationen wurden eingereicht |
| `checkout.closed` | Checkout-Overlay wurde geschlossen |
| `checkout.redirect` | Checkout führt eine Umleitung durch |
| `checkout.error` | Während des Checkouts trat ein Fehler auf |
...
| `checkout.redirect_requested` | Wird ausgelöst, wenn `manualRedirect` aktiviert ist. Enthält die URL, auf die der Kunde umgeleitet werden soll. |
...
Sie können das Erscheinungsbild des Checkouts anpassen, indem Sie ein `themeConfig`-Objekt im `options`-Parameter beim Öffnen des Checkouts übergeben. Die Themenkonfiguration unterstützt sowohl helle als auch dunkle Modi, sodass Sie Farben, Ränder, Text, Schaltflächen und Rahmenradius anpassen können.

<Info>
Dieser Abschnitt behandelt die **clientseitige** Themenkonfiguration mit dem Checkout SDK. Sie können Themen auch **serverseitig** konfigurieren, wenn Sie über die API eine Checkout-Sitzung mit dem `theme_config`-Parameter erstellen. Siehe [Checkout-Themenanpassung](/features/checkout#checkout-theme-customization) für die API-seitige Konfiguration.
</Info>
...
Für TypeScript-Nutzer werden alle Typen der Themenkonfiguration exportiert:
...
Das SDK bietet detaillierte Fehlermeldungen über das Ereignissystem. Implementieren Sie immer die richtige Fehlerbehandlung in Ihrem `onEvent`-Rückruf:
...
2. **Fehlerbehandlung**: Implementieren Sie immer die richtige Fehlerbehandlung in Ihrem Ereignisrückruf
3. **Testmodus**: Verwenden Sie `test`-Modus während der Entwicklung und wechseln Sie nur, wenn Sie bereit sind für die Produktion, zu `live`
4. **Ereignisbehandlung**: Behandeln Sie alle relevanten Ereignisse für ein vollständiges Benutzererlebnis
5. **Gültige URLs**: Verwenden Sie immer gültige Checkout-URLs aus der API zum Erstellen von Checkout-Sitzungen
...
8. **Manuelle Weiterleitungen**: Verwenden Sie `manualRedirect`, wenn Sie eine benutzerdefinierte Kontrolle über die Navigation nach dem Checkout benötigen
9. **Timer-Verwaltung**: Deaktivieren Sie den Timer (`showTimer: false`), wenn Sie die Sitzung manuell ablaufen lassen möchten
...
**Mögliche Ursachen:**
- Ereignishandler nicht korrekt eingerichtet
- JavaScript-Fehler verhindern die Ereignisweiterleitung
- SDK wurde nicht korrekt initialisiert

**Lösungen:**
- Bestätigen Sie, dass der Ereignishandler in `Initialize()` korrekt konfiguriert ist
...
- Verifizieren Sie, dass die Themen-Einstellungen korrekt sind
...
Für detaillierte Informationen zur Einrichtung von Google Pay und anderen digitalen Geldbörsen siehe die <a href="/features/payment-methods/digital-wallets">Seite zu digitalen Geldbörsen</a>.
...
| Am besten geeignet für | Schnelle Integration, vorhandene Seiten | Anpassbare Checkout-Seiten, hochkonvertierende Abläufe |

<Tip>
Verwenden Sie **Overlay-Checkout** für eine schnellere Integration mit minimalen Änderungen an Ihren vorhandenen Seiten. Verwenden Sie **Inline-Checkout**, wenn Sie maximale Kontrolle über das Checkout-Erlebnis und nahtloses Branding wünschen.
</Tip>
...
<Card title="Checkout Sessions API" icon="code" href="/api-reference/checkout-sessions/create">
    Erstellen Sie Checkout-Sitzungen, um Ihre Checkout-Erfahrungen zu ermöglichen.
</Card>
