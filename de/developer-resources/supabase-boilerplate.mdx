...
Setzen Sie die Umleitungs-URI auf: `https://[your-project-ref].supabase.co/auth/v1/callback` in Google Cloud, aktivieren Sie dann den Google-Anbieter in Supabase Auth mit Ihrer Client-ID und Ihrem Secret.
</Step>

<Step title="Dodo Payments konfigurieren">
Generieren Sie einen API-Schlüssel und ein Webhook-Geheimnis im Dodo-Dashboard. Setzen Sie die Umgebung auf `test_mode` während der Entwicklung.
</Step>
...
Die Preisgestaltung UI liest dieses `features` Array und rendert sie dynamisch.
</Step>
...
- Abonnements-Checkout über Dodo Payments
- Supabase Edge-Funktion für Webhooks (`dodo-webhook`)
- Drizzle ORM-Schema und Migrationen
- Dashboard mit Rechnungen, Abonnementstatus und Planfeatures

<Tip>
Behalten Sie `DODO_PAYMENTS_ENVIRONMENT` als `test_mode`, bis Sie die End-to-End-Tests abgeschlossen haben.
</Tip>
...
- Überprüfen Sie, ob der Headername in Ihrer Funktion korrekt ist (Dodo-Signature)
</Accordion>
<Accordion title="Datenbank-Push schlägt fehl">
- Überprüfen Sie die `DATABASE_URL` Syntax und den Supabase-Netzwerk-Ausgang
...
- Umleitungs-URI muss `https://[ref].supabase.co/auth/v1/callback` sein
- Stellen Sie sicher, dass dasselbe in Google Cloud und im Supabase Auth-Anbieter gilt
</Accordion>
</AccordionGroup>
...
<Info>
Original-Repository und detaillierte Schritte: <a href="https://github.com/dodopayments/dodo-supabase-subscription-starter">dodo-supabase-subscription-starter</a>.
</Info>
