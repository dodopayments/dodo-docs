---
sidebarTitle: Supabase
title: Supabase Boilerplate
description: Verwenden Sie das offizielle Next.js + Supabase + Dodo Payments
  Abonnement-Startpaket, um Abonnements schnell zu versenden. Enthält
  Authentifizierung, Datenbankschema, Webhook-Handler und Preis-UI.
icon: /images/logos/supabase.svg
---

<CardGroup cols={2}>
<Card title="GitHub-Repository" icon="github" href="https://github.com/dodopayments/dodo-supabase-subscription-starter">
    Minimal Next.js + Supabase + Dodo Payments Abonnement-Boilerplate
</Card>
<Card title="Live-Demo" icon="rocket" href="https://dodopayments-supabase-nextjs.vercel.app">
    Erkunden Sie die bereitgestellte Demo
</Card>
</CardGroup>

## Übersicht

Ein produktionsbereites Boilerplate für Abonnements mit Next.js 15, React 19, Supabase, Drizzle ORM und Dodo Payments. Es wird mit Google OAuth, Abonnement-Checkout, Webhook-Verarbeitung, Datenbankschema und einem grundlegenden Dashboard geliefert.

<Info>
Wenn Sie nur Routen-Handler für eine vorhandene App benötigen, sehen Sie sich die speziellen Adapter an: <a href="/developer-resources/nextjs-adaptor">Next.js-Adapter</a> und <a href="/developer-resources/express-adaptor">Express-Adapter</a>.
</Info>

## Voraussetzungen

- Node.js 18+ (oder Bun 1.0+)
- Supabase-Projekt (URL, Anon-Schlüssel, Service-Rollen-Schlüssel, Datenbank-URL)
- Dodo Payments-Konto (API-Schlüssel, Webhook-Geheimnis)
- Google Cloud OAuth-Client (Client-ID und Geheimnis)

## Schnellstart

<Steps>
<Step title="Klonen und installieren">

```bash
git clone https://github.com/dodopayments/dodo-supabase-subscription-starter.git
cd dodo-supabase-subscription-starter
# choose one
bun install
# or
npm install
# or
pnpm install
```

</Step>

<Step title="Supabase-Projekt erstellen">
Erstellen Sie ein Supabase-Projekt und kopieren Sie:
- NEXT_PUBLIC_SUPABASE_URL
- NEXT_PUBLIC_SUPABASE_ANON_KEY
- SUPABASE_SERVICE_ROLE_KEY
- DATABASE_URL (Verbindungszeichenfolge)
</Step>

<Step title="Google OAuth konfigurieren">
Setzen Sie die Umleitungs-URI auf: `https://[your-project-ref].supabase.co/auth/v1/callback` in Google Cloud, aktivieren Sie dann den Google-Anbieter in Supabase Auth mit Ihrer Client-ID und Ihrem Geheimnis.
</Step>

<Step title="Dodo-Zahlungen konfigurieren">
Generieren Sie einen API-Schlüssel und ein Webhook-Geheimnis im Dodo-Dashboard. Setzen Sie die Umgebung auf `test_mode` während der Entwicklung.
</Step>

<Step title=".env.local erstellen">

```env
# Supabase
NEXT_PUBLIC_SUPABASE_URL=https://your-project-ref.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=your-anon-key
SUPABASE_SERVICE_ROLE_KEY=your-service-role

# Database
DATABASE_URL=postgresql://postgres:[password]@db.[project-ref].supabase.co:5432/postgres

# Dodo Payments
DODO_PAYMENTS_API_KEY=your-dodo-api-key
DODO_WEBHOOK_SECRET=your-webhook-secret
DODO_PAYMENTS_ENVIRONMENT=test_mode
```

<Warning>
Verpflichten Sie niemals Geheimnisse. Verwenden Sie Umgebungsvariablen in Bereitstellungsumgebungen.
</Warning>
</Step>

<Step title="Datenbankschema bereitstellen">

```bash
bun run db:push
# or
npm run db:push
# or
pnpm run db:push
```

<Check>
Erstellte Tabellen: `users`, `subscriptions`, `payments`.
</Check>
</Step>

<Step title="Webhook-Funktion bereitstellen">

```bash
# login (one-time)
bunx supabase login
# or
npx supabase login

# deploy the edge function
bun run deploy:webhook --project-ref [your-project-ref]
# or
npm run deploy:webhook -- --project-ref [your-project-ref]
# or
pnpm run deploy:webhook --project-ref [your-project-ref]
```

```bash cURL
curl -X POST \
  'https://[your-project-ref].supabase.co/functions/v1/dodo-webhook' \
  -H 'Content-Type: application/json' \
  -H 'Dodo-Signature: <signature>' \
  -d '{"type":"payment.succeeded","data":{}}'
```

</Step>

<Step title="Webhook in Dodo Payments hinzufügen">
Setzen Sie die Endpunkt-URL auf:

```text
https://[your-project-ref].supabase.co/functions/v1/dodo-webhook
```

Wählen Sie Zahlungs- und Abonnementereignisse aus.
</Step>

<Step title="Produkte und Funktionen erstellen">
Im Dodo-Dashboard → Produkte → Produkt erstellen. Fügen Sie optional Metadaten hinzu:

```json
{
  "features": ["Feature 1", "Feature 2", "Feature 3"]
}
```

Die Preisgestaltung UI liest dieses `features` Array und rendert es dynamisch.
</Step>

<Step title="Dev-Server ausführen">

```bash
bun run dev
# or
npm run dev
# or
pnpm run dev
```

Öffnen Sie http://localhost:3000.
</Step>
</Steps>

## Was ist enthalten

- Authentifizierung über Supabase (Google OAuth konfiguriert)
- Abonnement-Checkout über Dodo-Zahlungen
- Supabase Edge-Funktion für Webhooks (`dodo-webhook`)
- Drizzle ORM-Schema und Migrationen
- Dashboard mit Rechnungen, Abonnementstatus und Planfeatures

<Tip>
Halten Sie `DODO_PAYMENTS_ENVIRONMENT` als `test_mode` bis Sie die End-to-End-Tests abgeschlossen haben.
</Tip>

## Wichtige Dateien und Pfade

<Tabs>
<Tab title="Edge-Funktion">

```text
supabase/functions/dodo-webhook/
  index.ts            # webhook handler verifying signatures
  deno.json           # permissions
```

</Tab>
<Tab title="Next.js-Routen">

```text
app/(marketing)/*     # landing + pricing UI
app/(dashboard)/*     # protected pages
app/api/*             # server actions & helpers
```

</Tab>
<Tab title="Datenbank (Drizzle)">

```text
lib/db/schema.ts      # users, subscriptions, payments
lib/db/index.ts       # client
```

</Tab>
</Tabs>

## Umgebungsvariablen

<AccordionGroup>
<Accordion title="Supabase">

```env
NEXT_PUBLIC_SUPABASE_URL=
NEXT_PUBLIC_SUPABASE_ANON_KEY=
SUPABASE_SERVICE_ROLE_KEY=
DATABASE_URL=
```

</Accordion>
<Accordion title="Dodo Payments">

```env
DODO_PAYMENTS_API_KEY=
DODO_WEBHOOK_SECRET=
DODO_PAYMENTS_ENVIRONMENT=test_mode|live_mode
```

</Accordion>
<Accordion title="Google OAuth">

```env
GOOGLE_CLIENT_ID=
GOOGLE_CLIENT_SECRET=
```

</Accordion>
</AccordionGroup>

## Überprüfung und Fehlersuche

<AccordionGroup>
<Accordion title="Webhook-Signatur ungültig (401)">
- Stellen Sie sicher, dass `DODO_WEBHOOK_SECRET` mit dem Wert aus dem Dodo-Dashboard übereinstimmt
- Bestätigen Sie, dass Sie die neueste `dodo-webhook` Funktion bereitgestellt haben
- Überprüfen Sie, ob der Header-Name in Ihrer Funktion korrekt ist (Dodo-Signatur)
</Accordion>
<Accordion title="Datenbank-Push schlägt fehl">
- Überprüfen Sie die `DATABASE_URL` Syntax und den Supabase-Netzwerkverkehr
- Warten Sie ~2–3 Minuten nach der Projekterstellung, bevor Sie den ersten Push durchführen
</Accordion>
<Accordion title="OAuth-Umleitungsfehler">
- Die Umleitungs-URI muss `https://[ref].supabase.co/auth/v1/callback` sein
- Stellen Sie sicher, dass dasselbe in Google Cloud und im Supabase Auth-Anbieter gilt
</Accordion>
</AccordionGroup>

<Check>
Sie haben jetzt ein funktionierendes Abonnement-SaaS mit Supabase und Dodo Payments erstellt.
</Check>

<Info>
Original-Repository und detaillierte Schritte: <a href="https://github.com/dodopayments/dodo-supabase-subscription-starter">dodo-supabase-subscription-starter</a>.
</Info>
