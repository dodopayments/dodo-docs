...
Nach der Bearbeitung von `convex.config.ts`, führe `npx convex dev` einmal aus, um die erforderlichen Typen zu generieren.
</Step>
...
<Warning>
Bevor Sie diese Abfrage verwenden, stellen Sie sicher, dass Sie das entsprechende Schema in Ihrer `convex/schema.ts` Datei definiert haben oder ändern Sie die Abfrage, um mit Ihrem vorhandenen Schema übereinzustimmen.
</Warning>
...
<Tab title="Kundenportal einrichten">
  <Info>
  Verwenden Sie diese Funktion, um Kunden zu ermöglichen, ihre Abonnements und Details über das Dodo Payments Kundenportal zu verwalten. Der Kunde wird automatisch über die `identify` Funktion identifiziert.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook-Handler einrichten">
  <Info>
  Verwenden Sie diesen Handler, um Dodo Payments Webhook-Ereignisse sicher in Ihrer Convex-App zu empfangen und zu verarbeiten. Alle Webhook-Handler erhalten den Convex `ActionCtx` als ersten Parameter, sodass Sie `ctx.runQuery()` und `ctx.runMutation()` verwenden können, um mit Ihrer Datenbank zu interagieren.
  </Info>
  <CodeGroup>
