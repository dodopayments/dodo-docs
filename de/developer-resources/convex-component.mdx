...
Nach dem Bearbeiten von `convex.config.ts` führe `npx convex dev` einmal aus, um die notwendigen Typen zu generieren.
</Step>
<Step title="Umgebungsvariablen einrichten">
Richte die Umgebungsvariablen in deinem Convex-Dashboard ein (**Einstellungen** → **Umgebungsvariablen**). Du kannst das Dashboard öffnen, indem du Folgendes ausführst:
...
- `DODO_PAYMENTS_API_KEY` - Dein Dodo Payments API-Schlüssel
- `DODO_PAYMENTS_ENVIRONMENT` - Setze auf `test_mode` oder `live_mode`
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Dein Webhook-Geheimnis (erforderlich für die Verarbeitung von Webhooks)
...
<Warning>
Stelle sicher, dass du das geeignete Schema in deiner `convex/schema.ts`-Datei definiert hast, bevor du diese Abfrage verwendest, oder ändere die Abfrage, um deinem bestehenden Schema zu entsprechen.
</Warning>
...
<Tab title="Kundenportal-Einrichtung">
  <Info>
  Verwende diese Funktion, um Kunden zu ermöglichen, ihre Abonnements und Details über das Dodo Payments Kundenportal zu verwalten. Der Kunde wird automatisch über die `identify`-Funktion identifiziert.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook-Handler-Einrichtung">
  <Info>
  Verwende diesen Handler, um Dodo Payments Webhook-Ereignisse sicher in deiner Convex-App zu empfangen und zu verarbeiten. Alle Webhook-Handler erhalten das Convex `ActionCtx` als ersten Parameter, wodurch du `ctx.runQuery()` und `ctx.runMutation()` nutzen kannst, um mit deiner Datenbank zu interagieren.
  </Info>
  <CodeGroup>
...
<Info>
Der Kunde wird automatisch mit der `identify`-Funktion identifiziert, die in deiner DodoPayments-Setup konfiguriert ist. Diese Funktion sollte die `dodoCustomerId` des Kunden zurückgeben.
</Info>
