...
2. Später erstellen Sie Zahlungen gegen dieses Abonnement mit benutzerdefinierten Beträgen über den speziellen Zahlungsendpunkt.
...
<Warning>
Das Berechnen eines Abonnements, das nicht On-Demand ist, kann fehlschlagen. Stellen Sie sicher, dass das Abonnement `on_demand: true` in seinen Details hat, bevor Sie die Zahlung vornehmen.
</Warning>
...
- **Intelligente Filterung**: Wiederholen Sie nur bei wiederholbaren Fehlern (z. B. Netzwerk-/Ausstellerfehler, unzureichende Mittel); niemals bei harten Ablehnungen wiederholen.
...
- Alternativ, wenn Sie die letzte erfolgreiche Zahlungszeit `T` speichern, planen Sie den nächsten Versuch um `T + X days`, um die zeitliche Ausrichtung zu bewahren.
...
- Verwenden Sie einen Scheduler/Warteschlange, der präzise Zeitstempel speichert; berechnen Sie den nächsten Versuch zum genauen Zeitversatz (z. B. `T + 3 days` zur gleichen HH:MM).
- Behalten Sie den letzten erfolgreichen Zahlungszeitstempel `T` bei, um den nächsten Versuch zu berechnen; bündeln Sie niemals mehrere Abonnements zur gleichen Zeit.
...
<Step title="Eine Zahlung auslösen">
Rufen Sie den Zahlungsendpunkt mit einem kleinen `product_price` (z. B. `100`) auf und überprüfen Sie, ob Sie `payment.succeeded` erhalten.
</Step>
...
- **422 Ungültige Anfrage**: Stellen Sie sicher, dass `on_demand.mandate_only` bei der Erstellung angegeben ist und `product_price` für Zahlungen angegeben ist.
