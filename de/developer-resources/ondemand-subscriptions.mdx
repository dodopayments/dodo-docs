...
Wenn die Wiederholungen weiterhin fehlschlagen, weil die Zahlungsmethode selbst fehlerhaft ist (abgelaufene Karte, geschlossenes Konto usw.), verwenden Sie [`POST /subscriptions/{subscription_id}/update-payment-method`](/api-reference/subscriptions/update-payment-method), um eine neue vom Kunden einzuziehen. Bei Erfolg kehrt das Abonnement zu `active` zurück und `payment.succeeded` gefolgt von `subscription.active` Webhooks werden ausgelöst.
</Step>
</Steps>
...
Verwenden Sie Ihren Test-API-Schlüssel, um das Abonnement zu erstellen, öffnen Sie dann den zurückgegebenen `checkout_url` und schließen Sie das Mandat ab.
</Step>
<Step title="Trigger a charge">
Rufen Sie den Charge-Endpunkt mit einem kleinen `product_price` (z.B. `100`) auf und überprüfen Sie, ob Sie `payment.succeeded` erhalten.
</Step>
<Step title="Go live">
