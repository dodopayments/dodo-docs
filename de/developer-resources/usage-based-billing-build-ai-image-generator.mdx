...
**Meter Name**: Kopiere und füge dies genau ein → `Image Generation Meter`

**Description**: Kopiere dies → `Tracks each AI image generation request made by customers using our DALL-E powered service`

**Event Name**: Dies ist entscheidend - kopiere genau → `image.generated`

<Warning>
Der Ereignisname `image.generated` muss genau mit dem übereinstimmen, was wir später aus unserem Anwendungscode senden. Ereignisnamen sind groß-/kleinschreibungsempfindlich!
</Warning>
</Step>
...
1. **Aktiviere die Ereignisfilterung**: Schalte dies **EIN**
...
**Product Name**: Kopiere dies → `PixelGen AI - Image Generation`

**Description**: Kopiere dies → `AI-powered image generation service with pay-per-use billing`
...
Zusätzlich setze den **Festpreis** auf `0`, um sicherzustellen, dass Kunden nur basierend auf ihrer Nutzung berechnet werden, ohne Grundgebühr.
...
**Price Per Unit**: Gib ein → `0.05` (das sind $0.05 pro Bild)

**Free Threshold**: Gib ein → `10` (Kunden erhalten 10 kostenlose Bilder pro Monat)
...
4. Kopiere die Kunden-ID - sie wird ungefähr so aussehen: `cus_abc123def456`
