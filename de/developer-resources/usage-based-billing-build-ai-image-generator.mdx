...
**Metrikname**: Kopiere und füge dies genau ein → `Image Generation Meter`

**Beschreibung**: Kopiere dies → `Tracks each AI image generation request made by customers using our DALL-E powered service`

**Ereignisname**: Dies ist entscheidend - kopiere genau → `image.generated`

<Warning>
Der Ereignisname `image.generated` muss genau mit dem übereinstimmen, was wir später aus unserem Anwendungscode senden werden. Ereignisnamen sind groß-/kleinschreibungsempfindlich!
</Warning>
</Step>
...
1. **Ereignisfilterung aktivieren**: Schalte dies **AN**
2. **Filterlogik**: Wähle **ODER** (das bedeutet "zähle, wenn EINE dieser Bedingungen wahr ist")
3. **Füge die erste Bedingung hinzu**:
   - Eigenschaftenname: `quality`
   - Vergleichsoperator: `equals`  
   - Wert: `standard`
4. **Klicke auf "Bedingung hinzufügen"** für die zweite:
   - Eigenschaftenname: `quality`
   - Vergleichsoperator: `equals`
   - Wert: `hd`
...
Zusätzlich setze den **Festpreis** auf `0`, um sicherzustellen, dass die Kunden nur basierend auf ihrer Nutzung, ohne Grundgebühr, belastet werden.
...
Der Zahlungslink wird ungefähr so aussehen: `https://test.checkout.dodopayments.com/buy/pdt_IgPWlRsfpbPd5jQKezzW1?quantity=1`
</Step>
...
<Step title="Finde deine Kunden-ID">
1. Gehe zurück zu deinem Dodo Payments-Dashboard
2. Navigiere zu **Kunden** in der linken Seitenleiste
3. Finde den Kunden, den du gerade erstellt hast (mit der Test-E-Mail)
4. Kopiere die Kunden-ID - sie wird so aussehen: `cus_abc123def456`
...
<Step title="Erstelle die Hauptanwendung">
Erstelle eine Datei mit dem Namen `index.ts` und kopiere diesen vollständigen Anwendungscode:
</Step>
</Steps>
