...
<Step title="Behandlung der Rückkehr">
Nach der Zahlung werden die Kunden mit zusätzlichen Abfrageparametern zum Zahlungsstatus an Ihre `return_url` umgeleitet.
</Step>
</Steps>
...
**Format**: Dargestellt in der niedrigsten Währungseinheit (z. B. Cent für USD). Um beispielsweise 1,00 $ zu berechnen, übergeben Sie `100`.
</ParamField>
</Expandable>
</ParamField>
...
**Format**: Einschließlich Ländercode, z. B. `"+1234567890"` für US-Nummern
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
**Beispiele**: `"94102"` (USA), `"M5V 3A8"` (Kanada), `"SW1A 1AA"` (UK)
</ParamField>
</Expandable>
</ParamField>
...
<Note>
Wenn `confirm` auf `true` gesetzt ist, wird der Kunde direkt zur Checkout-Seite geleitet, wobei alle Bestätigungsstufen umgangen werden.
</Note>
...
<Warning>
Wenn Sie `payment_method_id` verwenden, muss `confirm` auf `true` gesetzt werden und eine vorhandene `customer_id` muss bereitgestellt werden. Die Zahlungsmethode wird auf Berechtigung mit der Währung der Zahlung validiert.
</Warning>
...
<Note>
Wenn `redirect_immediately` aktiviert ist, werden die Kunden sofort nach Abschluss der Zahlung an Ihre `return_url` umgeleitet, wobei die Standard-Erfolgsseite vollständig übersprungen wird.
</Note>
