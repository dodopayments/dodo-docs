...
**Häufige Optionen**: `credit`, `debit`, `upi_collect`, `apple_pay`, `google_pay`, `amazon_pay`, `klarna`, `affirm`, `afterpay_clearpay`, `cashapp`, `multibanco`, `bancontact_card`, `eps`, `ideal`, `przelewy24`, `paypal`. Dies ist nicht die vollständige Liste — alle akzeptierten Werte findest du in der [API-Referenz zur Erstellung einer Checkout-Sitzung](/api-reference/checkout-sessions/create).
