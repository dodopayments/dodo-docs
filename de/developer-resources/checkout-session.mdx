...
**Format**: Wird in der niedrigsten Währungseinheit dargestellt (z. B. Cent für USD). Um beispielsweise 1,00 $ zu berechnen, übergeben Sie `100`.
</ParamField>
</Expandable>
</ParamField>
...
Erlauben Sie den Kunden, das Bundesland während des Checkouts zu bearbeiten.
</ParamField>
...
Wenn auf wahr gesetzt, wird keine Abbuchung vorgenommen und nur die Zahlungsdetails für die zukünftige Verwendung autorisiert.
</ParamField>
...
<Warning>
Beim Verwenden von `payment_method_id` muss `confirm` auf `true` gesetzt werden, und eine vorhandene `customer_id` muss bereitgestellt werden. Die Zahlungsmethode wird auf Berechtigung mit der Währungszahlung validiert.
</Warning>
...
<Tip>
Kurze Links sind perfekt für SMS, E-Mail oder Social-Media-Sharing. Sie sind leichter zu merken und schaffen mehr Vertrauen bei den Kunden als lange URLs.
</Tip>

### 13. Überspringen Sie die Zahlungsbestätigungsseite mit sofortiger Weiterleitung

Leiten Sie Kunden sofort nach Abschluss der Zahlung weiter und umgehen Sie die standardmäßige Bestätigungsseite:
...
<Tip>
Verwenden Sie `redirect_immediately: true`, wenn Sie eine benutzerdefinierte Bestätigungsseite haben, die eine bessere Benutzererfahrung als die standardmäßige Zahlungsbestätigungsseite bietet. Dies ist besonders nützlich für mobile Apps und eingebettete Checkout-Flows.
</Tip>

<Note>
Wenn `redirect_immediately` aktiviert ist, werden die Kunden sofort nach Abschluss der Zahlung zu Ihrem `return_url` weitergeleitet, wobei die standardmäßige Bestätigungsseite vollständig übersprungen wird.
</Note>

## Vorschau von Checkout-Sitzungen

Bevor Sie eine Checkout-Sitzung erstellen, können Sie die Preisaufstellung einschließlich Steuern, Rabatten und Gesamtsummen anzeigen. Dies ist nützlich, um den Kunden genaue Preise anzuzeigen, bevor sie zum Checkout übergehen.
...
<Card title="Vorschau API-Referenz" icon="code" href="/api-reference/checkout-sessions/preview">
  Sehen Sie die vollständige Dokumentation des Vorschau-Endpunkts ein.
</Card>
...
- Der Checkout-Fluss sammelt automatisch die fehlenden Details, bevor der Kunde zur Zahlungsseite weitergeleitet wird.
- Wenn Sie hingegen bereits alle erforderlichen Informationen haben und direkt zur Zahlungsseite überspringen möchten, können Sie den vollständigen Datensatz übergeben und `confirm=true` in Ihrem Anfragekörper einfügen.
