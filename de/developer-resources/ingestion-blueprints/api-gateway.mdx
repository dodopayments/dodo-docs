...
<Step title="Ein Meter erstellen">
Erstellen Sie einen Meter in Ihrem [Dodo Payments Dashboard](https://app.dodopayments.com/):
- **Ereignisname**: `api_call` (oder Ihrem bevorzugten Namen)
- **Aggregationsart**: `count` zur Verfolgung der Anrufanzahl
- Konfigurieren Sie zusätzliche Eigenschaften, falls Sie Metadaten wie Antwortzeiten, Statuscodes usw. verfolgen möchten.
</Step>

<Step title="Verfolgen Sie API-Aufrufe">
<CodeGroup>
...
Maximale Anzahl von Ereignissen vor automatischem Flush. Standard: `100`.
</ParamField>
...
<Tip>
**Verwendung von Batch-Verarbeitung für hohe Volumen**: Für Anwendungen, die mehr als 10 Anfragen pro Sekunde verarbeiten, verwenden Sie `createBatch()`, um den Overhead zu reduzieren und die Leistung zu verbessern.
</Tip>

<Warning>
**Batch immer bereinigen**: Rufen Sie `batch.cleanup()` beim Herunterfahren der Anwendung auf, um ausstehende Ereignisse zu flushen und Datenverluste zu vermeiden.
</Warning>
