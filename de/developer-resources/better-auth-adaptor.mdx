...
- Endpunkte zur Erfassung und Berichterstattung von nutzungsbasierten Abrechnungen
- Echtzeit-Webhooks mit Signaturverifizierung
- Vollständige TypeScript-Unterstützung

<Note>
  Für diese Integration benötigen Sie ein Dodo Payments-Konto und API-Keys.
</Note>
...
Führen Sie folgenden Befehl im Projektstamm aus:
...
Fügen Sie diese Ihrer <code>.env</code>-Datei hinzu:
...
<Warning>
API-Keys oder Geheimnisse niemals in die Versionskontrolle übernehmen.
</Warning>
</Step>
...
<Tip>
Setzen Sie <code>environment</code> für die Produktion auf <code>live_mode</code>.
</Tip>
</Step>
...
<Info>
  Verwenden Sie <code>authClient.dodopayments.checkoutSession</code> für neue Integrationen. Die veraltete <code>checkout</code>-Methode wird nur aus Gründen der Abwärtskompatibilität beibehalten.
</Info>
...
<Info>
  Im Gegensatz zur veralteten <code>checkout</code>-Methode erfordert <code>checkoutSession</code> keine Rechnungsinformationen im Voraus, da diese auf der Checkout-Seite vom Benutzer abgefragt werden. Sie können sie dennoch überschreiben, indem Sie den <code>billing</code>-Schlüssel im Argument angeben.
</Info>

<Info>
  Ähnlich wie die veraltete <code>checkout</code>-Methode verwendet <code>checkoutSession</code> E-Mail und Namen des Kunden aus deren better-auth-Sitzung, Sie können dies jedoch überschreiben, indem Sie das <code>customer</code>-Objekt mit E-Mail und Namen übergeben.
</Info>

<Info>
  Sie können die gleichen Optionen im Argument übergeben wie im Request-Body für den Endpunkt [Create Checkout Session](https://docs.dodopayments.com/api-reference/checkout-sessions/create).
</Info>

<Note>
  Die Rückgabe-URL wird aus dem in der Server-Plugin-Konfiguration angegebenen <code>successUrl</code> übernommen. Sie müssen <code>return_url</code> daher nicht in der Client-Anforderung angeben.
</Note>

## Veralteter Checkout (Nicht mehr empfohlen)

<Warning>
  Die <code>authClient.dodopayments.checkout</code>-Methode ist veraltet. Verwenden Sie für neue Implementierungen stattdessen <code>checkoutSession</code>.
</Warning>
...
## Verfolgen der nutzungsbasierten Abrechnung

Aktivieren Sie das <code>usage()</code>-Plugin auf dem Server, um nutzungsbasierte Ereignisse zu erfassen und es den Kunden zu ermöglichen, ihre nutzungsbasierte Abrechnung zu überprüfen.

- <code>authClient.dodopayments.usage.ingest</code> erfasst Ereignisse für den angemeldeten, E-Mail-verifizierten Benutzer.
- <code>authClient.dodopayments.usage.meters.list</code> listet die letzten Nutzungen für die mit den Abonnements dieses Kunden verbundenen Zähler auf.
...
<Tip>
  Zeitstempel, die älter als eine Stunde oder mehr als fünf Minuten in der Zukunft liegen, werden beim Erfassen von Nutzungsdaten abgelehnt.
</Tip>
<Tip>
  Wenn Sie <code>meter_id</code> beim Auflisten von Nutzungszählern weglassen, werden alle Zähler zurückgegeben, die mit den Abonnements des Kunden verknüpft sind.
</Tip>

# Webhooks

<Info>
  Das Webhooks-Plugin verarbeitet Echtzeit-Zahlungsereignisse von Dodo Payments mit sicherer Signaturprüfung. Der Standardendpunkt ist <code>/api/auth/dodopayments/webhooks</code>.
</Info>
...
<AccordionGroup>
  {/* LOCKED_PATTERN_9586fa213a07317c452129bb9c8365ad */}
    - <b>client</b> (erforderlich): DodoPayments-Clientinstanz - <b>createCustomerOnSignUp</b> (optional): Kunden bei der Anmeldung automatisch erstellen - <b>use</b> (erforderlich): Array von Plugins, die aktiviert werden sollen (checkout, portal, usage, webhooks)
  </Accordion>
  {/* LOCKED_PATTERN_a313db3f1395a5521aceb89c3251fabd */}
    - <b>products</b>: Array von Produkten oder asynchrone Funktion, die Produkte zurückgibt - <b>successUrl</b>: URL, auf die nach erfolgreicher Zahlung weitergeleitet wird - <b>authenticatedUsersOnly</b>: Benutzer-Authentifizierung erforderlich (Standard: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  {/* LOCKED_PATTERN_8cb977e6bc7a95fb0f5c0e3c34015b67 */}
    - <b>Ungültiger API-Key</b>: Überprüfen Sie erneut Ihren <code>DODO_PAYMENTS_API_KEY</code> in <code>.env</code>. - <b>Webhook-Signatur stimmt nicht überein</b>: Stellen Sie sicher, dass das Webhook-Geheimnis mit dem im Dodo Payments Dashboard gesetzten übereinstimmt. - <b>Kunde nicht erstellt</b>: Bestätigen Sie, dass <code>createCustomerOnSignUp</code> auf <code>true</code> gesetzt ist.
  </Accordion>
  <Accordion title="Best Practices">
    - Verwenden Sie Umgebungsvariablen für alle Geheimnisse und Schlüssel. - Testen Sie im <code>test_mode</code>, bevor Sie in den <code>live_mode</code> wechseln. - Protokollieren Sie Webhook-Ereignisse zur Fehlerbehebung und Prüfung.
  </Accordion>
</AccordionGroup>
