...
- Endpunkte zur Erfassung und Berichterstattung von verbrauchsabhängiger Nutzung für die nutzungsbasierte Abrechnung
...
## Verfolgung der verbrauchsabhängigen Nutzung

Aktivieren Sie das <code>usage()</code>-Plugin auf dem Server, um verbrauchsabhängige Ereignisse zu erfassen und es den Kunden zu ermöglichen, ihre nutzungsbasierte Abrechnung einzusehen.

- <code>authClient.dodopayments.usage.ingest</code> erfasst Ereignisse für den angemeldeten, E-Mail-verifizierten Benutzer.
- <code>authClient.dodopayments.usage.meters.list</code> listet die aktuelle Nutzung für die Zähler auf, die mit den Abonnements dieses Kunden verbunden sind.
...
<Tip>
  Zeitstempel, die älter als eine Stunde oder mehr als fünf Minuten in der Zukunft liegen, werden
  bei der Erfassung der Nutzung abgelehnt.
</Tip>
<Tip>
  Wenn Sie <code>meter_id</code> beim Auflisten der Nutzungsmesser weglassen, werden alle Zähler 
  zurückgegeben, die mit den Abonnements des Kunden verbunden sind.
</Tip>
...
Generieren Sie ein Webhook-Geheimnis für Ihre Endpunkt-URL (z. B. <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) im Dodo Payments Dashboard und legen Sie es in Ihrer <code>.env</code>-Datei fest:
...
<AccordionGroup>
  <Accordion title="Häufige Probleme">
    ...
    <b>Webhook-Signatur stimmt nicht überein</b>: Stellen Sie sicher, dass das Webhook-Geheimnis mit dem übereinstimmt,
    das im Dodo Payments Dashboard festgelegt ist. - <b>Kunde nicht erstellt</b>: Bestätigen Sie, dass{" "}
