...
使用 `PATCH /subscriptions/{id}` 可在下一个计费日期取消订阅、延长订阅周期、更新账单详细信息或修改 metadata。若要更改数量，请改用[Change Plan API](/api-reference/subscriptions/change-plan)——`PATCH` 不接受 `quantity`。
