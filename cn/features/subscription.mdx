...
暂停和恢复由 update subscription endpoint 上的同一个 `pause` 字段控制。不存在单独的暂停 endpoint。
...
<Warning>
`pause` 不能与任何其他字段同时使用——将其与其他内容一同发送会被拒绝，并返回 `422`。将 `status` 设置为 `paused` **不会**暂停订阅；请改用 `pause` 字段。
</Warning>
...
| 状态 | 含义 | 可恢复？ | 恢复路径 / 下一步 |
|--------|---------------|--------------|---------------------------|
| `pending` | 正在创建或处理订阅 | — | 等待 `subscription.active`（或 `subscription.failed`） |
| `active` | 订阅处于活跃状态，并会自动续订 | — | 无需操作 |
| `on_hold` | 续订付款（或方案变更费用）失败；续订已停止 | **是** | 更新支付方式以重新激活——通过 [Payment Retries](/features/recovery/payment-retries) 和 [Dunning](/features/recovery/subscription-dunning) 自动恢复，或通过 [Update Payment Method API](/api-reference/subscriptions/update-payment-method) 手动恢复 |
| `paused` | 订阅由你或客户主动暂停；计费被冻结，访问权限被撤销 | **是** | 使用 `pause: false` 恢复，或在启用自助暂停后从 Customer Portal 恢复——参见 [暂停和恢复订阅](#pausing-and-resuming-subscriptions) |
| `cancelled` | 订阅已取消，不会再续订 | 仅可重新购买 | 客户必须创建新的订阅；[Dunning](/features/recovery/subscription-dunning) 可以提示客户重新购买 |
| `failed` | 订阅**创建**失败（初始 mandate 或付款未成功） | **否——终止状态** | 客户必须使用可用的支付方式创建新订阅 |
| `expired` | 订阅已达到期限结束 | — | 如有需要，客户必须创建新订阅 |
...
暂停和恢复通过同一个 `PATCH /subscriptions/{subscription_id}` endpoint 完成，并使用 `pause` 字段：`pause: true` 会暂停活跃订阅，`pause: false` 会恢复订阅。该字段不能与同一请求中的任何其他字段结合使用。有关完整行为、计费影响及相关业务设置，请参见[暂停和恢复订阅](#pausing-and-resuming-subscriptions)。

<Card title="API Reference" icon="code" href="/api-reference/subscriptions/patch-subscriptions">
查看 update subscription API，包括 `pause` 字段。
</Card>
</Accordion>
