...
| 续订失败 → 已暂停 | `subscription.on_hold` |
| 订阅已暂停 | `subscription.paused` |
| 已暂停的订阅已恢复 | `subscription.unpaused` |
| 创建失败 | `subscription.failed` |
| 套餐升级/降级 | `subscription.plan_changed` |
| 已取消 | `subscription.cancelled` |
| 期限结束 | `subscription.expired` |
| 任意字段发生更改 | `subscription.updated` |
...
使用 `PATCH /subscriptions/{subscription_id}` 可在下一计费日期取消订阅、延长订阅期限、更新账单详情或修改元数据。若要更改数量，请改用[更改套餐 API](/api-reference/subscriptions/change-plan)，因为 `PATCH` 不接受 `quantity`。
...
### 延长订阅期限
通过向 `PATCH /subscriptions/{subscription_id}` 传入新的 `subscription_period_count` 和 `subscription_period_interval`，延长订阅的持续时间。订阅的到期时间会根据新的计数和间隔重新计算——例如，为客户当前的套餐提供额外时间：
