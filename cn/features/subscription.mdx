...
## Subscriptions with RBI-Compliant Mandates
  ...
  - **低于授权最低限额的费用（默认 ₹15,000）：** 我们为最低限额创建按需授权。根据您的订阅频率，订阅金额会定期收取，直至授权限额。
  - **等于或超过授权最低限额的费用：** 我们根据确切的订阅金额创建订阅授权（或按需授权）。

  每个商家或每个请求可以通过 `mandate_min_amount_inr_paise`（INR paise）配置最低授权限额。向银行注册的金额为 `max(mandate_floor, billing_amount)`，因此，当账单金额较低时，该限额实际上成为面向客户的授权上限。

有关符合RBI要求的授权和印度支付方式的可配置授权最低限额的详细信息，请参阅<a href="/features/payment-methods/india#configurable-mandate-floor">印度支付方式</a>页面。

  ### 升级和降级注意事项

  **重要：** 在升级或降级订阅时，请仔细考虑授权限额：

  - 如果升级/降级导致费用超出 ₹15,000 并超过现有的按需支付限额，交易费用可能会失败。
  ...
  **处理时间表：** 印度卡和UPI订阅的定期费用遵循独特的处理模式：

  - 根据您的订阅频率，费用将在计划日期**发起**。
  - 客户账户的实际**扣款**仅在支付发起后**48小时**进行。
  - 根据银行API响应，这个48小时窗口可能会延长**2-3小时**。
  ...
  - 客户可以通过其银行应用取消授权。
  - 如果客户在此期间取消授权，订阅仍将保持**活跃**（这是印度卡和UPI自动支付订阅的一个特殊情况）。
  - 然而，实际扣款可能会失败，在这种情况下，我们将暂停订阅。

  **特殊情况处理：** 如果您在费用发起后立即为客户提供优惠、积分或订阅使用，您需要在您的应用程序中妥善处理这48小时窗口。考虑：
  ...
  <Tip>
  监控订阅webhooks以跟踪支付状态变化，并处理在48小时窗口期间授权被取消的特殊情况。
  </Tip>

## 最佳实践

- **从清晰的层级开始**：2-3个计划，差异明显
- **沟通定价**：显示总额、按比例分配和下次续费
- **明智使用试用**：通过引导而非仅仅时间转化用户
- **利用附加组件**：保持基础计划简单并向上销售额外服务
- **测试更改**：在测试模式下验证计划更改和按比例分配

<Info>
订阅是定期收入的灵活基础。保持简单，彻底测试，并根据采用、流失和扩展指标迭代。
</Info>