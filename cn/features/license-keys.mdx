...
当客户完成对启用许可证密钥产品的购买时，许可证密钥会自动作为查询参数附加到您的`return_url`。这使您可以在成功页面上立即显示密钥，而无需额外的 API 调用。

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001&email=customer%40example.com
```

如果购买生成了多个许可证密钥（例如，数量 > 1），则它们以逗号分隔：

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001,LK-002&email=customer%40example.com
```

对于订阅，使用`subscription_id` 代替 `payment_id`：
...
<Tip>
在您的返回页面上解析 `license_key` 参数，以便立即向客户显示密钥，改善他们的购买后体验。
</Tip>
...
- **引导用户**：提供精确的激活说明和自助文档
- **自动检查**：在服务器端验证密钥后再授予访问权限
- **监控事件**：使用 webhook 来检测滥用并自动撤销
