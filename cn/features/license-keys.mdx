...
- **`auto`**（默认）：Dodo Payments 会在付款或订阅时自动生成密钥并通过电子邮件发送。上述行为适用于省略 `fulfillment_mode` 的情况。
- **`manual`**：购买会创建一个没有密钥的 `Pending` 授权，您需要自行提供每个密钥值。请参阅下方的[手动履行](#manual-fulfillment)。

### 激活许可证

<CodeGroup>

默认情况下，客户付款后，Dodo Payments 会立即生成许可证密钥并通过电子邮件发送。使用**手动履行**时，您需要自行提供密钥：购买会创建一个没有密钥的 `Pending` 授权，通知您，并等待您提交密钥值。当密钥来自您自己的系统、第三方供应商或有限的预打印代码池时，可以使用此方式。
...
当客户购买手动模式的产品时，系统会创建一个处于 `Pending` 状态且没有密钥的授权，并触发一个包含 `integration_type: "license_key"` 和 `status: "Pending"` 的 [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) webhook。您可以响应该 webhook，也可以使用 `integration_type` 和 `status` 过滤器轮询 [列出客户授权](/api-reference/entitlements/list-customer-grants) endpoint：
...
使用 [履行许可证密钥授权](/api-reference/entitlements/fulfill-license-key) endpoint 提交密钥。授权会转为 `Delivered` 状态，客户将自动收到密钥——与自动履行时收到的电子邮件相同。
