...
| **升级计划** | 将产品更换为同一集合中的更高等级产品 | 可配置（默认：禁用） |
| **降级计划** | 将产品更换为同一集合中的更低等级产品 | 可配置（默认：禁用） |
| **取消** | 完全取消订阅 | 始终可用 |
...
| **允许暂停订阅** | 客户可以从 Customer Portal 暂停和恢复自己的订阅 | 已禁用 |
| **暂停时计费使用量** | 基于使用量的暂停订阅会在暂停时就其欠付的使用量开具发票，而不是将使用量计入下一张常规发票 | 已启用 |
| **防止试用滥用** | 已经兑换过试用的客户将被降级为付费且无试用期的购买，而不是获得新的试用期 | 已禁用 |
...
<Info>
“允许暂停订阅”仅控制 Customer Portal——无论该设置如何，你都可以从控制面板或 API 暂停和恢复订阅。“暂停时计费使用量”仅适用于基于使用量的订阅，并且按计费周期记录，因此在周期中途更改该设置不会改变当前周期的结算方式。请参阅[暂停和恢复订阅](/features/subscription#pausing-and-resuming-subscriptions)。
</Info>

<Info>
有关如何匹配和记录试用兑换的详细信息，请参阅[防止试用滥用](/features/subscription#preventing-trial-misuse)。
</Info>
...
Product Collections 可以通过 Dodo Payments 控制面板进行管理，也可以通过 API 以编程方式进行管理。API 提供对集合创建、更新、图像上传、归档，以及嵌套组和产品管理的完整控制。

### 控制面板操作

- **创建**：设置包含产品和组的新集合
- **更新**：修改名称、描述、图像和产品组织方式
- **重新排序**：拖放以更改产品显示顺序
- **启用/禁用产品**：控制哪些产品显示在结账页面中
- **归档**：隐藏集合，但不会永久删除（之后可以取消归档）

<Frame>
  <img 
    src="/images/product-collection/collection-dashboard.png" 
    alt="显示集合管理操作的 Product Collection 控制面板屏幕截图" 
  style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>

### API 管理

以下 endpoints 允许你以编程方式创建、更新、检索、归档和组织 Product Collections，包括管理嵌套组及其中的产品。

<AccordionGroup>
<Accordion title="Listing Product Collections">
使用 `GET` 请求向 `/product-collections` endpoint 获取与你的账户关联的所有 Product Collections。支持分页、按品牌筛选以及包含已归档集合。

<Card title="List Product Collections API" icon="code" href="/api-reference/product-collections/list-product-collections">
  在 List Product Collections API 文档中查看详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Creating a Product Collection">
通过向 `/product-collections` endpoint 发送 `POST` 请求，并提供名称、描述和品牌等详细信息，创建新的 Product Collection。

<Card title="Create Product Collection API" icon="code" href="/api-reference/product-collections/create-product-collection">
  在 Create Product Collection API 文档中查看详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Retrieving a Product Collection">
使用 `GET` 请求向 `/product-collections/{id}` endpoint 获取特定 Product Collection 的详细信息，包括其组和产品项目。

<Card title="Get Product Collection API" icon="code" href="/api-reference/product-collections/get-product-collection">
  在 Get Product Collection API 文档中查看详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Updating a Product Collection">
通过向 `/product-collections/{id}` endpoint 发送 `PATCH` 请求，修改 Product Collection 的详细信息（名称、描述、品牌等）。

<Card title="Update Product Collection API" icon="code" href="/api-reference/product-collections/update-product-collection">
  在 Update Product Collection API 文档中查看详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Uploading Collection Images">
通过预签名 URL 上传图像，将其关联到集合。向 `/product-collections/{id}/images` endpoint 请求上传 URL，然后在 60 秒内将图像 `PUT` 到返回的 URL。

<Warning>
预签名 URL 将在 60 秒后过期，因此必须在此时间范围内上传图像。
</Warning>
...
<Card title="Update Collection Images API" icon="code" href="/api-reference/product-collections/update-product-collection-images">
  在 Update Collection Images API 文档中查看详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Archiving a Product Collection">
通过向 `/product-collections/{id}` endpoint 发送 `DELETE` 请求来归档集合。这样会隐藏集合，使其无法用于新的用途，但不会永久删除它。

<Card title="Archive Product Collection API" icon="code" href="/api-reference/product-collections/archive-product-collection">
  在 Archive Product Collection API 文档中查看详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Unarchiving a Product Collection">
通过向 `/product-collections/{id}/unarchive` endpoint 发送 `POST` 请求来恢复已归档的集合。
...
<Card title="Unarchive Product Collection API" icon="code" href="/api-reference/product-collections/unarchive-product-collection">
  在 Unarchive Product Collection API 文档中查看详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Managing Groups within a Collection">
组可以帮助你在集合中组织产品（例如，“月度计划”和“年度计划”）。使用组 endpoints 在集合中添加、更新或删除组。
...
管理组中的各个产品项目——添加新产品、更新现有项目（例如显示顺序），或将其完全删除。

- **向组添加产品**：`POST /product-collections/{id}/groups/{group_id}/items`
- **更新组项目**：`PATCH /product-collections/{id}/groups/{group_id}/items/{item_id}`
- **删除组项目**：`DELETE /product-collections/{id}/groups/{group_id}/items/{item_id}`
...
- **合理分组**：按计费周期（月度/年度）或功能等级（starter/pro/enterprise）组织产品
- **策略性排序**：将最受欢迎或推荐的计划放在首位，因为它会在结账时预先选中
- **使用清晰的命名**：产品名称应清楚传达价值差异
- **启用双向变更**：同时允许升级和降级，为客户提供灵活性
- **考虑按比例分摊**：选择符合业务模式的按比例分摊模式
- **充分测试**：上线前在测试模式下验证结账和计划变更流程

<Check>
你现在可以创建 Product Collections，并为客户提供统一的计划选择体验。
</Check>

<CardGroup cols={2}>
<Card title="Products" icon="box" href="/features/products">
  创建一次性、订阅或基于使用量的产品，以添加到集合中。
</Card>
