...
| **防止试用滥用** | 已兑换过试用的客户将被降级为付费且不含试用期的购买，而不是获得新的试用期 | 已禁用 |
...
Product Collections 可以通过 Dodo Payments 控制面板进行管理，也可以通过 API 以编程方式进行管理。API 提供对集合创建、更新、图片上传、归档以及嵌套组和产品管理的完整控制。
...
<Frame>
  <img 
    ...
    alt="Product Collection 控制面板截图，展示集合管理操作" 
  style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>

### API 管理

以下端点允许你以编程方式创建、更新、获取、归档和组织 Product Collections，包括管理嵌套组及其中的产品。

<AccordionGroup>
<Accordion title="Listing Product Collections">
使用 `GET` 请求向 `/product-collections` 端点获取与你的账户关联的所有 Product Collections。支持分页、按品牌筛选以及包含已归档的集合。

<Card title="List Product Collections API" icon="code" href="/api-reference/product-collections/list-product-collections">
  请参阅 List Product Collections API 文档，了解详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Creating a Product Collection">
向 `/product-collections` 端点发送 `POST` 请求，并提供名称、描述和品牌等详细信息，以创建新的 Product Collection。

<Card title="Create Product Collection API" icon="code" href="/api-reference/product-collections/create-product-collection">
  请参阅 Create Product Collection API 文档，了解详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Retrieving a Product Collection">
使用 `GET` 请求向 `/product-collections/{id}` 端点获取特定 Product Collection 的详细信息，包括其中的组和产品项。

<Card title="Get Product Collection API" icon="code" href="/api-reference/product-collections/get-product-collection">
  请参阅 Get Product Collection API 文档，了解详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Updating a Product Collection">
向 `/product-collections/{id}` 端点发送 `PATCH` 请求，以修改 Product Collection 的详细信息（名称、描述、品牌等）。

<Card title="Update Product Collection API" icon="code" href="/api-reference/product-collections/update-product-collection">
  请参阅 Update Product Collection API 文档，了解详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Uploading Collection Images">
通过预签名 URL 上传图片，将图片关联到集合。向 `/product-collections/{id}/images` 端点请求上传 URL，然后在 60 秒内将图片 `PUT` 到返回的 URL。
...
<Card title="Update Collection Images API" icon="code" href="/api-reference/product-collections/update-product-collection-images">
  请参阅 Update Collection Images API 文档，了解详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Archiving a Product Collection">
向 `/product-collections/{id}` 端点发送 `DELETE` 请求，以归档集合。此操作会使集合无法用于新的操作，但不会永久删除它。

<Card title="Archive Product Collection API" icon="code" href="/api-reference/product-collections/archive-product-collection">
  请参阅 Archive Product Collection API 文档，了解详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Unarchiving a Product Collection">
向 `/product-collections/{id}/unarchive` 端点发送 `POST` 请求，以恢复已归档的集合。
...
<Card title="Unarchive Product Collection API" icon="code" href="/api-reference/product-collections/unarchive-product-collection">
  请参阅 Unarchive Product Collection API 文档，了解详细的请求和响应结构。
</Card>
</Accordion>

<Accordion title="Managing Groups within a Collection">
组可以帮助你在集合内组织产品（例如，“月度方案”和“年度方案”）。使用组端点在集合中添加、更新或删除组。
...
管理组内的各个产品项——添加新产品、更新现有产品项（例如显示顺序），或将其彻底删除。

- **向组中添加产品**：`POST /product-collections/{id}/groups/{group_id}/items`
- **更新组产品项**：`PATCH /product-collections/{id}/groups/{group_id}/items/{item_id}`
- **删除组产品项**：`DELETE /product-collections/{id}/groups/{group_id}/items/{item_id}`
...
- **策略性排序**：将最受欢迎或推荐的方案放在第一位，因为它会在结账时预先选中
- **使用清晰的命名**：产品名称应清楚传达价值差异
- **启用双向变更**：同时允许升级和降级，为客户提供灵活性
- **考虑按比例计费**：选择符合业务模式的按比例计费模式
- **充分测试**：在正式上线前，在测试模式下验证结账和方案变更流程

<Check>
你现在可以创建 Product Collections，为客户提供统一的方案选择体验。
</Check>

<CardGroup cols={2}>
<Card title="Products" icon="box" href="/features/products">
  创建一次性、订阅或基于用量的产品，以添加到集合中。
</Card>
