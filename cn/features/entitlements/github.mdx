...
1. 客户完成结账。
2. Dodo Payments 在 `Pending` 状态下创建 grant。其 `oauth_url` 为 `null`，直到客户开始 accept flow；此时该字段会填充 GitHub authorization URL。
3. 客户通过其电子邮件或 Customer Portal 中的链接完成授权。
4. Dodo Payments 按照配置的 permission 邀请客户的 GitHub 账户加入 repository。创建邀请后，grant 将变为 `Delivered`。
5. 如果 subscription 被取消、退款或撤销，客户将被移除为 collaborator。
...
- `entitlement_grant.created` 会触发，并携带 `status: "Pending"`；此时 `oauth_url` 仍为 `null`。客户从 delivery email 或 Customer Portal 开始 accept flow 后，GitHub authorization URL 才会填充。
- 创建 collaborator 邀请后，`entitlement_grant.delivered` 会触发。
- 如果 GitHub App 失去对 repository 的访问权限，`entitlement_grant.failed` 会携带 `error_code: "github_permission_denied"`（或类似值）触发。
