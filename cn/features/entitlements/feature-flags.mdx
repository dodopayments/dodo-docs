...
| 订阅被暂停、取消或过期 | 使用匹配的 `revocation_reason` 撤销 Grant。 |
| 一次性付款退款 | 使用 `revocation_reason: refund` 撤销 Grant。 |
| 订阅恢复（例如，催收成功） | 将已撤销的 Grant 恢复为 `Delivered` — 同一 Grant `id`，清除撤销字段。 |
| 手动 API revoke | 使用 `revocation_reason: manual` 撤销 Grant。续订时不会自动恢复。 |
