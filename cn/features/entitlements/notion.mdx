...
1. 客户完成结账。
2. Dodo Payments 创建一个状态为 `Pending` 的授权，并生成一个 Notion `oauth_url`，供客户连接其工作区。
3. 客户通过电子邮件或 Customer Portal 中的链接完成授权，然后选择要将模板复制到的工作区。
4. Dodo Payments 复制模板。授权状态变为 `Delivered`。
5. 撤销时，授权会被标记为 `Revoked`；复制的模板仍会保留在客户的工作区中。
