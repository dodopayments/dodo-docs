...
<Step title="Webhook已发送">
`payment.succeeded` webhook 在实际扣款后发送，而不是在发起时。
</Step>
</Steps>

<Warning>
**请勿在扣款发起时授予福利。** 请等待`payment.succeeded` webhook，该webhook大约在计划扣款日期后48-51小时到达。
</Warning>
...
| 事件 | 时间 | 操作 |
| :---- | :--- | :----- |
| `subscription.created` | 授权的授权 | 记录订阅开始 |
| `payment.succeeded` | 扣款日期后~48小时 | 授予/继续访问 |
| `payment.failed` | 借记失败 | 通知客户，暂停访问 |
...
客户可以随时通过他们的银行应用程序取消授权。监控`subscription.on_hold` webhook，并提示客户重新订阅或更新付款方式。
</Accordion>
...
<AccordionGroup>
<Accordion title="结账时未显示UPI">
**检查：**
1. 账单国家设置为`IN`？
2. 货币设置为`INR`？
3. 如果是非印度商户：自适应货币是否已启用？
4. `upi_collect`是否包含在`allowed_payment_method_types`中？

**解决方案：** 验证账单地址是否具有`country: "IN"`和`billing_currency: "INR"`。
</Accordion>
...
**解决方案：** 下一次扣款将失败，订阅将转移到`on_hold`。监控`payment.failed` webhook。 
</Accordion>
</AccordionGroup>
