...
title: 印度支付方式
sidebarTitle: 印度
description: 在印度接受 UPI 和所有印度发行的卡，并符合 RBI 的订阅规定。了解 48 小时处理延迟，授权限制和 webhook 处理。
icon: 印度卢比符号
keywords:
  - Dodo Payments
  - UPI
...
印度拥有独特的支付基础设施，其中 UPI 占据了60%以上的数字交易，以及印度发行的卡（Visa、Mastercard、Rupay等）。Dodo Payments 完全支持这些支付方式，并完全符合 RBI 的订阅授权规定。
...
与大多数其他支付方式不同，UPI 和所有印度发行的卡（Visa、Mastercard、Rupay 等）支持通过 RBI 授权进行定期付款。
</Card>
</CardGroup>
...
*订阅需要符合 RBI 规定的授权，并具有特殊的处理规则。48 小时处理延迟适用于所有印度发行的卡和 UPI。
...
<Accordion title="Payment deduction delayed beyond 48 hours">
**原因：** 银行 API 延迟可能使处理时间延长 2-3 个小时。

**解决方案：** 这是预期情况。构建您的系统以处理高达总共约 51 小时的可变延迟。
</Accordion>

<Accordion title="Mandate cancelled but subscription still active">
**原因：** RBI 监管的边缘情况——在处理窗口期取消授权不会立即取消订阅。

**解决方案：** 下次收费将失败，订阅将转至 `on_hold`。监控 webhooks 以获取 `payment.failed`。
</Accordion>
</AccordionGroup>
