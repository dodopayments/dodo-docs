...
- **从简单开始**：先使用单一积分类型且不启用滚存。根据客户反馈和使用情况逐步增加复杂性。
- **设定明确预期**：在产品页面和客户门户显著展示积分分配、剩余额度和超额定价。
- **使用具意义的单位**：用积分所代表的内容命名（例如 “API 调用”、“AI 代币”），而非通用术语，有助于客户理解价值。
- **谨慎配置过期**：短期过期（7 天）可激发紧迫感，但可能让客户产生不满。大多数 SaaS 产品更适合较长时间窗口（30–90 天）。
- **监控低余额**：设置低余额阈值，在客户耗尽前提醒，减少突发性的超额费用。
- **在测试模式下验证**：创建积分、将其附加到测试产品，并在上线前模拟完整的购买 → 使用 → 扣除 → 过期流程。

<Info>
基于积分的计费可以与所有其他 Dodo Payments 功能无缝协作——包括含试用的订阅、带比例调整的计划变更以及客户门户。先从基础设置入手，随着定价模型演进逐步扩展。
</Info>
