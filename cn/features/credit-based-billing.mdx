...
所有分类账事件（`credit.added` 到 `credit.manual_adjustment`）包括完整的 `CreditLedgerEntry` 负载，其中包含变更前后的余额、超额前后的情况、来源参考以及授予来源订阅或付款的 `metadata`（如果是直接通过 API 创建的授予则为空）。`credit.balance_low` 事件包括阈值配置和当前余额。
