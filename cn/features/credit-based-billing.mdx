...
| 来源 | 描述 |
|--------|-------------|
| **订阅** | 购买订阅时发放的 Credits，并在每个计费周期重新发放 |
| **一次性** | 购买一次性付款产品时发放的 Credits |
| **附加项** | 添加到订阅中的附加项所发放的 Credits |
| **API** | 通过 API 或 dashboard 手动授予的 Credits |
| **结转** | 从之前计费周期结转的 Credits |
...
- Up to `10` - For high-precision units (e.g. fractional tokens or micro-usage)
...
2. **Meters aggregate events** - Using Count, Sum, Max, or Last aggregation
...
| 交易类型 | 描述 |
|-----------------|-------------|
| **Credit Added** | 已授予的 Credits（订阅、一次性付款或 API） |
| **Credit Deducted** | 通过使用或手动扣款消耗的 Credits |
| **Credit Expired** | 未结转而过期的 Credits |
| **Credit Rolled Over** | 结转至下一个周期的 Credits |
| **Rollover Forfeited** | 达到最大结转次数后失效的结转 Credits |
| **Overage Charged** | 启用超额用量后，超出 Credits 余额的用量所产生的费用 |
| **Overage Reset** | 在计费周期重置时清除已跟踪的超额用量 |
| **Auto Top-Up** | 余额较低时自动补充 Credits |
| **Manual Adjustment** | Merchant 手动应用的 Credit 或 debit |
| **Refund** | 退还的 Credits |
...
| 事件 | 描述 |
|-------|-------------|
| `credit.added` | 授予 Customer 的 Credits |
| `credit.deducted` | 通过使用或手动扣款消耗的 Credits |
| `credit.expired` | 未使用的 Credits 已过期 |
| `credit.rolled_over` | 结转至新发放记录的 Credits |
| `credit.rollover_forfeited` | 达到最大结转次数后失效的 Credits |
| `credit.overage_charged` | 已应用超额用量费用 |
| `credit.overage_reset` | 已跟踪的超额用量在计费周期结束时重置 |
| `credit.manual_adjustment` | 已手动进行 Credit/debit 调整 |
| `credit.balance_low` | 余额低于配置的阈值 |

所有 ledger 事件（除 `credit.balance_low` 事件外的每个 `credit.*` 事件）都包含完整的 `CreditLedgerEntry` payload，其中包括变更前后的余额、变更前后的超额用量、来源引用，以及发放记录来源订阅或付款的 `metadata`（直接通过 API 创建的发放记录为空）。`credit.balance_low` 事件包含阈值配置和当前余额。
