...
- **定价**：以最小货币单位设置附加项金额（`price`）。附加项会按照父订阅的计费周期进行计费
- **货币**：以 Dodo Payments 可收取的任意货币设置基础价格；可搜索的货币选择器会将 **USD、GBP、EUR 和 INR** 固定显示在顶部；基础货币以外的客户将通过 [Adaptive Currency](/features/adaptive-currency) 进行计费
- **数量**：将附加项关联到订阅时，客户可以选择数量
- **可用性**：控制哪些订阅产品可以使用该附加项
- **税费设置**：配置适用的 `tax_category`
