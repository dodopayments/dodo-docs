...
使用 `POST /refunds` 发起退款。对于部分退款和逐项退款，您可以使用 `tax_inclusive` 标志指定每个 `items` 数组条目是否包含税费（默认为 `true`）。省略 `items` 可全额退还该笔付款。
...
- **选择正确的税务类别**，确保按地区正确计算。
- 在产品文案中使用“Tax Inclusive”标签，**清晰传达总金额**。
- 在 **Test Mode** 中进行测试以完成验证。
