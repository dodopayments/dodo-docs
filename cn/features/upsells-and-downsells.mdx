...
| 模式 | 行为 | 最适用场景 |
|------|----------|----------|
| `difference_immediately` | 立即收取价格差额（$30→$80 = $50） | 简单升级 |
| `prorated_immediately` | 根据计费周期中的剩余时间收费 | 公平的按时间计费 |
| `full_immediately` | 收取新套餐的完整价格，忽略剩余时间 | 重置计费周期 |
| `do_not_bill` | 应用套餐变更但不立即收费；新套餐将在下次续订时计费，并保留原始计费日期 | 礼遇升级和免费迁移 |
