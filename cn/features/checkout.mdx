...
<Info>
本节涵盖使用 `customization.theme_config` 的**服务器端 API**主题配置。如果您正在使用**Checkout SDK**（overlay 或 inline checkout），请参阅 [Overlay Checkout](/developer-resources/overlay-checkout#theme-customization) 中的主题自定义部分，该部分使用 camelCase 属性（例如，使用 `bgPrimary` 而不是 `bg_primary`）。
</Info>
...
| `font_secondary_url` | 次字体的 URL |
| `font_size` | 字体大小：`xs`, `sm`, `md`, `lg`, `xl`, `2xl` |
| `font_weight` | 字体粗细：`normal`, `medium`, `bold`, `extraBold` |
| `radius` | UI 元素的边框半径（例如，`4px`, `0.5rem`, `8px`） |
| `pay_button_text` | 支付按钮的自定义文本（例如，“完成购买”、“立即订阅”） |

### 颜色配置（Light/Dark 模式）

每个模式（`light` 和 `dark`）支持以下颜色属性：
...
| `button_text_primary` | 主按钮文字颜色 |
| `button_text_secondary` | 次按钮文字颜色 |
| `input_focus_border` | 输入框焦点边框颜色 |
