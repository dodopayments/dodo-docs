...
对于重复产品，将 `price` 对象设置为重复价格（`type: recurring_price`），并指定 `payment_frequency_interval` 和 `subscription_period_interval`（`Day`，`Week`，`Month`，或 `Year`）以及它们对应的数量。
...
如果您不再希望展示或使用某个产品，可以通过向 `/products/{id}` 端点发送 `DELETE` 请求来存档它。此操作会隐藏产品，但不会永久删除它。
