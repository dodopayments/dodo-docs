...
| 机制 | 作用 | 客户看到的金额 |
| :--- | :--- | :--- |
| **本地化定价** | 为每种货币或国家设置的固定价格 | 恰好是您设置的金额 |
| [**自适应货币**](/features/adaptive-currency) | 自动按实时外汇汇率转换您的基础价格 | 按当前汇率转换的基础价格 |
| [**折扣码**](/features/discount-codes) | 按比例或固定减少基础价格 | 基础价格减去折扣 |
...
| 行为 | 详情 |
| :--- | :--- |
| **税前金额** | 本地化金额是税前价格。对于不含税的产品，税将在结账时添加，因此本地化金额不是最终总额。请参阅[税收包含定价](/features/tax-inclusive-pricing)。 |
| **不适用于您想支付的价款** | 本地化规则从不适用于[您想支付的价款](/features/pay-what-you-want)产品，顾客决定金额。 |
| **按货币与基础货币不同** | `by_currency` 规则必须使用不同于产品基础货币的货币。 |
| **每个市场只有一个规则** | 一个产品在每种货币（按货币计算）或每个国家（按国家计算）最多只能有一个活动规则。 |
| **所有产品类型** | 适用于一次性、订阅和按使用量计费的产品。 |
...
- **在广告总价前注意税额行。** 对于不含税的产品，客户支付本地化金额*加上*税，因此在广告中不要将其作为最终价格引用。
