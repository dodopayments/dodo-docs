...
<Warning>
目前我们仅支持**卡令牌**的迁移。由于这些提供商施加的令牌化限制，Apple Pay、Google Pay 及其他钱包支付方式无法在支付处理器之间转移。
</Warning>
...
<Note>
要开始订阅迁移，请联系我们的 [支持团队](mailto:support@dodopayments.com)。他们会将您与专属解决方案工程师对接，指导您完成整个流程。
</Note>
...
Dodo Payments 符合 PCI DSS 第 1 级标准。为满足合规要求，我们只能从另一家符合 PCI DSS 第 1 级标准的支付处理器接收卡数据。

您当前的处理器必须提供：
- 他们当前的 PCI 合规性证明（AOC），或他们在 Visa 全球服务提供商注册表上的列表
- 确认他们支持安全的卡数据导出

查看我们与当前处理器共享的[合规确认书（AOC）](https://pub-2a89184e4c954b599464449d23f506bb.r2.dev/dodo-payments-pci-dss-aoc.pdf)。
</Accordion>

{/* LOCKED_PATTERN_2b95405ae9968e886adf3cd132261858 */}
在开始迁移之前，请完成 Dodo Payments 的集成，并确保所有新客户都前往 Dodo Payments 结账。这可以防止过渡期间出现数据不一致。
</Accordion>

{/* LOCKED_PATTERN_ed553440512ecf0bf4ebb92bf3502eb8 */}
确保您具备向当前处理器请求数据导出的必要授权。通常需要：
- 验证账户所有者身份
- 获得针对敏感数据访问的书面授权
- 完成处理器的安全验证步骤
</Accordion>
...
联系您当前支付处理器的支持团队并提交正式请求，要求完整导出客户卡令牌。
...
<Info>
安全传输通过 SFTP 直接由服务器到服务器进行。您的方案工程师会提供 SFTP 凭据，供您与当前处理器共享。
</Info>

平均而言，该过程大约需要 **2 周**，尽管具体时间取决于当前处理器的政策和响应速度。
</Step>

{/* LOCKED_PATTERN_15cbd22b705fcdc8e946112a6df45d10 */}
在支付方式转移进行期间，请从旧系统导出您的订阅者数据。方案工程师会提供 CSV 模板并指导您填写所需字段。
...
| `email` | 客户的电子邮箱地址 |
| `name` | 客户全名 |
| `country` | 两位字母的国家代码（ISO 3166-1 alpha-2） |
| `currency` | 三位字母货币代码（ISO 4217） |
| `payment_amount` | 订阅金额，单位为最小货币单位 |
| `payment_interval` | 账单频率（月度、年度等） |
| `next_billing_date` | 下一个计划账单日期（ISO 8601） |
| `card_token` | 来自您当前处理器的唯一令牌 |

<Warning>
`card_token` 字段至关重要——它用于在导入期间将订阅者记录与其支付方式匹配。请确保其内容与支付方式导出中的令牌完全一致。
</Warning>
...
<Info>
对于标准迁移，典型处理时间为 **2-3 个工作日**，具体取决于订阅者数量。
</Info>
</Step>
...
<Note>
付款历史和交易记录无法迁移。我们建议在完成迁移之前，先从当前处理器导出这些数据以备存档。
</Note>
...
<Tip>
整个迁移通常从开始到完成需要 **3-4 周**。在支付方式转移进行期间开始准备订阅者数据，有助于缩短整体时间线。
</Tip>
...
探索我们的 API 以了解 Dodo Payments 中订阅的工作方式。
</Card>
</CardGroup>
