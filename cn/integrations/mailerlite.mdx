...
| 参数 | 类型 | 必填 | 描述 |
|-----------|------|----------|-------------|
| `email` | 字符串 | 是 | 有效的电子邮件地址 (RFC 2821) |
| `fields` | 对象 | 否 | 包含字段名称/值对的对象 |
| `fields.name` | 字符串 | 否 | 订阅者的名字 |
| `fields.last_name` | 字符串 | 否 | 订阅者的姓氏 |
| `fields.company` | 字符串 | 否 | 公司名称 |
| `fields.country` | 字符串 | 否 | 国家 |
| `fields.city` | 字符串 | 否 | 城市 |
| `fields.phone` | 字符串 | 否 | 电话号码 |
| `groups` | 数组 | 否 | 要添加订阅者的组 ID 数组 |
| `status` | 字符串 | 否 | 值之一：active, unsubscribed, unconfirmed, bounced, junk |
| `subscribed_at` | 字符串 | 否 | 格式为 `yyyy-MM-dd HH:mm:ss` 的日期 |
| `ip_address` | 字符串 | 否 | 订阅者的 IP 地址 |
