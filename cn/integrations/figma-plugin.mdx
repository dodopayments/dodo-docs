...
<Steps>
<Step title="创建基础插件">
使用 [Figma 的插件快速入门指南](https://www.figma.com/plugin-docs/plugin-quickstart-guide/) 创建你自己的 Figma 插件项目。这将生成带有 `manifest.json` 文件的脚手架。
...
<Step title="克隆示例实现">
克隆 [dodopayments-figma 仓库](https://github.com/dodopayments/dodopayments-figma)，并使用从生成的插件文件夹中获取的名称和 ID 更新它的 `manifest.json`。
...
- 更新 `Authenticated.tsx` 以向具有有效许可密钥的用户公开功能
- 自定义 `LicenseKeyInput.tsx` 以匹配你的设计偏好

<Note>
`Authenticated.tsx` 组件是你添加需要有效许可的插件高级功能的地方。
</Note>
</Step>
</Steps>
...
<Steps>
<Step title="配置网络访问">
插件需要网络访问以与 CORS 代理通信。将以下内容添加到你的 `manifest.json`：
...
<Tip>
仅在准备好生产并在测试模式下彻底测试后切换到 `live_mode`。
</Tip>
</Step>
</Steps>
