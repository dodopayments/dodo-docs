...
Dub 使用一个独特的点击 ID (`dub_id`) 来追踪访问者，当用户点击你的 Dub 短链接时，该 ID 会存储在一个 cookie 中。要将销售归因于你的链接，你需要：

1. **在创建结账会话时，从 `dub_id` cookie 中捕获 Dub 的点击 ID**
2. **将点击 ID 存储在你的支付元数据中，连同客户的外部 ID**
3. **在付款成功后，通过他们的 Track API 将销售数据发送给 Dub**
...
<Step title="获取你的 Dub API 密钥">
  导航至你的 [Dub 控制面板](https://app.dub.co) → 设置 → API 密钥，并创建一个具有 `conversions.write` 范围的新 API 密钥。
...
- **始终在元数据中包含点击 ID**：没有点击 ID，Dub 无法将收入归因于你的链接
- **一致使用外部 ID**：传递你在系统中使用的相同客户 ID，以实现准确的客户级分析
- **优雅处理自然流量**：当没有点击 ID 时设置 `webhook.cancel = true`，以避免不必要的 API 调用
- **使用样本付款进行测试**：在上线前验证集成是否正常工作
- **监控你的 Dub 控制面板**：检查销售是否正确出现并有适当的归因
...
<AccordionGroup>
<Accordion title="销售未在 Dub 中显示">
- 验证你的 Dub API 密钥是否正确，并具有 `conversions.write` 范围
...
- 确认用户在结账前点击了你的 Dub 短链接
- 验证 `dub_id` cookie 是否在你的域上正确设置
- 检查点击 ID 是否在创建结账和支付完成之间匹配
- 确保在创建结账会话之前捕获点击 ID
</Accordion>

<Accordion title="转换错误">
- 验证 JSON 结构与 Dub 的 Track Sale API 格式匹配
...
<Accordion title="重复销售被记录">
- 确保你只在 `payment.succeeded` 事件上进行跟踪，而不是 `payment.processing`
- 为每一笔销售使用独特的 `invoiceId` 值
- 对于订阅，请在续订时附加时间戳或计费周期以防止重复
</Accordion>
</AccordionGroup>
