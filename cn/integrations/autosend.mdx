...
<Step title="输入 API 密钥">
  ...
  <Card title="了解如何创建和管理 API 密钥" icon="key" href="https://docs.autosend.com/api-keys">
...
- **使用动态数据进行个性化**：使用 `dynamicData` 字段来个性化电子邮件，包含客户特定的信息，如姓名、付款金额和订阅详情。个性化的电子邮件具有更高的参与率。
...
- 验证 API 密钥是否正确且有效
- 检查发件人域名是否在 AutoSend 中验证
- 确保收件人电子邮件地址有效
- 查看 AutoSend 的发送限制和配额
...
- 检查所有必需字段是否存在 (`to`, `from`, `templateId` 或 `html`/`text`)
- 确保电子邮件地址格式正确
- 如果使用模板，验证 `templateId` 是否有效
- 检查 `dynamicData` 键是否与您的模板变量匹配
</Accordion>

<Accordion title="模板问题">
