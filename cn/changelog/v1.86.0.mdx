...
<Frame>
<img src="/images/CBB/Checkout.png" alt="结帐显示产品购买包含的积分" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Frame>
<img src="/images/CBB/Customer Portal.jpg" alt="客户门户积分视图，显示余额和交易历史" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Frame>
<img src="/images/design/general-color-settings.jpg" alt="高级设置已展开，显示明暗模式的完整颜色配置" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Tip>
使用这些字段来构建争议和退款仪表盘、触发争议状态变化的警报，或关注需要注意的付款。
</Tip>

了解更多：[支付列表 API](/api-reference/payments/get-payments)

### 4. **客户门户中的退款表格**

客户门户现在包含一个专门的**退款**部分，客户可以查看其付款关联的所有退款。每个退款条目显示退款金额、状态、日期以及其对应的原始付款，这使客户无需联系支持即可全面了解其退款历史。

<Check>
所有企业的客户门户中都会自动提供退款表格。无需进行配置。
</Check>
...
现在，您可以直接从仪表盘**从测试模式复制计量器到实时模式**。在测试模式中完成计量器配置的测试后，使用复制操作将其复制到实时模式，并保留所有设置：聚合类型、事件名称、过滤器和信用关联。这消除了从开发到生产时手动重新创建计量器的需求。

<Tip>
首先在测试模式中构建和验证您的计量器配置，然后在准备生产时将其复制到实时模式。这确保您的计费设置在处理真实客户使用之前得到测试。
</Tip>

了解更多：[基于使用的计费](/features/usage-based-billing/introduction) | [计量器](/features/usage-based-billing/meters)
