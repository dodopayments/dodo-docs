...
- **Subscription object**：新增 `cancellation_feedback` 字段（上述值之一）和 `cancellation_comment`（可选的自由文本），在客户取消时填充
- **`subscription.cancelled` webhook**：载荷中包含这两个字段
- **API**：以编程方式安排或执行取消时，将 `cancellation_feedback` 和 `cancellation_comment` 传递给 `PATCH /subscriptions/{subscription_id}`
...
您还可以使用 `adaptive_currency_fees_inclusive` 布尔值，为单笔付款、按需订阅扣款和计划变更覆盖商户默认设置：
