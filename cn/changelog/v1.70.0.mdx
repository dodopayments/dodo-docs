...
使用 `displayType: 'inline'` 初始化 SDK，并在容器元素中嵌入结账：
   ...
   - **实时细分事件**：当客户输入他们的详细信息时，接收 `checkout.breakdown` 事件
   - **自定义订单摘要**：构建自己的定价显示，实时更新
   - **无缝集成**：结账框处理支付收集，而您控制布局
   - **SDK 方法**：使用 `open()`、`close()` 和 `isOpen()` 以编程方式控制结账
...
2. **使用现有支付方式创建付款、订阅和结账**  
    通过使用现有客户的已保存支付方式来简化结账流程。新的 `payment_method_id` 参数允许您使用客户先前保存的支付方式创建付款、订阅或结账会话。
   ...
   创建结账会话、付款或订阅时使用 `payment_method_id`：
   ...
   <Warning>
   在结账会话中使用 `payment_method_id` 时，`confirm` 必须设置为 `true`，并且必须提供现有的 `customer_id`。支付方式将与支付的货币进行资格验证。
   </Warning>
...
5. **`redirect_immediately` 标志 - 跳过支付成功页面**  
   使用新的 `redirect_immediately` 标志控制结账流程。当启用时，客户在完成支付后会立即被重定向，跳过默认成功页面，获得更快、更流畅的体验。
  ...
  ```
   ...
   <Tip>
   当您有比默认支付成功页面提供更好用户体验的自定义成功页面时，请使用 `redirect_immediately: true`。
   </Tip>

   <Note>
   当 `redirect_immediately` 启用时，客户在完成支付后会立即重定向到您的 `return_url`，完全跳过默认成功页面。
   </Note>
