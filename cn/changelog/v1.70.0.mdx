...
1. **内嵌结账 - 完全嵌入式结账体验（测试版）**  
   ...
   使用 `displayType: 'inline'` 初始化 SDK，并将结账嵌入到容器元素中：
   ...
   - **实时事件分解**：当客户输入他们的详细信息时接收 `checkout.breakdown` 事件
...
2. **使用现有支付方式创建支付、订阅和结账**  
   ...
   <Warning>
   在结账会话中使用 `payment_method_id` 时，`confirm` 必须设置为 `true`，并且必须提供现有的 `customer_id`。支付方式将根据支付的货币进行资格验证。
   </Warning>
...
5. **`redirect_immediately` 标志 - 跳过支付成功页面**  
   使用新的 `redirect_immediately` 标志控制结账流程。当启用时，客户在支付完成后会立即被重定向，跳过默认的成功页面，以获得更快、更流畅的体验。
  ...
  ```
   ...
   <Tip>
   当您有一个自定义成功页面提供比默认支付成功页面更好的用户体验时，请使用 `redirect_immediately: true`。
   </Tip>

   <Note>
   当启用 `redirect_immediately` 时，客户在支付完成后会立即被重定向到您的 `return_url`，完全跳过默认的成功页面。
   </Note>
