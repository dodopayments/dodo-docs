...
<Info>
该样板使用 Next.js 16 应用路由、TypeScript、Tailwind CSS 4 和 `@dodopayments/nextjs` 适配器。
</Info>

### 特性

- **快速设置** - 在 5 分钟内开始使用
- **支付集成** - 使用 `@dodopayments/nextjs` 的预配置结账流程
- **现代 UI** - 使用 Tailwind CSS 的简洁深色主题定价页面
- **Webhook 处理程序** - 可用于支付事件的现成 webhook 端点
- **客户门户** - 一键订阅管理
- **TypeScript** - 完全类型化，类型简明集中
...
<Step title="添加您的产品">
使用您来自 Dodo Payments 的实际产品 ID 更新 `src/lib/products.ts`：
...
在 `src/app/components/ProductCard.tsx` 中，用您的实际用户数据替换硬编码值：
...
该样板演示了在 `src/app/api/webhook/route.ts` 处理两个 webhook 事件：
...
在 `src/app/components/Header.tsx` 中，用来自您 Dodo 仪表板的实际客户 ID 替换硬编码的 `CUSTOMER_ID`。
