...
**计量名称**: 请精确复制并粘贴此内容 → `Image Generation Meter`
...
<Warning>
事件名称 `image.generated` 必须与我们稍后从应用程序代码发送的完全匹配。事件名称对大小写敏感！
</Warning>
</Step>
...
1. **启用事件过滤**: 将此切换为 **开启**
2. **过滤逻辑**: 选择 **或**（这意味着 "只要满足这些条件中的任何一个"）
3. **添加第一个条件**:
   - 属性键: `quality`
   - 比较器: `equals`  
   - 值: `standard`
...
此外，将 **固定价格** 设置为 `0`，以确保客户仅根据其使用情况收费，没有基本费用。
...
付款链接将类似于: `https://test.checkout.dodopayments.com/buy/pdt_IgPWlRsfpbPd5jQKezzW1?quantity=1`
</Step>
