...
- 通过 Supabase 进行身份验证（Google OAuth 已配置）
- 通过 Dodo Payments 进行订阅结账
- Supabase Edge Function 用于 Webhook（`dodo-webhook`）
- Drizzle ORM 架构和迁移
- 包含发票、订阅状态和计划功能的仪表板

<Tip>
在完成端到端测试之前，请将 `DODO_PAYMENTS_ENVIRONMENT` 保持为 `test_mode`。
</Tip>
...
- 检查 `DATABASE_URL` 语法和 Supabase 网络发布
