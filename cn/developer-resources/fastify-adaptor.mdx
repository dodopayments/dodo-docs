---
title: Fastify 适配器
description: 学习如何使用我们的 NextJS 适配器将 Dodo Payments 集成到您的 Fastify App Router
...
<Warning>
切勿提交您的 <code>.env</code> 文件或机密信息到版本控制。
</Warning>
</Step>
</Steps>
...
固定收取的金额，单位为主要货币单位（例如，<code>12.5</code> 表示 12.50 美元）。仅适用于 Pay What You Want 产品；如果金额低于产品最低价格，则会忽略此参数。
</ParamField>
...
结账会话提供了更安全的托管结账体验，处理一次性购买和订阅的完整支付流程，并提供完全的定制控制。
...
客户门户路由处理程序使您能够将 Dodo Payments 客户门户无缝集成到您的 Fastify 应用中。
...
- **签名验证：** 使用 <code>webhookKey</code> 验证 webhook 签名。如果验证失败则返回 401。
- **有效负载验证：** 使用 Zod 验证。无效的负载返回 400。
- **错误处理：**
  - 401：无效签名
  - 400：无效负载
  - 500：验证期间的内部错误
