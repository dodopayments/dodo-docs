---
title: Bun 适配器
description: 了解如何使用我们的 Bun 适配器将 Dodo Payments 集成到您的 Bun 服务器项目中。涵盖结账、客户门户、Webhooks 和安全环境设置。
icon: code-merge
keywords:
  - Dodo Payments
  - Bun
...
<Info>
所有示例均假设您使用 Bun 的原生服务器 <code>Bun.serve()</code>。
</Info>
...
<Tab title="Webhook Handler">
  <Info>
  使用此处理程序在您的 Bun 服务器中安全地接收和处理 Dodo Payments Webhook 事件。
  </Info>
  <CodeGroup>
...
固定收取的金额，单位为主要货币单位（例如，<code>12.5</code> 表示 $12.50）。仅适用于 Pay What You Want 产品；如果金额低于产品最低价格，则会忽略该参数。
</ParamField>
...
结账会话提供更安全的托管结账体验，可处理一次性购买和订阅的完整付款流程，并具有完整的自定义控制。

请参阅 [Checkout Sessions Integration Guide](https://docs.dodopayments.com/developer-resources/checkout-session) 以获取更多详细信息和支持字段的完整列表。
...
如果设置为 <code>true</code>，则通过电子邮件将门户链接发送给客户。
</ParamField>
