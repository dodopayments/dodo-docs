...
您可以通过在参数 `scopes` 中指定这些实体的任意组合来同步它们。所有同步操作都是增量的，仅传输新的或更新的记录，以获得最佳性能。
...
要同步的数据实体的以逗号分隔的列表。可用的范围：`licences`，`payments`，`customers`，`subscriptions`。示例：`"payments,customers"`。
</ParamField>

<ParamField path="--api-key" type="string" required>
您的 Dodo Payments API 密钥。应以 `dp_live_` 开头以用于生产模式，或 `dp_test_` 用于测试模式。
</ParamField>
...
<Tip>
使用 `interval` 选项时，采用 `.start()` 进行自动同步是必需的。同步将在指定的间隔内持续运行，直到该过程被停止。
</Tip>
...
以下是如何在 PostgreSQL 中使用 `dodo-sync`：
...
以下是如何在 ClickHouse 中使用 `dodo-sync`：
...
要同步的实体数组。可用选项：`"licences"`，`"payments"`，`"customers"`，`"subscriptions"`。您可以包括这些的任意组合。
</ParamField>
...
- `environment`：必须是 `"test_mode"` 或 `"live_mode"`
</ParamField>

<ParamField body="interval" type="number">
自动同步之间的时间，以秒为单位。当使用 `.start()` 进行自动同步时是必需的。在使用 `.run()` 进行手动同步时是可选的。
</ParamField>
...
<Warning>
**MongoDB**：在您的数据库服务器上将自动创建一个名为 `dodopayments_sync` 的数据库。所有同步数据将存储在此处。此数据库名称目前是固定的，无法更改。

**PostgreSQL**：将在您连接 URI 中指定的数据库中创建表（`Subscriptions`，`Payments`，`Licenses`，`Customers`）。数据以 JSONB 格式存储。

**MySQL**：将在您连接 URI 中指定的数据库中创建表（`Subscriptions`，`Payments`，`Licenses`，`Customers`）。数据以 JSON 格式存储。

**ClickHouse**：将使用 ReplacingMergeTree 引擎创建表（`Subscriptions`，`Payments`，`Licenses`，`Customers`）。查询时，使用 `FINAL` 关键字以确保结果去重。
</Warning>
