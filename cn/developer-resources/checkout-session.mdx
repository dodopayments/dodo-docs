...
<Info>
支付方式必须属于客户并与支付货币兼容。这使得回头客可以一键购买。
</Info>
...
<Tip>
短链接非常适合 SMS、电子邮件或社交媒体分享。它们比长 URL 更容易记住，并能建立更多客户信任。
</Tip>

### 13. 立即重定向跳过支付成功页面

在付款完成后立即重定向客户，跳过默认的成功页面：
...
<Tip>
当您有一个提供比默认支付成功页面更好的用户体验的自定义成功页面时，请使用 `redirect_immediately: true`。这对于移动应用和嵌入式结账流程尤其有用。
</Tip>

<Note>
当 `redirect_immediately` 启用时，客户在付款完成后将立即重定向到您的 `return_url`，完全跳过默认的成功页面。
</Note>

## 预览结账会话

在创建结账会话之前，您可以预览定价明细，包括税费、折扣和总额。这对于在客户继续结账之前显示准确的定价非常有用。
