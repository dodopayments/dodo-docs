...
- **已提供 `payment_method_id`** — 该 charge 会立即处理，且 `checkout_url` 为 `null`。请改用返回的 `payment_id`。
- **`confirm: true` 在创建 session 时创建了 payment** — 响应还会包含 `payment_id`、`client_secret` 和 `publishable_key`，供 Dodo Payments checkout SDK 使用。
...
<Tip>
**Alternative Integration Options**：除了重定向之外，你还可以使用 [Overlay Checkout](/developer-resources/overlay-checkout)（模态覆盖层）或 [Inline Checkout](/developer-resources/inline-checkout)（完全嵌入）将 checkout 直接嵌入页面。在原生移动应用中，可将同一 URL 传递给适用于 Android、iOS、React Native 或 Flutter 的 [Mobile Checkout SDKs](/developer-resources/mobile-integration)。这些方式都使用相同的 checkout session URL。
</Tip>
</Step>

<Step title="Handle the return">
payment 完成后，客户会被重定向到你的 `return_url`，并附带查询参数，包括 payment/subscription ID、status、客户 email 以及任何 license keys。完整列表请参阅 [`return_url` parameter docs](#request-body)。
</Step>
</Steps>
...
Dodo Payments dashboard 中产品的唯一标识符。
...
**Example:** 单件商品使用 `1`，多件商品使用 `3`
</ParamField>

<ParamField body="addons" type="array">
要附加到产品的 addon 数组。仅当产品为 subscription 时有效。
...
启用 pay_what_you_want 时客户支付的金额。禁用时会忽略此字段。

**Format**：以货币的最小单位表示（例如 USD 的 cents）。例如，要收取 $1.00，请传入 `100`。
</ParamField>

<ParamField body="credit_entitlements" type="array">
针对已附加到此产品的 credit entitlements，按 checkout session 进行覆盖。使用此字段可仅为当前 session 授予不同数量的 credits——例如，授予 1,000 个 API credits 而非产品默认的 500 个促销套餐——无需创建单独的产品。

<Info>
每个 `credit_entitlement_id` 都必须已附加到该产品。此字段仅覆盖完成此 session 时授予的 `credits_amount`；产品级 credit entitlement 设置（过期、rollover 等）仍然适用。
</Info>
...
<Tip>
**Find Your Product IDs**：你可以在 Dodo Payments dashboard 的 Products → View Details 中找到产品 ID，也可以使用 [List Products API](/api-reference/products/get-products)。
</Tip>
...
客户的 email 地址。创建新客户时必填。
</ParamField>

<ParamField body="name" type="string">
客户在收据和发票上显示的全名。

**Example**: `"John Doe"` 或 `"Jane Smith"`
</ParamField>

<ParamField body="phone_number" type="string">
客户的国际格式电话号码。某些 payment methods 和 fraud prevention 场景要求提供此字段。

**Format**：包含国家/地区代码，例如美国号码使用 `"+1234567890"`
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
<Info>
当 `confirm` 设置为 `true` 时，所有 billing address 字段都必须填写，session 才能成功创建。
</Info>

<Expandable title="Billing address object properties">
<ParamField body="street" type="string">
完整街道地址，包括门牌号、街道名称，以及适用时的公寓/单元号。

**Example**: `"123 Main St, Apt 4B"` 或 `"456 Oak Avenue"`
</ParamField>
...
**Example**: `"California"` 或 `"CA"`、`"Ontario"` 或 `"ON"`
</ParamField>

<ParamField body="country" type="string" required>
两位字母的 ISO 国家/地区代码（ISO 3166-1 alpha-2）。提供 billing_address 时，此字段始终为必填项。

**Examples**: `"US"`（美国）、`"CA"`（加拿大）、`"GB"`（英国）、`"DE"`（德国）

<Card title="Country Code Reference" icon="globe" href="/api-reference/misc/supported-countries">
  查看支持的国家/地区及其 ISO 代码的完整列表
</Card>
</ParamField>

<ParamField body="zipcode" type="string">
根据国家/地区要求填写的邮政编码、ZIP code 或等效值。

**Examples**: `"94102"`（美国）、`"M5V 3A8"`（加拿大）、`"SW1A 1AA"`（英国）
</ParamField>
</Expandable>
</ParamField>
...
控制 checkout 期间客户可使用的 payment methods。此字段有助于针对特定市场或业务要求进行优化。

**常见选项**：`credit`、`debit`、`upi_collect`、`apple_pay`、`google_pay`、`amazon_pay`、`klarna`、`affirm`、`afterpay_clearpay`、`cashapp`、`ach`、`multibanco`、`bancontact_card`、`eps`、`ideal`、`blik`、`paypal`。这并非完整集合——有关每个可接受值，请参阅 [Create Checkout Session API reference](/api-reference/checkout-sessions/create)。

<Warning>
**Critical**：始终将 `credit` 和 `debit` 作为备用选项，以便在首选 payment methods 不可用时避免 checkout 失败。
</Warning>
...
使用固定的 billing currency 覆盖默认 currency 选择。使用 ISO 4217 currency codes。

**Supported Currencies**：`USD`、`EUR`、`GBP`、`CAD`、`AUD`、`INR` 等

**Example**：`"USD"` 表示美元，`"EUR"` 表示欧元
...
|-----------|------|-----------|
| `payment_id` | `string` | 一次性 payment 始终存在 |
| `subscription_id` | `string` | subscription payment 始终存在 |
| `status` | `string` | 始终存在 |
| `license_key` | `string` | 产品启用了 license keys 时存在；存在多个 key 时以逗号分隔 |
| `email` | `string` | 客户有已记录的 email 时存在 |
...
<Tip>
使用 `license_key` 和 `email` 查询参数，可以在返回页面上显示 license keys 或立即发送确认，而无需额外的 API 调用。
</Tip>
</ParamField>
...
将一个或多个堆叠的折扣代码应用于结账会话。代码按**数组顺序**应用（第一个代码会降低起始价格，第二个代码会降低应用第一个折扣后的价格，依此类推），每个会话最多可使用**20**个代码。启用 [Purchasing Power Parity](/features/purchasing-power-parity) 后，起始价格为经过 PPP 调整的金额，而不是基础价格。
...
<Info>
下面的单数 `discount_code` 字段已**弃用**，但仍完全受支持——现有 integrations 无需更改即可继续工作。它不能与同一 request 中的 `discount_codes` 组合使用。方便时请迁移到 `discount_codes`，以使用叠加功能。
</Info>
</ParamField>
...
**Example**：`"es"` 表示 Spanish，`"ja"` 表示 Japanese
</ParamField>

<ParamField body="theme_config" type="object">
自定义主题配置，包括浅色和深色模式的颜色、字体及按钮样式。这是已弃用的客户端 `themeConfig` 选项的服务器端替代方案。
...
主字体的 URL。必须是有效的 `https://` URL。
</ParamField>

<ParamField body="font_secondary_url" type="string">
辅助字体的 URL。必须是有效的 `https://` URL。
</ParamField>

<ParamField body="font_size" type="string">
checkout UI 的字体大小。选项：`xs`、`sm`、`md`、`lg`、`xl`、`2xl`。
</ParamField>

<ParamField body="font_weight" type="string">
checkout UI 的字体粗细。选项：`normal`、`medium`、`bold`、`extraBold`。
</ParamField>

<ParamField body="pay_button_text" type="string">
支付按钮的自定义文本（例如 `"Complete Purchase"` 或 `"Subscribe Now"`）。最多 100 个字符。
</ParamField>

<ParamField body="radius" type="string">
UI 元素的边框圆角。数字后跟 `px`、`rem` 或 `em`（例如 `"4px"`、`"0.5rem"`、`"1em"`）。
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
允许客户在 checkout 期间选择首选 currency。
</ParamField>

<ParamField body="allow_discount_code" type="boolean" default="true">
在 checkout 界面显示 discount code 输入字段。
</ParamField>

<ParamField body="allow_editing_addons" type="boolean" default="false">
允许客户在 checkout 期间为 subscription 产品添加或移除 addons。仅适用于 subscription 产品。
</ParamField>
...
要求客户提供电话号码才能完成 checkout。还要求 `allow_phone_number_collection` 为 true。
</ParamField>

<ParamField body="allow_tax_id" type="boolean" default="true">
允许客户输入 Tax identification numbers。
</ParamField>
...
允许客户在 checkout 期间编辑 zipcode。
</ParamField>

<ParamField body="allow_customer_editing_tax_id" type="boolean">
允许客户在 checkout 期间输入或编辑 Tax ID。
</ParamField>

<ParamField body="allow_customer_editing_business_name" type="boolean" default="false">
允许客户提供或编辑与 Tax ID 关联的 business name。独立于 `allow_customer_editing_tax_id` 工作——任一 flag（或 `allow_tax_id`）都足够。通常与 `allow_customer_editing_tax_id` 一起设置。
</ParamField>

<ParamField body="redirect_immediately" type="boolean" default="false">
跳过默认的 payment success page，并在 payment 完成后立即将客户重定向到你的 `return_url`。

<Tip>
当你拥有可提供更佳用户体验的自定义 success page，或用于[移动应用](/developer-resources/mobile-integration)和嵌入式 checkout 流程时，请使用此选项。
</Tip>
</ParamField>
</Expandable>
</ParamField>
...
此字段的唯一标识符。用作 webhook payload 和 API 响应中的 key。

**Example**: `"company_name"`、`"referral_source"`、`"team_size"`
</ParamField>

<ParamField body="label" type="string" required>
在 checkout 表单上向客户显示的标签。
...
`dropdown` 字段类型的选项列表。当 `field_type` 为 `dropdown` 时必填，对其他类型忽略。
...
<Info>
客户对自定义字段的响应包含在：
- **Webhooks**：`payment.succeeded`、`subscription.active` 及其他相关 event payload 包含 `custom_field_responses` 数组
...
首次 charge 前 trial period 的天数。必须介于 0 至 10000 天之间。

每个 session 不能设置付费 trial 金额——请改为在产品的 recurring price 上配置 `trial_amount`。请参阅 [Paid Trials](/features/subscription#paid-trials)。
</ParamField>

<ParamField body="on_demand" type="object">
基于用量或计量计费的 on-demand subscription 配置。

<Expandable title="On-demand object properties">
<ParamField body="mandate_only" type="boolean" required>
如果设置为 true，则不执行任何 charge，仅授权 payment method 详情以供将来使用。
</ParamField>

<ParamField body="product_price" type="integer">
向客户收取的初始 charge 的产品价格。如果未指定，则使用产品的存储价格。

**Format**：以货币的最小单位表示（例如 USD 的 cents）。例如，要收取 $1.00，请传入 `100`。
</ParamField>

<ParamField body="product_currency" type="string">
产品价格的可选 currency。如果未指定，则默认为产品的 currency。
</ParamField>

<ParamField body="product_description" type="string">
用于 billing 和 line items 的可选产品描述覆盖值。如果未指定，则使用产品的存储描述。
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
adaptive currency fees 是否包含在产品价格中（true），或在其上额外收取（false）。如果 adaptive pricing 未启用，则忽略此字段。
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
<Note>
当 `confirm` 设置为 `true` 时，客户会直接进入 checkout page，跳过所有确认步骤。
</Note>
...
<Tip>
当你拥有比默认 payment success page 提供更佳用户体验的自定义 success page 时，请使用 `redirect_immediately: true`。这对于[移动应用](/developer-resources/mobile-integration)和嵌入式 checkout 流程尤其有用。
</Tip>
...
<Info>
自定义字段响应会自动包含在 webhook payload（`payment.succeeded`、`subscription.active` 等）中，也可以通过 API 获取。你可以使用这些响应丰富 CRM、触发 onboarding 流程或自定义客户体验。
</Info>
...
<Note>
预览的 `current_breakup.subtotal` 已包含适用于该产品的 [Purchasing Power Parity](/features/purchasing-power-parity) 和 [Charm Pricing](/features/charm-pricing)。
</Note>

<Note>
当购物车中包含订阅产品时，预览响应还会返回一个 `next_billing_date`，即对即将到来的计费日期的预览，以便你在创建订阅前将其展示给用户。该日期是相对于当前时间计算的：试用适用时为 `now + trial period`，否则为 `now + one payment frequency`。仅包含一次性商品的购物车不会返回此字段。这是基于预览时间的估算值；权威的 `next_billing_date` 会在订阅激活时设置。
</Note>

<Note>
预览还会返回 `trial_period_days`（实际试用时长，可以是免费或付费）和 `trial_amount`（折扣后的每单位试用费用，以价格货币的最小单位表示）。`trial_amount` 仅在[付费试用](/features/subscription#paid-trials)时存在；免费试用或无试用时，其值为 `null`。使用 `current_breakup` 获取今天实际应付的含税总额。
</Note>
...
### 主要差异
以前，使用 Dynamic Links 创建支付链接时，必须提供客户完整的账单地址。

使用 Checkout Sessions 后，不再需要这样做。你只需传递已有的信息，剩余部分由我们处理。例如：

- 如果你只知道客户的账单国家/地区，只需提供该信息。
- 结账流程会在将客户转到支付页面之前，自动收集缺失的详细信息。
- 另一方面，如果你已经拥有所有必需的信息，并希望直接进入支付页面，则可以传递完整的数据集，并在请求正文中包含 `confirm=true`。
