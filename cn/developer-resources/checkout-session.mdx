...
跳过默认支付成功页面，在支付完成后立即将客户重定向到您的 `return_url`。

<Tip>
当您有一个自定义成功页面提供更好的用户体验时，或对于移动应用程序和嵌入式结算流程，使用此方法。
</Tip>
</ParamField>
</Expandable>
</ParamField>
...
在结账过程中，通过自定义表单字段收集客户的额外信息。每个结账会话最多可以定义 5 个自定义字段。客户的回答包含在 webhook 负载中，并可通过 API 获取。
...
<Info>
自定义字段的客户响应包含在：
- **Webhook**: `payment.succeeded`，`subscription.active` 和其他相关事件负载中包含 `custom_field_responses` 数组
...
首次收费之前的试用期天数。必须在 0 到 10000 天之间。
</ParamField>
...
如果设置为 true，则不会执行任何收费，只会授权支付方式详细信息供将来使用。
</ParamField>

<ParamField body="product_price" type="integer">
客户初次付费的产品价格。如果未指定，将使用产品存储的价格。

**格式**: 以货币的最低面额表示（例如，USD 的分）。例如，收费 $1.00，传递 `100`。
</ParamField>

<ParamField body="product_currency" type="string">
产品价格的可选货币。如果未指定，默认为产品货币。
</ParamField>

<ParamField body="product_description" type="string">
用于账单和行项目的可选产品描述覆写。如果未指定，将使用产品存储的描述。
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
是否应将自适应货币费用包含在产品价格内（true）或额外添加（false）。如果未启用自适应定价，则忽略。
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
以下是展示各种业务场景的不同结账会话配置的 10 个综合示例：
...
<Note>
`billing_currency` 覆盖仅在您的帐户设置中启用了自适应货币时生效。如果禁用了自适应货币，此参数将无效。
</Note>
...
使用客户保存的支付方式创建结账会话，立即处理，跳过支付方式收集：
...
<Warning>
使用 `payment_method_id` 时，必须将 `confirm` 设置为 `true`，并提供现有的 `customer_id`。支付方式将根据付款的货币进行合格性验证。
</Warning>

<Info>
支付方式必须属于客户，并与支付货币兼容。这使得回头客可以进行一键购买。
</Info>

### 12. 更简洁付款 URL 的短链接

使用自定义 slug 生成可分享的短链接：
...
<Tip>
短链接非常适合用于短信、电子邮件或社交媒体分享。它们比长 URL 更容易记住并能建立更多客户信任。
</Tip>
...
<Tip>
当您有一个自定义成功页面提供比默认支付成功页面更好的用户体验时，使用 `redirect_immediately: true`。这在移动应用程序和嵌入式结算流程中尤其有用。
</Tip>

<Note>
启用 `redirect_immediately` 时，客户在支付完成后立即重定向至您的 `return_url`，完全跳过默认成功页面。
</Note>
...
<Tip>
当您知道客户的首选语言（例如，从其帐户设置中）或在针对特定区域市场时，使用 `force_language`。
</Tip>
...
<Info>
自定义字段响应会自动包含在 webhook 负载中（`payment.succeeded`，`subscription.active` 等）并可通过 API 检索。使用它们来丰富您的 CRM、触发入门流程或定制客户体验。
</Info>
...
在创建结账会话之前，您可以预览包括税金、折扣和总额在内的价格明细。这样可以在客户进行结账之前向他们显示准确的价格。
...
### 关键差异
以前，在使用动态链接创建支付链接时，您需要提供客户完整的账单地址。

使用结账会话，这不再必要。您可以简单地传递您拥有的信息，其余的将由我们处理。例如：

- 如果您只知道客户的账单国家/地区，只需提供即可。
- 结账流程会自动收集缺失的详细信息，然后将客户转到支付页面。
- 另一方面，如果您已经有所有需要的信息并想直接跳到支付页面，可以传递完整的数据集并在请求主体中包含 `confirm=true`。
