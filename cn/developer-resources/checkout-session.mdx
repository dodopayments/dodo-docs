...
<Info>
客户对自定义字段的响应包括在：
- **Webhooks**：`payment.succeeded`、`subscription.active` 以及其他相关事件负载中包含 `custom_field_responses` 数组
- **API 响应**：付款和订阅对象包含 `custom_field_responses`
</Info>
...
<Info>
自定义字段的响应会自动包含在 webhook 负载（`payment.succeeded`、`subscription.active` 等）中，并且可以通过 API 检索。使用它们来丰富您的 CRM、触发入职流程或自定义客户体验。
</Info>
