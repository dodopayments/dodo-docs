...
结账会话提供更安全的托管结账体验，可以完全自定义控制一次性购买和订阅的完整支付流程。

请参考 [Checkout Sessions Integration Guide](https://docs.dodopayments.com/developer-resources/checkout-session) 以获取更多详细信息和支持字段的完整列表。
...
客户门户路由处理器让您可以无缝地将 Dodo Payments 客户门户集成到 Nuxt 应用中。
...
- **有效负载验证：** 使用 Zod 验证。如果负载无效，返回 400。
- **错误处理：**
  - 401：签名无效
  - 400：有效负载无效
  - 500：验证期间的内部错误
- **事件路由：** 根据负载类型调用相应的事件处理器。
