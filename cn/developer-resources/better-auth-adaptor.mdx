...
<Info>
  与传统的<code>checkout</code>方法不同，<code>checkoutSession</code>不需要预先获取账单信息，因为这些信息会在结账页面由用户提供。你仍然可以通过传递<code>billing</code>键来覆盖它。
</Info>

<Info>
  类似于传统的<code>checkout</code>方法，<code>checkoutSession</code>会从用户的better-auth会话中获取客户的电子邮件和姓名，不过，你可以通过传递包含电子邮件和姓名的<code>customer</code>对象来覆盖它。
</Info>

<Info>
  你可以在参数中传递与[创建结账会话](https://docs.dodopayments.com/api-reference/checkout-sessions/create)端点的请求体相同的选项。
</Info>

<Note>
  返回URL来自服务器插件中配置的<code>successUrl</code>。你无需在客户端负载中包含<code>return_url</code>。
</Note>

## 传统结账（已弃用）

<Warning>
  <code>authClient.dodopayments.checkout</code>方法已弃用。请在新的实现中使用<code>checkoutSession</code>。
</Warning>
...
在服务器上启用<code>usage()</code>插件以捕获计量事件，并让客户查看他们的使用量计费。

- <code>authClient.dodopayments.usage.ingest</code>为已登录且验证过电子邮件的用户摄取事件。
- <code>authClient.dodopayments.usage.meters.list</code>列出与客户订阅相关的计量器的最近使用情况。
...
<Tip>
  摄取使用情况时，早于1小时或超过5分钟的未来时间戳将被拒绝。
</Tip>
<Tip>
  如果在列出使用情况计量器时省略<code>meter_id</code>，将返回与客户订阅相关的所有计量器。
</Tip>

# Webhooks

<Info>
  webhooks插件通过安全签名验证处理来自Dodo Payments的实时支付事件。默认端点为<code>/api/auth/dodopayments/webhooks</code>。
</Info>

<Steps>
<Step title="Generate and set webhook secret">
在Dodo Payments仪表板中为你的端点URL（例如，<code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>）生成一个webhook密钥，并将其设置在你的<code>.env</code>文件中。
...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b>（必需）：DodoPayments客户端实例
    - <b>createCustomerOnSignUp</b>（可选）：在用户注册时自动创建客户
    - <b>use</b>（必需）：启用的插件数组（checkout, portal, usage, webhooks）
    - <b>getCustomerParams</b>（可选）：接收BetterAuth `User`并返回要附加到DodoPayments客户上的额外字段的函数（例如，`metadata`, `phone_number`）
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b>：产品数组或返回产品的异步函数 - <b>successUrl</b>：成功付款后重定向的URL - <b>authenticatedUsersOnly</b>：要求用户认证（默认：false）
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>无效的API密钥</b>：仔细检查你的<code>DODO_PAYMENTS_API_KEY</code>在<code>.env</code>中。 - <b>Webhook签名不匹配</b>：确保webhook密钥与Dodo Payments仪表板中设置的密钥匹配。 - <b>客户未创建</b>：确认<code>createCustomerOnSignUp</code>设置为<code>true</code>。
  </Accordion>
  <Accordion title="Best Practices">
    - 对所有密钥和密钥使用环境变量。 - 在切换到<code>live_mode</code>前在<code>test_mode</code>中测试。 - 记录webhook事件以进行调试和审核。
  </Accordion>
</AccordionGroup>
