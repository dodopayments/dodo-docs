...
<AccordionGroup>
  <Accordion title="Plugin Options">
    ...
    - <b>createCustomerOnSignUp</b> (optional): 用户注册时自动创建客户
    - <b>use</b> (required): 要启用的插件数组（checkout, portal, usage, webhooks）
    - <b>getCustomerParams</b> (optional): 接收 BetterAuth `User` 并返回附加到 DodoPayments 客户上的额外字段的函数（例如 `metadata`, `phone_number`）
  ...
  <Accordion title="Checkout Plugin Options">
    ...
    <b>successUrl</b>: 成功支付后的重定向 URL -{" "}
    <b>authenticatedUsersOnly</b>: 仅要求用户身份验证（默认: false）
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>无效的 API 密钥</b>: 请仔细检查你的
    <code>DODO_PAYMENTS_API_KEY</code> 在 <code>.env</code> 中。 -{" "}
    <b>Webhook 签名不匹配</b>: 确保 webhook 密钥与在 Dodo Payments 仪表板中设置的一致。 - <b>未创建客户</b>: 确认
    <code>createCustomerOnSignUp</code> 设置为 <code>true</code>。
  </Accordion>
  <Accordion title="Best Practices">
    - 使用环境变量存储所有密钥和密钥。 - 在
    <code>test_mode</code> 中测试然后再切换到 <code>live_mode</code>。 - 记录
    webhook 事件以进行调试和审计。
  </Accordion>
</AccordionGroup>
