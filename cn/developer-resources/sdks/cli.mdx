...
`dodo wh trigger` 可为以下 22 种事件类型发送模拟负载：
...
<Note>
这是 Dodo Payments 交付的部分事件。`payout.*`、`credit.*`、`abandoned_checkout.*`、`dunning.*`、`entitlement_grant.*`、`subscription.paused` 和 `subscription.update_payment_method` 尚无模拟生成器——请通过真实的测试模式活动来测试这些事件，并使用 `dodo wh listen`。完整列表请参阅 [Webhook Events](/developer-resources/webhooks/intents/webhook-events-guide)。
</Note>
...
CLI 会在启动时检查是否有新版本，并在有可用更新时于状态栏显示通知。升级方法如下：
...
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-cli/issues)中提交 issue
- **Email**：发送邮件至 support@dodopayments.com 联系我们
