---
title: Dodo CLI
description: The official command-line interface for Dodo Payments — manage
  resources, run AI-powered queries, create checkout sessions, and test webhooks
  from your terminal.
icon: terminal
tag: NEW
keywords:
...
`dodo wh trigger` 可以为 Dodo Payments 交付的全部 46 种事件类型发送 mock payload：

| 类别 | 事件 |
|----------|--------|
| **订阅** | `active`, `updated`, `on_hold`, `paused`, `renewed`, `plan_changed`, `cancelled`, `failed`, `expired`, `update_payment_method` |
...
| **放弃的结账** | `detected`, `recovered` |
| **催收** | `started`, `recovered` |
| **权益授予** | `created`, `delivered`, `failed`, `revoked` |

<Note>
mock payload 的结构遵循 API reference 中对应的 schema。有关每个事件的含义，以及 Dodo Payments 在生产环境中何时发出该事件，请参阅 [Webhook Events](/developer-resources/webhooks/intents/webhook-events-guide)。
</Note>

<Tip>
当 payout 仍报告 `not_initiated` 状态时，会发出 `payout.created`，因此 mock payload 也会反映这一点。有关完整的 payout lifecycle，请参阅 [Payout Events](/developer-resources/webhooks/intents/payout)。
</Tip>
...
|----------|-------------|
| `DODO_WH_TEST_SERVER_URL` | 覆盖 `dodo wh listen` 使用的默认 webhook relay server URL |

## 更新

CLI 启动时会检查是否有新版本；如果有可用更新，则会在 status bar 中显示通知。升级方法如下：

```text
/update
```

或者，重新运行 installer 以就地升级：
...
- **GitHub**：在[仓库](https://github.com/dodopayments/dodopayments-cli/issues)中提交 issue
