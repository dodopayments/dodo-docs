...
`dodo wh trigger` 可以为 Dodo Payments 提供的全部 47 种事件类型发送模拟 payload：

| 类别 | 事件 |
|----------|--------|
| **订阅** | `active`, `updated`, `on_hold`, `paused`, `unpaused`, `renewed`, `plan_changed`, `cancelled`, `failed`, `expired`, `update_payment_method` |
| **支付** | `succeeded`, `failed`, `processing`, `cancelled` |
| **退款** | `succeeded`, `failed` |
| **争议** | `opened`, `expired`, `accepted`, `cancelled`, `challenged`, `won`, `lost` |
| **许可证密钥** | `created` |
| **付款** | `created`, `in_progress`, `on_hold`, `failed`, `success` |
| **额度** | `added`, `deducted`, `expired`, `rolled_over`, `rollover_forfeited`, `overage_charged`, `overage_reset`, `manual_adjustment`, `balance_low` |
| **已放弃的结账** | `detected`, `recovered` |
