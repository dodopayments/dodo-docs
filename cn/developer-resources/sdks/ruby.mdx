...
<Info>
`billing` 至少需要两位字母的 ISO `country` 代码。`customer` 可以接受 `{ customer_id: "..." }` 以关联现有客户，也可以接受 `{ email: "...", name: "..." }` 以创建新客户。`product_price` 使用最小货币单位表示。
</Info>
...
<Tip>
针对速率限制错误，使用指数退避实现重试逻辑，确保应用能够平稳处理高流量场景。
</Tip>
...
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-ruby)中提交 issue

## 贡献

欢迎贡献！查看[贡献指南](https://github.com/dodopayments/dodopayments-ruby/blob/main/CONTRIBUTING.md)以开始参与。
