...
<Warning>
`POST /subscriptions`（SDK 的 `subscriptions.create` 方法）已弃用。对于现有集成，它仍然有效，但新集成应通过 [Checkout Session](/developer-resources/checkout-session) 创建订阅。
</Warning>
...
<Info>
`Billing` 至少需要两位字母的 ISO `Country` 代码。`Customer` 是一个 `CustomerRequestUnionParam` — 对于现有客户，请传入 `AttachExistingCustomerParam{CustomerID: ...}`；对于新客户，请传入 `NewCustomerParam{Email: ..., Name: ...}`。`ProductPrice` 使用货币的最小面额。
</Info>
...
当 API 返回非成功状态码时，SDK 会返回 `*dodopayments.Error` 类型的错误。它会公开 `StatusCode`、底层的 `*http.Request` 和
`*http.Response`，以及错误正文的 JSON。使用 `errors.As` 模式检查错误，并根据 `StatusCode` 分支处理特定情况：
...
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-go)中提交 issue

## 贡献

欢迎贡献！查看[贡献指南](https://github.com/dodopayments/dodopayments-go/blob/main/CONTRIBUTING.md)以开始贡献。
