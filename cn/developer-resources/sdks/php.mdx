...
<Info>
`billing` 至少需要两位字母的 ISO `country` 代码。传入 `AttachExistingCustomer::with(customerID: '...')` 以关联现有客户，或传入 `NewCustomer::with(email: '...', name: '...')` 以创建客户。`productPrice` 采用货币的最小面额。
</Info>
...
当库无法连接到 API，或收到非成功状态代码（4xx 或 5xx）时，会抛出 `APIException` 的子类：
...
<Tip>
  始终将 API 调用放在 try-catch 块中，以处理潜在错误，
  并向用户提供有意义的反馈。
</Tip>
...
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-php)中提交 issue

## 贡献

欢迎贡献！查看[贡献指南](https://github.com/dodopayments/dodopayments-php/blob/main/CONTRIBUTING.md)以开始贡献。
