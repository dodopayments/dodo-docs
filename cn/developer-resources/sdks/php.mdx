...
<Note>
  Dodo Payments PHP API 库目前处于 **测试版**。我们非常期待您来尝试！如有任何建议、错误报告或功能请求，请通过 [提交 issue](https://github.com/dodopayments/dodopayments-php/issues) 。
</Note>
...
<Info>
  该 SDK 需要 PHP 8.1.0 或更高版本，以及用于依赖管理的 Composer。
</Info>
...
<Warning>
  请使用环境变量安全存储 API 密钥。切勿在代码库中暴露它们或将其提交至版本控制。
</Warning>
...
当库无法连接 API 或收到非成功状态码（4xx 或 5xx）时，会抛出 `APIException` 的子类：
...
<Tip>
  始终将 API 调用包裹在 try-catch 块中，以优雅地处理潜在错误并向用户提供有意义的反馈。
</Tip>
