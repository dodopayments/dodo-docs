...
<Info>
`billing` 至少需要两位字母的 ISO `country` 代码。使用 `AttachExistingCustomer` 关联现有客户，或使用 `NewCustomer` 创建客户。`productPrice` 以货币的最小面额表示。
</Info>
...
<Tip>
使用 Kotlin 的 `runCatching`，结合 Result 类型以采用更函数式的错误处理方式。
</Tip>
...
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-kotlin)中提交 issue

## 贡献

我们欢迎贡献！请查看[贡献指南](https://github.com/dodopayments/dodopayments-kotlin/blob/main/CONTRIBUTING.md)以开始参与。
