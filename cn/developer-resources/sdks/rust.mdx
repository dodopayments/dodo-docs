...
<Warning>
`billing` 至少需要两位字母的 ISO `country` 代码。`customer` 是一个 `CustomerRequest` 枚举——对于现有客户传入 `AttachExisting`，对于新客户传入 `New`。金额字段（例如 `product_price`）使用货币的最小单位表示（例如，`2500` = 25.00 美元）。
</Warning>
...
列表端点返回一个类型化页面，其 `items` 字段包含当前页的结果。使用 `into_stream` 遍历所有页面中的每一项：
...
每个方法都会返回一个 `dodopayments::Result<T>`。失败由 `dodopayments::Error` 枚举表示。根据它进行匹配，即可将 API 错误与传输错误分别处理：
...
要调用尚未作为类型化方法公开的端点，请使用底层的 `request` 构建器；它会应用身份验证并设置基础 URL：
...
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-rust/issues)中提交 issue

## 贡献

欢迎贡献！请查看[贡献指南](https://github.com/dodopayments/dodopayments-rust/blob/main/CONTRIBUTING.md)以开始参与。
