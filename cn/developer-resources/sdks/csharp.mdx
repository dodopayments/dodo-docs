...
SDK 会根据 HTTP status code 抛出特定异常。所有 4xx 错误均继承自 `DodoPayments4xxException`。
...
- `DodoPaymentsInvalidDataException`：无法解释已解析的数据
...
使用 `Paginate` 方法遍历所有页面中的全部结果，该方法返回一个 `IAsyncEnumerable`：
...
<Tip>
在开发环境中，请使用 [用户机密](https://learn.microsoft.com/en-us/aspnet/core/security/app-secrets)，而不是将密钥存储在 `appsettings.json` 中：
...
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-csharp)中提交 issue
