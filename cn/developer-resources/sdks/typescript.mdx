...
<Warning>
`POST /subscriptions`（SDK 的 `subscriptions.create` 方法）已弃用。对于现有集成仍可正常工作，但新集成应通过 [Checkout Session](/developer-resources/checkout-session) 创建订阅。
</Warning>
...
<Info>
`billing` 至少需要两位字母的 ISO 国家代码。`customer` 是 `{ customer_id }`（用于关联现有客户）或 `{ email, name? }`（用于创建新客户）的联合类型。`product_price` 以货币的最小面额表示。
</Info>
...
<Info>
事件必须具有唯一的 `event_id` 值，以确保幂等性。同一请求中的重复 ID 将被拒绝，后续请求中使用已有 ID 的事件将被忽略。
</Info>
...
<Warning>
在调试级别下，所有 HTTP 请求和响应都会被记录，包括标头和请求体。部分身份验证标头会被编辑隐藏，但请求体中的敏感数据仍可能被查看。
</Warning>

## 从 Node.js SDK 迁移

如果你要从旧版 Node.js SDK 升级，TypeScript SDK 提供了更完善的类型安全性和更多功能：
...
DodoPayments API 中的列表方法采用分页机制。你可以使用 `for await … of` 语法遍历所有页面中的项目：
...
- Web 浏览器（最新版本的 Chrome、Firefox、Safari、Edge 等）
- Node.js 20 LTS 或更高版本（[非 EOL](https://endoflife.date/nodejs) 版本）
- Deno v1.28.0 或更高版本
- Bun 1.0 或更高版本
- Cloudflare Workers
- Vercel Edge Runtime
- Jest 28 或更高版本，并使用 `"node"` 环境
- Nitro v2.6 或更高版本
...
需要 TypeScript SDK 方面的帮助？

- **Discord**：加入我们的[社区服务器](https://discord.gg/bYqAp4ayYh)获取实时支持
- **Email**：通过 support@dodopayments.com 联系我们  
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-typescript)中提交问题

## 贡献

欢迎贡献！查看[贡献指南](https://github.com/dodopayments/dodopayments-typescript/blob/main/CONTRIBUTING.md)开始参与。
