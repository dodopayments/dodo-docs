...
<Warning>
`POST /subscriptions`（SDK 的 `subscriptions.create` 方法）已被弃用。对于现有集成，它仍然有效，但新集成应通过 [Checkout Session](/developer-resources/checkout-session) 创建订阅。
</Warning>
...
<Info>
`productPrice` 以货币的最小面额表示（例如，USD 的分、INR 的派萨）。要收取 $25.00，请传入 `2500`。
</Info>

<Tip>
通过 `subscriptions().charge(...)` 收费适用于[按需订阅](/developer-resources/ondemand-subscriptions)。标准的定时订阅会根据产品的定价计划自动计费。
</Tip>
...
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-java)中提交 issue

## 贡献

我们欢迎贡献！请查看[贡献指南](https://github.com/dodopayments/dodopayments-java/blob/main/CONTRIBUTING.md)开始参与。
