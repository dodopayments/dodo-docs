...
通过 `customization` 在 `start(...)` 上自定义工作表的关闭按钮、呈现样式和配色方案。所有字段均为可选；省略 `customization` 将使用 iOS 的默认 `SFSafariViewController` 外观。

<ParamField body="dismissButtonStyle" type="DismissButtonStyle">
关闭按钮的标签或图标：`done`、`close` 或 `cancel`。
</ParamField>
...
允许工具栏在滚动时折叠。仅当 `presentationStyle` 为 `fullScreen` 时可见 — 无论此设置如何，`pageSheet` 都会保持栏固定。
</ParamField>
...
`start` 仅在误用或平台故障时抛出 `CheckoutError`。已取消或被拒绝的支付始终是结果，而不是异常。

- `invalidCheckoutUrl`（`INVALID_CHECKOUT_URL`）：不是有效的 `checkout.dodopayments.com` 会话 URL。
- `invalidReturnUrl`（`INVALID_RETURN_URL`）：不是有效的绝对 URL。
- `alreadyInProgress`（`ALREADY_IN_PROGRESS`）：结账流程已在运行。
- `platformError`（`PLATFORM_ERROR`）：意外的平台故障。

## 已放弃的会话

<Info>
如果应用在结账过程中被终止，请在下次启动时恢复会话，并将其与后端进行对账。
</Info>
