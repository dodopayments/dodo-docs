...
<Warning>
`POST /subscriptions`（SDK 的 `subscriptions.create` 方法）已弃用。对于现有集成，它仍然可以正常工作，但新集成应通过 [Checkout Session](/developer-resources/checkout-session) 创建订阅。
</Warning>
...
<Info>
`billing` 至少需要两位 ISO 国家/地区代码。`customer` 接受 `{"customer_id": ...}` 以关联现有客户，或接受 `{"email": ..., "name": ...}` 以创建新客户。`product_price` 使用货币的最小面额表示。
</Info>
...
- **GitHub**：在[代码仓库](https://github.com/dodopayments/dodopayments-python)中提交 issue

## 贡献

我们欢迎贡献！查看[贡献指南](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md)以开始参与。
