...
| `subscription.on_hold` | 由于续订失败，订阅已暂停 |
| `subscription.paused` | 订阅已暂停 |
| `subscription.renewed` | 订阅已续订至下一个计费周期 |
| `subscription.plan_changed` | 订阅计划已升级、降级或修改 |
| `subscription.update_payment_method` | 订阅的付款方式已更新 |
| `subscription.cancelled` | 订阅已取消 |
| `subscription.failed` | 创建 mandate 期间订阅创建失败 |
| `subscription.expired` | 订阅已达到其期限 |
