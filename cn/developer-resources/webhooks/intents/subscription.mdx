...
| 事件 | 描述 |
|-------|-------------|
| `subscription.active` | 订阅已成功激活 |
| `subscription.updated` | 订阅对象已更新（任何字段变更都会触发此事件） |
| `subscription.on_hold` | 由于续期失败，订阅已被置于暂停状态 |
| `subscription.paused` | 订阅已暂停 |
| `subscription.unpaused` | 订阅暂停后已恢复 |
| `subscription.renewed` | 订阅已续期至下一个计费周期 |
| `subscription.plan_changed` | 订阅计划已升级、降级或修改 |
| `subscription.update_payment_method` | 订阅的支付方式已更新 |
| `subscription.cancelled` | 订阅已取消 |
| `subscription.failed` | 创建授权委托期间订阅创建失败 |
| `subscription.expired` | 订阅已达到其期限末尾 |
