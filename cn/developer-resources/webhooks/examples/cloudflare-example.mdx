...
<Card title="GitHub 仓库" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/cloudflare">
  完整的源代码和设置指南
</Card>
...
1. 转到 [DodoPayments Dashboard](https://app.dodopayments.com) → 开发者 → Webhooks
2. 创建一个新的 webhook 端点
...
✅ **幂等性** - 使用 webhook ID 防止重复处理  
✅ **事件日志** - 在 `webhook_events` 表中保留完整审计记录  
✅ **错误处理** - 记录并可重试

> **注意：** 本实现演示了处理三个核心订阅事件 (`subscription.active`、`subscription.cancelled`、`subscription.renewed`)，并且字段最少。您可以根据需要轻松扩展以支持其他事件类型和字段。

## 配置文件

<CodeGroup>

代码占位符   ```json package.json
  {
...
代码占位符   ```toml wrangler.toml expandable
...
代码占位符   ```sql schema.sql expandable
...
代码占位符 ```typescript worker.ts expandable
...
| 问题 | 解决方案 |
|-------|----------|
| 验证失败 | 检查 DodoPayments 仪表板上的 webhook 密钥是否正确 |
| 数据库连接错误 | 验证 Neon 连接字符串 |
| 工作未部署 | 运行 `npm install`，检查 `wrangler.toml` 语法 |
