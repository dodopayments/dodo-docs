...
1. 访问 [DodoPayments Dashboard](https://app.dodopayments.com) → 开发者 → Webhooks
2. 创建一个新的 webhook 端点
...
✅ **事件日志记录** - 在 `webhook_events` 表中完成审计跟踪  
✅ **错误处理** - 记录并可重试

> **注意：** 此实现演示了处理三个核心订阅事件 (`subscription.active`, `subscription.cancelled`, `subscription.renewed`)，并包含最小字段。你可以根据需求轻松扩展以支持其他事件类型和字段。
...
2. **检查重复** - 使用 webhook ID 防止同一事件被处理两次
