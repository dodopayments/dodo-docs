...
<Card title="GitHub 仓库" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/vercel">
  完整的源代码和设置指南
</Card>
...
> **注意：** 在部署后，我们将在您获得 webhook URL 后设置 `DODO_PAYMENTS_WEBHOOK_KEY`。
...
✅ **幂等性** - 防止使用 webhook ID 处理重复事件  
✅ **事件日志记录** - 在 `webhook_events` 表中完整审计跟踪  
✅ **错误处理** - 已记录并可重试

> **注意：** 此实现展示了处理三个核心订阅事件 (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) 的方法，字段极少。您可以轻松扩展它以支持根据需要添加更多事件类型和字段。
...
Webhook 处理程序：
1. **禁用主体解析** - 以访问原始主体进行签名验证
2. **验证签名** - 使用 HMAC-SHA256 确保请求来自 DodoPayments
3. **检查重复** - 使用 webhook ID 防止处理相同事件两次
4. **记录事件** - 将原始 webhook 存储在 `webhook_events` 表中以进行审计跟踪
5. **处理事件** - 在 Neon 中创建或更新客户和订阅
6. **处理错误** - 记录失败并将事件标记为未处理以进行重试。
