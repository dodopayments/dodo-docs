...
1. 前往您的 [Supabase Dashboard](https://supabase.com/dashboard)
2. 打开 SQL 编辑器
3. 创建一个新查询
...
> **注意：** 部署后，我们将在您拥有 webhook URL 后设置 `DODO_PAYMENTS_WEBHOOK_KEY`。
...
<Note>
由于 webhook 不包含 JWT 令牌，因此需要 `--no-verify-jwt` 标志。安全性通过 webhook 签名验证提供。
</Note>
