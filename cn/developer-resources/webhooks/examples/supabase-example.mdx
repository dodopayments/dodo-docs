...
<Card title="GitHub 仓库" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/supabase">
  完整的源代码和设置指南
</Card>

<Tip>
  <strong>查看 Supabase 的完整启动工具包</strong><br />
  一个使用 Next.js、Supabase 和 Dodo Payments 构建的最小订阅启动工具包。这个模板帮助您快速设置一个基于订阅的 SaaS，具有身份验证、支付和 Webhook。
  <a href="https://github.com/dodopayments/dodo-supabase-subscription-starter" target="_blank" rel="noopener noreferrer" >
    Dodo Payments Supabase 订阅启动工具包
  </a>
</Tip>
...
1. 前往你的 [Supabase 控制台](https://supabase.com/dashboard)
2. 打开 SQL 编辑器
3. 创建一个新的查询
...
Supabase 在运行时自动提供 `SUPABASE_URL` 和 `SUPABASE_SERVICE_ROLE_KEY`。
...
该函数已在 `functions/webhook/index.ts` 中设置 - 只需部署它：
...
1. 前往 [DodoPayments 控制台](https://app.dodopayments.com) → 开发者 → Webhooks
2. 创建一个新的 webhook 端点
...
✅ **幂等性** - 防止重复处理 webhook ID  
✅ **事件日志** - 在 `webhook_events` 表中完整审计跟踪  
✅ **错误处理** - 记录并可重试

> **注意：** 此实现展示了处理三个核心订阅事件（`subscription.active`, `subscription.cancelled`, `subscription.renewed`）的基本字段。你可以根据需求轻松扩展以支持其他事件类型和字段。
...
3. **记录事件** - 在 `webhook_events` 表中存储原始 webhook 数据
...
<Note>
由于 webhook 不包括 JWT 令牌，因此需要 `--no-verify-jwt` 标志。安全性由 webhook 签名验证提供。
</Note>
...
| 问题 | 解决方案 |
|-------|----------|
| 验证失败 | 检查 DodoPayments 控制台中的 webhook 密钥是否正确 |
| 数据库权限错误 | 确保使用服务角色密钥 |
| JWT 验证错误 | 部署时加上 `--no-verify-jwt` 标志 |
| 函数未找到 | 验证项目引用是否正确且函数已部署 |
