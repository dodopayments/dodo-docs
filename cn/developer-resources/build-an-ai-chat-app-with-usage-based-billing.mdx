...
<Warning>
事件名称 `ai_chat_usage` 必须与我们稍后从应用代码发送的名称完全匹配。事件名称是区分大小写的！
</Warning>
</Step>
...
<Info>
我们使用“总和”是因为我们想要在多个聊天消息中累加所有消耗的令牌。SDK 会在每个事件中自动发送 `totalTokens`。
</Info>
</Step>
...
1. 在你的 Dodo Payments 仪表盘中，转到 **开发者** → **API 密钥**
2. 点击 **创建 API 密钥**
3. 复制 API 密钥 - 它看起来像 `test_abc123...`

<Note>
保存这个 API 密钥 - 我们稍后会将其添加到我们的 `.env` 文件中。
</Note>
</Step>
...
此外，将 **固定价格** 设置为 `0`，以确保客户仅根据其使用量收费，没有基础费用。
...
**免费额度**: 输入 → `10000` （客户每月获得 10,000 个免费令牌）
...
<Step title="找出你的客户 ID">
1. 返回到你的 Dodo Payments 仪表盘
2. 在左侧边栏中导航到 **销售** -> **客户**
3. 找到你刚刚创建的客户（使用测试邮箱）
...
<Step title="创建后端服务器">
创建 `src/server.ts` 并复制这个完整的服务器代码：
...
现在让我们添加一个美观的聊天界面和完整的对话历史！创建 `public/index.html`：
...
1. 验证你的 `.env` 文件包含步骤 2 的所有 API 密钥
2. 启动开发服务器：
...
<Note>
确保你在 `server.ts` 中更新 `CUSTOMER_ID`，以使用步骤 4 中的实际测试客户 ID。
</Note>
</Step>
...
2. 检查客户 ID 是否存在于 Dodo Payments 仪表盘中
3. 用简单的 API 调用测试 API 密钥
4. 检查服务器日志以获取错误消息
</Accordion>
