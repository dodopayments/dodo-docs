...
在 Convex 配置中添加 Dodo Payments 组件：
...
编辑 `convex.config.ts` 后，运行 `npx convex dev` 一次以生成必要的类型。
</Step>
{/* LOCKED_PATTERN_85e536148c3d4a355f42242d1584f3dc */}
在 Convex 仪表板中设置环境变量（**Settings** → **Environment Variables**）。您可以通过运行以下命令访问仪表板：
...
- `DODO_PAYMENTS_WEBHOOK_SECRET` - 您的 Webhook 密钥（处理 Webhook 时必需）

<Warning>
始终为敏感信息使用 Convex 环境变量。切勿将密钥提交到版本控制。
</Warning>
</Step>
</Steps>
...
首先，创建一个内部查询以从数据库获取客户。这将在支付函数中用于识别客户。

<Warning>
在使用此查询之前，请确保在 `convex/schema.ts` 文件中定义了适当的模式，或根据现有模式更改查询。
</Warning>
...
{/* LOCKED_PATTERN_cfdbe95b4d26ef82d25c4a1bfd5d5aeb */}
  <Info>
  使用此函数将 Dodo Payments 结账集成到您的 Convex 应用中。使用基于会话的结账，具备完整功能支持。
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_de2bafe43db636b1864521f4f7e5aa25 */}
  <Info>
  使用此函数允许客户通过 Dodo Payments 客户门户管理其订阅和详细信息。客户会通过 `identify` 函数自动识别。
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_eeaa14c4f31af768a3b7e376ee69b626 */}
  <Info>
  使用此处理程序在 Convex 应用中安全接收并处理 Dodo Payments Webhook 事件。所有 Webhook 处理程序都将 Convex `ActionCtx` 作为第一个参数，允许您使用 `ctx.runQuery()` 和 `ctx.runMutation()` 与数据库交互。
  </Info>
  <CodeGroup>
  ...
  <Warning>
  确保为要处理的每个 Webhook 事件在 Convex 后端定义相应的数据库 mutation。例如，创建一个 <code>createPayment</code> mutation 用于记录成功支付，或创建一个 <code>createSubscription</code> mutation 用于管理订阅状态。
  </Warning>
</Tab>
</Tabs>
...
<Info>
Dodo Payments Convex 组件使用基于会话的结账，提供安全、可定制的结账体验，预先配置了产品购物车和客户详情。这是所有支付流程的推荐方式。
</Info>
...
<Info>
客户会自动通过 DodoPayments 设置中配置的 `identify` 函数识别。该函数应返回客户的 `dodoCustomerId`。
</Info>
