...
编辑 `convex.config.ts` 后，运行 `npx convex dev` 一次以生成所需的类型。  
</Step>
<Step title="设置环境变量">
...
<Warning>
在使用此查询之前，请确保在您的 `convex/schema.ts` 文件中定义适当的模式，或者更改查询以匹配您现有的模式。
</Warning>
...
<Tab title="客户门户设置">
  <Info>
  使用此功能允许客户通过 Dodo Payments 客户门户管理他们的订阅和详细信息。客户通过 `identify` 函数自动识别。
  </Info>
  <CodeGroup>
...
<Tab title="Webhook 处理程序设置">
  <Info>
  使用此处理程序安全地接收和处理 Dodo Payments webhook 事件。在您的 Convex 应用中，所有 webhook 处理程序以 Convex `ActionCtx` 作为第一个参数，从而允许您使用 `ctx.runQuery()` 和 `ctx.runMutation()` 与数据库交互。
  </Info>
  <CodeGroup>
...
<Info>
客户通过您在 DodoPayments 设置中配置的 `identify` 函数自动识别。此函数应返回客户的 `dodoCustomerId`。
</Info>
