...
<Info>
该模板使用FastAPI与async/await模式，使用Pydantic进行验证，并使用`dodopayments` Python SDK实现无缝API集成。
</Info>
...
<Warning>
切勿将您的 `.env` 文件提交到版本控制中。该文件已包含在 `.gitignore` 中。
</Warning>
</Step>
...
| `/webhook` | POST | 处理Dodo Payments Webhooks |
| `/customer-portal` | POST | 生成客户门户URL |
...
**常见原因：**
- 无效的产品ID - 确保它在您的Dodo仪表板中存在
- `.env` 中的API密钥或环境设置错误
- 检查FastAPI日志以获取详细错误消息
</Accordion>
...
在您的 [Dodo仪表板](https://app.dodopayments.com/developer/webhooks) 中将Webhook URL更新为ngrok URL。确保用正确的Webhook验证密钥更新您的 `.env` 文件。
</Accordion>

<Accordion title="Webhook签名验证失败">
- 确保 `DODO_PAYMENTS_WEBHOOK_KEY` 在您的 `.env` 中正确设置
- 验证您是否使用原始请求主体进行签名验证
- 检查您是否正确读取了 `webhook-signature` 头部
</Accordion>
</AccordionGroup>
