...
- 确保已在 `.env` 中正确设置 `DODO_PAYMENTS_WEBHOOK_KEY`
- 验证你使用的是原始请求正文进行签名验证
- 将全部三个 `webhook-id`、`webhook-timestamp` 和 `webhook-signature` headers 传递给 `webhook.verify()` — Standard Webhooks 签名涵盖 `id.timestamp.body`，而不仅仅是正文
</Accordion>
</AccordionGroup>
