...
<Info>
此模板使用 FastAPI 结合 async/await 模式，使用 Pydantic 进行验证，以及 `dodopayments` Python SDK 以实现无缝的 API 集成。
</Info>
...
| `/webhook` | POST | 处理 Dodo Payments webhooks |
...
- 验证您是否使用原始请求体进行签名验证
- 检查您是否正确读取了 `webhook-signature` 头
</Accordion>
</AccordionGroup>
