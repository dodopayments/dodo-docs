...
事件发生时的 ISO 8601 时间戳。如果未提供，则默认为当前 UTC 时间戳。时间戳必须处于过去 1 小时至未来 5 分钟的范围内——超出该时间范围的时间戳将被拒绝。
</ParamField>
...
<Info>
**请有意设置时间戳。** 对于实时事件，请省略 `timestamp`，系统会将其默认为当前 UTC 时间戳。对于延迟事件或批量事件，请明确设置 ISO 8601 时间戳，以便使用量计入正确的计费周期，但请注意，可接受的时间范围很窄：时间戳早于过去 **1 小时** 或晚于未来 **5 分钟** 的事件将被拒绝。不支持历史数据回填——请在一小时内刷新缓冲的事件。
</Info>
