...
<Warning>
Dynamic Checkout 代理已弃用的 `POST /payments` 和 `POST /subscriptions` 端点。它会继续为现有集成提供支持，但新集成应使用下方的 **Checkout Sessions**。
</Warning>

### 响应格式
Dynamic Checkout 返回包含结账 URL 的 JSON 响应：
...
Checkout Sessions 提供更加安全的托管式结账体验，可为一次性购买和订阅处理完整的支付流程，同时支持全面的自定义控制。

如需了解详情及完整的受支持字段列表，请参阅 [Checkout Sessions 集成指南](https://docs.dodopayments.com/developer-resources/checkout-session)。

### 响应格式
Checkout Sessions 返回包含结账 URL 的 JSON 响应：
...
Customer Portal 路由处理程序可让你将 Dodo Payments 客户门户无缝集成到 Next.js 应用程序中。
...
- **方法：** 仅支持 POST 请求。其他方法将返回 405。
- **签名验证：** 使用 <code>webhookKey</code> 验证 Webhook 签名。验证失败时返回 401。
- **Payload 验证：** 使用 Zod 进行验证。Payload 无效时返回 400。
- **错误处理：**
  - 401：签名无效
  - 400：Payload 无效
  - 500：验证期间发生内部错误
- **事件路由：** 根据 Payload 类型调用相应的事件处理程序。
