...
<Info>
所有示例假定您正在使用 Next.js 应用路由器。
</Info>

<Tabs>
{/* LOCKED_PATTERN_28fd81a4703a84d931eeff69f1520d54 */}
  <Info>
  使用此处理程序将 Dodo Payments 结账集成到您的 Next.js 应用。支持静态（GET）、动态（POST）和结账会话（POST）支付流程。
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_0026b036379d81a9d03c36b3b6e83ddd */}
  <Info>
  使用此处理程序在您的 Next.js 应用中安全地接收并处理 Dodo Payments Webhook 事件。
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments 支持三种将支付集成到您网站中的支付流程，本适配器支持所有类型的支付流程。
</Info>
...
任何以 <code>metadata_</code> 开头的查询参数都会作为元数据传递。
</ParamField>
...
结账会话提供更安全的托管结账体验，可处理一次性购买和订阅的完整支付流程，并具备全面的自定义控制。
