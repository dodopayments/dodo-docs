...
结账会话提供更安全的托管结账体验，处理一次性购买和订阅的完整支付流程，并具有完全的自定义控制。

请参考 [结账会话集成指南](https://docs.dodopayments.com/developer-resources/checkout-session) 以获取更多详细信息和支持字段的完整列表。

### 响应格式
结账会话返回具有结账 URL 的 JSON 响应：
...
客户门户路由处理器使您可以无缝集成 Dodo Payments 客户门户到您的 Remix 应用中。
...
如果设置为<code>true</code>，则会发送带有门户链接的电子邮件给客户。
</ParamField>
...
- **有效负载验证：** 使用 Zod 验证。对于无效的负载，返回 400。
- **错误处理：**
  - 401：无效签名
  - 400：无效负载
  - 500：验证过程中的内部错误
