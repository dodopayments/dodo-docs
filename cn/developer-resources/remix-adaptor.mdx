...
description: 了解如何使用我们的 Remix 适配器将 Dodo Payments 集成到您的 Remix App Router
  项目中。涵盖结账、客户门户、webhooks 和安全环境设置。
icon: code-merge
keywords:
  - Dodo Payments
  - Remix
...
<Warning>
绝不要将您的 <code>.env</code> 文件或机密提交到版本控制。
</Warning>
</Step>
</Steps>
...
<Info>
所有示例都假设您正在使用 Remix App Router。
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  使用此处理器将 Dodo Payments 结账集成到您的 Remix 应用中。支持静态 (GET)、动态 (POST) 和会话 (POST) 支付流。
  </Info>
  <CodeGroup>
...
<Tab title="Customer Portal Handler">
  <Info>
  使用此处理器允许客户通过 Dodo Payments 客户门户管理其订阅和详细信息。
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  使用此处理器在您的 Remix 应用中安全地接收和处理 Dodo Payments webhook 事件。
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments 支持将支付集成到您的网站的三种支付流，这个适配器支持所有类型的支付流。
</Info>

- **静态支付链接：** 可即时共享的 URL，用于快速、无代码的支付收集。
- **动态支付链接：** 通过 API 或 SDKs 以编程方式生成具有自定义详细信息的支付链接。
- **结账会话：** 创建带有预配置产品购物车和客户详细信息的安全、可自定义的结账体验。
...
固定收取的金额，单位为主要货币单位（例如，<code>12.5</code> 表示 12.50 美元）。仅适用于 Pay What You Want 产品；如果金额低于产品的最低价格，则会忽略此参数。
</ParamField>
...
- 作为 POST 请求中的 JSON 主体发送参数。
- 支持一次性和定期支付。
- 有关支持的 POST 主体字段的完整列表，请参考：
  - [一次性支付产品请求主体](https://docs.dodopayments.com/api-reference/payments/post-payments)
  - [订阅产品请求主体](https://docs.dodopayments.com/api-reference/subscriptions/post-subscriptions)
...
Checkout sessions 提供了更加安全的托管式结账体验，可为一次性购买和订阅处理完整的支付流程，并提供全面的自定义控制。

有关更多详细信息以及受支持字段的完整列表，请参阅 [Checkout Sessions 集成指南](https://docs.dodopayments.com/developer-resources/checkout-session)。
...
Customer Portal 路由处理程序使你能够将 Dodo Payments 客户门户无缝集成到 Remix 应用程序中。
