...
<Card title="Bun" icon="bolt" href="/developer-resources/bun-adaptor">
  用于结账、门户和网络钩子的原生 Bun.serve() 处理程序
</Card>
</CardGroup>
...
| **Webhook 处理程序** | 支持类型安全的事件处理程序进行安全签名验证 |
| **环境配置** | 通过环境变量简单设置 |
| **类型安全** | 完整的 TypeScript 支持以及类型化负载 |
...
<Step title="配置环境变量">
  ...
  <Warning>
  永远不要将你的 `.env` 文件或秘密提交到版本控制中。
  </Warning>
</Step>

<Step title="创建路由处理程序">
  设置你的结账、客户门户和 webhook 路由：
  ...
  <Check>
  你现在已经准备好处理支付了！访问各个适配器页面以获取详细指南和所有可用选项。
  </Check>
</Step>
</Steps>
...
使用静态结账生成简单、可共享的支付链接。将产品 ID 作为查询参数传递：

```
/api/checkout?productId=pdt_xxx&quantity=1
```

支持可选的客户预填充和通过查询参数的自定义。
</Accordion>

<Accordion title="动态结账 (POST)">
使用动态结账以编程方式创建带有自定义细节的支付：
...
所有适配器提供类型安全的 webhook 处理和细粒度事件回调：
...
<Info>
所有 webhook 处理程序自动验证签名并使用 Zod 模式验证有效负载。无效请求将以适当的错误代码被拒绝。
</Info>
...
| 框架 | 最佳场景 | 运行时 |
|-----------|----------|---------|
| **Next.js** | 使用 App Router 的全栈 React 应用 | Node.js, Edge |
| **Nuxt** | 全栈 Vue.js 应用程序 | Node.js |
...
| **SvelteKit** | 全栈 Svelte 应用程序 | Node.js |
| **Remix** | 带嵌套路由的全栈 React | Node.js |
...
- **Discord**: 加入我们的 [社区服务器](https://discord.gg/bYqAp4ayYh)，获得实时帮助
- **电子邮件**: 联系我们 support@dodopayments.com
- **GitHub**: 在各自的适配器存储库中打开问题
