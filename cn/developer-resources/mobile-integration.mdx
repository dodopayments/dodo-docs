...
{/* LOCKED_PATTERN_0b8bdecbf0cc0826a0625d7af7744edf */}
  提供 Android、iOS、React Native 和 Flutter 的完整代码示例
</Card>
</CardGroup>
...
了解如何在后端使用 Node.js、Python 等创建结账会话。在专门的 <strong>Checkout Sessions API</strong> 文档中查看完整示例和参数参考。
</Card>
<Note>
**Security**：结账会话必须在后端服务器上创建，绝不能在移动应用中。这样可以保护 API 密钥并确保正确的验证。
</Note>
...
<Note>
**Security**：移动应用仅与您的后端通信，绝不直接与 Dodo Payments API 通信。
</Note>
</Step>
...
通过 webhook 和重定向 URL 处理付款完成，以确认付款状态。
</Step>
</Steps>
...
<Tip>
使用 InAppBrowser 提供原生浏览器体验。处理支付回调的深度链接。
</Tip>
...
<Warning>
**Regional Compliance**：应用内支付 SDK 有地区限制。在 **iOS** 上，原生支付 SDK 仅在 **欧盟**（根据 DMA 条款）是合法的。在 **Android** 上，它们在用户选择计费（UCB）市场是合法的。对于其他地区，请使用上面的结账链接方法以确保 App Store 和 Play Store 合规。
</Warning>

{/* LOCKED_PATTERN_c0f6dca68080424327396739e988adac */}
  提供完整指南，介绍如何将 Dodo 的 React Native SDK 与原生支付表单集成、全定制选项以及直接支付处理。包括设置、实现和测试说明。
</Card>

<Info>
查看[App Store Fee Bypass Guide](/features/bypassing-app-store-fees)，以了解应用内支付集成的地区合规要求。
</Info>
...
<Tip>
使用 WebView 处理支付链接。处理重定向 URL 以检测付款完成。
</Tip>
