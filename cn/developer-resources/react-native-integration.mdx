...
<Step title="实现支付界面">
    使用 `useCheckout` hook 创建你的支付界面。以下是完整的实现：
...
你可以通过在调用 `initPaymentSession()` 时修改外观参数来自定义 React Native Unified Checkout 的设计，以匹配你的应用设计，包括颜色、字体等。

#### 颜色自定义

每个颜色类别确定用户界面中一个或多个组件的颜色。例如，`primary` 定义了支付按钮的颜色。
...
| `componentBorder` | 输入框、标签和其他组件的外边框颜色 |
