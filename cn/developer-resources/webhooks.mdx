...
<Expandable title="Data object properties">
  ...
  资源类型。其中之一：`Payment`，`Subscription`，`Refund`，`Dispute`，`LicenseKey`，`CreditLedgerEntry`，`CreditBalanceLow`，`AbandonedCheckout`，`DunningAttempt`，或 `EntitlementGrant`。
  </ResponseField>
  
  额外字段因事件类型而异。请参阅事件特定文档以获取完整模式。
</Expandable>
</ResponseField>
