...
以 <code>metadata_</code> 开头的任何查询参数将作为元数据传递。
</ParamField>
...
Checkout 会话提供更安全、托管的结账体验，处理一次性购买和订阅的完整支付流程，并提供完全的自定义控制。
...
客户门户路由处理器使您能够将 Dodo Payments 客户门户无缝集成到您的 SvelteKit 应用中。
...
如果设置为 <code>true</code>，则发送包含门户链接的电子邮件给客户。
</ParamField>
