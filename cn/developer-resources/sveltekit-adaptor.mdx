...
<Info>
所有示例假设您正在使用 SvelteKit 应用路由。
</Info>

<Tabs>
<Tab title="结账处理器">
  <Info>
  使用此处理器将 Dodo Payments 结账集成到您的 SvelteKit 应用中。支持静态 (GET)、动态 (POST) 和会话 (POST) 支付流程。
  </Info>
  <CodeGroup>
...
<Tab title="Webhook 处理器">
  <Info>
  使用此处理器在您的 SvelteKit 应用中安全地接收和处理 Dodo Payments Webhook 事件。
  </Info>
  <CodeGroup>
