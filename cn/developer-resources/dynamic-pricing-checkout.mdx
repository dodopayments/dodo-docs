...
1. **设定价格范围**：定义一个最低价格（必需）和可选的最高价格
2. **传递动态金额**：在创建结账会话时，在产品购物车中包含一个 `amount` 字段
3. **让客户选择**：如果未提供金额，客户可以输入自己的价格（在您的范围内）

<Info>
当您在产品购物车中传递一个 `amount` 时，该金额将用于结账。如果省略 `amount` 字段，客户可以在结账时选择自己的价格（遵循您的最低/最高设置）。
</Info>
...
**示例**：如果您的最低价格是 $5.00，输入 `5.00`（或 `500` 分）。
</Step>
...
<Step title="保存产品">
点击 **添加产品** 进行保存。注意您的产品 ID（例如，`pdt_123abc456def`），以便在结账会话中使用。
</Step>
</Steps>
...
一旦您的产品配置为“支付您想要的价格”，您可以创建具有动态金额的结账会话。产品购物车中的 `amount` 字段允许您为每个结账会话以编程方式设置价格。

### 理解金额字段

在创建结账会话时，您可以在每个产品购物车项目中包含一个 `amount` 字段：

- **如果提供了 `amount`**：结账将使用这个确切的金额（必须在您的最低/最高范围内）
- **如果省略 `amount`**：客户可以在结账时输入自己的价格（在您的范围内）

<Warning>
`amount` 字段仅适用于“支付您想要的价格”产品。对于常规产品，此字段将被忽略。
</Warning>
...
<Note>
**金额格式**：`amount` 字段必须为货币的最低面额。对于美元，这意味着分（例如，$25.00 = `2500`）。对于其他货币，使用最小单位（例如，印度的 paise）。
</Note>

## 第三步：让客户选择他们的价格

如果您希望客户在结账时选择自己的价格，只需从产品购物车中省略 `amount` 字段。结账页面将显示一个输入字段，客户可以在您的最低和最高限制内输入任何金额。
...
<Card title="追踪定价决策" icon="chart-line">
使用元数据追踪为何选择了特定的金额（例如，`pricing_tier`，`discount_code`，`user_segment`）。
</Card>
...
- 产品在仪表板中已启用 **支付您想要的价格**
- 产品为 **单次支付**（一次性）产品，而不是订阅
- 金额格式正确（最低货币面额，例如美元的分）
</Accordion>

<Accordion title="金额超过最高或低于最低">
如果金额违反您的产品价格范围，API 将拒绝结账会话。始终在创建结账会话之前验证金额，或通过省略 `amount` 字段让客户选择他们的价格。
</Accordion>

<Accordion title="客户无法输入自己的价格">
如果客户看不到价格输入字段，请确保您已从产品购物车中省略 `amount` 字段。当提供 `amount` 时，结账将使用该确切金额。
</Accordion>
</AccordionGroup>