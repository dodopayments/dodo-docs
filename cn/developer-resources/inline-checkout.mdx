...
转到**设置 → 支付方法**，在 **Apple Pay** 行上，点击**管理域**。

<Frame caption="Open Wallet domains from the Apple Pay row">
  <img src="/images/apple-pay-domains/manage-domains.png" alt="支付方法设置中 Apple Pay 行的管理域按钮" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
<Frame caption="Download the Apple Pay domain association file">
  <img src="/images/apple-pay-domains/wallet-domains-panel.png" alt="包含下载文件按钮的钱包域面板" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Register your domain">
单击**注册域**，输入嵌入内嵌结账的域（例如，`shop.example.com`），然后**继续**。

<Frame caption="Register the domain where you embed inline checkout">
  <img src="/images/apple-pay-domains/register-domain.png" alt="注册域表单，已输入域" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
它必须通过 HTTPS 提供、无需重定向即可访问，并使用 `Content-Type: application/octet-stream` 或 `text/plain` 提供。
</Step>

<Step title="Verify the domain">
单击**验证域**。Dodo Payments 确认该文件处于活动状态，并将您的域提交给 Apple。

<Frame caption="Verify the hosted association file">
  <img src="/images/apple-pay-domains/verify-domain.png" alt="包含关联文件主机路径和验证域按钮的验证您的域屏幕" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Confirm it's active">
当状态显示**激活**时，Apple Pay 已为该域启用。使用 **启用** 开关在每个域上打开或关闭它。

<Frame caption="Verified domains show an Active status">
  <img src="/images/apple-pay-domains/domains-active.png" alt="显示具有激活 Apple Pay 状态的域和启用开关的钱包域列表" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
| 品牌化 | 无缝 | 与页面分离 |
| 实施工作量 | 高 | 低 |
| 最佳用途 | 自定义结账页面，高转化率流程 | 快速集成，现有页面 |

<Tip>
当您希望最大程度地控制结账体验和无缝品牌化时，请使用**内嵌结账**。使用**覆盖结账**进行较少更改的快速集成。
</Tip>
