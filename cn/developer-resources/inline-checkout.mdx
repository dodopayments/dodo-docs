...
| Field | Description |
|-------|-------------|
| `subTotal` | 应用任何折扣或税费之前，session 中所有 line item 的总和。 |
| `discount` | 所有已应用折扣的总价值。 |
| `tax` | 计算得出的税额。在 `inline` 模式下，用户与地址字段交互时，该金额会动态更新。 |
| `total` | session 基础货币中 `subTotal - discount + tax` 的数学结果。 |
| `currency` | 标准小计、折扣和税费金额所使用的 ISO 货币代码（例如 `"USD"`）。 |
| `finalTotal` | 实际向客户收取的金额。其中可能包含额外的外汇调整或本地支付方式费用，这些费用不属于基本价格明细的一部分。 |
| `finalTotalCurrency` | 客户实际支付时使用的货币。如果启用了[购买力平价](/features/purchasing-power-parity)或本地货币转换，则该货币可能与 `currency` 不同。 |
