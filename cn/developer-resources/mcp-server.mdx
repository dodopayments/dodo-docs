...
[Dodo Agent Plugin](/developer-resources/build-with-ai-coding-agents) 可通过一个步骤为 Claude Code、Codex CLI、Cursor、VS Code / GitHub Copilot、Kiro 和 OpenCode 安装 MCP servers 以及全部十七个 skills。Gemini CLI 仅会获得两个 MCP servers，因为它没有 agent-skill 原语。有关各个 agent 的安装命令，请参阅 [AI Coding Agents 指南](/developer-resources/build-with-ai-coding-agents)。
