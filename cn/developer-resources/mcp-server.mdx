...
<Step title="保存并重启">
    ...
    <Check>
    通过询问 AI 助手您的 Dodo Payments 数据来验证连接。
    </Check>
</Step>
</Steps>
</Tab>
...
1. **`list_api_endpoints`** - 发现可用的方法并可选过滤
2. **`get_api_endpoint_schema`** - 获取特定方法的详细架构
3. **`invoke_api_endpoint`** - 执行任何方法并提供适当的参数
...
在远程部署时，始终要求通过 `Authorization` 头或 `x-dodo-payments-api-key` 头进行身份验证。
...
验证 `DODO_PAYMENTS_ENVIRONMENT` 是否设置正确（`live_mode` 用于生产）。
...
某些客户端不支持所有 JSON 架构特性。使用 `--describe-capabilities` 了解支持的内容。
</Accordion>
</AccordionGroup>
