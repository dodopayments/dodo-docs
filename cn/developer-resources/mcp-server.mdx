...
<Tip>
使用 `@latest` 来始终拉取最新版本，或像 `@1.0.0` 一样固定到特定版本以保证稳定性。
</Tip>
...
点击后，通过 **Cursor 设置** > **工具与 MCP** > **新 MCP 服务器** 在光标的 `mcp.json` 中设置你的环境变量。
...
<Tab title="Claude 桌面版">
在 Claude 桌面版中设置 Dodo Payments MCP 服务器，以便实现对支付数据的对话访问。
<Steps>
<Step title="找到配置文件">
    找到 Claude 桌面版配置文件：
...
点击后，通过 **命令面板** > **MCP: 打开用户配置** 在 VS Code 的 `mcp.json` 中设置你的环境变量。
...
在 Cline (VS Code) 中设置 Dodo Payments MCP 服务器，以便实现对支付数据的对话访问。
<Steps>
...
| 变量 | 描述 | 必需 |
|----------|-------------|----------|
| `DODO_PAYMENTS_API_KEY` | 你的 Dodo Payments API 密钥 | 是 |
| `DODO_PAYMENTS_WEBHOOK_KEY` | 你的 webhook 签名密钥 | 否 |
| `DODO_PAYMENTS_ENVIRONMENT` | 设为 `live_mode` 以用于生产 | 否 |
...
<Tip>
在开发期间使用 `--operation=read` 来防止意外修改你的数据。
</Tip>
...
1. **`list_api_endpoints`** - 发现可用方法及可选过滤
2. **`get_api_endpoint_schema`** - 获取特定方法的详细模式
3. **`invoke_api_endpoint`** - 使用适当的参数执行任何方法

<Info>
动态工具在方法超过 50 个的 API 中会自动启用。使用 `--tools=all` 来覆盖以包括所有工具。
</Info>
...
在远程部署时，始终要求通过 `Authorization` 头部或 `x-dodo-payments-api-key` 头部进行身份验证。
...
验证 `DODO_PAYMENTS_ENVIRONMENT` 是否正确设置（ `live_mode` 对于生产）。
...
某些客户端不支持所有 JSON 模式功能。使用 `--describe-capabilities` 了解支持的内容。
</Accordion>
</AccordionGroup>
