...
<Info>
此模板使用 Go 1.21+，结合干净架构模式（`cmd`，`internal`，`templates`）、HTML 模板，以及 `dodopayments-go` SDK 实现无缝的 API 集成。
</Info>
...
- **现代 UI** - 采用干净的深色主题定价页面和 HTML 模板
- **Webhook 处理** - 安全地验证和处理支付事件
- **客户门户** - 自助订阅管理
- **Go 最佳实践** - 采用干净架构，结合 `cmd`，`internal` 和 `templates`
- **预填充结账** - 演示传递客户数据以提升用户体验
...
在 `templates/index.html` 中，用您的实际用户数据替换硬编码的客户数据：
...
此模板演示了在 `internal/api/webhook.go` 中处理 webhook 事件。支持的事件包括：

| 事件 | 描述 |
|-------|-------------|
| `subscription.active` | 当订阅变为激活时触发 |
...
**常见原因：**
- 产品 ID 无效 - 请验证它在您的 Dodo 控制台中存在
- 在 `.env` 中设置的 API 密钥或环境错误
- 检查服务器日志以获取详细错误消息
</Accordion>
...
在您的 [Dodo 控制台](https://app.dodopayments.com/developer/webhooks) 中将 webhook URL 更新为 ngrok URL。确保在 `.env` 文件中更新正确的 webhook 验证密钥。
</Accordion>
