...
<Tip>
如果您希望客户通过托管结账来批准授权，请设置 `payment_link: true` 并提供 `return_url`。
</Tip>

## 按需工作原理

1. 您使用 `on_demand` 对象创建订阅以授权付款方式，并可以选择收取初始费用。
2. 之后，您使用专用收费端点对该订阅创建自定义金额的收费。
...
<AccordionGroup>
  <Accordion title="收费请求主体参数">
    <ParamField body="product_price" type="integer" required>
      收费金额（以最小货币单位计）。例如：要收取 $25.00，请传递 <code>2500</code>。
    </ParamField>
    ...
    <ParamField body="adaptive_currency_fees_inclusive" type="boolean">
      如果为 true，则在 <code>product_price</code> 中包含自适应货币费用。如果为 false，则费用会加在上面。
    </ParamField>
...
<Warning>
对不是按需计费的订阅收费可能会失败。确保在收费之前，订阅的详细信息中包含 `on_demand: true`。
</Warning>
...
- **回退机制**: 在重试之间使用指数回退。
- **重试限制**: 限制总重试次数（最多 3-4 次）。
- **智能过滤**: 仅对可重试失败（例如，网络/发行人错误，资金不足）进行重试；不要重试硬拒绝。
- **防止卡片测试**: 不要对以下失败进行重试：`DO_NOT_HONOR`，`STOLEN_CARD`，`LOST_CARD`，`PICKUP_CARD`，`FRAUDULENT`，`AUTHENTICATION_FAILURE`。
- **变化元数据（可选）**: 如果您维护自己的重试系统，通过元数据区分重试（例如，`retry_attempt`）。
...
- 示例: 如果客户今天下午 1:10 开始试用或授权，请根据您的回退计划在接下来的几天中安排后续重试（例如，+3 天 → 下午 1:10，+7 天 → 下午 1:10）。
...
<Tip>
仅在软/临时问题上重试（例如，`insufficient_funds`，`issuer_unavailable`，`processing_error`，网络超时）。如果同一拒绝重复发生，请暂停进一步重试。
</Tip>

### 实施指南（无代码）

- 使用持久化精确时间戳的计划程序/队列；在精确的时间偏移量处计算下一次尝试（例如，`T + 3 days` 在相同的 HH:MM）。
- 维护并引用最后一次成功付款时间戳 `T`，以计算下一次尝试；不要在同一时刻聚集多个订阅。
- 始终评估最后一次拒绝原因；对于上述跳过列表中的硬拒绝，停止重试。
- 限制每个客户和每个帐户的并发重试，以防止意外的激增。
- 主动沟通：在下一次计划尝试之前，通过电子邮件/SMS 让客户更新他们的付款方式。
- 仅将元数据用于可观察性（例如，`retry_attempt`）；不要通过旋转无关字段来“逃避”欺诈/风险系统。
...
<Tip>
对于按需流程，请关注 `payment.succeeded` 和 `payment.failed`，以对账基于使用的费用。
</Tip>
...
<Step title="触发收费">
使用小额 `product_price`（例如，`100`）调用收费端点，验证您收到 `payment.succeeded`。
</Step>
<Step title="上线">
...
- **货币错误**: 如果您覆盖 `product_currency`，请确认您的账户和客户支持该货币。
