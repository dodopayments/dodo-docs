...
<Tip>
本指南通过结算会话（`POST /checkouts`）创建按需订阅，该会话始终返回托管的`checkout_url`。将客户重定向到那里以批准授权，并设置`return_url`至他们之后应抵达的位置。
</Tip>
...
如果重试仍然失败，因为支付方式本身已损坏（如过期卡、关闭的账户等），请使用[`POST /subscriptions/{subscription_id}/update-payment-method`](/api-reference/subscriptions/update-payment-method)从客户处收集新的支付方式。成功后，订阅将返回到`active`，并且会触发`payment.succeeded`和随后触发的`subscription.active` webhook。
</Step>
</Steps>
...
调用收费端点并使用一个小的`product_price`（例如，`100`），验证您收到`payment.succeeded`。
</Step>
<Step title="Go live">
一旦验证了事件和内部状态更新，切换到您的实时API密钥。
</Step>
</Steps>
