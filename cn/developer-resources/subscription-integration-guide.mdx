...
| `timestamp` | string | 是 | 事件发生的时间戳（不一定与事件交付的时间相同） |
| `type` | string | 是 | 事件的类型。请参阅[订阅事件类型](#subscription-event-types) |
| `data` | object | 是 | 主要数据负载。请参阅[订阅 webhook 负载](/developer-resources/webhooks/intents/subscription) |
