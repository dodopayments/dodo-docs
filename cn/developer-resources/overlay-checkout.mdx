...
<Info>
本节介绍如何使用 Checkout SDK 进行**客户端**主题配置。您也可以在通过 API 使用 `theme_config` 参数创建结账会话时进行**服务器端**主题配置。有关 API 级别的配置，请参阅[结账主题自定义](/features/checkout#checkout-theme-customization)，或使用仪表板中的[设计页面](/features/design)进行可视化主题配置并实时预览。
</Info>
