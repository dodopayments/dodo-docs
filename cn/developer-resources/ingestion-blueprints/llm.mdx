...
<Warning>
在开发过程中请始终使用 `test_mode`，以避免影响生产指标。
</Warning>
</ParamField>
...
<Note>
使用通过 AI SDK（如 Google's Gemini 2.5 Flash with thinking mode）访问的具备推理能力的模型时，推理令牌会自动包含在 `outputTokens` 的计数中，以确保正确计费。
</Note>
</Accordion>
...
<Note>
**Gemini 思维模式：** 当使用具备思考/推理能力的 Gemini 模型（如 Gemini 2.5 Pro）时，SDK 自动将 `thoughtsTokenCount`（推理令牌）包含在 `outputTokens` 中，以准确反映全部计算成本。
</Note>
</Accordion>
</AccordionGroup>
...
- `provider` - LLM 提供者（如果包括在包装元数据中）
- 您在包装客户端时提供的任何自定义元数据

<Note>
**推理令牌：** 对于具备推理能力的模型，`outputTokens` 自动包含完成令牌和推理令牌。
</Note>
</ParamField>
