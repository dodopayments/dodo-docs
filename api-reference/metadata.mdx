...
- Metadata keys can be up to 40 characters long on most objects; usage event metadata keys can be up to 100 characters
...
const checkoutSession = await client.checkoutSessions.retrieve('cks_123');
...
// list() returns an auto-paginating iterable, so iterate rather than reading `.items`.
let matchingPayment;
