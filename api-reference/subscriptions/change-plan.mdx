...
You can apply one or more **stacked discount codes** when changing plans by passing the `discount_codes` array (max 20 entries, applied in array order). The singular `discount_code` field is deprecated and cannot be used together with `discount_codes`.
