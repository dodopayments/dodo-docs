...
Set `collect_via_payment_link` to `true` to send the customer to a hosted checkout page instead of charging their saved payment method. The response then includes `payment_link`, `client_secret`, and `expires_on` — redirect the customer to `payment_link` to complete the payment.
...
| `on_payment_failure: prevent_change` | The subscription must stay on its current plan until the checkout payment actually succeeds. |
...
<Info>
See the [Subscription Upgrade & Downgrade Guide](/developer-resources/subscription-upgrade-downgrade#collecting-payment-via-a-checkout-link) for the full flow, including how the request is retried and what happens if the link expires.
</Info>
