...
- `failure_details` — a structured object present whenever `error_code` is set, regardless of payment status. It contains `code`, `decline_type` (`soft` or `hard`), `customer_fixable` (boolean), `merchant_message`, `customer_message`, `customer_template` (`C1`–`C20`), and `customer_cta` (one of `edit_and_retry`, `use_another_method`, `try_again`, `try_later`, `retry_and_verify`, `restart`, `update_method`). Prefer reading `failure_details.decline_type` and `failure_details.customer_fixable` over hard-coding the Type and User Error columns below.
