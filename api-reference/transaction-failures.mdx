...
| `AUTHENTICATION_REQUIRED` | Soft | Yes | Additional authentication is needed to complete the transaction | Prompt the customer to complete 3DS authentication. For off-session renewals, ask them to return on-session and authenticate |
...
| `CARD_DECLINED` | Hard | No | The card was declined by the issuing bank | Ask the customer to contact their bank or use another card |
...
| `LIVE_MODE_TEST_CARD` | Soft | Yes | A test card was used in live mode | Use a real card — test cards are not accepted in live mode |
