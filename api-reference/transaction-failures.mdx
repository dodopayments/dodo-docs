...
- `error_message` — a human-readable explanation written for **you**, the merchant. When `error_code` is one of the standardized codes below, this is a headline plus the recommended action rather than the raw text from the payment processor.
- `retry_attempt` — `0` for the original charge, `1` or higher for each scheduled subscription renewal retry. Always `0` for non-subscription payments.
...
Every standardized failure code maps to two different messages, so the right audience sees the right level of detail:

| Audience | Where it appears | What it says |
|----------|------------------|--------------|
| **You** | `error_message` on the payment, the dashboard payment details page, and the merchant payment-failed email | The real reason plus a recommended action, for example *"The card hasn't been activated by the cardholder. Ask the customer to activate it with their bank, then retry."* |
...
<Note>
The Customer Portal returns the customer-facing wording in `error_message`, while the merchant API returns the merchant wording for the same payment. The `error_code` is identical in both.
</Note>
...
Dodo Payments already enforces this on the surfaces it controls: for these four codes, checkout, the Customer Portal, and dunning emails always fall back to a generic decline message, while your own copy keeps the true reason. Apply the same rule anywhere you render `error_message` from the merchant API to a customer.
</Warning>
