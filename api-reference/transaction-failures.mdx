...
- `failure_details` — purpose-built messaging derived from `error_code`, with a separate `merchant_message` and `customer_message`, plus `decline_type`, `customer_fixable`, and a suggested `customer_cta`. Present whenever `error_code` is set. See [Ready-Made Failure Messaging](/developer-resources/handle-payment-failures#ready-made-failure-messaging).
