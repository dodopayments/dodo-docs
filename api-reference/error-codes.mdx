...
- `DISCOUNT_CUSTOMER_NOT_ELIGIBLE`
  - **Trigger:** Customer does not satisfy the discount's `customer_eligibility` rule (e.g. a `first_time`-only code applied by a returning customer, or a customer not on a `specific`-eligibility discount's allowed list)
  - **Message:** Customer is not eligible for discount code '{code}'
...
- `DISCOUNT_NOT_YET_ACTIVE`
  - **Trigger:** Discount code applied before its scheduled `starts_at` date
  - **Message:** Discount code '{code}' is not yet active

- `DISCOUNT_PER_CUSTOMER_USAGE_LIMIT_EXCEEDED`
  - **Trigger:** Customer has already redeemed the discount up to its `per_customer_usage_limit`
  - **Message:** Discount code '{code}' has hit its per-customer usage limit
...
- `UNSUPPORTED_CURRENCY`
  - **Trigger:** Product or addon price is set in a currency Dodo Payments cannot charge. Product and addon prices now accept any chargeable currency (the full set of currencies with a payment minimum, ~87 currencies including `INR`); only three-decimal currencies (e.g. `BHD`, `KWD`, `OMR`) remain unsupported.
  - **Message:** Currency is not currently supported / Unsupported product currency / Unsupported currency for price / Unsupported currency for addon price / Currency Not Supported / Unexpected currency for Indian card subscripitons
