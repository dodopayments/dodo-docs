...
Customers stop seeing these grants in `GET /customer-portal/entitlements`, but their access on the underlying platform continues until the subscription/payment lifecycle revokes it (cancellation, refund) or you revoke each grant manually via [`DELETE /entitlements/{id}/grants/{grant_id}`](/api-reference/entitlements/revoke-grant).
