...
4. Generate your API key:
   ...
   - Copy the API key the in env named DOOD_PAYMENTS_API_KEY
...
client = DodoPayments(
    bearer_token=os.environ.get("DOOD_PAYMENTS_API_KEY"),  # This is the default and can be omitted (if using same name `DOOD_PAYMENTS_API_KEY`)
)
  ...
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_DODO_TEST_API}/payments`, {
      ...
      headers: {
        ...
        Authorization: `Bearer ${process.env.DOOD_PAYMENTS_API_KEY}`, // Replace with your API secret key generated from the Dodo Payments Dashboard
      },
      body: JSON.stringify({
        billing: {
          city: formData.city,
