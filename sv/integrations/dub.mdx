...
Dub spår besökare genom en unik klick-ID (`dub_id`) som lagras i en cookie när användare klickar på dina Dub kortlänkar. För att attribuera försäljning till dina länkar, behöver du:

1. **Fånga Dub's klick-ID** från `dub_id`-cookien när du skapar kassa-sessioner
2. **Lagra klick-ID** i din betalningsmetadata tillsammans med kundens externa ID
3. **Skicka försäljningsdata till Dub** när betalningar lyckas använda deras Track API
...
<Step title="Hämta din Dub API-nyckel">
  Navigera till din [Dub-instrumentpanel](https://app.dub.co) → Inställningar → API-nycklar och skapa en ny API-nyckel med `conversions.write`-scope.
...
- **Testa med provbetalningar**: Verifiera att integrationen fungerar korrekt innan du går live
...
- Kontrollera att din Dub API-nyckel är korrekt och har `conversions.write`-scope
- Kontrollera att `dub_click_id` fångas och lagras i betalningsmetadata
- Se till att webhook-transformeringen korrekt formaterar payloaden
- Verifiera att webhooken utlöses vid `payment.succeeded`-händelser
- Bekräfta att konverteringsspårning är aktiverad för dina Dub-länkar
</Accordion>

<Accordion title="Intäktsattribuering fungerar inte">
- Bekräfta att användare klickar på dina Dub kortlänkar innan kassan
- Verifiera att `dub_id`-cookien sätts korrekt på din domän
- Kontrollera att klick-ID:n matchar mellan skapandet av kassa och slutförande av betalning
- Se till att du fångar klick-ID innan du skapar kassa-sessionen
</Accordion>

<Accordion title="Transformationsfel">
- Validera att JSON-strukturen matchar Dub's Track Sale API-format
...
- Verifiera att API-endpointens URL är korrekt: `https://api.dub.co/track/sale`
- Testa transformationen med prov-webhook-payloads
</Accordion>

<Accordion title="Dubbla försäljningar spåras">
- Se till att du endast spårar på `payment.succeeded`-händelser, inte `payment.processing`
- Använd unika `invoiceId`-värden för varje försäljning
- För prenumerationer, lägg till tidsstämplar eller faktureringsperiod för att förhindra dubbletter vid förnyelser
</Accordion>
</AccordionGroup>
