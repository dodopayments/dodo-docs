...
För att UPI ska visas vid kassan:
1. **Betalningsland** måste vara Indien (`IN`)
2. **Valuta** måste vara INR
3. För icke-indiska handlare: **Adaptiv valuta** måste vara aktiverad
...
`payment.succeeded` webhook skickas efter faktisk debitering, inte vid initiativ.
</Step>
</Steps>

<Warning>
**Ge inte fördelar vid debiteringens start.** Vänta på `payment.succeeded` webhooken, som anländer ~48-51 timmar efter det schemalagda debiteringsdatumet.
</Warning>
...
| Händelse | När | Åtgärd |
| :---- | :--- | :----- |
| `subscription.created` | Mandatet auktoriserat | Registrera abonnemangets start |
| `payment.succeeded` | ~48h efter debiteringsdatum | Ge/fortsätt åtkomst |
| `payment.failed` | Debitering misslyckades | Informera kund, pausa åtkomst |
...
| Visa | Avslogs | `4706131211212123` | 06/32 | 123 |
| Mastercard | Framgång | `5409162669381034` | 06/32 | 123 |
| Mastercard | Avslogs | `5105105105105100` | 06/32 | 123 |
...
<Accordion title="Hantera mandatavbokningar">
Kunder kan avboka mandat via sina bankappar när som helst. Övervaka `subscription.on_hold` webhooks och uppmana kunder att återprenumerera eller uppdatera betalningsmetoder.
</Accordion>
...
<AccordionGroup>
<Accordion title="UPI visas inte vid kassan">
**Kontrollera:**
1. Är betalningsland inställt på `IN`?
2. Är valuta inställd på `INR`?
3. Om det är en icke-indisk handlare: Är Adaptiv valuta aktiverad?
4. Är `upi_collect` inkluderad i `allowed_payment_method_types`?
...
**Lösning:** Nästa debitering kommer att misslyckas och abonnemanget kommer att flyttas till `on_hold`. Övervaka webhooks för `payment.failed`.
</Accordion>
</AccordionGroup>
