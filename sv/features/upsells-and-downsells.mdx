...
Dodo Payments möjliggör dessa flöden genom parameter `payment_method_id`, som låter dig debitera en kunds sparade betalningsmetod utan att de behöver ange kortuppgifter på nytt.

### Nyckelfördelar

| Fördel | Påverkan |
|---------|--------|
| **Ett-klick köp** | Hoppa över betalningsformuläret helt för återkommande kunder |
| **Högre konvertering** | Minska friktionen i beslutsögonblicket |
| **Omedelbar bearbetning** | Debiteringar processas omedelbart med `confirm: true` |
| **Sömlös UX** | Kunderna stannar i din app genom hela flödet |
...
<Warning>
När du använder `payment_method_id` måste du ställa in `confirm: true` och tillhandahålla en befintlig `customer_id`. Betalningsmetoden måste tillhöra den kunden.
</Warning>
...
| `prorated_immediately` | Debiterar baserat på kvarvarande tid i faktureringscykeln | Rättvist tidsbaserat fakturering |
| `full_immediately` | Debiterar full pris för ny plan, ignorerar kvarvarande tid | Faktureringscykeln nollställs |

<Tip>
Använd `difference_immediately` för enkla uppgraderingsflöden. Använd `prorated_immediately` när du vill ta hänsyn till oanvänd tid på den aktuella planen.
</Tip>
...
<Info>
Krediter från nedgraderingar med `difference_immediately` är prenumerationsspecifika och tillämpas automatiskt på framtida förnyelser. De är skilda från [Kundkrediter](/features/customer-credit).
</Info>
...
<AccordionGroup>
<Accordion title="Tajma dina uppersell strategiskt">
...
<Accordion title="Validera betalningsmetodens berättigande">
...
<Accordion title="Hantera misslyckanden smidigt">
...
<Accordion title="Ge klart värdeerbjudande">
...
<Accordion title="Respektera kundens val">
...
|-------|---------|--------|
| `payment.succeeded` | Uppgradering/korsförsäljningsbetalning slutförd | Leverera produkt, uppdatera åtkomst |
| `payment.failed` | Ett-klick debitering misslyckades | Visa fel, erbjud försök igen eller alternativ |
| `subscription.plan_changed` | Uppgradering/nedgradering slutförd | Uppdatera funktioner, skicka bekräftelse |
| `subscription.active` | Prenumeration återaktiverad efter planändring | Ge åtkomst till ny nivå |

<Card title="Webbhook integrationsguide" icon="webhook" href="/developer-resources/webhooks">
...
<CardGroup cols={2}>
<Card title="Guide för abonnemangsuppgradering" icon="arrows-rotate" href="/developer-resources/subscription-upgrade-downgrade">
  Detaljerad guide om planändringar, prorateringslägen och hantering av misslyckanden.
</Card>

<Card title="Kassasessioner" icon="cart-shopping" href="/developer-resources/checkout-session">
  Komplett referens för att skapa kassasessioner med alla alternativ.
</Card>

<Card title="API för kundbetalningsmetoder" icon="credit-card" href="/api-reference/customers/get-customer-payment-methods">
  API-referens för att lista kundbetalningsmetoder.
</Card>

<Card title="Tillägg" icon="puzzle-piece" href="/features/addons">
  Förbättra abonnemang med flexibla tillägg för extra intäkter.
</Card>
</CardGroup>
