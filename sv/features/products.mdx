...
För återkommande produkter, ställ in objektet `price` till ett återkommande pris (`type: recurring_price`) och specificera `payment_frequency_interval` och `subscription_period_interval` (`Day`, `Week`, `Month`, eller `Year`) tillsammans med deras motsvarande antal.
...
Om du inte längre vill visa eller använda en produkt kan du arkivera den med en `DELETE`-begäran till `/products/{id}`-endpointen. Denna åtgärd döljer produkten men raderar den inte permanent.
