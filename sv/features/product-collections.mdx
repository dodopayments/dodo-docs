...
| **Tillåt uppsägning av prenumerationer** | Kunder kan säga upp sina egna prenumerationer från Customer Portal | Aktiverad |
| **Förhindra missbruk av provperioder** | Kunder som redan har använt en provperiod får i stället ett betalt köp utan provperiod, i stället för att få en ny provperiod | Inaktiverad |
...
<Info>
Mer information om hur användning av provperioder matchas och registreras finns i [Förhindra missbruk av provperioder](/features/subscription#preventing-trial-misuse).
</Info>

<Card title="Subscription Plan Changes" icon="repeat" href="/features/subscription#subscription-plan-changes">
  Läs mer om prorateringslägen och hur planändringar fungerar.
</Card>

## Hantera samlingar

Product Collections kan hanteras antingen via Dodo Payments-dashboarden eller programmatiskt via API. API:et ger full kontroll över skapande och uppdatering av samlingar, bilduppladdningar, arkivering samt hantering av kapslade grupper och produkter.
...
<Frame>
  <img 
    ...
    alt="Skärmbild av Product Collection-dashboarden som visar åtgärder för hantering av samlingar" 
...
Hämta alla Product Collections som är kopplade till ditt konto med en `GET`-begäran till endpointen `/product-collections`. Stöder paginering, filtrering efter varumärke och inkludering av arkiverade samlingar.
...
Skapa en ny Product Collection genom att skicka en `POST`-begäran till endpointen `/product-collections` med uppgifter som namn, beskrivning och varumärke.
...
Hämta detaljerad information om en specifik Product Collection — inklusive dess grupper och produktobjekt — med en `GET`-begäran till endpointen `/product-collections/{id}`.
...
Ändra uppgifterna för en Product Collection (namn, beskrivning, varumärke med mera) genom att skicka en `PATCH`-begäran till endpointen `/product-collections/{id}`.
...
Koppla en bild till en samling genom att ladda upp den via en försignerad URL. Begär en uppladdnings-URL från endpointen `/product-collections/{id}/images` och `PUT` sedan upp bilden till den returnerade URL:en inom 60 sekunder.
...
Grupper låter dig organisera produkter i en samling (till exempel "Månadsplaner" jämfört med "Årsplaner"). Använd group-endpoints för att lägga till, uppdatera eller ta bort grupper i en samling.
...
Hantera de enskilda produkterna i en grupp — lägg till nya produkter, uppdatera befintliga objekt (till exempel visningsordning) eller ta bort dem helt.
...
- **Gruppera logiskt**: Organisera produkter efter faktureringsintervall (månatligt/årligt) eller funktionsnivå (starter/pro/enterprise)
- **Ordna strategiskt**: Placera den populäraste eller rekommenderade planen först, eftersom den väljs i förväg i checkout
...
- **Ta hänsyn till proratering**: Välj ett prorateringsläge som passar din affärsmodell
- **Testa noggrant**: Verifiera flöden för checkout och planändringar i testläge innan du går live

<Check>
Du är redo att skapa Product Collections och ge kunderna en enhetlig upplevelse för att välja plan.
</Check>
...
<Card title="Checkout" icon="cart-shopping" href="/features/checkout">
  Visa produkter från samlingar i en enhetlig checkout-upplevelse.
</Card>
