---
sidebarTitle: Product Collections
title: Product Collections
description: Group related products together for unified checkout experiences,
  plan selection, and seamless upgrade/downgrade paths within the Customer
  Portal.
icon: layer-group
keywords:
  - Dodo Payments
  - produktkollektioner
  - betalningsplattform
  - SaaS-fakturering
---

<Info>
Product Collections let you group related products (e.g., Starter, Pro, Enterprise plans) under one umbrella. Display all options in a single checkout, define upgrade/downgrade paths, and give customers the flexibility to switch plans directly from the Customer Portal.
</Info>

<Frame>
  <img 
    src="/images/product-collection/checkout-page.png" 
    alt="Screenshot of the Product Collection checkout page showing multiple products displayed" 
  />
</Frame>

## Key Highlights

- **Collection-based structure**: Group related products (plans, tiers, pricing options) under a single collection for organized management.
- **One collection, many products**: Include multiple products like Starter, Pro, Lifetime, etc., each with its own pricing model.
- **Dynamic checkout experience**: Display all products from a collection in one checkout view, letting customers choose their preferred plan.
- **Merchant-level control**: Enable, disable, and reorder products within each collection. The first product is automatically pre-selected at checkout.
- **Lifecycle awareness**: Enable customers to upgrade or downgrade between products in the same collection via the Customer Portal.

## Creating a Product Collection

Product Collections are created and managed from the dashboard or via API. Each collection acts as a container for related products.

<Steps>
<Step title="Create the collection">
Define the collection with a name and optional description. Upload an image to visually represent the collection in checkout.

<Frame>
  <img 
    src="/images/product-collection/collection-form.png" 
    alt="Screenshot of the Product Collection creation form in the dashboard showing fields for name, description, and image upload" 
  />
</Frame>

**Collection fields:**
- **Name** (required): Display name for the collection (e.g., "SaaS Plans", "License Tiers")
- **Description** (optional): Brief explanation shown in checkout
- **Image** (optional): Visual branding for the collection

</Step>

<Step title="Add products to the collection">
Add existing products to your collection. Products can be organized into groups for better structure.

<Frame>
  <img 
    src="/images/product-collection/collection-form-products.png" 
    alt="Screenshot of the Product Collection products page showing a list of products and the ability to add them to the collection" 
  />
</Frame>

**Product organization:**
- **Groups**: Optionally organize products into named groups (e.g., "Monthly Plans", "Annual Plans")
- **Ungrouped products**: Products without a group appear at the collection level
- **Ordering**: Drag and drop to set the display order

<Warning>
Each product can only belong to one collection. If a product is already in another collection, you'll need to remove it first.
</Warning>

</Step>

<Step title="Configure ordering and visibility">
Control the display order and visibility of products within the collection.

**Configuration options:**
- **Product status**: Enable or disable individual products within the collection
- **Display order**: Drag and drop to set the sequence products appear in checkout

<Info>
The first product in the collection is automatically pre-selected as the default at checkout. Reorder products to change which one is selected by default.
</Info>

</Step>
</Steps>

## Collection Checkout

Collections enable a unified checkout experience where customers can view and select from all available products in one place.

### Checkout Types

| Type | Description | Use Case |
|------|-------------|----------|
| **Collection Checkout** | Displays all active products within a collection | Subscription plan selection, tiered pricing |
| **Single Product Checkout** | Displays only one specific product | Direct purchase, promotional links |

### Collection Checkout Experience

When using a collection checkout:

1. **All active products displayed**: Customers see every enabled product in the collection
2. **First product pre-selected**: The first product in the collection order is automatically selected
3. **Product details shown**: Each product displays its name, description, and pricing
4. **Single selection**: Customer selects one product to purchase
5. **Standard flow continues**: After selection, the checkout proceeds with the chosen product's pricing and billing settings

<Frame>
  <img 
    src="/images/product-collection/checkout-page.png" 
    alt="Screenshot of the Product Collection checkout page showing multiple products displayed" 
  />
</Frame>

<Tip>
Collection checkout is ideal for subscription businesses where you want customers to compare plans side-by-side before purchasing.
</Tip>

### API Integration

Create a checkout session for a collection:

```typescript
const session = await client.checkoutSessions.create({
  product_collection_id: 'pdc_abc123',
  product_cart: [], // Required: pass an empty array for collection checkout
  return_url: 'https://yoursite.com/return'
});

// Redirect customer to the checkout
window.location.href = session.checkout_url;
```

<Warning>
When using `product_collection_id`, discount codes cannot be applied at session creation. Customers can still enter discount codes during checkout if enabled.
</Warning>

## Customer Portal Integration

Customers can upgrade or downgrade between products within the same collection directly from the Customer Portal.

<Tip>
**Already have subscription products?** Simply add them to a Product Collection to enable upgrade/downgrade flows in the Customer Portal. No need to recreate your products.
</Tip>

### Plan Management Actions

| Åtgärd | Beskrivning | Merchant Control |
|--------|-------------|------------------|
| **Visa aktuell plan** | Visa aktuellt produktnamn, pris och förnyelsedatum | Alltid tillgängligt |
| **Uppgradera plan** | Byt till en produkt på en högre nivå i samma samling | Konfigurerbart (standard: inaktiverat) |
| **Nedgradera plan** | Byt till en produkt på en lägre nivå i samma samling | Konfigurerbart (standard: inaktiverat) |
| **Avsluta** | Avsluta prenumerationen helt | Alltid tillgängligt |

<Frame>
  <img 
    src="/images/product-collection/portal.png" 
    alt="Screenshot of the Product Collection customer portal plan change interface showing the plan management actions" 
  />
</Frame>

### Upgrade/Downgrade Rules

- Upgrades and downgrades are only available between products **within the same collection**
- Proration is applied based on your subscription settings
- Email notifications are sent to the business on every upgrade, downgrade, or cancellation

<Frame>
  <img 
    src="/images/product-collection/portal-change-plan.png" 
    alt="Screenshot of the Product Collection customer portal plan change interface showing the plan management actions" 
  />
</Frame>

<Info>
Customers cannot change to products outside their current collection. Create separate collections for distinct product lines.
</Info>

## Subscription Settings

Configure how subscriptions and plan changes work across your business from the Subscription Settings section in your dashboard.

<Frame>
  <img 
    src="/images/product-collection/business-settings.png" 
    alt="Screenshot of the subscription settings page showing Allow Multiple Subscriptions and Allow Subscription Updates toggles" 
  />
</Frame>

### Available Settings

| Inställning | Beskrivning | Standard |
|---------|-------------|---------|
| **Tillåt flera prenumerationer** | Kunder kan ha mer än en aktiv prenumeration samtidigt | Aktiverat |
| **Tillåt uppdateringar av prenumerationer** | Kunder kan när som helst uppgradera eller nedgradera sina befintliga prenumerationer via Customer Portal | Inaktiverat |
| **Tillåt avslut av prenumerationer** | Kunder kan avsluta sina egna prenumerationer från Customer Portal | Aktiverat |
| **Tillåt paus av prenumerationer** | Kunder kan pausa och återuppta sina egna prenumerationer från Customer Portal | Inaktiverat |
| **Fakturera användning vid paus** | En pausad användningsbaserad prenumeration faktureras för den användning som kunden är skyldig för när den pausas, i stället för att användningen förs över till nästa ordinarie faktura | Aktiverat |
| **Förhindra missbruk av provperioder** | Kunder som redan har löst in en provperiod får i stället ett betalt köp utan provperiod, i stället för att få en ny provperiod | Inaktiverat |

<Info>
Plan changes via Customer Portal are disabled by default. Enable "Allow Subscription Updates" in your Subscription Settings to let customers upgrade or downgrade between products in the same collection.
</Info>

<Info>
Om du inaktiverar "Tillåt uppsägning av prenumerationer" döljs möjligheten för kunder att själva säga upp prenumerationer, och uppsägningsbegäranden via Customer Portal API avvisas. Egna uppsägningar från merchantsidan påverkas inte. Se [Säga upp en prenumeration](/features/customer-portal#cancelling-a-subscription).
</Info>

<Info>
"Tillåt paus av prenumerationer" styr endast Customer Portal — du kan pausa och återuppta från dashboarden eller API:et oavsett hur inställningen är konfigurerad. "Fakturera användning vid paus" gäller endast användningsbaserade prenumerationer och registreras per faktureringscykel, så om inställningen ändras mitt i en cykel påverkar det inte hur den pågående cykeln regleras. Se [Pausa och återuppta prenumerationer](/features/subscription#pausing-and-resuming-subscriptions).
</Info>

<Info>
Mer information om hur inlösen av provperioder matchas och registreras finns i [Förhindra missbruk av provperioder](/features/subscription#preventing-trial-misuse).
</Info>

<Card title="Subscription Plan Changes" icon="repeat" href="/features/subscription#subscription-plan-changes">
  Läs mer om proration-lägen och hur planändringar fungerar.
</Card>

## Hantera samlingar

Product Collections kan hanteras antingen via dashboarden i Dodo Payments eller programmatiskt via API. API:et ger full kontroll över att skapa och uppdatera samlingar, ladda upp bilder, arkivera samt hantera kapslade grupper och produkter.

### Åtgärder i dashboarden

- **Skapa**: Konfigurera nya samlingar med produkter och grupper
- **Uppdatera**: Ändra namn, beskrivning, bild och produktorganisation
- **Ändra ordning**: Dra och släpp för att ändra visningsordningen för produkter
- **Aktivera/inaktivera produkter**: Styr vilka produkter som visas i checkout
- **Arkivera**: Dölj en samling utan att ta bort den permanent (den kan avarkiveras senare)

<Frame>
  <img 
    src="/images/product-collection/collection-dashboard.png" 
    alt="Skärmbild av Product Collection-dashboarden som visar åtgärder för att hantera samlingen" 
  style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>

### API-hantering

Följande endpoints låter dig skapa, uppdatera, hämta, arkivera och organisera Product Collections programmatiskt — inklusive hantering av kapslade grupper och produkterna i dem.

<AccordionGroup>
<Accordion title="Listing Product Collections">
Hämta alla Product Collections som är kopplade till ditt konto med en `GET`-förfrågan till endpointen `/product-collections`. Stöder paginering, filtrering efter varumärke och inkludering av arkiverade samlingar.

<Card title="List Product Collections API" icon="code" href="/api-reference/product-collections/list-product-collections">
  Se detaljerad struktur för request och response i API-dokumentationen för List Product Collections.
</Card>
</Accordion>

<Accordion title="Creating a Product Collection">
Skapa en ny Product Collection genom att skicka en `POST`-förfrågan till endpointen `/product-collections` med information som namn, beskrivning och varumärke.

<Card title="Create Product Collection API" icon="code" href="/api-reference/product-collections/create-product-collection">
  Se detaljerad struktur för request och response i API-dokumentationen för Create Product Collection.
</Card>
</Accordion>

<Accordion title="Retrieving a Product Collection">
Hämta detaljerad information om en specifik Product Collection — inklusive dess grupper och produktobjekt — med en `GET`-förfrågan till endpointen `/product-collections/{id}`.

<Card title="Get Product Collection API" icon="code" href="/api-reference/product-collections/get-product-collection">
  Se detaljerad struktur för request och response i API-dokumentationen för Get Product Collection.
</Card>
</Accordion>

<Accordion title="Updating a Product Collection">
Ändra informationen för en Product Collection (namn, beskrivning, varumärke med mera) genom att skicka en `PATCH`-förfrågan till endpointen `/product-collections/{id}`.

<Card title="Update Product Collection API" icon="code" href="/api-reference/product-collections/update-product-collection">
  Se detaljerad struktur för request och response i API-dokumentationen för Update Product Collection.
</Card>
</Accordion>

<Accordion title="Uploading Collection Images">
Koppla en bild till en samling genom att ladda upp den via en försignerad URL. Begär en uppladdnings-URL från endpointen `/product-collections/{id}/images` och `PUT` sedan bilden till den returnerade URL:en inom 60 sekunder.

<Warning>
Den försignerade URL:en upphör att gälla efter 60 sekunder, så bilden måste laddas upp inom den tidsramen.
</Warning>

<Card title="Update Collection Images API" icon="code" href="/api-reference/product-collections/update-product-collection-images">
  Se detaljerad struktur för request och response i API-dokumentationen för Update Collection Images.
</Card>
</Accordion>

<Accordion title="Archiving a Product Collection">
Arkivera en samling genom att skicka en `DELETE`-förfrågan till endpointen `/product-collections/{id}`. Detta döljer samlingen från ny användning men tar inte bort den permanent.

<Card title="Archive Product Collection API" icon="code" href="/api-reference/product-collections/archive-product-collection">
  Se detaljerad struktur för request och response i API-dokumentationen för Archive Product Collection.
</Card>
</Accordion>

<Accordion title="Unarchiving a Product Collection">
Återställ en arkiverad samling genom att skicka en `POST`-förfrågan till endpointen `/product-collections/{id}/unarchive`.

<Card title="Unarchive Product Collection API" icon="code" href="/api-reference/product-collections/unarchive-product-collection">
  Se detaljerad struktur för request och response i API-dokumentationen för Unarchive Product Collection.
</Card>
</Accordion>

<Accordion title="Managing Groups within a Collection">
Grupper låter dig organisera produkter i en samling (till exempel "Månadsplaner" jämfört med "Årsplaner"). Använd grupp-endpoints för att lägga till, uppdatera eller ta bort grupper i en samling.

- **Skapa en grupp**: `POST /product-collections/{id}/groups`
- **Uppdatera en grupp**: `PATCH /product-collections/{id}/groups/{group_id}`
- **Ta bort en grupp**: `DELETE /product-collections/{id}/groups/{group_id}`

<CardGroup cols={3}>
<Card title="Create Group" icon="code" href="/api-reference/product-collections/create-group">
  Lägg till en ny grupp i en Product Collection.
</Card>
<Card title="Update Group" icon="code" href="/api-reference/product-collections/update-group">
  Ändra en grupps namn eller attribut.
</Card>
<Card title="Delete Group" icon="code" href="/api-reference/product-collections/delete-group">
  Ta bort en grupp från en samling.
</Card>
</CardGroup>
</Accordion>

<Accordion title="Managing Products within a Group">
Hantera de enskilda produktobjekten i en grupp — lägg till nya produkter, uppdatera befintliga objekt (till exempel visningsordning) eller ta bort dem helt.

- **Lägg till produkter i en grupp**: `POST /product-collections/{id}/groups/{group_id}/items`
- **Uppdatera ett gruppobjekt**: `PATCH /product-collections/{id}/groups/{group_id}/items/{item_id}`
- **Ta bort ett gruppobjekt**: `DELETE /product-collections/{id}/groups/{group_id}/items/{item_id}`

<CardGroup cols={3}>
<Card title="Add Products to Group" icon="code" href="/api-reference/product-collections/add-group-items">
  Lägg till en eller flera produkter i en grupp i en samling.
</Card>
<Card title="Update Group Item" icon="code" href="/api-reference/product-collections/update-group-item">
  Uppdatera ett produktobjekt i en grupp.
</Card>
<Card title="Delete Group Item" icon="code" href="/api-reference/product-collections/delete-group-item">
  Ta bort ett produktobjekt från en grupp.
</Card>
</CardGroup>
</Accordion>
</AccordionGroup>

## Bästa praxis

- **Gruppera logiskt**: Organisera produkter efter faktureringsintervall (månadsvis/årligen) eller funktionsnivå (starter/pro/enterprise)
- **Ordna strategiskt**: Placera din mest populära eller rekommenderade plan först, eftersom den förväljs i checkout
- **Använd tydliga namn**: Produktnamn bör tydligt kommunicera skillnader i värde
- **Aktivera båda riktningarna**: Tillåt både uppgraderingar och nedgraderingar för att ge kunderna flexibilitet
- **Överväg proration**: Välj ett proration-läge som passar din affärsmodell
- **Testa noggrant**: Kontrollera checkout- och planändringsflöden i testläge innan du går live

<Check>
Du är redo att skapa Product Collections och erbjuda kunderna en enhetlig upplevelse för att välja plan.
</Check>

<CardGroup cols={2}>
<Card title="Products" icon="box" href="/features/products">
  Skapa engångsprodukter, prenumerationsprodukter eller användningsbaserade produkter som kan läggas till i samlingar.
</Card>

<Card title="Checkout" icon="cart-shopping" href="/features/checkout">
  Visa produkter från samlingen i en enhetlig checkout-upplevelse.
</Card>

<Card title="Customer Portal" icon="id-card" href="/features/customer-portal">
  Låt kunder uppgradera eller nedgradera inom samma samling.
</Card>

<Card title="Subscriptions" icon="repeat" href="/features/subscription">
  Hantera återkommande planer med proration och planändringar.
</Card>
</CardGroup>