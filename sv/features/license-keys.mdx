...
| Händelse | Beteende |
|---|---|
| `payment.succeeded` (engångsbetalning) | Generera en nyckel per köpt `quantity`. Nyckelns giltighetstid följer rättighetens varaktighet. |
| `subscription.active` | Generera en nyckel per prenumeration `quantity` (plats). Nyckeln har inget utgångsdatum; giltigheten är kopplad till prenumerationens status. |
| `subscription.renewed` | Ingen åtgärd. Befintliga nycklar finns kvar. |
| `subscription.on_hold` | Inaktivera nycklarna. De aktiveras igen när prenumerationen inte längre är pausad. |
| `subscription.paused` | Inaktivera nycklarna. De aktiveras igen när prenumerationen återupptas. |
| `subscription.cancelled` / `expired` | Inaktivera nycklarna permanent. |
| `subscription.plan_changed` | Inaktivera de gamla nycklarna och utfärda nya för den nya planen. |
| `refund.succeeded` (engångsbetalning) | Inaktivera nycklarna. |
| Manuell återkallelse via API/dashboard | Inaktivera nycklarna med `revocation_reason: manual`. De återställs inte automatiskt vid förnyelse av prenumerationen. |
| Licensnyckel inaktiverad direkt | Återkalla tilldelningen med `revocation_reason: license_key_disabled`. När nyckeln aktiveras igen återaktiveras tilldelningen automatiskt. |
...
Använd fältet `source` i licensnyckelposter för att skilja migrerat lager och manuellt uppfyllda nycklar från organiskt utfärdade nycklar vid avstämning eller granskning. Läs det från objektet `license_key` i tilldelningar som returneras av [List Grants](/api-reference/entitlements/list-grants); den äldre slutpunkten `GET /license_keys` är föråldrad.
...
<Warning>
`GET /license_keys`, `GET /license_keys/{id}` och `PATCH /license_keys/{id}` är föråldrade. Använd slutpunkterna för rättighetstilldelningar ([List Grants](/api-reference/entitlements/list-grants), [List Customer Grants](/api-reference/entitlements/list-customer-grants)) för läsning. `POST /license_keys` stöds fortfarande för import av befintliga nycklar.
</Warning>
...
Bläddra bland alla nycklar med information om status och användning.
</Card>
...
Hantera själva rättigheten för licensnyckeln: dess aktiveringsgräns, varaktighet och instruktioner.

<CardGroup cols={2}>
<Card title="Create Entitlement" icon="plus" href="/api-reference/entitlements/create-entitlement">
Skapa en rättighet för en licensnyckel.
</Card>
...
Visa nycklarna som utfärdats för en rättighet.
</Card>
<Card title="Revoke Grant" icon="ban" href="/api-reference/entitlements/revoke-grant">
Återkalla en kunds nyckel manuellt.
</Card>
</CardGroup>
</Accordion>
</AccordionGroup>
...
Leverans och återkallelse av licensnycklar utlöser de fyra [`entitlement_grant.*` webhook-händelserna](/developer-resources/webhooks/intents/entitlement-grant). Tilldelningspayloaden innehåller ett ifyllt objekt `license_key` med nyckeln, utgångsdatumet, använda aktiveringar och gränsen.

De äldre händelserna `license_key.*` (`license_key.created`) fortsätter att utlösas för livscykeln för den underliggande licensnyckelposten. Se sidan [Webhook-payload för licensnyckel](/developer-resources/webhooks/intents/license-key).

<Tip>
För nya integrationer ska du lyssna på `entitlement_grant.delivered` i stället för `license_key.created`. Rättighetshändelsen visar att leveransen är slutförd för alla integrationer på produkten, inte bara för licensnyckeln.
</Tip>
...
<Note>
Produkter som skapats med den äldre flaggan `license_key_enabled` har **migrerats automatiskt** till en rättighet för en licensnyckel. Migreringen är transparent: befintliga kunders nycklar fortsätter att fungera oförändrat, de offentliga slutpunkterna `/licenses/activate`, `/licenses/validate` och `/licenses/deactivate` fortsätter att fungera, och API-slutpunkterna `/license_keys/*` fortsätter att läsa från och skriva till samma nyckellager.

Det fristående dashboard-avsnittet **Licensnycklar** finns fortfarande kvar som en platt lista över alla utfärdade nycklar och är användbart för granskning och sökning. Ny konfiguration (ändring av aktiveringsgränser, varaktigheter eller instruktioner) bör göras genom att redigera den migrerade rättigheten för licensnyckeln under **Rättigheter**.
</Note>
...
- **Håll aktiveringsgränserna tydliga**: Välj rimliga standardvärden (1 för appar för enskilda användare, 3–5 för teamlicenser) och dokumentera dem.
- **Ge exakta aktiveringsinstruktioner**: Kunder klistrar in dessa från sitt e-postmeddelande, så exakta sökvägar och kommandon minskar antalet supportärenden.
- **Validera nycklar på serversidan**: För nätverksanslutna produkter ska du validera via `/licenses/validate` i stället för att cacha aktiveringen lokalt.
- **Använd webhooks för återkallelse**: Lyssna på `entitlement_grant.revoked` för att omedelbart inaktivera funktioner i appen när en kund avslutar eller återbetalas.
- **Testa med prenumerationer och engångsbetalningar**: Licensnycklarnas beteende skiljer sig subtilt mellan dessa, så testa båda innan du går live.
