...
- **`auto`** (standard): Dodo Payments genererar och skickar nyckeln via e-post automatiskt vid betalning eller prenumeration. Detta är beteendet som beskrivs ovan och gäller när `fulfillment_mode` utelämnas.
- **`manual`**: Köpet skapar en `Pending`-grant utan nyckel, och du anger själv varje nyckelvärde. Se [Manuell leverans](#manual-fulfillment) nedan.

### Aktivera en licens

<CodeGroup>

Som standard genererar Dodo Payments en licensnyckel och skickar den via e-post så snart en kund betalar. Med **manuell leverans** anger du själv nyckeln: köpet skapar en `Pending`-grant utan nyckel, meddelar dig och väntar på att du skickar in nyckelvärdet. Använd detta när nycklarna kommer från ditt eget system, en tredjepartsleverantör eller ett begränsat lager av förtryckta koder.
...
När en kund köper en produkt i manuellt läge skapas granten med statusen `Pending` utan nyckel, och en [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant)-webhook utlöses med `integration_type: "license_key"` och `status: "Pending"`. Du kan reagera på denna webhook eller fråga [List Customer Grants](/api-reference/entitlements/list-customer-grants)-endpointen med filtren `integration_type` och `status`:
...
Skicka in nyckeln med endpointen [Fulfill License Key Grant](/api-reference/entitlements/fulfill-license-key). Granten flyttas till `Delivered` och kunden får nyckeln automatiskt via e-post — samma e-postmeddelande som kunden skulle ha fått vid automatisk leverans.
