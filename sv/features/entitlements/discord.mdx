...
2. Dodo Payments skapar en grant med statusen `Pending`. Dess `oauth_url` är `null` tills kunden påbörjar accept-flödet, då den fylls i med en Discord-auktoriserings-URL.
3. Kunden får ett mejl med knappen "Join the Discord" (OAuth-länken). Länken visas även i Customer Portal.
4. När kunden har auktoriserat lägger boten till kunden på servern och tilldelar den konfigurerade rollen. Grantens status ändras till `Delivered`.
5. Om prenumerationen sägs upp, pausas eller löper ut, eller om merchant återkallar den manuellt, tar boten bort rollen och grantens status ändras till `Revoked`.
...
- `entitlement_grant.created` utlöses med `status: "Pending"`. `oauth_url` är `null` vid den här tidpunkten — den fylls i när kunden startar accept-flödet från sitt leveransmejl eller Customer Portal.
- `entitlement_grant.delivered` utlöses när rollen har tilldelats.
- `entitlement_grant.revoked` utlöses när rollen tas bort.
