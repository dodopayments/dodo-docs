...
Pausning och återupptagning hanteras via fältet `status` på slutpunkten för uppdatering av prenumerationen. Det finns ingen separat slutpunkt för pausning.
...
<Warning>
Skicka `paused` eller `active` separat — om någon av dem kombineras med ett annat fält avvisas begäran med `422`. Det äldre booleska fältet `pause` har tagits bort och misslyckas nu alltid med `422`, så en anropare som fortfarande använder det får ett tydligt fel i stället för en tyst no-op.
</Warning>
...
| `on_hold` | En betalning för förnyelse (eller en debitering vid planändring) misslyckades; förnyelser har stoppats | **Ja** | Uppdatera betalningsmetoden för att återaktivera — automatiskt via [Betalningsförsök](/features/recovery/payment-retries) och [Dunning](/features/recovery/subscription-dunning), eller manuellt via [API:t för uppdatering av betalningsmetod](/api-reference/subscriptions/update-payment-method) |
| `paused` | Prenumerationen pausades avsiktligt av dig eller kunden; debiteringen är fryst och åtkomsten har återkallats | **Ja** | Återuppta den med `status: active` eller från Customer Portal när självbetjänad pausning är aktiverad — se [Pausa och återuppta prenumerationer](#pausing-and-resuming-subscriptions) |
| `cancelled` | Prenumerationen har avslutats och kommer inte att förnyas | Endast nytt köp | Kunden måste starta en ny prenumeration; [Dunning](/features/recovery/subscription-dunning) kan uppmana till ett nytt köp |
| `failed` | **Skapandet** av prenumerationen misslyckades (det ursprungliga medgivandet eller betalningen lyckades inte) | **Nej — slutgiltigt** | Kunden måste skapa en ny prenumeration med en fungerande betalningsmetod |
...
Pausning och återupptagning hanteras via samma slutpunkt `PATCH /subscriptions/{subscription_id}` med hjälp av fältet `status`: `status: paused` pausar en aktiv prenumeration och `status: active` återupptar den. Inget av värdena kan kombineras med något annat fält i samma begäran. Mer information om beteendet, faktureringseffekterna och relaterade företagsinställningar finns i [Pausa och återuppta prenumerationer](#pausing-and-resuming-subscriptions).

<Card title="API Reference" icon="code" href="/api-reference/subscriptions/patch-subscriptions">
Visa API:t för uppdatering av prenumerationen, inklusive fältet `status`.
</Card>
</Accordion>
