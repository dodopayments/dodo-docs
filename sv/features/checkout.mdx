...
<Info>
Efter betalning omdirigeras kunder till din `return_url` med query-parameterar automatiskt bifogade -- inklusive `payment_id` eller `subscription_id`, `status`, `email`, och `license_key` (om tillämpligt). Se [Checkout Sessions guide](/developer-resources/checkout-session#request-body) för hela listan.
</Info>
...
<Frame>
  <img src="/images/checkout/theme-example.png" alt="Anpassad temakassasida" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Info>
Detta avsnitt täcker **server-side API**-temakonfiguration med `customization.theme_config`. Om du använder **Checkout SDK** (overlay eller inline checkout), se sektionerna för temaanpassning i [Overlay Checkout](/developer-resources/overlay-checkout#theme-customization) eller [Inline Checkout](/developer-resources/inline-checkout#theme-customization) som använder camelCase-egenskaper (t.ex., `bgPrimary` istället för `bg_primary`).
</Info>
