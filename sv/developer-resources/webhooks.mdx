...
Signaturverifiering är det rekommenderade sättet att autentisera en webhook. Den bevisar att begäran signerades med din webhook-hemlighet, vilket en kontroll på nätverksnivå inte kan göra.

Webhook-leveranser skickas från en pool med käll-IP-adresser som tillhör vår leveransinfrastruktur. Poolen ändras då och då, så betrakta adresserna som operativ information snarare än en fast egenskap hos integrationen.

<Warning>
Använd inte en allowlist för käll-IP som autentiseringsmekanism. En allowlist visar bara var en begäran kom ifrån, inte att den är äkta eller oförändrad — verifiera `webhook-signature`-headern för varje begäran, enligt beskrivningen i [Verifiera signaturer](#verifying-signatures).
</Warning>
...
- **Begär de aktuella intervallen** från [support@dodopayments.com](mailto:support@dodopayments.com) innan du låser en brandvägg, så att du använder en uppdaterad lista.
...
<Info>
En blockerad leverans behandlas som alla andra fel och görs om enligt schemat som beskrivs i [Automatiska omförsök](#automatic-retries). Om brandväggsregler gjorde att leveranser misslyckades kan du skicka dem igen när reglerna har åtgärdats — se [Spela upp och återställ meddelanden](#replaying-and-recovering-messages).
</Info>

### Svara på webhooks

* Din webhook-handler måste returnera en `2xx status code` för att bekräfta mottagandet av händelsen.
* Alla andra svar behandlas som fel och webhooken görs om.
...
<Accordion title="Respond immediately" icon="bolt">
  Returnera en `200`-statuskod omedelbart när webhooken tas emot. Bearbeta händelsen asynkront för att undvika timeouts.
...
<Accordion title="Handle duplicate events" icon="rotate">
  Implementera idempotens med `webhook-id`-headern för att säkert kunna bearbeta samma händelse flera gånger utan sidoeffekter.
</Accordion>

<Accordion title="Secure your webhook secret" icon="key">
  Lagra din webhook-hemlighet säkert med miljövariabler eller en secrets manager. Checka aldrig in hemligheter i versionshantering.
</Accordion>
</AccordionGroup>

## Webhook-payloadens struktur

Genom att förstå webhook-payloadens struktur blir det enklare att parsa och bearbeta händelser korrekt.
...
<Step title="Navigate to Webhooks">
    Gå till din Dodo Payments Dashboard och navigera till **Developer → Webhooks**.
</Step>

<Step title="Select Your Endpoint">
    Klicka på din webhook-endpoint för att öppna dess detaljsida.
</Step>
...
Här är en komplett Express.js-implementation som visar webhook-verifiering och hantering:
...
[Dodo Payments CLI](/developer-resources/sdks/cli) innehåller två kommandon för att testa webhooks under lokal utveckling utan att behöva lämna terminalen.
...
Med detta interaktiva verktyg kan du välja en händelsetyp och skicka en realistisk simulerad payload till din endpoint. Det körs i en loop så att du kan testa flera händelser i samma session.

Trigger-kommandot omfattar alla 47 händelsetyper som Dodo Payments levererar, inklusive familjerna subscription, payment, refund, dispute, license key, payout, credit, abandoned checkout, dunning och entitlement grant — se [Supported Webhook Events](/developer-resources/sdks/cli#supported-webhook-events) för den exakta listan.
...
Styr hur snabbt webhook-händelser levereras till din endpoint för att förhindra att systemet överbelastas.

<Steps>
<Step title="Open Advanced Tab">
    Klicka på fliken **Advanced** från endpointens detaljsida.
</Step>

<Step title="Configure Rate Limit">
    I avsnittet "Rate Limit (throttling)" klickar du på **Edit** för att ändra inställningarna för rate limit.
    
    <Info>
    Som standard tillämpas ingen rate limit på webhooks, vilket innebär att händelser levereras så snart de inträffar.
    </Info>
</Step>

<Step title="Set Your Limit">
    Konfigurera önskad rate limit för att styra webhook-leveransens frekvens och förhindra överbelastning av systemet.
</Step>
</Steps>

<Tip>
Använd rate limiting när din webhook-handler behöver tid för att bearbeta händelser eller när du vill samla flera händelser i batchar.
</Tip>
...
Med transformationer kan du ändra en webhooks payload och vid behov omdirigera den till en annan URL. Denna kraftfulla funktion gör det möjligt att:
...
<Step title="Enable Transformations">
    Aktivera reglaget **Enabled** för att aktivera transformationsfunktionen.
</Step>

<Step title="Configure Transformation">
    Klicka på **Edit transformation** för att definiera transformationsregler med JavaScript.
</Step>

<Step title="Test Transformation">
    Använd gränssnittet för transformationstestning för att verifiera att transformationen fungerar korrekt innan du går live.
</Step>
</Steps>
...
- Routing av händelser till olika mikrotjänster
</Tip>
...
Gå till **Developer → Webhooks** och öppna fliken **Activity** för att se leveransprestandan för alla dina endpoints.

**Delivery activity** visar försök över tid, grupperade som **Attempts per 5 minutes**, **Attempts per hour** eller **Attempts per day** beroende på tidsfönstret. Varje stapel delas upp efter resultat, och när du håller pekaren över ett segment visas status, antalet försök och dess andel av totalen. För en endpoint sammanfattar **Delivery stats (last 24h)** på fliken **Overview** samma information för det senaste dygnet.
...
Hur du skickar ett meddelande igen beror på hur många meddelanden du behöver hantera:
...
| Läge | Vad det gör | Vad du anger |
|------|--------------|--------------|
| **Recover failed messages** | Spelar upp varje meddelande till denna endpoint som misslyckades. | En startpunkt: **8 hours ago**, **Yesterday**, **3 days ago**, **Last week** eller **2 weeks ago** |
...
| **Bulk replay messages** | Spelar upp meddelanden som matchar de filter du väljer, inklusive meddelanden som redan levererats utan fel. | Gränserna **Since** och **Until**, som som standard omfattar de senaste två veckorna, samt valfria händelsetyper, kanal eller tagg |
...
Få en e-postavisering när webhook-leveranser till en endpoint misslyckas, så att du kan åtgärda problem innan de leder till en kö.
...
<Step title="Find Email Alerting">
  Leta upp kortet **Email alerting**.
</Step>

<Step title="Configure Email Addresses">
  Ange adresserna som ska ta emot aviseringarna. Separera flera adresser med kommatecken och lämna fältet tomt för att stänga av aviseringar.
</Step>
...
<Tip>
Aktivera e-postaviseringar för att upptäcka problem med webhook-leveranser tidigt och upprätthålla tillförlitliga integrationer.
</Tip>
