...
| Field | Description |
|-------|-------------|
| `subTotal` | Summan av alla radposter i sessionen innan rabatter eller skatter har tillämpats. |
| `discount` | Det totala värdet av alla tillämpade rabatter. |
| `tax` | Det beräknade skattebeloppet. I läget `inline` uppdateras detta dynamiskt när användaren interagerar med adressfälten. |
| `total` | Det matematiska resultatet av `subTotal - discount + tax` i sessionens basvaluta. |
| `currency` | ISO-valutakoden (t.ex. `"USD"`) för standardvärdena för delsumma, rabatt och skatt. |
| `finalTotal` | Det faktiska belopp som kunden debiteras. Detta kan inkludera ytterligare valutaomräkningsjusteringar eller avgifter för lokala betalningsmetoder som inte ingår i den grundläggande prisuppdelningen. |
| `finalTotalCurrency` | Valutan som kunden faktiskt betalar i. Den kan skilja sig från `currency` om [köpkraftsparitet](/features/purchasing-power-parity) eller konvertering till lokal valuta är aktiverad. |
...
<Info>
Domänverifiering krävs endast för **inline (embedded) checkout**. Det krävs inte för hosted checkout.
</Info>
...
<Frame caption="Open Wallet domains from the Apple Pay row">
  <img src="/images/apple-pay-domains/manage-domains.png" alt="Knappen Manage domains på Apple Pay-raden i inställningarna för Payment Methods" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Download the domain association file">
Ladda ner association-filen från panelen Wallet domains.

<Frame caption="Download the Apple Pay domain association file">
  <img src="/images/apple-pay-domains/wallet-domains-panel.png" alt="Panelen Wallet domains med knappen Download file" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Register your domain">
Klicka på **Register domain** och ange domänen där du bäddar in inline checkout (t.ex. `shop.example.com`), och klicka sedan på **Continue**.

<Frame caption="Register the domain where you embed inline checkout">
  <img src="/images/apple-pay-domains/register-domain.png" alt="Formuläret Register a domain med en angiven domän" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
Den måste tillhandahållas via HTTPS, vara nåbar utan omdirigeringar och tillhandahållas med `Content-Type: application/octet-stream` eller `text/plain`.
</Step>

<Step title="Verify the domain">
Klicka på **Verify domain**. Dodo Payments bekräftar att filen är tillgänglig och skickar din domän till Apple.

<Frame caption="Verify the hosted association file">
  <img src="/images/apple-pay-domains/verify-domain.png" alt="Skärmen Verify your domain med värdsökvägen till association-filen och knappen Verify domain" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Confirm it's active">
När statusen visar **Active** är Apple Pay aktiverat för den domänen. Använd växlingsknappen **Enabled** för att aktivera eller inaktivera det per domän.

<Frame caption="Verified domains show an Active status">
  <img src="/images/apple-pay-domains/domains-active.png" alt="Lista över Wallet domains som visar domäner med statusen Active för Apple Pay och växlingsknapparna Enabled" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
|---------|-----------------|------------------|
| Integrationsdjup | Fullständigt inbäddad på sidan | Modal ovanpå sidan |
| Layoutkontroll | Full kontroll | Begränsad |
| Varumärkesprofilering | Sömlös | Separat från sidan |
| Implementeringsinsats | Högre | Lägre |
| Bäst för | Anpassade checkout-sidor, flöden med hög konvertering | Snabb integration, befintliga sidor |

<Tip>
Använd **inline checkout** när du vill ha maximal kontroll över checkout-upplevelsen och en sömlös varumärkesprofilering. Använd **overlay checkout** för snabbare integration med minimala ändringar på dina befintliga sidor.
</Tip>
...
<Card title="Overlay Checkout" icon="layer-group" href="/developer-resources/overlay-checkout">
    Använd overlay checkout för en snabb modalbaserad integration.
</Card>

<Card title="Checkout Sessions API" icon="code" href="/api-reference/checkout-sessions/create">
    Skapa checkout sessions för att driva dina checkout-upplevelser.
</Card>

<Card title="Webhooks" icon="webhook" href="/developer-resources/webhooks">
    Hantera payment events på serversidan med webhooks.
</Card>
