...
Du kan synkronisera vilken som helst kombination av dessa enheter genom att specificera dem i parametern `scopes`. Alla synkroniseringsoperationer är inkrementella och överför endast nya eller uppdaterade poster för optimal prestanda.
...
Databastyp som ska användas. Stödda värden: `"mongodb"`, `"postgres"`, `"mysql"`, eller `"clickhouse"`.
</ParamField>
...
Komma-separerad lista över dataenheter som ska synkroniseras. Tillgängliga omfång: `licences`, `payments`, `customers`, `subscriptions`. Exempel: `"payments,customers"`.
</ParamField>

<ParamField path="--api-key" type="string" required>
Din Dodo Payments API-nyckel. Börjar med `dp_live_` för liveläge eller `dp_test_` för testläge.
</ParamField>

<ParamField path="--env" type="string" required>
Miljömål. Måste vara antingen `"live_mode"` eller `"test_mode"`. Detta avgör vilken Dodo Payments-miljö som ska synkroniseras från.
</ParamField>
...
<Tip>
Alternativet `interval` är obligatoriskt när du använder `.start()` för automatisk synkronisering. Synkroniseringen kommer att köras kontinuerligt vid angiven intervall tills processen stoppas.
</Tip>
...
<Tip>
Vid manuell synkronisering är alternativet `interval` inte obligatoriskt. Du kan anropa `.run()` när du behöver utföra en synkroniseringsoperation.
</Tip>
...
Namn på den databas som ska användas. Stödda värden: `"mongodb"`, `"postgres"`, `"mysql"`, eller `"clickhouse"`.
</ParamField>
...
Array av enheter som ska synkroniseras. Tillgängliga alternativ: `"licences"`, `"payments"`, `"customers"`, `"subscriptions"`. Du kan inkludera vilken kombination av dessa som helst.
</ParamField>
...
Tid i sekunder mellan automatiska synkroniseringar. Obligatoriskt när man använder `.start()` för automatisk synkronisering. Valfritt när man använder `.run()` för manuell synkronisering.
</ParamField>
...
<Warning>
**MongoDB**: En databas med namnet `dodopayments_sync` kommer automatiskt att skapas på din databasserver. All synkdata kommer att lagras där. Detta databasenamn är för närvarande fast och kan inte ändras.
...
**ClickHouse**: Tabeller (`Subscriptions`, `Payments`, `Licenses`, `Customers`) kommer att skapas med hjälp av ReplacingMergeTree-motorn. När du frågar, använd `FINAL`-nyckelordet för att säkerställa deduplicerade resultat.
</Warning>
