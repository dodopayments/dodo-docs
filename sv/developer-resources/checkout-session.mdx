...
Hoppa över standardsidan för betalningsframgång och omdirigera kunder direkt till din `return_url` efter slutförd betalning.
...
Visningsetikett som visas för kunden på kassans formulär.

**Exempel**: `"Company Name"`, `"How did you hear about us?"`, `"Team Size"`
</ParamField>

<ParamField body="field_type" type="string" required>
Typ av fält som bestämmer valideringsregler för inmatning.

**Tillgängliga typer**:
- `text` - Enkelrads textinmatning
...
Om detta fält måste fyllas i innan kassan kan slutföras.
</ParamField>
...
Lista över alternativ för `dropdown` fälttyp. Krävs när `field_type` är `dropdown`, ignoreras för andra typer.
...
- **Webhooks**: `payment.succeeded`, `subscription.active`, och andra relevanta händelse-payloads innehåller `custom_field_responses` arrayen
- **API-svar**: Betalnings- och abonnemangsobjekt inkluderar `custom_field_responses`
</Info>
...
Ytterligare konfiguration för kassasessioner som innehåller abonnemangsprodukter.

<Expandable title="Subscription data object properties">
<ParamField body="trial_period_days" type="integer">
Antal dagar för prövoperioden innan den första avgiften. Måste vara mellan 0 och 10000 dagar.
</ParamField>

<ParamField body="on_demand" type="object">
Efterfrågestöd abonnemangskonfiguration för användningsbaserad eller mätad fakturering.

<Expandable title="On-demand object properties">
<ParamField body="mandate_only" type="boolean" required>
Om inställd till true, utförs ingen avgift och betalningsmetodsdelen endast auktoriseras för framtida användning.
</ParamField>

<ParamField body="product_price" type="integer">
Produktpris för den första avgiften till kunden. Om inte specificerat, kommer det lagrade priset för produkten att användas.

**Format**: Representerat i den lägsta valören av valutan (t.ex. cent för USD). Till exempel, för att debitera $1,00, passera `100`.
</ParamField>

<ParamField body="product_currency" type="string">
Valfritt valuta för produktens pris. Om inte specificerat, standard till produktens valuta.
</ParamField>

<ParamField body="product_description" type="string">
Valfri produktbeskrivningsersättning för fakturering och linjeposter. Om inte specificerat, kommer den lagrade beskrivningen av produkten att användas.
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
Om anpassningsbara valutaavgifter ska inkluderas i produktpriset (true) eller läggas ovanpå (false). Ignoreras om anpassad prissättning inte är aktiverad.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
Här är 10 omfattande exempel som visar olika kassasessionkonfigurationer för olika affärsscenarier:
...
<Note>
När `confirm` är inställt på `true`, tas kunden direkt till kassans sida utan att gå igenom några bekräftelsesteg.
</Note>
...
<Note>
`billing_currency` överskrivning träder bara i kraft när anpassningsbar valuta är aktiverad i dina kontoinställningar. Om anpassningsbar valuta är inaktiverad kommer denna parameter inte att ha någon effekt.
</Note>
...
Använd en kunds sparade betalningsmetod för att skapa en kassasession som behandlas omedelbart, och hoppar över insamling av betalningsmetoder:
...
<Warning>
När du använder `payment_method_id`, måste `confirm` ställas in på `true` och en befintlig `customer_id` måste tillhandahållas. Betalningsmetoden kommer att valideras för behörighet med betalningens valuta.
</Warning>

<Info>
Betalningsmetoden måste tillhöra kunden och vara kompatibel med betalningens valuta. Detta möjliggör ett-klikksköp för återkommande kunder.
</Info>
...
Omdirigera kunder direkt efter slutförd betalning och hoppa över standardsidan för betalningsframgång:
...
<Tip>
Använd `redirect_immediately: true` när du har en anpassad framgångssida som ger bättre användarupplevelse än standardsidan för betalningsframgång. Detta är särskilt användbart för mobilappar och inbäddade kassaflöden.
</Tip>

<Note>
När `redirect_immediately` är aktiverad, omdirigeras kunder till din `return_url` omedelbart efter betalningsslut, och hoppar över standardsidan för betalningsframgång helt.
</Note>

### 14. Tvinga ett språk

Tvinga kassan att visas på ett specifikt språk, överstyr kundens webbläsarspråksdetektering:
...
**Stödda språk:** Arabiska (`ar`), Katalanska (`ca`), Kinesiska (`zh`), Nederländska (`nl`), Engelska (`en`), Franska (`fr`), Tyska (`de`), Hebreiska (`he`), Indonesiska (`id`), Italienska (`it`), Japanska (`ja`), Koreanska (`ko`), Malaysiska (`ms`), Polska (`pl`), Portugisiska (`pt`), Rumänska (`ro`), Ryska (`ru`), Spanska (`es`), Svenska (`sv`), Thailändska (`th`), Turkiska (`tr`)

### 15. Samla in anpassade fält

Samla in ytterligare information från kunder under kassan med hjälp av anpassade fält:
...
<Info>
Svaren på anpassade fält inkluderas automatiskt i webhook-payloads (`payment.succeeded`, `subscription.active`, etc.) och kan hämtas via API:et. Använd dem för att berika ditt CRM, utlösa onboardingflöden eller anpassa kundupplevelsen.
</Info>
...
Innan du skapar en kassasession kan du förhandsgranska prisuppdelningen, inklusive skatter, rabatter och totaler. Detta är användbart för att visa korrekt prissättning för kunder innan de går vidare till kassan.
...
<Card title="Preview API Reference" icon="code" href="/api-reference/checkout-sessions/preview">
  Visa den kompletta förhandsgranskningsdokumentationen för slutpunkt.
</Card>

## Flytta från dynamiska länkar till kassasessioner

### Viktiga skillnader
Tidigare, när man skapade en betalningslänk med dynamiska länkar, var man tvungen att ange kundens kompletta faktureringsadress.

Med kassasessioner är detta inte längre nödvändigt. Du kan helt enkelt skicka den information du har, och vi hanterar resten. Till exempel:
...
- Å andra sidan, om du redan har all nödvändig information och vill hoppa direkt till betalningssidan, kan du skicka den fullständiga datasetet och inkludera `confirm=true` i din begärans kropp.

### Migrationsprocess

Migration från dynamiska länkar till kassasessioner är enkel:
...
Justera begäran payload enligt formatet för kassasessioner.
</Step>

<Step title="That's it!">
Ja. Ingen ytterligare hantering eller speciella migrationssteg behövs på din sida.
</Step>
</Steps>
...
<Card title="Preview Checkout Session" icon="eye" href="/api-reference/checkout-sessions/preview">
  API-referens för att förhandsgranska prissättningar, skatter och totaler innan du skapar en session
</Card>
</CardGroup>