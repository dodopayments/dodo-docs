...
<Info>
**Sessionens giltighet**: Utcheckningssessioner är giltiga i 24 timmar som standard. Om du skickar `confirm=true` i din begäran, kommer sessionen endast att vara giltig i 15 minuter.
</Info>
...
Efter betalning omdirigeras kunderna till din `return_url` med ytterligare frågeparametrar för betalningsstatus.
</Step>
</Steps>
...
Array av produkter som ska inkluderas i utcheckningssessionen. Varje produkt måste ha en giltig `product_id` från din Dodo Payments-instrumentpanel.
...
<Info>
När `confirm` är inställt på `true`, blir alla fält för faktureringsadress obligatoriska för att sessionen ska skapas framgångsrikt.
</Info>
...
<Warning>
**Kritiskt**: Inkludera alltid `credit` och `debit` som fallback-alternativ för att förhindra utcheckningsfel när föredragna betalningsmetoder inte är tillgängliga.
</Warning>
...
<Note>
När `confirm` är inställt på `true`, kommer kunden att tas direkt till utcheckningssidan, vilket omgår eventuella bekräftelsesteg.
</Note>
...
<Note>
Den `billing_currency` överskrivningen träder endast i kraft när adaptiv valuta är aktiverad i dina kontoinställningar. Om adaptiv valuta är inaktiverad, kommer denna parameter inte att ha någon effekt.
</Note>
...
<Tip>
Använd `redirect_immediately: true` när du har en anpassad framgångssida som ger en bättre användarupplevelse än den standardiserade betalningsframgångssidan. Detta är särskilt användbart för mobilappar och inbäddade utcheckningsflöden.
</Tip>

<Note>
När `redirect_immediately` är aktiverat, omdirigeras kunderna till din `return_url` omedelbart efter betalningsslutförande, vilket helt hoppar över den standardiserade framgångssidan.
</Note>
...
- Utcheckningsflödet kommer automatiskt att samla in de saknade detaljerna innan kunden flyttas till betalningssidan.
- Å andra sidan, om du redan har all nödvändig information och vill hoppa direkt till betalningssidan, kan du skicka hela datamängden och inkludera `confirm=true` i din begäran.
