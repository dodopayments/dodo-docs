...
<Note>
Den förhandsvisade `current_breakup.subtotal` återspeglar redan [Purchasing Power Parity](/features/purchasing-power-parity) och [Charm Pricing](/features/charm-pricing) när de gäller för produkten.
</Note>

<Note>
När varukorgen innehåller en prenumerationsprodukt returnerar förhandsgranskningssvaret även en `next_billing_date` — en förhandsvisning av det kommande faktureringsdatumet, så att du kan visa det innan prenumerationen skapas. Det beräknas relativt till nu: `now + trial period` när en provperiod gäller, annars `now + one payment frequency`. Fältet utelämnas för varukorgar som endast innehåller engångsköp. Detta är en uppskattning baserad på tidpunkten för förhandsvisningen; det auktoritativa `next_billing_date` fastställs när prenumerationen aktiveras.
</Note>

<Note>
Förhandsvisningen returnerar även `trial_period_days` (den effektiva provperiodens längd, kostnadsfri eller betald) och `trial_amount` (avgiften per enhet under provperioden efter rabatter, i prisets valutas minsta enheter). `trial_amount` finns endast för en [betald provperiod](/features/subscription#paid-trials) och är `null` för en kostnadsfri provperiod eller ingen provperiod. Använd `current_breakup` för den beskattade totalsumma som faktiskt ska betalas idag.
</Note>
...
<Card title="Preview API Reference" icon="code" href="/api-reference/checkout-sessions/preview">
  Visa den fullständiga dokumentationen för förhandsvisningsendpointen.
</Card>

## Övergång från Dynamic Links till Checkout Sessions

### Viktiga skillnader
Tidigare behövde du ange kundens fullständiga faktureringsadress när du skapade en betalningslänk med Dynamic Links.

Med Checkout Sessions är detta inte längre nödvändigt. Du kan helt enkelt skicka med den information du har, så hanterar vi resten. Till exempel:

- Om du endast känner till kundens faktureringsland behöver du bara ange det.
- Checkout-flödet samlar automatiskt in de uppgifter som saknas innan kunden går vidare till betalningssidan.
- Om du däremot redan har all information som krävs och vill gå direkt till betalningssidan kan du skicka med hela datauppsättningen och inkludera `confirm=true` i request body.
...
Ja. Du behöver inte göra något ytterligare eller vidta några särskilda migreringsåtgärder.
</Step>
</Steps>
