...
När API:et returnerar en icke-lyckad statuskod, returnerar SDK:n ett fel av typen `*dodopayments.Error`. Den exponerar `StatusCode`, den underliggande `*http.Request` och `*http.Response`, samt JSON av felmeddelandekroppen. Använd mönstret `errors.As` för att inspektera det och förgren på `StatusCode` för att hantera specifika fall:
