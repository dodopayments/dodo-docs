...
<Info>
`billing` kräver minst en tvåbokstavs ISO `country`-kod. Använd `AttachExistingCustomer` för att koppla en befintlig kund, eller `NewCustomer` för att skapa en. `productPrice` anges i den lägsta valutaenheten.
</Info>
...
### Asynkront klientbibliotek

Använd det asynkrona klientbiblioteket för coroutine-baserade åtgärder:
...
<Tip>
Använd Kotlins `runCatching` för ett mer funktionellt sätt att hantera fel med Result-typer.
</Tip>
...
Ändra klientkonfigurationen tillfälligt:
...
- **GitHub**: Skapa ett ärende i [kodförrådet](https://github.com/dodopayments/dodopayments-kotlin)

## Bidra

Vi välkomnar bidrag! Läs [riktlinjerna för bidrag](https://github.com/dodopayments/dodopayments-kotlin/blob/main/CONTRIBUTING.md) för att komma igång.
