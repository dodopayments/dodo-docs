...
Anpassa verktygsfältet, knapparna och färgschemat för Custom Tab via
`customization` på `CheckoutParams`. Alla fält är valfria; om
`customization` utelämnas används Androids standardutseende för Custom Tab.
...
Avdelarens färg ovanför navigeringsfältet.
</ParamField>
...
Vilken sida av verktygsfältet stängningsknappen visas på: `START` eller `END`.
</ParamField>
...
Visar "Ladda ner sidan" i menyn med fler alternativ.
</ParamField>
...
- `PLATFORM_ERROR`: oväntat plattformsfel, inklusive en `returnUrl` vars schema inte matchar din `dodoCallbackScheme`-placeholder.

Om en användare avbryter eller en betalning nekas blir resultatet alltid (`CANCELLED` eller
`FAILED`), aldrig ett kastat fel. Med launcher-stilen kastas valideringsfel ut från `launcher.launch(...)`.

## Övergivna sessioner

Om appen avslutas eller användaren tvångsstoppar den under checkout sparar SDK:t sessionen lokalt. Nästa gång appen startas kontrollerar du om det finns en övergiven session och synkroniserar den med din backend:
...
<CardGroup cols={2}>
  <Card title="Mobile Integration Guide" icon="mobile" href="/developer-resources/mobile-integration">
    Metodtips för mobila checkout-flöden
  </Card>
  <Card title="Kotlin SDK" icon="code" href="/developer-resources/sdks/kotlin">
    Backend SDK för serverbaserade åtgärder
  </Card>
</CardGroup>
