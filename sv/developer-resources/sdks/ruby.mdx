...
<Warning>
`POST /subscriptions` (SDK:ts `subscriptions.create`-metoden) är **föråldrad**. Den fungerar fortfarande för befintliga integrationer, men nya integrationer bör skapa prenumerationer via en [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`billing` kräver minst den tvåbokstaviga ISO `country`-koden. `customer` accepterar antingen `{ customer_id: "..." }` för att koppla en befintlig kund eller `{ email: "...", name: "..." }` för att skapa en ny. `product_price` anges i den lägsta valutaenheten.
</Info>
...
För större kontroll över pagineringen:
...
Hantera olika fel från Dodo Payments API:
...
<Tip>
Implementera retry-logik med exponentiell backoff för rate limit-fel så att din applikation hanterar scenarier med hög volym på ett smidigt sätt.
</Tip>

## Typsäkerhet med Sorbet

Använd Sorbet för typsäkra request-parametrar:
...
Skicka requests till odokumenterade endpoints:
...
Använd i dina Rails-controllers:
...
- **GitHub**: Öppna ett ärende i [repositoryt](https://github.com/dodopayments/dodopayments-ruby)

## Bidra

Vi välkomnar bidrag! Läs [riktlinjerna för bidrag](https://github.com/dodopayments/dodopayments-ruby/blob/main/CONTRIBUTING.md) för att komma igång.
