...
`start` avvisar med ett `CheckoutError` endast vid felaktig användning eller ett plattformsfel. En annullerad eller nekad betalning är alltid ett resultat, aldrig ett undantag.

- `INVALID_CHECKOUT_URL`: inte en `checkout.dodopayments.com` sessions-URL.
...
Om appen eller JS-bundlen avslutas mitt under checkouten går promise-objektet förlorat, men det inbyggda lagret behåller sessionen. Återställ den vid nästa montering och stäm av den mot din backend.
