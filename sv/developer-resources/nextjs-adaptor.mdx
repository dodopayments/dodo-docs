...
<CardGroup cols={2}>
<Card title="Kassa-hanterare" icon="cart-shopping" href="#checkout-route-handler">
    Integrera Dodo Payments kassa med statiska, dynamiska och sessionsflöden.
</Card>
<Card title="Kundportal" icon="user" href="#customer-portal-route-handler">
...
<Tabs>
<Tab title="Kassa-hanterare">
  <Info>
  Använd denna hanterare för att integrera Dodo Payments kassa i din Next.js-app. Stöder statiska (GET), dynamiska (POST) och kassa-sessioner (POST) betalningsflöden.
  </Info>
  <CodeGroup>
...
<Tab title="Kundportal-hanterare">
  <Info>
  Använd denna hanterare för att låta kunder hantera sina prenumerationer och detaljer via Dodo Payments kundportal.
  </Info>
  <CodeGroup>
  </CodeGroup>
</Tab>
...
<Tab title="Webhook-hanterare">
  <Info>
  Använd denna hanterare för att ta emot och bearbeta Dodo Payments webhook-händelser säkert i din Next.js-app.
  </Info>
  <CodeGroup>
...
- För en komplett lista över stödda POST-kroppsfält, se:
  - [Begäran kropp för en engångsbetalningsprodukt](https://docs.dodopayments.com/api-reference/payments/post-payments)
...
<Accordion title="Kassa-sessioner (POST)">
Kassa-sessioner erbjuder en mer säker, värd kassaupplevelse som hanterar hela betalningsflödet för både engångsköp och prenumerationer med full anpassningskontroll.
