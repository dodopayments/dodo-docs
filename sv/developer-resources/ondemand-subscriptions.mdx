...
<Info>
Denna funktion kan behöva aktiveras på ditt konto. Kontakta supporten om du inte ser den i din instrumentpanel.
</Info>
...
2. Senare skapar du avgifter mot den prenumerationen med anpassade belopp via den dedikerade avgifts-API:en.
...
Vänligen se dem i [Skapa kassa-session](/api-reference/checkout-sessions/create)
...
<AccordionGroup>
  <Accordion title="Parametrar för avgiftsbegäran">
    ...
    <ParamField body="product_currency" type="string">
      Valfritt valutoöverskrivning för avgiften.
    </ParamField>

    <ParamField body="product_description" type="string">
      Valfri beskrivning för denna avgift.
    </ParamField>
    ...
    <ParamField body="metadata" type="object">
      Ytterligare metadata för betalningen. Om utelämnad används prenumerationsmetadata.
    </ParamField>
  </Accordion>
</AccordionGroup>
...
<Warning>
Burst-återförsöksmönster kan flaggas som bedrägliga eller misstänkt korttestning av våra risk- och processor-system. Undvik klusterade återförsök; följ nedan angiven backoff-schema och tidsjusteringsriktlinjer.
</Warning>

### Principer för säkra återförsökspolicys

- **Backoff-mekanism**: Använd exponentiell backoff mellan återförsök.
...
- **Förhindra korttestning**: Återförsök inte vid fel som `DO_NOT_HONOR`, `STOLEN_CARD`, `LOST_CARD`, `PICKUP_CARD`, `FRAUDULENT`, `AUTHENTICATION_FAILURE`.
...
### Föreslagen återförsökschema (prenumerationer)
...
- Exempel: Om kunden startar en provperiod eller mandat klockan 13:10 idag, schemalägg uppföljande återförsök klockan 13:10 på efterföljande dagar enligt din backoff (t.ex. +3 dagar → 13:10, +7 dagar → 13:10).
...
<Note>
För en omfattande lista över avslagsskäl och huruvida de kan rättas av användaren, se
[Transaktionsfel](/api-reference/transaction-failures) dokumentationen.
</Note>
...
Anropa avgifts-API:en med en liten `product_price` (t.ex. `100`) och verifiera att du får `payment.succeeded`.
</Step>
<Step title="Gå live">
...
- **Valuta fel**: Om du överskrider `product_currency`, bekräfta att den stöds för ditt konto och kund.
