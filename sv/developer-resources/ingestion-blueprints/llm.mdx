...
4. **Konfigurera din mätare**:
   - **Mätarnamn**: Välj ett beskrivande namn (t.ex. "LLM Token Användning")
   - **Eventnamn**: Ställ in ett unikt eventidentifierare (t.ex. `llm.chat_completion`)
   - **Aggregations typ**: Välj `sum` för att summera tokenräkningar
   - **Över Egenskap**: Välj vad som ska spåras:
     - `inputTokens` - Spåra inmatnings-/prompt tokens
     - `outputTokens` - Spåra utdata/kompletterings tokens (inkluderar resonemang tokens när det är tillämpligt)
...
<Warning>
Använd alltid `test_mode` under utveckling för att undvika att påverka produktionsmetricer.
</Warning>
</ParamField>
...
<Note>
När du använder modeller med resonemang genom AI SDK (som Googles Gemini 2.5 Flash med tänkande läge), inkluderas resonemang tokens automatiskt i `outputTokens`-räknaren för korrekt fakturering.
</Note>
</Accordion>
...
<Note>
**Gemini Tänkande Läge:** När du använder Gemini-modeller med tänkande/resonemangskapacitet (som Gemini 2.5 Pro), inkluderar SDK automatiskt `thoughtsTokenCount` (resonemang tokens) i `outputTokens` för att korrekt återspegla den fulla beräkningskostnaden.
</Note>
</Accordion>
</AccordionGroup>
...
- `inputTokens` - Antal inmatnings-/prompt tokens använt
- `outputTokens` - Antal utdata/kompletterings tokens använt (inkluderar resonemang tokens när det är tillämpligt)
- `totalTokens` - Totala tokens (inmatning + utdata)
- `model` - Den LLM-modell som används (t.ex. "gpt-4")
- `provider` - LLM-leverantören (om inkluderad i omslagsmetadata)
- Eventuella anpassade metadata du tillhandahållit när du slog in klienten

<Note>
**Resonemang Tokens:** För modeller med resonemangskapacitet, inkluderar `outputTokens` automatiskt både kompletterings tokens och resonemang tokens.
</Note>
</ParamField>
