...
> **Notera:** Vi kommer att sätta `DODO_PAYMENTS_WEBHOOK_KEY` efter distributionen när du har din webhook-URL.
...
✅ **Idempotens** - Förhindrar duplicerad behandling med webhook-ID:n  
✅ **Event logging** - Fullständig revisionsspårning i `webhook_events`-tabellen  
✅ **Felhantering** - Loggas och kan återförsökas

> **Notera:** Denna implementering demonstrerar hantering av tre centrala prenumerationshändelser (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) med minimala fält. Du kan enkelt utöka den för att stödja ytterligare händelsetyper och fält baserat på dina krav.
...
2. **Kontrollerar dubbletter** - Använder webhook-ID för att förhindra att samma händelse bearbetas två gånger
3. **Loggar händelsen** - Lagrar råa webhook i `webhook_events`-tabellen för revisionsspårning
4. **Bearbetar händelsen** - Skapar eller uppdaterar kunder och prenumerationer
5. **Hanterar fel** - Loggar misslyckanden och markerar händelsen som obehandlad för återförsök
...
| Problem | Lösning |
|-------|----------|
| Verifiering misslyckades | Kontrollera att webhook-nyckeln är korrekt från DodoPayments-instrumentpanelen |
| Databasanslutningsfel | Verifiera Neon-anslutningssträngen |
| Arbetare deployar inte | Kör `npm install`, kontrollera `wrangler.toml`-syntax |
