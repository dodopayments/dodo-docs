...
description: Distribuera DodoPayments webhooks till Cloudflare Workers
icon: cloud
---
...
### 4. Konfigurera initiala hemligheter
...
### 5. Uppdatera wrangler.toml

Redigera `wrangler.toml` och ställ in ditt arbetarnamn:
...
### 8. Registrera Webhook i DodoPayments Dashboard
...
### 9. Ställ in Webhook-nyckel och distribuera på nytt
...
4. **Bearbetar händelsen** - Skapar eller uppdaterar kunder och prenumerationer
5. **Hanterar fel** - Loggar misslyckanden och markerar händelsen som obehandlad för försök igen
...
2. Lägg till endpoint med din Worker-URL
...
| Databasanslutningsfel | Verifiera Neon-anslutningssträngen |
| Arbetare deployeras inte | Kör `npm install`, kontrollera `wrangler.toml` syntax |
