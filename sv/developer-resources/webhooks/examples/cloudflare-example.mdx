...
5. Kör frågan
6. Hämta din anslutningssträng från Neon → Anslutningsuppgifter
...
> **Obs:** Vi kommer att ställa in `DODO_PAYMENTS_WEBHOOK_KEY` efter driftsättning när du har din webhook-URL.

### 5. Uppdatera wrangler.toml

Redigera `wrangler.toml` och ange ditt worker-namn:
...
✅ **Händelseloggar** – Fullständig revisionsspår i `webhook_events`-tabellen  
✅ **Felhantering** – Loggas och kan försöka igen

> **Obs:** Denna implementation visar hantering av tre centrala prenumerationshändelser (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) med minimala fält. Du kan enkelt utöka den för att stödja ytterligare händelsetyper och fält baserat på dina krav.
...
Webhook-hanteraren:
1. **Verifierar signaturen** – Säkerställer att förfrågan kommer från DodoPayments med HMAC-SHA256
2. **Kontrollerar dubbletter** – Använder webhook-ID för att förhindra bearbetning av samma händelse två gånger
3. **Loggar händelsen** – Sparar rå webhook i `webhook_events`-tabellen för revisionsspår
4. **Bearbetar händelsen** – Skapar eller uppdaterar kunder och prenumerationer
5. **Hanterar fel** – Loggar misslyckanden och markerar händelsen som obearbetad för upprepning
...
| Databasanslutningsfel | Kontrollera Neon-anslutningssträngen |
| Worker deployar inte | Kör `npm install`, kontrollera syntaxen i `wrangler.toml` |
