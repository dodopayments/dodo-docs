...
> **Note:** Vi kommer att ställa in `DODO_PAYMENTS_WEBHOOK_KEY` efter distribution när du har din webhook-URL.
...
4. Aktivera dessa prenumerationsevent:
   - `subscription.active`
...
✅ **Signature verification** – Använder dodopayments-biblioteket  
✅ **Idempotency** – Förhindrar dubbelbehandling med webhook-ID:n  
✅ **Event logging** – Fullständig revisionsspår i `webhook_events` tabell  
✅ **Error handling** – Loggas och kan försöka igen

> **Note:** Denna implementation visar hur man hanterar tre centrala prenumerationsevent (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) med minimala fält. Du kan enkelt utöka den för att stödja ytterligare eventtyper och fält baserat på dina behov.
...
2. **Kontrollerar dubbletter** - Använder webhook-ID för att förhindra att samma event behandlas två gånger
3. **Loggar eventet** - Sparar den råa webhooken i `webhook_events` tabell för revisionsspår
4. **Bearbetar eventet** - Skapar eller uppdaterar kunder och prenumerationer i Neon
5. **Hanterar fel** - Loggar misslyckanden och markerar eventet som obearbetat för omförsök
...
3. Visa loggar i realtid och anropshistorik
...
| Problem | Lösning |
|---------|---------|
| Verifiering misslyckades | Kontrollera att webhook-nyckeln är korrekt i DodoPayments-panelen |
| Databasanslutningsfel | Verifiera Neon-anslutningssträngen och använd en poolad anslutning |
| Funktion hittades inte (404) | Kör `netlify init` och `npm run deploy` igen |
| Miljövariabler saknas | Ställ in variabler i CLI eller panelen och distribuera sedan på nytt |
