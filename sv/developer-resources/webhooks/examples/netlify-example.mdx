...
description: Distribuera DodoPayments webhooks till Netlify Functions
icon: diamond
---
...
### 8. Ställ in webhook-nyckel och distribuera igen
...
✅ **Händelseloggar** - Fullständig revisionsspår i `webhook_events` tabell  
...
Webhookhanteraren:
1. **Verifierar signaturen** - Säkerställer att begäran kommer från DodoPayments med HMAC-SHA256
2. **Kontrollerar dubbletter** - Använder webhook-ID för att förhindra bearbetning av samma händelse två gånger
3. **Loggar händelsen** - Lagrar rå webhook i `webhook_events` tabell för revisionsspår
4. **Bearbetar händelsen** - Skapar eller uppdaterar kunder och prenumerationer i Neon
5. **Hanterar fel** - Loggar misslyckanden och markerar händelsen som obearbetad för försök igen
...
**Visa loggar i Netlify Dashboard:**
1. Välj din webbplats → **Functions**-fliken
2. Klicka på `webhook` funktionen
3. Visa realtidsloggar och anropshistorik

**Konfigurera i DodoPayments Dashboard:**
1. Gå till Utvecklare → Webhooks
2. Lägg till endpoint med din Netlify Functions-URL
...
| Problem | Lösning |
|-------|----------|
| Verifiering misslyckades | Kontrollera att webhook-nyckeln är korrekt från DodoPayments-dashboarden |
| Databasanslutningsfel | Verifiera Neon-anslutningssträngen och använd poolad anslutning |
| Funktion hittades inte (404) | Kör `netlify init` och `npm run deploy` igen |
| Miljövariabler inte tillgängliga | Ställ in variabler i CLI eller dashboard, och distribuera sedan igen |
