...
> **Observera:** Vi kommer att ställa in `DODO_PAYMENTS_WEBHOOK_KEY` efter distribution när du har din webhook-URL.
...
4. Aktivera dessa abonnemangshändelser:
   - `subscription.active`
...
✅ **Händelseloggar** - Kompletterande revisionsspår i `webhook_events`-tabellen  
✅ **Felsökning** - Logger och retrybar

> **Observera:** Denna implementation visar hantering av tre kärnabonnemangshändelser (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) med minimala fält. Du kan enkelt utöka den för att stödja ytterligare händelsetyper och fält baserat på dina krav.
...
2. **Kontrollerar för dubbletter** - Använder webhook-ID för att förhindra bearbetning av samma händelse två gånger
3. **Loggar händelsen** - Lagrar rå webhook i `webhook_events`-tabellen för revisionsspår
4. **Bearbetar händelsen** - Skapar eller uppdaterar kunder och abonnemang i Neon
5. **Hanterar fel** - Logger misslyckanden och markerar händelsen som obearbetad för återförsök
...
1. Välj din webbplats → **Functions**-flik
...
| Databasanslutning fel | Verifiera Neon-anslutningssträngen och använd poolad anslutning |
| Funktion hittades inte (404) | Kör `netlify init` och `npm run deploy` igen |
| Miljövariabler är inte tillgängliga | Ställ in variabler i CLI eller dashboard, och distribuera sedan igen |
