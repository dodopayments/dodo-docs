...
Supabase tillhandahåller automatiskt `SUPABASE_URL` och `SUPABASE_SERVICE_ROLE_KEY` vid runtime.

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **Observera:** Vi kommer att ställa in `DODO_PAYMENTS_WEBHOOK_KEY` efter distributionen när du har din webhook-URL.
...
✅ **Idempotens** - Förhindrar duplicerat bearbetning med webhook-ID:n  
✅ **Händelseloggning** - Komplett revisionsspår i `webhook_events`-tabellen  
✅ **Felkoll** - Loggas och kan försöka igen

> **Observera:** Denna implementation visar hantering av tre kärnprenumerationshändelser (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) med minimala fält. Du kan enkelt utöka den för att stödja ytterligare händelsetyper och fält baserat på dina krav.
...
Deno-baserad Edge-funktion:
1. **Verifierar signatur** - Använder dodopayments-biblioteket för HMAC-SHA256-verifiering
2. **Kontrollerar idempotens** - Slår upp webhook-ID för att förhindra duplicerat bearbetning
3. **Loggar händelsen** - Lagrar råa webhook-data i `webhook_events`-tabellen
...
<Note>
Flaggan `--no-verify-jwt` krävs eftersom webhooks inte inkluderar JWT-token. Säkerhet tillhandahålls av verifiering av webhook-signatur.
</Note>
...
| Problem | Lösning |
|-------|----------|
| Verifiering misslyckades | Kontrollera att webhooknyckeln är korrekt från DodoPayments-dashboaden |
| Databasbehörighetsfel | Se till att använda Service Role Key |
| JWT-verifieringsfel | Distribuera med `--no-verify-jwt`-flaggan |
| Funktion inte funnen | Verifiera att projektreferensen är korrekt och att funktionen är distribuerad |
