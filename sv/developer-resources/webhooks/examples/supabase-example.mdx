...
<Tip>
  <strong>Kontrollera den kompletta startpaketet för supabase</strong><br />
  Ett minimalt abonnemang startpaket byggt med Next.js, Supabase och Dodo Payments. Denna mall hjälper dig att snabbt ställa in en abonnemangsbaserad SaaS med autentisering, betalningar och webhooks.
...
Supabase tillhandahåller automatiskt `SUPABASE_URL` och `SUPABASE_SERVICE_ROLE_KEY` vid körning.

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **Notera:** Vi kommer att ställa in `DODO_PAYMENTS_WEBHOOK_KEY` efter distribution när du har din webhook-URL.

### 5. Distribuera

Funktionen är redan inställd i `functions/webhook/index.ts` - bara distribuera den:
...
✅ **Händelseloggning** - Fullständig revisionsspårning i `webhook_events` tabellen  
...
2. **Kontrollerar idempotens** - Slår upp webhook-ID för att förhindra duplicerad bearbetning
3. **Loggar händelsen** - Lagrar rå webhook-data i `webhook_events` tabellen
4. **Bearbetar uppdateringar** - Skapar eller uppdaterar kunder och prenumerationer via Supabase-klienten
5. **Hanterar fel** - Loggar misslyckanden och markerar händelsen för försök igen
...
<Note>
Flaggan `--no-verify-jwt` är nödvändig eftersom webhooks inte inkluderar JWT-token. Säkerhet tillhandahålls genom verifiering av webhook-signatur.
</Note>
...
| Databasbehörighetsfel | Se till att använda Service Role Key |
| JWT-verifieringsfel | Distribuera med `--no-verify-jwt` flaggan |
| Funktion hittades inte | Verifiera att projektreferensen är korrekt och att funktionen är distribuerad |
