...
5. Kör frågan
6. Få din anslutningssträng från Neon → Anslutningsdetaljer
...
> **Notis:** Vi kommer att ställa in `DODO_PAYMENTS_WEBHOOK_KEY` efter distribution när du har din webhook-URL.
...
4. Aktivera följande prenumerationsevenemang:
   - `subscription.active`
...
✅ **Idempotens** - Förhindrar dubbelbearbetning med webhook-ID:n  
✅ **Evenemang loggning** - Kompletterande revisionsspår i `webhook_events`-tabellen  
✅ **Felhantering** - Loggat och går att göra om

> **Notis:** Denna implementation demonstrerar hantering av tre kärnprenumerationsevenemang (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) med minimala fält. Du kan enkelt utöka den för att stödja ytterligare evenemangstyper och fält baserat på dina krav.
...
Webhook-hanteraren:
1. **Inaktiverar kroppsanalysering** - För att få tillgång till rå kropp för signaturverifiering
2. **Verifierar signaturen** - Säkerställer att begäran kommer från DodoPayments med HMAC-SHA256
3. **Kontrollerar för dubbletter** - Använder webhook-ID för att förhindra bearbetning av samma evenemang två gånger
4. **Loggar evenemanget** - Lagrar rå webhook i `webhook_events`-tabellen för revisionsspår
5. **Bearbetar evenemanget** - Skapar eller uppdaterar kunder och prenumerationer i Neon
6. **Hanterar fel** - Loggar misslyckanden och markerar evenemang som obehandlad för försök igen
