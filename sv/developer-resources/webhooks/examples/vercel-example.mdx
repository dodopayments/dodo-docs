...
1. Registrera dig för [Neon](https://neon.com)
2. Skapa ett nytt projekt
3. Öppna SQL-redigeraren
4. Kopiera och klistra in innehållet från [`schema.sql`](#database-schema)
5. Kör frågan
6. Hämta din anslutningssträng från Neon → Anslutningsdetaljer
...
> **Notera:** Vi kommer att ställa in `DODO_PAYMENTS_WEBHOOK_KEY` efter distributionen när du har din webhook-URL.
...
1. Gå till [DodoPayments Dashboard](https://app.dodopayments.com) → Utvecklare → Webhooks
2. Skapa en ny webhook-endpoint
3. Konfigurera din webhook-URL som endpoint
4. Aktivera dessa prenumerationsevenemang:
   - `subscription.active`
...
✅ **Signaturverifiering** - Använder dodopayments-biblioteket  
✅ **Idempotens** - Förhindrar duplicerad bearbetning med webhook-ID:n  
✅ **Händelseloggar** - Fullständig revisionsspårning i `webhook_events` tabell  
✅ **Felhantering** - Loggad och kan försöka igen

> **Notera:** Denna implementation demonstrerar hantering av tre kärnprenumerationsevenemang (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) med minimala fält. Du kan enkelt utöka den för att stödja ytterligare händelsetyper och fält baserat på dina krav.
...
Webhook-hanteraren:
1. **Inaktiverar kroppsparsing** - För att få tillgång till rå kropp för signaturverifiering
2. **Verifierar signaturen** - Säkerställer att förfrågan kommer från DodoPayments med HMAC-SHA256
3. **Kontrollerar för dubbletter** - Använder webhook-ID för att förhindra bearbetning av samma händelse två gånger
4. **Loggar händelsen** - Lagrar rå webhook i `webhook_events` tabell för revisionsspårning
5. **Bearbetar händelsen** - Skapar eller uppdaterar kunder och prenumerationer i Neon
6. **Hanterar fel** - Loggar misslyckanden och markerar händelsen som obearbetad för försök igen
