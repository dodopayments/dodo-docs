...
| `revocation_reason` | Utlösare |
|---|---|
| `subscription_cancelled` | Kundens prenumeration avslutades (händelsen `subscription.cancelled`). |
| `subscription_on_hold` | Prenumerationen är pausad på grund av en misslyckad förnyelse (`subscription.on_hold`). Kan återställas: ett lyckat nytt försök utfärdar behörigheten på nytt. |
| `SubscriptionPaused` | Prenumerationen pausades (`subscription.paused`). Kan återställas: återupptagning utfärdar behörigheten på nytt. |
| `subscription_expired` | Prenumerationen nådde slutet av sin löptid (`subscription.expired`). |
| `plan_changed` | Planen ändrades; gamla behörigheter återkallas innan nya utfärdas (`subscription.plan_changed`). |
| `refund` | En återbetalning behandlades för den ursprungliga engångsbetalningen (`refund.succeeded`). |
| `manual` | En handlare återkallade behörigheten via API:et eller instrumentpanelen. Manuella återkallanden utfärdas inte automatiskt på nytt vid förnyelse av prenumerationen. |
| `license_key_disabled` | Licensnyckeln bakom en behörighet av typen licensnyckel inaktiverades. Behörigheten aktiveras automatiskt igen om licensnyckeln återaktiveras. |
| `platform_external` | Integreringens plattformssida hamnade ur synk (till exempel togs en Discord-roll bort manuellt, GitHub App förlorade åtkomst till lagringsplatsen eller en avstämning upptäckte ett saknat mål). Behörigheten utfärdas inte automatiskt på nytt vid förnyelse av prenumerationen förrän det underliggande plattformsproblemet har lösts. |
