...
description: Lär dig hur du integrerar Dodo Payments med ditt Tanstack App
  Router-projekt med hjälp av vår Tanstack Adaptor. Täcker kassa, kundportal,
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Integrera Dodo Payments-kassan i din Tanstack-app.
</Card>
...
<Warning>
Commit aldrig din <code>.env</code>-fil eller hemligheter till versionshantering.
</Warning>
</Step>
</Steps>
...
<Info>
Alla exempel förutsätter att du använder Tanstack App Router.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Använd denna handlare för att integrera Dodo Payments-kassan i din Tanstack-app. Stöder statiska (GET), dynamiska (POST) och sessionbaserade (POST) betalningsflöden.
  </Info>
  <CodeGroup>
...
<Tab title="Customer Portal Handler">
  <Info>
  Använd denna handlare för att låta kunder hantera sina prenumerationer och uppgifter via Dodo Payments kundportal.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Använd denna handlare för att ta emot och bearbeta Dodo Payments webhook-händelser säkert i din Tanstack-app.
  </Info>
  <CodeGroup>
...
<Warning>
Om <code>productId</code> saknas returnerar handlare ett 400-svar. Ogiltiga frågeparametrar resulterar också i ett 400-svar.
</Warning>
...
<Warning>
Dynamic Checkout använder proxyer för de utfasade `POST /payments`- och `POST /subscriptions`-endpoints. Det fortsätter att fungera för befintliga integrationer, men nya integrationer bör använda **Checkout Sessions** nedan.
</Warning>
...
Mer information och en fullständig lista över fält som stöds finns i [Integrationsguide för Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session).
...
Customer Portal Route Handler gör det möjligt att integrera Dodo Payments kundportal sömlöst i din Tanstack-applikation.
...
- **Event Routing:** Anropar rätt eventhanterare baserat på payload-typen.
