...
Fastställer det debiterade beloppet i större valutaenheter (t.ex. <code>12.5</code> för 12,50 USD). Gäller endast Pay What You Want-produkter och ignoreras om beloppet understiger produktens minimipris.
</ParamField>
...
<Warning>
Dynamic Checkout proxar de föråldrade `POST /payments`- och `POST /subscriptions`-endpoints. Det fortsätter att fungera för befintliga integrationer, men nya integrationer bör använda **Checkout Sessions** nedan.
</Warning>
...
Se [integrationsguiden för Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session) för mer information och en komplett lista över fält som stöds.
...
Route Handler för Customer Portal gör det möjligt att sömlöst integrera Dodo Payments kundportal i din Nuxt-applikation.
...
- **Metod:** Endast POST-förfrågningar stöds. Övriga metoder returnerar 405.
...
- **Eventroutning:** Anropar rätt eventhanterare baserat på payload-typen.
