...
Ställ in miljövariabler i din Convex-instrumentpanel (**Inställningar** → **Miljövariabler**). Du kan få tillgång till instrumentpanelen genom att köra:
...
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Din webhook-hemlighet (obligatorisk för webhook-hantering)
...
<Warning>
Innan du använder denna fråga, se till att definiera rätt schema i din `convex/schema.ts`-fil eller ändra frågan för att matcha ditt befintliga schema.
</Warning>
...
<Tab title="Inställning av kundportal">
  <Info>
  Använd denna funktion för att låta kunder hantera sina prenumerationer och detaljer via Dodo Payments kundportal. Kunder identifieras automatiskt via `identify`-funktionen.
  </Info>
  <CodeGroup>
...
<Tab title="Inställning av webhook-hanterare">
  <Info>
  Använd denna hanterare för att ta emot och bearbeta Dodo Payments webhook-händelser på ett säkert sätt i din Convex-app. Alla webhook-hanterare tar emot Convex `ActionCtx` som första parameter, vilket gör att du kan använda `ctx.runQuery()` och `ctx.runMutation()` för att interagera med din databas.
  </Info>
  <CodeGroup>
...
<Info>
Kunden identifieras automatiskt med hjälp av `identify`-funktionen som konfigureras i din DodoPayments-inställning. Denna funktion bör returnera kundens `dodoCustomerId`.
</Info>
