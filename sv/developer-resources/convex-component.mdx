...
{/* LOCKED_PATTERN_076a045a43785e7154357214ce1a1fe2 */}
    Integrera Dodo Payments checkout med sessionbaserat flöde.
</Card>
{/* LOCKED_PATTERN_dbb6954e19619bb187d501d5ccba29a3 */}
    Tillåt kunder att hantera prenumerationer och uppgifter.
</Card>
...
Ställ in miljövariabler i din Convex-instrumentpanel (**Settings** → **Environment Variables**). Du når panelen genom att köra:
...
- `DODO_PAYMENTS_ENVIRONMENT` - Sätt till `test_mode` eller `live_mode`
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Din webhook-hemlighet (krävs för webhook-hantering)

<Warning>
Använd alltid Convex miljövariabler för känslig information. Lämna aldrig ut hemligheter i versionskontrollen.
</Warning>
</Step>
</Steps>
...
Först, skapa en intern fråga för att hämta kunder från din databas. Denna kommer att användas i betalningsfunktionerna för att identifiera kunder.

<Warning>
Innan du använder denna fråga, se till att definiera lämpligt schema i din `convex/schema.ts`-fil eller ändra frågan så att den matchar ditt befintliga schema.
</Warning>
...
{/* LOCKED_PATTERN_cfdbe95b4d26ef82d25c4a1bfd5d5aeb */}
  <Info>
  Använd denna funktion för att integrera Dodo Payments checkout i din Convex-app. Använder sessionbaserad checkout med full funktionsstöd.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_de2bafe43db636b1864521f4f7e5aa25 */}
  <Info>
  Använd denna funktion för att låta kunder hantera sina prenumerationer och uppgifter via Dodo Payments kundportal. Kunden identifieras automatiskt via funktionen `identify`.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_eeaa14c4f31af768a3b7e376ee69b626 */}
  <Info>
  Använd denna hanterare för att ta emot och bearbeta Dodo Payments webhook-händelser säkert i din Convex-app. Alla webhook-hanterare får Convex `ActionCtx` som första parameter, vilket gör att du kan använda `ctx.runQuery()` och `ctx.runMutation()` för att interagera med din databas.
  </Info>
  <CodeGroup>
  ...
  <Warning>
  Se till att definiera motsvarande databasmutationer i din Convex-backend för varje webhook-händelse du vill hantera. Skapa till exempel en <code>createPayment</code>-mutation för att registrera lyckade betalningar eller en <code>createSubscription</code>-mutation för att hantera prenumerationstillstånd.
  </Warning>
</Tab>
</Tabs>
...
<Info>
Dodo Payments Convex-komponenten använder sessionbaserad checkout och erbjuder en säker, anpassningsbar checkout-upplevelse med förkonfigurerade produktkorgar och kunduppgifter. Detta är den rekommenderade metoden för alla betalningsflöden.
</Info>
...
Om det är inställt på <code>true</code> skickas ett mejl till kunden med portallänken.
</ParamField>

<Info>
Kunden identifieras automatiskt med funktionen `identify` som konfigurerats i din DodoPayments-inställning. Denna funktion bör returnera kundens `dodoCustomerId`.
</Info>
...
<Info>
Använd checkout-funktionen från dina React-komponenter med Convex hooks.
</Info>
