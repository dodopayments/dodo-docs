...
Ställ in omdirigerings-URI till: `https://[your-project-ref].supabase.co/auth/v1/callback` i Google Cloud, aktivera sedan Google-leverantören i Supabase Auth med ditt Klient-ID och Hemlig nyckel.
</Step>

<Step title="Konfigurera Dodo Payments">
Generera en API-nyckel och Webhook-hemlighet från Dodo-dashboarden. Ställ in miljön till `test_mode` under utvecklingen.
</Step>
...
Prissättnings-UI:n läser detta `features`-array och renderar det dynamiskt.
</Step>
...
- Supabase Edge-funktion för webhooks (`dodo-webhook`)
- Drizzle ORM-schema och migrationer
- Dashboard med fakturor, prenumerationsstatus och planfunktioner

<Tip>
Behåll `DODO_PAYMENTS_ENVIRONMENT` som `test_mode` tills du slutför tester från början till slut.
</Tip>
...
- Se till att `DODO_WEBHOOK_SECRET` matchar värdet från Dodo-dashboarden
...
- Kontrollera `DATABASE_URL`-syntax och Supabase nätverksutgress
- Vänta ~2–3 minuter efter projekt skapande innan första push
</Accordion>
<Accordion title="OAuth-omdirigering mismatch">
- Omdirigerings-URI måste vara `https://[ref].supabase.co/auth/v1/callback`
- Se till att samma gäller i Google Cloud och Supabase Auth-leverantören
</Accordion>
</AccordionGroup>
