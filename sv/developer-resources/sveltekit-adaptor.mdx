...
description: Lär dig hur du integrerar Dodo Payments med ditt SvelteKit App
  Router-projekt med vår SvelteKit Adaptor. Täcker kassa, kundportal, webhooks
  och säker miljöinställning.
icon: code-merge
keywords:
  - Dodo Payments
  - SvelteKit
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Integrera Dodo Payments-kassan i din SvelteKit-app.
</Card>
...
<Info>
Alla exempel förutsätter att du använder SvelteKit App Router.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Använd den här hanteraren för att integrera Dodo Payments-kassan i din SvelteKit-app. Stöder statiska (GET), dynamiska (POST) och session (POST) betalflöden.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Använd den här hanteraren för att ta emot och behandla Dodo Payments webhook-händelser säkert i din SvelteKit-app.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments stöder tre typer av betalflöden för att integrera betalningar på din webbplats, denna adapter stöder samtliga typer av betalflöden.
</Info>
...
- Skicka parametrarna som JSON-body i en POST-begäran.
- Stöder både engångs- och återkommande betalningar.
- För en komplett lista över stödda POST-body-fält, hänvisa till:
...
<Warning>
Dynamic Checkout proxar de föråldrade `POST /payments`- och `POST /subscriptions`-endpointsen. Det fortsätter att fungera för befintliga integrationer, men nya integrationer bör använda **Checkout Sessions** nedan.
</Warning>
...
Route Handler för Customer Portal gör det möjligt att sömlöst integrera Dodo Payments kundportal i din SvelteKit-applikation.
