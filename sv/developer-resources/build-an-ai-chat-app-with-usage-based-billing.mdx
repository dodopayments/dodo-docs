...
**Evenemangsnamn** → `ai_chat_usage`

<Warning>
Evenemangsnamnet `ai_chat_usage` måste matcha exakt vad vi kommer att skicka från vår applikationskod senare. Evenemangsnamn är skiftläget känsliga!
</Warning>
</Step>
...
<Info>
Vi använder "Summa" eftersom vi vill summera alla tokens som förbrukats under flera chattmeddelanden. SDK:n skickar automatiskt `totalTokens` i varje evenemang.
</Info>
</Step>
...
1. Gå till din Dodo Payments-instrumentpanel, navigera till **Utvecklare** → **API-nycklar**
...
Dessutom, ställ in **Fast pris** till `0` för att säkerställa att kunderna endast debiteras baserat på sin användning, utan grundavgift.
...
**Gratis gräns**: Ange → `10000` (kunder får 10 000 gratis tokens per månad)
...
2. Navigera till **Försäljning** -> **Kunder** i den vänstra sidofältet
3. Hitta kunden du just har skapat (med test-e-post)
...
3. Du borde se det vackra chattgränssnittet
...
**Lösningar:**
1. Verifiera att evenemangsnamnet matchar mätarens konfiguration exakt (skiftlägeskänsligt: `ai_chat_usage`)
2. Kontrollera att kund-ID:t finns i Dodo Payments-instrumentpanels
3. Testa API-nyckeln med ett enkelt API-anrop
4. Kontrollera servrarloggarna för felmeddelanden
</Accordion>
...
<Accordion title="Fel med ogiltig API-nyckel">
**Möjliga orsaker:**
...
- Ta bort eventuella citat runt nycklarna i `.env`-fil
