...
<Warning>
Dynamic Checkout proxar de föråldrade `POST /payments`- och `POST /subscriptions`-slutpunkterna. Det fortsätter att fungera för befintliga integrationer, men nya integrationer bör använda **Checkout Sessions** nedan.
</Warning>
...
Checkout sessions ger en säkrare, hostad checkout-upplevelse som hanterar hela betalningsflödet för både engångsköp och prenumerationer, med full kontroll över anpassningen.
...
Route Handler för Customer Portal gör det möjligt att sömlöst integrera Dodo Payments kundportal i din Remix-applikation.
...
- **Eventdirigering:** Anropar rätt eventhanterare baserat på payload-typen.
