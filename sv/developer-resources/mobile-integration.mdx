...
Varje SDK accepterar en valfri `customization`-parameter på `start(...)` /
`CheckoutParams` som styr utseendet och
beteendet för den inbyggda webbläsarytan – verktygsfältet, knapparna och presentationen. Detta är separat från
checkout-sidans eget tema, som du konfigurerar på serversidan via
[`customization.theme_config`](/developer-resources/checkout-session) i
checkout-session.

Alternativen är grupperade efter plattform eftersom Androids Custom Tab och iOS:s
`SFSafariViewController` exponerar olika inbyggda kontroller. Alla fält är
valfria; om `customization` utelämnas helt används varje plattforms standardutseende.
...
Låter verktygsfältet fällas ihop vid rullning. Visas endast när `presentationStyle` är `fullScreen` – `pageSheet` håller fälten fästa oavsett denna inställning.
</ParamField>
...
- **Säkerhet**: Skicka aldrig en API key i din app. Skapa checkout sessions på din backend och skicka endast den resulterande `checkout_url` till klienten.
- **Auktoritet**: Behandla `CheckoutResult.status` som en UI-hint. Bevilja åtkomst först efter att din backend har bekräftat betalningen.
- **Användarupplevelse**: Visa ett laddningstillstånd medan din backend skapar sessionen och hantera `cancelled` som ett normalt resultat i stället för ett fel.
- **Testning**: Använd testläge och testkort, och verifiera retur-URL:ens round trip på en riktig enhet såväl som i en simulator.

## Felsökning

### Vanliga problem

- **Callback anländer aldrig**: Schemat i `returnUrl` måste matcha det du registrerade. På Android är det `dodoCallbackScheme` i manifest placeholder; på iOS och React Native är det `Info.plist` URL type.
- **Checkout återgår till webbläsaren i stället för din app (iOS)**: Du har inte vidarebefordrat den inkommande URL:en. Anropa `DodoCheckout.handleOpenURL(url)` från `.onOpenURL`, `scene(_:openURLContexts:)` eller en `Linking`-lyssnare i React Native.
- **`PLATFORM_ERROR` på Android**: Oftast beror det på en schemamissmatchning. Det kan också inträffa om din `MainActivity` anger `android:taskAffinity=""` (standardvärdet för `flutter create`), vilket gör att vissa OEM-versioner kan tappa den pågående checkouten.
- **`ALREADY_IN_PROGRESS`**: En checkout är fortfarande öppen. Vänta på eller avvisa den föregående innan du startar en ny.
- **Bygget misslyckas med en olöst placeholder**: Du lade till Android SDK men angav aldrig `manifestPlaceholders["dodoCallbackScheme"]`.
...
- [Guide för Payment Integration](/developer-resources/integration-guide)
...
<Check>
Kontakta <a href="mailto:support@dodopayments.com">support@dodopayments.com</a> om du har frågor eller behöver support.
</Check>