...
5. **Webhook-verifiering i alla SDK:er**  
   Alla SDK:er inkluderar nu inbyggda funktioner för webhook-verifiering som säkert validerar inkommande webhook-händelser. Drivna av den branschstandardiserade [webhooks-biblioteket](https://github.com/standard-webhooks) gör dessa funktioner det enkelt att verifiera och bearbeta webhook-payloads.
   
   Två metoder finns tillgängliga:
   - `unwrap()`: Verifierar webhook-signaturer med din webhook-hemliga nyckel för produktionsanvändning
   - `unsafe_unwrap()`: Parser webhook-payloads utan verifiering för lokal testning och utveckling
   
   För att använda webhook-verifiering, ange din `DODO_PAYMENTS_WEBHOOK_KEY` när du initierar Dodo Payments-klienten. Båda funktionerna parser automagiskt webhook-payloaden och extraherar händelsetypen, vilket gör webhook-hantering säkrare och mer bekväm.
