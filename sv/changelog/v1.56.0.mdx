...
5. **Webhook-verifiering i alla SDK:er**  
   Alla SDK:er har nu inbyggda webhook-verifieringsfunktioner för att säkert validera inkommande webhook-händelser. Drivet av den branschstandardiserade [webhooks-biblioteket](https://github.com/standard-webhooks), gör dessa funktioner det enkelt att verifiera och bearbeta webhook-payloads.
   
   Två metoder är tillgängliga:
   - `unwrap()`: Verifierar webhook-signaturer med hjälp av din webhook-hemliga nyckel för produktionsanvändning
   - `unsafe_unwrap()`: Parserar webhook-payloads utan verifiering för lokal testning och utveckling
   
   För att använda webhook-verifiering, ange din `DODO_PAYMENTS_WEBHOOK_KEY` när du initierar Dodo Payments-klienten. Båda funktionerna parserar automatiskt webhook-payloaden och extraherar händelsetypen, vilket gör webhook-hanteringen säkrare och mer bekväm.
