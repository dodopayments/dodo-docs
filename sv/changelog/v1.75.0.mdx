...
### 5. **`invoice_url` Fält i Betalningssvar och Betalningslänkssvar**  
   ...
   Betalningssvaret och Betalningslänkssvaret inkluderar nu ett `invoice_url` fält, som ger direkt åtkomst till fakturan kopplad till varje betalning.
...
### 7. **Förbättrade Filtreringsmöjligheter för Kunder, Rabatter och Licensnycklar**  
   ...
   **Kunder API Filter**
   ...
   | `name` | Filtrera efter kundens namn (delvis matchning, skiftlägeskänsligt) |
   | `created_at_gte` | Filtrera kunder skapade på eller efter denna tidsstämpel |
   ...
   **Rabatter API Filter**
   ...
   | Filter | Beskrivning |
   |--------|-------------|
   | `code` | Filtrera efter rabattkod (delvis matchning, skiftlägeskänsligt) |
   | `discount_type` | Filtrera efter rabatttyp (procent) |
   | `active` | Filtrera efter aktiv status (true = ej utgången, false = utgången) |
