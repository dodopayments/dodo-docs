...
- **Subscription object**: Nytt `cancellation_feedback`-fält (ett av värdena ovan) och `cancellation_comment` (valfri fritext), fylls i när kunden avslutar prenumerationen
- **`subscription.cancelled` webhook**: Båda fälten inkluderas i payloaden
- **API**: Skicka `cancellation_feedback` och `cancellation_comment` till `PATCH /subscriptions/{subscription_id}` när en avslutning schemaläggs eller utförs programmatiskt
...
Du kan också åsidosätta handlarens standardinställning för enskilda betalningar, debiteringar för on-demand-prenumerationer och planändringar med hjälp av den booleska variabeln `adaptive_currency_fees_inclusive`:
