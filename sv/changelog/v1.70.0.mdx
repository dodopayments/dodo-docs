...
1. **Inline Checkout - Fullt inbäddade utcheckningsupplevelser (BETA)**  
   ...
   Initiera SDK:en med `displayType: 'inline'` ochEmbed checkout i en behållare:
   ...
   - **I realtid uppdelningsevenemang**: Ta emot `checkout.breakdown` evenemang när kunder anger sina uppgifter
   - **Anpassade ordersammanfattningar**: Bygg din egen prissättning som uppdateras i realtid
   - **Sömlös integration**: Checkout-ramen hanterar betalningsinsamling medan du kontrollerar layouten
   - **SDK-metoder**: Använd `open()`, `close()`, och `isOpen()` för att styra checkout programatiskt
...
2. **Skapa betalningar, prenumerationer och checkout med befintliga betalningsmetoder**  
    Effektivisera checkoutflöden genom att använda sparade betalningsmetoder från befintliga kunder. Den nya `payment_method_id`-parametern gör att du kan skapa betalningar, prenumerationer eller checkoutsessioner med en kunds tidigare sparade betalningsmetod.
   ...
   Använd `payment_method_id` när du skapar checkoutsessioner, betalningar eller prenumerationer:
   ...
   <Warning>
   När du använder `payment_method_id` i checkoutsessioner, måste `confirm` sättas till `true`, och en befintlig `customer_id` måste anges. Betalningsmetoden kommer att valideras för berättigande med betalningens valuta.
   </Warning>
...
5. **`redirect_immediately` Flagga - Hoppa över betalningsframgångssidan**  
   Kontrollera checkoutflödet med den nya `redirect_immediately`-flaggan. När den är aktiverad omdirigeras kunderna omedelbart efter betalningens slutförande, vilket undviker den standardiserade framgångssidan för en snabbare, mer strömlinjeformad upplevelse.
  ...
  ```
   ...
   <Tip>
   Använd `redirect_immediately: true` när du har en anpassad framgångssida som ger en bättre användarupplevelse än den standardiserade betalningsframsidan.
   </Tip>

   <Note>
   När `redirect_immediately` är aktiverad omdirigeras kunderna till din `return_url` omedelbart efter betalningens slutförande, vilket helt hoppar över den standardiserade framgångssidan.
   </Note>
