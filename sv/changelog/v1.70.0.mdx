...
1. **Inline Checkout - Fullt inbäddade utcheckningsupplevelser (BETA)**  
   ...
   Initiera SDK:n med `displayType: 'inline'` och bädda in kassan i ett behållarelement:
   ...
   - **Händelser för realtidsuppdelning**: Ta emot `checkout.breakdown` händelser när kunder anger sina uppgifter
   - **Anpassade ordersammanfattningar**: Bygg din egen prissättning som uppdateras i realtid
   - **Sömlös integration**: Kassaram handhar betalningsinsamling medan du kontrollerar layouten
   - **SDK-metoder**: Använd `open()`, `close()`, och `isOpen()` för att styra kassan programmässigt
...
2. **Skapa betalningar, prenumerationer och kassor med befintliga betalningsmetoder**  
    Effektivisera kassaflöden genom att använda sparade betalningsmetoder från befintliga kunder. Den nya `payment_method_id` parametern gör att du kan skapa betalningar, prenumerationer eller kassa-sessioner med en kunds tidigare sparade betalningsmetod.
   ...
   Använd `payment_method_id` när du skapar kassa-sessioner, betalningar eller prenumerationer:
   ...
   <Warning>
   När du använder `payment_method_id` i kassa-sessioner, måste `confirm` sättas till `true`, och en befintlig `customer_id` måste tillhandahållas. Betalningsmetoden kommer att valideras för berättigande med betalningens valuta.
   </Warning>
...
5. **`redirect_immediately` Flagga - Hoppa över betalningsframgångssidan**  
   Kontrollera kassaflödet med den nya `redirect_immediately` flaggan. När den är aktiverad omdirigeras kunder omedelbart efter betalningsslutförande, vilket gör att de hoppar över den standardiserade framgångssidan för en snabbare, mer strömlinjeformad upplevelse.
  ...
  ```
   ...
   <Tip>
   Använd `redirect_immediately: true` när du har en anpassad framgångssida som ger en bättre användarupplevelse än den standardiserade betalningsframgångssidan.
   </Tip>

   <Note>
   När `redirect_immediately` är aktiverad, omdirigeras kunder till din `return_url` omedelbart efter betalningsslutförande, vilket helt hoppar över den standardiserade framgångssidan.
   </Note>
