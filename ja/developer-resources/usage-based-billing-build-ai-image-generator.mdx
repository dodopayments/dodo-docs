...
**説明**: これをコピー → `Tracks each AI image generation request made by customers using our DALL-E powered service`

**イベント名**: これは重要です - 正確にコピー → `image.generated`

<Warning>
イベント名 `image.generated` は、後でアプリケーションコードから送信する内容と正確に一致する必要があります。イベント名は大文字と小文字を区別します！
</Warning>
</Step>
...
2. **フィルタロジック**: **OR** を選択します（これは「これらの条件のいずれかが真であればカウントする」という意味です）
3. **最初の条件を追加**:
   - プロパティキー: `quality`
   - 比較子: `equals`  
   - 値: `standard`
4. **「条件を追加」をクリック**して2つ目のものを追加します:
   - プロパティキー: `quality`
   - 比較子: `equals`
   - 値: `hd`
...
**製品名**: これをコピー → `PixelGen AI - Image Generation`

**説明**: これをコピー → `AI-powered image generation service with pay-per-use billing`
...
さらに、顧客が使用に基づいてのみ課金され、基本料金が発生しないように、**固定価格**を `0` に設定します。
...
**ユニットあたりの価格**: 入力 → `0.05` （これは画像1枚あたり$0.05です）

**無料のしきい値**: 入力 → `10` （顧客は月に10枚の画像を無料で取得します）
...
1. Dodo Payments ダッシュボードに戻ります
2. 左のサイドバーで **顧客** をクリックします
3. 作成した顧客（テストメールで）を見つけます
4. 顧客IDをコピーします - `cus_abc123def456` のように見えます
