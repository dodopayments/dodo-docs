...
依存関係をあなたの`build.gradle.kts`に追加してください:
...
<Tip>
最新のDodo Payments機能にアクセスするために、常に最新のSDKバージョンを使用してください。最新バージョンについては[Maven Central](https://central.sonatype.com/artifact/com.dodopayments.api/dodo-payments-kotlin)を確認してください。
</Tip>

<Info>
SDKはJava 8以降を必要とし、JVMおよびAndroidプラットフォームの両方と互換性があります。
</Info>
...
`Result`を使用して、機能的なエラーハンドリングを行います:
...
<Tip>
Kotlinの`runCatching`を使用して、Result型を用いたより機能的なエラーハンドリングを行ってください。
</Tip>
