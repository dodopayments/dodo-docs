...
<Info>
`billing` は、最低でも2文字のISO `country` コードが必要です。既存の顧客を添付するには`AttachExistingCustomer` を使用し、新しい顧客を作成するには`NewCustomer` を使用してください。`productPrice` は最小の通貨単位で表されます。
</Info>
...
<Tip>
Kotlinの `runCatching` を使用して、Resultタイプでより機能的なエラーハンドリングを行います。
</Tip>

## Android統合

Androidアプリケーションと一緒に使用します：
...
Kotlin SDKのサポートが必要ですか？

- **Discord**: リアルタイムサポートのために[コミュニティサーバー](https://discord.gg/bYqAp4ayYh)に参加
- **Email**: support@dodopayments.comにご連絡ください
- **GitHub**: [リポジトリ](https://github.com/dodopayments/dodopayments-kotlin)で問題をオープンする

## 貢献

貢献を歓迎します！[貢献ガイドライン](https://github.com/dodopayments/dodopayments-kotlin/blob/main/CONTRIBUTING.md)をチェックして始めましょう。
