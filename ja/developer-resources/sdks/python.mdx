...
<Info>
`billing`には、少なくとも2文字のISO国コードが必要です。`customer`には、既存の顧客を関連付けるための`{"customer_id": ...}`、または新しい顧客を作成するための`{"email": ..., "name": ...}`のいずれかを指定できます。`product_price`は、通貨の最小単位で指定します。
</Info>
...
パフォーマンスを向上させる非同期処理にaiohttpを使用します。
...
Python SDKについてサポートが必要ですか？

- **Discord**: リアルタイムのサポートを受けるには、[コミュニティサーバー](https://discord.gg/bYqAp4ayYh)に参加してください
- **メール**: support@dodopayments.comまでお問い合わせください
- **GitHub**: [リポジトリ](https://github.com/dodopayments/dodopayments-python)でissueを作成してください

## コントリビューション

コントリビューションを歓迎します！始めるには、[コントリビューションガイドライン](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md)を確認してください。
