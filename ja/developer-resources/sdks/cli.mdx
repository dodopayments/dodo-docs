...
`dodo wh trigger` は、Dodo Payments が配信する全 46 種類のイベントのモックペイロードを送信できます。

| カテゴリ | イベント |
|----------|--------|
| **サブスクリプション** | `active`, `updated`, `on_hold`, `paused`, `renewed`, `plan_changed`, `cancelled`, `failed`, `expired`, `update_payment_method` |
| **支払い** | `succeeded`, `failed`, `processing`, `cancelled` |
| **払い戻し** | `succeeded`, `failed` |
| **異議申し立て** | `opened`, `expired`, `accepted`, `cancelled`, `challenged`, `won`, `lost` |
| **ライセンスキー** | `created` |
| **売上金の振込** | `created`, `in_progress`, `on_hold`, `failed`, `success` |
| **クレジット** | `added`, `deducted`, `expired`, `rolled_over`, `rollover_forfeited`, `overage_charged`, `overage_reset`, `manual_adjustment`, `balance_low` |
| **放棄されたチェックアウト** | `detected`, `recovered` |
