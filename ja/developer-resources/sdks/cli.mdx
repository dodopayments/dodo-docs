...
`dodo wh trigger` は、これら22種類のイベントタイプのモックペイロードを送信できます。
...
<Note>
これは、Dodo Paymentsが配信するイベントの一部です。`payout.*`、`credit.*`、`abandoned_checkout.*`、`dunning.*`、`entitlement_grant.*`、`subscription.paused`、および `subscription.update_payment_method` にはまだモックジェネレーターがありません。実際のテストモードでのアクティビティで実行し、`dodo wh listen` を使用してください。完全なリストについては、[Webhook Events](/developer-resources/webhooks/intents/webhook-events-guide)をご覧ください。
</Note>
...
|----------|-------------|
| `DODO_WH_TEST_SERVER_URL` | `dodo wh listen` が使用するデフォルトのWebhookリレーサーバーURLを上書きします |

## Updates

CLIは起動時に新しいバージョンがないか確認し、利用可能な場合はステータスバーに通知を表示します。アップグレードするには、次のようにします。

```text
/update
```

または、インストーラーを再実行して、その場でアップグレードします。
...
- **GitHub**: [リポジトリ](https://github.com/dodopayments/dodopayments-cli/issues)でissueを作成
