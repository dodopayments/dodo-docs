...
<Info>
イベントは冪等性のために一意の `event_id` 値を持たなければなりません。同じリクエスト内での重複IDは拒否され、既存IDでの後続リクエストは無視されます。
</Info>
...
DodoPayments APIのリストメソッドはページネーションされています。すべてのページを通じて項目を反復処理するには、`for await … of` 構文を使用できます：
