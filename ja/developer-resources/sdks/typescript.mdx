...
<Info>
イベントは冪等性のためにユニークな `event_id` 値を持つ必要があります。同じリクエスト内の重複IDは拒否され、既存のIDを持つ後続のリクエストは無視されます。
</Info>
...
DodoPayments APIのリストメソッドはページネーションされています。`for await … of` 構文を使用して、すべてのページを通じてアイテムを反復処理できます:
...
- ウェブブラウザ（最新のChrome、Firefox、Safari、Edgeなど）
- Node.js 20 LTS以降のバージョン（[非EOL](https://endoflife.date/nodejs)）
- Deno v1.28.0以上
- Bun 1.0以降
- Cloudflare Workers
- Vercel Edge Runtime
