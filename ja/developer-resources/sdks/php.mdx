...
SDKは名前付きパラメータを使用してオプションの引数を指定します。静的な`with`コンストラクタを使用してバリューオブジェクトを初期化できます：
...
ライブラリがAPIに接続できない場合や、成功以外のステータスコード（4xxまたは5xx）を受信した場合、`APIException`のサブクラスがスローされます：
