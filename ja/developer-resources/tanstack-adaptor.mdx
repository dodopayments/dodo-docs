...
請求額を主要通貨単位で固定します（例：$12.50 の場合は <code>12.5</code>）。Pay What You Want products のみが対象で、商品最低価格を下回る場合は無視されます。
</ParamField>
...
<Warning>
Dynamic Checkoutは非推奨の`POST /payments`および`POST /subscriptions`エンドポイントをプロキシします。既存のインテグレーションは引き続き動作しますが、新しいインテグレーションでは以下の**Checkout Sessions**を使用してください。
</Warning>

### レスポンス形式
Dynamic checkoutは、チェックアウトURLを含むJSONレスポンスを返します。
...
Checkout sessionsは、より安全なホスト型チェックアウト体験を提供します。完全にカスタマイズでき、単発購入とサブスクリプションの両方で決済フロー全体を処理します。
...
### レスポンス形式
Checkout sessionsは、チェックアウトURLを含むJSONレスポンスを返します。
...
Customer Portal Route Handlerを使用すると、Dodo PaymentsのCustomer PortalをTanstackアプリケーションにシームレスに統合できます。

### クエリパラメータ
<ParamField query="customer_id" type="string" required>
ポータルセッションのcustomer ID（例：<code>?customer_id=cus_123</code>）。
</ParamField>
...
<Warning>
<code>customer_id</code>が指定されていない場合は400を返します。
</Warning>

## Webhook Route Handler

- **Method:** POSTリクエストのみサポートされます。その他のMethodには405を返します。
- **Signature Verification:** <code>webhookKey</code>を使用してWebhookの署名を検証します。検証に失敗した場合は401を返します。
- **Payload Validation:** Zodで検証されます。無効なPayloadには400を返します。
- **Error Handling:**
  - 401: 無効な署名
  - 400: 無効なPayload
  - 500: 検証中の内部エラー
- **Event Routing:** Payloadのタイプに基づいて、適切なイベントハンドラーを呼び出します。
