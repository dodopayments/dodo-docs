...
請求額を主要通貨単位で固定します（例：$12.50 の場合は <code>12.5</code>）。Pay What You Want products のみが対象で、product の最低価格を下回る場合は無視されます。
</ParamField>
<ParamField query="showDiscounts" type="boolean">
割引フィールドを表示します。
</ParamField>
...
<Warning>
Dynamic Checkout は非推奨となった `POST /payments` エンドポイントと `POST /subscriptions` エンドポイントのプロキシとして機能します。既存のインテグレーションでは引き続き動作しますが、新しいインテグレーションでは以下の **Checkout Sessions** を使用してください。
</Warning>
...
Checkout sessions は、完全にカスタマイズできる、より安全なホスト型チェックアウト体験を提供します。1回限りの購入とサブスクリプションの両方で、決済フロー全体に対応します。

詳細およびサポートされているフィールドの完全な一覧については、[Checkout Sessions Integration Guide](https://docs.dodopayments.com/developer-resources/checkout-session) を参照してください。
...
Customer Portal Route Handler を使用すると、Dodo Payments の Customer Portal を Nuxt アプリケーションにシームレスに統合できます。
...
<Warning>
<code>customer_id</code> が指定されていない場合は 400 を返します。
</Warning>

## Webhook Route Handler

- **Method:** POST リクエストのみがサポートされています。それ以外のメソッドには 405 を返します。
- **Signature Verification:** <code>webhookKey</code> を使用して webhook の署名を検証します。検証に失敗した場合は 401 を返します。
- **Payload Validation:** Zod で検証します。無効なペイロードには 400 を返します。
- **Error Handling:**
  - 401: 無効な署名
  - 400: 無効なペイロード
  - 500: 検証中の内部エラー
- **Event Routing:** ペイロードのタイプに基づいて、適切なイベントハンドラーを呼び出します。
