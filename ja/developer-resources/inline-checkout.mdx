...
<Info>
ドメインの検証は**インライン（埋め込み）チェックアウト**にのみ必要です。オーバーレイチェックアウト、ホスト型チェックアウト、支払いリンクには必要ありません。
</Info>

Apple Payはダッシュボード上でドメインごとに検証されます。

<Steps>
<Step title="Open Wallet domains">
**設定 → 支払い方法** に移動し、**Apple Pay** の行で **ドメインの管理** をクリックします。

<Frame caption="Open Wallet domains from the Apple Pay row">
  <img src="/images/apple-pay-domains/manage-domains.png" alt="支払い方法設定のApple Pay行にあるドメイン管理ボタン" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Download the domain association file">
ウォレットドメインパネルから関連ファイルをダウンロードします。

<Frame caption="Download the Apple Pay domain association file">
  <img src="/images/apple-pay-domains/wallet-domains-panel.png" alt="ファイルのダウンロードボタンがあるウォレットドメインパネル" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Register your domain">
**ドメインを登録** をクリックし、インラインチェックアウトを埋め込むドメインを入力します（例: `shop.example.com`）、その後 **続ける** をクリックします。

<Frame caption="Register the domain where you embed inline checkout">
  <img src="/images/apple-pay-domains/register-domain.png" alt="ドメインが入力されたドメイン登録フォーム" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
HTTPSで提供され、リダイレクトなしで到達でき、`Content-Type: application/octet-stream` または `text/plain` で提供される必要があります。
</Step>

<Step title="Verify the domain">
**ドメインを確認** をクリックします。Dodo Paymentsはファイルがライブであることを確認し、Appleにドメインを提出します。

<Frame caption="Verify the hosted association file">
  <img src="/images/apple-pay-domains/verify-domain.png" alt="関連ファイルのホストパスとドメインの確認ボタンが表示されたドメイン確認画面" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Confirm it's active">
ステータスが**アクティブ**と表示されたら、そのドメインでApple Payが有効になります。**有効** トグルを使用して、ドメインごとにオンまたはオフにできます。

<Frame caption="Verified domains show an Active status">
  <img src="/images/apple-pay-domains/domains-active.png" alt="アクティブなApple Payステータスと有効トグルが表示されたウォレットドメインリスト" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Test the integration">
1. Appleデバイスでチェックアウトを開く
2. Apple Payボタンが表示されることを確認
3. テストトランザクションを完了
</Step>
</Steps>
...
| 特徴 | インラインチェックアウト | オーバーレイチェックアウト |
|---------|-----------------|------------------|
| 統合の深さ | ページに完全に埋め込まれる | ページの上にモーダル |
| レイアウト制御 | 完全な制御 | 制限あり |
| ブランド | シームレス | ページから切り離された |
| 実装の労力 | 高い | 低い |
| 最適なユースケース | カスタムチェックアウトページ、高コンバージョンフロー | 迅速な統合、既存ページ |

<Tip>
チェックアウト体験とシームレスなブランド設計を最大限に制御したい場合は、**インラインチェックアウト** を使用します。既存のページに最小限の変更で迅速に統合したい場合は、**オーバーレイチェックアウト** を使用します。
</Tip>
...
<Card title="Overlay Checkout" icon="layer-group" href="/developer-resources/overlay-checkout">
    モーダルベースの迅速な統合にはオーバーレイチェックアウトを使用します。
</Card>

<Card title="Checkout Sessions API" icon="code" href="/api-reference/checkout-sessions/create">
    チェックアウト体験を実現するためにチェックアウトセッションを作成します。
</Card>

<Card title="Webhooks" icon="webhook" href="/developer-resources/webhooks">
    サーバーサイドでの支払いイベントをWebhookで処理します。
</Card>

<Card title="Integration Guide" icon="book" href="/developer-resources/integration-guide">
    Dodo Paymentsの統合に関する完全ガイド。
</Card>
</CardGroup>

さらなる支援が必要な場合は、[Discordコミュニティ](https://discord.gg/bYqAp4ayYh)を訪れるか、開発者サポートチームに連絡してください。
