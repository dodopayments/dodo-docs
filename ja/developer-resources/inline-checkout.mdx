...
description: "Dodo Payments checkout
  をウェブサイトに直接埋め込み、シームレスでブランドに適した決済体験を提供します。"
icon: credit-card
keywords:
  - "Dodo Payments"
  - 開発者ガイド
...
| フィールド | 説明 |
|-------|-------------|
| `subTotal` | 割引や税が適用される前の、session 内のすべての line item の合計。 |
| `discount` | 適用されたすべての割引の合計額。 |
| `tax` | 計算された税額。`inline` mode では、ユーザーが住所フィールドを操作すると動的に更新されます。 |
| `total` | session の基準通貨における `subTotal - discount + tax` の計算結果。 |
| `currency` | 標準の小計、割引、税額に使用される ISO 通貨コード（例: `"USD"`）。 |
| `finalTotal` | 顧客に実際に請求される金額。基本的な価格内訳に含まれない為替調整や現地決済手数料が含まれる場合があります。 |
| `finalTotalCurrency` | 顧客が実際に支払う通貨。購買力平価や現地通貨換算が有効な場合、`currency` と異なることがあります。 |
...
<Info>
ドメイン検証が必要なのは **inline（埋め込み）checkout** のみです。ホスト型 checkout では必要ありません。
</Info>
...
**Settings → Payment Methods** に移動し、**Apple Pay** 行の **Manage domains** をクリックします。
...
Wallet domains パネルから association file をダウンロードします。
...
**Register domain** をクリックし、inline checkout を埋め込むドメイン（例: `shop.example.com`）を入力して **Continue** をクリックします。
...
HTTPS 経由で提供し、リダイレクトなしで到達可能にする必要があります。また、`Content-Type: application/octet-stream` または `text/plain` とともに提供してください。
</Step>

<Step title="Verify the domain">
**Verify domain** をクリックします。Dodo Payments がファイルの公開状態を確認し、Apple にドメインを送信します。
...
ステータスが **Active** になると、そのドメインで Apple Pay が有効になります。**Enabled** toggle を使用して、ドメインごとに有効または無効にできます。
...
1. Apple デバイスで checkout を開きます
2. Apple Pay button が表示されることを確認します
3. テスト取引を完了します
</Step>
</Steps>
...
## Inline Checkout と Overlay Checkout の比較

用途に適した checkout タイプを選択してください。

| 機能 | Inline Checkout | Overlay Checkout |
|---------|-----------------|------------------|
| 統合の深さ | ページに完全埋め込み | ページ上のモーダル |
| レイアウト制御 | 完全な制御 | 制限あり |
| ブランディング | シームレス | ページから分離 |
| 実装の手間 | 大きい | 小さい |
| 最適な用途 | カスタム checkout page、高コンバージョンのフロー | 迅速な統合、既存ページ |

<Tip>
チェックアウト体験を最大限に制御し、シームレスなブランディングを実現したい場合は **inline checkout** を使用してください。既存ページを最小限の変更で迅速に統合する場合は **overlay checkout** を使用します。
</Tip>
...
<Card title="Overlay Checkout" icon="layer-group" href="/developer-resources/overlay-checkout">
    迅速なモーダルベースの統合には overlay checkout を使用します。
</Card>

<Card title="Checkout Sessions API" icon="code" href="/api-reference/checkout-sessions/create">
    checkout experience を支える checkout session を作成します。
</Card>

<Card title="Webhooks" icon="webhook" href="/developer-resources/webhooks">
    webhooks を使用してサーバー側で決済イベントを処理します。
</Card>
