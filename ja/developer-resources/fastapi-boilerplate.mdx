...
- `DODO_PAYMENTS_WEBHOOK_KEY` が `.env` に正しく設定されていることを確認してください
- 署名検証には raw request body を使用していることを確認してください
- 3 つすべての `webhook-id`、`webhook-timestamp`、`webhook-signature` ヘッダーを `webhook.verify()` に渡してください — Standard Webhooks の署名は body だけでなく `id.timestamp.body` を対象とします
</Accordion>
</AccordionGroup>
