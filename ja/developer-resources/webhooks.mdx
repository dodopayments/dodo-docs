...
<Expandable title="Data object properties">
  ...
  リソースのタイプ。次のいずれかです：`Payment`、`Subscription`、`Refund`、`Dispute`、`LicenseKey`、`CreditLedgerEntry`、`CreditBalanceLow`、`AbandonedCheckout`、`DunningAttempt`、または`EntitlementGrant`。
  </ResponseField>
...
<Warning>
`dodo wh trigger`からのモックWebhook payloadには署名がありません。テスト時のWebhook handlerでは、`unwrap()`ではなく`unsafe_unwrap()`を使用してください。
</Warning>
...
<Step title="Access Rate Limit Settings">
    **Advanced**タブで「Rate Limit (throttling)」セクションを探します。
</Step>

<Step title="Configure Rate Limit">
    **Edit**ボタンをクリックしてrate limit settingsを変更します。
    
    <Info>
    デフォルトでは「No rate limit」が適用され、イベントは発生するとすぐに配信されます。
    </Info>
</Step>
...
<Step title="Add Custom Header">
    「Custom Headers」セクションで、custom headerの**Key**と**Value**を入力します。
</Step>
...
<Step title="Enable Transformations">
    **Enabled**スイッチをオンにしてtransformation機能を有効にします。
</Step>

<Step title="Configure Transformation">
    **Edit transformation**をクリックしてtransformation rulesを定義します。
    
    <Info>
    JavaScriptを使用してWebhook payloadを変換し、異なるtarget URLを指定できます。
    </Info>
</Step>
...
<Step title="Navigate to Alerting Settings">
  Dodo Payments Dashboardに移動し、**Dashboard → Webhooks → Alerting**に進みます。
</Step>

<Step title="Enable Email Notifications">
  **Email notifications**をオンにして、Webhook配信の問題に関するアラートの受信を開始します。
</Step>
...
<Card title="Create Webhook" icon="code" href="/api-reference/webhooks/create-webhook">
  Webhook endpointsをプログラムで作成・設定するためのAPIリファレンス
</Card>
<Card title="List Webhooks" icon="list" href="/api-reference/webhooks/list-webhooks">
  Webhook endpointsを取得・管理するためのAPIリファレンス
</Card>
</CardGroup>