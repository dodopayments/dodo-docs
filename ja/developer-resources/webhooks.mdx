...
<Step title="Access Webhook Settings">
    DodoPayments Dashboard に移動し、`Developer > Webhooks` に進みます。
</Step>
...
<Step title="Navigate to Webhook Details">
    Dodo Payments Dashboard にアクセスし、`Developer > Webhooks` に移動します。
</Step>
...
この webhook をトリガーしたイベントタイプ（例: `payment.succeeded`、`subscription.active`）。
</ResponseField>
...
{/* LOCKED_PATTERN_e00062f1a73be4cdce7478c87f820958 */}
  Webhook エンドポイントをプログラムによって作成および構成するための API 参照
</Card>
{/* LOCKED_PATTERN_ebcc49f16b467c5d38d8a9f461e25ccd */}
  Webhook エンドポイントを取得および管理するための API 参照
</Card>
</CardGroup>
