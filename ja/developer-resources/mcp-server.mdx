...
[Dodo Agent Plugin](/developer-resources/build-with-ai-coding-agents) は、Claude Code、Codex CLI、Cursor、VS Code / GitHub Copilot、Kiro、OpenCode 向けに、MCP サーバーと 17 個すべてのスキルを 1 ステップでインストールします。Gemini CLI では agent-skill primitive がないため、2 つの MCP サーバーのみがインストールされます。エージェントごとのインストールコマンドについては、[AI Coding Agents ガイド](/developer-resources/build-with-ai-coding-agents)を参照してください。
