...
<Note>
Node.js 18以上が必要です。`mcp-remote`パッケージがリモートMCPサーバーへの接続を管理します。
</Note>
...
3. **クライアントログをチェック**してエラーメッセージを確認
4. **Node.jsバージョンを確認**: Node.js 18+が必要
...
<Tip>
最新バージョンを常に取得するには`@latest`を使用するか、安定のために特定のバージョンを`@1.0.0`のように固定してください。
</Tip>
...
クリック後、Cursorの`mcp.json`に環境変数を設定します。**Cursor Settings** > **Tools & MCP** > **New MCP Server**からアクセスできます。
...
<Tab title="Claude Desktop">
Dodo Payments MCPサーバーをClaude Desktopで設定し、支払いデータへの会話アクセスを有効にします。
<Steps>
...
クリック後、VS Codeの`mcp.json`に環境変数を設定します。**Command Palette** > **MCP: Open User Configuration**からアクセスできます。

**手動設定**

<Steps>
<Step title="MCP設定を開く">
    コマンドパレットを開き（`Cmd/Ctrl + Shift + P`）、"MCP: Open User Configuration"を検索します。
</Step>
...
<Step title="ウィンドウを再読み込み">
    VS Codeを再読み込みします（`Cmd/Ctrl + Shift + P` > "Reload Window"）して、MCPサーバーを有効にします。
</Step>
</Steps>
</Tab>

<Tab title="Cline (VS Code)">
Dodo Payments MCPサーバーをCline (VS Code)で設定し、支払いデータへの会話アクセスを有効にします。
<Steps>
...
<Step title="VS Codeを再読み込み">
    VS Codeウィンドウを再読み込みします（`Cmd/Ctrl + Shift + P` > "Reload Window"）して、MCPサーバー接続を有効にします。
</Step>
</Steps>
</Tab>
...
| `DODO_PAYMENTS_ENVIRONMENT` | 本番用には`live_mode`に設定 | いいえ |
...
| `x-dodo-payments-api-key` | 直接APIキーのヘッダー |
...
<Tip>
詳細な機能情報を取得するには`npx dodopayments-mcp --describe-capabilities`を実行してください。
</Tip>
...
<Tip>
データの誤った変更を防ぐために、開発中は`--operation=read`を使用してください。
</Tip>
...
1. **`list_api_endpoints`** - オプションフィルタリングを伴う利用可能なメソッドを発見
2. **`get_api_endpoint_schema`** - 特定のメソッドの詳細なスキーマを取得
3. **`invoke_api_endpoint`** - 適切なパラメータで任意のメソッドを実行

<Info>
動的ツールは、50以上のメソッドを持つAPIに対して自動的に有効になります。`--tools=all`でオーバーライドしてすべてのツールを含めます。
</Info>
...
リモートで展開する場合、常に`Authorization`ヘッダーまたは`x-dodo-payments-api-key`ヘッダーを介して認証を要求してください。
...
`DODO_PAYMENTS_ENVIRONMENT`が正しく設定されていることを確認してください（本番用には`live_mode`）。
...
一部のクライアントはすべてのJSONスキーマ機能をサポートしていません。`--describe-capabilities`を使用してサポートされている内容を理解してください。
</Accordion>
</AccordionGroup>
