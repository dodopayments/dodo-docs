...
`convex.config.ts` を編集した後、必要なタイプを生成するために `npx convex dev` を一度実行してください。
</Step>
<Step title="環境変数の設定">
Convex ダッシュボード (**Settings** → **Environment Variables**) で環境変数を設定します。ダッシュボードには次のコマンドを実行してアクセスできます:
...
- `DODO_PAYMENTS_ENVIRONMENT` - `test_mode` または `live_mode` に設定します
- `DODO_PAYMENTS_WEBHOOK_SECRET` - あなたの webhook シークレット (webhook 処理に必要)
...
<Tab title="カスタマーポータルの設定">
  <Info>
  この関数を使用すると、顧客が Dodo Payments カスタマーポータル経由で自分のサブスクリプションや詳細を管理できるようになります。顧客は `identify` 関数を介して自動的に識別されます。
  </Info>
  <CodeGroup>
...
<Tab title="Webhook ハンドラーの設定">
  <Info>
  このハンドラーを使用して、Dodo Payments の webhook イベントを安全に受信し、Convex アプリで処理します。すべての webhook ハンドラーは最初のパラメータとして Convex `ActionCtx` を受け取り、`ctx.runQuery()` と `ctx.runMutation()` を使用してデータベースと対話できます。
  </Info>
  <CodeGroup>
...
<Info>
顧客は、DodoPayments 設定に構成された `identify` 関数を使用して自動的に識別されます。この関数は顧客の `dodoCustomerId` を返す必要があります。
</Info>
