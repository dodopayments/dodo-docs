...
<Card title="顧客ポータル" icon="user" href="#customer-portal-function">
...
編集後に `convex.config.ts` を実行し、必要な型を生成するために `npx convex dev` を一度実行してください。
</Step>
<Step title="環境変数の設定">
Convex ダッシュボードで環境変数を設定します (**Settings** → **Environment Variables**)。ダッシュボードには次のコマンドを実行してアクセスできます:
...
次の環境変数を追加してください:
- `DODO_PAYMENTS_API_KEY` - あなたの Dodo Payments API キー
- `DODO_PAYMENTS_ENVIRONMENT` - `test_mode` または `live_mode` に設定
- `DODO_PAYMENTS_WEBHOOK_SECRET` - あなたのウェブフックシークレット（ウェブフック処理に必要）

<Warning>
常に機密情報にはConvex環境変数を使用してください。シークレットをバージョン管理にコミットしないでください。
</Warning>
</Step>
</Steps>
...
<Warning>
このクエリを使用する前に、適切なスキーマをあなたの `convex/schema.ts` ファイルに定義するか、クエリを既存のスキーマに合わせて変更してください。
</Warning>
...
<Tab title="カスタマーポータルの設定">
  <Info>
  この関数を使用して、顧客が Dodo Payments カスタマーポータルを通じてサブスクリプションや詳細を管理できるようにします。顧客は自動的に `identify` 関数を通じて識別されます。
  </Info>
  <CodeGroup>
...
<Tab title="ウェブフックハンドラーの設定">
  <Info>
  このハンドラーを使用して、Convex アプリ内で Dodo Payments のウェブフックイベントを安全に受信および処理します。すべてのウェブフックハンドラーは、最初のパラメータとして Convex `ActionCtx` を受け取り、`ctx.runQuery()` および `ctx.runMutation()` を使用してデータベースと対話できます。
  </Info>
  <CodeGroup>
...
<Info>
顧客は、DodoPayments 設定で構成された `identify` 関数を使用して自動的に識別されます。この関数は顧客の `dodoCustomerId` を返す必要があります。
</Info>
