---
title: Astro Adaptor
description: Astro App RouterプロジェクトにAstro Adaptorを使用して<CardGroup cols={2}>を統合する方法を説明します。checkout、customer portal、webhooks、セキュアな環境設定について解説します。
icon: code-merge
keywords:
  - Dodo Payments
  - Astro
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    AstroアプリにDodo Payments checkoutを統合します。
</Card>
...
<Warning>
<code>.env</code>ファイルやシークレットをバージョン管理システムにコミットしないでください。
</Warning>
</Step>
</Steps>
...
<Info>
すべての例では、Astro App Routerを使用していることを前提としています。
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  このhandlerを使用して、AstroアプリにDodo Payments checkoutを統合します。static（GET）、dynamic（POST）、session（POST）のpayment flowをサポートします。
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  このhandlerを使用して、AstroアプリでDodo Payments webhookイベントを安全に受信して処理します。
  </Info>
  <CodeGroup>
...
<Info>
Dodo Paymentsは、Webサイトに決済を統合するための3種類のpayment flowをサポートしており、このadaptorはすべてのpayment flowに対応しています。
</Info>

- **Static Payment Links:** すぐに共有できるURLで、コードなしですばやく決済を受け付けます。
- **Dynamic Payment Links:** APIまたはSDKを使用して、カスタム詳細を含むpayment linkをプログラムで生成します。
- **Checkout Sessions:** 事前設定された商品カートと顧客情報を使用し、カスタマイズ可能で安全なcheckout体験を作成します。
...
請求額を主要通貨単位で固定します（例：12.50ドルの場合は<code>12.5</code>）。Pay What You Want製品でのみ使用され、製品の最低価格を下回る場合は無視されます。
</ParamField>
...
- パラメータをPOSTリクエストのJSON bodyとして送信します。
- one-time決済とrecurring決済の両方をサポートします。
- サポートされているPOST bodyフィールドの完全な一覧については、次を参照してください:
...
Checkout sessionsは、より安全なホスト型checkout体験を提供します。one-time購入とサブスクリプションの完全な決済フローを処理し、柔軟にカスタマイズできます。

詳細とサポートされているフィールドの完全な一覧については、[Checkout Sessions Integration Guide](https://docs.dodopayments.com/developer-resources/checkout-session)を参照してください。
...
Customer Portal Route Handlerを使用すると、Dodo Payments customer portalをAstroアプリケーションにシームレスに統合できます。
...
- **Error Handling:**
  ...
  - 400: 無効なpayload
  - 500: 検証中の内部エラー
- **Event Routing:** payloadの種類に基づいて適切なevent handlerを呼び出します。
