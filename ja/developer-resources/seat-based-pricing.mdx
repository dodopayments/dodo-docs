...
<Step title="環境ファイルを作成">
Dodo Payments APIキーを使用して、`.env`ファイルを作成します:
...
<Step title="チェックアウトセッションの作成を実装">
次のコードを使用して、`src/server.ts`ファイルを作成します:
...
<Step title="シンプルなWebインターフェースを追加">
簡単にテストできるために、`public/index.html`ファイルを作成します:
...
<Step title="静的ファイルを配信">
HTMLファイルを配信するために、これを`src/server.ts`に追加します:
...
<Check>
**静的ファイルを構成しました！** デモインターフェースを見るには`http://localhost:3000`にアクセスしてください。
</Check>
</Step>
</Steps>
...
<Steps>
<Step title="サーバーを起動">
1. 正しいAPIキーを持つ`.env`ファイルを持っていることを確認してください
2. 実際の値で製品およびアドオンIDをコードに更新してください。Dodo Paymentsダッシュボードから取得した値
3. サーバーを起動します:
...
<Check>
サーバーが正常に起動し、「サーバーがhttp://localhost:3000 で実行中」と表示されるはずです。
</Check>
</Step>

<Step title="ウェブインターフェースをテストする">

<Frame>
  <img src="/images/cookbooks/seat-based/seat-based-billing-ui.png" alt="基本サブスクリプション製品の作成" />
</Frame>

1. ブラウザを開き、`http://localhost:3000`に移動します
2. シートベースの価格設定デモインターフェースが表示されるはずです
3. 異なるシート数（0、3、10など）を試してください
4. 各数量に対して「チェックアウトリンクを生成」をクリックします
...
<Step title="WebhookをリッスンしてDBを更新する">
