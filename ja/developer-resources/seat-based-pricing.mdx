...
あなたの Dodo Payments API キーを使って `.env` ファイルを作成します:
...
<Step title="シンプルなウェブインターフェースを追加">
...
<Step title="静的ファイルを提供">
この内容をあなたの `src/server.ts` に追加して HTML ファイルを提供します:
...
<Check>
**静的ファイルが設定されました！** デモインターフェースを確認するには `http://localhost:3000` を訪れてください。
</Check>
</Step>
</Steps>
...
1. 正しい API キーを持つ `.env` ファイルがあることを確認してください
2. Dodo Payments ダッシュボードから実際の値で製品およびアドオン ID をコードに更新してください
3. サーバーを起動します:
...
3. 異なるシート数 (0, 3, 10 など) を試してください
4. 各数量のために「チェックアウトリンクを生成」をクリックします
5. チェックアウト URL が正しく生成されていることを確認します
