...
2. **動的金額を渡す**: チェックアウトセッションを作成する際に、プロダクトカートに `amount` フィールドを含めます。
3. **顧客に選択させる**: 金額が提供されない場合、顧客は自分の価格（設定された範囲内）を入力できます。

<Info>
プロダクトカートに `amount` を渡すと、その金額がチェックアウトに使用されます。 `amount` フィールドを省略すると、顧客はチェックアウト時に自分の価格を選択できます（最低/最高設定に従う）。
</Info>
...
**例**: 最低価格が $5.00 の場合、`5.00`（または `500` セント）を入力します。
</Step>
...
<Step title="商品を保存">
**商品追加**をクリックして保存します。チェックアウトセッションで使用するために、あなたのプロダクトID（例: `pdt_123abc456def`）に注目してください。
</Step>
</Steps>
...
Pay What You Want で商品が設定されると、動的金額を使ってチェックアウトセッションを作成できます。プロダクトカートの `amount` フィールドを利用して、各チェックアウトセッションの価格をプログラムで設定できます。

### 金額フィールドの理解

チェックアウトセッションを作成する際には、各プロダクトカートアイテムに `amount` フィールドを含めることができます：

- **`amount` が提供される場合**: チェックアウトはこの正確な金額を使用します（最低/最高の範囲内である必要があります）
- **`amount` が省略される場合**: 顧客はチェックアウト時に自分の価格を入力できます（設定された範囲内）

<Warning>
`amount` フィールドは、Pay What You Want 商品にのみ適用されます。通常の商品にはこのフィールドは無視されます。
</Warning>
...
<Note>
**金額フォーマット**: `amount` フィールドは、通貨の最小単位である必要があります。USDの場合、これはセントを意味します（例えば、$25.00 = `2500`）。他の通貨の場合、最小単位を使用してください（例: INR の場合はパイセ）。
</Note>

## ステップ3：顧客に自分の価格を選ばせる

顧客にチェックアウト時に自分の価格を選択させたい場合は、単にプロダクトカートから `amount` フィールドを省略します。チェックアウトページには、顧客が最低および最高の範囲内で任意の金額を入力できる入力フィールドが表示されます。
...
あなたの `amount` フィールドが無視されている場合、以下を確認してください:
- ダッシュボードで **Pay What You Want** が有効になっている商品であること
- 商品が **単一支払い**（一回限り）商品であり、定期購入でないこと
- 金額が正しいフォーマットであること（通貨の最小単位、例: USD の場合はセント）
</Accordion>

<Accordion title="金額が最大を超えるか、最低を下回る">
APIは、金額が商品価格の範囲に違反しているチェックアウトセッションを拒否します。チェックアウトセッションを作成する前に金額を常に検証するか、`amount` フィールドを省略して顧客に価格を選択させてください。
</Accordion>

<Accordion title="顧客が自分の価格を入力できない">
顧客が価格入力フィールドを見ていない場合、製品カートから `amount` フィールドを省略したことを確認してください。 `amount` が提供された場合、チェックアウトはその正確な金額を使用します。
</Accordion>
</AccordionGroup>