...
私たちのデータベース同期機能は、**CLI**を介してまたはプログラム的に**コード**内で使用できます。どちらの方法も同じ機能を提供しますので、ワークフローに最適な方法を選択してください。
...
**インタラクティブモード**: 引数なしでコマンドを実行するだけで、インタラクティブセットアップウィザードが開始されます。
...
リクエストのレート制限（秒あたりのリクエスト数）。Dodo Payments APIを圧倒しないように、同期エンジンがAPIリクエストを行う速度を制御します。
</ParamField>
...
<Tip>
`interval`オプションは、自動同期のために`.start()`を使用する場合に必要です。同期は、プロセスが停止されるまで指定された間隔で継続的に実行されます。
</Tip>
...
<Tip>
手動同期を使用する場合、`interval`オプションは必要ありません。同期操作を実行する必要があるときに`.run()`を呼び出すことができます。
</Tip>
...
<Warning>
**MongoDB**: `dodopayments_sync`という名前のデータベースが自動的にデータベースサーバーに作成されます。すべての同期データはそこに保存されます。このデータベース名は現在固定されており、変更することはできません。
