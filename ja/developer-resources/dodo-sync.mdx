...
これらのエンティティの任意の組み合わせを、`scopes` パラメータに指定して同期できます。すべての同期操作はインクリメンタルであり、最適なパフォーマンスのために新しいまたは更新されたレコードのみを転送します。
...
同期するデータエンティティのカンマ区切りリスト。使用可能なスコープ: `licences`, `payments`, `customers`, `subscriptions`。例: `"payments,customers"`。
</ParamField>
...
<Tip>
`interval` オプションは、`.start()` を自動同期で使用する場合に必須です。同期は、プロセスが停止されるまで指定されたインターバルで継続的に実行されます。
</Tip>
...
<Tip>
手動同期を使用する場合、`interval` オプションは必須ではありません。必要なときに `.run()` を呼び出すことで同期操作を実行できます。
</Tip>

#### PostgreSQLの例

こちらは、PostgreSQLで `dodo-sync` を使用する方法です:
...
こちらは、MySQLで `dodo-sync` を使用する方法です:
...
こちらは、ClickHouseで `dodo-sync` を使用する方法です:
...
同期するエンティティの配列。利用可能なオプション: `"licences"`, `"payments"`, `"customers"`, `"subscriptions"`。これらの任意の組みネーションを含めることができます。
</ParamField>
...
自動同期の間隔を秒単位で指定します。`.start()` を使用する自動同期時に必要です。`.run()` を使用する手動同期時にはオプションです。
</ParamField>
...
<Warning>
**MongoDB**: `dodopayments_sync` という名前のデータベースが自動的にあなたのデータベースサーバーに作成されます。すべての同期データはそこに保存されます。このデータベース名は現在固定されており、変更することはできません。

**PostgreSQL**: テーブル (`Subscriptions`, `Payments`, `Licenses`, `Customers`) は、あなたの接続URIで指定されたデータベースに作成されます。データはJSONB形式で保存されます。

**MySQL**: テーブル (`Subscriptions`, `Payments`, `Licenses`, `Customers`) は、あなたの接続URIで指定されたデータベースに作成されます。データはJSON形式で保存されます。

**ClickHouse**: テーブル (`Subscriptions`, `Payments`, `Licenses`, `Customers`) はReplacingMergeTreeエンジンを使用して作成されます。クエリを実行する際は、重複のない結果を得るために `FINAL` キーワードを使用してください。
</Warning>
