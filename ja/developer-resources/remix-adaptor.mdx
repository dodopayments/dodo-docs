...
<Card title="チェックアウトハンドラー" icon="cart-shopping" href="#checkout-route-handler">
    Dodo PaymentsのチェックアウトをRemixアプリに統合します。
</Card>
...
<Info>
すべての例は、Remixアプリルーターを使用していることを前提としています。
</Info>

<Tabs>
<Tab title="チェックアウトハンドラー">
  <Info>
  このハンドラーを使用して、Dodo PaymentsのチェックアウトをRemixアプリに統合します。静的（GET）、動的（POST）、およびセッション（POST）支払いフローをサポートします。
  </Info>
  <CodeGroup>
...
チェックアウトセッションは、一回限りの購入とサブスクリプションの完全な支払いフローを処理する、安全でホストされたチェックアウト体験を提供します。
...
顧客ポータルルートハンドラーを使用すると、Dodo Paymentsの顧客ポータルをRemixアプリケーションにシームレスに統合できます。
