...
<Warning>
Dynamic Checkout は、非推奨となった `POST /payments` および `POST /subscriptions` エンドポイントをプロキシします。既存のインテグレーションでは引き続き動作しますが、新しいインテグレーションでは以下の **Checkout Sessions** を使用してください。
</Warning>

### Response Format
Dynamic checkout は、checkout URL を含む JSON レスポンスを返します：
...
Checkout sessions は、ワンタイム購入とサブスクリプションの両方に対応し、決済フロー全体を処理する、より安全でホスト型の checkout エクスペリエンスを提供します。また、完全なカスタマイズ制御が可能です。
...
### Response Format
Checkout sessions は、checkout URL を含む JSON レスポンスを返します：
...
Customer Portal Route Handler を使用すると、Dodo Payments の customer portal を Remix アプリケーションにシームレスに統合できます。

### Query Parameters
<ParamField query="customer_id" type="string" required>
ポータルセッションの customer ID（例：<code>?customer_id=cus_123</code>）。
</ParamField>
...
- **Error Handling:**
  ...
  - 400: 無効な payload
  - 500: 検証中の内部エラー
- **Event Routing:** payload のタイプに基づいて、適切な event handler を呼び出します。
