...
Checkout sessions は、完全にカスタマイズを制御でき、単回購入とサブスクリプションの両方で決済フロー全体を処理する、より安全なホスト型チェックアウト体験を提供します。
...
Customer Portal Route Handler を使用すると、Dodo Payments のカスタマーポータルを Next.js アプリケーションにシームレスに統合できます。
...
- **Payload Validation:** Zod で検証されます。ペイロードが無効な場合は 400 を返します。
- **Error Handling:**
  - 401: 署名が無効です
  - 400: ペイロードが無効です
  - 500: 検証中に内部エラーが発生しました
