...
<Warning>
このイベント名 `ai_chat_usage` は、後でアプリケーションコードから送信するものと正確に一致する必要があります。イベント名は大文字と小文字が区別されます！
</Warning>
</Step>
...
**集計期間**: タイプ → `totalTokens`

**測定単位**: タイプ → `tokens`

<Info>
私たちは「合計」を使用しています。なぜなら、複数のチャットメッセージにわたって消費されたすべてのトークンを合計したいからです。SDKは各イベントで自動的に `totalTokens` を送信します。
</Info>
</Step>
...
1. Dodo Paymentsダッシュボードで、**開発者** → **APIキー**に移動します。
2. **APIキーを作成**をクリックします。
3. APIキーをコピーします - それは `test_abc123...` のように見えます。
...
1. [aistudio.google.com](https://aistudio.google.com/api-keys)を訪問します
2. **APIキーを取得**をクリックします
3. 新しいAPIキーを作成するか、既存のものを使用します
4. キーをコピーします

<Note>
このキーを安全に保管してください - それを `.env` ファイルにも追加します。
</Note>
</Step>
</Steps>
...
メーターを接続する前に、製品の価格タイプとして**使用ベースの請求**を選択していることを確認してください。

さらに、**固定価格**を `0` に設定し、顧客が利用に基づいてのみ料金が請求され、基本料金が発生しないようにします。
...
**単位あたりの価格**: 入力 → `0.00001` (これは1,000トークンあたり$0.01、またはトークンあたり$0.00001です)

**無料閾値**: 入力 → `10000` (顧客は月に10,000の無料トークンを受け取ります)
...
2. 左のサイドバーで**販売** -> **顧客**に移動します。
3. 作成したばかりの顧客（テストメールの顧客）を見つけます。
4. 顧客IDをコピーします - それは `cus_123` のように見えます。
...
さあ、美しいチャットインターフェースを追加して、完全な会話履歴を実装しましょう！ `public/index.html` を作成します:
...
1. `.env` ファイルにすべてのAPIキーが含まれていることを確認します（ステップ2）
2. 開発サーバーを起動します:
...
<Note>
`server.ts` で、ステップ4からの実際のテスト顧客IDを使用して `CUSTOMER_ID` を更新することを忘れないでください。
</Note>
</Step>
...
**ソリューション:**
1. イベント名がメーター設定と正確に一致することを確認します（大文字小文字を区別します: `ai_chat_usage`）
2. 顧客IDがDodo Paymentsダッシュボードに存在することを確認します
3. 簡単なAPIコールでAPIキーをテストします
4. エラーメッセージのためにサーバーログを確認します
</Accordion>
...
**考えられる原因:**
- 環境の誤ったAPIキー
- `.env` ファイル内の余分なスペースや引用符

**ソリューション:**
- テストキーが `test_` で始まり、ライブキーが `live_` で始まることを確認します
- `.env` ファイル内のキーの周囲に引用符がないことを確認します
- 必要に応じて新しいキーを生成します
</Accordion>
</AccordionGroup>
