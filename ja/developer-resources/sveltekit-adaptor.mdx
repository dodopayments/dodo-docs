...
description: Dodo Payments を SvelteKit
  アプリルータープロジェクトに統合する方法を学びます。チェックアウト、顧客ポータル、ウェブフック、および安全な環境設定について説明します。
icon: code-merge
---
...
<Info>
すべての例は、SvelteKit アプリルーターを使用していることを前提としています。
</Info>

<Tabs>
<Tab title="チェックアウトハンドラー">
  <Info>
  このハンドラーを使用して、Dodo Payments のチェックアウトを SvelteKit アプリに統合します。静的 (GET)、動的 (POST)、およびセッション (POST) の支払いフローをサポートします。
  </Info>
  <CodeGroup>
...
<Tab title="ウェブフックハンドラー">
  <Info>
  このハンドラーを使用して、Dodo Payments のウェブフックイベントを安全に受信して処理します。
  </Info>
  <CodeGroup>
...
顧客ポータルルートハンドラーを使用すると、Dodo Payments の顧客ポータルを SvelteKit アプリケーションにシームレスに統合できます。
