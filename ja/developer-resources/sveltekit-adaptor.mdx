...
### Response Format
Dynamic checkoutはcheckout URLを含むJSONレスポンスを返します：
...
Checkout sessionsは、one-time purchaseとsubscriptionの両方について完全なpayment flowを処理する、より安全なホスト型checkout体験を提供します。カスタマイズも完全に制御できます。

詳細とサポートされるフィールドの完全な一覧については、[Checkout Sessions Integration Guide](https://docs.dodopayments.com/developer-resources/checkout-session)を参照してください。

### Response Format
Checkout sessionsはcheckout URLを含むJSONレスポンスを返します：
...
Customer Portal Route Handlerを使用すると、Dodo Payments customer portalをSvelteKitアプリケーションにシームレスに統合できます。
...
<code>true</code>に設定すると、portal linkを顧客にメールで送信します。
</ParamField>
...
- **Method：** POSTリクエストのみ対応しています。その他のmethodには405を返します。
