...
<Info>
カスタムフィールドに対する顧客の回答には以下が含まれます:
- **Webhooks**: `payment.succeeded`、`subscription.active`、および他の関連するイベントペイロードに`custom_field_responses`配列が含まれています
...
<Info>
カスタムフィールドの回答は、Webhookペイロード（`payment.succeeded`、`subscription.active`など）に自動的に含まれ、API経由で取得できます。これらを活用してCRMを強化したり、オンボーディングフローを起動したり、顧客体験をカスタマイズしたりしてください。
</Info>
