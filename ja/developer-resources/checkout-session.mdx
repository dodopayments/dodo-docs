...
**フォーマット**: 通貨の最小単位で表されます（例：USDのセント）。例えば、$1.00を請求するには、`100`を渡します。
</ParamField>

クレジット権限のあるこの製品にすでに付与されているクレジットを、チェックアウトセッションごとに上書きします。これを利用して、単一のセッションに対し、製品のデフォルトの500ではなく1,000のAPIクレジットを提供するプロモーショナルバンドルのように、異なる数のクレジットを付与します。別途製品を作成することなく実現できます。

各`credit_entitlement_id`はすでに製品に紐付けられている必要があります。このフィールドは、このセッションが完了したときに付与される`amount`のみを上書きします。製品レベルのクレジット権限設定（有効期限、繰越など）は引き続き適用されます。
</Info>

上書きするクレジット権限のID。既に製品に付与されている必要があります。
</ParamField>

このチェックアウトセッションに付与するクレジット数。製品レベルの`amount`を上書きします。ゼロより大きい必要があります。
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
**製品IDの検索**: 製品IDは、Dodo PaymentsダッシュボードのProducts → View Detailsで確認するか、[List Products API](/api-reference/products/get-products)を使用して確認できます。
</Tip>

### オプションのフィールド

これらのフィールドを構成して、チェックアウト体験をカスタマイズし、ビジネスロジックを支払いフローに追加します。

顧客情報。既存顧客をIDで紐付けるか、チェックアウト時に新しい顧客レコードを作成することができます。

既存の顧客IDを使用して、チェックアウトセッションに既存の顧客を接続します。

既存顧客をチェックアウトセッションにIDで紐付けます。

既存の顧客の一意の識別子。これを使用して、新しい顧客を作成する代わりに、既存の顧客にチェックアウトセッションを紐付けます。
</ParamField>
</Expandable>
</Tab>

チェックアウト時に新しい顧客レコードを作成します。

顧客のメールアドレス。新しい顧客を作成する際に必要です。
</ParamField>

領収書や請求書に記載されるべき顧客のフルネームです。

**例**: `John Doe`または`山田 太郎`
</ParamField>

国際形式の顧客の電話番号。一部の支払い方法や不正防止に必要です。

**フォーマット**: 国コードを含めます。例: `+1-800-555-0199`（米国番号）
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
税金の正確な計算、不正防止、規制の遵守のための請求先住所の情報。

`required_billing_address`が`true`に設定されている場合、すべての請求先住所フィールドはセッションの成功した作成に必要となります。
</Info>

住宅番号、通りの名前、アパート/ユニット番号を含む完全な住所。

**例**: `123 Main St`または`東京都渋谷区...`
</ParamField>

市または自治体の名前。

**例**: `San Francisco`, `渋谷区`, `ロンドン`
</ParamField>

州、県、または地域の名前。省略形または通常の名称を使用。

**例**: `CA`または`カリフォルニア`, `東京都`
</ParamField>

2文字のISO国コード（ISO 3166-1 alpha-2）。請求住所が提供される場合、常に必要です。

**例**: `US`（アメリカ）, `CA`（カナダ）, `GB`（イギリス）, `DE`（ドイツ）

サポートされている国とISOコードの完全リストを表示
</Card>
</ParamField>

国の要件に基づく郵便番号、ZIPコード、または同等なもの。

**例**: `94103`（US）、`H3A 1G1`（カナダ）、`SW1A 1AA`（UK）
</ParamField>
</Expandable>
</ParamField>

この設定は、特定の市場やビジネス要件に最適化できます。
