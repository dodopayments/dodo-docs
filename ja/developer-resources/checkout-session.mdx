...
**一般的なオプション**: `credit`, `debit`, `upi_collect`, `apple_pay`, `google_pay`, `amazon_pay`, `klarna`, `affirm`, `afterpay_clearpay`, `cashapp`, `ach`, `multibanco`, `bancontact_card`, `eps`, `ideal`, `blik`, `paypal`。これはすべての値を網羅しているわけではありません。受け付けられるすべての値については、[Create Checkout Session API reference](/api-reference/checkout-sessions/create)を参照してください。
...
チェックアウトセッションに、1つ以上の割引コードを順番に適用します。コードは **配列の順序** に従って適用されます（最初のコードが開始価格を割り引き、2つ目のコードが割引後の価格をさらに割り引く、というように適用されます）。1セッションあたり最大 **20** 個のコードまで適用できます。[Purchasing Power Parity](/features/purchasing-power-parity) が有効な場合、開始価格には基本価格ではなく、PPP調整後の金額が使用されます。
...
<Note>
プレビューされた `current_breakup.subtotal` には、商品に適用される [Purchasing Power Parity](/features/purchasing-power-parity) と [Charm Pricing](/features/charm-pricing) がすでに反映されています。
</Note>

<Note>
カートにサブスクリプション商品が含まれている場合、プレビューのレスポンスには `next_billing_date` も返されます。これは、サブスクリプションの作成前に表示できる、次回請求日のプレビューです。現在時刻を基準に計算されます。トライアルが適用される場合は `now + trial period`、それ以外の場合は `now + one payment frequency` です。買い切り商品のみのカートでは、このフィールドは省略されます。これはプレビュー時点を基準とした推定値です。正式な `next_billing_date` は、サブスクリプションが有効になった時点で設定されます。
</Note>

<Note>
プレビューには `trial_period_days`（無料または有料の実際のトライアル期間）と、`trial_amount`（割引適用後の、価格通貨の最小単位で表した1ユニットあたりのトライアル料金）も返されます。`trial_amount` は [有料トライアル](/features/subscription#paid-trials) の場合にのみ存在し、無料トライアルまたはトライアルなしの場合は `null` です。本日実際に支払う必要がある、税金を含む合計額には `current_breakup` を使用してください。
</Note>
...
## Dynamic Links から Checkout Sessions への移行

### 主な違い
以前は、Dynamic Links を使用して支払いリンクを作成する際、顧客の請求先住所を完全な形で指定する必要がありました。

Checkout Sessions では、これは不要になりました。手元にある情報をそのまま渡すだけで、残りの処理は自動的に行われます。例：

- 顧客の請求先国だけがわかっている場合は、その情報だけを指定します。
- チェックアウトフローによって不足している詳細情報が自動的に収集されてから、顧客が支払いページに進みます。
- 一方、必要な情報がすべて揃っていて、支払いページに直接進ませたい場合は、完全なデータセットを渡し、リクエストボディに `confirm=true` を含めます。

### 移行プロセス

Dynamic Links から Checkout Sessions への移行は簡単です。

<Steps>
<Step title="Update your integration">
新しい API または SDK メソッドを使用するようにインテグレーションを更新します。
</Step>

<Step title="Adjust request payload">
Checkout Sessions の形式に従ってリクエストペイロードを調整します。
</Step>

<Step title="That's it!">
はい。お客様側で追加の処理や特別な移行手順を行う必要はありません。
</Step>
</Steps>
...
<Card title="Create Checkout Session" icon="code" href="/api-reference/checkout-sessions/create">
  利用可能なすべてのパラメータとオプションを使用して Checkout Sessions を作成するための完全な API リファレンス
</Card>
<Card title="Preview Checkout Session" icon="eye" href="/api-reference/checkout-sessions/preview">
  セッション作成前に、価格、税金、合計額をプレビューするための API リファレンス
</Card>
</CardGroup>