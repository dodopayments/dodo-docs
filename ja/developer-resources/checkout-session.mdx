...
これにより、不要なフォームフィールドを排除することで、チェックアウトの摩擦が大幅に軽減されます。

<Tip>
迅速なチェックアウト完了のために最小限の住所を有効にしてください。完全な請求情報が必要なビジネス向けには、フル住所収集も利用可能です。
</Tip>
</ParamField>
...
**例**: `"en"` は英語用、 `"es"` はスペイン語用
</ParamField>
</Expandable>
</ParamField>

<ParamField body="feature_flags" type="object">
チェックアウトセッションの特定の機能と動作を構成します。
...
既存の顧客情報を更新するのではなく、新しい顧客レコードの作成を強制します。
</ParamField>
...
顧客への初回請求のための製品価格。指定しない場合は、製品の保存された価格が使用されます。

**形式**: 通貨の最小単位で表されます（例: USDの場合はセント）。例えば、$1.00を請求するには、 `100` を渡します。
</ParamField>

<ParamField body="product_currency" type="string">
製品価格のオプション通貨。指定しない場合は、製品の通貨がデフォルトとなります。
</ParamField>

<ParamField body="product_description" type="string">
請求およびラインアイテムのためのオプションの製品説明の上書き。指定しない場合は、製品の保存された説明が使用されます。
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
適応通貨手数料を製品価格に含めるか（true）、または上乗せするか（false）。適応価格設定が有効でない場合は無視されます。
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
<Note>
`billing_currency` オーバーライドは、アカウント設定で適応通貨が有効になっている場合にのみ有効になります。適応通貨が無効の場合、このパラメータは効果がありません。
</Note>
...
### 主な違い
以前は、ダイナミックリンクで支払いリンクを作成する際に、顧客の完全な請求住所を提供する必要がありました。

チェックアウトセッションでは、これがもはや必要ありません。お持ちの情報をそのまま渡すだけで、残りは私たちが処理します。例えば:

- 顧客の請求国しかわからない場合は、それを提供するだけです。
- チェックアウトフローは、顧客を支払いページに移動させる前に不足している詳細を自動的に収集します。
- 一方、必要な情報がすべて揃っていて、支払いページに直接スキップしたい場合は、フルデータセットを渡し、リクエストボディに `confirm=true` を含めることができます。
