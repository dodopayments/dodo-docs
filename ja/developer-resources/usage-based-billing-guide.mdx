...
<Tabs>
<Tab title="ANDロジック">
すべての条件が真である必要があります。複数の厳格な基準を同時に満たす必要がある場合に使用します。

**例:** `user_tier = "premium"` と `endpoint = "/api/v2/users"` の間でAPI呼び出しをカウントします
</Tab>

<Tab title="ORロジック">
少なくとも1つの条件が真である必要があります。複数の基準のいずれかを満たすイベントを含めたい場合に使用します。

**例:** `method = "POST"` または `method = "PUT"` または `method = "DELETE"` の間でイベントをカウントします
</Tab>
</Tabs>
...
<Step title="プロパティキーの構成">
イベントメタデータからのプロパティ名を指定します。
</Step>

<Step title="コンパレータの選択">
利用可能な演算子から選択してください:
- `equals` - 完全一致
- `not equals` - 除外フィルタ
- `greater than` - 数値比較  
...
- `contains` - 文字列に部分文字列が含まれる
- `does not contain` - 文字列除外フィルタ
</Step>
...
**例**: UNITあたり`$0.50`の設定は次のようになります:
- 消費された1,000ユニット = 1,000 × $0.50 = 500.00請求
- 消費された500ユニット = 500 × $0.50 = 250.00請求
- 消費された100ユニット = 100 × $0.50 = 50.00請求
</ParamField>
...
<Step title="設定を保存">
メーターと価格設定の構成を確認し、**変更を保存**をクリックして設定を確定します。
...
<ParamField path="現在の月" type="metric">
...
<Frame>
<img src="/images/guides/usage-based-billing/meter-quantities-chart.png" alt="使用傾向を示すメーター数量チャート（紫のグラデーションビジュアライゼーション）" />
</Frame>
...
フィルタ設定（ANDロジックを使用）:
- プロパティ: `plan_type`, コンパレータ: `equals`, 値: `premium`
- プロパティ: `hour`, コンパレータ: `greater_than_or_equal`, 値: `9`
- プロパティ: `hour`, コンパレータ: `less_than`, 値: `17`
...
結果：営業時間中（午前9時 - 午後5時）のプレミアムAPIコールがカウントされます。
</Tab>
</Tabs>
