...
**例:** `user_tier = "premium"` および `endpoint = "/api/v2/users"` のAPIコールをカウントする
</Tab>

<Tab title="ORロジック">
少なくとも1つの条件が真である必要があります。複数の基準のいずれかを満たすイベントを含めたい場合に使用します。

**例:** `method = "POST"` または `method = "PUT"` または `method = "DELETE"` のイベントをカウントする
</Tab>
</Tabs>
...
**例**: `$0.50` あたりの単位設定は次のようになります:
- 1,000ユニット消費 = 1,000 × $0.50 = 500.00 請求
- 500ユニット消費 = 500 × $0.50 = 250.00 請求
- 100ユニット消費 = 100 × $0.50 = 50.00 請求
</ParamField>
