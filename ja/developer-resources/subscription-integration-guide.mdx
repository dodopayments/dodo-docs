...
|----------|------|----------|-------------|
| `business_id` | string | Yes | ビジネスの一意の識別子 |
| `timestamp` | string | Yes | イベントが発生した時点のタイムスタンプ（配信された時点と必ずしも同じではありません） |
| `type` | string | Yes | イベントのタイプ。[Subscription Event Types](#subscription-event-types)を参照してください |
| `data` | object | Yes | メインのデータペイロード。[Subscription webhook payloads](/developer-resources/webhooks/intents/subscription)を参照してください |
