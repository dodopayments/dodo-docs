...
1. **[Dodo Payments Dashboard](https://app.dodopayments.com/)にログイン**
2. **製品** → メーターに移動
3. **「メーターを作成」をクリック**
4. **メーターを設定**:
   - **メーター名**: 説明的な名前を選択（例: "LLMトークン使用量"）
   - **イベント名**: 一意のイベント識別子を設定（例: `llm.chat_completion`）
   - **集計タイプ**: トークン数を合計するために`sum`を選択
   - **プロパティの上に**: 追跡するものを選択:
     - `inputTokens` - 入力/プロンプトトークンを追跡
     - `outputTokens` - 出力/完了トークンを追跡（該当する場合は推論トークンを含む）
     - `totalTokens` - 結合された入力 + 出力トークンを追跡
...
<Warning>
開発中は必ず`test_mode`を使用して、本番のメトリクスに影響を与えないようにしてください。
</Warning>
</ParamField>
...
<Note>
推論機能を持つモデルをAI SDK（GoogleのGemini 2.5 Flashの思考モードなど）を通じて使用する場合、推論トークンは自動的に`outputTokens`のカウントに含まれ、正確な請求が行われます。
</Note>
</Accordion>
...
<Note>
**Gemini思考モード:** 思考/推論機能を持つGeminiモデル（Gemini 2.5 Proなど）を使用する場合、SDKは自動的に`thoughtsTokenCount`（推論トークン）を`outputTokens`に含め、完全な計算コストを正確に反映します。
</Note>
</Accordion>
</AccordionGroup>
...
- `provider` - LLMプロバイダー（ラッパーメタデータに含まれている場合）
...
<Info>
あなたのDodo Paymentsメーターは、使用量と請求を計算するために`metadata`フィールド（特に`inputTokens`、`outputTokens`または`totalTokens`）を使用します。
</Info>
