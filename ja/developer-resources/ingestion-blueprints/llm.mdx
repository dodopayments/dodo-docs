...
<Card title="使用量に基づく請求ガイド" icon="arrow-trend-up" href="/developer-resources/usage-based-billing-guide">
  メーターを使用した使用量に基づく請求の包括的なガイド。
</Card>
</CardGroup>
...
1. **ログイン**して [Dodo Payments Dashboard](https://app.dodopayments.com/)
2. **ナビゲート**して Products → Meters
3. **「メーターを作成」**をクリック
4. **メーターを設定**:
   - **メーター名**: 説明的な名前を選択 (例: "LLMトークン使用")
   - **イベント名**: 一意のイベント識別子を設定 (例: `llm.chat_completion`)
   - **集計タイプ**: トークン数を合算するために `sum` を選択
   - **プロパティによる**: 追跡するものを選択:
     - `inputTokens` - 入力/プロンプトトークンを追跡
     - `outputTokens` - 出力/完了トークンを追跡 (適用可能な場合は推論トークンを含む)
     - `totalTokens` - 入力 + 出力トークンを追跡

<Info>
ここで設定した **イベント名** は、SDK に渡すものと正確に一致する必要があります（大文字と小文字を区別します）。
</Info>

詳細な手順については、[使用量に基づく請求ガイド](/developer-resources/usage-based-billing-guide) を参照してください。
</Step>
...
<Warning>
開発中は常に `test_mode` を使用して、本番の測定値に影響を与えないようにしてください。
</Warning>
</ParamField>
...
<Note>
推論機能を持つモデルを AI SDK (Google の Gemini 2.5 Flash などの思考モード) を通じて使用する場合、推論トークンは自動的に `outputTokens` のカウントに含まれ、正確な請求情報が得られます。
</Note>
</Accordion>
...
<Note>
**Gemini思考モード**:思考/推論機能を持つGeminiモデル（Gemini 2.5 Proなど）を使用する際、SDKは自動的に `thoughtsTokenCount` (推論トークン) を `outputTokens` に含め、全計算コストを正確に反映します。
</Note>
</Accordion>
</AccordionGroup>
...
フォーマット: `llm_[timestamp]_[random]`
</ParamField>
...
- `outputTokens` - 使用された出力/完了トークンの数 (適用可能な場合は推論トークンを含む)
- `totalTokens` - 合計トークン (入力 + 出力)
- `model` - 使用されたLLMモデル (例: "gpt-4")
- `provider` - LLMプロバイダー (ラッパーメタデータに含まれる場合)
- クライアントをラップする際に提供したカスタムメタデータ

<Note>
**推論トークン**:推論機能を持つモデルに対して、`outputTokens` は自動的に完了トークンと推論トークンの両方を含みます。
</Note>
</ParamField>

<Info>
あなたのDodo Paymentsメーターは、`metadata` フィールド (特に `inputTokens`, `outputTokens` または `totalTokens`) を使用して、使用量と請求を計算します。
</Info>
