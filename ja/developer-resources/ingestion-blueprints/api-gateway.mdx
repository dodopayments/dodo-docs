...
<Step title="メーターを作成">
[ドドペイメントダッシュボード](https://app.dodopayments.com/) でメーターを作成します:
- **イベント名**: `api_call`（または好みの名前）
- **集計タイプ**: `count` コール数の追跡のため
...
自動フラッシュ間隔（ミリ秒単位）。デフォルト: `5000`（5秒）。
</ParamField>

## ベストプラクティス

<Tip>
**高ボリューム用にバッチ処理を使用**: 1秒あたり10回以上のリクエストを処理するアプリケーションでは、`createBatch()`を使用してオーバーヘッドを削減し、性能を向上させてください。
</Tip>

<Warning>
**バッチを常にクリーンアップ**: アプリケーションのシャットダウン時に`batch.cleanup()`を呼び出して、保留中のイベントをフラッシュし、データ損失を防いでください。
</Warning>
