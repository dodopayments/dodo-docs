...
1. [Supabase ダッシュボード](https://supabase.com/dashboard)に移動します
2. SQL エディタを開きます
3. 新しいクエリを作成します
4. [`schema.sql`](#database-schema)の全内容をコピーして貼り付けます
5. クエリを実行します

### 4. 初期シークレットの設定

Supabase は、実行時に自動的に `SUPABASE_URL` と `SUPABASE_SERVICE_ROLE_KEY` を提供します。

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **注意:** デプロイ後にウェブフック URL を取得したら、`DODO_PAYMENTS_WEBHOOK_KEY` を設定します。
...
1. [DodoPayments ダッシュボード](https://app.dodopayments.com) → 開発者 → ウェブフックに移動します
2. 新しいウェブフックエンドポイントを作成します
3. ウェブフック URL をエンドポイントとして設定します
...
✅ **冪等性** - ウェブフック ID で重複処理を防止  
✅ **イベントログ** - `webhook_events` テーブルに完全な監査証跡  
✅ **エラーハンドリング** - ログに記録され、再試行可能

> **注意:** この実装は、最小限のフィールドで三つのコアサブスクリプションイベント (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) を処理することを示しています。要件に応じて、追加のイベントタイプやフィールドをサポートするように簡単に拡張できます。
...
Deno ベースのエッジ関数:
1. **署名を検証** - HMAC-SHA256 検証のために dodopayments ライブラリを使用
2. **冪等性をチェック** - ウェブフック ID を検索して重複処理を防止
3. **イベントをログに記録** - 生のウェブフックデータを `webhook_events` テーブルに保存
4. **更新を処理** - Supabase クライアントを介して顧客とサブスクリプションを作成または更新
...
<Note>
`--no-verify-jwt` フラグは、ウェブフックが JWT トークンを含まないため、必須です。セキュリティはウェブフック署名検証によって提供されます。
</Note>
...
| 問題 | 解決策 |
|-------|----------|
| 検証に失敗しました | DodoPayments ダッシュボードからウェブフックキーが正しいか確認してください |
| データベースの権限エラー | サービスロールキーを使用していることを確認してください |
| JWT 検証エラー | `--no-verify-jwt` フラグを使用してデプロイしてください |
| 関数が見つかりません | プロジェクト参照が正しいか、関数がデプロイされているか確認してください |
