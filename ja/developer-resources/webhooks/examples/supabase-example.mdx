...
1. [Supabase Dashboard](https://supabase.com/dashboard)に移動します。
2. SQLエディタを開きます。
3. 新しいクエリを作成します。
4. [`schema.sql`](#database-schema)の全文をコピーします。
5. クエリを実行します。

### 4. 初期シークレットの設定

Supabaseは、実行時に自動的に`SUPABASE_URL`および`SUPABASE_SERVICE_ROLE_KEY`を提供します。

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **注意:** デプロイ後、webhook URLを取得した後に`DODO_PAYMENTS_WEBHOOK_KEY`を設定します。

### 5. デプロイ

関数はすでに`functions/webhook/index.ts`に設定されているので、それをデプロイします：
...
1. [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooksに移動します。
2. 新しいwebhookエンドポイントを作成します。
3. webhook URLをエンドポイントとして設定します。
4. 次のサブスクリプションイベントを有効にします：
   - `subscription.active`
   - `subscription.cancelled`
   - `subscription.renewed`
5. **Signing Secret**をコピーします。
...
✅ **エラーハンドリング** - ログを記録し、再試行可能

> **注意:** この実装は、最低限のフィールドで三つのコアサブスクリプションイベント（`subscription.active`、`subscription.cancelled`、`subscription.renewed`）を処理する方法を示しています。必要に応じて、追加のイベントタイプやフィールドをサポートするように簡単に拡張できます。
...
2. **冪等性を確認** - 重複処理を防ぐためにwebhook IDを検索
3. **イベントをログ** - 生のwebhookデータを`webhook_events`テーブルに保存
4. **更新を処理** - Supabaseクライアントを通じて顧客とサブスクリプションを作成または更新
5. **エラーを処理** - 失敗をログし、再試行のためにイベントに印を付ける
...
<Note>
`--no-verify-jwt`フラグは必須です。なぜなら、webhookにはJWTトークンが含まれていないからです。セキュリティはwebhook署名検証によって提供されます。
</Note>
...
| 問題 | 解決策 |
|-------|----------|
| 検証失敗 | DodoPaymentsダッシュボードからwebhookキーが正しいか確認 |
| データベース権限エラー | サービスロールキーを使用していることを確認 |
| JWT検証エラー | `--no-verify-jwt`フラグでデプロイ |
| 関数が見つからない | プロジェクト参照が正しく、関数がデプロイされていることを確認 |
