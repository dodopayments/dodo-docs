...
> **注:** デプロイ後にあなたのウェブフックURLが設定されたときに `DODO_PAYMENTS_WEBHOOK_KEY` を設定します。
...
1. [DodoPayments ダッシュボード](https://app.dodopayments.com) → 開発者 → ウェブフック に移動
2. 新しいウェブフックエンドポイントを作成
3. エンドポイントとしてウェブフックURLを構成
...
✅ **署名確認** - dodopayments ライブラリを使用  
✅ **冪等性** - ウェブフック ID による重複処理の防止  
✅ **イベントロギング** - `webhook_events` テーブルでの完全な監査トレイル  
✅ **エラーハンドリング** - ログ記録と再試行可能

> **注:** この実装は、最小限のフィールドで3つのコアサブスクリプションイベント (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) の処理を示しています。それに基づいて、追加のイベントタイプとフィールドをサポートするように簡単に拡張できます。
...
ウェブフックハンドラー：
1. **ボディ解析を無効化** - 署名確認のために生のボディにアクセス
2. **署名を確認** - リクエストがDodoPaymentsからであることをHMAC-SHA256を使用して確認
3. **重複をチェック** - ウェブフックIDを使用して同じイベントを二重処理するのを防ぐ
4. **イベントをログ** - 監査トレイルのために生ウェブフックを `webhook_events` テーブルに保存
5. **イベントを処理** - Neonで顧客とサブスクリプションを作成または更新
6. **エラーを処理** - 失敗をログに記録し、再試行のためにイベントを未処理としてマーク
...
| 関数のタイムアウト | クエリを最適化; プロプランは長いタイムアウト（60秒）を持っています |
