...
2. 新しいプロジェクトを作成
3. SQL エディタを開く
4. [`schema.sql`](#database-schema) の内容をコピーして貼り付け
5. クエリを実行
...
1. [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks にアクセス
...
> **Note:** この実装では、3 つの主要なサブスクリプションイベント (`subscription.active`、`subscription.cancelled`、`subscription.renewed`) を最小限のフィールドで処理する方法を示しています。要件に応じて追加のイベントタイプやフィールドを容易に拡張できます。
