...
> **注意:** デプロイ後にウェブフックURLを取得したら`DODO_PAYMENTS_WEBHOOK_KEY`を設定します。
...
1. [DodoPayments Dashboard](https://app.dodopayments.com)に移動 → 開発者 → ウェブフック
2. 新しいウェブフックエンドポイントを作成します
3. ウェブフックURLをエンドポイントとして設定します
...
✅ **冪等性** - ウェブフックIDで重複処理を防止  
✅ **イベントログ** - `webhook_events`テーブルに完全な監査証跡  
✅ **エラーハンドリング** - ログに記録され、再試行可能

> **注意:** この実装は、最小限のフィールドで3つのコアサブスクリプションイベント（`subscription.active`、`subscription.cancelled`、`subscription.renewed`）を処理することを示しています。要件に応じて、追加のイベントタイプやフィールドをサポートするように簡単に拡張できます。
...
ウェブフックハンドラー:
1. **署名を検証** - HMAC-SHA256を使用してリクエストがDodoPaymentsからのものであることを確認します
2. **重複をチェック** - ウェブフックIDを使用して同じイベントを二重に処理しないようにします
3. **イベントをログに記録** - 監査証跡のために生のウェブフックを`webhook_events`テーブルに保存します
4. **イベントを処理** - Neonで顧客とサブスクリプションを作成または更新します
5. **エラーを処理** - 失敗をログに記録し、再試行のためにイベントを未処理としてマークします
...
**Netlify Dashboardでログを表示:**
1. サイトを選択 → **Functions**タブ
2. `webhook`関数をクリックします
3. リアルタイムのログと呼び出し履歴を表示します
...
| 問題 | 解決策 |
|-------|----------|
| 検証に失敗しました | DodoPaymentsダッシュボードからウェブフックキーが正しいか確認してください |
| データベース接続エラー | Neon接続文字列を確認し、プール接続を使用してください |
| 関数が見つかりません (404) | `netlify init`と`npm run deploy`を再度実行してください |
| 環境変数が利用できません | CLIまたはダッシュボードで変数を設定し、再デプロイしてください |
