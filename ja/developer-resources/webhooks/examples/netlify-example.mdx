...
> **注意:** デプロイ後、Webhook URL を取得した後に `DODO_PAYMENTS_WEBHOOK_KEY` を設定します。
...
1. [DodoPayments ダッシュボード](https://app.dodopayments.com) → 開発者 → Webhook に移動します
2. 新しいWebhookエンドポイントを作成します
3. Webhook URL をエンドポイントとして設定します
...
✅ **冪等性** - Webhook ID で重複処理を防止  
✅ **イベントのログ記録** - `webhook_events` テーブルに完全な監査証跡  
✅ **エラー処理** - ログ記録され、再試行可能

> **注意:** この実装は、最小限のフィールドで3つのコアサブスクリプションイベント (`subscription.active`、`subscription.cancelled`、`subscription.renewed`) を処理することを示しています。要件に基づいて追加のイベントタイプやフィールドを簡単に拡張できます。
...
Webhook ハンドラー：
1. **署名を検証** - リクエストが HMAC-SHA256 を使用して DodoPayments からのものであることを確認します
2. **重複をチェック** - Webhook ID を使用して同じイベントが二度処理されることを防ぎます
3. **イベントをログ記録** - 監査証跡のために `webhook_events` テーブルに生のWebhook を保存します
4. **イベントを処理** - Neon に顧客とサブスクリプションを作成または更新します
5. **エラーを処理** - 失敗をログ記録し、再試行のためにイベントを未処理としてマークします
...
**Netlify ダッシュボードでログを表示する**：
1. サイトを選択 → **Functions** タブ
2. `webhook` 関数をクリック
3. リアルタイムログと呼び出し履歴を表示します
...
| 問題 | 解決策 |
|-------|----------|
| 検証に失敗しました | DodoPaymentsダッシュボードからWebhookキーが正しいことを確認 |
| データベース接続エラー | Neon接続文字列を確認し、プール接続を使用 |
| 関数が見つかりません（404） | `netlify init` と `npm run deploy` を再実行 |
| 環境変数が利用できません | CLI またはダッシュボードで変数を設定し、その後再デプロイ |
