...
1. [Neon](https://neon.com) にサインアップします
2. 新しいプロジェクトを作成します
3. SQL エディタを開きます
4. [`schema.sql`](#database-schema) の内容をコピーして貼り付けます
5. クエリを実行します
6. Neon → Connection Details から接続文字列を取得します
...
> **注意：** 配備後に webhook URL を取得したら `DODO_PAYMENTS_WEBHOOK_KEY` を設定します。
...
1. [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks に移動します
2. 新しい webhook エンドポイントを作成します
3. エンドポイントとして webhook URL を構成します
...
✅ **署名の検証** - dodopayments ライブラリを使用  
✅ **冪等性** - webhook ID により重複処理を防止  
✅ **イベントログ** - `webhook_events` テーブルで完全な監査記録  
✅ **エラーハンドリング** - ログ記録され、再試行可能

> **注意：** この実装では主要なサブスクリプションイベント（`subscription.active`、`subscription.cancelled`、`subscription.renewed`）の3種類を最小フィールドで処理しています。要件に応じて他のイベントタイプやフィールドを簡単に追加できます。
...
webhook ハンドラ：
1. **署名を検証** - HMAC-SHA256 を使用してリクエストが DodoPayments からのものであることを確認
2. **重複を確認** - webhook ID を使用して同じイベントを2回処理しないようにする
3. **イベントを記録** - 監査用に `webhook_events` テーブルに生の webhook を保存
4. **イベントを処理** - Neon で顧客とサブスクリプションを作成または更新
...
**Netlify ダッシュボードでログを表示：**
1. サイトを選択 → **Functions** タブ
2. `webhook` 関数をクリック
3. リアルタイムログと呼び出し履歴を表示
...
| 問題 | 解決策 |
|-------|----------|
| 検証に失敗 | DodoPayments ダッシュボードから webhook キーが正しいか確認 |
| データベース接続エラー | Neon 接続文字列を確認し、プール接続を使用 |
| 関数が見つからない（404） | `netlify init` と `npm run deploy` を再度実行 |
| 環境変数が利用できない | CLI またはダッシュボードで変数を設定し、再デプロイ |
