...
> **注意:** デプロイ後にウェブフック URL が取得できたら `DODO_PAYMENTS_WEBHOOK_KEY` を設定します。
...
✅ **冪等性** - ウェブフック ID で重複処理を防ぎます  
✅ **イベントログ** - `webhook_events` テーブル内に完全な監査追跡  
✅ **エラーハンドリング** - ログされ、再試行可能

> **注意:** この実装は、最小限のフィールドで 3 つのコアサブスクリプションイベント (`subscription.active`、`subscription.cancelled`、`subscription.renewed`) の処理を示しています。要件に基づいて、追加のイベントタイプやフィールドを簡単に拡張できます。
...
ウェブフックハンドラー：
1. **署名を検証します** - HMAC-SHA256 を使用してリクエストが DodoPayments からのものであることを確認します。
2. **重複をチェックします** - ウェブフック ID を使用して同じイベントを二重に処理しないようにします。
3. **イベントを記録します** - 監査トレイル用に生のウェブフックを `webhook_events` テーブルに保存します。
4. **イベントを処理します** - 顧客とサブスクリプションを作成または更新します。
5. **エラーをハンドリングします** - 失敗を記録し、再試行のためにイベントを未処理としてマークします。
...
| データベース接続エラー | Neon 接続文字列を確認してください |
| ワーカーがデプロイされない | `npm install` を実行し、`wrangler.toml` 構文を確認してください |
