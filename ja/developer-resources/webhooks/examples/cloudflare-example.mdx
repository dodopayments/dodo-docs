...
> **注:** デプロイ後にウェブフック URL を取得してから `DODO_PAYMENTS_WEBHOOK_KEY` を設定します。
...
3. エンドポイントとしてご自身の webhook URL を設定します
4. 以下の購読イベントを有効にします:
   - `subscription.active`
...
✅ **冪等性** - webhook ID による重複処理の防止  
✅ **イベントログ** - `webhook_events` テーブルに完全な監査記録  
✅ **エラー処理** - ログ記録および再試行可能

> **注:** この実装は、最小限のフィールドで 3 つの主要な購読イベント (`subscription.active`、`subscription.cancelled`、`subscription.renewed`) を処理する方法を示しています。必要に応じて追加のイベントタイプやフィールドに簡単に拡張できます。
...
Webhook ハンドラー:
1. **署名を検証** - HMAC-SHA256 を使用して DodoPayments からのリクエストであることを確認します
2. **重複をチェック** - webhook ID を使用して同じイベントの二重処理を防ぎます
3. **イベントを記録** - 監査証跡のため `webhook_events` テーブルに生の webhook を保存します
4. **イベントを処理** - 顧客および購読を新規作成または更新します
5. **エラーを処理** - 障害を記録し、再試行用に未処理としてマークします
...
| 問題 | 解決策 |
|-------|----------|
| 検証に失敗しました | DodoPayments ダッシュボードから webhook キーが正しいか確認してください |
| データベース接続エラー | Neon の接続文字列を確認してください |
