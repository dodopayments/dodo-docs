...
1. [DodoPayments Dashboard](https://app.dodopayments.com) → 開発者 → Webhooksに移動します
...
2. **重複をチェック** - Webhook IDを使用して同じイベントを二重に処理しないようにします
3. **イベントをログに記録** - 監査証跡のために生のWebhookを`webhook_events`テーブルに保存します
4. **イベントを処理** - 顧客とサブスクリプションを作成または更新します
