...
| `revocation_reason` | トリガー |
|---|---|
| `subscription_cancelled` | 顧客のサブスクリプションがキャンセルされた（`subscription.cancelled` イベント）。 |
| `subscription_on_hold` | 更新に失敗したため、サブスクリプションが保留中になっている（`subscription.on_hold`）。復旧可能：再試行が成功すると、権限が再付与されます。 |
| `SubscriptionPaused` | サブスクリプションが一時停止された（`subscription.paused`）。復旧可能：再開すると、権限が再付与されます。 |
| `subscription_expired` | サブスクリプションの契約期間が終了した（`subscription.expired`）。 |
| `plan_changed` | プランが変更されたため、新しい権限が発行される前に古い権限が取り消された（`subscription.plan_changed`）。 |
| `refund` | 元の一回限りの支払いに対する返金が処理された（`refund.succeeded`）。 |
| `manual` | Merchant が API またはダッシュボードを介して権限を取り消した。手動で取り消された権限は、サブスクリプションの更新時に自動的に再付与されません。 |
| `license_key_disabled` | license-key grant に紐付けられたライセンスキーが無効化された。キーが再び有効になると、権限は自動的に再有効化されます。 |
| `platform_external` | 統合のプラットフォーム側で同期のずれが発生した（たとえば、Discord のロールが手動で削除された、GitHub App がリポジトリへのアクセス権を失った、または照合処理によって対象の欠落が検出された）。基盤となるプラットフォームの問題が解決されるまで、サブスクリプションの更新時に権限は自動的に再付与されません。 |
