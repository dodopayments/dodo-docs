...
- **型安全性**: コンパイル時の安全性と、より優れた IDE サポートを実現する強い型付け
- **エラーハンドリング**: 詳細なエラーメッセージを備えた包括的な例外処理
- **認証**: 環境変数をサポートしたシンプルな API キー認証
- **非同期処理のサポート**: 必要に応じた最新の async/await パターン
- **自動ページネーション**: リストレスポンスの自動ページネーション
- **従量課金**: 使用量イベントの追跡と取り込みに対応する組み込み機能
- **テスト**: 開発およびテスト用の完全な Test Mode サポート
