...
<Info>
このセクションでは、Checkout SDK を使用した**クライアント側**のテーマ設定について説明します。Checkout セッションを API を介して作成する際に `theme_config` パラメーターを使用して**サーバー側**でテーマを設定することもできます。API レベルの設定については[Checkout Theme Customization](/features/checkout#checkout-theme-customization)を参照するか、ダッシュボードの[Design page](/features/design)を使用してライブプレビュー付きで視覚的にテーマを構成してください。
</Info>
