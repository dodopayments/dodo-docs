...
Node.js、Python などを使用してバックエンドでチェックアウトセッションを作成する方法を学びましょう。専用の<strong>Checkout Sessions API</strong>ドキュメントで、完全な例とパラメータ参照をご覧ください。
</Card>
<Note>
...
支払い完了リダイレクトを処理するためにURLスキームを設定します：
...
この依存関係を `package.json` に追加してください:
...
`android/app/src/main/AndroidManifest.xml` に追加してください:
...
`ios/YourApp/Info.plist` に追加してください:
...
<Tip>
ネイティブなブラウザ体験には InAppBrowser を使用し、支払いコールバック用のディープリンクを処理してください。
</Tip>
...
<Warning>
**Regional Compliance**: インアプリ決済 SDK には地域ごとの制限があります。**iOS** ではネイティブ決済 SDK は **EU**（DMA 条項の下）でのみ合法です。**Android** では User Choice Billing（UCB）市場で合法です。他の地域では、上記のチェックアウトリンク方式を使用して App Store および Play Store への準拠を確保してください。
</Warning>

{/* LOCKED_PATTERN_c0f6dca68080424327396739e988adac */}
  Dodo の React Native SDK をネイティブの決済シートと統合するための完全ガイドで、フルカスタマイズオプションと直接決済処理を含みます。セットアップ、実装、テスト手順も網羅しています。
</Card>

<Info>
地域の準拠要件を理解するために [App Store Fee Bypass Guide](/features/bypassing-app-store-fees) を確認してください。
</Info>
...
<Tip>
決済リンクには WebView を使用し、リダイレクト URL を処理して支払い完了を検出します。
</Tip>
...
<Check>
ご質問やサポートが必要な場合は <a href="mailto:support@dodopayments.com">support@dodopayments.com</a> までお問い合わせください。
</Check>