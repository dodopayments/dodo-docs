...
商品の識別子（例: <code>?productId=pdt_xxx</code>）。
</ParamField>
...
請求額を主要通貨単位で固定します（例: $12.50 の場合は <code>12.5</code>）。Pay What You Want 商品専用で、商品の最低価格を下回る場合は無視されます。
</ParamField>
<ParamField query="showDiscounts" type="boolean">
割引フィールドを表示します。
</ParamField>
<ParamField query="metadata_*" type="string">
<code>metadata_</code> で始まる Query Parameter はメタデータとして渡されます。
</ParamField>
