...
<Step title="決済ユーティリティ関数を作成">
    ...
    <Note>
    この関数は、支払いインテントを作成し、クライアントシークレットを返すバックエンドAPIエンドポイントがあることを前提としています。支払いの作成を処理するために、バックエンドが適切に設定されていることを確認してください。バックエンドのセットアップ例については、[API統合チュートリアル](/developer-resources/integration-guide)をご覧ください。
    </Note>
</Step>

<Step title="支払い画面の実装">
    `useCheckout`フックを使用して支払い画面を作成します。完全な実装は次のとおりです:
...
React Native Unified Checkoutを使用して、`initPaymentSession()`を呼び出す際に、色、フォントなどを変更することでアプリのデザインに合わせてカスタマイズできます。
...
| 色カテゴリ | 使用法 |
|----------------|-------|
| `primary` | Payボタンと選択されたアイテムのための主要な色 |
| `background` | 支払いページの背景色 |
| `componentBackground` | 入力、タブ、その他のコンポーネントの背景色 |
| `componentBorder` | 入力、タブ、その他のコンポーネントの外部境界色 |
| `componentDivider` | コンポーネントの内部境界色（共有境界） |
| `primaryText` | ヘッダーのテキスト色 |
| `secondaryText` | 入力フィールドのラベルテキスト色 |
| `componentText` | 入力テキスト色（例：カード番号、郵便番号） |
| `placeholderText` | 入力フィールドのプレースホルダーテキスト色 |
| `icon` | アイコンの色（例：閉じるボタン） |
| `error` | エラーメッセージと破壊的アクションの色 |
