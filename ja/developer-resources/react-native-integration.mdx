...
<Step title="支払い画面の実装">
    `useCheckout` フックを使用して支払い画面を作成します。完全な実装はこちら:
...
React Native Unified Checkout の外観パラメータを使用して、色、フォントなどを変更することで、アプリのデザインに合わせてカスタマイズできます `initPaymentSession()`。

#### 色のカスタマイズ

各色カテゴリーは、UI内の1つまたは複数のコンポーネントの色を決定します。たとえば、`primary` は、Payボタンの色を定義します。

| 色カテゴリー | 使用法 |
|----------------|-------|
| `primary` | Payボタンと選択されたアイテムの主要な色 |
...
| `secondaryText` | 入力欄のラベルテキスト色 |
| `componentText` | 入力テキスト色（例: カード番号、郵便番号） |
| `placeholderText` | 入力欄のプレースホルダーテキスト色 |
