...
<Tip>
このガイドは、チェックアウトセッションを通じてオンデマンドサブスクリプションを作成します (`POST /checkouts`)。これは常にホストされた`checkout_url`を返します。お客様をそこにリダイレクトして、命令を承認させ、`return_url`に続いてどこに進むべきかを設定します。
</Tip>
...
支払い方法自体が無効（有効期限切れカード、閉鎖アカウントなど）で再試行が失敗し続ける場合は、顧客から新しい情報を収集するために[`POST /subscriptions/{subscription_id}/update-payment-method`](/api-reference/subscriptions/update-payment-method)を使用します。成功すると、サブスクリプションは`active`に戻り、`payment.succeeded`に続いて`subscription.active`ウェブフックが発行されます。
</Step>
</Steps>
...
テストAPIキーを使用してサブスクリプションを作成し、返された`checkout_url`を開いて命令を完了します。
</Step>
<Step title="Trigger a charge">
小さな`product_price`（例：`100`）で課金エンドポイントを呼び出し、`payment.succeeded`を受信することを確認します。
</Step>
<Step title="Go live">
イベントと内部状態の更新を検証したら、本番用のAPIキーに切り替えます。
</Step>
</Steps>
