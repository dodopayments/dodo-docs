...
<Warning>
オンデマンドではないサブスクリプションへの請求は失敗する可能性があります。請求を行う前に、サブスクリプションの詳細に `on_demand: true` が含まれていることを確認してください。
</Warning>
...
サブスクリプションは `on_hold` 状態に移行し、`subscription.on_hold` webhook を発行することがあります（[Subscription States → On Hold](/features/subscription#on-hold-state) を参照）。これはシグナルであり、ロックではありません。オンデマンドサブスクリプションでは、`on_hold` になっても、再度請求することを妨げません。
</Step>

<Step title="Retry the charge (your call)">
オンデマンドフローでは、Dodo は自動 retry を行いません。いつでも `POST /subscriptions/{subscription_id}/charge` を再度呼び出して retry できます。下記の [安全な retry ポリシー](#payment-retries) に従い、exponential backoff を使用し、hard decline をスキップして、burst パターンを避けてください。これにより、retry が当社の fraud および risk システムによってフラグ付けされるのを防げます。
</Step>

<Step title="Optionally, ask the customer for a new payment method">
支払い方法自体に問題があるために retry が繰り返し失敗する場合（カードの有効期限切れ、アカウントの閉鎖など）は、[`POST /subscriptions/{subscription_id}/update-payment-method`](/api-reference/subscriptions/update-payment-method) を使用して、顧客から新しい支払い方法を取得します。成功すると、サブスクリプションは `active` に戻り、続けて `payment.succeeded` および `subscription.active` webhook が発行されます。
</Step>
</Steps>
...
| 順序 | Event | 意味 |
|-------|-------|---------|
| 1 | `payment.failed` | オンデマンド請求の試行が成功しなかった（拒否理由を含む） |
| 2 | `subscription.on_hold` | サブスクリプションが保留になった（情報提供のみで、以降の請求をブロックしない） |
| 3* | `payment.succeeded` | その後の請求（retry または支払い方法の更新後）が成功した |
| 4* | `subscription.active` | 請求の成功後、サブスクリプションが `active` に戻った |
...
[Subscription Dunning](/features/recovery/subscription-dunning) — 組み込みのメールによる回収シーケンス — は、スケジュールされたサブスクリプションの *更新* 支払いの失敗と、顧客が開始したキャンセルを対象としています。オンデマンド請求の失敗向けには設計されていません。支払い方法を更新する必要があると判断した場合は、顧客に直接（transactional email やアプリ内プロンプトなどで）連絡してください。
...
<Note>
decline reason の包括的な一覧と、ユーザーが修正可能かどうかについては、[Transaction Failures](/api-reference/transaction-failures) ドキュメントを参照してください。
</Note>
...
顧客が [Customer Portal](/features/customer-portal) からオンデマンドサブスクリプションをキャンセルすると、デフォルトではキャンセルが**次回の billing date にスケジュール**されます。オンデマンドサブスクリプションでは、**Cancel Now** オプションは意図的に表示されません。

理由は、オンデマンドサブスクリプションには予測可能な定期更新日がなく、次回の請求時刻が完全に利用イベントによって決まるためです。次回の billing date にキャンセルをスケジュールすることで、期間の境界まで mandate を有効なまま維持し、進行中の利用分を請求できるようにしてから、サブスクリプションを適切に終了できます。
...
オンデマンドサブスクリプションは、いつでも API を介してキャンセルできます。キャンセルを即時に行うか、スケジュールするかを制御できます。
...
Customer Portal の動作に合わせ、スケジュールされたキャンセル日までサブスクリプションを有効なまま維持し、その後終了します。
...
| Event | 発行されるタイミング |
|-------|---------------|
| `subscription.cancelled` | サブスクリプションが完全にキャンセルされ、請求できなくなったとき |
| `subscription.plan_changed` | `cancel_at_next_billing_date` が切り替えられたとき（スケジュールされたキャンセルが設定または取り消されたとき） |

<Tip>
handler でオンデマンドのキャンセルとスケジュールされたサブスクリプションのキャンセルを区別するには、webhook の処理時にサブスクリプションの `on_demand` flag を確認してください。
</Tip>
...
- **subscription.active**：Mandate が authorized になり、サブスクリプションが有効化された
- **subscription.failed**：作成に失敗した（例：mandate failure）
- **subscription.on_hold**：サブスクリプションが保留になった（例：unpaid state）
- **subscription.cancelled**：サブスクリプションが完全にキャンセルされた（[Cancellation](#cancellation) を参照）
- **payment.succeeded**：請求が成功した
- **payment.failed**：請求が失敗した

<Tip>
オンデマンドフローでは、利用量ベースの請求を照合するため、`payment.succeeded` と `payment.failed` に注目します。`payment.failed` に続いて `subscription.on_hold` が発生した場合は、サブスクリプションを回復するために [Handling failed charges](#handling-failed-charges) を参照してください。
</Tip>
...
test API key を使用してサブスクリプションを作成し、返された `checkout_url` を開いて mandate を完了します。
</Step>
<Step title="Trigger a charge">
少額の `product_price`（例：`100`）を指定して請求エンドポイントを呼び出し、`payment.succeeded` を受信したことを確認します。
</Step>
<Step title="Go live">
...
- **422 Invalid Request**：作成時に `on_demand.mandate_only` が、請求時に `product_price` が指定されていることを確認してください。
- **Currency errors**：`product_currency` を上書きする場合は、アカウントと顧客でサポートされていることを確認してください。
- **No webhooks received**：webhook URL と signature secret の設定を確認してください。
