...
2. 商品を作成します（一括払いまたはサブスクリプション）。サブスクリプション商品の価格は、少なくとも**$1**（または選択した通貨の同等額）である必要があります。この最低金額を下回る金額はサポートされません。
...
<Card title="Create Checkout Session" icon="code" href="/api-reference/checkout-sessions/create">
  一回限りの支払いとサブスクリプションのための、安全なホスト型チェックアウトセッションを作成するためのAPIリファレンス
</Card>
<Card title="Create Payment Link" icon="link" href="/api-reference/payments/post-payments">
  動的な支払いリンクをプログラムで作成するためのAPIリファレンス
</Card>
</CardGroup>