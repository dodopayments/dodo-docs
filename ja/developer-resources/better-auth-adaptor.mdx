...
<Info>
  レガシーの<code>checkout</code>メソッドとは異なり、<code>checkoutSession</code>
  はチェックアウトページでユーザーから請求情報を取得するため、事前に請求情報は必要ありません。ただし、引数で
  <code>billing</code>キーを渡すことで上書きすることもできます。
</Info>

<Info>
  レガシーの<code>checkout</code>メソッドと同様に、
  <code>checkoutSession</code>は顧客のメールアドレスと名前をBetter-Authセッションから取得しますが、
  <code>customer</code>オブジェクトにメールと名前を渡すことで上書きすることができます。
</Info>

<Info>
  [Checkout
  Sessionを作成](https://docs.dodopayments.com/api-reference/checkout-sessions/create)
  エンドポイントのリクエストボディとして同じオプションを引数に渡すことができます。
</Info>

<Note>
  戻りURLはサーバプラグインで設定されている<code>successUrl</code>から取得されます。
  クライアントのペイロードに<code>return_url</code>を含める必要はありません。
</Note>

## レガシーチェックアウト (非推奨)

<Warning>
  <code>authClient.dodopayments.checkout</code>メソッドは廃止されました。新しい実装では
  <code>checkoutSession</code>を使用してください。
</Warning>
...
計測イベントをキャプチャし、使用に基づく請求を顧客が確認できるようにするためにサーバーで<code>usage()</code>プラグインを有効にします。

- <code>authClient.dodopayments.usage.ingest</code> はサインイン済みでメール確認済みのユーザーのためにイベントを取り込みます。
- <code>authClient.dodopayments.usage.meters.list</code> はその顧客のサブスクリプションに紐付けられたメーターの最近の使用状況をリストします。
...
<Tip>
  タイムスタンプが1時間以上前または5分以上将来の場合、使用の取り込み時に拒否されます。
</Tip>
<Tip>
  使用メーターをリストする際に<code>meter_id</code>を省略した場合、顧客のサブスクリプションに紐付けられたすべてのメーターが返されます。
</Tip>

# Webhooks

<Info>
  WebhooksプラグインはDodo Paymentsからのリアルタイムの支払いイベントを安全に署名検証して処理します。デフォルトのエンドポイントは
  <code>/api/auth/dodopayments/webhooks</code>です。
</Info>

<Steps>
<Step title="Generate and set webhook secret">
Dodo PaymentsダッシュボードでエンドポイントURL（例：<code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>）のためのWebhookシークレットを生成し、<code>.env</code>ファイルに設定します。
...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (必須): DodoPaymentsクライアントインスタンス
    - <b>createCustomerOnSignUp</b> (オプション): ユーザーサインアップ時に顧客を自動作成
    - <b>use</b> (必須): 有効にするプラグインの配列 (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (オプション): BetterAuth `User`を受け取り、作成時にDodoPayments顧客に添付する追加フィールドを返す関数（例： `metadata`, `phone_number`）
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b>: プロダクトの配列またはプロダクトを返す非同期関数 -
    <b>successUrl</b>: 支払い成功後のリダイレクトURL -
    <b>authenticatedUsersOnly</b>: ユーザー認証を要求 (デフォルト: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>無効なAPIキー</b>: .envファイル内の<code>DODO_PAYMENTS_API_KEY</code>をもう一度確認してください。 -
    <b>Webhook署名の不一致</b>: WebhookシークレットがDodo Paymentsダッシュボードに設定したものと一致していることを確認してください。 - <b>顧客が作成されない</b>: <code>createCustomerOnSignUp</code>が<code>true</code>に設定されていることを確認してください。
  </Accordion>
  <Accordion title="Best Practices">
    - すべてのシークレットおよびキーを環境変数で使用してください。 - <code>live_mode</code>に切り替える前に<code>test_mode</code>でテストしてください。 - デバッグと監査のためにWebhookイベントをログに記録してください。
  </Accordion>
</AccordionGroup>
