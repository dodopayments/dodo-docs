...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (必須): DodoPayments クライアントインスタンス
    - <b>createCustomerOnSignUp</b> (オプション): ユーザー登録時に自動的に顧客を作成
    - <b>use</b> (必須): 有効にするプラグインの配列 (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (オプション): BetterAuth `User` を受け取り、DodoPayments 顧客に作成および更新時に添付する追加フィールドを返す関数 (例: `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b>: 製品の配列または製品を返す非同期関数 -{" "}
    <b>successUrl</b>: 支払い成功後のリダイレクト URL -{" "}
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>無効な API キー</b>: <code>.env</code> 内の <code>DODO_PAYMENTS_API_KEY</code> を再確認してください。 -{" "}
    <b>Webhook 署名の不一致</b>: Webhook シークレットが Dodo Payments ダッシュボードに設定されたものと一致していることを確認してください。 - <b>顧客が作成されていない</b>: <code>createCustomerOnSignUp</code> が <code>true</code> に設定されていることを確認してください。
  </Accordion>
  <Accordion title="Best Practices">
    - すべてのシークレットとキーに環境変数を使用してください。 - <code>test_mode</code> でテストしてから <code>live_mode</code> に切り替えてください。 - デバッグと監査のために webhook イベントを記録してください。
  </Accordion>
</AccordionGroup>
