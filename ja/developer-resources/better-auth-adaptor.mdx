...
<Step title="サーバーサイド統合の設定">
<code>src/lib/auth.ts</code> を作成または更新します：
...
<Tip>
本番環境では <code>environment</code> を <code>live_mode</code> に設定してください。
</Tip>
</Step>

<Step title="クライアントサイド統合の設定">
<code>src/lib/auth-client.ts</code> を作成または更新します：
...
<Info>
  新しい統合には <code>authClient.dodopayments.checkoutSession</code> を推奨します。
  従来の <code>checkout</code> メソッドは非推奨であり、後方互換性のためにのみ保持されています。
</Info>
...
<Info>
  従来の <code>checkout</code> メソッドとは異なり、<code>checkoutSession</code>
  は、チェックアウトページでユーザーから取得されるため、事前に請求情報を必要としません。
  引数に <code>billing</code> キーを渡すことで上書きすることもできます。
</Info>

<Info>
  従来の <code>checkout</code> メソッドと同様に、
  <code>checkoutSession</code> は顧客のメールアドレスと名前をその
  better-auth セッションから取得しますが、<code>customer</code> オブジェクトを渡すことで上書きできます。
</Info>

<Info>
  引数に渡すオプションは、[チェックアウトセッションの作成](https://docs.dodopayments.com/api-reference/checkout-sessions/create)
  エンドポイントのリクエストボディと同じです。
</Info>

<Note>
  戻りURLは、サーバープラグインで設定された <code>successUrl</code> から取得されます。
  クライアントペイロードに <code>return_url</code> を含める必要はありません。
</Note>

## 従来のチェックアウト（非推奨）

<Warning>
  <code>authClient.dodopayments.checkout</code> メソッドは非推奨です。
  新しい実装には <code>checkoutSession</code> を使用してください。
</Warning>
...
<Tip>
  1時間以上前または5分以上先のタイムスタンプは、使用量を取り込む際に拒否されます。
</Tip>
<Tip>
  使用メーターをリストする際に<code>meter_id</code>を省略すると、その顧客のサブスクリプションに関連付けられたすべてのメーターが返されます。
</Tip>

# Webhooks

<Info>
  Webhooksプラグインは、Dodo Paymentsからのリアルタイムの支払いイベントを安全な署名検証で処理します。デフォルトのエンドポイントは<code>/api/auth/dodopayments/webhooks</code>です。
</Info>

<Steps>
<Step title="Webhookシークレットの生成と設定">
エンドポイントURL（例：<code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>）のためにDodo PaymentsダッシュボードでWebhookシークレットを生成し、<code>.env</code>ファイルに設定します：
...
<AccordionGroup>
  <Accordion title="プラグインオプション">
    - <b>client</b>（必須）：DodoPaymentsクライアントインスタンス - <b>createCustomerOnSignUp</b>（オプション）：ユーザーサインアップ時に顧客を自動作成 - <b>use</b>（必須）：有効にするプラグインの配列（チェックアウト、ポータル、使用量、Webhook）
  </Accordion>
  <Accordion title="チェックアウトプラグインオプション">
    - <b>products</b>：製品の配列または製品を返す非同期関数 - <b>successUrl</b>：支払い成功後にリダイレクトするURL - <b>authenticatedUsersOnly</b>：ユーザー認証を要求（デフォルト：false）
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="一般的な問題">
    - <b>無効なAPIキー</b>：<code>DODO_PAYMENTS_API_KEY</code>を<code>.env</code>で再確認してください。 - <b>Webhook署名の不一致</b>：WebhookシークレットがDodo Paymentsダッシュボードで設定したものと一致していることを確認してください。 - <b>顧客が作成されていない</b>：<code>createCustomerOnSignUp</code>が<code>true</code>に設定されていることを確認してください。
  </Accordion>
  <Accordion title="ベストプラクティス">
    - すべてのシークレットとキーに環境変数を使用してください。 - <code>live_mode</code>に切り替える前に<code>test_mode</code>でテストしてください。 - デバッグと監査のためにWebhookイベントをログに記録してください。
  </Accordion>
</AccordionGroup>
