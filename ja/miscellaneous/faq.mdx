...
<Accordion title="Q65: 誰かがサブスクリプションを開始したとき、2つのWebhookイベントを受け取りますか？">
  A: 最初のサブスクリプションの支払いが成功すると、トライアル期間がない場合、通常は次の3つのWebhookを受け取ります:
  - `subscription.created` Webhook
  - `payment.succeeded` Webhook
  - `subscription.renewal` Webhook
</Accordion>
...
<Accordion title="Q66: When does a subscription get charged?">
  ...
  キャンセルリクエストが行われたとき:
  - **即時キャンセル**: subscription.cancelled_at タイムスタンプは即座に設定され、サブスクリプションはその時点で終了します。
  - **スケジュールキャンセル**: 現在の請求期間の終了時にキャンセルするには、subscription patch APIで`cancel_at_next_billing_date`パラメータを使用します。
...
<Accordion title="Q101: なぜ最初のサブスクリプションに対して`subscription.renewed` Webhookを使用するのですか？">
  A: 最初の支払いが成功すると、サブスクリプションは最初の請求期間に入り、次の請求日が設定されます。
  `subscription.renewed` Webhookは、請求期間が現在アクティブであり、次の請求サイクルがスケジュールされたことを示すために送信されます。
  このWebhookは、最初の請求期間を含むすべての請求期間で使用されるため、請求ロジックを一貫して処理できます。
</Accordion>
...
<Accordion title="Q104: 私のWebhookはPOSTリクエストを受信せず、GETのみを受信しています。なぜですか？">
...
<Accordion title="Q107: WebhookとリダイレクトURLの違いは何ですか？">
A:
- **Webhook URL**: ダッシュボードで設定されます。イベントに関するPOSTペイロードを受信します。
- **リダイレクトURL**: 支払いAPIリクエストで送信されます。クエリパラメータ（`?status=success`）を使用してユーザーをアプリに戻します。
...
<Accordion title="Q108: サブスクリプション製品のpayment.succeeded Webhookで製品IDを受け取らなかったのはなぜですか？">
A: サブスクリプション製品のpayment.succeeded Webhookイベントにはproduct_idが含まれていません。代わりに、サブスクリプションに関連付けられた`subscription_id`が提供され、他の関連詳細と共に提供されます。
</Accordion>

<Accordion title="Q109: 受信したWebhookが本当にDodo Paymentsからのものであり、悪意のあるアクターからのものでないことを確認するにはどうすればよいですか？">
A: 受信したWebhookがDodo Paymentsからのものであり、悪意のあるアクターからのものでないことを確認するには、次の手順に従ってください:
1. **署名を確認**: Dodo Paymentsから送信される各Webhookには、**`webhook-signature`** ヘッダーに一意の署名が含まれています。この署名は、Webhookの秘密鍵とイベントのペイロードを使用して生成されます。
2. **秘密鍵を取得**: Webhookの秘密鍵を持っていることを確認してください。これは、Dodo PaymentsダッシュボードのWebhook設定で見つけることができます。
3. **署名を再計算**: Dodo Paymentsと同じ署名アルゴリズム（HMAC-SHA256）を使用して、サーバー上で署名を計算します。WebhookのペイロードとWebhookの秘密鍵を使用します。
4. **署名を比較**: 再計算した署名を**`webhook-signature`** ヘッダーに提供された署名と比較します。一致すればWebhookは正当であり、一致しなければ拒否する必要があります。
5. **HTTPSを使用**: WebhookエンドポイントがHTTPSを使用して通信を保護し、データ送信中の改ざんを防ぐことを確認してください。
これらの手順を実施することで、Dodo Paymentsから送信されたWebhookの正当性を自信を持って確認できます。

If you only see GETs with ?status=success, you are debugging the redirect URL, not the webhook.
</Accordion>

<Accordion title="Q110: なぜ最初の月の支払いでもsubscription.renew Webhookを受信しているのですか？">
A: サブスクリプションの場合、`subscription.renew`は、サブスクリプション金額が差し引かれるたびにトリガーされます。したがって、これは予期される動作です。
</Accordion>
