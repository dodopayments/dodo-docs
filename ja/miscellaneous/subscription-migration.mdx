...
description: 現在の決済プロセッサからDodo Paymentsへの既存のサブスクライバーと支払い方法を移行する方法を学びます。
icon: file-import
---
...
<Warning>
現在、**カードトークン**の移行のみをサポートしています。Apple Pay、Google Pay、その他のウォレット決済方法は、これらのプロバイダーによって課せられたトークン化の制限により、決済プロセッサ間で移行できません。
</Warning>
...
<Note>
サブスクリプション移行を開始するには、[サポートチーム](mailto:support@dodopayments.com)に連絡してください。専任のソリューションエンジニアがプロセス全体を案内します。
</Note>
...
<AccordionGroup>
<Accordion title="PCI DSS準拠の確認">
Dodo PaymentsはPCI DSSレベル1に準拠しています。準拠義務を満たすために、他のPCI DSSレベル1準拠の決済プロセッサからのみカードデータを受け取ることができます。

現在のプロセッサは以下を提供する必要があります：
- 現在のPCI準拠証明書（AOC）またはVisaのサービスプロバイダーのグローバルレジストリへのリスト
- 安全なカードデータエクスポートをサポートしていることの確認

現在のプロセッサと共有するための[準拠証明書（AOC）](/assets/dodo-payments-pci-dss-aoc.pdf)をご覧ください。
</Accordion>

<Accordion title="Dodo Payments統合">
Dodo Paymentsの統合を完了し、すべての新しい顧客がDodo Paymentsのチェックアウトに誘導されることを確認してください。これにより、移行期間中のデータの不整合を防ぎます。
</Accordion>

<Accordion title="データアクセスの承認">
現在のプロセッサからデータエクスポートを要求するために必要な承認を持っていることを確認してください。これには通常、以下が必要です：
- アカウント所有者の確認
...
<Steps>
<Step title="現在のプロセッサからの支払い方法エクスポートのリクエスト">
現在の決済プロセッサのサポートチームに連絡し、顧客カードトークンの完全なエクスポートを正式にリクエストしてください。

以下を提供する必要があります：
- 身元と承認の確認
- Dodo PaymentsのPGP公開鍵を使用してエクスポートを暗号化するようにリクエスト
- 暗号化されたファイルを当社の安全なSFTPサーバーに直接転送するように依頼

<Info>
安全な転送は、サーバーからサーバーへSFTPを介して直接行われます。ソリューションエンジニアが現在のプロセッサと共有するためのSFTP資格情報を提供します。
</Info>

このプロセスは平均して約**2週間**かかりますが、タイミングは現在のプロセッサのポリシーや応答時間によって異なります。
</Step>

<Step title="サブスクライバーデータエクスポートの準備">
...
<Warning>
`card_token`フィールドは重要です。これは、インポート中にサブスクライバーのレコードとその支払い方法を一致させるために使用されます。このトークンが支払い方法エクスポートのものと正確に一致することを確認してください。
</Warning>
...
- 支払い方法データのファイルの整合性と暗号化を確認します
- 正しいフォーマットのためにサブスクライバーCSVを検証します
- サンドボックス環境でテストインポートを実行します
- データマッピングの問題を特定し解決します
...
1. PCI準拠のカードボールトに支払い方法データをインポートします
2. サブスクライバーデータをインポートします
3. カードトークンを使用してサブスクライバーをその支払い方法に一致させます
4. すべての移行されたサブスクリプションをアクティブにします

<Info>
標準的な移行の場合、典型的な処理時間は**2-3営業日**です。サブスクライバーの数によって異なります。
</Info>
</Step>
...
<Note>
支払い履歴と取引記録は移行できません。このデータを現在のプロセッサからエクスポートして、移行を完了する前に記録として保持することをお勧めします。
</Note>

## PGP暗号化

機密カードデータを転送する際の最大のセキュリティを確保するために、すべてのファイルは**必ず**当社のPGP公開鍵を使用して暗号化する必要があります。これにより、輸送レベルの暗号化を超えた追加の保護層が提供されます。
...
<Tip>
全体の移行は通常、開始から完了まで**3-4週間**かかります。支払い方法の移行が進行中の間にサブスクライバーデータの準備を開始することで、全体のタイムラインを短縮できます。
</Tip>

## 始めましょう

Dodo Paymentsへのサブスクリプション移行の準備はできましたか？プロセスを開始するために、当社のチームに連絡してください：

<CardGroup cols={2}>
<Card title="サポートに連絡" icon="envelope" href="mailto:support@dodopayments.com">
移行を開始し、ソリューションエンジニアとつながるためにお問い合わせください。
</Card>

<Card title="APIドキュメント" icon="code" href="/api-reference">
Dodo Paymentsでのサブスクリプションの仕組みを理解するために、APIを探索してください。
</Card>
</CardGroup>
