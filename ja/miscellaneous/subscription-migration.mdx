...
- 支払い方法データのファイル整合性と暗号化を確認する
- サブスクライバーの CSV が正しい形式であることを検証する
- Test Mode でテストインポートを実行する
- データマッピングに関する問題を特定して解決する
...
推奨事項:
- ダッシュボードで移行したサブスクリプションの一部を確認する
- Test Mode でいくつかのサブスクリプション更新をテストする
- 新しい ID を使用して社内データベースの参照を更新する
- 最初の請求サイクルでは並行モニタリングを実施する
</Step>
</Steps>
