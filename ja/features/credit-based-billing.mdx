...
<img src="/images/CBB/Checkout.png" alt="製品購入に含まれるクレジットを表示しているチェックアウト" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
クレジットを、トークン、API 呼び出し、コンピューティング時間、その他プロダクトにとって意味のある指標など、独自の単位で定義できます。カスタム単位では、設定した精度（小数点以下 0～10 桁）が使用されます。
...
| Source | Description |
|--------|-------------|
| **Subscription** | サブスクリプション購入で発行され、各請求サイクルで再発行されるクレジット |
| **One-Time** | 一回払いの商品で発行されるクレジット |
| **Add-on** | サブスクリプションに追加されたアドオンで発行されるクレジット |
| **API** | APIまたはダッシュボードから手動で付与されるクレジット |
| **Rollover** | 前回の請求サイクルから繰り越されるクレジット |
...
<img src="/images/CBB/Desktop - Entitlements  - Credits.jpg" alt="作成されたクレジット資格を表示するクレジット一覧ページ" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<img src="/images/CBB/Desktop - Create Credit.jpg" alt="基本情報、一般設定、サブスクリプション設定を表示するクレジット作成フォーム" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
カスタム単位のクレジットにのみ適用されます。許可される小数点以下の桁数は、`0` から `10` までです。

- `0` - 整数（API 呼び出しなど、数えられる項目に最適）
- `1` - 小数点以下 1 桁（0.0）
- `2` - 小数点以下 2 桁（0.00） - **デフォルト**
- `3` - 小数点以下 3 桁（0.000）
- 最大 `10` - 高精度の単位（例：小数トークンや微細な使用量）向け
...
<img src="/images/CBB/Desktop - Attach Credit - Subscription.jpg" alt="製品権限セクションに表示されるクレジット添付ボタン" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
<img src="/images/CBB/Desktop - Attach Credit - Subscription-2.jpg" alt="クレジット選択ドロップダウンを含むクレジット追加パネル" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<img src="/images/CBB/Desktop - Attach Credit - Subscription-4.jpg" alt="請求サイクル、試用、按分設定を含むクレジット構成フォーム" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
<img src="/images/CBB/Desktop - Attach Credit - Subscription-5.jpg" alt="詳細付きの選択されたクレジットを表示するクレジット追加パネル" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
</Steps>
...
<img src="/images/CBB/Desktop - Attach Credit - OTP.jpg" alt="単一支払いが選択された製品の価格設定セクション" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
<img src="/images/CBB/Desktop - Attach Credit - UBB.jpg" alt="使用量に基づく請求価格設定" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
<img src="/images/CBB/Desktop - Attach Credit - UBB-3.jpg" alt="無料のしきい値とクレジット切り替えを表示するメーターパネルを選択" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
<img src="/images/CBB/Desktop - Attach Credit - UBB-5.jpg" alt="クレジット選択とクレジットあたりのメーター単位を備えたメーター設定" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
<img src="/images/CBB/Desktop - Attach Credit - UBB-6.jpg" alt="UBB製品のためのクレジット設定" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
<img src="/images/CBB/Desktop - Attach Credit - UBB-1.jpg" alt="クレジット添付の詳細を示す構成済みメーター" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
</Steps>
...
<img src="/images/CBB/Desktop - Usage Billing.jpg" alt="クレジット消費を伴うイベントテーブルを表示する使用料金ダッシュボード" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<img src="/images/CBB/Checkout.png" alt="APIコールクレジットが含まれている製品を表示するチェックアウトページ" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<img src="/images/CBB/Customer Portal.jpg" alt="残高と取引履歴を含む顧客ポータルクレジットビュー" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<img src="/images/CBB/Desktop - Subscription Details.jpg" alt="権利と使用履歴を表示するサブスクリプション詳細ページ" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<img src="/images/CBB/Desktop - Transactions - Payment Summary.jpg" alt="クレジット資格を表示する取引詳細ページ" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<img src="/images/CBB/Desktop - Entitlements  - Credits.jpg" alt="製品セクションのクレジット一覧ページ" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<img src="/images/CBB/Desktop - Customer Details.jpg" alt="残高と取引を表示するクレジットタブ付きの顧客詳細ページ" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
|-----------------|-------------|
| **Credit Added** | 付与されたクレジット（サブスクリプション、一回払い、またはAPI） |
| **Credit Deducted** | 使用または手動の借方記入によって消費されたクレジット |
| **Credit Expired** | 繰り越されずに期限切れとなったクレジット |
| **Credit Rolled Over** | 次の期間に繰り越されたクレジット |
| **Rollover Forfeited** | 最大繰り越し回数に達した後に失効した繰り越しクレジット |
| **Overage Charged** | 超過料金が有効な状態で、クレジット残高を超えて使用された分 |
| **Overage Reset** | 請求サイクルのリセット時に追跡中の超過分が消去された状態 |
| **Auto Top-Up** | 残高が少なくなった際に自動で補充されるクレジット |
| **Manual Adjustment** | Merchantによって手動で適用されたクレジットまたは借方調整 |
| **Refund** | 返金されたクレジット |
...
| `credit.deducted` | 使用または手動の借方記入によって消費されたクレジット |
| `credit.expired` | 未使用のクレジットが期限切れになった状態 |
| `credit.rolled_over` | 新しい付与分に繰り越されたクレジット |
| `credit.rollover_forfeited` | 最大繰り越し回数に達した時点で失効したクレジット |
| `credit.overage_charged` | 超過料金が適用された状態 |
| `credit.overage_reset` | 請求サイクルの終了時に追跡中の超過分がリセットされた状態 |
| `credit.manual_adjustment` | クレジットまたは借方の調整が手動で行われた状態 |
| `credit.balance_low` | 残高が設定されたしきい値を下回った状態 |

すべての台帳イベント（`credit.balance_low` イベントを除くすべての `credit.*` イベント）には、残高の変更前後、超過分の変更前後、ソース参照、および付与分のソースとなるサブスクリプションまたは支払いの `metadata` を含む完全な `CreditLedgerEntry` ペイロードが含まれます（API経由で直接作成された付与分では空です）。`credit.balance_low` イベントには、しきい値設定と現在の残高が含まれます。
