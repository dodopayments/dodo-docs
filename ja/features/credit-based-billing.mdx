...
**構成:**
- クレジットタイプ: カスタム単位（"AI Tokens"）
- 精度: 0（整数トークン）
- ロールオーバー: 最大25%、期間1か月
- オーバーエイジ: 有効、請求時にオーバーエイジを請求
- メーター: `ai.generation` で `tokens` フィールドに対して合計集計
</Accordion>
...
**構成:**
- クレジットタイプ: カスタム単位（"API Calls"）
- 精度: 0（整数回）
- ロールオーバー: 無効
- オーバーエイジ: Developer+ プランはオーバーエイジを許可（リセット時に帳消し）、Freeプランはオーバーエイジを無効化
- メーター: `api.request` で集計カウント
</Accordion>
...
**構成:**
- クレジットタイプ: カスタム単位（"GB-hours"）
- 精度: 2（小数点以下2桁）
- ロールオーバー: 最大50%、1回繰り越し
- オーバーエイジ: 200%上限で有効
- メーター: `storage.usage` で合計集計
</Accordion>
</AccordionGroup>
...
- **シンプルに始める**: 最初は単一のクレジットタイプとロールオーバーなしで開始し、顧客のフィードバックと利用状況に応じて複雑さを追加します。
- **明確な期待値を設定**: クレジットの割り当て、残高、オーバーエイジ価格をプロダクトページとカスタマーポータルに明示的に表示します。
- **意味のある単位を使用**: クレジットを一般的な用語ではなく（例: "API Calls", "AI Tokens"）、表す内容に応じた名称にします。これにより顧客は価値を理解しやすくなります。
- **有効期限を慎重に設定**: 短期間（7日）の有効期限は緊急性を生む一方、顧客を苛立たせる可能性があります。多くのSaaS製品では30〜90日の長い期間が顧客に優しいです。
- **残高が少なくなったら監視**: 残高が少なくなった段階で顧客に通知を出し、予期せぬオーバーエイジ請求を減らします。
- **テストモードでテスト**: クレジットを作成し、テスト製品に付与し、購入→使用→差し引き→有効期限終了のサイクルを本番前にシミュレーションします。

<Info>
クレジットベースの請求は、トライアル付きのサブスクリプション、比例配分されたプラン変更、カスタマーポータルなど、他のすべてのDodo Payments機能とシームレスに連携します。基本的なセットアップから始め、価格モデルの進化に合わせて拡張してください。
</Info>
