...
はい。[Bring Your Own Processor (BYOP)](/features/byop) を使用すると、お客様自身の Stripe または Adyen アカウントを接続し、選択した国からの決済をそのアカウント経由で処理できます。その間も Dodo Payments は、プロダクト、サブスクリプション、請求、分析を引き続き提供します。明示的に経由先を指定していない国からの決済は、Dodo Payments をフルサービスの Merchant of Record として引き続き利用します。これにより、当該経路における税務上および不正利用に関する責任を当社が負うことができます。
</Accordion>
