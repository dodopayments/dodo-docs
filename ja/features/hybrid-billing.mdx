...
**解決策**:
1. イベントの取り込みが機能しているか確認する（APIレスポンスをチェック）
2. `customer_id` がサブスクリプション顧客に一致することを確認する
3. `event_name` がメーター設定に一致するか確認する
4. イベントのタイムスタンプが正しいことを確認する（未来の日付ではない）
</Accordion>
...
**解決策**:
1. `previewChangePlan` API を使用して、確認前に正確な料金を表示します。
2. プロレーションがサブスクリプションおよびアドオンに適用されることを伝えます。
3. より簡単なアップグレード請求のために `difference_immediately` を使用することを検討します。
</Accordion>
