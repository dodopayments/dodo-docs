...
<Info>
**近日提供予定**：座席数 + 使用量のハイブリッド請求に対するネイティブサポートをロードマップに追加しています。これにより、同じサブスクリプション商品に使用量メーターと座席アドオンの両方を関連付けられるようになります。
</Info>

### 仕組み

**Team Analytics：1ユーザーあたり\$20/月**

**各ユーザーに含まれる内容：**
- 月5GBのデータ処理
- 超過料金：利用枠を超えた分について1GBあたり\$2

**計算例**（10ユーザーのチームが合計80GBを使用した場合）：
- 座席料金：10 × \$20 = \$200.00
- 含まれるデータ：10 × 5GB = 50GB
- 超過料金：(80 - 50) × \$2 = \$60.00
- **合計：\$260.00/月**
...
- **分析プラットフォーム**：アナリスト単位 + データ処理
- **デザインツール**：デザイナー単位 + ストレージまたはエクスポート
- **開発環境**：開発者単位 + コンピューティング時間
- **コミュニケーションツール**：ユーザー単位 + メッセージまたは通話量

### 実装オプション

同じサブスクリプションに使用量メーターとアドオンの両方を関連付けることはできないため、次のいずれかの方法を選択します：

<Tabs>
<Tab title="Option A: Usage Product + App-Managed Seats">
使用量ベースのサブスクリプションを使用し、アプリケーション内で座席の請求を管理します。
...
座席にはアドオンを使用し、使用量はオンデマンド料金で手動請求します。
...
<Info>
**推奨**：オプションB（座席アドオン + オンデマンド使用量）は、Dodoが座席請求を自動的に処理するため、実装しやすいことが多い方法です。使用量の超過分を追跡して請求するだけで済みます。
</Info>

---

## ハイブリッドモデル5：サブスクリプション + 座席数 + 使用量（3要素のハイブリッド）

最も包括的なモデルです。プラットフォーム料金 + ユーザー単位の料金 + 消費量で構成されます。

<Warning>
**制約**：Dodo Paymentsでは現在、同じサブスクリプション商品に使用量メーターとアドオンの両方を関連付けることはできません。このモデルでは回避策が必要です。
</Warning>

<Info>
**近日提供予定**：3要素のハイブリッド請求（基本料金 + 座席数 + 使用量）に対するネイティブサポートをロードマップに追加しています。これにより、同じサブスクリプション商品に使用量メーターと座席アドオンの両方を関連付けられるようになります。
</Info>
...
- **プラットフォーム料金**：\$199/月
- **座席単位**：1ユーザーあたり\$25/月
- **使用量**：API呼び出し1,000回あたり\$0.10（50K回を含む）

**計算例**（20ユーザーの企業が150,000回のAPI呼び出しを行った場合）：
- プラットフォーム：\$199.00
- 座席：20 × \$25 = \$500.00
...
- **エンタープライズSaaS**：プラットフォーム + チーム + 消費量
- **データプラットフォーム**：ワークスペース + アナリスト + クエリ
- **インテグレーションプラットフォーム**：ハブ + コネクタ + トランザクション
- **AIプラットフォーム**：ワークスペース + 開発者 + 推論

### 実装オプション

3要素のハイブリッド請求を実装するには、次のいずれかの方法を選択します：

<Tabs>
<Tab title="Option A: Base + Seats (Add-on) + On-Demand Usage">
座席アドオン付きのサブスクリプションを使用し、オンデマンド料金で使用量を手動請求します。

**これは推奨される方法です**。Dodoがプラットフォーム料金と座席請求を自動的に処理するためです。
...
使用量メーター付きのサブスクリプションを使用し、アプリケーション内で座席請求を管理します。
...
<Step title="Manage Seats in Your Application">
座席数を追跡し、それに応じて基本サブスクリプション価格を調整します。
...
<Info>
**推奨**：オプションA（基本料金 + 座席数 + オンデマンド使用量）は、Dodoがプラットフォーム料金と座席請求を自動的に処理するため、一般的に実装しやすい方法です。使用量を追跡し、各請求サイクルの終了時に料金を送信するだけで済みます。
</Info>

---

## ハイブリッドモデル6：段階制基本料金 + 使用量超過料金

含まれる利用枠と超過料金率が異なる、複数のサブスクリプション段階を用意します。

### 仕組み

| 段階 | 価格 | 含まれる呼び出し数 | 超過料金率 |
|------|-------|----------------|--------------|
| **Starter** | \$19/月 | 1,000 | 1回あたり\$0.02 |
| **Pro** | \$79/月 | 25,000 | 1回あたり\$0.01 |
| **Business** | \$199/月 | 100,000 | 1回あたり\$0.005 |
| **Enterprise** | \$499/月 | 500,000 | 1回あたり\$0.002 |

### 実装

段階ごとに個別のサブスクリプション商品を作成し、それぞれに独自の使用量設定を構成します：
...
継続サブスクリプションに、サービスや超過分に対する変動する手動料金を組み合わせます。
...
<Info>
これらの例では、理想的な価格構成を示します。使用量メーターとアドオンを同じ商品に関連付けられないという制約があるため、一部の組み合わせでは回避策（使用量にはオンデマンド料金、座席数にはアプリケーション管理を使用）が必要です。
</Info>

### 例1：AI SaaSプラットフォーム

**価格構成：**
- **基本サブスクリプション**：\$99/月（プラットフォームアクセス、5座席を含む）
- **座席アドオン**：1座席あたり\$20/月
- **機能アドオン**：Custom Models（\$49/月）、API Access（\$29/月）、Priority Queue（\$19/月）
- **使用量超過料金**：100Kトークンを超えた分について1,000トークンあたり\$0.02（オンデマンドで請求）

**実装**：座席アドオンと機能アドオン付きのサブスクリプションを使用します。アプリケーション内でトークン使用量を追跡し、請求サイクルの終了時にオンデマンド料金で超過分を請求します。

**顧客例**（12ユーザー、500Kトークン、Custom Models + API Access）：
| 項目 | 計算 | 金額 |
|-----------|-------------|--------|
| 基本料金 | プラットフォーム料金 | \$99 |
| 追加座席 | 7 × \$20 | \$140 |
| Custom Models | アドオン | \$49 |
| API Access | アドオン | \$29 |
...
| **ストレージ** | 1 GB | 10 GB | 100 GB |

**実装オプション：**

**オプションA**（使用量重視）：ビルドとストレージ用の使用量メーターを備えた商品を作成します。ユーザーはアプリケーション内で管理します。

**オプションB**（座席数重視）：座席アドオン付きの商品を作成します。ビルドとストレージの使用量を追跡し、オンデマンドで超過料金を請求します。

**アドオン（オプションBを使用する場合）：**
- 追加ユーザー：1ユーザーあたり\$10/月
- 優先ビルド：\$19/月
- カスタムドメイン：1ドメインあたり\$9/月

### 例3：マーケティングオートメーション

**価格構成：**
- **基本プラン**：\$79/月（コア自動化機能、3座席を含む）
- **連絡先段階**（アドオン）：1Kを含む、5K（+\$30）、25K（+\$80）、100K（+\$200）
- **機能アドオン**：SMS Marketing（\$29/月）、Landing Pages（\$19/月）、A/B Testing（\$29/月）
- **チーム座席**：1ユーザーあたり\$15/月のアドオン
- **メール量**：アプリケーション内で追跡し、オンデマンドで超過分を請求（上限超過分1,000通あたり\$1）

**実装**：連絡先段階のアドオン、機能アドオン、座席アドオン付きのサブスクリプションを使用します。アプリケーション内でメール送信数を追跡し、オンデマンド料金で超過分を請求します。
...
<Tip>
ハイブリッド価格を理解しやすくします。価格ページでは、基本料金、含まれる内容、超過料金の仕組みを目立つように表示します。
</Tip>

**良い例**：「\$49/月で10,000回のAPI呼び出しを含む。追加の呼び出し：1回あたり\$0.005」
...
すべての構成要素を示す詳細な請求書を提供します：

| 明細 | 金額 |
|-----------|--------|
| Proプラン（月額） | \$49.00 |
| 追加座席（7 × \$15.00） | \$105.00 |
| API使用量 - 含まれる分（10,000回） | \$0.00 |
| API使用量 - 超過分（15,420回 × \$0.005） | \$77.10 |
| Advanced Analyticsアドオン | \$19.00 |
| **小計** | **\$250.10** |
| 税（8.5%） | \$21.26 |
| **請求合計** | **\$271.36** |
...
**症状**：使用量が0、または正しく表示されない。

**解決策**：
1. イベントの取り込みが機能していることを確認します（APIレスポンスを確認）
2. `customer_id` がサブスクリプションの顧客と一致することを確認します
3. `event_name` がメーター設定と一致することを確認します
4. イベントのタイムスタンプが正しいことを確認します（未来の日付になっていないこと）
</Accordion>
...
**解決策**：
1. 確定前に正確な料金を表示するため、`previewChangePlan` APIを使用します
2. 日割り計算がサブスクリプションとアドオンの両方に適用されることを伝えます
3. アップグレード請求を簡素化するために `difference_immediately` の使用を検討します
</Accordion>
...
**解決策**：
1. 使用量ベースの商品に無料しきい値が設定されていることを確認します
2. しきい値の単位がイベントの集計単位（呼び出し数とリクエスト数など）と一致することを確認します
3. 使用量メーターがサブスクリプション商品に正しく関連付けられていることを確認します
</Accordion>

<Accordion title="Add-ons not appearing in checkout">
**症状**：Checkout中に座席や機能を追加できない。

**解決策**：
1. ダッシュボードでアドオンがサブスクリプション商品に関連付けられていることを確認します
2. API呼び出しでアドオンIDが正しいことを確認します
3. アドオンの通貨がサブスクリプション商品の通貨と一致することを確認します
</Accordion>
</AccordionGroup>
