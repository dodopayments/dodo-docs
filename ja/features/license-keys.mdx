...
顧客がマニュアルモード製品を購入すると、権限が`pending`ステータスでキーなしで作成され、[`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) webhookが`integration_type: "license_key"`および`status: "pending"`とともに発生します。そのwebhookに反応するか、`integration_type`および`status`フィルターで[お客様の権限リスト](/api-reference/entitlements/list-customer-grants)エンドポイントをポーリングできます。
