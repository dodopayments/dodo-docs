...
お客様がライセンスキーを有効にした製品の購入を完了すると、ライセンスキーがクエリパラメータとして自動的にあなたの`return_url`に追加されます。これにより、追加のAPIコールを行うことなく、成功ページに即座にキーを表示することができます。

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001&email=customer%40example.com
```

購入で複数のライセンスキーが生成される場合（例：数量 > 1）、それらはカンマで区切られます：
...
<Tip>
あなたの戻りページで`license_key`パラメータを解析して、購入後の体験を向上させるために即座にお客様にキーを表示します。
</Tip>

## ベストプラクティス

- **限界を明確に保つ**: 有効期限とアクティベーションのために妥当なデフォルトを選択する
- **ユーザーをガイドする**: 正確なアクティベーション指示とセルフサーブドキュメントを提供する
- **チェックを自動化する**: アクセスを許可する前にサーバサイドでキーを検証する
- **イベントを監視する**: Webhooksを使用して不正を検知し、自動的に取り消す
