...
ISO 8601 timestamp (UTC)。省略した場合は、現在のサーバー時刻がデフォルトで使用されます。遅延イベントやバッチイベントを正確に請求するには、この値を含めてください。1時間を超えて古いタイムスタンプ、または5分を超えて未来のタイムスタンプは拒否されます。
</ParamField>
...
<Tip>
パフォーマンス向上のため、リクエストごとに最大1,000件のイベントを一括処理します。APIは1回のコールにつき、1,000件のイベントのハードリミットを強制します。
</Tip>
