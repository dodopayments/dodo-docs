---
title: Payment Methods
sidebarTitle: Overview
description: Complete guide to payment methods supported by Dodo Payments,
  including credit/debit cards, digital wallets, BNPL, UPI/Rupay, European
  options, and how to configure them for your checkout.
icon: layer-group
keywords:
  - Dodo Payments
  - payment methods
  - payment platform
  - SaaS billing
---

Dodo Payments supports a wide range of payment methods to help you serve customers globally. Turn on new payment methods instantly — no additional contracts, integrations, or merchant accounts needed.

## Why Offer Multiple Payment Methods?

<CardGroup cols={3}>
<Card title="Boost Conversion" icon="chart-line">
Local methods increase sales 20-30% in target markets.
</Card>

<Card title="One Platform" icon="layer-group">
All methods managed through a single API.
</Card>

<Card title="Smart Routing" icon="route">
Right options shown based on location and device.
</Card>
</CardGroup>

## How It Works

Payment methods are automatically presented to customers based on:

- **Customer Location** — Regional methods like iDEAL (Netherlands) or UPI (India) only appear for customers in those regions
- **Transaction Currency** — Payment methods are filtered by supported currencies
- **Device Type** — Apple Pay only appears on compatible Apple devices and browsers
- **Transaction Type** — Some methods don't support subscriptions or have minimum amounts

<Info>
**Automatic Activation:** All payment methods are pre-configured and managed by Dodo Payments. No additional setup needed — they become available once your account is verified and live mode is enabled.
</Info>

## Supported Payment Methods

| 支払い方法 | 対応国 | 対応通貨 | サブスクリプション | 最低金額 |
| :----------------------- | :------------------- | :------------------- | :-----------: | :--------- |
| **クレジットカード** | グローバル | すべての通貨 | はい | $0.50 |
| **デビットカード** | グローバル | すべての通貨 | はい | $0.50 |
| **Apple Pay** | グローバル (インド除く) | すべての通貨 | はい | $0.50 |
| **Google Pay** | グローバル (インド除く) | すべての通貨 | はい | $0.50 |
| **Amazon Pay** | グローバル (インド除く) | USD | いいえ | $0.50 |
| **Cash App Pay** | 米国 | USD | いいえ | $0.50 |
| **Klarna** | 米国、ヨーロッパ | USD、EUR、GBP + 7通貨 | いいえ | $50.01 |
| **Afterpay** | 米国、英国 | USD、GBP | いいえ | $50.01 |
| **Sunbit** | 米国 | USD | いいえ | $60.00 (最大 $19,999) |
| **UPI** | インド | INR | はい* | ₹1 |
| **Rupay** | インド | INR | はい* | ₹1 |
| **iDEAL** | オランダ | EUR | いいえ | €0.50 |
| **Bancontact** | ベルギー | EUR | いいえ | €0.50 |
| **EPS** | オーストリア | EUR | いいえ | €0.50 |
| **Multibanco** | ポルトガル | EUR | いいえ | €0.50 |
| **RevolutPay** | グローバル | GBP | いいえ | £0.50 |
| **Billie** | グローバル | GBP | いいえ | £0.50 |
| **Pix** | ブラジル | BRL | いいえ | $0.50 |
| **Stablecoins** | グローバル (インド除く) | USD | いいえ | $0.50 |
| **WeChat Pay** | グローバル | USD、CNY | いいえ | $0.50 |

<Note>
*インド発行のすべてのカード（Visa、Mastercard、Rupayなど）およびUPIは、RBI準拠のマンダートをサポートしています。48時間の処理遅延は、インド発行のすべてのカードとUPIに適用されます。マンダート要件の詳細については、<a href="/features/payment-methods/india">インドの支払い方法</a>ページをご覧ください。
</Note>

<Warning>
**PayPal Support Paused:** PayPal is temporarily unavailable for new transactions. We'll update this page when restored.
</Warning>

## Payment Method Categories

<CardGroup cols={2}>
<Card title="Credit & Debit Cards" icon="credit-card" href="/features/payment-methods/cards">
Accept Visa, Mastercard, Amex, Discover, JCB, UnionPay, and regional networks like Cartes Bancaires and Interac.
</Card>

<Card title="Digital Wallets" icon="wallet" href="/features/payment-methods/digital-wallets">
Apple Pay, Google Pay, Amazon Pay, Cash App Pay, and RevolutPay for fast, secure one-tap checkout.
</Card>

<Card title="Buy Now Pay Later" icon="calendar-days" href="/features/payment-methods/bnpl">
Klarna、Afterpay、Billie、Sunbitの分割払いオプションで、平均注文額と転換率を向上。
</Card>

<Card title="India" icon="indian-rupee-sign" href="/features/payment-methods/india">
UPI and Rupay cards with RBI-compliant subscription mandates for the Indian market.
</Card>

<Card title="Europe" icon="euro-sign" href="/features/payment-methods/europe">
iDEAL (Netherlands), Bancontact (Belgium), EPS (Austria), and Multibanco (Portugal).
</Card>

<Card title="Pix" icon="brazilian-real-sign" href="/features/payment-methods/pix">
Instant bank transfers for the Brazilian market via QR code or copy-paste key.
</Card>

<Card title="Stablecoins" icon="dollar-sign" href="/features/payment-methods/stablecoins">
インド以外の全世界の顧客からステーブルコインによる支払いを受け付け、USDで請求します。
</Card>

<Card title="WeChat Pay" icon="comment-dollar" href="/features/payment-methods/wechat">
QR code payments for Chinese customers via WeChat, supporting USD and CNY.
</Card>
</CardGroup>

## ビジネスレベルの支払い方法の無効化

ダッシュボードからビジネスレベルで特定の支払い方法を無効にすることができます。これは、商品やセッションごとに設定する必要なく、ビジネスのすべてのチェックアウトセッションに適用されます。

<Steps>
<Step title="Navigate to Settings">
Dodo Paymentsのダッシュボードで**設定 > 支払い方法**に移動します。
</Step>

<Step title="Disable Payment Methods">
提供したくない支払い方法を無効にします。
</Step>
</Steps>

<Tip>
この機能を使用して、地域で詐欺率の高い支払い方法を無効にしたり、ビジネスが運用上サポートする方法のみに制限したりできます。
</Tip>

<Info>
ビジネスレベルの設定はグローバルフィルタとして機能します。`allowed_payment_method_types`によるチェックアウトごとの制限で、ビジネスレベルで有効化された方法の中からさらに利用可能な方法を絞り込みます。
</Info>

## チェックアウトごとの支払い方法の設定

ビジネスレベルの設定に加えて、`allowed_payment_method_types`を使用して、個別のチェックアウトセッションに表示される方法を制限できます。

<Tabs>
<Tab title="JavaScript">

```javascript
const session = await client.checkoutSessions.create({
  product_cart: [{ product_id: 'prod_123', quantity: 1 }],
  allowed_payment_method_types: [
    'credit',
    'debit', 
    'apple_pay',
    'google_pay'
  ],
  return_url: 'https://example.com/success'
});
```

</Tab>

<Tab title="Python">

```python
session = client.checkout_sessions.create(
    product_cart=[{"product_id": "prod_123", "quantity": 1}],
    allowed_payment_method_types=[
        "credit",
        "debit",
        "apple_pay", 
        "google_pay"
    ],
    return_url="https://example.com/success"
)
```

</Tab>

<Tab title="cURL">

```bash
curl -X POST https://test.dodopayments.com/checkouts \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "product_cart": [{"product_id": "prod_123", "quantity": 1}],
    "allowed_payment_method_types": ["credit", "debit", "apple_pay", "google_pay"],
    "return_url": "https://example.com/success"
  }'
```

</Tab>
</Tabs>

### 利用可能な方法の種類

| タイプ | 説明 |
| :--- | :---------- |
| `credit` | クレジットカード (Visa, Mastercard, Amexなど) |
| `debit` | デビットカード |
| `apple_pay` | Apple Pay |
| `google_pay` | Google Pay |
| `amazon_pay` | Amazon Pay |
| `cashapp` | Cash App Pay |
| `klarna` | Klarna BNPL |
| `afterpay_clearpay` | Afterpay / Clearpay |
| `upi_collect` | UPI Collect (QRコード) |
| `ideal` | iDEAL (オランダ) |
| `bancontact_card` | Bancontact (ベルギー) |
| `eps` | EPS (オーストリア) |
| `multibanco` | Multibanco (ポルトガル) |
| `revolut_pay` | RevolutPay |
| `billie` | Billie B2B BNPL |
| `sunbit` | Sunbit BNPL (米国) |
| `pix` | Pix (ブラジル) |
| `crypto` | Stablecoins |
| `wechat_pay` | WeChat Pay |

| タイプ | 説明 |
| :--- | :---------- |
| `credit` | クレジットカード（Visa、Mastercard、Amex など） |
| `debit` | デビットカード |
| `apple_pay` | Apple Pay |
| `google_pay` | Google Pay |
| `amazon_pay` | Amazon Pay |
| `cashapp` | Cash App Pay |
| `klarna` | Klarna BNPL |
| `afterpay_clearpay` | Afterpay / Clearpay |
| `upi_collect` | UPI Collect (QRコード) |
| `ideal` | iDEAL (オランダ) |
| `bancontact_card` | Bancontact (ベルギー) |
| `eps` | EPS (オーストリア) |
| `multibanco` | Multibanco (ポルトガル) |
| `przelewy24` | Przelewy24 / P24 (ポーランド) |
| `blik` | BLIK (ポーランド) |
| `satispay` | Satispay (ヨーロッパ) |
| `payco` | PAYCO (韓国) |
| `naver_pay` | Naver Pay (韓国) |
| `kakao_pay` | Kakao Pay (韓国) |
| `revolut_pay` | RevolutPay |
| `billie` | Billie B2B BNPL |
| `sunbit` | Sunbit BNPL (米国) |
| `pix` | Pix (ブラジル) |
| `crypto_currency` | ステーブルコイン |
| `we_chat_pay` | WeChat Pay |

### APIエンドポイント

<CardGroup cols={3}>
  <Card title="Checkout Sessions" icon="cart-shopping" href="/api-reference/checkout-sessions/create">
    許可された方法でチェックアウトを作成します。
  </Card>
  <Card title="Payments" icon="credit-card" href="/api-reference/payments/post-payments">
    方法フィルタリングによる一回限りの支払い。
  </Card>
  <Card title="Subscriptions" icon="repeat" href="/api-reference/subscriptions/post-subscriptions">
    方法制限による継続的な支払い。
  </Card>
</CardGroup>

## ベストプラクティス

<AccordionGroup>
<Accordion title="Match methods to your market">
ターゲット市場向けに地域の支払い方法を含める。iDEALはオランダのオンライン支払いの約60％を占め、UPIはインドで支配的です。これらの方法を省略すると、コンバージョンに大きな影響があります。
</Accordion>

<Accordion title="Always provide card fallbacks">
特定の方法をターゲットにする場合であっても、`credit`と`debit`をフォールバックとして含めます。すべての顧客がすべての支払い方法にアクセスできるわけではありません。
</Accordion>

<Accordion title="Consider transaction minimums">
BNPLメソッドにはプロバイダーごとの最低額があります。KlarnaとAfterpayは$50.01、Sunbitは$60.00（最大$19,999）を必要とします。これらの範囲を超える製品には含めないでください — 単純に表示されませんが問題ありません。
</Accordion>

<Accordion title="Test across regions and devices">
テストモードを使用して、異なる顧客の場所、通貨、デバイスタイプに対する支払い方法の利用可能性を確認してからライブに移行します。
</Accordion>
</AccordionGroup>

## トラブルシューティング

<AccordionGroup>
<Accordion title="Payment method not appearing">
**これらの要因を確認してください:**
1. メソッドが`allowed_payment_method_types`に含まれていますか？
2. 顧客の所在地がメソッドの対応地域と一致しますか？
3. トランザクション通貨がメソッドに対応していますか？
    4. トランザクション額がメソッドの対応範囲内ですか（例: Klarna/Afterpay最小$50.01; Sunbit $60.00–$19,999）？
5. サブスクリプションについて: メソッドが定期支払いをサポートしていますか？

**解決策：** 一時的に`allowed_payment_method_types`を削除して、所有している方法をすべて表示し、問題を絞り込みます。
</Accordion>

<Accordion title="Unexpected methods appearing">
**可能性のある原因：**
- `allowed_payment_method_types`が正しく渡されていない
- パラメータのフォーマットが間違っている（文字列配列である必要があります）

**解決策：** APIリクエストペイロードをログし、パラメータ構造を確認します。
</Accordion>

<Accordion title="Regional method not available">
**確認事項：**
- 顧客の請求先住所が方法の地域に一致していること
- 通貨が正しく設定されていること（例：iDEALに対してEUR）
- 顧客が国際的な場合、アダプティブ通貨が有効になっていること

**解決策：** APIリクエストで請求先住所の国コードと通貨を確認します。
</Accordion>

<Accordion title="Indian payment method issues">
UPIおよびRupayのサブスクリプションには、48時間の処理遅延やRBIのマンダート制限など、特有の要件があります。

**参照：** <a href="/features/payment-methods/india">インドの支払い方法</a>の完全なガイダンス。
</Accordion>
</AccordionGroup>

## 関連ページ

<CardGroup cols={2}>
<Card title="Adaptive Currency" icon="globe" href="/features/adaptive-currency">
サポートされている通貨と自動変換。
</Card>

<Card title="Checkout Guide" icon="book" href="/developer-resources/checkout-session">
完全なチェックアウト実装ガイド。
</Card>

<Card title="Testing Process" icon="flask" href="/miscellaneous/testing-process">
テストカード、UPI ID、およびウォレットテスト。
</Card>

<Card title="Subscriptions" icon="repeat" href="/features/subscription">
定期請求と支払い方法の更新。
</Card>
</CardGroup>

## 支払いオプションの拡大

新しい支払い方法を継続的に追加しています。ここに記載されていない特定の方法が必要な場合は、[support@dodopayments.com](mailto:support@dodopayments.com)までご連絡ください。

新しい支払い方法を随時追加しています。ここに記載されていない特定の方法が必要な場合は、[support@dodopayments.com](mailto:support@dodopayments.com) までご連絡ください。