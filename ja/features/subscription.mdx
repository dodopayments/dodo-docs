...
| 遷移 | イベント |
|------------|-------|
| Subscription が有効化されました | `subscription.active` |
| 更新に成功しました | `subscription.renewed` |
| 更新に失敗 → 一時停止 | `subscription.on_hold` |
| Subscription が一時停止されました | `subscription.paused` |
| 一時停止中の Subscription が再開されました | `subscription.unpaused` |
| 作成に失敗しました | `subscription.failed` |
| Plan がアップグレード／ダウングレードされました | `subscription.plan_changed` |
| キャンセルされました | `subscription.cancelled` |
| 期間が終了しました | `subscription.expired` |
| 任意のフィールドが変更されました | `subscription.updated` |
...
次回の請求日にキャンセルする、Subscription の期間を延長する、請求の詳細を更新する、またはメタデータを変更するには、`PATCH /subscriptions/{subscription_id}` を使用します。数量を変更する場合は、代わりに [Plan 変更 API](/api-reference/subscriptions/change-plan) を使用してください — `PATCH` は `quantity` を受け付けません。
...
### Subscription の期間を延長する
新しい `subscription_period_count` と `subscription_period_interval` を `PATCH /subscriptions/{subscription_id}` に渡すことで、Subscription の期間を延長できます。Subscription の有効期限は、新しい回数と間隔に基づいて再計算されます。たとえば、現在の Plan に追加の期間を付与する場合は、次のようにします。
