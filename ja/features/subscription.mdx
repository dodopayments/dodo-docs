...
プラン変更を確定する前に、正確な料金と結果のサブスクリプションをプレビューします：
...
<Info>
`on_hold` サブスクリプションの支払い方法を正常に更新した後、`payment.succeeded` と `subscription.active` のウェブフックイベントを受け取ります。
</Info>
...
`POST /subscriptions` を使用して、製品からプログラム的にサブスクリプションを作成します。トライアルやアドオンもオプションです。
...
サブスクリプションの支払い方法を更新します。アクティブなサブスクリプションの場合、これは今後の更新のために支払い方法を更新します。`on_hold` 状態のサブスクリプションの場合、残りの未払い料金の請求を作成することでサブスクリプションを再活性化します。
...
- **SaaSおよびAPI**: シートや使用量のための階層的アクセス
- **コンテンツおよびメディア**: 月額アクセスと導入トライアル
- **B2Bサポートプラン**: プレミアムサポートアドオン付きの年間契約
- **ツールおよびプラグイン**: ライセンスキーとバージョンリリース

## 統合例

### チェックアウトセッション（サブスクリプション）
チェックアウトセッションを作成する際は、サブスクリプション製品とオプションのアドオンを含めます：
...
## RBI準拠のmandateを持つサブスクリプション

  UPIおよびインドのカードサブスクリプションは、特定のmandate要件を持つRBI（インド準備銀行）規制の下で運営されています：

  ### Mandateの制限

  Mandateの種類と金額は、サブスクリプションの定期的な料金に依存します：

  - **₹15,000未満の料金**: ₹15,000 INRのオンデマンドmandateを作成します。サブスクリプションの金額は、サブスクリプションの頻度に応じて定期的に請求され、mandateの制限まで請求されます。
  - **₹15,000以上の料金**: 正確なサブスクリプション金額のためにサブスクリプションmandate（またはオンデマンドmandate）を作成します。

  ### アップグレードおよびダウングレードの考慮事項

  **重要:** サブスクリプションをアップグレードまたはダウングレードする際は、mandateの制限を慎重に考慮してください：

  - アップグレード/ダウングレードにより、料金が₹15,000を超え、既存のオンデマンド支払い制限を超える場合、取引料金が失敗する可能性があります。
  ...
  **処理タイムライン:** インドのカードおよびUPIサブスクリプションの定期的な料金は、独自の処理パターンに従います：
  ...
  - 実際の**控除**は、支払い開始から**48時間**後に顧客の口座から行われます。
  - この48時間のウィンドウは、銀行のAPI応答に応じて**2-3時間**延長される場合があります。

  ### Mandateキャンセルウィンドウ

  48時間の処理ウィンドウ中：

  - 顧客は銀行アプリを通じてmandateをキャンセルできます。
  ...
  **エッジケースの処理:** 料金開始時に顧客に利益、クレジット、またはサブスクリプションの使用を即座に提供する場合、この48時間のウィンドウを適切にアプリケーションで処理する必要があります。考慮すべき点：
  ...
  <Tip>
  支払い状況の変更を追跡し、48時間のウィンドウ中にmandateがキャンセルされた場合のエッジケースを処理するために、サブスクリプションのウェブフックを監視してください。
  </Tip>
...
<Info>
サブスクリプションは、定期的な収益のための柔軟な基盤です。シンプルに始め、徹底的にテストし、採用、解約、拡張の指標に基づいて反復してください。
</Info>