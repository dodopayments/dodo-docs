...
<Warning>
`pause` は他のすべての field と排他的です。他の field と一緒に送信すると `422` で拒否されます。`status` を `paused` に設定してもサブスクリプションは一時停止されません。代わりに `pause` field を使用してください。
</Warning>
...
|--------|---------------|--------------|---------------------------|
| `pending` | サブスクリプションを作成または処理中です | — | `subscription.active`（または `subscription.failed`）を待ちます |
| `active` | サブスクリプションはアクティブで、自動的に更新されます | — | 対応不要 |
| `on_hold` | 更新 payment（または plan-change charge）が失敗し、更新が停止しています | **Yes** | [Payment Retries](/features/recovery/payment-retries) と [Dunning](/features/recovery/subscription-dunning) により自動的に、または [Update Payment Method API](/api-reference/subscriptions/update-payment-method) により手動で payment method を更新して再有効化します |
| `paused` | サブスクリプションはあなたまたは顧客によって意図的に一時停止され、billing が凍結され、アクセスが取り消されています | **Yes** | `pause: false` で再開するか、セルフサービスの一時停止が有効な場合は Customer Portal から再開します。[Pausing and Resuming Subscriptions](#pausing-and-resuming-subscriptions) を参照してください |
| `cancelled` | サブスクリプションはキャンセルされ、更新されません | 再購入のみ | 顧客は新しいサブスクリプションを開始する必要があります。[Dunning](/features/recovery/subscription-dunning) により再購入を促せます |
| `failed` | サブスクリプションの **creation** に失敗しました（initial mandate または payment が成功しませんでした） | **No — terminal** | 顧客は有効な payment method を使用して新しいサブスクリプションを作成する必要があります |
...
一時停止と再開は、`pause` field を使用して同じ `PATCH /subscriptions/{subscription_id}` endpoint で行います。`pause: true` はアクティブなサブスクリプションを一時停止し、`pause: false` は再開します。この field は同じ request 内で他の field と組み合わせることはできません。動作、billing への影響、関連する business settings の詳細については、[Pausing and Resuming Subscriptions](#pausing-and-resuming-subscriptions) を参照してください。
