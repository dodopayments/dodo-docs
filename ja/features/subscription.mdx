...
<Info>
プラン変更では即時請求が発生します。請求に失敗すると、サブスクリプションが `on_hold` 状態になることがあります。変更は `subscription.plan_changed` webhook event で追跡してください。
</Info>
...
`PATCH /subscriptions/{id}` を使用して数量の更新、次回請求日のキャンセル、metadata の変更を行います。
