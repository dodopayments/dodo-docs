...
title: インドの支払い方法
sidebarTitle: インド
description: RBIのコンプライアンスに基づいたサブスクリプションの指令を使って、インドでUPIと全てのインド発行カードを受け入れます。48時間の処理遅延、指令の制限、Webhookの処理について理解します。
...
インドは独自の決済インフラを持ち、UPI（デジタルトランザクションの60％以上）とインド発行カード（Visa、Mastercard、Rupayなど）が支配しています。Dodo Paymentsは、サブスクリプション指令に関して完全なRBIコンプライアンスと共にこれらすべてをサポートします。
...
ほとんどの代替支払い方法とは異なり、UPIおよびすべてのインド発行カード（Visa、Mastercard、Rupayなど）はRBI指令を通じて定期支払いをサポートします。
</Card>
</CardGroup>
...
*サブスクリプションには特別な処理ルールを持つRBIコンプライアントな指令が必要です。48時間の処理遅延は、すべてのインド発行カードとUPIに適用されます。
...
<Accordion title="Payment deduction delayed beyond 48 hours">
**原因:** 銀行APIの遅延により処理がさらに2～3時間延長されることがあります。

**解決策:** これは予想される事象です。システムを構築し、合計約51時間までの変動する遅延に対応できるようにしてください。
</Accordion>

<Accordion title="Mandate cancelled but subscription still active">
**原因:** RBI規制のエッジケース — 処理ウィンドウ内での委任状のキャンセルは、すぐにサブスクリプションをキャンセルしません。

**解決策:** 次の請求は失敗し、サブスクリプションは`on_hold`に移行します。`payment.failed`のウェブフックを監視してください。
</Accordion>
</AccordionGroup>
