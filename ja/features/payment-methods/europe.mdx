...
<Step title="適切な請求先住所を設定">
請求先住所の国を支払い方法に合わせて設定:
- `NL` は iDEAL 用
- `BE` は Bancontact 用
- `AT` は EPS 用
- `PT` は Multibanco 用
...
<Accordion title="カードのフォールバックを提供">
すべてのヨーロッパの顧客がこれらの地域の方法にアクセスできるわけではありません（観光客、 expatriates など）。常に `credit` と `debit` をフォールバックとして含めてください。
</Accordion>
...
**解決策:** ヨーロッパのメソッドは厳密に地域的です。請求先国が `DE` （ドイツ）の顧客は、オランダ専用の iDEAL を表示できません。
</Accordion>
