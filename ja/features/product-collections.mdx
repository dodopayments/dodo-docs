...
| アクション | 説明 | Merchant Control |
|--------|-------------|------------------|
| **現在のプランを表示** | 現在の商品名、価格、更新日を表示 | 常に利用可能 |
| **プランをアップグレード** | 同じコレクション内の上位ティアの商品に移行 | 設定可能（デフォルト：無効） |
| **プランをダウングレード** | 同じコレクション内の下位ティアの商品に移行 | 設定可能（デフォルト：無効） |
...
| 設定 | 説明 | デフォルト |
|---------|-------------|---------|
| **複数のサブスクリプションを許可** | 顧客が同時に複数のアクティブなサブスクリプションを保有できる | 有効 |
| **サブスクリプションの更新を許可** | 顧客がCustomer Portalを通じて、既存のサブスクリプションをいつでもアップグレードまたはダウングレードできる | 無効 |
| **サブスクリプションのキャンセルを許可** | 顧客がCustomer Portalから自身のサブスクリプションをキャンセルできる | 有効 |
| **サブスクリプションの一時停止を許可** | 顧客がCustomer Portalから自身のサブスクリプションを一時停止および再開できる | 無効 |
| **一時停止時に使用量を請求** | 使用量ベースのサブスクリプションを一時停止した時点で、未請求の使用量を請求書に含める。次回の通常の請求書まで使用量を繰り越さない | 有効 |
| **トライアルの不正利用を防止** | すでにトライアルを利用した顧客を、新たなトライアルを付与せず、有料・トライアルなしの購入に変更する | 無効 |
...
<Info>
「サブスクリプションのキャンセルを許可」を無効にすると、顧客向けのセルフサービスによるキャンセルが非表示になり、Customer Portal API経由で行われたキャンセルリクエストが拒否されます。マーチャント側で行うキャンセルには影響しません。[サブスクリプションのキャンセル](/features/customer-portal#cancelling-a-subscription)を参照してください。
</Info>

<Info>
「Allow Subscription Pause」はCustomer Portalのみに適用されます。設定にかかわらず、ダッシュボードまたはAPIから一時停止および再開できます。「Bill Usage at Pause」は使用量ベースのサブスクリプションにのみ適用され、請求サイクルごとに記録されます。そのため、サイクルの途中で設定を変更しても、進行中のサイクルの精算方法は変わりません。[サブスクリプションの一時停止と再開](/features/subscription#pausing-and-resuming-subscriptions)を参照してください。
</Info>

<Info>
トライアルの利用がどのように照合・記録されるかについては、[トライアルの不正利用を防止](/features/subscription#preventing-trial-misuse)を参照してください。
</Info>

<Card title="Subscription Plan Changes" icon="repeat" href="/features/subscription#subscription-plan-changes">
  日割り計算モードとプラン変更の動作について詳しく説明します。
</Card>

## コレクションの管理

Product Collectionsは、Dodo PaymentsダッシュボードまたはAPIを介してプログラムで管理できます。APIでは、コレクションの作成、更新、画像のアップロード、アーカイブ、ネストされたグループや商品の管理を完全に制御できます。

### ダッシュボード操作

- **作成**: 商品とグループを含む新しいコレクションを設定
- **更新**: 名前、説明、画像、商品の構成を変更
- **並べ替え**: ドラッグ＆ドロップで商品の表示順を変更
- **商品の有効化／無効化**: チェックアウトに表示する商品を制御
- **アーカイブ**: コレクションを完全に削除せず非表示にする（後でアーカイブ解除可能）

<Frame>
  <img 
    src="/images/product-collection/collection-dashboard.png" 
    alt="コレクション管理操作を示すProduct Collectionダッシュボードのスクリーンショット" 
  style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>

### API管理

次のエンドポイントを使用すると、Product Collectionsをプログラムで作成、更新、取得、アーカイブ、整理できます。ネストされたグループと、その中の商品も管理できます。

<AccordionGroup>
<Accordion title="Listing Product Collections">
`GET`リクエストを`/product-collections`エンドポイントに送信して、アカウントに関連付けられたすべてのProduct Collectionsを取得します。ページネーション、ブランドによるフィルタリング、アーカイブ済みコレクションの取得に対応しています。

<Card title="List Product Collections API" icon="code" href="/api-reference/product-collections/list-product-collections">
  詳細なリクエストおよびレスポンスの構造については、List Product Collections APIのドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Creating a Product Collection">
名前、説明、ブランドなどの詳細を含むリクエストを`POST`エンドポイントに送信して、新しいProduct Collectionを作成します。

<Card title="Create Product Collection API" icon="code" href="/api-reference/product-collections/create-product-collection">
  詳細なリクエストおよびレスポンスの構造については、Create Product Collection APIのドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Retrieving a Product Collection">
`GET`リクエストを`/product-collections/{id}`エンドポイントに送信して、特定のProduct Collectionに関する詳細情報（グループと商品アイテムを含む）を取得します。

<Card title="Get Product Collection API" icon="code" href="/api-reference/product-collections/get-product-collection">
  詳細なリクエストおよびレスポンスの構造については、Get Product Collection APIのドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Updating a Product Collection">
`PATCH`リクエストを`/product-collections/{id}`エンドポイントに送信して、Product Collectionの詳細（名前、説明、ブランドなど）を変更します。

<Card title="Update Product Collection API" icon="code" href="/api-reference/product-collections/update-product-collection">
  詳細なリクエストおよびレスポンスの構造については、Update Product Collection APIのドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Uploading Collection Images">
署名付きURLを使用して画像をアップロードし、コレクションに関連付けます。`/product-collections/{id}/images`エンドポイントからアップロードURLをリクエストし、返されたURLに60秒以内に画像を`PUT`します。

<Warning>
署名付きURLは60秒で期限切れになるため、その時間内に画像をアップロードする必要があります。
</Warning>
...
<Card title="Update Collection Images API" icon="code" href="/api-reference/product-collections/update-product-collection-images">
  詳細なリクエストおよびレスポンスの構造については、Update Collection Images APIのドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Archiving a Product Collection">
`DELETE`リクエストを`/product-collections/{id}`エンドポイントに送信して、コレクションをアーカイブします。これにより新たな利用からコレクションが非表示になりますが、完全に削除されるわけではありません。

<Card title="Archive Product Collection API" icon="code" href="/api-reference/product-collections/archive-product-collection">
  詳細なリクエストおよびレスポンスの構造については、Archive Product Collection APIのドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Unarchiving a Product Collection">
`POST`リクエストを`/product-collections/{id}/unarchive`エンドポイントに送信して、アーカイブ済みのコレクションを復元します。
...
<Card title="Unarchive Product Collection API" icon="code" href="/api-reference/product-collections/unarchive-product-collection">
  詳細なリクエストおよびレスポンスの構造については、Unarchive Product Collection APIのドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Managing Groups within a Collection">
グループを使用すると、コレクション内の商品を整理できます（例：「月額プラン」と「年額プラン」）。グループのエンドポイントを使用して、コレクション内のグループを追加、更新、削除します。
...
<Card title="Create Group" icon="code" href="/api-reference/product-collections/create-group">
  Product Collectionに新しいグループを追加します。
</Card>
...
グループ内の個々の商品アイテムを管理します。新しい商品の追加、既存のアイテム（表示順など）の更新、または完全な削除が可能です。

- **グループに商品を追加**: `POST /product-collections/{id}/groups/{group_id}/items`
- **グループアイテムを更新**: `PATCH /product-collections/{id}/groups/{group_id}/items/{item_id}`
- **グループアイテムを削除**: `DELETE /product-collections/{id}/groups/{group_id}/items/{item_id}`
...
- **論理的にグループ化する**: 請求間隔（月額／年額）または機能ティア（starter／pro／enterprise）ごとに商品を整理する
- **戦略的に並べる**: 最も人気のあるプランまたは推奨プランを先頭に配置する。チェックアウト時にあらかじめ選択されます
- **わかりやすい名前を使用する**: 商品名で価値の違いを明確に伝える
- **両方向を有効にする**: 顧客に柔軟性を提供するため、アップグレードとダウングレードの両方を許可する
- **日割り計算を検討する**: ビジネスモデルに合った日割り計算モードを選択する
- **十分にテストする**: 本番稼働前に、テストモードでチェックアウトとプラン変更のフローを確認する

<Check>
Product Collectionsを作成し、顧客に統一されたプラン選択エクスペリエンスを提供する準備が整いました。
</Check>

<CardGroup cols={2}>
<Card title="Products" icon="box" href="/features/products">
  コレクションに追加する単発、サブスクリプション、または使用量ベースの商品を作成します。
</Card>

<Card title="Checkout" icon="cart-shopping" href="/features/checkout">
  統一されたチェックアウトエクスペリエンスでコレクションの商品を表示します。
</Card>
