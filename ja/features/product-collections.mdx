...
| 設定 | 説明 | デフォルト |
|---------|-------------|---------|
| **複数のサブスクリプションを許可** | 顧客が同時に複数のアクティブなサブスクリプションを保有できるようにします | 有効 |
| **サブスクリプションの更新を許可** | 顧客がCustomer Portalを介して、既存のサブスクリプションをいつでもアップグレードまたはダウングレードできるようにします | 無効 |
| **サブスクリプションのキャンセルを許可** | 顧客がCustomer Portalから自分のサブスクリプションをキャンセルできるようにします | 有効 |
| **トライアルの不正利用を防止** | すでにトライアルを利用した顧客には、新しいトライアルを付与せず、トライアルなしの有料購入に切り替えます | 無効 |
...
<Card title="Subscription Plan Changes" icon="repeat" href="/features/subscription#subscription-plan-changes">
  按分モードとプラン変更の動作について詳しく説明します。
</Card>

## コレクションの管理

商品コレクションは、Dodo PaymentsのダッシュボードまたはAPI経由のプログラムで管理できます。APIでは、コレクションの作成、更新、画像のアップロード、アーカイブ、ネストされたグループや商品の管理を完全に制御できます。

### ダッシュボードでの操作

- **作成**: 商品やグループを含む新しいコレクションを設定します
- **更新**: 名前、説明、画像、商品の構成を変更します
- **並べ替え**: ドラッグ＆ドロップで商品の表示順を変更します
- **商品の有効化／無効化**: チェックアウトに表示する商品を制御します
- **アーカイブ**: コレクションを完全に削除せずに非表示にします（後でアーカイブを解除できます）

<Frame>
  <img 
    src="/images/product-collection/collection-dashboard.png" 
    alt="商品コレクションのダッシュボードにコレクション管理操作が表示されているスクリーンショット" 
  style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>

### API管理

次のエンドポイントを使用すると、ネストされたグループやその中の商品を含め、商品コレクションをプログラムで作成、更新、取得、アーカイブ、整理できます。

<AccordionGroup>
<Accordion title="Listing Product Collections">
アカウントに関連付けられたすべての商品コレクションを、`GET`リクエストで`/product-collections`エンドポイントから取得します。ページネーション、ブランドによるフィルタリング、アーカイブ済みコレクションの取得に対応しています。

<Card title="List Product Collections API" icon="code" href="/api-reference/product-collections/list-product-collections">
  詳細なリクエストとレスポンスの構造については、「List Product Collections API」のドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Creating a Product Collection">
名前、説明、ブランドなどの詳細を含む`POST`リクエストを`/product-collections`エンドポイントに送信して、新しい商品コレクションを作成します。

<Card title="Create Product Collection API" icon="code" href="/api-reference/product-collections/create-product-collection">
  詳細なリクエストとレスポンスの構造については、「Create Product Collection API」のドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Retrieving a Product Collection">
特定の商品コレクションについて、グループや商品項目を含む詳細情報を、`GET`リクエストで`/product-collections/{id}`エンドポイントから取得します。

<Card title="Get Product Collection API" icon="code" href="/api-reference/product-collections/get-product-collection">
  詳細なリクエストとレスポンスの構造については、「Get Product Collection API」のドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Updating a Product Collection">
`PATCH`リクエストを`/product-collections/{id}`エンドポイントに送信して、商品コレクションの詳細（名前、説明、ブランドなど）を変更します。

<Card title="Update Product Collection API" icon="code" href="/api-reference/product-collections/update-product-collection">
  詳細なリクエストとレスポンスの構造については、「Update Product Collection API」のドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Uploading Collection Images">
事前署名付きURLを介して画像をアップロードし、コレクションに関連付けます。`/product-collections/{id}/images`エンドポイントからアップロードURLをリクエストし、返されたURLに60秒以内に画像を`PUT`します。
...
<Card title="Update Collection Images API" icon="code" href="/api-reference/product-collections/update-product-collection-images">
  詳細なリクエストとレスポンスの構造については、「Update Collection Images API」のドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Archiving a Product Collection">
`DELETE`リクエストを`/product-collections/{id}`エンドポイントに送信して、コレクションをアーカイブします。これにより新たな利用からは非表示になりますが、完全に削除されることはありません。

<Card title="Archive Product Collection API" icon="code" href="/api-reference/product-collections/archive-product-collection">
  詳細なリクエストとレスポンスの構造については、「Archive Product Collection API」のドキュメントを参照してください。
</Card>
</Accordion>

<Accordion title="Unarchiving a Product Collection">
`POST`リクエストを`/product-collections/{id}/unarchive`エンドポイントに送信して、アーカイブ済みコレクションを復元します。
...
<Card title="Unarchive Product Collection API" icon="code" href="/api-reference/product-collections/unarchive-product-collection">
  詳細なリクエストとレスポンスの構造については、「Unarchive Product Collection API」のドキュメントを参照してください。
</Card>
</Accordion>
...
グループ内の個々の商品項目を管理します。新しい商品の追加、既存の項目（表示順など）の更新、項目の完全な削除が可能です。
...
- **論理的にグループ化する**: 請求間隔（月額／年額）または機能階層（スターター／プロ／エンタープライズ）で商品を整理します
- **戦略的に並べる**: 最も人気のあるプランや推奨プランを先頭に配置します。チェックアウト時にあらかじめ選択されます
- **わかりやすい名前を使用する**: 商品名から価値の違いが明確に伝わるようにします
- **両方向を有効にする**: 顧客が柔軟に選択できるよう、アップグレードとダウングレードの両方を許可します
- **按分を検討する**: ビジネスモデルに適した按分モードを選択します
- **十分にテストする**: 本番環境で公開する前に、テストモードでチェックアウトとプラン変更のフローを確認します

<Check>
これで商品コレクションを作成し、顧客に統一されたプラン選択エクスペリエンスを提供できます。
</Check>
...
<Card title="Checkout" icon="cart-shopping" href="/features/checkout">
  コレクションの商品を統一されたチェックアウトエクスペリエンスで表示します。
</Card>
