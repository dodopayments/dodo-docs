...
Checkout sessions、payments、subscriptionsでは、`discount_codes`配列を介して、最大 **20個の重複コード**（最大20エントリ）を受け付けます。コードは **配列の順序で**適用されるため、最初に適用可能なコードが開始価格を最初に引き下げ、次のコードは割引適用後の価格をさらに引き下げる、という順序になります。[購買力平価](/features/purchasing-power-parity)が有効な場合、開始価格には基本価格ではなく、PPP調整済みの金額が使用されます。適用された割引の完全なセットは、レスポンスの`discount_ids`（payments/subscriptionsの場合）および`discounts`（位置や残りのサブスクリプションサイクルを含む、割引ごとのより詳細な情報）に返されます。
...
### 割引コードフィールドを非表示にする
割引入力はデフォルトで表示されるため、事前にコードを渡していなくても、顧客はいつでもコードを入力できます。フィールドを完全に非表示にするには、`allow_discount_code` を `false` に設定してください。
