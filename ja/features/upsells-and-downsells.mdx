...
| モード | 動作 | 最適な用途 |
|------|----------|----------|
| `difference_immediately` | 価格差を即時に請求（$30→$80 = $50） | シンプルなアップグレード |
| `prorated_immediately` | 請求サイクルの残り時間に基づいて請求 | 時間に応じた公平な請求 |
| `full_immediately` | 新しいプランの料金全額を請求し、残り時間は考慮しない | 請求サイクルをリセット |
| `do_not_bill` | 即時請求なしでプラン変更を適用。新しいプランは次回更新時に請求され、元の請求日は維持される | 好意によるアップグレードと無料移行 |
