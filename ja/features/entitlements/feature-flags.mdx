...
| トリガー | 効果 |
|---|---|
| 一回限りの支払いが成功する / サブスクリプションがアクティブになる | `status: Delivered` と `delivered_at` が設定された Grant が作成されます。 |
| サブスクリプションが保留、キャンセル、または期限切れになる | 対応する `revocation_reason` によって Grant が取り消されます。 |
| 一回限りの支払いが返金される | `revocation_reason: refund` によって Grant が取り消されます。 |
| サブスクリプションが回復する（例：督促による支払いが成功する） | 取り消された Grant が `Delivered` に復元されます — 同じ Grant の `id` が維持され、取り消しフィールドがクリアされます。 |
| 手動による API revoke | `revocation_reason: manual` によって Grant が取り消されます。更新時に自動復元されることはありません。 |
