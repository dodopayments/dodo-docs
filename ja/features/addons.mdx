...
- **価格設定**: アドオンの一回限りの価格または継続的な価格を設定します
- **通貨**: Dodo Payments が請求できる任意の通貨で基本価格を設定します。検索可能な通貨セレクターでは、**USD、GBP、EUR、INR** が上部に固定表示されます。基本通貨以外の通貨圏にいる顧客には、[Adaptive Currency](/features/adaptive-currency) を通じて請求されます
- **請求サイクル**: サブスクリプションの請求サイクルと一致している必要があります
- **数量制限**: 顧客ごとの最小数量と最大数量を設定します
- **利用可能性**: アドオンを利用できるサブスクリプション商品を制御します
- **税設定**: 適切な税区分を設定します
