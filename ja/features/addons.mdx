...
顧客に利益をもたらす追加の機能、サービス、または容量を特定します。
...
<AccordionGroup>
<Accordion title="アドオンの作成">
カスタム価格、説明、および設定オプションを使用して新しいアドオンを作成するには、`POST /addons`エンドポイントを使用します。
...
<Accordion title="アドオンの更新">
価格、説明、または可用性を更新するには、`PATCH /addons/{id}`エンドポイントを使用して既存のアドオンを修正します。
...
すべてのアドオンをリストするには`GET /addons`を使用するか、特定のアドオンの詳細を取得するには`GET /addons/{id}`を使用します。
...
製品のプレゼンテーションを向上させるために、`PUT /addons/{id}/images`エンドポイントを使用してアドオンの画像を更新します。
