...
価格、説明、または可用性を更新するために、`PATCH /addons/{id}` エンドポイントを使用して既存のアドオンを変更します。
...
すべてのアドオンをリストするには `GET /addons` を、特定のアドオン詳細を取得するには `GET /addons/{id}` を使用します。
...
製品プレゼンテーションを向上させるために、`PUT /addons/{id}/images` エンドポイントを使用してアドオン画像を更新します。
