...
<Frame>
  <img src="/images/product-collection/checkout-page.png" alt="製品コレクション" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
定期購入製品の場合、`price` オブジェクトを定期価格 (`type: recurring_price`) に設定し、`payment_frequency_interval` と `subscription_period_interval` (`Day`、`Week`、`Month`、または `Year`) を対応するカウントと共に指定します。
...
製品を表示または使用したくない場合、`DELETE` リクエストを `/products/{id}` エンドポイントに送信してアーカイブできます。この操作により製品が非表示になりますが、完全に削除されることはありません。
