...
| イベント | 動作 |
|---|---|
| `payment.succeeded`（1回限り） | grantを発行します。grantには約15分間有効な署名付きダウンロードURLが含まれます。顧客は、メールリンクまたはCustomer Portalのページを再度開くことでURLを更新できます。 |
| `subscription.active` | grantを発行します。サブスクリプションが有効な間、ファイルにアクセスできます。 |
| `subscription.renewed` | 何もしません。同じgrantが継続され、取得するたびに新しい署名付きURLが生成されます。 |
| `subscription.on_hold` / `cancelled` / `expired` | grantを取り消します。新しい署名付きURLは発行されなくなります。 |
| `subscription.paused` | grantを取り消します。サブスクリプションが再開されるまで、新しい署名付きURLは発行されません。 |
| `subscription.plan_changed` | 古いgrantを取り消し、新しいプランのentitlement用に新しいgrantを発行します。 |
| `refund.succeeded`（1回限り） | grantを取り消します。 |
| 手動での取り消し | `revocation_reason: manual`で取り消します。 |
