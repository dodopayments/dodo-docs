...
description: Set your own fixed price for each currency or country instead of
  letting exchange rates decide. Charge round, market-tuned prices and run
  per-country promotions.
tag: NEW
icon: earth-americas
keywords:
  - Dodo Payments
...
| メカニズム | その機能 | 顧客に表示される数値 |
| :--- | :--- | :--- |
| **ローカライズされた価格設定** | 通貨または国ごとに設定する固定価格 | あなたが設定したそのままの金額 |
| [**適応通貨**](/features/adaptive-currency) | 基本価格をライブの外国為替レートで自動的に変換 | 現行の為替レートで変換された基本価格 |
| [**割引コード**](/features/discount-codes) | 基本価格からの割合減少 | 基本価格から割引を引いた額 |
