...
<Info>
このセクションでは`customization.theme_config`を使った**サーバーサイドAPI**テーマ設定について説明しています。**Checkout SDK**（オーバーレイまたはインラインチェックアウト）をご利用の場合は、camelCaseプロパティ（例：`bgPrimary`ではなく`bg_primary`）を使用する[Overlay Checkout](/developer-resources/overlay-checkout#theme-customization)または[Inline Checkout](/developer-resources/inline-checkout#theme-customization)のテーマカスタマイズセクションをご覧ください。
</Info>
...
各モード（`light`および`dark`）は次のカラー設定プロパティをサポートします：
...
| `text_placeholder` | プレースホルダーテキストの色 |
| `text_error` | エラーテキストの色 |
| `text_success` | 成功テキストの色 |
| `border_primary` | ボーダーのプライマリカラー |
| `border_secondary` | ボーダーのセカンダリカラー |
| `button_primary` | プライマリボタン背景色 |
| `button_primary_hover` | プライマリボタンのホバーカラー |
| `button_secondary` | セカンダリボタン背景色 |
| `button_secondary_hover` | セカンダリボタンのホバーカラー |
| `button_text_primary` | プライマリボタンテキストの色 |
| `button_text_secondary` | セカンダリボタンテキストの色 |
| `input_focus_border` | 入力フォーカス時のボーダー色 |

<Info>
すべてのカラー項目は標準のCSSカラーフォーマットを受け入れます：
- 16進：`#fff`、`#ffffff`、`#ffffffff`
...
<Tip>
すべてのカラー設定を指定する必要はありません。未指定のプロパティはデフォルトテーマ値が使用されます。
</Tip>
