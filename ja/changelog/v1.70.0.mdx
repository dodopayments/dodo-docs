...
1. **インラインチェックアウト - 完全に埋め込まれたチェックアウト体験 (BETA)**  
   ...
   SDKを初期化し、`displayType: 'inline'`を使用してチェックアウトをコンテナ要素に埋め込みます:
   ...
   - **リアルタイムの内訳イベント**: 顧客が詳細を入力する際に`checkout.breakdown`イベントを受信します
   - **カスタム注文サマリー**: リアルタイムで更新される独自の価格表示を構築します
   - **シームレスな統合**: チェックアウトフレームが支払いの収集を処理し、レイアウトを制御します
   - **SDKメソッド**: チェックアウトをプログラム的に制御するために`open()`、`close()`、および`isOpen()`を使用します
...
2. **既存の支払い方法を使用して支払い、サブスクリプション、およびチェックアウトを作成する**  
...
5. **`redirect_immediately` フラグ - 支払い成功ページをスキップ**  
   新しい`redirect_immediately`フラグを使用してチェックアウトフローを制御します。有効にすると、顧客は支払い完了後にすぐにリダイレクトされ、デフォルトの成功ページをバイパスして、より迅速でスムーズな体験が得られます。
  ...
  ```
   ...
   <Tip>
   デフォルトの支払い成功ページよりも優れたユーザーエクスペリエンスを提供するカスタム成功ページがある場合は`redirect_immediately: true`を使用してください。
   </Tip>

   <Note>
   `redirect_immediately`が有効になっている場合、顧客は支払い完了後にすぐにあなたの`return_url`にリダイレクトされ、デフォルトの成功ページを完全にスキップします。
   </Note>
