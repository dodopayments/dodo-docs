...
1. **インラインチェックアウト - 完全に埋め込まれたチェックアウト体験 (BETA)**  
   ...
   SDKを`displayType: 'inline'`で初期化し、チェックアウトをコンテナ要素に埋め込みます：
   ...
   - **リアルタイムの内訳イベント**: 顧客が詳細を入力する際に`checkout.breakdown`イベントを受け取ります
   - **カスタムオーダーサマリー**: リアルタイムで更新される独自の価格表示を構築します
   - **シームレスな統合**: チェックアウトフレームが支払い収集を処理し、レイアウトを制御します
   - **SDKメソッド**: チェックアウトをプログラムで制御するために`open()`、`close()`、および`isOpen()`を使用します
...
2. **既存の支払い方法を使用して支払い、サブスクリプション、チェックアウトを作成する**  
    既存の顧客からの保存された支払い方法を使用してチェックアウトフローを合理化します。新しい`payment_method_id`パラメータを使用すると、顧客の以前に保存された支払い方法を使用して支払い、サブスクリプション、またはチェックアウトセッションを作成できます。
   ...
   <Warning>
   チェックアウトセッションで`payment_method_id`を使用する場合、`confirm`は`true`に設定する必要があり、既存の`customer_id`が提供されなければなりません。支払い方法は、支払いの通貨に対して適格性が検証されます。
   </Warning>
...
5. **`redirect_immediately`フラグ - 支払い成功ページをスキップする**  
   新しい`redirect_immediately`フラグでチェックアウトフローを制御します。これを有効にすると、顧客は支払い完了後すぐにリダイレクトされ、デフォルトの成功ページをバイパスして、より迅速でスムーズな体験が得られます。
  ...
  ```
   ...
   <Tip>
   デフォルトの支払い成功ページよりもユーザーエクスペリエンスが良いカスタム成功ページがある場合は、`redirect_immediately: true`を使用してください。
   </Tip>

   <Note>
   `redirect_immediately`が有効な場合、顧客は支払い完了後すぐにあなたの`return_url`にリダイレクトされ、デフォルトの成功ページが完全にスキップされます。
   </Note>
