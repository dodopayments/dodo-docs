---
title: AutoSend
description: AutoSendのメールAPIを使用して、Dodo
  Paymentsのトランザクション、返金、および支払いイベントの自動化されたトランザクションメールを送信する方法を学びます。
icon: paper-plane
---

## はじめに

AutoSendとDodo Paymentsの統合により、成功したトランザクションから失敗した試行、返金確認まで、すべての支払いイベントに対してリアルタイムのメール通知を自動的に送信できます。
...
Dodo PaymentsとAutoSendを統合するための手順は次のとおりです。
...
<Step title="APIキーを入力">
  ...
  <Card title="APIキーの作成と管理方法を学ぶ" icon="key" href="https://docs.autosend.com/api-keys">
    APIキーの作成と管理に関する詳細な手順については、AutoSendのドキュメントを訪問してください。
  </Card>
</Step>
...
- **パーソナライズのために動的データを使用する**: `dynamicData`フィールドを使用して、顧客固有の情報（名前、支払い金額、サブスクリプションの詳細など）でメールをパーソナライズします。パーソナライズされたメールは、エンゲージメント率が高くなります。

- **明確な件名を書く**: メールの目的を明確に示す説明的な件名を書く。スパムを引き起こす単語を避け、件名は簡潔に（50文字以内）保ちます。

- **本番前にテストする**: 本番環境で送信する前に、常にメールをテストします。これにより、メールコンテンツが正しく表示され、すべての動的データが適切にマッピングされていることが確認できます。
...
- ペイロードに必要なパラメータが含まれていることを確認
</Accordion>

<Accordion title="変換エラー">
- JSON構造がAutoSend APIフォーマットに一致していることを確認
- すべての必要なフィールドが存在することを確認（`to`, `from`, `templateId`または`html`/`text`)
- メールアドレスが正しくフォーマットされていることを確認
- テンプレートを使用している場合は`templateId`が有効であることを確認
