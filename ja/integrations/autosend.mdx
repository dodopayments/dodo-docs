...
- **個人化のために動的データを利用する**: `dynamicData` フィールドを使用して、名前、支払い金額、サブスクリプションの詳細など、顧客固有の情報でメールを個人化します。個人化されたメールはより高いエンゲージメント率を持っています。
...
<AccordionGroup>
<Accordion title="メールが送信されない場合">
- APIキーが正しくアクティブであることを確認する
- AutoSendで送信者ドメインが確認されていることを確認する
- 受信者のメールアドレスが有効であることを確認する
- AutoSendの送信制限とクォータを確認する
- APIエンドポイントURLが正しいことを確認する: `https://api.autosend.com/v1/mails/send`
- 必要なパラメータがペイロードに存在することを確認する
</Accordion>

<Accordion title="変換エラー">
- JSON構造がAutoSend APIフォーマットに一致していることを確認する
- すべての必須フィールドが存在することを確認する (`to`, `from`, `templateId` または `html`/`text`)
- メールアドレスが正しくフォーマットされていることを確認する
- テンプレートを使用している場合は`templateId`が有効であることを確認する
- `dynamicData`キーがテンプレート変数と一致していることを確認する
</Accordion>

<Accordion title="テンプレートの問題">
- テンプレートIDが正しくアクティブであることを確認する（AutoSend内）
- `dynamicData`キーがテンプレートで使用される変数と一致していることを確認する
- すべての必要なテンプレート変数が提供されていることを確認する
- AutoSendダッシュボードでテンプレートを独立してテストする
</Accordion>
</AccordionGroup>
