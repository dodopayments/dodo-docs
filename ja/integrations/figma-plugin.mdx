...
[Figmaのプラグインクイックスタートガイド](https://www.figma.com/plugin-docs/plugin-quickstart-guide/)を使用して、自分のFigmaプラグインプロジェクトを作成します。これにより、`manifest.json`ファイルを含むスキャフォールドが生成されます。
...
<Step title="環境の設定">
`src/ui/api.ts`で`API_MODE`を設定します:
...
<Warning>
開発中は、実際の支払いを処理しないように、常に`test_mode`を使用してください。
</Warning>
</Step>
...
- 有効なライセンスキーを持つユーザー向けに機能を公開するために`Authenticated.tsx`を更新します
