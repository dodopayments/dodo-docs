...
[Figmaのプラグインクイックスタートガイド](https://www.figma.com/plugin-docs/plugin-quickstart-guide/)を使用して、自分自身のFigmaプラグインプロジェクトを作成します。これにより、`manifest.json`ファイルを持つスキャフォールドが生成されます。
...
<Step title="サンプル実装のクローン">
[dodopayments-figmaリポジトリ](https://github.com/dodopayments/dodopayments-figma)をクローンし、生成されたプラグインフォルダからの名前とIDを使用してその`manifest.json`を更新します。
...
<Step title="コンポーネントのカスタマイズ">
プラグインコンポーネントをニーズに合わせてカスタマイズします:

- `Authenticated.tsx`を更新して、有効なライセンスキーを持つユーザーの機能を公開します
...
<Steps>
<Step title="ネットワークアクセスの設定">
プラグインはCORSプロキシと通信するためにネットワークアクセスが必要です。次の内容を`manifest.json`に追加します:
...
<Tip>
本番用の準備が整い、テストモードで徹底的にテストが完了したときのみ、`live_mode`に切り替えてください。
</Tip>
</Step>
</Steps>
...
2. テストモードと本番モードの両方で徹底的にテスト
3. [Figmaのプラグイン公開ガイドライン](https://www.figma.com/plugin-docs/publishing-plugins/)に従う
4. Figma Communityを通じてレビューを提出
