...
2. **訪問者IDを** 支払いメタデータに格納します
3. **Payment APIを使用して、支払いが成功したときにDataFastに支払いデータを送信します**
...
<Step title="DataFastスクリプトのインストール">
  まず、ウェブサイトにDataFastトラッキングスクリプトをインストールします。これにより、訪問者を追跡する`datafast_visitor_id`クッキーが作成されます。
  
  プラットフォームに特有のインストール手順については、[DataFastのドキュメント](https://datafa.st/docs)を訪れてください。
</Step>
...
- 支払いメタデータに`datafast_visitor_id`がキャプチャされ、保存されていることを確認してください
- ウェブフック変換が正しくペイロードをフォーマットしていることを確認してください
- `payment.succeeded`イベントでウェブフックがトリガーされていることを確認してください
...
- JSON構造がDataFastのPayment API形式に一致していることを確認してください
- 必須フィールド（`amount`、`currency`、`transaction_id`、`datafast_visitor_id`）がすべて存在することを確認してください
- 金額が正しく変換されていることを確認してください（ほとんどの通貨の場合は100で割ります。ただし、小数点がゼロの場合を除きます）
