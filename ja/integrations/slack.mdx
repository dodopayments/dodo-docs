...
<Step title="Open the Webhook Section">
  Dodo Paymentsダッシュボードの<b>Webhook</b>セクションに移動します。<b>+ エンドポイント追加</b>ボタンをクリックし、webhookドロップダウンを開いて他の統合を表示します。
  <Frame>
    <img src="/images/integrations/slack/1.png" alt="Add Endpointボタンと統合ドロップダウンを表示しているDodo Paymentsダッシュボード" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>

<Step title="Select Slack Integration">
  <b>Slack</b>統合を選択し、<b>Connect your Slack workspace</b>をクリックします。
  <Frame>
    <img src="/images/integrations/slack/2.png" alt="Slack統合カードと接続ボタン" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>

<Step title="Grant Slack Permissions">
  Incoming Webhooks Slackアプリに必要な権限を付与し、選択したチャネルにメッセージを投稿できるようにします。
  <Frame>
    <img src="/images/integrations/slack/3.png" alt="Incoming WebhooksアプリのためのSlack OAuth権限画面" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>

<Step title="Customize Transformation Code">
  変換コードを追加または編集して、あなたのユースケースに合わせたSlack通知をカスタマイズします。あらかじめ用意されたテンプレートを使用するか、自分でロジックを書きます。
  <Frame>
    <img src="/images/integrations/slack/4.png" alt="Slack統合のための変換コードエディター" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>

<Step title="Test and Create">
  カスタムまたは事前に作成されたイベントペイロードを使用して変換コードをテストします。満足したら、<b>Create</b>をクリックして統合を有効にします。
  <Frame>
    <img src="/images/integrations/slack/5.png" alt="変換テストと作成ボタン" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>
...
Slack通知を効果的にするには：
- 明確性と実用性を高めるため、色、フィールド、書式設定を備えたリッチメッセージの添付ファイルを使用します。
- すばやく識別できるよう、金額、顧客のメールアドレス、IDなどの重要なデータを必ず含めます。
- イベントの種類に合った色を選択します：成功には緑（`good`）、disputeまたは失敗には赤（`danger`）、キャンセルには黄色（`warning`）、情報イベントには青（`#3aa3e3`）を使用します。
- 各イベントの発生時刻を追跡できるよう、タイムスタンプを追加します。
