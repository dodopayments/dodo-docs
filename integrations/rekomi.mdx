...
<Note>
The example below uses `POST /payments`, which is **deprecated**. It still works for existing integrations, but new integrations should use [Checkout Sessions](/developer-resources/checkout-session) (`POST /checkouts`) above; `metadata` is passed the same way.
</Note>
