...
icon: "wordpress"
keywords: ["Dodo Payments", "WooCommerce", "WordPress", "ecommerce plugin", "woocommerce plugin"]
---
...
14. (Optional) Use the **Checkout Feature Flags** settings section to control the hosted checkout's `feature_flags` directly from the plugin settings page. Each flag — including currency selection, discount codes, tax ID, phone number collection, addon editing, and the customer field-editing options — offers a tri-state select of **Default**, **Enabled**, or **Disabled**. Leave a flag on **Default** to let the API keep control of its own default; only flags you explicitly set are sent with the checkout request. For per-order overrides in code, use the `dodo_payments_checkout_session_feature_flags` filter.
