...
- `UNSUPPORTED_BILLING_CURRENCY`
  - **Trigger:** 요청된 청구 통화는 이 구독에서 지원되지 않습니다
  - **Message:** USD가 아닌 청구 통화는 구독에서 지원되지 않습니다
...
- `UNSUPPORTED_CURRENCY`
  - **Trigger:** 상품 또는 addon의 통화는 Dodo Payments에서 청구할 수 있는 통화가 아닙니다. 기본 가격은 청구 가능한 모든 통화로 설정할 수 있으므로, 이는 일반적으로 통화 코드가 잘못되었거나 아직 지원되지 않음을 의미합니다.
  - **Message:** 현재 통화가 지원되지 않음 / 현재 USD 및 INR 상품만 지원됨 / addon 가격에는 USD 및 INR만 지원됨 / billing_currency에는 USD 또는 INR만 요청할 수 있음 / 지원되지 않는 통화 / 인도 카드 구독에 예기치 않은 통화
