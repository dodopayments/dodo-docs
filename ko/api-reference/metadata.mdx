...
- 메타데이터 값은 문자열, 숫자 또는 불리언일 수 있으며, 문자열은 최대 500자까지 가능합니다.
- 객체, 배열 및 `null`는 메타데이터 값으로 허용되지 않습니다.
- 객체당 최대 50개의 메타데이터 키-값 쌍을 사용할 수 있습니다.
- 키는 영숫자, 대시, 밑줄만 포함해야 합니다.
- 메타데이터는 API를 통해 검색할 수 없지만, API 응답과 웹훅에 반환됩니다.
