...
| 실패 코드 | 사용자 오류 | 설명 |
|----------------|------------|-------------|
| `AUTHENTICATION_FAILURE` | 예 | 거래 중에 인증 과정이 실패했습니다. |
| `AUTHENTICATION_REQUIRED` | 예 | 거래를 완료하기 위해 추가 인증이 필요합니다. |
| `AUTHENTICATION_TIMEOUT` | 예 | 인증 과정이 시간 초과되었습니다. |
| `CARD_DECLINED` | 아니요 | 카드가 발급 은행에 의해 거부되었습니다. |
| `CARD_NOT_ACTIVATED` | 예 | 카드 소유자가 카드를 활성화하지 않았습니다. |
| `CARD_VELOCITY_EXCEEDED` | 예 | 짧은 시간에 너무 많은 거래가 시도되었습니다. |
| `CUSTOMER_CANCELLED` | 예 | 고객이 거래를 취소했습니다. |
| `DO_NOT_HONOR` | 아니요 | 발급 은행이 거래를 거부했습니다. |
| `EXPIRED_CARD` | 예 | 카드가 만료되었습니다. |
| `FRAUDULENT` | 예 | 거래가 잠재적으로 사기성이라고 표시되었습니다. |
| `GENERIC_DECLINE` | 아니요 | 특정 사유 없이 거래가 거부되었습니다. |
| `INCORRECT_CVC` | 예 | 제공된 CVC 코드가 잘못되었습니다. |
...
| `MANDATE_INVALID` | 예 | 지불 승인 요청이 유효하지 않습니다. |
| `MANDATE_REQUIRED` | 예 | 이 거래에는 승인이 필요합니다. |
| `MANDATE_REQUIRED_SYSTEM` | 아니요 | 시스템에서 이 거래 유형에 대한 승인이 필요합니다. |
| `NETWORK_ERROR` | 아니요 | 거래 중 네트워크 오류가 발생했습니다. |
| `NETWORK_TIMEOUT` | 아니요 | 네트워크 요청이 시간 초과되었습니다. |
| `ORDER_CREATION_FAILED` | 아니요 | 거래를 위한 주문 생성에 실패했습니다. |
| `PAYMENT_METHOD_PROVIDER_DECLINED` | 예 | 결제 수단 제공자가 거래를 거부했습니다. |
| `PAYMENT_METHOD_UNSUPPORTED` | 예 | 이 거래에 대해 지원되지 않는 결제 수단입니다. |
| `PICKUP_CARD` | 예 | 카드가 도난으로 보고되었으며 회수해야 합니다. |
...
| `STOLEN_CARD` | 예 | 카드가 도난으로 보고되었습니다. |
| `TRANSACTION_NOT_ALLOWED` | 예 | 이 카드 또는 계좌에 대해 거래가 허용되지 않습니다. |
| `TRANSACTION_NOT_APPROVED` | 예 | 거래가 승인되지 않았습니다. |
| `TRY_AGAIN_LATER` | 아니요 | 거래는 나중에 다시 시도해야 합니다. |
| `UNKNOWN_ERROR` | 아니요 | 알 수 없는 오류가 발생했습니다. 

<Note>
  **사용자 오류**는 결제 거부가 고객에 의해 해결될 수 있는지를 나타냅니다. `true`인 경우, 고객은 문제를 해결하기 위해 조치를 취할 수 있습니다(예: 올바른 카드 세부정보 입력). `false`인 경우, 거부는 고객이 해결할 수 없는 시스템 수준의 문제나 은행 제한으로 인해 발생합니다.
</Note>
