...
| 모드 | 동작 | 적합한 용도 |
|------|----------|----------|
| `difference_immediately` | 가격 차액을 즉시 청구합니다($30→$80 = $50) | 간단한 업그레이드 |
| `prorated_immediately` | 결제 주기에 남은 시간을 기준으로 청구합니다 | 시간 기준의 공정한 청구 |
| `full_immediately` | 새 플랜의 전체 가격을 청구하고 남은 시간은 무시합니다 | 결제 주기 재설정 |
| `do_not_bill` | 즉시 청구 없이 플랜 변경을 적용합니다. 새 플랜은 다음 갱신 시 청구되며 기존 결제일은 유지됩니다 | 호의성 업그레이드 및 무료 마이그레이션 |
...
<Info>
`difference_immediately`을(를) 사용한 다운그레이드로 발생한 크레딧은 구독 범위이며 향후 갱신에 자동으로 적용됩니다. 이는 [Credit-Based Billing](/features/credit-based-billing) 권한과는 별개입니다.
</Info>
