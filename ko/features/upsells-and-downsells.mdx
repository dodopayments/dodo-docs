...
Dodo Payments는 `payment_method_id` 매개변수를 통해 이러한 흐름을 가능하게 하며, 이를 통해 고객이 카드 세부 정보를 다시 입력하지 않고도 저장된 결제 방법으로 요금을 청구할 수 있습니다.

### 주요 이점

| 혜택 | 영향 |
|---------|--------|
| **원클릭 구매** | 재방문 고객을 위해 결제 양식을 완전히 생략 |
| **높은 전환율** | 결정 순간의 마찰 감소 |
| **즉각 처리** | 요금이 즉시 처리됨 `confirm: true` |
| **매끄러운 사용자 경험** | 고객이 흐름 전반에 걸쳐 귀하의 앱에 머무름 |
...
<Step title="Webhook 엔드포인트">
  `payment.succeeded`, `payment.failed`, 및 `subscription.plan_changed` 이벤트를 처리하기 위해 웹후크를 설정하세요.
</Step>
</Steps>
...
<Warning>
`payment_method_id`를 사용할 때, `confirm: true`를 설정하고 기존의 `customer_id`를 제공해야 합니다. 결제 방법은 해당 고객에게 속해야 합니다.
</Warning>
...
| `full_immediately` | 새로운 플랜의 전체 가격을 청구하고 남은 시간은 무시 | 청구 주기 초기화 |
...
<Tip>
직관적인 업그레이드 흐름을 위해 `difference_immediately`를 사용하세요. 현재 플랜의 미사용 시간을 고려하고 싶다면 `prorated_immediately`를 사용하세요.
</Tip>
...
<Info>
`difference_immediately`를 통해 발생한 다운그레이드로 인한 크레딧은 구독 범위 내에서 자동으로 향후 갱신에 적용됩니다. 이들은 [고객 크레딧](/features/customer-credit)와 구별됩니다.
</Info>
...
| 이벤트 | 트리거 | 동작 |
|-------|---------|--------|
| `payment.succeeded` | 업셀/크로스셀 결제 완료 | 제품 배달, 액세스 업데이트 |
| `payment.failed` | 원클릭 청구 실패 | 오류 표시, 재시도 또는 대체 제공 |
| `subscription.plan_changed` | 업그레이드/다운그레이드 완료 | 기능 업데이트, 확인 메시지 전송 |
| `subscription.active` | 플랜 변경 후 구독 재활성화 | 새로운 계층에 대한 액세스 부여 |
