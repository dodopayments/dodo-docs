...
- **간단하게 시작하세요**: 단일 크레딧 유형과 이월 없이 시작하세요. 고객 피드백과 사용 패턴에 따라 복잡도를 추가하세요.
- **명확한 기대치 설정**: 제품 페이지와 고객 포털에 크레딧 할당량, 남은 잔액, 초과 요금 가격을 눈에 띄게 표시하세요.
- **의미 있는 단위를 사용하세요**: 크레딧을 일반적인 용어 대신 실제 의미(예: "API Calls", "AI Tokens")로 명명하여 고객이 가치를 이해하도록 돕습니다.
- **만료를 신중히 구성하세요**: 짧은 만료 기간(7일)은 긴박감을 조성하지만 고객을 불편하게 만들 수 있습니다. 대부분 SaaS 제품에서는 더 긴 기간(30~90일)이 더 친절합니다.
- **잔액이 부족할 때 모니터링**: 잔액 부족 임계값을 설정하여 고객이 소진되기 전에 알림을 받아 놀라운 초과 요금 발생을 줄이세요.
- **테스트 모드에서 테스트하세요**: 크레딧을 생성하여 테스트 상품에 연결하고 실제 구매→사용→차감→만료 주기를 시뮬레이션한 후 운영하세요.

<Info>
Credit-Based Billing은 체험판이 포함된 구독, 할당 변경과 함께한 요금 계산, 고객 포털 등 다른 모든 Dodo Payments 기능과 원활하게 작동합니다. 기본 설정으로 시작하여 가격 모델에 맞춰 확장하세요.
</Info>
