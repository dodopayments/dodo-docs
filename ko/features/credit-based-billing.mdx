...
모든 원장 이벤트(`credit.added`에서 `credit.manual_adjustment`까지)에는 잔액 전/후, 초과 전/후, 출처 참조 및 API를 통해 직접 생성된 보조금의 경우 비어있는 보조금의 출처 구독 또는 지불의 `metadata`을 포함한 전체 `CreditLedgerEntry` 페이로드가 포함됩니다. `credit.balance_low` 이벤트에는 임계값 구성과 현재 잔액이 포함됩니다.
