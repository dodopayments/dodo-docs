...
<Step title="활성화 요청">
이메일 **support@dodopayments.com**로 다음을 포함하여 보내십시오:
- 귀하의 프로덕션 도메인(예: `https://yourdomain.com`)
- 파일이 호스팅되고 있음을 확인
...
<Warning>
Apple Pay는 HTTPS를 요구합니다. 적절한 SSL 설정 없이는 `localhost`에서 나타나지 않습니다.
</Warning>
...
<Accordion title="항상 카드 대체 수단 포함">
모든 고객이 디지털 월렛을 가지고 있는 것은 아닙니다. 항상 `credit` 및 `debit`를 허용된 방법에 포함하십시오.
</Accordion>
...
**확인:**
1. Dodo Payments와 도메인 인증 완료?
2. HTTPS 사용 중?
3. 기기가 iOS 17+/Safari 17+/iPadOS 17+인가요?
4. 고객이 Apple Wallet에 카드가 있나요?
5. `apple_pay`가 `allowed_payment_method_types`에 포함되어 있나요?
...
**확인:**
1. Chrome 또는 호환 브라우저 사용 중인가요?
2. 고객이 Google Pay에 카드가 있나요?
3. Chrome 설정 "사이트가 결제 방법을 확인하도록 허용"이 활성화되어 있나요?
4. `google_pay`가 `allowed_payment_method_types`에 포함되어 있나요?
