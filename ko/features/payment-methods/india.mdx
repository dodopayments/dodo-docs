...
title: 인도 결제 수단
sidebarTitle: 인도
description: RBI 규정을 준수하는 정기 결제 명령을 통해 인도의 UPI 및 모든 인도 발급 카드 결제를 허용하세요. 48시간 처리
  지연, 명령 한도 및 웹훅 처리를 이해하세요.
...
인도는 UPI(디지털 거래의 60% 이상)와 인도 발급 카드(Visa, Mastercard, Rupay 등)가 지배하는 독특한 결제 인프라를 가지고 있습니다. Dodo Payments는 구독 명령을 위한 완전한 RBI 준수를 통해 이 모든 것을 지원합니다.
...
대부분의 대체 결제 수단과 달리 UPI 및 모든 인도 발급 카드 (Visa, Mastercard, Rupay 등)는 RBI 명령을 통해 정기 결제를 지원합니다.
</Card>
</CardGroup>
```
...
*구독은 특별 처리 규칙이 있는 RBI 준수 명령이 필요합니다. 48시간 처리 지연은 모든 인도 발급 카드와 UPI에 적용됩니다.
