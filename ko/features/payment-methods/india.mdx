...
sidebarTitle: India
description: Accept UPI and all Indian-issued cards in India with RBI-compliant
  subscription mandates. Understand the 48-hour processing delay, mandate
  limits, and webhook handling.
...
<Accordion title="Payment deduction delayed beyond 48 hours">
**원인:** 은행 API 지연으로 인해 처리가 2-3시간 추가될 수 있습니다.

**해결책:** 이는 예상된 상황입니다. 총 ~51시간까지 가변적인 지연을 처리할 수 있도록 시스템을 구축하십시오.
</Accordion>

<Accordion title="Mandate cancelled but subscription still active">
**원인:** RBI 규정의 극단적인 경우 — 처리 창 동안의 위임 취소가 즉시 구독을 취소하지 않습니다.

**해결책:** 다음 청구가 실패하고 구독은 `on_hold`로 이동합니다. `payment.failed`에 대한 웹 후크를 모니터링하십시오.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="Payment Methods Overview" icon="credit-card" href="/features/payment-methods">
지원되는 모든 결제 방법을 참조하십시오.
</Card>

<Card title="Subscriptions" icon="repeat" href="/features/subscription">
RBI 규정을 포함한 완전한 구독 문서.
</Card>
...
<Card title="Testing Process" icon="flask" href="/miscellaneous/testing-process">
UPI ID 및 인도 카드 등을 포함한 모든 테스트 데이터.
</Card>
</CardGroup>