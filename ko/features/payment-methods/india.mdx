...
| 이벤트 | 시점 | 조치 |
| :---- | :--- | :----- |
| `subscription.created` | 지침 승인됨 | 구독 시작 기록 |
| `payment.succeeded` | 청구일로부터 약 48시간 후 | 접근 권한 부여/유지 |
| `payment.failed` | 출금 실패 | 고객에게 알리고 접근 일시정지 |
| `subscription.on_hold` | 결제 실패 | 결제 수단 업데이트 유도 |
| `subscription.active` | 결제 후 재활성화됨 | 접근 권한 복원 |
