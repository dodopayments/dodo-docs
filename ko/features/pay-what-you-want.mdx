...
description: Let customers choose their price above a minimum. Optionally show a
  suggested price. Optimized for one‑time purchases.
icon: dollar-sign
keywords:
  - Dodo Payments
  - 원하는 만큼 지불하기 가격
...
제품에서 `POST /checkouts`을 사용하여 일회성 결제 세션을 생성합니다. 결제 시 구매자가 PWYW 최소 금액을 충족하는 금액을 입력하라는 메시지가 표시됩니다.
