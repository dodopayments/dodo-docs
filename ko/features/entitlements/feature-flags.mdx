...
| 트리거 | 효과 |
|---|---|
| 일회성 결제가 성공하거나 구독이 활성화됨 | `status: Delivered` 및 `delivered_at`가 설정된 Grant가 생성됩니다. |
| 구독이 보류되거나 취소되거나 만료됨 | 일치하는 `revocation_reason`와 함께 Grant가 취소됩니다. |
| 일회성 결제가 환불됨 | `revocation_reason: refund`와 함께 Grant가 취소됩니다. |
| 구독이 복구됨(예: 연체 결제가 성공함) | 취소된 Grant가 `Delivered`로 복원됩니다. 동일한 Grant `id`가 유지되고 취소 필드는 삭제됩니다. |
| 수동 API revoke | `revocation_reason: manual`와 함께 Grant가 취소됩니다. 갱신 시 자동으로 복원되지 않습니다. |
