...
고객이 라이선스 키가 활성화된 제품을 구매 완료하면, 라이선스 키가 귀하의 `return_url`에 쿼리 매개변수로 자동 추가됩니다. 이를 통해 성공 페이지에서 추가 API 호출 없이 바로 키를 표시할 수 있습니다.

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001&email=customer%40example.com
```

구매 시 여러 개의 라이선스 키가 생성되는 경우(예: 수량 > 1), 키는 쉼표로 구분됩니다:

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001,LK-002&email=customer%40example.com
```

구독의 경우 `subscription_id` 대신 `payment_id`가 사용됩니다:
...
<Tip>
반환 페이지에서 `license_key` 매개변수를 구문 분석하여 고객에게 키를 즉시 표시함으로써 구매 후 경험을 개선하십시오.
</Tip>

## 모범 사례

- **제한 사항을 명확히 유지**: 만료 및 활성화에 대한 합리적인 기본 설정 선택
- **사용자 안내**: 정밀한 활성화 지침 및 자가 지원 문서 제공
- **자동 검사**: 서버 측에서 키 유효성 검사 후 접근 권한 부여
- **이벤트 모니터링**: 웹훅을 사용하여 남용을 탐지하고 자동으로 취소
