...
| 이벤트 | 동작 |
|---|---|
| `payment.succeeded` (일회성) | 구매한 각 `quantity`마다 키 하나를 생성합니다. 키 만료는 entitlement의 duration을 따릅니다. |
| `subscription.active` | 각 subscription `quantity`(seat)마다 키 하나를 생성합니다. 키는 만료되지 않으며 유효성은 subscription 상태에 연결됩니다. |
| `subscription.renewed` | No-op. 기존 키는 유지됩니다. |
| `subscription.on_hold` | 키를 비활성화합니다. subscription이 hold 상태에서 해제되면 다시 활성화됩니다. |
| `subscription.paused` | 키를 비활성화합니다. subscription이 재개되면 다시 활성화됩니다. |
| `subscription.cancelled` / `expired` | 키를 영구적으로 비활성화합니다. |
| `subscription.plan_changed` | 기존 키를 비활성화하고 새 플랜에 대한 키를 발급합니다. |
| `refund.succeeded` (일회성) | 키를 비활성화합니다. |
| API/dashboard를 통한 수동 revoke | `revocation_reason: manual`를 사용하여 키를 비활성화합니다. subscription 갱신 시 자동으로 다시 부여되지 않습니다. |
| License key disabled directly | `revocation_reason: license_key_disabled`로 grant를 revoke합니다. 키를 다시 활성화하면 grant도 자동으로 다시 활성화됩니다. |
...
license-key 레코드의 `source` 필드를 사용하면 조정 또는 감사 시 마이그레이션된 inventory와 수동으로 fulfilled된 키를 자동으로 발급된 키와 구분할 수 있습니다. [List Grants](/api-reference/entitlements/list-grants)에서 반환되는 grant의 `license_key` 객체에서 해당 값을 읽을 수 있습니다. 레거시 `GET /license_keys` endpoint는 deprecated되었습니다.
...
<Warning>
`GET /license_keys`, `GET /license_keys/{id}` 및 `PATCH /license_keys/{id}`는 deprecated되었습니다. 읽기 작업에는 entitlement grant endpoint([List Grants](/api-reference/entitlements/list-grants), [List Customer Grants](/api-reference/entitlements/list-customer-grants))를 사용하세요. `POST /license_keys`는 기존 키를 import하는 용도로 계속 지원됩니다.
</Warning>
...
License-key delivery 및 revocation은 네 가지 [`entitlement_grant.*` webhook events](/developer-resources/webhooks/intents/entitlement-grant)를 발생시킵니다. grant payload에는 키, 만료, 사용된 activations 및 limit이 입력된 `license_key` 객체가 포함됩니다.

레거시 `license_key.*` events(`license_key.created`)는 기본 license-key 레코드 lifecycle에 대해 계속 발생합니다. 자세한 내용은 [License Key webhook payload page](/developer-resources/webhooks/intents/license-key)를 참조하세요.

<Tip>
새로운 integration에서는 `license_key.created` 대신 `entitlement_grant.delivered`를 수신 대기하세요. entitlement event는 license key뿐 아니라 product의 모든 integration에서 delivery가 완료되었음을 알려줍니다.
</Tip>
...
<Note>
이전 `license_key_enabled` flag를 사용하여 생성된 product는 License Key entitlement로 **자동 마이그레이션**되었습니다. 마이그레이션은 투명하게 진행됩니다. 기존 고객의 키는 변경 없이 계속 작동하고, public `/licenses/activate`, `/licenses/validate`, `/licenses/deactivate` endpoint는 계속 작동하며, `/license_keys/*` API endpoint는 동일한 key store에서 계속 읽고 씁니다.

독립적인 **License Keys** dashboard section은 발급된 모든 키의 flat list로 계속 제공되며 감사 및 검색에 유용합니다. 새로운 구성(activation limit, duration 또는 instructions 변경)은 **Entitlements**에서 마이그레이션된 License Key entitlement를 편집하여 수행해야 합니다.
</Note>
...
- **server-side에서 키를 검증하세요**: 네트워크에 연결된 product의 경우 activation을 로컬에 caching하는 대신 `/licenses/validate`를 통해 검증하세요.
- **revocation에는 webhooks를 사용하세요**: 고객이 취소하거나 환불할 때 앱 내 기능을 즉시 비활성화하려면 `entitlement_grant.revoked`를 수신 대기하세요.
- **subscription 및 일회성 결제로 테스트하세요**: 두 경우의 license key 동작에는 미묘한 차이가 있으므로 live 전 두 경우 모두 테스트하세요.
