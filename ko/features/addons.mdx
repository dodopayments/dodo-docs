...
<AccordionGroup>
<Accordion title="추가 기능 생성">
`POST /addons` 엔드포인트를 사용하여 사용자 지정 가격, 설명 및 구성 옵션으로 새 추가 기능을 만드세요.
...
`PATCH /addons/{id}` 엔드포인트를 사용하여 기존 추가 기능의 가격, 설명 또는 가용성을 수정하세요.
...
모든 추가 기능 목록을 보려면 `GET /addons`를 사용하거나 특정 추가 기능 세부정보를 검색하려면 `GET /addons/{id}`를 사용하세요.
...
더 나은 제품 프레젠테이션을 위해 추가 기능의 이미지를 업데이트하려면 `PUT /addons/{id}/images` 엔드포인트를 사용하세요.
