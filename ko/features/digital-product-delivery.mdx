...
| 이벤트 | 동작 |
|---|---|
| `payment.succeeded` (일회성) | 권한을 발급합니다. 권한에는 약 15분 동안 유효한 사전 서명 다운로드 URL이 포함되며, 고객은 이메일 링크 또는 Customer Portal 페이지를 다시 열어 URL을 갱신할 수 있습니다. |
| `subscription.active` | 권한을 발급합니다. 구독이 활성 상태인 동안 파일에 계속 액세스할 수 있습니다. |
| `subscription.renewed` | 아무 작업도 수행하지 않습니다. 동일한 권한이 계속 유지되며, 가져올 때마다 새로운 사전 서명 URL이 생성됩니다. |
| `subscription.on_hold` / `cancelled` / `expired` | 권한을 취소합니다. 더 이상 새로운 사전 서명 URL이 발급되지 않습니다. |
| `subscription.paused` | 권한을 취소합니다. 구독이 재개될 때까지 새로운 사전 서명 URL이 발급되지 않습니다. |
| `subscription.plan_changed` | 기존 권한을 취소하고, 새 요금제의 권한에 대해 새로운 권한을 발급합니다. |
| `refund.succeeded` (일회성) | 권한을 취소합니다. |
| 수동 취소 | `revocation_reason: manual`을 사용하여 취소합니다. |
