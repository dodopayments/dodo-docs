...
<Info>
이 섹션에서는 `customization.theme_config`을 사용한 **서버 측 API** 테마 구성을 다룹니다. **Checkout SDK**(오버레이 또는 인라인 체크아웃)를 사용하는 경우, camelCase 속성(`bgPrimary` 대신 `bg_primary` 등)을 사용하는 [Overlay Checkout](/developer-resources/overlay-checkout#theme-customization) 또는 [Inline Checkout](/developer-resources/inline-checkout#theme-customization)의 테마 사용자 정의 섹션을 참조하세요.
</Info>
...
| `pay_button_text` | 결제 버튼의 사용자 정의 텍스트(예: "Complete Purchase", "Subscribe Now") |
...
| `text_placeholder` | 플레이스홀더 텍스트 색상 |
| `text_error` | 오류 텍스트 색상 |
| `text_success` | 성공 텍스트 색상 |
