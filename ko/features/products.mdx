...
- **가격**: 기본 금액 및 통화.
- **할인 (%)**: 결제 및 청구서에 표시되는 선택적 인라인 할인.
- 구독의 경우 **반복 주기**(예: 1개월 또는 1년)와 필요시 **체험 기간**을 설정하세요. 구독 가격은 최소 **$1**(또는 선택한 통화의 해당 금액) 이상이어야 합니다. 이 최소 금액 이하로 설정할 경우 구독이 작동하지 않습니다.
...
<Frame>
  <img src="/images/product-collection/checkout-page.png" alt="제품 컬렉션" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
반복 제품의 경우, `price` 객체를 반복 가격(`type: recurring_price`)으로 설정하고 `payment_frequency_interval` 및 `subscription_period_interval` (`Day`, `Week`, `Month`, 또는 `Year`)과 해당 수량을 지정합니다.
...
제품을 더 이상 표시하거나 사용하지 않으려면 `DELETE` 요청을 `/products/{id}` 엔드포인트에 사용하여 제품을 아카이브할 수 있습니다. 이 작업은 제품을 숨기지만 영구적으로 삭제하지는 않습니다.
