...
**해결책**:
1. 이벤트 수집이 정상적으로 작동하는지 확인 (API 응답 체크)
2. `customer_id`가 구독 고객과 일치하는지 확인
3. `event_name`가 미터 설정과 일치하는지 확인
4. 이벤트에 올바른 타임스탬프가 있는지 확인 (미래 날짜가 아닌)
</Accordion>
...
**해결책**:
1. `previewChangePlan` API를 사용하여 확인하기 전 정확한 요금을 보여줍니다.
2. 비례 청구가 구독 및 추가 기능에 적용된다고 알립니다.
