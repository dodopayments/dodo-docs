...
<Info>
하이브리드 청구는 두 개 이상의 청구 모델을 단일 가격 전략으로 결합합니다. 이를 통해 반복 요금, 사용량, 사용자, 기능 등 다양한 측면에서 가치를 포착하면서 고객에게 유연성과 예측 가능성을 제공합니다.
</Info>
...
<Info>
사용 미터는 구독 상품에 직접 연결됩니다. 사용 요금은 자동으로 계산되어 구독 청구서에 추가됩니다.
</Info>
</Step>
...
무료 임계값은 기본 구독에 포함된 사용량을 포함합니다.

**Pro Plan: $49/month**
- 포함: 10,000 API 호출
- 초과: 10,000회 이후 호출당 $0.005
- 고객이 8,000회를 사용함 → $49를 지불 (추과 없음)
</Tab>

{/* LOCKED_PATTERN_4b0b3630c3d8a0c04ce973c73e2ba3e1 */}
기본 요금 없이 모든 단위가 첫 사용부터 청구됩니다.

**Pay-As-You-Go: $0/month base**
- 포함: 0 API 호출
- 사용: 첫 호출부터 호출당 $0.01
- 고객이 5,000회를 사용함 → $50를 지불
</Tab>

{/* LOCKED_PATTERN_10686b36a88b2863e366bf192c2300ce */}
다른 티어에는 서로 다른 허용량이 포함됩니다.

- **Starter**: $19/month (1,000 calls 포함)
- **Pro**: $49/month (10,000 calls 포함)
- **Enterprise**: $199/month (100,000 calls 포함)
- 모든 티어: 초과 호출당 $0.005
</Tab>
</Tabs>
...
플랫폼 요금을 포함한 구독 상품을 생성하고 애드온을 연결합니다.
...
기본 플랜에는 일부 좌석이 포함되며 추가 좌석에 대해 요금이 부과됩니다.

**Team Plan: $99/month**
- 포함: 5 좌석
- 추가 좌석: 좌석당 $15/월
- 사용자 20명 = \$99 + (15 × \$15) = \$324/월
</Tab>

{/* LOCKED_PATTERN_a6e71fc0117c5efb38c37fa43a7f298e */}
플랫폼 요금 없이 사용자당 요금만 부과됩니다.
...
구현: 기본 구독 가격을 $0으로 설정하고 좌석 애드온만 사용합니다.
</Tab>

{/* LOCKED_PATTERN_bea65d9754adb8507ad8835ac69b9bdc */}
좌석당 가격은 상위 티어로 갈수록 낮아집니다.

- **스타터**: $20/좌석 (1-10 좌석)
- **성장**: $15/좌석 (11-50 좌석)
- **기업**: $10/좌석 (51개 이상 좌석)

구현: 다양한 애드온 가격으로 각 티어별 구독 상품을 별도로 생성합니다.
</Tab>
</Tabs>
...
<Warning>
**제약 사항**: Dodo Payments는 현재 동일한 구독 상품에 사용 미터와 애드온을 모두 연결하는 것을 지원하지 않습니다. 이 모델은 애플리케이션 수준의 로직을 활용한 우회가 필요합니다.
</Warning>

<Info>
**Coming Soon**: 좌석 + 사용률 하이브리드 청구에 대한 기본 지원이 로드맵에 있습니다. 이를 통해 사용 미터와 좌석 애드온을 동일한 구독 상품에 모두 연결할 수 있게 됩니다.
</Info>
...
사용 기반 구독을 사용하고 좌석 청구는 애플리케이션에서 관리합니다.
...
좌석에는 애드온을 사용하고, 사용량은 주문형 청구로 수동 청구합니다.
...
<Info>
**권장 사항**: 옵션 B(좌석 애드온 + 주문형 사용량)는 Dodo가 좌석 청구를 자동으로 처리하므로 구현이 더 쉬운 경우가 많습니다. 사용량 초과만 추적하고 청구하면 됩니다.
</Info>
...
<Warning>
**제약 사항**: Dodo Payments는 현재 동일한 구독 상품에 사용 미터와 애드온을 모두 연결하는 것을 지원하지 않습니다. 이 모델은 우회 접근이 필요합니다.
</Warning>

<Info>
**Coming Soon**: 기본 + 좌석 + 사용량을 포함한 트리플 하이브리드 청구에 대한 지원이 곧 제공될 예정입니다. 이를 통해 사용 미터와 좌석 애드온을 동일한 구독 상품에 모두 연결할 수 있게 됩니다.
</Info>
...
좌석 애드온이 적용된 구독을 사용하고 주문형 요금으로 사용량을 청구합니다.
...
사용 미터가 적용된 구독을 사용하고 좌석 청구는 애플리케이션에서 관리합니다.
...
{/* LOCKED_PATTERN_c1b7656f0b028707bab5623531700d5b */}
좌석 수를 추적하고 기본 구독 가격을 해당 수치에 맞게 조정합니다.
...
<Info>
**권장 사항**: 옵션 A(기본 + 좌석 + 주문형 사용량)가 일반적으로 더 쉽습니다. Dodo가 플랫폼 및 좌석 청구를 자동으로 처리하므로 사용량만 추적하고 각 청구 주기 말에 청구하면 됩니다.
</Info>
...
<Info>
이 예시는 이상적인 가격 구조를 보여줍니다. 사용 미터와 애드온을 동일한 상품에 연결할 수 없는 제약으로 인해 일부 조합은 사용량을 주문형으로 청구하거나 앱에서 좌석을 관리하는 방식으로 우회해야 합니다.
</Info>
...
<Tip>
하이브리드 가격을 이해하기 쉽게 만듭니다. 가격 페이지에 기본 비용, 포함 항목, 초과 요금 방식을 눈에 띄게 표시하세요.
</Tip>
...
**해결 방법**:
1. `previewChangePlan` API를 사용하여 확인 전에 정확한 요금을 보여줍니다
2. 구독 및 애드온 모두에 대해 정산이 적용된다는 점을 안내합니다
3. 업그레이드 청구를 단순화하려면 `difference_immediately` 사용을 고려합니다
</Accordion>

{/* LOCKED_PATTERN_d337cbb8bdb19d84c536e224dd1b400a */}
**증상**: 무료여야 할 사용량에 대해 고객에게 요금이 청구됩니다.

**해결 방법**:
1. 사용 기반 상품에 무료 임계값이 설정되어 있는지 확인합니다
2. 임계값 단위가 이벤트 집계(호출 vs 요청)와 일치하는지 확인합니다
3. 사용 미터가 구독 상품에 올바르게 연결되어 있는지 확인합니다
</Accordion>

{/* LOCKED_PATTERN_8a9af10b57914dc3264e4590fd0118c5 */}
**증상**: 결제 중 좌석이나 기능을 추가할 수 없습니다.

**해결 방법**:
1. 대시보드에서 구독 상품에 애드온이 연결되어 있는지 확인합니다
2. API 호출에서 애드온 ID가 정확한지 확인합니다
3. 애드온 통화가 구독 상품 통화와 일치하는지 확인합니다
</Accordion>
</AccordionGroup>
