...
Pause 및 resume은 update subscription endpoint의 `status` field를 통해 실행됩니다. 별도의 pause endpoint는 없습니다.
...
<Warning>
`paused` 또는 `active` 중 하나만 단독으로 전송하세요. 둘 중 하나를 다른 field와 결합하면 `422` 오류와 함께 거부됩니다. 이전의 boolean `pause` field는 제거되었으며 이제 항상 `422` 오류가 발생합니다. 따라서 아직 해당 field를 사용하는 caller는 조용히 아무 작업도 수행하지 않고 넘어가는 대신 명확한 오류를 받게 됩니다.
</Warning>
...
| Status | 의미 | 복구 가능 여부 | 복구 경로 / 다음 단계 |
|--------|---------------|--------------|---------------------------|
| `pending` | 구독을 생성하거나 처리하는 중입니다 | — | `subscription.active`(또는 `subscription.failed`)를 기다리세요 |
| `active` | 구독이 활성 상태이며 자동으로 갱신됩니다 | — | 별도의 작업이 필요하지 않습니다 |
| `on_hold` | 갱신 결제(또는 플랜 변경 요금)가 실패하여 갱신이 중지되었습니다 | **예** | [Payment Retries](/features/recovery/payment-retries) 및 [Dunning](/features/recovery/subscription-dunning)을 통해 자동으로, 또는 [Update Payment Method API](/api-reference/subscriptions/update-payment-method)를 통해 수동으로 결제 수단을 업데이트하여 다시 활성화하세요 |
| `paused` | 회원님 또는 고객이 구독을 의도적으로 일시 중지했습니다. 청구가 동결되고 액세스 권한이 취소됩니다 | **예** | `status: active`를 사용하거나, self-service pause가 활성화된 경우 Customer Portal에서 resume하세요. 자세한 내용은 [구독 일시 중지 및 재개](#pausing-and-resuming-subscriptions)를 참조하세요 |
| `cancelled` | 구독이 취소되었으며 갱신되지 않습니다 | 재구매만 가능 | 고객이 새 구독을 시작해야 합니다. [Dunning](/features/recovery/subscription-dunning)을 통해 재구매를 유도할 수 있습니다 |
| `failed` | 구독 **생성**에 실패했습니다(초기 mandate 또는 payment가 성공하지 않음) | **아니요 — terminal** | 고객이 사용 가능한 payment method로 새 구독을 생성해야 합니다 |
| `expired` | 구독 기간이 종료되었습니다 | — | 원하는 경우 고객이 새 구독을 시작해야 합니다 |
...
Pause 및 resume은 `status` field를 사용하는 동일한 `PATCH /subscriptions/{subscription_id}` endpoint를 통해 실행됩니다. `status: paused`는 활성 구독을 일시 중지하고 `status: active`는 일시 중지된 구독을 resume합니다. 동일한 request에서 두 값 모두 다른 field와 결합할 수 없습니다. 전체 동작, billing 효과 및 관련 business settings는 [구독 일시 중지 및 재개](#pausing-and-resuming-subscriptions)를 참조하세요.

<Card title="API Reference" icon="code" href="/api-reference/subscriptions/patch-subscriptions">
`status` field를 포함한 update subscription API를 확인하세요.
</Card>
</Accordion>
