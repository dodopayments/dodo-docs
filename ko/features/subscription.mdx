...
`PATCH /subscriptions/{subscription_id}`을 사용하면 다음 결제일에 취소하거나, 구독 기간을 연장하거나, 결제 정보를 업데이트하거나, 메타데이터를 수정할 수 있습니다. 수량을 변경하려면 대신 [Change Plan API](/api-reference/subscriptions/change-plan)를 사용하세요. `PATCH`은(는) `quantity`을(를) 허용하지 않습니다.
...
### 구독 기간 연장
새로운 `subscription_period_count` 및 `subscription_period_interval`을(를) `PATCH /subscriptions/{subscription_id}`에 전달하여 구독이 유지되는 기간을 연장합니다. 새로운 수와 간격을 기준으로 구독 만료일이 다시 계산됩니다. 예를 들어 고객에게 현재 플랜을 추가로 이용할 시간을 제공하려면:
