...
`PATCH /subscriptions/{id}`을 사용하면 다음 billing date에 취소하거나, 구독 기간을 연장하거나, billing details를 업데이트하거나, metadata를 수정할 수 있습니다. 수량을 변경하려면 대신 [플랜 변경 API](/api-reference/subscriptions/change-plan)를 사용하세요. `PATCH`은(는) `quantity`을 허용하지 않습니다.
