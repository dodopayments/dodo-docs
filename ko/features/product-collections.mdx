...
|---------|-------------|---------|
| **Allow Multiple Subscriptions** | 고객은 동시에 둘 이상의 활성 구독을 보유할 수 있습니다 | Enabled |
| **Allow Subscription Updates** | 고객은 Customer Portal을 통해 언제든지 기존 구독을 업그레이드하거나 다운그레이드할 수 있습니다 | Disabled |
| **Prevent Trial Misuse** | 이미 trial을 사용한 고객은 새로운 trial을 받는 대신 유료 및 trial 없는 구매로 처리됩니다 | Disabled |
...
<Info>
trial 사용이 매칭되고 기록되는 방식에 대한 자세한 내용은 [Preventing Trial Misuse](/features/subscription#preventing-trial-misuse)를 참조하세요.
</Info>

<Card title="Subscription Plan Changes" icon="repeat" href="/features/subscription#subscription-plan-changes">
  proration 모드와 플랜 변경 동작에 대해 자세히 알아보세요.
</Card>

## Collection 관리

Product Collection은 Dodo Payments 대시보드 또는 API를 통해 프로그래밍 방식으로 관리할 수 있습니다. API를 사용하면 collection 생성, 업데이트, 이미지 업로드, 보관 처리, 중첩된 그룹 및 제품 관리를 모두 제어할 수 있습니다.

### 대시보드 작업

- **생성**: 제품 및 그룹을 포함한 새 collection 설정
- **업데이트**: 이름, 설명, 이미지 및 제품 구성 수정
- **순서 변경**: 드래그 앤 드롭으로 제품 표시 순서 변경
- **제품 활성화/비활성화**: checkout에 표시할 제품 제어
- **보관**: collection을 영구적으로 삭제하지 않고 숨김 처리(나중에 보관 해제 가능)
...
다음 endpoint를 사용하면 Product Collection을 프로그래밍 방식으로 생성, 업데이트, 조회, 보관 처리 및 구성할 수 있으며, 중첩된 그룹과 그룹 내 제품도 관리할 수 있습니다.

<AccordionGroup>
<Accordion title="Listing Product Collections">
`GET` 요청을 `/product-collections` endpoint로 전송하여 계정과 연결된 모든 Product Collection을 가져옵니다. 페이지네이션, 브랜드별 필터링 및 보관된 collection 포함을 지원합니다.

<Card title="List Product Collections API" icon="code" href="/api-reference/product-collections/list-product-collections">
  List Product Collections API 문서에서 자세한 request 및 response 구조를 확인하세요.
</Card>
</Accordion>
...
<Card title="Create Product Collection API" icon="code" href="/api-reference/product-collections/create-product-collection">
  Create Product Collection API 문서에서 자세한 request 및 response 구조를 확인하세요.
</Card>
</Accordion>

<Accordion title="Retrieving a Product Collection">
`GET` 요청을 `/product-collections/{id}` endpoint로 전송하여 특정 Product Collection에 대한 자세한 정보를 가져옵니다. 여기에는 해당 collection의 그룹과 제품 항목이 포함됩니다.

<Card title="Get Product Collection API" icon="code" href="/api-reference/product-collections/get-product-collection">
  Get Product Collection API 문서에서 자세한 request 및 response 구조를 확인하세요.
</Card>
</Accordion>
...
<Card title="Update Product Collection API" icon="code" href="/api-reference/product-collections/update-product-collection">
  Update Product Collection API 문서에서 자세한 request 및 response 구조를 확인하세요.
</Card>
</Accordion>

<Accordion title="Uploading Collection Images">
사전 서명된 URL을 통해 이미지를 업로드하여 collection에 연결합니다. `/product-collections/{id}/images` endpoint에서 업로드 URL을 요청한 다음, 60초 이내에 반환된 URL로 이미지를 `PUT`합니다.
...
<Card title="Update Collection Images API" icon="code" href="/api-reference/product-collections/update-product-collection-images">
  Update Collection Images API 문서에서 자세한 request 및 response 구조를 확인하세요.
</Card>
</Accordion>

<Accordion title="Archiving a Product Collection">
`DELETE` 요청을 `/product-collections/{id}` endpoint로 전송하여 collection을 보관 처리합니다. 이렇게 하면 collection을 새로 사용할 수 없게 되지만 영구적으로 삭제되지는 않습니다.

<Card title="Archive Product Collection API" icon="code" href="/api-reference/product-collections/archive-product-collection">
  Archive Product Collection API 문서에서 자세한 request 및 response 구조를 확인하세요.
</Card>
</Accordion>

<Accordion title="Unarchiving a Product Collection">
`POST` 요청을 `/product-collections/{id}/unarchive` endpoint로 전송하여 보관된 collection을 복원합니다.
...
<Card title="Unarchive Product Collection API" icon="code" href="/api-reference/product-collections/unarchive-product-collection">
  Unarchive Product Collection API 문서에서 자세한 request 및 response 구조를 확인하세요.
</Card>
</Accordion>

<Accordion title="Managing Groups within a Collection">
그룹을 사용하면 collection 내에서 제품을 구성할 수 있습니다(예: "월간 플랜"과 "연간 플랜"). 그룹 endpoint를 사용하여 collection 내 그룹을 추가, 업데이트 또는 삭제하세요.
...
<Card title="Delete Group" icon="code" href="/api-reference/product-collections/delete-group">
  collection에서 그룹을 삭제합니다.
</Card>
</CardGroup>
</Accordion>

<Accordion title="Managing Products within a Group">
그룹 내부의 개별 제품 항목을 관리합니다. 새 제품을 추가하고, 기존 항목(예: 표시 순서)을 업데이트하거나, 항목을 완전히 삭제할 수 있습니다.
...
- **전략적으로 순서 지정**: 가장 인기 있거나 권장하는 플랜을 첫 번째에 배치하면 checkout에서 미리 선택됩니다
- **명확한 이름 사용**: 제품 이름에 가치 차이가 명확하게 드러나야 합니다
- **양방향 변경 활성화**: 고객에게 유연성을 제공할 수 있도록 업그레이드와 다운그레이드를 모두 허용
- **proration 고려**: 비즈니스 모델에 적합한 proration 모드 선택
- **철저한 테스트**: 실제 운영을 시작하기 전에 test mode에서 checkout 및 플랜 변경 흐름 확인

<Check>
이제 Product Collection을 생성하고 고객에게 통합된 플랜 선택 경험을 제공할 수 있습니다.
</Check>

<CardGroup cols={2}>
<Card title="Products" icon="box" href="/features/products">
  collection에 추가할 일회성, subscription 또는 사용량 기반 제품을 생성합니다.
</Card>

<Card title="Checkout" icon="cart-shopping" href="/features/checkout">
  통합된 checkout 경험에서 collection 제품을 표시합니다.
</Card>

<Card title="Customer Portal" icon="id-card" href="/features/customer-portal">
  고객이 동일한 collection 내에서 업그레이드하거나 다운그레이드할 수 있도록 합니다.
</Card>
