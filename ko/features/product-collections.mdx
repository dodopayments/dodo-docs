...
| 설정 | 설명 | 기본값 |
|---------|-------------|---------|
| **다중 구독 허용** | 고객이 동시에 두 개 이상의 활성 구독을 보유할 수 있습니다 | 활성화됨 |
| **구독 업데이트 허용** | 고객이 Customer Portal을 통해 언제든지 기존 구독을 업그레이드하거나 다운그레이드할 수 있습니다 | 비활성화됨 |
| **구독 취소 허용** | 고객이 Customer Portal에서 직접 구독을 취소할 수 있습니다 | 활성화됨 |
| **체험판 오용 방지** | 이미 체험판을 사용한 고객은 새로운 체험판을 받는 대신 유료 체험판 없는 구매로 전환됩니다 | 비활성화됨 |
...
<Info>
체험판 사용 기록이 매칭되고 저장되는 방식에 대한 자세한 내용은 [체험판 오용 방지](/features/subscription#preventing-trial-misuse)를 참조하세요.
</Info>
...
Product Collections는 Dodo Payments 대시보드 또는 API를 통해 프로그래밍 방식으로 관리할 수 있습니다. API를 사용하면 컬렉션 생성, 업데이트, 이미지 업로드, 보관 처리, 중첩 그룹 및 제품 관리에 대한 모든 제어 권한을 제공합니다.

### 대시보드 작업

- **생성**: 제품과 그룹을 포함한 새 컬렉션 설정
- **업데이트**: 이름, 설명, 이미지 및 제품 구성 수정
- **순서 변경**: 드래그 앤 드롭으로 제품 표시 순서 변경
- **제품 활성화/비활성화**: checkout에 표시되는 제품 제어
- **보관**: 컬렉션을 영구적으로 삭제하지 않고 숨김 처리(나중에 보관 해제 가능)
...
다음 endpoints를 사용하면 중첩 그룹 및 그룹 내 제품 관리를 포함하여 Product Collections를 프로그래밍 방식으로 생성, 업데이트, 조회, 보관 처리하고 구성할 수 있습니다.

<AccordionGroup>
<Accordion title="Listing Product Collections">
`GET` 요청을 사용하여 계정과 연결된 모든 Product Collections를 `/product-collections` endpoint에서 가져옵니다. 페이지네이션, 브랜드별 필터링 및 보관된 컬렉션 포함을 지원합니다.
...
pre-signed URL을 통해 이미지를 업로드하여 컬렉션에 연결합니다. `/product-collections/{id}/images` endpoint에서 업로드 URL을 요청한 다음, 60초 이내에 반환된 URL로 이미지를 `PUT`합니다.
...
`DELETE` 요청을 `/product-collections/{id}` endpoint로 전송하여 컬렉션을 보관 처리합니다. 이렇게 하면 컬렉션을 새로 사용할 수 없게 되지만 영구적으로 삭제되지는 않습니다.
...
그룹을 사용하면 컬렉션 내 제품을 구성할 수 있습니다(예: "월간 플랜"과 "연간 플랜"). groups endpoints를 사용하여 컬렉션 내 그룹을 추가, 업데이트 또는 삭제합니다.
...
- **명확한 이름 사용**: 제품 이름만으로도 가치 차이를 명확하게 전달해야 합니다
- **양방향 변경 허용**: 고객에게 유연성을 제공하도록 업그레이드와 다운그레이드를 모두 허용합니다
- **proration 고려**: 비즈니스 모델에 적합한 proration 모드를 선택합니다
- **철저한 테스트**: 라이브 환경으로 전환하기 전에 test mode에서 checkout 및 플랜 변경 흐름을 확인합니다

<Check>
이제 Product Collections를 생성하고 고객에게 통합된 플랜 선택 환경을 제공할 수 있습니다.
</Check>
...
<Card title="Subscriptions" icon="repeat" href="/features/subscription">
  proration 및 플랜 변경을 통해 반복 결제 플랜을 관리합니다.
</Card>
</CardGroup>