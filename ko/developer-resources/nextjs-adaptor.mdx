...
<Warning>
Dynamic Checkout은 더 이상 사용되지 않는 `POST /payments` 및 `POST /subscriptions` 엔드포인트를 프록시합니다. 기존 통합에서는 계속 작동하지만, 새 통합에서는 아래의 **Checkout Sessions**를 사용해야 합니다.
</Warning>
...
Checkout Sessions는 더욱 안전한 호스팅 checkout 환경을 제공하며, 완전한 사용자 지정 제어를 통해 일회성 구매와 구독 모두에 대한 전체 결제 흐름을 처리합니다.

자세한 내용과 지원되는 필드의 전체 목록은 [Checkout Sessions Integration Guide](https://docs.dodopayments.com/developer-resources/checkout-session)를 참조하세요.
...
<code>true</code>로 설정하면 customer에게 포털 링크가 포함된 이메일을 보냅니다.
</ParamField>
...
- **Signature Verification:** <code>webhookKey</code>를 사용하여 webhook 서명을 확인합니다. 확인에 실패하면 401을 반환합니다.
- **Payload Validation:** Zod를 사용하여 검증합니다. 유효하지 않은 payload에는 400을 반환합니다.
- **Error Handling:**
  - 401: 유효하지 않은 서명
  - 400: 유효하지 않은 payload
  - 500: 확인 중 내부 오류
- **Event Routing:** payload 유형에 따라 적절한 이벤트 핸들러를 호출합니다.
