...
Google Cloud에서 리디렉션 URI를 `https://[your-project-ref].supabase.co/auth/v1/callback`로 설정한 후, Supabase Auth에서 클라이언트 ID 및 비밀을 사용하여 Google 공급자를 활성화하세요.
</Step>

<Step title="Dodo Payments 구성">
Dodo 대시보드에서 API 키 및 웹훅 비밀을 생성하세요. 개발 중에는 환경을 `test_mode`로 설정하세요.
</Step>
