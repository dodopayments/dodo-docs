...
이러한 엔티티의 조합을 `scopes` 매개변수에 지정하여 동기화할 수 있습니다. 모든 동기화 작업은 점진적이며 최적의 성능을 위해 새로운 또는 수정된 레코드만 전송합니다.
...
동기화할 데이터 엔티티의 쉼표로 구분된 목록입니다. 사용 가능한 범위: `licences`, `payments`, `customers`, `subscriptions`. 예: `"payments,customers"`.
</ParamField>

<ParamField path="--api-key" type="string" required>
귀하의 Dodo Payments API 키입니다. 실시간 모드의 경우 `dp_live_`로 시작해야 하며, 테스트 모드의 경우 `dp_test_`로 시작해야 합니다.
</ParamField>

<ParamField path="--env" type="string" required>
환경 대상입니다. `"live_mode"` 또는 `"test_mode"`이여야 합니다. 이는 동기화할 Dodo Payments 환경을 결정합니다.
</ParamField>
...
<Tip>
`interval` 옵션은 자동 동기화를 위한 `.start()`을 사용할 때 필요합니다. 동기화 작업은 프로세스가 중지될 때까지 지정된 간격으로 계속 실행됩니다.
</Tip>
...
<Tip>
수동 동기화를 사용할 때 `interval` 옵션은 필요하지 않습니다. 필요할 때마다 `.run()`를 호출하여 동기화 작업을 수행할 수 있습니다.
</Tip>

#### PostgreSQL 예시

PostgreSQL과 함께 `dodo-sync`를 사용하는 방법입니다:
...
MySQL과 함께 `dodo-sync`를 사용하는 방법입니다:
...
ClickHouse와 함께 `dodo-sync`를 사용하는 방법입니다:
...
동기화할 엔티티의 배열입니다. 사용 가능한 옵션: `"licences"`, `"payments"`, `"customers"`, `"subscriptions"`. 이들의 조합을 포함할 수 있습니다.
</ParamField>
...
**필수 속성:**
- `bearerToken`: 귀하의 Dodo Payments API 키
- `environment`: `"test_mode"` 또는 `"live_mode"`
</ParamField>

<ParamField body="interval" type="number">
자동 동기화 간의 시간(초)입니다. 자동 동기화를 위한 `.start()`을 사용할 때 필수입니다. 수동 동기화를 위한 `.run()`을 사용할 때는 선택 사항입니다.
</ParamField>
...
<Warning>
**MongoDB**: `dodopayments_sync`라는 이름의 데이터베이스가 데이터베이스 서버에 자동으로 생성됩니다. 모든 동기화 데이터는 이곳에 저장됩니다. 데이터베이스 이름은 현재 고정되어 있으며 변경할 수 없습니다.

**PostgreSQL**: 귀하의 연결 URI에 지정된 데이터베이스에 (`Subscriptions`, `Payments`, `Licenses`, `Customers`) 테이블이 생성됩니다. 데이터는 JSONB 형식으로 저장됩니다.

**MySQL**: 귀하의 연결 URI에 지정된 데이터베이스에 (`Subscriptions`, `Payments`, `Licenses`, `Customers`) 테이블이 생성됩니다. 데이터는 JSON 형식으로 저장됩니다.

**ClickHouse**: ReplacingMergeTree 엔진을 사용하여 (`Subscriptions`, `Payments`, `Licenses`, `Customers`) 테이블이 생성됩니다. 쿼리할 때 결과의 중복 제거를 보장하기 위해 `FINAL` 키워드를 사용하십시오.
</Warning>
