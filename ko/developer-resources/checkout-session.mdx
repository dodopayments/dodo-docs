...
### 10. BNPL (지금 구매하고 나중에 결제하기) Checkout
...
### 11. 즉시 Checkout을 위한 기존 결제 방법 사용

고객의 저장된 결제 방법을 사용하여 결제 방법 수집을 건너뛰고 즉시 처리되는 Checkout 세션을 생성합니다:
...
<Warning>
`payment_method_id`를 사용할 때는 `confirm`를 `true`로 설정하고 기존 `customer_id`를 제공해야 합니다. 결제 방법은 결제 통화에 대한 적격성을 검증합니다.
</Warning>

<Info>
결제 방법은 고객에게 속해야 하며 결제 통화와 호환되어야 합니다. 이를 통해 반복 고객을 위한 원클릭 구매가 가능합니다.
</Info>
...
<Tip>
사용자 정의 성공 페이지가 기본 결제 성공 페이지보다 더 나은 사용자 경험을 제공하는 경우 `redirect_immediately: true`를 사용하세요. 이는 모바일 앱 및 임베드된 Checkout 흐름에 특히 유용합니다.
</Tip>

<Note>
`redirect_immediately`가 활성화되면 고객은 결제 완료 후 즉시 귀하의 `return_url`로 리디렉션되며 기본 성공 페이지를 완전히 건너뜁니다.
</Note>

## Checkout 세션 미리보기

Checkout 세션을 생성하기 전에 세금, 할인 및 총액을 포함한 가격 세부 정보를 미리 볼 수 있습니다. 이는 고객이 Checkout으로 진행하기 전에 정확한 가격을 표시하는 데 유용합니다.
...
## 동적 링크에서 Checkout 세션으로 이동하기

### 주요 차이점
이전에는 동적 링크로 결제 링크를 생성할 때 고객의 전체 청구 주소를 제공해야 했습니다.

Checkout 세션에서는 더 이상 필요하지 않습니다. 가지고 있는 정보를 전달하기만 하면 나머지는 처리합니다. 예를 들어:

- 고객의 청구 국가만 알고 있다면 그 정보만 제공하세요.
- Checkout 흐름은 결제 페이지로 이동하기 전에 누락된 세부 정보를 자동으로 수집합니다.
- 반면에 모든 필수 정보를 이미 알고 있고 결제 페이지로 직접 건너뛰고 싶다면 전체 데이터 세트를 전달하고 요청 본문에 `confirm=true`를 포함할 수 있습니다.

### 마이그레이션 프로세스

동적 링크에서 Checkout 세션으로의 마이그레이션은 간단합니다:
...
Checkout 세션 형식에 따라 요청 페이로드를 조정하세요.
</Step>
