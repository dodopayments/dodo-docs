...
**형식**: 국가 코드를 포함합니다, 예: `"+1234567890"` 미국 번호에 대한
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
<Warning>
**중요**: 항상 `credit` 및 `debit`를 대체 옵션으로 포함하여 선호하는 결제 방법이 사용 불가능할 때 체크아웃 실패를 방지하십시오.
</Warning>
...
**지원되는 통화**: `USD`, `EUR`, `GBP`, `CAD`, `AUD`, `INR`, 및 기타

**예시**: `"USD"` 미국 달러에 대한, `"EUR"` 유로에 대한
...
**형식**: 통화의 가장 낮은 단위로 표시됩니다 (예: 센트는 USD의 경우). 예를 들어, $1.00를 청구하려면 `100`를 전달합니다.
</ParamField>
...
<Warning>
`payment_method_id`를 사용할 때, `confirm`는 `true`로 설정되어야 하며, 기존의 `customer_id`가 제공되어야 합니다. 결제 방법은 결제 통화에 대한 적격성을 검증합니다.
</Warning>
...
<Tip>
사용자 경험이 기본 결제 성공 페이지보다 더 나은 사용자 경험을 제공하는 사용자 정의 성공 페이지가 있는 경우 `redirect_immediately: true`를 사용하십시오. 이는 모바일 앱 및 임베디드 체크아웃 흐름에 특히 유용합니다.
</Tip>
...
- 고객의 청구 국가만 알고 있다면, 그 정보만 제공하십시오.
- 체크아웃 흐름은 고객을 결제 페이지로 이동하기 전에 누락된 세부 정보를 자동으로 수집합니다.
- 반면, 모든 필수 정보를 이미 가지고 있고 결제 페이지로 직접 건너가고 싶다면, 전체 데이터 세트를 전달하고 요청 본문에 `confirm=true`를 포함할 수 있습니다.
