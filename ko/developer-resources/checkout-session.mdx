...
체크아웃 세션에 하나 이상의 누적 할인 코드를 적용합니다. 코드는 **배열 순서대로** 적용됩니다(첫 번째 코드는 시작 가격을 할인하고, 두 번째 코드는 이미 할인된 가격을 추가로 할인하는 방식). 세션당 최대 **20개**의 코드를 적용할 수 있습니다. [Purchasing Power Parity](/features/purchasing-power-parity)가 활성화된 경우 시작 가격은 기본 가격이 아니라 PPP가 조정된 금액입니다.
...
<Note>
미리 보기된 `current_breakup.subtotal`에는 제품에 적용되는 [Purchasing Power Parity](/features/purchasing-power-parity) 및 [Charm Pricing](/features/charm-pricing)이 이미 반영되어 있습니다.
</Note>

<Note>
장바구니에 구독 제품이 포함된 경우 미리 보기 응답은 `next_billing_date`도 반환합니다. 이는 예정된 청구일의 미리 보기이므로 구독이 생성되기 전에 표시할 수 있습니다. 이 값은 현재 시점을 기준으로 계산됩니다. 체험 기간이 적용되는 경우에는 `now + trial period`이고, 그렇지 않은 경우에는 `now + one payment frequency`입니다. 일회성 상품만 포함된 장바구니에서는 이 필드가 생략됩니다. 이는 미리 보기 시점을 기준으로 한 추정치이며, 실제 `next_billing_date`은 구독이 활성화될 때 설정됩니다.
</Note>

<Note>
미리 보기 응답은 `trial_period_days`(무료 또는 유료 여부와 관계없이 실제 적용되는 체험 기간) 및 `trial_amount`(할인 후 단위당 체험 요금으로, 가격 통화의 최소 단위로 표시됨)도 반환합니다. `trial_amount`은 [유료 체험 기간](/features/subscription#paid-trials)에만 포함되며, 무료 체험 기간이거나 체험 기간이 없으면 `null`입니다. 오늘 실제로 납부해야 하는 세금 포함 총액에는 `current_breakup`을 사용하세요.
</Note>
...
## Dynamic Links에서 Checkout Sessions로 전환

### 주요 차이점
이전에는 Dynamic Links를 사용해 결제 링크를 생성할 때 고객의 전체 청구지 주소를 제공해야 했습니다.

Checkout Sessions에서는 더 이상 이 작업이 필요하지 않습니다. 가지고 있는 정보만 전달하면 나머지는 시스템에서 처리합니다. 예를 들어 다음과 같습니다.

- 고객의 청구 국가만 알고 있다면 해당 정보만 제공하면 됩니다.
- 체크아웃 흐름에서 고객을 결제 페이지로 이동시키기 전에 누락된 세부 정보를 자동으로 수집합니다.
- 반대로 필요한 정보를 모두 이미 가지고 있고 바로 결제 페이지로 이동하려면 전체 데이터 세트를 전달하고 요청 본문에 `confirm=true`을 포함하면 됩니다.
...
Checkout Sessions 형식에 따라 요청 페이로드를 조정합니다.
</Step>

<Step title="That's it!">
예. 귀하의 측에서 추가 처리나 특별한 마이그레이션 단계는 필요하지 않습니다.
</Step>
</Steps>
...
<Card title="Create Checkout Session" icon="code" href="/api-reference/checkout-sessions/create">
  사용 가능한 모든 매개변수와 옵션을 포함한 Checkout Sessions 생성 전체 API 레퍼런스
</Card>
<Card title="Preview Checkout Session" icon="eye" href="/api-reference/checkout-sessions/preview">
  세션 생성 전에 가격, 세금 및 총액을 미리 확인하기 위한 API 레퍼런스
</Card>
</CardGroup>