...
<Info>
고객의 사용자 정의 필드에 대한 응답은 다음 항목에 포함됩니다:
- **웹후크**: `payment.succeeded`, `subscription.active`, 기타 관련 이벤트 페이로드에는 `custom_field_responses` 배열이 포함됩니다
- **API 응답**: 결제 및 구독 객체에는 `custom_field_responses`가 포함됩니다
</Info>
...
<Info>
사용자 정의 필드 응답은 웹후크 페이로드(`payment.succeeded`, `subscription.active` 등)에 자동으로 포함되며 API를 통해 검색할 수 있습니다. 이를 사용하여 CRM을 풍부하게 하거나 온보딩 흐름을 트리거하거나 고객 경험을 맞춤화하세요.
</Info>
