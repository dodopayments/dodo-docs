...
<Info>
**세션 유효성**: Checkout 세션은 기본적으로 24시간 동안 유효합니다. 요청에 `confirm=true`를 전달하면 세션은 15분 동안만 유효합니다.
</Info>
...
<Step title="반환 처리">
결제 후 고객은 결제 상태에 대한 추가 쿼리 매개변수와 함께 귀하의 `return_url`로 리디렉션됩니다.
</Step>
</Steps>
...
Checkout 세션에 포함할 제품의 배열입니다. 각 제품은 Dodo Payments 대시보드에서 유효한 `product_id`를 가져야 합니다.
...
**형식**: 국가 코드를 포함하세요. 예: `"+1234567890"`는 미국 번호에 해당합니다.
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
<Warning>
**중요**: 항상 `credit` 및 `debit`를 대체 옵션으로 포함하여 선호하는 결제 방법이 사용 불가능할 때 Checkout 실패를 방지하세요.
</Warning>
...
**지원되는 통화**: `USD`, `EUR`, `GBP`, `CAD`, `AUD`, `INR`, 등등

**예시**: `"USD"`는 미국 달러, `"EUR"`는 유로입니다.
...
Checkout 인터페이스에 주문 세부정보 섹션을 표시합니다.
</ParamField>

<ParamField body="show_on_demand_tag" type="boolean" default="true">
적용 가능한 제품에 대해 "on-demand" 태그를 표시합니다.
</ParamField>

<ParamField body="force_language" type="string">
Checkout 인터페이스를 특정 언어로 렌더링하도록 강제합니다.

**예시**: `"en"`는 영어, `"es"`는 스페인어입니다.
</ParamField>
</Expandable>
</ParamField>
...
사용 기반 또는 계량 청구를 위한 주문형 구독 구성입니다.

<Expandable title="주문형 객체 속성">
<ParamField body="mandate_only" type="boolean" required>
true로 설정하면 청구를 수행하지 않고 결제 방법 세부정보만 미래에 사용할 수 있도록 승인합니다.
</ParamField>

<ParamField body="product_price" type="integer">
고객에게 최초 청구를 위한 제품 가격입니다. 지정하지 않으면 제품의 저장된 가격이 사용됩니다.

**형식**: 통화의 가장 낮은 단위로 표시됩니다 (예: 센트는 USD의 경우). 예를 들어, $1.00을 청구하려면 `100`를 전달하세요.
</ParamField>

<ParamField body="product_currency" type="string">
제품 가격의 선택적 통화입니다. 지정하지 않으면 제품의 통화로 기본값이 설정됩니다.
</ParamField>

<ParamField body="product_description" type="string">
청구 및 라인 항목에 대한 선택적 제품 설명 재정의입니다. 지정하지 않으면 제품의 저장된 설명이 사용됩니다.
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
적응형 통화 수수료가 제품 가격에 포함되어야 하는지 (true) 또는 추가되어야 하는지 (false) 여부입니다. 적응형 가격 책정이 비활성화된 경우 무시됩니다.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
<Note>
`confirm`가 `true`로 설정되면 고객은 확인 단계를 건너뛰고 Checkout 페이지로 직접 이동합니다.
</Note>
...
BNPL 구성 및 테스트에 대한 자세한 내용은 <a href="/features/payment-methods/bnpl">지금 사서 나중에 결제하기 (BNPL)</a> 페이지를 참조하세요.
...
<Warning>
`payment_method_id`을 사용할 때, `confirm`는 `true`로 설정되어야 하며 기존의 `customer_id`이 제공되어야 합니다. 결제 방법은 결제 통화에 대해 적합 여부를 확인합니다.
</Warning>
...
<Tip>
사용자 정의 성공 페이지가 기본 결제 성공 페이지보다 더 나은 사용자 경험을 제공하는 경우 `redirect_immediately: true`를 사용하세요. 이는 특히 모바일 앱과 임베디드 체크아웃 흐름에 유용합니다.
</Tip>

<Note>
`redirect_immediately`가 활성화되면, 고객은 결제가 완료된 즉시 당신의 `return_url`로 리디렉션되며 기본 성공 페이지를 완전히 건너뜁니다.
</Note>
...
<Card title="미리 보기 API 참조" icon="code" href="/api-reference/checkout-sessions/preview">
...
- 반면에, 필요한 모든 정보를 이미 알고 있고 결제 페이지로 직접 건너가고 싶다면 전체 데이터 세트를 전달하고 요청 본문에 `confirm=true`를 포함하세요.
...
<Tip>
기본 결제 성공 페이지보다 더 나은 사용자 경험을 제공하는 커스텀 성공 페이지를 사용하는 경우 `redirect_immediately: true`을 사용하세요. 이는 모바일 앱과 임베디드 체크아웃 흐름에서 특히 유용합니다.
</Tip>

<Note>
`redirect_immediately`이 활성화되어 있으면 고객은 결제 완료 직후 기본 성공 페이지를 완전히 건너뛰고 즉시 `return_url`으로 리디렉션됩니다.
</Note>

### 14. 언어 강제 지정

고객의 브라우저 언어 감지를 무시하고 체크아웃을 특정 언어로 표시하도록 강제합니다:
...
<Tip>
고객의 선호 언어(예: 계정 설정에서 확인한 값)를 알고 있거나 특정 지역 시장을 타깃팅할 때 `force_language`을 사용하세요.
</Tip>
...
커스텀 필드를 사용하여 체크아웃 중 고객으로부터 추가 정보를 수집하세요:
...
<Info>
커스텀 필드 응답은 웹훅 페이로드(`payment.succeeded`, `subscription.created` 등)에 자동으로 포함되며 API를 통해서도 조회할 수 있습니다. 이를 활용해 CRM을 강화하거나 온보딩 흐름을 트리거하거나 고객 경험을 맞춤화하세요.
</Info>
...
체크아웃 세션을 생성하기 전에 세금, 할인 및 총액을 포함한 가격 내역을 미리 확인할 수 있습니다. 이는 고객이 체크아웃에 진행하기 전에 정확한 가격을 보여주는 데 유용합니다.
...
{/* LOCKED_PATTERN_5c479be80ff82df5704d654ba0e0d13d */}
  전체 미리보기 엔드포인트 문서를 확인하세요.
</Card>

## 다이나믹 링크에서 체크아웃 세션으로 이동하기

### 주요 차이점
과거 다이나믹 링크로 결제 링크를 생성할 때는 고객의 전체 청구 주소를 제공해야 했습니다.

체크아웃 세션을 사용하면 더 이상 그럴 필요 없습니다. 가지고 있는 정보만 전달하면 나머지는 저희가 처리합니다. 예를 들어:

- 고객의 청구 국가만 알고 있다면 그 정보만 제공하세요.
- 체크아웃 흐름이 결제 페이지로 이동하기 전에 누락된 세부 정보를 자동으로 수집합니다.
- 반대로 이미 필요한 모든 정보를 보유하고 있고 결제 페이지로 바로 이동하려면 전체 데이터 세트를 전달하고 요청 본문에 `confirm=true`를 포함하세요.
...
통합을 업데이트하여 새로운 API 또는 SDK 메서드를 사용하세요.
</Step>

{/* LOCKED_PATTERN_9a0d7dfb0d22a80cdd653a6f423ff618 */}
요청 페이로드를 체크아웃 세션 형식에 맞게 조정하세요.
</Step>

{/* LOCKED_PATTERN_ea09e0cebc00c36bba010ba9740abe0d */}
네. 별도의 처리나 특별한 마이그레이션 단계는 필요하지 않습니다.
</Step>
</Steps>
