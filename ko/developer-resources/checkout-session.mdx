...
**Common options**: `credit`, `debit`, `upi_collect`, `apple_pay`, `google_pay`, `amazon_pay`, `klarna`, `affirm`, `afterpay_clearpay`, `cashapp`, `multibanco`, `bancontact_card`, `eps`, `ideal`, `przelewy24`, `paypal`. 전체 목록은 아니므로 모든 허용 값을 확인하려면 [Create Checkout Session API reference](/api-reference/checkout-sessions/create)를 참조하세요.
