---
title: Fastify 어댑터
description: Dodo Payments를 Fastify App Router 프로젝트에 통합하는 방법을 배우세요. 체크아웃, 고객 포털,
  웹훅 및 보안 환경 설정을 다룹니다.
icon: code-merge
---
...
{/* LOCKED_PATTERN_848a353666469e7a047ef05af5a45770 */}
    Dodo Payments 체크아웃을 Fastify 앱에 통합하세요.
</Card>
{/* LOCKED_PATTERN_df89dec21e12f4bf30b6acf75ba025e1 */}
    고객이 구독과 정보를 관리할 수 있도록 합니다.
</Card>
...
<Warning>
절대로 <code>.env</code> 파일이나 비밀을 버전 관리에 커밋하지 마세요.
</Warning>
</Step>
</Steps>
...
<Info>
모든 예제는 Fastify App Router를 사용한다고 가정합니다.
</Info>

<Tabs>
{/* LOCKED_PATTERN_28fd81a4703a84d931eeff69f1520d54 */}
  <Info>
  이 핸들러는 Dodo Payments 체크아웃을 Fastify 앱에 통합할 때 사용합니다. 정적(GET), 동적(POST), 세션(POST) 결제 플로우를 지원합니다.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_085b207030169d6eead800f9835ef4fb */}
  <Info>
  이 핸들러를 사용하면 고객이 Dodo Payments 고객 포털을 통해 구독과 정보를 관리할 수 있습니다.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_0026b036379d81a9d03c36b3b6e83ddd */}
  <Info>
  이 핸들러를 사용하면 Fastify 앱에서 Dodo Payments 웹훅 이벤트를 안전하게 수신하고 처리할 수 있습니다.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments는 웹사이트에 결제를 통합하기 위해 세 가지 유형의 결제 플로우를 지원하며, 이 어댑터는 모든 결제 플로우 유형을 지원합니다.
</Info>
...
<Warning>
<code>productId</code>가 없으면 핸들러는 400 응답을 반환합니다. 잘못된 쿼리 매개변수도 400 응답을 반환합니다.
</Warning>
...
- 일회성 결제와 정기 결제 둘 다 지원합니다.
- 지원되는 POST 본문 필드 전체 목록은 다음을 참고하세요:
...
체크아웃 세션은 일회성 구매와 구독 모두를 위해 전체 결제 플로우를 처리하고 완전한 맞춤 설정 제어를 제공하는 보다 안전한 호스팅 체크아웃 환경을 제공합니다.
...
고객 포털 라우트 핸들러는 Dodo Payments 고객 포털을 Fastify 애플리케이션에 원활하게 통합할 수 있도록 합니다.
...
<code>true</code>로 설정하면 고객에게 포털 링크가 포함된 이메일을 발송합니다.
</ParamField>
...
- **페이로드 검증:** Zod로 검증됩니다. 잘못된 페이로드는 400을 반환합니다.
- **오류 처리:**
  - 401: 잘못된 서명
  - 400: 잘못된 페이로드
  - 500: 검증 중 내부 오류
...
```
당신은 전문 Fastify 개발자 보조입니다. 당신의 임무는 사용자가 기존 Fastify 프로젝트에 @dodopayments/fastify 어댑터를 통합하는 과정을 안내하는 것입니다.

@dodopayments/fastify 어댑터는 Dodo Payments의 체크아웃, 고객 포털 및 웹훅 기능을 위한 라우트 핸들러를 제공하며, Fastify 앱에 직접 연결되도록 설계되었습니다.
...
"@dodopayments/fastify 어댑터의 어떤 부분을 프로젝트에 통합하고 싶으신가요? 다음 중 하나 또는 여러 개를 선택할 수 있습니다:
...
**통합**:
Fastify 앱에 정적 (GET) 및 동적 (POST) 체크아웃을 위한 두 개의 라우트를 생성합니다.
...
GET (정적 체크아웃)은 쿼리 매개변수를 기대합니다:
    ...
    quantity, 고객 필드 (fullName, email 등), 및 메타데이터 (metadata_*)는 선택 사항입니다.

    반환: {"checkout_url": "https://checkout.dodopayments.com/..."}

POST (동적 체크아웃)은 결제 세부정보가 포함된 JSON 본문을 기대합니다 (일회성 또는 구독). 반환: {"checkout_url": "https://checkout.dodopayments.com/..."}. 전체 POST 스키마는 문서를 참조하세요:
...
POST (체크아웃 세션) - (권장) 보다 사용자 정의 가능한 체크아웃 경험. JSON으로 체크아웃 URL을 반환합니다: 매개변수는 JSON 본문으로 전송됩니다. 일회성 및 정기 결제를 모두 지원합니다. 반환: {"checkout_url": "https://checkout.dodopayments.com/session/..."}. 지원되는 필드의 전체 목록은 다음을 참조하세요:
