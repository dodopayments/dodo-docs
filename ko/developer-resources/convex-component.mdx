...
`convex.config.ts`를 편집한 후, 필요한 타입을 생성하기 위해 `npx convex dev`를 한 번 실행하세요.
</Step>
<Step title="환경 변수 설정">
Convex 대시보드에서 환경 변수를 설정하세요 (**Settings** → **Environment Variables**). 대시보드에 접근하려면 다음 명령을 실행하세요:
...
다음 환경 변수를 추가하세요:
- `DODO_PAYMENTS_API_KEY` - 귀하의 Dodo Payments API 키
- `DODO_PAYMENTS_ENVIRONMENT` - `test_mode` 또는 `live_mode`로 설정
- `DODO_PAYMENTS_WEBHOOK_SECRET` - 귀하의 웹훅 시크릿 (웹훅 처리를 위해 필요)
...
<Warning>
이 쿼리를 사용하기 전에, `convex/schema.ts` 파일에 적절한 스키마를 정의하였거나, 쿼리를 기존 스키마에 맞게 변경했는지 확인하세요.
</Warning>
...
<Tab title="고객 포털 설정">
  <Info>
  이 기능을 사용하여 고객이 Dodo Payments 고객 포털을 통해 구독 및 세부 정보를 관리할 수 있습니다. 고객은 `identify` 함수를 통해 자동으로 식별됩니다.
  </Info>
  <CodeGroup>
...
<Tab title="웹훅 핸들러 설정">
  <Info>
  이 핸들러를 사용하여 Dodo Payments 웹훅 이벤트를 안전하게 수신하고 처리합니다. 모든 웹훅 핸들러는 첫 번째 매개변수로 Convex `ActionCtx`를 수신하여 `ctx.runQuery()` 및 `ctx.runMutation()`와 상호 작용할 수 있습니다.
  </Info>
  <CodeGroup>
