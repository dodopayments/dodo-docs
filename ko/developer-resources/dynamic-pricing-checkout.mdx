...
2. **동적 금액 전달**: 체크아웃 세션 생성 시 제품 장바구니에 `amount` 필드를 포함합니다.
3. **고객 선택 권한 부여**: 금액이 제공되지 않으면 고객이 자신의 가격(설정한 범위 내) 입력할 수 있습니다.

<Info>
제품 장바구니에서 `amount`를 전달하면 해당 금액이 체크아웃에 사용됩니다. `amount` 필드를 생략하면 고객이 체크아웃 중 자신의 가격을 선택할 수 있습니다(최소/최대 설정에 따라).
</Info>
...
**예제**: 최소 가격이 $5.00인 경우 `5.00`(또는 `500` 센트)를 입력합니다.
</Step>
...
<Step title="제품 저장">
**제품 추가**를 클릭하여 저장합니다. 체크아웃 세션에서 사용할 제품 ID(예: `pdt_123abc456def`)를 주의해 두세요.
</Step>
</Steps>
...
Pay What You Want로 제품이 구성되면, 동적 금액으로 체크아웃 세션을 생성할 수 있습니다. 제품 장바구니의 `amount` 필드를 사용하여 각 체크아웃 세션의 가격을 프로그래밍 방식으로 설정할 수 있습니다.

### 금액 필드 이해하기

체크아웃 세션을 생성할 때 각 제품 장바구니 항목에 `amount` 필드를 포함할 수 있습니다:

- **`amount`가 제공되는 경우**: 체크아웃은 이 정확한 금액을 사용합니다(최소/최대 범위 내여야 함).
- **`amount`가 생략된 경우**: 고객은 체크아웃 중 자신의 가격을 입력할 수 있습니다(설정한 범위 내).
...
<Note>
**금액 형식**: `amount` 필드는 통화의 최저 단위로 입력되어야 합니다. USD의 경우 이는 센트를 의미합니다(예: $25.00 = `2500`). 다른 통화의 경우 가장 작은 단위를 사용하세요(예: INR의 경우 파이세).
</Note>

## 3단계: 고객이 자신의 가격 선택하기

고객이 체크아웃 중 자신의 가격을 선택하도록 하려면 제품 장바구니에서 `amount` 필드를 생략하면 됩니다. 체크아웃 페이지에는 고객이 설정한 최소 및 최대 범위 내의 금액을 입력할 수 있는 입력 필드가 표시됩니다.
...
<AccordionGroup>
<Accordion title="금액 무시됨">
`amount` 필드가 무시되는 경우, 다음을 확인하세요:
- 대시보드에서 제품에 **Pay What You Want**가 활성화되어 있는지 확인
- 제품이 **단일 결제**(일회성) 제품인지 확인, 구독아님
- 금액이 올바른 형식(최저 통화 단위, 예: USD의 경우 센트)인지 확인
</Accordion>

<Accordion title="금액이 최대를 초과하거나 최소 미만">
API는 금액이 제품의 가격 범위를 위반하는 경우 체크아웃 세션을 거부합니다. 체크아웃 세션을 생성하기 전에 항상 금액을 검증하거나 `amount` 필드를 생략하여 고객이 가격을 선택하게 하세요.
</Accordion>

<Accordion title="고객이 자신의 가격을 입력할 수 없음">
고객이 가격 입력 필드를 보지 못하는 경우, 제품 장바구니에서 `amount` 필드를 생략했는지 확인하세요. `amount`가 제공되는 경우 체크아웃은 해당 정확한 금액을 사용합니다.
</Accordion>
</AccordionGroup>