...
1. **로그인** [Dodo Payments 대시보드](https://app.dodopayments.com/)에
2. **이동** 제품 → 계량기
3. **클릭** "계량기 생성하기"
4. **계량기 구성**:
   - **계량기 이름**: 설명적인 이름 선택 (예: "LLM 토큰 사용")
...
- `test_mode` - 개발 및 테스트 용도 사용
- `live_mode` - 프로덕션 용도 사용
...
<Warning>
개발 중에는 항상 `test_mode`를 사용하여 프로덕션 메트릭에 영향을 미치지 않도록 하세요.
</Warning>
</ParamField>
...
<Note>
추론 가능 모델을 AI SDK를 통해 사용할 때(예: Google의 Gemini 2.5 Flash에 사고 모드), 추론 토큰은 정확한 청구를 위해 `outputTokens` 수에 자동으로 포함됩니다.
</Note>
</Accordion>
...
<Note>
**Gemini 사고 모드:** 사고/추론 능력을 가진 Gemini 모델을 사용할 때(Gemini 2.5 Pro처럼), SDK는 전체 계산 비용을 정확하게 반영하기 위해 `thoughtsTokenCount` (추론 토큰)을 `outputTokens`에 자동으로 포함합니다.
</Note>
</Accordion>
</AccordionGroup>
...
<Note>
**추론 토큰:** 추론 기능이 있는 모델의 경우, `outputTokens`는 완료 토큰과 추론 토큰 모두를 자동으로 포함합니다.
</Note>
</ParamField>

<Info>
당신의 Dodo Payments 계량기는 사용량 및 청구를 계산하기 위해 `metadata` 필드(특히 `inputTokens`, `outputTokens` 또는 `totalTokens`)를 사용합니다.
</Info>
