...
**메터 이름**: 이 내용을 정확히 복사하여 붙여넣기 → `Image Generation Meter`

**설명**: 이 내용을 복사하세요 → `Tracks each AI image generation request made by customers using our DALL-E powered service`

**이벤트 이름**: 이것은 중요합니다 - 정확히 복사하세요 → `image.generated`

<Warning>
이벤트 이름 `image.generated`은(는) 나중에 애플리케이션 코드에서 보낼 내용과 정확히 일치해야 합니다. 이벤트 이름은 대소문자를 구분합니다!
</Warning>
</Step>
...
1. **이벤트 필터링 활성화**: 이 설정을 **켭니다**.
2. **필터 논리**: **OR** 선택 (이는 "이 조건 중 ANY가 참이면 계산"을 의미합니다.)
3. **첫 번째 조건 추가**:
   - 속성 키: `quality`
   - 비교자: `equals`  
   - 값: `standard`
4. **두 번째 조건을 추가하기 위해 "조건 추가" 클릭**:
   - 속성 키: `quality`
   - 비교자: `equals`
   - 값: `hd`
...
**제품 이름**: 이 내용을 복사하세요 → `PixelGen AI - Image Generation`

**설명**: 이 내용을 복사하세요 → `AI-powered image generation service with pay-per-use billing`
...
또한, **고정 가격**을 `0`로 설정하여 고객이 사용량에 따라 요금이 부과되도록 하세요. 기본 요금은 없습니다.
...
**단위당 가격**: 입력하세요 → `0.05` (이것은 이미지 당 $0.05입니다)

**무료 기준**: 입력하세요 → `10` (고객은 매월 10개의 무료 이미지를 받을 수 있습니다)
...
4. 고객 ID를 복사하세요 - `cus_abc123def456`처럼 보일 것입니다.
...
<Step title="주 응용 프로그램 만들기">
`index.ts`라는 파일을 생성하고 이 전체 응용 프로그램 코드를 복사하세요:
</Step>
</Steps>
...
1. `.env` 파일을 `pixelgen-ai` 디렉터리에 생성합니다.
2. 실제 API 키를 추가하세요:
