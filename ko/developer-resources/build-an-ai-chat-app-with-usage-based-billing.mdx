...
**집계 유형**: 타입 → `totalTokens`

**측정 단위**: 타입 → `tokens`

<Info>
우리는 "합계"를 사용하고 있습니다. 여러 채팅 메시지에서 소비된 모든 토큰을 합산하려고 하기 때문입니다. SDK는 각 이벤트에서 `totalTokens`을 자동으로 전송합니다.
</Info>
</Step>
...
2. **API 키 생성** 클릭
3. API 키를 복사하세요 - `test_abc123...` 형식입니다.
...
추가로, 고객이 사용량에 따라 요금만 청구되도록 **고정 가격**을 `0`로 설정합니다. 기본 요금은 없습니다.
...
**단위당 가격**: 입력 → `0.00001` (1,000토큰당 $0.01 또는 토큰당 $0.00001입니다)

**무료 한도**: 입력 → `10000` (고객은 한 달에 10,000개의 무료 토큰을 받을 수 있습니다)
...
`src/server.ts`을 생성하고 이 전체 서버 코드를 복사하세요:
...
이제 전체 대화 기록이 있는 멋진 채팅 인터페이스를 추가해보겠습니다! `public/index.html`을 생성하세요:
...
1. `.env` 파일에 Step 2의 모든 API 키가 포함되었는지 확인하세요.
2. 개발 서버를 시작합니다:
...
3. 멋진 채팅 인터페이스를 확인해야 합니다.

<Note>
`CUSTOMER_ID`을 업데이트하여 Step 4에서 실제 테스트 고객 ID로 교체해야 합니다.
</Note>
</Step>
...
**가능한 원인:**
- 환경에 대한 잘못된 API 키
- `.env` 파일에 여분의 공백이나 따옴표가 있음

**해결 방법:**
- 테스트 키가 `test_`로 시작하고, 라이브 키가 `live_`로 시작하는지 확인합니다.
