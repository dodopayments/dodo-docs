...
description: SvelteKit Adaptor를 사용하여 <CardGroup cols={2}>을(를) SvelteKit App Router
  프로젝트에 통합하는 방법을 알아보세요. checkout, customer portal, webhooks 및 보안 환경 설정을 다룹니다.
icon: code-merge
keywords:
  - Dodo Payments
  - SvelteKit
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    SvelteKit 앱에 Dodo Payments checkout을 통합합니다.
</Card>
...
<Warning>
<code>.env</code> 파일이나 secret을 version control에 커밋하지 마세요.
</Warning>
</Step>
</Steps>
...
<Info>
모든 예시는 SvelteKit App Router를 사용한다고 가정합니다.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  이 handler를 사용하여 Dodo Payments checkout을 SvelteKit 앱에 통합합니다. static (GET), dynamic (POST) 및 session (POST) payment flow를 지원합니다.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  이 handler를 사용하여 SvelteKit 앱에서 Dodo Payments webhook 이벤트를 안전하게 수신하고 처리합니다.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments은 웹사이트에 결제를 통합하기 위한 세 가지 payment flow를 지원하며, 이 adaptor는 모든 payment flow를 지원합니다.
</Info>

- **Static Payment Links:** 빠르고 코드 없이 결제를 수집할 수 있는 즉시 공유 가능한 URL입니다.
- **Dynamic Payment Links:** API 또는 SDK를 사용하여 사용자 지정 세부 정보가 포함된 payment link를 프로그래밍 방식으로 생성합니다.
- **Checkout Sessions:** 사전 구성된 product cart 및 customer details와 함께 안전하고 사용자 지정 가능한 checkout 환경을 생성합니다.
...
청구 금액을 주요 통화 단위로 고정합니다(예: $12.50의 경우 <code>12.5</code>). Pay What You Want 상품에만 적용되며, 상품의 최소 가격보다 낮으면 무시됩니다.
</ParamField>
...
- 일회성 및 recurring payment를 모두 지원합니다.
...
<Warning>
Dynamic Checkout은 더 이상 사용되지 않는 `POST /payments` 및 `POST /subscriptions` 엔드포인트를 프록시합니다. 기존 통합에서는 계속 작동하지만, 새 통합에는 아래의 **Checkout Sessions**를 사용해야 합니다.
</Warning>
...
Checkout sessions는 일회성 구매와 구독 모두에 대해 전체 결제 플로우를 처리하며, 완전한 사용자 지정 제어 기능을 제공하는 더 안전한 호스팅 checkout 환경을 제공합니다.
...
Customer Portal Route Handler를 사용하면 Dodo Payments customer portal을 SvelteKit 애플리케이션에 원활하게 통합할 수 있습니다.
