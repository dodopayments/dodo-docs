---
title: SvelteKit 어댑터
description: SvelteKit App Router 프로젝트에 Dodo Payments를 통합하는 방법을 배우세요. 체크아웃, 고객
...
{/* LOCKED_PATTERN_848a353666469e7a047ef05af5a45770 */}
    Dodo Payments 체크아웃을 SvelteKit 앱에 통합합니다.
</Card>
...
<Info>
모든 예제는 SvelteKit 앱 라우터를 사용한다고 가정합니다.
</Info>

<Tabs>
{/* LOCKED_PATTERN_28fd81a4703a84d931eeff69f1520d54 */}
  <Info>
  이 핸들러를 사용하여 Dodo Payments 체크아웃을 SvelteKit 앱에 통합하세요. 정적(GET), 동적(POST), 세션(POST) 결제 플로우를 모두 지원합니다.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_0026b036379d81a9d03c36b3b6e83ddd */}
  <Info>
  이 핸들러를 사용하여 SvelteKit 앱에서 Dodo Payments 웹훅 이벤트를 안전하게 수신하고 처리합니다.
  </Info>
  <CodeGroup>
...
<Warning>
<code>productId</code>가 누락되면 핸들러는 400 응답을 반환합니다. 유효하지 않은 쿼리 매개변수도 400 응답을 반환합니다.
</Warning>
...
- POST 요청에서 매개변수를 JSON 본문으로 전송합니다.
...
체크아웃 세션은 일회성 구매와 구독 모두에 대해 전체 결제 흐름을 처리하고 완전한 맞춤형 제어가 가능한 보다 안전한 호스팅된 체크아웃 경험을 제공합니다.
...
고객 포털 경로 핸들러는 Dodo Payments 고객 포털을 SvelteKit 애플리케이션에 원활하게 통합할 수 있도록 합니다.

### 쿼리 매개변수
<ParamField query="customer_id" type="string" required>
포털 세션용 고객 ID (예: <code>?customer_id=cus_123</code>).
</ParamField>
