...
<Step title="환경 파일 생성">
Dodo Payments API 키가 포함된 `.env` 파일을 생성하세요:
...
<Warning>
결코 API 키를 버전 관리에 커밋하지 마십시오. `.env`를 `.gitignore` 파일에 추가하세요.
</Warning>
</Step>

<Step title="체크아웃 세션 생성 구현">
...
<Step title="간단한 웹 인터페이스 추가">
손쉬운 테스트를 위해 `public/index.html` 파일을 생성하세요:
...
<Check>
**정적 파일이 설정되었습니다!** 데모 인터페이스를 보려면 `http://localhost:3000`를 방문하세요.
</Check>
</Step>
</Steps>
...
2. 제품 및 추가 모듈 ID를 Dodo Payments 대시보드의 실제 값으로 업데이트하세요.
3. 서버를 시작하세요:
...
2. 좌석 기반 가격 책정 데모 인터페이스가 나타나야 합니다.
