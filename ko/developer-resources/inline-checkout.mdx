...
| 옵션 | 유형 | 필수 | 설명 |
|--------|------|----------|-------------|
| `checkoutUrl` | `string` | 예 | 체크아웃 세션 URL입니다. |
| `elementId` | `string` | 예 | 체크아웃이 렌더링되어야 하는 DOM 요소의 `id`입니다. |
| `options.showTimer` | `boolean` | 아니요 | 체크아웃 타이머를 표시하거나 숨깁니다. 기본값은 `true`입니다. 비활성화하면 세션이 만료될 때 `checkout.link_expired` 이벤트를 수신합니다. |
| `options.showSecurityBadge` | `boolean` | 아니요 | 보안 배지를 표시하거나 숨깁니다. 기본값은 `true`입니다. |
| `options.manualRedirect` | `boolean` | 아니요 | 활성화하면 결제 완료 후 자동 리디렉션이 진행되지 않습니다. 대신 `checkout.status` 및 `checkout.redirect_requested` 이벤트를 받아 직접 리디렉션을 처리해야 합니다. |
| `options.themeConfig` | `ThemeConfig` | 아니요 | 사용자 정의 테마 구성입니다. |
| `options.payButtonText` | `string` | 아니요 | 결제 버튼에 표시할 사용자 정의 텍스트입니다. |
| `options.fontSize` | `FontSize` | 아니요 | 체크아웃의 글로벌 글꼴 크기입니다. |
| `options.fontWeight` | `FontWeight` | 아니요 | 체크아웃의 글로벌 글꼴 두께입니다. |
...
`options` 매개변수에 `themeConfig` 객체를 전달하여 체크아웃 외관을 사용자 정의할 수 있습니다. 테마 구성은 라이트 및 다크 모드를 모두 지원하며, 색상, 테두리, 텍스트, 버튼 및 테두리 반경을 사용자 정의할 수 있습니다.

<Info>
이 섹션에서는 Checkout SDK를 사용한 **클라이언트 측** 테마 구성을 다룹니다. API를 통해 체크아웃 세션을 생성할 때 `theme_config` 매개변수로 **서버 측** 테마 구성도 가능합니다. API 수준 구성을 보려면 [Checkout Theme Customization](/features/checkout#checkout-theme-customization)을 참조하세요.
</Info>
...
인라인 결제는 **구독의 결제 수단 업데이트**를 지원합니다. 활성 구독이든 보류 상태 구독을 다시 활성화하는 경우든, 결제 수단 업데이트 흐름을 페이지 레이아웃 내에서 직접 렌더링할 수 있습니다.

### 작동 방식

1. [Update Payment Method API](/features/subscription#update-payment-method-for-active-subscription)를 호출하여 `payment_link`을 가져옵니다:
...
2. 반환된 `payment_link`을 `checkoutUrl`으로 전달하여 인라인 결제를 엽니다:
...
인라인 프레임은 결제 수단 수집 폼만 렌더링합니다. 고객은 페이지를 벗어나지 않고 새 카드 정보를 입력하거나 저장된 결제 수단을 선택할 수 있습니다.

### 보류 중인 구독의 경우

`on_hold` 상태의 구독에 대해 결제 수단을 업데이트하면, Dodo Payments는 남은 금액에 대해 자동으로 청구를 생성합니다. 재활성화를 확인하려면 `payment.succeeded` 및 `subscription.active` 웹후크를 모니터링하세요.
...
<Tip>
기존 저장된 결제 수단을 사용하고 싶다면 `type: 'existing'`과 `payment_method_id`를 Update Payment Method API에 전달하면 됩니다.
</Tip>

## 오류 처리

SDK는 이벤트 시스템을 통해 자세한 오류 정보를 제공합니다. `onEvent` 콜백에서 적절한 오류 처리를 항상 구현하세요:
...
<Warning>
문제가 발생할 경우 `checkout.error` 이벤트를 반드시 처리하여 사용자 경험을 개선하세요.
</Warning>

## 모범 사례

1. **반응형 디자인**: 컨테이너 요소의 너비와 높이가 충분한지 확인하세요. iframe은 일반적으로 컨테이너를 채우도록 확장됩니다.
2. **동기화**: `checkout.breakdown` 이벤트를 사용하여 사용자 정의 주문 요약이나 가격표가 결제 프레임에서 보는 내용과 일치하도록 유지하세요.
...
<Info>
다크 모드 구현에서는 인라인 결제 프레임과의 최적 시각적 통합을 위해 `#0d0d0d`를 배경 색상으로 사용하는 것이 권장됩니다.
</Info>

## 결제 상태 검증

<Warning>
결제 성공 또는 실패를 판단할 때 인라인 결제 이벤트만 신뢰하지 마세요. 웹훅 및/또는 폴링을 사용하여 항상 서버 측 검증을 구현하세요.
</Warning>

### 서버 측 검증이 필수인 이유

`checkout.status`와 같은 인라인 결제 이벤트는 실시간 피드백을 제공하지만, 결제 상태에 대한 유일한 진실된 정보원으로 사용해서는 **안 됩니다**. 네트워크 문제, 브라우저 충돌 또는 사용자가 페이지를 닫으면 이벤트가 누락될 수 있습니다. 신뢰할 수 있는 결제 검증을 위해 다음을 수행하세요:

1. **서버에서 웹훅 이벤트를 청취하세요** - Dodo Payments는 결제 상태 변경에 대한 웹훅을 전송합니다.
2. **폴링 메커니즘을 구현하세요** - 프런트엔드에서 상태 업데이트를 확인하기 위해 서버를 폴링하세요.
3. **두 방법을 모두 결합하세요** - 웹훅을 주요 정보원으로 사용하고 폴링을 예비 수단으로 활용하세요.
...
**1. 결제 이벤트 청취** - 사용자가 결제 버튼을 클릭하면 상태를 검증할 준비를 시작하세요:
...
**2. 서버 폴링** - 데이터베이스에서 결제 상태를 확인하는 엔드포인트를 만드세요(웹훅으로 업데이트됨):
...
**3. 서버 측 웹훅 처리** - Dodo가 `payment.succeeded` 또는 `payment.failed` 웹훅을 보낼 때 데이터베이스를 업데이트하세요. 자세한 내용은 [Webhooks documentation](/developer-resources/webhooks)를 참조하세요.

### 리디렉션 처리(3DS, Google Pay, UPI)

`manualRedirect: true`를 사용할 때 일부 결제 수단은 인증을 위해 사용자를 페이지에서 리디렉션해야 합니다:

- **3D Secure (3DS)** - 카드 인증
- **Google Pay** - 일부 플로우에서의 지갑 인증
- **UPI** - 인도 결제 수단 리디렉션
...
인증이 완료되면(성공 또는 실패) 사용자가 귀하의 페이지로 돌아옵니다. 단순히 사용자가 돌아왔다는 이유만으로 성공을 가정하지 마세요. 대신 다음을 수행하세요:

1. 사용자가 리디렉션에서 돌아오는 중인지 확인하세요(예: `sessionStorage`를 통해)
2. 확인된 결제 상태를 위해 서버를 폴링하세요
3. 폴링 중에 "결제 확인 중..." 상태를 표시하세요
4. 서버에서 확인된 상태에 따라 성공/실패 UI를 표시하세요

<Tip>
리디렉션 후에는 항상 서버 측에서 결제 상태를 검증하세요. 사용자가 페이지로 돌아왔다고 해서 결제가 성공했다는 의미는 아닙니다.
</Tip>
...
- `elementId`가 실제 DOM에 존재하는 `div`의 `id`와 일치하는지 확인하세요.
...
Apple Pay, Google Pay 및 기타 디지털 지갑 설정에 대한 자세한 내용은 <a href="/features/payment-methods/digital-wallets">Digital Wallets</a> 페이지를 참조하세요.

### Apple Pay 빠른 설정

<Steps>
{/* LOCKED_PATTERN_052fb22f687ef020f4d37c90a8329c23 */}
[Apple Pay 도메인 연동 파일](http://checkout.dodopayments.com/.well-known/apple-developer-merchantid-domain-association)를 다운로드하세요.
</Step>

{/* LOCKED_PATTERN_3a44bb73d2b6657e3ef8dfb6df5437c3 */}
운영 도메인 URL을 포함하여 **support@dodopayments.com**으로 이메일을 보내고 Apple Pay 활성화를 요청하세요.
</Step>

{/* LOCKED_PATTERN_f4cb4ca481d047ba1b29ad7a11be722d */}
확인 후 Apple Pay가 결제에 표시되는지 확인하고 전체 흐름을 테스트하세요.
</Step>
</Steps>

<Warning>
Apple Pay는 운영 환경에 표시되기 전에 도메인 인증이 필요합니다. Apple Pay 제공을 계획 중이라면 라이브 전환 전에 지원팀에 문의하세요.
</Warning>
...
사용 사례에 맞는 결제 유형을 선택하세요:

| 기능 | 인라인 결제 | 오버레이 결제 |
|---------|-----------------|------------------|
| 통합 깊이 | 페이지에 완전히 내장됨 | 페이지 위의 모달 |
| 레이아웃 제어 | 완전한 제어 | 제한적 |
| 브랜딩 | 자연스러운 통합 | 페이지와 분리됨 |
| 구현 노력 | 더 높음 | 더 낮음 |
| 적합한 용도 | 맞춤형 결제 페이지, 높은 전환 흐름 | 신속한 통합, 기존 페이지 |

<Tip>
체크아웃 경험을 최대한 제어하고 브랜드 일체감을 유지하려면 **인라인 결제**를 사용하세요. 기존 페이지에 최소한의 변경으로 빠르게 통합하려면 **오버레이 결제**를 사용하세요.
</Tip>
