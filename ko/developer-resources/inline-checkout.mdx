...
| 필드 | 설명 |
|-------|-------------|
| `subTotal` | 할인 또는 세금이 적용되기 전 세션의 모든 항목 금액 합계입니다. |
| `discount` | 적용된 모든 할인 금액의 합계입니다. |
| `tax` | 계산된 세금 금액입니다. `inline` 모드에서는 사용자가 주소 필드와 상호작용할 때 동적으로 업데이트됩니다. |
| `total` | 세션의 기본 통화로 계산된 `subTotal - discount + tax`의 수학적 결과입니다. |
| `currency` | 표준 소계, 할인 및 세금 값에 사용되는 ISO 통화 코드(예: `"USD"`)입니다. |
| `finalTotal` | 고객에게 실제로 청구되는 금액입니다. 여기에는 기본 가격 내역에 포함되지 않는 추가 외환 조정 또는 현지 결제 수단 수수료가 포함될 수 있습니다. |
| `finalTotalCurrency` | 고객이 실제로 결제하는 통화입니다. [구매력 평가](/features/purchasing-power-parity) 또는 현지 통화 변환이 활성화된 경우 `currency`와 다를 수 있습니다. |
...
<Info>
**inline (embedded) checkout**에만 도메인 확인이 필요합니다. hosted checkout에는 필요하지 않습니다.
</Info>

<Warning>
overlay checkout에서는 Apple Pay를 사용할 수 없습니다.
</Warning>

Apple Pay는 대시보드에서 도메인별로 확인됩니다.

<Steps>
<Step title="Open Wallet domains">
**Settings → Payment Methods**로 이동한 다음, **Apple Pay** 행에서 **Manage domains**를 클릭합니다.

<Frame caption="Open Wallet domains from the Apple Pay row">
  <img src="/images/apple-pay-domains/manage-domains.png" alt="Payment Methods 설정의 Apple Pay 행에 있는 Manage domains 버튼" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
<Frame caption="Download the Apple Pay domain association file">
  <img src="/images/apple-pay-domains/wallet-domains-panel.png" alt="Download file 버튼이 있는 Wallet domains 패널" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Register your domain">
**Register domain**을 클릭하고 inline checkout을 삽입할 도메인(예: `shop.example.com`)을 입력한 다음 **Continue**를 클릭합니다.

<Frame caption="Register the domain where you embed inline checkout">
  <img src="/images/apple-pay-domains/register-domain.png" alt="도메인이 입력된 Register a domain 양식" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
HTTPS를 통해 제공되어야 하며, 리디렉션 없이 연결할 수 있어야 하고, `Content-Type: application/octet-stream` 또는 `text/plain`와 함께 제공되어야 합니다.
</Step>

<Step title="Verify the domain">
**Verify domain**을 클릭합니다. Dodo Payments가 파일이 정상적으로 제공되는지 확인하고 Apple에 도메인을 제출합니다.

<Frame caption="Verify the hosted association file">
  <img src="/images/apple-pay-domains/verify-domain.png" alt="association 파일 호스트 경로와 Verify domain 버튼이 표시된 도메인 확인 화면" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Confirm it's active">
상태가 **Active**로 표시되면 해당 도메인에서 Apple Pay가 활성화된 것입니다. **Enabled** 토글을 사용하여 도메인별로 켜거나 끌 수 있습니다.

<Frame caption="Verified domains show an Active status">
  <img src="/images/apple-pay-domains/domains-active.png" alt="Active Apple Pay 상태와 Enabled 토글이 표시된 Wallet domains 목록" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
## Inline Checkout와 Overlay Checkout 비교

사용 사례에 맞는 checkout 유형을 선택하세요:

| 기능 | Inline Checkout | Overlay Checkout |
|---------|-----------------|------------------|
| 통합 수준 | 페이지에 완전히 삽입됨 | 페이지 위에 표시되는 모달 |
| 레이아웃 제어 | 전체 제어 가능 | 제한적 |
| 브랜딩 | 페이지와 자연스럽게 통합 | 페이지와 별도로 표시 |
| 구현 작업량 | 높음 | 낮음 |
| 적합한 용도 | 맞춤형 checkout 페이지, 높은 전환율의 플로우 | 빠른 통합, 기존 페이지 |

<Tip>
checkout 환경을 최대한 제어하고 브랜딩을 자연스럽게 통합하려면 **inline checkout**을 사용하세요. 기존 페이지를 최소한으로 변경하면서 더 빠르게 통합하려면 **overlay checkout**을 사용하세요.
</Tip>
...
<Card title="Overlay Checkout" icon="layer-group" href="/developer-resources/overlay-checkout">
    빠르게 모달 기반으로 통합하려면 overlay checkout을 사용하세요.
</Card>

<Card title="Checkout Sessions API" icon="code" href="/api-reference/checkout-sessions/create">
    checkout 환경을 구축할 checkout session을 생성하세요.
</Card>

<Card title="Webhooks" icon="webhook" href="/developer-resources/webhooks">
    webhook을 사용하여 서버 측에서 payment event를 처리하세요.
</Card>
