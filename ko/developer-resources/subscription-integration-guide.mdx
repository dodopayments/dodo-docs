...
후속 청구의 경우 [POST /subscriptions/{subscription_id}/charge](/api-reference/subscriptions/create-charge) 엔드포인트를 사용하고 해당 거래에 대해 고객에게 청구할 금액을 지정하십시오.
...
후속 요금을 청구하려면, [POST /subscriptions/{subscription_id}/charge](/api-reference/subscriptions/create-charge) 엔드포인트를 사용하고 해당 거래에 대해 고객에게 청구할 금액을 지정하세요.

<Note>
요청/응답 예제, 안전한 재시도 정책 및 웹훅 처리 등을 포함한 완전한 단계별 가이드는 <a href="/developer-resources/ondemand-subscriptions">온디맨드 구독 가이드</a>를 참조하세요.
</Note>
