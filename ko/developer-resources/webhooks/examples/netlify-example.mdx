...
1. [Neon](https://neon.com)에 가입하기
2. 새 프로젝트 만들기
3. SQL 편집기 열기
4. [`schema.sql`](#database-schema)의 내용을 복사하여 붙여넣기
5. 쿼리 실행하기
...
> **참고:** 배포 후 웹훅 URL을 받으면 `DODO_PAYMENTS_WEBHOOK_KEY`를 설정합니다.
...
1. [DodoPayments 대시보드](https://app.dodopayments.com) → 개발자 → 웹훅으로 이동
2. 새 웹훅 엔드포인트 만들기
3. 웹훅 URL을 엔드포인트로 구성하기
4. 다음 구독 이벤트를 활성화하기:
   - `subscription.active`
...
웹훅 핸들러:
1. **서명 검증** - HMAC-SHA256을 사용하여 요청이 DodoPayments에서 온 것인지 확인
2. **중복 확인** - 웹훅 ID를 사용하여 동일한 이벤트를 두 번 처리하지 않도록 방지
3. **이벤트 로깅** - 감사 추적을 위해 원시 웹훅을 `webhook_events` 테이블에 저장
4. **이벤트 처리** - Neon에서 고객 및 구독을 생성하거나 업데이트
5. **오류 처리** - 실패를 기록하고 이벤트를 재시도할 수 있도록 미처리로 표시
...
| 함수 없음 (404) | `netlify init` 및 `npm run deploy`를 다시 실행 |
| 환경 변수 사용 불가 | CLI 또는 대시보드에서 변수를 설정한 후 다시 배포 |
