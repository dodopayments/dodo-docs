...
2. 새 프로젝트를 생성하세요.
3. SQL 편집기를 엽니다.
4. [`schema.sql`](#database-schema)의 내용을 복사하여 붙여넣습니다.
5. 쿼리를 실행합니다.
6. Neon에서 연결 문자열을 가져옵니다 → 연결 세부정보
...
> **참고:** 배포 후 웹훅 URL을 받으면 `DODO_PAYMENTS_WEBHOOK_KEY`을 설정합니다.
...
1. [DodoPayments 대시보드](https://app.dodopayments.com) → 개발자 → 웹훅으로 이동합니다.
2. 새로운 웹훅 엔드포인트를 생성합니다.
...
✅ **멱등성** - 웹훅 ID로 중복 처리 방지  
✅ **이벤트 로깅** - `webhook_events` 테이블에 완전한 감사 흔적  
✅ **오류 처리** - 기록 및 재시도 가능

> **참고:** 이 구현은 최소 필드를 사용하여 세 가지 주요 구독 이벤트 (`subscription.active`, `subscription.cancelled`, `subscription.renewed`)를 처리하는 방법을 보여줍니다. 필요에 따라 추가 이벤트 유형 및 필드를 쉽게 확장할 수 있습니다.
...
웹훅 핸들러:
1. **서명을 검증합니다** - HMAC-SHA256을 사용하여 요청이 DodoPayments에서 온 것인지 확인합니다.
2. **중복을 확인합니다** - 웹훅 ID를 사용해 동일한 이벤트를 두 번 처리하지 않도록 합니다.
3. **이벤트를 기록합니다** - 감사 흔적을 위해 `webhook_events` 테이블에 원본 웹훅을 저장합니다.
4. **이벤트를 처리합니다** - Neon에서 고객 및 구독을 생성하거나 업데이트합니다.
5. **오류를 처리합니다** - 실패를 기록하고 이벤트를 재시도를 위해 처리되지 않음으로 표시합니다.
...
| 환경 변수 없음 | CLI 또는 대시보드에서 변수를 설정한 후 재배포하세요. |
