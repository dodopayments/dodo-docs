...
> **참고:** 배포 후 웹훅 URL을 가지고 `DODO_PAYMENTS_WEBHOOK_KEY`를 설정할 것입니다.
...
웹훅 핸들러:
1. **본문 파싱 비활성화** - 서명 검증을 위해 원시 본문에 접근하기 위해
2. **서명 검증** - HMAC-SHA256을 사용하여 요청이 DodoPayments에서 온 것인지 확인
3. **중복 확인** - 웹훅 ID를 사용하여 동일한 이벤트를 두 번 처리하지 않도록 방지
4. **이벤트 로깅** - 감사 추적을 위해 원시 웹훅을 `webhook_events` 테이블에 저장
5. **이벤트 처리** - Neon에서 고객 및 구독을 생성하거나 업데이트
6. **오류 처리** - 실패를 기록하고 이벤트를 재시도를 위해 처리되지 않은 것으로 표시
