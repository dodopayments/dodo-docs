...
1. [Neon](https://neon.com) 가입하기
2. 새 프로젝트 생성하기
3. SQL 편집기 열기
4. [`schema.sql`](#database-schema) 내용을 복사하여 붙여넣기
5. 쿼리 실행하기
6. Neon에서 연결 문자열 가져오기 → 연결 세부정보
...
> **참고:** 배포 후 귀하의 웹훅 URL이 설정된 후 `DODO_PAYMENTS_WEBHOOK_KEY`를 설정하겠습니다.
...
1. [DodoPayments 대시보드](https://app.dodopayments.com) → 개발자 → 웹훅
2. 새 웹훅 엔드포인트 생성하기
3. 웹훅 URL을 엔드포인트로 설정하기
4. 다음 구독 이벤트를 활성화하기:
   - `subscription.active`
...
✅ **항상 같은 패턴** - 웹훅 ID로 중복 처리 방지  
✅ **이벤트 로깅** - `webhook_events` 테이블에 완전한 감사 추적  
✅ **오류 처리** - 기록되고 재시도 가능

> **참고:** 이 구현은 최소한의 필드로 세 가지 핵심 구독 이벤트 (`subscription.active`, `subscription.cancelled`, `subscription.renewed`)를 처리하는 방법을 보여줍니다. 필요에 따라 추가 이벤트 유형과 필드를 지원하도록 쉽게 확장할 수 있습니다.
...
2. **서명 검증** - HMAC-SHA256을 사용하여 DodoPayments에서 요청이 발송되었는지 확인
3. **중복 검사** - 웹훅 ID를 사용하여 동일 이벤트의 처리 방지
4. **이벤트 로깅** - 감사 추적을 위해 원시 웹훅을 `webhook_events` 테이블에 저장
5. **이벤트 처리** - Neon에서 고객 및 구독 생성 또는 업데이트
6. **오류 처리** - 실패를 기록하고 이벤트를 재처리 대기 상태로 표시
