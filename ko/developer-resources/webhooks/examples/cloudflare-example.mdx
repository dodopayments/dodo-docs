...
`wrangler.toml`를 편집하고 작업자 이름을 설정하세요:
...
3. **이벤트 기록** - 감사 추적을 위해 원시 웹훅을 `webhook_events` 테이블에 저장
4. **이벤트 처리** - 고객 및 구독을 생성하거나 업데이트
5. **오류 처리** - 실패를 기록하고 이벤트를 재시도를 위해 처리되지 않은 것으로 표시
...
| 데이터베이스 연결 오류 | Neon 연결 문자열 확인 |
| 작업자 배포 실패 | `npm install` 실행, `wrangler.toml` 구문 확인 |
