...
2. 새 프로젝트를 생성합니다.
3. SQL 편집기를 엽니다.
4. [`schema.sql`](#database-schema)의 내용을 복사하고 붙여넣습니다.
5. 쿼리를 실행합니다.
...
> **참고:** 웹후크 URL을 얻은 후 배포 후 `DODO_PAYMENTS_WEBHOOK_KEY`를 설정합니다.

### 5. wrangler.toml 업데이트

`wrangler.toml`를 편집하고 작업자 이름을 설정합니다:
...
✅ **서명 검증** - dodopayments 라이브러리를 사용함  
✅ **Idempotency** - 웹후크 ID로 중복 처리를 방지함  
✅ **이벤트 로깅** - `webhook_events` 테이블에 완전한 감사 로그  
✅ **오류 처리** - 기록되고 재시도 가능

> **참고:** 이 구현은 최소한의 필드로 세 가지 핵심 구독 이벤트 (`subscription.active`, `subscription.cancelled`, `subscription.renewed`)를 처리하는 방법을 보여줍니다. 요구 사항에 따라 쉽게 추가 이벤트 유형과 필드를 지원하도록 확장할 수 있습니다.
...
웹후크 처리기:
1. **서명을 검증합니다** - 요청이 DodoPayments에서 오는지 HMAC-SHA256을 사용하여 확인함
2. **중복을 확인합니다** - 웹후크 ID를 사용하여 동일한 이벤트를 두 번 처리하지 않도록 방지함
3. **이벤트를 기록합니다** - 감사를 위해 `webhook_events` 테이블에 원시 웹후크를 저장함
4. **이벤트를 처리합니다** - 고객 및 구독을 생성하거나 업데이트함
5. **오류를 처리합니다** - 실패를 기록하고 이벤트를 처리되지 않은 상태로 표시하여 재처리를 위한 대기 상태로 둡니다.
...
| 데이터베이스 연결 오류 | Neon 연결 문자열을 확인 |
| 작업자가 배포되지 않음 | `npm install`를 실행하고 `wrangler.toml` 구문을 확인 |
