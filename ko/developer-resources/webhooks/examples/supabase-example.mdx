...
<Tip>
  <strong>Supabase를 위한 완전한 스타터 키트를 확인하세요</strong><br />
...
1. [Supabase 대시보드](https://supabase.com/dashboard)에 들어갑니다.
2. SQL 편집기를 엽니다.
3. 새 쿼리를 생성합니다.
...
Supabase는 런타임 중에 자동으로 `SUPABASE_URL` 및 `SUPABASE_SERVICE_ROLE_KEY`를 제공합니다.

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **주의:** 배포 후 웹후크 URL을 가지고 `DODO_PAYMENTS_WEBHOOK_KEY`을 설정합니다.

### 5. 배포

함수는 `functions/webhook/index.ts`에서 이미 설정되어 있습니다 - 단순히 배포하세요:
...
1. [DodoPayments 대시보드](https://app.dodopayments.com)로 이동 → 개발자 → 웹후크
2. 새 웹후크 엔드포인트를 생성합니다.
3. 해당 엔드포인트로 웹후크 URL을 구성합니다.
4. 다음 구독 이벤트를 활성화합니다:
   - `subscription.active`
...
✅ **Idempotency** - 웹후크 ID로 중복 처리를 방지  
✅ **이벤트 로깅** - `webhook_events` 테이블에서 완전한 감사 추적  
✅ **오류 처리** - 로깅 및 재시도 가능

> **주의:** 이 구현은 최소한의 필드로 세 가지 핵심 구독 이벤트 (`subscription.active`, `subscription.cancelled`, `subscription.renewed`)를 처리하는 방법을 보여줍니다. 요구 사항에 따라 추가 이벤트 유형과 필드를 지원하도록 쉽게 확장할 수 있습니다.
...
2. **Idempotency 검증** - 중복 처리를 방지하기 위해 웹후크 ID를 조회
3. **이벤트 로깅** - 원시 웹후크 데이터를 `webhook_events` 테이블에 저장
4. **업데이트 처리** - Supabase 클라이언트를 통해 고객 및 구독 생성 또는 업데이트
5. **오류 처리** - 실패를 로깅하고 이벤트를 재시도할 수 있도록 표시
...
<Note>
`--no-verify-jwt` 플래그는 웹후크가 JWT 토큰을 포함하지 않기 때문에 필요합니다. 보안은 웹후크 서명 검증에 의해 제공됩니다.
</Note>
...
| 문제 | 해결책 |
|-------|----------|
| 검증 실패 | DodoPayments 대시보드에서 웹후크 키가 올바른지 확인 |
| 데이터베이스 권한 오류 | 서비스 역할 키를 사용하고 있는지 확인 |
| JWT 검증 오류 | `--no-verify-jwt` 플래그로 배포 |
| 함수가 없음 | 프로젝트 참조가 올바르고 함수가 배포되었는지 확인 |
