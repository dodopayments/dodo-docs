...
Supabase는 런타임에 `SUPABASE_URL` 및 `SUPABASE_SERVICE_ROLE_KEY`을 자동으로 제공합니다.
...
함수는 `functions/webhook/index.ts`에 이미 설정되어 있습니다 - 그냥 배포하세요:
...
2. **Idempotency 확인** - 중복 처리를 방지하기 위해 웹훅 ID를 조회
3. **이벤트 로깅** - 원시 웹훅 데이터를 `webhook_events` 테이블에 저장
4. **업데이트 처리** - Supabase 클라이언트를 통해 고객 및 구독을 생성 또는 업데이트
...
<Note>
`--no-verify-jwt` 플래그는 웹훅에 JWT 토큰이 포함되지 않기 때문에 필요합니다. 보안은 웹훅 서명 검증을 통해 제공됩니다.
</Note>
...
| 함수 없음 | 프로젝트 참조가 올바르고 함수가 배포되었는지 확인 |
