...
즉시 청구를 처리하는 방법: `prorated_immediately`, `full_immediately`, 또는 `difference_immediately`.
</ParamField>
...
- `subscription.plan_changed`: 구독 요금제 변경됨 (업그레이드/다운그레이드/애드온 업데이트)
- `subscription.on_hold`: 요금제 변경 요금 청구 실패, 구독 일시 중지됨
...
<Warning>
즉시 청구가 실패하면, 구독은 `subscription.on_hold` 상태로 이동할 수 있으며, 결제가 성공할 때까지 대기합니다.
</Warning>
...
#### `prorated_immediately`
- 현재 주기의 부분 차이에 대한 요금 청구
- 시험 중인 경우, 즉시 요금 청구하고 지금 새로운 요금제로 전환
- 다운그레이드: 향후 갱신에 적용될 비례 신용을 생성할 수 있음

#### `full_immediately`
- 새로운 요금제의 전체 금액을 즉시 청구
- 이전 요금제의 남은 시간을 무시
...
#### `difference_immediately`
- 업그레이드: 이전 요금제와 새로운 요금제 간의 가격 차이를 즉시 청구
- 다운그레이드: 남은 가치를 구독에 내부 신용으로 추가하고 갱신 시 자동 적용
...
<Tip>
공정한 시간 회계를 위해 `prorated_immediately`를 선택하고, 청구를 재시작하려면 `full_immediately`를 선택하며, 간단한 업그레이드와 다운그레이드 시 자동 신용을 위해 `difference_immediately`를 사용하세요.
</Tip>
...
### 처리할 이벤트 유형
- `subscription.active`: 구독 활성화됨
- `subscription.plan_changed`: 구독 요금제 변경됨 (업그레이드/다운그레이드/애드온 변경)
- `subscription.on_hold`: 청구 실패, 구독 일시 중지됨
- `subscription.renewed`: 갱신 성공
- `payment.succeeded`: 요금제 변경 또는 갱신에 대한 결제 성공
- `payment.failed`: 결제 실패
...
**일반적인 원인**:
- 비례 요금제 기대치: 다운그레이드는 `difference_immediately`와 함께 전체 요금제 가격 차이를 신용으로 부여하며, `prorated_immediately`는 주기 내 남은 시간을 기준으로 비례 신용을 생성합니다.
- 신용 잔액은 구독별로 특정되며 구독 간에 이전되지 않습니다.
- 고객 대시보드에서 신용 잔액이 보이지 않음

**해결책**:
- 자동 신용을 원할 때 다운그레이드에 `difference_immediately`를 사용하세요.
- 고객에게 신용이 동일한 구독의 향후 갱신에 적용된다고 설명하세요.
- 신용 잔액을 표시하기 위해 고객 포털을 구현하세요.
- 적용된 신용을 확인하기 위해 다음 청구서 미리보기를 확인하세요.
</Accordion>
...
**해결책**:
- `payment.failed` 웹훅 이벤트를 적절히 처리하세요.
- 고객에게 결제 방법을 업데이트하라고 알리세요.
- 일시적인 실패에 대한 재시도 로직을 구현하세요.
- 실패한 즉시 청구로 요금제 변경을 허용하는 것을 고려하세요.
</Accordion>

<Accordion title="요금제 변경 후 구독 보류">
**증상**: 요금제 변경 요금 청구 실패 및 구독이 `on_hold` 상태로 이동

**무슨 일이 발생하나요**:
요금제 변경 요금 청구가 실패하면, 구독은 자동으로 `on_hold` 상태로 배치됩니다. 결제 방법이 업데이트될 때까지 구독은 자동으로 갱신되지 않습니다.

실패한 플랜 변경 후 `on_hold` 상태에서 구독을 재활성화하려면:

실패한 요금제 변경 후 `on_hold` 상태에서 구독을 재활성화하려면:

1. **결제 방법 업데이트**: Update Payment Method API를 사용하여 결제 방법을 업데이트하세요.
2. **자동 청구 생성**: API가 남은 요금에 대한 청구를 자동으로 생성합니다.
3. **청구서 생성**: 청구에 대한 청구서가 생성됩니다.
4. **결제 처리**: 새로운 결제 방법을 사용하여 결제가 처리됩니다.
...
**모니터링할 웹훅 이벤트**:
- `subscription.on_hold`: 구독이 보류됨 (요금제 변경 요금 청구 실패 시 수신됨)
- `payment.succeeded`: 남은 요금에 대한 결제 성공 (결제 방법 업데이트 후)
- `subscription.active`: 성공적인 결제 후 구독 재활성화
...
<Step title="다양한 비례 요금제 테스트">
- 다양한 청구 주기 위치에서 `prorated_immediately`를 테스트하세요.
- 업그레이드 및 다운그레이드에 대해 `difference_immediately`를 테스트하세요.
- 청구 주기를 재설정하기 위해 `full_immediately`를 테스트하세요.
- 신용 계산이 올바른지 확인하세요.
</Step>
...
<Accordion title="401 Unauthorized">
유효하지 않거나 누락된 API 키입니다. `DODO_PAYMENTS_API_KEY`가 올바르고 적절한 권한이 있는지 확인하세요.
</Accordion>
...
- <a href="/features/customer-credit">Customer Credits</a>를 탐색하세요.
