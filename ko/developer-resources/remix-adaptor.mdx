...
청구 금액을 주요 통화 단위로 고정합니다(예: $12.50의 경우 <code>12.5</code>). Pay What You Want 제품에만 해당하며, 제품의 최소 가격보다 낮으면 무시됩니다.
</ParamField>
...
<Warning>
Dynamic Checkout은 더 이상 사용되지 않는 `POST /payments` 및 `POST /subscriptions` 엔드포인트를 프록시합니다. 기존 통합은 계속 작동하지만, 새로운 통합에는 아래의 **Checkout Sessions**를 사용해야 합니다.
</Warning>
...
Checkout sessions는 일회성 구매와 구독 모두에 대해 전체 결제 흐름을 처리하고 완전한 사용자 지정 제어 기능을 제공하는 더욱 안전한 호스팅 checkout 환경을 제공합니다.

자세한 내용과 지원되는 필드의 전체 목록은 [Checkout Sessions 통합 가이드](https://docs.dodopayments.com/developer-resources/checkout-session)를 참조하세요.
...
## Customer Portal 라우트 핸들러

Customer Portal 라우트 핸들러를 사용하면 Dodo Payments 고객 포털을 Remix 애플리케이션에 원활하게 통합할 수 있습니다.

### 쿼리 매개변수
<ParamField query="customer_id" type="string" required>
포털 세션의 고객 ID입니다(예: <code>?customer_id=cus_123</code>).
</ParamField>
...
- **서명 검증:** <code>webhookKey</code>를 사용하여 webhook 서명을 검증합니다. 검증에 실패하면 401을 반환합니다.
- **페이로드 검증:** Zod로 검증합니다. 유효하지 않은 페이로드에는 400을 반환합니다.
- **오류 처리:**
  - 401: 유효하지 않은 서명
  - 400: 유효하지 않은 페이로드
  - 500: 검증 중 내부 오류
