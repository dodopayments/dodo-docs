...
{/* LOCKED_PATTERN_848a353666469e7a047ef05af5a45770 */}
    Dodo Payments 체크아웃을 Remix 앱에 통합합니다.
</Card>
...
<Warning>
버전 관리에 <code>.env</code> 파일이나 비밀 정보를 절대 커밋하지 마세요.
</Warning>
</Step>
</Steps>
...
{/* LOCKED_PATTERN_28fd81a4703a84d931eeff69f1520d54 */}
  <Info>
  이 핸들러를 사용하여 Dodo Payments 체크아웃을 Remix 앱에 통합합니다. 정적(GET), 동적(POST), 세션(POST) 결제 흐름을 모두 지원합니다.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_085b207030169d6eead800f9835ef4fb */}
  <Info>
  이 핸들러를 사용하면 고객이 Dodo Payments 고객 포털을 통해 구독 및 정보를 관리할 수 있습니다.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_0026b036379d81a9d03c36b3b6e83ddd */}
  <Info>
  이 핸들러를 사용하여 Remix 앱에서 Dodo Payments 웹훅 이벤트를 안전하게 수신하고 처리할 수 있습니다.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments는 웹사이트에 결제를 통합하기 위해 세 가지 유형의 결제 흐름을 지원하며, 이 어댑터는 모든 유형을 지원합니다.
</Info>
...
<Warning>
productId가 누락되면 핸들러는 400 응답을 반환합니다. 잘못된 쿼리 매개변수 또한 400 응답을 발생시킵니다.
</Warning>
...
체크아웃 세션은 일회성 구매와 구독 모두에 대해 전체 결제 흐름을 처리하는 보다 안전한 호스팅 체크아웃 경험을 제공하며 완전한 커스터마이제이션 제어를 제공합니다.
...
<ParamField query="send_email" type="boolean">
true로 설정하면 고객에게 포털 링크를 이메일로 전송합니다.
</ParamField>
