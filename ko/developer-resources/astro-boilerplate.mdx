...
<Info>
이 보일러플레이트는 Astro 5와 TypeScript, Tailwind CSS 4, 그리고 `@dodopayments/astro` 어댑터를 사용합니다.
</Info>
...
- **모던 UI** - Tailwind CSS로 구성된 깔끔하고 어두운 테마의 가격 페이지
- **Webhook 핸들러** - 결제 이벤트를 위한 즉시 사용 가능한 웹훅 엔드포인트
- **고객 포털** - 클릭 한 번으로 구독 관리
- **TypeScript** - 최소한의 집중된 타입으로 완전히 타입이 지정됨
- **사전 입력된 체크아웃** - 고객 데이터를 전달하여 UX를 향상시키는 방법을 보여줌
...
<Warning>
결코 `.env` 파일을 버전 관리에 커밋하지 마세요. 이미 `.gitignore`에 포함되어 있습니다.
</Warning>
</Step>
...
`src/components/Header.astro`에서 하드코딩된 고객 ID를 인증 시스템 또는 데이터베이스의 실제 고객 ID로 교체하세요:
...
이 보일러플레이트는 `src/pages/api/webhook.ts`에서 웹훅 이벤트를 처리하는 방법을 시연합니다:
...
생산 환경에서 웹훅 서명 키가 배포된 도메인에 맞도록 `DODO_PAYMENTS_WEBHOOK_KEY` 환경 변수를 업데이트하는 것도 잊지 마세요.
...
<Accordion title="체크아웃 리다이렉트 실패">
**일반적인 원인:**
- 유효하지 않은 제품 ID - Dodo 대시보드에서 존재 여부 확인
- `.env`에서 잘못된 API 키 또는 환경 설정
- 오류 확인을 위해 브라우저 콘솔 및 터미널 확인
</Accordion>
...
`src/components/Header.astro`에서 하드코딩된 `CUSTOMER_ID`를 Dodo 대시보드에서의 실제 고객 ID로 교체하세요.
