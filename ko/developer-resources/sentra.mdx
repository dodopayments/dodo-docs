...
<Tab title="VS Code">
<Steps>
<Step title="확장 프로그램 마켓플레이스 열기">
1. VS Code를 엽니다.
2. 사이드바에서 확장 프로그램 아이콘을 클릭합니다 (macOS에서는 `Cmd+Shift+X`를 누르거나 Windows/Linux에서는 `Ctrl+Shift+X`를 누릅니다).
...
1. IDE (VS Code, Cursor 또는 Windsurf)를 엽니다.
2. 명령 팔레트를 엽니다 (macOS에서는 `Cmd+Shift+P` 또는 Windows/Linux에서는 `Ctrl+Shift+P`를 누릅니다).
3. "Sentra: Configure"를 입력하고 선택합니다.
4. 안내에 따라 Dodo Payments API 키를 입력합니다.
...
2. **설치 확인**: 확장 프로그램이 설치되었는지 확인하려면 확장 프로그램으로 가서 "Sentra"를 검색합니다.
