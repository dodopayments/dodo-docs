...
<Info>
이 보일러플레이트는 FastAPI를 사용하여 async/await 패턴을 적용하고, 유효성 검증을 위해 Pydantic을 사용하며, 원활한 API 통합을 위해 `dodopayments` Python SDK를 사용합니다.
</Info>
...
**일반적인 원인:**
- 잘못된 제품 ID - Dodo 대시보드에서 존재 여부를 확인하세요.
- `.env`의 API 키 또는 환경 설정이 잘못되었습니다.
- 상세한 오류 메시지를 위해 FastAPI 로그를 확인하세요.
</Accordion>
...
- `DODO_PAYMENTS_WEBHOOK_KEY`이 `.env`에서 올바르게 설정되었는지 확인하세요.
- 서명 검증을 위해 원시 요청 본문을 사용하고 있는지 확인하세요.
