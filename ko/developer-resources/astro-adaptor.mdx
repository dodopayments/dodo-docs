---
title: Astro Adaptor
description: Astro Adaptor를 사용하여 Astro App Router 프로젝트에 <CardGroup cols={2}>을(를) 통합하는 방법을 알아보세요.
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Astro 앱에 Dodo Payments checkout을 통합합니다.
</Card>
...
<Info>
모든 예시는 Astro App Router를 사용한다고 가정합니다.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  이 handler를 사용하여 Astro 앱에 Dodo Payments checkout을 통합합니다. 정적(GET), 동적(POST) 및 세션(POST) 결제 흐름을 지원합니다.
  </Info>
  <CodeGroup>
...
<Tab title="Customer Portal Handler">
  <Info>
  이 handler를 사용하여 고객이 Dodo Payments customer portal을 통해 구독 및 세부 정보를 관리할 수 있도록 합니다.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  이 handler를 사용하여 Astro 앱에서 Dodo Payments webhook 이벤트를 안전하게 수신하고 처리합니다.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments은(는) 웹사이트에 결제를 통합할 수 있도록 세 가지 결제 흐름을 지원하며, 이 adaptor는 모든 유형의 결제 흐름을 지원합니다.
</Info>

- **정적 결제 링크:** 빠르고 코드가 필요 없는 결제 수집을 위해 즉시 공유할 수 있는 URL입니다.
- **동적 결제 링크:** API 또는 SDK를 사용하여 맞춤 세부 정보가 포함된 결제 링크를 프로그래밍 방식으로 생성합니다.
- **Checkout 세션:** 사전 구성된 product cart 및 고객 세부 정보와 함께 안전하고 맞춤 설정 가능한 checkout 환경을 생성합니다.
...
<Warning>
<code>productId</code>가 누락되면 handler는 400 response를 반환합니다. 잘못된 query parameter도 400 response를 반환합니다.
</Warning>
...
Checkout sessions는 일회성 구매와 구독 모두에 대해 전체 결제 흐름을 처리하며, 완전한 customization control을 제공하는 더욱 안전한 hosted checkout 환경을 제공합니다.
...
Customer Portal Route Handler를 사용하면 Astro 애플리케이션에 Dodo Payments customer portal을 원활하게 통합할 수 있습니다.
...
<code>true</code>로 설정하면 고객에게 portal link가 포함된 이메일을 보냅니다.
</ParamField>
...
- **Payload Validation:** Zod로 validation합니다. 잘못된 payload에는 400을 반환합니다.
- **Error Handling:**
  - 401: 잘못된 signature
  - 400: 잘못된 payload
