---
title: Astro Adaptor
description: Astro App Router 프로젝트에 Dodo Payments를 통합하는 방법을 배우세요. 체크아웃, 고객 포털,
  웹훅 및 보안 환경 설정을 다룹니다.
icon: code-merge
---
...
{/* LOCKED_PATTERN_848a353666469e7a047ef05af5a45770 */}
    Astro 앱에 Dodo Payments 결제 흐름을 통합하세요.
</Card>
{/* LOCKED_PATTERN_df89dec21e12f4bf30b6acf75ba025e1 */}
    고객이 구독 및 정보를 관리할 수 있도록 허용하세요.
</Card>
...
<Warning>
버전 관리 시스템에 <code>.env</code> 파일이나 시크릿을 절대 커밋하지 마세요.
</Warning>
</Step>
</Steps>
...
<Info>
모든 예제는 Astro App Router를 사용한다고 가정합니다.
</Info>

<Tabs>
{/* LOCKED_PATTERN_28fd81a4703a84d931eeff69f1520d54 */}
  <Info>
  이 핸들러를 사용하여 Astro 앱에 Dodo Payments 결제를 통합하세요. 정적(GET), 동적(POST), 세션(POST) 결제 흐름을 모두 지원합니다.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_085b207030169d6eead800f9835ef4fb */}
  <Info>
  이 핸들러를 사용하여 고객이 Dodo Payments 고객 포털을 통해 구독과 정보를 관리하도록 하세요.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_0026b036379d81a9d03c36b3b6e83ddd */}
  <Info>
  이 핸들러를 사용하여 Astro 앱에서 Dodo Payments 웹훅 이벤트를 안전하게 수신하고 처리하세요.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments는 웹사이트에 결제를 통합하기 위한 결제 흐름 세 가지 유형을 지원하며, 이 어댑터는 모든 유형을 지원합니다.
</Info>
...
- 매개변수를 POST 요청의 JSON 본문으로 보냅니다.
- 일회성 결제와 반복 결제 모두 지원합니다.
...
체크아웃 세션은 더 안전한 호스팅 결제 환경을 제공하며 일회성 구매와 구독에 대한 전체 결제 흐름을 완전한 커스터마이징 제어와 함께 처리합니다.
...
고객 포털 라우트 핸들러를 사용하면 Dodo Payments 고객 포털을 Astro 애플리케이션에 원활하게 통합할 수 있습니다.
