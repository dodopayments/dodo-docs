...
<Card title="CLI" icon="terminal" href="/developer-resources/sdks/cli">
  터미널에서 API와 상호 작용하기 위한 명령줄 인터페이스
</Card>
</CardGroup>
...
모바일 애플리케이션을 위한 전용 React Native SDK를 제공합니다:

<Card title="React Native SDK" icon="mobile" href="/developer-resources/react-native-integration">
  iOS 및 Android 앱을 위한 네이티브 UI 구성 요소와 단순화된 결제 데이터 수집으로 안전한 결제 경험을 구축
</Card>
...
- 다양한 결제 방법 지원
- React Native 앱과 원활한 통합

<Info>
React Native SDK는 현재 Apple Pay, Google Pay, Cash App 및 UPI를 지원하지 않습니다. 이러한 결제 방법 지원은 향후 릴리스에 계획되어 있습니다.
</Info>

## 명령줄 인터페이스

터미널 기반 워크플로 및 자동화를 위해:

<Card title="CLI" icon="terminal" href="/developer-resources/sdks/cli">
  모든 78개의 API 엔드포인트에 대한 지원이 포함된 자동 생성 명령줄 인터페이스
</Card>

**기능들:**
- 직관적인 사용을 위한 리소스 기반 명령 구조
- 다양한 출력 형식 (JSON, YAML, 예쁘게, 대화형)
- bash, zsh, fish에 대한 셸 완성
- 스크립팅 및 CI/CD 자동화에 완벽
...
<Info>
우리는 Node.js SDK에서 새로운 TypeScript SDK로 마이그레이션했습니다. 구형 Node.js SDK를 사용 중인 경우, 통합을 업데이트하려면 [마이그레이션 가이드](https://github.com/dodopayments/dodopayments-typescript/blob/main/MIGRATION.md)를 참조하세요.
</Info>

## 프레임워크 어댑터

프레임워크 어댑터를 사용하여 10줄 미만의 코드로 통합하세요. 권장 프레임워크 중 하나를 선택하거나 지원되는 모든 옵션을 탐색하세요.
...
<Card title="Supabase" icon="database" href="developer-resources/supabase-boilerplate">
  Postgres 및 Auth를 포함한 오픈 소스 Firebase 대안
</Card>
...
- **Discord**: 실시간 도움을 위해 [커뮤니티 서버](https://discord.gg/bYqAp4ayYh)에 가입하세요.
- **Email**: support@dodopayments.com으로 문의하세요.
- **GitHub**: 해당 SDK 리포지토리에서 이슈를 생성하세요.
- **Documentation**: [API 레퍼런스](/api-reference/introduction)를 방문하세요.
...
각 SDK 페이지를 방문하여 GitHub 리포지토리와 기여 가이드를 액세스하세요.