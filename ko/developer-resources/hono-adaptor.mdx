...
{/* LOCKED_PATTERN_848a353666469e7a047ef05af5a45770 */}
    Dodo Payments 체크아웃을 Hono 앱에 통합하세요.
</Card>
{/* LOCKED_PATTERN_df89dec21e12f4bf30b6acf75ba025e1 */}
    고객이 구독 및 정보를 관리할 수 있도록 하세요.
</Card>
...
<Info>
모든 예시는 Hono App Router를 사용하는 것으로 가정합니다.
</Info>

<Tabs>
{/* LOCKED_PATTERN_28fd81a4703a84d931eeff69f1520d54 */}
  <Info>
  이 핸들러를 사용하여 Dodo Payments 체크아웃을 Hono 앱에 통합하세요. 정적(GET), 동적(POST), 세션(POST) 흐름을 지원합니다.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_085b207030169d6eead800f9835ef4fb */}
  <Info>
  이 핸들러를 사용하여 고객이 Dodo Payments 고객 포털을 통해 구독 및 정보를 관리할 수 있도록 하세요.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_0026b036379d81a9d03c36b3b6e83ddd */}
  <Info>
  이 핸들러를 사용하여 Hono 앱에서 Dodo Payments 웹훅 이벤트를 안전하게 수신하고 처리하세요.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments는 웹사이트에 결제를 통합하기 위한 세 가지 결제 흐름을 지원하며, 이 어댑터는 모든 유형의 결제 흐름을 지원합니다.
</Info>
...
- POST 요청에서 매개변수를 JSON 본문으로 전송하세요.
- 일회성 및 반복 결제를 모두 지원합니다.
- 지원되는 POST 본문 필드 전체 목록은 다음을 참고하세요:
  - [일회성 결제 상품을 위한 요청 본문](https://docs.dodopayments.com/api-reference/payments/post-payments)
  - [구독 상품을 위한 요청 본문](https://docs.dodopayments.com/api-reference/subscriptions/post-subscriptions)
...
체크아웃 세션은 보안이 강화된 호스팅 체크아웃 환경을 제공하며 일회성 구매 및 구독의 전체 결제 흐름을 처리하며 완벽한 커스터마이징 제어를 제공합니다.
...
<ParamField query="send_email" type="boolean">
<code>true</code>로 설정하면 포털 링크가 포함된 이메일을 고객에게 전송합니다.
</ParamField>
