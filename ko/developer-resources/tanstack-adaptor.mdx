...
<Warning>
Dynamic Checkout은 더 이상 사용되지 않는 `POST /payments` 및 `POST /subscriptions` endpoint를 프록시합니다. 기존 통합에서는 계속 작동하지만, 새 통합에서는 아래의 **Checkout Sessions**를 사용해야 합니다.
</Warning>
...
Checkout sessions는 일회성 구매와 구독 모두에 대해 전체 결제 흐름을 처리하고, 완전한 customization control을 제공하는 더욱 안전한 hosted checkout experience를 제공합니다.
...
Customer Portal Route Handler를 사용하면 Dodo Payments customer portal을 Tanstack application에 원활하게 통합할 수 있습니다.
...
true</code>로 설정하면 customer에게 portal link가 포함된 email을 보냅니다.
</ParamField>
...
- **Payload Validation:** Zod로 validation됩니다. 유효하지 않은 payload에는 400을 반환합니다.
- **Error Handling:**
