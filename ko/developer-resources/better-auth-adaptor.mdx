...
<AccordionGroup>
  <Accordion title="Plugin Options">
    ...
    - <b>createCustomerOnSignUp</b> (선택사항): 사용자 가입 시 고객 자동 생성
    - <b>use</b> (필수): 활성화할 플러그인 배열 (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (선택사항): BetterAuth `User`를 수신하고 생성 및 업데이트 시 DodoPayments 고객에게 첨부할 추가 필드를 반환하는 함수 (예: `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    ...
    <b>successUrl</b>: 결제 성공 후 리디렉션할 URL -{" "}
    <b>authenticatedUsersOnly</b>: 사용자 인증 필수 (기본값: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>유효하지 않은 API 키</b>: <code>.env</code>의 <code>DODO_PAYMENTS_API_KEY</code>를 다시 확인하세요. -{" "}
    <b>웹훅 서명 불일치</b>: 웹훅 시크릿이 Dodo Payments 대시보드에 설정된 것과 일치하는지 확인하세요. - <b>고객이 생성되지 않음</b>: <code>createCustomerOnSignUp</code>이 <code>true</code>로 설정되어 있는지 확인하세요.
  </Accordion>
  <Accordion title="Best Practices">
    - 모든 비밀과 키에 대해 환경 변수를 사용하세요. - 테스트 모드에서 테스트 후 라이브 모드로 전환하세요. - 웹훅 이벤트를 로그하여 디버깅 및 감사에 활용하세요.
  </Accordion>
</AccordionGroup>
