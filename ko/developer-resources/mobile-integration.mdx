...
<CardGroup cols={2}>
<Card title="빠른 시작" icon="rocket" href="#integration-workflow">
  4단계로 모바일 결제 통합을 시작하세요
</Card>

<Card title="플랫폼 예제" icon="code" href="#platform-specific-integration">
  Android, iOS, React Native 및 Flutter에 대한 완전한 코드 예제
</Card>
</CardGroup>
...
<Steps>
<Step title="백엔드: 체크아웃 세션 생성">
<Card title="체크아웃 세션 API 문서" icon="book" href="/developer-resources/checkout-session">
Node.js, Python 등을 사용하여 백엔드에서 체크아웃 세션을 생성하는 방법을 알아보세요. 전용 <strong>체크아웃 세션 API</strong> 문서에서 완전한 예제와 매개변수 참조를 확인하세요.
</Card>
<Note>
...
이 종속성을 당신의 `package.json`에 추가하세요:
...
`android/app/src/main/AndroidManifest.xml`에 추가하세요:
...
`ios/YourApp/Info.plist`에 추가하세요:
...
<Warning>
**지역 규정 준수**: 인앱 결제 SDK는 지역 제한이 있습니다. **iOS**에서는 네이티브 결제 SDK가 **EU**에서만 합법입니다( DMA 조건에 따라). **Android**에서는 사용자 선택 청구(UCB) 시장에서 합법입니다. 다른 지역에서는 위의 체크아웃 링크 접근 방식을 사용하여 App Store 및 Play Store 규정을 준수하세요.
</Warning>

<Card 
  title="React Native SDK 통합 가이드" 
  icon="mobile-screen" 
...
<Info>
인앱 결제 통합에 대한 지역 규정 준수 요구 사항을 이해하려면 [App Store 수수료 우회 가이드](/features/bypassing-app-store-fees)를 검토하세요.
</Info>
...
<Check>
질문이나 지원이 필요하면 <a href="mailto:support@dodopayments.com">support@dodopayments.com</a>에 문의하세요.
</Check>