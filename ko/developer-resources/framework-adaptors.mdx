---
sidebarTitle: 개요
title: 프레임워크 어댑터 개요
description: 최소한의 코드로 몇 분 안에 Dodo Payments를 통합할 수 있는 인기 웹 프레임워크용 사전 구축된 어댑터
icon: box
---

Dodo Payments는 결제 통합을 간소화하는 공식 프레임워크 어댑터를 제공합니다. 각 어댑터는 프레임워크의 관례와 원활하게 작동하도록 설계되어 있으며, 체크아웃, 고객 포털 및 웹훅 처리를 즉시 제공합니다.

<Tip>
프레임워크 어댑터를 사용하면 10줄 이하의 코드로 Dodo Payments를 통합할 수 있습니다. 이들은 인증, 요청 파싱 및 응답 형식을 자동으로 처리합니다.
</Tip>

## 사용 가능한 프레임워크 어댑터

귀하의 프레임워크에 맞는 어댑터를 선택하세요:

<CardGroup cols={2}>
<Card title="Next.js" icon="react" href="/developer-resources/nextjs-adaptor">
  체크아웃, 포털 및 웹훅을 위한 라우트 핸들러가 있는 앱 라우터 지원
</Card>

<Card title="Nuxt" icon="vuejs" href="/developer-resources/nuxt-adaptor">
  서버 라우트 통합이 있는 Vue 기반 풀스택 프레임워크
</Card>

<Card title="Express" icon="node-js" href="/developer-resources/express-adaptor">
  인기 있는 Node.js 프레임워크를 위한 미들웨어 기반 핸들러
</Card>

<Card title="Fastify" icon="bolt" href="/developer-resources/fastify-adaptor">
  플러그인 아키텍처를 갖춘 고성능 Node.js 프레임워크
</Card>

<Card title="Hono" icon="cloud" href="/developer-resources/hono-adaptor">
  엣지, Cloudflare Workers 등을 위한 초고속 웹 프레임워크
</Card>

<Card title="Astro" icon="star" href="/developer-resources/astro-adaptor">
  서버 엔드포인트 지원이 있는 콘텐츠 중심 프레임워크
</Card>

<Card title="SvelteKit" icon="code" href="/developer-resources/sveltekit-adaptor">
  서버 훅 통합이 있는 풀스택 Svelte 프레임워크
</Card>

<Card title="Remix" icon="react" href="/developer-resources/remix-adaptor">
  로더 및 액션 핸들러가 있는 풀스택 React 프레임워크
</Card>

<Card title="TanStack Start" icon="chart-line" href="/developer-resources/tanstack-adaptor">
  서버 기능이 있는 타입 안전 풀스택 React 프레임워크
</Card>

<Card title="Better Auth" icon="shield" href="/developer-resources/better-auth-adaptor">
  원활한 인증 + 결제를 위한 인증 프레임워크 플러그인
</Card>

<Card title="Convex" icon="database" href="/developer-resources/convex-component">
  실시간 결제 동기화를 위한 Backend-as-a-Service 컴포넌트
</Card>

<Card title="Bun" icon="bolt" href="/developer-resources/bun-adaptor">
  결제, 포털 및 웹훅을 위한 Native Bun.serve() 핸들러
</Card>
</CardGroup>

## 핵심 기능

모든 프레임워크 어댑터는 다음과 같은 기본 기능을 제공합니다:

| 기능 | 설명 |
|---------|-------------|
| **체크아웃 핸들러** | 정적, 동적 및 세션 기반 체크아웃 흐름 지원 |
| **고객 포털** | 구독 및 청구 관리용 미리 구축된 핸들러 |
| **웹훅 핸들러** | 유형화된 이벤트 핸들러와 함께 안전한 서명 검증 |
| **환경 구성** | 환경 변수를 통한 간단한 설정 |
| **유형 안전성** | 유형화된 페이로드와 함께 완전한 TypeScript 지원 |

## 빠른 시작

세 단계로 모든 프레임워크 어댑터를 시작하세요:

<Steps>
<Step title="어댑터 설치">
  패키지 관리자를 사용하여 프레임워크 전용 어댑터를 설치하세요:

  <Tabs>
  <Tab title="Next.js">

  ```bash
  npm install @dodopayments/nextjs
  ```

  </Tab>
  
  <Tab title="Nuxt">

  ```bash
  npm install @dodopayments/nuxt
  ```

  </Tab>
  
  <Tab title="Express">

  ```bash
  npm install @dodopayments/express
  ```

  </Tab>
  
  <Tab title="Hono">

  ```bash
  npm install @dodopayments/hono
  ```

  </Tab>

  <Tab title="Astro">

  ```bash
  npm install @dodopayments/astro
  ```

  </Tab>

  <Tab title="SvelteKit">

  ```bash
  npm install @dodopayments/sveltekit
  ```

  </Tab>
  </Tabs>
</Step>

<Step title="환경 변수 구성">
  Dodo Payments 자격 증명을 환경에 추가하세요:

  ```env
  DODO_PAYMENTS_API_KEY=your-api-key
  DODO_PAYMENTS_WEBHOOK_KEY=your-webhook-secret
  DODO_PAYMENTS_RETURN_URL=https://yourdomain.com/checkout/success
  DODO_PAYMENTS_ENVIRONMENT="test_mode" # or "live_mode"
  ```

  <Warning>
  `.env` 파일 또는 비밀 정보를 버전 관리에 절대 커밋하지 마세요.
  </Warning>
</Step>

<Step title="경로 핸들러 생성">
  체크아웃, 고객 포털 및 웹훅 경로를 설정하세요:

  <Tabs>
  <Tab title="Next.js">

  ```typescript
  // app/checkout/route.ts
  import { Checkout } from "@dodopayments/nextjs";

  export const GET = Checkout({
    bearerToken: process.env.DODO_PAYMENTS_API_KEY,
    returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
    environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
  });
  ```

  </Tab>
  
  <Tab title="Express">

  ```typescript
  import { checkoutHandler } from '@dodopayments/express';

  app.get('/api/checkout', checkoutHandler({
    bearerToken: process.env.DODO_PAYMENTS_API_KEY,
    returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
    environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
  }));
  ```

  </Tab>
  
  <Tab title="Hono">

  ```typescript
  import { Checkout } from "@dodopayments/hono";

  app.get('/checkout', Checkout({
    bearerToken: process.env.DODO_PAYMENTS_API_KEY,
    returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
    environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
  }));
  ```

  </Tab>
  </Tabs>

  <Check>
  이제 결제를 처리할 준비가 되었습니다! 각 어댑터 페이지를 방문하여 상세 가이드와 사용 가능한 모든 옵션을 확인하세요.
  </Check>
</Step>
</Steps>

## 체크아웃 흐름 유형

모든 어댑터는 세 가지 체크아웃 흐름 유형을 지원합니다:

<AccordionGroup>
<Accordion title="정적 체크아웃 (GET)">
간단하고 공유 가능한 결제 링크를 위한 정적 체크아웃을 사용하세요. 제품 ID를 쿼리 매개변수로 전달하세요:

```
/api/checkout?productId=pdt_xxx&quantity=1
```

선택적 고객 자동 채움 및 쿼리 매개변수를 통한 사용자 정의를 지원합니다.
</Accordion>

<Accordion title="동적 체크아웃 (POST)">
사용자 정의 세부정보로 결제를 프로그래밍 방식으로 생성하기 위해 동적 체크아웃을 사용하세요:

```json
{
  "product_id": "pdt_xxx",
  "customer": {
    "email": "customer@example.com",
    "name": "John Doe"
  },
  "quantity": 1
}
```

일회성 결제 및 구독 모두 지원합니다.
</Accordion>

<Accordion title="체크아웃 세션 (POST)">
장바구니 지원과 함께 가장 유연한 체크아웃 경험을 위해 체크아웃 세션을 사용하세요:

```json
{
  "product_cart": [
    { "product_id": "pdt_xxx", "quantity": 1 },
    { "product_id": "pdt_yyy", "quantity": 2 }
  ],
  "customer": {
    "email": "customer@example.com"
  }
}
```

자세한 내용은 [체크아웃 세션 안내서](/developer-resources/checkout-session)에서 확인하세요.
</Accordion>
</AccordionGroup>

## 웹훅 이벤트 처리

모든 어댑터는 세분화된 이벤트 콜백을 통한 유형 안전한 웹훅 처리를 제공합니다:

```typescript
Webhooks({
  webhookKey: process.env.DODO_PAYMENTS_WEBHOOK_KEY,
  onPayload: async (payload) => {
    // Handle any webhook event
  },
  onPaymentSucceeded: async (payload) => {
    // Handle successful payments
  },
  onSubscriptionActive: async (payload) => {
    // Handle new subscriptions
  },
  // ... 20+ event types supported
});
```

<Info>
모든 웹훅 핸들러는 자동으로 서명을 검증하고 Zod 스키마를 사용하여 페이로드를 검증합니다. 유효하지 않은 요청은 적절한 오류 코드와 함께 거부됩니다.
</Info>

## 적절한 어댑터 선택

| 프레임워크 | 최적용 | 런타임 |
|-----------|----------|---------|
| **Next.js** | 앱 라우터를 이용한 풀스택 React 앱 | Node.js, Edge |
| **Nuxt** | 풀스택 Vue.js 애플리케이션 | Node.js |
| **Express** | REST API 및 전통적인 Node.js 앱 | Node.js |
| **Fastify** | 고성능 API | Node.js |
| **Hono** | 엣지 배포, Cloudflare Workers | Edge, Node.js |
| **Astro** | 서버 엔드포인트가 있는 콘텐츠 사이트 | Node.js, Edge |
| **SvelteKit** | 풀스택 Svelte 애플리케이션 | Node.js |
| **Remix** | 중첩 라우팅을 활용한 풀스택 React | Node.js |
| **TanStack Start** | 유형 안전한 풀스택 React | Node.js |
| **Better Auth** | 이미 Better Auth를 사용하는 앱 | 다양한 |
| **Convex** | 백엔드에 Convex를 사용하는 앱 | Convex 런타임 |
| **Bun** | 네이티브 Bun 서버 애플리케이션 | Bun |

## 도움 요청

프레임워크 어댑터에 대한 도움이 필요하신가요?

- **Discord**: 실시간 도움을 받기 위해 [커뮤니티 서버](https://discord.gg/bYqAp4ayYh)에 참여하세요
- **Email**: support@dodopayments.com으로 연락주세요
- **GitHub**: 해당 어댑터 리포지토리에서 이슈를 열어주세요
- **Documentation**: [API 참조](/api-reference/introduction)를 방문하세요
