...
Dodo Payments는 결제 통합을 간소화하는 공식 프레임워크 어댑터를 제공합니다. 각 어댑터는 프레임워크의 규칙과 원활하게 작동하도록 설계되어 있으며, 체크아웃, 고객 포털 및 웹훅 처리를 기본적으로 제공합니다.
...
| **웹훅 핸들러** | 유형 안전 이벤트 핸들러와 함께하는 보안 서명 검증 |
| **환경 구성** | 환경 변수를 통한 간단한 설정 |
| **타입 안전성** | 유형화된 페이로드와 함께하는 전체 TypeScript 지원 |
...
선택적 고객 자동 채우기 및 쿼리 매개변수를 통한 사용자 정의를 지원합니다.
</Accordion>
...
<Info>
모든 웹훅 핸들러는 자동으로 서명을 검증하고 Zod 스키마를 사용하여 페이로드를 검증합니다. 유효하지 않은 요청은 적절한 오류 코드로 거부됩니다.
</Info>
