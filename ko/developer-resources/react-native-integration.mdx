...
<Step title="결제 제공자 설정">
    앱을 `DodoPaymentProvider`로 래핑하세요:
...
<Step title="결제 화면 구현">
    `useCheckout` 훅을 사용하여 결제 화면을 생성하세요. 전체 구현은 다음과 같습니다:
...
React Native 통합 체크아웃을 사용하여 색상, 글꼴 및 기타를 수정하여 앱의 디자인에 맞게 사용자 정의할 수 있습니다. `initPaymentSession()`를 호출할 때 appearance 매개변수를 사용하세요.

#### 색상 사용자 정의

각 색상 카테고리는 UI에서 하나 이상의 구성 요소의 색상을 결정합니다. 예를 들어, `primary`는 Pay 버튼의 색상을 정의합니다.

| 색상 카테고리 | 용도 |
|----------------|-------|
| `primary` | Pay 버튼 및 선택 항목의 기본 색상 |
| `background` | 결제 페이지의 배경 색상 |
| `componentBackground` | 입력 필드, 탭 및 기타 구성 요소의 배경 색상 |
| `componentBorder` | 입력 필드, 탭 및 기타 구성 요소의 외부 경계 색상 |
| `componentDivider` | 구성 요소의 내부 경계 색상(공유 경계) |
| `primaryText` | 헤더 텍스트 색상 |
| `secondaryText` | 입력 필드의 레이블 텍스트 색상 |
| `componentText` | 입력 텍스트 색상(예: 카드 번호, 우편번호) |
| `placeholderText` | 입력 필드의 자리 표시자 텍스트 색상 |
| `icon` | 아이콘 색상(예: 닫기 버튼) |
