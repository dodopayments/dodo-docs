...
**예제:** `user_tier = "premium"` AND `endpoint = "/api/v2/users"`의 API 호출 수 계산
</Tab>

<Tab title="OR 논리">
이벤트가 계산되기 위해서는 최소한 하나의 조건이 참이어야 합니다. 여러 기준 중 하나라도 충족하는 이벤트를 포함하고 싶을 때 사용하세요.

**예제:** `method = "POST"` OR `method = "PUT"` OR `method = "DELETE"`의 이벤트 수 계산
</Tab>
</Tabs>
...
- `not equals` - 제외 필터
- `greater than` - 수치 비교
- `greater than or equals` - 포함된 수치 비교
- `less than` - 수치 비교
- `less than or equals` - 포함된 수치 비교
- `contains` - 문자열 포함
...
**예제**: 단위당 `$0.50` 설정은 다음과 의미합니다:
- 소비된 1,000개 단위 = 1,000 × $0.50 = 500.00 청구됨
- 소비된 500개 단위 = 500 × $0.50 = 250.00 청구됨
- 소비된 100개 단위 = 100 × $0.50 = 50.00 청구됨
</ParamField>
...
필터 구성 (AND 논리를 사용하는 경우):
- 속성: `plan_type`, 비교기: `equals`, 값: `premium`
- 속성: `hour`, 비교기: `greater_than_or_equal`, 값: `9`
- 속성: `hour`, 비교기: `less_than`, 값: `17`
