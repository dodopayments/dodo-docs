...
<Tip>
이 가이드는 체크아웃 세션(`POST /checkouts`)을 통해 온디맨드 구독을 생성하며, 항상 호스팅된 `checkout_url`를 반환합니다. 고객을 그곳으로 리디렉션하여 승인 권한을 얻고, 이후 고객이 도착할 위치를 `return_url`로 설정하십시오.
</Tip>
...
테스트 API 키를 사용하여 구독을 생성한 후, 반환된 `checkout_url`를 열고 권한을 완료하세요.
</Step>
<Step title="Trigger a charge">
작은 `product_price` (예: `100`)으로 청구 엔드포인트를 호출하고 `payment.succeeded`를 수신하는지 확인하세요.
</Step>
<Step title="Go live">
