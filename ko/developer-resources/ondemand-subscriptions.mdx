...
<Tip>
고객이 호스티드 체크아웃을 통해 위임을 승인하도록 하려면, `payment_link: true`를 설정하고 `return_url`를 제공하세요.
</Tip>

## 온디맨드 작동 방식

1. `on_demand` 객체로 구독을 생성하여 결제 방법을 승인하고 선택적으로 초기 요금을 수집합니다.
2. 나중에 전용 청구 끝점을 사용하여 해당 구독에 대해 사용자 지정 금액으로 청구를 생성합니다.
3. 웹 후크 (예: `payment.succeeded`, `payment.failed`)를 수신하여 시스템을 업데이트합니다.
...
<Warning>
수요에 따라 가지 않는 구독을 청구하면 실패할 수 있습니다. 청구하기 전에 구독의 세부 정보에 `on_demand: true`이(가) 포함되어 있는지 확인하세요.
</Warning>
...
- **백오프 메커니즘**: 재시도 간 지수 백오프를 사용하세요.
- **재시도 제한**: 총 재시도를 제한하세요 (최대 3-4회 시도).
- **지능형 필터링**: 재시도 가능한 실패 (예: 네트워크/발급자 오류, 자금 부족)에서만 재시도하고, 하드 거부는 재시도하지 마세요.
- **카드 테스트 방지**: `DO_NOT_HONOR`, `STOLEN_CARD`, `LOST_CARD`, `PICKUP_CARD`, `FRAUDULENT`, `AUTHENTICATION_FAILURE`와 같은 실패를 재시도하지 마세요.
- **메타데이터 다양화 (선택 사항)**: 자체 재시도 시스템을 유지하는 경우, 메타데이터를 통해 재시도를 구별하세요 (예: `retry_attempt`).
...
- 기본 승인 타임스탬프에 따라 재시도를 연결하여 포트폴리오 전반의 "버스트" 동작을 방지하세요.
- 예: 고객이 오늘 오후 1:10에 체험판이나 위임을 시작하는 경우, 이후 날짜에 따라 추가 재시도를 오후 1:10에 예약하세요 (예: +3일 → 오후 1:10, +7일 → 오후 1:10).
- 또는 마지막 성공적인 결제 시간을 `T`에 저장하는 경우, 다음 시도를 `T + X days`에 예약하여 시간 일치를 유지하세요.
...
<Tip>
소프트/일시적인 문제에서만 재시도하세요 (예: `insufficient_funds`, `issuer_unavailable`, `processing_error`, 네트워크 타임아웃). 동일한 거부가 반복되면 추가 재시도를 중지하세요.
</Tip>

### 구현 지침 (코드 없음)

- 정확한 타임스탬프를 유지하는 스케줄러/큐를 사용하고; 다음 시도를 정확한 시간대 오프셋에서 계산하세요 (예: `T + 3 days` 같은 HH:MM에서).
- 마지막 성공적인 결제 타임스탬프 `T`를 유지하고 참조하여 다음 시도를 계산하세요; 여러 구독을 동시에 묶지 마세요.
- 항상 마지막 거부 원인을 평가하고, 위의 스킵 목록에서 하드 거부에 대한 재시도를 중지하세요.
- 우발적인 급증을 방지하기 위해 고객별 및 계정별로 동시 재시도를 제한하세요.
- 미리 소통하세요: 고객에게 이메일/SMS를 보내 다음 예정된 시도 전에 결제 방법을 업데이트하도록 하세요.
- 메타데이터는 관찰성을 위해서만 사용하고 (예: `retry_attempt`); 중요하지 않은 필드를 회전시켜 사기/위험 시스템을 "회피"하려고 하지 마세요.
...
<Tip>
수요 기반 흐름의 경우, 사용 기반 요금을 조정하기 위해 `payment.succeeded` 및 `payment.failed`에 집중하세요.
</Tip>
...
테스트 API 키를 사용하여 `payment_link: true`로 구독을 생성한 다음, 링크를 열고 위임을 완료하세요.
</Step>
<Step title="청구 트리거하기">
작은 `product_price` (예: `100`)로 청구 끝점을 호출하고 `payment.succeeded`을(를) 받는지 확인하세요.
</Step>
<Step title="실제 운영하기">
이벤트 및 내부 상태 업데이트를 검증한 후 실제 API 키로 전환하세요.
</Step>
</Steps>

## 문제 해결

- **422 잘못된 요청**: 생성 시 `on_demand.mandate_only`가 제공되었는지 그리고 청구에 `product_price`가 제공되었는지 확인하세요.
- **통화 오류**: `product_currency`를 재정의하는 경우, 계정과 고객에 대해 지원되는지 확인하세요.
- **웹 후크 없음**: 웹 후크 URL 및 서명 비밀 구성 확인.
