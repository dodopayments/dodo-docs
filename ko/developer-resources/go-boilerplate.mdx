...
<Info>
이 보일러플레이트는 Go 1.21+에서 클린 아키텍처 패턴(`cmd`, `internal`, `templates`), HTML 템플릿 및 `dodopayments-go` SDK를 사용하여 원활한 API 통합을 제공합니다.
</Info>

### 기능

- **빠른 설정** - 5분 이내에 시작
- **결제 통합** - `dodopayments-go` SDK를 사용하는 미리 설정된 체크아웃 흐름
- **현대적 UI** - 깔끔한 어두운 테마의 가격 페이지와 HTML 템플릿
- **Webhook 처리** - 결제 이벤트를 안전하게 확인하고 처리
- **고객 포털** - 자가 관리 구독 관리
- **Go 모범 사례** - `cmd`, `internal`, `templates`의 클린 아키텍처
...
<Step title="제품 추가">
Dodo Payments의 실제 제품 ID로 `internal/lib/products.go`를 업데이트하십시오:
...
| `/api/webhook` | POST | Dodo Payments webhook 처리 |
...
`internal/lib/products.go`를 수정하여 다음을 변경하십시오:
- 제품 ID(당신의 Dodo 대시보드에서)
- 가격
- 기능
- 설명
...
`templates/index.html`에서 하드코딩된 고객 데이터를 실제 사용자 데이터로 교체하십시오:
...
보일러플레이트는 `internal/api/webhook.go`에서 webhook 이벤트 처리를 시연합니다. 지원되는 이벤트는 다음과 같습니다:

| 이벤트 | 설명 |
|-------|-------------|
| `subscription.active` | 구독이 활성화될 때 트리거됨 |
| `payment.succeeded` | 결제가 성공적으로 이루어질 때 트리거됨 |
...
웹훅 URL을 [Dodo 대시보드](https://app.dodopayments.com/developer/webhooks)에서 ngrok URL로 업데이트하십시오. 올바른 웹훅 검증 키로 `.env` 파일을 업데이트해야 합니다.
</Accordion>
