...
<Tip>
가장 최근 버전을 항상 가져오려면 `@latest`을 사용하거나 안정성을 위해 `@1.0.0`와 같은 특정 버전으로 고정합니다.
</Tip>
...
클릭 후 **Cursor Settings** > **Tools & MCP** > **New MCP Server**에서 Cursor의 `mcp.json`에 환경 변수를 설정합니다.

**수동 구성**

<Steps>
<Step title="Cursor 설정 열기">
    **Cursor Settings** > **Features** > **Model Context Protocol**로 이동하거나 `Cmd/Ctrl + Shift + P`을 눌러 "MCP Settings"를 검색합니다.
</Step>
...
<Tab title="Claude Desktop">
결제 데이터에 대한 대화형 접근을 활성화하기 위해 Claude Desktop에서 Dodo Payments MCP 서버를 설정합니다.
<Steps>
<Step title="구성 파일 찾기">
...
클릭 후 **Command Palette** > **MCP: Open User Configuration**에서 VS Code의 `mcp.json`에 환경 변수를 설정합니다.

**수동 구성**

<Steps>
<Step title="MCP 구성 열기">
    Command Palette (`Cmd/Ctrl + Shift + P`)을 열고 "MCP: Open User Configuration"을 검색합니다.
</Step>
...
<Step title="창 새로 고침">
    VS Code를 새로 고침합니다 (`Cmd/Ctrl + Shift + P` > "Reload Window")하여 MCP 서버를 활성화합니다.
</Step>
</Steps>
</Tab>

<Tab title="Cline (VS Code)">
결제 데이터에 대한 대화형 접근을 활성화하기 위해 Cline (VS Code)에서 Dodo Payments MCP 서버를 설정합니다.
<Steps>
...
<Step title="VS Code 새로 고침">
    VS Code 창을 새로 고칩니다 (`Cmd/Ctrl + Shift + P` > "Reload Window")하여 MCP 서버 연결을 활성화합니다.
</Step>
</Steps>
</Tab>
...
사용 가능한 기능:
- `top-level-unions` - 최상위 유니온 유형 지원
- `valid-json` - JSON 문자열 인수 구문 분석을 올바르게 수행
- `refs` - 스키마에서 $ref 포인터 지원
- `unions` - 유니온 유형 지원 (anyOf)
- `formats` - 형식 유효성 검사 지원
- `tool-name-length=N` - 도구 이름 최대 길이

<Tip>
자세한 기능 정보를 보려면 `npx dodopayments-mcp --describe-capabilities`을 실행하세요.
</Tip>
...
<Tip>
데이터의 우발적인 수정 방지를 위해 개발 중에는 `--operation=read`을 사용하세요.
</Tip>
...
1. **`list_api_endpoints`** - 선택적 필터링이 있는 사용 가능한 방법을 검색합니다.
2. **`get_api_endpoint_schema`** - 특정 방법에 대한 상세 스키마를 가져옵니다.
3. **`invoke_api_endpoint`** - 적절한 매개변수로 모든 방법을 실행합니다.

<Info>
동적 도구는 50개 이상의 메서드를 가진 API에 대해 자동으로 활성화됩니다. `--tools=all`로 재정의하여 모든 도구를 포함할 수 있습니다.
</Info>
...
`DODO_PAYMENTS_ENVIRONMENT`이 올바르게 설정되었는지 확인합니다 (프로덕션용은 `live_mode`).
...
일부 클라이언트는 모든 JSON 스키마 기능을 지원하지 않습니다. `--describe-capabilities`을 사용하여 지원되는 내용을 이해하세요.
</Accordion>
</AccordionGroup>
