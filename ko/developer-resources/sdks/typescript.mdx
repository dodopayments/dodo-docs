...
<Info>
이벤트는 idempotency를 위해 고유한 `event_id` 값을 가져야 합니다. 동일 요청 내에서 중복된 ID는 거부되며, 기존 ID와 함께한 후속 요청은 무시됩니다.
</Info>
...
DodoPayments API의 목록 메소드는 페이지네이션됩니다. 모든 페이지에서 항목을 반복하려면 `for await … of` 구문을 사용할 수 있습니다:
