...
<Warning>
`POST /subscriptions` (SDK의 `subscriptions.create` 메서드)는 **deprecated** 상태입니다. 기존 통합에서는 계속 작동하지만, 새 통합에서는 [Checkout Session](/developer-resources/checkout-session)을 통해 구독을 생성해야 합니다.
</Warning>
...
<Info>
`billing`에는 최소한 두 글자의 ISO 국가 코드가 필요합니다. `customer`는 기존 고객을 연결하기 위한 `{ customer_id }` 또는 새 고객을 생성하기 위한 `{ email, name? }`의 union입니다. `product_price`는 통화의 최소 단위로 표현됩니다.
</Info>
...
사용량 기반 청구를 위한 custom event를 추적합니다:
...
<Info>
멱등성을 위해 이벤트에는 고유한 `event_id` 값이 있어야 합니다. 동일한 요청 내의 중복 ID는 거부되며, 기존 ID가 포함된 후속 요청은 무시됩니다.
</Info>

### 사용량 이벤트 조회

사용량 이벤트에 대한 자세한 정보를 가져옵니다:
...
환경 변수 또는 클라이언트 옵션을 사용하여 로그 상세 수준을 제어합니다:
...
<Warning>
디버그 수준에서는 헤더와 본문을 포함한 모든 HTTP 요청과 응답이 로그에 기록됩니다. 일부 인증 헤더는 redacted 처리되지만, 본문에 포함된 민감한 데이터는 여전히 표시될 수 있습니다.
</Warning>

## Node.js SDK에서 마이그레이션

레거시 Node.js SDK에서 업그레이드하는 경우 TypeScript SDK는 향상된 type safety와 기능을 제공합니다:
...
DodoPayments API의 목록 메서드는 페이지로 나뉩니다. `for await … of` 구문을 사용하면 모든 페이지의 항목을 순회할 수 있습니다:
...
- Cloudflare Workers
- Vercel Edge Runtime
- `"node"` 환경에서 Jest 28 이상
- Nitro v2.6 이상

TypeScript >= 4.9를 지원합니다.
...
TypeScript SDK에 도움이 필요하신가요?

- **Discord**: 실시간 지원을 받으려면 [커뮤니티 서버](https://discord.gg/bYqAp4ayYh)에 참여하세요
- **Email**: support@dodopayments.com으로 문의하세요  
- **GitHub**: [repository](https://github.com/dodopayments/dodopayments-typescript)에 issue를 등록하세요
