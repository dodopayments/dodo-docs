...
<Info>
이벤트는 멱등성을 위해 고유한 `event_id` 값을 가져야 합니다. 동일한 요청 내에서 중복 ID는 거부되며 기존 ID를 가진 이후 요청은 무시됩니다.
</Info>
...
<Warning>
디버그 수준에서는 모든 HTTP 요청 및 응답(헤더 및 본문 포함)이 기록됩니다. 일부 인증 헤더는 가려지지만 본문의 민감한 데이터는 여전히 표시될 수 있습니다.
</Warning>
...
DodoPayments API의 목록 메서드는 페이지네이션됩니다. 모든 페이지의 항목을 반복하려면 `for await … of` 구문을 사용할 수 있습니다:
