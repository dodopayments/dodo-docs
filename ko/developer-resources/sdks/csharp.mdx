...
SDK는 HTTP status code에 따라 특정 예외를 발생시킵니다. 모든 4xx 오류는 `DodoPayments4xxException`를 상속합니다.
...
기타 예외 유형:
- `DodoPaymentsIOException`: I/O 네트워크 오류
- `DodoPaymentsInvalidDataException`: 파싱된 데이터를 해석하지 못함
...
`Paginate` 메서드를 사용하여 모든 페이지의 모든 결과를 순회합니다. 이 메서드는 `IAsyncEnumerable`를 반환합니다:
