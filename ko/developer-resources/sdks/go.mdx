...
API가 성공이 아닌 상태 코드를 반환할 때, SDK는 `*dodopayments.Error` 유형의 오류를 반환합니다. 이는 `StatusCode`, 기본 `*http.Request` 및 `*http.Response`를 노출하며, 오류 본문의 JSON을 제공합니다. 이를 검사하려면 `errors.As` 패턴을 사용하고, 특정 상황을 처리하기 위해 `StatusCode`에서 분기하십시오:
