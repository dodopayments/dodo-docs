...
<Card title="Context Support" icon="clock">
  `context.Context`의 취소 및 시간 초과에 대한 완전한 지원
</Card>
...
<Warning>
`POST /subscriptions` (SDK의 `subscriptions.create` 메서드)는 **deprecated** 상태입니다. 기존 통합에서는 계속 작동하지만, 새로운 통합에서는 [Checkout Session](/developer-resources/checkout-session)을 통해 구독을 생성해야 합니다.
</Warning>
...
<Info>
`Billing`에는 최소한 두 자리 ISO `Country` 코드가 필요합니다. `Customer`은(는) `CustomerRequestUnionParam`입니다. 기존 고객에게는 `AttachExistingCustomerParam{CustomerID: ...}`을(를), 신규 고객에게는 `NewCustomerParam{Email: ..., Name: ...}`을(를) 전달하세요. `ProductPrice`은(는) 가장 낮은 통화 단위로 표시됩니다.
</Info>
...
API가 성공하지 않은 상태 코드를 반환하면 SDK는
`*dodopayments.Error` 유형의 오류를 반환합니다. 이 오류는 `StatusCode`, 기본 `*http.Request` 및
`*http.Response`, 그리고 오류 본문의 JSON을 제공합니다. `errors.As` 패턴을 사용하여
오류를 검사하고, 특정 사례를 처리하려면 `StatusCode`을(를) 기준으로 분기하세요:
...
로깅 또는 메트릭을 위한 사용자 지정 미들웨어를 추가합니다:
...
- **GitHub**: [repository](https://github.com/dodopayments/dodopayments-go)에 이슈를 등록하세요
