...
사용량 기반 결제를 위한 사용자 지정 이벤트를 추적합니다:
...
페이지네이션을 더 세밀하게 제어하려면 다음을 사용합니다:
...
환경 변수를 설정하여 로깅을 활성화합니다:

```bash
export DODO_PAYMENTS_LOG=info
```

또는 디버그 수준 로깅을 사용하려면 다음을 설정합니다:
...
- **GitHub**: [repository](https://github.com/dodopayments/dodopayments-python)에 이슈를 등록하세요

## 기여

여러분의 기여를 환영합니다! 시작하려면 [기여 가이드라인](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md)을 확인하세요.
