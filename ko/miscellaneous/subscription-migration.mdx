...
- 결제 수단 데이터의 파일 무결성 및 암호화 확인
- 올바른 형식인지 구독자 CSV 검증
- Test Mode에서 테스트 가져오기 실행
- 데이터 매핑 문제 식별 및 해결
...
다음을 권장합니다:
- 대시보드에서 마이그레이션된 구독 샘플 검증
- Test Mode에서 몇 가지 구독 갱신 테스트
- 새 ID로 내부 데이터베이스 참조 업데이트
- 첫 번째 청구 주기 동안 병렬 모니터링 실행
</Step>
</Steps>
...
Dodo Payments에서 구독이 어떻게 작동하는지 이해하려면 우리의 API를 탐색하세요.
</Card>
</CardGroup>
