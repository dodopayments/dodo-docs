...
| `payment_amount` | 가장 작은 통화 단위의 구독 금액 |
| `payment_interval` | 청구 주기 (매월, 매년 등) |
| `next_billing_date` | 다음 예정된 청구 날짜 (ISO 8601) |
| `card_token` | 현재 프로세서에서 받은 고유 토큰 |

<Warning>
`card_token` 필드는 중요합니다—이 필드는 가져오기 중에 가입자 기록을 그들의 결제 방법과 일치시키는 데 사용됩니다. 결제 방법 내보내기의 토큰과 정확히 일치하는지 확인하세요.
</Warning>
