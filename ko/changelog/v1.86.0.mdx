...
<Frame>
<img src="/images/CBB/Checkout.png" alt="제품 구매와 함께 제공된 크레딧을 보여주는 체크아웃" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Frame>
<img src="/images/CBB/Customer Portal.jpg" alt="잔액과 거래 내역이 있는 고객 포털 크레딧 보기" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Frame>
<img src="/images/design/general-overview.jpg" alt="체크아웃, 고객 포털 및 스토어프론트의 라이브 미리보기가 있는 디자인 설정 페이지" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Frame>
<img src="/images/design/general-color-settings.jpg" alt="전체 색상 구성이 표시된 고급 설정 확장, 라이트 및 다크 모드용" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
<Tip>
이 필드를 사용하여 분쟁 및 환불 대시보드를 구축하거나, 분쟁 상태 변경에 대한 알림을 트리거하거나, 주의가 필요한 결제를 표시하세요.
</Tip>

자세히 알아보기: [결제 목록 API](/api-reference/payments/get-payments)

### 4. **고객 포털의 환불 테이블**

고객 포털에는 이제 고객이 자신의 결제와 관련된 모든 환불을 볼 수 있는 전용 **환불** 섹션이 포함되어 있습니다. 각 환불 항목에는 환불 금액, 상태, 날짜 및 환불이 발행된 원래 결제가 표시되어 고객이 지원에 문의하지 않고도 환불 내역을 완전히 투명하게 볼 수 있습니다.

<Check>
환불 테이블은 모든 비즈니스의 고객 포털에 자동으로 제공됩니다. 설정이 필요하지 않습니다.
</Check>

자세히 알아보기: [고객 포털](/features/customer-portal)

### 5. **미터 라이브 모드로 복사**

이제 대시보드에서 **테스트 모드의 미터를 라이브 모드로 복사**할 수 있습니다. 테스트 모드에서 미터 구성을 완료한 후 복사 작업을 사용하여 모든 설정이 보존된 상태로 라이브 모드로 복제하세요: 집계 유형, 이벤트 이름, 필터 및 크레딧 연결. 이를 통해 개발에서 프로덕션으로 이동할 때 미터를 수동으로 다시 생성할 필요가 없습니다.

<Tip>
미터 구성을 먼저 테스트 모드에서 빌드하고 검증한 후, 프로덕션으로 이동할 준비가 되면 라이브 모드로 복사하세요. 이를 통해 실제 고객 사용량을 처리하기 전에 결제 설정이 테스트되었는지 확인할 수 있습니다.
</Tip>

자세히 알아보기: [사용량 기반 결제](/features/usage-based-billing/introduction) | [미터](/features/usage-based-billing/meters)
