...
- 명확성과 실행 가능성을 위해 색상, 필드 및 형식을 갖춘 풍부한 메시지 첨부파일을 사용하세요.
- 빠른 식별을 위해 항상 금액, 고객 이메일 및 ID와 같은 핵심 데이터를 포함하세요.
- 이벤트 유형에 맞는 색상을 선택하세요: 성공을 나타내는 녹색 (`good`), 분쟁이나 실패를 나타내는 빨간색 (`danger`), 취소를 나타내는 노란색 (`warning`), 정보 이벤트를 나타내는 파란색 (`#36a64f`).
