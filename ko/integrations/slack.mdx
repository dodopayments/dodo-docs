...
<Step title="Open the Webhook Section">
  Dodo Payments 대시보드에서 <b>Webhook</b> 섹션으로 이동합니다. <b>+ Add Endpoint</b> 버튼을 클릭한 후, 웹훅 드롭다운을 열어 다른 통합을 확인하세요.
  <Frame>
    <img src="/images/integrations/slack/1.png" alt="Dodo Payments 대시보드에 표시된 Endpoint 추가 버튼과 통합 드롭다운" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>

<Step title="Select Slack Integration">
  <b>Slack</b> 통합을 선택하고 <b>Connect your Slack workspace</b>를 클릭합니다.
  <Frame>
    <img src="/images/integrations/slack/2.png" alt="Slack 통합 카드와 연결 버튼" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>

<Step title="Grant Slack Permissions">
  선택한 채널에 메시지를 게시할 수 있도록 Incoming Webhooks Slack App에 필요한 권한을 부여하세요.
  <Frame>
    <img src="/images/integrations/slack/3.png" alt="Incoming Webhooks 앱을 위한 Slack OAuth 권한 화면" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>

<Step title="Customize Transformation Code">
  Slack 알림을 사용 사례에 맞게 맞춤화하려면 변환 코드를 추가하거나 편집하세요. 미리 만들어진 템플릿을 사용하거나 직접 로직을 작성할 수 있습니다.
  <Frame>
    <img src="/images/integrations/slack/4.png" alt="Slack 통합을 위한 변환 코드 편집기" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>

<Step title="Test and Create">
  사용자 지정 또는 미리 만들어진 이벤트 페이로드로 변환 코드를 테스트하세요. 만족하면 <b>Create</b>를 클릭하여 통합을 활성화합니다.
  <Frame>
    <img src="/images/integrations/slack/5.png" alt="변환 테스트 및 생성 버튼" style={{ maxHeight: '500px', width: 'auto' }} />
  </Frame>
</Step>
...
Slack 알림을 효과적으로 만들려면:
- 명확성과 실행 가능성을 높이기 위해 색상, 필드, 서식이 포함된 rich message attachments를 사용하세요.
- 빠른 식별을 위해 금액, 고객 이메일, ID와 같은 주요 데이터를 항상 포함하세요.
- 이벤트 유형에 맞는 색상을 선택하세요: 성공에는 녹색 (`good`), 분쟁 또는 실패에는 빨간색 (`danger`), 취소에는 노란색 (`warning`), 정보 제공 이벤트에는 파란색 (`#3aa3e3`)을 사용하세요.
- 각 이벤트가 발생한 시점을 추적할 수 있도록 타임스탬프를 추가하세요.
