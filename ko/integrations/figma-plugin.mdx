...
<Steps>
<Step title="기본 플러그인 생성">
[Figma의 플러그인 퀵스타트 가이드](https://www.figma.com/plugin-docs/plugin-quickstart-guide/)를 사용하여 나만의 Figma 플러그인 프로젝트를 생성합니다. 이렇게 하면 `manifest.json` 파일이 포함된 초기 구조가 생성됩니다.
...
<Step title="예제 구현 복제">
[dodopayments-figma 저장소](https://github.com/dodopayments/dodopayments-figma)를 복제하고 생성한 플러그인 폴더에서 이름과 ID로 `manifest.json`을 업데이트합니다.
...
<Step title="환경 구성">
`API_MODE`을 `src/ui/api.ts`에 설정하십시오:
...
<Warning>
개발 중에는 항상 `test_mode`를 사용하여 실제 결제를 처리하지 않도록 하십시오.
</Warning>
</Step>
...
- 유효한 라이선스 키를 가진 사용자를 위한 기능을 노출하도록 `Authenticated.tsx`을 업데이트하십시오.
- 디자인 선호도에 맞게 `LicenseKeyInput.tsx`을 사용자 정의하십시오.

<Note>
`Authenticated.tsx` 구성 요소는 유효한 라이선스가 필요한 플러그인의 프리미엄 기능을 추가하는 곳입니다.
</Note>
</Step>
</Steps>
...
<Steps>
<Step title="네트워크 접근 구성">
플러그인은 CORS 프록시와 통신하기 위해 네트워크 접근이 필요합니다. 다음을 `manifest.json`에 추가하십시오:
...
<Step title="API 모드 구성">
`src/ui/api.ts`에서 API 모드를 구성하십시오:
...
<Tip>
생산 준비가 완료되고 테스트 모드에서 철저히 테스트한 경우에만 `live_mode`로 전환하십시오.
</Tip>
</Step>
</Steps>
...
| 명령어 | 설명 |
|---------|-------------|
| `npm run build` | 플러그인을 생산용으로 빌드 |
| `npm run dev` | 파일 감시와 함께 개발 서버 시작 |
| `npm run lint` | 코드 스타일 및 오류 점검 |
| `npm run format` | Prettier로 코드 형식화 |
...
1. 생산을 위한 빌드: `npm run build`
2. 테스트 및 라이브 모드에서 철저히 테스트
3. [Figma의 플러그인 게시 가이드라인](https://www.figma.com/plugin-docs/publishing-plugins/)을 따르십시오.
4. Figma 커뮤니티를 통해 검토를 제출하십시오.
