...
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Tu secreto de webhook (requerido para el manejo de webhooks)
...
<Tab title="Configuración del Portal de Clientes">
  <Info>
  Usa esta función para permitir que los clientes gestionen sus suscripciones y detalles a través del portal de clientes de Dodo Payments. El cliente se identifica automáticamente a través de la función `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="Configuración del Manejador de Webhooks">
  <Info>
  Usa este manejador para recibir y procesar eventos de webhook de Dodo Payments de manera segura en tu aplicación Convex. Todos los manejadores de webhooks reciben el `ActionCtx` de Convex como primer parámetro, lo que te permite usar `ctx.runQuery()` y `ctx.runMutation()` para interactuar con tu base de datos.
  </Info>
  <CodeGroup>
