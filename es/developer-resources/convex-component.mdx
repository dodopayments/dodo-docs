...
Configura las variables de entorno en tu tablero de Convex (**Configuraciones** → **Variables de Entorno**). Puedes acceder al tablero ejecutando:
...
<Tab title="Configuración del Portal del Cliente">
  <Info>
  Utiliza esta función para permitir a los clientes gestionar sus suscripciones y detalles a través del portal de clientes de Dodo Payments. El cliente es identificado automáticamente a través de la función `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="Configuración del Manejador de Webhooks">
  <Info>
  Utiliza este manejador para recibir y procesar eventos de webhook de Dodo Payments de manera segura en tu aplicación Convex. Todos los manejadores de webhook reciben el `ActionCtx` de Convex como primer parámetro, lo que te permite usar `ctx.runQuery()` e `ctx.runMutation()` para interactuar con tu base de datos.
  </Info>
  <CodeGroup>
...
<Info>
El cliente es identificado automáticamente utilizando la función `identify` configurada en tu configuración de DodoPayments. Esta función debe devolver el `dodoCustomerId` del cliente.
</Info>
