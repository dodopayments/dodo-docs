...
{/* LOCKED_PATTERN_076a045a43785e7154357214ce1a1fe2 */}
    Integra el checkout de Dodo Payments con flujo basado en sesiones.
</Card>
...
Configura las variables de entorno en tu panel de Convex (**Settings** → **Environment Variables**). Puedes acceder al panel ejecutando:
...
- `DODO_PAYMENTS_API_KEY` - Tu clave de API de Dodo Payments
- `DODO_PAYMENTS_ENVIRONMENT` - Establece en `test_mode` o `live_mode`
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Tu secreto de webhook (requerido para manejar webhooks)

<Warning>
Usa siempre las variables de entorno de Convex para información sensible. Nunca cometas secretos en el control de versiones.
</Warning>
</Step>
</Steps>
...
Primero, crea una consulta interna para obtener clientes de tu base de datos. Esta se usará en las funciones de pago para identificar a los clientes.

<Warning>
Antes de usar esta consulta, asegúrate de definir el esquema apropiado en tu archivo `convex/schema.ts` o modifica la consulta para que coincida con tu esquema existente.
</Warning>
...
{/* LOCKED_PATTERN_cfdbe95b4d26ef82d25c4a1bfd5d5aeb */}
  <Info>
  Usa esta función para integrar el checkout de Dodo Payments en tu app de Convex. Emplea un checkout basado en sesiones con soporte completo de funcionalidades.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_de2bafe43db636b1864521f4f7e5aa25 */}
  <Info>
  Usa esta función para permitir que los clientes gestionen sus suscripciones y datos mediante el portal de clientes de Dodo Payments. El cliente se identifica automáticamente a través de la función `identify`.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_eeaa14c4f31af768a3b7e376ee69b626 */}
  <Info>
  Usa este manejador para recibir y procesar eventos webhook de Dodo Payments de forma segura en tu app de Convex. Todos los manejadores de webhook reciben el `ActionCtx` de Convex como primer parámetro, lo que te permite utilizar `ctx.runQuery()` e `ctx.runMutation()` para interactuar con tu base de datos.
  </Info>
  <CodeGroup>
...
<Info>
El componente Convex de Dodo Payments utiliza checkout basado en sesiones, lo que brinda una experiencia de pago segura y personalizable con carritos de productos y datos de clientes preconfigurados. Este es el enfoque recomendado para todos los flujos de pago.
</Info>
...
<Info>
El cliente se identifica automáticamente usando la función `identify` configurada en tu integración de DodoPayments. Esta función debe devolver el `dodoCustomerId` del cliente.
</Info>
...
<Info>
Usa la función de checkout desde tus componentes de React con los hooks de Convex.
</Info>
