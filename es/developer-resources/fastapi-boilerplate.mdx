...
<Card title="Repositorio de GitHub" icon="github" href="https://github.com/dodopayments/fastapi-boilerplate">
...
<Info>
Este boilerplate utiliza FastAPI con patrones de async/await, Pydantic para la validación y el SDK de Python `dodopayments` para una integración de API sin problemas.
</Info>
...
<Step title="Obtener Credenciales de API">
...
<Warning>
Nunca commits tu archivo `.env` a control de versiones. Ya está incluido en `.gitignore`.
</Warning>
</Step>
...
|----------|--------|-------------|
| `/checkout` | POST | Crear una nueva sesión de checkout |
| `/webhook` | POST | Manejar los webhooks de Dodo Payments |
| `/customer-portal` | POST | Generar la URL del portal del cliente |
...
- Usa claves API de producción desde el panel de control
...
<AccordionGroup>
<Accordion title="Errores de importación o módulos faltantes">
...
<Accordion title="Fallas en la creación de sesion de checkout">
**Causas comunes:**
- ID de producto inválido - verifica que exista en tu panel de Dodo
- Clave API o configuración incorrecta en `.env`
...
<Accordion title="Fallas en la verificación de la firma del webhook">
- Asegúrate de que `DODO_PAYMENTS_WEBHOOK_KEY` esté configurado correctamente en tu `.env`
- Verifica que estás usando el cuerpo de la solicitud en crudo para la verificación de la firma
- Asegúrate de que estás leyendo el header `webhook-signature` correctamente
</Accordion>
</AccordionGroup>
