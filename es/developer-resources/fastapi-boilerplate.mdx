...
- Verifica que estés utilizando el cuerpo sin procesar de la solicitud para la verificación de la firma
- Pasa los tres encabezados `webhook-id`, `webhook-timestamp` e `webhook-signature` a `webhook.verify()` — la firma de Standard Webhooks cubre `id.timestamp.body`, no solo el cuerpo
</Accordion>
</AccordionGroup>
