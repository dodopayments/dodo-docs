...
- **Pago en Superposición (embebido)**: Úsalo cuando necesites una experiencia en la página que embeba el pago alojado en tu sitio.
...
<Step title="Redirigir al cliente al pago">
...
#### 2. Pago en Superposición

Para una experiencia de pago en página sin interrupciones, explora nuestra integración de [Pago en Superposición](/developer-resources/overlay-checkout) que permite a los clientes completar pagos sin salir de tu sitio web.
...
<Steps>
<Step title="Construir tu enlace de pago">
...
<Step title="Controlar campos de formulario (opcional)">
  ...
  Para deshabilitar un campo, proporciona su valor y establece la bandera correspondiente <code>disable...</code> en <code>true</code>:
  <CodeGroup>
  ...
  <Note>
  Establecer <code>showDiscounts=false</code> deshabilitará y ocultará la sección de descuentos en el formulario de pago. Usa esto si deseas evitar que los clientes ingresen códigos de cupón o promoción durante el pago.
  </Note>
...
<Step title="Agregar controles avanzados (opcional)">
  ...
  - <ParamField query="paymentAmount" type="integer">Monto en centavos (solo para precios de Paga lo que Quieras).</ParamField>
...
<Tip>
Prefiere las Sesiones de Pago para la mayoría de los casos de uso, ya que ofrecen más flexibilidad y control.
</Tip>

Creados a través de una llamada a la API o nuestro SDK con detalles del cliente. Aquí hay un ejemplo:
