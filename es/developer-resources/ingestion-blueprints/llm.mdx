...
1. **Inicie sesión** en el [Tablero de Dodo Payments](https://app.dodopayments.com/)
2. **Navegue a** Productos → Medidores
3. **Haga clic en** "Crear Medidor"
4. **Configure su medidor**:
   - **Nombre del Medidor**: Elija un nombre descriptivo (p. ej., "Uso de Token LLM")
   - **Nombre del Evento**: Establezca un identificador de evento único (p. ej., `llm.chat_completion`)
   - **Tipo de Agrupación**: Seleccione `sum` para sumar los conteos de tokens
   - **Sobre Propiedad**: Elija qué rastrear:
...
<Warning>
Siempre use `test_mode` durante el desarrollo para evitar afectar las métricas de producción.
</Warning>
</ParamField>
...
<Note>
Al usar modelos que tienen capacidades de razonamiento a través de la AI SDK (como el Gemini 2.5 Flash de Google con modo de pensamiento), los tokens de razonamiento se incluyen automáticamente en el conteo de `outputTokens` para la facturación precisa.
</Note>
</Accordion>
...
<Note>
**Modo de Pensamiento de Gemini:** Al usar modelos Gemini con capacidades de razonamiento (como Gemini 2.5 Pro), el SDK incluye automáticamente `thoughtsTokenCount` (tokens de razonamiento) en `outputTokens` para reflejar con precisión el costo computacional completo.
</Note>
</Accordion>
</AccordionGroup>
...
- Cualquier metadato personalizado que haya proporcionado al envolver el cliente
...
<Info>
Su medidor de Dodo Payments utiliza los campos `metadata` (especialmente `inputTokens`, `outputTokens` o `totalTokens`) para calcular el uso y la facturación.
</Info>
