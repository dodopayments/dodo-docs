...
<Card title="Manejador de Checkout" icon="cart-shopping" href="#checkout-route-handler">
    Integra el proceso de pago de Dodo Payments en tu aplicación Express.
</Card>
...
<Tab title="Manejador de Webhook">
  <Info>
  Usa este manejador para recibir y procesar eventos de webhook de Dodo Payments de manera segura en tu aplicación Express.
  </Info>
  <CodeGroup> 
...
- Para una lista completa de los campos del cuerpo POST soportados, consulta:
...
Si se establece en <code>true</code>, se envía un correo electrónico al cliente con el enlace del portal.
</ParamField>
...
Aquí tienes cómo deberías estructurar tu respuesta:
...
**Integración**:
Crea rutas en tu aplicación Express para checkouts estáticos (GET), dinámicos (POST) y sesiones de checkout (POST).
...
GET (checkout estático) espera parámetros de consulta:
    ...
    cantidad, campos del cliente (fullName, email, etc.) y metadatos (metadata_*) son opcionales.
