...
Por favor, encuéntralos en [Crear sesión de pago](/api-reference/checkout-sessions/create)
...
Después de que el mandato sea autorizado, crea cargos según sea necesario.
...
<AccordionGroup>
  <Accordion title="Parámetros del cuerpo de la solicitud de cargo">
    ...
    <ParamField body="product_currency" type="string">
      Anulación opcional de la moneda para el cargo.
    </ParamField>

    <ParamField body="product_description" type="string">
      Anulación opcional de la descripción para este cargo.
    </ParamField>
...
Nuestro sistema de detección de fraudes puede bloquear patrones de reintento agresivos (y puede marcarlos como pruebas de tarjeta potenciales). Sigue una política de reintento segura.
...
- Alternativamente, si almacenas la última hora de pago exitosa `T`, programa el siguiente intento a las `T + X days` para preservar la alineación de la hora del día.

<Note>
Zona horaria y DST: usa un estándar de tiempo consistente para la programación y convierte solo para la visualización para mantener los intervalos.
</Note>

### Códigos de rechazo que no deberías reintentar
...
[Documentación de fallos de transacción](/api-reference/transaction-failures).
</Note>
...
- Mantén y referencia la última marca de tiempo de pago exitosa `T` para calcular el siguiente intento; no agrupe múltiples suscripciones en el mismo instante.
