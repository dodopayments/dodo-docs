---
sidebarTitle: Descripción general
title: Descripción general de los adaptadores de framework
description: Adaptadores preconstruidos para frameworks web populares para
  integrar Dodo Payments en minutos con un código mínimo
icon: box
---

Dodo Payments proporciona adaptadores de framework oficiales que simplifican la integración de pagos. Cada adaptador está diseñado para funcionar sin problemas con las convenciones de tu framework, ofreciendo manejo de checkout, portal de clientes y webhooks desde el primer momento.

<Tip>
Los adaptadores de framework te permiten integrar Dodo Payments en menos de 10 líneas de código. Manejan la autenticación, el análisis de solicitudes y el formato de respuestas automáticamente.
</Tip>

## Adaptadores de Framework Disponibles

Elige el adaptador que coincida con tu framework:

<CardGroup cols={2}>
<Card title="Next.js" icon="react" href="/developer-resources/nextjs-adaptor">
  Soporte para App Router con controladores de ruta para checkout, portal y webhooks
</Card>

<Card title="Nuxt" icon="vuejs" href="/developer-resources/nuxt-adaptor">
  Framework full-stack basado en Vue con integración de rutas del servidor
</Card>

<Card title="Express" icon="node-js" href="/developer-resources/express-adaptor">
  Controladores basados en middleware para el popular framework de Node.js
</Card>

<Card title="Fastify" icon="bolt" href="/developer-resources/fastify-adaptor">
  Framework de Node.js de alto rendimiento con arquitectura de plugins
</Card>

<Card title="Hono" icon="cloud" href="/developer-resources/hono-adaptor">
  Framework web ultrarrápido para el edge, Cloudflare Workers y más
</Card>

<Card title="Astro" icon="star" href="/developer-resources/astro-adaptor">
  Framework centrado en contenido con soporte para endpoints del servidor
</Card>

<Card title="SvelteKit" icon="code" href="/developer-resources/sveltekit-adaptor">
  Framework Svelte full-stack con integración de hooks del servidor
</Card>

<Card title="Remix" icon="react" href="/developer-resources/remix-adaptor">
  Framework React full-stack con controladores de carga y acción
</Card>

<Card title="TanStack Start" icon="chart-line" href="/developer-resources/tanstack-adaptor">
  Framework React full-stack seguro en tipos con funciones del servidor
</Card>

<Card title="Better Auth" icon="shield" href="/developer-resources/better-auth-adaptor">
  Plugin de framework de autenticación para una integración fluida de auth + pagos
</Card>

<Card title="Convexo" icon="database" href="/developer-resources/convex-component">
  Componente Backend-as-a-Service para la sincronización de pagos en tiempo real
</Card>

<Card title="Bun" icon="bolt" href="/developer-resources/bun-adaptor">
  Controladores nativos de Bun.serve() para checkout, portal y webhooks
</Card>
</CardGroup>

## Características Principales

Todos los adaptadores de framework proporcionan estas capacidades integradas:

| Característica | Descripción |
|---------------|-------------|
| **Controlador de Checkout** | Soporte para flujos de checkout estáticos, dinámicos y basados en sesiones |
| **Portal del Cliente** | Controlador preconstruido para gestión de suscripciones y facturación |
| **Controlador de Webhooks** | Verificación segura de firmas con controladores de eventos tipados |
| **Configuración del Entorno** | Configuración simple a través de variables de entorno |
| **Seguridad de Tipo** | Soporte completo de TypeScript con cargas útiles tipadas |

## Inicio Rápido

Comienza con cualquier adaptador de framework en tres pasos:

<Steps>
<Step title="Instalar el Adaptador">
  Usa tu gestor de paquetes para instalar el adaptador específico del framework:

  <Tabs>
  <Tab title="Next.js">

  ```bash
  npm install @dodopayments/nextjs
  ```

  </Tab>
  
  <Tab title="Nuxt">

  ```bash
  npm install @dodopayments/nuxt
  ```

  </Tab>
  
  <Tab title="Express">

  ```bash
  npm install @dodopayments/express
  ```

  </Tab>
  
  <Tab title="Hono">

  ```bash
  npm install @dodopayments/hono
  ```

  </Tab>

  <Tab title="Astro">

  ```bash
  npm install @dodopayments/astro
  ```

  </Tab>

  <Tab title="SvelteKit">

  ```bash
  npm install @dodopayments/sveltekit
  ```

  </Tab>
  </Tabs>
</Step>

<Step title="Configurar Variables de Entorno">
  Agrega tus credenciales de Dodo Payments a tu entorno:

  ```env
  DODO_PAYMENTS_API_KEY=your-api-key
  DODO_PAYMENTS_WEBHOOK_KEY=your-webhook-secret
  DODO_PAYMENTS_RETURN_URL=https://yourdomain.com/checkout/success
  DODO_PAYMENTS_ENVIRONMENT="test_mode" # or "live_mode"
  ```

  <Warning>
  Nunca commits tu archivo `.env` o secretos en el control de versiones.
  </Warning>
</Step>

<Step title="Crear Controladores de Rutas">
  Configura tus rutas de checkout, portal del cliente y webhooks:

  <Tabs>
  <Tab title="Next.js">

  ```typescript
  // app/checkout/route.ts
  import { Checkout } from "@dodopayments/nextjs";

  export const GET = Checkout({
    bearerToken: process.env.DODO_PAYMENTS_API_KEY,
    returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
    environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
  });
  ```

  </Tab>
  
  <Tab title="Express">

  ```typescript
  import { checkoutHandler } from '@dodopayments/express';

  app.get('/api/checkout', checkoutHandler({
    bearerToken: process.env.DODO_PAYMENTS_API_KEY,
    returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
    environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
  }));
  ```

  </Tab>
  
  <Tab title="Hono">

  ```typescript
  import { Checkout } from "@dodopayments/hono";

  app.get('/checkout', Checkout({
    bearerToken: process.env.DODO_PAYMENTS_API_KEY,
    returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
    environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
  }));
  ```

  </Tab>
  </Tabs>

  <Check>
  ¡Ahora estás listo para procesar pagos! Visita las páginas de adaptadores individuales para guías detalladas y todas las opciones disponibles.
  </Check>
</Step>
</Steps>

## Tipos de Flujos de Checkout

Todos los adaptadores admiten tres tipos de flujos de checkout:

<AccordionGroup>
<Accordion title="Checkout Estático (GET)">
Usa el checkout estático para enlaces de pago simples y compartibles. Pasa el ID del producto como parámetro de consulta:

```
/api/checkout?productId=pdt_xxx&quantity=1
```

Admite la opción de prellenar al cliente y personalización a través de parámetros de consulta.
</Accordion>

<Accordion title="Checkout Dinámico (POST)">
Usa checkout dinámico para crear pagos programáticamente con detalles personalizados:

```json
{
  "product_id": "pdt_xxx",
  "customer": {
    "email": "customer@example.com",
    "name": "John Doe"
  },
  "quantity": 1
}
```

Admite tanto pagos únicos como suscripciones.
</Accordion>

<Accordion title="Sesiones de Checkout (POST)">
Usa sesiones de checkout para la experiencia de checkout más flexible con soporte de carrito:

```json
{
  "product_cart": [
    { "product_id": "pdt_xxx", "quantity": 1 },
    { "product_id": "pdt_yyy", "quantity": 2 }
  ],
  "customer": {
    "email": "customer@example.com"
  }
}
```

Aprende más en la [Guía de Sesiones de Checkout](/developer-resources/checkout-session).
</Accordion>
</AccordionGroup>

## Manejo de Eventos de Webhook

Todos los adaptadores proporcionan un manejo de webhook seguro con tipo, con callbacks de eventos granulares:

```typescript
Webhooks({
  webhookKey: process.env.DODO_PAYMENTS_WEBHOOK_KEY,
  onPayload: async (payload) => {
    // Handle any webhook event
  },
  onPaymentSucceeded: async (payload) => {
    // Handle successful payments
  },
  onSubscriptionActive: async (payload) => {
    // Handle new subscriptions
  },
  // ... 20+ event types supported
});
```

<Info>
Todos los controladores de webhook verifican automáticamente las firmas y validan las cargas útiles utilizando esquemas de Zod. Las solicitudes no válidas se rechazan con los códigos de error apropiados.
</Info>

## Elegir el Adaptador Correcto

| Framework | Mejor Para | Runtime |
|-----------|------------|---------|
| **Next.js** | Aplicaciones React de pila completa con App Router | Node.js, Edge |
| **Nuxt** | Aplicaciones Vue.js de pila completa | Node.js |
| **Express** | API REST y aplicaciones Node.js tradicionales | Node.js |
| **Fastify** | API de alto rendimiento | Node.js |
| **Hono** | Implementaciones Edge, Cloudflare Workers | Edge, Node.js |
| **Astro** | Sitios de contenido con endpoints en el servidor | Node.js, Edge |
| **SvelteKit** | Aplicaciones Svelte de pila completa | Node.js |
| **Remix** | React de pila completa con enrutamiento anidado | Node.js |
| **TanStack Start** | React de pila completa con seguridad de tipos | Node.js |
| **Better Auth** | Aplicaciones que ya utilizan Better Auth | Varios |
| **Convexo** | Aplicaciones que utilizan Convexo para el backend | Convexo Runtime |
| **Bun** | Aplicaciones de servidor Bun nativas | Bun |

## Obtener Ayuda

¿Necesitas ayuda con adaptadores de framework?

- **Discord**: Únete a nuestro [servidor comunitario](https://discord.gg/bYqAp4ayYh) para ayuda en tiempo real
- **Email**: Contáctanos en support@dodopayments.com
- **GitHub**: Abre un problema en el repositorio del respectivo adaptador
- **Documentación**: Visita nuestra [referencia API](/api-reference/introduction)
