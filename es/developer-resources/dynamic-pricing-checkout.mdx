...
2. **Pasar montos dinámicos**: Incluye un campo `amount` en el carrito de productos al crear sesiones de pago.
3. **Permitir a los clientes elegir**: Si no se proporciona un monto, los clientes pueden ingresar su propio precio (dentro de tus límites).

<Info>
Cuando pasas un `amount` en el carrito de productos, ese monto se utiliza para el pago. Si omites el campo `amount`, los clientes pueden seleccionar su propio precio durante el pago (sujeto a tus configuraciones de mínimo/máximo).
</Info>
...
Haz clic en **Agregar Producto** para guardar. Toma nota de tu ID de producto (por ejemplo, `pdt_123abc456def`) para usar en las sesiones de pago.
</Step>
</Steps>
...
Una vez que tu producto esté configurado con Paga lo que quieras, puedes crear sesiones de pago con montos dinámicos. El campo `amount` en el carrito de productos te permite establecer el precio programáticamente para cada sesión de pago.

### Entendiendo el Campo de Monto

Al crear una sesión de pago, puedes incluir un campo `amount` en cada elemento del carrito de productos:

- **Si se proporciona `amount`**: El pago utiliza este monto exacto (debe estar dentro de tus límites de mínimo/máximo).
- **Si se omite `amount`**: Los clientes pueden ingresar su propio precio durante el pago (dentro de tus límites).

<Warning>
El campo `amount` solo es aplicable para productos de Paga lo que quieras. Para productos regulares, este campo es ignorado.
</Warning>
...
<Note>
**Formato del Monto**: El campo `amount` debe estar en la denominación más baja de la moneda. Para USD, esto significa centavos (por ejemplo, $25.00 = `2500`). Para otras monedas, utiliza la unidad más pequeña (por ejemplo, paise para INR).
</Note>

## Paso 3: Dejar que los Clientes Elijan su Precio

Si deseas que los clientes seleccionen su propio precio durante el pago, simplemente omite el campo `amount` del carrito de productos. La página de pago mostrará un campo de entrada donde los clientes pueden ingresar cualquier monto dentro de tus límites mínimos y máximos.
...
<Card title="Rastrea las decisiones de precios" icon="chart-line">
...
- El producto tiene habilitado **Paga lo que quieres** en el panel de control.
- El producto es un producto de **Pago Único** (único), no una suscripción.
- El monto está en el formato correcto (la denominación más baja de la moneda, por ejemplo, centavos para USD).
</Accordion>

<Accordion title="El monto excede el máximo o está por debajo del mínimo">
La API rechazará sesiones de pago donde el monto viole los límites de precio de tu producto. Siempre valida los montos antes de crear sesiones de pago, o permite que los clientes elijan su precio omitiendo el campo `amount`.
</Accordion>

<Accordion title="El cliente no puede ingresar su propio precio">
Si los clientes no ven el campo de entrada de precio, asegúrate de haber omitido el campo `amount` del carrito de productos. Cuando se proporciona `amount`, el pago utiliza ese monto exacto.
</Accordion>
</AccordionGroup>