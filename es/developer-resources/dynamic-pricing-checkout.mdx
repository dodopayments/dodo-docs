...
2. **Pasar montos dinámicos**: Incluye un `amount` en el carrito de productos al crear sesiones de pago
...
<Info>
Cuando pasas un `amount` en el carrito de productos, ese monto se utiliza para el pago. Si omites el campo {/* INLINE_CODE_PLACEHOLDER_ed202d24820b800650e5cfc3bb366e40 */, los clientes pueden seleccionar su propio precio durante el pago (sujeto a tus configuraciones de mínimo/máximo).
</Info>
...
Haz clic en **Agregar Producto** para guardar. Toma nota de tu ID de producto (por ejemplo, `pdt_123abc456def`) para usar en sesiones de pago.
</Step>
</Steps>
...
Al crear una sesión de pago, puedes incluir un campo `amount` en cada artículo del carrito de productos:
...
<Warning>
El campo `amount` solo es aplicable para productos de Paga lo que quieras. Para productos regulares, este campo se ignora.
</Warning>
...
<Note>
**Formato de Monto**: El campo `amount` debe estar en la denominación más baja de la moneda. Para USD, esto significa centavos (por ejemplo, $25.00 = `2500`). Para otras monedas, usa la unidad más pequeña (por ejemplo, paise para INR).
</Note>
...
- El producto tiene **Paga lo que quieras** habilitado en el panel de control
- El producto es un producto de **Pago Único** (una sola vez), no una suscripción
...
La API rechazará las sesiones de pago donde el monto viole los límites de precio de tu producto. Siempre valida los montos antes de crear sesiones de pago, o deja que los clientes elijan su precio omitiendo el campo `amount`.
</Accordion>

<Accordion title="El cliente no puede ingresar su propio precio">
Si los clientes no ven el campo de entrada de precio, asegúrate de haber omitido el campo `amount` del carrito de productos. Cuando se proporciona {/* INLINE_CODE_PLACEHOLDER_ed202d24820b800650e5cfc3bb366e40 */, el pago utiliza ese monto exacto.
</Accordion>
</AccordionGroup>