...
2. **Pasar montos dinámicos**: Incluye un `amount` en el carrito del producto al crear sesiones de checkout
...
<Info>
Cuando pasas un `amount` en el carrito del producto, ese monto se utiliza para el checkout. Si omites el campo `amount`, los clientes pueden seleccionar su propio precio durante el checkout (sujeto a tus configuraciones de mínimo/máximo).
</Info>
...
Una vez que tu producto esté configurado con Paga lo que Quieras, puedes crear sesiones de checkout con montos dinámicos. El campo `amount` en el carrito del producto te permite establecer el precio programáticamente para cada sesión de checkout.

### Entendiendo el Campo de Monto

Al crear una sesión de checkout, puedes incluir un campo `amount` en cada artículo del carrito del producto:
...
Si deseas que los clientes seleccionen su propio precio durante el checkout, simplemente omite el campo `amount` del carrito del producto. La página de checkout mostrará un campo de entrada donde los clientes pueden ingresar cualquier monto dentro de tus límites mínimos y máximos.
...
- El producto tiene habilitado **Paga lo que Quieras** en el panel
...
La API rechazará las sesiones de checkout donde el monto viole los límites de precio de tu producto. Siempre valida los montos antes de crear sesiones de checkout, o deja que los clientes elijan su precio omitiendo el campo `amount`.
</Accordion>

<Accordion title="El cliente no puede ingresar su propio precio">
Si los clientes no ven el campo de entrada de precio, asegúrate de haber omitido el campo `amount` del carrito del producto. Cuando se proporciona `amount`, el checkout utiliza ese monto exacto.
</Accordion>
</AccordionGroup>