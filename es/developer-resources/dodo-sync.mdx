...
<Tip>
La opción `interval` es requerida cuando se utiliza `.start()` para sincronización automática. La sincronización se ejecutará continuamente en el intervalo especificado hasta que se detenga el proceso.
</Tip>
...
<Tip>
Al utilizar sincronización manual, la opción `interval` no es requerida. Puedes llamar a `.run()` siempre que necesites realizar una operación de sincronización.
</Tip>

#### Ejemplo de PostgreSQL

Aquí te mostramos cómo usar `dodo-sync` con PostgreSQL:
...
Aquí te mostramos cómo usar `dodo-sync` con MySQL:
...
Aquí te mostramos cómo usar `dodo-sync` con ClickHouse:
...
Array de entidades a sincronizar. Opciones disponibles: `"licences"`, `"payments"`, `"customers"`, `"subscriptions"`. Puedes incluir cualquier combinación de estas.
</ParamField>
...
Tiempo en segundos entre sincronizaciones automáticas. Requerido al usar `.start()` para sincronización automática. Opcional al usar `.run()` para sincronización manual.
</ParamField>
...
<Warning>
**MongoDB**: Se creará automáticamente en tu servidor de base de datos una base de datos llamada `dodopayments_sync`. Todos los datos de sincronización se almacenarán allí. Este nombre de base de datos es actualmente fijo y no puede ser cambiado.
...
**ClickHouse**: Se crearán tablas (`Subscriptions`, `Payments`, `Licenses`, `Customers`) utilizando el motor ReplacingMergeTree. Al consultar, usa la palabra clave `FINAL` para asegurar resultados deduplicados.
</Warning>
