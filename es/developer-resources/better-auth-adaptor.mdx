...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (obligatorio): instancia de cliente DodoPayments
    - <b>createCustomerOnSignUp</b> (opcional): Crear automáticamente clientes al registrarse el usuario
    - <b>use</b> (obligatorio): Array de plugins a habilitar (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (opcional): Función que recibe el BetterAuth `User` y devuelve campos adicionales para adjuntar al cliente de DodoPayments en la creación y actualización (por ejemplo, `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b>: Array de productos o función async que devuelve productos -{" "}
    <b>successUrl</b>: URL para redirigir después de un pago exitoso -{" "}
    <b>authenticatedUsersOnly</b>: Requiere autenticación de usuario (por defecto: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>Clave de API inválida</b>: Verifica tu{" "}
    <code>DODO_PAYMENTS_API_KEY</code> en <code>.env</code>. -{" "}
    <b>Desajuste de firma de webhook</b>: Asegúrate de que el secreto del webhook coincida con el establecido en el Panel de Control de Dodo Payments. - <b>Cliente no creado</b>: Confirma que{" "}
    <code>createCustomerOnSignUp</code> esté configurado en <code>true</code>.
  </Accordion>
