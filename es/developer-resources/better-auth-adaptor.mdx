...
- Puntos finales de ingestión y reporte de uso medido para facturación basada en el uso
...
Habilita el plugin <code>usage()</code> en el servidor para capturar eventos medidos y permitir que los clientes inspeccionen su facturación basada en el uso.
...
- <code>authClient.dodopayments.usage.meters.list</code> lista el uso reciente de los medidores vinculados a las suscripciones de ese cliente.
...
<Tip>
  Las marcas de tiempo más antiguas de una hora o más de cinco minutos en el futuro son
  rechazadas al ingerir el uso.
</Tip>
<Tip>
  Si omites <code>meter_id</code> al listar los medidores de uso, se devuelven todos los medidores 
  vinculados a las suscripciones del cliente.
</Tip>
...
<Info>
  ...
  verificación de firma segura. El punto final predeterminado es{" "}
  <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
<Step title="Generar y establecer el secreto del webhook">
Genera un secreto de webhook para tu URL de punto final (por ejemplo, <code>https://&lt;tu-dominio&gt;/api/auth/dodopayments/webhooks</code>) en el Panel de Dodo Payments y configúralo en tu archivo <code>.env</code>:
...
<AccordionGroup>
  ...
  <Accordion title="Opciones del Plugin de Checkout">
    - <b>products</b>: Array de productos o función asíncrona que devuelve productos -{" "}
    <b>successUrl</b>: URL para redirigir después de un pago exitoso -{" "}
...
<AccordionGroup>
  <Accordion title="Problemas Comunes">
    - <b>Clave API inválida</b>: Verifica tu{" "}
    <code>DODO_PAYMENTS_API_KEY</code> en <code>.env</code>. -{" "}
    <b>Desajuste de firma del webhook</b>: Asegúrate de que el secreto del webhook coincida con el que
    se estableció en el Panel de Dodo Payments. - <b>Cliente no creado</b>: Confirma{" "}
    <code>createCustomerOnSignUp</code> está configurado en <code>true</code>.
  </Accordion>
  <Accordion title="Mejores Prácticas">
    - Usa variables de entorno para todos los secretos y claves. - Prueba en{" "}
    <code>test_mode</code> antes de cambiar a <code>live_mode</code>. - Registra
