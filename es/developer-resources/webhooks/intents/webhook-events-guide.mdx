...
description: Comprende los diferentes eventos de webhook activados por Dodo
  Payments durante actividades de pago, reembolso, suscripción, disputa, clave
  de licencia, recuperación y cobranza
icon: bell
keywords:
  - Dodo Payments
  - eventos de webhook
...
| **Nombre del evento**           | **Tipo de evento**           | **Descripción**                                                                 |
|--------------------------|--------------------------|---------------------------------------------------------------------------------|
| **Crédito añadido**         | `credit.added`           | Se activa cuando se otorgan créditos a un cliente (suscripción, compra única, complemento o API). |
| **Crédito deducido**      | `credit.deducted`        | Ocurre cuando los créditos se consumen mediante uso o débito manual.                 |
| **Crédito caducado**       | `credit.expired`         | Se activa cuando los créditos no utilizados caducan tras el período de expiración configurado.        |
| **Crédito transferido**   | `credit.rolled_over`     | Ocurre cuando los créditos no utilizados se trasladan a una nueva asignación al final del ciclo.     |
| **Transferencia perdida**   | `credit.rollover_forfeited` | Se activa cuando los créditos se pierden porque se alcanzó el número máximo de transferencias. |
| **Sobrecargo aplicado**      | `credit.overage_charged` | Ocurre cuando se aplican cargos por uso excesivo más allá del saldo cero.           |
| **Ajuste manual**    | `credit.manual_adjustment` | Se activa cuando se realiza un ajuste manual de crédito o débito.                    |
| **Saldo bajo**          | `credit.balance_low`     | Se activa cuando el saldo de créditos cae por debajo del umbral configurado.             |

## **Eventos de Recuperación**

| **Nombre del Evento**           | **Tipo de Evento**                  | **Descripción**                                                                 |
|--------------------------|---------------------------------|---------------------------------------------------------------------------------|
| **Detección de Carrito Abandonado** | `abandoned_checkout.detected` | Se activa cuando se detecta un carrito incompleto o fallido después de 60 minutos.  |
| **Recuperación de Carrito Abandonado** | `abandoned_checkout.recovered` | Ocurre cuando un cliente completa el pago a través de un enlace de recuperación.              |
...
| **Nombre del Evento**           | **Tipo de Evento**           | **Descripción**                                                                 |
|--------------------------|--------------------------|---------------------------------------------------------------------------------|
| **Cobranza Iniciada**      | `dunning.started`        | Se activa cuando comienza un intento de cobranza para una suscripción que fue suspendida o cancelada. |
| **Cobranza Recuperada**    | `dunning.recovered`      | Ocurre cuando un cliente actualiza su método de pago y el cargo resultante tiene éxito. |
