...
- **Espere a `entitlement_grant.delivered` antes de desbloquear funciones dependientes.** Un evento `payment.succeeded` le indica que el dinero se ha aclarado; no le dice que el cliente ya tiene el repositorio de GitHub o el rol de Discord. El evento `delivered` es la fuente de verdad para el cumplimiento.
- **Asocie `revocation_reason` a flujos de retención.** Una revocación `subscription_on_hold` generalmente significa que la tarjeta del cliente falló y la próxima renovación volverá a otorgar acceso. Una revocación `manual` o `subscription_cancelled` es intencional. Trátelas de manera diferente en los mensajes al cliente.
- **Utilice el otorgamiento `id` como su clave de idempotencia.** Un solo otorgamiento emite como máximo un evento `created` y un máximo de un evento terminal (`delivered` o `failed`), y como máximo un evento `revoked`. Las reentregas del sistema de webhook pueden repetir eventos; dedúplicar en el otorgamiento `id` más `type`.
- **Lea `integration_type` para reconocer el tipo de otorgamiento.** La carga lleva `integration_type` directamente (por ejemplo `license_key`, `digital_files`, `discord`). Los objetos anidados `license_key` e `digital_product_delivery` se completan una vez que se entregan sus respectivos otorgamientos; un otorgamiento de clave de licencia cumplido manualmente permanece `pending` con `integration_type: "license_key"` y un `null` `license_key` hasta que lo cumpla.
- **Para otorgamientos basados en OAuth, muestre `oauth_url` al cliente.** El evento `entitlement_grant.created` para los flujos de suscriptores de Discord, GitHub o Notion incluye un `oauth_url` e `oauth_expires_at`. Envíelo por correo electrónico al cliente o muéstrelo en su aplicación para desbloquear la entrega.
