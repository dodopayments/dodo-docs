...
| Event | Description |
|-------|-------------|
| `subscription.active` | La suscripción se activó correctamente |
| `subscription.updated` | El objeto de suscripción se actualizó (cualquier cambio en un campo activa este evento) |
| `subscription.on_hold` | La suscripción se puso en espera debido a una renovación fallida |
| `subscription.paused` | La suscripción está pausada |
| `subscription.unpaused` | La suscripción se reanudó después de estar pausada |
| `subscription.renewed` | La suscripción se renovó para el siguiente período de facturación |
| `subscription.plan_changed` | El plan de suscripción se actualizó, degradó o modificó |
| `subscription.update_payment_method` | El método de pago de la suscripción se actualizó |
| `subscription.cancelled` | La suscripción se canceló |
