...
<Card title="Repositorio de GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/supabase">
  Código fuente completo y guía de configuración
</Card>

<Tip>
  <strong>Consulta el kit de inicio completo para supabase</strong><br />
  Un kit de inicio de suscripción mínimo construido con Next.js, Supabase y Dodo Payments. Este boilerplate te ayuda a configurar rápidamente un SaaS basado en suscripción con autenticación, pagos y webhooks.
...
Supabase proporciona automáticamente `SUPABASE_URL` y `SUPABASE_SERVICE_ROLE_KEY` en tiempo de ejecución.
...
La función ya está configurada en `functions/webhook/index.ts` - solo despliega:
...
2. **Verifica la idempotencia** - Busca el ID de webhook para prevenir el procesamiento duplicado
3. **Registra el evento** - Almacena los datos de webhook en bruto en la tabla `webhook_events`
4. **Procesa actualizaciones** - Crea o actualiza clientes y suscripciones a través del cliente de Supabase
5. **Maneja errores** - Registra fallos y marca el evento para reintento
...
<Note>
La bandera `--no-verify-jwt` es requerida porque los webhooks no incluyen tokens JWT. La seguridad se proporciona mediante la verificación de la firma del webhook.
</Note>
...
| Problema | Solución |
|-------|----------|
| Verificación fallida | Verifica que la clave del webhook sea correcta desde el panel de DodoPayments |
| Error de permiso de base de datos | Asegúrate de usar la Clave de Rol de Servicio |
| Error de verificación de JWT | Despliega con la bandera `--no-verify-jwt` |
| Función no encontrada | Verifica que la referencia del proyecto sea correcta y que la función esté desplegada |
