...
<Card title="Repositorio de GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/netlify">
...
1. Ve al [Tablero de DodoPayments](https://app.dodopayments.com) → Desarrollador → Webhooks
...
✅ **Registro de eventos** - Completa auditoría en la tabla `webhook_events`  
✅ **Manejo de errores** - Registrado y reintentable

> **Nota:** Esta implementación demuestra el manejo de tres eventos de suscripción principales (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con campos mínimos. Puedes extenderlo fácilmente para soportar tipos de eventos adicionales y campos según tus requerimientos.
...
1. **Verifica la firma** - Asegura que la solicitud provenga de DodoPayments usando HMAC-SHA256
2. **Verifica duplicados** - Usa el ID de webhook para evitar procesar el mismo evento dos veces
3. **Registra el evento** - Almacena el webhook crudo en la tabla `webhook_events` para auditoría
...
**Ver registros en el Tablero de Netlify:**
1. Selecciona tu sitio → pestaña **Funciones**
2. Haz clic en la función `webhook`
3. Visualiza registros en tiempo real e historial de invocaciones
...
| Problema | Solución |
|-------|----------|
| Verificación fallida | Verifica que la clave del webhook sea correcta desde el tablero de DodoPayments |
| Error de conexión a la base de datos | Verifica la cadena de conexión de Neon y utiliza conexión agrupada |
| Función no encontrada (404) | Ejecuta `netlify init` e `npm run deploy` nuevamente |
| Variables de entorno no disponibles | Establece las variables en CLI o en el tablero, luego vuelve a desplegar |
