...
Edita `wrangler.toml` y establece tu nombre de trabajador:
...
3. **Registra el evento** - Almacena el webhook en crudo en la tabla `webhook_events` para el registro de auditoría
...
| Trabajador no desplegando | Ejecuta `npm install`, verifica la sintaxis de `wrangler.toml` |
