...
1. Ve al [DodoPayments Dashboard](https://app.dodopayments.com) → Desarrollador → Webhooks
...
> **Nota:** Esta implementación demuestra el manejo de tres eventos de suscripción principales (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con campos mínimos. Puedes extenderlo fácilmente para soportar tipos de eventos y campos adicionales según tus requisitos.
...
1. **Desactiva el análisis del cuerpo** - Para acceder al cuerpo en bruto para la verificación de firma
2. **Verifica la firma** - Asegura que la solicitud provenga de DodoPayments usando HMAC-SHA256
3. **Verifica duplicados** - Usa el ID de webhook para prevenir el procesamiento del mismo evento dos veces
4. **Registra el evento** - Almacena el webhook en bruto en la tabla `webhook_events` para la auditoría
5. **Procesa el evento** - Crea o actualiza clientes y suscripciones en Neon
6. **Maneja errores** - Registra fallos y marca el evento como no procesado para reintentar
