...
<Card title="Repositorio de GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/vercel">
...
1. Ve a [DodoPayments Dashboard](https://app.dodopayments.com) → Desarrollador → Webhooks
...
✅ **Verificación de firma** - Usando la biblioteca de dodopayments  
...
> **Nota:** Esta implementación demuestra el manejo de tres eventos de suscripción principales (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con campos mínimos. Puedes extenderlo fácilmente para admitir tipos de eventos y campos adicionales según tus requerimientos.
...
1. **Desactiva el análisis del cuerpo** - Para acceder al cuerpo en crudo para la verificación de la firma
2. **Verifica la firma** - Asegura que la solicitud provenga de DodoPayments usando HMAC-SHA256
3. **Verifica duplicados** - Usa el ID de webhook para evitar procesar el mismo evento dos veces
4. **Registra el evento** - Almacena el webhook en crudo en la tabla `webhook_events` para auditoría
5. **Procesa el evento** - Crea o actualiza clientes y suscripciones en Neon
6. **Maneja errores** - Registra fallos y marca el evento como no procesado para reintento
