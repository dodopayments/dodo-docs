...
1. **Limpiar caché de autenticación MCP**: `rm -rf ~/.mcp-auth`
...
<Tip>
Usa `@latest` para obtener siempre la versión más reciente, o fija a una versión específica como `@1.0.0` para estabilidad.
</Tip>
...
Después de hacer clic, establece tus variables de entorno en `mcp.json` de **Configuraciones de Cursor** > **Herramientas y MCP** > **Nuevo servidor MCP**.

**Configuración Manual**

<Steps>
<Step title="Abrir configuraciones de Cursor">
    Navega a **Configuraciones de Cursor** > **Características** > **Protocolo de Contexto del Modelo** o presiona `Cmd/Ctrl + Shift + P` y busca "Configuraciones MCP".
</Step>
...
| `x-dodo-payments-api-key` | Cabecera de clave API directa |
...
| `claude` | Claude AI web o escritorio |
...
<Tip>
Usa `--operation=read` durante el desarrollo para prevenir modificaciones accidentales a tus datos.
</Tip>
...
Al implementar de forma remota, siempre requiere autenticación a través de la cabecera `Authorization` o cabecera `x-dodo-payments-api-key`.
...
Algunos clientes no son compatibles con todas las características del esquema JSON. Usa `--describe-capabilities` para entender qué está soportado.
</Accordion>
</AccordionGroup>
