...
<Info>
Este boilerplate utiliza Go 1.21+ con patrones de arquitectura limpia (`cmd`, `internal`, `templates`), plantillas HTML y el SDK `dodopayments-go` para una integración de API sin problemas.
</Info>
...
- **Manejo de Webhooks** - Verificar y procesar eventos de pago de forma segura
- **Portal del Cliente** - Gestión de suscripciones de autoservicio
...
<Warning>
Nunca comitas tu archivo `.env` al control de versiones. Ya está incluido en `.gitignore`.
</Warning>
</Step>
...
| `/api/checkout` | POST | Crear una nueva sesión de checkout |
...
- IDs de producto (desde tu tablero Dodo)
- Precios
- Características
- Descripciones
...
En `templates/index.html`, reemplaza los datos del cliente codificados con los datos reales de tus usuarios:
...
- Usa claves API de producción desde el tablero
...
<Accordion title="La creación de la sesión de checkout falla">
**Causas comunes:**
- ID de producto inválido - verifica que exista en tu tablero Dodo
- Clave API o configuración de entorno incorrecta en `.env`
- Revisa los registros del servidor para obtener mensajes de error detallados
</Accordion>
...
Actualiza la URL del webhook en tu [tablero Dodo](https://app.dodopayments.com/developer/webhooks) a la URL de ngrok. Asegúrate de actualizar tu archivo `.env` con la clave de verificación de webhook correcta.
</Accordion>
