...
1. **`subscription.active`** - La suscripción se ha activado con éxito.
2. **`subscription.updated`** - El objeto de suscripción fue actualizado (se activa en cualquier cambio de campo).
3. **`subscription.on_hold`** - La suscripción está en espera debido a la renovación fallida.
...
<Tip>
Utiliza `subscription.updated` para recibir notificaciones en tiempo real sobre cualquier cambio en la suscripción, manteniendo el estado de tu aplicación sincronizado sin necesidad de hacer polling a la API.
</Tip>
...
- **El pago de renovación falla**: El cargo de renovación automática falla debido a fondos insuficientes, tarjeta expirada o rechazo del banco
- **El cargo por cambio de plan falla**: Un cargo inmediato durante la actualización/bajada de plan falla
- **La autorización del método de pago falla**: El método de pago no puede ser autorizado para cargos recurrentes
...
Para reactivar una suscripción desde el estado `on_hold`, utiliza la API de Actualización de Método de Pago. Esto automáticamente:
...
| `timestamp` | string | Sí | La marca de tiempo de cuándo ocurrió el evento (no necesariamente la misma que cuando fue entregado) |
...
## Cambiando Planes de Suscripción
...
#### 2. `full_immediately`
- Cobra al cliente el monto completo de suscripción por el nuevo plan
- Ignora cualquier tiempo restante o créditos del plan anterior
- Útil cuando deseas reiniciar el ciclo de facturación o cobrar el monto completo independientemente de la prorrata

#### 3. `difference_immediately`
- Al actualizar, se cobra inmediatamente al cliente la diferencia entre los dos montos del plan.
- Por ejemplo, si el plan actual es de 30 Dólares y el cliente actualiza a un plan de 80 Dólares, se le cobra $50 instantáneamente. 
...
<Tip>
**Elige tu opción de prorrata cuidadosamente**: Usa `prorated_immediately` para una facturación justa que tenga en cuenta el tiempo no utilizado, o `full_immediately` cuando desees cobrar el monto completo del nuevo plan independientemente del ciclo de facturación actual.
</Tip>
