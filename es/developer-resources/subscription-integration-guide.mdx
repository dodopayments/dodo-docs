...
2. **`subscription.on_hold`** - La suscripción se pone en espera debido a una renovación fallida.
3. **`subscription.failed`** - La creación de la suscripción falló durante la creación del mandato.
4. **`subscription.renewed`** - La suscripción se renueva para el próximo período de facturación.
...
1. Fallo de Suscripción
  - `subscription.failed` - La creación de la suscripción falló debido a la falta de creación de un mandato.
...
<Info>
  Las suscripciones a demanda te permiten cobrar a los clientes de manera flexible, no solo en un horario fijo. Contacta al soporte para habilitar esta función.
</Info>

**Para crear una suscripción a demanda:**

Para crear una suscripción a demanda, utiliza el endpoint de API [POST /subscriptions](/api-reference/subscriptions/post-subscriptions) e incluye el campo `on_demand` en el cuerpo de tu solicitud. Esto te permite autorizar un método de pago sin un cargo inmediato, o establecer un precio inicial personalizado.

**Para cobrar una suscripción a demanda:**
...
<Note>
Para una guía completa, paso a paso—incluyendo ejemplos de solicitud/respuesta, políticas de reintento seguras y manejo de webhooks—consulta la <a href="/developer-resources/ondemand-subscriptions">Guía de Suscripciones a Demanda</a>.
</Note>
