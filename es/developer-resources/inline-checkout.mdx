...
| Campo | Descripción |
|-------|-------------|
| `subTotal` | La suma de todas las partidas de la sesión antes de aplicar descuentos o impuestos. |
| `discount` | El valor total de todos los descuentos aplicados. |
| `tax` | El importe de impuestos calculado. En el modo `inline`, este valor se actualiza dinámicamente a medida que el usuario interactúa con los campos de dirección. |
| `total` | El resultado matemático de `subTotal - discount + tax` en la moneda base de la sesión. |
| `currency` | El código de moneda ISO (por ejemplo, `"USD"`) para los valores estándar de subtotal, descuento e impuestos. |
| `finalTotal` | El importe real que se cobra al cliente. Esto puede incluir ajustes adicionales de divisas o comisiones de métodos de pago locales que no forman parte del desglose básico del precio. |
| `finalTotalCurrency` | La moneda en la que el cliente realiza el pago. Puede diferir de `currency` si la [paridad del poder adquisitivo](/features/purchasing-power-parity) o la conversión a moneda local están activas. |
...
<Info>
La verificación del dominio solo es necesaria para el **checkout integrado (embedded)**. No es necesaria para el checkout alojado.
</Info>
...
Ve a **Settings → Payment Methods** y, en la fila de **Apple Pay**, haz clic en **Manage domains**.
...
Haz clic en **Register domain** e introduce el dominio donde insertas el checkout integrado (por ejemplo, `shop.example.com`) y, a continuación, haz clic en **Continue**.
...
| Función | Checkout integrado | Checkout superpuesto |
|---------|--------------------|---------------------|
| Nivel de integración | Completamente integrado en la página | Modal sobre la página |
| Control del diseño | Control total | Limitado |
| Marca | Integrada de forma fluida | Separada de la página |
...
<Tip>
Usa el **checkout integrado** cuando quieras tener el máximo control sobre la experiencia de checkout y una marca integrada de forma fluida. Usa el **checkout superpuesto** para lograr una integración más rápida con cambios mínimos en tus páginas existentes.
</Tip>
