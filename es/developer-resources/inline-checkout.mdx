...
<Info>
La verificación de dominio solo es necesaria para el **checkout incrustado (embebido)**. El checkout en superposición, el checkout alojado y los enlaces de pago no lo necesitan.
</Info>

Apple Pay se verifica por dominio desde el panel de control.

<Steps>
<Step title="Open Wallet domains">
Ve a **Configuración → Métodos de Pago** y, en la fila de **Apple Pay**, haz clic en **Administrar dominios**.

<Frame caption="Open Wallet domains from the Apple Pay row">
  <img src="/images/apple-pay-domains/manage-domains.png" alt="Botón de administrar dominios en la fila de Apple Pay en la configuración de Métodos de Pago" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
Haz clic en **Registrar dominio** e introduce el dominio donde incrustas el checkout embebido (ejemplo: `shop.example.com`), luego **Continuar**.
...
Debe ser servido sobre HTTPS, accesible sin redireccionamientos, y servido con `Content-Type: application/octet-stream` o `text/plain`.
</Step>

<Step title="Verify the domain">
Haz clic en **Verificar dominio**. Dodo Payments confirma que el archivo está activo y envía tu dominio a Apple.

<Frame caption="Verify the hosted association file">
  <img src="/images/apple-pay-domains/verify-domain.png" alt="Pantalla de verificar tu dominio con la ruta de alojamiento del archivo de asociación y el botón de verificar dominio" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Confirm it's active">
Cuando el estado muestra **Activo**, Apple Pay está habilitado para ese dominio. Usa el interruptor de **Habilitado** para activarlo o desactivarlo por dominio.

<Frame caption="Verified domains show an Active status">
  <img src="/images/apple-pay-domains/domains-active.png" alt="Lista de dominios de Wallet mostrando dominios con estado de Apple Pay Activo e interruptores de Habilitado" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
El SDK de Checkout de Dodo Payments es compatible con los siguientes navegadores:
...
## Checkout Embebido vs Checkout en Superposición

Elige el tipo de checkout adecuado para tu caso de uso:

| Característica | Checkout Embebido | Checkout en Superposición |
|----------------|-------------------|--------------------------|
...
| Marca | Integración sin fisuras | Separado de la página |
...
<Tip>
Usa **checkout embebido** cuando deseas el máximo control sobre la experiencia de checkout y una marca sin fisuras. Usa **checkout en superposición** para una integración más rápida con cambios mínimos a tus páginas existentes.
</Tip>
...
<Card title="Overlay Checkout" icon="layer-group" href="/developer-resources/overlay-checkout">
    Usa el checkout en superposición para una integración rápida basada en modal.
</Card>
...
<Card title="Webhooks" icon="webhook" href="/developer-resources/webhooks">
    Maneja eventos de pago en el servidor con webhooks.
</Card>
