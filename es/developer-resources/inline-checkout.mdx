...
Inicializa el SDK y especifica `displayType: 'inline'`. También debes escuchar el evento `checkout.breakdown` para actualizar tu interfaz de usuario con cálculos de impuestos y totales en tiempo real.
...
<Check>
Deberías ver eventos `checkout.breakdown` registrados en la consola de tu navegador si agregaste un registro en la función de callback `onEvent`.
</Check>
...
Este ejemplo demuestra cómo implementar un resumen de pedido personalizado junto con el proceso de pago en línea, manteniéndolos sincronizados utilizando el evento `checkout.breakdown`.
...
|--------|------|----------|-------------|
| `mode` | `"test" \| "live"` | Sí | Modo de ambiente. |
| `displayType` | `"inline" \| "overlay"` | Sí | Debe establecerse en `"inline"` para incrustar el proceso de pago. |
...
| `options.showTimer` | `boolean` | No | Mostrar u ocultar el temporizador de pago. Por defecto, está en `true`. Cuando está deshabilitado, recibirás el evento `checkout.link_expired` cuando expira la sesión. |
| `options.showSecurityBadge` | `boolean` | No | Mostrar u ocultar la insignia de seguridad. Por defecto, está en `true`. |
| `options.manualRedirect` | `boolean` | No | Cuando está habilitado, el proceso de pago no redirigirá automáticamente después de completarse. En su lugar, recibirás eventos `checkout.status` y `checkout.redirect_requested` para manejar la redirección tú mismo. |
| `options.themeConfig` | `ThemeConfig` | No | Configuración de tema personalizado. |
...
Al usar `manualRedirect`, maneja la finalización del proceso de pago en tu función de callback `onEvent`:
...
El SDK proporciona eventos en tiempo real a través de la función de callback `onEvent`. Para el proceso de pago en línea, `checkout.breakdown` es particularmente útil para sincronizar tu interfaz de usuario.

| Tipo de Evento | Descripción |
|------------|-------------|
| `checkout.opened` | El marco de pago se ha cargado. |
| `checkout.form_ready` | El formulario de pago está listo para recibir la entrada del usuario. Útil para esconder estados de carga y mostrar la interfaz de pago. |
| `checkout.breakdown` | Disparado cuando se actualizan precios, impuestos o descuentos. |
| `checkout.customer_details_submitted` | Se han enviado los detalles del cliente. |
| `checkout.pay_button_clicked` | Disparado cuando el cliente hace clic en el botón de pago. Útil para análisis y seguimiento de embudos de conversión. |
| `checkout.redirect` | El proceso de pago realizará una redirección (por ejemplo, a una página bancaria). |
| `checkout.error` | Ocurrió un error durante el proceso de pago. |
| `checkout.link_expired` | Disparado cuando la sesión de pago expira. Solo se recibe cuando `showTimer` se establece en `false`. |
| `checkout.status` | Disparado cuando `manualRedirect` está habilitado. Contiene el estado del pago (`succeeded`, `failed` o `processing`). |
| `checkout.redirect_requested` | Disparado cuando `manualRedirect` está habilitado. Contiene la URL a la que redirigir al cliente. |
...
El evento `checkout.breakdown` es la forma principal de mantener sincronizada la interfaz de usuario de tu aplicación con el estado de pago de Dodo Payments.
...
| Campo | Descripción |
|-------|-------------|
| `subTotal` | La suma de todos los artículos en la sesión antes de aplicar descuentos o impuestos. |
| `discount` | El valor total de todos los descuentos aplicados. |
| `tax` | El monto de impuesto calculado. En el modo `inline`, esto se actualiza dinámicamente a medida que el usuario interactúa con los campos de dirección. |
| `total` | El resultado matemático de `subTotal - discount + tax` en la moneda base de la sesión. |
| `currency` | El código de moneda ISO (por ejemplo, `"USD"`) para los valores de subtotal estándar, descuento e impuesto. |
| `finalTotal` | El monto real que se cobra al cliente. Esto puede incluir ajustes de cambio de divisas adicionales o tarifas de métodos de pago locales que no son parte del desglose de precios básico. |
| `finalTotalCurrency` | La moneda en la que el cliente está pagando realmente. Esto puede diferir de `currency` si la paridad del poder adquisitivo o la conversión a moneda local están activas. |

**Consejos Clave de Integración:**

1.  **Formato de moneda**: Los precios siempre se devuelven como enteros en la unidad de moneda más pequeña (por ejemplo, centavos para USD, yenes para JPY). Para mostrarlos, divide por 100 (o la potencia adecuada de 10) o utiliza una biblioteca de formato como `Intl.NumberFormat`.
2.  **Manejo de estados iniciales**: Cuando el proceso de pago se carga por primera vez, `tax` e `discount` pueden estar `0` o `null` hasta que el usuario proporcione su información de facturación o aplique un código. Tu interfaz de usuario debe manejar estos estados de manera adecuada (por ejemplo, mostrando un guion `—` o ocultando la fila).
3.  **El "Total Final" vs "Total"**: Mientras `total` te da el cálculo de precio estándar, `finalTotal` es la fuente confiable para la transacción. Si `finalTotal` está presente, refleja exactamente lo que se cobrará a la tarjeta del cliente, incluyendo cualquier ajuste dinámico.
4.  **Retroalimentación en tiempo real**: Usa el campo `tax` para mostrar a los usuarios que se están calculando impuestos en tiempo real. Esto brinda una sensación de "en vivo" a tu página de pago y reduce la fricción durante el paso de entrada de dirección.
...
Puedes personalizar la apariencia del proceso de pago pasando un objeto `themeConfig` en el parámetro `options` al abrir el proceso de pago. La configuración del tema admite modos claro y oscuro, permitiéndote personalizar colores, bordes, texto, botones y radio de borde.

<Info>
Esta sección cubre la configuración del tema **del lado del cliente** usando el SDK de Checkout. También puedes configurar temas **del lado del servidor** al crear una sesión de pago a través de la API utilizando el parámetro `theme_config`. Consulta [Personalización del Tema de Pago](/features/checkout#checkout-theme-customization) para la configuración a nivel de API.
</Info>
...
Puedes anular solo propiedades específicas. El proceso de pago utilizará valores predeterminados para las propiedades que no especifiques:
...
Para usuarios de TypeScript, se exportan todos los tipos de configuración de tema:
...
1. Llama a la [API de Actualización del Método de Pago](/features/subscription#update-payment-method-for-active-subscription) para obtener un `payment_link`:
...
El marco en línea renderiza solo el formulario de colección de método de pago. Los clientes pueden ingresar nuevos detalles de tarjeta o seleccionar un método de pago guardado sin salir de tu página.

### Para Suscripciones en Espera

Al actualizar el método de pago para una suscripción en estado `on_hold`, Dodo Payments crea automáticamente un cargo por cualquier cuota restante. Monitorea los webhooks `payment.succeeded` y `subscription.active` para confirmar la reactivación.
...
<Tip>
También puedes usar un método de pago guardado existente en lugar de recopilar nuevos detalles pasando `type: 'existing'` con un `payment_method_id` a la API de Actualización del Método de Pago.
</Tip>

## Manejo de Errores

El SDK proporciona información detallada de errores a través del sistema de eventos. Siempre implementa un manejo de errores adecuado en tu función de callback `onEvent`:
...
1. **Diseño Responsivo**: Asegúrate de que tu elemento contenedor tenga suficiente ancho y altura. El iframe generalmente se expandirá para llenar su contenedor.
2. **Sincronización**: Utiliza el evento `checkout.breakdown` para mantener tu resumen de pedido personalizado o tablas de precios sincronizadas con lo que el usuario ve en el marco de pago.
3. **Estados de Carga**: Muestra un indicador de carga en tu contenedor hasta que se dispare el evento `checkout.opened`.
...
<Warning>
No confíes únicamente en eventos de pago en línea para determinar el éxito o fracaso del pagamento. Siempre implementa validación del lado del servidor utilizando webhooks y/o polling.
</Warning>

### Por Qué la Validación del Lado del Servidor es Esencial

Mientras que los eventos de pago en línea como `checkout.status` brindan retroalimentación en tiempo real, **no** deben ser tu única fuente de verdad para el estado del pago. Problemas de red, cierres del navegador o usuarios cerrando la página pueden causar que se pierdan eventos. Para asegurarte de una validación del pago confiable:

1. **Tu servidor debe escuchar eventos de webhook** - Dodo Payments envía webhooks para cambios en el estado del pago.
2. **Implementa un mecanismo de polling** - Tu frontend debe consultar a tu servidor por actualizaciones de estado.
3. **Combina ambos enfoques** - Utiliza webhooks como fuente principal y polling como respaldo.
...
**1. Escucha eventos de pago** - Cuando el usuario haga clic en pagar, comienza a prepararte para verificar el estado:
...
**2. Consulta a tu servidor** - Crea un endpoint que verifique tu base de datos por el estado de pago (actualizado por webhooks):
...
2. Comienza a consultar a tu servidor por el estado de pago confirmado
3. Muestra un estado de "Verificando pago..." mientras consultas
4. Muestra la interfaz de éxito/fracaso basado en el estado confirmado por el servidor
...
- Verifica que el `checkoutUrl` es válido.
</Accordion>

<Accordion title="Los impuestos no se están actualizando en mi interfaz">
...
## Habilitación de Monederos Digitales

Para información detallada sobre cómo configurar Apple Pay, Google Pay y otros monederos digitales, consulta la página de <a href="/features/payment-methods/digital-wallets">Monederos Digitales</a>.

### Configuración Rápida para Apple Pay

<Steps>
<Step title="Descargar archivo de asociación de dominio">
...
<Step title="Prueba después de la confirmación">
Una vez confirmado, verifica que Apple Pay aparezca en el pago y prueba todo el flujo.
</Step>
</Steps>

<Warning>
Apple Pay requiere verificación de dominio antes de que aparezca en producción. Contacta al soporte antes de activar si planeas ofrecer Apple Pay.
</Warning>
...
## Pago en Línea vs Pago Emergente

Elige el tipo de pago adecuado para tu caso de uso:

| Característica | Pago en Línea | Pago Emergente |
|---------|-----------------|------------------|
...
<Tip>
Usa **pago en línea** cuando quieras el máximo control sobre la experiencia de pago y presentación de marca. Usa **pago emergente** para una integración más rápida con cambios mínimos en tus páginas existentes.
</Tip>

## Recursos Relacionados

<CardGroup cols={2}>
<Card title="Pago Emergente" icon="layer-group" href="/developer-resources/overlay-checkout">
    Usa el pago emergente para una integración rápida basada en modal.
</Card>
