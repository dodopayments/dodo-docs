---
title: Adaptador de Astro
description: Aprende a integrar <CardGroup cols={2}> con tu proyecto Astro App
  Router usando nuestro adaptador de Astro. Incluye checkout, portal de
  clientes, webhooks y configuración segura del entorno.
icon: code-merge
keywords:
  - Dodo Payments
  - Astro
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Integra el checkout de Dodo Payments en tu aplicación Astro.
</Card>
...
<Info>
Todos los ejemplos asumen que utilizas Astro App Router.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Utiliza este handler para integrar el checkout de Dodo Payments en tu aplicación Astro. Admite flujos de pago estáticos (GET), dinámicos (POST) y de sesión (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Customer Portal Handler">
  <Info>
  Utiliza este handler para permitir que los clientes gestionen sus suscripciones y datos mediante el portal de clientes de Dodo Payments.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Utiliza este handler para recibir y procesar de forma segura los eventos de webhook de Dodo Payments en tu aplicación Astro.
  </Info>
  <CodeGroup>
...
- **Enlaces de pago estáticos:** URL que se pueden compartir al instante para recopilar pagos rápidamente y sin código.
...
Fija el importe cobrado, en unidades monetarias mayores (p. ej., <code>12.5</code> para 12,50 USD). Solo para productos Pay What You Want; se ignora si es inferior al precio mínimo del producto.
</ParamField>
...
<Warning>
Si falta <code>productId</code>, el handler devuelve una respuesta 400. Los parámetros de consulta no válidos también generan una respuesta 400.
</Warning>
...
- Para consultar la lista completa de campos compatibles del cuerpo POST, visita:
...
Las sesiones de checkout proporcionan una experiencia de checkout alojada y más segura que gestiona el flujo de pago completo, tanto para compras únicas como para suscripciones, con control total de personalización.

Consulta la [guía de integración de Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session) para obtener más detalles y la lista completa de campos compatibles.
...
El Route Handler de Customer Portal te permite integrar sin problemas el portal de clientes de Dodo Payments en tu aplicación Astro.
