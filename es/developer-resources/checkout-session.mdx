...
Forzar a la interfaz de pago a renderizar en un idioma específico.
...
Recopilar números de teléfono de clientes durante el pago.
</ParamField>
...
Descripción opcional del producto para la facturación y los artículos de línea. Si no se especifica, se usará la descripción almacenada del producto.
</ParamField>
...
Para obtener información detallada sobre la configuración y prueba de UPI, consulte la página de <a href="/features/payment-methods/india">Métodos de Pago en India</a>.
...
Para obtener información detallada sobre la configuración y prueba de BNPL, consulte la página de <a href="/features/payment-methods/bnpl">Compra Ahora, Paga Después (BNPL)</a>.
...
Con las Sesiones de Checkout, esto ya no es necesario. Simplemente puede proporcionar la información que tenga, y nosotros nos encargaremos del resto. Por ejemplo:
...
<Steps>
<Step title="Actualice su integración">
Actualice su integración para usar el nuevo método de API o SDK.
</Step>

<Step title="Ajuste la carga útil de la solicitud">
...
Forzar que el proceso de pago se muestre en un idioma específico, anulando la detección del idioma del navegador del cliente:
...
<Tip>
Utiliza `force_language` cuando conozcas el idioma preferido de tu cliente (por ejemplo, desde la configuración de su cuenta) o cuando te dirijas a mercados regionales específicos.
</Tip>
...
### 15. Recopilación de campos personalizados

Recopila información adicional de los clientes durante el proceso de pago utilizando campos personalizados:
...
<Info>
Las respuestas de los campos personalizados se incluyen automáticamente en las cargas útiles de los webhooks (`payment.succeeded`, `subscription.created`, etc.) y se pueden recuperar a través de la API. Úsalos para enriquecer tu CRM, activar flujos de incorporación o personalizar la experiencia del cliente.
</Info>

**Tipos de campo disponibles:** `text`, `number`, `email`, `url`, `date`, `dropdown`, `boolean`

## Vista previa de sesiones de pago

Antes de crear una sesión de pago, puedes obtener una vista previa del desglose de precios, incluidos impuestos, descuentos y totales. Esto es útil para mostrar precios precisos a los clientes antes de que procedan al pago.
...
Anteriormente, al crear un enlace de pago con Enlaces dinámicos, debías proporcionar la dirección de facturación completa del cliente.

Con las sesiones de pago, esto ya no es necesario. Puedes simplemente enviar la información que tengas y nosotros nos encargamos del resto. Por ejemplo:
- Si solo conoces el país de facturación del cliente, solo proporciona eso.
- El flujo de pago recopilará automáticamente los detalles faltantes antes de dirigir al cliente a la página de pago.
- Por otro lado, si ya tienes toda la información requerida y quieres pasar directamente a la página de pago, puedes enviar el conjunto completo de datos e incluir `confirm=true` en el cuerpo de tu solicitud.
...
Actualiza tu integración para utilizar el nuevo método de API o SDK.
</Step>

{/* LOCKED_PATTERN_9a0d7dfb0d22a80cdd653a6f423ff618 */}
Ajusta la carga útil de la solicitud según el formato de las sesiones de pago.
</Step>

{/* LOCKED_PATTERN_ea09e0cebc00c36bba010ba9740abe0d */}
Sí. No se necesita ningún manejo adicional ni pasos especiales de migración de tu parte.
</Step>
</Steps>
