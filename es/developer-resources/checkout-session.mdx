...
**Formato**: Incluye el código del país, por ejemplo, `"+1234567890"` para números de EE. UU.
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
<Note>
El anulación de `billing_currency` solo tiene efecto cuando la moneda adaptativa está habilitada en la configuración de tu cuenta. Si la moneda adaptativa está deshabilitada, este parámetro no tendrá efecto.
</Note>
...
<Warning>
Al usar `payment_method_id`, `confirm` debe configurarse en `true` y debe proporcionarse un `customer_id` existente. El método de pago será validado para su elegibilidad con la moneda del pago.
</Warning>
