...
`customization` en `CheckoutParams`. Las opciones están agrupadas por plataforma porque
...
`standard` muestra el icono de sistema "X"; `back` dibuja una flecha de retroceso en su lugar.
</ParamField>
...
Permite que la barra de herramientas se contraiga al desplazarse. Solo está visible cuando `presentationStyle` es `fullScreen`; `pageSheet` mantiene las barras fijadas independientemente de esta configuración.
</ParamField>
