...
<Card title="Context Support" icon="clock">
  Compatibilidad total con `context.Context` para cancelaciones y tiempos de espera
</Card>
...
<Info>
`Billing` requiere como mínimo el código ISO de dos letras `Country`. `Customer` es un `CustomerRequestUnionParam`: pasa `AttachExistingCustomerParam{CustomerID: ...}` para un cliente existente o `NewCustomerParam{Email: ..., Name: ...}` para uno nuevo. `ProductPrice` se expresa en la denominación más baja de la moneda.
</Info>
...
Cuando la API devuelve un código de estado que no indica éxito, el SDK devuelve un error de tipo
`*dodopayments.Error`. Expone `StatusCode`, el `*http.Request` subyacente y
`*http.Response`, además del JSON del cuerpo del error. Usa el patrón `errors.As` para
inspeccionarlo y evalúa `StatusCode` para gestionar casos específicos:
...
Añade middleware personalizado para el registro o las métricas:
...
El cliente admite un uso simultáneo seguro:
...
¡Damos la bienvenida a las contribuciones! Consulta las [directrices para contribuir](https://github.com/dodopayments/dodopayments-go/blob/main/CONTRIBUTING.md) para comenzar.
