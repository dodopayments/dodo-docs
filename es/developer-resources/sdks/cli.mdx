...
`dodo wh trigger` puede enviar payloads simulados para estos 22 tipos de eventos:
...
<Note>
Este es un subconjunto de los eventos que Dodo Payments entrega. `payout.*`, `credit.*`, `abandoned_checkout.*`, `dunning.*`, `entitlement_grant.*`, `subscription.paused` y `subscription.update_payment_method` aún no tienen un generador de simulaciones; pruébalos con actividad real en modo de prueba y `dodo wh listen`. Consulta [Webhook Events](/developer-resources/webhooks/intents/webhook-events-guide) para ver la lista completa.
</Note>
...
La CLI comprueba si hay una versión más reciente al iniciarse y muestra una notificación en la barra de estado cuando hay una disponible. Para actualizar:
