...
<Info>
`billing` requiere como mínimo el código ISO de dos letras `country`. Pasa `AttachExistingCustomer::with(customerID: '...')` para asociar un cliente existente, o `NewCustomer::with(email: '...', name: '...')` para crear uno. `productPrice` está expresado en la denominación monetaria más baja.
</Info>
...
<Tip>
  Envuelve siempre las llamadas a la API en bloques try-catch para gestionar los posibles errores
  de forma adecuada y proporcionar información útil a los usuarios.
</Tip>
...
Envía parámetros no documentados a cualquier endpoint o lee propiedades de respuesta no documentadas:
...
<Note>
  Los parámetros `extra*` con el mismo nombre reemplazan a los parámetros documentados.
</Note>
...
Añade la configuración en `config/services.php`:
...
¡Damos la bienvenida a las contribuciones! Consulta las [pautas para contribuir](https://github.com/dodopayments/dodopayments-php/blob/main/CONTRIBUTING.md) para empezar.
