...
<Info>
Estamos usando "Suma" porque queremos sumar todos los tokens consumidos en múltiples mensajes de chat. El SDK envía automáticamente `totalTokens` en cada evento.
</Info>
</Step>
...
<Steps>
<Step title="Obtener la clave de API de Dodo Payments">
1. En tu panel de Dodo Payments, ve a **Desarrolladores** → **Claves de API**
2. Haz clic en **Crear clave de API**
3. Copia la clave de API - se verá como `test_abc123...`

<Note>
Guarda esta clave de API - la agregaremos a nuestro archivo `.env` más tarde.
</Note>
</Step>
...
Además, establece el **Precio Fijo** en `0` para asegurar que los clientes solo sean cargados según su uso, sin tarifa base.
...
**Precio por unidad**: Ingrese → `0.00001` (esto es $0.01 por 1,000 tokens o $0.00001 por token)

**Umbral Gratuito**: Ingrese → `10000` (los clientes obtienen 10,000 tokens gratis por mes)
...
1. Verifica que tu archivo `.env` tenga todas las claves de API del Paso 2
...
- ID del cliente: Tu ID de cliente de prueba
...
2. Verifica que el ID del cliente exista en el panel de Dodo Payments
3. Prueba la clave de API con una llamada API simple
...
2. Actualiza a la última SDK de Blueprints: `npm install @dodopayments/ingestion-blueprints@latest`
</Accordion>

<Accordion title="Error de clave de API inválida">
**Posibles causas:**
- Clave de API incorrecta para el entorno
- Espacios o comillas extra en el archivo `.env`

**Soluciones:**
- Asegúrate de que la clave de prueba comience con `test_`, la clave en vivo comience con `live_`
- Elimina las comillas alrededor de las claves en el archivo `.env`
