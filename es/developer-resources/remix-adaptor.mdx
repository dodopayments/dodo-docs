...
Especificar la moneda de pago (p. ej., <code>USD</code>).
</ParamField>
...
Fija el importe cobrado, en unidades principales de la moneda (p. ej., <code>12.5</code> para 12,50 USD). Solo para productos Pay What You Want; se ignora si es inferior al precio mínimo del producto.
</ParamField>
...
<Warning>
Dynamic Checkout actúa como proxy de los endpoints `POST /payments` y `POST /subscriptions` obsoletos. Sigue funcionando para las integraciones existentes, pero las nuevas integraciones deben usar **Checkout Sessions** a continuación.
</Warning>
...
Checkout Sessions ofrece una experiencia de checkout alojada más segura que gestiona el flujo de pago completo tanto para compras únicas como para suscripciones, con control total de la personalización.

Consulta la [Guía de integración de Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session) para obtener más detalles y una lista completa de los campos compatibles.
...
El manejador de rutas de Customer Portal te permite integrar sin problemas el portal de clientes de Dodo Payments en tu aplicación de Remix.
...
- **Enrutamiento de eventos:** Llama al manejador de eventos adecuado según el tipo de payload.
