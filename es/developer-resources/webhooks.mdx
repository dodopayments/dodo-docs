...
<Expandable title="Data object properties">
  ...
  Tipo de recurso. Puede ser uno de los siguientes: `Payment`, `Subscription`, `Refund`, `Dispute`, `LicenseKey`, `CreditLedgerEntry`, `CreditBalanceLow`, `AbandonedCheckout`, `DunningAttempt`, `EntitlementGrant` o `Payout`.
  </ResponseField>
  
  Los campos adicionales varían según el tipo de evento. Consulta la documentación específica del evento para ver los schemas completos.
</Expandable>
</ResponseField>
...
La [Dodo Payments CLI](/developer-resources/sdks/cli) proporciona dos comandos para probar webhooks durante el desarrollo local sin necesidad de salir del terminal.
...
Esta herramienta interactiva te permite elegir un tipo de evento y envía un payload simulado realista a tu endpoint. Funciona en bucle para que puedas probar varios eventos en una misma sesión.

El comando trigger incluye los 46 tipos de eventos que Dodo Payments entrega, incluidas las familias de subscription, payment, refund, dispute, license key, payout, credit, abandoned checkout, dunning y entitlement grant. Consulta [Supported Webhook Events](/developer-resources/sdks/cli#supported-webhook-events) para ver la lista exacta.
...
Ve a **Developer → Webhooks** y abre la pestaña **Activity** para consultar el rendimiento de entrega de todos tus endpoints.
...
|------|--------------|--------------|
| **Recover failed messages** | Reproduce cada mensaje dirigido a este endpoint que haya fallado. | Un punto de inicio: **8 hours ago**, **Yesterday**, **3 days ago**, **Last week** o **2 weeks ago** |
...
| **Bulk replay messages** | Reproduce los mensajes que coincidan con los filtros elegidos, incluidos los que ya se entregaron correctamente. | Límites **Since** y **Until**, con un valor predeterminado de las últimas dos semanas, además de tipos de evento, canal o tag opcionales |
