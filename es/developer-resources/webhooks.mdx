...
<Steps>
<Step title="Acceder a la configuración de Webhook">
...
<Step title="Agregar URL del Endpoint">
    Ingresa la URL donde deseas recibir eventos de webhook.
    
</Step>

<Step title="Seleccionar eventos a recibir">
    Elige los eventos específicos a los que tu endpoint de webhook debe estar atento seleccionándolos de la lista de eventos.
...
<Step title="Obtener clave secreta">
...
<Step title="Rotar secreto (opcional)">
    ...
    <Warning>
    Rotar el secreto **lo expirará** y **lo reemplazará** por uno nuevo. El antiguo secreto solo será válido durante las próximas 24 horas. Después de eso, intentar verificar con el antiguo secreto fallará.
    </Warning>
...
<Tip>
Procesa los webhooks de manera asíncrona reconociendo la recepción inmediatamente con un código de estado `200`, y luego maneja el procesamiento real en segundo plano.
</Tip>

### Reintentos automáticos

Si una entrega de webhook falla, Dodo Payments reintenta automáticamente con retroceso exponencial para evitar abrumar tu sistema.
...
| 3 | 5 minutos | Tercer intento con un aumento de retroceso |
...
<Info>
Recibirás la **carga útil más reciente en el momento de la entrega**, independientemente de cuándo se emitió originalmente el evento de webhook.
</Info>
...
<Accordion title="Asegura tu secreto de webhook" icon="key">
  Almacena tu secreto de webhook de manera segura usando variables de entorno o un gestor de secretos. Nunca cometas secretos en el control de versiones.
</Accordion>
</AccordionGroup>
...
<Steps>
<Step title="Verificar tu endpoint">
    Monitorea los registros de tu endpoint de webhook para confirmar que el evento de prueba fue recibido.
</Step>
...
Aquí tienes una implementación completa de Express.js que muestra la verificación y el manejo de webhooks:
...
<Step title="Probar transformación">
    Usa la interfaz de prueba para verificar que tu transformación funcione correctamente antes de entrar en producción.
</Step>
</Steps>
...
Mantente informado sobre la salud de tu webhook con notificaciones automáticas por correo electrónico. Cuando las entregas de webhook comienzan a fallar o tu endpoint deja de responder, recibirás alertas por correo electrónico para que puedas abordar rápidamente los problemas y mantener tus integraciones funcionando sin problemas.
...
<Steps>
<Step title="Navegar a Configuración de Alertas">
...
<Step title="Habilitar Notificaciones por Correo Electrónico">
  Activa **Notificaciones por correo electrónico** para comenzar a recibir alertas sobre problemas de entrega de webhook.
</Step>

<Step title="Configurar Dirección de Correo Electrónico">
  Ingresa la dirección de correo electrónico donde deseas recibir alertas de webhook. Enviaremos notificaciones a esta dirección cuando ocurran ciertos eventos con la configuración de tus webhooks, como problemas de entrega que puedan afectar tus integraciones.
</Step>
</Steps>

<Tip>
Habilita las alertas por correo electrónico para detectar problemas de entrega de webhook temprano y mantener integraciones confiables. Serás notificado cuando las entregas fallen o tu endpoint se vuelva no responsivo.
</Tip>

## Desplegar en Plataformas en la Nube

¿Listo para desplegar tu manejador de webhook en producción? Proporcionamos guías específicas para cada plataforma para ayudarte a desplegar webhooks en proveedores de nube populares con las mejores prácticas para cada plataforma.

<CardGroup cols={2}>
  <Card title="Vercel" icon="triangle" href="/developer-resources/webhooks/examples/vercel-example">
    Desplegar webhooks en Vercel con funciones sin servidor
  </Card>
  <Card title="Cloudflare Workers" icon="cloud" href="/developer-resources/webhooks/examples/cloudflare-example">
    Ejecutar webhooks en la red de borde de Cloudflare
  </Card>
  ...
  <Card title="Netlify Functions" icon="circle-nodes" href="/developer-resources/webhooks/examples/netlify-example">
    Desplegar webhooks como funciones sin servidor de Netlify
  </Card>
</CardGroup>
