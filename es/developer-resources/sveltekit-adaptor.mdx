---
title: Adaptador de SvelteKit
description: Aprende a integrar Dodo Payments con tu proyecto SvelteKit App
  Router utilizando nuestro Adaptador de SvelteKit. Cubre el proceso de pago, el
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Integra el checkout de Dodo Payments en tu app SvelteKit.
</Card>
...
<Warning>
Nunca subas tu archivo <code>.env</code> ni secretos al control de versiones.
</Warning>
</Step>
</Steps>
...
<Info>
Todos los ejemplos asumen que estás usando el App Router de SvelteKit.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Utiliza este manejador para integrar el checkout de Dodo Payments en tu app SvelteKit. Soporta flujos de pago estáticos (GET), dinámicos (POST) y de sesión (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Customer Portal Handler">
  <Info>
  Utiliza este manejador para permitir que los clientes gestionen sus suscripciones y datos mediante el portal de clientes de Dodo Payments.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Utiliza este manejador para recibir y procesar de forma segura eventos webhook de Dodo Payments en tu app SvelteKit.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments admite tres tipos de flujos de pago para integrar pagos en tu sitio web; este adaptador los soporta todos.
</Info>
...
- Para obtener una lista completa de los campos compatibles en el cuerpo POST, consulta:
...
<Warning>
Dynamic Checkout actúa como proxy de los endpoints obsoletos `POST /payments` e `POST /subscriptions`. Sigue funcionando para las integraciones existentes, pero las nuevas integraciones deben usar **Checkout Sessions** que se indican a continuación.
</Warning>
...
Checkout sessions ofrecen una experiencia de checkout alojada más segura que gestiona el flujo de pago completo tanto para compras únicas como para suscripciones, con control total de personalización.
...
El Route Handler de Customer Portal te permite integrar sin problemas el portal de clientes de Dodo Payments en tu aplicación de SvelteKit.
