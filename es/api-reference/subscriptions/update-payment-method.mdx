...
Actualiza el método de pago para una suscripción. Este endpoint soporta tanto suscripciones activas como suscripciones en `on_hold` estado.

<Info>
Para suscripciones en `on_hold` estado, actualizar el método de pago crea automáticamente un cargo por las deudas restantes, genera una factura y reactiva la suscripción a `active` estado tras el pago exitoso.
</Info>
...
5. Recibirás eventos de webhook: `payment.succeeded` seguido por `subscription.active`
