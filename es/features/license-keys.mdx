...
Cuando un cliente compra un producto en modo manual, la concesión se crea en estado `pending` sin clave y un webhook [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) se activa con `integration_type: "license_key"` e `status: "pending"`. Puedes reaccionar a ese webhook o consultar el endpoint [List Customer Grants](/api-reference/entitlements/list-customer-grants) con los filtros `integration_type` e `status`:
