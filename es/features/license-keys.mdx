...
<Step title="Configurar los ajustes de la clave de licencia">
    Establezca cómo se emitirán y utilizarán sus claves de licencia especificando estos campos:
    
    - **Fecha de caducidad de la clave de licencia**: Defina cuánto tiempo se mantiene válida la clave. Elija una duración específica (por ejemplo, 30 días o 1 año), o seleccione "sin caducidad" para una clave perpetua.
    - **Límite de activación**: Establezca el número máximo de activaciones concurrentes por clave. Utilice un número específico para escenarios como Licencias de un solo usuario (1) o Licencias de equipo (5), o seleccione "ilimitado" para no tener un límite de activación.
...
Cuando un cliente completa una compra de un producto con claves de licencia habilitadas, la clave de licencia se anexa automáticamente a su `return_url` como un parámetro de consulta. Esto le permite mostrar la clave inmediatamente en su página de éxito sin realizar una llamada API adicional.

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001&email=customer%40example.com
```

Si la compra genera múltiples claves de licencia (por ejemplo, cantidad > 1), están separadas por comas:

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001,LK-002&email=customer%40example.com
```

Para suscripciones, se utiliza `subscription_id` en lugar de `payment_id`:
...
<Tip>
Analice el parámetro `license_key` en su página de retorno para mostrar la clave inmediatamente a los clientes, mejorando la experiencia posterior a la compra.
</Tip>

## Mejores Prácticas

- **Mantenga los límites claros**: Elija valores predeterminados sensatos para caducidad y activaciones
- **Guíe a los usuarios**: Proporcione instrucciones de activación precisas y documentación de autoservicio
- **Automatice las verificaciones**: Valide las claves del lado del servidor antes de otorgar acceso
- **Monitoree los eventos**: Utilice webhooks para detectar abusos y automatizar las revocaciones
