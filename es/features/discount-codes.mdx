...
- **Descuentos flexibles**: Descuentos basados en porcentajes
- **Códigos acumulables**: Aplica hasta 20 códigos por checkout, pago o suscripción — combina campañas (por ejemplo, `WELCOME10` + `BLACKFRIDAY20`) sin crear códigos a medida
- **Control dirigido**: Restringir por producto y ciclos de suscripción
- **Gobernanza de campaña**: Fechas de expiración y límites de uso
- **Checkout fluido**: Campo de UI y soporte API a través de sesiones de checkout
...
- **Nombre del descuento** (requerido): Nombre de visualización interna y en el panel
- **Código** (requerido): La cadena que los clientes ingresan en el checkout
- **Tipo y cantidad** (requerido): Establece un valor porcentual (actualmente solo se admiten descuentos basados en porcentajes), o genera un código aleatorio usando el botón proporcionado
- **Fecha de expiración** (opcional): Fecha después de la cual el código se vuelve inválido
- **Límite de uso** (opcional): Máximo de redenciones totales de todos los clientes
- **Restricción de productos** (opcional): Limita la aplicabilidad a productos seleccionados
- **Límite de ciclos de suscripción** (opcional): Número de ciclos de facturación a los que se aplica el descuento
- **Metadatos** (opcional): Adjunta pares de clave-valor personalizados para seguimiento interno o integraciones
