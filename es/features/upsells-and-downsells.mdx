...
Dodo Payments permite estos flujos a través del parámetro `payment_method_id`, que te permite cargar un método de pago guardado de un cliente sin requerir que ingresen nuevamente los detalles de la tarjeta.

### Beneficios Clave

| Beneficio | Impacto |
|-----------|--------|
| **Compras con un clic** | Omite completamente el formulario de pago para clientes recurrentes |
...
| **Experiencia de usuario fluida** | Los clientes permanecen en tu aplicación durante todo el flujo |
...
| `full_immediately` | Carga el precio completo del nuevo plan, ignora el tiempo restante | Reinicios del ciclo de facturación |

<Tip>
Usa `difference_immediately` para flujos de actualización sencillos. Usa `prorated_immediately` cuando desees tener en cuenta el tiempo no utilizado en el plan actual.
</Tip>
...
<Info>
Los créditos de degradaciones usando `difference_immediately` son específicos de la suscripción y se aplican automáticamente a futuras renovaciones. Son distintos de los [Créditos de Cliente](/features/customer-credit).
</Info>
...
| Evento | Desencadenador | Acción |
|--------|---------------|--------|
| `payment.succeeded` | Pago de upsell/cross-sell completado | Entregar producto, actualizar acceso |
| `payment.failed` | Fallo en el cargo con un clic | Mostrar error, ofrecer reintento o alternativa |
| `subscription.plan_changed` | Actualización/degradación completada | Actualizar características, enviar confirmación |
| `subscription.active` | Suscripción reactivada después de un cambio de plan | Conceder acceso al nuevo nivel |
