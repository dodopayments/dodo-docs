...
Una suscripción Team Pro cuesta $99/mes e incluye 5 asientos. Si tienes más de 5 usuarios, pagas un adicional de $15/mes por cada asiento extra.
...
- Costo total mensual: $99 + $150 = $249 por 15 asientos
...
| Modo | Añadiendo Asientos | Eliminando Asientos |
|------|------------------|---------------------|
| `prorated_immediately` | Cargar por los días restantes en el ciclo | Crédito por días no usados |
| `difference_immediately` | Cargar el precio completo del asiento | Crédito aplicado a renovaciones futuras |
| `full_immediately` | Cargar el precio completo del asiento, restablecer ciclo de facturación | Sin crédito |

### Ejemplos de Prorrateo

**Escenario: 15 días de ciclo de facturación restantes, añadiendo 5 asientos a $10/asiento**
...
El cliente paga $25 ahora, luego $50/mes al renovarse.
</Tab>
...
El cliente paga el total de $50 ahora, independientemente de la posición en el ciclo.
</Tab>

<Tab title="completo_inmediatamente">
...
**Escenario: Eliminando 3 asientos a mitad de ciclo con prorrateo_inmediatamente**
...
<Tip>
**Elegir un modo de prorrateo para cambios de asientos**: Usa `prorated_immediately` para una facturación justa basada en días cuando los equipos ajustan asientos con frecuencia. Usa `difference_immediately` para cálculos más simples que cargan o acreditan el precio completo del asiento. Consulta la [Guía de Prorrateo](/developer-resources/subscription-upgrade-downgrade#proration-modes) para comparaciones detalladas.
</Tip>
...
| Evento | Cuando se Activa | Caso de Uso |
|-------|------------------|----------|
| `subscription.active` | Nueva suscripción activada | Provisionar asientos iniciales |
| `subscription.plan_changed` | Asientos añadidos/eliminados | Actualizar el conteo de asientos en tu aplicación |
...
Permite exceder con una advertencia y periodo de gracia.
...
Añade asientos automáticamente cuando se alcanza el límite.
...
Ofrece precios de asientos anuales descontados:
...
**Implementación:** Crea productos separados para planes mensuales y anuales con diferentes precios de complementos.
...
- **Comunicación Clara**: Mostrar el precio por asiento de manera prominente en tu página de precios.
...
- **Incentivos Anuales**: Descuenta planes anuales para mejorar el flujo de caja y la retención.

### Mejores Prácticas Técnicas

- **Cachear Conteos de Asientos**: Cachea los conteos de asientos de suscripción localmente para evitar llamadas API en cada solicitud.
- **Sincronizar Regularmente**: Sincroniza periódicamente tu conteo de asientos local con Dodo Payments a través de la API.
- **Manejar Errores**: Si un cambio de asiento falla, muestra mensajes de error claros y opciones de reintento.
...
- **Pasos de Confirmación**: Requiere confirmación antes de realizar cambios de facturación.
- **Transparencia en Prorrateo**: Explica claramente los cargos prorrateados antes de aplicarlos.
- **Fáciles Downgrades**: No dificultes la reducción de asientos (eso genera confianza).
...
3. Establece un TTL para la caché para asegurar una actualización regular.
</Accordion>

<Accordion title="Cargos de prorrateo inesperados">
...
2. Muestra un desglose claro: "Añadiendo X asientos = $Y hoy (prorrateado por Z días)"
...
<Accordion title="No se puede reducir el número de asientos por debajo del uso actual">
**Síntoma**: El cliente desea reducir asientos pero tiene usuarios asignados.
...
3. Considera un periodo de gracia antes de hacer cumplir la reducción de asientos.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
<Card title="Tutorial de Precios Basados en Asientos" icon="code" href="/developer-resources/seat-based-pricing">
  Guía de implementación completa con código.
</Card>

<Card title="Complementos" icon="puzzle" href="/features/addons">
  Entiende el sistema de complementos en profundidad.
</Card>

<Card title="Cambios de Plan y Prorrateo" icon="arrows-rotate" href="/developer-resources/subscription-upgrade-downgrade">
  Maneja las modificaciones de suscripción.
</Card>
