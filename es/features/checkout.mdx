...
<Info>
Esta sección cubre la configuración del tema en la **API del lado del servidor** usando `customization.theme_config`. Si estás usando el **Checkout SDK** (checkout en superposición o inline), consulta las secciones de personalización de tema en [Overlay Checkout](/developer-resources/overlay-checkout#theme-customization) o [Inline Checkout](/developer-resources/inline-checkout#theme-customization), que usan propiedades camelCase (por ejemplo, `bgPrimary` en lugar de `bg_primary`).
</Info>
...
| `font_primary_url` | URL de la tipografía principal |
| `font_secondary_url` | URL de la tipografía secundaria |
...
| `pay_button_text` | Texto personalizado para el botón de pago (por ejemplo, "Complete Purchase", "Subscribe Now") |
...
| `input_focus_border` | Color del borde enfocado de los campos |
...
<Tip>
No necesitas especificar todas las propiedades de color. Las propiedades no especificadas usarán los valores predeterminados del tema.
</Tip>
