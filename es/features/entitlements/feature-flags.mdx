...
| Desencadenador | Efecto |
|---|---|
| El pago único se completa correctamente / la suscripción pasa a estar activa | Grant creado con `status: delivered` y `delivered_at` establecidos. |
| La suscripción se pone en pausa, se cancela o caduca | Grant revocado con el `revocation_reason` correspondiente. |
| Reembolso de un pago único | Grant revocado con `revocation_reason: refund`. |
| La suscripción se recupera (por ejemplo, el proceso de cobro de deuda tiene éxito) | El grant revocado se restaura a `Delivered` — mismo `id` del grant, campos de revocación borrados. |
| Revocación manual mediante la API | Grant revocado con `revocation_reason: manual`. No se restaura automáticamente al renovar. |
