...
1. El cliente completa el proceso de checkout.
2. Dodo Payments crea una concesión con estado `Pending`. Su `oauth_url` es `null` hasta que el cliente inicia el flujo de aceptación; en ese momento, se completa con una URL de autorización de GitHub.
3. El cliente autoriza mediante el enlace incluido en su correo electrónico o en el portal del cliente.
...
5. Si la suscripción se cancela, reembolsa o revoca, el cliente se elimina como colaborador.
...
- `entitlement_grant.created` se activa con `status: "Pending"` y `oauth_url` aún en `null`; la URL de autorización de GitHub se completa una vez que el cliente inicia el flujo de aceptación desde el correo electrónico de entrega o el portal del cliente.
- `entitlement_grant.delivered` se activa una vez creada la invitación al colaborador.
- `entitlement_grant.failed` se activa con `error_code: "github_permission_denied"` (o un valor similar) si la GitHub App pierde el acceso al repositorio.
