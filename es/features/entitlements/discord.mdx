...
1. El cliente completa el proceso de pago.
2. Dodo Payments crea un grant con el estado `Pending`. Su `oauth_url` es `null` hasta que el cliente inicia el flujo de aceptación; en ese momento, se completa con una URL de autorización de Discord.
3. El cliente recibe un correo electrónico con un botón "Join the Discord" (el enlace de OAuth). El enlace también está visible en su customer portal.
4. Después de que el cliente autoriza, el bot lo añade al servidor y le asigna el rol configurado. El grant pasa a `Delivered`.
5. Si la suscripción se cancela, se pausa o caduca, o si el merchant la revoca manualmente, el bot elimina el rol y el grant pasa a `Revoked`.
...
- `entitlement_grant.created` se activa con `status: "Pending"`. `oauth_url` es `null` en este punto; se completa cuando el cliente inicia el flujo de aceptación desde su correo electrónico de entrega o customer portal.
