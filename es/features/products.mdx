...
Para productos recurrentes, configure el objeto `price` a un precio recurrente (`type: recurring_price`) y especifique `payment_frequency_interval` e `subscription_period_interval` (`Day`, `Week`, `Month` o `Year`) junto con sus correspondientes cantidades.
...
Si ya no desea mostrar o usar un producto, puede archivarlo mediante una solicitud `DELETE` al endpoint `/products/{id}`. Esta acción oculta el producto pero no lo elimina permanentemente.
