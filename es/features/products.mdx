...
Un producto puede ser un artículo único o un servicio basado en suscripción. Para crear un nuevo producto, envíe una `POST` request al endpoint `/products` con detalles como nombre, descripción, precio, moneda y si se trata de un producto recurrente.

Para productos recurrentes, establezca `recurring: true` y especifique un `billing_cycle` (`daily`, `weekly`, `monthly`, `yearly`).
...
Para modificar un producto existente, envíe una `PATCH` request al endpoint `/products/{product_id}`. Puede actualizar propiedades como nombre, precio y descripción mientras mantiene otros detalles sin cambios.

Asegúrese de que la `product_id` en el endpoint coincida con un producto existente.
...
Puede obtener una lista de productos almacenados en su cuenta utilizando una `GET` request al endpoint `/products`. Esto le permite recuperar detalles del producto, incluidos productos activos y archivados.
...
Puede asociar una imagen con un producto subiéndola a AWS S3 utilizando una URL pre-firmada proporcionada por la API. Primero, solicite una URL de carga de imagen al endpoint `/products/{product_id}/images`, luego use la URL proporcionada para subir la imagen dentro de los 60 segundos.

<Warning>
La URL prefirmada expira en 60 segundos, así que la imagen debe ser cargada dentro de ese período de tiempo.
</Warning>

Una vez que haya recibido la URL pre-firmada de la API, suba la imagen utilizando el método `PUT`. Esto asegura un acceso seguro y temporal a AWS S3 para subir la imagen.
...
Si la carga es exitosa, AWS S3 devolverá un `200 OK` status, indicando que la imagen se ha almacenado con éxito.
...
Si ya no desea mostrar o usar un producto, puede archivarlo utilizando una `POST` request al endpoint `/products/{product_id}/archive`. Esta acción oculta el producto pero no lo elimina permanentemente.
...
Si necesita restaurar un producto archivado, envíe una `POST` request al endpoint `/products/{product_id}/unarchive`. Esto reactivará el producto y lo hará disponible para su uso nuevamente.
...
<Check>
Está listo para crear productos y comenzar a vender: únicos, recurrentes o por uso.
</Check>

## Relacionado

<Card title="Análisis de Productos" icon="chart-mixed" href="/features/analytics-and-reporting#product-level-analytics">
Realice un seguimiento de ingresos, clientes, retención, suscriptores y MRR para cada producto individual.
</Card>
