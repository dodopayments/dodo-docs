...
Pausar y reanudar se realizan mediante un único campo `pause` en el endpoint de actualización de suscripciones. No existe un endpoint de pausa separado.
...
<Warning>
`pause` es exclusivo respecto a cualquier otro campo: enviarlo junto con cualquier otra cosa se rechaza con `422`. Establecer `status` en `paused` **no** pausa una suscripción; utiliza en su lugar el campo `pause`.
</Warning>
...
| `active` | La suscripción está activa y se renovará automáticamente | — | No se necesita ninguna acción |
| `on_hold` | Falló un pago de renovación (o un cargo por cambio de plan); las renovaciones se han detenido | **Sí** | Actualiza el método de pago para reactivarla, automáticamente mediante [Payment Retries](/features/recovery/payment-retries) y [Dunning](/features/recovery/subscription-dunning), o manualmente mediante la [API de actualización del método de pago](/api-reference/subscriptions/update-payment-method) |
| `paused` | Tú o el cliente pausaron deliberadamente la suscripción; la facturación está congelada y el acceso revocado | **Sí** | Reanúdala con `pause: false` o desde el Customer Portal cuando la pausa de autoservicio esté habilitada; consulta [Pausar y reanudar suscripciones](#pausing-and-resuming-subscriptions) |
| `cancelled` | La suscripción se canceló y no se renovará | Solo volver a comprar | El cliente debe iniciar una nueva suscripción; [Dunning](/features/recovery/subscription-dunning) puede impulsar una nueva compra |
| `failed` | Falló la **creación** de la suscripción (el mandato o pago inicial no se completó correctamente) | **No — terminal** | El cliente debe crear una nueva suscripción con un método de pago válido |
...
La pausa y la reanudación utilizan el mismo endpoint `PATCH /subscriptions/{subscription_id}` mediante el campo `pause`: `pause: true` pausa una suscripción activa y `pause: false` la reanuda. El campo no se puede combinar con ningún otro campo en la misma solicitud. Para conocer el comportamiento completo, los efectos de facturación y la configuración empresarial relacionada, consulta [Pausar y reanudar suscripciones](#pausing-and-resuming-subscriptions).

<Card title="API Reference" icon="code" href="/api-reference/subscriptions/patch-subscriptions">
Consulta la API de actualización de suscripciones, incluido el campo `pause`.
</Card>
</Accordion>
