...
### Vista previa de cambios en el plan

Antes de comprometerse a un cambio de plan, vista previa del cargo exacto y la suscripción resultante:
...
<Card title="Vista previa de la API de cambio de plan" icon="eye" href="/api-reference/subscriptions/preview-change-plan">
  Vista previa de los cambios en el plan antes de comprometerse a ellos.
</Card>
...
<Warning>
Cuando una suscripción está en estado `on_hold`, no se renovará automáticamente. Debe actualizar el método de pago para reactivar la suscripción.
</Warning>

### Reactivación desde en espera
...
<Info>
Después de actualizar exitosamente el método de pago para una suscripción en estado `on_hold`, recibirás eventos de webhook `payment.succeeded` seguidos de `subscription.active`.
</Info>
...
Utilice `POST /subscriptions` para crear suscripciones programáticamente a partir de productos, con pruebas y complementos opcionales.

<Card title="Referencia de API" icon="code" href="/api-reference/subscriptions/post-subscriptions">
Ver la API de creación de suscripciones.
</Card>
</Accordion>

<Accordion title="Actualizar suscripciones">
Utilice `PATCH /subscriptions/{id}` para actualizar cantidades, cancelar al final del período o modificar metadatos.

<Card title="Referencia de API" icon="code" href="/api-reference/subscriptions/patch-subscriptions">
Aprenda cómo actualizar los detalles de la suscripción.
</Card>
</Accordion>
...
Revise las opciones de cambio de plan.
</Card>
</Accordion>
...
Utilice `GET /subscriptions` para listar todas las suscripciones y `GET /subscriptions/{id}` para recuperar una.

<Card title="Referencia de API" icon="code" href="/api-reference/subscriptions/get-subscriptions">
Navegue por las APIs de listado y recuperación.
</Card>
</Accordion>

<Accordion title="Historial de uso">
Obtenga el uso registrado para modelos de precios medidos o híbridos.

<Card title="Referencia de API" icon="code" href="/api-reference/subscriptions/get-usage-history">
Vea la API de historial de uso.
</Card>
</Accordion>

<Accordion title="Actualizar método de pago">
Actualice el método de pago para una suscripción. Para suscripciones activas, esto actualiza el método de pago para futuras renovaciones. Para suscripciones en estado `on_hold`, esto reactiva la suscripción creando un cargo por las deudas restantes.

<Card title="Referencia de API" icon="code" href="/api-reference/subscriptions/update-payment-method">
Aprenda cómo actualizar métodos de pago y reactivar suscripciones.
</Card>
</Accordion>
</AccordionGroup>
...
Al crear sesiones de pago, incluya su producto de suscripción y complementos opcionales:
...
Mejore o degrade una suscripción y controle el comportamiento del prorrateo:
...
Programe una cancelación sin terminación inmediata del acceso:
...
Cree una suscripción a demanda y cobre más tarde según sea necesario:
...
Actualice el método de pago para una suscripción activa:
...
## Suscripciones con mandatos compatibles con RBI
  ...
  El tipo y monto del mandato dependen del cargo recurrente de su suscripción:

  - **Cargos inferiores a ₹15,000:** Creamos un mandato a demanda por ₹15,000 INR. El monto de la suscripción se cobra periódicamente de acuerdo con la frecuencia de su suscripción, hasta el límite del mandato.
  ...
  **Importante:** Al mejorar o degradar suscripciones, considere cuidadosamente los límites del mandato:

  - Si una mejora/degradación resulta en un monto de cargo que excede ₹15,000 y va más allá del límite de pago a demanda existente, el cargo de la transacción puede fallar.
  ...
  **Cronograma de procesamiento:** Los cargos recurrentes en tarjetas indias y suscripciones UPI siguen un patrón de procesamiento único:
  ...
  - Los cargos son **iniciados** en la fecha programada de acuerdo con la frecuencia de su suscripción.
  ...
  **Manejo de casos límite:** Si proporciona beneficios, créditos o uso de suscripción a los clientes inmediatamente después de la iniciación del cargo, debe manejar esta ventana de 48 horas adecuadamente en su aplicación. Considere:
  ...
  - Manejar estados de espera de suscripción en la lógica de su aplicación

  <Tip>
  Monitoree los webhooks de suscripción para rastrear cambios en el estado de pago y manejar casos límite donde los mandatos son cancelados durante la ventana de 48 horas.
  </Tip>

## Mejores prácticas

- **Comience con niveles claros**: 2–3 planes con diferencias obvias
- **Comunique precios**: Muestre totales, prorrateo y próxima renovación
- **Utilice pruebas de manera reflexiva**: Convierta con incorporación, no solo con tiempo
- **Aproveche los complementos**: Mantenga los planes base simples y ofrezca extras
- **Pruebe cambios**: Valide cambios de plan y prorrateo en modo de prueba

<Info>
Las suscripciones son una base flexible para ingresos recurrentes. Comience simple, pruebe a fondo e itere según la adopción, la pérdida y las métricas de expansión.
</Info>