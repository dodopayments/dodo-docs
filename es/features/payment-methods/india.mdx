...
Para que UPI aparezca en el proceso de pago:
1. **El país de facturación** debe ser India (`IN`)
2. **Moneda** debe ser INR
3. Para comerciantes no indios: **Moneda adaptativa** debe estar habilitada
...
<Warning>
**No otorgar beneficios en la iniciación del cargo.** Espere el webhook `payment.succeeded`, que llega ~48-51 horas después de la fecha programada del cargo.
</Warning>
...
| `payment.succeeded` | ~48h después de la fecha del cargo | Otorgar/continuar acceso |
...
<Accordion title="Gestionar cancelaciones de mandato">
Los clientes pueden cancelar mandatos a través de sus aplicaciones bancarias en cualquier momento. Monitorear los webhooks `subscription.on_hold` y solicitar a los clientes que se re-subscriban o actualicen sus métodos de pago.
</Accordion>
...
<AccordionGroup>
<Accordion title="UPI no aparece en el proceso de pago">
**Verificar:**
1. ¿El país de facturación está configurado en `IN`?
2. ¿La moneda está configurada en `INR`?
3. Si es un comerciante no indio: ¿Está habilitada la moneda adaptativa?
4. ¿`upi_collect` incluido en `allowed_payment_method_types`?

**Solución:** Verificar que la dirección de facturación tenga `country: "IN"` y `billing_currency: "INR"`.
</Accordion>
...
**Solución:** El próximo cargo fallará y la suscripción se moverá a `on_hold`. Monitorear los webhooks para `payment.failed`.
</Accordion>
</AccordionGroup>
