...
<Step title="Establecer la dirección de facturación apropiada">
Establecer el país de la dirección de facturación para que coincida con el método de pago:
- `NL` para iDEAL
- `BE` para Bancontact
- `AT` para EPS
- `PT` para Multibanco
...
<Accordion title="Proporcionar alternativas de tarjeta">
...
2. ¿La moneda está configurada a EUR?
3. ¿El método está incluido en `allowed_payment_method_types`?

**Solución:** Los métodos europeos son estrictamente regionales. Un cliente con el país de facturación `DE` (Alemania) no verá iDEAL, que es exclusivo para los Países Bajos.
</Accordion>
