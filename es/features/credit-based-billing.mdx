...
Define credits in your own unit - tokens, API calls, compute hours, or any metric meaningful to your product. Custom units use the precision you set (0 to 10 decimal places).
...
| Source | Description |
|--------|-------------|
| **Subscription** | Créditos emitidos con la compra de una suscripción y reemitidos en cada ciclo de facturación |
| **One-Time** | Créditos emitidos con un producto de pago único |
| **Add-on** | Créditos emitidos con un complemento asociado a una suscripción |
| **API** | Créditos otorgados manualmente mediante la API o el dashboard |
| **Rollover** | Créditos transferidos desde un ciclo de facturación anterior |
...
- Hasta `10` - Para unidades de alta precisión (por ejemplo, tokens fraccionarios o uso en microescala)
...
|---------------------|-------------|
| **Crédito añadido** | Créditos otorgados (por suscripción, pago único o API) |
| **Crédito deducido** | Créditos consumidos mediante uso o débito manual |
| **Crédito vencido** | Créditos vencidos sin rollover |
| **Crédito transferido** | Créditos transferidos al siguiente periodo |
| **Rollover forfeited** | Créditos transferidos que se pierden después de alcanzar el número máximo de rollovers |
| **Exceso cobrado** | Uso que supera el saldo de créditos con el exceso habilitado |
| **Exceso restablecido** | Exceso registrado borrado al restablecerse el ciclo de facturación |
| **Recarga automática** | Reposición automática de créditos cuando el saldo es bajo |
| **Ajuste manual** | Crédito o débito aplicado manualmente por el merchant |
...
| `credit.rolled_over` | Créditos transferidos a una nueva concesión |
| `credit.rollover_forfeited` | Créditos perdidos al alcanzar el número máximo de rollovers |
| `credit.overage_charged` | Cargos por exceso aplicados |
| `credit.overage_reset` | El exceso registrado se restableció al final del ciclo de facturación |
...
Todos los eventos del ledger (cada evento `credit.*` excepto `credit.balance_low`) incluyen el payload completo `CreditLedgerEntry` con el saldo anterior y posterior, el exceso anterior y posterior, la referencia de origen y el `metadata` de la suscripción o el pago de origen de la concesión (vacío para las concesiones creadas directamente mediante la API). El evento `credit.balance_low` incluye la configuración del umbral y el saldo actual.
