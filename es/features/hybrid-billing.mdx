...
**Analytics del equipo: $20/usuario/mes**

**Cada usuario incluye:**
- 5 GB de procesamiento de datos/mes
- Excedente: $2/GB después del límite incluido

**Cálculo de ejemplo** (equipo de 10 usuarios que utiliza 80 GB en total):
- Tarifas de puestos: 10 × $20 = $200.00
...
- **Plataformas de Analytics**: por analista + procesamiento de datos
- **Herramientas de diseño**: por diseñador + almacenamiento/exportaciones
- **Entornos de desarrollo**: por desarrollador + horas de cómputo
- **Herramientas de comunicación**: por usuario + volumen de mensajes/llamadas

### Opciones de implementación

Dado que no puedes asociar medidores de uso y complementos a la misma suscripción, elige uno de estos enfoques:

<Tabs>
<Tab title="Option A: Usage Product + App-Managed Seats">
Usa una suscripción basada en el uso y gestiona la facturación de puestos en tu aplicación.
...
Registra la cantidad de puestos y calcula las tarifas de los puestos por separado.
...
Ajusta el uso incluido según la cantidad de puestos.
...
Usa complementos para los puestos y cobra el uso manualmente mediante cargos bajo demanda.
...
<Info>
**Recomendación**: La opción B (complemento de puestos + uso bajo demanda) suele ser más fácil de implementar porque Dodo gestiona automáticamente la facturación de los puestos. Solo necesitas registrar y cobrar los excedentes de uso.
</Info>

---

## Modelo híbrido 5: Suscripción + puestos + uso (híbrido triple)

El modelo más completo: tarifa de plataforma + tarifa por usuario + consumo.

<Warning>
**Restricción**: Dodo Payments actualmente no permite asociar medidores de uso y complementos al mismo producto de suscripción. Este modelo requiere un enfoque alternativo.
</Warning>
...
- **Por puesto**: $25/usuario/mes
- **Uso**: $0.10/1000 llamadas a la API (50K incluidas)

**Cálculo de ejemplo** (empresa con 20 usuarios y 150,000 llamadas a la API):
- Plataforma: $199.00
- Puestos: 20 × $25 = $500.00
- Uso: (150K - 50K) × \$0.10/1K = \$10.00
- **Total: $709.00/mes**

### Casos de uso

- **SaaS Enterprise**: plataforma + equipo + consumo
...
Usa una suscripción con complementos de puestos y cobra el uso manualmente mediante cargos bajo demanda.

**Este es el enfoque recomendado** porque Dodo gestiona automáticamente la tarifa de plataforma y la facturación de los puestos.
...
Usa una suscripción con un medidor de uso y gestiona la facturación de los puestos en tu aplicación.
...
<Step title="Manage Seats in Your Application">
Registra la cantidad de puestos y ajusta el precio de la suscripción base según corresponda.
...
<Info>
**Recomendación**: La opción A (base + puestos + uso bajo demanda) suele ser más fácil porque Dodo gestiona automáticamente la facturación de la plataforma y los puestos. Solo necesitas registrar el uso y enviar los cargos al final de cada ciclo de facturación.
</Info>
...
Diferentes niveles de suscripción con distintos límites incluidos y tarifas de excedente.
...
Cuando los clientes actualizan de nivel, obtienen:
- Un límite incluido más alto
...
- **Planes de soporte**: tarifa de SLA + por ticket o por hora
...
<Info>
Estos ejemplos muestran estructuras de precios ideales. Debido a la restricción de que los medidores de uso y los complementos no pueden asociarse al mismo producto, algunas combinaciones requieren soluciones alternativas (mediante cargos bajo demanda para el uso o puestos gestionados por la aplicación).
</Info>

### Ejemplo 1: Plataforma SaaS de IA

**Estructura de precios:**
- **Suscripción base**: $99/mes (acceso a la plataforma, 5 puestos incluidos)
- **Complemento de puesto**: $20/puesto/mes
- **Complementos de funciones**: Modelos personalizados (\$49/mes), acceso a la API (\$29/mes), cola prioritaria (\$19/mes)
- **Excedente de uso**: $0.02 por cada 1,000 tokens después de 100K (cobrado mediante cargos bajo demanda)

**Implementación**: Usa una suscripción con complementos de puestos y funciones. Registra el uso de tokens en tu aplicación y cobra los excedentes mediante cargos bajo demanda al final del ciclo de facturación.

**Cliente de ejemplo** (12 usuarios, 500K tokens, Modelos personalizados + acceso a la API):
| Componente | Cálculo | Importe |
|-----------|-------------|--------|
| Base | Tarifa de plataforma | $99 |
| Puestos adicionales | 7 × $20 | $140 |
...
**Opción A** (centrada en el uso): Crea productos con medidores de uso para builds/almacenamiento. Gestiona los usuarios en tu aplicación.

**Opción B** (centrada en puestos): Crea productos con complementos de puestos. Registra el uso de builds/almacenamiento y cobra los excedentes mediante cargos bajo demanda.

**Complementos (si usas la opción B):**
- Usuarios adicionales: $10/usuario/mes
- Builds prioritarias: $19/mes
- Dominios personalizados: $9/dominio/mes

### Ejemplo 3: Automatización de marketing

**Estructura de precios:**
- **Base**: $79/mes (funciones principales de automatización, 3 puestos incluidos)
- **Niveles de contactos** (complementos): 1K incluidos, 5K (+\$30), 25K (+\$80), 100K (+\$200)
- **Complementos de funciones**: Marketing por SMS (\$29/mes), páginas de destino (\$19/mes), pruebas A/B (\$29/mes)
- **Puestos del equipo**: complemento de $15/usuario/mes
- **Volumen de correo electrónico**: regístralo en la aplicación y cobra el excedente mediante cargos bajo demanda ($1/1,000 correos por encima del límite)

**Implementación**: Usa una suscripción con complementos de niveles de contactos, funciones y puestos. Registra los envíos de correo electrónico en tu aplicación y cobra los excedentes mediante cargos bajo demanda.
...
<Tip>
Haz que los precios híbridos sean fáciles de entender. Muestra de forma destacada en tu página de precios los costos base, lo que está incluido y cómo funcionan los excedentes.
</Tip>
...
| Puestos adicionales (7 × $15.00) | $105.00 |
| Uso de la API - Incluido (10,000 llamadas) | $0.00 |
| Uso de la API - Excedente (15,420 llamadas × $0.005) | $77.10 |
| Complemento de Analytics avanzada | $19.00 |
| **Subtotal** | **$250.10** |
| Impuesto (8.5%) | $21.26 |
| **Total adeudado** | **$271.36** |

---

## Solución de problemas de la facturación híbrida
...
**Soluciones**:
1. Usa la API de `previewChangePlan` para mostrar los cargos exactos antes de confirmar
2. Comunica que la prorrata se aplica a la suscripción Y a los complementos
3. Considera usar `difference_immediately` para simplificar la facturación de las actualizaciones
</Accordion>

<Accordion title="Free threshold not applying correctly">
**Síntomas**: Se cobra al cliente por un uso que debería ser gratuito.
...
3. Confirma que el medidor de uso esté correctamente asociado al producto de suscripción
</Accordion>

<Accordion title="Add-ons not appearing in checkout">
**Síntomas**: No se pueden añadir puestos ni funciones durante el checkout.

**Soluciones**:
1. Verifica que los complementos estén asociados al producto de suscripción en el dashboard
...
<Card title="Usage-Based Billing Guide" icon="chart-line" href="/developer-resources/usage-based-billing-guide">
  Implementación completa de la facturación basada en el uso.
</Card>
