...
**Soluciones**:
1. Verifique que la ingestión de eventos esté funcionando (verifique las respuestas de la API)
2. Confirme que `customer_id` coincide con el cliente de suscripción
3. Verifique que `event_name` coincida con la configuración del medidor
4. Verifique que los eventos tengan marcas de tiempo correctas (no con fecha futura)
</Accordion>
...
**Soluciones**:
1. Use la API de `previewChangePlan` para mostrar los cargos exactos antes de confirmar
2. Comunique que la prorrata se aplica a la suscripción Y a los complementos
