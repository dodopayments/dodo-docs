...
- **Objeto de suscripción**: Nuevo campo `cancellation_feedback` (uno de los valores anteriores) y `cancellation_comment` (texto libre opcional), completados cuando el cliente cancela
- **Webhook de `subscription.cancelled`**: Ambos campos se incluyen en el payload
- **API**: Pasa `cancellation_feedback` y `cancellation_comment` a `PATCH /subscriptions/{subscription_id}` al programar o ejecutar una cancelación mediante programación
...
También puedes anular el valor predeterminado del merchant para pagos individuales, cargos de suscripción bajo demanda y cambios de plan usando el booleano `adaptive_currency_fees_inclusive`:
