...
|-----------|--------|----------|-------------|
| `email` | cadena | Sí       | Dirección de correo electrónico válida (RFC 2821) |
| `fields` | objeto | No       | Objeto con pares de nombre/valor del campo |
| `fields.name` | cadena | No       | Nombre del suscriptor |
| `fields.last_name` | cadena | No       | Apellido del suscriptor |
| `fields.company` | cadena | No       | Nombre de la empresa |
| `fields.country` | cadena | No       | País |
| `fields.city` | cadena | No       | Ciudad |
| `fields.phone` | cadena | No       | Número de teléfono |
| `groups` | array  | No       | Array de IDs de grupos a los que agregar al suscriptor |
| `status` | cadena | No       | Uno de: activo, dado de baja, no confirmado, rebotado, correo no deseado |
| `subscribed_at` | cadena | No       | Fecha en formato `yyyy-MM-dd HH:mm:ss` |
| `ip_address` | cadena | No       | Dirección IP del suscriptor |
