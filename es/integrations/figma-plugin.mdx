...
Crea tu propio proyecto de plugin de Figma utilizando la [guía rápida para plugins de Figma](https://www.figma.com/plugin-docs/plugin-quickstart-guide/). Esto generará un esqueleto con un archivo `manifest.json`.
...
<Warning>
Siempre utiliza `test_mode` durante el desarrollo para evitar procesar pagos reales.
</Warning>
</Step>
...
- Personaliza `LicenseKeyInput.tsx` para coincidir con tus preferencias de diseño
...
| `npm run lint` | Verifica estilo de código y errores |
...
1. Compila para producción: `npm run build`
