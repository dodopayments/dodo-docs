...
2. **Almacenar el ID de clic** en tus metadatos de pago junto con el ID externo del cliente
3. **Enviar datos de venta a Dub** cuando los pagos tengan éxito utilizando su API Track
...
<Paso title="Obtén tu clave API de Dub">
  Navega a tu [tablero de Dub](https://app.dub.co) → Configuración → Claves API y crea una nueva clave API con el alcance `conversions.write`.
...
- **Siempre incluye el ID de clic en los metadatos**: Sin el ID de clic, Dub no puede atribuir ingresos a tus enlaces
- **Usa IDs externos de manera consistente**: Pasa el mismo ID de cliente que usas en tu sistema para una analítica precisa a nivel de cliente
- **Maneja el tráfico orgánico con gracia**: Establece `webhook.cancel = true` cuando no haya ID de clic para evitar llamadas a la API innecesarias
- **Prueba con pagos de muestra**: Verifica que la integración funcione correctamente antes de lanzarla en producción
- **Monitorea tu tablero de Dub**: Verifica que las ventas aparezcan correctamente con la atribución adecuada
...
<AccordionGroup>
<Accordion title="Las ventas no aparecen en Dub">
- Verifica que tu clave API de Dub sea correcta y tenga el alcance `conversions.write`
- Asegúrate de que se esté capturando y almacenando el `dub_click_id` en los metadatos de pago
- Asegúrate de que la transformación del webhook esté formateando correctamente la carga útil
- Verifica que el webhook se esté activando en eventos `payment.succeeded`
- Confirma que el seguimiento de conversiones esté habilitado para tus enlaces de Dub
</Accordion>
...
- Confirma que los usuarios estén haciendo clic en tus enlaces cortos de Dub antes de realizar el pago
- Verifica que la cookie `dub_id` se esté estableciendo correctamente en tu dominio
- Comprueba que los IDs de clic coincidan entre la creación del checkout y la finalización del pago
- Asegúrate de que estés capturando el ID de clic antes de crear la sesión de checkout
</Accordion>
...
<Accordion title="Se están rastreando ventas duplicadas">
...
- Para suscripciones, agrega marcas de tiempo o período de facturación para evitar duplicados en las renovaciones
</Accordion>
</AccordionGroup>
...
<Info>
¿Necesitas ayuda? Contacta con el soporte de Dodo Payments en [support@dodopayments.com](mailto:support@dodopayments.com) para obtener asistencia con la integración.
</Info>
