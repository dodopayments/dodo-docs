...
<Tip>
**Integración del Programa de Afiliados**: Esta integración también funciona sin problemas con **Dub Partners** para rastrear referencias y comisiones de afiliados. Utiliza el seguimiento de conversiones de Dub para atribuir ventas a tus enlaces de afiliados y medir el rendimiento de los socios. Aprende más sobre cómo configurar programas de afiliados en nuestra [guía de funciones de Afiliados](/features/affiliates).
</Tip>
...
2. **Almacenar el ID de clic** en los metadatos de pago junto con el ID externo del cliente
3. **Enviar datos de venta a Dub** cuando los pagos sean exitosos utilizando su API de seguimiento

Esto permite que Dub asocie las ventas exitosas con el clic original del enlace, dándote una atribución completa de conversiones.
...
3. Tu clave API de Dub (disponible en tu panel de control de Dub bajo Configuración → Claves API)

## Comenzando

<Steps>
<Step title="Habilitar el Seguimiento de Conversiones en Dub">
  En tu panel de control de Dub, habilita el seguimiento de conversiones para los enlaces que deseas rastrear ventas. Esto permite que Dub registre eventos de venta cuando los clientes completan compras.
...
<Step title="Obtén tu Clave API de Dub">
  Navega a tu [panel de control de Dub](https://app.dub.co) → Configuración → Claves API y crea una nueva clave API con el alcance `conversions.write`.
...
<Step title="Capturar el ID de Clic en el Pago">
  Al crear una sesión de pago, captura el ID de clic de Dub de la cookie y agrégalo a tus metadatos de pago.
</Step>
...
<Step title="¡Listo!">
  Los eventos de conversión de ventas ahora aparecerán en tu panel de análisis de Dub con la atribución completa a tus enlaces.
</Step>
</Steps>
...
Al crear una sesión de pago, captura el ID de clic de Dub de la cookie e inclúyelo en tus metadatos de pago junto con el ID externo de tu cliente.
...
<Step title="Abrir la Sección de Webhook">
  En tu panel de control de Dodo Payments, navega a **Webhooks → + Agregar Endpoint** y expande el menú desplegable de integraciones.
  <Frame>
...
Rastrear tanto suscripciones iniciales como pagos recurrentes:
...
Envía solo el monto antes de impuestos a Dub para un seguimiento preciso de ingresos:
...
- **Reembolsos**: Considera rastrear reembolsos por separado si es necesario para un informe preciso de ingresos
</Note>
...
- Verifica que los IDs de clic coincidan entre la creación del pago y la finalización del pago
- Asegúrate de que estás capturando el ID de clic antes de crear la sesión de pago
</Accordion>
...
- Asegúrate de que solo estás rastreando en eventos de {/* INLINE_CODE_PLACEHOLDER_bfeed7a4ab5393d394108752765f7cfb */, no en `payment.processing`
- Usa valores únicos de `invoiceId` para cada venta
