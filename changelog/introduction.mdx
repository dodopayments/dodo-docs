...
icon: "calendar-days"
keywords: ["Dodo Payments", "changelog", "release notes", "product updates"]
---
...
|---------|--------------|------------|
| [v1.113.6](/changelog/v1.113.6) | August 22, 2026 | Subscription pause and resume through the status field — the boolean pause field is removed and now returns 422 — with customer portal support, native Purchasing Power Parity with per-country percentages, Charm Pricing for psychologically appealing prices, archive businesses and brands with eligibility checklist, Milestones celebration page, guided setup experience, and billing accuracy improvements for license keys, Canadian tax, usage thresholds, and trial proration |
| [v1.112.0](/changelog/v1.112.0) | August 5, 2026 | Discount codes expanded with Amount discounts, `starts_at` scheduling, `customer_eligibility` rules, per-currency options, and `per_customer_usage_limit`, plus a rebuilt native webhooks experience with email alerting, Cash App Pay for subscriptions, SEPA Direct Debit for one-time EUR payments, customer-friendly payment failure messages across 46 unified error codes, payout webhooks, self-service login email change, an enforced setting that lets customers cancel their own subscriptions, a `currency` filter on list payments, a dispute response window extended to 10 days, and new analytics widgets |
| [v1.109.0](/changelog/v1.109.0) | July 21, 2026 | A completely rebuilt Analytics v3 dashboard with real-time revenue, one-click period comparisons, customizable layouts, and shareable chart cards, plus paid trials for subscriptions and checkout, trial-misuse prevention, Payout Accounts for managing multiple bank accounts, product and add-on pricing in any chargeable currency, a new `subscription.update_payment_method` webhook, native social login in the mobile app, `next_billing_date` in checkout preview, `cancel_at_next_billing_date` subscription filter, copy checkout design from test to live, and per-mode product drafts |
...
| [v1.99.0](/changelog/v1.99.0) | May 25, 2026 | Stacked discount codes (up to 20 per checkout, payment, or subscription), seven new customer notification emails for refunds and subscription lifecycle events, checkout payment page overhaul lifting success rates by 2–3%, product form rework with live preview and autosave, and Business Settings redesign |
...
| [v1.87.0](/changelog/v1.87.0) | March 9, 2026 | Support for discounts in change plan and minor improvements across the platform |
| [v1.86.0](/changelog/v1.86.0) | March 2, 2026 | Credit-based billing with rollover, overage, and expiration controls, design and theme customization hub, refund_status and dispute_status in list payments API, refunds table in customer portal, and copy to live mode for meters |
| [v1.84.0](/changelog/v1.84.0) | February 20, 2026 | Product-level analytics, Verifi RDR dispute prevention, DodoPayments CLI, customer portal UI revamp, inline checkout payment method updates, Tax ID prefill for checkout sessions, license_key and email in return URL, customer email update API, and Bun adapter |
| [v1.81.0](/changelog/v1.81.0) | February 6, 2026 | Third-party analytics (GA4, GTM, Meta Pixel), checkout theme customization, prevent plan change on payment failure, balance ledger API, and refund_status in payment response |
...
| [v1.75.0](/changelog/v1.75.0) | January 23, 2026 | Product collections, RevolutPay & Billie for GBP one-time payments, checkout session preview, metadata in change plan requests, invoice_url in API responses, discount validation endpoint, and enhanced dashboard filtering |
| [v1.70.0](/changelog/v1.70.0) | December 24, 2025 | Inline checkout support, use existing payment methods for instant checkouts, manage subscription plans from dashboard, generate short payment links, skip success pages with redirect_immediately, and enable on-demand subscriptions for all businesses |
| [v1.67.0](/changelog/v1.67.0) | December 17, 2025 | Mixed cart checkout for one-time and subscription products, Unified Customer Portal, invoice_id in Payment Response |
