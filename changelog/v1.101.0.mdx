...
description: "Business-level proration defaults with per-product-collection overrides, business name collection for B2B invoices, and automatic subscription payment retries to recover failed renewal revenue — plus bug fixes and improvements."
...
The customer-portal plan change resolves each setting in this order:

```
collection field (if set) → business field → system default
```
...
### 2. **Collect Business Name for B2B Invoices**
...
### 3. **Subscription Payment Retries**
