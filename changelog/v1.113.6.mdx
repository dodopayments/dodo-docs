...
**A new `subscription.unpaused` webhook event.** `subscription.paused` already existed and was wired for delivery, but nothing outside a merchant data import could put a subscription into the paused state to trigger it. Now that subscriptions can be paused directly, both events fire in normal operation.
