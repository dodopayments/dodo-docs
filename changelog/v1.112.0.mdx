...
Set `starts_at` to schedule a code for a future launch — leaving it unset keeps the code active immediately, and it must fall strictly before `expires_at`. Use `per_customer_usage_limit` to cap how often a single customer can redeem a code, independently of the overall `usage_limit`, which it cannot exceed when both are set.
...
- **Activity and logs** — browse delivered messages, open a **message detail** page to inspect the payload and every delivery attempt with its response code and duration, then resend or replay individual messages or replay in bulk.
- **Event catalog** — browse every event type Dodo Payments sends, with its schema.
- **Testing** — send example events to an endpoint and simulate deliveries before you go live.
...
Instead of writing a receiver, you can point an endpoint at a third-party service directly. Connectors ship for **Slack**, **Discord**, **Microsoft Teams**, **Zapier**, **Windmill**, **Resend**, **SendGrid**, **Loops**, **Customer.io**, **Segment**, **Close CRM**, and **HubSpot**, and the endpoint form includes step-by-step setup instructions for each one.
