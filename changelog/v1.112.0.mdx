...
Set `starts_at` to schedule a code for a future launch — leaving it unset keeps the code active immediately, and it must fall strictly before `expires_at`. Use `per_customer_usage_limit` to cap how often a single customer can redeem a code, independently of the overall `usage_limit`.
...
Learn more: [Payments](/features/transactions/payments) | [Get Payment Detail](/api-reference/payments/get-payments-1)
