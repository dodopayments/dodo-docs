...
Payment and dispute responses now include a `payment_provider` discriminator (`stripe`, `adyen`, or `dodo`) so you can identify how each transaction was routed, and the checkout session preview response returns `is_byop`. The fields are additive — Merchant-of-Record traffic continues to report `payment_provider: dodo`.
