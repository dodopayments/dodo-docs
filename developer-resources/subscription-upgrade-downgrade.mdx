...
icon: "arrows-rotate"
keywords: ["Dodo Payments", "developer guide", "plan upgrade", "plan downgrade", "subscription change"]
---
...
Requires the business's `allow_plan_change_via_payment_link` capability (**Settings → Subscriptions → Collect Plan Change Payments by Payment Link**), `effective_at: immediately`, and `on_payment_failure: prevent_change`. See [Collecting Payment via a Checkout Link](#collecting-payment-via-a-checkout-link).
...
A successful plan change returns `200 OK`. The body is a `ChangePlanResponse`. `payment_id` is set whenever the change triggers an immediate charge — with or without `collect_via_payment_link` — and is `null` for a scheduled change or a `do_not_bill` change. `payment_link`, `client_secret`, and `expires_on` are handles for a **hosted checkout page**: they're `null` for an ordinary charge against the saved payment method, and only populated when the request set `collect_via_payment_link: true`.

<Note>
The response doesn't confirm the outcome of a charge, only that the request was accepted. Always rely on webhook events (e.g. <code>subscription.plan_changed</code>, <code>payment.succeeded</code>, <code>payment.failed</code>) to confirm the outcome, or re-read the subscription with <code>GET /subscriptions/&#123;subscription_id&#125;</code>.
</Note>
...
<Info>
This is also what powers the **Collect Plan Change Payments by Payment Link** toggle in **Settings → Subscriptions**, which routes the built-in [Customer Portal](/features/customer-portal#collecting-plan-change-payments-by-payment-link)'s plan-change flow through checkout instead of the saved card.
</Info>
...
- `on_payment_failure` is `prevent_change`. The subscription cannot move to the new plan before the checkout payment succeeds.

<Info>
The dashboard tooltip for this setting describes it as covering an "immediate upgrade," but `collect_via_payment_link` isn't limited to upgrades — it applies to any immediate change that results in a charge, downgrades included, as long as the requirements above are met.
</Info>

`proration_billing_mode: do_not_bill` has nothing to charge, so it applies immediately with no checkout page even when `collect_via_payment_link: true` is set — `payment_link` and friends come back `null`. Other modes can also net to a small or zero amount depending on where the subscription is in its billing cycle. If you're setting `collect_via_payment_link` on plan changes generically (not just clear-cut upgrades), call [Preview Plan Change](/api-reference/subscriptions/preview-change-plan) first and only request the link when the previewed amount is worth collecting.
...
<Warning>
Once an immediate payment-link change is issued, every further request on that subscription — including the side-effect-free preview — is blocked until the link resolves. Don't issue a link you don't intend to have the customer pay right away.
</Warning>
...
<Info>
Credits created by downgrades using <code>difference_immediately</code> are subscription-scoped and distinct from <a href="/features/credit-based-billing">Credit-Based Billing</a> entitlements. They automatically apply to future renewals of the same subscription and are not transferable between subscriptions.
</Info>
...
|---------|----------------------|------------------------|-------------------|--------------|
| **Upgrade charge** | Prorated difference for remaining days | Full price difference between plans | Full new plan price | No charge |
| **Downgrade credit** | Prorated credit for remaining days | Full price difference as credit | No credit | No credit |
| **Billing cycle** | Resets to today | Resets to today | Resets to today | Unchanged |
| **Trial behavior** | Ends trial, charges immediately | Ends trial, charges immediately | Ends trial, charges full amount | Ends trial, no charge |
| **Best for** | Fair time-based billing | Simple upgrade/downgrade math | Resetting billing cycles | Free migrations or courtesy switches |
...
Instead of passing proration parameters on every plan change, you can set **default upgrade & downgrade behavior** once at the business level. These defaults apply to all customer-portal plan changes, and can be **overridden per product collection**.

There are separate defaults for upgrades and downgrades:
...
Configure business defaults under **Settings → Subscriptions**, and collection overrides on each product collection. Each collection field is independent — leave it unset to **inherit from the business default**, or set a value to override it for that collection only.
...
Plan change request processed successfully. The response body is empty. If `on_payment_failure=prevent_change`, the plan change stays pending until payment succeeds.
</Accordion>
...
A pending plan change already exists for this subscription (`PendingPlanChangeExists`). For a *scheduled* change, cancel it with `DELETE /subscriptions/{subscription_id}/change-plan/scheduled` before submitting a new one. For a pending *payment-link* change, there's no cancel endpoint — wait for the customer to pay, for the link to expire, or for the stuck-payment sweep to cancel it.
</Accordion>

<Accordion title="422 Unprocessable Entity">
The subscription is inactive or on-demand, or the request is not eligible for `collect_via_payment_link` — the business doesn't have the capability enabled, `effective_at` isn't `immediately`, or `on_payment_failure` isn't `prevent_change`. See [Requirements](#requirements).
</Accordion>
