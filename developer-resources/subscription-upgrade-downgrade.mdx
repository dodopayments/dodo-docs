...
A successful plan change returns `200 OK`. The body is a `ChangePlanResponse` — `payment_id`, `payment_link`, `client_secret`, and `expires_on` are `null` unless the change involved an immediate charge; a `collect_via_payment_link` request populates all four with checkout handles.
...
```json
{
  "payment_id": "pay_0NlruUwJxqYVIJ13lPxFw",
  "payment_link": "https://checkout.dodopayments.com/3bxQRpRl",
  "client_secret": "pay_0NlruUwJxqYVIJ13lPxFw_secret_dw1PuO23DxLTXcpLQula",
  "expires_on": "2026-08-21T11:08:17.414Z"
}
```
...
- If the link is never paid, it stops working after `expires_on`, and the subscription's stuck-payment sweep eventually cancels the pending change so the subscription is free to accept a new plan-change request.
