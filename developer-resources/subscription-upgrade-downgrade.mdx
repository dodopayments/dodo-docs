...
A successful plan change returns `200 OK` with an **empty response body** — `payment_id`, `payment_link`, `client_secret`, and `expires_on` are all `null` — for a scheduled change, a `do_not_bill` change, and an ordinary immediate charge against the saved payment method alike. The one exception is a successful `collect_via_payment_link` request, which populates all four with checkout handles instead.
