...
<Tabs>
  <Tab title="Node.js SDK">
    ...
    async function changePlan() {
      // change-plan returns 200 with a body whose fields are all null for a standard change.
  ...
  <Tab title="Python SDK">
    ...
    # change_plan returns 200 with a body whose fields are all null for a standard change.
...
A successful plan change returns `200 OK` with a JSON body containing `payment_id`, `payment_link`, `client_secret`, and `expires_on`. For a standard merchant-initiated plan change — where the saved card is charged — all four fields are `null`.

<Note>
Because the response carries no result of the charge, always rely on webhook events (e.g. <code>subscription.plan_changed</code>, <code>payment.succeeded</code>, <code>payment.failed</code>) to confirm the outcome, or re-read the subscription with <code>GET /subscriptions/&#123;subscription_id&#125;</code>. Treat the four fields as nullable rather than asserting on them.
</Note>
