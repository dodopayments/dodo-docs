...
<Warning>
The client-side `themeConfig` option is **deprecated** and will be removed in the next major version of the Checkout SDK (v2.0.0). Passing it logs a deprecation warning in the browser console. Configure your theme when creating the checkout session via the API instead, using the `customization.theme_config` parameter — see [Checkout Theme Customization](/features/checkout#checkout-theme-customization) — or visually on the [Design page](/features/design) in the dashboard. Session-configured themes apply to overlay, inline, and hosted checkout alike.
</Warning>
