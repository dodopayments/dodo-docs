...
**Common options**: `credit`, `debit`, `upi_collect`, `apple_pay`, `google_pay`, `amazon_pay`, `klarna`, `affirm`, `afterpay_clearpay`, `cashapp`, `multibanco`, `bancontact_card`, `eps`, `ideal`, `blik`, `paypal`. This is not the full set — see the [Create Checkout Session API reference](/api-reference/checkout-sessions/create) for every accepted value.
