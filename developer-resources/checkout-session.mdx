...
icon: "cart-shopping"
keywords: ["Dodo Payments", "developer guide", "checkout sessions"]
---
...
Quantity of the product. Must be at least `1`.

A quantity large enough to push the cart total past the maximum amount a payment can carry is rejected with a `422` asking for a smaller quantity, rather than failing the checkout page with an unexplained error.
...
**Common options**: `credit`, `debit`, `upi_collect`, `apple_pay`, `google_pay`, `amazon_pay`, `klarna`, `affirm`, `afterpay_clearpay`, `cashapp`, `ach`, `multibanco`, `bancontact_card`, `eps`, `ideal`, `blik`, `paypal`. This is not the full set — see the [Create Checkout Session API reference](/api-reference/checkout-sessions/create) for every accepted value.
...
Apply one or more stacked discount codes to the checkout session. Codes are applied **in array order** (the first code reduces the starting price, the second reduces the already-discounted price, and so on), up to a maximum of **20** codes per session. When [Purchasing Power Parity](/features/purchasing-power-parity) is enabled, the starting price is the PPP-adjusted amount, not the base price.
...
**Supported languages:** Arabic (`ar`), Catalan (`ca`), Chinese (`zh`), Dutch (`nl`), English (`en`), French (`fr`), Georgian (`ka`), German (`de`), Hebrew (`he`), Indonesian (`id`), Italian (`it`), Japanese (`ja`), Korean (`ko`), Malay (`ms`), Polish (`pl`), Portuguese (`pt`), Romanian (`ro`), Russian (`ru`), Spanish (`es`), Swedish (`sv`), Thai (`th`), Turkish (`tr`)
