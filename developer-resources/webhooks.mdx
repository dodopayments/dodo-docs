...
| Mode | What it does |
|------|--------------|
| **Recover failed messages** | Replays every message to this endpoint that failed. |
| **Replay missing messages** | Replays messages that were never dispatched to this endpoint, for example after you subscribe it to a new event type. |
| **Bulk replay messages** | Replays messages matching the filters you choose, including ones that already delivered successfully. |
...
<Step title="Narrow the Selection">
    **Bulk replay messages** additionally accepts **Since** and **Until** bounds, specific event types, and a channel or tag. Leave them alone to replay every event type in the window.
</Step>
