...
Open the endpoint from **Developer → Webhooks**, where **Delivery stats (last 24h)** summarizes its recent attempts.
...
Every run appears under **Replay history** on the endpoint, with its mode, time range, status, and the number of messages resent.
