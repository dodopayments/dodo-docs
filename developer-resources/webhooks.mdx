...
<Note>
**Where things live.** **Endpoints**, **Event catalog**, **Logs**, **Activity**, and **Settings** are tabs of the webhooks portal under **Developer → Webhooks**. **Overview**, **Testing**, and **Advanced** appear only once you open an individual endpoint, along with its signing secret and the replay actions.
</Note>
...
**Delivery activity** plots attempts over time, bucketed as **Attempts per 5 minutes**, **Attempts per hour**, or **Attempts per day** depending on the window. Each bar is split by outcome, and hovering a segment shows the status, the number of attempts, and its share of the total.
...
Re-driving messages is done per endpoint, not across the portal. Open the endpoint from **Developer → Webhooks**, where **Delivery stats (last 24h)** summarizes its recent attempts.

<Steps>
<Step title="Open More Actions">
    On the endpoint, open **More actions** and choose the mode that matches the situation.
</Step>

<Step title="Choose How Far Back to Go">
    Pick a starting point — **8 hours ago**, **Yesterday**, **3 days ago**, **Last week**, or **2 weeks ago**.
</Step>
</Steps>
...
| **Bulk replay messages** | Replays messages on this endpoint matching the filters you set, including ones that already delivered successfully. |

To re-drive a single message instead, open it and choose **Replay this message**. Every run appears under **Replay history** on the endpoint, with its mode, time range, status, and the number of messages resent.
