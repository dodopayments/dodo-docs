...
| `error_message` | string \| null | A human-readable explanation of the failure, written for you rather than your customer. For a standardized `error_code` this is a headline plus the recommended action, not the raw processor text. |
...
<Warning>
`error_message` from the merchant API is **merchant-facing copy**. It can name the real reason for a decline, including fraud-related ones, so never render it directly to a customer. Map `error_code` to your own customer-safe wording instead, as shown in [Surface Errors to Customers Safely](#surface-errors-to-customers-safely).
</Warning>
...
Show customers a friendly message — never the raw `error_code`, and never the merchant-facing `error_message`.

<Info>
On surfaces Dodo Payments controls — checkout, the [Customer Portal](/features/customer-portal), and dunning emails — this mapping is already done for you, including the fallback to a generic message for fraud-related declines. You only need the mapping below where you render failures in your own product.
</Info>
