...
| `failure_details` | object \| null | Ready-to-use messaging derived from `error_code`, for both you and your customer. See [Ready-Made Failure Messaging](#ready-made-failure-messaging). |
...
### Ready-Made Failure Messaging

Rather than maintaining your own copy for every failure reason, read `failure_details`. It is present whenever `error_code` is set, and unrecognised codes still resolve through a generic fallback instead of being omitted.

| Field | Type | Description |
|-------|------|-------------|
| `code` | string | The unified error code. Echoes `error_code`. |
| `decline_type` | string | `soft` or `hard`. Soft declines are worth retrying; hard declines are not. |
| `customer_fixable` | boolean | Whether the customer can resolve this themselves, for example by correcting a CVC. |
| `merchant_message` | string | A headline plus recommended action written for you, not the customer. |
| `customer_message` | string | The string to show the customer. |
| `customer_template` | string | Identifier for the customer message template (`C1` to `C20`). |
| `customer_cta` | string | The primary call to action to offer the customer, for example `try_again`. |

```json
"failure_details": {
  "code": "PROCESSING_ERROR",
  "decline_type": "soft",
  "customer_fixable": false,
  "merchant_message": "An error occurred while processing. Transient — retry; if it persists, ask the customer to contact their bank.",
  "customer_message": "Something went wrong while processing your payment. Please try again in a few moments.",
  "customer_template": "C16",
  "customer_cta": "try_again"
}
```

<Warning>
Show `customer_message` to customers and keep `merchant_message` internal. For fraud-sensitive declines — lost, stolen, pickup card, and fraudulent — `customer_message` is deliberately generic so the real reason is never leaked, and `merchant_message` carries a do-not-reveal warning.
</Warning>
