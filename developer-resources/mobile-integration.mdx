...
<Tabs>
  ...
  <Tab title="Dynamic Payment Links">
    **Best for:** Custom pricing, multiple products, or dynamic data
    ...
    <Tabs>
      <Tab title="Swift (iOS)">
        ```swift PaymentService.swift icon="apple" expandable
        import Foundation
      ...
      <Tab title="Kotlin (Android)">
        ```kotlin PaymentService.kt icon="android" expandable
      ...
      <Tab title="Dart (Flutter)">
        ```dart payment_service.dart icon="mobile-screen" expandable
        import 'dart:convert';
...
```javascript webhook-test.js icon="js" expandable
// test.js
const { Webhook } = require("standardwebhooks");
...
```kotlin PaymentActivity.kt icon="android" expandable
...
```swift PaymentViewController.swift icon="apple" expandable
...
2. Handle the redirect in AppDelegate:
   ```swift AppDelegate.swift icon="apple" expandable
...
3. Add observer in your view controller:
   ```swift PaymentHandler.swift icon="apple" expandable
...
```dart payment_screen.dart icon="mobile-screen" expandable
