...
<Info>
**Prefer an SDK over hand-rolled browser code.** Dodo Payments ships an
official checkout SDK for [iOS](/developer-resources/ios-checkout-sdk),
[Android](/developer-resources/android-checkout-sdk), and
[React Native](/developer-resources/react-native-checkout-sdk), with Flutter
coming soon. Each one wraps the pattern documented below (open the checkout
URL, capture the return, parse the result) behind a single typed
`start(...)` call, with abandoned-session recovery built in. The manual
code below still works and shows what the SDKs do internally, but start
with the SDK for new integrations where one's available.
</Info>
...
<Card
  ...
  icon="android"
  href="/developer-resources/android-checkout-sdk"
>
  Recommended: one `DodoCheckout.start(...)` call over Chrome Custom Tabs,
  with typed results and abandoned-session recovery built in. The manual
  implementation below is what it does internally.
</Card>

#### Chrome Custom Tabs Implementation (manual)
...
<Card
  ...
  icon="apple"
  href="/developer-resources/ios-checkout-sdk"
>
  Recommended: one `DodoCheckout.start(...)` call over SFSafariViewController,
  with typed results and abandoned-session recovery built in. The manual
  implementation below is what it does internally.
</Card>

#### SFSafariViewController Implementation (manual)
...
<Card
  ...
  icon="react"
  href="/developer-resources/react-native-checkout-sdk"
>
  Recommended: one `DodoCheckout.start(...)` call over the native iOS/Android
  checkout cores, with typed results and abandoned-session recovery built
  in. No `react-native-inappbrowser-reborn` dependency needed. The manual
  implementation below uses a generic in-app browser library instead.
</Card>

#### Using Checkout Links (InAppBrowser, manual)
