...
official checkout SDK for [Android](/developer-resources/android-checkout-sdk)
and [React Native](/developer-resources/react-native-checkout-sdk), with iOS
and Flutter coming soon. Each one wraps the pattern documented below (open
the checkout URL, capture the return, parse the result) behind a single
typed `start(...)` call, with abandoned-session recovery built in. The
manual code below still works and shows what the SDKs do internally, but
start with the SDK for new integrations where one's available.
</Info>
