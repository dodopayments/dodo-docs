...
const app = express();
app.use(cors());
// Parse JSON for API routes; webhook routes MUST use raw body middleware
// (not express.json()) so the original bytes are preserved for signature
// verification. For security, apply raw body parsing only on the webhook path.
