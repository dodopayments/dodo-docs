...
tag: "NEW"
keywords: ["Dodo Payments", "FastAPI", "Python", "starter template", "quick start", "fastapi boilerplate"]
---
...
- Pass all three `webhook-id`, `webhook-timestamp`, and `webhook-signature` headers to `webhook.verify()` — the Standard Webhooks signature covers `id.timestamp.body`, not the body alone
</Accordion>
</AccordionGroup>
