...
| **Subscription Past Due** | `subscription.past_due` | Sent when a subscription enters its grace period after a failed renewal. The payload carries `past_due_ends_at`, when the subscription moves to `on_hold` or `cancelled`. |
