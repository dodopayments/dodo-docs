...
| **Subscription Payment Method Updated** | `subscription.update_payment_method` | Triggered when a payment created to swap a subscription's payment method succeeds. Carries the [Payment](/developer-resources/webhooks/intents/payment) payload (not the Subscription payload). |
