...
| `SubscriptionPaused` | The subscription was paused (`subscription.paused`). Recoverable: resuming produces a re-grant. **Do not hardcode this value** — unlike every other reason here it is capitalized instead of lower-case with underscores, so treat it as unstable and branch on the `subscription.paused` webhook instead. |
