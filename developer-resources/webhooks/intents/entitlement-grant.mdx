...
- **Platform-direct integrations** (Telegram, Figma, Framer, Digital Files) sit in `pending` only briefly while the platform call runs, then move to `delivered`.
...
For all other integration types (Discord, GitHub, Telegram, Figma, Framer, Notion) both fields are `null`; the relevant configuration is captured in the entitlement itself, not the grant.
