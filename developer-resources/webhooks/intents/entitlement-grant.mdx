...
| `SubscriptionPaused` | The subscription was paused (`subscription.paused`). Recoverable: resuming produces a re-grant. **Do not hardcode this value** — unlike every other reason here it is not snake_case, so treat it as unstable and branch on the `subscription.paused` webhook instead. |
