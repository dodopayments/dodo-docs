...
icon: "repeat"
keywords: ["Dodo Payments", "subscription webhook", "subscription events", "webhook integration", "subscription"]
---
...
| `subscription.update_payment_method` | A payment created to swap the subscription's payment method succeeded |
...
<Note>
Unlike the other events in this table, `subscription.update_payment_method` carries the [Payment](/developer-resources/webhooks/intents/payment) payload — the same payload as `payment.succeeded` — for the payment that swapped the subscription's payment method, not the Subscription payload. On that payload, `is_update_payment_method` is `true` and `payment_method_id` identifies the payment method that was set. Listen for this event to confirm a customer's payment-method update went through.
</Note>
