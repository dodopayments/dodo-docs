...
description: 'Official SDKs for TypeScript, Python, PHP, Go, Ruby, Java, Kotlin, C#, and Rust to integrate Dodo Payments into your applications'
icon: "box"
keywords: ["Dodo Payments", "developer guide", "SDKs", "API client libraries", "developer tools", "official sdks overview"]
---
...
lightweight checkout SDKs. One call opens Dodo's hosted checkout in the
platform's native browser surface and returns a typed result:

<CardGroup cols={2}>
<Card title="Kotlin Mobile SDK" icon="android" href="/developer-resources/android-checkout-sdk">
  Chrome Custom Tab-based checkout for Kotlin apps, with Google Pay support
</Card>

<Card title="React Native SDK" icon="react" href="/developer-resources/react-native-checkout-sdk">
  Thin Turbo Module wrapper over the native iOS and Android checkout cores
</Card>
</CardGroup>
...
payment sheet, use the mobile checkout SDK for your platform instead. Both
share the same one-call contract: `start(...)` returns a typed
`CheckoutResult`, neither holds an API key, and Apple Pay/Google Pay work
the same way they do on the open web since checkout opens in the
platform's real browser surface (`SFSafariViewController` / Custom Tabs),
not a `WebView`:

<CardGroup cols={2}>
<Card title="Kotlin Mobile SDK" icon="android" href="/developer-resources/android-checkout-sdk">
  Maven Central, Chrome Custom Tabs, Google Pay
</Card>

<Card title="React Native SDK" icon="react" href="/developer-resources/react-native-checkout-sdk">
  npm, Turbo Module, wraps the native iOS/Android cores
</Card>
</CardGroup>
