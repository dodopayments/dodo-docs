...
<Tabs>
  <Tab title="Static Payment Links">
    **Best for:** Simple subscriptions with predefined products
    ...
    ```text subscription-url.txt icon="link" expandable
    https://checkout.dodopayments.com/buy/{subscription_product_id}?quantity=1&redirect_url={return_url}
    ```
    
    **Example:**
    ```text subscription-example-url.txt icon="link" expandable
...
<Tab title="Node.js SDK">
    ```javascript subscription-creation.js icon="js" expandable
...
main();
  ...
  ```python subscription-creation.py icon="python" expandable
import os
...
print(subscription.subscription_id)
  ...
  <Tab title="GO SDK">
  ```go subscription-creation.go icon="go" expandable
package main
...
func main() {
  ...
  ```javascript subscription-api-route.js icon="js" expandable
...
```json subscription-response.json icon="code" expandable
{
