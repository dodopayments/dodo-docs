...
description: "Build an end-to-end system that sells a product and lets you supply your own license keys on demand: create the product and entitlement, detect pending grants, and deliver the key to the customer."
icon: "key"
keywords: ["Dodo Payments", "developer guide", "license keys", "manual fulfillment", "entitlements"]
---
...
Select **License Key** as the integration and give it a **Name**. Configure how each issued key behaves: the **License Length**, **Activations Limit**, and an optional customer-facing **Activation Message**.
...
Open the **Fulfillment Mode** dropdown and change it from **Automatic** to **Manual** so each purchase creates a pending grant for you to fulfill, then click **Create Entitlement**.
</Step>
</Steps>
</Tab>
...
Open the product you want to sell, expand **Advanced Settings → Entitlements & Credits**, and select your License Key entitlement. A single product can deliver this license key alongside other entitlements on the same purchase.
...
If you'd rather not rely on webhooks, list grants for the entitlement and filter by `integration_type` and `status`:
