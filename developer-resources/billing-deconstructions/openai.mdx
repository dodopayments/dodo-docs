...
### Predictable Revenue and Low Risk

By requiring prepaid credits for API usage, OpenAI eliminates the risk of users running up massive bills they can't pay. This is crucial when dealing with expensive GPU compute time. You get the money upfront, and the user gets the service as they use it.

### Scalability for Developers

Developers love pay-as-you-go models because they can start small. A $5 top-up is a low barrier to entry. As their application grows, they can automate top-ups or buy larger packs. The friction to start is almost zero, but the ceiling for growth is unlimited.

### User Psychology

Denominating credits in fiat currency (USD) instead of abstract "tokens" or "points" makes the value clear. Users know exactly what they're paying for. It feels like a bank account for AI services, which builds trust and makes budgeting easier for companies.
...
The API uses prepaid fiat-denominated credits. Users top up their accounts with $5, $10, $50, or more. These credits show a dollar value but have no monetary value outside OpenAI. You can't withdraw them once they're purchased.

OpenAI bills per-token with different rates for input and output tokens. Each model has its own pricing. For example, GPT-4o costs more than GPT-4o-mini. Credits never expire, which is a change from their previous 1-year policy. When a user's balance hits $0, their API calls fail immediately.

### ChatGPT Plus, Team, and Enterprise

These are flat-rate subscriptions. ChatGPT Plus costs $20 per month, while the Team plan is $25 per user per month. These plans have soft usage caps. When a user exceeds their limit, they aren't blocked. Instead, they get downgraded to a smaller, less capable model.

### Spend-based rate tiers

OpenAI uses spend-based rate tiers to manage trust and capacity. As you spend more total money over time, you unlock higher API rate limits. This isn't a billing feature itself, but it's a trust-based access scaling system tied directly to your billing history. Tier 1 users have lower limits than Tier 5 users.

### Pricing Tables

The following tables show how OpenAI structures its pricing across different models and plans.
...
### Fiat-denominated credits

Credits feel like money because they're denominated in USD. This makes pricing transparent and easy to understand for developers. They don't have to do mental math to figure out how many "compute units" a dollar buys.

### No expiry

Never-expiring balances reduce the "use it or lose it" pressure. Users feel comfortable topping up larger amounts because they know the value won't disappear. This increases the average transaction value for OpenAI.

### Multi-dimensional metering

Input and output tokens are tracked separately but deduct from the same credit balance. This allows OpenAI to price expensive output tokens differently from cheaper input tokens. It's a single meter from the user's perspective, but two dimensions from the billing perspective.

### Trust tiers

Linking rate limits to total spend encourages users to stay on the platform and rewards long-term customers with better performance. It's a clever way to use billing data to manage infrastructure load.
...
<Steps>
  ...
  <Step title="Create Usage Meters">
    ...
    * `llm.input_tokens`: Sum aggregation on the `input_tokens` property.
    * `llm.output_tokens`: Sum aggregation on the `output_tokens` property.
  ...
  <Step title="Handle Balance Depletion">
    ...
    async function checkCreditsBeforeRequest(customerId: string) {
      const response = await fetch(`https://api.dodopayments.com/credit-entitlements/balance?customer_id=${customerId}&credit_entitlement_id=credit_entitlement_id`, {
        headers: {
          'Authorization': `Bearer ${process.env.DODO_API_KEY}`
        }
      });
    ...
    ```typescript
    // Example webhook handler for credit.balance_low
...
const dodo = new DodoPayments({ apiKey: process.env.DODO_API_KEY });
...
export async function handleApiRequest(req, res) {
  ...
  try {
    ...
    const balanceResponse = await fetch(`https://api.dodopayments.com/credit-entitlements/balance?customer_id=${customerId}&credit_entitlement_id=credit_entitlement_id`, {
...
The flow for this billing model is straightforward. A developer starts by purchasing a credit pack through a one-time payment. Dodo Payments adds these credits to the user's fiat credit balance. As the developer uses your API, your application sends usage events to Dodo.

The meters track these tokens and automatically deduct the corresponding value from the user's credit balance based on your configured rates. When the balance hits $0, your application detects this and blocks further API calls until the user tops up again.
