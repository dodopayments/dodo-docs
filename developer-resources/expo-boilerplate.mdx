...
tag: "NEW"
keywords: ["Dodo Payments", "Expo", "React Native", "starter template", "quick start", "expo boilerplate"]
---
...
<Warning>
`EXPO_PUBLIC_*` variables are embedded in the shipped JavaScript bundle and are readable by anyone who downloads your app. Dodo Payments API keys are bearer secrets, so use this only with a Test Mode key while developing. For production, create checkout sessions on your backend and pass only the resulting `checkout_url` to the app — see [Mobile Integration](/developer-resources/mobile-integration).
</Warning>
</Step>
