...
`dodo wh trigger` can send mock payloads for these 22 event types:
...
<Note>
This is a subset of the events Dodo Payments delivers. `payout.*`, `credit.*`, `abandoned_checkout.*`, `dunning.*`, `entitlement_grant.*`, `subscription.paused`, and `subscription.update_payment_method` have no mock generator yet — exercise them with real test-mode activity and `dodo wh listen`. See [Webhook Events](/developer-resources/webhooks/intents/webhook-events-guide) for the full list.
</Note>
