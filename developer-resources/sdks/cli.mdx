...
`dodo wh trigger` can send mock payloads for all 48 event types Dodo Payments delivers:

| Category | Events |
|----------|--------|
| **Subscription** | `active`, `updated`, `on_hold`, `past_due`, `paused`, `unpaused`, `renewed`, `plan_changed`, `cancelled`, `failed`, `expired`, `update_payment_method` |
