...
Customize the sheet's dismiss button, presentation style, and color scheme
via `customization` on `start(...)`. All fields are optional; omitting
`customization` uses iOS's default `SFSafariViewController` appearance.

<ParamField body="dismissButtonStyle" type="DismissButtonStyle">
Label or icon for the dismiss button: `done`, `close`, or `cancel`.
</ParamField>
