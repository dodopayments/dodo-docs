...
Subscription `on_hold` state में जा सकती है और `subscription.on_hold` webhook भेज सकती है (देखें [Subscription States → On Hold](/features/subscription#on-hold-state))। यह एक signal है — lock नहीं। On-demand subscriptions के लिए, `on_hold` आपको दोबारा charge करने से **नहीं रोकता**।
</Step>

<Step title="Retry the charge (your call)">
On-demand flows के लिए, Dodo अपने-आप retry **नहीं करता**। Retry करने के लिए आप किसी भी समय `POST /subscriptions/{subscription_id}/charge` को फिर से call कर सकते हैं। नीचे दी गई [safe retry policy](#payment-retries) लागू करें — exponential backoff का उपयोग करें, hard declines को छोड़ दें और burst patterns से बचें — ताकि हमारे fraud और risk systems retries को flag न करें।
</Step>

<Step title="Optionally, ask the customer for a new payment method">
यदि payment method के खराब होने के कारण retries लगातार विफल होती रहें (expired card, closed account आदि), तो customer से नया payment method लेने के लिए [`POST /subscriptions/{subscription_id}/update-payment-method`](/api-reference/subscriptions/update-payment-method) का उपयोग करें। सफल होने पर subscription `active` में लौट आती है और `payment.succeeded` के बाद `subscription.active` webhooks भेजे जाते हैं।
</Step>
</Steps>
...
| 2 | `subscription.on_hold` | Subscription को hold पर रखा गया (informational; आगे के charges को block नहीं करता) |
| 3* | `payment.succeeded` | बाद का charge — आपका retry या payment method update के बाद का charge — सफल हुआ |
| 4* | `subscription.active` | सफल charge के बाद subscription `active` में लौट आई |
...
[Subscription Dunning](/features/recovery/subscription-dunning) — built-in email recovery sequence — scheduled subscriptions पर विफल *renewal* payments और customer-initiated cancellations तक सीमित है। इसे on-demand charge failures के लिए डिज़ाइन नहीं किया गया है। जब आपको लगे कि payment method को update करना आवश्यक है, तो customer से सीधे संपर्क करें (जैसे transactional email या in-app prompt)।
...
<Note>
Decline reasons की comprehensive list और यह जानने के लिए कि उन्हें user ठीक कर सकता है या नहीं, [Transaction Failures](/api-reference/transaction-failures) documentation देखें।
</Note>
...
जब कोई customer [Customer Portal](/features/customer-portal) से on-demand subscription cancel करता है, तो cancellation default रूप से **अगली billing date के लिए schedule** की जाती है। On-demand subscriptions के लिए **Cancel Now** option जानबूझकर नहीं दिखाया जाता।

कारण यह है: on-demand subscriptions में predictable recurring renewal dates नहीं होतीं — अगला charge time पूरी तरह आपके usage events से निर्धारित होता है। अगली billing date पर cancellation schedule करने से period boundary तक mandate active रहता है, ताकि in-flight usage के charges अभी भी लिए जा सकें और फिर subscription को सही ढंग से समाप्त किया जा सके।

Customer द्वारा cancellation की पुष्टि करने के बाद:
...
आप किसी भी समय API के माध्यम से on-demand subscription cancel कर सकते हैं। Cancellation immediate होगी या scheduled, यह आप नियंत्रित करते हैं।
...
Customer portal behavior को mirror करें — scheduled cancellation date तक subscription active रखें, फिर इसे समाप्त करें।
...
| Event | कब भेजा जाता है |
|-------|---------------|
| `subscription.cancelled` | Subscription पूरी तरह cancel हो जाती है और chargeable नहीं रहती |
| `subscription.plan_changed` | `cancel_at_next_billing_date` toggle किया गया (scheduled cancellation set या undone हुई) |
...
<Tip>
On-demand flows के लिए usage-based charges को reconcile करने हेतु `payment.succeeded` और `payment.failed` पर focus करें। जब `payment.failed` के बाद `subscription.on_hold` आए, तो subscription recover करने के लिए [Handling failed charges](#handling-failed-charges) देखें।
</Tip>
