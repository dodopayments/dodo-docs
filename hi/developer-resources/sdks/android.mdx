...
`DodoCheckout.start` केवल गलत उपयोग या platform
failure के लिए `CheckoutError` throw करता है। `CheckoutError.code` से code पढ़ें:
...
- `PLATFORM_ERROR`: unexpected platform failure, जिसमें ऐसा `returnUrl` शामिल है जिसका
  scheme आपके `dodoCallbackScheme` placeholder से मेल नहीं खाता।

User द्वारा cancel करना या declined payment हमेशा एक result (`CANCELLED` या
`FAILED`) होता है, thrown error कभी नहीं। Launcher style के साथ, validation errors
