...
2. **अपने क्लाइंट एप्लिकेशन को पुनः चालू करें**
3. **क्लाइंट लॉग्स की जांच करें** त्रुटि संदेशों के लिए
...
<Tip>
हमेशा सबसे हाल का संस्करण खींचने के लिए `@latest` का उपयोग करें, या स्थिरता के लिए `@1.0.0` जैसे विशिष्ट संस्करण पर पिन करें।
</Tip>
...
क्लिक करने के बाद, **Cursor Settings** > **Tools & MCP** > **New MCP Server** के माध्यम से Cursor के `mcp.json` में अपने पर्यावरण चर सेट करें।

**मैनुअल कॉन्फ़िगरेशन**

<Steps>
<Step title="Cursor सेटिंग्स खोलें">
    Navigate to **Cursor Settings** > **Features** > **Model Context Protocol** या `Cmd/Ctrl + Shift + P` दबाएँ और "MCP Settings" खोजें।
</Step>

<Step title="कॉन्फ़िगरेशन संपादित करें">
    **Edit Config** पर क्लिक करें ताकि `mcp_config.json` फ़ाइल खुल सके।
</Step>
...
<Tab title="Claude Desktop">
Dodo Payments MCP सर्वर को Claude Desktop में सेट करें ताकि आप अपनी भुगतान डेटा में संवादी पहुँच प्राप्त कर सकें।
<Steps>
<Step title="कॉन्फ़िगरेशन फ़ाइल ढूंढें">
    Claude Desktop कॉन्फ़िगरेशन फ़ाइल ढूंढें:
...
कॉन्फ़िगरेशन आपकी होम निर्देशिका के `.claude.json` फ़ाइल में संग्रहित की जाएगी।
...
क्लिक करने के बाद, **Command Palette** > **MCP: Open User Configuration** के माध्यम से VS Code के `mcp.json` में अपने पर्यावरण चर सेट करें।

**मैनुअल कॉन्फ़िगरेशन**

<Steps>
<Step title="MCP कॉन्फ़िगरेशन खोलें">
    Command Palette (`Cmd/Ctrl + Shift + P`) खोलें और "MCP: Open User Configuration" के लिए खोजें।
</Step>
...
Dodo Payments MCP सर्वर को Cline (VS Code) में सेट करें ताकि आप अपनी भुगतान डेटा में संवादी पहुँच प्राप्त कर सकें।
<Steps>
<Step title="Cline सेटिंग्स खोलें">
    Cline कॉन्फ़िगरेशन फ़ाइल ढूंढें:
...
उपलब्ध क्षमताएँ:
- `top-level-unions` - शीर्ष स्तर के संघ प्रकार का समर्थन
- `valid-json` - सही JSON स्ट्रिंग तर्क पार्सिंग
- `refs` - स्कीमाओं में $ref पॉइंटर्स का समर्थन
- `unions` - संघ प्रकार (anyOf) का समर्थन
- `formats` - फ़ॉर्मेट वैलिडेशन्स का समर्थन
...
<Tip>
अपने डेटा में अप्रत्याशित परिवर्तनों से रोकने के लिए विकास के दौरान `--operation=read` का उपयोग करें।
</Tip>
...
2. **`get_api_endpoint_schema`** - एक विशिष्ट विधि के लिए विस्तृत स्कीमा प्राप्त करें
3. **`invoke_api_endpoint`** - उचित पैरामीटर के साथ कोई भी विधि निष्पादित करें

<Info>
डायनेमिक उपकरण स्वचालित रूप से 50 से अधिक विधियों वाले APIs के लिए सक्षम होते हैं। सभी उपकरणों को शामिल करने के लिए `--tools=all` के साथ ओवरराइड करें।
</Info>
...
दूर से तैनात करते समय, हमेशा `Authorization` हेडर या `x-dodo-payments-api-key` हेडर के माध्यम से प्रमाणीकरण की आवश्यकता होती है।
...
सत्यापित करें कि `DODO_PAYMENTS_ENVIRONMENT` सही सेट किया गया है (INAILED_CODE_PLACEHOLDER_f2ae2e94534df218_END प्रोडक्शन के लिए)।
