...
<Info>
  पुरानी <code>checkout</code> विधि के विपरीत, <code>checkoutSession</code> चेकआउट पृष्ठ पर उपयोगकर्ता से बिलिंग जानकारी लेता है। आप <code>billing</code> कुंजी को तर्क में पास करके इसे अधिलेखित कर सकते हैं।
</Info>

<Info>
  पुरानी <code>checkout</code> विधि के समान, <code>checkoutSession</code> ग्राहक की ईमेल और नाम को उनकी बेहतर-प्रमाणित सत्र से लेती है, हालांकि, आप इसे <code>customer</code> वस्तु के माध्यम से अधिलेखित कर सकते हैं।
</Info>

<Info>
  आप समान विकल्पों को तर्क में पास कर सकते हैं जैसे कि अनुरोध शरीर [Create Checkout Session](https://docs.dodopayments.com/api-reference/checkout-sessions/create) एंडपॉइंट के लिए।
</Info>

<Note>
  वापसी URL सर्वर प्लगइन में कॉन्फ़िगर किए गए <code>successUrl</code> से लिया जाता है। आपको ग्राहक पेलोड में <code>return_url</code> शामिल करने की आवश्यकता नहीं है।
</Note>

## लेगेसी चेकआउट (अप्रचलित)

<Warning>
  <code>authClient.dodopayments.checkout</code> विधि अप्रचलित है। नए कार्यान्वयन के लिए <code>checkoutSession</code> का उपयोग करें।
</Warning>
...
## मीटर्ड उपयोग को ट्रैक करना

<code>usage()</code> प्लगइन को सर्वर पर सक्षम करें ताकि मीटर्ड इवेंट कैप्चर किया जा सके और ग्राहक अपने उपयोग-आधारित बिलिंग का निरीक्षण कर सकें।

- <code>authClient.dodopayments.usage.ingest</code> साइन-इन, ईमेल-सत्यापित उपयोगकर्ता के लिए घटनाओं को ग्रहण करता है।
- <code>authClient.dodopayments.usage.meters.list</code> उन ग्राहकों की सदस्यताओं से जुड़े मीटर के लिए हालिया उपयोग को सूचीबद्ध करता है।
...
<Tip>
  जब उपयोग को ग्रहण किया जा रहा होता है, तो एक घंटे से पुराने या पांच मिनट से अधिक भविष्य के टाइमस्टैम्प अस्वीकार कर दिए जाते हैं।
</Tip>
<Tip>
  यदि आप उपयोग मीटर सूचीबद्ध करते समय <code>meter_id</code> छोड़ देते हैं, तो ग्राहक की सदस्यताओं से जुड़े सभी मीटर लौटाए जाते हैं।
</Tip>

# वेबहुक्स

<Info>
  वेबहुक प्लगइन Dodo Payments से सुरक्षित हस्ताक्षर सत्यापन के साथ रीयल-टाइम भुगतान घटनाओं को संसाधित करता है। डिफ़ॉल्ट एंडपॉइंट <code>/api/auth/dodopayments/webhooks</code> है।
</Info>

<Steps>
<Step title="Generate and set webhook secret">
अपने एंडपॉइंट URL (जैसे, <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) के लिए डोडो पेमेंट्स डैशबोर्ड में एक वेबहुक सीक्रेट जनरेट करें और इसे अपने <code>.env</code> फ़ाइल में सेट करें:
...
<AccordionGroup>
  <Accordion title="Plugin Options">
    ...
    - <b>createCustomerOnSignUp</b> (वैकल्पिक): उपयोगकर्ता साइनअप पर ऑटो-क्लाइंट बनाएं
    - <b>use</b> (आवश्यक): सक्षम करने के लिए प्लगइन्स की सूची (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (वैकल्पिक): फ़ंक्शन जो BetterAuth `User` को प्राप्त करता है और DodoPayments क्लाइंट के लिए जोड़ने और अपडेट करने के लिए अतिरिक्त फ़ील्ड्स लौटाता है (उदा. `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b>: उत्पादों की सूची या उत्पाद लौटाने वाला async फ़ंक्शन -{" "}
    <b>successUrl</b>: सफल भुगतान के बाद पुनर्निर्देशित होने वाला URL -{" "}
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>अमान्य API कुंजी</b>: अपनी <code>DODO_PAYMENTS_API_KEY</code> को <code>.env</code> में दोबारा जांचें। -{" "}
    <b>वेबहुक हस्ताक्षर बेमेल</b>: सुनिश्चित करें कि वेबहुक सीक्रेट Dodo Payments डैशबोर्ड में सेट की गई के समान है। - <b>ग्राहक नहीं बनाया गया</b>: पुष्टि करें कि <code>createCustomerOnSignUp</code> को <code>true</code> पर सेट किया गया है।
  </Accordion>
  <Accordion title="Best Practices">
    - सभी सीक्रेट्स और कुंजियों के लिए एन्वायरनमेंट वेरिएबल्स का उपयोग करें। - 
    <code>test_mode</code> में परीक्षण करें, उसके बाद <code>live_mode</code> पर स्विच करें। - डिबगिंग और ऑडिट के लिए वेबहुक इवेंट्स को लॉग करें।
  </Accordion>
</AccordionGroup>
