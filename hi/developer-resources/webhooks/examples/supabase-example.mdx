...
1. अपने [Supabase Dashboard](https://supabase.com/dashboard) पर जाएँ
2. SQL Editor खोलें
3. एक नया क्वेरी बनाएँ
4. [`schema.sql`](#database-schema) की पूरी सामग्री को कॉपी और पेस्ट करें
5. क्वेरी चलाएँ

### 4. प्रारंभिक रहस्यों को सेट करें

Supabase स्वचालित रूप से रनटाइम में `SUPABASE_URL` और `SUPABASE_SERVICE_ROLE_KEY` प्रदान करता है।

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **नोट:** हम तैनाती के बाद `DODO_PAYMENTS_WEBHOOK_KEY` को सेट करेंगे जब आपके पास आपका वेबहुक URL होगा।

### 5. तैनात करें

फंक्शन `functions/webhook/index.ts` में पहले से सेट है - इसे बस तैनात करें:
...
1. [DodoPayments Dashboard](https://app.dodopayments.com) पर जाएँ → Developer → Webhooks
2. एक नया वेबहुक एंडपॉइंट बनाएँ
3. अपने वेबहुक URL को अंत बिंदु के रूप में कॉन्फ़िगर करें
4. इन सब्सक्रिप्शन ईवेंट्स को सक्षम करें:
   - `subscription.active`
...
✅ **Signature verification** - dodopayments लाइब्रेरी का उपयोग कर  
✅ **Idempotency** - वेबहुक IDs के साथ डुप्लिकेट प्रोसेसिंग को रोकता है  
✅ **Event logging** - `webhook_events` तालिका में पूरा ऑडिट ट्रेल  
✅ **Error handling** - लॉग और फिर से प्रयास करने योग्य

> **नोट:** यह कार्यान्वयन तीन मुख्य सब्सक्रिप्शन ईवेंट्स (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) को न्यूनतम फ़ील्ड के साथ संभालने का प्रदर्शन करता है। आप इसे अपनी आवश्यकताओं के आधार पर अतिरिक्त ईवेंट प्रकारों और फ़ील्ड्स का समर्थन करने के लिए आसानी से बढ़ा सकते हैं।
...
Deno-आधारित Edge Function:
1. **Signature verifies** - HMAC-SHA256 सत्यापन के लिए dodopayments लाइब्रेरी का उपयोग करता है
2. **Idempotency checks** - डुप्लिकेट प्रोसेसिंग को रोकने के लिए वेबहुक ID की जांच करता है
3. **Event logs the** - कच्चे वेबहुक डेटा को `webhook_events` तालिका में संग्रहीत करता है
4. **Updates processes** - Supabase क्लाइंट के माध्यम से ग्राहकों और सब्सक्रिप्शनों को बनाता या अपडेट करता है
5. **Error handling** - विफलताओं को लॉग करता है और पुनः प्रयास के लिए ईवेंट को चिह्नित करता है
...
<Note>
`--no-verify-jwt` ध्वज आवश्यक है क्योंकि वेबहुक JWT टोकन नहीं शामिल करते हैं। सुरक्षा वेबहुक सिग्नेचर सत्यापन द्वारा प्रदान की जाती है।
</Note>
...
| समस्या | समाधान |
|-------|----------|
| सत्यापन विफल | DodoPayments डैशबोर्ड से वेबहुक कुंजी की जाँच करें |
| डेटाबेस अनुमति त्रुटि | सुनिश्चित करें कि सेवा भूमिका कुंजी का उपयोग किया जा रहा है |
| JWT सत्यापन त्रुटि | `--no-verify-jwt` ध्वज के साथ तैनात करें |
| कार्यक्षमता नहीं मिली | सत्यापित करें कि परियोजना संदर्भ सही है और कार्यक्षमता तैनात है |
