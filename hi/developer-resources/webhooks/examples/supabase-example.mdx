...
<Tip>
  <strong>Supabase के लिए संपूर्ण स्टार्टर किट देखें</strong><br />
  यह एक न्यूनतम सब्सक्रिप्शन स्टार्टर किट है जिसे Next.js, Supabase, और Dodo Payments के साथ बनाया गया है। यह बॉयलरप्लेट आपको प्रमाणीकरण, भुगतान, और वेबहुक्स के साथ एक सब्सक्रिप्शन-आधारित SaaS को जल्दी सेट अप करने में मदद करता है।
...
4. [`schema.sql`](#database-schema) की संपूर्ण सामग्री कॉपी करें और पेस्ट करें
5. क्वेरी चलाएँ

### 4. प्रारंभिक रहस्यों को सेट करें

Supabase स्वचालित रूप से रनटाइम पर `SUPABASE_URL` और `SUPABASE_SERVICE_ROLE_KEY` प्रदान करता है।

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **नोट:** हम तैनाती के बाद आपके वेबहुक URL मिलने के बाद `DODO_PAYMENTS_WEBHOOK_KEY` सेट करेंगे।

### 5. तैनात करें

यह फ़ंक्शन पहले से ही `functions/webhook/index.ts` में सेट है - बस इसे तैनात करें:
...
1. [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks पर जाएँ
2. एक नया वेबहुक एंडपॉइंट बनाएं
3. एंडपॉइंट के रूप में अपना वेबहुक URL कॉन्फ़िगर करें
...
✅ **Signature verification** - dodopayments लाइब्रेरी का उपयोग करना  
✅ **Idempotency** - वेबहुक आईडी के साथ डुप्लीकेट प्रोसेसिंग से रोकथाम  
✅ **Event logging** - `webhook_events` तालिका में पूर्ण ऑडिट ट्रेल  
✅ **Error handling** - लॉग और फिर से प्रयास योग्य

> **नोट:** यह कार्यान्वयन तीन मुख्य सब्सक्रिप्शन इवेंट्स (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) को न्यूनतम फ़ील्ड्स के साथ संभालने को दिखाता है। आप इसे अपनी आवश्यकताओं के अनुसार अतिरिक्त इवेंट प्रकारों और फ़ील्ड्स को सपोर्ट करने के लिए आसानी से बढ़ा सकते हैं।
...
Deno-आधारित Edge फ़ंक्शन:
1. **सिग्नेचर की पुष्टि करता है** - HMAC-SHA256 सत्यापन के लिए dodopayments लाइब्रेरी का उपयोग करता है
2. **Idempotency की जांच करता है** - डुप्लीकेट प्रोसेसिंग रोकने के लिए वेबहुक ID को देखता है
3. **इवेंट को लॉग करता है** - कच्चे वेबहुक डेटा को `webhook_events` तालिका में संग्रहित करता है
4. **अपडेट्स को प्रोसेस करता है** - Supabase क्लाइंट के माध्यम से ग्राहक और सब्सक्रिप्शन बनाता या अपडेट करता है
5. **त्रुटियों को संभालता है** - विफलताओं को लॉग करता है और इवेंट को फिर से प्रयास के लिए चिह्नित करता है
...
<Note>
`--no-verify-jwt` फ़्लैग आवश्यक है क्योंकि वेबहुक्स में JWT टोकन शामिल नहीं होते। सुरक्षा वेबहुक सिग्नेचर सत्यापन द्वारा प्रदान की जाती है।
</Note>
...
| Issue | Solution |
|-------|----------|
| Verification failed | जांचें कि DodoPayments डैशबोर्ड से वेबहुक कुंजी सही है |
| Database permission error | सुनिश्चित करें कि Service Role Key का उपयोग हो रहा है |
| JWT verification error | `--no-verify-jwt` फ़्लैग के साथ तैनात करें |
| Function not found | सत्यापित करें कि प्रोजेक्ट रेफ सही है और फ़ंक्शन तैनात है |
