...
- **Subscription object**: नया `cancellation_feedback` field (ऊपर दिए गए मानों में से एक) और `cancellation_comment` (वैकल्पिक free-text), customer द्वारा cancel करने पर populated
- **`subscription.cancelled` webhook**: payload में दोनों fields शामिल हैं
- **API**: cancellation को programmatically schedule या execute करते समय `cancellation_feedback` और `cancellation_comment` को `PATCH /subscriptions/{subscription_id}` में pass करें
...
आप individual payments, on-demand subscription charges और plan changes के लिए `adaptive_currency_fees_inclusive` boolean का उपयोग करके merchant default को override भी कर सकते हैं:
