...
<Info>
**Coming Soon**: Seats + Usage hybrid billing के लिए native support हमारे roadmap पर है। इससे आप एक ही subscription product में usage meters और seat add-ons दोनों attach कर सकेंगे।
</Info>
...
**प्रत्येक user में शामिल है:**
- 5 GB data processing/month
- Overage: allowance के बाद $2/GB

**Example calculation** (10-user team कुल 80 GB का उपयोग करती है):
...
चूँकि आप एक ही subscription में usage meters और add-ons दोनों attach नहीं कर सकते, इनमें से कोई एक approach चुनें:
...
Seat count track करें और seat fees की अलग से गणना करें।
...
Seat count के आधार पर included usage समायोजित करें।
...
Seats के लिए add-ons का उपयोग करें और on-demand charges के माध्यम से usage का manually charge करें।
...
Usage overages की manually गणना करें और charge करें।
...
<Info>
**Recommendation**: Option B (Seat Add-on + On-Demand Usage) लागू करना अक्सर आसान होता है क्योंकि Dodo seat billing को automatically manage करता है। आपको केवल usage overages track करके charge करने होंगे।
</Info>

---

## हाइब्रिड Model 5: Subscription + Seats + Usage (Triple Hybrid)

यह सबसे व्यापक model है: platform fee + per-user + consumption।

<Warning>
**Constraint**: Dodo Payments वर्तमान में एक ही subscription product में usage meters और add-ons दोनों attach करने का समर्थन नहीं करता। इस model के लिए workaround approach आवश्यक है।
</Warning>

<Info>
**Coming Soon**: Triple Hybrid billing (Base + Seats + Usage) के लिए native support हमारे roadmap पर है। इससे आप एक ही subscription product में usage meters और seat add-ons दोनों attach कर सकेंगे।
</Info>
...
**Example calculation** (20 users वाली company, 150,000 API calls):
- Platform: $199.00
- Seats: 20 × $25 = $500.00
...
Seat add-ons वाली subscription का उपयोग करें और on-demand charges के माध्यम से usage manually charge करें।
...
<Step title="Track Usage in Your Application">
अपने system में usage events store करें।
...
<Step title="Charge Usage at End of Cycle">
On-demand charge के माध्यम से usage overages की गणना करें और charge करें।
...
Usage meter वाली subscription का उपयोग करें और seat billing को अपने application में manage करें।
...
<Step title="Manage Seats in Your Application">
Seat count track करें और base subscription price को उसी के अनुसार समायोजित करें।
...
<Info>
**Recommendation**: Option A (Base + Seats + On-Demand Usage) सामान्यतः आसान है क्योंकि Dodo platform और seat billing को automatically manage करता है। आपको केवल usage track करके प्रत्येक billing cycle के अंत में charges submit करने होंगे।
</Info>

---

## हाइब्रिड Model 6: Tiered Base + Usage Overage

अलग-अलग included allowances और overage rates वाले विभिन्न subscription tiers।
...
प्रत्येक tier के लिए अलग subscription products बनाएँ और प्रत्येक में अपना usage configuration रखें:
...
जब ग्राहक tiers upgrade करते हैं, तो उन्हें मिलता है:
- अधिक included allowance
- कम overage rates
- प्रत्येक dollar के बदले अधिक value
...
## हाइब्रिड Model 7: Subscription + On-Demand Charges

Services या overages के लिए recurring subscription के साथ variable manual charges।
...
On-demand charging enabled वाली subscription सेट अप करें।
...
Services deliver होने पर charges बनाएँ।
...
## वास्तविक दुनिया के Examples

<Info>
ये examples आदर्श pricing structures दिखाते हैं। Usage meters और add-ons को एक ही product से attach न कर पाने की constraint के कारण, कुछ combinations में workarounds आवश्यक हैं (usage के लिए on-demand charges या app-managed seats का उपयोग करके)।
</Info>
...
- **Usage Overage**: 100K के बाद $0.02 प्रति 1,000 tokens (on-demand के माध्यम से charge)

**Implementation**: Seat और feature add-ons वाली subscription का उपयोग करें। अपने application में token usage track करें और billing cycle के अंत में on-demand charges के माध्यम से overages charge करें।
...
**Implementation**: Contact tier add-ons, feature add-ons और seat add-ons वाली subscription का उपयोग करें। अपने application में email sends track करें और on-demand charges के माध्यम से overages charge करें।
...
<Tip>
हाइब्रिड pricing को समझना आसान बनाएँ। अपनी pricing page पर base costs, क्या included है और overages कैसे काम करते हैं, इन्हें प्रमुखता से दिखाएँ।
</Tip>

**Good**: "\$49/month में 10,000 API calls शामिल हैं। अतिरिक्त calls: प्रत्येक \$0.005"

**Bad**: "\$49/month + usage fees"

### Cost Predictability

ग्राहकों को अपनी लागत का अनुमान लगाने में सहायता करें:
...
ग्राहकों को उनका usage real-time में दिखाएँ:
...
सभी components दिखाने वाले detailed invoices दें:
...
## हाइब्रिड Billing की Troubleshooting
...
**Symptoms**: Plans बदलते समय customer से unexpected amounts charge होते हैं।

**Solutions**:
1. Confirm करने से पहले exact charges दिखाने के लिए `previewChangePlan` API का उपयोग करें
2. Communicate करें कि proration subscription और add-ons दोनों पर लागू होती है
...
**Symptoms**: Customer से ऐसे usage के लिए charge होता है जो free होना चाहिए।
...
3. Confirm करें कि usage meter subscription product से सही तरह attach है
</Accordion>
...
2. API calls में add-on IDs सही हैं, यह check करें
3. Ensure करें कि add-on currency subscription product currency से match करती है
</Accordion>
</AccordionGroup>
...
<Card title="Subscription Management" icon="repeat" href="/features/subscription">
  Recurring subscriptions manage करना।
</Card>

<Card title="Add-ons" icon="puzzle" href="/features/addons">
  Add-ons के साथ subscriptions का विस्तार करना।
</Card>
</CardGroup>
