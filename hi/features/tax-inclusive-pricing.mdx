---
sidebarTitle: कर समावेशी मूल्य निर्धारण
title: कर समावेशी मूल्य निर्धारण
description: अंतिम, कर-समावेशी कीमतें पहले से दिखाएं। डोडो स्थान के अनुसार कर की
  गणना करता है और इसे चेकआउट और चालानों पर स्पष्ट रूप से दर्शाता है।
icon: file-invoice
keywords:
  - डोडो पेमेंट्स
  - कर समावेशी मूल्य निर्धारण
  - पेमेंट प्लेटफ़ॉर्म
  - सास बिलिंग
---

## कर समावेशी मूल्य निर्धारण क्या है?

कर समावेशी मूल्य निर्धारण आपको प्रदर्शित उत्पाद मूल्य में सीधे लागू करों को शामिल करने की अनुमति देता है। ग्राहक एक अंतिम राशि को पहले से देखते हैं, जिससे अधिक पारदर्शी और रूपांतरण-हितैषी अनुभव मिलता है। चेकआउट पर, डोडो ग्राहक के स्थान के आधार पर प्रदर्शित मूल्य से कर-बहिष्कृत भाग और कर राशि की गणना करता है, और उन्हें चालान पर स्पष्ट रूप से दर्शाता है।

<Info>
अंतिम कर अभी भी ग्राहक के स्थान और कर श्रेणी के अनुसार भिन्न होता है। चेकआउट और चालान प्रदर्शित मूल्य से निकाले गए गणना किए गए कर हिस्से को दिखाते हैं।
</Info>

## प्रमुख लाभ

- **ग्राहकों के लिए स्पष्टता**: एक अंतिम मूल्य पहले से प्रदर्शित किया गया
- **क्षेत्र के अनुसार अनुपालन**: कर स्थान के अनुसार गणना की जाती है और स्पष्ट रूप से दर्शाई जाती है
- **सटीक चालान**: चालान पर शुद्ध राशि और मूल्य से निकाली गई कर दिखाई देती है
- **रूपांतरण के अनुकूल**: चेकआउट पर आश्चर्य कम होते हैं

## उत्पाद सेटअप

अपने डैशबोर्ड में एक उत्पाद बनाएं या संपादित करें, फिर मूल्य निर्धारण सेटिंग्स के तहत कर-समावेशी मूल्य निर्धारण सक्षम करें।

#### मूल्य निर्धारण

- **मूल्य** (आवश्यक): वह आधार मूल्य जो ग्राहक को प्रदर्शित किया जाता है।
- **कर समावेशी मूल्य निर्धारण**: प्रदर्शित मूल्य को कर समावेशी के रूप में मानने के लिए टॉगल करें। डोडो ग्राहक के स्थान और कर श्रेणी के आधार पर चेकआउट पर शुद्ध (पूर्व-कर) राशि और कर भाग की गणना करेगा।

<Warning>
केवल भविष्य की खरीदारी ही अद्यतन tax‑inclusive सेटिंग्स को दर्शाती है। मौजूदा खरीदारी और सक्रिय सदस्यताएं पीछे की ओर से बदली नहीं जातीं।
</Warning>

<Tip>
ग्राहकों को एक नजर में कुल राशि समझ में आए इसके लिए “Tax inclusive” जैसे सुसंगत उत्पाद कॉपी का उपयोग करें।
</Tip>

## एपीआई प्रबंधन

<AccordionGroup>
<Accordion title="Create products (tax inclusive)">
`POST /products` का उपयोग उन उत्पादों को बनाने के लिए करें जिनमें `tax_inclusive` को `true` पर सेट किया गया है।

<Card title="API Reference" icon="code" href="/api-reference/products/post-products">
API के माध्यम से उत्पाद बनाएं
</Card>
</Accordion>

<Accordion title="Update products">
`PATCH /products/{id}` का उपयोग मौजूदा उत्पादों पर `tax_inclusive` को टॉगल करने के लिए करें।

<Card title="API Reference" icon="code" href="/api-reference/products/patch-products">
API के माध्यम से उत्पाद अपडेट करें
</Card>
</Accordion>

<Accordion title="Checkout Sessions">
INL‌INE_CODE_PLACEHOLDER_d2ce3d9f60c4199f_END का उपयोग उत्पादों को बेचने के लिए करें जिन्हें कर समावेशी के रूप में चिह्नित किया गया है। प्रदर्शित मूल्य समान रहता है; चेकआउट शुद्ध राशि और कर प्राप्त करता है।

<Card title="API Reference" icon="code" href="/api-reference/checkout-sessions/create">
चेकआउट सत्र बनाएँ
</Card>
</Accordion>

<Accordion title="Refunds (tax handling)">
रिफंड जारी करने के लिए `POST /refunds` का उपयोग करें। आंशिक, प्रति-आइटम रिफंड के लिए आप प्रत्येक एंट्री में `tax_inclusive` फ़्लैग का उपयोग करके यह निर्दिष्ट कर सकते हैं कि टैक्स शामिल है या नहीं। यह डिफ़ॉल्ट रूप से `true` होता है। भुगतान की पूरी राशि रिफंड करने के लिए `items` को छोड़ दें।

<Card title="API Reference" icon="code" href="/api-reference/refunds/post-refunds">
एक रिफंड बनाएं
</Card>
</Accordion>
</AccordionGroup>

## एकीकरण उदाहरण

### कर-समावेशी मूल्य निर्धारण के साथ उत्पाद बनाएं

```typescript
const product = await client.products.create({
  name: 'Pro Plan',
  description: 'All features included',
  tax_category: 'saas',
  price: {
    type: 'one_time_price',
    currency: 'USD',
    price: 10000,
    discount: 0,
    purchasing_power_parity: false,
    tax_inclusive: true
  }
});
```

### कर समावेशी टॉगल करने के लिए मौजूदा उत्पाद को अपडेट करें

```typescript
// `price` is a discriminated union, so send the complete price object,
// not just the field you want to change.
await client.products.update(product.product_id, {
  price: {
    type: 'one_time_price',
    currency: 'USD',
    price: 10000,
    discount: 0,
    purchasing_power_parity: false,
    tax_inclusive: true
  }
});
```

## सर्वोत्तम प्रथाएँ

- **सही कर श्रेणी चुनें** ताकि प्रत्येक क्षेत्र के अनुसार सही गणना सुनिश्चित हो।
- उत्पाद विवरण में “Tax Inclusive” लेबल के साथ **कुल राशि स्पष्ट रूप से बताएं**।
- सत्यापन के लिए **Test Mode में परीक्षण करें**।

## संबंधित

<CardGroup cols={2}>
<Card title="Purchasing Power Parity" icon="money-bill-trend-up" href="/features/purchasing-power-parity">
कीमत-संवेदनशील देशों में स्वचालित रूप से कम शुल्क लें।
</Card>

<Card title="Charm Pricing" icon="tag" href="/features/charm-pricing">
परिवर्तित कीमतों को साफ़-सुथरे और आकर्षक अंतिम अंकों में राउंड करें।
</Card>

<Card title="Products" icon="dollar-sign" href="/features/products">
प्रोडक्ट्स के लिए मूल्य निर्धारण विकल्पों को मैनेज करें।
</Card>

<Card title="Subscriptions" icon="repeat" href="/features/subscription">
उन्नत सेटिंग्स के साथ आवर्ती प्रोडक्ट्स को कॉन्फ़िगर करें।
</Card>

<Card title="Discount Codes" icon="percent" href="/features/discount-codes">
इनवॉइस की स्पष्टता बनाए रखते हुए प्रमोशन ऑफ़र करें।
</Card>
</CardGroup>
