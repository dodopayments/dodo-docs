...
सदस्यता अपडेट endpoint पर `status` फ़ील्ड के ज़रिए pause और resume किए जाते हैं। इसके लिए कोई अलग pause endpoint नहीं है।
...
<Warning>
`paused` या `active` को अकेले भेजें — इनमें से किसी को भी किसी अन्य फ़ील्ड के साथ मिलाने पर अनुरोध `422` के साथ अस्वीकार कर दिया जाता है। पुराना boolean `pause` फ़ील्ड हटा दिया गया है और अब यह हमेशा `422` के साथ विफल होता है, इसलिए अभी भी इसका उपयोग करने वाले caller को silent no-op के बजाय स्पष्ट error मिलता है।
</Warning>
...
| Status | इसका अर्थ | Recoverable? | Recovery path / अगला चरण |
|--------|---------------|--------------|---------------------------|
| `pending` | सदस्यता बनाई या process की जा रही है | — | `subscription.active` (या `subscription.failed`) का इंतज़ार करें |
| `active` | सदस्यता active है और अपने-आप renew होगी | — | किसी कार्रवाई की आवश्यकता नहीं |
| `on_hold` | renewal payment (या plan-change charge) विफल हो गया; renewals रोक दिए गए हैं | **Yes** | Payment method अपडेट करके इसे reactivate करें — [Payment Retries](/features/recovery/payment-retries) और [Dunning](/features/recovery/subscription-dunning) के ज़रिए अपने-आप, या [Update Payment Method API](/api-reference/subscriptions/update-payment-method) के ज़रिए manually |
| `paused` | सदस्यता को आपने या customer ने जानबूझकर pause किया है; billing रोक दी गई है और access revoke कर दिया गया है | **Yes** | इसे `status: active` के साथ resume करें, या self-service pause enabled होने पर Customer Portal से — [Pausing and Resuming Subscriptions](#pausing-and-resuming-subscriptions) देखें |
| `cancelled` | सदस्यता cancel कर दी गई है और renew नहीं होगी | केवल पुनः खरीदना | Customer को working payment method के साथ नई सदस्यता शुरू करनी होगी; [Dunning](/features/recovery/subscription-dunning) पुनः खरीदने के लिए prompt कर सकता है |
| `failed` | Subscription **creation** विफल रही (initial mandate या payment सफल नहीं हुआ) | **No — terminal** | Customer को working payment method के साथ नई सदस्यता बनानी होगी |
| `expired` | सदस्यता अपनी अवधि के अंत तक पहुँच गई है | — | यदि वांछित हो, तो Customer को नई सदस्यता शुरू करनी होगी |
...
Pause और resume, `status` फ़ील्ड का उपयोग करके उसी `PATCH /subscriptions/{subscription_id}` endpoint के ज़रिए किए जाते हैं: `status: paused` active सदस्यता को pause करता है और `status: active` उसे resume करता है। इनमें से किसी भी value को उसी request में किसी अन्य फ़ील्ड के साथ नहीं मिलाया जा सकता। पूरे behavior, billing effects और संबंधित business settings के लिए [Pausing and Resuming Subscriptions](#pausing-and-resuming-subscriptions) देखें।

<Card title="API Reference" icon="code" href="/api-reference/subscriptions/patch-subscriptions">
Update subscription API देखें, जिसमें `status` फ़ील्ड शामिल है।
</Card>
</Accordion>
