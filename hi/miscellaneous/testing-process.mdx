...
| क्षेत्र | ब्रांड | कार्ड नंबर | परिदृश्य | परिणामी `error_code` |
| :----- | :---- | :---------- | :------- | :--------------------- |
| US | Visa | `4000000000000002` | सामान्य अस्वीकृति | `GENERIC_DECLINE` |
| US | Visa | `4000000000009995` | अपर्याप्त धनराशि | `INSUFFICIENT_FUNDS` |
| India | Visa | `4706131211212123` | सामान्य अस्वीकृति | `GENERIC_DECLINE` |
| India | Mastercard | `5105105105105100` | सामान्य अस्वीकृति | `GENERIC_DECLINE` |
...
पूर्ण BNPL documentation, जिसमें Billie B2B testing शामिल है, के लिए [Buy Now Pay Later](/features/payment-methods/bnpl) पेज देखें।

## Digital Wallets का परीक्षण करें
...
अपने Apple Wallet में एक वास्तविक card जोड़ें। Test mode में card से charge नहीं लिया जाएगा।
</Step>
...
Test cards अपने wallet में अपने-आप जोड़ने के लिए [Google Pay test card group में शामिल हों](https://groups.google.com/g/googlepay-test-mode-stub-data)।
</Step>
...
<Step title="Complete test purchase">
Transaction पूरा करने के लिए Google Pay में दिए गए test cards में से किसी एक को चुनें।
</Step>
</Steps>

### Amazon Pay, Cash App Pay और RevolutPay

अपनी test API keys का उपयोग करें और standard checkout flow का पालन करें। Test transactions को वास्तविक charges के बिना simulate किया जाता है।

Apple Pay के लिए domain verification सहित, complete digital wallet documentation के लिए [Digital Wallets](/features/payment-methods/digital-wallets) पेज देखें।

## ACH Direct Debit का परीक्षण
...
इनमें से किसी भी account number के साथ routing number `110000000` का उपयोग करें:

| Account Number | Behavior |
| :------------- | :------- |
| `000123456789` | Payment सफल होता है। |
| `000222222227` | अपर्याप्त funds के कारण payment विफल होता है। |
| `000111111113` | Account बंद होने के कारण payment विफल होता है। |
| `000555555559` | Payment सफल होता है, फिर dispute trigger होता है। |
| `000000000009` | Payment अनिश्चित समय तक processing में रहता है। |

Test accounts की पूरी सूची के लिए [ACH Direct Debit](/features/payment-methods/ach#testing) page देखें।

## European Methods का परीक्षण

European payment methods (iDEAL, Bancontact, EPS, Multibanco) का Test Mode में परीक्षण किया जा सकता है।
...
Billing address country को payment method के अनुसार सेट करें:
- `NL` iDEAL के लिए
...
Test environment में simulated bank authentication flow का अनुसरण करें।
</Step>
</Steps>
...
| `DE35370400440532013002` | Payment सफल होता है, फिर तुरंत dispute हो जाता है। |
| `DE65370400440002222227` | अपर्याप्त funds के कारण payment विफल होता है। |

प्रत्येक country के values सहित test IBANs की पूरी सूची के लिए [Europe](/features/payment-methods/europe#sepa-test-ibans) page देखें।

European payment methods के complete documentation के लिए [Europe](/features/payment-methods/europe) page देखें।
...
सुनिश्चित करें कि आपके webhook endpoints सभी event types को सही तरीके से process करते हैं, विशेष रूप से `payment.succeeded`, `payment.failed` और subscription events को।

Webhooks को locally test करने के लिए [Dodo Payments CLI](/developer-resources/sdks/cli#webhooks) का उपयोग करें:
...
Test events के लिए webhooks सेट अप करें।
</Card>

<Card title="CLI Webhook Testing" icon="terminal" href="/developer-resources/sdks/cli#webhooks">
Dodo Payments CLI के साथ webhooks को locally test करें।
</Card>
</CardGroup>
