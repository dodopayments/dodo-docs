...
<Warning>
Includere sempre `credit` e `debit` come fallback. Non tutti i clienti sono idonei per BNPL e le transazioni al di sotto di $50,01 non saranno idonee.
</Warning>
...
Questo è il comportamento previsto. Non includere BNPL in `allowed_payment_method_types` per prodotti sotto i $50.
...
Per testare il fallimento: chiudi il modulo di Afterpay nella pagina di reindirizzamento. Lo stato del pagamento passa a `requires_payment_method`.
</Step>
</Steps>
...
### Mappatura delle valute e dei paesi
...
<Info>
Ad esempio, una transazione in USD mostrerà solo opzioni BNPL ai clienti negli Stati Uniti. Una transazione in EUR mostrerà opzioni BNPL in tutti i paesi europei supportati. Una transazione in GBP mostrerà opzioni BNPL ai clienti nel Regno Unito e in tutti i paesi europei supportati.
</Info>
...
3. La valuta è supportata dal fornitore di BNPL?
4. Il metodo BNPL è incluso in `allowed_payment_method_types`?

**Soluzione:** Nella maggior parte dei casi, la transazione è al di sotto del minimo. Verifica che l'importo soddisfi la soglia di $50,01.
</Accordion>
...
**Soluzione:** Questo è previsto per alcuni clienti. Assicurati che le fallback delle carte siano disponibili. Non esporre motivi specifici di diniego.
</Accordion>
