...
Definisci i crediti nella tua unità personalizzata: token, chiamate API, ore di calcolo o qualsiasi metrica significativa per il tuo prodotto. Le unità personalizzate utilizzano la precisione che imposti (da 0 a 10 cifre decimali).
...
| Fonte | Descrizione |
|--------|-------------|
| **Abbonamento** | Crediti emessi con l'acquisto di un abbonamento, riemessi a ogni ciclo di fatturazione |
| **Una tantum** | Crediti emessi con un prodotto a pagamento una tantum |
| **Add-on** | Crediti emessi con un add-on associato a un abbonamento |
| **API** | Crediti assegnati manualmente tramite API o dashboard |
| **Riporto** | Crediti riportati da un ciclo di fatturazione precedente |
...
- `0` - Numeri interi (ideale per elementi quantificabili come le chiamate API)
- `1` - Una cifra decimale (0,0)
- `2` - Due cifre decimali (0,00) - **predefinito**
- `3` - Tre cifre decimali (0,000)
- Fino a `10` - Per unità ad alta precisione (ad es. token frazionari o micro-utilizzo)
...
|-----------------|-------------|
| **Credito aggiunto** | Crediti assegnati (tramite abbonamento, pagamento una tantum o API) |
| **Credito detratto** | Crediti consumati tramite utilizzo o addebito manuale |
| **Credito scaduto** | Crediti scaduti senza essere riportati |
| **Credito riportato** | Crediti trasferiti al periodo successivo |
| **Riporto perso** | Crediti riportati persi dopo il raggiungimento del numero massimo di riporti |
| **Eccedenza addebitata** | Utilizzo oltre il saldo dei crediti con l'eccedenza abilitata |
| **Reimpostazione dell'eccedenza** | Eccedenza tracciata azzerata al momento della reimpostazione del ciclo di fatturazione |
| **Ricarica automatica** | Rifornimento automatico di crediti quando il saldo è basso |
| **Rettifica manuale** | Credito o addebito applicato manualmente dal merchant |
...
| Evento | Descrizione |
|-------|-------------|
| `credit.added` | Crediti assegnati a un cliente |
...
| `credit.rollover_forfeited` | Crediti persi al raggiungimento del numero massimo di riporti |
| `credit.overage_charged` | Addebiti per eccedenza applicati |
| `credit.overage_reset` | L'eccedenza tracciata è stata reimpostata al termine del ciclo di fatturazione |
| `credit.manual_adjustment` | Rettifica manuale del credito/addebito effettuata |
| `credit.balance_low` | Il saldo è sceso al di sotto della soglia configurata |

Tutti gli eventi del ledger (ogni evento `credit.*` tranne `credit.balance_low`) includono il payload completo `CreditLedgerEntry` con il saldo prima/dopo, l'eccedenza prima/dopo, il riferimento della fonte e l'`metadata` dell'abbonamento o del pagamento di origine dell'assegnazione (vuoto per le assegnazioni create direttamente tramite API). L'evento `credit.balance_low` include la configurazione della soglia e il saldo corrente.
