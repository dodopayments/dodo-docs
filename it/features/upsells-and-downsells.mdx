...
| **Maggiore conversione** | Riduci le frizioni nel momento decisionale |
| **Elaborazione istantanea** | Gli addebiti vengono elaborati immediatamente con `confirm: true` |
| **Esperienza utente fluida** | I clienti rimangono nella tua app durante tutto il flusso |
...
| `full_immediately` | Addebita l'intero prezzo del nuovo piano, ignora il tempo rimanente | Il ciclo di fatturazione si resetta |
...
<Info>
I crediti dai downgrade utilizzando `difference_immediately` sono limitati all'abbonamento e applicati automaticamente ai rinnovi futuri. Sono distinti dai [Crediti Cliente](/features/customer-credit).
</Info>
...
|--------|---------|--------|
| `payment.succeeded` | Pagamento upsell/cross-sell completato | Consegna prodotto, aggiorna accesso |
| `payment.failed` | Addebito con un clic fallito | Mostra errore, offri riprovare o ripiego |
| `subscription.plan_changed` | Upgrade/downgrade completato | Aggiorna funzionalità, invia conferma |
| `subscription.active` | Abbonamento riattivato dopo cambiamento del piano | Concedi accesso al nuovo livello |
