...
Dodo Payments abilita questi flussi attraverso il parametro `payment_method_id`, che ti consente di addebitare un metodo di pagamento salvato del cliente senza richiedere nuovamente i dettagli della carta.
...
| **Maggiore conversione** | Riduci il attrito al momento della decisione |
| **Elaborazione istantanea** | Gli addebiti vengono elaborati immediatamente con `confirm: true` |
| **Esperienza utente senza soluzione di continuità** | I clienti rimangono nella tua app durante tutto il flusso |
...
<Steps>
<Step title="Cliente con metodo di pagamento salvato">
...
<Step title="Prodotti configurati">
...
<Step title="Webhook Endpoint">
  Configura i webhook per gestire gli eventi `payment.succeeded`, `payment.failed` e `subscription.plan_changed`.
</Step>
</Steps>
...
| Modalità | Comportamento | Migliore per |
|------|----------|----------|
| `difference_immediately` | Addebita la differenza di prezzo istantaneamente ($30→$80 = $50) | Aggiornamenti semplici |
| `prorated_immediately` | Addebita in base al tempo rimanente nel ciclo di fatturazione | Fatturazione equa basata sul tempo |
| `full_immediately` | Addebita l'intero prezzo del nuovo piano, ignora il tempo rimanente | Il ciclo di fatturazione si ripristina |

<Tip>
Utilizza `difference_immediately` per flussi di aggiornamento semplici. Utilizza `prorated_immediately` quando desideri tenere conto del tempo non utilizzato sul piano attuale.
</Tip>
...
<Info>
I crediti derivanti da downgrade utilizzando `difference_immediately` sono limitati all'abbonamento e vengono applicati automaticamente ai rinnovi futuri. Sono distinti dai [Crediti Cliente](/features/customer-credit).
</Info>
...
<AccordionGroup>
<Accordion title="Tempistica strategica degli upsell">
...
<Accordion title="Valida l'idoneità del metodo di pagamento">
...
<Accordion title="Gestisci i fallimenti con grazia">
...
<Accordion title="Fornisci una chiara proposta di valore">
...
<Accordion title="Rispetta la scelta del cliente">
...
| Evento | Attivatore | Azione |
|-------|---------|--------|
| `payment.succeeded` | Pagamento di upsell/cross-sell completato | Consegna del prodotto, aggiornamento accesso |
| `payment.failed` | Addebito con un clic fallito | Mostra errore, offri riprovare o fallback |
| `subscription.plan_changed` | Aggiornamento/downgrade completato | Aggiorna le funzionalità, invia conferma |
| `subscription.active` | Abbonamento riattivato dopo il cambiamento del piano | Abilita l'accesso al nuovo livello |

<Card title="Guida all'integrazione dei webhook" icon="webhook" href="/developer-resources/webhooks">
...
<CardGroup cols={2}>
<Card title="Guida all'aggiornamento di abbonamento" icon="arrows-rotate" href="/developer-resources/subscription-upgrade-downgrade">
  Guida dettagliata sui cambiamenti di piano, modalità di proporzione e gestione dei fallimenti.
</Card>

<Card title="Sessioni di checkout" icon="cart-shopping" href="/developer-resources/checkout-session">
  Riferimento completo per la creazione di sessioni di checkout con tutte le opzioni.
</Card>

<Card title="API dei metodi di pagamento dei clienti" icon="credit-card" href="/api-reference/customers/get-customer-payment-methods">
  Riferimento API per l'elenco dei metodi di pagamento dei clienti.
</Card>

<Card title="Addon" icon="puzzle-piece" href="/features/addons">
  Migliora gli abbonamenti con addon flessibili per ulteriori ricavi.
</Card>
</CardGroup>
