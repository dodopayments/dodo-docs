...
- **Sconto (%)**: Sconto opzionale visualizzato nel checkout e nelle fatture.
- Per gli abbonamenti, impostare **Ripeti ogni** (es. 1 mese o 1 anno) e **Giorni di prova** se necessario. Il prezzo dell'abbonamento deve essere almeno **$1** (o l'equivalente nella valuta scelta); importi inferiori a questo minimo non sono supportati e l'abbonamento non funzionerà.
...
<Frame>
  <img src="/images/product-collection/checkout-page.png" alt="Collezioni di Prodotti" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
Per i prodotti ricorrenti, imposta l'oggetto `price` su un prezzo ricorrente (`type: recurring_price`) e specifica `payment_frequency_interval` e `subscription_period_interval` (`Day`, `Week`, `Month` o `Year`) insieme ai relativi conteggi.
...
Se non desideri più visualizzare o utilizzare un prodotto, puoi archiviarlo utilizzando una richiesta `DELETE` all'endpoint `/products/{id}`. Questa azione nasconde il prodotto ma non lo elimina in modo permanente.
