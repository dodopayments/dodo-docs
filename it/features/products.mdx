...
<AccordionGroup>
<Accordion title="Creare un Prodotto">
...
<Accordion title="Aggiornare un Prodotto">
Per modificare un prodotto esistente, invia una richiesta `PATCH` all'endpoint `/products/{product_id}`. Puoi aggiornare proprietà come nome, prezzo e descrizione mantenendo invariati gli altri dettagli.
...
<Accordion title="Recuperare Prodotti">
Puoi recuperare un elenco di prodotti memorizzati nel tuo account utilizzando una richiesta `GET` all'endpoint `/products`. Questo ti consente di recuperare dettagli sui prodotti, inclusi quelli attivi e archiviati.
...
<Accordion title="Caricamento Immagini del Prodotto">
Puoi associare un'immagine a un prodotto caricandola su AWS S3 utilizzando un URL pre-firmato fornito dall'API. Prima, richiedi un URL per il caricamento dell'immagine all'endpoint `/products/{product_id}/images`, quindi utilizza l'URL fornito per caricare l'immagine entro 60 secondi.

<Warning>
L'URL prefirmato scade in 60 secondi, quindi l'immagine deve essere caricata entro quel lasso di tempo.
</Warning>

Una volta ricevuto l'URL pre-firmato dall'API, carica l'immagine utilizzando il metodo `PUT`. Questo assicura un accesso sicuro e temporaneo a AWS S3 per il caricamento dell'immagine.
...
Se il caricamento è riuscito, AWS S3 restituirà uno stato `200 OK`, indicando che l'immagine è stata memorizzata correttamente.
...
<Accordion title="Archiviare un Prodotto">
Se non desideri più visualizzare o utilizzare un prodotto, puoi archiviarlore inviando una richiesta `POST` all'endpoint `/products/{product_id}/archive`. Questa azione nasconde il prodotto ma non lo elimina permanentemente.
...
<Accordion title="Ripristinare un Prodotto">
Se hai bisogno di ripristinare un prodotto archiviato, invia una richiesta `POST` all'endpoint `/products/{product_id}/unarchive`. Questo riattiverà il prodotto e lo renderà nuovamente disponibile per l'uso.
...
<Check>
Sei pronto a creare prodotti e iniziare a vendere - una tantum, ricorrenti o per uso.
</Check>

## Correlati

<Card title="Analisi dei Prodotti" icon="chart-mixed" href="/features/analytics-and-reporting#product-level-analytics">
Monitora ricavi, clienti, retention, abbonati e MRR per ciascun prodotto individuale.
</Card>
