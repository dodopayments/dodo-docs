...
Un abbonamento Team Pro costa $99/mese e include 5 posti. Se hai più di 5 utenti, paghi un supplemento di $15/mese per ogni posto aggiuntivo.

Ad esempio, se il tuo team ha bisogno di 15 posti:
- Piano Base: $99/mese (incluse 5 posti)
- Add-ons: 10 posti extra × $15/mese = $150/mese
...
| Modalità | Aggiunta Posti | Rimozione Posti |
|------|-------------|----------------|
| `prorated_immediately` | Addebito per i giorni rimanenti nel ciclo | Credito per i giorni non utilizzati |
| `difference_immediately` | Addebito del prezzo completo del posto | Credito applicato per rinnovi futuri |
| `full_immediately` | Addebito del prezzo completo del posto, ripristino del ciclo di fatturazione | Nessun credito |

### Esempi di Prorata

**Scenario: 15 giorni rimanenti nel ciclo di fatturazione, aggiunta di 5 posti a $10/posto**
...
Il cliente paga l'intero importo di $50 ora, indipendentemente dalla posizione nel ciclo.
</Tab>
...
<Tip>
**Scelta di una modalità di prorata per le modifiche ai posti**: Usa `prorated_immediately` per una fatturazione equa basata sui giorni quando i team modificano frequentemente i posti. Usa `difference_immediately` per un calcolo più semplice che addebita o accredita il prezzo completo del posto. Vedi la [Guida sulla Prorata](/developer-resources/subscription-upgrade-downgrade#proration-modes) per confronti dettagliati.
</Tip>

### Anteprima Prima delle Modifiche

Controlla sempre la prorata prima di effettuare modifiche:
...
## Monitoraggio dei Posti con i Webhook

Monitora le modifiche ai posti ascoltando i webhook delle sottoscrizioni:
...
| `subscription.renewed` | Sottoscrizione rinnovata | Conferma che il conteggio dei posti non sia cambiato |
| `subscription.cancelled` | Sottoscrizione annullata | Deprovvisionare tutti i posti |
...
## Esecuzione dei Limiti di Posti

La tua applicazione deve eseguire i limiti di posti. Dodo Payments tiene traccia della fatturazione, ma tu controlli l'accesso.

### Strategie di Esecuzione

<Tabs>
<Tab title="Limite Rigido">
Previeni rigorosamente l'aggiunta di utenti oltre il numero di posti.
...
<Tab title="Auto-Aggiornamento">
Aggiungi automaticamente posti quando si raggiunge il limite.
...
### Tipi di Posti Differenti

Offri diversi tipi di posti con diversi prezzi:
...
**Implementazione:** Crea add-ons separati per ciascun tipo di posto.
...
Offri un prezzo scontato per i posti annuali:
...
**Implementazione:** Crea prodotti separati per piani mensili e annuali con prezzi add-on diversi.

### Requisiti Minimi di Posti

Richiedi un numero minimo di posti per alcuni piani:
...
- **Comunicazione Chiara**: Mostra il prezzo per posto in modo evidente sulla tua pagina dei prezzi
- **Posti Inclusi**: Considera di includere alcuni posti nel prezzo base per ridurre l'attrito
- **Sconti per Volume**: Offri tariffe inferiori per posto per team più grandi per vincere contratti aziendali
- **Incentivi Annuali**: Sconta i piani annuali per migliorare il flusso di cassa e la fidelizzazione

### Migliori Pratiche Tecniche

- **Memorizza in Cache i Conteggi dei Posti**: Memorizza in cache i conteggi dei posti della sottoscrizione localmente per evitare chiamate API a ogni richiesta
- **Sincronizzazione Regolare**: Sincronizza periodicamente il tuo conteggio locale dei posti con Dodo Payments tramite API
- **Gestione dei Fallimenti**: Se un cambiamento del posto fallisce, mostra messaggi di errore chiari e opzioni di ripetizione
...
- **Feedback in Tempo Reale**: Mostra immediatamente l'impatto sui costi quando si modificano i posti
- **Passaggi di Conferma**: Richiedi conferma prima di apportare modifiche alla fatturazione
- **Trasparenza nella Prorata**: Spiega chiaramente le spese prorate prima di applicarle
- **Facilità nei Declassamenti**: Non rendere difficile ridurre i posti (costruisce fiducia)
...
**Sintomo**: La tua app mostra un conteggio dei posti diverso rispetto alla sottoscrizione.

**Cause**:
- Webhook non ricevuto o elaborato
- Condizione di gara durante il cambiamento del posto
- Dati memorizzati in cache non aggiornati

**Soluzioni**:
1. Implementa gestori webhook per `subscription.plan_changed`
2. Aggiungi un pulsante "Sincronizza con la fatturazione" che recupera la sottoscrizione corrente
3. Imposta un TTL per la cache per garantire un aggiornamento regolare
</Accordion>

<Accordion title="Spese di prorata inaspettate">
**Sintomo**: Il cliente è confuso riguardo all'importo dell'addebito a metà ciclo.
...
**Sintomo**: L'add-on per i posti non è disponibile durante il checkout.

**Cause**:
- Add-on non collegato al prodotto
- Add-on archiviato o eliminato
- Differenza di valuta tra prodotto e add-on

**Soluzioni**:
1. Verifica che l'add-on sia collegato nelle impostazioni del prodotto
2. Controlla lo stato dell'add-on nel cruscotto degli Add-Ons
3. Assicurati che le valute corrispondano esattamente
</Accordion>

<Accordion title="Impossibile ridurre i posti al di sotto dell'uso corrente">
...
<CardGroup cols={2}>
<Card title="Tutorial di Prezzi Basati sui Posti" icon="code" href="/developer-resources/seat-based-pricing">
  Guida completa all'implementazione con codice.
</Card>

<Card title="Add-ons" icon="puzzle" href="/features/addons">
  Comprendi il sistema di add-on in dettaglio.
</Card>

<Card title="Modifiche ai Piani & Prorata" icon="arrows-rotate" href="/developer-resources/subscription-upgrade-downgrade">
  Gestisci le modifiche alla sottoscrizione.
</Card>

<Card title="Webhook per le Sottoscrizioni" icon="bell" href="/developer-resources/webhooks/intents/subscription">
  Tieni traccia degli eventi di sottoscrizione.
</Card>
</CardGroup>
