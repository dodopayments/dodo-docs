...
Quando un cliente acquista un prodotto in modalità manuale, la concessione viene creata nello stato `pending` senza chiave e un [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) webhook viene attivato con `integration_type: "license_key"` e `status: "pending"`. Puoi reagire a quel webhook o interrogare l'endpoint [List Customer Grants](/api-reference/entitlements/list-customer-grants) con i filtri `integration_type` e `status`:
