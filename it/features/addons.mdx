...
- **Pricing**: Imposta l'importo dell'add-on (`price`) nell'unità monetaria minima. L'add-on viene addebitato in base al ciclo dell'abbonamento principale
- **Currency**: Imposta il prezzo di base in qualsiasi valuta che Dodo Payments può addebitare. Il selettore di valuta ricercabile mantiene **USD, GBP, EUR e INR** in cima; ai clienti al di fuori della tua valuta di base viene addebitato l'importo tramite [Adaptive Currency](/features/adaptive-currency)
- **Quantity**: I clienti scelgono una quantità quando l'add-on viene associato a un abbonamento
- **Availability**: Controlla quali prodotti in abbonamento possono utilizzare l'add-on
- **Tax settings**: Configura il parametro `tax_category`
