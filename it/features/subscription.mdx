...
- **Tipo di Prezzo**: Scegliere <b>Abbonamento</b> (questa guida). Le alternative sono Pagamento Singolo e Fatturazione Basata sull'Uso.
- **Prezzo** (obbligatorio): Prezzo ricorrente base con valuta. Il prezzo deve essere almeno **$1** (o l'equivalente nella valuta scelta). Importi inferiori a questo minimo non sono supportati e l'abbonamento non funzionerà.
- **Sconto Applicabile (%)**: Percentuale di sconto opzionale applicata al prezzo base; riflessa nel checkout e nelle fatture.
- **Ripeti pagamento ogni** (obbligatorio): Intervallo per i rinnovi, ad esempio, ogni 1 Mese. Selezionare la cadenza (mesi o anni) e la quantità.
- **Periodo di Abbonamento** (obbligatorio): Termine totale per cui l'abbonamento rimane attivo (ad esempio, 10 Anni). Dopo la fine di questo periodo, i rinnovi si fermano a meno che non venga esteso.
- **Giorni di Periodo di Prova** (obbligatorio): Impostare la durata della prova in giorni. Usare 0 per disabilitare le prove. Il primo addebito avviene automaticamente alla fine della prova.
- **Seleziona componente aggiuntivo**: Allegare fino a 10 componenti aggiuntivi che i clienti possono acquistare insieme al piano base.
...
|---|---|---|---|---|
| **Upgrade** | Addebito proporzionale per i giorni rimanenti | Addebitata la differenza di prezzo intero | Addebitato il prezzo intero del nuovo piano | Nessun addebito — cambio immediato |
| **Downgrade** | Credito proporzionale per i giorni rimanenti | Differenza di prezzo intero come credito | Nessun credito, addebito intero | Nessun credito — cambio immediato |
| **Ciclo di fatturazione** | Ripristinato a oggi | Ripristinato a oggi | Ripristinato a oggi | Rimane lo stesso |
| **Ideale per** | Fatturazione equa basata sul tempo | Cambi di livello semplici | Ripristini del ciclo di fatturazione | Migrazioni gratuite o cambi di cortesia |
</Info>
...
Prossimo rinnovo il **15 febbraio** (16 gennaio + 30 giorni): **$80.00/mese**.
...
## Subscriptions with RBI-Compliant Mandates
  ...
  - **Addebiti al di sotto della soglia del mandato (default ₹15,000):** Creiamo un mandato su richiesta per l'importo minimo. L'importo dell'abbonamento viene addebitato periodicamente secondo la frequenza di abbonamento, fino al limite del mandato.
  - **Addebiti alla soglia del mandato o superiori:** Creiamo un mandato di abbonamento (o un mandato su richiesta) per l'importo esatto dell'abbonamento.

  La soglia del mandato è configurabile per ciascun commerciante o per richiesta tramite `mandate_min_amount_inr_paise` (INR paise). L'importo registrato presso la banca è `max(mandate_floor, billing_amount)` — quindi la soglia diventa effettivamente il tetto di autorizzazione visibile al cliente quando la fatturazione è inferiore.

Per informazioni dettagliate sui mandati conformi alla RBI e sulla soglia del mandato configurabile per i metodi di pagamento indiani, consultare la pagina <a href="/features/payment-methods/india#configurable-mandate-floor">Metodi di Pagamento in India</a>.

  ### Considerazioni su Upgrade e Downgrade

  **Importante:** Quando si eseguono upgrade o downgrade degli abbonamenti, considerare attentamente i limiti del mandato:

  - Se un upgrade/downgrade comporta un importo dell'addebito che supera Rs 15,000 e va oltre il limite di pagamento su richiesta esistente, l'addebito della transazione potrebbe fallire.
  ...
  - Il cliente sarà invitato dalla propria banca ad autorizzare la transazione.
  - Se il cliente non riesce ad autorizzare la transazione, la transazione fallirà e l'abbonamento sarà sospeso.

  ### Ritardo di Elaborazione di 48 Ore

  **Sequenza di Elaborazione:** Gli addebiti ricorrenti su carte indiane e abbonamenti UPI seguono un modello di elaborazione unico:

  - Gli addebiti sono **iniziati** alla data programmata secondo la frequenza di abbonamento.
  - L'effettivo **addebito** dal conto del cliente avviene solo dopo **48 ore** dall'inizio del pagamento.
  ...
  - I clienti possono annullare il mandato tramite le app bancarie.
  - Se un cliente annulla il mandato durante questo periodo, l'abbonamento rimarrà **attivo** (questo è un caso specifico per abbonamenti con carte indiane e UPI AutoPay).
  - Tuttavia, l'effettivo addebito potrebbe fallire e, in tal caso, metteremo l'abbonamento **in pausa**.

  **Gestione dei Casi Particolari:** Se fornirete vantaggi, crediti o utilizzi di abbonamento ai clienti immediatamente all'inizio dell'addebito, sarà necessario gestire appropriatamente questa finestra di 48 ore nella vostra applicazione. Considerare:
  ...
  - Gestire stati di sospensione dell'abbonamento nella logica dell'applicazione

  <Tip>
  Monitorare i webhook degli abbonamenti per tracciare i cambiamenti di stato dei pagamenti e gestire i casi particolari in cui i mandati sono annullati durante la finestra di 48 ore.
  </Tip>

## Migliori Pratiche

- **Iniziare con livelli chiari**: 2–3 piani con differenze evidenti
- **Comunicare i prezzi**: Mostrare i totali, le prorate e il prossimo rinnovo
- **Usare i trial in modo oculato**: Convertire con l'onboarding, non solo con il tempo
- **Sfruttare i componenti aggiuntivi**: Mantenere i piani base semplici e vendere extra
- **Testare le modifiche**: Convalidare i cambiamenti dei piani e le prorate in modalità di test

<Info>
Gli abbonamenti sono una base flessibile per i ricavi ricorrenti. Iniziare in modo semplice, testare a fondo e iterare in base ai metriche di adozione, churn e espansione.
</Info>

  Durante la finestra di elaborazione di 48 ore:

  - I clienti possono annullare il mandato tramite le loro app bancarie.
  - Se un cliente annulla il mandato durante questo periodo, l'abbonamento rimarrà **attivo** (questo è un caso limite specifico per carte indiane e abbonamenti UPI AutoPay).
  - Tuttavia, l'effettiva detrazione potrebbe fallire e, in tal caso, metteremo l'abbonamento **in attesa**.

  **Gestione dei Casi Limite:** Se offri vantaggi, crediti o l'utilizzo dell'abbonamento ai clienti immediatamente all'inizio dell'addebito, è necessario gestire adeguatamente questa finestra di 48 ore nella tua applicazione. Considera di:
  ...
  - Gestire stati di attesa dell'abbonamento nella logica della tua applicazione

  <Tip>
  Monitora i webhook degli abbonamenti per tenere traccia dei cambiamenti nello stato dei pagamenti e gestire i casi limite in cui i mandati vengono cancellati durante la finestra di 48 ore.
  </Tip>
...
- **Comunica il prezzo**: Mostra i totali, le prorrate e il prossimo rinnovo
- **Usa le prove in modo ponderato**: Converti con l'onboarding, non solo con il tempo
- **Sfrutta gli extra**: Mantieni i piani base semplici e vendi extra
- **Testa le modifiche**: Valida le modifiche ai piani e la prorrata in modalità di test

<Info>
Gli abbonamenti sono una base flessibile per i ricavi ricorrenti. Inizia in modo semplice, testa a fondo e itera basandoti su metriche di adozione, abbandono e espansione.
</Info>