...
La sospensione e la ripresa utilizzano un unico campo `pause` sull'endpoint di aggiornamento dell'abbonamento. Non esiste un endpoint separato per la sospensione.
...
<Warning>
`pause` è esclusivo rispetto a ogni altro campo: se lo invii insieme ad altri dati, la richiesta viene rifiutata con `422`. Impostare `status` su `paused` **non** sospende un abbonamento; usa invece il campo `pause`.
</Warning>
...
| `active` | L'abbonamento è attivo e si rinnoverà automaticamente | — | Non è richiesta alcuna azione |
| `on_hold` | Un pagamento di rinnovo (o un addebito per la modifica del piano) non è riuscito; i rinnovi sono stati interrotti | **Sì** | Aggiorna il metodo di pagamento per riattivarlo, automaticamente tramite [Payment Retries](/features/recovery/payment-retries) e [Dunning](/features/recovery/subscription-dunning), oppure manualmente tramite [Update Payment Method API](/api-reference/subscriptions/update-payment-method) |
| `paused` | L'abbonamento è stato sospeso deliberatamente da te o dal cliente; la fatturazione è bloccata e l'accesso è revocato | **Sì** | Riprendilo con `pause: false` o dal Customer Portal quando la sospensione self-service è abilitata; consulta [Sospensione e ripresa degli abbonamenti](#pausing-and-resuming-subscriptions) |
| `cancelled` | L'abbonamento è stato cancellato e non si rinnoverà | Solo nuovo acquisto | Il cliente deve avviare un nuovo abbonamento; [Dunning](/features/recovery/subscription-dunning) può sollecitare un nuovo acquisto |
| `failed` | La **creazione** dell'abbonamento non è riuscita (il mandato o il pagamento iniziale non è andato a buon fine) | **No — terminale** | Il cliente deve creare un nuovo abbonamento con un metodo di pagamento funzionante |
| `expired` | L'abbonamento ha raggiunto la fine del periodo previsto | — | Se lo desidera, il cliente deve avviare un nuovo abbonamento |
...
La sospensione e la ripresa passano dallo stesso endpoint `PATCH /subscriptions/{subscription_id}` utilizzando il campo `pause`: `pause: true` sospende un abbonamento attivo e `pause: false` lo riprende. Il campo non può essere combinato con altri campi nella stessa richiesta. Per informazioni complete sul comportamento, sugli effetti di fatturazione e sulle impostazioni aziendali correlate, consulta [Sospensione e ripresa degli abbonamenti](#pausing-and-resuming-subscriptions).

<Card title="API Reference" icon="code" href="/api-reference/subscriptions/patch-subscriptions">
Visualizza l'API per aggiornare un abbonamento, incluso il campo `pause`.
</Card>
</Accordion>
