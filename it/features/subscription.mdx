...
| Rinnovo completato | `subscription.renewed` |
| Rinnovo non riuscito → in pausa | `subscription.on_hold` |
| Abbonamento in pausa | `subscription.paused` |
| Abbonamento in pausa ripreso | `subscription.unpaused` |
| Creazione non riuscita | `subscription.failed` |
| Piano aggiornato/declassato | `subscription.plan_changed` |
| Annullato | `subscription.cancelled` |
| Termine del periodo | `subscription.expired` |
...
Usa `PATCH /subscriptions/{subscription_id}` per annullare alla prossima data di fatturazione, estendere il periodo dell'abbonamento, aggiornare i dettagli di fatturazione o modificare i metadati. Per modificare la quantità, usa invece la [Change Plan API](/api-reference/subscriptions/change-plan) — `PATCH` non accetta `quantity`.
...
### Estendere il periodo dell'abbonamento
