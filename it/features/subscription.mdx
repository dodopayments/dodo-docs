...
Usa `PATCH /subscriptions/{id}` per annullare alla prossima data di fatturazione, estendere il periodo della subscription, aggiornare i dettagli di fatturazione o modificare i metadati. Per modificare la quantità, usa invece la [Change Plan API](/api-reference/subscriptions/change-plan): `PATCH` non accetta `quantity`.
