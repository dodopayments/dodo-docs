...
<Info>
La pagina Design è il tuo hub centrale per il branding di tutte le superfici rivolte ai clienti: checkout, storefront e Customer Portal. Le modifiche vengono applicate istantaneamente a tutte e tre e visualizzate in anteprima in tempo reale prima del salvataggio. La modalità test e la modalità live hanno design separati — consulta [Copy Design From Test to Live Mode](#copy-design-from-test-to-live-mode).
</Info>

## Punti chiave

1. **Hub di design unificato** - Configura l’aspetto del checkout, dello storefront e del Customer Portal da un’unica pagina.
2. **Temi predefiniti** - Inizia con un tema selezionato e personalizzalo da lì.
3. **Modalità chiara e scura** - Definisci palette di colori separate per ogni modalità.
4. **Anteprima in tempo reale** - Visualizza l’aspetto delle modifiche su checkout, Customer Portal e storefront prima del salvataggio.
5. **Override per sezione** - Perfeziona singole sezioni (checkout, storefront, Customer Portal) senza influire sulle altre.
6. **Controllo programmatico** - Sostituisci i temi al momento del checkout tramite API o Checkout SDK.
7. **Copia il design dal test alla modalità live** - Trasferisci in un unico passaggio un design perfezionato in modalità test alla modalità live.
...
## Copia il design dalla modalità test alla modalità live

La modalità test e la modalità live mantengono configurazioni di design separate, quindi un tema creato durante i test non viene visualizzato automaticamente dai clienti reali. Invece di ricrearlo manualmente, copia il design dalla modalità test alla modalità live in un unico passaggio.

La copia include **tutte le superfici di design** — impostazioni generali, checkout, Customer Portal e override dello storefront — insieme al branding del tuo storefront.

<Warning>
La copia sovrascrive il design esistente della modalità live. Dopo la copia, controlla il checkout e lo storefront live per verificare che tutto abbia l’aspetto previsto.
</Warning>
...
<Card title="Checkout Features" icon="cart-shopping" href="/features/checkout">
  Funzionalità di checkout tra cui valuta, lingua, imposte, sconti e altro.
</Card>
...
<Card title="Storefront" icon="store" href="/features/storefront">
  Configura e gestisci il tuo storefront con il tuo branding.
</Card>

<Card title="Customer Portal" icon="user" href="/features/customer-portal">
  Portale self-service per i clienti per gestire abbonamenti e fatturazione.
</Card>
