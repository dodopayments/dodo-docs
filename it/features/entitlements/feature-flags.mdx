...
| Trigger | Effetto |
|---|---|
| Il pagamento una tantum va a buon fine / l'abbonamento diventa attivo | Viene creato un grant con `status: Delivered` e `delivered_at` impostati. |
| L'abbonamento viene sospeso, annullato o scade | Il grant viene revocato con il corrispondente `revocation_reason`. |
| Rimborso per un pagamento una tantum | Il grant viene revocato con `revocation_reason: refund`. |
| L'abbonamento si ripristina (ad esempio, il recupero crediti va a buon fine) | Il grant revocato viene ripristinato in `Delivered` — stesso grant `id`, campi di revoca cancellati. |
| Revoca manuale tramite API | Il grant viene revocato con `revocation_reason: manual`. Non viene ripristinato automaticamente al rinnovo. |
