...
<Steps>
<Step title="Created">
Un grant viene creato quando un pagamento viene completato o un abbonamento diventa attivo. I feature flag passano direttamente a `Delivered`. Anche le license key passano direttamente a `Delivered` quando l'entitlement utilizza `fulfillment_mode: auto` (il valore predefinito); con `fulfillment_mode: manual` il grant viene creato in `Pending` senza una chiave, finché non ne fornisci una tramite [Completa il grant della license key](/api-reference/entitlements/fulfill-license-key). Ogni altra integrazione inizia in `Pending`. Le integrazioni basate su OAuth (Discord, GitHub, Notion) includono un `oauth_url` che il cliente deve visitare per completare il consenso. Le integrazioni dirette alla piattaforma (Telegram, Framer, Digital Files) rimangono in `Pending` solo per il breve periodo necessario al provisioning della consegna, quindi passano a `Delivered`.
</Step>
