...
Assicurati che i tuoi endpoint webhook elaborino correttamente tutti i tipi di evento, in particolare `payment.succeeded`, `payment.failed` e gli eventi di sottoscrizione.

Usa la [Dodo Payments CLI](/developer-resources/sdks/cli#webhooks) per testare i webhook localmente:
- `dodo wh listen` inoltra i webhook di test live al tuo server locale
- `dodo wh trigger` invia payload mock per tutti i tipi di evento webhook supportati
</Accordion>
