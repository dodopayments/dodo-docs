...
|----------------|------------------|-------------|
| `AUTHENTICATION_FAILURE` | Sì | Il processo di autenticazione non è riuscito durante la transazione |
...
| `CARD_VELOCITY_EXCEEDED` | Sì | Sono state effettuate troppe transazioni in un breve periodo |
...
| `INCORRECT_CVC` | Sì | Il codice CVC fornito non era corretto |
| `INCORRECT_NUMBER` | Sì | Il numero della carta è stato inserito in modo errato |
| `INSUFFICIENT_FUNDS` | Sì | Il conto non dispone di fondi sufficienti per completare la transazione |
...
| `INVALID_CARD_OWNER` | Sì | Le informazioni sul titolare della carta non sono valide |
| `INVALID_CVC` | Sì | Il formato del CVC non è valido |
| `INVALID_EXPIRY_YEAR` | Sì | L’anno di scadenza della carta non è valido |
| `INVALID_PIN` | Sì | Il PIN fornito non è corretto |
...
| `LIVE_MODE_TEST_CARD` | Sì | È stata usata una carta di test in modalità live |
| `LOST_CARD` | Sì | La carta è stata segnalata come smarrita |
| `MANDATE_INVALID` | Sì | Il mandato di pagamento non è valido |
| `MANDATE_REQUIRED` | Sì | Per questa transazione è richiesto un mandato |
...
| `PROVIDER_UNSUPPORTED` | No | Il provider di pagamento non supporta questo tipo di transazione |
...
| `TRANSACTION_NOT_ALLOWED` | Sì | La transazione non è consentita per questa carta o questo conto |
| `TRANSACTION_NOT_APPROVED` | Sì | La transazione non è stata approvata |
| `TRY_AGAIN_LATER` | No | La transazione dovrebbe essere riprovata più tardi |
| `UNKNOWN_ERROR` | No | Si è verificato un errore sconosciuto |

<Note>
  **Errore dell’utente** indica se il rifiuto del pagamento può essere risolto dal cliente. Quando `true`, il cliente può intervenire per risolvere il problema (ad esempio, inserendo i dati della carta corretti). Quando `false`, il rifiuto è dovuto a problemi a livello di sistema o restrizioni bancarie che il cliente non può risolvere.
</Note>
