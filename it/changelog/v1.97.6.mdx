...
- **Subscription object**: Nuovo campo `cancellation_feedback` (uno dei valori sopra) e `cancellation_comment` (testo libero facoltativo), valorizzati quando il cliente annulla
- **`subscription.cancelled` webhook**: Entrambi i campi sono inclusi nel payload
- **API**: Passa `cancellation_feedback` e `cancellation_comment` a `PATCH /subscriptions/{subscription_id}` durante la pianificazione o l'esecuzione programmatica di un'annullamento
