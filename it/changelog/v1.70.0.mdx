...
1. **Checkout Inline - Esperienze di Checkout Completamente Incorporate (BETA)**  
   ...
   - **Eventi di breakdown in tempo reale**: Ricevi eventi di `checkout.breakdown` mentre i clienti inseriscono i loro dettagli
   - **Riepiloghi degli ordini personalizzati**: Crea la tua visualizzazione dei prezzi che si aggiorna in tempo reale
   - **Integrazione senza soluzione di continuità**: Il frame di checkout gestisce la raccolta dei pagamenti mentre tu controlli il layout
   - **Metodi SDK**: Usa `open()`, `close()`, e `isOpen()` per controllare il checkout in modo programmatico
...
2. **Crea Pagamenti, Abbonamenti e Checkout Utilizzando Metodi di Pagamento Esistenti**  
    Semplifica i flussi di checkout utilizzando metodi di pagamento salvati da clienti esistenti. Il nuovo parametro `payment_method_id` consente di creare pagamenti, abbonamenti o sessioni di checkout utilizzando un metodo di pagamento precedentemente salvato dal cliente.
   ...
   <Warning>
   Quando utilizzi `payment_method_id` nelle sessioni di checkout, `confirm` deve essere impostato su `true`, e deve essere fornito un `customer_id` esistente. Il metodo di pagamento sarà convalidato per idoneità con la valuta del pagamento.
   </Warning>
...
5. **`redirect_immediately` Flag - Salta la Pagina di Successo del Pagamento**  
   Controlla il flusso di checkout con il nuovo flag `redirect_immediately`. Quando abilitato, i clienti vengono reindirizzati immediatamente dopo il completamento del pagamento, saltando la pagina di successo predefinita per un'esperienza più veloce e semplificata.
