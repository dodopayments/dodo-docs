...
1. **Checkout Inline - Esperienze di Checkout Completamente Incorporate (BETA)**  
   ...
   - **Eventi di contenimento in tempo reale**: Ricevi `checkout.breakdown` eventi quando i clienti inseriscono i loro dettagli
   - **Riepiloghi degli ordini personalizzati**: Costruisci il tuo display prezzi che si aggiorna in tempo reale
...
2. **Crea Pagamenti, Abbonamenti e Checkouts Utilizzando Metodi di Pagamento Esistenti**  
    Semplifica i flussi di checkout utilizzando metodi di pagamento salvati da clienti esistenti. Il nuovo parametro `payment_method_id` consente di creare pagamenti, abbonamenti o sessioni di checkout utilizzando un metodo di pagamento salvato in precedenza da un cliente.
   ...
   Usa `payment_method_id` durante la creazione di sessioni di checkout, pagamenti o abbonamenti:
   ...
   <Warning>
   Quando si utilizza `payment_method_id` nelle sessioni di checkout, `confirm` deve essere impostato su `true`, e deve essere fornito un `customer_id` esistente. Il metodo di pagamento sarà convalidato per idoneità con la valuta del pagamento.
   </Warning>
...
5. **`redirect_immediately` Flag - Salta la Pagina di Successo del Pagamento**  
   Controlla il flusso di checkout con il nuovo flag `redirect_immediately`. Quando abilitato, i clienti vengono reindirizzati immediatamente dopo il completamento del pagamento, bypassando la pagina di successo predefinita per un'esperienza più veloce e fluida.
  ...
  ```
   ...
   <Tip>
   Usa `redirect_immediately: true` quando hai una pagina di successo personalizzata che offre un'esperienza utente migliore rispetto alla pagina di successo predefinita.
   </Tip>

   <Note>
   Quando `redirect_immediately` è abilitato, i clienti vengono reindirizzati a `return_url` immediatamente dopo il completamento del pagamento, saltando completamente la pagina di successo predefinita.
   </Note>
