...
<CardGroup cols={2}>
<Card title="Gestore Checkout" icon="cart-shopping" href="#checkout-route-handler">
    Integra il checkout di Dodo Payments nella tua app Hono.
</Card>
<Card title="Portale Clienti" icon="user" href="#customer-portal-route-handler">
    Consenti ai clienti di gestire abbonamenti e dettagli.
</Card>
<Card title="Webhook" icon="bell" href="#webhook-route-handler">
    Ricevi e gestisci eventi webhook di Dodo Payments.
</Card>
</CardGroup>
...
Crea un file <code>.env</code> nella radice del tuo progetto:
...
<Tabs>
<Tab title="Gestore Checkout">
  <Info>
  Usa questo gestore per integrare il checkout di Dodo Payments nella tua app Hono. Supporta flussi statici (GET), dinamici (POST) e di sessione (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Gestore Portale Clienti">
  <Info>
  Usa questo gestore per consentire ai clienti di gestire i loro abbonamenti e dettagli tramite il portale clienti di Dodo Payments.
  </Info>
  <CodeGroup>
...
<Tab title="Gestore Webhook">
  <Info>
  Usa questo gestore per ricevere e gestire eventi webhook di Dodo Payments in modo sicuro nella tua app Hono.
  </Info>
  <CodeGroup>
...
Qualsiasi parametro di query che inizia con <code>metadata_</code> sarà passato come metadati.
</ParamField>
...
- Per un elenco completo dei campi del corpo POST supportati, fare riferimento a:
  ...
  - [Corpo della richiesta per un prodotto di abbonamento](https://docs.dodopayments.com/api-reference/subscriptions/post-subscriptions)
...
Le sessioni di checkout forniscono un'esperienza di checkout più sicura e ospitata che gestisce l'intero flusso di pagamento sia per acquisti una tantum che per abbonamenti con pieno controllo di personalizzazione.
