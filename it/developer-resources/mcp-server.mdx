...
Il [Dodo Agent Plugin](/developer-resources/build-with-ai-coding-agents) installa sia i server MCP sia tutte le diciassette competenze in un unico passaggio per Claude Code, Codex CLI, Cursor, VS Code / GitHub Copilot, Kiro e OpenCode. Gemini CLI riceve solo i due server MCP, poiché non dispone di una primitiva per le competenze degli agenti. Consulta la [guida agli agenti di codifica AI](/developer-resources/build-with-ai-coding-agents) per i comandi di installazione specifici per ogni agente.
