...
Il [Dodo Agent Plugin](/developer-resources/build-with-ai-coding-agents) installa sia i server MCP sia tutte le diciassette skill in un unico passaggio per Claude Code, Codex CLI, Cursor e OpenCode. Consulta la [guida agli agenti di codifica AI](/developer-resources/build-with-ai-coding-agents) per i comandi di installazione specifici per ogni agente.
