...
1. **Pulisci la cache di autenticazione MCP**: `rm -rf ~/.mcp-auth`
...
<Tip>
Usa `@latest` per estrarre sempre la versione più recente, oppure fissa a una versione specifica come `@1.0.0` per stabilità.
</Tip>
...
Dopo aver cliccato, imposta le tue variabili d'ambiente in Cursor's `mcp.json` tramite **Impostazioni Cursor** > **Strumenti e MCP** > **Nuovo server MCP**.
...
Dopo aver cliccato, imposta le tue variabili d'ambiente in VS Code's `mcp.json` tramite **Command Palette** > **MCP: Apri Configurazione Utente**.
...
<Step title="Ricarica VS Code">
    Ricarica la finestra di VS Code (`Cmd/Ctrl + Shift + P` > "Ricarica Finestra") per attivare la connessione del server MCP.
</Step>
</Steps>
</Tab>
...
| `DODO_PAYMENTS_ENVIRONMENT` | Imposta su `live_mode` per produzione | No |
...
| `x-dodo-payments-api-key` | Header chiave API diretta |
...
| `openai-agents` | Piattaforma di agenti OpenAI |
...
2. **`get_api_endpoint_schema`** - Ottiene schema dettagliato per un metodo specifico
3. **`invoke_api_endpoint`** - Esegue qualsiasi metodo con parametri appropriati

<Info>
Gli strumenti dinamici sono automaticamente abilitati per le API con più di 50 metodi. Sovrascrivi con `--tools=all` per includere tutti gli strumenti.
</Info>
...
Quando distribuisci in remoto, richiedi sempre l'autenticazione tramite il header `Authorization` o il header `x-dodo-payments-api-key`.
...
Verifica `DODO_PAYMENTS_ENVIRONMENT` è impostato correttamente (`live_mode` per produzione).
