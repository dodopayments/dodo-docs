...
Il boilerplate dimostra come gestire gli eventi webhook in `src/pages/api/webhook.ts`:
...
Ricorda anche di aggiornare la variabile d'ambiente `DODO_PAYMENTS_WEBHOOK_KEY` nel tuo ambiente di produzione per corrispondere alla chiave di firma webhook per il tuo dominio distribuito.
...
Sostituisci il valore hardcoded `CUSTOMER_ID` in `src/components/Header.astro` con un ID cliente reale dal tuo dashboard Dodo.
