...
- Una chiave API Google AI ([Ottieni una da aistudio](https://aistudio.google.com/api-keys))
...
<Steps>
<Step title="Apri la sezione Contatori">
...
<Frame>
  <img src="/images/cookbooks/ai-chat-app/create-meter.png" alt="Crea Contatore"/>
</Frame>

Dovresti vedere un modulo dove configureremo il nostro tracciamento dei token.
</Step>

<Step title="Compila le informazioni di base del contatore">
Ora inseriremo i dettagli specifici per il nostro servizio di chat AI:

**Nome del Misuratore** → `AI Token Usage Meter`

**Descrizione** → `Tracks token consumption from AI chat conversations using AI SDK`

**Nome dell'Evento** → `ai_chat_usage`

<Warning>
Il nome dell'evento `ai_chat_usage` deve corrispondere esattamente a quello che invieremo dal nostro codice applicativo in seguito. I nomi degli eventi fanno distinzione tra maiuscole e minuscole!
</Warning>
</Step>

<Step title="Configura come contiamo i token">
Imposta l'aggregazione (come il contatore conta i nostri eventi):

**Tipo di Aggregazione**: Seleziona **Somma** dal menu a discesa

**Aggregare su**: Tipo → `totalTokens`
...
<Step title="Crea il tuo contatore">
1. Controlla che tutte le tue impostazioni corrispondano ai valori sopra
2. Clicca su **Crea Contatore**

<Frame>
  <img src="/images/cookbooks/ai-chat-app/meter-configuration.png" alt="Configurazione del Contatore"/>
</Frame>
...
<Steps>
<Step title="Ottieni la chiave API di Dodo Payments">
1. Nella tua dashboard di Dodo Payments, vai su **Sviluppatori** → **Chiavi API**
2. Clicca su **Crea Chiave API**
3. Copia la chiave API - avrà un aspetto simile a `test_abc123...`

<Note>
Conserva questa chiave API - la aggiungeremo al nostro `.env` file più tardi.
</Note>
</Step>

<Step title="Ottieni la Chiave API di Google AI">
...
<Step title="Inserisci i dettagli del prodotto">
Compila i dettagli richiesti:

**Nome del Prodotto**: → `AI Chat Service`
...
<Step title="Collega il tuo contatore">
Prima di collegare il tuo contatore, assicurati di aver selezionato **Fatturazione Basata sull'Utilizzo** come tipo di prezzo per il tuo prodotto.

Inoltre, imposta il **Prezzo Fisso** su `0` per garantire che i clienti siano addebitati solo in base al loro utilizzo, senza alcuna tassa fissa.
...
<Step title="Imposta il tuo prezzo">
Ecco dove definiamo il nostro modello di business:

**Prezzo per Unità**: Inserisci → `0.00001` (questo è $0,01 per 1.000 token o $0,00001 per token)

**Soglia Gratuita**: Inserisci → `10000` (i clienti ricevono 10.000 token gratuiti al mese)

<Frame>
  <img src="/images/cookbooks/ai-chat-app/product-pricing.png" alt="Prezzi del Prodotto"/>
</Frame>

<Tip>
**Come funziona la fatturazione**: Se un cliente utilizza 25.000 token in un mese, verrà addebitato per 15.000 token (25.000 - 10.000 gratuiti) = 15.000 × $0.00001 = $0.15
</Tip>
</Step>

<Step title="Salva il tuo prodotto">
...
<Steps>
<Step title="Ottieni il tuo link di pagamento">
...
<Step title="Completa un acquisto di prova">
...
<Step title="Trova il tuo ID cliente">
1. Torna alla tua dashboard di Dodo Payments
2. Naviga su **Vendite** -> **Clienti** nella barra laterale sinistra
...
<Step title="Configura TypeScript">
Crea `tsconfig.json`:
...
<Step title="Crea la struttura del progetto">
...
<Step title="Imposta le variabili d'ambiente">
...
<Step title="Crea il server backend">
Crea `src/server.ts` e copia questo codice server completo:
...
Ora aggiungiamo un'interfaccia chat bellissima con la cronologia completa delle conversazioni! Crea `public/index.html`:
...
1. Verifica che il tuo file `.env` contenga tutte le chiavi API dal Passo 2
...
<Step title="Apri l'interfaccia chat">
1. Apri il tuo browser
2. Naviga a `http://localhost:3000`
3. Dovresti vedere la bellissima interfaccia chat

<Note>
Assicurati di aggiornare il `CUSTOMER_ID` nel `server.ts` con il tuo effettivo ID cliente di prova dal Passo 4.
</Note>
</Step>

<Step title="Fai la tua prima conversazione">
...
<Step title="Controlla il tuo dashboard Dodo Payments">
...
<Frame>
  <img src="/images/cookbooks/ai-chat-app/meter-event.png" alt="Eventi del Contatore"/>
</Frame>
...
<Step title="Verifica il conteggio dei token">
...
<Frame>
  <img src="/images/cookbooks/ai-chat-app/aggregation.png" alt="Token Clienti del Contatore"/>
</Frame>

<Info>
Il misuratore sta sommando automaticamente tutti i valori `totalTokens`!
</Info>
</Step>

<Step title="Testa il livello gratuito">
...
<Frame>
  <img src="/images/cookbooks/ai-chat-app/free-tier-test.png" alt="Test del Livello Gratuito"/>
</Frame>
...
<AccordionGroup>
<Accordion title="Eventi non appaiono nel dashboard">
**Possibili cause:**
...
**Soluzioni:**
1. Verifica che il nome dell'evento corrisponda esattamente alla configurazione del misuratore (case-sensitive: `ai_chat_usage`)
...
<Accordion title="Conteggi dei token che mostrano zero">
**Possibili cause:**
...
2. Aggiorna all'ultima SDK Blueprints: `npm install @dodopayments/ingestion-blueprints@latest`
</Accordion>

<Accordion title="Errore chiave API non valida">
**Possibili cause:**
...
<CardGroup cols={2}>
  ...
  <Card title="Ingestion Blueprints SDK" icon="github" href="https://github.com/dodopayments/ingestion-blueprints">
    Dai un'occhiata al repository GitHub per ulteriori esempi
  </Card>
  
  <Card title="Unisciti alla Comunità" icon="discord" href="https://discord.gg/bYqAp4ayYh">
