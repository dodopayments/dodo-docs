...
<Warning>
Il nome dell'evento `ai_chat_usage` deve corrispondere esattamente a quello che invieremo dal nostro codice applicativo in seguito. I nomi degli eventi sono case-sensitive!
</Warning>
</Step>
...
1. Nel tuo dashboard di Dodo Payments, vai su **Sviluppatori** → **Chiavi API**
2. Clicca su **Crea Chiave API**
3. Copia la chiave API - apparirà come `test_abc123...`
...
<Note>
Tieni al sicuro questa chiave - la aggiungeremo anche al nostro `.env` file.
</Note>
</Step>
</Steps>
...
Inoltre, imposta il **Prezzo Fisso** a `0` per garantire che i clienti vengano addebitati solo in base al loro utilizzo, senza alcuna tassa di base.
...
4. Copia l'ID cliente - apparirà come `cus_123`
...
Ora aggiungiamo una bellissima interfaccia di chat con la cronologia completa delle conversazioni! Crea `public/index.html`:
...
<Step title="Apri l'interfaccia di chat">
1. Apri il tuo browser
2. Naviga a `http://localhost:3000`
3. Dovresti vedere la bellissima interfaccia di chat

<Note>
Assicurati di aggiornare il `CUSTOMER_ID` in `server.ts` con il tuo effettivo ID cliente di test dal Passo 4.
</Note>
</Step>
...
<Info>
Il meter sta sommando automaticamente tutti i valori di `totalTokens`!
</Info>
</Step>
...
**Soluzioni:**
1. Verifica che il nome dell'evento corrisponda esattamente alla configurazione del meter (case-sensitive: `ai_chat_usage`)
...
2. Aggiorna all'ultima versione del Blueprints SDK: `npm install @dodopayments/ingestion-blueprints@latest`
</Accordion>
