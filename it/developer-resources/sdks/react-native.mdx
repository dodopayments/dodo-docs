...
Personalizza la barra degli strumenti, i pulsanti e lo schema di colori del browser di checkout tramite
`customization` su `start(...)`. Le opzioni sono raggruppate per piattaforma perché
Custom Tab di Android e `SFSafariViewController` di iOS espongono controlli nativi differenti. Tutti i campi sono facoltativi; se ometti `customization` viene utilizzato l'aspetto predefinito di ciascuna
piattaforma.
