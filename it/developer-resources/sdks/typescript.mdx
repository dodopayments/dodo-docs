---
title: TypeScript
description: Integra Dodo Payments nelle tue applicazioni TypeScript e Node.js
  con sicurezza dei tipi e supporto moderno per async/await
icon: js
keywords:
  - Dodo Payments
  - SDK TypeScript
  - Node.js
  - pacchetto npm
  - typescript
---

Il SDK TypeScript fornisce un accesso conveniente al server all'API REST di Dodo Payments per applicazioni TypeScript e JavaScript. Presenta definizioni di tipo complete, gestione degli errori, ritentativi, timeout e auto-paginazione per un'elaborazione dei pagamenti senza soluzione di continuità.

## Installazione

Installa l'SDK utilizzando il gestore di pacchetti che preferisci:
<CodeGroup>

```bash npm
npm install dodopayments
```

```bash yarn
yarn add dodopayments
```

```bash pnpm
pnpm add dodopayments
```

</CodeGroup>

## Inizio Veloce

Inizializza il client con la tua chiave API e inizia a elaborare i pagamenti:

```javascript
import DodoPayments from 'dodopayments';

const client = new DodoPayments({
  bearerToken: process.env['DODO_PAYMENTS_API_KEY'], // This is the default and can be omitted
  environment: 'test_mode', // defaults to 'live_mode'
});

const checkoutSessionResponse = await client.checkoutSessions.create({
  product_cart: [{ product_id: 'product_id', quantity: 1 }],
});

console.log(checkoutSessionResponse.session_id);
```

<Warning>
Conserva sempre le chiavi API in modo sicuro utilizzando variabili d'ambiente. Non committarle mai nel controllo di versione né esporle nel codice client-side.
</Warning>

## Caratteristiche Principali

<CardGroup cols={2}>
<Card title="TypeScript First" icon="shield-check">
  Supporto completo per TypeScript con definizioni di tipo esaustive per tutti gli endpoint API
</Card>

<Card title="Auto-Pagination" icon="arrows-rotate">
  La paginazione automatica per le risposte di elenco rende semplice lavorare con dataset di grandi dimensioni
</Card>

<Card title="Error Handling" icon="triangle-exclamation">
  Tipi di errore integrati con messaggi dettagliati per diversi scenari di fallimento
</Card>

<Card title="Smart Retries" icon="repeat">
  Ritentativi automatici configurabili con backoff esponenziale per errori temporanei
</Card>
</CardGroup>

## Configurazione

### Variabili di Ambiente

Imposta variabili di ambiente per una configurazione sicura:

```bash .env
DODO_PAYMENTS_API_KEY=your_api_key_here
```

### Configurazione del Timeout

Configura i timeout delle richieste globalmente o per richiesta:

```typescript
// Configure default timeout for all requests (default is 1 minute)
const client = new DodoPayments({
  timeout: 20 * 1000, // 20 seconds
});

// Override per-request
await client.checkoutSessions.create(
  { product_cart: [{ product_id: 'product_id', quantity: 1 }] },
  { timeout: 5 * 1000 },
);
```

### Configurazione dei Ritenti

Configura il comportamento dei ritentativi automatici:

```javascript
// Configure default for all requests (default is 2 retries)
const client = new DodoPayments({
  maxRetries: 0, // disable retries
});

// Override per-request
await client.checkoutSessions.create(
  { product_cart: [{ product_id: 'product_id', quantity: 1 }] },
  { maxRetries: 5 },
);
```

<Tip>
Lo SDK ritenta automaticamente le richieste che falliscono a causa di errori di rete o problemi lato server (risposte 5xx) con backoff esponenziale.
</Tip>

## Operazioni Comuni

### Crea una Sessione di Checkout

Genera una sessione di checkout per raccogliere informazioni di pagamento:

```typescript
const session = await client.checkoutSessions.create({
  product_cart: [
    {
      product_id: 'pdt_123',
      quantity: 1
    }
  ],
  return_url: 'https://yourdomain.com/return'
});

console.log('Redirect to:', session.checkout_url);
```

### Gestisci i Clienti

Crea e recupera informazioni sui clienti:

```typescript
// Create a customer
const customer = await client.customers.create({
  email: 'customer@example.com',
  name: 'John Doe',
  metadata: {
    user_id: '12345'
  }
});

// Retrieve customer
const retrieved = await client.customers.retrieve('cus_123');
console.log(`Customer: ${retrieved.name} (${retrieved.email})`);
```

### Gestisci Abbonamenti

Crea e gestisci abbonamenti ricorrenti:

<Warning>
`POST /subscriptions` (il metodo `subscriptions.create` dell'SDK) è **deprecato**. Continua a funzionare per le integrazioni esistenti, ma per le nuove integrazioni è consigliabile creare le sottoscrizioni tramite una [Checkout Session](/developer-resources/checkout-session).
</Warning>

```typescript
// Create a subscription
const subscription = await client.subscriptions.create({
  billing: {
    country: 'US',
    city: 'San Francisco',
    state: 'CA',
    street: '1 Market St',
    zipcode: '94105',
  },
  customer: {
    customer_id: 'cus_123', // or pass { email, name } to create a new customer
  },
  product_id: 'pdt_456',
  quantity: 1,
});

// Charge an on-demand subscription
// product_price is in the lowest currency denomination (e.g., 2500 = $25.00 USD)
const chargeResponse = await client.subscriptions.charge(subscription.subscription_id, {
  product_price: 2500,
});

// Retrieve subscription usage history (for metered subscriptions)
const usageHistory = await client.subscriptions.retrieveUsageHistory(subscription.subscription_id, {
  start_date: '2024-01-01T00:00:00Z',
  end_date: '2024-03-31T23:59:59Z',
});
```

<Info>
`billing` richiede almeno il codice paese ISO di due lettere. `customer` è un'unione di `{ customer_id }` (per collegare un customer esistente) o `{ email, name? }` (per crearne uno nuovo). `product_price` è espresso nella denominazione più piccola della valuta.
</Info>

## Fatturazione basata sull'utilizzo

### Acquisizione degli eventi di utilizzo

Tieni traccia degli eventi personalizzati per la fatturazione basata sull'utilizzo:

```typescript
await client.usageEvents.ingest({
  events: [
    {
      event_id: 'api_call_12345',
      customer_id: 'cus_abc123',
      event_name: 'api_request',
      timestamp: '2024-01-15T10:30:00Z',
      metadata: {
        endpoint: '/api/v1/users',
        method: 'GET',
        tokens_used: '150'
      }
    }
  ]
});
```

<Info>
Gli eventi devono avere valori `event_id` univoci per garantire l'idempotenza. Gli ID duplicati all'interno della stessa richiesta vengono rifiutati, mentre le richieste successive con ID già esistenti vengono ignorate.
</Info>

### Recuperare gli eventi di utilizzo

Recupera informazioni dettagliate sugli eventi di utilizzo:

```typescript
// Get a specific event
const event = await client.usageEvents.retrieve('api_call_12345');

// List events with filtering
const events = await client.usageEvents.list({
  customer_id: 'cus_abc123',
  event_name: 'api_request',
  start: '2024-01-14T10:30:00Z',
  end: '2024-01-15T10:30:00Z'
});
```

## Configurazione proxy

Configura le impostazioni del proxy per diversi runtime:

### Node.js (utilizzando undici)

```typescript
import DodoPayments from 'dodopayments';
import * as undici from 'undici';

const proxyAgent = new undici.ProxyAgent('http://localhost:8888');
const client = new DodoPayments({
  fetchOptions: {
    dispatcher: proxyAgent,
  },
});
```

### Bun

```typescript
import DodoPayments from 'dodopayments';

const client = new DodoPayments({
  fetchOptions: {
    proxy: 'http://localhost:8888',
  },
});
```

### Deno

```typescript
import DodoPayments from 'npm:dodopayments';

const httpClient = Deno.createHttpClient({ proxy: { url: 'http://localhost:8888' } });
const client = new DodoPayments({
  fetchOptions: {
    client: httpClient,
  },
});
```

## Logging

Controlla il livello di dettaglio dei log utilizzando le variabili d'ambiente o le opzioni del client:

```typescript
// Via client option
const client = new DodoPayments({
  logLevel: 'debug', // Show all log messages
});
```

```bash
# Via environment variable
export DODO_PAYMENTS_LOG=debug
```

**Livelli di log disponibili:**
- `'debug'` - Mostra i messaggi di debug, le informazioni, gli avvisi e gli errori
- `'info'` - Mostra le informazioni, gli avvisi e gli errori  
- `'warn'` - Mostra gli avvisi e gli errori (predefinito)
- `'error'` - Mostra solo gli errori
- `'off'` - Disabilita completamente il logging

<Warning>
Al livello di debug, tutte le richieste e le risposte HTTP vengono registrate, inclusi header e body. Alcuni header di autenticazione vengono oscurati, ma i dati sensibili nei body potrebbero essere comunque visibili.
</Warning>

## Migrazione dall'SDK Node.js

Se stai effettuando l'upgrade dal legacy SDK Node.js, l'SDK TypeScript offre una sicurezza dei tipi e funzionalità migliorate:

<Card
  title="View Migration Guide"
  icon="arrow-right"
  href="https://github.com/dodopayments/dodopayments-typescript/blob/main/MIGRATION.md">
  Scopri come eseguire la migrazione dall'SDK Node.js all'SDK TypeScript
</Card>

## Paginazione automatica

I metodi di elenco nell'API DodoPayments sono paginati. Puoi utilizzare la sintassi `for await … of` per scorrere gli elementi di tutte le pagine:

```typescript
async function fetchAllPayments() {
  const allPayments = [];
  // Automatically fetches more pages as needed.
  for await (const paymentListResponse of client.payments.list()) {
    allPayments.push(paymentListResponse);
  }
  return allPayments;
}
```

In alternativa, puoi richiedere una singola pagina alla volta:

```typescript
let page = await client.payments.list();
for (const paymentListResponse of page.items) {
  console.log(paymentListResponse);
}

// Convenience methods are provided for manually paginating:
while (page.hasNextPage()) {
  page = await page.getNextPage();
  // ...
}
```

## Requisiti

Sono supportati i seguenti runtime:

- Browser web (versioni aggiornate di Chrome, Firefox, Safari, Edge e altri)
- Node.js 20 LTS o versioni successive ([non-EOL](https://endoflife.date/nodejs))
- Deno v1.28.0 o versioni successive
- Bun 1.0 o versioni successive
- Cloudflare Workers
- Vercel Edge Runtime
- Jest 28 o versioni successive con l'ambiente `"node"`
- Nitro v2.6 o versioni successive

È supportato TypeScript >= 4.9.

## Risorse

<CardGroup cols={2}>
<Card title="GitHub Repository" icon="github" href="https://github.com/dodopayments/dodopayments-typescript">
  Visualizza il codice sorgente e contribuisci
</Card>

<Card title="API Reference" icon="book" href="/api-reference/introduction">
  Documentazione completa dell'API
</Card>

<Card title="Discord Community" icon="discord" href="https://discord.gg/bYqAp4ayYh">
  Ottieni assistenza e connettiti con gli sviluppatori
</Card>

<Card title="Report Issues" icon="bug" href="https://github.com/dodopayments/dodopayments-typescript/issues">
  Segnala bug o richiedi funzionalità
</Card>
</CardGroup>

## Supporto

Hai bisogno di aiuto con l'SDK TypeScript?

- **Discord**: Unisciti al nostro [server della community](https://discord.gg/bYqAp4ayYh) per ricevere supporto in tempo reale
- **Email**: Contattaci all'indirizzo support@dodopayments.com  
- **GitHub**: Apri una issue nel [repository](https://github.com/dodopayments/dodopayments-typescript)

## Contribuire

Accogliamo con piacere i contributi! Consulta le [linee guida per i contributi](https://github.com/dodopayments/dodopayments-typescript/blob/main/CONTRIBUTING.md) per iniziare.
