...
<Info>
Gli eventi devono avere valori univoci `event_id` per l'idempotenza. Gli ID duplicati all'interno della stessa richiesta vengono rifiutati e le richieste successive con ID esistenti vengono ignorate.
</Info>
...
I metodi dell'API DodoPayments sono paginati. Puoi utilizzare la sintassi `for await … of` per iterare attraverso gli elementi su tutte le pagine:
...
- Node.js 20 LTS o versioni più recenti ([non-EOL](https://endoflife.date/nodejs))
