...
<Warning>
`POST /subscriptions` (il metodo `subscriptions.create` dell'SDK) è **deprecato**. Continua a funzionare per le integrazioni esistenti, ma per le nuove integrazioni è consigliabile creare gli abbonamenti tramite una [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`billing` richiede almeno il codice paese ISO di due lettere. `customer` accetta `{"customer_id": ...}` per associare un cliente esistente oppure `{"email": ..., "name": ...}` per crearne uno nuovo. `product_price` è espresso nella denominazione più bassa della valuta.
</Info>
...
Monitora gli eventi personalizzati per la fatturazione basata sull'utilizzo:
...
Scorri automaticamente tutti gli elementi:
...
Per avere un maggiore controllo sulla paginazione:
...
Usa aiohttp per migliorare le prestazioni asincrone:
...
Oppure per abilitare il logging a livello di debug:
...
Hai bisogno di aiuto con l'SDK Python?
...
- **GitHub**: Apri una segnalazione nel [repository](https://github.com/dodopayments/dodopayments-python)

## Contributi

Accogliamo con piacere i contributi! Consulta le [linee guida per i contributi](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md) per iniziare.
