...
<Info>
`Billing` richiede almeno il codice `Country` ISO a due lettere. `Customer` è un `CustomerRequestUnionParam` — passa `AttachExistingCustomerParam{CustomerID: ...}` per un cliente esistente o `NewCustomerParam{Email: ..., Name: ...}` per uno nuovo. `ProductPrice` è nella denominazione più bassa della valuta.
</Info>
...
Quando l'API restituisce un codice di stato non di successo, l'SDK restituisce un errore di tipo
`*dodopayments.Error`. Esso espone `StatusCode`, il sottostante `*http.Request` e
`*http.Response`, e il JSON del corpo dell'errore. Utilizza il pattern `errors.As` per
ispezionarlo, e ramifica su `StatusCode` per gestire casi specifici:
...
Hai bisogno di aiuto con il Go SDK?
...
- **GitHub**: Apri un problema sul [repository](https://github.com/dodopayments/dodopayments-go)
