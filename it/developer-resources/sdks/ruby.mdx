...
<Warning>
Il metodo `POST /subscriptions` (il metodo `subscriptions.create` dell'SDK) è **deprecato**. Continua a funzionare per le integrazioni esistenti, ma le nuove integrazioni dovrebbero creare le subscriptions tramite una [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`billing` richiede almeno il codice ISO a due lettere `country`. `customer` accetta `{ customer_id: "..." }` per associare un customer esistente oppure `{ email: "...", name: "..." }` per crearne uno nuovo. `product_price` è espresso nella denominazione più bassa della valuta.
</Info>
...
Esegui automaticamente l'iterazione su tutte le pagine:
...
Gestisci i vari errori dell'API di Dodo Payments:
...
<Tip>
Implementa una logica di retry con exponential backoff per gli errori di rate limit, per assicurarti che la tua applicazione gestisca con affidabilità gli scenari ad alto volume.
</Tip>

## Type safety con Sorbet

Usa Sorbet per parametri di richiesta type-safe:
...
Usalo nei tuoi controller Rails:
...
Usalo nelle applicazioni Sinatra:
...
- **GitHub**: Apri una issue nel [repository](https://github.com/dodopayments/dodopayments-ruby)

## Contribuire

Accogliamo con piacere i contributi! Consulta le [linee guida per i contributi](https://github.com/dodopayments/dodopayments-ruby/blob/main/CONTRIBUTING.md) per iniziare.
