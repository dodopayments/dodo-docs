...
<Step title="Add advanced controls (optional)">
  - <ParamField query="paymentCurrency" type="string">Specifica la valuta del pagamento. Per impostazione predefinita usa la valuta del Paese di fatturazione.</ParamField>
  - <ParamField query="showCurrencySelector" type="boolean" default="true">Mostra o nascondi il selettore della valuta.</ParamField>
  - <ParamField query="paymentAmount" type="integer">Importo in centesimi (solo per la modalità Pay What You Want).</ParamField>
  - <ParamField query="metadata_*" type="string">Campi metadata personalizzati (ad esempio, <code>metadata_orderId=123</code>).</ParamField>
</Step>
<Step title="Share the link">
  Invia il payment link completato al tuo cliente. Quando lo visita, tutti i query parameters vengono raccolti e memorizzati con un ID sessione. L'URL viene quindi semplificato per includere solo il parametro della sessione (ad esempio, <code>?session=sess_1a2b3c4d</code>). Le informazioni memorizzate persistono dopo i refresh della pagina e sono accessibili durante l'intero processo di checkout.
  <Check>
...
Creati tramite una chiamata API o il nostro SDK con i dati del cliente. Ecco un esempio:

Esistono due API per creare dynamic payment links:

- One-time Payment Link API [riferimento API](/api-reference/payments/post-payments)
- Subscription Payment Link API [riferimento API](/api-reference/subscriptions/post-subscriptions)

La guida seguente riguarda la creazione di payment link una tantum.
...
<Info>Dopo aver creato il payment link, reindirizza i tuoi clienti per completare il pagamento.</Info>
...
| Tipo di evento           | Quando viene generato                                              | Operazione                                                                 |
| -------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------- |
...
| `payment.failed`     | Il tentativo di pagamento non riesce (carta rifiutata, errore, ecc.).       | Informa il cliente e chiedigli di riprovare; non completare l'ordine.                    |
...
| `payment.cancelled`  | Il pagamento viene annullato prima del completamento.                   | Rilascia qualsiasi stato mantenuto e contrassegna l'ordine come abbandonato.                       |
...
Se vendi prodotti digitali con license keys, gestisci anche `license_key.created`. Per l'elenco completo degli eventi, inclusi gli eventi relativi ad abbonamenti, entitlement, crediti, recovery e dunning, consulta la [guida agli eventi webhook](/developer-resources/webhooks/intents/webhook-events-guide).

Puoi consultare questo progetto con un'implementazione demo su [GitHub](https://github.com/dodopayments/dodo-checkout-demo) che utilizza Next.js e TypeScript.

Puoi provare l'implementazione live [qui](https://atlas.dodopayments.com/).
