...
1. **Imposta i limiti di prezzo**: Definisci un prezzo minimo (obbligatorio) e opzionalmente un prezzo massimo
2. **Passa importi dinamici**: Includi un `amount` nel carrello del prodotto quando crei sessioni di checkout
3. **Lascia scegliere ai clienti**: Se non viene fornito alcun importo, i clienti possono inserire il proprio prezzo (all'interno dei tuoi limiti)

<Info>
Quando passi un `amount` nel carrello del prodotto, quell'importo viene utilizzato per il checkout. Se ometti il campo {/* INLINE_CODE_PLACEHOLDER_ed202d24820b800650e5cfc3bb366e40 */, i clienti possono selezionare il proprio prezzo durante il checkout (soggetto alle tue impostazioni minime/massime).
</Info>
...
Clicca su **Aggiungi Prodotto** per salvare. Prendi nota del tuo ID prodotto (ad es., `pdt_123abc456def`) per utilizzarlo nelle sessioni di checkout.
</Step>
</Steps>
...
Una volta che il tuo prodotto è configurato con "Pay What You Want", puoi creare sessioni di checkout con importi dinamici. Il campo `amount` nel carrello del prodotto ti consente di impostare il prezzo programmaticamente per ogni sessione di checkout.
...
<Warning>
Il campo `amount` è applicabile solo per i prodotti "Pay What You Want". Per i prodotti normali, questo campo viene ignorato.
</Warning>
...
<Note>
**Formato Importo**: Il campo `amount` deve essere nella più bassa denominazione della valuta. Per USD, questo significa centesimi (ad es., $25.00 = `2500`). Per altre valute, usa l'unità più piccola (ad es., paise per INR).
</Note>

## Passo 3: Consenti ai Clienti di Scegliere il Loro Prezzo

Se vuoi che i clienti selezionino il proprio prezzo durante il checkout, ometti semplicemente il campo `amount` dal carrello del prodotto. La pagina di checkout mostrerà un campo di input dove i clienti possono inserire qualsiasi importo all'interno dei tuoi limiti minimi e massimi.
...
<Card title="Traccia le decisioni sui prezzi" icon="chart-line">
Usa i metadati per tracciare perché sono stati scelti importi specifici (ad es., `pricing_tier`, `discount_code`, `user_segment`).
</Card>
...
- Il prodotto è un prodotto **Pagamento Singolo** (una tantum), non un abbonamento
- L'importo è nel formato corretto (la più bassa denominazione della valuta, ad es., centesimi per USD)
</Accordion>

<Accordion title="L'importo supera il massimo o è al di sotto del minimo">
L'API rifiuterà le sessioni di checkout in cui l'importo viola i limiti di prezzo del tuo prodotto. Valida sempre gli importi prima di creare sessioni di checkout, o lascia che i clienti scelgano il proprio prezzo omettendo il campo `amount`.
</Accordion>
