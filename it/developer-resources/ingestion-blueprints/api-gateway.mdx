...
- **Tipo di Aggregazione**: `count` per tracciare il numero di chiamate
...
Numero massimo di eventi prima dell'auto-svuotamento. Predefinito: `100`.
</ParamField>

<ParamField path="flushInterval" type="number">
Intervallo di auto-svuotamento in millisecondi. Predefinito: `5000` (5 secondi).
</ParamField>

## Migliori Pratiche

<Tip>
**Usa il Batching per Alto Volume**: Per applicazioni che gestiscono più di 10 richieste al secondo, usa `createBatch()` per ridurre il sovraccarico e migliorare le prestazioni.
</Tip>

<Warning>
**Pulisci Sempre i Batch**: Chiama `batch.cleanup()` allo spegnimento dell'applicazione per svuotare gli eventi in sospeso e prevenire la perdita di dati.
</Warning>
