...
2. **Naviga a** Prodotti → Meter
3. **Clicca** su "Crea Meter"
4. **Configura il tuo meter**:
   - **Nome del Meter**: Scegli un nome descrittivo (es. "Utilizzo token LLM")
   - **Nome Evento**: Imposta un identificatore evento unico (es. `llm.chat_completion`)
   - **Tipo Aggregazione**: Seleziona `sum` per sommare il conteggio dei token
   - **Sopra Proprietà**: Scegli cosa tracciare:
     - `inputTokens` - Tracciamento token di input/prompt
     - `outputTokens` - Tracciamento token di output/completamento (inclusi i token di ragionamento quando applicabile)
     - `totalTokens` - Tracciamento del totale combinato dei token di input + output
...
<Note>
Quando utilizzi modelli con capacità di ragionamento tramite AI SDK (come Google’s Gemini 2.5 Flash con modalità di pensiero), i token di ragionamento sono automaticamente inclusi nel conteggio di `outputTokens` per una fatturazione accurata.
</Note>
</Accordion>
...
- `outputTokens` - Numero di token di output/completamento utilizzati (include token di ragionamento quando applicabile)
...
- Qualsiasi metadato personalizzato fornito durante il wrapping del client

<Note>
**Token di Ragionamento:** Per modelli con capacità di ragionamento, `outputTokens` include automaticamente sia i token di completamento che i token di ragionamento.
</Note>
</ParamField>

<Info>
Il tuo meter Dodo Payments utilizza i campi `metadata` (soprattutto `inputTokens`, `outputTokens` o `totalTokens`) per calcolare l'uso e la fatturazione.
</Info>
