...
- **Impostazione Veloce** - Inizia in meno di 5 minuti
- **Integrazione Pagamenti** - Flusso di checkout preconfigurato utilizzando `@dodopayments/nextjs`
- **UI Moderna** - Pagina dei prezzi pulita a tema scuro con Tailwind CSS
- **Gestore Webhook** - Endpoint webhook pronto per l'uso per gli eventi di pagamento
- **Portale Clienti** - Gestione degli abbonamenti con un clic
- **TypeScript** - Completamente tipizzato con tipi minimi e focalizzati
- **Checkout Precompilato** - Dimostra come passare i dati cliente per migliorare l'UX
...
<Step title="Configura Variabili d'Ambiente">
Crea un file `.env` nella directory radice:
...
<Warning>
Non compromettere mai il tuo file `.env` nel controllo della versione. È già incluso in `.gitignore`.
</Warning>
</Step>

<Step title="Aggiungi i Tuoi Prodotti">
Aggiorna `src/lib/products.ts` con i tuoi veri ID prodotto da Dodo Payments:
...
- ID Prodotto (dal tuo cruscotto Dodo)
- Prezzi
- Caratteristiche
- Descrizioni
...
In `src/app/components/ProductCard.tsx`, sostituisci i valori codificati in modo fisso con i tuoi veri dati utente:
...
In `src/app/components/Header.tsx`, sostituisci l'ID cliente codificato in modo fisso:
...
Il boilerplate dimostra come gestire due eventi webhook in `src/app/api/webhook/route.ts`:
...
<Accordion title="Reindirizzamento checkout non riuscito">
**Cause comuni:**
- ID prodotto non valido - verifica che esista nel tuo cruscotto Dodo
- Chiave API o impostazione dell'ambiente errate in `.env`
...
Sostituisci l'`CUSTOMER_ID` codificato in modo fisso in `src/app/components/Header.tsx` con un vero ID cliente dal tuo cruscotto Dodo.
