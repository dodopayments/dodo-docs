...
<Info>
Questo boilerplate utilizza Go 1.21+ con schemi di architettura pulita (`cmd`, `internal`, `templates`), modelli HTML e il SDK `dodopayments-go` per un'integrazione API senza soluzione di continuità.
</Info>

### Caratteristiche

- **Configurazione Veloce** - Inizia in meno di 5 minuti
- **Integrazione Pagamenti** - Flusso di checkout pre-configurato utilizzando il SDK `dodopayments-go`
- **UI Moderna** - Pagina di pricing pulita e scura con modelli HTML
- **Gestione Webhook** - Verifica e gestisce in modo sicuro gli eventi di pagamento
- **Portale Cliente** - Gestione autonoma degli abbonamenti
- **Migliori Pratiche Go** - Architettura pulita con `cmd`, `internal`, e `templates`
- **Checkout Pre-compilato** - Dimostra il passaggio dei dati del cliente per migliorare l'esperienza utente
...
<Step title="Configura Variabili di Ambiente">
...
<Warning>
Non impegnare mai il tuo file `.env` nel controllo versione. È già incluso in `.gitignore`.
</Warning>
</Step>
...
|----------|--------|-------------|
| `/` | GET | Pagina di pricing con elenco prodotti |
| `/api/checkout` | POST | Crea una nuova sessione di checkout |
| `/api/webhook` | POST | Gestisci webhook Dodo Payments |
| `/api/customer-portal` | POST | Genera URL portale clienti |
...
- Aggiorna l'URL del webhook nel tuo dominio di produzione
...
<Accordion title="Creazione sessione di checkout fallita">
**Cause comuni:**
- ID prodotto non valido - verifica che esista nel tuo dashboard Dodo
- Chiave API errata o impostazione dell'ambiente in `.env`
- Controlla i registri del server per messaggi di errore dettagliati
</Accordion>
...
Aggiorna l'URL del webhook nel tuo [dashboard Dodo](https://app.dodopayments.com/developer/webhooks) all'URL ngrok. Assicurati di aggiornare il tuo file `.env` con la chiave di verifica webhook corretta.
</Accordion>
