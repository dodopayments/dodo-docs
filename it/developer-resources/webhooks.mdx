...
<Step title="Crea Endpoint Webhook">
...
<Step title="Ottieni la Chiave Segreta">
    Ottieni il tuo webhook `Secret Key` dalla pagina delle impostazioni. Userai questo per verificare l'autenticità dei webhook ricevuti.
    
    <Warning>
    Tieni la tua chiave segreta del webhook al sicuro e non esporla mai nel codice client-side o nei repository pubblici.
    </Warning>
</Step>
...
<Tip>
Elabora i webhook in modo asincrono riconoscendo immediatamente la ricezione con un codice di stato `200`, quindi gestisci l'elaborazione effettiva in background.
</Tip>
...
Ogni evento webhook include un'intestazione unica `webhook-id`. Usa questo identificatore per implementare l'idempotenza e prevenire l'elaborazione duplicata.
...
1. Costruisci il messaggio firmato concatenando `webhook-id`, `webhook-timestamp`, e la stringa esatta del payload raw stringificato {/* INLINE_CODE_PLACEHOLDER_63d7f7ecaa944ca147c58ddfdab12252 */, separati da punti (`.`).
...
<Accordion title="Rispondi immediatamente" icon="bolt">
  Restituisci un codice di stato `200` immediatamente dopo aver ricevuto il webhook. Elabora l'evento in modo asincrono per evitare timeout.
...
<Accordion title="Gestisci eventi duplicati" icon="rotate">
  Implementa l'idempotenza utilizzando l'intestazione `webhook-id` per elaborare in sicurezza lo stesso evento più volte senza effetti collaterali.
</Accordion>
...
<Expandable title="Proprietà dell'oggetto dati">
  ...
  I campi aggiuntivi variano in base al tipo di evento. Consulta la documentazione specifica per evento per schemi completi.
</Expandable>
</ResponseField>
...
<Steps>
<Step title="Seleziona Tipo di Evento">
