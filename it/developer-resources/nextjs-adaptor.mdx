...
Imposta l'importo addebitato in unità principali della valuta (ad es., <code>12.5</code> per 12,50 $). Solo per i prodotti Pay What You Want; il valore viene ignorato se inferiore al prezzo minimo del prodotto.
</ParamField>
<ParamField query="showDiscounts" type="boolean">
Mostra i campi per gli sconti.
</ParamField>
<ParamField query="metadata_*" type="string">
Qualsiasi parametro di query che inizi con <code>metadata_</code> verrà passato come metadata.
</ParamField>
...
<Warning>
Dynamic Checkout fa da proxy per gli endpoint deprecati `POST /payments` e `POST /subscriptions`. Continua a funzionare per le integrazioni esistenti, ma per le nuove integrazioni è consigliato usare **Checkout Sessions** qui sotto.
</Warning>
...
Checkout sessions offrono un'esperienza di checkout ospitata più sicura, che gestisce l'intero flusso di pagamento sia per gli acquisti una tantum sia per gli abbonamenti, con pieno controllo sulla personalizzazione.
...
### Formato della risposta
Checkout sessions restituiscono una risposta JSON con l'URL del checkout:
...
## Route Handler di Customer Portal

Il Route Handler di Customer Portal consente di integrare facilmente il portale clienti di Dodo Payments nella tua applicazione Next.js.
...
Se impostato su <code>true</code>, invia al cliente un'e-mail con il link al portale.
</ParamField>
