...
Qualsiasi parametro di query che inizia con <code>metadata_</code> verrà passato come metadati.
</ParamField>
...
<Warning>
Dynamic Checkout fa da proxy per gli endpoint `POST /payments` e `POST /subscriptions` deprecati. Continua a funzionare per le integrazioni esistenti, ma per le nuove integrazioni è consigliato usare **Checkout Sessions** qui sotto.
</Warning>

### Formato della risposta
Dynamic checkout restituisce una risposta JSON con l'URL di checkout:
...
Checkout Sessions offre un'esperienza di checkout ospitata e più sicura, che gestisce l'intero flusso di pagamento sia per gli acquisti una tantum sia per gli abbonamenti, con pieno controllo sulla personalizzazione.

Consulta la [guida all'integrazione di Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session) per ulteriori dettagli e per l'elenco completo dei campi supportati.

### Formato della risposta
Checkout Sessions restituisce una risposta JSON con l'URL di checkout:
...
## Gestore della route del Customer Portal

Il gestore della route del Customer Portal consente di integrare perfettamente il portale clienti di Dodo Payments nella tua applicazione Remix.
...
## Gestore della route dei webhook
