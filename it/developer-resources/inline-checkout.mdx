...
| `currency` | Il codice valuta ISO (ad es., `"USD"`) per il totale standard, sconti e valori delle tasse. |
| `finalTotal` | L'importo effettivo addebitato al cliente. Questo può includere aggiustamenti di cambio valuta o commissioni di metodo di pagamento locale che non fanno parte della suddivisione del prezzo di base. |
| `finalTotalCurrency` | La valuta in cui il cliente sta effettivamente pagando. Questo può differire da `currency` se sono attivi parità di potere d'acquisto o conversione valuta locale. |
