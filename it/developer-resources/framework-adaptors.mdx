---
sidebarTitle: Panoramica
title: Panoramica degli Adattatori del Framework
description: Adattatori predefiniti per i framework web più popolari per
  integrare Dodo Payments in pochi minuti con codice minimo
icon: box
---

Dodo Payments fornisce adattatori ufficiali per framework che semplificano l'integrazione dei pagamenti. Ogni adattatore è progettato per funzionare senza problemi con le convenzioni del tuo framework, offrendo checkout, portale clienti e gestione dei webhook pronti all'uso.

<Tip>
Gli adattatori del framework ti consentono di integrare Dodo Payments in meno di 10 righe di codice. Gestiscono automaticamente l'autenticazione, l'analisi delle richieste e la formattazione delle risposte.
</Tip>

## Adattatori del Framework Disponibili

Scegli l'adattatore che corrisponde al tuo framework:

<CardGroup cols={2}>
<Card title="Next.js" icon="react" href="/developer-resources/nextjs-adaptor">
  Supporto per App Router con gestori di route per checkout, portale e webhook
</Card>

<Card title="Nuxt" icon="vuejs" href="/developer-resources/nuxt-adaptor">
  Framework full-stack basato su Vue con integrazione delle route del server
</Card>

<Card title="Express" icon="node-js" href="/developer-resources/express-adaptor">
  Gestori basati su middleware per il popolare framework Node.js
</Card>

<Card title="Fastify" icon="bolt" href="/developer-resources/fastify-adaptor">
  Framework Node.js ad alte prestazioni con architettura a plugin
</Card>

<Card title="Hono" icon="cloud" href="/developer-resources/hono-adaptor">
  Framework web ultrarapido per edge, Cloudflare Workers e altro
</Card>

<Card title="Astro" icon="star" href="/developer-resources/astro-adaptor">
  Framework incentrato sui contenuti con supporto per endpoint del server
</Card>

<Card title="SvelteKit" icon="code" href="/developer-resources/sveltekit-adaptor">
  Framework Svelte full-stack con integrazione degli hook del server
</Card>

<Card title="Remix" icon="react" href="/developer-resources/remix-adaptor">
  Framework React full-stack con gestori di caricamento e azione
</Card>

<Card title="TanStack Start" icon="chart-line" href="/developer-resources/tanstack-adaptor">
  Framework React full-stack a tipo sicuro con funzioni del server
</Card>

<Card title="Better Auth" icon="shield" href="/developer-resources/better-auth-adaptor">
  Plugin per framework di autenticazione per un'autenticazione + pagamenti senza soluzione di continuità
</Card>

<Card title="Convex" icon="database" href="/developer-resources/convex-component">
  Componente Backend-as-a-Service per la sincronizzazione dei pagamenti in tempo reale
</Card>

<Card title="Bun" icon="bolt" href="/developer-resources/bun-adaptor">
  Gestori Bun.serve() nativi per checkout, portale e webhook
</Card>
</CardGroup>

## Caratteristiche Principali

Tutti gli adattatori del framework offrono queste funzionalità integrate:

| Caratteristica | Descrizione |
|----------------|-------------|
| **Gestore Checkout** | Supporto per flussi di checkout statici, dinamici e basati su sessioni |
| **Portale Clienti** | Gestore predefinito per la gestione di abbonamenti e fatturazione |
| **Gestore Webhook** | Verifica della firma sicura con gestori di eventi tipizzati |
| **Config Ambientale** | Configurazione semplice tramite variabili d'ambiente |
| **Sicurezza dei Tipi** | Full supporto TypeScript con payload tipizzati |

## Guida Rapida

Inizia con qualsiasi adattatore del framework in tre passaggi:

<Steps>
<Step title="Installa l'Adattatore">
  Usa il tuo gestore di pacchetti per installare l'adattatore specifico per il framework:

  <Tabs>
  <Tab title="Next.js">

  ```bash
  npm install @dodopayments/nextjs
  ```

  </Tab>
  
  <Tab title="Nuxt">

  ```bash
  npm install @dodopayments/nuxt
  ```

  </Tab>
  
  <Tab title="Express">

  ```bash
  npm install @dodopayments/express
  ```

  </Tab>
  
  <Tab title="Hono">

  ```bash
  npm install @dodopayments/hono
  ```

  </Tab>

  <Tab title="Astro">

  ```bash
  npm install @dodopayments/astro
  ```

  </Tab>

  <Tab title="SvelteKit">

  ```bash
  npm install @dodopayments/sveltekit
  ```

  </Tab>
  </Tabs>
</Step>

<Step title="Configura le Variabili d'Ambiente">
  Aggiungi le tue credenziali Dodo Payments al tuo ambiente:

  ```env
  DODO_PAYMENTS_API_KEY=your-api-key
  DODO_PAYMENTS_WEBHOOK_KEY=your-webhook-secret
  DODO_PAYMENTS_RETURN_URL=https://yourdomain.com/checkout/success
  DODO_PAYMENTS_ENVIRONMENT="test_mode" # or "live_mode"
  ```

  <Warning>
  Non commettere mai il tuo `.env` file o segreti nel controllo di versione.
  </Warning>
</Step>

<Step title="Crea Gestori di Percorso">
  Configura le tue rotte di checkout, portale clienti e webhook:

  <Tabs>
  <Tab title="Next.js">

  ```typescript
  // app/checkout/route.ts
  import { Checkout } from "@dodopayments/nextjs";

  export const GET = Checkout({
    bearerToken: process.env.DODO_PAYMENTS_API_KEY,
    returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
    environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
  });
  ```

  </Tab>
  
  <Tab title="Express">

  ```typescript
  import { checkoutHandler } from '@dodopayments/express';

  app.get('/api/checkout', checkoutHandler({
    bearerToken: process.env.DODO_PAYMENTS_API_KEY,
    returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
    environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
  }));
  ```

  </Tab>
  
  <Tab title="Hono">

  ```typescript
  import { Checkout } from "@dodopayments/hono";

  app.get('/checkout', Checkout({
    bearerToken: process.env.DODO_PAYMENTS_API_KEY,
    returnUrl: process.env.DODO_PAYMENTS_RETURN_URL,
    environment: process.env.DODO_PAYMENTS_ENVIRONMENT,
  }));
  ```

  </Tab>
  </Tabs>

  <Check>
  Ora sei pronto per elaborare i pagamenti! Visita le singole pagine degli adattatori per guide dettagliate e tutte le opzioni disponibili.
  </Check>
</Step>
</Steps>

## Tipi di Flusso Checkout

Tutti gli adattatori supportano tre tipi di flusso di checkout:

<AccordionGroup>
<Accordion title="Checkout Statico (GET)">
Usa il checkout statico per link di pagamento semplici e condivisibili. Passa l'ID prodotto come parametro di query:

```
/api/checkout?productId=pdt_xxx&quantity=1
```

Supporta il precompilazione opzionale del cliente e la personalizzazione tramite parametri di query.
</Accordion>

<Accordion title="Checkout Dinamico (POST)">
Usa il checkout dinamico per creare pagamenti programmaticamente con dettagli personalizzati:

```json
{
  "product_id": "pdt_xxx",
  "customer": {
    "email": "customer@example.com",
    "name": "John Doe"
  },
  "quantity": 1
}
```

Supporta sia pagamenti una tantum che abbonamenti.
</Accordion>

<Accordion title="Sessioni di Checkout (POST)">
Usa le sessioni di checkout per l'esperienza di checkout più flessibile con supporto per il carrello:

```json
{
  "product_cart": [
    { "product_id": "pdt_xxx", "quantity": 1 },
    { "product_id": "pdt_yyy", "quantity": 2 }
  ],
  "customer": {
    "email": "customer@example.com"
  }
}
```

Scopri di più nella [Guida alle Sessioni di Checkout](/developer-resources/checkout-session).
</Accordion>
</AccordionGroup>

## Gestione degli Eventi Webhook

Tutti gli adattatori forniscono gestione dei webhook con tipi sicuri e callback di eventi granulari:

```typescript
Webhooks({
  webhookKey: process.env.DODO_PAYMENTS_WEBHOOK_KEY,
  onPayload: async (payload) => {
    // Handle any webhook event
  },
  onPaymentSucceeded: async (payload) => {
    // Handle successful payments
  },
  onSubscriptionActive: async (payload) => {
    // Handle new subscriptions
  },
  // ... 20+ event types supported
});
```

<Info>
Tutti i gestori di webhook verificano automaticamente le firme e convalidano i payload utilizzando gli schemi Zod. Le richieste non valide vengono respinte con codici di errore appropriati.
</Info>

## Scegliere l'Adattatore Giusto

| Framework | Migliore per | Runtime |
|-----------|--------------|---------|
| **Next.js** | App React full-stack con App Router | Node.js, Edge |
| **Nuxt** | Applicazioni Vue.js full-stack | Node.js |
| **Express** | API REST e app Node.js tradizionali | Node.js |
| **Fastify** | API ad alte prestazioni | Node.js |
| **Hono** | Distribuzioni Edge, Cloudflare Workers | Edge, Node.js |
| **Astro** | Siti di contenuto con endpoint server | Node.js, Edge |
| **SvelteKit** | Applicazioni Svelte full-stack | Node.js |
| **Remix** | React full-stack con routing nidificato | Node.js |
| **TanStack Start** | React full-stack sicuro per i tipi | Node.js |
| **Better Auth** | App già in uso con Better Auth | Varie |
| **Convex** | App che utilizzano Convex per il backend | Runtime Convex |
| **Bun** | Applicazioni server Bun native | Bun |

## Ottieni Aiuto

Hai bisogno di assistenza con gli adattatori del framework?

- **Discord**: Unisciti al nostro [server della community](https://discord.gg/bYqAp4ayYh) per aiuto in tempo reale
- **Email**: Contattaci a support@dodopayments.com
- **GitHub**: Apri un problema nel rispettivo repository dell'adattatore
- **Documentazione**: Visita la nostra [riferimento API](/api-reference/introduction)
