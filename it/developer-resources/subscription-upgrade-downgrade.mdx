...
- `subscription.active`: Modifica del piano completata con successo, abbonamento aggiornato
- `subscription.plan_changed`: Piano dell'abbonamento modificato (upgrade/downgrade/aggiornamento dell'add-on)
- `subscription.on_hold`: Addebito per la modifica del piano non riuscito, rinnovi interrotti
- `payment.succeeded`: Addebito immediato per la modifica del piano completato con successo
...
A successful plan change returns `200 OK` with an **empty response body**.

<Note>
Because there is no response payload, always rely on webhook events (e.g. <code>subscription.plan_changed</code>, <code>payment.succeeded</code>, <code>payment.failed</code>) to confirm the outcome, or re-read the subscription with <code>GET /subscriptions/&#123;subscription_id&#125;</code>.
</Note>
...
- `subscription.plan_changed`: piano dell'abbonamento modificato (upgrade/downgrade/modifiche agli add-on)
- `subscription.on_hold`: addebito non riuscito, rinnovi interrotti
- `subscription.renewed`: rinnovo completato con successo
- `payment.succeeded`: pagamento per la modifica del piano o il rinnovo completato con successo
...
A pending plan change already exists for this subscription (`PendingPlanChangeExists`). Cancel the scheduled change with `DELETE /subscriptions/{subscription_id}/change-plan/scheduled` before submitting a new one.
</Accordion>

<Accordion title="422 Unprocessable Entity">
The subscription is inactive or on-demand; plan change is not supported.
</Accordion>
