...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (obbligatorio): Istanza del client di DodoPayments
    - <b>createCustomerOnSignUp</b> (opzionale): Creazione automatica dei clienti al momento dell'iscrizione
    - <b>use</b> (obbligatorio): Array di plugin da abilitare (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (opzionale): Funzione che riceve il BetterAuth `User` e restituisce campi aggiuntivi da allegare al cliente di DodoPayments durante la creazione e l'aggiornamento (es. `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    ...
    <b>successUrl</b>: URL di reindirizzamento dopo un pagamento riuscito -{" "}
    <b>authenticatedUsersOnly</b>: Richiede l'autenticazione dell'utente (default: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>Invalid API key</b>: Verifica nuovamente il tuo{" "}
    <code>DODO_PAYMENTS_API_KEY</code> nel file <code>.env</code>. -{" "}
    <b>Webhook signature mismatch</b>: Assicurati che il segreto del webhook corrisponda a quello impostato nella Dashboard di Dodo Payments. - <b>Customer non creato</b>: Conferma che{" "}
    <code>createCustomerOnSignUp</code> sia impostato su <code>true</code>.
  </Accordion>
  <Accordion title="Best Practices">
    - Usa le variabili di ambiente per tutti i segreti e le chiavi. - Testa in{" "}
    <code>test_mode</code> prima di passare a <code>live_mode</code>. - Registra
    gli eventi dei webhook per debug e audit.
  </Accordion>
</AccordionGroup>
