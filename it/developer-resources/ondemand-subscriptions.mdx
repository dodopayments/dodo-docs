...
Le sottoscrizioni on-demand ti consentono di autorizzare il metodo di pagamento di un cliente una sola volta e poi addebitare importi variabili ogni volta che ne hai bisogno, invece di seguire un programma fisso.

<Info>
Questa funzionalità potrebbe dover essere abilitata sul tuo account. Contatta il supporto se non la vedi nel tuo dashboard.
</Info>
...
Per una configurazione generale dell'abbonamento, vedere la [Guida all'integrazione degli abbonamenti](/developer-resources/subscription-integration-guide).
...
<AccordionGroup>
  <Accordion title="Parametri del corpo della richiesta di addebito">
    ...
    <ParamField body="adaptive_currency_fees_inclusive" type="boolean">
      Se vero, include le commissioni di valuta adattiva all'interno di <code>product_price</code>. Se falso, le commissioni vengono aggiunte sopra.
    </ParamField>
...
<Note>
Fuso orario e ora legale: utilizza uno standard temporale coerente per la programmazione e converti solo per la visualizzazione per mantenere gli intervalli.
</Note>
...
- Utilizza un pianificatore/coda che persiste timestamp precisi; calcola il prossimo tentativo all'esatto offset dell'ora del giorno (ad es., `T + 3 days` alla stessa HH:MM).
...
- Valuta sempre l'ultima ragione di rifiuto; interrompi le ripetizioni per i rifiuti definitivi nell'elenco di esclusione sopra.
...
## Traccia i risultati con i webhook
...
<Tip>
Per flussi on-demand, concentrati su `payment.succeeded` e `payment.failed` per riconciliare gli addebiti basati sull'uso.
</Tip>
...
Utilizza la tua chiave API di test per creare la sottoscrizione con `payment_link: true`, quindi apri il link e completa il mandato.
</Step>
...
- **Nessun webhook ricevuto**: Verifica la tua URL del webhook e la configurazione del segreto della firma.
