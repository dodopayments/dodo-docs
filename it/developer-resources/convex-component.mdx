...
Imposta le variabili d'ambiente nel tuo dashboard di Convex (**Impostazioni** → **Variabili d'ambiente**). Puoi accedere al dashboard eseguendo:
...
<Tab title="Impostazione del Portale Clienti">
  <Info>
  Usa questa funzione per consentire ai clienti di gestire i loro abbonamenti e dettagli tramite il portale clienti di Dodo Payments. Il cliente viene automaticamente identificato tramite la funzione `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="Impostazione del Gestore Webhook">
  <Info>
  Usa questo gestore per ricevere e elaborare gli eventi webhook di Dodo Payments in modo sicuro nella tua app Convex. Tutti i gestori webhook ricevono il `ActionCtx` di Convex come primo parametro, permettendoti di utilizzare `ctx.runQuery()` e `ctx.runMutation()` per interagire con il tuo database.
  </Info>
  <CodeGroup>
...
<Info>
Il cliente viene automaticamente identificato utilizzando la funzione `identify` configurata nel tuo setup di DodoPayments. Questa funzione dovrebbe restituire il `dodoCustomerId` del cliente.
</Info>
