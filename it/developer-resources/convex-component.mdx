...
<CardGroup cols={2}>
<Card title="Funzione di Checkout" icon="cart-shopping" href="#checkout-function">
    Integra il checkout di Dodo Payments con un flusso basato su sessione.
</Card>
<Card title="Portale Clienti" icon="user" href="#customer-portal-function">
    Consenti ai clienti di gestire abbonamenti e dettagli.
</Card>
<Card title="Webhook" icon="bell" href="#webhook-handler">
    Ricevi e gestisci gli eventi webhook di Dodo Payments.
</Card>
</CardGroup>
...
<Step title="Aggiungi il Componente alla Configurazione di Convex">
...
Imposta le variabili d'ambiente nel tuo dashboard Convex (**Impostazioni** → **Variabili d'ambiente**). Puoi accedere al dashboard eseguendo:
...
- `DODO_PAYMENTS_ENVIRONMENT` - Imposta su `test_mode` o `live_mode`
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Il tuo segreto webhook (richiesto per la gestione del webhook)

<Warning>
Usa sempre le variabili d'ambiente di Convex per informazioni sensibili. Non commettere mai segreti nel controllo di versione.
</Warning>
</Step>
</Steps>
...
<Steps>
<Step title="Crea una Query Interna">
Per prima cosa, crea una query interna per recuperare i clienti dal tuo database. Questa sarà utilizzata nelle funzioni di pagamento per identificare i clienti.

<Warning>
Prima di utilizzare questa query, assicurati di definire lo schema appropriato nel tuo file `convex/schema.ts` o modifica la query per adattarla al tuo schema esistente.
</Warning>
...
<Step title="Configura il Componente DodoPayments">
 <CodeGroup>
...
<Tabs>
<Tab title="Configurazione della Funzione di Checkout">
  <Info>
  Usa questa funzione per integrare il checkout di Dodo Payments nella tua app Convex. Utilizza un checkout basato su sessione con supporto completo per le funzionalità.
  </Info>
  <CodeGroup>
...
<Tab title="Impostazione del portale clienti">
  <Info>
  Usa questa funzione per consentire ai clienti di gestire le loro sottoscrizioni e dettagli tramite il portale clienti di Dodo Payments. Il cliente viene identificato automaticamente tramite la funzione `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="Impostazione del gestore webhook">
  <Info>
  Usa questo gestore per ricevere e elaborare in modo sicuro gli eventi webhook di Dodo Payments nella tua app Convex. Tutti i gestori webhook ricevono il parametro Convex `ActionCtx` come primo parametro, consentendoti di utilizzare `ctx.runQuery()` e `ctx.runMutation()` per interagire con il tuo database.
  </Info>
  <CodeGroup>
  ...
  <Warning>
  Assicurati di definire le corrispondenti mutazioni del database nel tuo backend Convex per ogni evento webhook che desideri gestire. Ad esempio, crea una mutazione <code>createPayment</code> per registrare i pagamenti riusciti o una mutazione <code>createSubscription</code> per gestire lo stato degli abbonamenti.
  </Warning>
</Tab>
</Tabs>
...
<Info>
Il componente Convex di Dodo Payments utilizza un checkout basato su sessione, fornendo un'esperienza di checkout sicura e personalizzabile con carrelli prodotti preconfigurati e dettagli dei clienti. Questo è l'approccio raccomandato per tutti i flussi di pagamento.
</Info>
...
<Info>
Il cliente viene identificato automaticamente utilizzando la funzione `identify` configurata nel tuo setup di DodoPayments. Questa funzione dovrebbe restituire il `dodoCustomerId` del cliente.
</Info>
...
<Info>
Usa la funzione di checkout dai tuoi componenti React con i hook di Convex.
</Info>
