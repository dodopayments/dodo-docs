---
title: Adaptor Nuxt
description: Integra Dodo Payments nel tuo progetto Nuxt utilizzando il modulo
  Nuxt ufficiale. Include checkout, customer portal, webhook e configurazione
...
Disabilita il campo del codice postale.
</ParamField>
...
Imposta l'importo addebitato, espresso nelle unità principali della valuta (ad es., <code>12.5</code> per 12,50 $). Solo per i prodotti Pay What You Want; viene ignorato se inferiore al prezzo minimo del prodotto.
</ParamField>
...
<Warning>
Dynamic Checkout funge da proxy per gli endpoint `POST /payments` e `POST /subscriptions` deprecati. Continua a funzionare per le integrazioni esistenti, ma per le nuove integrazioni è consigliabile utilizzare **Checkout Sessions** qui sotto.
</Warning>

### Formato della risposta
Dynamic checkout restituisce una risposta JSON con l'URL del checkout:
...
Checkout Sessions offre un'esperienza di checkout ospitata più sicura, che gestisce l'intero flusso di pagamento sia per gli acquisti una tantum sia per gli abbonamenti, con pieno controllo sulla personalizzazione.

Per ulteriori dettagli e per l'elenco completo dei campi supportati, consulta la [Checkout Sessions Integration Guide](https://docs.dodopayments.com/developer-resources/checkout-session).

### Formato della risposta
Checkout Sessions restituisce una risposta JSON con l'URL del checkout:
...
Il gestore della route Customer Portal consente di integrare perfettamente il portale clienti di Dodo Payments nella tua applicazione Nuxt.
...
- **Verifica della firma:** verifica la firma del webhook utilizzando <code>webhookKey</code>. Restituisce 401 se la verifica non va a buon fine.
- **Convalida del payload:** viene eseguita con Zod. Restituisce 400 per i payload non validi.
- **Gestione degli errori:**
  - 401: firma non valida
  - 400: payload non valido
  - 500: errore interno durante la verifica
- **Instradamento degli eventi:** chiama il gestore degli eventi appropriato in base al tipo di payload.
