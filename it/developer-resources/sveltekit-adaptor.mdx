...
<Info>
Tutti gli esempi assumono che tu stia utilizzando il SvelteKit App Router.
</Info>

<Tabs>
<Tab title="Gestore Checkout">
  <Info>
  Usa questo gestore per integrare il checkout di Dodo Payments nella tua app SvelteKit. Supporta flussi di pagamento statici (GET), dinamici (POST) e di sessione (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Gestore Webhook">
  <Info>
  Usa questo gestore per ricevere e gestire in modo sicuro gli eventi webhook di Dodo Payments nella tua app SvelteKit.
  </Info>
  <CodeGroup>
...
- Per un elenco completo dei campi del corpo POST supportati, fai riferimento a:
