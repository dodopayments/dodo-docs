...
description: Scopri come integrare Dodo Payments con il tuo progetto SvelteKit
  App Router utilizzando il nostro SvelteKit Adaptor. Copre checkout, portale
  clienti, webhook e configurazione dell'ambiente sicuro.
icon: code-merge
keywords:
  - Dodo Payments
  - SvelteKit
  - integrazione SvelteKit
  - Svelte
  - integrazione dei pagamenti
  - adattatore sveltekit
---

<CardGroup cols={2}>
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Integra il checkout di Dodo Payments nella tua app SvelteKit.
</Card>
...
Crea un <code>.env</code> file nella radice del tuo progetto:
...
<Warning>
Non commettere mai il tuo <code>.env</code> file o i segreti nel controllo versione.
</Warning>
</Step>
</Steps>
...
<Info>
Tutti gli esempi presumono che tu stia usando il router dell'app SvelteKit.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Usa questo handler per integrare il checkout di Dodo Payments nella tua app SvelteKit. Supporta flussi di pagamento statici (GET), dinamici (POST) e di sessione (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Usa questo handler per ricevere ed elaborare in modo sicuro gli eventi webhook di Dodo Payments nella tua app SvelteKit.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments supporta tre tipi di flussi di pagamento per integrare pagamenti nel tuo sito web, questo adattatore supporta tutti i tipi di flussi di pagamento.
</Info>
...
Imposta l'importo addebitato, espresso nelle unità principali della valuta (ad es., <code>12.5</code> per 12,50 $). Solo per i prodotti Pay What You Want; il valore viene ignorato se inferiore al prezzo minimo del prodotto.
</ParamField>
...
<Warning>
Dynamic Checkout utilizza i deprecated `POST /payments` e `POST /subscriptions` endpoints. Continua a funzionare per le integrazioni esistenti, ma per le nuove integrazioni è consigliabile utilizzare **Checkout Sessions** qui sotto.
</Warning>
...
Checkout sessions offrono un'esperienza di checkout ospitata e più sicura, che gestisce l'intero flusso di pagamento sia per gli acquisti una tantum sia per gli abbonamenti, con pieno controllo sulla personalizzazione.
...
### Formato della risposta
Checkout sessions restituiscono una risposta JSON con l'URL di checkout:
...
## Gestore di route Customer Portal

Il gestore di route Customer Portal consente di integrare senza problemi il portale clienti di Dodo Payments nella tua applicazione SvelteKit.

### Parametri di query
<ParamField query="customer_id" type="string" required>
L'ID del cliente per la sessione del portale (ad esempio, <code>?customer_id=cus_123</code>).
</ParamField>
...
- **Convalida del payload:** viene convalidato con Zod. Restituisce 400 per i payload non validi.
- **Gestione degli errori:**
  - 401: firma non valida
  - 400: payload non valido
  - 500: errore interno durante la verifica
- **Instradamento degli eventi:** chiama il gestore di eventi appropriato in base al tipo di payload.
