...
description: Comprendere i diversi eventi webhook attivati ​​da Dodo Payments
  durante le attività di pagamento, rimborso, abbonamento, disputa, chiave di
  licenza, recupero e dunning
icon: bell
keywords:
  - Dodo Payments
  - eventi webhook
...
|--------------------------|---------------------------------|---------------------------------------------------------------------------------|
| **Checkout Abbandonato Rilevato** | `abandoned_checkout.detected` | Attivato quando un checkout incompleto o fallito viene rilevato dopo 60 minuti.  |
| **Checkout Abbandonato Recuperato** | `abandoned_checkout.recovered` | Si verifica quando un cliente completa il pagamento tramite un link di recupero.              |
...
| **Nome Evento**           | **Tipo Evento**           | **Descrizione**                                                                 |
|--------------------------|--------------------------|---------------------------------------------------------------------------------|
| **Dunning Avviato**      | `dunning.started`        | Attivato quando inizia un tentativo di dunning per un abbonamento sospeso o cancellato. |
| **Dunning Recuperato**    | `dunning.recovered`      | Si verifica quando un cliente aggiorna il suo metodo di pagamento e l'addebito risultante ha successo. |

Per schemi dettagliati del payload, descrizioni dei campi ed esempi di handler, vedere [Webhook di Recupero](/developer-resources/webhooks/intents/recovery).

Questi eventi webhook sono progettati per fornire ai commercianti aggiornamenti critici per gestire efficacemente le loro operazioni. Implementare handler robusti per ogni evento rilevante nella vostra integrazione webhook per garantire funzionalità fluida e affidabile.
