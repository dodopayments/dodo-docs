...
{/* LOCKED_PATTERN_168b18b8d8742ed481ee0ff6b3f8bcb1 */}
  Guida completa al codice sorgente e alla configurazione
</Card>
...
5. Esegui la query
6. Ottieni la stringa di connessione da Neon → Dettagli di Connessione
...
1. Vai alla [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
...
✅ **Registrazione degli eventi** - Traccia completa nella tabella `webhook_events`  
✅ **Gestione degli errori** - Registrati e ripetibili

> **Nota:** Questa implementazione dimostra la gestione di tre eventi principali di sottoscrizione (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con i campi minimi. Puoi facilmente estenderla per supportare tipi di eventi aggiuntivi e campi in base alle tue esigenze.
...
1. **Verifica la firma** - Garantisce che la richiesta provenga da DodoPayments utilizzando HMAC-SHA256
2. **Controlla i duplicati** - Utilizza l'ID del webhook per prevenire l'elaborazione dello stesso evento due volte
3. **Registra l'evento** - Conserva il webhook grezzo nella tabella `webhook_events` per la tracciabilità
4. **Elabora l'evento** - Crea o aggiorna clienti e sottoscrizioni in Neon
5. **Gestisce gli errori** - Registra i fallimenti e contrassegna l'evento come non elaborato per il tentativo successivo
...
**Visualizza i log nel Dashboard di Netlify:**
1. Seleziona il tuo sito → scheda **Funzioni**
2. Fai clic sulla funzione `webhook`
3. Visualizza log in tempo reale e cronologia delle invocazioni
...
| Problema | Soluzione |
|----------|-----------|
| Verifica fallita | Verifica che la chiave webhook sia corretta nella dashboard di DodoPayments |
| Errore di connessione al database | Verifica la stringa di connessione Neon e utilizza connessioni in pool |
| Funzione non trovata (404) | Esegui `netlify init` e `npm run deploy` di nuovo |
| Variabili d'ambiente non disponibili | Imposta le variabili nella CLI o nella dashboard, quindi ridistribuisci |
