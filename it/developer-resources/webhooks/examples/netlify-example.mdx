...
> **Nota:** Questa implementazione dimostra la gestione di tre eventi di sottoscrizione core (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con campi minimi. Puoi facilmente estenderla per supportare tipi di eventi e campi aggiuntivi in base alle tue esigenze.
...
1. **Verifica la firma** - Assicura che la richiesta provenga da DodoPayments utilizzando HMAC-SHA256
2. **Controlla i duplicati** - Utilizza l'ID webhook per prevenire l'elaborazione dello stesso evento due volte
3. **Registra l'evento** - Memorizza il webhook grezzo nella tabella `webhook_events` per audit trail
4. **Elabora l'evento** - Crea o aggiorna clienti e sottoscrizioni in Neon
5. **Gestisce gli errori** - Registra i fallimenti e contrassegna l'evento come non elaborato per il retry
...
2. Clicca sulla funzione `webhook`
3. Visualizza i log in tempo reale e la cronologia delle invocazioni
...
| Problema | Soluzione |
|-------|----------|
| Verifica fallita | Controlla che la chiave webhook sia corretta dal dashboard di DodoPayments |
| Errore di connessione al database | Verifica la stringa di connessione Neon e utilizza la connessione pool |
| Funzione non trovata (404) | Esegui di nuovo `netlify init` e `npm run deploy` |
| Variabili d'ambiente non disponibili | Imposta le variabili nella CLI o nel dashboard, quindi ridistribuisci |
