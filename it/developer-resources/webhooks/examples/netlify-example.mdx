---
title: Funzioni Netlify
description: Distribuisci i webhook di DodoPayments su Funzioni Netlify
icon: diamond
---

<Card title="Repository GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/netlify">
...
### 1. Requisiti
- [Account Netlify](https://app.netlify.com/signup)
...
1. Registrati per [Neon](https://neon.com)
2. Crea un nuovo progetto
3. Apri l'Editor SQL
4. Copia e incolla il contenuto di [`schema.sql`](#database-schema)
5. Esegui la query
6. Ottieni la tua stringa di connessione da Neon → Dettagli di Connessione
...
1. Vai al [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
...
✅ **Idempotenza** - Previene il processamento duplicato con gli ID del webhook  
✅ **Registrazione eventi** - Completa audit trail nella tabella `webhook_events`  
✅ **Gestione errori** - Registrata e ripetibile

> **Nota:** Questa implementazione dimostra la gestione di tre eventi di abbonamento core (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con campi minimi. Puoi facilmente estenderla per supportare ulteriori tipi di eventi e campi in base alle tue esigenze.
...
1. **Verifica la firma** - Garantisce che la richiesta provenga da DodoPayments usando HMAC-SHA256
2. **Controlla i duplicati** - Usa l'ID del webhook per prevenire il processamento dello stesso evento due volte
3. **Registra l'evento** - Memorizza il webhook raw nella tabella `webhook_events` per audit trail
4. **Processa l'evento** - Crea o aggiorna clienti e abbonamenti in Neon
5. **Gestisce gli errori** - Registra i fallimenti e contrassegna l'evento come non processato per il retry
...
**Visualizza i log nel Netlify Dashboard:**
1. Seleziona il tuo sito → scheda **Funzioni**
2. Fai clic sulla funzione `webhook`
3. Visualizza i log in tempo reale e la storia delle invocazioni

**Configura nel Dashboard di DodoPayments:**
1. Vai a Sviluppatori → Webhook
2. Aggiungi endpoint con il tuo URL delle Funzioni Netlify
...
| Problema | Soluzione |
|-------|----------|
| Verifica non riuscita | Controlla che la chiave del webhook sia corretta dal dashboard di DodoPayments |
| Errore di connessione al database | Verifica la stringa di connessione di Neon e utilizza una connessione in pool |
| Funzione non trovata (404) | Esegui di nuovo `netlify init` e `npm run deploy` |
| Variabili Env non disponibili | Imposta le variabili nella CLI o nel dashboard, quindi ridistribuisci |

## Risorse

- [Documentazione Funzioni Netlify](https://docs.netlify.com/functions/overview/)
- [Netlify CLI](https://docs.netlify.com/cli/get-started/)
- [Documentazione Neon](https://neon.com/docs/)
- [Guida agli Eventi Webhook](/developer-resources/webhooks/intents/webhook-events-guide)
- [Repo GitHub](https://github.com/dodopayments/cloud-functions/tree/main/netlify)
