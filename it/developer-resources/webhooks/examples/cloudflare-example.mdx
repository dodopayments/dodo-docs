...
<Card title="Repository GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/cloudflare">
...
> **Nota:** Imposteremo `DODO_PAYMENTS_WEBHOOK_KEY` dopo il deploy una volta che hai il tuo URL webhook.

### 5. Aggiorna wrangler.toml

Modifica `wrangler.toml` e imposta il nome del tuo worker:
...
2. **Controlla i duplicati** - Usa l'ID webhook per prevenire l'elaborazione dello stesso evento due volte
...
5. **Gestisce errori** - Registra i fallimenti e contrassegna l'evento come non elaborato per il retry
...
| Errore di connessione al database | Verifica la stringa di connessione di Neon |
| Worker non in fase di deploy | Esegui `npm install`, controlla la sintassi di `wrangler.toml` |
