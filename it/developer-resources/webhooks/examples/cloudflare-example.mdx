...
description: Distribuisci i webhook di DodoPayments su Cloudflare Workers
icon: cloud
---

{/* LOCKED_PATTERN_2b313346674bb180793c904f406c4c27 */}
  Guida completa all'installazione e al codice sorgente
</Card>

## Configurazione Rapida

### 1. Requisiti
- [Account Cloudflare](https://dash.cloudflare.com/sign-up)
...
5. Esegui la query
6. Recupera la stringa di connessione da Neon → Connection Details

### 4. Configura i Segreti Iniziali
...
Modifica `wrangler.toml` e imposta il nome del worker:
...
### 7. Ottieni il Tuo URL del Webhook
...
### 8. Registra il Webhook nel Dashboard di DodoPayments

1. Vai su [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
...
### 9. Imposta la Chiave del Webhook e Ridistribuisci
...
✅ **Idempotenza** - Previene la duplicazione dell'elaborazione grazie agli ID webhook  
✅ **Registrazione eventi** - Trail di audit completo nella tabella `webhook_events`  
✅ **Gestione degli errori** - Registrata e ritentabile

> **Nota:** Questa implementazione mostra la gestione di tre eventi principali di sottoscrizione (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con campi minimi. Puoi facilmente estenderla per supportare altri tipi di eventi e campi in base alle tue esigenze.
...
1. **Verifica la firma** - Assicura che la richiesta provenga da DodoPayments usando HMAC-SHA256
2. **Controlla i duplicati** - Usa l'ID del webhook per evitare di elaborare lo stesso evento due volte
3. **Registra l'evento** - Memorizza il webhook grezzo nella tabella `webhook_events` per il trail di audit
4. **Elabora l'evento** - Crea o aggiorna clienti e sottoscrizioni
5. **Gestisce gli errori** - Registra i fallimenti e marca l'evento come non elaborato per un nuovo tentativo
...
2. Aggiungi endpoint con il tuo URL Worker
...
| Problema | Soluzione |
|-------|----------|
| Verifica fallita | Controlla che la chiave webhook sia corretta dalla dashboard di DodoPayments |
| Errore di connessione al database | Verifica la stringa di connessione Neon |
| Worker non deployato | Esegui `npm install`, controlla la sintassi di `wrangler.toml` |
...
- [Repo GitHub](https://github.com/dodopayments/cloud-functions/tree/main/cloudflare)
