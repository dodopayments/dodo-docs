...
> **Nota:** Imposteremo `DODO_PAYMENTS_WEBHOOK_KEY` dopo il deployment una volta che avrai il tuo URL di webhook.
...
✅ **Verifica della firma** - Utilizza la libreria dodopayments  
✅ **Idempotenza** - Previene l'elaborazione duplicata con gli ID dei webhook  
✅ **Logging degli eventi** - Traccia di controllo completa nella tabella `webhook_events`  
✅ **Gestione degli errori** - Registrati e ritentabili

> **Nota:** Questa implementazione dimostra la gestione di tre eventi di sottoscrizione principali (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con campi minimi. Puoi facilmente estenderla per supportare ulteriori tipi di eventi e campi in base alle tue esigenze.
...
2. **Controlla l'idempotenza** - Verifica l'ID del webhook per evitare elaborazioni duplicate
3. **Registra l'evento** - Memorizza i dati grezzi del webhook nella tabella `webhook_events`
4. **Elabora gli aggiornamenti** - Crea o aggiorna clienti e sottoscrizioni tramite il client Supabase
5. **Gestisce gli errori** - Registra i fallimenti e segnala l'evento per un ritentativo
...
Il flag `--no-verify-jwt` è richiesto perché i webhook non includono token JWT. La sicurezza è fornita dalla verifica della firma del webhook.
...
| Problema | Soluzione |
|----------|-----------|
| Verifica fallita | Controlla che la chiave webhook sia corretta dalla dashboard DodoPayments |
| Errore di permessi del database | Assicurati di utilizzare la Service Role Key |
| Errore di verifica JWT | Distribuisci con il flag `--no-verify-jwt` |
| Funzione non trovata | Verifica che il riferimento del progetto sia corretto e che la funzione sia distribuita |
