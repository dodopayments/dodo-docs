...
description: Distribuisci i webhook di DodoPayments a Supabase Edge Functions
icon: bolt
---

<Card title="Repository GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/supabase">
  Codice sorgente completo e guida all'installazione
</Card>

<Tip>
  <strong>Controlla il kit di avvio completo per supabase</strong><br />
...
### 1. Requisiti
- [Account Supabase](https://supabase.com/dashboard)
...
Ottieni il tuo riferimento progetto da [Supabase Dashboard](https://supabase.com/dashboard) → Impostazioni progetto

### 3. Configurazione del Database

1. Vai al tuo [Supabase Dashboard](https://supabase.com/dashboard)
2. Apri l'Editor SQL
3. Crea una nuova query
4. Copia e incolla l'intero contenuto di [`schema.sql`](#database-schema)
5. Esegui la query
...
> **Nota:** Imposteremo `DODO_PAYMENTS_WEBHOOK_KEY` dopo il deploy, una volta che avrai il tuo URL webhook.

### 5. Distribuisci

La funzione è già impostata in `functions/webhook/index.ts` - basta distribuirla:
...
1. Vai al [DodoPayments Dashboard](https://app.dodopayments.com) → Sviluppatore → Webhooks
...
✅ **Idempotenza** - Impedisce processi duplicati con gli ID dei webhook  
✅ **Registrazione eventi** - Audit trail completo nella tabella `webhook_events`  
✅ **Gestione degli errori** - Registrati e ripetibili

> **Nota:** Questa implementazione dimostra la gestione di tre eventi di abbonamento principali (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con campi minimi. Puoi facilmente estenderla per supportare ulteriori tipi di eventi e campi in base alle tue esigenze.
...
1. **Verifica la firma** - Usa la libreria dodopayments per la verifica HMAC-SHA256
2. **Controlla l'idempotenza** - Cerca l'ID del webhook per prevenire elaborazioni duplicate
3. **Registra l'evento** - Memorizza i dati grezzi del webhook nella tabella `webhook_events`
4. **Elabora gli aggiornamenti** - Crea o aggiorna clienti e abbonamenti tramite il client Supabase
5. **Gestisce gli errori** - Registra i fallimenti e contrassegna l'evento per ripetere
...
<Note>
Il flag `--no-verify-jwt` è necessario perché i webhook non includono token JWT. La sicurezza è fornita dalla verifica della firma webhook.
</Note>
...
Oppure nel [Supabase Dashboard](https://supabase.com/dashboard) → Funzioni Edge → webhook → scheda Log
...
| Problema | Soluzione |
|-------|----------|
| Verifica fallita | Controlla che la chiave del webhook sia corretta nel dashboard di DodoPayments |
| Errore di autorizzazione del database | Assicurati di usare la Service Role Key |
| Errore di verifica JWT | Distribuisci con il flag `--no-verify-jwt` |
| Funzione non trovata | Verifica che il riferimento del progetto sia corretto e la funzione sia distribuita |

## Risorse

- [Documentazione delle Funzioni Edge di Supabase](https://supabase.com/docs/guides/functions)
- [Supabase CLI](https://supabase.com/docs/reference/cli)
- [Deno Runtime](https://deno.land/)
- [Guida agli Eventi Webhook](/developer-resources/webhooks/intents/webhook-events-guide)
- [Repo GitHub](https://github.com/dodopayments/cloud-functions/tree/main/supabase)
