...
<Card title="Repository GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/vercel">
...
3. Apri l'editor SQL
4. Copia e incolla il contenuto di [`schema.sql`](#database-schema)
5. Esegui la query
6. Ottieni la tua stringa di connessione da Neon → Dettagli connessione
...
> **Nota:** Imposteremo `DODO_PAYMENTS_WEBHOOK_KEY` dopo il deployment una volta che avrai il tuo URL del webhook.
...
1. Vai al [DodoPayments Dashboard](https://app.dodopayments.com) → Sviluppatore → Webhook
2. Crea un nuovo endpoint webhook
3. Configura il tuo URL del webhook come endpoint
...
✅ **Idempotenza** - Previene l'elaborazione duplicata con gli ID dei webhook  
✅ **Registrazione degli eventi** - Audit trail completo nella tabella `webhook_events`  
✅ **Gestione degli errori** - Registrato e ripetibile

> **Nota:** Questa implementazione dimostra la gestione di tre eventi di sottoscrizione principali (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) con campi minimi. Puoi facilmente estenderla per supportare tipi di eventi e campi aggiuntivi in base alle tue esigenze.
...
1. **Disabilita l'analisi del corpo** - Per accedere al corpo raw per la verifica della firma
2. **Verifica la firma** - Assicura che la richiesta provenga da DodoPayments utilizzando HMAC-SHA256
3. **Controlla i duplicati** - Utilizza l'ID del webhook per prevenire l'elaborazione dello stesso evento due volte
4. **Registra l'evento** - Memorizza il webhook raw nella tabella `webhook_events` per audit trail
5. **Elabora l'evento** - Crea o aggiorna clienti e sottoscrizioni in Neon
6. **Gestisce gli errori** - Registra i fallimenti e contrassegna l'evento come non elaborato per il retry
