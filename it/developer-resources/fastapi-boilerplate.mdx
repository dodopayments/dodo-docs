...
<Card title="Repository GitHub" icon="github" href="https://github.com/dodopayments/fastapi-boilerplate">
...
<Info>
Questo boilerplate utilizza FastAPI con schemi async/await, Pydantic per la convalida e il SDK Python `dodopayments` per un'integrazione API senza soluzione di continuità.
</Info>
...
<Step title="Crea Ambiente Virtuale">
...
<Warning>
Non inserire mai il tuo file `.env` nel controllo delle versioni. È già incluso in `.gitignore`.
</Warning>
</Step>
...
| `/webhook` | POST | Gestisci i webhook dei pagamenti Dodo |
...
- Usa le chiavi API di produzione dal pannello di controllo
- Aggiorna l'URL del webhook sul tuo dominio di produzione
...
<AccordionGroup>
<Accordion title="Errori di importazione o moduli mancanti">
...
<Accordion title="Il fallimento della creazione della sessione di checkout">
**Cause comuni:**
- ID prodotto non valido - verifica che esista nel tuo pannello di controllo Dodo
- Chiave API o impostazione di ambiente errata in `.env`
...
Aggiorna l'URL del webhook nel tuo [pannello di controllo Dodo](https://app.dodopayments.com/developer/webhooks) con l'URL di ngrok. Assicurati di aggiornare il tuo file `.env` con la corretta chiave di verifica del webhook.
</Accordion>

<Accordion title="Il fallimento della verifica della firma del webhook">
- Assicurati che `DODO_PAYMENTS_WEBHOOK_KEY` sia impostato correttamente nel tuo `.env`
- Verifica che stai utilizzando il corpo della richiesta raw per la verifica della firma
