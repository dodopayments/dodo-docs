...
- Verifica di utilizzare il corpo della richiesta originale per la verifica della firma
- Passa tutti e tre gli header `webhook-id`, `webhook-timestamp` e `webhook-signature` a `webhook.verify()` — la firma Standard Webhooks copre `id.timestamp.body`, non solo il corpo
</Accordion>
</AccordionGroup>
