...
| `onEvent` | `function` | Sì | Funzione di callback per gestire eventi di checkout |
...
| Opzione | Tipo | Richiesto | Descrizione |
|--------|------|----------|-------------|
| `checkoutUrl` | `string` | Sì | URL di sessione di checkout dall' [API di creazione della sessione di checkout](/api-reference/checkout-sessions/create) |
...
| `options.manualRedirect` | `boolean` | No | Quando abilitato, il checkout non reindirizzerà automaticamente dopo il completamento. Invece, riceverai gli eventi `checkout.status` e `checkout.redirect_requested` per gestire il reindirizzamento tu stesso. |
...
L'SDK fornisce eventi in tempo reale a cui puoi ascoltare attraverso il callback `onEvent`:
...
|------------|-------------|
| `checkout.opened` | Il sovrapposizione del checkout è stata aperta |
| `checkout.form_ready` | Il modulo di checkout è pronto per ricevere input dall'utente. Utile per nascondere stati di caricamento. |
...
| `checkout.closed` | Il sovrapposizione del checkout è stata chiusa |
...
Puoi personalizzare l'aspetto del checkout passando un oggetto `themeConfig` nel parametro `options` quando apri il checkout. La configurazione del tema supporta sia le modalità chiare che scure, consentendoti di personalizzare colori, bordi, testo, pulsanti e raggio di bordo.

<Info>
Questa sezione tratta della configurazione del tema **lato client** utilizzando l'SDK di Checkout. Puoi anche configurare i temi **lato server** durante la creazione di una sessione di checkout tramite l'API utilizzando il parametro `theme_config`. Vedi [Personalizzazione del Tema di Checkout](/features/checkout#checkout-theme-customization) per configurazione a livello API.
</Info>

#### Configurazione del Tema di Base
...
Se vuoi solo personalizzare il tema chiaro:
...
Se vuoi solo personalizzare il tema scuro:
...
Puoi sovrascrivere solo proprietà specifiche. Il checkout utilizzerà valori predefiniti per le proprietà che non specificate:
...
2. **Gestione degli errori**: Implementa sempre una corretta gestione degli errori nel tuo callback di evento
...
- Assicurati che non ci siano conflitti di z-index con il sovrapposizione
...
Per informazioni dettagliate su come impostare Google Pay e altri wallet digitali, consulta la pagina <a href="/features/payment-methods/digital-wallets">Wallet Digitali</a>.

<Note>
Apple Pay non è ancora supportato nel checkout in overlay. Il supporto per Apple Pay arriverà presto.
</Note>

## Supporto Browser

L'SDK di Checkout di Dodo Payments supporta i seguenti browser:
...
## Checkout in Overlay vs Inline

Scegli il tipo di checkout giusto per il tuo caso d'uso:

| Caratteristica | Checkout Overlay | Checkout Inline |
|---------|-----------------|-----------------|
| Profondità di integrazione | Modale sopra la pagina | Completamente incorporato nella pagina |
| Controllo del layout | Limitato | Controllo completo |
| Branding | Separato dalla pagina | Semplice |
...
<Tip>
Usa il **checkout in overlay** per un'integrazione più rapida con minime modifiche alle tue pagine esistenti. Usa il **checkout inline** quando desideri il massimo controllo sull'esperienza di checkout e un branding senza soluzione di continuità.
</Tip>
