...
Utilizza le Checkout Sessions per vendere prodotti in abbonamento con un checkout sicuro e ospitato. Passa il tuo prodotto in abbonamento in `product_cart` e reindirizza i clienti al `checkout_url` restituito.
...
3. **`subscription.on_hold`** - L'abbonamento è in sospeso a causa di un rinnovo non riuscito.
...
<Tip>
Utilizza `subscription.updated` per ricevere notifiche in tempo reale su qualsiasi modifica dell'abbonamento, mantenendo lo stato della tua applicazione sincronizzato senza interrogare l'API.
</Tip>
...
2. `payment.succeeded` - Conferma il pagamento iniziale:
    ...
    - Per giorni di prova: ogni volta che termina
...
2. Abbonamento in sospeso
  - `subscription.on_hold` - L'abbonamento è in sospeso a causa di un pagamento di rinnovo non riuscito o di un addebito per modifica piano non riuscito.
  - Quando un abbonamento è in sospeso, non si rinnoverà automaticamente fino a quando il metodo di pagamento non sarà aggiornato.
...
Quando un abbonamento entra nello stato `on_hold`, è necessario aggiornare il metodo di pagamento per riattivarlo. Questa sezione spiega quando gli abbonamenti vanno in sospeso e come gestirli.
...
Per riattivare un abbonamento dallo stato `on_hold`, utilizza l'API Aggiorna Metodo di Pagamento. Questo automaticamente:
...
Quando ricevi un webhook {/* INLINE_CODE_PLACEHOLDER_5e9a0875003c752d72379e80bcc55356 */, aggiorna lo stato della tua applicazione e notifica il cliente:
...
| `type` | string | Sì | Il tipo di evento. Vedi [Tipi di Evento](#event-types) |
...
- Calcola l'importo proporzionale in base al tempo rimanente nel ciclo di fatturazione attuale
...
- Utile quando si desidera ripristinare il ciclo di fatturazione o addebitare l'importo totale indipendentemente dalla proporzione

#### 3. `difference_immediately`
- Quando si effettua un upgrade, al cliente viene immediatamente addebitata la differenza tra i due importi del piano.
- Ad esempio, se il piano attuale è di 30 Dollari e il cliente passa a un piano di 80 Dollari, gli viene addebitato $50 immediatamente. 
- Quando si effettua un downgrade, l'importo non utilizzato dal piano attuale viene aggiunto come credito interno e applicato automaticamente ai futuri rinnovi dell'abbonamento.
...
- Quando invochi questa API, Dodo Payments avvia immediatamente un addebito in base all'opzione di proporzione selezionata
- Se il cambiamento del piano è un downgrade e utilizzi `prorated_immediately`, i crediti verranno calcolati automaticamente e aggiunti al saldo di credito dell'abbonamento. Questi crediti sono specifici per quell'abbonamento e verranno utilizzati solo per compensare i futuri pagamenti ricorrenti dello stesso abbonamento
- L'opzione `full_immediately` salta i calcoli dei crediti e addebita l'intero importo del nuovo piano

<Tip>
**Scegli attentamente la tua opzione di proporzione**: Utilizza `prorated_immediately` per una fatturazione equa che tiene conto del tempo non utilizzato, oppure `full_immediately` quando desideri addebitare l'intero importo del nuovo piano indipendentemente dal ciclo di fatturazione attuale.
</Tip>
