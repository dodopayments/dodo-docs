...
<Steps>
<Step title="Crea il Plugin Base">
Crea il tuo progetto di plugin Figma utilizzando la [guida rapida ai plugin di Figma](https://www.figma.com/plugin-docs/plugin-quickstart-guide/). Questo genererà uno scaffold con un file `manifest.json`.
...
- Personalizza `LicenseKeyInput.tsx` per soddisfare le tue preferenze di design
...
Il plugin necessita di accesso alla rete per comunicare con il proxy CORS. Aggiungi quanto segue al tuo `manifest.json`:
...
| `npm run dev` | Avvia il server di sviluppo con il file watching |
...
4. Invia per revisione tramite la Comunità Figma
