...
1. Vai al tuo cruscotto di MailerLite
2. Naviga a **Campi abbonati**
3. Clicca su **Crea campo** e aggiungi campi come:
   - `total_spent` (Numero)
   - `customer_since` (Data)
...
| `fields` | oggetto | No | Oggetto con coppie nome/valore del campo |
| `fields.name` | string | No | Nome di battesimo dell'abbonato |
| `fields.last_name` | string | No | Cognome dell'abbonato |
...
| `groups` | array | No | Array di ID gruppi a cui aggiungere l'abbonato |
| `status` | string | No | Uno di: attivo, disiscritto, non confermato, rimbalzo, spam |
| `subscribed_at` | string | No | Data nel formato `yyyy-MM-dd HH:mm:ss` |
| `ip_address` | string | No | Indirizzo IP dell'abbonato |
