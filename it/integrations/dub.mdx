...
Dub traccia i visitatori attraverso un ID clic unico (`dub_id`) memorizzato in un cookie quando gli utenti fanno clic sui tuoi link brevi di Dub. Per attribuire le vendite ai tuoi link, è necessario:

1. **Catturare l'ID clic di Dub** dal cookie `dub_id` quando si creano sessioni di checkout
2. **Memorizzare l'ID clic** nei metadati di pagamento insieme all'ID esterno del cliente
...
<Step title="Abilita il Monitoraggio delle Conversioni in Dub">
  ...
  <Info>
  Scopri di più su come abilitare il monitoraggio delle conversioni nella [documentazione di Dub](https://dub.co/docs/conversions/quickstart).
  </Info>
</Step>

<Step title="Ottieni la tua Chiave API di Dub">
  Naviga nel tuo [dashboard Dub](https://app.dub.co) → Impostazioni → Chiavi API e crea una nuova chiave API con ambito `conversions.write`.

<Step title="Ottieni la tua chiave API di Dub">
  Naviga al tuo [dashboard di Dub](https://app.dub.co) → Impostazioni → Chiavi API e crea una nuova chiave API con ambito `conversions.write`.
...
- **Includi sempre l'ID clic nei metadati**: Senza l'ID clic, Dub non può attribuire entrate ai tuoi link
- **Utilizza ID esterni in modo coerente**: Passa lo stesso ID cliente che usi nel tuo sistema per un'analisi accurata a livello di cliente
- **Gestisci il traffico organico con cura**: Imposta `webhook.cancel = true` quando non c'è ID clic per evitare chiamate API non necessarie
- **Testa con pagamenti di esempio**: Verifica che l'integrazione funzioni correttamente prima di andare in produzione
- **Monitora il tuo dashboard di Dub**: Controlla che le vendite appaiano correttamente con attribuzione adeguata
...
- Verifica che il webhook venga attivato sugli eventi `payment.succeeded`
- Conferma che il tracciamento delle conversioni sia abilitato per i tuoi link di Dub
</Accordion>

<Accordion title="L'attribuzione delle entrate non funziona">
- Conferma che gli utenti stiano facendo clic sui tuoi link brevi di Dub prima del checkout
...
- Assicurati di tracciare solo sugli eventi `payment.succeeded`, non `payment.processing`
