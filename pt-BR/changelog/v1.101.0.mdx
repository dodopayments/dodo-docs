...
| Surface | Field | Notes |
|---------|-------|-------|
| Checkout Sessions | `customer_business_name`, `feature_flags.allow_customer_editing_business_name` | Máx. 250 caracteres; flag assume `false` por padrão |
| Payments | `customer_business_name` | Máx. 250 caracteres |
| Subscriptions | `customer_business_name` | Defina ou limpe via `PATCH /subscriptions/{subscription_id}` |
