...
### 5. **`invoice_url` Campo na Resposta de Pagamento e Resposta de Link de Pagamento**  
   
   A Resposta de Pagamento e a Resposta de Link de Pagamento agora incluem um campo `invoice_url`, proporcionando acesso direto à fatura associada a cada pagamento.
...
### 7. **Capacidades de Filtragem Aprimoradas para Clientes, Descontos e Chaves de Licença**  
   ...
   **Filtros da API de Clientes**
   ...
   | Filtro | Descrição |
   |--------|-------------|
   | `email` | Filtrar por e-mail do cliente |
   | `name` | Filtrar por nome do cliente (casamento parcial, sem diferenciar maiúsculas de minúsculas) |
   | `created_at_gte` | Filtrar clientes criados a partir deste timestamp |
   | `created_at_lte` | Filtrar clientes criados antes deste timestamp |
   ...
   **Filtros da API de Descontos**
   ...
   | Filtro | Descrição |
   |--------|-------------|
   | `code` | Filtrar por código de desconto (casamento parcial, sem diferenciar maiúsculas de minúsculas) |
   | `discount_type` | Filtrar por tipo de desconto (percentual) |
   ...
   **Filtros da API de Chaves de Licença**
   ...
   | `status` | Filtrar por status de chave de licença (`active`, `expired`, `disabled`) |
   | `product_id` | Filtrar por ID do produto |
   | `created_at_gte` | Filtrar chaves de licença criadas a partir deste timestamp |
   | `created_at_lte` | Filtrar chaves de licença criadas antes deste timestamp |
