...
- **Objeto de assinatura**: Novo campo `cancellation_feedback` (um dos valores acima) e `cancellation_comment` (texto livre opcional), preenchidos quando o cliente cancela
- **Webhook `subscription.cancelled`**: Ambos os campos são incluídos no payload
- **API**: Passe `cancellation_feedback` e `cancellation_comment` para `PATCH /subscriptions/{subscription_id}` ao agendar ou executar um cancelamento programaticamente
