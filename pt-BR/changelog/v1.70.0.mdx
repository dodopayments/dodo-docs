...
keywords:
  ...
  - v1.70.0 (24 de dezembro de 2025)
---
...
1. **Checkout Inline - Experiências de Checkout Totalmente Embutidas (BETA)**  
   ...
   <Frame>
   <img src="/images/inline-checkout/cover.png" alt="Imagem de capa do checkout inline" style={{ maxHeight: '500px', width: 'auto' }} />
   </Frame>
   ...
   - **Eventos de detalhamento em tempo real**: Receba `checkout.breakdown` à medida que os clientes inserem seus dados
   - **Resumos de pedidos personalizados**: Construa sua própria exibição de preços que atualiza em tempo real
   - **Integração perfeita**: O quadro de checkout lida com a coleta do pagamento enquanto você controla o layout
   - **Métodos do SDK**: Use `open()`, `close()` e `isOpen()` para controlar o checkout programaticamente

   <Check>
   Resultado: Experiências de checkout totalmente integradas que combinam com a sua marca e oferecem controle máximo sobre o fluxo de pagamento.
   </Check>

   <Tip>
   Use o checkout inline quando desejar controle máximo sobre o design do checkout. Use o [checkout em sobreposição](/developer-resources/overlay-checkout) para uma integração mais rápida com mudanças mínimas nas páginas existentes.
   </Tip>

   Saiba mais: [Guia de Checkout Inline](/developer-resources/inline-checkout)

2. **Crie Pagamentos, Assinaturas e Checkouts Usando Métodos de Pagamento Existentes**  
    Simplifique os fluxos de checkout usando métodos de pagamento salvos de clientes existentes. O novo parâmetro `payment_method_id` permite criar pagamentos, assinaturas ou sessões de checkout utilizando o método de pagamento previamente salvo de um cliente.
   ...
   <Warning>
   Ao utilizar `payment_method_id` em sessões de checkout, `confirm` deve ser definido como `true`, e um `customer_id` existente deve ser fornecido. O método de pagamento será validado quanto à elegibilidade com a moeda do pagamento.
   </Warning>
...
3. **Mudanças no Plano de Assinatura no Painel com Atualizações da Próxima Data de Cobrança**  
    ...
    <Frame>
    <img src="/images/changelog/subscription-plan-changes.png" alt="Alterações no plano de assinatura no dashboard" style={{ maxHeight: '500px', width: 'auto' }} />
    </Frame>
   ...
   <Check>
   Resultado: Controle completo de gerenciamento de assinaturas diretamente pelo painel, reduzindo a necessidade de chamadas de API para ajustes comuns de assinaturas.
   </Check>

   <Tip>
   Use o painel para mudanças rápidas de plano de assinatura e a API para gerenciamento programático de assinaturas em sua aplicação.
   </Tip>
...
4. **Links Curtos para URLs de Pagamento**  
   ...
   <Frame>
   <img src="/images/changelog/short-links.png" alt="Recurso de links curtos para URLs de pagamento" style={{ maxHeight: '500px', width: 'auto' }} />
   </Frame>
   ...
   <Check>
   Resultado: Um link de pagamento encurtado que é mais fácil de compartilhar e com aparência mais profissional, melhorando taxas de conversão e a confiança do cliente.
   </Check>

   <Info>
   Os links curtos são gerenciados no seu painel e podem ser visualizados por meio da [List Short Links API](/api-reference/products/get-products-short-links).
   </Info>

5. **Sinalizador `redirect_immediately` - Pular a Página de Sucesso do Pagamento**  
   Controle o fluxo de checkout com o novo sinalizador `redirect_immediately`. Quando ativado, os clientes são redirecionados imediatamente após a conclusão do pagamento, ignorando a página de sucesso padrão para uma experiência mais rápida e simplificada.
  ...
  ```
   ...
   <Tip>
   Use `redirect_immediately: true` quando você tiver uma página de sucesso personalizada que fornece uma experiência melhor do que a página de sucesso padrão.
   </Tip>

   <Note>
   Quando `redirect_immediately` estiver ativado, os clientes serão redirecionados para o seu `return_url` imediatamente após a conclusão do pagamento, ignorando completamente a página de sucesso padrão.
   </Note>
