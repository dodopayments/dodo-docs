...
1. **Checkout Inline - Experiências de Checkout Totalmente Embutidas (BETA)**  
   ...
   Inicialize o SDK com `displayType: 'inline'` e embuta o checkout em um elemento contêiner:
   ...
   - **Resumos de pedidos personalizados**: Crie sua própria exibição de preços que atualiza em tempo real
   - **Integração sem costura**: O quadro de checkout gerencia a coleta de pagamentos enquanto você controla o layout
   - **Métodos SDK**: Use `open()`, `close()` e `isOpen()` para controlar o checkout programaticamente
...
2. **Crie Pagamentos, Assinaturas e Checkouts Usando Métodos de Pagamento Existentes**  
    Otimize os fluxos de checkout usando métodos de pagamento salvos de clientes existentes. O novo parâmetro `payment_method_id` permite criar pagamentos, assinaturas ou sessões de checkout usando um método de pagamento previamente salvo pelo cliente.
   ...
   <Warning>
   Ao usar `payment_method_id` em sessões de checkout, `confirm` deve ser definido como `true`, e um `customer_id` existente deve ser fornecido. O método de pagamento será validado para elegibilidade com a moeda do pagamento.
   </Warning>
...
5. **`redirect_immediately` Flag - Pular Página de Sucesso do Pagamento**  
   Controle o fluxo de checkout com a nova flag `redirect_immediately`. Quando habilitada, os clientes são redirecionados imediatamente após a conclusão do pagamento, ignorando a página de sucesso padrão para uma experiência mais rápida e otimizada.
  ...
  ```
   ...
   <Tip>
   Use `redirect_immediately: true` quando você tiver uma página de sucesso personalizada que oferece uma melhor experiência ao usuário do que a página de sucesso de pagamento padrão.
   </Tip>

   <Note>
   Quando `redirect_immediately` está habilitada, os clientes são redirecionados para sua `return_url` imediatamente após a conclusão do pagamento, pulando totalmente a página de sucesso padrão.
   </Note>
