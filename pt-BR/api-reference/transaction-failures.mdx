...
Leia `error_code` do `payment.failed` webhook ou do objeto de pagamento, mapeie para a ação recomendada acima, e decida se deve tentar novamente. Para renovações de assinatura, recusas suaves são tentadas novamente automaticamente — veja [Recuperações de Pagamento de Assinatura](/features/recovery/payment-retries).

Para erros de nível API e lógica de negócios (como `PAYMENT_NOT_SUCCEEDED` ou `REFUND_WINDOW_EXPIRED`) que não são recusas de pagamento, veja a referência [Códigos de Erro](/api-reference/error-codes).
