...
| `AUTHENTICATION_REQUIRED` | Sim | É necessária autenticação adicional para concluir a transação |
...
| `INVALID_ACCOUNT` | Sim | Os dados da conta fornecidos são inválidos |
| `INVALID_AMOUNT` | Sim | O valor da transação é inválido |
| `INVALID_CARD_NUMBER` | Sim | O formato do número do cartão é inválido |
| `INVALID_CARD_OWNER` | Sim | As informações do titular do cartão são inválidas |
...
| `INVALID_REQUEST` | Sim | A solicitação da transação contém dados inválidos |
...
| `LOST_CARD` | Sim | O cartão foi reportado como perdido |
| `MANDATE_INVALID` | Sim | O mandato de pagamento é inválido |
| `MANDATE_REQUIRED` | Sim | Um mandato é necessário para esta transação |
| `MANDATE_REQUIRED_SYSTEM` | Não | O sistema exige um mandato para este tipo de transação |
...
| `PAYMENT_METHOD_PROVIDER_DECLINED` | Sim | O provedor do meio de pagamento recusou a transação |
| `PAYMENT_METHOD_UNSUPPORTED` | Sim | O método de pagamento não é compatível com esta transação |
| `PICKUP_CARD` | Sim | O cartão foi reportado como roubado e deve ser recuperado |
...
| `STOLEN_CARD` | Sim | O cartão foi reportado como roubado |
...
<Note>
  **Erro do Usuário** indica se a recusa do pagamento pode ser resolvida pelo cliente. Quando `true`, o cliente pode agir para corrigir o problema (por exemplo, inserindo os dados corretos do cartão). Quando `false`, a recusa se deve a problemas de nível de sistema ou restrições do banco que não podem ser resolvidas pelo cliente.
</Note>
