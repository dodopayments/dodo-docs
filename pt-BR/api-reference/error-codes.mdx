...
- `ACTIVATION_LIMIT_LESS_THAN_CURRENT_AMOUNT`
  ...
  - **Mensagem:** O novo limite de ativação não pode ser menor que a contagem de instâncias atuais

- `ADDONS_IN_USAGE_BASED_BILLING_NOT_SUPPORTED`
  - **Gatilho:** Tentativa de adicionar addons a assinaturas de cobrança baseadas em uso
  - **Mensagem:** Addons em Assinaturas não são suportados para Cobrança Baseada em Uso

- `ADDONS_NOT_ALLOWED_FOR_ON_DEMAND`
  - **Gatilho:** Tentativa de adicionar addons a assinaturas sob demanda
  - **Mensagem:** Addons não são permitidos para assinaturas sob demanda

- `BRAND_MISMATCH`
  - **Gatilho:** Itens no carrinho pertencem a marcas diferentes
...
- `BRAND_SUBMISSION_NOT_ENABLED`
  - **Gatilho:** Reenvio da verificação da marca não habilitado
  - **Mensagem:** O reenvio da verificação da marca não está habilitado

- `CHARGE_NOT_ALLOWED_FOR_SCHEDULED_CANCELLATION`
  - **Gatilho:** Tentativa de cobrar uma assinatura programada para cancelamento
...
- `DISCOUNT_CODE_EXPIRED`
  ...
  - **Mensagem:** Código de desconto expirou

- `DISCOUNT_CODE_USAGE_LIMIT_EXCEEDED`
  - **Gatilho:** Desconto reutilizado após `usage_limit` atingido
  - **Mensagem:** O limite de uso não pode ser menor que times_used / limite de uso do código de desconto
...
- `DUPLICATE_METER_IDS_IN_REQUEST`
  - **Gatilho:** O mesmo ID do medidor aparece várias vezes na solicitação
  - **Mensagem:** IDs de medidor duplicados não são permitidos

- `EXCHANGE_RATE_NOT_FOUND`
  - **Gatilho:** Sem taxa de câmbio para o par de moeda `from → to`
  - **Mensagem:** A taxa de câmbio não foi encontrada para converter de Moeda para Moeda
...
- `INACTIVE_SUBSCRIPTION_PLAN_CHANGE_NOT_SUPPORTED`
  - **Gatilho:** Alteração de plano em assinatura inativa
  - **Mensagem:** Alterar planos não é suportado para assinaturas inativas
...
- `INVALID_REQUEST_BODY`
  ...
  - **Mensagem:** Seu corpo de solicitação é inválido. Por favor, verifique seus cabeçalhos de solicitação e objeto.

- `INVALID_REQUEST_PARAMETERS`
  - **Gatilho:** Semântica errada (por exemplo, data no passado)
  - **Mensagem:** Não é possível alterar next_billing_date para um tempo no passado

- `INVALID_SUGGESTED_PRICE`
  - **Gatilho:** Preço PWYW < preço mínimo permitido
  - **Mensagem:** Preço sugerido não pode ser menor que o preço mínimo. No caso de 'pague o quanto quiser', o preço é considerado como o valor mínimo aceito

- `INVALID_TAX_ID`
  - **Gatilho:** Validação de VAT/GST/TIN falhou
  - **Mensagem:** ID fiscal é inválido
...
- `LINE_ITEM_REFUND_AMOUNT_TOO_HIGH`
  ...
  - **Mensagem:** O valor do reembolso solicitado para o item de linha {id} incluindo imposto é {amount} que está acima do valor pago {amount}
...
- `MERCHANT_NOT_LIVE`
  - **Gatilho:** Negócio ainda no modo de teste/sandbox
...
- `MISSING_ADDON_IDS`
  ...
  - **Mensagem:** Um ou mais IDs de produtos não existem: {id}

- `MISSING_METER_IDS`
  - **Gatilho:** Lista de IDs de medidores vazia ou contém IDs inválidos
...
- `NO_ELIGIBLE_PAYMENT_METHODS`
  - **Gatilho:** Após filtrar, nada restou
  - **Mensagem:** Nenhum método de pagamento elegível encontrado

- `NO_EXPIRY_ON_SUBSCRIPTION_LICENSE_KEYS`
  - **Gatilho:** Tentativa de definir expiração na chave baseada em assinatura
  - **Mensagem:** Não é possível definir a data de expiração para a chave de licença baseada em assinatura
...
- `ON_DEMAND_PLAN_CHANGE_NOT_SUPPORTED`
  ...
  - **Mensagem:** Alterar planos não é suportado para assinaturas sob demanda

- `ON_DEMAND_SUBSCRIPTIONS_NOT_ENABLED`
  - **Gatilho:** Negócio possui a flag de recurso desativada
...
- `PAY_AS_YOU_WANT_AMOUNT_REQUIRED`
  ...
  - **Mensagem:** O valor é obrigatório para o produto 'pague o quanto quiser'
...
- `PLAN_CHANGE_NOT_ALLOWED_FOR_SCHEDULED_CANCELLATION`
  - **Gatilho:** Tentativa de mudança de plano em assinatura programada para cancelamento
  - **Mensagem:** Assinatura programada para cancelamento

- `PREVIOUS_PAYMENT_PENDING`
  - **Gatilho:** Tentativa de criar cobrança enquanto a anterior está em estado não terminal
  - **Mensagem:** Não é possível criar uma nova cobrança, pois o pagamento anterior ainda não foi bem-sucedido
...
- `PRODUCT_IS_DELETED`
  - **Gatilho:** Produto excluído de forma suave
  - **Mensagem:** *Sem mensagens*

- `REFUND_AMOUNT_EXCEEDS_PAID_AMOUNT`
  - **Gatilho:** Valor total agregado de reembolso > valor pago
  - **Mensagem:** O valor de reembolso calculado é maior que o valor pago

- `REFUND_WINDOW_EXPIRED`
  - **Gatilho:** Fora da janela de reembolso permitida
  - **Mensagem:** Reembolsos não podem ser iniciados {days} dias após a criação do pagamento. Contate support@dodopayments.com.
...
- `SUBSCRIPTION_PAYMENT_RETRY_LIMIT_EXCEEDED`
  - **Gatilho:** Tentativas de pagamento da assinatura excederam o máximo permitido
...
- `TOTAL_PAYMENT_AMOUNT_BELOW_MINIMUM_AMOUNT`
  - **Gatilho:** Total do carrinho combinado < mínimo do gateway
  - **Mensagem:** Valor mínimo de {display_str} é necessário para processar pagamento

- `UNABLE_TO_EDIT_PRIMARY_BRAND`
  - **Gatilho:** Tentativa de atualizar a marca primária via API regular
  - **Mensagem:** A marca primária não pode ser atualizada através deste endpoint da API.

- `UNAUTHORIZED`
  - **Gatilho:** Sem chave de API ou token inválido / escopo
  - **Mensagem:** Você não está autorizado a executar esta ação

- `UNSUPPORTED_ACTION`
  - **Gatilho:** Ação não suportada para o tipo de recurso
  - **Mensagem:** Alterar planos para assinaturas baseadas em uso não é suportado

- `UNSUPPORTED_BILLING_CURRENCY`
  - **Gatilho:** Assinaturas restritas a USD
  - **Mensagem:** Moeda de cobrança diferente de USD não é suportada para assinaturas

- `UNSUPPORTED_COUNTRY`
  - **Gatilho:** Geo ainda não suportada
  - **Mensagem:** País {country_name} atualmente não suportado

- `UNSUPPORTED_CURRENCY`
  - **Gatilho:** Produto ou moeda do addon inválido
  - **Mensagem:** A moeda não é atualmente suportada / Apenas USD e INR produtos são suportados atualmente / Apenas USD e INR suportados para preço de addon / Somente USD ou INR podem ser solicitados para billing_currency / Moeda Não Suportada / Moeda inesperada para assinaturas de cartão indiano

- `UNSUPPORTED_DISCOUNT_TYPE`
  - **Gatilho:** Descontos de quantia fixa, etc., ainda não estão ativos
...
- `UNSUCCESSFUL_PAYMENT_ID`
  - **Gatilho:** ID de pagamento referencia pagamento malsucedido
...
- `ZERO_AMOUNT_PAYMENT_REFUND_NOT_ALLOWED`
  ...
  - **Mensagem:** Não é possível reembolsar um pagamento com valor zero
