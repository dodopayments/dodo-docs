...
description: Implante webhooks do DodoPayments nas Supabase Edge Functions
icon: bolt
---

<Card title="Repositório do GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/supabase">
  Código-fonte completo e guia de configuração
</Card>

<Tip>
  <strong>Confira o kit inicial completo para supabase</strong><br />
  Um kit inicial de assinatura minimalista construído com Next.js, Supabase e Dodo Payments. Este boilerplate ajuda você a configurar rapidamente um SaaS baseado em assinatura com autenticação, pagamentos e webhooks.
...
### 1. Pré-requisitos
- [Conta Supabase](https://supabase.com/dashboard)
- Um projeto Supabase criado
- Chave da API DodoPayments do [painel](https://app.dodopayments.com/)

### 2. Autenticar & Vincular
...
Obtenha sua referência de projeto no [Painel do Supabase](https://supabase.com/dashboard) → Configurações do Projeto

### 3. Configuração do Banco de Dados

1. Vá para seu [Painel do Supabase](https://supabase.com/dashboard)
2. Abra o Editor SQL
3. Crie uma nova consulta
4. Copie e cole todo o conteúdo de [`schema.sql`](#database-schema)
5. Execute a consulta

### 4. Definir Segredos Iniciais

O Supabase fornece automaticamente `SUPABASE_URL` e `SUPABASE_SERVICE_ROLE_KEY` em tempo de execução.

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **Nota:** Definiremos `DODO_PAYMENTS_WEBHOOK_KEY` após a implantação, assim que você tiver sua URL de webhook.

### 5. Implantar

A função já está configurada em `functions/webhook/index.ts` - basta implantá-la:

```bash
npm run deploy
```

### 6. Obtenha Sua URL de Webhook
...
1. Vá para [Painel do DodoPayments](https://app.dodopayments.com) → Desenvolvedor → Webhooks
...
### 8. Definir Chave de Webhook & Reimplantar
...
Processa eventos de assinatura e os armazena no Supabase PostgreSQL:
...
✅ **Idempotência** - Previne processamento duplicado com IDs de webhook  
✅ **Registro de eventos** - Registro completo na tabela `webhook_events`  
✅ **Tratamento de erros** - Registrado e recuperável

> **Nota:** Esta implementação demonstra o tratamento de três eventos de assinatura principais (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) com campos mínimos. Você pode facilmente estendê-la para suportar tipos de eventos e campos adicionais com base em seus requisitos.
...
A Função Edge baseada em Deno:
1. **Verifica a assinatura** - Usa a biblioteca dodopayments para verificação HMAC-SHA256
2. **Verifica a idempotência** - Procura o ID do webhook para prevenir processamento duplicado
3. **Registra o evento** - Armazena os dados brutos do webhook na tabela `webhook_events`
4. **Processa atualizações** - Cria ou atualiza clientes e assinaturas através do cliente Supabase
5. **Lida com erros** - Registra falhas e marca o evento para nova tentativa
...
<Nota>
A flag `--no-verify-jwt` é necessária porque os webhooks não incluem tokens JWT. A segurança é fornecida pela verificação de assinatura do webhook.
</Nota>
...
Ou no [Painel do Supabase](https://supabase.com/dashboard) → Edge Functions → webhook → guia Logs

**Configure no Painel do DodoPayments:**
1. Vá para Desenvolvedores → Webhooks
2. Adicione um endpoint com sua URL do Supabase
...
| Erro de permissão no banco de dados | Certifique-se de usar a Chave de Função de Serviço |
| Erro de verificação JWT | Implemente com a flag `--no-verify-jwt` |
| Função não encontrada | Verifique se a referência do projeto está correta e se a função está implantada |

## Recursos

- [Documentação das Supabase Edge Functions](https://supabase.com/docs/guides/functions)
- [Supabase CLI](https://supabase.com/docs/reference/cli)
- [Deno Runtime](https://deno.land/)
- [Guia de Eventos de Webhook](/developer-resources/webhooks/intents/webhook-events-guide)
- [Repositório do GitHub](https://github.com/dodopayments/cloud-functions/tree/main/supabase)
