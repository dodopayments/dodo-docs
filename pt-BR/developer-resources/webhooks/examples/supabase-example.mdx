...
> **Nota:** Vamos definir `DODO_PAYMENTS_WEBHOOK_KEY` após a implantação, uma vez que você tenha sua URL de webhook.
...
1. Vá para o [Painel do DodoPayments](https://app.dodopayments.com) → Desenvolvedor → Webhooks
2. Crie um novo endpoint de webhook
3. Configure sua URL de webhook como o endpoint
4. Habilite esses eventos de assinatura:
   - `subscription.active`
...
✅ **Registro de eventos** - Rastro de auditoria completo na tabela `webhook_events`  
✅ **Tratamento de erros** - Registrado e recuperável

> **Nota:** Esta implementação demonstra o tratamento de três eventos de assinatura principais (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) com campos mínimos. Você pode facilmente estendê-la para suportar tipos de eventos e campos adicionais com base em suas necessidades.
...
2. **Verifica a idempotência** - Procura o ID do webhook para evitar processamento duplicado
3. **Registra o evento** - Armazena os dados brutos do webhook na tabela `webhook_events`
4. **Processa atualizações** - Cria ou atualiza clientes e assinaturas via cliente Supabase
5. **Lida com erros** - Registra falhas e marca o evento para reexecução
...
<Nota>
A flag `--no-verify-jwt` é necessária porque os webhooks não incluem tokens JWT. A segurança é fornecida pela verificação da assinatura do webhook.
</Nota>
...
| Erro de permissão do banco de dados | Certifique-se de usar a Chave de Função de Serviço |
