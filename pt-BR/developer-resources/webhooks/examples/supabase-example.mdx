...
<Tip>
  ...
  Um kit inicial mínimo de assinaturas construído com Next.js, Supabase e Dodo Payments. Este boilerplate ajuda você a configurar rapidamente um SaaS baseado em assinaturas com autenticação, pagamentos e webhooks.
...
> **Nota:** Vamos definir `DODO_PAYMENTS_WEBHOOK_KEY` após o deploy assim que você tiver sua URL de webhook.

### 5. Implantar

A função já está configurada em `functions/webhook/index.ts` - é só implantá-la:
...
1. Vá para [Painel do DodoPayments](https://app.dodopayments.com) → Developer → Webhooks
2. Crie um novo endpoint de webhook
3. Configure a URL do webhook como o endpoint
4. Habilite estes eventos de assinatura:
   - `subscription.active`
...
> **Nota:** Esta implementação demonstra o tratamento de três eventos principais de assinatura (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) com campos mínimos. Você pode estendê-la facilmente para suportar tipos de eventos adicionais e campos conforme suas necessidades.
...
2. **Confere idempotência** - Consulta o ID do webhook para evitar processamentos duplicados
...
5. **Trata erros** - Registra falhas e marca o evento para nova tentativa
...
A flag `--no-verify-jwt` é necessária porque webhooks não incluem tokens JWT. A segurança é fornecida pela verificação da assinatura do webhook.
...
| Problema | Solução |
|-------|----------|
| Falha na verificação | Verifique se a chave de webhook está correta no painel do DodoPayments |
| Erro de permissão no banco de dados | Certifique-se de usar a Chave de Função de Serviço |
| Erro de verificação de JWT | Faça o deploy com a flag `--no-verify-jwt` |
| Função não encontrada | Verifique se o ref do projeto está correto e se a função foi implantada |
