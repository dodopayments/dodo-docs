...
> **Nota:** Vamos definir `DODO_PAYMENTS_WEBHOOK_KEY` após a implantação, assim que você tiver sua URL do webhook.
...
1. Vá para o [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
...
✅ **Registro de eventos** - Registro completo no banco de dados `webhook_events`  
✅ **Tratamento de erros** - Registrado e recuperável

> **Nota:** Esta implementação demonstra o manuseio de três eventos de assinatura essenciais (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) com campos mínimos. Você pode facilmente estendê-la para suportar tipos de eventos e campos adicionais com base em suas necessidades.
...
2. **Verifica a assinatura** - Garante que a solicitação seja do DodoPayments usando HMAC-SHA256
3. **Verifica duplicados** - Usa o ID do webhook para evitar o processamento do mesmo evento duas vezes
...
6. **Tratamento de erros** - Registra falhas e marca o evento como não processado para reprocessamento
