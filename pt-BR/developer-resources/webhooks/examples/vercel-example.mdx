...
<Card title="Repositório do GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/vercel">
...
✅ **Idempotência** - Previne o processamento duplicado com IDs de webhook  
✅ **Registro de eventos** - Rastro de auditoria completo na tabela `webhook_events`  
✅ **Tratamento de erros** - Registrado e reprocessável
...
1. **Desabilita o parsing do corpo** - Para acessar o corpo bruto para verificação de assinatura
2. **Verifica a assinatura** - Garante que a solicitação é do DodoPayments usando HMAC-SHA256
3. **Verifica duplicatas** - Usa o ID do webhook para evitar o processamento do mesmo evento duas vezes
4. **Registra o evento** - Armazena o webhook bruto na tabela `webhook_events` para rastreamento de auditoria
5. **Processa o evento** - Cria ou atualiza clientes e assinaturas no Neon
6. **Lida com erros** - Registra falhas e marca o evento como não processado para reprocessamento
