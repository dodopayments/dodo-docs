...
> **Nota:** Definiremos `DODO_PAYMENTS_WEBHOOK_KEY` após a implantação, uma vez que você tenha sua URL de webhook.

### 5. Atualizar wrangler.toml

Edite `wrangler.toml` e defina o nome do seu trabalhador:
...
> **Nota:** Esta implementação demonstra o manejo de três eventos de assinatura principais (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) com campos mínimos. Você pode facilmente estendê-la para suportar tipos de eventos e campos adicionais com base em suas necessidades.
...
1. **Verifica a assinatura** - Garante que a solicitação seja do DodoPayments usando HMAC-SHA256
...
5. **Lida com erros** - Registra falhas e marca o evento como não processado para tentativas futuras
...
| Problema | Solução |
|-------|----------|
| Verificação falhou | Verifique se a chave do webhook está correta no dashboard do DodoPayments |
| Erro de conexão com o banco de dados | Verifique a string de conexão do Neon |
| Trabalhador não implantando | Execute `npm install`, verifique a sintaxe de `wrangler.toml` |
