...
6. Obtenha sua string de conexão do Neon → Connection Details
...
> **Observação:** Configuraremos `DODO_PAYMENTS_WEBHOOK_KEY` após a implantação assim que você tiver a URL do webhook.
...
✅ **Verificação da assinatura** - Usando a biblioteca dodopayments  
✅ **Idempotência** - Evita processamentos duplicados com IDs de webhook  
✅ **Registro de eventos** - Trilhas completas na tabela `webhook_events`  
✅ **Tratamento de erros** - Registrados e passíveis de nova tentativa

> **Observação:** Esta implementação demonstra o tratamento de três eventos principais de assinatura (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) com campos mínimos. Você pode facilmente estendê-la para suportar tipos de eventos adicionais e campos conforme suas necessidades.
...
O manipulador de webhooks:
1. **Verifica a assinatura** - Garante que a requisição é da DodoPayments usando HMAC-SHA256
...
**Visualize os logs no Painel do Netlify:**
1. Selecione seu site → aba **Functions**
2. Clique na função `webhook`
3. Veja logs em tempo real e o histórico de invocações
...
| Erro na conexão com o banco | Verifique a string de conexão do Neon e use conexão com pool |
