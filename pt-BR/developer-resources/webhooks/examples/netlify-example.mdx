...
> **Nota:** Definiremos `DODO_PAYMENTS_WEBHOOK_KEY` após a implementação, uma vez que você tenha sua URL de webhook.
...
1. Vá para [DodoPayments Dashboard](https://app.dodopayments.com) → Desenvolvedor → Webhooks  
2. Crie um novo endpoint de webhook  
3. Configure sua URL de webhook como o endpoint  
4. Habilite os seguintes eventos de assinatura:  
   - `subscription.active`  
   - `subscription.cancelled`  
   - `subscription.renewed`  
5. Copie o **Segredo da Assinatura**
...
✅ **Tratamento de erros** - Registrados e recuperáveis

> **Nota:** Esta implementação demonstra o tratamento de três eventos de assinatura principais (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) com campos mínimos. Você pode facilmente expandi-la para suportar tipos e campos de eventos adicionais com base em suas necessidades.
...
3. **Registra o evento** - Armazena o webhook bruto na tabela `webhook_events` para auditoria  
4. **Processa o evento** - Cria ou atualiza clientes e assinaturas no Neon  
5. **Trata erros** - Registra falhas e marca o evento como não processado para recuperação
...
1. Selecione seu site → guia **Funções**  
...
| Erro de conexão com o banco de dados | Verifique a string de conexão do Neon e use a conexão em pool |  
| Função não encontrada (404) | Execute `netlify init` e `npm run deploy` novamente |  
| Variáveis de ambiente não disponíveis | Defina as variáveis no CLI ou painel e, em seguida, reimplante |
