---
title: Funções do Netlify
description: Implante webhooks do DodoPayments nas Funções do Netlify
icon: diamond
---

<Card title="Repositório do GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/netlify">
...
### 1. Pré-requisitos
- [Conta no Netlify](https://app.netlify.com/signup)
- Conta no [Neon database](https://neon.com)
...
1. **Verifica a assinatura** - Garante que a solicitação é do DodoPayments usando HMAC-SHA256
2. **Verifica duplicatas** - Usa o ID do webhook para evitar o processamento do mesmo evento duas vezes
3. **Registra o evento** - Armazena o webhook bruto na tabela `webhook_events` para rastreamento de auditoria
4. **Processa o evento** - Cria ou atualiza clientes e assinaturas no Neon
5. **Lida com erros** - Registra falhas e marca o evento como não processado para reprocessamento
...
**Visualizar logs no Painel do Netlify:**
1. Selecione seu site → aba **Funções**
2. Clique na função `webhook`
3. Visualize logs em tempo real e histórico de invocações

**Configure no Painel do DodoPayments:**
1. Vá para Desenvolvedores → Webhooks
2. Adicione o endpoint com sua URL das Funções do Netlify
...
| Função não encontrada (404) | Execute `netlify init` e `npm run deploy` novamente |
| Variáveis de ambiente não disponíveis | Defina variáveis na CLI ou no painel, e então reimplante |

## Recursos

- [Documentação das Funções do Netlify](https://docs.netlify.com/functions/overview/)
...
- [Repositório do GitHub](https://github.com/dodopayments/cloud-functions/tree/main/netlify)
