...
description: Compreenda os diferentes eventos de webhook acionados pelo Dodo
  Payments durante atividades de pagamento, reembolso, assinatura, disputa,
  chave de licença, recuperação e cobrança
icon: bell
keywords:
  - Dodo Payments
  - eventos de webhook
...
| **Nome do Evento**                | **Tipo de Evento**                 | **Descrição**                                                                 |
|--------------------------|---------------------------------|---------------------------------------------------------------------------------|
| **Checkout Abandonado Detectado** | `abandoned_checkout.detected` | Acionado quando um checkout incompleto ou falho é detectado após 60 minutos.  |
| **Checkout Abandonado Recuperado** | `abandoned_checkout.recovered` | Ocorre quando um cliente conclui o pagamento através de um link de recuperação.              |
...
| **Nome do Evento**           | **Tipo de Evento**           | **Descrição**                                                                 |
|--------------------------|--------------------------|---------------------------------------------------------------------------------|
| **Cobrança Iniciada**      | `dunning.started`        | Acionado quando uma tentativa de cobrança começa para uma assinatura que foi pausada ou cancelada. |
| **Cobrança Recuperada**    | `dunning.recovered`      | Ocorre quando um cliente atualiza seu método de pagamento e a cobrança resultante é bem-sucedida. |

Para esquemas de payload detalhados, descrições de campos e exemplos de manipuladores, consulte [Webhooks de Recuperação](/developer-resources/webhooks/intents/recovery).

Esses eventos de webhook foram projetados para fornecer aos comerciantes atualizações críticas para gerenciar suas operações de forma eficaz. Implemente manipuladores robustos para cada evento relevante na sua integração de webhook para garantir uma funcionalidade suave e confiável.
