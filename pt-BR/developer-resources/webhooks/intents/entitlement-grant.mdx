...
| `revocation_reason` | Gatilho |
|---|---|
| `subscription_cancelled` | A assinatura do cliente foi cancelada (evento `subscription.cancelled`). |
| `subscription_on_hold` | A assinatura está suspensa devido a uma renovação malsucedida (`subscription.on_hold`). Recuperável: uma nova tentativa bem-sucedida gera uma nova concessão. |
| `SubscriptionPaused` | A assinatura foi pausada (`subscription.paused`). Recuperável: retomar a assinatura gera uma nova concessão. |
| `subscription_expired` | A assinatura chegou ao fim do prazo (`subscription.expired`). |
| `plan_changed` | O plano foi alterado; as concessões antigas são revogadas antes que novas sejam emitidas (`subscription.plan_changed`). |
| `refund` | Um reembolso foi processado para o pagamento único original (`refund.succeeded`). |
| `manual` | Um merchant revogou a concessão por meio da API ou do dashboard. Revogações manuais não geram concessões automaticamente novamente na renovação da assinatura. |
| `license_key_disabled` | A chave de licença associada a uma concessão de chave de licença foi desativada. A concessão é reativada automaticamente se a chave for reativada. |
| `platform_external` | O lado da plataforma de uma integração ficou fora de sincronia (por exemplo, uma função do Discord foi removida manualmente, o GitHub App perdeu acesso ao repositório ou uma etapa de reconciliação detectou um destino ausente). A concessão não é gerada automaticamente novamente na renovação da assinatura até que o problema subjacente na plataforma seja resolvido. |
