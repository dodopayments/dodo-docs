...
- **Aguarde `entitlement_grant.delivered` antes de desbloquear recursos dependentes.** Um evento `payment.succeeded` informa que o pagamento foi compensado; ele não informa que o cliente já recebeu o repositório do GitHub ou a função do Discord. O evento `delivered` é a fonte de verdade para o fulfillment.
- **Mapeie `revocation_reason` para os fluxos de retenção.** Um revoke de `subscription_on_hold` geralmente significa que o cartão do cliente falhou e que a próxima renovação concederá o acesso novamente. Um revoke de `manual` ou `subscription_cancelled` é intencional. Trate-os de forma diferente nas mensagens ao cliente.
- **Use o grant `id` como sua chave de idempotência.** Um único grant emite no máximo um evento `created` e no máximo um evento terminal (`delivered` ou `failed`), além de no máximo um evento `revoked`. As reentregas do sistema de webhook podem repetir eventos; elimine duplicidades com base em `id` mais `type`.
- **Leia `integration_type` para identificar o tipo de grant.** O payload contém `integration_type` diretamente (por exemplo, `license_key`, `digital_files`, `discord`). Os objetos aninhados `license_key` e `digital_product_delivery` são preenchidos quando seus respectivos grants são entregues; um grant de chave de licença cumprido manualmente permanece como `pending` com `integration_type: "license_key"` e um `null` `license_key` até que você o cumpra.
- **Para grants baseados em OAuth, mostre `oauth_url` ao cliente.** O evento `entitlement_grant.created` para fluxos de assinantes do Discord, GitHub ou Notion inclui `oauth_url` e `oauth_expires_at`. Envie-o por e-mail ao cliente ou exiba-o no seu app para desbloquear a entrega.
