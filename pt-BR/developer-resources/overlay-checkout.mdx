...
O Apple Pay permite que os clientes concluam pagamentos de forma rápida e segura usando seus métodos de pagamento salvos. Quando habilitado, os clientes podem abrir o modal do Apple Pay diretamente da sobreposição do checkout em dispositivos suportados.
...
Carregue o arquivo em seu servidor web em `/.well-known/apple-developer-merchantid-domain-association`. Por exemplo, se seu site for `example.com`, disponibilize o arquivo em `https://example.com/well-known/apple-developer-merchantid-domain-association`.
...
- A URL do seu domínio de produção (por exemplo, `https://example.com`)
