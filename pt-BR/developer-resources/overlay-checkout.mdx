...
<Info>
Esta seção aborda a configuração de tema **no lado do cliente** usando o Checkout SDK. Você também pode configurar temas **no lado do servidor** ao criar uma sessão de checkout pela API usando o parâmetro `theme_config`. Veja [Checkout Theme Customization](/features/checkout#checkout-theme-customization) para configuração em nível de API, ou use a [página Design](/features/design) no painel para configurar temas visualmente com visualização ao vivo.
</Info>
