...
- Certifique-se de que `DODO_PAYMENTS_WEBHOOK_KEY` esteja configurado corretamente em seu `.env`
- Verifique se você está usando o corpo bruto da solicitação para a verificação da assinatura
- Passe todos os três headers, `webhook-id`, `webhook-timestamp` e `webhook-signature`, para `webhook.verify()` — a assinatura do Standard Webhooks cobre `id.timestamp.body`, não apenas o body
</Accordion>
</AccordionGroup>
