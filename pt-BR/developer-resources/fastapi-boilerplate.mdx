...
<Info>
Este boilerplate utiliza FastAPI com padrões async/await, Pydantic para validação e o SDK Python `dodopayments` para integração de API sem problemas.
</Info>
...
| `/webhook` | POST | Manipular webhooks de pagamentos Dodo |
...
**Causas comuns:**
- ID de produto inválido - verifique se existe no seu painel Dodo
- Chave de API errada ou configuração de ambiente em `.env`
...
Atualize a URL do webhook no seu [painel Dodo](https://app.dodopayments.com/developer/webhooks) para a URL do ngrok. Certifique-se de atualizar seu arquivo `.env` com a chave de verificação do webhook correta.
</Accordion>

<Accordion title="Falhas na verificação da assinatura do webhook">
- Certifique-se de que `DODO_PAYMENTS_WEBHOOK_KEY` está corretamente definido em seu `.env`
- Verifique se você está usando o corpo da solicitação bruto para verificação de assinatura
- Verifique se está lendo o cabeçalho `webhook-signature` corretamente
</Accordion>
</AccordionGroup>
