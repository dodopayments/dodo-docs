---
title: Adaptador Tanstack
description: Saiba como integrar <CardGroup cols={2}> ao seu projeto Tanstack
  App Router usando nosso Adaptador Tanstack. Abrange checkout, portal do
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Integre o checkout do Dodo Payments ao seu aplicativo Tanstack.
</Card>
...
<Warning>
Nunca faça commit do arquivo <code>.env</code> nem de secrets no controle de versão.
</Warning>
</Step>
</Steps>
...
<Info>
Todos os exemplos pressupõem que você está usando o Tanstack App Router.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Use este handler para integrar o checkout do Dodo Payments ao seu aplicativo Tanstack. Oferece suporte a fluxos de pagamento estático (GET), dinâmico (POST) e de sessão (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Customer Portal Handler">
  <Info>
  Use este handler para permitir que os clientes gerenciem suas assinaturas e dados pelo portal do cliente do Dodo Payments.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Use este handler para receber e processar eventos de webhook do Dodo Payments com segurança no seu aplicativo Tanstack.
  </Info>
  <CodeGroup>
...
<Info>
Dodo Payments oferece suporte a três tipos de fluxos de pagamento para integrar pagamentos ao seu site; este adaptador oferece suporte a todos os tipos de fluxos de pagamento.
</Info>

- **Links de pagamento estáticos:** URLs compartilháveis instantaneamente para coletar pagamentos de forma rápida e sem código.
- **Links de pagamento dinâmicos:** Gere links de pagamento programaticamente com dados personalizados usando a API ou SDKs.
...
Define o valor cobrado, em unidades principais da moeda (por exemplo, <code>12.5</code> para US$ 12,50). Disponível apenas para produtos Pay What You Want e ignorado se estiver abaixo do preço mínimo do produto.
</ParamField>
...
- Para obter a lista completa de campos compatíveis no corpo da requisição POST, consulte:
...
<Warning>
O Dynamic Checkout atua como proxy para os endpoints `POST /payments` e `POST /subscriptions`, que foram descontinuados. Ele continua funcionando para integrações existentes, mas novas integrações devem usar **Checkout Sessions** abaixo.
</Warning>
...
As Checkout Sessions oferecem uma experiência de checkout hospedada mais segura, que gerencia todo o fluxo de pagamento para compras únicas e assinaturas, com controle total de personalização.
...
## Route Handler do Customer Portal

O Route Handler do Customer Portal permite integrar perfeitamente o portal do cliente do Dodo Payments ao seu aplicativo Tanstack.
