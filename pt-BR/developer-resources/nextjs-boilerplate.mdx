...
<Info>
Esse boilerplate usa o Next.js 16 App Router com TypeScript, Tailwind CSS 4 e o adaptador `@dodopayments/nextjs`.
</Info>
...
- **Portal do Cliente** - Gerenciamento de assinaturas com um clique
...
<Warning>
Nunca commit seu arquivo `.env` no controle de versão. Ele já está incluído em `.gitignore`.
</Warning>
</Step>
...
- IDs de Produto (do seu painel do Dodo)
- Preços
- Recursos
- Descrições
...
Em `src/app/components/ProductCard.tsx`, substitua os valores hardcoded pelos dados reais do seu usuário:
...
Em `src/app/components/Header.tsx`, substitua o ID do cliente hardcoded:
...
<AccordionGroup>
<Accordion title="Módulo não encontrado ou erros de build">
...
<Accordion title="Falha na redirecionamento do checkout">
**Causas comuns:**
...
<Accordion title="Link do portal do cliente não funciona">
Substitua o `CUSTOMER_ID` hardcoded em `src/app/components/Header.tsx` por um ID de cliente real do seu painel do Dodo.
