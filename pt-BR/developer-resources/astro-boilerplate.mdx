...
<Info>
Este boilerplate utiliza Astro 5 com TypeScript, Tailwind CSS 4, e o adaptador `@dodopayments/astro`.
</Info>
...
- **Interface Moderna** - Página de preços limpa e com tema escuro usando Tailwind CSS
...
- **Checkout Pré-preenchido** - Demonstra o envio de dados do cliente para melhorar a experiência do usuário
...
Atualize `src/lib/products.ts` com seus IDs de produtos reais do Dodo Payments:
...
Em `src/components/ProductCard.astro`, substitua os valores hardcoded pelos dados reais do seu usuário:
...
Em `src/components/Header.astro`, substitua o ID do cliente hardcoded pelo ID real do cliente do seu sistema de autenticação ou banco de dados:
...
O boilerplate demonstra o manuseio de eventos de webhook em `src/pages/api/webhook.ts`:
...
Também lembre-se de atualizar a variável de ambiente `DODO_PAYMENTS_WEBHOOK_KEY` em seu ambiente de produção para corresponder à chave de assinatura do webhook para seu domínio implantado.
...
<Accordion title="Falha no redirecionamento de checkout">
**Causas comuns:**
...
Atualize a URL do webhook em seu [painel Dodo](https://app.dodopayments.com/developer/webhooks) para a URL do ngrok. Lembre-se de atualizar seu arquivo .env com a chave de verificação do webhook correta.
</Accordion>

<Accordion title="O link do portal do cliente não funciona">
Substitua o `CUSTOMER_ID` hardcoded em `src/components/Header.astro` com um ID de cliente real do seu painel do Dodo.

Ou integre seu sistema de autenticação e banco de dados para buscar o ID do cliente dinamicamente.
</Accordion>

<Accordion title="Build falha com erro de adapter">
Este boilerplate usa saída estática com rotas de API sob demanda. Você precisa instalar um adapter para implantação:

Veja os [guias de implantação do Astro](https://docs.astro.build/en/guides/deploy/) para detalhes.
</Accordion>
</AccordionGroup>
