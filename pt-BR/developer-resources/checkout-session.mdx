...
Código ISO de país de duas letras (ISO 3166-1 alpha-2). Este campo é sempre necessário quando endereço_de_cobrança é fornecido.
...
Forçar a criação de um novo registro de cliente em vez de atualizar registros existentes.
</ParamField>

<ParamField body="allow_customer_editing_email" type="boolean">
Permitir que os clientes editem seu endereço de email durante o checkout.
</ParamField>
...
Pule a página padrão de sucesso de pagamento e redirecione os clientes imediatamente para o seu `return_url` após a conclusão do pagamento.

<Tip>
Use isso quando você tiver uma página de sucesso personalizada que forneça uma melhor experiência do usuário, ou para aplicativos móveis e fluxos de checkout incorporados.
</Tip>
</ParamField>
</Expandable>
</ParamField>
...
Coletar informações adicionais dos clientes durante o checkout com campos de formulário personalizados. Você pode definir até 5 campos personalizados por sessão de checkout. As respostas dos clientes são incluídas em payloads de webhook e disponíveis através do API.

<Expandable title="Custom field object properties">
<ParamField body="key" type="string" required>
Identificador único para este campo. Usado como a chave em payloads de webhook e respostas de API.
...
Tipo de campo que determina regras de validação de entrada.
...
<Info>
As respostas dos clientes para campos personalizados estão incluídas em:
...
- **Respostas de API**: Objetos de pagamento e assinatura incluem `custom_field_responses`
</Info>
...
Se definido como verdadeiro, não efetua nenhuma cobrança e apenas autoriza os detalhes do método de pagamento para uso futuro.
</ParamField>
...
Moeda opcional do preço do produto. Se não especificado, será adotada a moeda do produto.
</ParamField>

<ParamField body="product_description" type="string">
Descrição opcional do produto para faturamento e itens de linha. Se não especificado, a descrição armazenada do produto será usada.
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
Se as taxas de moeda adaptativas devem ser incluídas no preço do produto (verdadeiro) ou adicionadas separadamente (falso). Ignorado se o preço adaptativo não estiver habilitado.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
Aqui estão 10 exemplos abrangentes mostrando diferentes configurações de sessões de checkout para vários cenários de negócios:
...
<Note>
A sobrescrição `billing_currency` só entra em vigor quando a moeda adaptativa está habilitada nas configurações de sua conta. Se a moeda adaptativa estiver desativada, este parâmetro não terá efeito.
</Note>
...
Para informações detalhadas sobre a configuração e teste do UPI, veja a página <a href="/features/payment-methods/india">Métodos de Pagamento na Índia</a>.
...
### 10. Checkout BNPL (Compra Agora Pague Depois)

Para informações detalhadas sobre a configuração e teste do BNPL, veja a página <a href="/features/payment-methods/bnpl">Compra Agora Pague Depois (BNPL)</a>.
...
<Tip>
Links curtos são perfeitos para SMS, email ou compartilhamento em redes sociais. Eles são mais fáceis de lembrar e geram mais confiança do cliente do que URLs longas.
</Tip>
...
<Tip>
Use `redirect_immediately: true` quando você tiver uma página de sucesso personalizada que forneça melhor experiência do usuário do que a página de sucesso de pagamento padrão. Isso é especialmente útil para aplicativos móveis e fluxos de checkout incorporados.
</Tip>
...
<Tip>
Use `force_language` quando você souber o idioma preferido de seu cliente (por exemplo, a partir das configurações de conta deles) ou ao segmentar mercados regionais específicos.
</Tip>
...
Coletar informações adicionais dos clientes durante o checkout usando campos personalizados:
...
<Info>
As respostas dos campos personalizados são automaticamente incluídas em payloads de webhooks (`payment.succeeded`, `subscription.active`, etc.) e podem ser recuperadas através do API. Use-as para enriquecer seu CRM, acionar fluxos de integração ou personalizar a experiência do cliente.
</Info>
...
Antes de criar uma sessão de checkout, você pode visualizar o detalhamento de preços incluindo impostos, descontos e totais. Isso é útil para exibir preços precisos aos clientes antes de prosseguirem para o checkout.
...
- Se você só souber o país de cobrança do cliente, forneça apenas isso.
- O fluxo de checkout coletará automaticamente os detalhes ausentes antes de mover o cliente para a página de pagamento.
- Por outro lado, se você já tiver todas as informações necessárias e quiser pular diretamente para a página de pagamento, pode passar o conjunto completo de dados e incluir `confirm=true` em seu corpo de solicitação.

### Processo de Migração

Migrar de Links Dinâmicos para Sessões de Checkout é direto:
...
Sim. Nenhum tratamento adicional ou etapas especiais de migração são necessários do seu lado.
</Step>
</Steps>
...
<Card title="Create Checkout Session" icon="code" href="/api-reference/checkout-sessions/create">
  Referência completa de API para criar sessões de checkout com todos os parâmetros e opções disponíveis
</Card>
