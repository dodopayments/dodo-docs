...
Após o pagamento, os clientes são redirecionados para o seu `return_url` com parâmetros de consulta adicionais para o status do pagamento.
</Step>
</Steps>
...
Array de produtos a serem incluídos na sessão de checkout. Cada produto deve ter um `product_id` válido do seu painel de controle do Dodo Payments.
...
<Note>
A substituição de `billing_currency` só tem efeito quando a moeda adaptativa está habilitada nas configurações da sua conta. Se a moeda adaptativa estiver desativada, este parâmetro não terá efeito.
</Note>
...
<Tip>
Use `redirect_immediately: true` quando você tiver uma página de sucesso personalizada que oferece uma melhor experiência ao usuário do que a página de sucesso de pagamento padrão. Isso é especialmente útil para aplicativos móveis e fluxos de checkout incorporados.
</Tip>

<Note>
Quando `redirect_immediately` está habilitado, os clientes são redirecionados para o seu `return_url` imediatamente após a conclusão do pagamento, pulando completamente a página de sucesso padrão.
</Note>
...
- Por outro lado, se você já tiver todas as informações necessárias e quiser pular diretamente para a página de pagamento, você pode passar o conjunto completo de dados e incluir `confirm=true` em seu corpo de solicitação.
