...
Aplique um ou mais códigos de desconto empilhados à sessão de checkout. Os códigos são aplicados **na ordem do array** (o primeiro código reduz o preço inicial, o segundo reduz o preço já com desconto, e assim por diante), até o máximo de **20** códigos por sessão. Quando a [Paridade do Poder de Compra](/features/purchasing-power-parity) está habilitada, o preço inicial é o valor ajustado pela PPP, não o preço base.
...
<Note>
O `current_breakup.subtotal` visualizado já considera a [Paridade do Poder de Compra](/features/purchasing-power-parity) e o [Charm Pricing](/features/charm-pricing), quando aplicáveis ao produto.
</Note>

<Note>
Quando o carrinho contém um produto de assinatura, a resposta de pré-visualização também retorna um `next_billing_date` — uma pré-visualização da próxima data de cobrança, para que você possa exibi-la antes de a assinatura ser criada. Ele é calculado em relação ao momento atual: `now + trial period` quando um período de avaliação se aplica; caso contrário, `now + one payment frequency`. O campo é omitido para carrinhos exclusivamente de compras únicas. Esta é uma estimativa baseada no momento da pré-visualização; o `next_billing_date` oficial é definido quando a assinatura é ativada.
</Note>

<Note>
A pré-visualização também retorna `trial_period_days` (a duração efetiva do período de avaliação, gratuito ou pago) e `trial_amount` (a cobrança de avaliação por unidade após os descontos, nas unidades menores da moeda do preço). `trial_amount` só está presente para uma [avaliação paga](/features/subscription#paid-trials) e é `null` para uma avaliação gratuita ou quando não há avaliação. Use `current_breakup` para o total com impostos efetivamente devido hoje.
</Note>
...
<Card title="Preview API Reference" icon="code" href="/api-reference/checkout-sessions/preview">
  Consulte a documentação completa do endpoint de pré-visualização.
</Card>

## Migração de Dynamic Links para Checkout Sessions
...
Com Checkout Sessions, isso não é mais necessário. Basta enviar as informações que você tiver, e cuidaremos do restante. Por exemplo:

- Se você souber apenas o país de cobrança do cliente, forneça somente essa informação.
- O fluxo de checkout coletará automaticamente os dados ausentes antes de encaminhar o cliente à página de pagamento.
- Por outro lado, se você já tiver todas as informações necessárias e quiser ir diretamente para a página de pagamento, poderá enviar o conjunto completo de dados e incluir `confirm=true` no corpo da sua solicitação.
