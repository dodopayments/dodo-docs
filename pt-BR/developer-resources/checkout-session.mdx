...
<Info>
As respostas de campos personalizados são automaticamente incluídas nos payloads de webhook (`payment.succeeded`, `subscription.active` etc.) e podem ser recuperadas via API. Use-as para enriquecer seu CRM, acionar fluxos de integração ou personalizar a experiência do cliente.
</Info>
