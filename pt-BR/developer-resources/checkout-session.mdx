...
Array de produtos a serem incluídos na sessão de checkout. Cada produto deve ter um válido `product_id` do seu painel do Dodo Payments.
...
- **Código ZIP/Postagem**: Apenas em regiões onde é necessário para cálculo de imposto sobre vendas, VAT ou GST

Isso reduz significativamente a fricção do checkout, eliminando campos de formulário desnecessários.

<Tip>
Habilite o endereço mínimo para uma conclusão de checkout mais rápida. A coleta completa de endereço permanece disponível para empresas que exigem detalhes de cobrança completos.
</Tip>
</ParamField>

</Accordion>

<Accordion title="Personalização da UI e Recursos">
...
Permitir que os clientes editem o endereço durante o checkout.
</ParamField>
...
<Note>
Quando `confirm` é definido como `true`, o cliente será levado diretamente para a página de checkout, pulando quaisquer etapas de confirmação.
</Note>
...
<Note>
A substituição de `billing_currency` só tem efeito quando a moeda adaptativa está habilitada nas configurações da sua conta. Se a moeda adaptativa estiver desabilitada, este parâmetro não terá efeito.
</Note>
...
Use um método de pagamento salvo de um cliente para criar uma sessão de checkout que processa imediatamente, pulando a coleta do método de pagamento:
...
<Info>
O método de pagamento deve pertencer ao cliente e ser compatível com a moeda do pagamento. Isso possibilita compras com um clique para clientes retornantes.
</Info>
...
<Tip>
Use `redirect_immediately: true` quando você tiver uma página de sucesso personalizada que oferece uma melhor experiência do usuário do que a página de sucesso padrão do pagamento. Isso é especialmente útil para aplicativos móveis e fluxos de checkout incorporados.
</Tip>
...
Antes de criar uma sessão de checkout, você pode pré-visualizar a divisão de preços, incluindo impostos, descontos e totais. Isso é útil para exibir preços precisos aos clientes antes que eles prossigam para o checkout.
...
<Step title="Ajuste a carga útil da solicitação">
Ajuste a carga útil da solicitação de acordo com o formato das Sessões de Checkout.
</Step>
...
<Step title="Ajustar a carga útil da solicitação">
Ajuste a carga útil da solicitação de acordo com o formato das Sessões de Checkout.
</Step>

<Step title="É isso aí!">
Sim. Nenhum manuseio adicional ou etapas especiais de migração são necessárias do seu lado.
</Step>
</Steps>
