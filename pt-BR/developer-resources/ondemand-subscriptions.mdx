...
Se as tentativas continuarem falhando porque o método de pagamento em si está com problema (cartão expirado, conta fechada, etc.), use [`POST /subscriptions/{subscription_id}/update-payment-method`](/api-reference/subscriptions/update-payment-method) para coletar um novo do cliente. Com sucesso, a assinatura retorna para `active` e `payment.succeeded` seguido por `subscription.active` webhooks são emitidos.
</Step>
</Steps>
...
Use sua chave de API de teste para criar a assinatura, depois abra o `checkout_url` retornado e complete o mandato.
</Step>
...
<Step title="Go live">
Alterne para sua chave de API ao vivo assim que tiver validado eventos e atualizações de estado interno.
</Step>
</Steps>
