...
<Note>
Para uma lista abrangente de motivos de recusa e se eles podem ser corrigidos pelo usuário, consulte a documentação de
[Falhas de Transação](/api-reference/transaction-failures).
</Note>

<Tip>
Tente novamente apenas em problemas leves/temporários (por exemplo, `insufficient_funds`, `issuer_unavailable`, `processing_error`, timeouts de rede). Se a mesma recusa se repetir, pause novas tentativas.
</Tip>

### Diretrizes de implementação (sem código)

- Use um agendador/fila que persista carimbos de tempo precisos; calcule a próxima tentativa no exato deslocamento de horário (por exemplo, `T + 3 days` no mesmo HH:MM).
- Mantenha e faça referência ao carimbo de tempo do último pagamento bem-sucedido `T` para calcular a próxima tentativa; não agrupe várias assinaturas ao mesmo tempo.
- Sempre avalie o último motivo de recusa; pare as tentativas para recusas definitivas na lista de exceções acima.
- Limite tentativas concorrentes por cliente e por conta para evitar aumentos acidentais.
...
- Use metadados apenas para observabilidade (por exemplo, `retry_attempt`); nunca tente "evitar" sistemas de fraude/risco rotacionando campos inconsequentes.

## Acompanhe resultados com webhooks

Implemente o gerenciamento de webhooks para acompanhar a jornada do cliente. Veja [Implementar Webhooks](/developer-resources/integration-guide#implementing-webhooks).

- **subscription.active**: Mandato autorizado e assinatura ativada
- **subscription.failed**: Criação falhou (por exemplo, falha no mandato)
- **subscription.on_hold**: Assinatura em espera (por exemplo, estado não pago)
- **payment.succeeded**: Cobrança bem-sucedida
- **payment.failed**: Cobrança falhou

<Tip>
Para fluxos sob demanda, concentre-se em `payment.succeeded` e `payment.failed` para reconciliar cobranças baseadas em uso. Quando `payment.failed` é seguido por `subscription.on_hold`, consulte [Gerenciamento de cobranças falhas](#handling-failed-charges) para recuperar a assinatura.
</Tip>
...
<Step title="Trigger a charge">
Chame o endpoint de cobrança com uma pequena `product_price` (por exemplo, `100`) e verifique se você recebe `payment.succeeded`.
</Step>
<Step title="Go live">
Altere para sua chave de API ao vivo uma vez que você tenha validado eventos e atualizações de estado interno.
</Step>
</Steps>
...
- **Erros de moeda**: Se você substituir `product_currency`, confirme que é suportado para sua conta e cliente.
- **Nenhum webhook recebido**: Verifique sua URL de webhook e a configuração do segredo de assinatura.
