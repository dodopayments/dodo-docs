...
As assinaturas sob demanda permitem que você autorize o método de pagamento de um cliente uma vez e, em seguida, cobre valores variáveis sempre que precisar, em vez de em um cronograma fixo.

<Info>
Esse recurso pode precisar ser ativado na sua conta. Entre em contato com o suporte se você não o vê no seu painel.
</Info>
...
Por favor, encontre-os em [Criar Sessão de Checkout](/api-reference/checkout-sessions/create)
...
<Warning>
Padrões de tentativas em massa podem ser sinalizados como fraudulentos ou suspeitos de teste de cartão pelos nossos sistemas de risco e processadores. Evite tentativas agrupadas; siga o cronograma de retrocesso e as orientações de alinhamento de tempo abaixo.
</Warning>
...
- Exemplo: Se o cliente iniciar um teste ou mandato às 13h10 de hoje, agende tentativas de acompanhamento às 13h10 nos dias subsequentes conforme seu retrocesso (por exemplo, +3 dias → 13h10, +7 dias → 13h10).
...
<Note>
Para uma lista abrangente de razões de recusa e se elas são corrigíveis pelo usuário, consulte a
[Falhas de Transação](/api-reference/transaction-failures) documentação.
</Note>
...
- Use metadados apenas para observabilidade (por exemplo, `retry_attempt`); nunca tente "evadir" sistemas de fraude/risco girando campos irrelevantes.

## Acompanhe os resultados com webhooks

Implemente o manuseio de webhooks para acompanhar a jornada do cliente. Veja [Implementando Webhooks](/developer-resources/integration-guide#implementing-webhooks).
...
- **Erros de moeda**: Se você sobrescrever `product_currency`, confirme que é suportado para sua conta e cliente.
