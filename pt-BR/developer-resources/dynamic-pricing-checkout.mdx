...
2. **Passe valores dinâmicos**: Inclua um campo `amount` no carrinho do produto ao criar sessões de checkout
...
<Info>
Quando você passa um `amount` no carrinho do produto, esse valor é usado para o checkout. Se você omitir o campo `amount`, os clientes podem selecionar seu próprio preço durante o checkout (sujeito às configurações de mínimo/máximo).
</Info>
...
Clique em **Adicionar Produto** para salvar. Anote seu ID do produto (por exemplo, `pdt_123abc456def`) para uso nas sessões de checkout.
</Step>
</Steps>
...
Uma vez que seu produto está configurado com O Que Você Quiser Pagar, você pode criar sessões de checkout com valores dinâmicos. O campo `amount` no carrinho do produto permite que você defina o preço programaticamente para cada sessão de checkout.
...
- **Se `amount` for fornecido**: O checkout usa exatamente esse valor (deve estar dentro dos seus limites mínimo/máximo)
- **Se `amount` for omitido**: Os clientes podem inserir seu próprio preço durante o checkout (dentro dos seus limites)

<Warning>
O campo `amount` é aplicável apenas para produtos O Que Você Quiser Pagar. Para produtos regulares, esse campo é ignorado.
</Warning>
...
<Note>
**Formato do Valor**: O campo `amount` deve estar na menor denominação da moeda. Para a moeda brasileira, isso significa centavos (por exemplo, R$25,00 = `2500`). Para outras moedas, use a menor unidade (por exemplo, paise para INR).
</Note>
...
<Card title="Acompanhe Decisões de Preços" icon="chart-line">
...
<AccordionGroup>
<Accordion title="Valor está sendo ignorado">
Se seu campo `amount` está sendo ignorado, verifique se:
- O produto tem **O Que Você Quiser Pagar** habilitado no painel
- O produto é um produto de **Pagamento Único** (um único pagamento), não uma assinatura
- O valor está no formato correto (menor denominação da moeda, por exemplo, centavos para BRL)
</Accordion>

<Accordion title="Valor excede o máximo ou está abaixo do mínimo">
A API rejeitará sessões de checkout onde o valor viola os limites de preço do seu produto. Sempre valide os valores antes de criar sessões de checkout, ou permita que os clientes escolham seu preço omitindo o campo `amount`.
</Accordion>

<Accordion title="Cliente não pode inserir seu próprio preço">
Se os clientes não estão vendo o campo de entrada de preço, certifique-se de que você omitiu o campo `amount` do carrinho do produto. Quando `amount` é fornecido, o checkout usa exatamente esse valor.
</Accordion>
</AccordionGroup>