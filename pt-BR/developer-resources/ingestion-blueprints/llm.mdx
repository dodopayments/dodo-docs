...
1. **Faça login** no [Dodo Payments Dashboard](https://app.dodopayments.com/)
2. **Navegue até** Produtos → Medidores
3. **Clique** em "Criar Medidor"
4. **Configure seu medidor**:
   - **Nome do Medidor**: Escolha um nome descritivo (ex: "Uso de Token LLM")
   ...
   - **Sobre Propriedade**: Escolha o que acompanhar:
     - `inputTokens` - Acompanhar tokens de entrada/prompt
     - `outputTokens` - Acompanhar tokens de saída/completamento (inclui tokens de raciocínio quando aplicável)
     - `totalTokens` - Acompanhar tokens combinados de entrada + saída
...
<Note>
Ao usar modelos com capacidade de raciocínio através do AI SDK (como o Gemini 2.5 Flash com modo de pensamento), os tokens de raciocínio são automaticamente incluídos na contagem de `outputTokens` para cobrança precisa.
</Note>
</Accordion>
...
- `outputTokens` - Número de tokens de saída/completamento utilizados (inclui tokens de raciocínio quando aplicável)
...
<Note>
**Tokens de Raciocínio:** Para modelos com capacidades de raciocínio, `outputTokens` inclui automaticamente tanto os tokens de completamento quanto os tokens de raciocínio.
</Note>
</ParamField>
