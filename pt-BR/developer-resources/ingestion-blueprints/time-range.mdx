...
Crie um medidor no seu [Painel de Controle do Dodo Payments](https://app.dodopayments.com/):
- **Nome do Evento**: `time_range_usage` (ou seu nome preferido)
- **Tipo de Agregação**: `sum` para rastrear a duração total
- **Sobre Propriedade**: `durationSeconds`, `durationMinutes`, ou `durationMs`
</Step>
...
<Warning>
**Cronometragem Precisa**: Use `Date.now()` ou `performance.now()` para rastreamento de tempo preciso, especialmente para funções serverless.
</Warning>
