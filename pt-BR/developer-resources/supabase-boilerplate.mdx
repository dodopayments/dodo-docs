...
Defina a URI de redirecionamento para: `https://[your-project-ref].supabase.co/auth/v1/callback` no Google Cloud, e então ative o provedor Google no Supabase Auth usando seu Client ID e Secret.
</Step>

<Step title="Configurar Dodo Payments">
Gere uma chave de API e um segredo de Webhook no painel do Dodo. Defina o ambiente para `test_mode` durante o desenvolvimento.
</Step>
...
<AccordionGroup>
<Accordion title="Assinatura de webhook inválida (401)">
- Certifique-se de que `DODO_WEBHOOK_SECRET` corresponda ao valor do painel do Dodo
...
- Certifique-se de que seja o mesmo no Google Cloud e no provedor de autenticação do Supabase
</Accordion>
</AccordionGroup>
