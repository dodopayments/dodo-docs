...
<Step title="Acessar Configurações do Webhook">
    Navegue até o Painel do DodoPayments e vá para `Settings > Webhooks`.
</Step>
...
<Step title="Obter Chave Secreta">
    Obtenha sua chave secreta do webhook `Secret Key` na página de configurações. Você usará isso para verificar a autenticidade dos webhooks recebidos.
    
    <Warning>
    Mantenha sua chave secreta do webhook segura e nunca a exponha em código do lado do cliente ou repositórios públicos.
    </Warning>
</Step>
...
<Step title="Navegar para Detalhes do Webhook">
    Vá para o seu Painel do Dodo Payments e navegue até `Settings > Webhooks`.
</Step>
...
<Tip>
Processe webhooks de forma assíncrona reconhecendo o recebimento imediatamente com um código de status `200`, e depois trate o processamento real em segundo plano.
</Tip>
...
Cada solicitação de webhook inclui um cabeçalho {/* INLINE_CODE_PLACEHOLDER_47db72705c744912eeda683b13e3c948 */, uma assinatura HMAC SHA256 do payload do webhook e timestamp, assinada com sua chave secreta.
...
<Tip>
  Forneça sua chave secreta do webhook via `DODO_PAYMENTS_WEBHOOK_KEY` ao inicializar o cliente do Dodo Payments.
</Tip>
...
1. Construa a mensagem assinada concatenando `webhook-id`, `webhook-timestamp`, e a string exata do payload {/* INLINE_CODE_PLACEHOLDER_63d7f7ecaa944ca147c58ddfdab12252 */, separadas por pontos (`.`).
2. Calcule o HMAC SHA256 dessa string usando sua chave secreta do webhook do Painel.
3. Compare a assinatura calculada com o cabeçalho `webhook-signature`. Se coincidirem, o webhook é autêntico.
...
* Qualquer outra resposta será tratada como uma falha, e o webhook será refeito.
...
<Step title="Selecionar Tipo de Evento">
    Use o menu suspenso para selecionar o tipo de evento específico que você deseja testar (por exemplo, `payment.succeeded`, `payment.failed`, etc.).
