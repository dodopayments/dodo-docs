...
<Warning>
O nome do evento `ai_chat_usage` deve corresponder exatamente ao que enviaremos do nosso código de aplicação mais tarde. Os nomes dos eventos diferenciam maiúsculas de minúsculas!
</Warning>
</Step>
...
**Unidade de Medição**: Tipo → `tokens`
...
<Steps>
<Step title="Obter Chave da API do Dodo Payments">
...
<Note>
Salve esta chave da API - nós a adicionaremos ao nosso arquivo `.env` depois.
</Note>
</Step>
...
<Note>
Mantenha esta chave em segurança - também a adicionaremos ao nosso arquivo `.env`.
</Note>
</Step>
</Steps>
...
**Preço Por Unidade**: Insira → `0.00001` (isto é $0.01 por 1.000 tokens ou $0.00001 por token)

**Limite Gratuito**: Insira → `10000` (clientes ganham 10.000 tokens grátis por mês)
...
1. Verifique se seu arquivo `.env` tem todas as chaves da API da Etapa 2
...
3. Você deve ver a linda interface de chat

<Note>
Certifique-se de atualizar a `CUSTOMER_ID` em `server.ts` com seu ID de cliente de teste real da Etapa 4.
</Note>
</Step>
...
<Info>
O medidor está somando todos os valores `totalTokens` automaticamente!
</Info>
</Step>
...
**Soluções:**
1. Verifique se o nome do evento corresponde exatamente à configuração do medidor (diferencia maiúsculas de minúsculas: `ai_chat_usage`)
2. Verifique se o ID do cliente existe no painel do Dodo Payments
3. Teste a chave da API com uma simples chamada da API
...
2. Atualize para o SDK mais recente do Blueprints: `npm install @dodopayments/ingestion-blueprints@latest`
</Accordion>

<Accordion title="Erro de chave API inválida">
**Possíveis causas:**
- Chave API errada para o ambiente
- Espaços ou aspas extras no arquivo `.env`

**Soluções:**
- Verifique se a chave de teste começa com `test_`, a chave ao vivo começa com `live_`
