...
description: Este guia mostra como integrar os Pagamentos Dodo em seu fluxo de
  autenticação usando o adaptador Better-Auth.
icon: code-merge
---

# Visão Geral

O <b>Adaptador Better Auth</b> para Pagamentos Dodo fornece:
...
- Projeto existente usando [better-auth](https://www.npmjs.com/package/better-auth)
...
Adicione estes ao seu <code>.env</code> file:
...
Crie ou atualize <code>src/lib/auth.ts</code>:
...
<Tip>
Defina <code>environment</code> como <code>live_mode</code> para produção.
</Tip>
</Step>

{/* LOCKED_PATTERN_673a79a3fa2d75e23b52c6942c035dd2 */}
Crie ou atualize <code>src/lib/auth-client.ts</code>:
...
<Info>
  Prefira <code>authClient.dodopayments.checkoutSession</code> para novas
  integrações. O método legado <code>checkout</code> está obsoleto e mantido
...
<Info>
  Ao contrário do método legado <code>checkout</code>, o <code>checkoutSession</code>
  não exige informações de faturamento antecipadamente, pois elas serão obtidas
  do usuário na página de checkout. Ainda assim, você pode substituí-las ao
  passar a chave <code>billing</code> no argumento.
</Info>

<Info>
  Semelhante ao método legado <code>checkout</code>, o <code>checkoutSession</code>
  obtém o e-mail e o nome do cliente da sessão do better-auth; no entanto, você
  pode sobrescrevê-los passando o objeto <code>customer</code> com e-mail e nome.
</Info>

<Info>
  Você pode passar as mesmas opções no argumento que no corpo da requisição para o endpoint [Criar Sessão de Checkout](https://docs.dodopayments.com/api-reference/checkout-sessions/create).
</Info>

<Note>
  A URL de retorno é obtida do <code>successUrl</code> configurado no plugin de
  servidor. Você não precisa incluir <code>return_url</code> no payload do cliente.
</Note>

## Checkout Legado (Obsoleto)

<Warning>
  O método <code>authClient.dodopayments.checkout</code> está obsoleto. Utilize
  <code>checkoutSession</code> em vez dele para novas implementações.
</Warning>
...
## Rastreando Uso Medido

Ative o plugin <code>usage()</code> no servidor para capturar eventos medidos e permitir que os clientes inspecionem seu faturamento baseado em uso.

- <code>authClient.dodopayments.usage.ingest</code> ingere eventos para o usuário autenticado e com e-mail verificado.
- <code>authClient.dodopayments.usage.meters.list</code> lista o uso recente para os medidores vinculados às assinaturas desse cliente.
...
<Tip>
  Carimbos de data/hora com mais de uma hora de idade ou com mais de cinco minutos
  no futuro são rejeitados ao ingerir o uso.
</Tip>
<Tip>
  Se você omitir <code>meter_id</code> ao listar medidores de uso, todos os
  medidores vinculados às assinaturas do cliente são retornados.
</Tip>

# Webhooks

<Info>
  O plugin de webhooks processa eventos de pagamento em tempo real da Dodo Payments com
  verificação segura de assinatura. O endpoint padrão é
  <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
{/* LOCKED_PATTERN_1f3f4c5fbf255932f2c0aedf7d16c279 */}
Gere um segredo de webhook para a URL do seu endpoint (ex.: <code>https://&lt;seu-dominio&gt;/api/auth/dodopayments/webhooks</code>) no Painel da Dodo Payments e defina-o no seu arquivo <code>.env</code>:
...
<AccordionGroup>
  {/* LOCKED_PATTERN_9586fa213a07317c452129bb9c8365ad */}
    - <b>cliente</b> (obrigatório): instância do cliente DodoPayments -{" "}
    <b>createCustomerOnSignUp</b> (opcional): cria clientes automaticamente ao
    cadastrar usuários - <b>use</b> (obrigatório): array de plugins para ativar
    (checkout, portal, usage, webhooks)
  </Accordion>
  {/* LOCKED_PATTERN_a313db3f1395a5521aceb89c3251fabd */}
    - <b>products</b>: array de produtos ou função assíncrona retornando produtos
    - <b>successUrl</b>: URL para redirecionar após o pagamento bem-sucedido
    - <b>authenticatedUsersOnly</b>: exige autenticação de usuário (padrão: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  {/* LOCKED_PATTERN_8cb977e6bc7a95fb0f5c0e3c34015b67 */}
    - <b>Chave de API inválida</b>: verifique novamente sua
    <code>DODO_PAYMENTS_API_KEY</code> no <code>.env</code>. -{" "}
    <b>Assinatura de webhook divergente</b>: certifique-se de que o segredo do webhook corresponda ao definido no Painel da Dodo Payments. - <b>Cliente não criado</b>: confirme se
    <code>createCustomerOnSignUp</code> está definido como <code>true</code>.
  </Accordion>
  {/* LOCKED_PATTERN_328949dc9617c5646d2623ce7f82eb78 */}
    - Utilize variáveis de ambiente para todos os segredos e chaves. - Teste em
    <code>test_mode</code> antes de mudar para <code>live_mode</code>. - Registre
    eventos de webhook para depuração e auditoria.
  </Accordion>
</AccordionGroup>
