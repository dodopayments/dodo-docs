...
| Campo | Descrição |
|-------|-------------|
| `subTotal` | A soma de todos os itens de linha na sessão antes da aplicação de quaisquer descontos ou impostos. |
| `discount` | O valor total de todos os descontos aplicados. |
| `tax` | O valor calculado do imposto. No modo `inline`, ele é atualizado dinamicamente à medida que o usuário interage com os campos de endereço. |
...
| `finalTotal` | O valor efetivamente cobrado do cliente. Isso pode incluir ajustes adicionais de câmbio ou taxas de métodos de pagamento locais que não fazem parte do detalhamento básico do preço. |
| `finalTotalCurrency` | A moeda na qual o cliente está efetivamente pagando. Ela pode ser diferente de `currency` se a [paridade do poder de compra](/features/purchasing-power-parity) ou a conversão para a moeda local estiver ativa. |
