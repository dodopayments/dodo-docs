...
<Tip>
Certifique-se de ter um elemento de contêiner com o correspondente `id` na sua página: `<div id="dodo-inline-checkout"></div>`.
</Tip>
...
<Check>
Você deve ver eventos de `checkout.breakdown` registrados no console do seu navegador se você adicionou um log de console no callback de `onEvent`.
</Check>
...
| `options.showTimer` | `boolean` | Não | Mostrar ou ocultar o temporizador do checkout. O padrão é `true`. Quando desativado, você receberá o evento `checkout.link_expired` quando a sessão expirar. |
| `options.showSecurityBadge` | `boolean` | Não | Mostrar ou ocultar o selo de segurança. O padrão é `true`. |
| `options.manualRedirect` | `boolean` | Não | Quando habilitado, o checkout não redirecionará automaticamente após a conclusão. Em vez disso, você receberá eventos de `checkout.status` e `checkout.redirect_requested` para gerenciar o redirecionamento você mesmo. |
...
Ao usar `manualRedirect`, manipule a conclusão do checkout no seu callback de `onEvent`:
...
O SDK fornece eventos em tempo real através do callback de `onEvent`. Para o checkout inline, `checkout.breakdown` é particularmente útil para sincronizar sua interface.
...
O evento `checkout.breakdown` é a principal maneira de manter a interface da sua aplicação sincronizada com o estado do checkout do Dodo Payments.
...
| `finalTotal` | O valor real que o cliente é cobrado. Isso pode incluir ajustes adicionais de câmbio ou taxas de métodos de pagamento locais que não fazem parte da divisão de preços básica. |
...
2.  **Manipulação de Estados Iniciais**: Quando o checkout é carregado pela primeira vez, `tax` e `discount` podem estar `0` ou `null` até que o usuário forneça suas informações de cobrança ou aplique um código. Sua interface deve lidar com esses estados de forma adequada (por exemplo, mostrando um traço `—` ou ocultando a linha).
...
O SDK fornece informações detalhadas sobre erros através do sistema de eventos. Sempre implemente um tratamento de erros adequado no seu callback de `onEvent`:
...
1. **Design Responsivo**: Certifique-se de que seu elemento de contêiner tenha largura e altura suficientes. O iframe geralmente se expandirá para preencher seu contêiner.
...
<Info>
Para implementações em modo escuro, é recomendável usar `#0d0d0d` como a cor de fundo para uma melhor integração visual com o quadro de checkout inline.
</Info>
...
Embora eventos de checkout inline como `checkout.status` forneçam feedback em tempo real, eles **não** devem ser sua única fonte de verdade para o status do pagamento. Problemas de rede, falhas no navegador ou usuários fechando a página podem fazer com que eventos sejam perdidos. Para garantir uma validação de pagamento confiável:

1. **Seu servidor deve escutar eventos de webhook** - Dodo Payments envia webhooks para alterações no status do pagamento
2. **Implemente um mecanismo de polling** - Seu frontend deve fazer polling no seu servidor para atualizações de status
...
**1. Escute eventos de checkout** - Quando o usuário clicar em pagar, comece a se preparar para verificar o status:
...
**2. Faça polling no seu servidor** - Crie um endpoint que verifica seu banco de dados para o status do pagamento (atualizado por webhooks):
...
**3. Manipule webhooks do lado do servidor** - Atualize seu banco de dados quando o Dodo enviar webhooks de `payment.succeeded` ou `payment.failed`. Veja nossa [documentação sobre Webhooks](/developer-resources/webhooks) para detalhes.

### Manipulando Redirecionamentos (3DS, Google Pay, UPI)
...
- **UPI** - Redirecionamentos de método de pagamento indiano
...
Após a autenticação ser concluída (sucesso ou falha), o usuário retorna à sua página. **Não assuma sucesso apenas porque o usuário retornou.** Em vez disso:

1. Verifique se o usuário está retornando de um redirecionamento (por exemplo, via `sessionStorage`)
2. Comece a fazer polling no seu servidor para o status de pagamento confirmado
3. Mostre um estado de "Verificando pagamento..." enquanto faz polling
4. Exiba a interface de sucesso/falha com base no status confirmado pelo servidor

<Tip>
Sempre verifique o status do pagamento do lado do servidor após redirecionamentos. O retorno do usuário à sua página significa apenas que a autenticação foi concluída—não indica se o pagamento foi bem-sucedido ou falhou.
</Tip>
...
O Apple Pay permite que os clientes concluam pagamentos de forma rápida e segura usando seus métodos de pagamento salvos. Quando habilitado, os clientes podem abrir o modal do Apple Pay diretamente da sobreposição de checkout em dispositivos suportados.
...
<Steps>
<Step title="Baixe e faça upload do arquivo de associação de domínio do Apple Pay">
...
Faça upload do arquivo para o seu servidor web em `/.well-known/apple-developer-merchantid-domain-association`. Por exemplo, se seu site for `example.com`, disponibilize o arquivo em `https://example.com/well-known/apple-developer-merchantid-domain-association`.
...
<Step title="Verifique a disponibilidade do Apple Pay">

Após receber a confirmação, teste o Apple Pay no seu checkout:
...
<Warning>
O Apple Pay deve ser habilitado para seu domínio antes de aparecer como uma opção de pagamento em produção. Entre em contato com o suporte antes de ir ao vivo se você planeja oferecer Apple Pay.
</Warning>
...
<Tip>
Use **checkout inline** quando quiser controle máximo sobre a experiência de checkout e branding sem costura. Use **checkout overlay** para uma integração mais rápida com mudanças mínimas nas suas páginas existentes.
</Tip>
...
<Card title="Webhooks" icon="webhook" href="/developer-resources/webhooks">
    Manipule eventos de pagamento do lado do servidor com webhooks.
</Card>
