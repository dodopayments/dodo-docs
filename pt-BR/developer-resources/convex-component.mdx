...
<CardGroup cols={2}>
<Card title="Função de Checkout" icon="cart-shopping" href="#checkout-function">
    Integre o checkout dos pagamentos Dodo com fluxo baseado em sessão.
</Card>
<Card title="Portal do Cliente" icon="user" href="#customer-portal-function">
...
<Step title="Adicione o Componente à Configuração do Convex">
...
Configure variáveis de ambiente no seu painel do Convex (**Configurações** → **Variáveis de Ambiente**). Você pode acessar o painel executando:
...
- `DODO_PAYMENTS_API_KEY` - Sua chave da API do Dodo Payments
- `DODO_PAYMENTS_ENVIRONMENT` - Defina como `test_mode` ou `live_mode`
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Seu segredo de webhook (requerido para manipulação de webhook)
...
<Steps>
<Step title="Crie uma Consulta Interna">
...
<Warning>
Antes de usar esta consulta, certifique-se de definir o esquema apropriado no seu arquivo `convex/schema.ts` ou altere a consulta para corresponder ao seu esquema existente.
</Warning>
...
<Step title="Configure o Componente DodoPayments">
 <CodeGroup>
...
<Tabs>
<Tab title="Configuração da Função de Checkout">
  <Info>
...
<Tab title="Configuração do Portal do Cliente">
  <Info>
  Use esta função para permitir que os clientes gerenciem suas assinaturas e detalhes através do portal do cliente do Dodo Payments. O cliente é automaticamente identificado pela função `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="Configuração do Manipulador de Webhook">
  <Info>
  Use este manipulador para receber e processar eventos de webhook do Dodo Payments de forma segura na sua aplicação Convex. Todos os manipuladores de webhook recebem o Convex `ActionCtx` como primeiro parâmetro, permitindo que você use `ctx.runQuery()` e `ctx.runMutation()` para interagir com seu banco de dados.
  </Info>
  <CodeGroup>
  ...
  <Warning>
  Certifique-se de definir as mutações de banco de dados correspondentes no seu backend Convex para cada evento de webhook que você deseja manipular. Por exemplo, crie uma mutação <code>createPayment</code> para registrar pagamentos bem-sucedidos ou uma mutação <code>createSubscription</code> para gerenciar o estado da assinatura.
  </Warning>
</Tab>
</Tabs>
