...
Configure as variáveis de ambiente no seu painel do Convex (**Configurações** → **Variáveis de Ambiente**). Você pode acessar o painel executando:
...
<Tab title="Configuração do Portal do Cliente">
  <Info>
  Use esta função para permitir que os clientes gerenciem suas assinaturas e detalhes através do portal do cliente do Dodo Payments. O cliente é automaticamente identificado através da função `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="Configuração do Manipulador de Webhook">
  <Info>
  Use este manipulador para receber e processar eventos de webhook do Dodo Payments de forma segura em seu aplicativo Convex. Todos os manipuladores de webhook recebem o `ActionCtx` do Convex como o primeiro parâmetro, permitindo que você use `ctx.runQuery()` e `ctx.runMutation()` para interagir com seu banco de dados.
  </Info>
  <CodeGroup>
