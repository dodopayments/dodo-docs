...
Use Checkout Sessions para vender produtos de assinatura com um checkout seguro e hospedado. Passe seu produto de assinatura em `product_cart` e redirecione os clientes para o `checkout_url` retornado.
...
3. **`subscription.on_hold`** - A assinatura está suspensa devido a falha na renovação.
...
<Tip>
Use `subscription.updated` para obter notificações em tempo real sobre quaisquer alterações na assinatura, mantendo o estado da sua aplicação em sincronia sem fazer polling na API.
</Tip>
...
2. Assinatura Suspensa
  - `subscription.on_hold` - A assinatura está suspensa devido a falha no pagamento de renovação ou falha na cobrança de alteração de plano.
...
Para reativar uma assinatura do estado {/* INLINE_CODE_PLACEHOLDER_833e01167b2e3da70d83bdfb406adc7f */, use a API Update Payment Method. Isso automaticamente:
...
Quando você receber um webhook {/* INLINE_CODE_PLACEHOLDER_5e9a0875003c752d72379e80bcc55356 */, atualize o estado da sua aplicação e notifique o cliente:
...
#### 2. `full_immediately`
- Cobra o cliente o valor total da assinatura para o novo plano
- Ignora qualquer tempo restante ou créditos do plano anterior
- Útil quando você deseja redefinir o ciclo de cobrança ou cobrar o valor total independentemente da proporcionalidade

#### 3. `difference_immediately`
- Ao fazer um upgrade, o cliente é imediatamente cobrado pela diferença entre os valores dos dois planos.
- Por exemplo, se o plano atual é 30 Dólares e o cliente faz upgrade para um plano de 80 Dólares, ele é cobrado $50 instantaneamente. 
- Ao fazer downgrade, o valor não utilizado do plano atual é adicionado como crédito interno e aplicado automaticamente às futuras renovações de assinatura.
- Por exemplo, se o plano atual é 50 Dólares e o cliente muda para um plano de 20 Dólares, os $30 restantes são creditados e usados para o próximo ciclo de cobrança.

### Comportamento

- Quando você invoca esta API, o Dodo Payments inicia imediatamente uma cobrança com base na sua opção de proporcionalidade selecionada
- Se a mudança de plano for um downgrade e você usar `prorated_immediately`, os créditos serão calculados automaticamente e adicionados ao saldo de crédito da assinatura. Esses créditos são específicos para essa assinatura e serão usados apenas para compensar futuros pagamentos recorrentes da mesma assinatura
- A opção `full_immediately` ignora os cálculos de crédito e cobra o valor total do novo plano

<Tip>
**Escolha sua opção de proporcionalidade com cuidado**: Use `prorated_immediately` para uma cobrança justa que considere o tempo não utilizado, ou `full_immediately` quando você deseja cobrar o valor total do novo plano independentemente do ciclo de cobrança atual.
</Tip>
