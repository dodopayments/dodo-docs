...
Após clicar, defina suas variáveis de ambiente em **Cursor's** `mcp.json` através de **Cursor Settings** > **Tools & MCP** > **New MCP Server**.
...
<Tab title="Claude Desktop">
Configure o servidor MCP de Dodo Payments no Claude Desktop para habilitar acesso conversacional aos seus dados de pagamento.
<Steps>
...
A configuração será armazenada no arquivo `.claude.json` do seu diretório pessoal.
...
Após clicar, defina suas variáveis de ambiente na configuração de usuário do VS Code através de **Command Palette** > **MCP: Open User Configuration**.

**Configuração Manual**

<Steps>
<Step title="Abrir configuração do MCP">
    Abra o Command Palette (`Cmd/Ctrl + Shift + P`) e pesquise por "MCP: Open User Configuration".
</Step>
...
Configure o servidor MCP de Dodo Payments no Cline (VS Code) para habilitar acesso conversacional aos seus dados de pagamento.
<Steps>
...
<Step title="Recarregar VS Code">
    Recarregue a janela do VS Code (`Cmd/Ctrl + Shift + P` > "Reload Window") para ativar a conexão do servidor MCP.
</Step>
</Steps>
</Tab>
...
| `x-dodo-payments-api-key` | Cabeçalho de chave da API direta |
...
- `valid-json` - Análise correta do argumento de string JSON
...
<Tip>
Execute `npx dodopayments-mcp --describe-capabilities` para obter informações detalhadas sobre capacidades.
</Tip>
...
2. **`get_api_endpoint_schema`** - Obtém o esquema detalhado para um método específico
...
Ao implantar remotamente, sempre exija autenticação através do cabeçalho `Authorization` ou cabeçalho `x-dodo-payments-api-key`.
