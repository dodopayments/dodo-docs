...
`start` rejeita com um `CheckoutError` somente em caso de uso incorreto ou falha da plataforma. Um pagamento cancelado ou recusado sempre é um resultado, nunca uma exceção.

- `INVALID_CHECKOUT_URL`: não é uma URL de sessão `checkout.dodopayments.com`.
- `INVALID_RETURN_URL`: não é uma URL absoluta válida.
- `ALREADY_IN_PROGRESS`: já existe um checkout em andamento.
