...
O próprio plugin de configuração do pacote registra o scheme para ambas as plataformas em
`prebuild`. Passe a ele uma opção `scheme` em `app.json`:
...
`scheme` deve corresponder ao valor passado em `returnUrl` (por exemplo,
`myappcheckout://return`) e deve ser diferente de `expo.scheme`. O Expo já
registra esse último em `MainActivity`, e reutilizá-lo pode direcionar o retorno do checkout
para a tela errada.

Recompile o projeto nativo depois de editar `app.json`:
...
Se `android/app/build.gradle` for mantido manualmente e não tiver um bloco
`defaultConfig { }` padrão para o plugin encontrar, adicione o placeholder
manualmente. Consulte a aba Android acima.
</Tab>
</Tabs>
</Step>
</Steps>
...
`customization` em `start(...)`. As opções são agrupadas por plataforma porque
