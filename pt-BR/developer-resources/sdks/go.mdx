...
Quando a API retorna um status code que não indica sucesso, o SDK retorna um erro do tipo
`*dodopayments.Error`. Ele expõe o `StatusCode`, o `*http.Request` subjacente e
`*http.Response`, além do JSON do corpo do erro. Use o padrão `errors.As` para
inspecioná-lo e use `StatusCode` para tratar casos específicos:
...
Adicione um middleware personalizado para logging ou métricas:
...
Aceitamos contribuições! Consulte as [diretrizes de contribuição](https://github.com/dodopayments/dodopayments-go/blob/main/CONTRIBUTING.md) para começar.
