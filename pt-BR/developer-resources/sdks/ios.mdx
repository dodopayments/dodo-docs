...
Personalize o botão de fechar, o estilo de apresentação e o esquema de cores da tela
por meio de `customization` em `start(...)`. Todos os campos são opcionais; omitir
`customization` usa a aparência padrão do `SFSafariViewController` no iOS.

<ParamField body="dismissButtonStyle" type="DismissButtonStyle">
Rótulo ou ícone para o botão de fechar: `done`, `close` ou `cancel`.
</ParamField>

<ParamField body="presentationStyle" type="PresentationStyle">
`pageSheet` é apresentado como um cartão que pode ser fechado deslizando; `fullScreen` cobre a tela inteira.
</ParamField>
...
`start` lança `CheckoutError` apenas em caso de uso incorreto ou falha da plataforma. Um pagamento cancelado ou recusado é sempre um resultado, nunca uma exceção.
...
- `alreadyInProgress` (`ALREADY_IN_PROGRESS`): um checkout já está em andamento.
- `platformError` (`PLATFORM_ERROR`): falha inesperada da plataforma.

## Sessões abandonadas

<Info>
Se o aplicativo for encerrado durante o checkout, recupere a sessão na próxima inicialização e concilie-a com o seu backend.
</Info>
