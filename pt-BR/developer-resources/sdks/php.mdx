...
<Info>
`billing` requer, no mínimo, o código ISO de duas letras `country`. Passe `AttachExistingCustomer::with(customerID: '...')` para associar um cliente existente ou `NewCustomer::with(email: '...', name: '...')` para criar um. `productPrice` está na menor denominação da moeda.
</Info>
...
Quando a biblioteca não consegue se conectar à API ou recebe um status code de não sucesso (4xx ou 5xx), uma subclasse de `APIException` é lançada:
...
| Tempo limite          | `APITimeoutException`          |
| Erro de rede    | `APIConnectionException`       |

<Tip>
  Sempre envolva as chamadas da API em blocos try-catch para lidar com possíveis erros
  de forma adequada e fornecer feedback significativo aos usuários.
</Tip>
...
<Card title="GitHub Repository" icon="github" href="https://github.com/dodopayments/dodopayments-php">
  Consulte o código-fonte e contribua
</Card>
...
Aceitamos contribuições! Consulte as [diretrizes de contribuição](https://github.com/dodopayments/dodopayments-php/blob/main/CONTRIBUTING.md) para começar.
