...
O SDK utiliza parâmetros nomeados para especificar argumentos opcionais. Você pode inicializar objetos de valor usando o construtor estático `with`:
...
Quando a biblioteca não consegue conectar-se à API ou recebe um código de status não-sucedido (4xx ou 5xx), uma subclasse de `APIException` é lançada:
...
Registre-se em `config/services.yaml`:
