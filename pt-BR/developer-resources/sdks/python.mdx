...
<Info>
O SDK requer Python 3.9 ou superior. Recomendamos usar a versão estável mais recente do Python para melhor experiência e atualizações de segurança.
</Info>
...
<Warning>
Armazene sempre suas chaves de API de forma segura usando variáveis de ambiente. Nunca as inclua no controle de versão.
</Warning>
...
<Card title="Pythonic Interface" icon="code">
  Código Python limpo e idiomático que segue as diretrizes do PEP 8 e as convenções do Python
</Card>
...
<Card title="Type Hints" icon="check">
  Anotações de tipo completas para melhor suporte das IDEs e verificação de tipos com mypy
</Card>

<Card title="Auto-Pagination" icon="arrows-rotate">
  Paginação automática para respostas de listas com iteração simples
</Card>
</CardGroup>
...
<Info>
`billing` exige, no mínimo, o código de país ISO de duas letras. `customer` aceita `{"customer_id": ...}` para associar um customer existente ou `{"email": ..., "name": ...}` para criar um novo. `product_price` está na menor denominação da moeda.
</Info>
...
Rastreie eventos personalizados para faturamento baseado em uso:
...
Itere automaticamente por todos os itens:
...
Para ter mais controle sobre a paginação:
...
Personalize o cliente `httpx` subjacente:
...
Use aiohttp para obter desempenho assíncrono aprimorado:
...
Ative o logging definindo a variável de ambiente:

```bash
export DODO_PAYMENTS_LOG=info
```

Ou para logging no nível de debug:
...
Precisa de ajuda com o Python SDK?

- **Discord**: participe do nosso [servidor da comunidade](https://discord.gg/bYqAp4ayYh) para obter suporte em tempo real
- **Email**: entre em contato conosco pelo endereço support@dodopayments.com
- **GitHub**: abra uma issue no [repositório](https://github.com/dodopayments/dodopayments-python)

## Contribuição

Aceitamos contribuições! Consulte as [diretrizes de contribuição](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md) para começar.
