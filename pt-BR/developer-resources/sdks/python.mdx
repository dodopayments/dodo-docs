...
Iterar por todos os itens automaticamente:
...
Para mais controle sobre paginação:
...
Customize o cliente `httpx` subjacente:
...
Use aiohttp para melhorar o desempenho assíncrono:
...
Ative o logging configurando a variável de ambiente:

```bash
export DODO_PAYMENTS_LOG=info
```

Ou para logging em nível de depuração:
...
- **Discord**: Entre em nosso [servidor da comunidade](https://discord.gg/bYqAp4ayYh) para suporte em tempo real
...
Nós recebemos contribuições! Confira as [diretrizes de contribuição](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md) para começar.
