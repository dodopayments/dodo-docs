...
`dodo wh trigger` pode enviar payloads simulados para estes 22 tipos de evento:
...
<Note>
Este é um subconjunto dos eventos que Dodo Payments entrega. `payout.*`, `credit.*`, `abandoned_checkout.*`, `dunning.*`, `entitlement_grant.*`, `subscription.paused` e `subscription.update_payment_method` ainda não têm um gerador de simulação — teste-os com atividades reais no modo de teste e `dodo wh listen`. Consulte [Webhook Events](/developer-resources/webhooks/intents/webhook-events-guide) para ver a lista completa.
</Note>
...
| Variável | Descrição |
|----------|-------------|
| `DODO_WH_TEST_SERVER_URL` | Substitui a URL padrão do servidor de retransmissão de webhooks usada por `dodo wh listen` |

## Atualizações

A CLI verifica se há uma versão mais recente na inicialização e exibe uma notificação na barra de status quando disponível. Para atualizar:
...
<Card title="GitHub Repository" icon="github" href="https://github.com/dodopayments/dodopayments-cli">
  Ver o código-fonte e as versões
</Card>
