...
`dodo wh trigger` pode enviar payloads simulados para todos os 47 tipos de evento que o Dodo Payments entrega:

| Categoria | Eventos |
|----------|--------|
| **Assinatura** | `active`, `updated`, `on_hold`, `paused`, `unpaused`, `renewed`, `plan_changed`, `cancelled`, `failed`, `expired`, `update_payment_method` |
