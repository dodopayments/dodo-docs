...
<Tip>
Sempre use a versão mais recente do SDK para acessar os recursos mais novos do Dodo Payments. Consulte o [Maven Central](https://central.sonatype.com/artifact/com.dodopayments.api/dodo-payments-java) para ver a versão mais recente.
</Tip>

<Info>
O SDK oferece suporte ao Java 8 e a todas as versões posteriores, incluindo Java 11, 17 e 21.
</Info>
...
<Warning>
Sempre armazene suas chaves de API com segurança usando variáveis de ambiente, propriedades do sistema ou um sistema seguro de gerenciamento de configurações. Nunca codifique-as diretamente no seu código-fonte.
</Warning>
...
<Card title="Thread-Safe" icon="bolt">
  Seguro para uso concorrente em aplicações multithread
</Card>

<Card title="Builder Pattern" icon="layer-group">
  Padrão de builder intuitivo para construção de requisições
</Card>
...
Configure para o ambiente Test Mode:
...
<Warning>
`POST /subscriptions` (o método `subscriptions.create` do SDK) está **obsoleto**. Ele ainda funciona para integrações existentes, mas novas integrações devem criar assinaturas por meio de uma [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`productPrice` é expresso na menor denominação da moeda (por exemplo, centavos para USD e paise para INR). Para cobrar $25.00, passe `2500`.
</Info>

<Tip>
A cobrança via `subscriptions().charge(...)` destina-se a [assinaturas sob demanda](/developer-resources/ondemand-subscriptions). As assinaturas agendadas padrão são cobradas automaticamente com base no cronograma de preços do produto.
</Tip>
...
Crie e gerencie medidores para acompanhar o uso:
...
### Ingestão em lote de eventos

Ingira vários eventos com eficiência (máximo de 1000 por solicitação):
...
<Tip>
O SDK tenta novamente as solicitações automaticamente em caso de erros de conexão, erros 408, 409, 429 e 5xx, usando backoff exponencial.
</Tip>
...
Precisa de ajuda com o Java SDK?
...
Aceitamos contribuições! Consulte as [diretrizes de contribuição](https://github.com/dodopayments/dodopayments-java/blob/main/CONTRIBUTING.md) para começar.
