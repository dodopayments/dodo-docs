...
Este guia mostra como criar um sistema completo de **fulfillment manual de chaves de licença**. Em vez de o Dodo Payments gerar automaticamente uma chave após o pagamento, cada compra cria um grant `Pending` e aguarda que *você* forneça o valor da chave a partir do seu próprio sistema, de um fornecedor terceirizado ou de um pool finito de códigos.
...
- Uma conta de merchant da Dodo Payments.
- Sua chave de API (`DODO_PAYMENTS_API_KEY`) e a chave secreta do webhook no painel. Consulte o [guia de geração de chaves de API](/api-reference/introduction#api-key-management-and-authentication).
...
<Frame>
<img src="/images/entitlements/license-keys/create.png" alt="Formulário de novo entitlement de chave de licença com nome, modo de fulfillment, duração da licença, limite de ativações e mensagem de ativação" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
<Step title="Set Fulfillment Mode to Manual">
Abra o menu suspenso **Fulfillment Mode** e altere de **Automatic** para **Manual**. Essa é a configuração que orienta todo este guia — sem ela, as chaves são geradas e enviadas por e-mail automaticamente, e nenhum grant pendente é criado. Com **Manual** selecionado, cada compra cria um grant `Pending` para você processar. Clique em **Create Entitlement** para salvar.
</Step>
</Steps>
</Tab>
...
<Note>
O modo de fulfillment é uma propriedade do **entitlement**, não do produto. Como você o definiu como **Manual** na Etapa 1, todo produto ao qual esse entitlement estiver associado criará grants de chave de licença `Pending` no momento da compra — não há nada adicional para configurar aqui.
</Note>
...
Quando um cliente compra o produto, o Dodo Payments cria um grant com status `Pending`, **sem nenhuma chave associada**, e dispara um webhook `entitlement_grant.created`. Esse é o seu sinal de que um cliente está aguardando uma chave.
...
Em caso de sucesso, o grant passa para `Delivered`, o cliente recebe a chave automaticamente (o mesmo e-mail que receberia no fulfillment automático), e os eventos de webhook `license_key.created` e `entitlement_grant.delivered` são disparados.
...
| Status | Significado | O que fazer |
|---|---|---|
| `200` | Chave entregue; o grant agora está em `Delivered`. | Concluído. |
| `400` | Não é um grant de chave de licença, ou a chave está vazia/em branco. | Corrija a solicitação; não tente novamente como está. |
| `404` | Não há nenhum grant com esse ID para sua empresa. | Verifique o `grant_id`. |
| `409` | O grant não está aguardando fulfillment (já foi entregue ou já possui uma chave), **ou** o valor da chave já existe. | Se já tiver sido entregue, trate como sucesso. Se a chave for duplicada, forneça uma chave diferente. |
| `422` | O corpo da solicitação falhou na validação (por exemplo, `activations_limit < 1`). | Corrija o campo e tente novamente. |
...
1. Compre o produto no modo de teste (consulte os [guias de checkout](/developer-resources/integration-guide)).
2. Confirme que seu webhook recebeu `entitlement_grant.created` com `status: "Pending"` e `integration_type: "license_key"`, ou que o grant aparece na resposta de List Grants com esses filtros.
3. Chame o endpoint de fulfill com uma chave de teste.
4. Confirme que a resposta mostra `status: "Delivered"` com um `license_key` preenchido, que o cliente recebeu o e-mail com a chave e que `entitlement_grant.delivered` foi disparado.
...
<Card title="Fulfill License Key Grant" icon="code" href="/api-reference/entitlements/fulfill-license-key">
Forneça o valor da chave e faça a transição do grant para o status delivered.
</Card>
<Card title="Entitlement Grant Webhooks" icon="webhook" href="/developer-resources/webhooks/intents/entitlement-grant">
