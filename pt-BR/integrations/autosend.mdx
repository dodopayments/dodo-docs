...
- **Use dados dinâmicos para personalização**: Use o `dynamicData` campo para personalizar e-mails com informações específicas do cliente, como nomes, valores de pagamento e detalhes de assinatura. E-mails personalizados têm taxas de engajamento mais altas.
...
- Certifique-se de que os endereços de e-mail estão formatados corretamente
- Verifique se `templateId` é válido se estiver usando templates
- Verifique se as chaves do `dynamicData` correspondem às suas variáveis de template
</Accordion>

<Accordion title="Problemas de template">
- Verifique se o ID do seu template está correto e ativo no AutoSend
- Certifique-se de que as chaves do `dynamicData` correspondem às variáveis usadas no seu template
- Verifique se todas as variáveis de template obrigatórias estão fornecidas
