...
Quando um cliente compra um produto em modo manual, a concessão é criada em status `pending` sem chave e um webhook [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) é disparado com `integration_type: "license_key"` e `status: "pending"`. Você pode reagir a esse webhook ou consultar o endpoint [List Grants](/api-reference/entitlements/list-grants) com os filtros `integration_type` e `status`:
