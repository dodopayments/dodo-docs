...
<Frame>
    <img src="/images/customer-portal/cancellation-reasons.png" alt="Modal de cancelamento com o menu suspenso 'Por que você está cancelando?' mostrando razões como Muito caro, Faltando funcionalidades, e Outros" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
O valor selecionado é escrito no campo `cancellation_feedback` da assinatura. Um texto livre opcional `cancellation_comment` também pode ser capturado.
...
Ambos os campos também estão incluídos no payload do webhook `subscription.cancelled`, e você pode preenchê-los programaticamente em `PATCH /subscriptions/{id}` ao agendar ou executar um cancelamento através da API.

<Tip>
Combine `cancellation_feedback` com [Subscription Dunning](/features/recovery/subscription-dunning) para personalizar e-mails de recuperação — por exemplo, envie um código de desconto para canceladores `too_expensive` e uma pesquisa "o que está faltando?" para canceladores `missing_features`.
</Tip>
...
Quando uma assinatura é colocada em estado `on_hold` devido a uma falha de pagamento, os clientes devem atualizar seu método de pagamento para reativá-la. O processo de atualização automaticamente:

1. **Cria uma cobrança** para valores devidos remanescentes
2. **Gera uma fatura** para a cobrança
3. **Processa o pagamento** usando o novo método de pagamento
4. **Reativa a assinatura** para o estado `active` após o pagamento bem-sucedido

<Warning>
Assinaturas em estado `on_hold` não renovarão automaticamente. Os clientes devem atualizar seu método de pagamento para liquidar débitos e reativar a assinatura.
</Warning>
...
Confirme a atualização. Para assinaturas `on_hold`, isso criará automaticamente uma cobrança por débitos remanescentes.
</Step>
...
Crie uma sessão de Portal do Cliente com prazo definido para um cliente específico via API, depois redirecione o usuário para a URL da sessão.
...
Além dos portais de clientes específicos para empresas, o Dodo Payments oferece um **Portal do Cliente Unificado** em [customer.dodopayments.com](https://customer.dodopayments.com) onde os clientes podem visualizar e gerenciar todas as suas compras e assinaturas em diferentes empresas usando o Dodo Payments.
...
<Tip>
O Portal do Cliente Unificado complementa os portais específicos para empresas. Os clientes podem usar tanto o portal do comerciante com marca quanto o portal unificado, de acordo com sua preferência.
</Tip>
