...
Quando uma assinatura é colocada no estado `on_hold` devido a um pagamento falhado, os clientes devem atualizar seu método de pagamento para reativá-la. O processo de atualização automaticamente:
...
<Warning>
Assinaturas no estado `on_hold` não serão renovadas automaticamente. Os clientes devem atualizar seu método de pagamento para quitar as pendências e reativar a assinatura.
</Warning>
...
Clique em "Confirmar Alterações" para atualizar o método de pagamento. Para assinaturas `on_hold`, isso criará automaticamente uma cobrança para as pendências restantes.
</Step>
