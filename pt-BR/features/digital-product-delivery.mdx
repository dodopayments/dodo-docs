...
|---|---|
| `payment.succeeded` (uma vez) | Emitir um grant. O grant contém URLs de download presigned válidas por aproximadamente 15 minutos; os clientes podem atualizá-las reabrindo o link no e-mail ou a página do Customer Portal. |
| `subscription.active` | Emitir um grant. Os arquivos permanecem acessíveis enquanto a assinatura estiver ativa. |
| `subscription.renewed` | No-op. O mesmo grant continua válido; novas URLs presigned são geradas a cada busca. |
| `subscription.on_hold` / `cancelled` / `expired` | Revogar o grant. Novas URLs presigned não são mais emitidas. |
| `subscription.paused` | Revogar o grant. Novas URLs presigned não são emitidas até que a assinatura seja retomada. |
| `subscription.plan_changed` | Revogar o grant antigo e emitir um novo para o entitlement do novo plano. |
| `refund.succeeded` (uma vez) | Revogar o grant. |
...
<Note>
Os produtos configurados com o bloco `digital_product_delivery` mais antigo diretamente no produto foram **migrados automaticamente** para um entitlement de Digital Files. Os arquivos existentes enviados pela API legada de arquivos do produto foram preservados; eles continuam disponíveis para download e aparecem nos payloads de concessão junto com os arquivos enviados recentemente. Na configuração `digital_files` do entitlement, eles são rastreados na lista `legacy_file_ids`. Atualizações futuras (adicionar arquivos, alterar instruções, substituir a URL externa) devem ser feitas editando o entitlement de Digital Files migrado em **Entitlements**.
