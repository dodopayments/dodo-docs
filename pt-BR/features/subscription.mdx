...
Pausar e retomar são controlados por um único campo `pause` no endpoint de atualização de assinatura. Não existe um endpoint separado para pausar.
...
<Warning>
`pause` é exclusivo em relação a todos os outros campos — enviá-lo junto com qualquer outra coisa é rejeitado com `422`. Definir `status` como `paused` **não** pausa uma assinatura; use o campo `pause`.
</Warning>
...
| Status | Significado | Recuperável? | Caminho de recuperação / próximo passo |
|--------|---------------|--------------|---------------------------|
...
| `on_hold` | Um pagamento de renovação (ou cobrança de alteração de plano) falhou; as renovações foram interrompidas | **Sim** | Atualize o método de pagamento para reativar — automaticamente por meio de [Payment Retries](/features/recovery/payment-retries) e [Dunning](/features/recovery/subscription-dunning), ou manualmente por meio da [Update Payment Method API](/api-reference/subscriptions/update-payment-method) |
| `paused` | A assinatura foi pausada deliberadamente por você ou pelo cliente; a cobrança está congelada e o acesso foi revogado | **Sim** | Retome-a com `pause: false` ou pelo Customer Portal quando a pausa self-service estiver habilitada — consulte [Pausando e retomando assinaturas](#pausing-and-resuming-subscriptions) |
| `cancelled` | A assinatura foi cancelada e não será renovada | Somente recomprável | O cliente deve iniciar uma nova assinatura; [Dunning](/features/recovery/subscription-dunning) pode solicitar uma recompra |
| `failed` | A **criação** da assinatura falhou (o mandato ou pagamento inicial não foi bem-sucedido) | **Não — terminal** | O cliente deve criar uma nova assinatura com um método de pagamento válido |
