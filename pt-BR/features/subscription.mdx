...
<Warning>
O valor de `trial_period_days` deve estar entre 0 e 10.000 dias.
</Warning>
...
Cobranças proporcionais com base no tempo restante no ciclo de faturamento atual. Melhor para faturamento justo que considera o tempo não utilizado.
...
#### `full_immediately`
Cobra o valor total do novo plano imediatamente, ignorando o tempo restante. Melhor para redefinir ciclos de faturamento.
...
<Info>
Mudanças de plano acionam cobranças imediatas. Cobranças falhadas podem mover a assinatura para o status `on_hold`. Acompanhe as mudanças através de eventos de webhook `subscription.plan_changed`.
</Info>
...
- **`on_hold`**: A assinatura está pausada devido a falha no pagamento. Atualização do método de pagamento é necessária para reativar
...
<Info>
Após atualizar com sucesso o método de pagamento para uma assinatura {/* INLINE_CODE_PLACEHOLDER_833e01167b2e3da70d83bdfb406adc7f */, você receberá `payment.succeeded` seguido por eventos de webhook `subscription.active`.
</Info>
...
Atualize o método de pagamento para uma assinatura. Para assinaturas ativas, isso atualiza o método de pagamento para renovações futuras. Para assinaturas no estado {/* INLINE_CODE_PLACEHOLDER_833e01167b2e3da70d83bdfb406adc7f */, isso reativa a assinatura criando uma cobrança para as pendências restantes.
