...
| Ação | Descrição | Controle do Merchant |
|--------|-------------|------------------|
| **Ver plano atual** | Exibe o nome, o preço e a data de renovação do produto atual | Sempre disponível |
| **Fazer upgrade do plano** | Muda para um produto de nível superior na mesma coleção | Configurável (padrão: desabilitado) |
| **Fazer downgrade do plano** | Muda para um produto de nível inferior na mesma coleção | Configurável (padrão: desabilitado) |
| **Cancelar** | Cancela a assinatura completamente | Sempre disponível |
...
|---------|-------------|---------|
| **Permitir várias assinaturas** | Os clientes podem manter mais de uma assinatura ativa ao mesmo tempo | Habilitado |
| **Permitir atualizações de assinatura** | Os clientes podem fazer upgrade ou downgrade de suas assinaturas existentes a qualquer momento pelo Customer Portal | Desabilitado |
| **Permitir cancelamento de assinatura** | Os clientes podem cancelar suas próprias assinaturas pelo Customer Portal | Habilitado |
| **Permitir pausa de assinatura** | Os clientes podem pausar e retomar suas próprias assinaturas pelo Customer Portal | Desabilitado |
| **Faturar uso na pausa** | Uma assinatura baseada em uso pausada é faturada pelo uso devido no momento em que é pausada, em vez de transferir o uso para a próxima fatura regular | Habilitado |
| **Impedir uso indevido do trial** | Os clientes que já resgataram um trial são direcionados para uma compra paga sem trial, em vez de receberem um novo trial | Desabilitado |
...
<Info>
Desativar "Permitir cancelamento de assinatura" oculta dos clientes a opção de cancelamento self-service e rejeita solicitações de cancelamento feitas pela API do Customer Portal. Os cancelamentos realizados pelo seu próprio lado do merchant não são afetados. Consulte [Cancelando uma assinatura](/features/customer-portal#cancelling-a-subscription).
</Info>

<Info>
"Permitir pausa de assinatura" controla apenas o Customer Portal — você pode pausar e retomar pelo dashboard ou pela API, independentemente da configuração. "Faturar uso na pausa" aplica-se somente a assinaturas baseadas em uso e é registrado por ciclo de cobrança; portanto, alterá-lo no meio do ciclo não muda como o ciclo em andamento é liquidado. Consulte [Pausar e retomar assinaturas](/features/subscription#pausing-and-resuming-subscriptions).
</Info>

<Info>
Para obter detalhes sobre como os resgates de trial são correspondidos e registrados, consulte [Impedir uso indevido do trial](/features/subscription#preventing-trial-misuse).
</Info>
...
As Product Collections podem ser gerenciadas pelo dashboard do Dodo Payments ou programaticamente pela API. A API oferece controle total sobre a criação, atualização, upload de imagens e arquivamento de coleções, além do gerenciamento de grupos aninhados e produtos.
...
<Frame>
  <img 
    ...
    alt="Captura de tela do dashboard da Product Collection mostrando as operações de gerenciamento de coleções" 
  style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>

### Gerenciamento pela API

Os endpoints a seguir permitem criar, atualizar, consultar, arquivar e organizar Product Collections programaticamente — incluindo o gerenciamento de grupos aninhados e dos produtos contidos neles.

<AccordionGroup>
<Accordion title="Listing Product Collections">
Busque todas as Product Collections associadas à sua conta usando uma solicitação `GET` para o endpoint `/product-collections`. É compatível com paginação, filtragem por marca e inclusão de coleções arquivadas.
...
Crie uma nova Product Collection enviando uma solicitação `POST` para o endpoint `/product-collections` com detalhes como nome, descrição e marca.
...
Obtenha informações detalhadas sobre uma Product Collection específica — incluindo seus grupos e itens de produto — usando uma solicitação `GET` para o endpoint `/product-collections/{id}`.
...
Modifique os detalhes de uma Product Collection (nome, descrição, marca etc.) enviando uma solicitação `PATCH` para o endpoint `/product-collections/{id}`.
...
Associe uma imagem a uma coleção fazendo upload por meio de uma URL pré-assinada. Solicite uma URL de upload pelo endpoint `/product-collections/{id}/images` e, em seguida, `PUT` a imagem para a URL retornada em até 60 segundos.
...
<Card title="Create Group" icon="code" href="/api-reference/product-collections/create-group">
  Adicione um novo grupo a uma Product Collection.
</Card>
...
- **Agrupe logicamente**: Organize os produtos por intervalo de cobrança (mensal/anual) ou nível de recursos (inicial/pro/empresarial)
...
<Check>
Você está pronto para criar Product Collections e oferecer aos clientes uma experiência unificada de seleção de planos.
</Check>
