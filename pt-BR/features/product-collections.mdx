...
|---------|-------------|---------|
| **Permitir múltiplas assinaturas** | Os clientes podem ter mais de uma assinatura ativa ao mesmo tempo | Ativado |
| **Permitir atualizações de assinatura** | Os clientes podem fazer upgrade ou downgrade de suas assinaturas existentes a qualquer momento via Customer Portal | Desativado |
| **Permitir cancelamento de assinatura** | Os clientes podem cancelar suas próprias assinaturas pelo Customer Portal | Ativado |
| **Impedir uso indevido do período de teste** | Clientes que já resgataram um período de teste passam a fazer uma compra paga sem período de teste, em vez de receber um novo período de teste | Desativado |
...
<Info>
Para obter detalhes sobre como os resgates de períodos de teste são correspondidos e registrados, consulte [Impedindo uso indevido do período de teste](/features/subscription#preventing-trial-misuse).
</Info>
...
As coleções de produtos podem ser gerenciadas pelo dashboard do Dodo Payments ou programaticamente via API. A API oferece controle total sobre a criação e atualização de coleções, o upload de imagens, o arquivamento e o gerenciamento de grupos aninhados e produtos.

### Operações pelo dashboard
...
Os endpoints a seguir permitem criar, atualizar, recuperar, arquivar e organizar coleções de produtos programaticamente — incluindo o gerenciamento de grupos aninhados e dos produtos contidos neles.
...
Associe uma imagem a uma coleção fazendo upload por meio de uma URL pré-assinada. Solicite uma URL de upload do endpoint `/product-collections/{id}/images` e, em seguida, faça `PUT` da imagem para a URL retornada em até 60 segundos.
...
Os grupos permitem organizar produtos dentro de uma coleção (por exemplo, "Planos mensais" e "Planos anuais"). Use os endpoints de grupos para adicionar, atualizar ou remover grupos de uma coleção.
...
- **Agrupe logicamente**: Organize os produtos por intervalo de cobrança (mensal/anual) ou nível de recursos (inicial/profissional/empresarial)
...
- **Ative ambas as direções**: Permita upgrades e downgrades para oferecer flexibilidade aos clientes
- **Considere o proration**: Escolha um modo de proration alinhado ao seu modelo de negócios
- **Teste detalhadamente**: Verifique os fluxos de checkout e de alteração de plano no modo de teste antes de entrar em produção

<Check>
Você já pode criar coleções de produtos e oferecer aos clientes uma experiência unificada de seleção de planos.
</Check>

<CardGroup cols={2}>
<Card title="Products" icon="box" href="/features/products">
  Crie produtos avulsos, de assinatura ou baseados em uso para adicioná-los às coleções.
</Card>
