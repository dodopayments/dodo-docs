...
<Info>
Upsells e downsells permitem que você ofereça produtos adicionais ou mudanças de plano para clientes usando seus métodos de pagamento salvos. Isso possibilita compras de um clique que pulam a coleta de pagamento, melhorando drasticamente as taxas de conversão.
</Info>

<CardGroup cols={3}>
<Card title="Upsells Pós-Compra" icon="cart-plus">
  Ofereça produtos complementares imediatamente após o checkout com compras de um clique.
</Card>

<Card title="Upgrades de Assinatura" icon="arrow-up">
  Transfira clientes para níveis superiores com prorratação automática e faturamento instantâneo.
</Card>

<Card title="Cross-Sells" icon="grid-2-plus">
  Adicione produtos relacionados a clientes existentes sem precisar reinserir os detalhes de pagamento.
</Card>
</CardGroup>
...
- **Upsells**: Ofereça um produto ou upgrade de maior valor (por exemplo, plano Pro em vez de Básico)
- **Downsells**: Ofereça uma alternativa de menor preço quando um cliente recusa ou rebaixa
- **Cross-sells**: Sugira produtos complementares (por exemplo, complementos, itens relacionados)

Dodo Payments habilita esses fluxos através do parâmetro `payment_method_id`, que permite cobrar o método de pagamento salvo de um cliente sem exigir que ele reinsira os detalhes do cartão.

### Principais Benefícios

| Benefício | Impacto |
|---------|--------|
| **Compras de um clique** | Pule o formulário de pagamento completamente para clientes recorrentes |
| **Maior conversão** | Reduza a fricção no momento da decisão |
| **Processamento instantâneo** | Compras processadas imediatamente com `confirm: true` |
| **Experiência do usuário sem interrupções** | Os clientes permanecem em seu aplicativo durante todo o fluxo |
...
Antes de implementar upsells e downsells, certifique-se de que você tenha:

<Steps>
<Step title="Cliente com Método de Pagamento Salvo">
  Os clientes devem ter completado pelo menos uma compra. Os métodos de pagamento são salvos automaticamente quando os clientes finalizam o checkout.
</Step>

<Step title="Produtos Configurados">
  Crie seus produtos de upsell no painel do Dodo Payments. Estes podem ser pagamentos únicos, assinaturas ou complementos.
</Step>

<Step title="Webhook Endpoint">
  Configure webhooks para lidar com `payment.succeeded`, `payment.failed`, e `subscription.plan_changed` eventos.
</Step>
</Steps>

## Obtendo Métodos de Pagamento dos Clientes
...
<Info>
Os métodos de pagamento são salvos automaticamente quando os clientes completam o checkout. Você não precisa salvá-los explicitamente.
</Info>

## Upsells de Um Clique Pós-Compra
...
<Warning>
Ao usar `payment_method_id`, você deve configurar `confirm: true` e fornecer um `customer_id` existente. O método de pagamento deve pertencer a esse cliente.
</Warning>

## Upgrades de Assinatura

Transfira clientes para planos de assinatura de nível superior com manuseio de prorratação automática.
...
Sempre visualize as mudanças de plano para mostrar aos clientes exatamente quanto serão cobrados:
...
### Modos de Prorratação

Escolha como os clientes são cobrados ao atualizar:
...
| `prorated_immediately` | Cobra com base no tempo restante no ciclo de faturamento | Faturamento justo com base no tempo |
| `full_immediately` | Cobra o preço total do novo plano, ignora o tempo restante | Reinicia o ciclo de faturamento |

<Tip>
Use `difference_immediately` para fluxos de upgrade diretos. Use `prorated_immediately` quando quiser considerar o tempo não utilizado no plano atual.
</Tip>

## Cross-Sells

Adicione produtos complementares para clientes existentes sem exigir que eles reinseram os detalhes de pagamento.
...
Quando os clientes desejam mudar para um plano de nível inferior, gerencie a transição de forma suave com créditos automáticos.

### Como os Downgrades Funcionam

1. O cliente solicita o downgrade (Pro → Básico)
2. O sistema calcula o valor restante no plano atual
3. O crédito é adicionado à assinatura para renovações futuras
4. O cliente muda para o novo plano imediatamente
...
<Info>
Os créditos de downgrades usando `difference_immediately` são limitados à assinatura e aplicados automaticamente nas renovações futuras. Eles são distintos de [Créditos de Clientes](/features/customer-credit).
</Info>
...
<AccordionGroup>
<Accordion title="Cronometre Seus Upsells estrategicamente">
...
<Accordion title="Valide a Elegibilidade do Método de Pagamento">
Antes de tentar uma cobrança de um clique, verifique o método de pagamento:
- É compatível com a moeda do produto
- Não expirou
- Pertence ao cliente

A API validará isso, mas verificar proativamente melhora a experiência do usuário.
</Accordion>

<Accordion title="Lide com Falhas de Forma Suave">
Quando as cobranças de um clique falham:
1. Volte ao fluxo padrão de checkout
2. Notifique o cliente com mensagens claras
3. Ofereça a opção de atualizar o método de pagamento
4. Não tente repetidamente cobranças que falharam
</Accordion>

<Accordion title="Forneça uma Proposta de Valor Clara">
Upsells convertem melhor quando os clientes entendem o valor:
- Mostre o que eles estão recebendo em comparação com o plano atual
- Destaque a diferença de preço, não o preço total
- Use prova social (por exemplo, "Upgrade mais popular")
</Accordion>

<Accordion title="Respeite a Escolha do Cliente">
- Sempre forneça uma maneira fácil de recusar
- Não mostre o mesmo upsell repetidamente após a recusa
- Acompanhe e analise quais upsells convertem para otimizar ofertas
</Accordion>
</AccordionGroup>
...
| Evento | Gatilho | Ação |
|-------|---------|--------|
| `payment.succeeded` | Pagamento de upsell/cross-sell concluído | Entregar produto, atualizar acesso |
| `payment.failed` | Cobrança de um clique falhou | Mostrar erro, oferecer tentativa ou fallback |
| `subscription.plan_changed` | Upgrade/downgrade concluído | Atualizar recursos, enviar confirmação |
| `subscription.active` | Assinatura reativada após mudança de plano | Conceder acesso ao novo nível |

<Card title="Guia de Integração de Webhooks" icon="webhook" href="/developer-resources/webhooks">
...
<CardGroup cols={2}>
<Card title="Guia de Upgrade de Assinatura" icon="arrows-rotate" href="/developer-resources/subscription-upgrade-downgrade">
  Guia detalhado sobre mudanças de plano, modos de prorratação e como lidar com falhas.
</Card>
...
<Card title="API de Métodos de Pagamento do Cliente" icon="credit-card" href="/api-reference/customers/get-customer-payment-methods">
  Referência da API para listar métodos de pagamento do cliente.
</Card>
