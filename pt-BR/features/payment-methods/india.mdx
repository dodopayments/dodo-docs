...
description: Aceite UPI e todos os cartões emitidos na Índia com mandatos de
  assinatura em conformidade com o RBI. Entenda o atraso de processamento de 48
  horas, limites de mandato e manuseio de webhook.
...
A Índia possui uma infraestrutura de pagamento única, dominada por UPI (mais de 60% das transações digitais) e cartões emitidos na Índia (Visa, Mastercard, Rupay, etc.). Pagamentos Dodo suporta todos eles com total conformidade ao RBI para mandatos de assinatura.
...
Ao contrário da maioria dos métodos de pagamento alternativos, UPI e todos os cartões emitidos na Índia (Visa, Mastercard, Rupay, etc.) suportam pagamentos recorrentes via mandatos do RBI.
</Card>
</CardGroup>
...
*Assinaturas exigem mandatos em conformidade com o RBI com regras especiais de processamento. O atraso de processamento de 48 horas se aplica a todos os cartões emitidos na Índia e UPI.
...
| Evento | Quando | Ação |
| :---- | :--- | :----- |
| `subscription.active` | Mandato autorizado | Registrar início da assinatura |
| `payment.succeeded` | ~48h após a data de cobrança | Conceder/continuar acesso |
| `payment.failed` | Débito falhou | Notificar cliente, pausar acesso |
| `subscription.on_hold` | Falha no pagamento | Solicitar atualização do método de pagamento |
| `subscription.active` | Reativado após pagamento | Restaurar acesso |
...
<Accordion title="Mandate cancelled but subscription still active">
**Causa:** Caso de borda nas regulamentações do RBI — cancelamento de mandato durante a janela de processamento não cancela a assinatura imediatamente.
...
<Card title="Subscriptions" icon="repeat" href="/features/subscription">
Documentação completa de assinaturas incluindo mandatos RBI.
</Card>

<Card title="Webhooks" icon="webhook" href="/developer-resources/webhooks">
Manipulação de webhooks para eventos de pagamento.
</Card>
