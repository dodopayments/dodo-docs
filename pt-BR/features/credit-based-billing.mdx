...
Todos os eventos do razão (`credit.added` até `credit.manual_adjustment`) incluem a carga completa `CreditLedgerEntry` com saldo antes/depois, excesso antes/depois, referência de origem e o `metadata` da assinatura ou pagamento da fonte da concessão (vazio para concessões criadas diretamente via API). O evento `credit.balance_low` inclui a configuração de limite e o saldo atual.
