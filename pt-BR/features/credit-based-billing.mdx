...
Defina créditos na sua própria unidade — tokens, chamadas de API, horas de computação ou qualquer métrica relevante para o seu produto. As unidades personalizadas usam a precisão que você definir (de 0 a 10 casas decimais).
...
| Fonte | Descrição |
|--------|-------------|
| **Assinatura** | Créditos emitidos com a compra de uma assinatura e reemitidos a cada ciclo de cobrança |
| **Pagamento único** | Créditos emitidos com um produto de pagamento único |
| **Complemento** | Créditos emitidos com um complemento anexado a uma assinatura |
| **API** | Créditos concedidos manualmente via API ou dashboard |
| **Transferência** | Créditos transferidos de um ciclo de cobrança anterior |
...
- Até `10` - Para unidades de alta precisão (por exemplo, tokens fracionários ou uso em microssegmentos)
...
1. **Seu aplicativo envia eventos de uso** - Cada evento inclui um ID de cliente, nome de evento e metadados
2. **Medições agregam eventos** - Usando Contagem, Soma, Máximo ou Último
3. **Créditos são deduzidos automaticamente** - Um trabalhador em segundo plano processa eventos a cada minuto, converte unidades de medição em créditos usando sua taxa configurada e deduz do saldo do cliente usando FIFO (primeiro a entrar, primeiro a sair)
4. **Excedente é monitorado** - Se o saldo de crédito chegar a zero e o excedente estiver habilitado, o sistema rastreia o uso excedente para faturamento no final do ciclo
...
|-----------------|-------------|
| **Crédito adicionado** | Créditos concedidos (assinatura, pagamento único ou API) |
| **Crédito deduzido** | Créditos consumidos por meio de uso ou débito manual |
| **Crédito expirado** | Créditos expirados sem transferência |
| **Crédito transferido** | Créditos transferidos para o próximo período |
| **Transferência perdida** | Créditos transferidos perdidos após atingir a quantidade máxima de transferências |
| **Cobrança de excedente** | Uso além do saldo de créditos com o excedente habilitado |
| **Redefinição de excedente** | Excedente registrado limpo na redefinição do ciclo de cobrança |
| **Recarga automática** | Reposição automática de créditos quando o saldo está baixo |
| **Ajuste manual** | Crédito ou débito aplicado manualmente pelo merchant |
...
| `credit.deducted` | Créditos consumidos por meio de uso ou débito manual |
| `credit.expired` | Créditos não utilizados expirados |
| `credit.rolled_over` | Créditos transferidos para uma nova concessão |
| `credit.rollover_forfeited` | Créditos perdidos ao atingir a quantidade máxima de transferências |
| `credit.overage_charged` | Cobranças de excedente aplicadas |
| `credit.overage_reset` | O excedente registrado foi redefinido ao final do ciclo de cobrança |
| `credit.manual_adjustment` | Ajuste manual de crédito/débito realizado |
| `credit.balance_low` | O saldo caiu abaixo do limite configurado |

Todos os eventos do ledger (cada evento `credit.*`, exceto `credit.balance_low`) incluem o payload completo `CreditLedgerEntry`, com o saldo antes/depois, o excedente antes/depois, a referência da origem e o `metadata` da assinatura ou do pagamento de origem da concessão (vazio para concessões criadas diretamente via API). O evento `credit.balance_low` inclui a configuração do limite e o saldo atual.
