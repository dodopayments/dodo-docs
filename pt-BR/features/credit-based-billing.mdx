...
- **Comece simples**: Inicie com um único tipo de crédito e sem acúmulo. Adicione complexidade com base no feedback dos clientes e nos padrões de uso.
- **Defina expectativas claras**: Exiba alocações de créditos, saldos restantes e preços de excedente de forma destacada nas páginas do produto e no portal do cliente.
- **Use unidades significativas**: Nomeie os créditos de acordo com o que representam (por exemplo, "API Calls", "AI Tokens") em vez de termos genéricos. Isso ajuda os clientes a entender o valor.
- **Configure a expiração de forma cuidadosa**: Janelas curtas de expiração (7 dias) geram senso de urgência, mas podem frustrar os clientes. Janelas mais longas (30–90 dias) são mais amigáveis para a maioria dos produtos SaaS.
- **Monitore saldos baixos**: Defina limiares de saldo baixo para alertar os clientes antes que se esgotem, reduzindo cobranças inesperadas de excedente.
- **Teste em modo de teste**: Crie créditos, anexe-os a produtos de teste e simule o ciclo completo de compra → uso → dedução → expiração antes de entrar no ar.

<Info>
A cobrança baseada em créditos funciona perfeitamente com todos os outros recursos do Dodo Payments - assinaturas com testes, alterações de planos com rateio e o portal do cliente. Comece com uma configuração básica e expanda conforme seu modelo de preços evolui.
</Info>
