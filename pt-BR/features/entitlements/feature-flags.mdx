...
| Gatilho | Efeito |
|---|---|
| Pagamento único concluído / assinatura ativada | Concessão criada com `status: Delivered` e `delivered_at` definidos. |
| Assinatura colocada em espera, cancelada ou expirada | Concessão revogada com o `revocation_reason` correspondente. |
| Reembolso de um pagamento único | Concessão revogada com `revocation_reason: refund`. |
| Assinatura recuperada (por exemplo, dunning concluído com sucesso) | A concessão revogada é restaurada para `Delivered` — a mesma concessão `id`, com os campos de revogação limpos. |
| Revogação manual pela API | Concessão revogada com `revocation_reason: manual`. Não é restaurada automaticamente na renovação. |
