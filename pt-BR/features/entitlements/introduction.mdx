...
<Steps>
<Step title="Created">
Uma concessão é criada quando um pagamento é concluído ou uma assinatura se torna ativa. Feature flags vão diretamente para `Delivered`. As chaves de licença também vão diretamente para `Delivered` quando o entitlement usa `fulfillment_mode: auto` (o padrão); com `fulfillment_mode: manual`, a concessão é criada em `Pending` sem uma chave, até que você forneça uma por meio de [Conceder chave de licença](/api-reference/entitlements/fulfill-license-key). Todas as outras integrações começam em `Pending`. As integrações baseadas em OAuth (Discord, GitHub, Notion) incluem um `oauth_url` que o cliente deve acessar para concluir o consentimento. As integrações diretamente com a plataforma (Telegram, Framer, Digital Files) permanecem em `Pending` apenas brevemente enquanto a entrega é provisionada e, em seguida, passam para `Delivered`.
</Step>
