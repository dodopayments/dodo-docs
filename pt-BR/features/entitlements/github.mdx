...
2. Dodo Payments cria um grant com status `Pending`. Seu `oauth_url` é `null` até que o cliente inicie o fluxo de aceitação, momento em que ele é preenchido com uma URL de autorização do GitHub.
3. O cliente autoriza por meio do link no e-mail ou no Customer Portal.
4. Dodo Payments convida a conta do GitHub do cliente para o repositório com a permissão configurada. O grant passa para `Delivered` assim que o convite é criado.
5. Se a subscription for cancelada, reembolsada ou revogada, o cliente será removido como colaborador.
...
- `entitlement_grant.created` é disparado com `status: "Pending"` e `oauth_url` ainda como `null`; a URL de autorização do GitHub é preenchida assim que o cliente inicia o fluxo de aceitação pelo e-mail de entrega ou pelo Customer Portal.
- `entitlement_grant.delivered` é disparado assim que o convite ao colaborador é criado.
- `entitlement_grant.failed` é disparado com `error_code: "github_permission_denied"` (ou similar) se o GitHub App perder o acesso ao repositório.
