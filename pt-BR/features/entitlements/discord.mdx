...
2. Dodo Payments cria uma concessão no status `Pending` com um `oauth_url` apontando para o Discord.
3. O cliente recebe um e-mail com um botão "Join the Discord" (o link OAuth). O link também fica visível no Customer Portal.
4. Depois que o cliente autoriza, o bot adiciona o cliente ao servidor e atribui a função configurada. A concessão passa para `Delivered`.
5. Se a assinatura for cancelada, pausada ou expirar, ou se o merchant revogar manualmente, o bot remove a função e a concessão passa para `Revoked`.
...
- `entitlement_grant.delivered` é acionado assim que a função é atribuída.
