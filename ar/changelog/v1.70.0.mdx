...
1. **الدفع المدمج - تجارب دفع مدمجة بالكامل (بيتا)**  
   ...
   قم بتهيئة SDK باستخدام `displayType: 'inline'` وادمجcheckout في عنصر حاوية:
   ...
   - **أحداث تفصيلية في الوقت الحقيقي**: تلقي `checkout.breakdown` الأحداث عند إدخال العملاء تفاصيلهم
   - **ملخصات طلب مخصصة**: أنشئ عرض تسعير خاص بك يتم تحديثه في الوقت الحقيقي
   - **تكامل سلس**: إطار الدفع يدير جمع المدفوعات بينما تتحكم في التصميم
...
2. **إنشاء المدفوعات، الاشتراكات، والعمليات باستخدام طرق الدفع الحالية**  
    قم بتبسيط تدفقات الدفع باستخدام طرق الدفع المحفوظة من العملاء الحاليين. يسمح لك المعامل الجديد `payment_method_id` بإنشاء المدفوعات، الاشتراكات، أو جلسات الدفع باستخدام طريقة الدفع المحفوظة مسبقاً للعميل.
   ...
   <تحذير>
   عند استخدام `payment_method_id` في جلسات الدفع، يجب تعيين `confirm` إلى `true`، ويجب تقديم `customer_id` موجود. سيتم التحقق من صلاحية طريقة الدفع بالنسبة لعملة الدفع.
   </تحذير>
...
5. **`redirect_immediately` العلم - تجاوز صفحة نجاح الدفع**  
   تحكم في تدفق الدفع مع العلم الجديد `redirect_immediately`. عند التمكين، يتم توجيه العملاء مباشرة بعد إتمام الدفع، متجاوزين صفحة النجاح الافتراضية لتجربة أسرع وأكثر سلاسة.
  ...
  ```
   ...
   <نصيحة>
   استخدم `redirect_immediately: true` عندما يكون لديك صفحة نجاح مخصصة تقدم تجربة مستخدم أفضل من صفحة نجاح الدفع الافتراضية.
   </نصيحة>

   <ملاحظة>
   عند تمكين `redirect_immediately`، يتم توجيه العملاء إلى `return_url` مباشرة بعد إتمام الدفع، متجاوزين صفحة النجاح الافتراضية بالكامل.
   </ملاحظة>
