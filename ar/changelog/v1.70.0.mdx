...
1. **الدفع المدمج - تجارب دفع مدمجة بالكامل (بيتا)**  
   ...
   قم بتهيئة SDK باستخدام `displayType: 'inline'` ودمج عملية الدفع في عنصر حاوية:
   ...
   - **أحداث تحليل الوقت الحقيقي**: استقبل `checkout.breakdown` الأحداث عند إدخال العملاء لبياناتهم
   - **ملخصات الطلبات المخصصة**: أنشئ عرض تسعير خاص بك يتم تحديثه في الوقت الحقيقي
   - **تكامل سلس**: إطار الدفع يتعامل مع جمع المدفوعات بينما تتحكم في التخطيط
   - **طرق SDK**: استخدم `open()`، `close()`، و`isOpen()` للتحكم في عملية الدفع برمجياً
...
2. **إنشاء المدفوعات، الاشتراكات، وعمليات الدفع باستخدام طرق الدفع الموجودة**  
    قم بتبسيط تدفقات الدفع باستخدام طرق الدفع المحفوظة من العملاء الحاليين. يسمح لك المعامل الجديد `payment_method_id` بإنشاء المدفوعات، الاشتراكات، أو جلسات الدفع باستخدام طريقة الدفع المحفوظة مسبقًا للعميل.
...
5. **`redirect_immediately` علامة - تخطي صفحة نجاح الدفع**  
   تحكم في تدفق الدفع باستخدام العلامة الجديدة `redirect_immediately`. عند تفعيلها، يتم إعادة توجيه العملاء مباشرة بعد إتمام الدفع، متجاوزين صفحة النجاح الافتراضية لتجربة أسرع وأكثر سلاسة.
  ...
  ```
   ...
   <Note>
   عندما تكون `redirect_immediately` مفعلة، يتم إعادة توجيه العملاء إلى `return_url` الخاصة بك مباشرة بعد إتمام الدفع، متجاوزين صفحة النجاح الافتراضية تمامًا.
   </Note>
