...
<Info>
تُدرج إجابات العملاء تلقائياً في أحمال الويب هوك (`payment.succeeded`, `subscription.created`) واستجابات API عبر مصفوفة `custom_field_responses`. يمكنك تعريف ما يصل إلى 5 حقول مخصصة لكل جلسة دفع.
</Info>
...
<Info>
يغطي هذا القسم تكوين الثيم باستخدام **واجهة برمجة الخادم** عبر `customization.theme_config`. إذا كنت تستخدم **Checkout SDK** (النافذة المنبثقة أو الدفع المدمج)، راجع أقسام تخصيص الثيم في [النافذة المنبثقة للدفع](/developer-resources/overlay-checkout#theme-customization) أو [الدفع المدمج](/developer-resources/inline-checkout#theme-customization) التي تستخدم خصائص camelCase (مثلاً `bgPrimary` بدلاً من `bg_primary`).
</Info>
...
| الخاصية | الوصف |
|----------|-------------|
| `light` | تكوين الألوان للوضع الفاتح |
| `dark` | تكوين الألوان للوضع الداكن |
| `font_primary_url` | عنوان URL للخط الأساسي |
| `font_secondary_url` | عنوان URL للخط الثانوي |
| `font_size` | حجم الخط: `xs`، `sm`، `md`، `lg`، `xl`، `2xl` |
| `font_weight` | سمك الخط: `normal`، `medium`، `bold`، `extraBold` |
| `radius` | نصف قطر الحدود لعناصر الواجهة (مثل `4px`، `0.5rem`، `8px`) |
| `pay_button_text` | نص مخصص لزر الدفع (مثل "إكمال الشراء"، "اشترك الآن") |

### تكوين الألوان (الوضع الفاتح/الداكن)
...
| `text_error` | لون النص عند الخطأ |
| `text_success` | لون النص عند النجاح |
| `border_primary` | لون الحدود الأساسي |
| `border_secondary` | لون الحدود الثانوي |
| `button_primary` | لون خلفية الزر الأساسي |
| `button_primary_hover` | لون التمرير لزر الأساسي |
| `button_secondary` | لون خلفية الزر الثانوي |
| `button_secondary_hover` | لون التمرير لزر الثانوي |
...
- هكس: `#fff`، `#ffffff`، `#ffffffff`
- RGB/RGBA: `rgb(255, 255, 255)`، `rgba(255, 255, 255, 0.5)`
- HSL/HSLA: `hsl(120, 100%, 50%)`، `hsla(120, 100%, 50%, 0.5)`
- الأسماء المعروفة: `red`، `blue`، `transparent`
</Info>
...
<Tip>
لا تحتاج إلى تحديد كل خصائص الألوان. سيتم استخدام القيم الافتراضية للثيم لأي خصائص غير محددة.
</Tip>
