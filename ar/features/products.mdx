...
قياس الاستهلاك والفاتورة وفقًا للاستخدام الفعلي.
</Card>
</CardGroup>
...
<Frame>
  <img src="/images/products.png" alt="المنتجات" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
- **الخصم (%)**: خصم اختياري يُظهر أثناء الخروج وفي الفواتير.
- للاشتراكات، قم بضبط **التكرار كل** (مثل شهر واحد أو سنة واحدة) و**أيام التجربة** إذا لزم الأمر. يجب أن يكون سعر الاشتراك على الأقل **$1** (أو ما يعادله بعملتك المختارة)؛ المبالغ الأقل من هذا الحد الأدنى غير مدعومة ولن يعمل الاشتراك.
...
<Frame>
  <img src="/images/product-collection/checkout-page.png" alt="مجموعات المنتجات" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
بالنسبة للمنتجات المتكررة، قم بتعيين الكائن `price` إلى سعر متكرر (`type: recurring_price`) وحدد `payment_frequency_interval` و`subscription_period_interval` (`Day`، `Week`، `Month`، أو `Year`) مع الأعداد المقابلة لها.
...
إذا لم تعد ترغب في عرض أو استخدام منتج، يمكنك أرشفته باستخدام طلب `DELETE` إلى نقطة النهاية `/products/{id}`. هذا الإجراء يخفي المنتج لكنه لا يحذفه بشكل دائم.
