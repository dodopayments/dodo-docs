...
## مفاتيح الترخيص في عنوان URL العودة

عندما يكمل العميل عملية شراء لمنتج مع تمكين مفاتيح الترخيص، يتم إلحاق مفتاح الترخيص تلقائيًا إلى `return_url` كمعامل استعلام. يتيح لك ذلك عرض المفتاح فورًا على صفحة النجاح الخاصة بك بدون إجراء استدعاء API إضافي.

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001&email=customer%40example.com
```

إذا كانت عملية الشراء تولد مفاتيح ترخيص متعددة (على سبيل المثال، كمية > 1)، فإنها تكون مفصولة بفواصل:
...
<Tip>
قم بتحليل معيار `license_key` على صفحة العودة الخاصة بك لعرض المفتاح فورًا للعملاء، مما يحسن تجربة ما بعد الشراء.
</Tip>

## أفضل الممارسات

- **حافظ على الحدود واضحة**: اختر افتراضات منطقية للانتهاء والتفعيل
- **إرشاد المستخدمين**: قدم تعليمات تفعيل دقيقة ووثائق خدمة ذاتية
- **أتمتة الفحوصات**: تحقق من المفاتيح من جانب الخادم قبل منح الوصول
- **مراقبة الأحداث**: استخدم webhooks لاكتشاف الإساءة وأتمتة الإلغاءات
