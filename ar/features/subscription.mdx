...
| فشل التجديد ← توقف مؤقت | `subscription.on_hold` |
...
| أي تغييرات على الحقول | `subscription.updated` |
...
استخدم `PATCH /subscriptions/{id}` للإلغاء في تاريخ الفوترة التالي، أو لتمديد فترة الاشتراك، أو لتحديث تفاصيل الفوترة، أو لتعديل البيانات الوصفية. لتغيير الكمية، استخدم [Change Plan API](/api-reference/subscriptions/change-plan) بدلًا من ذلك — لا يقبل `PATCH` قيمة `quantity`.
...
### تمديد فترة الاشتراك
مدّد مدة تشغيل الاشتراك من خلال تمرير `subscription_period_count` و`subscription_period_interval` جديدين إلى `PATCH /subscriptions/{id}`. تتم إعادة حساب انتهاء الاشتراك من العدد والفاصل الجديدين — على سبيل المثال، لمنح العميل وقتًا إضافيًا على خطته الحالية:
