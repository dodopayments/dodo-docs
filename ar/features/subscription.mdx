...
يتم تنفيذ الإيقاف المؤقت والاستئناف عبر الحقل `status` في endpoint تحديث الاشتراك. لا توجد نقطة نهاية منفصلة للإيقاف المؤقت.
...
<Warning>
أرسل `paused` أو `active` منفردًا — إذ يؤدي دمج أي منهما مع أي حقل آخر إلى رفض الطلب مع `422`. تمت إزالة الحقل المنطقي الأقدم `pause`، وهو يفشل الآن دائمًا مع `422`، لذا يحصل المستدعي الذي لا يزال يستخدمه على خطأ واضح بدلًا من عدم تنفيذ العملية بصمت.
</Warning>
...
| الحالة | ما الذي تعنيه | هل يمكن استردادها؟ | مسار الاسترداد / الخطوة التالية |
|--------|---------------|--------------|---------------------------|
| `pending` | جارٍ إنشاء الاشتراك أو معالجته | — | انتظر وصوله إلى `subscription.active` (أو `subscription.failed`) |
| `active` | الاشتراك نشط وسيُجدَّد تلقائيًا | — | لا يلزم اتخاذ أي إجراء |
| `on_hold` | فشلت دفعة التجديد (أو الرسوم الناتجة عن تغيير الخطة)؛ وقد توقفت عمليات التجديد | **نعم** | حدّث طريقة الدفع لإعادة التفعيل — تلقائيًا عبر [إعادة محاولات الدفع](/features/recovery/payment-retries) و[التحصيل من الاشتراك](/features/recovery/subscription-dunning)، أو يدويًا عبر [واجهة Update Payment Method API](/api-reference/subscriptions/update-payment-method) |
| `paused` | تم إيقاف الاشتراك مؤقتًا عمدًا من جانبك أو جانب العميل؛ وتم تجميد الفوترة وإلغاء الوصول | **نعم** | استأنفه باستخدام `status: active`، أو من Customer Portal عند تفعيل الإيقاف المؤقت بالخدمة الذاتية — راجع [إيقاف الاشتراكات مؤقتًا واستئنافها](#pausing-and-resuming-subscriptions) |
| `cancelled` | تم إلغاء الاشتراك ولن يُجدَّد | إعادة الشراء فقط | يجب على العميل بدء اشتراك جديد؛ ويمكن لـ[التحصيل من الاشتراك](/features/recovery/subscription-dunning) أن يطلب إعادة الشراء |
| `failed` | فشل **إنشاء** الاشتراك (لم ينجح التفويض الأولي أو الدفع الأولي) | **لا — نهائي** | يجب على العميل إنشاء اشتراك جديد باستخدام طريقة دفع صالحة |
...
يتم تنفيذ الإيقاف المؤقت والاستئناف عبر endpoint `PATCH /subscriptions/{subscription_id}` نفسه باستخدام الحقل `status`: إذ يؤدي `status: paused` إلى إيقاف اشتراك نشط مؤقتًا، بينما يؤدي `status: active` إلى استئنافه. لا يمكن دمج أي من القيمتين مع أي حقل آخر في الطلب نفسه. للاطلاع على السلوك الكامل وتأثيرات الفوترة وإعدادات الأعمال ذات الصلة، راجع [إيقاف الاشتراكات مؤقتًا واستئنافها](#pausing-and-resuming-subscriptions).

<Card title="API Reference" icon="code" href="/api-reference/subscriptions/patch-subscriptions">
اطّلع على API تحديث الاشتراك، بما في ذلك الحقل `status`.
</Card>
</Accordion>
