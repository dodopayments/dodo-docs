...
sidebarTitle: الهند
description: اقبل UPI وكل البطاقات الصادرة في الهند في الهند مع الامتثال
  لمتطلبات الاشتراك الخاصة بمصرف الهند الاحتياطي (RBI). فهم تأخير المعالجة الذي
  يمتد 48 ساعة، وحدود التفويض، وتعامل webhook.
...
تمتلك الهند بنية تحتية فريدة للدفع تهيمن عليها UPI (أكثر من 60% من المعاملات الرقمية) والبطاقات الصادرة في الهند (Visa, Mastercard, Rupay, إلخ). يدعم Dodo Payments كل هذه البطاقات مع الامتثال الكامل لمتطلبات الاشتراك الخاصة بمصرف الهند الاحتياطي (RBI).
...
خلافًا لمعظم طرق الدفع البديلة، تدعم UPI وكل البطاقات الصادرة في الهند (Visa, Mastercard, Rupay, إلخ) المدفوعات المتكررة عبر تفويضات RBI.
</Card>
</CardGroup>
...
*تتطلب الاشتراكات تفويضات متوافقة مع متطلبات مصرف الهند الاحتياطي (RBI) مع قواعد معالجة خاصة. ينطبق تأخير المعالجة البالغ 48 ساعة على جميع البطاقات الصادرة في الهند و UPI.
...
| الحدث | متى | الإجراء |
| :---- | :--- | :----- |
| `subscription.active` | تفويض مُصدق | تسجيل بدء الاشتراك |
| `payment.succeeded` | بعد نحو ٤٨ ساعة من تاريخ التحصيل | منح/استمرار الوصول |
| `payment.failed` | فشل السحب | إخطار العميل، إيقاف الوصول |
| `subscription.on_hold` | فشل الدفع | طلب تحديث وسيلة الدفع |
| `subscription.active` | إعادة التنشيط بعد الدفع | استعادة الوصول |
...
<Accordion title="Payment deduction delayed beyond 48 hours">
**السبب:** يمكن أن تؤدي تأخيرات واجهة برمجة تطبيقات البنك إلى تمديد المعالجة بمقدار 2-3 ساعات إضافية.

**الحل:** هذا متوقع. قم ببناء نظامك للتعامل مع التأخيرات المتغيرة حتى إجمالي ~51 ساعة.
</Accordion>

<Accordion title="Mandate cancelled but subscription still active">
**السبب:** حالة خاصة في لوائح RBI — إلغاء التفويض أثناء نافذة المعالجة لا يلغي الاشتراك فورًا.

**الحل:** ستفشل الرسوم التالية وسينتقل الاشتراك إلى `on_hold`. راقب الشبكات للأحداث الخاصة بـ `payment.failed`.
</Accordion>
</AccordionGroup>
...
<Card title="Subscriptions" icon="repeat" href="/features/subscription">
التوثيق الكامل للاشتراكات بما في ذلك تفويضات RBI.
</Card>

<Card title="Webhooks" icon="webhook" href="/developer-resources/webhooks">
معالجة الشبكات لأحداث الدفع.
</Card>
