...
1. **التقاط معرف الزائر من DataFast** من ملف تعريف الارتباط `datafast_visitor_id` عند إنشاء جلسات الدفع
2. **تخزين معرف الزائر** في بيانات الدفع الخاصة بك
3. **إرسال بيانات الدفع إلى DataFast** عند نجاح المدفوعات باستخدام واجهة برمجة التطبيقات للدفع الخاصة بهم
...
<خطوات>
<خطوة العنوان="تثبيت سكربت DataFast">
  أولاً، قم بتثبيت سكربت تتبع DataFast على موقع الويب الخاص بك. هذا ينشئ ملف تعريف الارتباط `datafast_visitor_id` الذي يتتبع زوارك.
  
  تفضل بزيارة [وثائق DataFast](https://datafa.st/docs) للحصول على تعليمات التثبيت الخاصة بمنصتك.
</خطوة>
...
<AccordionGroup>
<Accordion title="عدم ظهور المدفوعات في DataFast">
- تحقق من أن مفتاح API لـ DataFast صحيح وفعال
...
- تحقق من لوحة معلومات DataFast للحصول على أي رسائل خطأ أو سجلات API
</Accordion>

<Accordion title="عدم عمل نسبة الإيرادات">
- تأكد من تثبيت سكربت تتبع DataFast وعمله على موقع الويب الخاص بك
- تحقق من أنه يتم تعيين ملف تعريف الارتباط `datafast_visitor_id` بشكل صحيح
- تحقق من تطابق معرفات الزائر بين إنشاء الخروج وإتمام الدفع
- تأكد من أنك تلتقط معرف الزائر قبل إنشاء جلسة الخروج
- راجع [وثائق واجهة برمجة التطبيقات للدفع من DataFast](https://datafa.st/docs/payments-api) للحصول على مزيد من الإرشادات
</Accordion>

<Accordion title="أخطاء التحويل">
- تحقق من أن بنية JSON تتطابق مع تنسيق واجهة برمجة التطبيقات للدفع من DataFast
- تحقق من وجود جميع الحقول المطلوبة (`amount`، `currency`، `transaction_id`، `datafast_visitor_id`)
- تأكد من تحويل المبلغ بشكل صحيح (قسمه على 100 لمعظم العملات، باستثناء العملات ذات الفواصل العشرية الصفرية)
- تحقق من أن عنوان URL لنقطة نهاية API صحيح: `https://datafa.st/api/v1/payments`
- اختبر التحويل باستخدام عينات حمولة الويب
</Accordion>
