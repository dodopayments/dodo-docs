...
يمكن لـ `dodo wh trigger` إرسال حمولات وهمية لجميع أنواع الأحداث البالغ عددها 46 نوعًا التي يرسلها Dodo Payments:

| الفئة | الأحداث |
|----------|--------|
| **الاشتراك** | `active`, `updated`, `on_hold`, `paused`, `renewed`, `plan_changed`, `cancelled`, `failed`, `expired`, `update_payment_method` |
| **الدفع** | `succeeded`, `failed`, `processing`, `cancelled` |
| **المبلغ المسترد** | `succeeded`, `failed` |
| **النزاع** | `opened`, `expired`, `accepted`, `cancelled`, `challenged`, `won`, `lost` |
| **مفتاح الترخيص** | `created` |
| **التحويل** | `created`, `in_progress`, `on_hold`, `failed`, `success` |
| **الرصيد الدائن** | `added`, `deducted`, `expired`, `rolled_over`, `rollover_forfeited`, `overage_charged`, `overage_reset`, `manual_adjustment`, `balance_low` |
| **إتمام شراء متروك** | `detected`, `recovered` |
| **تحصيل الديون** | `started`, `recovered` |
| **منح الاستحقاق** | `created`, `delivered`, `failed`, `revoked` |

<Note>
تتبع أشكال الحمولات الوهمية المخططات المقابلة لها في مرجع API. راجع [أحداث Webhook](/developer-resources/webhooks/intents/webhook-events-guide) لمعرفة معنى كل حدث ومتى يصدره Dodo Payments في بيئة الإنتاج.
</Note>

<Tip>
يصدر `payout.created` بينما لا تزال حالة التحويل مُسجّلة على أنها `not_initiated`، ولذلك تعكس الحمولة الوهمية ذلك أيضًا. راجع [أحداث التحويل](/developer-resources/webhooks/intents/payout) للاطلاع على دورة حياة التحويل الكاملة.
</Tip>
...
| المتغير | الوصف |
|----------|-------------|
| `DODO_WH_TEST_SERVER_URL` | تجاوز عنوان URL الافتراضي لخادم ترحيل Webhook الذي يستخدمه `dodo wh listen` |

## التحديثات

يتحقق CLI من وجود إصدار أحدث عند بدء التشغيل، ويعرض إشعارًا في شريط الحالة عند توفر إصدار جديد. للترقية:

```text
/update
```

أو أعد تشغيل برنامج التثبيت لإجراء الترقية في مكانه:
...
- **GitHub**: افتح issue في [المستودع](https://github.com/dodopayments/dodopayments-cli/issues)
