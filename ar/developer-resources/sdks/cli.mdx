...
`dodo wh trigger` يمكنه إرسال حمولات وهمية لجميع أنواع الأحداث الـ47 التي يوفّرها Dodo Payments:

| الفئة | الأحداث |
|----------|--------|
| **الاشتراك** | `active`, `updated`, `on_hold`, `paused`, `unpaused`, `renewed`, `plan_changed`, `cancelled`, `failed`, `expired`, `update_payment_method` |
| **الدفع** | `succeeded`, `failed`, `processing`, `cancelled` |
| **استرداد الأموال** | `succeeded`, `failed` |
...
| **الدفع المتروك** | `detected`, `recovered` |
