...
`dodo wh trigger` يمكنه إرسال حمولات وهمية لأنواع الأحداث الـ22 هذه:
...
<Note>
هذه مجموعة فرعية من الأحداث التي يرسلها Dodo Payments. لا يتوفر بعد مولّد وهمي لكل من `payout.*` و`credit.*` و`abandoned_checkout.*` و`dunning.*` و`entitlement_grant.*` و`subscription.paused` و`subscription.update_payment_method` — اختبرها باستخدام نشاط حقيقي في وضع الاختبار و`dodo wh listen`. راجع [أحداث Webhook](/developer-resources/webhooks/intents/webhook-events-guide) للاطلاع على القائمة الكاملة.
</Note>
...
| المتغير | الوصف |
|----------|-------------|
| `DODO_WH_TEST_SERVER_URL` | تجاوز عنوان URL الافتراضي لخادم ترحيل Webhook المستخدم بواسطة `dodo wh listen` |
...
أو أعد تشغيل المُثبّت لإجراء الترقية في المكان نفسه:
...
- **GitHub**: أبلغ عن مشكلة في [المستودع](https://github.com/dodopayments/dodopayments-cli/issues)
