...
<Card title="Context Support" icon="clock">
  دعم كامل لـ `context.Context` للإلغاء وانتهاء المهلات
</Card>
...
<Warning>
`POST /subscriptions` (طريقة `subscriptions.create` في SDK) **مهملة**. لا تزال تعمل مع عمليات التكامل الحالية، ولكن يجب أن تنشئ عمليات التكامل الجديدة الاشتراكات من خلال [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
يتطلب `Billing` على الأقل رمز `Country` وفق معيار ISO مكوّنًا من حرفين. `Customer` هو `CustomerRequestUnionParam` — مرّر `AttachExistingCustomerParam{CustomerID: ...}` لعميل حالي أو `NewCustomerParam{Email: ..., Name: ...}` لعميل جديد. تكون قيمة `ProductPrice` بأدنى فئة من فئات العملة.
</Info>
...
عندما تُرجع API رمز حالة غير ناجح، يُرجع SDK خطأً من النوع
`*dodopayments.Error`. ويكشف عن `StatusCode` و`*http.Request` الأساسي و
`*http.Response`، بالإضافة إلى JSON لنص الخطأ. استخدم نمط `errors.As`
لفحصه، وتفرّع استنادًا إلى `StatusCode` للتعامل مع حالات محددة:
...
أضف Middleware مخصّصًا للتسجيل أو المقاييس:
...
هل تحتاج إلى مساعدة بشأن Go SDK؟
...
- **GitHub**: افتح مشكلة في [المستودع](https://github.com/dodopayments/dodopayments-go)

## المساهمة

نرحّب بالمساهمات! راجع [إرشادات المساهمة](https://github.com/dodopayments/dodopayments-go/blob/main/CONTRIBUTING.md) للبدء.
