...
تستخدم الـ SDK معلمات مسماة لتحديد الوسائط الاختيارية. يمكنك تهيئة كائنات القيمة باستخدام الباني الثابت `with`:
...
عندما لا يستطيع المكتبة الاتصال بالـ API أو تتلقى رمز حالة غير ناجح (4xx أو 5xx)، يتم رمي فئة فرعية من `APIException`:
...
<Note>
  تعتبر الـ `extra*` المعلمات بنفس الاسم تجاوزًا للمعلمات الموثقة.
</Note>
...
أضف تكوين في `config/services.php`:
...
قم بالتسجيل في `config/services.yaml`:
