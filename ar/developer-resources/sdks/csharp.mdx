...
<Info>
يتطلب `Billing` الحد الأدنى المتمثل في رمز ISO المكوّن من حرفين `Country`. استخدم `AttachExistingCustomer` لإرفاق عميل موجود، أو `NewCustomer` لإنشاء عميل. تكون قيمة `ProductPrice` بوحدة العملة الأصغر.
</Info>

## معالجة الأخطاء

يطرح SDK استثناءات محددة استنادًا إلى رمز حالة HTTP. وترث جميع أخطاء 4xx من `DodoPayments4xxException`.
...
- `DodoPaymentsIOException`: أخطاء الشبكة والإدخال/الإخراج
- `DodoPaymentsInvalidDataException`: تعذّر تفسير البيانات التي تم تحليلها
...
كرّر على جميع النتائج عبر كل الصفحات باستخدام طريقة `Paginate`، التي تُرجع `IAsyncEnumerable`:
...
سجّل العميل في حاوية DI الخاصة بك:
...
<Tip>
لأغراض التطوير، استخدم [user secrets](https://learn.microsoft.com/en-us/aspnet/core/security/app-secrets) بدلًا من تخزين المفاتيح في `appsettings.json`:
