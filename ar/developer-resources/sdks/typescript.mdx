...
<Info>
يجب أن تكون لديك قيم فريدة لـ `event_id` لأحداث idempotency. سيتم رفض المعرفات المكررة ضمن نفس الطلب، وستتم تجاهل الطلبات اللاحقة مع المعرفات الموجودة.
</Info>
...
- `'warn'` - عرض التحذيرات والأخطاء (الوضع الافتراضي)
...
تتم عملية الصفحات لطرق القائمة في واجهة برمجة تطبيقات DodoPayments. يمكنك استخدام صيغة `for await … of` للتنقل عبر العناصر عبر جميع الصفحات:
...
- متصفحات الويب (Chrome وFirefox وSafari وEdge وغيرها من المتصفحات الحديثة)
- Node.js 20 LTS أو إصدار أحدث ([غير EOL](https://endoflife.date/nodejs))
- Deno v1.28.0 أو أعلى
- Bun 1.0 أو إصدار أحدث
- Cloudflare Workers
- Vercel Edge Runtime
- Jest 28 أو أعلى مع البيئة `"node"`
- Nitro v2.6 أو أعلى
