...
1. **تحديد حدود السعر**: تحديد سعر أدنى (مطلوب) وإمكانية تحديد سعر أقصى
2. **تمرير المبالغ الديناميكية**: إدخال حقل `amount` في سلة المنتج عند إنشاء جلسات الدفع
3. **دع العملاء يختارون**: إذا لم يتم تقديم مبلغ، يمكن للعملاء إدخال سعرهم الخاص (ضمن حدودك)

<Info>
عند تمرير `amount` في سلة المنتج، يتم استخدام ذلك المبلغ للدفع. إذا تم حذف حقل `amount`، يمكن للعملاء اختيار سعرهم الخاص أثناء الدفع (بما يتماشى مع الإعدادات الدنيا/القصوى الخاصة بك).
</Info>
...
اضغط على **إضافة منتج** لحفظه. لاحظ معرف المنتج الخاص بك (على سبيل المثال، `pdt_123abc456def`) للاستخدام في جلسات الدفع.
</Step>
</Steps>
...
بمجرد تكوين منتجك مع "ادفع ما تريد"، يمكنك إنشاء جلسات دفع بمبالغ ديناميكية. يسمح حقل `amount` في سلة المنتج بتعيين السعر برمجيًا لكل جلسة دفع.

### فهم حقل المبلغ

عند إنشاء جلسة دفع، يمكنك تضمين حقل `amount` في كل عنصر من عناصر سلة المنتج:

- **إذا تم تزويد `amount`**: يتم استخدام هذا المبلغ بالضبط للدفع (يجب أن يكون ضمن حدودك الدنيا/القصوى)
- **إذا تم حذف `amount`**: يمكن للعملاء إدخال سعرهم الخاص أثناء الدفع (ضمن حدودك)

<Warning>
حقل `amount` ينطبق فقط على المنتجات التي تدعم "ادفع ما تريد". بالنسبة للمنتجات العادية، يتم تجاهل هذا الحقل.
</Warning>
...
<Note>
**تنسيق المبلغ**: يجب أن يكون حقل `amount` في أقل وحدة من العملة. بالنسبة للدولار الأمريكي، يعني ذلك السنتات (على سبيل المثال، 25.00 دولار = `2500`). بالنسبة للعملات الأخرى، استخدم أصغر وحدة (مثل بايسي للروبية الهندية).
</Note>

## الخطوة 3: السماح للعملاء باختيار سعرهم

إذا كنت ترغب في أن يختار العملاء سعرهم الخاص أثناء الدفع، ببساطة احذف حقل `amount` من سلة المنتج. ستظهر صفحة الدفع حقل إدخال حيث يمكن للعملاء إدخال أي مبلغ ضمن حدودك الدنيا والقصوى.
...
استخدم البيانات الوصفية لتتبع سبب اختيار مبالغ محددة (على سبيل المثال، `pricing_tier`، `discount_code`، `user_segment`).
</Card>
...
- أن المنتج هو **دفع لمرة واحدة**، وليس اشتراكًا
- أن المبلغ بتنسيق صحيح (أقل وحدة من العملة، على سبيل المثال، سنتات للدولار الأمريكي)
</Accordion>

<Accordion title="المبلغ يتجاوز الحد الأقصى أو أدنى من الحد الأدنى">
سيرفض API جلسات الدفع حيث ينتهك المبلغ حدود سعر المنتج. تحقق دائمًا من المبالغ قبل إنشاء جلسات الدفع، أو دع العملاء يختارون سعرهم من خلال حذف حقل `amount`.
</Accordion>

<Accordion title="لا يمكن للعميل إدخال سعره الخاص">
إذا كان العملاء لا يرون حقل إدخال السعر، تأكد من أنك حذفت حقل `amount` من سلة المنتج. عندما يتم تقديم `amount`، يستخدم الدفع ذلك المبلغ بالضبط.
</Accordion>
</AccordionGroup>