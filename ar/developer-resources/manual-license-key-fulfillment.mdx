...
يستعرض هذا الدليل كيفية بناء نظام **لتسليم مفاتيح الترخيص يدويًا** من البداية إلى النهاية. بدلًا من أن يتولى Dodo Payments إنشاء مفتاح تلقائيًا عند الدفع، ينشئ كل شراء grant من النوع `Pending` وينتظر منك تزويده بقيمة المفتاح من نظامك الخاص أو من مورّد تابع لجهة خارجية أو من مجموعة محدودة من الرموز.
...
- مفتاح API الخاص بك (`DODO_PAYMENTS_API_KEY`) ومفتاح سر webhook من لوحة التحكم. راجع [دليل إنشاء مفتاح API](/api-reference/introduction#api-key-management-and-authentication).
- نقطة نهاية backend يمكنها استقبال webhooks.
...
<Frame>
<img src="/images/entitlements/license-keys/create.png" alt="نموذج استحقاق مفتاح ترخيص جديد يتضمن الاسم ووضع التسليم ومدة الترخيص وحدّ عمليات التفعيل ورسالة التفعيل" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
<Step title="Set Fulfillment Mode to Manual">
افتح القائمة المنسدلة **Fulfillment Mode** وغيّرها من **Automatic** إلى **Manual**. هذا هو الإعداد الذي يعتمد عليه هذا الدليل بالكامل — فبدونه يتم إنشاء المفاتيح وإرسالها بالبريد الإلكتروني تلقائيًا، ولا يتم إنشاء grant معلّق. عند تحديد **Manual**، ينشئ كل شراء grant من النوع `Pending` لتتولى تسليمه. انقر على **Create Entitlement** للحفظ.
</Step>
</Steps>
</Tab>
...
<Note>
وضع التسليم خاصية من خصائص **entitlement**، وليس المنتج. وبما أنك عيّنته على **Manual** في الخطوة 1، فإن كل منتج مرتبط بهذا الاستحقاق ينشئ grants لمفاتيح الترخيص من النوع `Pending` عند الشراء — ولا حاجة إلى إعداد أي شيء إضافي هنا.
</Note>
...
عندما يشتري العميل المنتج، ينشئ Dodo Payments grant بالحالة `Pending` **من دون إرفاق مفتاح**، ويرسل webhook من النوع `entitlement_grant.created`. هذه هي الإشارة إلى أن عميلًا ينتظر مفتاحًا.
...
عند النجاح، ينتقل grant إلى `Delivered`، ويتم إرسال المفتاح إلى العميل تلقائيًا (وهو نفس البريد الإلكتروني الذي كان سيتلقاه في حالة التسليم التلقائي)، ويتم إطلاق أحداث webhook من النوعين `license_key.created` و`entitlement_grant.delivered`.
...
|---|---|---|
| `200` | تم تسليم المفتاح، وأصبح grant الآن في الحالة `Delivered`. | تم. |
| `400` | ليس grant لمفتاح ترخيص، أو أن المفتاح فارغ/يتكون من مسافات بيضاء. | أصلح الطلب؛ ولا تعاود المحاولة كما هو. |
| `404` | لا يوجد grant بهذا المعرّف لنشاطك التجاري. | تحقّق من `grant_id`. |
| `409` | لا ينتظر grant التسليم (تم تسليمه بالفعل أو يحتوي على مفتاح بالفعل)، **أو أن قيمة المفتاح موجودة مسبقًا**. | إذا كان قد تم تسليمه بالفعل، فاعتبر العملية ناجحة. وإذا كان المفتاح مكررًا، فأرسل مفتاحًا مختلفًا. |
| `422` | فشل التحقق من صحة نص الطلب (مثل `activations_limit < 1`). | صحّح الحقل ثم أعد المحاولة. |
...
1. اشترِ المنتج في وضع الاختبار (راجع [أدلة إتمام الدفع](/developer-resources/integration-guide)).
2. تأكد من أن webhook الخاص بك استلم `entitlement_grant.created` مع `status: "Pending"` و`integration_type: "license_key"`، أو من ظهور grant في استجابة List Grants باستخدام عوامل التصفية نفسها.
3. استدعِ نقطة نهاية fulfill باستخدام مفتاح اختباري.
4. تأكد من أن الاستجابة تعرض `status: "Delivered"` مع `license_key` مملوءًا، وأن العميل استلم رسالة البريد الإلكتروني التي تتضمن المفتاح، وأن `entitlement_grant.delivered` قد تم إطلاقه.
