...
قم بإنشاء عداد في لوحة [Dodo Payments](https://app.dodopayments.com/):
- **اسم الحدث**: `time_range_usage` (أو اسم تفضله)
- **نوع التجميع**: `sum` لتتبع إجمالي المدة
- **على الخاصية**: `durationSeconds`، `durationMinutes`، أو `durationMs`
</Step>
...
<Warning>
**توقيت دقيق**: استخدم `Date.now()` أو `performance.now()` لتتبع الوقت بدقة، خاصةً للوظائف الخالية من الخادم.
</Warning>
