...
قم بإنشاء عداد في لوحة تحكم [دودو بايمنتس](https://app.dodopayments.com/):
- **اسم الحدث**: `api_call` (أو الاسم المفضل لديك)
- **نوع التجميع**: `count` لتتبع عدد المكالمات
- قم بتكوين خصائص إضافية إذا كنت تتبع بيانات وصفية مثل أوقات الاستجابة، رموز الحالة، إلخ.
</Step>
...
<Tip>
**استخدم التجميع للأحجام العالية**: للتطبيقات التي تتعامل مع أكثر من 10 طلبات في الثانية، استخدم `createBatch()` لتقليل الحمل وزيادة الأداء.
</Tip>

<Warning>
**تأكد دائماً من تنظيف الدفعات**: استدعِ `batch.cleanup()` عند إيقاف التطبيق لتفريغ الأحداث المعلقة ومنع فقدان البيانات.
</Warning>
