...
keywords:
  - Dodo Payments
  - Midjourney
  - الفوترة في Midjourney
  - تسعير الصور بالذكاء الاصطناعي
  - نموذج الفوترة
  - نموذج الفوترة Midjourney
---
...
async function handleGenerationRequest(customerId: string, prompt: string) {
  ...
  if (usage.current >= threshold) {
    if (subscription.product_id === 'pdt_basic') {
...
const session = await client.checkoutSessions.create({
  product_cart: [
    { product_id: 'pdt_extra_gpu_hour', quantity: 5 }
  ],
...
const session = await client.checkoutSessions.create({
  product_cart: [
    { product_id: 'pdt_mj_standard', quantity: 1 }
  ],
