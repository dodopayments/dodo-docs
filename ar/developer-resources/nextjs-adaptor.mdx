...
يثبّت المبلغ المُحصّل بوحدات العملة الأساسية (مثلًا، <code>12.5</code> مقابل 12.50 دولارًا). لمنتجات Pay What You Want فقط، ويتم تجاهله إذا كان أقل من الحد الأدنى لسعر المنتج.
</ParamField>
...
<Warning>
يعمل Dynamic Checkout كوكيل لنقطتي النهاية المهجورتين `POST /payments` و`POST /subscriptions`. ويستمر في العمل مع عمليات التكامل الحالية، ولكن ينبغي لعمليات التكامل الجديدة استخدام **Checkout Sessions** أدناه.
</Warning>

### تنسيق الاستجابة
يعرض Dynamic Checkout استجابة JSON تتضمن عنوان URL لصفحة الدفع:
...
توفر Checkout Sessions تجربة دفع مستضافة وأكثر أمانًا، تتولى تدفق الدفع الكامل لكل من عمليات الشراء لمرة واحدة والاشتراكات، مع تحكم كامل في خيارات التخصيص.
...
### تنسيق الاستجابة
تعرض Checkout Sessions استجابة JSON تتضمن عنوان URL لصفحة الدفع:
...
يتيح لك معالج مسار Customer Portal دمج بوابة عملاء Dodo Payments بسلاسة في تطبيق Next.js الخاص بك.
...
إذا تم تعيينها إلى <code>true</code>، يرسل بريدًا إلكترونيًا إلى العميل يتضمن رابط البوابة.
</ParamField>
...
- **الطريقة:** لا يدعم سوى طلبات POST. وتُرجع الطرق الأخرى الحالة 405.
- **التحقق من التوقيع:** يتحقق من توقيع webhook باستخدام <code>webhookKey</code>. ويُرجع الحالة 401 إذا فشل التحقق.
- **التحقق من Payload:** يتم التحقق باستخدام Zod. وتُرجع الحمولة غير الصالحة الحالة 400.
- **معالجة الأخطاء:**
  - 401: توقيع غير صالح
  - 400: حمولة غير صالحة
  - 500: خطأ داخلي أثناء التحقق
- **توجيه الأحداث:** يستدعي معالج الأحداث المناسب استنادًا إلى نوع الحمولة.
