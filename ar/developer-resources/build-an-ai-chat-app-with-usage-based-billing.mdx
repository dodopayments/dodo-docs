...
**اسم العداد** → `AI Token Usage Meter`
...
<تحذير>
يجب أن يتطابق اسم الحدث `ai_chat_usage` بالضبط مع ما سنرسله من كود تطبيقنا لاحقًا. أسماء الأحداث حساسة لحالة الأحرف!
</تحذير>
</خطوة>
...
<معلومات>
نحن نستخدم "المجموع" لأننا نريد جمع جميع الرموز المستهلكة عبر رسائل الدردشة المتعددة. يقوم SDK تلقائيًا بإرسال `totalTokens` في كل حدث.
</معلومات>
</خطوة>
...
<خطوات>
<خطوة عنوان="احصل على مفتاح API لمدفوعات Dodo">
1. في لوحة معلومات مدفوعات Dodo، انتقل إلى **المطورون** → **مفاتيح API**
2. انقر على **إنشاء مفتاح API**
3. انسخ مفتاح API - سيبدو كـ `test_abc123...`

<ملاحظة>
احفظ مفتاح API هذا - سنضيفه إلى ملفنا `.env` لاحقًا.
</ملاحظة>
</خطوة>
...
<ملاحظة>
احتفظ بهذا المفتاح آمنًا - سنضيفه أيضًا إلى ملفنا `.env`.
</ملاحظة>
</خطوة>
</خطوات>
...
بالإضافة إلى ذلك، قم بتعيين **السعر الثابت** إلى `0` للتأكد من أن العملاء يتم تحصيلهم فقط بناءً على استخدامهم، دون أي رسوم أساسية.
...
**السعر لكل وحدة**: أدخل → `0.00001` (هذا هو 0.01 دولار لكل 1000 رمز أو 0.00001 دولار لكل رمز)

**الحد المجاني**: أدخل → `10000` (الحصول على 10,000 رمز مجاني شهريًا)
...
<خطوة عنوان="ابحث عن معرف العميل الخاص بك">
1. عد إلى لوحة معلومات مدفوعات Dodo
2. انتقل إلى **المبيعات** -> **العملاء** في الشريط الجانبي الأيسر
3. ابحث عن العميل الذي قمت بإنشائه للتو (بالبريد الإلكتروني التجريبي)
4. انسخ معرف العميل - سيبدو كـ `cus_123`
...
<خطوة عنوان="تهيئة TypeScript">
قم بإنشاء `tsconfig.json`:
...
قم بتحديث `package.json` لإضافة نوع الوحدة والسكربتات:
...
<خطوة عنوان="إعداد متغيرات البيئة">
...
<خطوة عنوان="إنشاء خادم الخلفية">
قم بإنشاء `src/server.ts` ونسخ كود الخادم الكامل هذا:
...
الآن دعنا نضيف واجهة دردشة جميلة مع سجل المحادثات الكامل! أنشئ `public/index.html`:
...
<خطوة عنوان="افتح واجهة الدردشة">
1. افتح المستعرض الخاص بك
2. انتقل إلى `http://localhost:3000`
3. يجب أن ترى واجهة الدردشة الجميلة

<ملاحظة>
تأكد من تحديث `CUSTOMER_ID` في `server.ts` مع معرف العميل التجريبي الفعلي الخاص بك من الخطوة 4.
</ملاحظة>
</خطوة>
...
2. تحقق من أن معرف العميل موجود في لوحة معلومات مدفوعات Dodo
...
2. قم بالتحديث إلى أحدث SDK من Blueprints: `npm install @dodopayments/ingestion-blueprints@latest`
</توسيع>

<توسيع عنوان="خطأ مفتاح API غير صالح">
**الأسباب المحتملة:**
- مفتاح API خاطئ للبيئة
- مسافات إضافية أو اقتباسات في ملف `.env`

**الحلول:**
- تأكد من أن المفتاح التجريبي يبدأ بـ `test_`، المفتاح النشط يبدأ بـ `live_`
- احذف أي اقتباسات حول المفاتيح في ملف `.env`
