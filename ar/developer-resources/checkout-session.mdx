...
**التنسيق**: تضمين رمز الدولة، مثل `"+1234567890"` للأرقام الأمريكية
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
إذا تم تعيينها على true، لا يتم إجراء أي شحنة وتفويض تفاصيل طريقة الدفع للاستخدام المستقبلي فقط.
</ParamField>
...
عملة سعر المنتج الاختيارية. إذا لم يتم تحديدها، يتم تعيينها على عملة المنتج.
</ParamField>

<ParamField body="product_description" type="string">
تجاوز وصف المنتج الاختياري للفوترة والعناصر. إذا لم يتم تحديده، سيتم استخدام الوصف المخزن للمنتج.
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
ما إذا كانت رسوم العملة التكيفية يجب أن تكون مشمولة في سعر المنتج (true) أو مضافة فوقها (false). يتم تجاهلها إذا لم يتم تمكين التسعير التكيفي.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
### 10. BNPL (اشتر الآن وادفع لاحقًا) دفع
...
استخدم طريقة الدفع المحفوظة للعميل لإنشاء جلسة دفع تتم معالجتها على الفور، متجاوزة جمع طريقة الدفع:
...
### 12. روابط قصيرة لروابط الدفع الأنظف
...
<Tip>
الروابط القصيرة مثالية لمشاركة عبر SMS أو البريد الإلكتروني أو وسائل التواصل الاجتماعي. إنها أسهل في التذكر وتبني ثقة أكبر لدى العملاء مقارنةً بالروابط الطويلة.
</Tip>

### 13. تخطي صفحة نجاح الدفع مع إعادة توجيه فورية

إعادة توجيه العملاء على الفور بعد إتمام الدفع، متجاوزًا صفحة النجاح الافتراضية:
...
<Note>
عندما يتم تمكين `redirect_immediately`، يتم إعادة توجيه العملاء إلى `return_url` الخاص بك مباشرة بعد إتمام الدفع، متجاوزين صفحة النجاح الافتراضية تمامًا.
</Note>

## معاينة جلسات الدفع

قبل إنشاء جلسة دفع، يمكنك معاينة تفاصيل التسعير بما في ذلك الضرائب والخصومات والمجموعات. هذا مفيد لعرض الأسعار الدقيقة للعملاء قبل أن يتقدموا إلى الدفع.
...
<Card title="مرجع API للمعاينة" icon="code" href="/api-reference/checkout-sessions/preview">
...
### الاختلافات الرئيسية
سابقًا، عند إنشاء رابط دفع باستخدام الروابط الديناميكية، كان مطلوبًا منك تقديم عنوان الفواتير الكامل للعميل.
...
- من ناحية أخرى، إذا كان لديك جميع المعلومات المطلوبة وترغب في الانتقال مباشرة إلى صفحة الدفع، يمكنك تمرير مجموعة البيانات الكاملة وتضمين `confirm=true` في جسم طلبك.
...
قم بتحديث تكاملتك لاستخدام API أو طريقة SDK الجديدة.
</Step>
