...
<Info>
تشمل استجابات العملاء للحقول المخصصة ما يلي:
- **الويب هوكس**: `payment.succeeded` و`subscription.active` وغيرها من حمولات الأحداث ذات الصلة تحتوي على مصفوفة `custom_field_responses`
- **استجابات واجهة برمجة التطبيقات**: تتضمن كائنات الدفع والاشتراك `custom_field_responses`
</Info>
...
<Info>
تُدرج ردود الحقول المخصصة تلقائيًا في حمولات الويب هوكس (`payment.succeeded` و`subscription.active` وما إلى ذلك) ويمكن استرجاعها عبر واجهة برمجة التطبيقات. استخدمها لتغذية نظام إدارة علاقات العملاء الخاص بك، أو تشغيل تدفقات بدء التشغيل، أو تخصيص تجربة العميل.
</Info>
