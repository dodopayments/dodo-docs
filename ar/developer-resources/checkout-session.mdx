...
**الخيارات الشائعة**: `credit`، `debit`، `upi_collect`، `apple_pay`، `google_pay`، `amazon_pay`، `klarna`، `affirm`، `afterpay_clearpay`، `cashapp`، `ach`، `multibanco`، `bancontact_card`، `eps`، `ideal`، `blik`، `paypal`. هذه ليست المجموعة الكاملة — راجع [مرجع Create Checkout Session API](/api-reference/checkout-sessions/create) للاطلاع على كل قيمة مقبولة.
...
طبّق رمز خصم مكدّسًا واحدًا أو أكثر على جلسة الدفع. تُطبَّق الرموز **بترتيب المصفوفة** (يخفض الرمز الأول السعر الابتدائي، ثم يخفض الرمز الثاني السعر بعد الخصم، وهكذا)، بحد أقصى **20** رمزًا لكل جلسة. عند تفعيل [Purchasing Power Parity](/features/purchasing-power-parity)، يكون السعر الابتدائي هو المبلغ المعدّل وفق PPP، وليس السعر الأساسي.
...
<Note>
يعكس `current_breakup.subtotal` الذي تمت معاينته بالفعل [Purchasing Power Parity](/features/purchasing-power-parity) و[Charm Pricing](/features/charm-pricing) حيثما ينطبقان على المنتج.
</Note>

<Note>
عندما تحتوي سلة التسوق على منتج اشتراك، يعرض رد المعاينة أيضًا `next_billing_date` — وهي معاينة لتاريخ الفوترة القادم، حتى تتمكن من عرضه قبل إنشاء الاشتراك. ويُحتسب هذا التاريخ نسبةً إلى الوقت الحالي: `now + trial period` عند تطبيق فترة تجريبية، وإلا فهو `now + one payment frequency`. يُحذف الحقل من السلال التي تحتوي على منتجات تُشترى لمرة واحدة فقط. هذا تقدير يستند إلى وقت المعاينة؛ أما `next_billing_date` المعتمد فيُحدَّد عند تفعيل الاشتراك.
</Note>

<Note>
تعرض المعاينة أيضًا `trial_period_days` (مدة الفترة التجريبية الفعلية، سواء كانت مجانية أو مدفوعة) و`trial_amount` (رسوم الفترة التجريبية لكل وحدة بعد الخصومات، بوحدات العملة الصغرى لعملة السعر). لا يظهر `trial_amount` إلا عند استخدام [paid trial](/features/subscription#paid-trials)، وتكون قيمته `null` عند استخدام فترة تجريبية مجانية أو عدم استخدام فترة تجريبية. استخدم `current_breakup` لمعرفة الإجمالي الخاضع للضريبة المستحق فعليًا اليوم.
</Note>
...
<Card title="Preview API Reference" icon="code" href="/api-reference/checkout-sessions/preview">
  اطّلع على الوثائق الكاملة لنقطة نهاية المعاينة.
</Card>

## الانتقال من Dynamic Links إلى Checkout Sessions

### الفروقات الرئيسية
في السابق، عند إنشاء رابط دفع باستخدام Dynamic Links، كان مطلوبًا منك تقديم عنوان الفوترة الكامل للعميل.

مع Checkout Sessions، لم يعد ذلك ضروريًا. يمكنك ببساطة تمرير أي معلومات لديك، وسنتولى الباقي. على سبيل المثال:

- إذا كنت تعرف بلد فوترة العميل فقط، فما عليك سوى تقديمه.
- ستجمع تجربة الدفع تلقائيًا التفاصيل الناقصة قبل نقل العميل إلى صفحة الدفع.
...
الترحيل من Dynamic Links إلى Checkout Sessions straightforward:
...
نعم. لا يلزم إجراء أي معالجة إضافية أو خطوات ترحيل خاصة من جانبك.
</Step>
</Steps>
...
<Card title="Create Checkout Session" icon="code" href="/api-reference/checkout-sessions/create">
  مرجع API الكامل لإنشاء جلسات الدفع باستخدام جميع المَعلمات والخيارات المتاحة
</Card>
