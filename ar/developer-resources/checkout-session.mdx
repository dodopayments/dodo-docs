...
<Info>
**صلاحية الجلسة**: تكون جلسات الدفع صالحة لمدة 24 ساعة بشكل افتراضي. إذا قمت بتمرير `confirm=true` في طلبك، ستكون الجلسة صالحة لمدة 15 دقيقة فقط.
</Info>
...
<Steps>
<Step title="احصل على رابط الدفع">
...
مصفوفة من المنتجات التي يجب تضمينها في جلسة الدفع. يجب أن تحتوي كل منتج على `product_id` صالح من لوحة تحكم Dodo Payments.
...
**التنسيق**: تضمين رمز الدولة، على سبيل المثال، `"+1234567890"` للأرقام الأمريكية
</ParamField>
</Expandable>
</Tab>
</Tabs>
...
<Note>
تجاوز `billing_currency` ساري المفعول فقط عند تمكين العملة التكيفية في إعدادات حسابك. إذا كانت العملة التكيفية معطلة، فلن يكون لهذا المعامل أي تأثير.
</Note>
...
<Tip>
استخدم `redirect_immediately: true` عندما يكون لديك صفحة نجاح مخصصة توفر تجربة مستخدم أفضل من صفحة نجاح الدفع الافتراضية. هذا مفيد بشكل خاص لتطبيقات الهواتف المحمولة وتدفقات الدفع المدمجة.
</Tip>
