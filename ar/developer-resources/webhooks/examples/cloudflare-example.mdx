...
5. نفّذ الاستعلام
6. احصل على سلسلة الاتصال من Neon → تفاصيل الاتصال
...
> **Note:** سنقوم بإعداد `DODO_PAYMENTS_WEBHOOK_KEY` بعد النشر بمجرد حصولك على عنوان URL الخاص بالويب هوك.

### 5. تحديث wrangler.toml

حرّر `wrangler.toml` وحدد اسم العامل الخاص بك:
...
1. اذهب إلى [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
2. أنشئ نقطة نهاية ويب هوك جديدة
3. قم بتكوين عنوان URL للويب هوك كنقطة النهاية
4. فعّل أحداث الاشتراك التالية:
   - `subscription.active`
   - `subscription.cancelled`
   - `subscription.renewed`
5. انسخ **السرّ الخاص بالتوقيع**
...
✅ **Signature verification** - باستخدام مكتبة dodopayments  
✅ **Idempotency** - يمنع المعالجة المكررة باستخدام معرفات الويب هوك  
✅ **Event logging** - سجل تدقيق كامل في جدول `webhook_events`  
✅ **Error handling** - يتم تسجيلها وقابلة لإعادة المحاولة

> **Note:** يوضح هذا التّنفيذ معالجة ثلاثة من أحداث الاشتراك الأساسية (`subscription.active`، `subscription.cancelled`، `subscription.renewed`) باستخدام أقل عدد من الحقول. يمكنك توسيعه بسهولة لدعم أنواع وحقول إضافية بناءً على متطلباتك.
...
2. **يتحقق من التكرارات** - يستخدم معرف الويب هوك لمنع معالجة نفس الحدث مرتين
3. **يسجل الحدث** - يخزن الويب هوك الخام في جدول `webhook_events` لسجل التدقيق
4. **يعالج الحدث** - ينشئ العملاء والاشتراكات أو يُحدثها
5. **يتعامل مع الأخطاء** - يسجل الإخفاقات ويعلم بأن الحدث لم يُعالج لإعادة المحاولة
...
| المشكلة | الحل |
|----------|-----|
| فشل التحقق | تحقق من أن مفتاح الويب هوك صحيح من لوحة DodoPayments |
| خطأ في اتصال قاعدة البيانات | تحقق من سلسلة الاتصال في Neon |
| عدم نشر العامل | نفّذ `npm install`، تحقق من صحة بناء الجملة في `wrangler.toml` |
