...
> **ملاحظة:** سنقوم بإعداد `DODO_PAYMENTS_WEBHOOK_KEY` بعد النشر بمجرد حصولك على عنوان URL الخاص بالويب هوك.

### 5. تحديث wrangler.toml

قم بتحرير `wrangler.toml` واضبط اسم العامل الخاص بك:
...
1. اذهب إلى [لوحة معلومات DodoPayments](https://app.dodopayments.com) → المطور → ويب هوكس
2. أنشئ نقطة نهاية ويب هوك جديدة
3. قم بتكوين عنوان URL الخاص بالويب هوك كنقطة النهاية
...
✅ **عدم التكرار** - يمنع المعالجة المكررة باستخدام معرفات الويب هوك  
✅ **تسجيل الأحداث** - سجل تدقيق كامل في جدول `webhook_events`  
✅ **معالجة الأخطاء** - مسجلة وقابلة لإعادة المحاولة

> **ملاحظة:** توضح هذه التنفيذ كيفية التعامل مع ثلاثة أحداث اشتراك أساسية (`subscription.active`، `subscription.cancelled`، `subscription.renewed`) مع الحد الأدنى من الحقول. يمكنك تمديدها بسهولة لدعم أنواع ونطاقات الأحداث الإضافية وفقًا لمتطلباتك.
...
معالج الويب هوك:
1. **يحقق من التوقيع** - يضمن أن الطلب من DodoPayments باستخدام HMAC-SHA256
2. **التحقق من التكرارات** - يستخدم معرف الويب هوك لمنع معالجة نفس الحدث مرتين
3. **تسجيل الحدث** - يخزن الويب هوك الخام في جدول `webhook_events` لتدقيق السجل
4. **معالجة الحدث** - ينشئ أو يقوم بتحديث العملاء والاشتراكات
5. **معالجة الأخطاء** - يسجل الفشل ويعتبر الحدث غير المعالج لإعادة المحاولة
...
| خطأ في الاتصال بقاعدة البيانات | تحقق من سلسلة الاتصال في Neon |
| العامل غير منتشر | قم بتشغيل `npm install`، وتحقق من بناء جملة `wrangler.toml` |
