...
1. انتقل إلى [DodoPayments Dashboard](https://app.dodopayments.com) → المطور → الويب هوكس
2. أنشئ نقطة نهاية ويب هوك جديدة
3. قم بتكوين عنوان URL الخاص بك كنقطة نهاية
4. قم بتمكين هذه أحداث الاشتراك:
   - `subscription.active`
...
✅ **التماثل** - يمنع المعالجة المكررة باستخدام معرفات الويب هوك  
✅ **تسجيل الأحداث** - تدقيق كامل في جدول `webhook_events`  
✅ **معالجة الأخطاء** - مسجلة ويمكن إعادة المحاولة

> **ملاحظة:** توضح هذه التنفيذ كيفية التعامل مع ثلاث أحداث اشتراك رئيسية (`subscription.active`، `subscription.cancelled`، `subscription.renewed`) مع الحد الأدنى من الحقول. يمكنك بسهولة توسيعها لدعم أنواع الأحداث والحقول الإضافية بناءً على متطلباتك.
...
معالج الويب هوك:
1. **التحقق من التوقيع** - يضمن أن الطلب من DodoPayments باستخدام HMAC-SHA256
2. **التحقق من التكرار** - يستخدم معرف الويب هوك لمنع معالجة نفس الحدث مرتين
3. **تسجيل الحدث** - يخزن الويب هوك الخام في جدول `webhook_events` لسجل التدقيق
4. **معالجة الحدث** - ينشئ أو يحدث العملاء والاشتراكات في Neon
5. **معالجة الأخطاء** - يسجل الفشل ويحدد الحدث على أنه غير معالج لإعادة المحاولة
...
**عرض السجلات في لوحة معلومات Netlify:**
1. اختر موقعك → علامة التبويب **الوظائف**
2. انقر على وظيفة `webhook`
3. عرض السجلات الحية وتاريخ الاستدعاء
...
| خطأ في اتصال قاعدة البيانات | تحقق من سلسلة اتصال Neon واستخدم الاتصال المجمع |
| الوظيفة غير موجودة (404) | قم بتشغيل `netlify init` و `npm run deploy` مرة أخرى |
| المتغيرات البيئية غير متاحة | قم بتعيين المتغيرات في CLI أو اللوحة، ثم أعد النشر |
