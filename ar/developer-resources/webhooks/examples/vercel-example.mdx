...
> **ملاحظة:** سنقوم بتعيين `DODO_PAYMENTS_WEBHOOK_KEY` بعد النشر بمجرد أن تحصل على عنوان URL الخاص بك.
...
1. الانتقال إلى [لوحة معلومات DodoPayments](https://app.dodopayments.com) → المطور → Webhooks
2. إنشاء نقطة نهاية ويب هوك جديدة
3. تكوين عنوان URL الخاص بك كنقطة نهاية
4. تمكين أحداث الاشتراك التالية:
   - `subscription.active`
...
✅ **التماثلية** - يمنع المعالجة المكررة باستخدام معرفات الويب هوك  
✅ **تسجيل الأحداث** - سجل تدقيق كامل في جدول `webhook_events`  
✅ **معالجة الأخطاء** - مسجلة وقابلة لإعادة المحاولة

> **ملاحظة:** توضح هذه التنفيذ كيفية التعامل مع ثلاثة أحداث اشتراك أساسية (`subscription.active`، `subscription.cancelled`، `subscription.renewed`) مع الحد الأدنى من الحقول. يمكنك بسهولة توسيعها لدعم أنواع الأحداث وحقول إضافية بناءً على متطلباتك.
...
معالج الويب هوك:
1. **يحتاج إلى تعطيل تحليل الجسم** - للوصول إلى الجسم الخام للتحقق من التوقيع
2. **يتحقق من التوقيع** - يضمن أن الطلب من DodoPayments باستخدام HMAC-SHA256
3. **يتحقق من التكرارات** - يستخدم معرف الويب هوك لمنع معالجة نفس الحدث مرتين
4. **يسجل الحدث** - يخزن الويب هوك الخام في جدول `webhook_events` لتدقيق السجل
5. **يعالج الحدث** - ينشئ أو يحدث العملاء والاشتراكات في Neon
6. **يتعامل مع الأخطاء** - يسجل الفشل ويضع الحدث كغير معالج لإعادة المحاولة
