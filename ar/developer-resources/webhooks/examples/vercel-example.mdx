...
{/* LOCKED_PATTERN_b68b0431a2cfe225bd9aae20530def93 */}
  رمز المصدر الكامل ودليل الإعداد
</Card>
...
1. سجّل للحصول على حساب في [Neon](https://neon.com)
...
> **ملاحظة:** سنقوم بتعيين `DODO_PAYMENTS_WEBHOOK_KEY` بعد النشر بمجرد أن يكون لديك عنوان URL لويب هوك.
...
1. اذهب إلى لوحة تحكم [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
2. أنشئ نقطة نهاية ويب هوك جديدة
3. قم بتكوين عنوان URL الخاص بالويب هوك كنقطة نهاية
...
✅ **القابلية للمعالجة الأحادية** - يمنع المعالجة المكررة باستخدام معرفات الويب هوك  
✅ **تسجيل الأحداث** - سجل تدقيق كامل في جدول `webhook_events`  
✅ **معالجة الأخطاء** - يتم تسجيلها وقابلة لإعادة المحاولة

> **ملاحظة:** يوضح هذا التنفيذ معالجة ثلاثة من أحداث الاشتراك الأساسية (`subscription.active`، `subscription.cancelled`، `subscription.renewed`) مع الحد الأدنى من الحقول. يمكنك توسيعه بسهولة لدعم أنواع حقول إضافية وأحداث إضافية بناءً على متطلباتك.
...
3. **يتحقق من التكرار** - يستخدم معرف الويب هوك لمنع معالجة الحدث نفسه مرتين
4. **يسجل الحدث** - يخزن الويب هوك الخام في جدول `webhook_events` لتتبع التدقيق
5. **يعالج الحدث** - ينشئ أو يحدث العملاء والاشتراكات في Neon
6. **يتعامل مع الأخطاء** - يسجل الفشل ويعلم الحدث على أنه غير معالج لإعادة المحاولة
