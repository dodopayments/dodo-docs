...
1. انتقل إلى [لوحة معلومات Supabase](https://supabase.com/dashboard)
2. افتح محرر SQL
3. أنشئ استعلامًا جديدًا
4. انسخ والصق كل محتويات [`schema.sql`](#database-schema)
5. قم بتشغيل الاستعلام
...
> **ملاحظة:** سنقوم بتعيين `DODO_PAYMENTS_WEBHOOK_KEY` بعد النشر بمجرد حصولك على عنوان URL للويب هوك.
...
1. انتقل إلى [لوحة معلومات DodoPayments](https://app.dodopayments.com) → المطور → ويب هوكس
2. أنشئ نقطة نهاية ويب هوك جديدة
3. قم بتكوين عنوان URL الخاص بك كأداة نقطية
...
✅ **التكرار** - يمنع المعالجة المكررة باستخدام معرفات الويب هوك  
✅ **تسجيل الأحداث** - سجل تدقيق كامل في جدول `webhook_events`  
✅ **معالجة الأخطاء** - مسجلة وقابلة لإعادة المحاولة

> **ملاحظة:** توضح هذه التنفيذ كيفية التعامل مع ثلاثة أحداث اشتراك أساسية (`subscription.active`، `subscription.cancelled`، `subscription.renewed`) مع الحد الأدنى من الحقول. يمكنك توسيعه بسهولة لدعم أنواع الأحداث والحقول الإضافية بناءً على متطلباتك.
...
وظيفة Edge المعتمدة على Deno:
1. **التحقق من التوقيع** - يستخدم مكتبة dodopayments للتحقق من HMAC-SHA256
2. **التحقق من التكرار** - يبحث عن معرف الويب هوك لمنع المعالجة المكررة
3. **تسجيل الحدث** - يقوم بتخزين بيانات الويب هوك الخام في جدول `webhook_events`
4. **معالجة التحديثات** - يقوم بإنشاء أو تحديث العملاء والاشتراكات عبر عميل Supabase
5. **معالجة الأخطاء** - يسجل الفشل ويحدد الحدث لإعادة المحاولة
...
<Note>
العلامة `--no-verify-jwt` مطلوبة لأن الويب هوكس لا تتضمن رموز JWT. تُوفر الأمان من خلال التحقق من توقيع الويب هوك.
</Note>
...
| المشكلة | الحل |
|-------|----------|
| فشل التحقق | تحقق من أن مفتاح الويب هوك صحيح من لوحة معلومات DodoPayments |
| خطأ في إذن قاعدة البيانات | تأكد من استخدام مفتاح دور الخدمة |
| خطأ تحقق JWT | نشر مع علامة `--no-verify-jwt` |
