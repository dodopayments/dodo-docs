...
{/* LOCKED_PATTERN_22a8d1fd3fdf6a73444bca527433b154 */}
  الدليل الكامل للشفرة المصدرية وإعدادها
</Card>

<Tip>
  <strong>اطّلع على مجموعة البداية الكاملة لـ supabase</strong><br />
  مجموعة بداية اشتراك بسيطة مبنية باستخدام Next.js و Supabase و Dodo Payments. يساعدك هذا القالب على إعداد SaaS قائم على الاشتراكات مع المصادقة والمدفوعات وwebhooks بسرعة.
...
1. انتقل إلى لوحة معلومات [Supabase Dashboard](https://supabase.com/dashboard)
2. افتح محرر SQL
3. أنشئ استعلامًا جديدًا
4. انسخ والصق المحتوى الكامل لـ [`schema.sql`](#database-schema)
5. نفّذ الاستعلام

### 4. تعيين الأسرار الأولية

توفر Supabase تلقائيًا `SUPABASE_URL` و `SUPABASE_SERVICE_ROLE_KEY` أثناء وقت التشغيل.

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **ملاحظة:** سنضبط `DODO_PAYMENTS_WEBHOOK_KEY` بعد النشر بمجرد حصولك على عنوان URL الخاص بالـ webhook.
...
1. انتقل إلى [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
2. أنشئ نقطة نهاية webhook جديدة
3. اضبط عنوان URL الخاص بالـ webhook كنقطة النهاية
...
✅ **اللا تدويرية (Idempotency)** - يمنع المعالجة المكررة باستخدام معرّفات webhook  
✅ **تسجيل الأحداث** - سجل تدقيق كامل في جدول `webhook_events`  
✅ **معالجة الأخطاء** - مسجلة وقابلة لإعادة المحاولة

> **ملاحظة:** يوضح هذا التّنفيذ التعامل مع ثلاث أحداث اشتراك أساسية (`subscription.active`، `subscription.cancelled`، `subscription.renewed`) مع حقول قليلة. يمكنك توسيعه بسهولة لدعم أنواع وحقول إضافية حسب متطلباتك.
...
وظيفة Edge المبنية على Deno:
1. **تتحقق من التوقيع** - تستخدم مكتبة dodopayments للتحقق عبر HMAC-SHA256
2. **تفحص اللا تدويرية** - تبحث عن معرف الـ webhook لمنع المعالجة المكررة
3. **تسجل الحدث** - تخزن بيانات الـ webhook الخام في جدول `webhook_events`
4. **تعالج التحديثات** - تنشئ أو تحدّث العملاء والاشتراكات عبر عميل Supabase
5. **تعالج الأخطاء** - تسجل الإخفاقات وتعلّم الحدث لإعادة المحاولة
...
<Note>
العلم `--no-verify-jwt` مطلوب لأن webhooks لا تتضمن رموز JWT. يتم توفير الأمان من خلال التحقق من توقيع الـ webhook.
</Note>
...
| المشكلة | الحل |
|-------|----------|
| فشل التحقق | تأكد من أن مفتاح الـ webhook صحيح من لوحة DodoPayments |
| خطأ في أذونات قاعدة البيانات | تأكد من استخدام Service Role Key |
| خطأ تحقق JWT | انشر مع العلم `--no-verify-jwt` |
| الوظيفة غير موجودة | تحقق من أن مرجع المشروع صحيح وأن الوظيفة منشورة |
