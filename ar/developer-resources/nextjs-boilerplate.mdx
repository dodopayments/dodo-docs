...
<Info>
هذا القالب يستخدم Next.js 16 App Router مع TypeScript و Tailwind CSS 4 و موصل `@dodopayments/nextjs`.
</Info>

### الميزات

- **إعداد سريع** - ابدأ في أقل من 5 دقائق
- **تكامل الدفع** - تدفق سداد مُعدل مسبقًا باستخدام `@dodopayments/nextjs`
- **واجهة مستخدم حديثة** - صفحة تسعير نظيفة ذات ثيم داكن مع Tailwind CSS
- **معالج الويب هوك** - نقطة نهاية ويب هوك جاهزة للاستخدام لأحداث الدفع
- **بوابة العملاء** - إدارة الاشتراكات بنقرة واحدة
- **TypeScript** - مكتوب بالكامل بأنواع دقيقة وموجهة
- **سداد مُعبأ مسبقًا** - يظهر تمرير بيانات العملاء لتحسين تجربة المستخدم
...
<Warning>
لا تقم بارتكاب ملف `.env` إلى نظام التحكم في الإصدارات. إنه مدرج بالفعل ضمن `.gitignore`.
</Warning>
</Step>

<Step title="إضافة منتجاتك">
قم بتحديث `src/lib/products.ts` بمعرفات منتجاتك الفعلية من Dodo Payments:
...
عدل `src/lib/products.ts` لتعديل:
...
يظهر القالب معالجة حدثي ويب هوك في `src/app/api/webhook/route.ts`:

- `onSubscriptionActive` - يتم تشغيله عندما يصبح الاشتراك نشطًا
- `onPaymentSucceeded` - يتم تشغيله عندما يكون الدفع ناجحًا
...
**أسباب شائعة:**
- معرف منتج غير صالح - تحقق من وجوده في لوحة تحكم Dodo الخاصة بك
- مفتاح API خاطئ أو إعداد في البيئة في `.env`
...
استبدل `CUSTOMER_ID` الثابت في `src/app/components/Header.tsx` بمعرف عميل فعلي من لوحة تحكم Dodo الخاصة بك.
