...
تحديد عملة الدفع (مثلًا، <code>USD</code>).
</ParamField>
<ParamField query="showCurrencySelector" type="boolean">
إظهار محدد العملة.
</ParamField>
<ParamField query="paymentAmount" type="number">
يثبّت المبلغ الذي يتم تحصيله بوحدات العملة الأساسية (مثلًا، <code>12.5</code> مقابل 12.50 دولارًا). متاح لمنتجات Pay What You Want فقط، ويتم تجاهله إذا كان أقل من الحد الأدنى لسعر المنتج.
</ParamField>
<ParamField query="showDiscounts" type="boolean">
إظهار حقول الخصم.
</ParamField>
<ParamField query="metadata_*" type="string">
سيتم تمرير أي معلمة Query تبدأ بـ <code>metadata_</code> كبيانات وصفية.
</ParamField>
...
<Warning>
يعمل Dynamic Checkout على توجيه نقاط النهاية المهجورة `POST /payments` و `POST /subscriptions`. ويستمر في العمل مع عمليات التكامل الحالية، ولكن ينبغي لعمليات التكامل الجديدة استخدام **Checkout Sessions** أدناه.
</Warning>

### تنسيق الاستجابة
يعرض Dynamic checkout استجابة JSON تحتوي على عنوان URL لصفحة الدفع:
...
توفر Checkout Sessions تجربة دفع مستضافة وأكثر أمانًا، وتتولى معالجة تدفق الدفع بالكامل لكل من عمليات الشراء لمرة واحدة والاشتراكات، مع تحكم كامل في التخصيص.

راجع [دليل تكامل Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session) لمزيد من التفاصيل والقائمة الكاملة بالحقول المدعومة.

### تنسيق الاستجابة
تعرض Checkout Sessions استجابة JSON تحتوي على عنوان URL لصفحة الدفع:
...
يتيح لك معالج مسار Customer Portal دمج بوابة عملاء Dodo Payments بسلاسة في تطبيق Nuxt الخاص بك.
...
إذا تم تعيينه إلى <code>true</code>، فسيُرسل بريدًا إلكترونيًا إلى العميل يتضمن رابط البوابة.
</ParamField>

<Warning>
يعرض 400 إذا كان <code>customer_id</code> مفقودًا.
</Warning>

## معالج مسار Webhook

- **Method:** لا يُسمح إلا بطلبات POST. وتُرجع الطرق الأخرى الحالة 405.
- **التحقق من التوقيع:** يتحقق من توقيع Webhook باستخدام <code>webhookKey</code>. ويُرجع الحالة 401 عند فشل التحقق.
- **التحقق من Payload:** يتم التحقق باستخدام Zod. ويُرجع الحالة 400 عند عدم صلاحية Payload.
- **معالجة الأخطاء:**
  - 401: توقيع غير صالح
  - 400: Payload غير صالح
  - 500: خطأ داخلي أثناء التحقق
- **توجيه الأحداث:** يستدعي معالج الأحداث المناسب بناءً على نوع Payload.
