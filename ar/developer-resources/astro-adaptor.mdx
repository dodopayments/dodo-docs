---
title: محول أسترو
description: تعلم كيفية دمج مدفوعات دودو مع مشروع موجه تطبيق أسترو الخاص بك
  باستخدام محول أسترو. يغطي الدفع، بوابة العملاء، الويب هوكس، وإعداد بيئة آمنة.
icon: code-merge
---

<CardGroup cols={2}>
{/* LOCKED_PATTERN_848a353666469e7a047ef05af5a45770 */}
    دمج عملية الدفع Dodo Payments في تطبيق Astro الخاص بك.
</Card>
{/* LOCKED_PATTERN_df89dec21e12f4bf30b6acf75ba025e1 */}
    تمكين العملاء من إدارة الاشتراكات والتفاصيل.
</Card>
{/* LOCKED_PATTERN_727e199da3d921456f615ec3f0aa16ca */}
    تلقي ومعالجة أحداث الويب هوك لـ Dodo Payments.
</Card>
</CardGroup>
...
<Info>
تفترض جميع الأمثلة أنك تستخدم موجّه تطبيق Astro.
</Info>

<Tabs>
{/* LOCKED_PATTERN_28fd81a4703a84d931eeff69f1520d54 */}
  <Info>
  استخدم هذا المعالج لدمج صفحة الدفع Dodo Payments في تطبيق Astro الخاص بك. يدعم تدفقات الدفع الثابتة (GET)، والديناميكية (POST)، وتدفقات الجلسة (POST).
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_0026b036379d81a9d03c36b3b6e83ddd */}
  <Info>
  استخدم هذا المعالج لتلقي ومعالجة أحداث الويب هوك لـ Dodo Payments بأمان في تطبيق Astro الخاص بك.
  </Info>
  <CodeGroup>
...
<Info>
يدعم Dodo Payments ثلاثة أنواع من تدفقات الدفع لدمج المدفوعات في موقعك، ويدعم هذا الموّجه جميع أنواع التدفقات.
</Info>
...
<Warning>
إذا كان <code>productId</code> مفقودًا، يعيد المعالج استجابة 400. تؤدي المعاملات غير الصالحة أيضًا إلى استجابة 400.
</Warning>
...
توفر جلسات الدفع تجربة دفع مستضافة أكثر أمانًا تتولى تدفق الدفع الكامل لكل من المشتريات لمرة واحدة والاشتراكات مع تحكم تخصيص كامل.
...
يمكنك من خلال معالج مسار بوابة العملاء دمج بوابة عملاء مدفوعات دودو بسلاسة في تطبيق أسترو الخاص بك.

### معاملات الاستعلام
<ParamField query="customer_id" type="string" required>
معرف العميل لجلسة البوابة (مثلاً <code>?customer_id=cus_123</code>).
</ParamField>
<ParamField query="send_email" type="boolean">
إذا تم ضبطه على <code>true</code>، يُرسل بريدًا إلكترونيًا للعميل برابط البوابة.
</ParamField>

<Warning>
يعيد 400 إذا كانت <code>customer_id</code> مفقودة.
</Warning>
...
### المعالجات المدعومة لأحداث الويب هوك
<CodeGroup>
