...
**اسم المتر**: انسخ والصق هذا تمامًا → `Image Generation Meter`

**الوصف**: انسخ هذا → `Tracks each AI image generation request made by customers using our DALL-E powered service`

**اسم الحدث**: هذا أمر حاسم - انسخ تمامًا → `image.generated`

<Warning>
اسم الحدث `image.generated` يجب أن يتطابق تمامًا مع ما سنرسله من الكود المصدر في وقت لاحق. أسماء الأحداث حساسة لحالة الأحرف!
</Warning>
</Step>
...
1. **تمكين تصفية الأحداث**: قم بتبديل هذا **تشغيل**
2. **منطق التصفية**: اختر **أو** (هذا يعني "احسب إذا كانت أي من هذه الشروط صحيحة")
3. **أضف الشرط الأول**:
   - مفتاح الملكية: `quality`
   - المقارن: `equals`  
   - القيمة: `standard`
4. **انقر على "إضافة شرط"** للشرط الثاني:
   - مفتاح الملكية: `quality`
   - المقارن: `equals`
   - القيمة: `hd`
...
بالإضافة إلى ذلك، قم بتعيين **السعر الثابت** إلى `0` لضمان عدم تعرض العملاء لأي رسوم أساسية، بل لفاتورة وفقًا لاستخدامهم.
...
**السعر لكل وحدة**: أدخل → `0.05` (هذا هو $0.05 لكل صورة)

**الحد المجاني**: أدخل → `10` (يحصل العملاء على 10 صور مجانية في الشهر)
...
سيبدو رابط الدفع شيئًا مثل: `https://test.checkout.dodopayments.com/buy/pdt_IgPWlRsfpbPd5jQKezzW1?quantity=1`
</Step>
...
3. ابحث عن العميل الذي أنشأته للتو (مع البريد الإلكتروني التجريبي)
...
أنشئ ملفًا باسم `index.ts` وانسخ هذا الكود الكامل للتطبيق:
</Step>
</Steps>
...
1. أنشئ ملف `.env` في مجلد `pixelgen-ai` الخاص بك
