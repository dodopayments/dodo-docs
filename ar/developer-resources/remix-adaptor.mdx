...
يثبّت المبلغ المُحصّل بوحدات العملة الرئيسية (مثلًا، <code>12.5</code> مقابل 12.50 دولارًا). لمنتجات Pay What You Want فقط، ويتم تجاهله إذا كان أقل من الحد الأدنى لسعر المنتج.
</ParamField>
...
<Warning>
يعمل Dynamic Checkout كوكيلين لنقطتي النهاية المهجورتين `POST /payments` و `POST /subscriptions`. ويستمر في العمل مع عمليات التكامل الحالية، ولكن ينبغي أن تستخدم عمليات التكامل الجديدة **Checkout Sessions** أدناه.
</Warning>

### تنسيق الاستجابة
يعيد Dynamic Checkout استجابة JSON تتضمن عنوان URL لصفحة الدفع:
...
توفر Checkout Sessions تجربة دفع مستضافة وأكثر أمانًا، وتتعامل مع تدفق الدفع الكامل لكل من عمليات الشراء لمرة واحدة والاشتراكات، مع تحكم كامل في التخصيص.
...
### تنسيق الاستجابة
تعيد Checkout Sessions استجابة JSON تتضمن عنوان URL لصفحة الدفع:
...
يتيح لك معالج مسار Customer Portal دمج بوابة عملاء Dodo Payments بسلاسة في تطبيق Remix الخاص بك.
...
- **الطريقة:** لا تُدعم سوى طلبات POST. وتُرجع الطرق الأخرى 405.
- **التحقق من التوقيع:** يتحقق من توقيع Webhook باستخدام <code>webhookKey</code>. ويُرجع 401 إذا فشل التحقق.
- **التحقق من Payload:** يتم التحقق منه باستخدام Zod. ويُرجع 400 للـ Payload غير الصالح.
- **معالجة الأخطاء:**
  - 401: توقيع غير صالح
...
### معالجات Webhook للأحداث المدعومة
<CodeGroup>
