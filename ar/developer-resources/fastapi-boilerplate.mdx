...
- تأكد من تعيين `DODO_PAYMENTS_WEBHOOK_KEY` بشكل صحيح في `.env` الخاص بك
- تحقّق من استخدام raw request body للتحقق من التوقيع
- مرّر جميع ترويسات `webhook-id` و`webhook-timestamp` و`webhook-signature` الثلاثة إلى `webhook.verify()` — إذ يغطي توقيع Standard Webhooks `id.timestamp.body`، وليس body وحده
</Accordion>
</AccordionGroup>
