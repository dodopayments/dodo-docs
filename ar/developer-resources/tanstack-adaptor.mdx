---
title: مهيئ Tanstack
description: تعرّف على كيفية دمج <CardGroup cols={2}> مع مشروع Tanstack App
  Router باستخدام مهيئ Tanstack الخاص بنا. يغطي ذلك صفحة الدفع، وبوابة العملاء،
  وwebhooks، وإعداد البيئة الآمن.
icon: code-merge
keywords:
  - Dodo Payments
  - TanStack
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    ادمج صفحة الدفع الخاصة بـ Dodo Payments في تطبيق Tanstack الخاص بك.
</Card>
<Card title="Customer Portal" icon="user" href="#customer-portal-route-handler">
    اسمح للعملاء بإدارة الاشتراكات والتفاصيل الخاصة بهم.
</Card>
<Card title="Webhooks" icon="bell" href="#webhook-route-handler">
    استلم أحداث webhook الخاصة بـ Dodo Payments وعالجها.
</Card>
</CardGroup>
...
<Warning>
لا تُضمّن ملف <code>.env</code> أو الأسرار في نظام التحكم في الإصدارات.
</Warning>
</Step>
</Steps>
...
<Tab title="Checkout Handler">
  <Info>
  استخدم هذا المعالج لدمج صفحة الدفع الخاصة بـ Dodo Payments في تطبيق Tanstack الخاص بك. وهو يدعم تدفقات الدفع الثابتة (GET) والديناميكية (POST) وتدفقات الجلسات (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  استخدم هذا المعالج لاستلام أحداث webhook الخاصة بـ Dodo Payments ومعالجتها بأمان في تطبيق Tanstack الخاص بك.
  </Info>
  <CodeGroup>
...
<Info>
يدعم Dodo Payments ثلاثة أنواع من تدفقات الدفع لدمج المدفوعات في موقعك الإلكتروني، ويدعم هذا المهيئ جميع أنواع تدفقات الدفع.
</Info>
...
يثبّت المبلغ المُحصّل، بوحدات العملة الرئيسية (مثلًا، <code>12.5</code> مقابل 12.50 دولارًا). للمنتجات من نوع Pay What You Want فقط، ويتم تجاهله إذا كان أقل من الحد الأدنى لسعر المنتج.
</ParamField>
<ParamField query="showDiscounts" type="boolean">
إظهار حقول الخصم.
</ParamField>
<ParamField query="metadata_*" type="string">
سيتم تمرير أي معامل استعلام يبدأ بـ <code>metadata_</code> كـ metadata.
</ParamField>

<Warning>
إذا كان <code>productId</code> مفقودًا، يُرجع المعالج استجابة 400. كما تؤدي Query Parameters غير الصالحة إلى استجابة 400.
</Warning>

### تنسيق الاستجابة
يعيد الدفع الثابت استجابة JSON تحتوي على عنوان URL لصفحة الدفع:
...
- للحصول على قائمة كاملة بالحقول المدعومة في POST body، راجع:
...
<Warning>
يعمل Dynamic Checkout كوسيط لنقطتي النهاية المهجورتين `POST /payments` و`POST /subscriptions`. ويستمر في العمل مع عمليات التكامل الحالية، لكن ينبغي لعمليات التكامل الجديدة استخدام **Checkout Sessions** أدناه.
</Warning>

### تنسيق الاستجابة
يعيد Dynamic Checkout استجابة JSON تحتوي على عنوان URL الخاص بصفحة الدفع:
...
توفر Checkout Sessions تجربة دفع مستضافة وأكثر أمانًا، تتولى تدفق الدفع الكامل لكل من عمليات الشراء لمرة واحدة والاشتراكات، مع تحكم كامل في التخصيص.
...
### تنسيق الاستجابة
تعيد Checkout Sessions استجابة JSON تحتوي على عنوان URL الخاص بصفحة الدفع:
...
يتيح لك معالج مسار Customer Portal دمج بوابة عملاء Dodo Payments بسلاسة في تطبيق Tanstack الخاص بك.
...
إذا تم تعيينه إلى <code>true</code>، يُرسل بريدًا إلكترونيًا إلى العميل يحتوي على رابط البوابة.
</ParamField>
...
- **التحقق من التوقيع:** يتحقق من توقيع Webhook باستخدام <code>webhookKey</code>. ويُرجع 401 إذا فشل التحقق.
- **التحقق من Payload:** يتم التحقق باستخدام Zod. ويُرجع 400 للـ Payload غير الصالح.
- **معالجة الأخطاء:**
  - 401: توقيع غير صالح
  - 400: Payload غير صالح
  - 500: خطأ داخلي أثناء التحقق
- **توجيه الأحداث:** يستدعي معالج الأحداث المناسب استنادًا إلى نوع Payload.
