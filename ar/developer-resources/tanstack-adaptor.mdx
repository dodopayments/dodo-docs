...
### تنسيق الاستجابة
يعيد الدفع الديناميكي استجابة JSON تحتوي على عنوان URL لصفحة الدفع:
...
توفر جلسات الدفع تجربة دفع مستضافة وأكثر أمانًا تتعامل مع تدفق الدفع الكامل لكل من عمليات الشراء لمرة واحدة والاشتراكات، مع تحكم كامل في التخصيص.

لمزيد من التفاصيل والقائمة الكاملة بالحقول المدعومة، راجع [Checkout Sessions Integration Guide](https://docs.dodopayments.com/developer-resources/checkout-session).

### تنسيق الاستجابة
تعيد جلسات الدفع استجابة JSON تحتوي على عنوان URL لصفحة الدفع:
...
## Customer Portal معالج المسار

يتيح لك معالج المسار Customer Portal دمج بوابة عملاء Dodo Payments بسلاسة في تطبيق Tanstack الخاص بك.
...
- **Method:** لا يُدعم سوى طلبات POST. وتُرجع الطرق الأخرى 405.
- **التحقق من التوقيع:** يتحقق من توقيع webhook باستخدام <code>webhookKey</code>. ويُرجع 401 إذا فشل التحقق.
- **التحقق من Payload:** يتم التحقق منه باستخدام Zod. ويُرجع 400 إذا كان Payload غير صالح.
- **معالجة الأخطاء:**
  - 401: توقيع غير صالح
