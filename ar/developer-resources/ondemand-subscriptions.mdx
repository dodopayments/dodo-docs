---
title: الاشتراكات عند الطلب
description: دمج الاشتراكات عند الطلب من خلال تفويض التفويضات، وإنشاء رسوم
  متغيرة، والتعامل مع الويب هوكس، وتنفيذ سياسات إعادة المحاولة الآمنة.
icon: bolt
---

## نظرة عامة

تتيح لك الاشتراكات عند الطلب تفويض وسيلة الدفع الخاصة بالعميل مرة واحدة ثم تحصيل مبالغ متغيرة كلما احتجت، بدلاً من جدول زمني ثابت. هذه الميزة متاحة لجميع الحسابات - لا حاجة لموافقة.

استخدم هذا الدليل لـ:
- إنشاء اشتراك عند الطلب (تفويض تفويض مع سعر أولي اختياري)
- تحفيز الرسوم اللاحقة بمبالغ مخصصة
- تتبع النتائج باستخدام الويب هوكس

لإعداد اشتراك عام، راجع [دليل تكامل الاشتراك](/developer-resources/subscription-integration-guide).

## المتطلبات الأساسية

- حساب تاجر دودي بايمنتس ومفتاح API
- سر الويب هوك مُعد ونقطة نهاية لاستقبال الأحداث
- منتج اشتراك في كتالوجك

<Tip>
إذا كنت تريد من العميل الموافقة على التفويض من خلال عملية الدفع المستضافة، قم بتعيين `payment_link: true` وقدم `return_url`.
</Tip>

## كيف تعمل الاشتراكات عند الطلب

1. تقوم بإنشاء اشتراك باستخدام كائن `on_demand` لتفويض طريقة الدفع وجمع رسوم أولية اختيارية.
2. لاحقًا، تقوم بإنشاء رسوم لهذا الاشتراك بمبالغ مخصصة باستخدام نقطة نهاية الرسوم المخصصة.
3. تستمع إلى الويب هوكس (على سبيل المثال، `payment.succeeded`، `payment.failed`) لتحديث نظامك.

## إنشاء اشتراك عند الطلب

نقطة النهاية: [POST /checkouts](/api-reference/checkout-sessions/create)

حقول الطلب الرئيسية (الجسم):  
يرجى العثور عليها في [إنشاء جلسة الخروج](/api-reference/checkout-sessions/create)

### إنشاء اشتراك عند الطلب

<Tabs>
<Tab title="Node.js SDK">

```javascript
import DodoPayments from 'dodopayments';

const client = new DodoPayments({
  bearerToken: process.env.DODO_PAYMENTS_API_KEY,
  environment: 'test_mode', // defaults to 'live_mode'
});

async function main() {
  const subscription = await client.checkoutSessions.create({
    product_cart: [{ product_id: 'pdt_123', quantity: 1 }],
    billing_address:  { city: 'SF', country: 'US', state: 'CA', street: '1 Market St', zipcode: '94105' },
    customer: { customer_id: 'cus_123' },
    return_url: 'https://example.com/billing/success',
    subscription_data: {
        on_demand: {
            mandate_only: true // set false to collect an initial charge
            // product_price: 1000, // optional: charge $10.00 now if mandate_only is false
            // product_currency: 'USD',
            // product_description: 'Custom initial charge',
            // adaptive_currency_fees_inclusive: false,
        }
    }
  });

  console.log(subscription.checkout_url);
}

main().catch(console.error);
```

</Tab>
<Tab title="Python SDK">

```python
import os
from dodopayments import DodoPayments

client = DodoPayments(
    bearer_token=os.environ.get('DODO_PAYMENTS_API_KEY'),
    environment="test_mode",  # defaults to "live_mode"
)

checkout = client.checkout_sessions.create(
    product_cart=[
        {"product_id": "pdt_123", "quantity": 1}
    ],
    billing_address={
        "city": "SF",
        "country": "US",
        "state": "CA",
        "street": "1 Market St",
        "zipcode": "94105",
    },
    customer={
        "customer_id": "cus_123",
    },
    return_url="https://example.com/billing/success",
    subscription_data={
      "on_demand":{
      "mandate_only": True,  # set False to collect an initial charge
      # "product_price": 1000,  # optional: charge $10.00 now if mandate_only is false
      # "product_currency": "USD",
      # "product_description": "Custom initial charge",
      # "adaptive_currency_fees_inclusive": False,
      }
    },
)

print(checkout.checkout_url)
```

</Tab>
<Tab title="Go SDK">

```go
package main

import (
    "fmt"
    "os"

    dodo "github.com/dodopayments/dodopayments-go"
)

func main() {
    client := dodo.NewClient(dodo.Config{
        BearerToken: os.Getenv("DODO_PAYMENTS_API_KEY"),
        Environment: dodo.TestMode, // defaults to LiveMode
    })

    checkout, err := client.CheckoutSessions.Create(dodo.CheckoutSessionCreateParams{
        ProductCart: []dodo.ProductCartItem{
            {
                ProductID: "pdt_123",
                Quantity:  1,
            },
        },
        BillingAddress: &dodo.BillingAddress{
            City:    "SF",
            Country: "US",
            State:   "CA",
            Street:  "1 Market St",
            Zipcode: "94105",
        },
        Customer: &dodo.CustomerRef{
            CustomerID: "cus_123",
        },
        ReturnURL: "https://example.com/billing/success",
        SubscriptionData: &dodo.SubscriptionData{
            OnDemand: &dodo.OnDemandSubscription{
                MandateOnly: true, // set false to collect an initial charge
                // ProductPrice: 1000,  // optional: charge $10.00 now if mandate_only is false
                // ProductCurrency: "USD",
                // ProductDescription: "Custom initial charge",
                // AdaptiveCurrencyFeesInclusive: false,
            },
        },
    })
    if err != nil {
        panic(err)
    }

    fmt.Println(checkout.CheckoutURL)
}
```

</Tab>
<Tab title="cURL">

```bash
curl -X POST "$DODO_API/checkouts" \
  -H "Authorization: Bearer $DODO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "product_cart": [
      {
        "product_id": "pdt_123",
        "quantity": 1
      }
    ],
    "customer": {
      "customer_id": "cus_123"
    },
    "billing_address": {
      "street": "1 Market St",
      "city": "SF",
      "state": "CA",
      "country": "US",
      "zipcode": "94105"
    },
    "subscription_data": {
      "on_demand": {
        "mandate_only": true
      }
    },
    "return_url": "https://example.com/billing/success"
  }'
```

</Tab>
</Tabs>

```json Success
{
  "session_id": "cks_123",
  "checkout_url": "https://test.checkout.dodopayments.com/session/cks123"
}
```

## تحصيل اشتراك عند الطلب

بعد تفويض التفويض، قم بإنشاء رسوم حسب الحاجة.

نقطة النهاية: [POST /subscriptions/\{subscription_id\}/charge](/api-reference/subscriptions/create-charge)

حقول الطلب الرئيسية (الجسم):

<AccordionGroup>
  <Accordion title="معلمات جسم طلب الرسوم">
    <ParamField body="product_price" type="integer" required>
      المبلغ المراد تحصيله (بالوحدة الأصغر للعملة). مثال: لتحصيل 25.00 دولار، مرر <code>2500</code>.
    </ParamField>

    <ParamField body="product_currency" type="string">
      تجاوز العملة الاختياري للرسوم.
    </ParamField>

    <ParamField body="product_description" type="string">
      تجاوز الوصف الاختياري لهذه الرسوم.
    </ParamField>

    <ParamField body="adaptive_currency_fees_inclusive" type="boolean">
      إذا كانت صحيحة، تشمل رسوم العملة التكيفية ضمن <code>product_price</code>. إذا كانت خاطئة، تضاف الرسوم فوق ذلك.
    </ParamField>

    <ParamField body="metadata" type="object">
      بيانات وصفية إضافية للدفع. إذا تم حذفها، يتم استخدام بيانات الاشتراك الوصفية.
    </ParamField>
  </Accordion>
</AccordionGroup>

<Tabs>
<Tab title="Node.js SDK">

```javascript
import DodoPayments from 'dodopayments';

const client = new DodoPayments({ bearerToken: process.env.DODO_PAYMENTS_API_KEY });

async function chargeNow(subscriptionId) {
  const res = await client.subscriptions.charge(subscriptionId, { product_price: 2500 });
  console.log(res.payment_id);
}

chargeNow('sub_123').catch(console.error);
```

</Tab>
<Tab title="Python SDK">

```python
import os
from dodopayments import DodoPayments

client = DodoPayments(bearer_token=os.environ.get('DODO_PAYMENTS_API_KEY'))

response = client.subscriptions.charge(
    subscription_id="sub_123",
    product_price=2500,
)

print(response.payment_id)
```

</Tab>
<Tab title="Go SDK">

```go
package main

import (
  "context"
  "fmt"
  "github.com/dodopayments/dodopayments-go"
  "github.com/dodopayments/dodopayments-go/option"
)

func main() {
  client := dodopayments.NewClient(option.WithBearerToken("YOUR_API_KEY"))
  res, err := client.Subscriptions.Charge(context.TODO(), "sub_123", dodopayments.SubscriptionChargeParams{
    ProductPrice: dodopayments.F(int64(2500)),
  })
  if err != nil { panic(err) }
  fmt.Println(res.PaymentID)
}
```

</Tab>
<Tab title="cURL">

```bash
curl -X POST "$DODO_API/subscriptions/sub_123/charge" \
  -H "Authorization: Bearer $DODO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "product_price": 2500,
    "product_description": "Extra usage for March"
  }'
```

</Tab>
</Tabs>

```json Success
{ "payment_id": "pay_abc123" }
```

<Warning>
قد تفشل الرسوم على اشتراك لا يتمتع بميزة الطلب عند الطلب. تأكد من أن الاشتراك يحتوي على `on_demand: true` في تفاصيله قبل القيام بالرسوم.
</Warning>

## محاولات الدفع

قد يقوم نظام كشف الاحتيال لدينا بحظر أنماط المحاولة العدوانية (ويمكن أن يحددها كنماذج اختبار بطاقات محتملة). اتبع سياسة إعادة المحاولة الآمنة.

<Warning>
يمكن أن يتم تصنيف أنماط المحاولة المتفجرة على أنها احتيالية أو اختبار بطاقات مشبوهة من قبل أنظمة المخاطر ومعالجاتنا. تجنب المحاولات المتجمعة؛ اتبع جدول التراجع وإرشادات محاذاة الوقت أدناه.
</Warning>

### مبادئ سياسة إعادة المحاولة الآمنة

- **آلية التراجع**: استخدم التراجع المتزايد بين المحاولات.
- **حدود المحاولة**: حدد الحد الأقصى لعدد المحاولات (3-4 محاولات كحد أقصى).
- **التصفية الذكية**: أعد المحاولة فقط في حالة فشل قابل للإعادة (مثل أخطاء الشبكة/الجهة المصدرة، عدم كفاية الأموال)؛ لا تعيد المحاولة مطلقًا في حالات الرفض الصارمة.
- **منع اختبار البطاقة**: لا تعيد المحاولة في حالات الفشل مثل `DO_NOT_HONOR`، `STOLEN_CARD`، `LOST_CARD`، `PICKUP_CARD`، `FRAUDULENT`، `AUTHENTICATION_FAILURE`.
- **تنويع البيانات الوصفية (اختياري)**: إذا كنت تدير نظام الإعادة الخاص بك، قم بتمييز المحاولات عبر البيانات الوصفية (مثل `retry_attempt`).

### جدول إعادة المحاولة المقترح (الاشتراكات)

- **المحاولة الأولى**: فورية عند إنشاء الرسوم
- **المحاولة الثانية**: بعد 3 أيام
- **المحاولة الثالثة**: بعد 7 أيام أخرى (10 أيام إجمالاً)
- **المحاولة الرابعة (الأخيرة)**: بعد 7 أيام أخرى (17 يومًا إجمالاً)

الخطوة النهائية: إذا لم يتم الدفع بعد، قم بوضع علامة على الاشتراك كغير مدفوع أو قم بإلغائه، بناءً على سياستك. قم بإخطار العميل خلال الفترة الزمنية لتحديث وسيلة الدفع الخاصة بهم.

### تجنب المحاولات المتفجرة؛ التوافق مع وقت التفويض

- قم بتعليق المحاولات على الطابع الزمني للتفويض الأصلي لتجنب سلوك "الانفجار" عبر محفظتك.
- مثال: إذا بدأ العميل فترة تجريبية أو تفويضًا في الساعة 1:10 مساءً اليوم، جدولة المحاولات المتابعة في الساعة 1:10 مساءً في الأيام التالية وفقًا لتراجعك (على سبيل المثال، +3 أيام → 1:10 مساءً، +7 أيام → 1:10 مساءً).
- بدلاً من ذلك، إذا كنت تخزن وقت الدفع الناجح الأخير `T`، جدولة المحاولة التالية في `T + X days` للحفاظ على محاذاة وقت اليوم.

<Note>
المنطقة الزمنية وDST: استخدم معيار وقت ثابت للجدولة وتحويله للعرض فقط للحفاظ على الفترات.
</Note>

### رموز الرفض التي يجب ألا تعيد المحاولة عليها

- `STOLEN_CARD`
- `DO_NOT_HONOR`
- `FRAUDULENT`
- `PICKUP_CARD`
- `AUTHENTICATION_FAILURE`
- `LOST_CARD`

<Note>
للحصول على قائمة شاملة بأسباب الرفض وما إذا كانت قابلة للتصحيح من قبل المستخدم، راجع
[أخطاء المعاملات](/api-reference/transaction-failures) الوثائق.
</Note>

<Tip>
قم بإعادة المحاولة فقط في القضايا الناعمة/المؤقتة (مثل `insufficient_funds`، `issuer_unavailable`، `processing_error`، انقطاعات الشبكة). إذا تكرر الرفض نفسه، أوقف المحاولات الإضافية.
</Tip>

### إرشادات التنفيذ (بدون كود)

- استخدم جدولة/طابور يحتفظ بطوابع زمنية دقيقة؛ قم بحساب المحاولة التالية في الوقت الدقيق للتوقيت اليومي (مثل `T + 3 days` في نفس HH:MM).
- احتفظ وارجع إلى طابع الزمن لآخر دفعة ناجحة `T` لحساب المحاولة التالية؛ لا تجمع عدة اشتراكات في نفس اللحظة.
- دائمًا قم بتقييم سبب آخر رفض؛ توقف عن المحاولات في حالات الرفض القاسية في قائمة التجاوز أعلاه.
- حدد الحد الأقصى للمحاولات المتزامنة لكل عميل ولكل حساب لمنع الارتفاعات العرضية.
- تواصل بشكل استباقي: أرسل بريدًا إلكترونيًا/رسالة نصية للعميل لتحديث طريقة دفعهم قبل المحاولة المجدولة التالية.
- استخدم البيانات الوصفية فقط من أجل الرؤية (مثل `retry_attempt`)؛ لا تحاول "تجنب" أنظمة الاحتيال/المخاطر عن طريق تدوير الحقول غير الجوهرية.

## تتبع النتائج باستخدام الويب هوكس

قم بتنفيذ معالجة الويب هوك لتتبع رحلة العميل. راجع [تنفيذ الويب هوكس](/developer-resources/integration-guide#implementing-webhooks).

- **subscription.active**: تم تفويض التفويض وتم تفعيل الاشتراك
- **subscription.failed**: فشل الإنشاء (مثل فشل التفويض)
- **subscription.on_hold**: تم وضع الاشتراك في حالة تعليق (مثل الحالة غير المدفوعة)
- **payment.succeeded**: تم تحصيل الرسوم بنجاح
- **payment.failed**: فشل تحصيل الرسوم

<Tip>
بالنسبة للتدفقات عند الطلب، ركز على `payment.succeeded` و `payment.failed` لمصالحة الرسوم المعتمدة على الاستخدام.
</Tip>

## الاختبار والخطوات التالية

<Steps>
<Step title="إنشاء في وضع الاختبار">
استخدم مفتاح API الخاص بك في وضع الاختبار لإنشاء الاشتراك باستخدام `payment_link: true`، ثم افتح الرابط واكمل التفويض.
</Step>
<Step title="تفعيل الرسوم">
قم باستدعاء نقطة نهاية الرسوم بمبلغ صغير `product_price` (مثل `100`) وتحقق أنك تتلقى `payment.succeeded`.
</Step>
<Step title="اذهب للعيش">
قم بالتبديل إلى مفتاح API الحي الخاص بك بمجرد التحقق من الأحداث والتحديثات الداخلية للحالة.
</Step>
</Steps>

## استكشاف الأخطاء وإصلاحها

- **422 طلب غير صالح**: تأكد من توفير `on_demand.mandate_only` عند الإنشاء و `product_price` في حالة الرسوم.
- **أخطاء العملة**: إذا كنت override `product_currency`، تأكد من أنه مدعوم لحسابك وعميلك.
- **لم يتم تلقي الويب هوكس**: تحقق من عنوان URL الخاص بالويب هوك وتكوين سر التوقيع.
