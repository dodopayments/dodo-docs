...
يثبّت المبلغ المُحصّل، بوحدات العملة الأساسية (مثل <code>12.5</code> مقابل 12.50 دولارًا). ينطبق على منتجات Pay What You Want فقط، ويتم تجاهله إذا كان أقل من الحد الأدنى لسعر المنتج.
</ParamField>
...
<Warning>
يعمل Dynamic Checkout كوسيط لنقطتي النهاية المهجورتين `POST /payments` و`POST /subscriptions`. ويستمر في العمل مع عمليات التكامل الحالية، ولكن ينبغي لعمليات التكامل الجديدة استخدام **Checkout Sessions** أدناه.
</Warning>

### تنسيق الاستجابة
يعرض Dynamic checkout استجابة JSON تحتوي على عنوان URL لإتمام الدفع:
...
توفر Checkout sessions تجربة دفع مستضافة وأكثر أمانًا، تتولى معالجة دورة الدفع الكاملة لكل من عمليات الشراء لمرة واحدة والاشتراكات، مع تحكم كامل في التخصيص.
...
### تنسيق الاستجابة
تعرض Checkout sessions استجابة JSON تحتوي على عنوان URL لإتمام الدفع:
...
يتيح لك معالج مسار Customer Portal دمج بوابة عملاء Dodo Payments بسلاسة في تطبيق SvelteKit الخاص بك.
...
- **Signature Verification:** يتحقق من توقيع Webhook باستخدام <code>webhookKey</code>. ويُرجع الحالة 401 إذا فشل التحقق.
- **Payload Validation:** تتم عملية التحقق باستخدام Zod. وتُرجع الحالة 400 للبيانات غير الصالحة.
- **Error Handling:**
  - 401: توقيع غير صالح
  - 400: بيانات غير صالحة
  - 500: خطأ داخلي أثناء التحقق
- **Event Routing:** يستدعي معالج الأحداث المناسب استنادًا إلى نوع البيانات.
