...
**سعر متكرر**: إدخال → `49.00`

**دورة الفوترة**: اختر → `Monthly`

**العملة**: اختر عملتك المفضلة (على سبيل المثال، `USD`)
...
قم بإنشاء ملف `.env` مع مفتاح واجهة برمجة التطبيقات الخاصة بك من Dodo Payments:
...
<Warning>
لا تقم أبداً بتسجيل مفتاح واجهة برمجة التطبيقات الخاصة بك في التحكم في النسخ. أضف `.env` إلى ملف `.gitignore` الخاص بك.
</Warning>
</Step>
...
<Check>
**تم تكوين الملفات الثابتة!** انتقل إلى `http://localhost:3000` لرؤية واجهة العرض التجريبية الخاصة بك.
</Check>
</Step>
</Steps>
...
<Steps>
<Step title="بدء الخادم الخاص بك">
1. تأكد من وجود ملف `.env` مع مفتاح واجهة برمجة التطبيقات الصحيح
2. تحديث معرفات المنتج والإضافة في الكود الخاص بك مع القيم الفعلية من لوحة معلومات Dodo Payments الخاصة بك
3. بدء الخادم الخاص بك:
...
1. افتح المتصفح لديك وانتقل إلى `http://localhost:3000`
2. يجب أن ترى واجهة العرض التجريبية القابلة للتسعير حسب المقعد
3. جرب كميات مختلفة من المقاعد (0، 3، 10، إلخ.)
