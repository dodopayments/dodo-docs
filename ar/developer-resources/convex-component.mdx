...
<CardGroup cols={2}>
<Card title="وظيفة الدفع" icon="cart-shopping" href="#checkout-function">
    دمج عملية الدفع من Dodo مع تدفق يعتمد على الجلسة.
</Card>
<Card title="بوابة العملاء" icon="user" href="#customer-portal-function">
    السماح للعملاء بإدارة الاشتراكات والتفاصيل.
</Card>
<Card title="الويب هوكس" icon="bell" href="#webhook-handler">
    استلام ومعالجة أحداث الويب هوك من Dodo Payments.
</Card>
</CardGroup>
...
<Step title="إضافة المكون إلى إعدادات Convex">
أضف مكون مدفوعات Dodo إلى إعدادات Convex الخاصة بك:
...
بعد تعديل `convex.config.ts`، قم بتشغيل `npx convex dev` مرة واحدة لتوليد الأنواع اللازمة.
</Step>
<Step title="إعداد متغيرات البيئة">
قم بإعداد متغيرات البيئة في لوحة معلومات Convex الخاصة بك (**الإعدادات** → **متغيرات البيئة**). يمكنك الوصول إلى لوحة المعلومات عن طريق تشغيل:
...
- `DODO_PAYMENTS_API_KEY` - مفتاح واجهة برمجة التطبيقات لدفع دودي الخاص بك
- `DODO_PAYMENTS_ENVIRONMENT` - اضبطه على `test_mode` أو `live_mode`
- `DODO_PAYMENTS_WEBHOOK_SECRET` - سر webhook الخاص بك (مطلوب لمعالجة webhook)

<Warning>
استخدم دائمًا متغيرات البيئة في Convex للمعلومات الحساسة. لا تقم أبدًا بالتزام الأسرار في التحكم في الإصدارات.
</Warning>
</Step>
</Steps>
...
<Steps>
<Step title="إنشاء استعلام داخلي">
أولاً، قم بإنشاء استعلام داخلي لجلب العملاء من قاعدة بياناتك. سيتم استخدام هذا في وظائف الدفع لتحديد العملاء.

<Warning>
قبل استخدام هذا الاستعلام، تأكد من تحديد المخطط المناسب في ملف `convex/schema.ts` الخاص بك أو تغيير الاستعلام ليتناسب مع المخطط الموجود لديك.
</Warning>
...
<Step title="تكوين مكون DodoPayments">
 <CodeGroup>
...
<Tabs>
<Tab title="إعداد وظيفة الدفع">
  <Info>
  استخدم هذه الوظيفة لدمج عملية الدفع من Dodo في تطبيق Convex الخاص بك. تستخدم عملية الدفع المعتمدة على الجلسة مع دعم كامل للميزات.
  </Info>
  <CodeGroup>
...
<Tab title="إعداد بوابة العملاء">
  <Info>
  استخدم هذه الوظيفة للسماح للعملاء بإدارة اشتراكاتهم وتفاصيلهم عبر بوابة العملاء لدفع دودي. يتم التعرف على العميل تلقائيًا عبر وظيفة `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="إعداد معالج webhook">
  <Info>
  استخدم هذا المعالج لتلقي ومعالجة أحداث webhook لدفع دودي بأمان في تطبيق Convex الخاص بك. يتلقى جميع معالجات webhook الـ Convex `ActionCtx` كأول معامل، مما يتيح لك استخدام `ctx.runQuery()` و `ctx.runMutation()` للتفاعل مع قاعدة بياناتك.
  </Info>
  <CodeGroup>
  ...
  <Warning>
  تأكد من تعريف التحولات المقابلة في قاعدة بيانات Convex الخاصة بك لكل حدث ويب هوك تريد معالجته. على سبيل المثال، أنشئ تحول <code>createPayment</code> لتسجيل المدفوعات الناجحة أو تحول <code>createSubscription</code> لإدارة حالة الاشتراك.
  </Warning>
</Tab>
</Tabs>
...
<Info>
يستخدم مكون Convex لمدفوعات Dodo عملية دفع تعتمد على الجلسة، مما يوفر تجربة دفع آمنة وقابلة للتخصيص مع عربات منتجات مُعدة مسبقًا وتفاصيل العملاء. هذه هي الطريقة الموصى بها لجميع تدفقات الدفع.
</Info>
...
إذا تم تعيينه إلى <code>true</code>، يتم إرسال بريد إلكتروني إلى العميل مع رابط البوابة.
</ParamField>

<Info>
يتم التعرف على العميل تلقائيًا باستخدام وظيفة `identify` المكونة في إعداد الدفع لدودي الخاص بك. يجب أن تُرجع هذه الوظيفة `dodoCustomerId` للعميل.
</Info>
...
<Info>
استخدم وظيفة الدفع من مكونات React الخاصة بك مع خطافات Convex.
</Info>
