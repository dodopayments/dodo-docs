...
قم بإعداد متغيرات البيئة في لوحة تحكم Convex الخاصة بك (**الإعدادات** → **متغيرات البيئة**). يمكنك الوصول إلى لوحة التحكم عن طريق تشغيل:
...
- `DODO_PAYMENTS_API_KEY` - مفتاح API الخاص بـ Dodo Payments
...
<Warning>
قبل استخدام هذا الاستعلام، تأكد من تعريف المخطط المناسب في ملفك `convex/schema.ts` أو تغيير الاستعلام ليتناسب مع المخطط الموجود لديك.
</Warning>
...
<Tab title="إعداد بوابة العملاء">
  <Info>
  استخدم هذه الوظيفة للسماح للعملاء بإدارة اشتراكاتهم وتفاصيلهم عبر بوابة عملاء Dodo Payments. يتم التعرف على العميل تلقائيًا عبر وظيفة `identify`.
  </Info>
  <CodeGroup>
...
<Tab title="إعداد معالج الويب هوك">
  <Info>
  استخدم هذا المعالج لتلقي ومعالجة أحداث الويب هوك الخاصة بـ Dodo Payments بشكل آمن في تطبيق Convex الخاص بك. جميع معالجات الويب هوك تتلقى `ActionCtx` كأول معلمة، مما يتيح لك استخدام `ctx.runQuery()` و`ctx.runMutation()` للتفاعل مع قاعدة بياناتك.
  </Info>
  <CodeGroup>
...
<Info>
يتم التعرف على العميل تلقائيًا باستخدام وظيفة `identify` المكونة في إعدادات DodoPayments الخاصة بك. يجب أن تعيد هذه الوظيفة `dodoCustomerId` للعميل.
</Info>
