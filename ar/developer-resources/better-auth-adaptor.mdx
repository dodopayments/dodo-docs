---
title: محول Better-Auth
description: يوضح هذا الدليل كيفية دمج مدفوعات دودي في تدفق المصادقة الخاص بك
  باستخدام محول Better-Auth.
icon: code-merge
---

# نظرة عامة

يوفر <b>محول Better Auth</b> لمدفوعات دودي:

- إنشاء العملاء تلقائيًا عند التسجيل
...
- مشروع موجود يستخدم [better-auth](https://www.npmjs.com/package/better-auth)

# التثبيت

<Steps>
{/* LOCKED_PATTERN_855c4055647e4dbb75296080183cd51d */}
شغل الأمر التالي في جذر مشروعك:
...
أنشئ أو حدّث <code>src/lib/auth.ts</code>:
...
<Tip>
عيّن <code>environment</code> على <code>live_mode</code> للإنتاج.
</Tip>
</Step>

{/* LOCKED_PATTERN_673a79a3fa2d75e23b52c6942c035dd2 */}
أنشئ أو حدّث <code>src/lib/auth-client.ts</code>:
...
<Info>
يفضَّل استخدام <code>authClient.dodopayments.checkoutSession</code> للتكاملات الجديدة. إن طريقة <code>checkout</code> القديمة مُهملة وتُحفظ فقط للتوافق مع الإصدارات السابقة.
</Info>

## إنشاء جلسة دفع (المفضلة)
...
<Info>
على عكس طريقة <code>checkout</code> القديمة، <code>checkoutSession</code> لا تتطلب معلومات الفوترة مُسبقًا إذ ستُؤخذ من المستخدم في صفحة الدفع. لا يزال بإمكانك تجاوزها بتمرير مفتاح <code>billing</code> في الوسيط.
</Info>

<Info>
مشابه للطريقة القديمة، يأخذ <code>checkoutSession</code> بريد العميل الإلكتروني واسمهم من جلسة better-auth، ومع ذلك يمكنك تجاوزه بتمرير كائن <code>customer</code> يحتوي على البريد الإلكتروني والاسم.
</Info>

<Info>
يمكنك تمرير نفس الخيارات في الوسيط كما في جسم الطلب لواجهة [إنشاء جلسة دفع](https://docs.dodopayments.com/api-reference/checkout-sessions/create).
</Info>

<Note>
يُؤخذ رابط العودة من <code>successUrl</code> المُكوَّن في البرنامج المُساعد على الخادم. لا تحتاج إلى تضمين <code>return_url</code> في حمولة العميل.
</Note>

## الدفع القديم (مُهمل)

<Warning>
الطريقة <code>authClient.dodopayments.checkout</code> مُهملة. استخدم <code>checkoutSession</code> بدلاً منها في التنفيذات الجديدة.
</Warning>
...
قم بتمكين ملحق <code>usage()</code> على الخادم لالتقاط الأحداث المعتمدة على القياس ودع العملاء يفحصون فواتيرهم المعتمدة على الاستخدام.

- <code>authClient.dodopayments.usage.ingest</code> يستوعب الأحداث للمستخدم المسجل، الذي تم التحقق من بريده الإلكتروني.
- <code>authClient.dodopayments.usage.meters.list</code> يسرد الاستخدام الأخير للعدادات المرتبطة باشتراكات ذلك العميل.
...
<Tip>
يُرفض أي طابع زمني أقدم من ساعة أو يزيد عن خمسة دقائق في المستقبل عند تسجيل الاستخدام.
</Tip>
<Tip>
إذا حذفت <code>meter_id</code> عند سرد عدادات الاستخدام، فسيتم إرجاع جميع العدادات المرتبطة باشتراكات العميل.
</Tip>

# أحداث الويب

<Info>
يتعامل ملحق الويب هوكس مع أحداث الدفع في الوقت الحقيقي من Dodo Payments مع التحقق الآمن من التوقيع. النقطة النهائيّة الافتراضية هي <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
{/* LOCKED_PATTERN_1f3f4c5fbf255932f2c0aedf7d16c279 */}
أنشئ سر ويب هوك لعنوان نقطة النهاية الخاص بك (مثلاً <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) في لوحة تحكم Dodo Payments وضعه في ملف <code>.env</code> الخاص بك:
...
<AccordionGroup>
  {/* LOCKED_PATTERN_9586fa213a07317c452129bb9c8365ad */}
    - <b>client</b> (مطلوب): مثيل عميل DodoPayments - <b>createCustomerOnSignUp</b> (اختياري): إنشاء العملاء تلقائيًا عند تسجيل المستخدم - <b>use</b> (مطلوب): مصفوفة من الإضافات لتفعيلها (checkout, portal, usage, webhooks)
  </Accordion>
  {/* LOCKED_PATTERN_a313db3f1395a5521aceb89c3251fabd */}
    - <b>products</b>: مصفوفة من المنتجات أو دالة غير متزامنة تعيد المنتجات - <b>successUrl</b>: عنوان URL لإعادة التوجيه بعد الدفع الناجح - <b>authenticatedUsersOnly</b>: يتطلب مصادقة المستخدم (الافتراضي: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  {/* LOCKED_PATTERN_8cb977e6bc7a95fb0f5c0e3c34015b67 */}
    - <b>مفتاح API غير صالح</b>: تحقق جيدًا من <code>DODO_PAYMENTS_API_KEY</code> في <code>.env</code>. - <b>تعارض توقيع الويب هوك</b>: تأكد من أن سر الويب هوك يطابق ذلك الموجود في لوحة تحكم Dodo Payments. - <b>لم يتم إنشاء العميل</b>: تأكد من تعيين <code>createCustomerOnSignUp</code> على <code>true</code>.
  </Accordion>
  {/* LOCKED_PATTERN_328949dc9617c5646d2623ce7f82eb78 */}
    - استخدم متغيرات البيئة لجميع الأسرار والمفاتيح. - اختبر في <code>test_mode</code> قبل التحويل إلى <code>live_mode</code>. - سجّل أحداث الويب هوك لأغراض التصحيح والتدقيق.
  </Accordion>
</AccordionGroup>
