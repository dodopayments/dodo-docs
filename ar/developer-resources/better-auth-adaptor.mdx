...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (required): ديدو بايمنتس عميل
    - <b>createCustomerOnSignUp</b> (optional): إنشاء العملاء تلقائيًا عند تسجيل المستخدم
    - <b>use</b> (required): مجموعة من الإضافات لتمكين (الدفع، البوابة، الاستخدام، الويب هوكس)
    - <b>getCustomerParams</b> (optional): دالة تستقبل BetterAuth `User` وتعيد الحقول الإضافية لإرفاقها مع عميل ديدو بايمنتس عند الإنشاء والتحديث (مثل `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b>: مجموعة من المنتجات أو دالة غير متزامنة تعيد المنتجات -{" "}
    <b>successUrl</b>: رابط لإعادة التوجيه بعد الدفع الناجح -{" "}
    <b>authenticatedUsersOnly</b>: تتطلب مصادقة المستخدم (الافتراضي: false)
  </Accordion>
</AccordionGroup>

# تحري الخلل وإصلاحه ونصائح

<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>مفتاح API غير صالح</b>: تحقق من الخاصية {" "}
    <code>DODO_PAYMENTS_API_KEY</code> في <code>.env</code>. -{" "}
    <b>عدم تطابق توقيع الويب هوك</b>: تأكد من أن السر الذي يستخدم مع الويب هوك يطابق السر المحدد في لوحة ديدو بايمنتس. - <b>عدم إنشاء العميل</b>: تأكد من أن {" "}
    <code>createCustomerOnSignUp</code> مضبوط على <code>true</code>.
  </Accordion>
  <Accordion title="Best Practices">
    - استخدم متغيرات البيئة لجميع الأسرار والمفاتيح. - اختبر في {" "}
    <code>test_mode</code> قبل التحويل إلى <code>live_mode</code>. - سجل
    أحداث الويب هوك للتصحيح والتدقيق.
  </Accordion>
</AccordionGroup>

# توجيه لنماذج الذكاء الاصطناعي
