...
<Info>
  على عكس طريقة <code>checkout</code> القديمة، لا يتطلب <code>checkoutSession</code>
  معلومات الفوترة مقدمًا حيث سيتم أخذها من المستخدم في صفحة الدفع. يمكنك مع ذلك تجاوزها عن طريق تمرير مفتاح
  <code>billing</code> في الوسيط.
</Info>

<Info>
  على غرار طريقة <code>checkout</code> القديمة، يأخذ <code>checkoutSession</code>
  البريد الإلكتروني والاسم من جلسة التوثيق النموذجي للمستخدم، ومع ذلك، يمكنك تجاوزها عن طريق تمرير
  <code>customer</code> مع البريد الإلكتروني والاسم.
</Info>

<Info>
  يمكنك تمرير نفس الخيارات في الوسيط كجسم الطلب لنقطة النهاية [Create Checkout Session](https://docs.dodopayments.com/api-reference/checkout-sessions/create).
</Info>

<Note>
  يتم أخذ عنوان URL للعودة من <code>successUrl</code> المحفوظة في المكون الملحق بالخادم. لا تحتاج لإدراج <code>return_url</code> في حمولة العميل.
</Note>

## عملية الدفع القديمة (مهمشة)

<Warning>
  تم تهميش طريقة <code>authClient.dodopayments.checkout</code>. استخدم
  <code>checkoutSession</code> لأجل الواجهات الجديدة.
</Warning>
...
## تتبع الاستخدام المحسوب

تمكين مكون <code>usage()</code> على الخادم لالتقاط الأحداث المحسوبة وترك العملاء يفحصون فوترتهم المستندة إلى الاستخدام.

- يستوعب <code>authClient.dodopayments.usage.ingest</code> الأحداث للمستخدم المسجل والمتحقق من البريد الإلكتروني.
- <code>authClient.dodopayments.usage.meters.list</code> يسرد الاستخدام الأخير لأجهزة القياس المرتبطة باشتراكات ذلك العميل.
...
<Tip>
  يتم رفض الطوابع الزمنية الأقدم من ساعة أو أكثر من خمس دقائق في المستقبل عند استيعاب الاستخدام.
</Tip>
<Tip>
  إذا قمت بإغفال <code>meter_id</code> عند سرد أجهزة قياس الاستخدام، يتم إرجاع جميع الأجهزة المرتبطة باشتراكات العميل.
</Tip>

# الويب هوكس

<Info>
  يعالج مكون الويب هوكس أحداث الدفع في الوقت الحقيقي من Dodo Payments مع التحقق الآمن من التوقيع. نقطة النهاية الافتراضية هي
  <code>/api/auth/dodopayments/webhooks</code>.
</Info>

<Steps>
<Step title="Generate and set webhook secret">
انشئ سر ويب هوك لعنوان URL لنقطة النهاية الخاصة بك (مثل <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) في لوحة Dodo Payments واضبطه في ملف <code>.env</code> الخاص بك.
...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (إلزامي): مثيل عميل DodoPayments
    - <b>createCustomerOnSignUp</b> (اختياري): إنشاء العملاء تلقائيًا عند تسجيل المستخدم
    - <b>use</b> (إلزامي): مصفوفة من المكونات الإضافية للتمكين (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (اختياري): دالة تستقبل `User` BetterAuth وتعيد حقول إضافية لإلصاقها بعميل DodoPayments عند الإنشاء والتحديث (مثل `metadata`, `phone_number`)
  ...
  <Accordion title="Checkout Plugin Options">
    - <b>products</b>: مصفوفة من المنتجات أو دالة غير متزامنة تعيد المنتجات - <b>successUrl</b>: عنوان URL لإعادة التوجيه بعد الدفع الناجح - <b>authenticatedUsersOnly</b>: يتطلب توثيق المستخدم (الافتراضي: false)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>رمز API غير صالح</b>: تحقق من <code>DODO_PAYMENTS_API_KEY</code> في <code>.env</code>. - <b>عدم تطابق توقيع ويب هوك</b>: تأكد من أن سر الويب هوك يتطابق مع الذي تم تعيينه في لوحة Dodo Payments. - <b>العميل غير منشئ</b>: تأكد من ضبط
    <code>createCustomerOnSignUp</code> على <code>true</code>.
  </Accordion>
  <Accordion title="Best Practices">
    - استخدم متغيرات البيئة لجميع الأسرار والمفاتيح. - اختبر في
    <code>test_mode</code> قبل التحويل إلى <code>live_mode</code>. - سجل أحداث الويب هوك لأغراض التصحيح والتدقيق.
  </Accordion>
</AccordionGroup>

# المطالبة لموديلات الذكاء الاصطناعي اللغوية
