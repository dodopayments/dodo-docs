...
## Menguji Metode Eropa

Metode pembayaran Eropa (iDEAL, Bancontact, EPS, Multibanco) dapat diuji dalam sandbox mode.
...
Jangan hanya menguji pembayaran yang berhasil. Uji penolakan, pembatalan, dan kasus edge seperti saldo tidak mencukupi.
</Accordion>

<Accordion title="Verify webhook handling">
Pastikan endpoint webhook Anda memproses semua jenis event dengan benar, terutama `payment.succeeded`, `payment.failed`, dan event subscription.
...
- `dodo wh trigger` mengirim payload tiruan untuk semua jenis event webhook yang didukung
</Accordion>

<Accordion title="Test on real devices">
Untuk Apple Pay dan Google Pay, lakukan pengujian di perangkat iOS dan Android sebenarnya. Simulator tidak sepenuhnya mereplikasi perilaku wallet.
</Accordion>
