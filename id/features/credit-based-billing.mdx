...
- **Mulai sederhana**: Mulailah dengan satu jenis kredit dan tanpa rollover. Tambahkan kompleksitas berdasarkan umpan balik pelanggan dan pola penggunaan.
- **Tetapkan ekspektasi yang jelas**: Tampilkan alokasi kredit, saldo tersisa, dan harga overage secara jelas di halaman produk dan portal pelanggan Anda.
- **Gunakan satuan yang bermakna**: Beri nama kredit sesuai dengan apa yang mereka wakili (misalnya, "API Calls", "AI Tokens") daripada istilah umum. Ini membantu pelanggan memahami nilai.
- **Konfigurasikan kedaluwarsa dengan bijak**: Jendela kedaluwarsa pendek (7 hari) mendorong urgensi tetapi dapat membuat pelanggan frustrasi. Jendela lebih lama (30–90 hari) lebih ramah pelanggan untuk kebanyakan produk SaaS.
...
- **Uji dalam mode pengujian**: Buat kredit, lampirkan ke produk uji, dan simulasi siklus penuh pembelian → penggunaan → pengurangan → kadaluwarsa sebelum tayang.

<Info>
Penagihan Berbasis Kredit bekerja mulus dengan semua fitur Dodo Payments lainnya - langganan dengan masa percobaan, perubahan paket dengan prorata, dan portal pelanggan. Mulailah dengan pengaturan dasar dan kembangkan seiring evolusi model harga Anda.
</Info>
