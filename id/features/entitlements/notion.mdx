...
2. Dodo Payments membuat grant dalam status `Pending` dengan `oauth_url` Notion agar pelanggan dapat menghubungkan workspace mereka.
3. Pelanggan melakukan otorisasi melalui tautan di email atau Customer Portal mereka, lalu memilih workspace tempat template akan digandakan.
4. Dodo Payments menggandakan template. Grant berpindah ke `Delivered`.
5. Saat pencabutan, grant ditandai sebagai `Revoked`; salinan yang digandakan tetap berada di workspace pelanggan.
