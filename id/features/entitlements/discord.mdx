...
2. Dodo Payments membuat grant dalam status `Pending` dengan `oauth_url` yang mengarah ke Discord.
3. Pelanggan menerima email dengan tombol "Bergabung ke Discord" (tautan OAuth). Tautan tersebut juga terlihat di Customer Portal mereka.
...
5. Jika subscription dibatalkan, dijeda, atau berakhir, atau jika merchant mencabutnya secara manual, bot menghapus role tersebut dan grant berpindah ke `Revoked`.
...
- `entitlement_grant.created` dipicu dengan `status: "Pending"` dan `oauth_url` agar pelanggan memberikan otorisasi.
- `entitlement_grant.delivered` dipicu setelah role ditetapkan.
- `entitlement_grant.revoked` dipicu ketika role dihapus.
