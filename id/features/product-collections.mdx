...
|---------|-------------|---------|
| **Izinkan Multiple Subscriptions** | Customer dapat memiliki lebih dari satu subscription aktif pada saat yang sama | Diaktifkan |
| **Izinkan Pembaruan Subscription** | Customer dapat melakukan upgrade atau downgrade subscription yang sudah ada kapan saja melalui Customer Portal | Dinonaktifkan |
| **Izinkan Pembatalan Subscription** | Customer dapat membatalkan subscription mereka sendiri dari Customer Portal | Diaktifkan |
| **Cegah Penyalahgunaan Trial** | Customer yang sudah pernah menukarkan trial akan dialihkan ke pembelian berbayar tanpa trial, bukan menerima trial baru | Dinonaktifkan |
...
<Info>
Untuk mengetahui detail tentang cara penukaran trial dicocokkan dan dicatat, lihat [Mencegah Penyalahgunaan Trial](/features/subscription#preventing-trial-misuse).
</Info>

<Card title="Subscription Plan Changes" icon="repeat" href="/features/subscription#subscription-plan-changes">
  Pelajari lebih lanjut tentang mode proration dan perilaku perubahan plan.
</Card>

## Mengelola Collection

Product Collection dapat dikelola melalui dashboard Dodo Payments atau secara programatik melalui API. API memberikan kontrol penuh atas pembuatan collection, pembaruan, upload gambar, pengarsipan, serta pengelolaan group dan product bertingkat.

### Operasi Dashboard

- **Buat**: Siapkan collection baru dengan product dan group
- **Perbarui**: Ubah nama, deskripsi, gambar, dan pengaturan product
- **Atur ulang**: Seret dan lepas untuk mengubah urutan tampilan product
- **Aktifkan/Nonaktifkan product**: Kontrol product yang ditampilkan saat checkout
...
Endpoint berikut memungkinkan Anda membuat, memperbarui, mengambil, mengarsipkan, dan mengatur collection product secara programatik — termasuk mengelola group bertingkat dan product di dalamnya.

<AccordionGroup>
<Accordion title="Listing Product Collections">
Ambil semua collection product yang terkait dengan akun Anda menggunakan request `GET` ke endpoint `/product-collections`. Mendukung pagination, filtering berdasarkan brand, dan penyertaan collection yang diarsipkan.
...
Buat collection product baru dengan mengirimkan request `POST` ke endpoint `/product-collections` yang berisi detail seperti nama, deskripsi, dan brand.
...
Dapatkan informasi mendetail tentang collection product tertentu — termasuk group dan item product di dalamnya — menggunakan request `GET` ke endpoint `/product-collections/{id}`.
...
Ubah detail collection product (nama, deskripsi, brand, dan sebagainya) dengan mengirimkan request `PATCH` ke endpoint `/product-collections/{id}`.
...
Hubungkan gambar dengan collection dengan mengunggahnya melalui URL yang telah ditandatangani sebelumnya. Minta URL upload dari endpoint `/product-collections/{id}/images`, lalu `PUT` gambar ke URL yang dikembalikan dalam waktu 60 detik.

<Warning>
URL yang telah ditandatangani sebelumnya akan kedaluwarsa dalam 60 detik, jadi gambar harus diunggah dalam jangka waktu tersebut.
</Warning>
...
Group memungkinkan Anda mengatur product di dalam collection (misalnya, "Paket Bulanan" vs. "Paket Tahunan"). Gunakan endpoint group untuk menambahkan, memperbarui, atau menghapus group dalam collection.
...
<Card title="Create Group" icon="code" href="/api-reference/product-collections/create-group">
  Tambahkan group baru ke collection product.
</Card>
...
Kelola setiap item product di dalam group — tambahkan product baru, perbarui item yang sudah ada (seperti urutan tampilan), atau hapus item tersebut sepenuhnya.
...
- **Atur urutan secara strategis**: Tempatkan plan paling populer atau yang direkomendasikan di urutan pertama karena akan dipilih sebelumnya saat checkout
- **Gunakan penamaan yang jelas**: Nama product harus menyampaikan perbedaan nilai dengan jelas
...
<Check>
Anda siap membuat collection product dan menawarkan pengalaman pemilihan plan terpadu kepada customer.
</Check>

<CardGroup cols={2}>
<Card title="Products" icon="box" href="/features/products">
  Buat product one-time, subscription, atau berbasis usage untuk ditambahkan ke collection.
</Card>
