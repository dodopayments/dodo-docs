...
Ketika seorang pelanggan membeli produk mode manual, grant dibuat dalam status `pending` tanpa kunci dan webhook [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) terpicu dengan `integration_type: "license_key"` dan `status: "pending"`. Anda dapat bereaksi terhadap webhook tersebut, atau mem-polling endpoint [List Customer Grants](/api-reference/entitlements/list-customer-grants) dengan filter `integration_type` dan `status`:
