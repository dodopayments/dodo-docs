...
| Event | Behavior |
|---|---|
| `payment.succeeded` (one-time) | Buat satu key untuk setiap `quantity` yang dibeli. Masa berlaku key mengikuti durasi entitlement. |
| `subscription.active` | Buat satu key untuk setiap `quantity` langganan (seat). Key tidak memiliki masa berlaku; validitasnya terkait dengan status langganan. |
| `subscription.renewed` | No-op. Key yang ada tetap dipertahankan. |
| `subscription.on_hold` | Nonaktifkan key. Key akan diaktifkan kembali saat langganan tidak lagi ditangguhkan. |
| `subscription.paused` | Nonaktifkan key. Key akan diaktifkan kembali saat langganan dilanjutkan. |
| `subscription.cancelled` / `expired` | Nonaktifkan key secara permanen. |
| `subscription.plan_changed` | Nonaktifkan key lama; terbitkan key baru untuk plan baru. |
| `refund.succeeded` (one-time) | Nonaktifkan key. |
| Pencabutan manual melalui API/dashboard | Nonaktifkan key dengan `revocation_reason: manual`. Key ini tidak diberikan kembali secara otomatis saat langganan diperbarui. |
| License key dinonaktifkan secara langsung | Cabut grant dengan `revocation_reason: license_key_disabled`. Mengaktifkan kembali key akan mengaktifkan kembali grant secara otomatis. |
...
Gunakan field `source` pada record license key untuk membedakan inventaris yang dimigrasikan dan key yang dipenuhi secara manual dari key yang diterbitkan secara organik saat melakukan rekonsiliasi atau audit. Baca field tersebut dari objek `license_key` pada grant yang dikembalikan oleh [List Grants](/api-reference/entitlements/list-grants); endpoint lama `GET /license_keys` sudah deprecated.
...
<Warning>
`GET /license_keys`, `GET /license_keys/{id}`, dan `PATCH /license_keys/{id}` sudah deprecated. Gunakan endpoint entitlement grant ([List Grants](/api-reference/entitlements/list-grants), [List Customer Grants](/api-reference/entitlements/list-customer-grants)) untuk membaca data. `POST /license_keys` tetap didukung untuk mengimpor key yang sudah ada.
</Warning>

<CardGroup cols={2}>
<Card title="Create License Key" icon="code" href="/api-reference/licenses/create-license-key">
Buat license key baru atau impor license key yang sudah ada.
</Card>

<Card title="List License Keys" icon="code" href="/api-reference/licenses/list-license-keys">
Telusuri semua key beserta detail status dan penggunaannya.
</Card>

<Card title="Get License Key" icon="code" href="/api-reference/licenses/get-license-key">
Ambil key tertentu beserta metadata-nya.
</Card>

<Card title="Update License Key" icon="code" href="/api-reference/licenses/update-license-key">
Ubah masa berlaku, batas aktivasi, atau status aktif/nonaktif key.
</Card>
</CardGroup>
</Accordion>

<Accordion title="Entitlement Management">
Kelola entitlement License Key itu sendiri: batas aktivasi, durasi, dan instruksinya.

<CardGroup cols={2}>
<Card title="Create Entitlement" icon="plus" href="/api-reference/entitlements/create-entitlement">
Buat entitlement License Key.
</Card>
<Card title="Update Entitlement" icon="pen" href="/api-reference/entitlements/update-entitlement">
Perbarui konfigurasi entitlement.
</Card>
<Card title="List Grants" icon="users" href="/api-reference/entitlements/list-grants">
Tampilkan daftar key yang diterbitkan untuk sebuah entitlement.
</Card>
<Card title="Revoke Grant" icon="ban" href="/api-reference/entitlements/revoke-grant">
Cabut key pelanggan secara manual.
</Card>
</CardGroup>
</Accordion>
</AccordionGroup>
...
Pengiriman dan pencabutan license key memicu empat [`entitlement_grant.*` peristiwa webhook](/developer-resources/webhooks/intents/entitlement-grant). Payload grant mencakup objek `license_key` yang telah diisi dengan key, masa berlaku, jumlah aktivasi yang digunakan, dan batasnya.

Peristiwa `license_key.*` lama (`license_key.created`) tetap dipicu untuk lifecycle record license key yang mendasarinya; lihat [halaman payload webhook License Key](/developer-resources/webhooks/intents/license-key).

<Tip>
Untuk integrasi baru, dengarkan `entitlement_grant.delivered`, bukan `license_key.created`. Peristiwa entitlement memberi tahu Anda bahwa pengiriman telah selesai di semua integrasi pada produk, bukan hanya pada license key.
</Tip>
...
<Note>
Produk yang dibuat dengan flag `license_key_enabled` versi lama telah **dimigrasikan secara otomatis** ke entitlement License Key. Migrasi ini transparan: key pelanggan yang sudah ada terus berfungsi tanpa perubahan, endpoint publik `/licenses/activate`, `/licenses/validate`, `/licenses/deactivate` terus berfungsi, dan endpoint API `/license_keys/*` terus membaca dan menulis ke key store yang sama.

Bagian dashboard **License Keys** mandiri tetap tersedia sebagai daftar datar setiap key yang diterbitkan, sehingga berguna untuk audit dan pencarian. Konfigurasi baru (mengubah batas aktivasi, durasi, atau instruksi) harus dilakukan dengan mengedit entitlement License Key yang telah dimigrasikan di **Entitlements**.
</Note>
...
- **Tetapkan batas aktivasi dengan jelas**: Pilih default yang wajar (1 untuk aplikasi satu pengguna, 3–5 untuk lisensi tim) dan dokumentasikan.
- **Berikan instruksi aktivasi yang tepat**: Pelanggan menyalin instruksi ini dari email mereka, sehingga path dan command yang tepat dapat mengurangi tiket dukungan.
- **Validasi key di sisi server**: Untuk produk yang terhubung ke jaringan, lakukan validasi melalui `/licenses/validate`, bukan dengan menyimpan aktivasi secara lokal.
- **Gunakan webhook untuk pencabutan**: Dengarkan `entitlement_grant.revoked` untuk segera menonaktifkan fitur dalam aplikasi saat pelanggan membatalkan atau meminta refund.
- **Uji dengan langganan dan pembelian one-time**: Perilaku license key sedikit berbeda di antara keduanya, jadi uji keduanya sebelum ditayangkan.
