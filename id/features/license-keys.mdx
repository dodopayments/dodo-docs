...
- **`auto`** (default): Dodo Payments membuat dan mengirimkan key melalui email secara otomatis saat pembayaran atau subscription. Ini adalah perilaku yang dijelaskan di atas dan berlaku jika `fulfillment_mode` tidak disertakan.
- **`manual`**: Pembelian membuat grant `Pending` tanpa key, dan Anda menyediakan setiap nilai key sendiri. Lihat [Manual Fulfillment](#manual-fulfillment) di bawah.

### Aktivasi lisensi

<CodeGroup>

Secara default, Dodo Payments membuat dan mengirimkan license key melalui email saat pelanggan melakukan pembayaran. Dengan **manual fulfillment**, Anda menyediakan key sendiri: pembelian membuat grant `Pending` tanpa key, memberi tahu Anda, dan menunggu Anda mengirimkan nilai key. Gunakan opsi ini jika key berasal dari sistem Anda sendiri, vendor pihak ketiga, atau kumpulan kode tercetak yang jumlahnya terbatas.
...
Saat pelanggan membeli produk dalam mode manual, grant dibuat dalam status `Pending` tanpa key dan webhook [`entitlement_grant.created`](/developer-resources/webhooks/intents/entitlement-grant) dipicu dengan `integration_type: "license_key"` dan `status: "Pending"`. Anda dapat merespons webhook tersebut atau melakukan polling pada endpoint [List Customer Grants](/api-reference/entitlements/list-customer-grants) menggunakan filter `integration_type` dan `status`:
...
Kirimkan key menggunakan endpoint [Fulfill License Key Grant](/api-reference/entitlements/fulfill-license-key). Grant berpindah ke `Delivered` dan pelanggan secara otomatis dikirimi key — melalui email yang sama seperti yang akan diterimanya dalam auto fulfillment.
...
<Tip>
Parse parameter `license_key` pada halaman pengembalian Anda untuk menampilkan kunci segera, meningkatkan pengalaman purna-beli.
</Tip>
...
<AccordionGroup>
<Accordion title="Lifecycle Operations (Public Endpoints)">
Aktivasi, deaktivasi, dan validasi bersifat publik; tidak memerlukan kunci API.
...
<Card title="Deactivate License" icon="code" href="/api-reference/licenses/deactivate-license">
Batalkan aktivasi sebelumnya untuk membebaskan kapasitas.
</Card>
...
<Accordion title="License Key Management">
Buat, daftarkan, ambil, dan perbarui catatan kunci lisensi individu. Gunakan ini untuk mengimpor kunci yang ada atau mengambil detail penggunaan.

<CardGroup cols={2}>
<Card title="Create License Key" icon="code" href="/api-reference/licenses/create-license-key">
Buat kunci lisensi baru atau impor yang sudah ada.
</Card>
...
<Card title="Update License Key" icon="code" href="/api-reference/licenses/update-license-key">
Modifikasi batas kadaluarsa, aktivasi, atau aktifkan/nonaktifkan kunci.
</Card>
</CardGroup>
</Accordion>

<Accordion title="Entitlement Management">
Kelola hak kunci lisensi itu sendiri: batasan aktivasi, durasi, dan instruksi.
...
<Card title="List Grants" icon="users" href="/api-reference/entitlements/list-grants">
Daftarkan kunci yang diterbitkan untuk hak tersebut.
</Card>
<Card title="Revoke Grant" icon="ban" href="/api-reference/entitlements/revoke-grant">
Cabut manual kunci pelanggan.
</Card>
</CardGroup>
</Accordion>
</AccordionGroup>
...
Pengiriman dan pencabutan kunci lisensi akan memicu empat [`entitlement_grant.*` peristiwa webhook](/developer-resources/webhooks/intents/entitlement-grant). Payload pemberian mencakup objek `license_key` yang terisi dengan kunci, kedaluwarsa, penggunaan aktivasi, dan batasan.

Peristiwa warisan `license_key.*` (`license_key.created`) tetap berjalan untuk siklus hidup catatan kunci lisensi; lihat [halaman payload webhook Kunci Lisensi](/developer-resources/webhooks/intents/license-key).

<Tip>
Untuk integrasi baru, dengarkan `entitlement_grant.delivered` dibandingkan dengan `license_key.created`. Peristiwa hak memberi tahu Anda bahwa pengiriman selesai di semua integrasi pada produk, bukan hanya kunci lisensi.
</Tip>

---

## Kunci Lisensi Warisan

<Note>
Produk yang dibuat dengan bendera lama `license_key_enabled` telah **di-migrasikan secara otomatis** ke hak kunci lisensi. Migrasi ini transparan: kunci pelanggan yang ada tetap berfungsi tanpa perubahan, endpoint publik `/licenses/activate`, `/licenses/validate`, `/licenses/deactivate` tetap berfungsi, dan endpoint API `/license_keys/*` terus membaca dan menulis pada penyimpanan kunci yang sama.

Bagian dashboard **Kunci Lisensi** mandiri tetap tersedia sebagai daftar rata dari setiap kunci yang diterbitkan, berguna untuk audit dan pencarian. Konfigurasi baru (mengubah batasan aktivasi, durasi, atau instruksi) harus dilakukan dengan mengedit hak kunci lisensi yang dimigrasikan di bawah **Entitlements**.
</Note>
...
- **Jaga agar batas aktivasi jelas**: Pilih default yang masuk akal (1 untuk aplikasi pengguna tunggal, 3–5 untuk lisensi tim) dan dokumentasikan.
- **Berikan instruksi aktivasi yang tepat**: Pelanggan menyalin ini dari email mereka, sehingga jalur dan perintah yang tepat menghemat tiket dukungan.
- **Validasi kunci server-side**: Untuk produk yang terhubung jaringan, validasi melalui `/licenses/validate` dibandingkan dengan menyimpan aktivasi secara lokal.
- **Gunakan webhooks untuk pencabutan**: Dengar `entitlement_grant.revoked` untuk menonaktifkan fitur dalam aplikasi segera ketika pelanggan membatalkan atau mengembalikan uang.
- **Uji dengan langganan dan pembelian satu kali**: Perilaku kunci lisensi berbeda sedikit antara keduanya, jadi uji keduanya sebelum live.
