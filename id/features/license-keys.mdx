...
Ketika seorang pelanggan menyelesaikan pembelian untuk produk dengan kunci lisensi diaktifkan, kunci lisensi tersebut secara otomatis ditambahkan ke `return_url` Anda sebagai parameter kueri. Ini memungkinkan Anda menampilkan kunci tersebut langsung di halaman sukses Anda tanpa perlu melakukan panggilan API tambahan.

```text
https://yoursite.com/return?payment_id=pay_xxx&status=succeeded&license_key=LK-001&email=customer%40example.com
```

Jika pembelian menghasilkan beberapa kunci lisensi (misalnya, kuantitas > 1), mereka dipisahkan dengan koma:
...
<Tip>
Analisis parameter `license_key` di halaman pengembalian Anda untuk segera menunjukkan kunci kepada pelanggan, meningkatkan pengalaman pasca pembelian mereka.
</Tip>

## Praktik Terbaik

- **Tetap jelas mengenai batasan**: Pilih default yang masuk akal untuk kedaluwarsa dan aktivasi
- **Panduan pengguna**: Berikan instruksi aktivasi yang tepat dan dokumentasi mandiri
- **Otomatisasi pengecekan**: Validasi kunci di sisi server sebelum memberikan akses
- **Pantau kejadian**: Gunakan webhooks untuk mendeteksi penyalahgunaan dan otomatisasi pencabutan
