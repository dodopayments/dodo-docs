...
Langganan Team Pro berharga \$99/bulan dan mencakup 5 kursi. Jika Anda memiliki lebih dari 5 pengguna, Anda membayar tambahan \$15/bulan untuk setiap kursi tambahan.

Contohnya, jika tim Anda membutuhkan 15 kursi:
- Paket Dasar: \$99/bulan (mencakup 5 kursi)
...
| Mode | Menambah Kursi | Mengurangi Kursi | Siklus Penagihan |
|------|----------------|------------------|------------------|
| `prorated_immediately` | Biaya untuk hari tersisa dalam siklus | Kredit untuk hari yang tidak terpakai | Disetel ulang ke hari ini |
| `difference_immediately` | Biaya harga kursi penuh | Kredit diterapkan pada pembaruan berikutnya | Disetel ulang ke hari ini |
| `full_immediately` | Biaya harga kursi penuh | Tidak ada kredit | Disetel ulang ke hari ini |
| `do_not_bill` | Kursi ditambah, tidak ada biaya sekarang | Kursi dihapus, tidak ada kredit | Tetap sama |

<Warning>
`prorated_immediately`, `difference_immediately`, dan `full_immediately` semuanya **mengatur ulang siklus penagihan ke tanggal perubahan** — pembaruan berikutnya dimulai dari hari ketika perubahan kursi diterapkan. Hanya **`do_not_bill`** yang menjaga tanggal pembaruan asli (jumlah kursi baru ditagihkan penuh pada pembaruan berikutnya, tanpa biaya pada saat perubahan).
</Warning>

### Contoh Prorata

**Skenario: Siklus penagihan 15 hari tersisa, menambah 5 kursi seharga $10/kursi**
...
Pelanggan membayar jumlah prorata sekarang, dan siklus penagihan dimulai ulang ke tanggal perubahan — pembaruan berikutnya ($50/bulan untuk kursi tambahan) ditagihkan satu siklus dari hari ini, bukan dari tanggal pembaruan asli.
</Tab>
...
Pelanggan membayar harga kursi penuh sekarang, terlepas dari posisi siklus, dan siklus penagihan dimulai ulang ke tanggal perubahan.
</Tab>
...
Kursi ditambahkan segera tanpa biaya pada saat perubahan. Tanggal pembaruan asli tetap terjaga, dan jumlah kursi baru ditagihkan penuh pada pembaruan berikutnya. Gunakan ini ketika Anda perlu menjaga siklus penagihan yang ada.
</Tab>
</Tabs>
...
<Tip>
**Memilih mode prorata untuk perubahan kursi**: Gunakan `prorated_immediately` untuk penagihan berbasis hari yang adil ketika tim sering mengubah kursi. Gunakan `difference_immediately` untuk perhitungan yang lebih sederhana yang mengenakan biaya atau memberikan kredit dengan harga kursi penuh. Jika Anda perlu menjaga tanggal pembaruan yang ada tidak berubah, gunakan `do_not_bill` — satu-satunya mode yang tidak mengatur ulang siklus penagihan (jumlah kursi baru ditagihkan pada pembaruan berikutnya). Lihat [Panduan Prorata](/developer-resources/subscription-upgrade-downgrade#proration-modes) untuk perbandingan terperinci.
</Tip>

### Pratinjau Sebelum Mengubah

Selalu pratinjau prorata sebelum membuat perubahan:
...
| Peristiwa | Kapan Dipicu | Kasus Penggunaan |
|----------|--------------|-----------------|
...
Mencegah penambahan pengguna dengan tegas lebih dari jumlah kursi.
...
Secara otomatis menambahkan kursi ketika batas tercapai.
...
Tawarkan jenis kursi yang berbeda dengan harga berbeda:
...
Tawarkan harga kursi tahunan yang didiskon:
...
Memerlukan jumlah kursi minimum untuk beberapa paket:
...
- **Komunikasi Jelas**: Tampilkan harga per kursi dengan jelas di halaman harga Anda
- **Kursi yang Termasuk**: Pertimbangkan untuk memasukkan beberapa kursi dalam harga dasar untuk mengurangi hambatan
- **Diskon Volume**: Tawarkan tarif per kursi lebih rendah untuk tim yang lebih besar untuk memenangkan kesepakatan perusahaan
...
- **Sinkronkan Secara Teratur**: Sinkronkan secara berkala jumlah kursi lokal Anda dengan Dodo Payments melalui API
- **Tangani Kegagalan**: Jika perubahan kursi gagal, tampilkan pesan kesalahan yang jelas dan opsi ulang
- **Jejak Audit**: Catat semua perubahan kursi untuk perselisihan penagihan dan kepatuhan

### Praktik Terbaik Pengalaman Pengguna

- **Umpan Balik Real-Time**: Tampilkan dampak biaya langsung saat menyesuaikan kursi
- **Langkah Konfirmasi**: Memerlukan konfirmasi sebelum perubahan penagihan
- **Transparansi Prorata**: Jelaskan dengan jelas biaya prorata sebelum menerapkan
- **Penurunan Mudah**: Jangan mempersulit untuk mengurangi kursi (ini membangun kepercayaan)
...
**Gejala**: Aplikasi Anda menunjukkan jumlah kursi yang berbeda dari langganan.
...
**Solusi**:
1. Implementasikan pengolah webhook untuk `subscription.plan_changed`
2. Tambahkan tombol "Sinkronkan dengan penagihan" yang mengambil langganan saat ini
3. Tetapkan cache TTL untuk memastikan penyegaran rutin
</Accordion>

<Accordion title="Proration charges unexpected">
**Gejala**: Pelanggan bingung dengan jumlah biaya di tengah siklus.

**Penyebab**:
- Mode prorata tidak dikomunikasikan dengan jelas
- Pelanggan tidak melihat pratinjau sebelum mengkonfirmasi

**Solusi**:
1. Selalu gunakan `previewChangePlan` sebelum membuat perubahan
2. Tunjukkan rincian yang jelas: "Menambah X kursi = $Y hari ini (prorata untuk Z hari)"
3. Dokumentasikan kebijakan prorata Anda di pusat bantuan
</Accordion>

<Accordion title="Add-on not appearing in checkout">
**Gejala**: Add-on kursi tidak tersedia selama pembayaran.

**Penyebab**:
- Add-on tidak terlampir pada produk
- Add-on diarsipkan atau dihapus
- Ketidakcocokan mata uang antara produk dan add-on

**Solusi**:
1. Verifikasi add-on terlampir di pengaturan produk
2. Periksa status add-on di dasbor Tambahan
3. Pastikan mata uang cocok dengan tepat
</Accordion>

<Accordion title="Cannot reduce seats below current usage">
**Gejala**: Pelanggan ingin mengurangi kursi tetapi memiliki pengguna yang ditugaskan.
...
3. Pertimbangkan masa tenggang sebelum menerapkan pengurangan kursi
</Accordion>
</AccordionGroup>
...
<Card title="Add-ons" icon="puzzle" href="/features/addons">
  Memahami sistem add-on secara mendalam.
</Card>
