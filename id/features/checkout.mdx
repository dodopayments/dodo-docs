...
<Info>
Bagian ini mencakup konfigurasi tema **API server-side** menggunakan `customization.theme_config`. Jika Anda menggunakan **Checkout SDK** (overlay atau inline checkout), lihat bagian kustomisasi tema di [Overlay Checkout](/developer-resources/overlay-checkout#theme-customization) yang menggunakan properti camelCase (misalnya, `bgPrimary` bukannya `bg_primary`).
</Info>
