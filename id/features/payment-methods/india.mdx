...
<Step title="Webhook Terkirim">
`payment.succeeded` webhook dikirim setelah debit aktual, bukan saat inisiasi.
</Step>
</Steps>

<Warning>
**Jangan berikan manfaat pada inisiasi biaya.** Tunggu webhook `payment.succeeded` yang tiba ~48-51 jam setelah tanggal biaya yang dijadwalkan.
</Warning>
...
| :---- | :--- | :----- |
| `subscription.created` | Mandat diotorisasi | Catat mulai langganan |
| `payment.succeeded` | ~48 jam setelah tanggal biaya | Berikan/lanjutkan akses |
| `payment.failed` | Debit gagal | Beri tahu pelanggan, jeda akses |
| `subscription.on_hold` | Pembayaran gagal | Peringatkan untuk memperbarui metode pembayaran |
...
| Merek | Skenario | Nomor Kartu | Kedaluwarsa | CVV |
| :---- | :------- | :---------- | :----- | :-- |
...
<Accordion title="Menangani pembatalan mandat">
...
3. Jika pedagang non-India: Mata uang adaptif diaktifkan?
...
**Solusi:** Biaya berikutnya akan gagal dan langganan akan berpindah ke `on_hold`. Pantau webhook untuk `payment.failed`.
</Accordion>
</AccordionGroup>
