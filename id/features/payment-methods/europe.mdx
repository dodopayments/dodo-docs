...
<Accordion title="Sediakan fallback kartu">
Tidak semua pelanggan Eropa memiliki akses ke metode regional ini (turis, ekspatriat, dll.). Selalu masukkan `credit` dan `debit` sebagai fallback.
</Accordion>
...
**Periksa:**
1. Apakah negara penagihan pelanggan cocok dengan negara metode?
2. Apakah mata uang diatur ke EUR?
3. Apakah metode termasuk dalam `allowed_payment_method_types`?

**Solusi:** Metode Eropa sangat regional. Seorang pelanggan dengan negara penagihan `DE` (Jerman) tidak akan melihat iDEAL, yang hanya untuk Belanda.
</Accordion>
