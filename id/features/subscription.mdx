...
| Langganan dijeda | `subscription.paused` |
| Langganan yang dijeda dilanjutkan | `subscription.unpaused` |
| Pembuatan gagal | `subscription.failed` |
| Paket ditingkatkan/diturunkan | `subscription.plan_changed` |
| Dibatalkan | `subscription.cancelled` |
| Periode berakhir | `subscription.expired` |
| Perubahan pada bidang apa pun | `subscription.updated` |
...
Gunakan `PATCH /subscriptions/{subscription_id}` untuk membatalkan pada tanggal penagihan berikutnya, memperpanjang periode langganan, memperbarui detail penagihan, atau mengubah metadata. Untuk mengubah kuantitas, gunakan [Change Plan API](/api-reference/subscriptions/change-plan) sebagai gantinya — `PATCH` tidak menerima `quantity`.
...
### Perpanjang periode langganan
Perpanjang durasi langganan dengan meneruskan `subscription_period_count` dan `subscription_period_interval` baru ke `PATCH /subscriptions/{subscription_id}`. Waktu kedaluwarsa langganan dihitung ulang berdasarkan jumlah dan interval baru — misalnya, untuk memberikan waktu tambahan kepada pelanggan pada paket mereka saat ini:
