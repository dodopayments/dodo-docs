...
Sebuah produk bisa berupa item satu kali atau layanan berbasis langganan. Untuk membuat produk baru, kirim permintaan `POST` ke endpoint `/products` dengan detail seperti nama, deskripsi, harga, mata uang, dan apakah itu produk yang berulang.
...
Untuk memodifikasi produk yang ada, kirim permintaan `PATCH` ke endpoint `/products/{product_id}`. Anda dapat memperbarui atribut seperti nama, harga, dan deskripsi sambil menjaga detail lainnya tidak berubah.
...
Anda dapat mengasosiasikan gambar dengan produk dengan mengunggahnya ke AWS S3 menggunakan URL pra-tanda tangan yang diberikan oleh API. Pertama, minta URL unggahan gambar dari endpoint `/products/{product_id}/images`, lalu gunakan URL yang diberikan untuk mengunggah gambar dalam waktu 60 detik.
...
Setelah URL pra-tanda tangan diterima dari API, unggah gambar menggunakan metode `PUT`. Ini memastikan akses yang aman dan sementara ke AWS S3 untuk mengunggah gambar.
...
Jika Anda tidak ingin menampilkan atau menggunakan produk, Anda dapat mengarsipkannya menggunakan permintaan `POST` ke endpoint `/products/{product_id}/archive`. Tindakan ini menyembunyikan produk tetapi tidak menghapusnya secara permanen.
...
<Accordion title="Mengembalikan Produk Dari Arsip">
Jika Anda perlu mengembalikan produk yang diarsipkan, kirim permintaan `POST` ke endpoint `/products/{product_id}/unarchive`. Ini akan mengaktifkan kembali produk dan menjadikannya tersedia untuk digunakan lagi.
...
<Card title="Analitik Produk" icon="chart-mixed" href="/features/analytics-and-reporting#product-level-analytics">
Lacak pendapatan, pelanggan, retensi, pelanggan tetap, dan MRR untuk setiap produk individu.
</Card>
