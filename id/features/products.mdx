...
<Frame>
  <img src="/images/product-collection/checkout-page.png" alt="Koleksi Produk" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
Untuk produk berulang, atur objek `price` ke harga berulang (`type: recurring_price`) dan tentukan `payment_frequency_interval` serta `subscription_period_interval` (`Day`, `Week`, `Month`, atau `Year`) beserta hitungan yang sesuai.
...
Jika Anda tidak ingin lagi menampilkan atau menggunakan produk, Anda dapat mengarsipkannya menggunakan permintaan `DELETE` ke endpoint `/products/{id}`. Tindakan ini menyembunyikan produk tetapi tidak menghapusnya secara permanen.
