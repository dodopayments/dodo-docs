...
1. **Checkout Inline - Pengalaman Checkout Terintegrasi Penuh (BETA)**  
   ...
   - **Acara pemecahan waktu nyata**: Terima `checkout.breakdown` acara saat pelanggan memasukkan detail mereka
   - **Ringkasan pesanan kustom**: Bangun tampilan harga Anda sendiri yang diperbarui secara waktu nyata
   - **Integrasi yang mulus**: Frame checkout menangani pengumpulan pembayaran sementara Anda mengontrol tata letak
   - **Metode SDK**: Gunakan `open()`, `close()`, dan `isOpen()` untuk mengontrol checkout secara programatik
...
2. **Buat Pembayaran, Langganan, dan Checkout Menggunakan Metode Pembayaran yang Ada**  
    Permudah alur checkout dengan menggunakan metode pembayaran yang disimpan dari pelanggan yang ada. Parameter `payment_method_id` yang baru memungkinkan Anda untuk membuat pembayaran, langganan, atau sesi checkout menggunakan metode pembayaran yang sebelumnya disimpan oleh pelanggan.
...
5. **`redirect_immediately` Flag - Lewati Halaman Sukses Pembayaran**  
   Kontrol alur checkout dengan flag `redirect_immediately` yang baru. Saat diaktifkan, pelanggan diarahkan segera setelah penyelesaian pembayaran, melewati halaman sukses default untuk pengalaman yang lebih cepat dan lebih efisien.
  ...
  ```
   ...
   <Tip>
   Gunakan `redirect_immediately: true` saat Anda memiliki halaman sukses kustom yang memberikan pengalaman pengguna yang lebih baik dibandingkan halaman sukses pembayaran default.
   </Tip>

   <Note>
   Saat `redirect_immediately` diaktifkan, pelanggan diarahkan ke `return_url` Anda segera setelah penyelesaian pembayaran, sepenuhnya melewatkan halaman sukses default.
   </Note>
