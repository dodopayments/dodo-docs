...
1. **Checkout Inline - Pengalaman Checkout Terintegrasi Penuh (BETA)**  
   ...
   - **Peristiwa pemecahan waktu nyata**: Terima `checkout.breakdown` peristiwa saat pelanggan memasukkan detail mereka
   - **Ringkasan pesanan kustom**: Buat tampilan harga Anda sendiri yang diperbarui secara waktu nyata
   - **Integrasi yang mulus**: Kerangka checkout menangani pengumpulan pembayaran sementara Anda mengontrol tata letak
...
2. **Buat Pembayaran, Langganan, dan Checkout Menggunakan Metode Pembayaran yang Ada**  
   ...
   <Warning>
   Saat menggunakan `payment_method_id` dalam sesi checkout, `confirm` harus diatur ke `true`, dan sebuah `customer_id` yang ada harus disediakan. Metode pembayaran akan divalidasi untuk kelayakan dengan mata uang pembayaran.
   </Warning>
...
5. **`redirect_immediately` Flag - Lewati Halaman Sukses Pembayaran**  
   Kontrol alur checkout dengan flag baru `redirect_immediately`. Ketika diaktifkan, pelanggan akan diarahkan segera setelah pembayaran selesai, melewati halaman sukses default untuk pengalaman yang lebih cepat dan lebih efisien.
  ...
  ```
   ...
   <Note>
   Ketika `redirect_immediately` diaktifkan, pelanggan akan diarahkan ke `return_url` Anda segera setelah pembayaran selesai, sepenuhnya melewati halaman sukses default.
   </Note>
