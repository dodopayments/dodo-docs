...
| Kode Kesalahan | Kesalahan Pengguna | Deskripsi |
|----------------|------------------|-------------|
| `AUTHENTICATION_FAILURE` | Ya | Proses autentikasi gagal selama transaksi |
| `AUTHENTICATION_REQUIRED` | Ya | Autentikasi tambahan diperlukan untuk menyelesaikan transaksi |
| `AUTHENTICATION_TIMEOUT` | Ya | Proses autentikasi telah waktunya habis |
...
| `EXPIRED_CARD` | Ya | Kartu telah kadaluarsa |
...
| `INSUFFICIENT_FUNDS` | Ya | Saldo akun tidak mencukupi untuk menyelesaikan transaksi |
| `INVALID_ACCOUNT` | Ya | Rincian akun yang diberikan tidak valid |
...
| `INVALID_EXPIRY_YEAR` | Ya | Tahun kadaluarsa kartu tidak valid |
...
| `LOST_CARD` | Ya | Kartu dilaporkan hilang |
...
| `NETWORK_TIMEOUT` | Tidak | Permintaan jaringan telah waktunya habis |
...
| `UNKNOWN_ERROR` | Tidak | Terjadi kesalahan tidak dikenal |

<Note>
  **Kesalahan Pengguna** menunjukkan apakah penolakan pembayaran dapat diselesaikan oleh pelanggan. Ketika `true`, pelanggan dapat mengambil tindakan untuk memperbaiki masalah (mis., memasukkan rincian kartu yang benar). Ketika `false`, penolakan disebabkan oleh masalah tingkat sistem atau pembatasan bank yang tidak dapat diselesaikan oleh pelanggan.
</Note>
