...
<Steps>
<Step title="Buat Plugin Dasar">
Buat proyek plugin Figma Anda sendiri menggunakan [panduan cepat plugin Figma](https://www.figma.com/plugin-docs/plugin-quickstart-guide/). Ini akan menghasilkan kerangka dengan file `manifest.json`.
...
<Step title="Kloning Implementasi Contoh">
Klon [repositori dodopayments-figma](https://github.com/dodopayments/dodopayments-figma) dan perbarui `manifest.json` dengan nama dan ID dari folder plugin yang Andahasilkan.
...
<Step title="Kustomisasi Komponen">
...
- Sesuaikan `LicenseKeyInput.tsx` agar sesuai dengan preferensi desain Anda
...
<Steps>
<Step title="Konfigurasi Akses Jaringan">
Plugin memerlukan akses jaringan untuk berkomunikasi dengan proksi CORS. Tambahkan yang berikut ke `manifest.json`:
...
<Step title="Konfigurasi Mode API">
...
<Tip>
Beralih ke `live_mode` hanya ketika Anda siap untuk produksi dan telah diuji secara menyeluruh dalam mode pengujian.
</Tip>
</Step>
</Steps>
...
3. Ikuti [pedoman penerbitan plugin Figma](https://www.figma.com/plugin-docs/publishing-plugins/)
