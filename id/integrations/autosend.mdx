...
- **Gunakan data dinamis untuk personalisasi**: Gunakan `dynamicData` field untuk mempersonalisasi email dengan informasi spesifik pelanggan seperti nama, jumlah pembayaran, dan rincian langganan. Email yang dipersonalisasi memiliki tingkat keterlibatan yang lebih tinggi.
...
- Verifikasi API Key adalah benar dan aktif
...
- Periksa bahwa parameter yang dibutuhkan ada dalam payload
</Accordion>

<Accordion title="Kesalahan transformasi">
- Validasi struktur JSON sesuai dengan format API AutoSend
- Periksa bahwa semua field yang dibutuhkan ada (`to`, `from`, `templateId` atau `html`/`text`)
...
- Periksa bahwa semua variabel template yang dibutuhkan disediakan
