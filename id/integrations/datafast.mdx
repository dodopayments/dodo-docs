...
<Steps>
<Step title="Pasang Skrip DataFast">
  ...
  Kunjungi [dokumen DataFast](https://datafa.st/docs) untuk petunjuk pemasangan spesifik untuk platform Anda.
</Step>
...
- Verifikasi kunci API DataFast Anda benar dan aktif
- Periksa bahwa `datafast_visitor_id` ditangkap dan disimpan dalam metadata pembayaran
- Pastikan transformasi webhook sedang memformat payload dengan benar
- Verifikasi webhook memicu pada peristiwa `payment.succeeded`
...
- Verifikasi cookie `datafast_visitor_id` diatur dengan benar
...
- Tinjau [dokumen API Pembayaran DataFast](https://datafa.st/docs/payments-api) untuk panduan tambahan
</Accordion>

<Accordion title="Kesalahan transformasi">
- Validasi struktur JSON cocok dengan format API Pembayaran DataFast
- Periksa bahwa semua kolom yang diperlukan (`amount`, `currency`, `transaction_id`, `datafast_visitor_id`) ada
- Pastikan jumlah dikonversi dengan benar (bagi dengan 100 untuk sebagian besar mata uang, kecuali mata uang desimal nol)
