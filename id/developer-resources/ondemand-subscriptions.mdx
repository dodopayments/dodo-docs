...
Gunakan kunci API percobaan Anda untuk membuat langganan, kemudian buka `checkout_url` yang dikembalikan dan selesaikan mandat.
</Step>
<Step title="Trigger a charge">
Panggil endpoint charge dengan `product_price` kecil (misalnya, `100`) dan verifikasi Anda menerima `payment.succeeded`.
</Step>
<Step title="Go live">
Beralih ke kunci API live Anda setelah Anda memvalidasi peristiwa dan pembaruan status internal.
</Step>
</Steps>
