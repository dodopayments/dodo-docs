...
- Pastikan `DODO_PAYMENTS_WEBHOOK_KEY` telah diatur dengan benar di `.env` Anda
- Verifikasi bahwa Anda menggunakan raw request body untuk signature verification
- Teruskan ketiga header `webhook-id`, `webhook-timestamp`, dan `webhook-signature` ke `webhook.verify()` — signature Standard Webhooks mencakup `id.timestamp.body`, bukan hanya body
</Accordion>
</AccordionGroup>
