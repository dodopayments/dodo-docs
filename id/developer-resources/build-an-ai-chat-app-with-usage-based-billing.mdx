...
<Warning>
Nama acara `ai_chat_usage` harus cocok persis dengan yang akan kami kirim dari kode aplikasi kami nanti. Nama acara bersifat peka huruf besar/kecil!
</Warning>
</Step>
...
**Agregat Selama**: Tipe → `totalTokens`
...
<Note>
Simpan kunci API ini - kami akan menambahkannya ke dalam file `.env` kami nanti.
</Note>
</Step>
...
<Note>
Jaga keamanan kunci ini - kami juga akan menambahkannya ke dalam file `.env` kami.
</Note>
</Step>
</Steps>
...
<Step title="Cari ID pelanggan Anda">
1. Kembali ke dasbor Dodo Payments Anda
2. Arahkan ke **Penjualan** -> **Pelanggan** di sidebar kiri
3. Temukan pelanggan yang baru saja Anda buat (dengan email percobaan)
...
<Step title="Siapkan variabel lingkungan">
...
<Info>
Meter ini menjumlahkan semua nilai `totalTokens` secara otomatis!
</Info>
</Step>
...
**Solusi:**
1. Verifikasi nama acara cocok persis dengan konfigurasi meter (peka huruf besar/kecil: `ai_chat_usage`)
...
- Kunci API salah untuk lingkungan
- Spasi atau tanda kutip tambahan di file `.env`

**Solusi:**
- Pastikan kunci uji dimulai dengan `test_`, kunci langsung dimulai dengan `live_`
- Hapus tanda kutip apa pun di sekitar kunci dalam file `.env`
