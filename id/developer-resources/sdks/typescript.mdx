...
<Warning>
`POST /subscriptions` (metode `subscriptions.create` milik SDK) sudah deprecated. Metode ini masih berfungsi untuk integrasi yang sudah ada, tetapi integrasi baru sebaiknya membuat subscription melalui [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`billing` memerlukan setidaknya kode negara ISO dua huruf. `customer` merupakan gabungan dari `{ customer_id }` (untuk mengaitkan customer yang sudah ada) atau `{ email, name? }` (untuk membuat customer baru). `product_price` dinyatakan dalam denominasi mata uang terkecil.
</Info>

## Penagihan Berbasis Penggunaan

### Menerima Event Penggunaan
...
<Info>
Event harus memiliki nilai `event_id` yang unik untuk idempotensi. ID duplikat dalam request yang sama akan ditolak, dan request berikutnya dengan ID yang sudah ada akan diabaikan.
</Info>

### Mengambil Event Penggunaan

Ambil informasi mendetail tentang event penggunaan:
...
Kontrol tingkat detail log menggunakan environment variables atau opsi client:
...
<Warning>
Pada level debug, semua request dan respons HTTP dicatat, termasuk header dan body. Beberapa header autentikasi disamarkan, tetapi data sensitif dalam body masih dapat terlihat.
</Warning>

## Migrasi dari Node.js SDK

Jika Anda melakukan upgrade dari Node.js SDK lama, TypeScript SDK menawarkan keamanan tipe dan fitur yang lebih baik:
...
Metode list di DodoPayments API menggunakan pagination. Anda dapat menggunakan sintaks `for await … of` untuk melakukan iterasi pada item di semua halaman:
...
Atau, Anda dapat meminta satu halaman dalam satu waktu:
...
- Browser web (Chrome, Firefox, Safari, Edge, dan lainnya versi terbaru)
- Node.js 20 LTS atau yang lebih baru, versi ([non-EOL](https://endoflife.date/nodejs))
- Deno v1.28.0 atau lebih tinggi
- Bun 1.0 atau lebih baru
- Cloudflare Workers
- Vercel Edge Runtime
- Jest 28 atau lebih tinggi dengan environment `"node"`
...
Perlu bantuan terkait TypeScript SDK?
...
- **GitHub**: Buka issue di [repository](https://github.com/dodopayments/dodopayments-typescript)

## Berkontribusi

Kami menyambut kontribusi! Lihat [panduan kontribusi](https://github.com/dodopayments/dodopayments-typescript/blob/main/CONTRIBUTING.md) untuk memulai.
