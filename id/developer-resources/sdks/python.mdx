...
<Warning>
`POST /subscriptions` (the SDK's `subscriptions.create` method) sudah **deprecated**. Metode ini masih berfungsi untuk integrasi yang sudah ada, tetapi integrasi baru sebaiknya membuat subscription melalui [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`billing` memerlukan setidaknya kode negara ISO dua huruf. `customer` menerima `{"customer_id": ...}` untuk menautkan customer yang sudah ada atau `{"email": ..., "name": ...}` untuk membuat customer baru. `product_price` menggunakan denominasi mata uang terkecil.
</Info>
...
Lacak event kustom untuk penagihan berbasis penggunaan:
...
Iterasikan semua item secara otomatis:
...
Sesuaikan client `httpx` yang mendasarinya:
...
## Asinkron dengan aiohttp

Gunakan aiohttp untuk performa asinkron yang lebih baik:
...
Aktifkan logging dengan menetapkan environment variable:
...
- **GitHub**: Buka issue di [repository](https://github.com/dodopayments/dodopayments-python)

## Berkontribusi

Kami menyambut kontribusi! Lihat [panduan kontribusi](https://github.com/dodopayments/dodopayments-python/blob/main/CONTRIBUTING.md) untuk memulai.
