...
<Warning>
`POST /subscriptions` (metode `subscriptions.create` milik SDK) **deprecated**. Metode ini masih berfungsi untuk integrasi yang sudah ada, tetapi integrasi baru sebaiknya membuat subscription melalui [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`billing` memerlukan setidaknya kode ISO dua huruf `country`. Teruskan `AttachExistingCustomer::with(customerID: '...')` untuk mengaitkan customer yang sudah ada, atau `NewCustomer::with(email: '...', name: '...')` untuk membuat customer baru. `productPrice` menggunakan denominasi mata uang terkecil.
</Info>

## Pagination

Gunakan respons daftar berpaginasi:
...
Ketika library tidak dapat terhubung ke API atau menerima status code non-sukses (4xx atau 5xx), subclass dari `APIException` akan dilempar:
...
<Tip>
  Selalu bungkus panggilan API dalam blok try-catch untuk menangani error yang mungkin terjadi
  dengan baik dan memberikan umpan balik yang bermakna kepada pengguna.
</Tip>
...
Buat request ke endpoint yang tidak didokumentasikan:
...
<Note>
  Parameter `extra*` dengan nama yang sama akan menggantikan parameter yang didokumentasikan.
</Note>
...
Buat service untuk aplikasi Laravel:
...
- **GitHub**: Buka issue di [repository](https://github.com/dodopayments/dodopayments-php)

## Berkontribusi

Kami menyambut kontribusi! Baca [panduan kontribusi](https://github.com/dodopayments/dodopayments-php/blob/main/CONTRIBUTING.md) untuk memulai.
