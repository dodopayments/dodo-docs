...
SDK menggunakan parameter bernama untuk menentukan argumen opsional. Anda dapat menginisialisasi objek nilai menggunakan konstruktor static `with`:
...
Ketika pustaka tidak dapat terhubung ke API atau menerima kode status bukan keberhasilan (4xx atau 5xx), subclass dari `APIException` dilemparkan:
...
<Catatan>
  Parameter `extra*` dengan nama yang sama menimpa parameter yang didokumentasikan.
</Catatan>
...
Tambahkan konfigurasi dalam `config/services.php`:
