...
<Warning>
`POST /subscriptions` (metode `subscriptions.create` pada SDK) sudah **deprecated**. Metode ini masih berfungsi untuk integrasi yang sudah ada, tetapi integrasi baru sebaiknya membuat subscription melalui [Checkout Session](/developer-resources/checkout-session).
</Warning>
...
<Info>
`Billing` memerlukan setidaknya kode ISO dua huruf `Country`. `Customer` adalah sebuah `CustomerRequestUnionParam` — teruskan `AttachExistingCustomerParam{CustomerID: ...}` untuk customer yang sudah ada atau `NewCustomerParam{Email: ..., Name: ...}` untuk customer baru. `ProductPrice` menggunakan denominasi mata uang terkecil.
</Info>
...
Saat API mengembalikan status code non-success, SDK mengembalikan error bertipe
`*dodopayments.Error`. Error ini mengekspos `StatusCode`, `*http.Request` yang mendasarinya, dan
`*http.Response`, serta JSON dari error body. Gunakan pola `errors.As` untuk
memeriksanya, lalu gunakan `StatusCode` untuk menangani kasus tertentu:
...
Tambahkan middleware khusus untuk logging atau metrik:
...
Client aman digunakan secara bersamaan:
...
- **GitHub**: Buka issue di [repository](https://github.com/dodopayments/dodopayments-go)

## Berkontribusi

Kami menyambut kontribusi! Lihat [panduan kontribusi](https://github.com/dodopayments/dodopayments-go/blob/main/CONTRIBUTING.md) untuk memulai.
