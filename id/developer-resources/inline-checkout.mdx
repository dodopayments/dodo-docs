...
| Kolom | Deskripsi |
|-------|-------------|
| `subTotal` | Jumlah seluruh item baris dalam sesi sebelum diskon atau pajak apa pun diterapkan. |
| `discount` | Nilai total semua diskon yang diterapkan. |
| `tax` | Jumlah pajak yang dihitung. Dalam mode `inline`, nilai ini diperbarui secara dinamis saat pengguna berinteraksi dengan kolom alamat. |
| `total` | Hasil perhitungan matematis `subTotal - discount + tax` dalam mata uang dasar sesi. |
| `currency` | Kode mata uang ISO (misalnya, `"USD"`) untuk nilai subtotal, diskon, dan pajak standar. |
| `finalTotal` | Jumlah sebenarnya yang dibebankan kepada pelanggan. Nilai ini dapat mencakup penyesuaian valuta asing tambahan atau biaya metode pembayaran lokal yang tidak termasuk dalam perincian harga dasar. |
| `finalTotalCurrency` | Mata uang yang sebenarnya digunakan pelanggan untuk membayar. Nilai ini dapat berbeda dari `currency` jika [paritas daya beli](/features/purchasing-power-parity) atau konversi mata uang lokal sedang aktif. |
...
<Info>
Verifikasi domain hanya diperlukan untuk **inline (embedded) checkout**. Ini tidak diperlukan untuk hosted checkout.
</Info>
...
Buka **Settings → Payment Methods** dan, pada baris **Apple Pay**, klik **Manage domains**.
...
File tersebut harus disajikan melalui HTTPS, dapat dijangkau tanpa redirect, dan disajikan dengan `Content-Type: application/octet-stream` atau `text/plain`.
</Step>

<Step title="Verify the domain">
Klik **Verify domain**. Dodo Payments mengonfirmasi bahwa file tersebut tersedia dan mengirimkan domain Anda ke Apple.
...
Saat status menampilkan **Active**, Apple Pay diaktifkan untuk domain tersebut. Gunakan toggle **Enabled** untuk mengaktifkan atau menonaktifkannya per domain.
...
| Branding | Menyatu | Terpisah dari halaman |
| Upaya implementasi | Lebih tinggi | Lebih rendah |
| Paling sesuai untuk | Halaman checkout khusus, alur dengan konversi tinggi | Integrasi cepat, halaman yang sudah ada |

<Tip>
Gunakan **inline checkout** saat Anda menginginkan kontrol maksimum atas pengalaman checkout dan branding yang menyatu. Gunakan **overlay checkout** untuk integrasi yang lebih cepat dengan perubahan minimal pada halaman yang sudah ada.
</Tip>
...
<Card title="Webhooks" icon="webhook" href="/developer-resources/webhooks">
    Tangani payment events di sisi server dengan webhook.
</Card>
...
Untuk bantuan lebih lanjut, kunjungi [komunitas Discord kami](https://discord.gg/bYqAp4ayYh) atau hubungi tim dukungan developer kami.
