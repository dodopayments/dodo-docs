...
Contoh ini menunjukkan cara menerapkan ringkasan pesanan yang kustom di samping checkout inline, menjaga sinkronisasi menggunakan event `checkout.breakdown`.
...
| `options.showTimer` | `boolean` | Tidak | Tampilkan atau sembunyikan timer checkout. Secara default adalah `true`. Saat dinonaktifkan, Anda akan menerima event `checkout.link_expired` ketika sesi berakhir. |
| `options.showSecurityBadge` | `boolean` | Tidak | Tampilkan atau sembunyikan lencana keamanan. Secara default adalah `true`. |
| `options.manualRedirect` | `boolean` | Tidak | Ketika diaktifkan, checkout tidak akan mengalihkan secara otomatis setelah selesai. Sebagai gantinya, Anda akan menerima `checkout.status` dan `checkout.redirect_requested` untuk menangani pengalihan sendiri. |
| `options.themeConfig` | `ThemeConfig` | Tidak | Konfigurasi tema kustom. |
| `options.payButtonText` | `string` | Tidak | Teks kustom yang ditampilkan di tombol bayar. |
| `options.fontSize` | `FontSize` | Tidak | Ukuran font global untuk checkout. |
| `options.fontWeight` | `FontWeight` | Tidak | Berat font global untuk checkout. |
...
| `checkout.breakdown` | Ditembak saat harga, pajak, atau diskon diperbarui. |
| `checkout.customer_details_submitted` | Rincian pelanggan telah dikirim. |
| `checkout.pay_button_clicked` | Ditembak saat pelanggan mengklik tombol bayar. Berguna untuk analitik dan pelacakan funnel konversi. |
...
| `checkout.link_expired` | Ditembak saat sesi checkout berakhir. Hanya diterima ketika `showTimer` diatur ke `false`. |
| `checkout.status` | Ditembak saat `manualRedirect` diaktifkan. Berisi status checkout (`succeeded`, `failed`, atau `processing`). |
| `checkout.redirect_requested` | Ditembak saat `manualRedirect` diaktifkan. Berisi URL untuk mengalihkan pelanggan.
...
Ketika `manualRedirect` diaktifkan, Anda akan menerima event `checkout.status` dengan data berikut:
...
Ketika `manualRedirect` diaktifkan, Anda akan menerima event `checkout.redirect_requested` dengan data berikut:
...
| `tax` | Jumlah pajak yang dihitung. Dalam mode `inline`, ini terupdate secara dinamis saat pengguna berinteraksi dengan field alamat. |
...
| `finalTotal` | Jumlah yang sebenarnya dibebankan kepada pelanggan. Ini dapat mencakup penyesuaian nilai tukar atau biaya metode pembayaran lokal yang tidak termasuk dalam rincian harga dasar. |
...
2.  **Menangani Status Awal**: Ketika checkout pertama kali dimuat, `tax` dan `discount` dapat `0` atau `null` hingga pengguna memberikan informasi penagihan mereka atau menerapkan kode. UI Anda harus menangani status ini dengan baik (misalnya, menunjukkan tanda hubung `—` atau menyembunyikan baris).
3.  **"Total Akhir" vs "Total"**: Sementara `total` memberi Anda perhitungan harga standar, `finalTotal` adalah sumber kebenaran untuk transaksi. Jika `finalTotal` ada, itu mencerminkan persis apa yang akan dibebankan kepada kartu pelanggan, termasuk penyesuaian dinamis.
4.  **Umpan Balik Real-time**: Gunakan field `tax` untuk menunjukkan kepada pengguna bahwa pajak dihitung secara real-time. Ini memberikan nuansa "hidup" pada halaman checkout Anda dan mengurangi gesekan selama langkah entri alamat.
...
Anda dapat menyesuaikan penampilan checkout dengan mengirimkan objek `themeConfig` ke parameter `options` saat membuka checkout. Konfigurasi tema mendukung mode terang dan gelap, memungkinkan Anda menyesuaikan warna, batas, teks, tombol, dan radius batas.

<Info>
Bagian ini mencakup konfigurasi tema **sisi klien** menggunakan SDK Checkout. Anda juga dapat mengonfigurasi tema **sisi server** saat membuat sesi checkout melalui API menggunakan parameter `theme_config`. Lihat [Kustomisasi Tema Checkout](/features/checkout#checkout-theme-customization) untuk konfigurasi tingkat API.
</Info>
...
#### Overriding Tema Paruh Waktu

Anda dapat mengoverride hanya properti tertentu. Checkout akan menggunakan nilai default untuk properti yang tidak Anda tentukan:
...
#### Konfigurasi Tema dengan Opsi Lainnya
...
Checkout inline mendukung **pembaruan metode pembayaran** untuk langganan. Ketika pelanggan perlu memperbarui metode pembayaran mereka -- baik untuk langganan aktif atau untuk mengaktifkan kembali langganan yang tertahan -- Anda dapat merender alur pembaruan langsung di tata letak halaman Anda.
...
2. Lewatkan `payment_link` yang dikembalikan sebagai `checkoutUrl` untuk membuka checkout inline:
...
Iframe inline hanya merender formulir pengumpulan metode pembayaran. Pelanggan dapat memasukkan rincian kartu baru atau memilih metode pembayaran yang disimpan tanpa meninggalkan halaman Anda.

### Untuk Langganan Tertahan

Saat memperbarui metode pembayaran untuk langganan dalam status `on_hold`, Dodo Payments secara otomatis membuat biaya untuk semua tunggakan yang tersisa. Pantau `payment.succeeded` dan `subscription.active` webhook untuk mengonfirmasi aktivasi kembali.
...
<Tip>
Anda juga dapat menggunakan metode pembayaran yang disimpan yang ada alih-alih mengumpulkan rincian baru dengan mengirimkan `type: 'existing'` dengan `payment_method_id` ke API Pembaruan Metode Pembayaran.
</Tip>

## Penanganan Kesalahan

SDK menyediakan informasi kesalahan yang rinci melalui sistem event. Selalu terapkan penanganan kesalahan yang tepat di callback `onEvent` Anda:
...
1. **Desain Responsif**: Pastikan elemen kontainer Anda memiliki lebar dan tinggi yang cukup. Iframe biasanya akan mengembang untuk mengisi kontainernya.
...
3. **Status Skelet**: Tampilkan indikator pemuatan di kontainer Anda hingga event `checkout.opened` muncul.
...
<Warning>
Jangan bergantung hanya pada event checkout inline untuk menentukan keberhasilan atau kegagalan pembayaran. Selalu terapkan validasi sisi server menggunakan webhook dan/atau polling.
</Warning>

### Mengapa Validasi Sisi Server Penting

Meskipun event checkout inline seperti `checkout.status` memberikan umpan balik real-time, mereka **tidak** boleh menjadi satu-satunya sumber kebenaran untuk status pembayaran. Masalah jaringan, kerusakan browser, atau pengguna menutup halaman dapat menyebabkan event hilang. Untuk memastikan validasi pembayaran yang handal:

1. **Server Anda harus mendengarkan event webhook** - Dodo Payments mengirimkan webhook untuk perubahan status pembayaran
2. **Terapkan mekanisme polling** - Frontend Anda harus mempoll server Anda untuk pembaruan status
...
**1. Dengarkan event checkout** - Ketika pengguna mengklik bayar, mulai siapkan untuk memverifikasi status:
...
**2. Poll server Anda** - Buat endpoint yang memeriksa database Anda untuk status pembayaran (diperbarui oleh webhook):
...
**3. Tangani webhook di sisi server** - Perbarui database Anda ketika Dodo mengirimkan `payment.succeeded` atau `payment.failed` webhook. Lihat [dokumentasi Webhook](/developer-resources/webhooks) kami untuk detail.

### Menangani Pengalihan (3DS, Google Pay, UPI)

Saat menggunakan `manualRedirect: true`, beberapa metode pembayaran memerlukan pengalihan pengguna dari halaman Anda untuk otentikasi:
...
Setelah otentikasi selesai (berhasil atau gagal), pengguna kembali ke halaman Anda. **Jangan asumsi keberhasilan hanya karena pengguna kembali.** Sebagai gantinya:

1. Periksa apakah pengguna kembali dari pengalihan (misalnya, melalui `sessionStorage`)
2. Mulai polling server Anda untuk status pembayaran yang telah dikonfirmasi
3. Tampilkan status "Memeriksa pembayaran..." saat polling
4. Tampilkan UI keberhasilan/kegagalan berdasarkan status yang dikonfirmasi server
...
<AccordionGroup>
<Accordion title="Frame checkout tidak muncul">
- Verifikasi bahwa `elementId` cocok dengan `id` dari `div` yang benar-benar ada di DOM.
- Pastikan `displayType: 'inline'` telah dikirim ke `Initialize`.
...
Untuk informasi lebih lanjut tentang cara mengatur Apple Pay, Google Pay, dan dompet digital lainnya, lihat halaman <a href="/features/payment-methods/digital-wallets">Dompet Digital</a>.
...
Setelah dikonfirmasi, verifikasi bahwa Apple Pay muncul di checkout dan uji alur lengkapnya.
</Step>
</Steps>

<Warning>
Apple Pay memerlukan verifikasi domain sebelum muncul di produksi. Hubungi dukungan sebelum pergi langsung jika Anda berencana untuk menawarkan Apple Pay.
</Warning>
...
Pilih jenis checkout yang tepat untuk penggunaan Anda:

| Fitur | Checkout Inline | Checkout Overlay |
|---------|-----------------|------------------|
| Kedalaman integrasi | Sepenuhnya tersemat di halaman | Modal di atas halaman |
