...
<Info>
Verifikasi domain hanya diperlukan untuk **pembayaran bawaan (tertanam)**. Pembayaran overlay, pembayaran host, dan tautan pembayaran tidak membutuhkannya.
</Info>

Apple Pay diverifikasi per domain dari dasbor.

<Steps>
<Step title="Open Wallet domains">
Pergi ke **Pengaturan → Metode Pembayaran** dan, di baris **Apple Pay**, klik **Kelola domain**.

<Frame caption="Open Wallet domains from the Apple Pay row">
  <img src="/images/apple-pay-domains/manage-domains.png" alt="Tombol Kelola domain di baris Apple Pay dalam pengaturan Metode Pembayaran" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Download the domain association file">
Dari panel domain Wallet, unduh berkas asosiasi.
...
Klik **Daftarkan domain** dan masukkan domain tempat Anda menanam pembayaran bawaan (mis. `shop.example.com`), lalu **Lanjutkan**.
...
Itu harus disajikan melalui HTTPS, dapat dijangkau tanpa pengalihan, dan disajikan dengan `Content-Type: application/octet-stream` atau `text/plain`.
</Step>

<Step title="Verify the domain">
Klik **Verifikasi domain**. Dodo Payments mengonfirmasi file aktif dan mengirimkan domain Anda ke Apple.

<Frame caption="Verify the hosted association file">
  <img src="/images/apple-pay-domains/verify-domain.png" alt="Layar verifikasi domain Anda dengan jalur host berkas asosiasi dan tombol Verifikasi domain" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>

<Step title="Confirm it's active">
Ketika status menunjukkan **Aktif**, Apple Pay diaktifkan untuk domain tersebut. Gunakan toggle **Diaktifkan** untuk menghidupkan atau mematikannya per domain.

<Frame caption="Verified domains show an Active status">
  <img src="/images/apple-pay-domains/domains-active.png" alt="Daftar domain Wallet yang menunjukkan domain dengan status Apple Pay Aktif dan tombol Diaktifkan" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
</Step>
...
## Checkout Bawaan vs Overlay

Pilih jenis checkout yang tepat untuk kasus penggunaan Anda:

| Fitur | Checkout Bawaan | Checkout Overlay |
|---------|-----------------|------------------|
| Kedalaman integrasi | Sepenuhnya tertanam di halaman | Modal di atas halaman |
| Kontrol layout | Kontrol penuh | Terbatas |
| Branding | Tanpa batas | Terpisah dari halaman |
| Upaya implementasi | Lebih tinggi | Lebih rendah |
| Terbaik untuk | Halaman checkout khusus, aliran konversi tinggi | Integrasi cepat, halaman yang ada |

<Tip>
Gunakan **checkout bawaan** jika Anda ingin kontrol maksimum atas pengalaman checkout dan branding yang mulus. Gunakan **checkout overlay** untuk integrasi lebih cepat dengan perubahan minimal pada halaman yang ada.
</Tip>
...
<Card title="Overlay Checkout" icon="layer-group" href="/developer-resources/overlay-checkout">
    Gunakan checkout overlay untuk integrasi cepat berbasis modal.
</Card>

<Card title="Checkout Sessions API" icon="code" href="/api-reference/checkout-sessions/create">
    Buat sesi checkout untuk memperkuat pengalaman checkout Anda.
</Card>
