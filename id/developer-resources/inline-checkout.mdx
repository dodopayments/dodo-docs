...
<Check>
Anda harus melihat event `checkout.breakdown` yang dicatat di konsol browser Anda jika Anda menambahkan log konsol di callback `onEvent`.
</Check>
...
Contoh ini menunjukkan cara mengimplementasikan ringkasan pesanan kustom di samping checkout inline, menjaga keduanya tetap sinkron menggunakan event `checkout.breakdown`.
...
| `elementId` | `string` | Ya | `id` dari elemen DOM tempat checkout harus dirender. |
| `options.showTimer` | `boolean` | Tidak | Tampilkan atau sembunyikan timer checkout. Defaultnya adalah `true`. Ketika dinonaktifkan, Anda akan menerima event `checkout.link_expired` ketika sesi berakhir. |
| `options.showSecurityBadge` | `boolean` | Tidak | Tampilkan atau sembunyikan badge keamanan. Defaultnya adalah `true`. |
| `options.manualRedirect` | `boolean` | Tidak | Ketika diaktifkan, checkout tidak akan secara otomatis mengalihkan setelah selesai. Sebagai gantinya, Anda akan menerima event `checkout.status` dan `checkout.redirect_requested` untuk menangani pengalihan sendiri. |
| `options.themeConfig` | `ThemeConfig` | Tidak | Konfigurasi tema kustom. |
| `options.payButtonText` | `string` | Tidak | Teks kustom untuk ditampilkan di tombol bayar. |
...
| `checkout.link_expired` | Ditembak saat sesi checkout berakhir. Hanya diterima saat `showTimer` diatur ke `false`. |
...
| `discount` | Nilai total semua diskon yang diterapkan. |
...
| `finalTotal` | Jumlah sebenarnya yang dibebankan kepada pelanggan. Ini mungkin termasuk penyesuaian nilai tukar asing tambahan atau biaya metode pembayaran lokal yang bukan bagian dari rincian harga dasar. |
...
2.  **Menangani Status Awal**: Saat checkout pertama kali dimuat, `tax` dan `discount` mungkin adalah `0` atau `null` sampai pengguna memberikan informasi penagihan mereka atau menerapkan kode. UI Anda harus menangani status ini dengan baik (misalnya, menampilkan dash `—` atau menyembunyikan baris).
...
SDK menyediakan informasi kesalahan yang detail melalui sistem event. Selalu terapkan penanganan kesalahan yang tepat di callback `onEvent`:
...
3. **Status Skeleton**: Tampilkan indikator pemuatan di kontainer Anda sampai event `checkout.opened` dipicu.
...
### Mengapa Validasi Sisi Server Sangat Penting

Meskipun event checkout inline seperti `checkout.status` memberikan umpan balik real-time, mereka **tidak** boleh menjadi satu-satunya sumber kebenaran untuk status pembayaran. Masalah jaringan, kerusakan browser, atau pengguna yang menutup halaman dapat menyebabkan event terlewat. Untuk memastikan validasi pembayaran yang dapat diandalkan:

1. **Server Anda harus mendengarkan event webhook** - Dodo Payments mengirimkan webhook untuk perubahan status pembayaran
2. **Terapkan mekanisme polling** - Frontend Anda harus melakukan polling ke server Anda untuk pembaruan status
...
**1. Dengarkan event checkout** - Ketika pengguna mengklik bayar, mulai mempersiapkan untuk memverifikasi status:
...
**3. Tangani webhook di sisi server** - Perbarui database Anda saat Dodo mengirimkan webhook `payment.succeeded` atau `payment.failed`. Lihat [dokumentasi Webhook kami](/developer-resources/webhooks) untuk detailnya.

### Menangani Pengalihan (3DS, Google Pay, UPI)

Saat menggunakan `manualRedirect: true`, metode pembayaran tertentu memerlukan pengalihan pengguna dari halaman Anda untuk otentikasi:
...
4. Tampilkan UI keberhasilan/kegagalan berdasarkan status yang dikonfirmasi oleh server
...
Untuk informasi lebih lanjut tentang pengaturan Apple Pay, Google Pay, dan dompet digital lainnya, lihat halaman <a href="/features/payment-methods/digital-wallets">Dompet Digital</a>.
...
Setelah dikonfirmasi, pastikan Apple Pay muncul di checkout dan uji seluruh alur prosesnya.
</Step>
</Steps>

<Warning>
Apple Pay memerlukan verifikasi domain sebelum muncul di produksi. Hubungi dukungan sebelum Anda melakukan live jika Anda berencana untuk menawarkan Apple Pay.
</Warning>
...
|---------|-----------------|------------------|
| Kedalaman integrasi | Sepenuhnya terintegrasi di halaman | Modal di atas halaman |
...
<Tip>
Gunakan **checkout inline** ketika Anda ingin kontrol maksimal atas pengalaman checkout dan branding yang mulus. Gunakan **checkout overlay** untuk integrasi yang lebih cepat dengan perubahan minimal pada halaman yang sudah ada.
</Tip>
