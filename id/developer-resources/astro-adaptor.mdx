---
title: Adaptor Astro
description: Pelajari cara mengintegrasikan <CardGroup cols={2}> dengan proyek Astro App
  Router Anda menggunakan Astro Adaptor kami. Mencakup checkout, customer
  portal, webhook, dan penyiapan environment yang aman.
icon: code-merge
keywords:
  - Dodo Payments
  - Astro
...
<Warning>
Jangan pernah commit file <code>.env</code> atau secret Anda ke version control.
</Warning>
</Step>
</Steps>
...
<Tab title="Checkout Handler">
  <Info>
  Gunakan handler ini untuk mengintegrasikan checkout Dodo Payments ke dalam aplikasi Astro Anda. Mendukung alur pembayaran static (GET), dynamic (POST), dan session (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Customer Portal Handler">
  <Info>
  Gunakan handler ini untuk memungkinkan pelanggan mengelola langganan dan detail mereka melalui customer portal Dodo Payments.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Gunakan handler ini untuk menerima dan memproses event webhook Dodo Payments secara aman di aplikasi Astro Anda.
  </Info>
  <CodeGroup>
...
- **Payment Links Statis:** URL yang dapat langsung dibagikan untuk pengumpulan pembayaran yang cepat dan tanpa kode.
- **Payment Links Dinamis:** Buat payment link secara terprogram dengan detail khusus menggunakan API atau SDK.
...
Negara bagian/provinsi Customer.
</ParamField>
...
Menetapkan jumlah yang ditagihkan dalam satuan mata uang utama (misalnya, <code>12.5</code> untuk $12.50). Hanya untuk produk Pay What You Want, dan diabaikan jika nilainya di bawah harga minimum produk.
</ParamField>
...
<Warning>
Jika <code>productId</code> tidak ada, handler mengembalikan response 400. Query parameter yang tidak valid juga menghasilkan response 400.
</Warning>

### Format Response
Checkout statis mengembalikan response JSON dengan URL checkout:
...
- Untuk daftar lengkap field POST body yang didukung, lihat:
...
### Format Response
Checkout dinamis mengembalikan response JSON dengan URL checkout:
...
Checkout sessions menyediakan pengalaman checkout yang di-host dan lebih aman, yang menangani seluruh alur pembayaran untuk pembelian satu kali maupun langganan dengan kontrol kustomisasi penuh.
...
Customer Portal Route Handler memungkinkan Anda mengintegrasikan customer portal Dodo Payments ke dalam aplikasi Astro Anda dengan lancar.
...
Jika diatur ke <code>true</code>, email berisi link portal akan dikirim kepada pelanggan.
</ParamField>
