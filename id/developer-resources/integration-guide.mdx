...
4. Buat kunci API Anda:
   - Buka Developer > API
   - [Panduan Terperinci](/api-reference/introduction#api-key-management-and-authentication)
...
<Step title="Add advanced controls (optional)">
  ...
  - <ParamField query="paymentAmount" type="number">Menetapkan jumlah yang dibebankan dalam unit mata uang utama (misalnya, <code>12.5</code> untuk $12.50). Hanya untuk produk Pay What You Want. Nilai ini diabaikan jika berada di bawah harga minimum produk.</ParamField>
  - <ParamField query="metadata_*" type="string">Kolom metadata khusus (misalnya, <code>metadata_orderId=123</code>).</ParamField>

  <Warning>
  `paymentAmount` pada payment link bukan unit yang sama dengan kolom `amount` di Checkout Sessions API. Parameter link menggunakan unit mata uang utama (`12.5` = $12.50), sedangkan `product_cart[].amount` milik API menggunakan denominasi terkecil (`1250` = $12.50). Lihat [Dynamic Pricing](/developer-resources/dynamic-pricing-checkout) untuk kolom API tersebut.
  </Warning>
</Step>
<Step title="Share the link">
  Kirim payment link yang telah selesai kepada pelanggan Anda. Saat mereka mengunjunginya, semua parameter query dikumpulkan dan disimpan bersama ID sesi. URL kemudian disederhanakan sehingga hanya menyertakan parameter sesi (misalnya, <code>?session=sess_1a2b3c4d</code>). Informasi yang disimpan tetap ada setelah halaman dimuat ulang dan dapat diakses selama proses checkout.
  <Check>
...
- One-time Payment Link API [Referensi API](/api-reference/payments/post-payments)
- Subscription Payment Link API [Referensi API](/api-reference/subscriptions/post-subscriptions)

<Warning>
Kedua endpoint pembuatan link tersebut **sudah deprecated**. `POST /payments` dan `POST /subscriptions` tetap berfungsi untuk integrasi yang sudah ada, tetapi integrasi baru sebaiknya menggunakan [Checkout Sessions](/developer-resources/checkout-session) (`POST /checkouts`).
</Warning>

Panduan di bawah ini ditujukan untuk pembuatan one-time payment link.

Untuk petunjuk terperinci tentang mengintegrasikan subscription, lihat [Panduan Integrasi Subscription](/developer-resources/subscription-integration-guide) ini.
...
Implementasi webhook kami mengikuti spesifikasi [Standard Webhooks](https://standardwebhooks.com/). Untuk definisi jenis webhook, lihat [Panduan Event Webhook](/developer-resources/webhooks/intents/webhook-events-guide) kami.
...
| Jenis Event           | Kapan dipicu                                              | Tindakan yang harus dilakukan                                                                 |
| -------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------- |
...
| `payment.processing` | Pembayaran diterima tetapi masih diproses (metode async). | Tunggu `payment.succeeded`/`payment.failed` final — jangan penuhi pesanan dulu. |
| `payment.cancelled`  | Pembayaran dibatalkan sebelum selesai.                   | Lepaskan status yang ditahan dan tandai pesanan sebagai ditinggalkan.                       |
...
Jika Anda menjual produk digital dengan license key, tangani juga `license_key.created`. Untuk daftar lengkap event — termasuk event subscription, entitlement, credit, recovery, dan dunning — lihat [Panduan Event Webhook](/developer-resources/webhooks/intents/webhook-events-guide).

Anda dapat merujuk ke project ini yang memiliki implementasi demo di [GitHub](https://github.com/dodopayments/dodo-checkout-demo) menggunakan Next.js dan TypeScript.
