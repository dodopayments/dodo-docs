...
<Info>
**Dibangun dengan [ContextMCP.ai](https://contextmcp.ai)**: Dodo Knowledge MCP didukung oleh ContextMCP, memungkinkan pencarian semantik cepat di seluruh dokumentasi kami menggunakan embedding vektor.
</Info>
...
<Step title="Otentikasi dan konfigurasi">
...
<Tip>
Gunakan `@latest` untuk selalu menarik versi terbaru, atau tetapkan ke versi tertentu seperti `@1.0.0` untuk stabilitas.
</Tip>
...
<Tabs>
<Tab title="Cursor">
Atur server Dodo Payments MCP di Cursor untuk memungkinkan akses percakapan ke data pembayaran Anda.
...
Setelah mengklik, atur variabel lingkungan Anda di Cursor's `mcp.json` melalui **Pengaturan Cursor** > **Alat & MCP** > **Server MCP Baru**.
...
<Step title="Simpan dan mulai ulang">
...
<Tab title="Claude Desktop">
Atur server MCP Dodo Payments di Claude Desktop untuk mengaktifkan akses percakapan ke data pembayaran Anda.
<Steps>
...
<Step title="Mulai ulang Claude Desktop">
...
<Tab title="Claude Code">
Atur server Dodo Payments MCP di CLI Claude Code.
...
Konfigurasi akan disimpan di file `.claude.json` dalam direktori home Anda.
...
<Tab title="VS Code">
Atur server Dodo Payments MCP di VS Code.
...
Setelah mengklik, atur variabel lingkungan Anda di VS Code's `mcp.json` melalui **Command Palette** > **MCP: Buka Konfigurasi Pengguna**.
...
<Step title="Tambahkan konfigurasi server">
    Tambahkan server Dodo Payments MCP:
...
Atur server MCP Dodo Payments di Cline (VS Code) untuk mengaktifkan akses percakapan ke data pembayaran Anda.
<Steps>
...
<Tab title="Zed">
Atur server Dodo Payments MCP di Zed untuk memungkinkan akses percakapan ke data pembayaran Anda.
<Steps>
...
Atur server Dodo Payments MCP di klien lain yang kompatibel dengan MCP.
...
| `DODO_PAYMENTS_ENVIRONMENT` | Setel ke `live_mode` untuk produksi | Tidak |
...
| Klien | Deskripsi |
|--------|-------------|
| `infer` | Deteksi klien otomatis (default) |
...
- `top-level-unions` - Dukungan untuk jenis union tingkat atas
- `valid-json` - Parsing argumen string JSON yang benar
- `refs` - Dukungan untuk pointer $ref dalam skema
- `unions` - Dukungan untuk jenis union (anyOf)
- `formats` - Dukungan untuk validasi format
- `tool-name-length=N` - Panjang maksimum nama alat

<Tip>
Jalankan `npx dodopayments-mcp --describe-capabilities` untuk informasi kemampuan yang detail.
</Tip>
...
<Tip>
Gunakan `--operation=read` selama pengembangan untuk mencegah modifikasi yang tidak sengaja pada data Anda.
</Tip>
...
2. **`get_api_endpoint_schema`** - Mendapatkan skema detail untuk metode tertentu
3. **`invoke_api_endpoint`** - Menjalankan metode apapun dengan parameter yang sesuai

<Info>
Alat dinamis secara otomatis diaktifkan untuk API dengan lebih dari 50 metode. Gantilah dengan `--tools=all` untuk menyertakan semua alat.
</Info>
...
Saat melakukan penyebaran jarak jauh, selalu memerlukan otentikasi melalui header `Authorization` atau header `x-dodo-payments-api-key`.
...
<Accordion title="Kesalahan otentikasi">
...
Verifikasi `DODO_PAYMENTS_ENVIRONMENT` telah diatur dengan benar (`live_mode` untuk produksi).
...
Verifikasi operasi berfungsi saat memanggil API Dodo Payments secara langsung melalui curl atau Postman.
</Accordion>
