...
1. **Masuk** ke [Dasbor Pembayaran Dodo](https://app.dodopayments.com/)
2. **Navigasi ke** Produk → Meter
3. **Klik** "Buat Meter"
4. **Konfigurasi meter Anda**:
   - **Nama Meter**: Pilih nama deskriptif (misalnya, "Penggunaan Token LLM")
   - **Nama Acara**: Tetapkan pengidentifikasi acara yang unik (misalnya, `llm.chat_completion`)
   ...
   - **Atas Properti**: Pilih apa yang akan dilacak:
     ...
     - `outputTokens` - Lacak token output/penyelesaian (termasuk token penalaran jika berlaku)
...
<Note>
Saat menggunakan model yang mampu berpikir melalui AI SDK (seperti Gemini 2.5 Flash dengan mode berpikir), token penalaran secara otomatis termasuk dalam jumlah `outputTokens` untuk penagihan yang akurat.
</Note>
</Accordion>
...
- `outputTokens` - Jumlah token output/penyelesaian yang digunakan (termasuk token penalaran jika berlaku)
