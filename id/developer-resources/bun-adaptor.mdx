...
description: Pelajari cara mengintegrasikan Dodo Payments dengan proyek server
  Bun Anda menggunakan Bun Adaptor kami. Menyediakan checkout, portal pelanggan,
  webhooks, dan pengaturan lingkungan yang aman.
icon: code-merge
---
...
<Warning>
Jangan pernah mengkomit file <code>.env</code> atau rahasia Anda ke kontrol versi.
</Warning>
</Step>
</Steps>
...
<Info>
Semua contoh mengasumsikan Anda menggunakan server bawaan Bun dengan <code>Bun.serve()</code>.
</Info>
...
<Tab title="Webhook Handler">
  <Info>
  Gunakan handler ini untuk menerima dan memproses acara webhook Dodo Payments dengan aman di server Bun Anda.
  </Info>
  <CodeGroup>
...
- **Static Payment Links:** URL yang dapat dibagikan dengan mudah untuk pengumpulan pembayaran cepat, tanpa kode.
- **Dynamic Payment Links:** Menghasilkan URL pembayaran secara programatis dengan detail kustom menggunakan API atau SDK.
- **Checkout Sessions:** Buat pengalaman checkout yang aman dan dapat disesuaikan dengan keranjang produk dan detail pelanggan yang telah dikonfigurasi sebelumnya.
...
Pengidentifikasi produk (misalnya, <code>?productId=pdt_xxx</code>).
</ParamField>
...
<Warning>
Jika <code>productId</code> hilang, handler akan mengembalikan respons 400. Parameter query yang tidak valid juga menghasilkan respons 400.
</Warning>
...
- Untuk daftar lengkap field body POST yang didukung, lihat:
  - [Request body untuk Produk Pembayaran Sekali](https://docs.dodopayments.com/api-reference/payments/post-payments)
  - [Request body untuk Produk Langganan](https://docs.dodopayments.com/api-reference/subscriptions/post-subscriptions)
...
Sesi checkout menyediakan pengalaman checkout yang lebih aman dan dihosting yang menangani alur pembayaran lengkap untuk baik pembelian satu kali maupun langganan dengan kontrol kustomisasi penuh.

Lihat [Panduan Integrasi Sesi Checkout](https://docs.dodopayments.com/developer-resources/checkout-session) untuk informasi lebih lanjut dan daftar lengkap field yang didukung.
...
Customer Portal Route Handler memungkinkan Anda untuk mengintegrasikan portal pelanggan Dodo Payments dengan mulus ke dalam aplikasi server Bun Anda.
...
- **Routing Acara:** Memanggil penangan acara yang sesuai berdasarkan tipe payload.
...
const webhookHandler = Webhooks({
  ...
  onPaymentSucceeded: async (payload) => {
    console.log("Pembayaran berhasil:", payload);
    // Logika bisnis Anda di sini
  },
  onSubscriptionActive: async (payload) => {
    console.log("Langganan diaktifkan:", payload);
    // Logika bisnis Anda di sini
  },
});
...
Panduan Tambahan:
• Jelaskan bahwa handler adalah bawaan Bun dan bekerja tanpa masalah dengan Bun.serve()
• Soroti penggunaan objek Permintaan dan Respons Standar Web
• Sebutkan bahwa penanganan kesalahan sudah terintegrasi dan mengembalikan kode status HTTP yang sesuai
• Berikan tautan ke dokumentasi Dodo Payments untuk informasi lebih lanjut
```
