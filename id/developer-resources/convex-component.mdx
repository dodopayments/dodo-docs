---
title: Komponen Convex
description: Pelajari cara mengintegrasikan Dodo Payments dengan backend Convex
  Anda menggunakan Komponen Convex kami. Mencakup fungsi checkout, portal
...
{/* LOCKED_PATTERN_076a045a43785e7154357214ce1a1fe2 */}
    Integrasikan checkout Dodo Payments dengan alur berbasis sesi.
</Card>
...
Setelah mengedit `convex.config.ts`, jalankan `npx convex dev` sekali untuk menghasilkan tipe yang diperlukan.
</Step>
{/* LOCKED_PATTERN_85e536148c3d4a355f42242d1584f3dc */}
Atur variabel lingkungan di dasbor Convex Anda (**Settings** → **Environment Variables**). Anda dapat mengakses dasbor dengan menjalankan:
...
- `DODO_PAYMENTS_API_KEY` - Kunci API Dodo Payments Anda
- `DODO_PAYMENTS_ENVIRONMENT` - Atur ke `test_mode` atau `live_mode`
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Rahasia webhook Anda (diperlukan untuk penanganan webhook)

<Warning>
Selalu gunakan variabel lingkungan Convex untuk informasi sensitif. Jangan pernah menyertakan rahasia di version control.
</Warning>
</Step>
</Steps>

## Contoh Pengaturan Komponen

<Steps>
{/* LOCKED_PATTERN_545146c954909e50b35e2e31b703436f */}
Pertama, buat query internal untuk mengambil pelanggan dari database Anda. Ini akan digunakan dalam fungsi pembayaran untuk mengidentifikasi pelanggan.

<Warning>
Sebelum menggunakan query ini, pastikan untuk mendefinisikan skema yang sesuai di file `convex/schema.ts` Anda atau ubah query agar sesuai dengan skema yang sudah ada.
</Warning>
...
{/* LOCKED_PATTERN_cfdbe95b4d26ef82d25c4a1bfd5d5aeb */}
  <Info>
  Gunakan fungsi ini untuk mengintegrasikan checkout Dodo Payments ke dalam aplikasi Convex Anda. Menggunakan checkout berbasis sesi dengan dukungan fitur lengkap.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_de2bafe43db636b1864521f4f7e5aa25 */}
  <Info>
  Gunakan fungsi ini untuk memungkinkan pelanggan mengelola langganan dan detail mereka melalui portal pelanggan Dodo Payments. Pelanggan secara otomatis diidentifikasi melalui fungsi `identify`.
  </Info>
  <CodeGroup>
...
{/* LOCKED_PATTERN_eeaa14c4f31af768a3b7e376ee69b626 */}
  <Info>
  Gunakan handler ini untuk menerima dan memproses peristiwa webhook Dodo Payments secara aman di aplikasi Convex Anda. Semua handler webhook menerima `ActionCtx` Convex sebagai parameter pertama, memungkinkan Anda menggunakan `ctx.runQuery()` dan `ctx.runMutation()` untuk berinteraksi dengan database Anda.
  </Info>
  <CodeGroup>
  ...
  <Warning>
  Pastikan untuk mendefinisikan mutasi database yang sesuai di backend Convex Anda untuk setiap peristiwa webhook yang ingin Anda tangani. Misalnya, buat mutasi <code>createPayment</code> untuk merekam pembayaran yang berhasil atau mutasi <code>createSubscription</code> untuk mengelola status langganan.
  </Warning>
</Tab>
</Tabs>
...
<Info>
Komponen Convex Dodo Payments menggunakan checkout berbasis sesi, menyediakan pengalaman checkout yang aman dan dapat disesuaikan dengan keranjang produk dan detail pelanggan yang telah dikonfigurasi sebelumnya. Ini adalah pendekatan yang direkomendasikan untuk semua alur pembayaran.
</Info>
...
Lihat [Sesi Checkout](/developer-resources/checkout-session) untuk detail lebih lanjut dan daftar lengkap bidang yang didukung.

### Format Respons
Fungsi checkout mengembalikan respons JSON dengan URL checkout:
...
## Fungsi Portal Pelanggan

Fungsi Portal Pelanggan memungkinkan Anda untuk mengintegrasikan portal pelanggan Dodo Payments ke dalam aplikasi Convex Anda dengan mulus.
...
Jika diatur ke <code>true</code>, mengirimkan email ke pelanggan dengan tautan portal.
</ParamField>

<Info>
Pelanggan secara otomatis diidentifikasi menggunakan fungsi `identify` yang dikonfigurasi dalam setup DodoPayments Anda. Fungsi ini harus mengembalikan `dodoCustomerId` pelanggan.
</Info>

## Penangan Webhook

- **Metode:** Hanya permintaan POST yang didukung. Metode lain mengembalikan 405.
- **Verifikasi Tanda Tangan:** Memverifikasi tanda tangan webhook menggunakan <code>DODO_PAYMENTS_WEBHOOK_SECRET</code>. Mengembalikan 401 jika verifikasi gagal.
...
### Penangan Event Webhook yang Didukung
<CodeGroup>
...
<Info>
Gunakan fungsi checkout dari komponen React Anda dengan hook Convex.
</Info>
