...
Atur variabel lingkungan di dasbor Convex Anda (**Pengaturan** → **Variabel Lingkungan**). Anda dapat mengakses dasbor dengan menjalankan:
...
<Warning>
Sebelum menggunakan kueri ini, pastikan untuk mendefinisikan skema yang sesuai di file `convex/schema.ts` Anda atau ubah kueri agar sesuai dengan skema yang ada.
</Warning>
...
<Tab title="Pengaturan Penangan Webhook">
  <Info>
  Gunakan penangan ini untuk menerima dan memproses acara webhook Dodo Payments dengan aman di aplikasi Convex Anda. Semua penangan webhook menerima `ActionCtx` Convex sebagai parameter pertama, memungkinkan Anda untuk menggunakan `ctx.runQuery()` dan `ctx.runMutation()` untuk berinteraksi dengan database Anda.
  </Info>
  <CodeGroup>
...
<Info>
Pelanggan secara otomatis diidentifikasi menggunakan fungsi `identify` yang dikonfigurasi dalam pengaturan DodoPayments Anda. Fungsi ini harus mengembalikan `dodoCustomerId` pelanggan.
</Info>
