...
<CardGroup cols={2}>
<Card title="Fungsi Checkout" icon="cart-shopping" href="#checkout-function">
    Integrasikan checkout Dodo Payments dengan alur berbasis sesi.
</Card>
<Card title="Portal Pelanggan" icon="user" href="#customer-portal-function">
...
<Step title="Tambahkan Komponen ke Konfigurasi Convex">
...
Setelah mengedit `convex.config.ts`, jalankan `npx convex dev` sekali untuk membuat tipe yang diperlukan.
</Step>
...
- `DODO_PAYMENTS_ENVIRONMENT` - Diatur ke `test_mode` atau `live_mode`
- `DODO_PAYMENTS_WEBHOOK_SECRET` - Rahasia webhook Anda (diperlukan untuk penanganan webhook)

<Warning>
Selalu gunakan variabel lingkungan Convex untuk informasi sensitif. Jangan pernah mengkomit rahasia ke kontrol versi.
</Warning>
</Step>
</Steps>
...
<Steps>
<Step title="Buat Query Internal">
...
<Warning>
Sebelum menggunakan kueri ini, pastikan untuk mendefinisikan skema yang sesuai di file `convex/schema.ts` Anda atau ubah kueri agar sesuai dengan skema Anda yang ada.
</Warning>
...
<Step title="Konfigurasi Komponen DodoPayments">
 <CodeGroup>
...
<Tabs>
<Tab title="Pengaturan Fungsi Checkout">
  <Info>
...
<Tab title="Pengaturan Portal Pelanggan">
  <Info>
...
<Tab title="Pengaturan Penangan Webhook">
  <Info>
  Gunakan penangan ini untuk menerima dan memproses peristiwa webhook Dodo Payments dengan aman di aplikasi Convex Anda. Semua penangan webhook menerima `ActionCtx` sebagai parameter pertama, memungkinkan Anda untuk menggunakan `ctx.runQuery()` dan `ctx.runMutation()` untuk berinteraksi dengan basis data Anda.
  </Info>
  <CodeGroup>
  ...
  <Warning>
  Pastikan untuk mendefinisikan mutasi database yang sesuai di backend Convex Anda untuk setiap acara webhook yang ingin Anda tangani. Misalnya, buat mutasi <code>createPayment</code> untuk mencatat pembayaran yang berhasil atau mutasi <code>createSubscription</code> untuk mengelola status langganan.
  </Warning>
</Tab>
</Tabs>
...
<Info>
Komponen Convex Dodo Payments menggunakan checkout berbasis sesi, memberikan pengalaman checkout yang aman dan dapat disesuaikan dengan keranjang produk dan detail pelanggan yang telah dikonfigurasi sebelumnya. Ini adalah pendekatan yang disarankan untuk semua alur pembayaran.
</Info>
...
<Info>
Pelanggan secara otomatis diidentifikasi menggunakan fungsi `identify` yang telah dikonfigurasi dalam pengaturan DodoPayments Anda. Fungsi ini harus mengembalikan `dodoCustomerId` pelanggan.
</Info>
