...
<Info>
Boilerplate ini menggunakan Go 1.21+ dengan pola arsitektur bersih (`cmd`, `internal`, `templates`), templat HTML, dan SDK `dodopayments-go` untuk integrasi API yang mulus.
</Info>
...
- **UI Modern** - Halaman harga bertema gelap yang bersih dengan templat HTML
- **Penanganan Webhook** - Memverifikasi dan memproses peristiwa pembayaran dengan aman
...
<Warning>
Jangan pernah mengirimkan file `.env` Anda ke pengendalian versi. Ini sudah termasuk dalam `.gitignore`.
</Warning>
</Step>
...
Di `templates/index.html`, ganti data pelanggan yang dikodekan keras dengan data pengguna aktual Anda:
...
Boilerplate ini menunjukkan penanganan acara webhook di `internal/api/webhook.go`. Acara yang didukung termasuk:

| Acara | Deskripsi |
|-------|-------------|
| `subscription.active` | Ditrigger ketika langganan menjadi aktif |
| `payment.succeeded` | Ditrigger ketika pembayaran berhasil |
...
- Ganti `DODO_PAYMENTS_ENVIRONMENT` menjadi `live_mode`
...
<Accordion title="Kegagalan dalam membuat sesi checkout">
**Penyebab umum:**
- ID produk tidak valid - verifikasi apakah ID tersebut ada di dasbor Dodo Anda
- Kunci API atau pengaturan lingkungan yang salah di `.env`
- Periksa log server untuk pesan kesalahan secara mendetail
</Accordion>
