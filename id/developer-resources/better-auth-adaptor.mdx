...
<Info>
  Tidak seperti metode <code>checkout</code> lama, <code>checkoutSession</code>
  tidak memerlukan informasi penagihan di awal karena akan diambil dari pengguna
  di halaman pembayaran. Anda masih dapat menimpanya dengan melewatkan kunci
  <code>billing</code> dalam argumen.
</Info>

<Info>
  Mirip dengan metode <code>checkout</code> lama,
  <code>checkoutSession</code> mengambil email dan nama pelanggan dari sesi
  better-auth mereka, namun, Anda dapat menimpanya dengan melewatkan objek
  <code>customer</code> dengan email dan nama.
</Info>

<Info>
  Anda dapat melewatkan opsi yang sama dalam argumen sebagai body request untuk
  [Create Checkout Session](https://docs.dodopayments.com/api-reference/checkout-sessions/create)
  endpoint.
</Info>

<Note>
  URL pengembalian diambil dari <code>successUrl</code> yang dikonfigurasi dalam
  plugin server. Anda tidak perlu memasukkan <code>return_url</code> dalam
  payload klien.
</Note>

## Legacy Checkout (Kadaluwarsa)

<Warning>
  Metode <code>authClient.dodopayments.checkout</code> sudah kadaluwarsa. Gunakan
...
Aktifkan plugin <code>usage()</code> di server untuk menangkap acara terukur dan memungkinkan pelanggan memeriksa penagihan berbasis penggunaan mereka.

- <code>authClient.dodopayments.usage.ingest</code> menerima acara untuk pengguna yang sudah masuk dan terverifikasi emailnya.
- <code>authClient.dodopayments.usage.meters.list</code> mencantumkan penggunaan terbaru untuk meter yang terikat dengan langganan pelanggan tersebut.
...
<Tip>
  Tanda waktu yang lebih lama dari satu jam atau lebih dari lima menit ke depan
  akan ditolak ketika memasukkan penggunaan.
</Tip>
<Tip>
  Jika Anda mengabaikan <code>meter_id</code> saat mendaftarkan meter penggunaan, semua meter 
  yang terikat dengan langganan pelanggan akan ditampilkan.
</Tip>

# Webhooks

<Info>
  Plugin webhooks memproses acara pembayaran real-time dari Dodo Payments dengan
...
Hasilkan rahasia webhook untuk URL endpoint Anda (misalnya, <code>https://&lt;your-domain&gt;/api/auth/dodopayments/webhooks</code>) di Dashboard Dodo Payments dan atur di file <code>.env</code> Anda:
...
<AccordionGroup>
  <Accordion title="Plugin Options">
    - <b>client</b> (wajib): Instansi klien DodoPayments
    - <b>createCustomerOnSignUp</b> (opsional): Buat pelanggan secara otomatis saat pengguna mendaftar
    - <b>use</b> (wajib): Array plugin untuk diaktifkan (checkout, portal, usage, webhooks)
    - <b>getCustomerParams</b> (opsional): Fungsi yang menerima BetterAuth `User` dan mengembalikan bidang tambahan untuk dilampirkan pada pelanggan DodoPayments saat pembuatan dan pembaruan (misalnya, `metadata`, `phone_number`)
...
<AccordionGroup>
  <Accordion title="Common Issues">
    - <b>Kunci API Tidak Valid</b>: Periksa kembali
    <code>DODO_PAYMENTS_API_KEY</code> Anda di <code>.env</code>. -{" "}
    <b>Ketidakcocokan tanda tangan Webhook</b>: Pastikan rahasia webhook cocok dengan yang
    diatur di Dashboard Dodo Payments. - <b>Pelanggan tidak dibuat</b>: Pastikan
  ...
  <Accordion title="Best Practices">
    ...
    acara webhook untuk debugging dan audit.
  </Accordion>
</AccordionGroup>
