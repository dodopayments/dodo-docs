---
title: Fungsi Netlify
description: Terapkan webhook DodoPayments ke Fungsi Netlify
icon: diamond
---
...
### 1. Prasyarat
- [Akun Netlify](https://app.netlify.com/signup)
...
2. Buat proyek baru
3. Buka Editor SQL
4. Salin dan tempel isi [`schema.sql`](#database-schema)
5. Jalankan kueri
6. Dapatkan string koneksi Anda dari Neon → Rincian Koneksi
...
> **Catatan:** Kami akan mengatur `DODO_PAYMENTS_WEBHOOK_KEY` setelah deployment begitu Anda memiliki URL webhook Anda.

### 5. Inisialisasi & Terapkan
...
4. Aktifkan event langganan berikut ini:
   - `subscription.active`
   - `subscription.cancelled`
   - `subscription.renewed`
5. Salin **Signing Secret**

### 8. Atur Kunci Webhook & Terapkan Kembali
...
✅ **Penanganan error** - Dicatat dan bisa dicoba ulang

> **Catatan:** Implementasi ini menunjukkan penanganan tiga event langganan inti (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) dengan field minimal. Anda bisa dengan mudah memperluasnya untuk mendukung tipe event dan field tambahan sesuai kebutuhan Anda.
...
4. **Memproses event** - Membuat atau memperbarui pelanggan dan langganan di Neon
5. **Menangani error** - Mencatat kegagalan dan menandai event sebagai belum diproses agar bisa dicoba ulang
...
**Lihat log di Dashboard Netlify:**
1. Pilih situs Anda → tab **Functions**
2. Klik fungsi `webhook`
3. Lihat log real-time dan riwayat pemanggilan

**Konfigurasi di Dasbor DodoPayments:**
1. Pergi ke Pengembang → Webhooks
2. Tambahkan endpoint dengan URL Fungsi Netlify Anda
...
| Masalah | Solusi |
|---------|---------|
| Verifikasi gagal | Periksa apakah kunci webhook benar dari dashboard DodoPayments |
| Kesalahan koneksi database | Verifikasi string koneksi Neon dan gunakan koneksi pooled |
| Fungsi tidak ditemukan (404) | Jalankan `netlify init` dan `npm run deploy` lagi |
| Variabel env tidak tersedia | Tetapkan variabel di CLI atau dashboard, lalu deploy ulang |

## Sumber Daya

- [Dokumentasi Fungsi Netlify](https://docs.netlify.com/functions/overview/)
...
- [Repositori GitHub](https://github.com/dodopayments/cloud-functions/tree/main/netlify)
