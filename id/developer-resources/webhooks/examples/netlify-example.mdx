...
<Card title="Repositori GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/netlify">
...
2. Buat proyek baru
3. Buka SQL Editor
4. Salin dan tempel konten dari [`schema.sql`](#database-schema)
5. Jalankan kueri
...
> **Catatan:** Kami akan mengatur `DODO_PAYMENTS_WEBHOOK_KEY` setelah penerapan ketika Anda memiliki URL webhook Anda.
...
2. Buat titik akhir webhook baru
3. Konfigurasi URL webhook Anda sebagai titik akhir
...
✅ **Pencatatan acara** - Jejak audit lengkap dalam tabel `webhook_events`  
✅ **Penanganan kesalahan** - Dicatat dan dapat dicoba ulang

> **Catatan:** Implementasi ini menunjukkan penanganan tiga acara langganan inti (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) dengan minimal field. Anda dapat dengan mudah memperluasnya untuk mendukung tipe acara tambahan dan field berdasarkan kebutuhan Anda.
...
2. **Memeriksa duplikat** - Menggunakan ID webhook untuk mencegah pemrosesan acara yang sama dua kali
3. **Mencatat acara** - Menyimpan webhook mentah di tabel `webhook_events` untuk jejak audit
4. **Memproses acara** - Membuat atau memperbarui pelanggan dan langganan di Neon
5. **Menangani kesalahan** - Mencatat kegagalan dan menandai acara sebagai tidak diproses untuk dicoba ulang
...
**Lihat log di Netlify Dashboard:**
...
| Kesalahan koneksi basis data | Verifikasi string koneksi Neon dan gunakan koneksi pooled |
| Fungsi tidak ditemukan (404) | Jalankan `netlify init` dan `npm run deploy` lagi |
| Variabel env tidak tersedia | Atur variabel di CLI atau dashboard, lalu lakukan redeploy |
