---
title: Fungsi Vercel
description: Deploy webhook DodoPayments ke Vercel
icon: segitiga
---

<Card title="Repositori GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/vercel">
...
1. Daftar untuk [Neon](https://neon.com)
2. Buat proyek baru
3. Buka SQL Editor
...
> **Catatan:** Kami akan mengatur `DODO_PAYMENTS_WEBHOOK_KEY` setelah penempatan sekali Anda memiliki URL webhook Anda.
...
4. Aktifkan peristiwa langganan ini:
   - `subscription.active`
...
✅ **Pencatatan peristiwa** - Jejak audit lengkap di tabel `webhook_events`  
✅ **Penanganan kesalahan** - Dicatat dan dapat diulang

> **Catatan:** Implementasi ini menunjukkan penanganan tiga peristiwa langganan inti (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) dengan bidang minimal. Anda dapat dengan mudah memperluasnya untuk mendukung jenis dan bidang peristiwa tambahan berdasarkan kebutuhan Anda.
...
2. **Verifikasi tanda tangan** - Memastikan permintaan berasal dari DodoPayments menggunakan HMAC-SHA256
3. **Memeriksa duplikat** - Menggunakan ID webhook untuk mencegah pemrosesan peristiwa yang sama dua kali
4. **Mencatat peristiwa** - Menyimpan webhook mentah di tabel `webhook_events` untuk jejak audit
5. **Memproses peristiwa** - Membuat atau memperbarui pelanggan dan langganan di Neon
6. **Menangani kesalahan** - Mencatat kegagalan dan menandai peristiwa sebagai belum diproses untuk diulang
...
| Kesalahan koneksi database | Verifikasi string koneksi Neon dan gunakan koneksi terpool |
| Timeout fungsi | Optimalkan kueri; Pro plan memiliki timeout lebih lama (60s) |
| Env vars tidak tersedia | Atur di dasbor atau CLI, pastikan semua lingkungan dipilih, redeploy |

## Sumber Daya

- [Dokumentasi Fungsi Vercel](https://vercel.com/docs/functions)
...
- [Repositori GitHub](https://github.com/dodopayments/cloud-functions/tree/main/vercel)
