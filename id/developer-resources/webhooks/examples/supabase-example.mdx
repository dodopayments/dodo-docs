...
2. Buka SQL Editor
3. Buat kueri baru
4. Salin dan tempel seluruh konten dari [`schema.sql`](#database-schema)
5. Jalankan kueri
...
Fungsi sudah disiapkan di `functions/webhook/index.ts` - cukup sebarkan:
...
1. Pergi ke [DodoPayments Dashboard](https://app.dodopayments.com) → Developer → Webhooks
...
3. **Mencatat acara** - Menyimpan data webhook mentah di tabel `webhook_events`
4. **Memproses pembaruan** - Membuat atau memperbarui pelanggan dan langganan melalui klien Supabase
5. **Menangani kesalahan** - Mencatat kegagalan dan menandai acara untuk dicoba ulang
...
| Kesalahan verifikasi JWT | Sebarkan dengan `--no-verify-jwt` flag |
| Fungsi tidak ditemukan | Verifikasi referensi proyek benar dan fungsi telah disebarkan |
