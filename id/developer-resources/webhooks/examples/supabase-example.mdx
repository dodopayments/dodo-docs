...
<Card title="Repositori GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/supabase">
  Kode sumber lengkap dan panduan pengaturan
</Card>

<Tip>
  <strong>Periksa paket starter lengkap untuk supabase</strong><br />
  Paket starter langganan minimal yang dibangun dengan Next.js, Supabase, dan Dodo Payments. Boilerplate ini membantu Anda dengan cepat menyiapkan SaaS berbasis langganan dengan otentikasi, pembayaran, dan webhook.
...
1. Pergi ke [Dasbor Supabase](https://supabase.com/dashboard)
2. Buka SQL Editor
3. Buat query baru
4. Salin dan tempel seluruh isi dari [`schema.sql`](#database-schema)
5. Jalankan query
...
> **Catatan:** Kami akan mengatur `DODO_PAYMENTS_WEBHOOK_KEY` setelah penyebaran begitu Anda memiliki URL webhook Anda.

### 5. Terapkan

Fungsi ini sudah disiapkan di `functions/webhook/index.ts` - cukup sebar: 
...
1. Pergi ke [Dasbor DodoPayments](https://app.dodopayments.com) → Developer → Webhooks
...
✅ **Pencatatan acara** - Jejak audit lengkap di tabel `webhook_events`  
...
2. **Memeriksa idempotensi** - Mencari ID webhook untuk mencegah pemrosesan duplikat
3. **Mencatat acara** - Menyimpan data mentah webhook di tabel `webhook_events`
4. **Memproses pembaruan** - Membuat atau memperbarui pelanggan dan langganan melalui klien Supabase
5. **Menangani kesalahan** - Mencatat kegagalan dan menandai acara untuk percobaan ulang
...
| Masalah | Solusi |
|-------|----------|
| Verifikasi gagal | Periksa kunci webhook benar dari dasbor DodoPayments |
| Kesalahan izin database | Pastikan menggunakan Service Role Key |
| Kesalahan verifikasi JWT | Sebarkan dengan flag `--no-verify-jwt` |
| Fungsi tidak ditemukan | Verifikasi referensi proyek benar dan fungsi sudah disebar |
