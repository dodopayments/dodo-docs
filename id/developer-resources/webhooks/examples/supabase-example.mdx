...
description: Terapkan webhook DodoPayments ke Supabase Edge Functions
icon: bolt
---

{/* LOCKED_PATTERN_22a8d1fd3fdf6a73444bca527433b154 */}
  Kode sumber lengkap dan panduan pemasangan
</Card>

<Tip>
  <strong>Lihat paket awal lengkap untuk Supabase</strong><br />
  Paket awal langganan minimal yang dibangun dengan Next.js, Supabase, dan Dodo Payments. Boilerplate ini membantu Anda dengan cepat menyiapkan SaaS berbasis langganan dengan otentikasi, pembayaran, dan webhook.
...
Dapatkan referensi proyek Anda dari [Supabase Dashboard](https://supabase.com/dashboard) → Pengaturan Proyek
...
2. Buka Editor SQL
3. Buat kueri baru
4. Salin dan tempel seluruh isi [`schema.sql`](#database-schema)
5. Jalankan kueri

### 4. Atur Rahasia Awal

Supabase secara otomatis menyediakan `SUPABASE_URL` dan `SUPABASE_SERVICE_ROLE_KEY` saat runtime.

```bash
npx supabase secrets set DODO_PAYMENTS_API_KEY=your-api-key
```

> **Catatan:** Kami akan mengatur `DODO_PAYMENTS_WEBHOOK_KEY` setelah penerapan setelah Anda memiliki URL webhook Anda.

### 5. Terapkan

Fungsi sudah disiapkan di `functions/webhook/index.ts` - cukup terapkan:
...
### 7. Daftarkan Webhook di DodoPayments Dashboard
...
4. Aktifkan event langganan ini:
   - `subscription.active`
   - `subscription.cancelled`
   - `subscription.renewed`
5. Salin **Signing Secret**

### 8. Atur Kunci Webhook & Terapkan Ulang
...
Memproses acara langganan dan menyimpannya di Supabase PostgreSQL:
...
✅ **Signature verification** - Menggunakan pustaka dodopayments  
✅ **Idempotency** - Mencegah pemrosesan ganda dengan ID webhook  
✅ **Event logging** - Jejak audit lengkap di tabel `webhook_events`  
✅ **Error handling** - Dicatat dan dapat dicoba ulang

> **Catatan:** Implementasi ini menunjukkan penanganan tiga event langganan inti (`subscription.active`, `subscription.cancelled`, `subscription.renewed`) dengan bidang minimal. Anda dapat dengan mudah memperluasnya untuk mendukung jenis event dan bidang tambahan sesuai kebutuhan Anda.
...
Fungsi Edge berbasis Deno:
1. **Memverifikasi tanda tangan** - Menggunakan pustaka dodopayments untuk verifikasi HMAC-SHA256
2. **Memeriksa idempoten** - Mengecek ID webhook untuk mencegah pemrosesan duplikat
3. **Mencatat event** - Menyimpan data mentah webhook di tabel `webhook_events`
4. **Memproses pembaruan** - Membuat atau memperbarui pelanggan dan langganan melalui klien Supabase
5. **Menangani kesalahan** - Mencatat kegagalan dan menandai event untuk dicoba ulang
...
<Note>
Flag `--no-verify-jwt` diperlukan karena webhook tidak menyertakan token JWT. Keamanan disediakan oleh verifikasi tanda tangan webhook.
</Note>
...
Atau di [Supabase Dashboard](https://supabase.com/dashboard) → Edge Functions → webhook → tab Logs

**Konfigurasi di DodoPayments Dashboard:**
1. Pergi ke Pengembang → Webhook
2. Tambahkan endpoint dengan URL Supabase Anda
...
| Masalah | Solusi |
|-------|----------|
| Verification failed | Periksa apakah kunci webhook benar dari dasbor DodoPayments |
| Database permission error | Pastikan menggunakan Service Role Key |
| JWT verification error | Terapkan dengan flag `--no-verify-jwt` |
| Function not found | Verifikasi referensi proyek sudah benar dan fungsi sudah diterapkan |

## Sumber Daya

- [Dokumen Supabase Edge Functions](https://supabase.com/docs/guides/functions)
- [Supabase CLI](https://supabase.com/docs/reference/cli)
- [Deno Runtime](https://deno.land/)
- [Panduan Acara Webhook](/developer-resources/webhooks/intents/webhook-events-guide)
- [Repositori GitHub](https://github.com/dodopayments/cloud-functions/tree/main/supabase)
