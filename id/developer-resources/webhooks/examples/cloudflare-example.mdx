...
<Card title="Repositori GitHub" icon="github" href="https://github.com/dodopayments/cloud-functions/tree/main/cloudflare">
...
> **Catatan:** Kami akan mengatur `DODO_PAYMENTS_WEBHOOK_KEY` setelah deployment sekali Anda memiliki URL webhook Anda.

### 5. Perbarui wrangler.toml

Edit `wrangler.toml` dan atur nama pekerja Anda:
...
✅ **Penanganan kesalahan** - Dicatat dan dapat dicoba kembali
...
2. **Memeriksa duplikat** - Menggunakan ID webhook untuk mencegah memproses acara yang sama dua kali
3. **Mencatat acara** - Menyimpan webhook mentah di tabel `webhook_events` untuk jejak audit
4. **Memproses acara** - Membuat atau memperbarui pelanggan dan langganan
5. **Menangani kesalahan** - Mencatat kegagalan dan menandai acara sebagai tidak diproses untuk dicoba kembali
...
| Kesalahan koneksi database | Verifikasi string koneksi Neon |
| Pekerja tidak terdeploy | Jalankan `npm install`, periksa sintaks `wrangler.toml` |
