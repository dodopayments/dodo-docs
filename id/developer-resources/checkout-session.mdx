...
Izinkan pelanggan untuk menambahkan atau menghapus addon pada produk berlangganan selama checkout. Hanya berlaku untuk produk berlangganan.
</ParamField>
...
Lewati halaman sukses pembayaran default dan alihkan pelanggan segera ke `return_url` setelah penyelesaian pembayaran.

<Tip>
Gunakan ini ketika Anda memiliki halaman sukses kustom yang memberikan pengalaman pengguna lebih baik, atau untuk aplikasi seluler dan alur checkout tertanam.
</Tip>
</ParamField>
</Expandable>
</ParamField>
...
Pengenal unik untuk bidang ini. Digunakan sebagai kunci dalam payload webhook dan respons API.

**Contoh**: `"company_name"`, `"referral_source"`, `"team_size"`
</ParamField>

<ParamField body="label" type="string" required>
Label tampilan ditampilkan kepada pelanggan pada formulir checkout.

**Contoh**: `"Company Name"`, `"How did you hear about us?"`, `"Team Size"`
</ParamField>

<ParamField body="field_type" type="string" required>
Tipe bidang menentukan aturan validasi input.

**Jenis yang Tersedia**:
- `text` - Masukan teks satu baris
- `number` - Masukan numerik
...
Teks placeholder ditampilkan di bidang input.
</ParamField>

<ParamField body="options" type="array">
Daftar opsi untuk tipe bidang `dropdown`. Diperlukan saat `field_type` adalah `dropdown`, diabaikan untuk jenis lain.
...
Konfigurasi langganan on-demand untuk penagihan berdasarkan penggunaan atau penghitungan.

<Expandable title="On-demand object properties">
<ParamField body="mandate_only" type="boolean" required>
Jika diatur ke true, tidak melakukan biaya apa pun dan hanya mengotorisasikan rincian metode pembayaran untuk penggunaan di masa depan.
</ParamField>
...
**Format**: Dinyatakan dalam nilai terendah mata uang (misalnya, sen untuk USD). Misalnya, untuk biaya $ 1,00, masukkan `100`.
</ParamField>

<ParamField body="product_currency" type="string">
Mata uang produk opsional. Jika tidak ditentukan, akan menggunakan mata uang produk.
</ParamField>

<ParamField body="product_description" type="string">
Deskripsi produk opsional yang dapat diganti untuk penagihan dan item baris. Jika tidak ditentukan, deskripsi produk yang disimpan akan digunakan.
</ParamField>
...
Untuk informasi mendetail tentang konfigurasi dan pengujian UPI, lihat halaman <a href="/features/payment-methods/india">Metode Pembayaran India</a>.
...
Untuk informasi mendetail tentang configurasi dan pengujian BNPL, lihat halaman <a href="/features/payment-methods/bnpl">Beli Sekarang Bayar Nanti (BNPL)</a>.
...
### Perbedaan Kunci
Sebelumnya, saat membuat tautan pembayaran dengan Tautan Dinamis, Anda diharuskan menyediakan alamat penagihan lengkap pelanggan.

Dengan Sesi Checkout, hal ini tidak lagi diperlukan. Anda dapat menyampaikan informasi apa pun yang Anda miliki, dan kami akan menanganinya. Sebagai contoh:
