...
<Info>
**Validitas Sesi**: Sesi checkout berlaku selama 24 jam secara default. Jika Anda mengirimkan `confirm=true` dalam permintaan Anda, sesi hanya akan berlaku selama 15 menit.
</Info>
...
<Tip>
**Opsi Integrasi Alternatif**: Alih-alih mengarahkan, Anda dapat menyematkan checkout langsung di halaman Anda menggunakan [Overlay Checkout](/developer-resources/overlay-checkout) (overlay modal) atau [Inline Checkout](/developer-resources/inline-checkout) (sepenuhnya disematkan). Kedua opsi menggunakan URL sesi checkout yang sama.
</Tip>
</Step>

<Step title="Tangani pengembalian">
Setelah pembayaran, pelanggan akan diarahkan ke `return_url` Anda dengan parameter kueri termasuk ID pembayaran/langganan, status, email pelanggan, dan kunci lisensi apa pun. Lihat [dokumentasi parameter `return_url`](#request-body) untuk daftar lengkap.
</Step>
</Steps>
...
Array produk yang akan disertakan dalam sesi checkout. Setiap produk harus memiliki `product_id` yang valid dari dashboard Dodo Payments.
...
**Contoh:** `1` untuk satu item, `3` untuk beberapa kuantitas
</ParamField>
...
**Format**: Diwakili dalam denominasi terendah dari mata uang (misalnya, sen untuk USD). Sebagai contoh, untuk dikenakan biaya $1,00, kirim `100`.
</ParamField>
</Expandable>
</ParamField>

<Tip>
**Temukan ID Produk Anda**: Anda dapat menemukan ID produk di dasbor Dodo Payments Anda di bawah Produk → Lihat Detail, atau dengan menggunakan [List Products API](/api-reference/products/get-products).
</Tip>
...
<Info>
Ketika `confirm` diatur ke `true`, semua bidang alamat penagihan menjadi wajib untuk pembuatan sesi yang sukses.
</Info>
...
**Contoh**: `"US"` (Amerika Serikat), `"CA"` (Kanada), `"GB"` (Britania Raya), `"DE"` (Jerman)
...
**Contoh**: `"94102"` (AS), `"M5V 3A8"` (Kanada), `"SW1A 1AA"` (UK)
</ParamField>
</Expandable>
</ParamField>
...
Mengontrol metode pembayaran mana yang tersedia untuk pelanggan selama checkout. Ini membantu mengoptimalkan untuk pasar atau persyaratan bisnis tertentu.
...
Tindakan untuk mengesampingkan pemilihan mata uang default dengan mata uang penagihan tetap. Menggunakan kode mata uang ISO 4217.
...
URL untuk mengarahkan pelanggan setelah pembayaran selesai. Aplikasi Dodo Payments akan menambahkan parameter kueri berikut ke URL Anda setelah pengalihan:
...
| `license_key` | `string` | Ada jika produk memiliki kunci lisensi yang diaktifkan. Dipisahkan dengan koma jika ada beberapa kunci |
| `email` | `string` | Ada jika pelanggan memiliki email di catatan |
...
<Tip>
Gunakan parameter kueri `license_key` dan `email` untuk menampilkan kunci lisensi atau mengirim konfirmasi segera di halaman pengembalian Anda, tanpa perlu panggilan API tambahan.
</Tip>
</ParamField>

<ParamField body="confirm" type="boolean" default="false">
Jika true, menyelesaikan semua detail sesi segera. API akan menghasilkan kesalahan jika data yang diperlukan hilang.
</ParamField>
...
Gantikan perilaku 3DS default merchant untuk sesi ini.
</ParamField>
...
- **Kode POS/Zip**: Hanya di daerah yang diperlukan untuk perhitungan pajak penjualan, PPN, atau GST

Ini secara signifikan mengurangi gesekan checkout dengan menghilangkan kolom formulir yang tidak perlu.
...
Kustomisasi penampilan dan perilaku antarmuka checkout.
...
`ar` (Arab), `ca` (Katalan), `de` (Jerman), `en` (Inggris), `es` (Spanyol), `fr` (Prancis), `he` (Ibrani), `id` (Indonesia), `it` (Italia), `ja` (Jepang), `ko` (Korea), `ms` (Melayu), `nl` (Belanda), `pl` (Polandia), `pt` (Portugis), `ro` (Rumania), `ru` (Rusia), `sv` (Swedia), `th` (Thai), `tr` (Turki), `zh` (Cina)
...
Konfigurasi fitur dan perilaku tertentu untuk sesi checkout.

<Expandable title="Properti objek flag fitur">
<ParamField body="allow_currency_selection" type="boolean" default="true">
Izinkan pelanggan untuk memilih mata uang yang diinginkan selama checkout.
</ParamField>
...
Lewati halaman sukses pembayaran default dan arahkan pelanggan segera ke `return_url` setelah pembayaran selesai.

<Tip>
Gunakan ini ketika Anda memiliki halaman sukses khusus yang memberikan pengalaman pengguna yang lebih baik, atau untuk aplikasi seluler dan alur checkout tertanam.
</Tip>
</ParamField>
</Expandable>
</ParamField>
...
<Accordion title="Field Kustom">

<ParamField body="custom_fields" type="array">
Kumpulkan informasi tambahan dari pelanggan selama checkout dengan kolom formulir kustom. Anda dapat mendefinisikan hingga 5 kolom kustom per sesi checkout. Tanggapan pelanggan disertakan dalam payload webhook dan tersedia melalui API.

<Expandable title="Properti objek field kustom">
<ParamField body="key" type="string" required>
Pengidentifikasi unik untuk kolom ini. Digunakan sebagai kunci dalam payload webhook dan respons API.

**Contoh**: `"company_name"`, `"referral_source"`, `"team_size"`
</ParamField>

<ParamField body="label" type="string" required>
Label tampilan yang ditampilkan kepada pelanggan di formulir checkout.

**Contoh**: `"Company Name"`, `"How did you hear about us?"`, `"Team Size"`
</ParamField>

<ParamField body="field_type" type="string" required>
Tipe field yang menentukan aturan validasi input.
...
Teks placeholder yang ditampilkan dalam kolom input.
</ParamField>

<ParamField body="options" type="array">
Daftar opsi untuk jenis field `dropdown`. Diperlukan ketika `field_type` adalah `dropdown`, diabaikan untuk tipe lainnya.
...
<Info>
Tanggapan pelanggan terhadap kolom kustom disertakan dalam:
- **Webhook**: `payment.succeeded`, `subscription.created`, dan payload peristiwa relevan lainnya berisi array `custom_field_responses`
...
Konfigurasi langganan sesuai permintaan untuk penagihan berbasis penggunaan atau terukur.

<Expandable title="Properti objek sesuai permintaan">
...
Harga produk untuk biaya pertama kepada pelanggan. Jika tidak ditentukan, harga yang tersimpan dari produk akan digunakan.

**Format**: Diwakili dalam denominasi terendah dari mata uang (misalnya, sen untuk USD). Misalnya, untuk dikenakan biaya $1,00, kirim `100`.
</ParamField>

<ParamField body="product_currency" type="string">
Mata uang opsional dari harga produk. Jika tidak ditentukan, secara default menggunakan mata uang produk.
</ParamField>

<ParamField body="product_description" type="string">
Override deskripsi produk opsional untuk penagihan dan item baris. Jika tidak ditentukan, deskripsi yang tersimpan dari produk akan digunakan.
</ParamField>

<ParamField body="adaptive_currency_fees_inclusive" type="boolean">
Apakah biaya mata uang adaptif harus termasuk dalam harga produk (true) atau ditambahkan di atas (false). Diabaikan jika harga adaptif tidak diaktifkan.
</ParamField>
</Expandable>
</ParamField>
</Expandable>
...
<Note>
Ketika `confirm` diatur ke `true`, pelanggan akan langsung diarahkan ke halaman checkout, melewati langkah konfirmasi mana pun.
</Note>
...
### 5. Checkout dengan Ganti Mata Uang

<Note>
Override `billing_currency` hanya berfungsi saat mata uang adaptif diaktifkan dalam pengaturan akun Anda. Jika mata uang adaptif dinonaktifkan, parameter ini tidak akan berfungsi.
</Note>
...
Gunakan metode pembayaran yang disimpan pelanggan untuk membuat sesi checkout yang memproses segera, melewati pengumpulan metode pembayaran:
...
<Warning>
Ketika menggunakan `payment_method_id`, `confirm` harus diatur ke `true` dan `customer_id` yang ada harus disediakan. Metode pembayaran akan divalidasi untuk kelayakan dengan mata uang pembayaran.
</Warning>
...
<Tip>
Tautan pendek sangat cocok untuk SMS, email, atau berbagi media sosial. Mereka lebih mudah diingat dan membangun lebih banyak kepercayaan pelanggan dibandingkan URL yang panjang.
</Tip>

### 13. Lewati Halaman Sukses Pembayaran dengan Pengalihan Langsung

Arahkan pelanggan segera setelah pembayaran selesai, melewati halaman sukses default:
...
<Tip>
Gunakan `redirect_immediately: true` ketika Anda memiliki halaman sukses kustom yang memberikan pengalaman pengguna yang lebih baik daripada halaman sukses pembayaran default. Ini sangat berguna untuk aplikasi seluler dan alur checkout tertanam.
</Tip>

<Note>
Ketika `redirect_immediately` diaktifkan, pelanggan diarahkan ke `return_url` Anda segera setelah pembayaran selesai, sepenuhnya melewati halaman sukses default.
</Note>

### 14. Memaksa Sebuah Bahasa
...
<Tip>
Gunakan `force_language` ketika Anda tahu bahasa yang diinginkan pelanggan (misalnya, dari pengaturan akun mereka) atau saat menargetkan pasar regional tertentu.
</Tip>

**Bahasa yang Didukung:** Arab (`ar`), Katalan (`ca`), Cina (`zh`), Belanda (`nl`), Inggris (`en`), Prancis (`fr`), Jerman (`de`), Ibrani (`he`), Indonesia (`id`), Italia (`it`), Jepang (`ja`), Korea (`ko`), Melayu (`ms`), Polandia (`pl`), Portugis (`pt`), Rumania (`ro`), Rusia (`ru`), Spanyol (`es`), Swedia (`sv`), Thai (`th`), Turki (`tr`)
