...
<Step title="Create Webhook Endpoint">
    ...
    <Frame>
        <img src="/images/webhooks/create-endpoint.png" alt="Tambah Webhook" style={{ maxHeight: '500px', width: 'auto' }} />
    </Frame>
</Step>
...
<Expandable title="Data object properties">
  ...
  Field tambahan berbeda-beda berdasarkan jenis event. Lihat dokumentasi khusus event untuk skema lengkap.
</Expandable>
</ResponseField>
...
<Frame>
    <img src="/images/webhooks/endpoints.png" alt="Detail Endpoint" style={{ maxHeight: '500px', width: 'auto' }} />
</Frame>
...
Perintah trigger mencakup event subscription, payment, refund, dispute, dan license key — lihat [Supported Webhook Events](/developer-resources/sdks/cli#supported-webhook-events) untuk daftar lengkapnya. Event payout, credit, abandoned checkout, dunning, dan entitlement grant belum tersedia sebagai mock trigger; uji event tersebut menggunakan transaksi test-mode nyata dengan `dodo wh listen`.

<Warning>
Mock payload webhook dari `dodo wh trigger` tidak ditandatangani. Gunakan `unsafe_unwrap()`, bukan `unwrap()`, di webhook handler Anda hanya selama pengujian.
</Warning>
...
Tab Advanced Settings menyediakan opsi konfigurasi tambahan untuk menyempurnakan perilaku endpoint webhook Anda.

### Rate Limiting (Throttling)

Kontrol laju pengiriman event webhook ke endpoint Anda untuk mencegah sistem Anda kewalahan.
...
<Step title="Configure Rate Limit">
    Klik tombol **Edit** untuk mengubah pengaturan rate limit.
    
    <Info>
    Secara default, webhook menerapkan "No rate limit", yang berarti event dikirim segera setelah terjadi.
    </Info>
</Step>
...
<Tip>
Gunakan rate limiting jika webhook handler Anda memerlukan waktu untuk memproses event atau jika Anda ingin mengelompokkan beberapa event secara bersamaan.
</Tip>

### Custom Headers

Tambahkan HTTP headers khusus ke semua permintaan webhook yang dikirim ke endpoint Anda. Fitur ini berguna untuk authentication, routing, atau menambahkan metadata ke permintaan webhook Anda.

<Steps>
<Step title="Add Custom Header">
    Di bagian "Custom Headers", masukkan **Key** dan **Value** untuk header khusus Anda.
</Step>

<Step title="Add Multiple Headers">
    Klik tombol **+** untuk menambahkan custom headers tambahan sesuai kebutuhan.
</Step>

<Step title="Save Configuration">
    Custom headers Anda akan disertakan dalam semua permintaan webhook ke endpoint ini.
</Step>
</Steps>

### Transformations

Transformations memungkinkan Anda mengubah payload webhook dan mengarahkannya ke URL yang berbeda. Fitur canggih ini memungkinkan Anda untuk:
...
<Step title="Configure Transformation">
    Klik **Edit transformation** untuk menentukan aturan transformation Anda.
    
    <Info>
    Anda dapat menggunakan JavaScript untuk mengubah payload webhook dan menentukan target URL yang berbeda.
    </Info>
</Step>

<Step title="Test Transformation">
    Gunakan interface pengujian untuk memastikan transformation Anda berfungsi dengan benar sebelum digunakan di production.
</Step>
</Steps>

<Warning>
Transformation dapat berdampak signifikan pada performa pengiriman webhook. Lakukan pengujian secara menyeluruh dan jaga agar logika transformation tetap sederhana serta efisien.
</Warning>
...
- Mengarahkan event ke microservices yang berbeda
</Tip>
...
Tab Activity memberikan insight real-time tentang performa pengiriman webhook Anda melalui analitik visual.
...
Dapatkan informasi terbaru tentang kesehatan webhook Anda melalui notifikasi email otomatis. Saat pengiriman webhook mulai gagal atau endpoint Anda berhenti merespons, Anda akan menerima email alerts agar dapat segera menangani masalah dan menjaga integrasi tetap berjalan lancar.
...
### Mengaktifkan Email Alerts

<Steps>
<Step title="Navigate to Alerting Settings">
  Buka Dashboard Dodo Payments dan navigasikan ke **Dashboard → Webhooks → Alerting**.
</Step>

<Step title="Enable Email Notifications">
  Aktifkan **Email notifications** untuk mulai menerima alert tentang masalah pengiriman webhook.
</Step>

<Step title="Configure Email Address">
  Masukkan alamat email tempat Anda ingin menerima alert webhook. Kami akan mengirimkan notifikasi ke alamat ini ketika event tertentu terjadi pada setup webhook Anda, seperti masalah pengiriman yang dapat memengaruhi integrasi Anda.
</Step>
</Steps>

<Tip>
Aktifkan email alerts untuk mendeteksi masalah pengiriman webhook sejak dini dan mempertahankan integrasi yang andal. Anda akan diberi tahu saat pengiriman gagal atau endpoint Anda menjadi tidak responsif.
</Tip>

## Deploy ke Cloud Platforms

Siap men-deploy webhook handler Anda ke production? Kami menyediakan panduan khusus platform untuk membantu Anda men-deploy webhook ke cloud provider populer, lengkap dengan praktik terbaik untuk setiap platform.
