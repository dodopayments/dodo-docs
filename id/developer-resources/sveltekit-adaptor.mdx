---
title: Adaptor SvelteKit
description: Pelajari cara mengintegrasikan Dodo Payments dengan proyek
  SvelteKit App Router Anda menggunakan Adaptor SvelteKit kami. Mencakup
  checkout, portal pelanggan, webhook, dan pengaturan lingkungan yang aman.
icon: code-merge
keywords:
  - Dodo Payments
  - SvelteKit
...
<Card title="Checkout Handler" icon="cart-shopping" href="#checkout-route-handler">
    Integrasikan checkout Dodo Payments ke dalam aplikasi SvelteKit Anda.
</Card>
...
<Warning>
Jangan pernah commit file <code>.env</code> atau rahasia ke kontrol versi.
</Warning>
</Step>
</Steps>
...
<Info>
Semua contoh mengasumsikan Anda menggunakan SvelteKit App Router.
</Info>

<Tabs>
<Tab title="Checkout Handler">
  <Info>
  Gunakan handler ini untuk mengintegrasikan checkout Dodo Payments ke dalam aplikasi SvelteKit Anda. Mendukung alur pembayaran statis (GET), dinamis (POST), dan sesi (POST).
  </Info>
  <CodeGroup>
...
<Tab title="Customer Portal Handler">
  <Info>
  Gunakan handler ini untuk memungkinkan pelanggan mengelola langganan dan detail melalui portal pelanggan Dodo Payments.
  </Info>
  <CodeGroup>
...
<Tab title="Webhook Handler">
  <Info>
  Gunakan handler ini untuk menerima dan memproses event webhook Dodo Payments secara aman di aplikasi SvelteKit Anda.
  </Info>
  <CodeGroup>
...
Menetapkan jumlah yang dikenakan biaya, dalam satuan utama mata uang (misalnya, <code>12.5</code> untuk $12.50). Hanya untuk produk Pay What You Want dan diabaikan jika nilainya di bawah harga minimum produk.
</ParamField>
...
<Warning>
Jika <code>productId</code> hilang, handler mengembalikan respons 400. Parameter kueri tidak valid juga menghasilkan respons 400.
</Warning>
...
<Warning>
Dynamic Checkout memproksikan endpoint `POST /payments` dan `POST /subscriptions` yang sudah tidak digunakan lagi. Fitur ini tetap berfungsi untuk integrasi yang sudah ada, tetapi integrasi baru sebaiknya menggunakan **Checkout Sessions** di bawah.
</Warning>

### Format Respons
Dynamic checkout mengembalikan respons JSON dengan URL checkout:
...
Checkout sessions menyediakan pengalaman checkout yang di-host dengan keamanan lebih baik, yang menangani seluruh alur pembayaran untuk pembelian satu kali dan subscription dengan kontrol kustomisasi penuh.

Lihat [Panduan Integrasi Checkout Sessions](https://docs.dodopayments.com/developer-resources/checkout-session) untuk detail selengkapnya dan daftar lengkap field yang didukung.
...
## Route Handler Customer Portal

Route Handler Customer Portal memungkinkan Anda mengintegrasikan customer portal Dodo Payments ke dalam aplikasi SvelteKit Anda dengan lancar.
...
- **Verifikasi Signature:** Memverifikasi signature webhook menggunakan <code>webhookKey</code>. Mengembalikan 401 jika verifikasi gagal.
...
- **Routing Event:** Memanggil event handler yang sesuai berdasarkan jenis payload.
