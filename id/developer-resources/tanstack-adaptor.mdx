...
Menetapkan jumlah yang dibebankan dalam unit utama mata uang (misalnya, <code>12.5</code> untuk $12.50). Hanya untuk produk Pay What You Want, dan diabaikan jika berada di bawah harga minimum produk.
</ParamField>
...
<Warning>
Dynamic Checkout memproksikan endpoint `POST /payments` dan `POST /subscriptions` yang sudah deprecated. Dynamic Checkout tetap berfungsi untuk integrasi yang sudah ada, tetapi integrasi baru sebaiknya menggunakan **Checkout Sessions** di bawah ini.
</Warning>
...
Checkout sessions menyediakan pengalaman checkout hosted yang lebih aman dan menangani seluruh alur pembayaran untuk pembelian satu kali maupun subscription, dengan kontrol kustomisasi penuh.
...
Customer Portal Route Handler memungkinkan Anda mengintegrasikan customer portal Dodo Payments ke dalam aplikasi Tanstack Anda dengan lancar.
