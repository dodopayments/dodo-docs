...
ACH Direct Debit can be tested in Test Mode using a simulated bank authorization flow.
...
Follow the simulated bank authorization flow, then confirm your webhook handler receives the final payment status — ACH payments settle asynchronously rather than at checkout.
</Step>
</Steps>
...
If the customer verifies with micro-deposits, use the amounts `32` and `45` (or descriptor code `SM11AA`) to verify the account successfully.

For the full list of test accounts and micro-deposit scenarios, see the [ACH Direct Debit](/features/payment-methods/ach#testing) page.
...
For the full list of test IBANs, including per-country values, see the [Europe](/features/payment-methods/europe#testing) page.
