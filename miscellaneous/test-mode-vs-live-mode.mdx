...
icon: "toggle-on"
keywords: ["Dodo Payments", "test mode vs live mode"]
---
...
Live Mode is available from day 1, but **live payments and payouts** are enabled only after Account Verification is complete. You can accept real payments only in \`live mode\`. See [Account Verification](/miscellaneous/verification-process) for the required steps.
...
2. API keys and Webhooks created in test mode will only be limited to test mode environment and will not affect live mode deployment and vice versa. You can use the [CLI webhook listener](/developer-resources/sdks/cli#listen-for-webhooks) (`dodo wh listen`) to forward test mode webhooks to your local development server in real time
...
Please watch this video on how you can perform a transaction in Test Mode.
