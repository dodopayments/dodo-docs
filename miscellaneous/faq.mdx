...
<Accordion title="Q1: How do I login to Dodo Payments Dashboard?">
  A: Visit our dashboard and enter your account email. Dodo Payments uses passwordless login — we send a **6-digit one-time code (OTP)** to your email, which you enter to sign in. No password is required. In case you face any issues please reach out to our support team.
</Accordion>
...
<Accordion title="Q7: I want to integrate my other business with Dodo payments. Should I create a new account?">
  A: No, you can manage multiple businesses through a single account. To add a new business, click the **+** icon in the top-left corner of the dashboard next to your current business name, select **Add New**, then **Add New Business**.
You can refer to [Managing Multiple Businesses](/miscellaneous/accounts#managing-multiple-businesses) for more details
</Accordion>

<Accordion title="Q8: Should I wait for verification to complete before integrating dodo payments in my system?">
  A: No, there is no need for you to wait. You can start integrating using the Test mode and once your verification is successful you can start making Live payments in production. Learn more about [Test Mode vs Live Mode](/miscellaneous/test-mode-vs-live-mode).
</Accordion>
...
<Accordion title="Q10: Does Dodo Payments have an affiliate program?">
  A: Yes — you can run your own affiliate program through our integrations with affiliate partners **[Affonso](https://affonso.io)** and **[Dub Partners](https://dub.co)**. These integrations handle commission attribution, real-time reporting, and referral tracking on top of your Dodo Payments checkout. See the [affiliates documentation](/features/affiliates) for setup steps.
</Accordion>
...
<Accordion title="Q20: My INR subscription product is showing USD on checkout. How do I fix that?">
  A: You can set a product's base price in any currency Dodo Payments can charge, so check that the product's base currency is INR. Customers outside your base currency are billed via [Adaptive Currency](/features/adaptive-currency), so enable Adaptive Currency to display prices in the customer's local currency and unlock the corresponding payment methods.
</Accordion>
...
<Accordion title="Q38: Why is my ID verification form failing?">
  ...
  Identity Verification does not reopen automatically after a failed attempt, so you may see that resubmissions are paused. Email support@dodopayments.com from your registered email address to have the form re-enabled. See [If Identity Verification Fails](/miscellaneous/verification-process#if-identity-verification-fails) for the full list of causes and what to prepare before retrying.
</Accordion>
...
<Accordion title="Q40: What details are required in the Product Verification Form (PVF)?">
  ...
  Your site also needs to publicly show your pricing, Terms of Service, Privacy Policy, Refund and Cancellation Policy, and a contact route. See [Preparing Your Website](/miscellaneous/verification-process#preparing-your-website) for the full checklist.
</Accordion>
...
<Accordion title="Q54: What should I do if a payment fails?">
  ...
  Look up the specific decline code in the [Transaction Failures](/api-reference/transaction-failures) reference for its meaning and recommended action, and see [Handle Payment Failures](/developer-resources/handle-payment-failures) to handle failures programmatically.
</Accordion>
...
<Accordion title="Q56: What happens to a subscription if a payment fails?">
  A: The subscription is not cancelled immediately. On a failed renewal it first moves to `past_due`, a grace period during which retries and dunning run. The `subscription.past_due` webhook carries `past_due_ends_at`, the moment the grace period ends — at that point the subscription moves to on-hold (or cancelled, depending on your recovery settings) and stays there until a successful payment is made.
  ...
  Dodo Payments can recover these failures for you automatically: [Subscription Payment Retries](/features/recovery/payment-retries) re-attempt soft declines, and [Subscription Dunning](/features/recovery/subscription-dunning) emails the customer to update their payment method.
</Accordion>

<Accordion title="Q57: If I retry payments myself, do I need the failed payment ID?">
  A: It depends on what you want to do. To retry the specific failed payment, use `POST /payments/{payment_id}/retry` — that route needs the failed payment's ID. To simply raise a fresh charge on the subscription, use `POST /subscriptions/{subscription_id}/charge`, which needs only the subscription ID. If you would rather not manage retries yourself, enable [Subscription Payment Retries](/features/recovery/payment-retries) to recover failed renewals automatically.
</Accordion>
...
<Accordion title="Q58: How can a subscription payment fail with INCORRECT_NUMBER if previous cycles were successful?">
  ...
  This can happen after the previous billing cycle. Dodo Payments simply displays the error message returned by the payment provider. See the [Transaction Failures](/api-reference/transaction-failures) reference for what `INCORRECT_NUMBER` and other decline codes mean and how to respond.
</Accordion>
...
<Accordion title="Q60: Why is payment.status sometimes null?">
  ...
  Once set, it is one of: succeeded, failed, cancelled, processing, requires_customer_action, requires_merchant_action, requires_payment_method, requires_confirmation, requires_capture, partially_captured, or partially_captured_and_capturable. The terminal values are succeeded, failed, and cancelled.
...
<Accordion title="Q63: My customer completed payment but I didn't receive webhook?">
  ...
  - Check whether a firewall or source IP allowlist is blocking the request before it reaches your handler — deliveries are sent from a pool of addresses that changes over time, so a rule that once worked can start dropping them. See [Source IP Addresses](/developer-resources/webhooks#source-ip-addresses).
</Accordion>

<Accordion title="Q64: Why is my dashboard showing success, but my app didn't update?">
  A: It's likely you're relying solely on `return_url` query params and not handling:
  - Webhooks
...
<Accordion title="Q66: Does Dodo support UPI for subscriptions?">
  ...
  <Note>
  UPI subscriptions have the same RBI-compliant mandate requirements as Indian card subscriptions. For detailed information, see the [Payment Methods documentation](/features/payment-methods/india).
  </Note>
</Accordion>
...
<Accordion title="Q71: How many webhook events do I get when someone starts a subscription?">
...
<Accordion title="Q74: What does the Change Plan API return?">
  A: The Change Plan API returns `200` with a JSON body containing `payment_id`, `payment_link`, `client_secret`, and `expires_on`. All four fields are nullable and are only populated when the plan change needs a customer-facing checkout to settle — for example when you pass `collect_via_payment_link`. When the change is settled off-session on the saved payment method, the link fields are `null`, so the response can look mostly empty even though the change succeeded. See [Change Plan](/api-reference/subscriptions/change-plan).
</Accordion>
...
<Accordion title="Q82: Do you support ACH payments?">
  A: Yes. ACH Direct Debit is supported for one-time payments from customers with a US billing address paying in USD. It is not available for subscriptions. ACH payments take 4 business days to clear, so only fulfill an order once the payment reaches a succeeded state rather than when the customer authorizes it. See the [ACH Direct Debit](/features/payment-methods/ach) page for setup and testing details.
</Accordion>
...
<Accordion title="Q84: What currencies does Dodo Payments natively support?">
  A: Dodo Payments natively supports three payout currencies: USD, GBP, and EUR. INR remains supported as a transaction currency (UPI, RuPay, Indian-card subscriptions) but is no longer a native payout wallet. For other currencies, you can enable [Adaptive Currency](/features/adaptive-currency) to automatically convert prices to your customer's local currency.
...
<Accordion title="Q87: How do I add an addon to an existing subscription?">
  A: Use the Change Plan API with the same product_id and include the addons array. The base plan is unchanged and the customer is charged a prorated amount immediately.

  The `addons` array **replaces** the subscription's addon list — it is not an append. Send every addon you want the subscription to keep, including ones it already has; leaving it empty removes all existing addons.
...
<Accordion title="Q88: How long can a subscription stay in On-Hold state?">
  A: A subscription stays On-Hold until the customer updates their payment method and a successful payment is made. The payment method can be updated at any time — from the Customer Portal or via the [Update Payment Method API](/api-reference/subscriptions/update-payment-method) — whether or not a payment has failed.
</Accordion>
...
<Accordion title="Q91: Can I switch a product to a different brand?">
  A: Yes. While the brand cannot be switched during product creation, you can edit an existing product and change its brand from the product edit page.
</Accordion>

<Accordion title="Q92: How does seat-based billing pricing work for subscriptions?">
  A: Seat-based billing combines a base subscription with a per-seat addon. For example, a Team Pro subscription costs $99/month and includes 5 seats — if you have more than 5 users, you pay an additional $15/month for each extra seat.

  See the [Seat-Based Pricing guide](/developer-resources/seat-based-pricing) for how to configure this.
</Accordion>

<Accordion title="Q93: How do I charge less in price-sensitive countries like India or Brazil?">
  A: Use [Purchasing Power Parity](/features/purchasing-power-parity). You set one percentage per country, turn it on for the product prices you choose, and Dodo Payments applies it automatically at checkout based on the customer's billing country. It requires [Adaptive Currency](/features/adaptive-currency), which you enable under **Settings → Business**. If you would rather set an exact price per country, use [Localized Pricing](/features/localized-pricing) instead.
</Accordion>

<Accordion title="Q94: How do I show clean prices like 49.99 instead of converted amounts like 52.37?">
  A: Enable [Charm Pricing](/features/charm-pricing) under **Settings → Business**. It rounds the converted price to a clean ending in each currency, so customers see a familiar number instead of a raw conversion. It runs after currency conversion and also requires [Adaptive Currency](/features/adaptive-currency).
</Accordion>

## Payouts & Bank Integration

<Accordion title="Q95: How do payouts work with Dodo Payments?">
...
<Accordion title="Q96: What is the minimum threshold amount after fees and taxes to process payouts?">
  ...
  <Note>
  The flexible threshold feature is available for the USD/EUR/GBP wallets. Support for the INR native wallet has been discontinued.
  </Note>
</Accordion>

<Accordion title="Q97: How do I link my bank account to receive payouts?">
...
<Accordion title="Q98: Can I track the status of my payouts?">
  A: Yes, you can track your payout status from the **Payouts** section of the dashboard.
</Accordion>

<Accordion title="Q99: What are payout fees and how much are they?">
...
<Accordion title="Q100: I received a &quot;payout processed&quot; email — when will the funds arrive?">
...
<Accordion title="Q101: How do I add Payoneer as a payout account?">
...
<Accordion title="Q102: Can I receive payouts to a Payoneer account?">
...
<Accordion title="Q103: My bank info is still in review — what should I do?">
...
<Accordion title="Q104: What happens if my payout day falls on a Friday or weekend?">
...
<Accordion title="Q105: How can I expedite payout compliance if I have an urgent launch or ad campaign planned?">
...
<Accordion title="Q106: I submitted the wrong bank account details. How do I update them?">
...
<Accordion title="Q107: Can I use my personal bank account if I have a registered company?">
  A: No. If you're registered as a company or legal entity, you must use the entity's bank account.
</Accordion>

<Accordion title="Q108: My country doesn't use SWIFT codes. How will that affect my verification?">
...
<Accordion title="Q109: What alternative documentation can I provide if my country doesn't use SWIFT codes?">
...
<Accordion title="Q110: Can I set a payout threshold lower than $50?">
...
<Accordion title="Q111: Do you support daily payouts?">
...
<Accordion title="Q112: Does Dodo Payments generate invoices automatically?">
...
<Accordion title="Q113: Can I customize the invoices?">
...
<Accordion title="Q114: How do I access or download an invoice?">
...
<Accordion title="Q115: How do I view reports on my transactions and payouts?">
...
<Accordion title="Q116: Can I change the billing address on my invoice?">
  A: Yes. Go to the Customer Portal, download the invoice, and select **Download with full address details**. You can then update the billing address.

  This option is currently available for the following countries: AU, BR, CA, FR, DE, IT, NL, PT, ES, GB, and US.
</Accordion>

<Accordion title="Q117: If I change the billing address, will it update all my existing invoices?">
  A: No. The address change applies only to the specific invoice you are downloading. It will not update the billing address on your other existing invoices.
</Accordion>

## Refunds & Disputes

<Accordion title="Q118: How do I issue a refund?">
...
<Accordion title="Q119: How long does it take for a customer to receive their refund?">
...
<Accordion title="Q120: I'm facing a dispute or chargeback. What can I do?">
...
<Accordion title="Q121: The customer disputed the charge after using the service. Can I appeal this?">
  A: Yes — you can:
...
<Accordion title="Q122: Why is my refund failing with an 'insufficient funds' error?">
...
<Accordion title="Q123: How is proration calculated — on the product price or the total amount?">
...
<Accordion title="Q124: Why do we use the `subscription.renewed` webhook for the first subscription?">
...
<Accordion title="Q125: Can I manually verify Dodo Payments webhooks in Python?">
...
<Accordion title="Q126: Where can I find official docs on manual webhook verification?">
...
<Accordion title="Q127: My webhook isn't receiving a POST request, only a GET. Why?">
...
<Accordion title="Q128: I get a 401 Unauthorized error using the Python SDK. Why?">
...
<Accordion title="Q129: Is there a way to manually trigger a webhook event for testing?">
A: You can do it through the webhook dashboard. Go to Dashboard -> Developers -> Webhooks
</Accordion>

<Accordion title="Q130: What's the difference between webhook and redirect URL?">
A:
...
<Accordion title="Q131: Why didn't I receive the product id in the payment.succeeded webhook for a subscription product?">
...
<Accordion title="Q132: How do I verify the webhooks I receive are actually from dodo payments and not from any malicious actors?">
...
<Accordion title="Q133: Why am I receiving a subscription.renewed webhook even for the first month payment?">
A: For subscriptions, `subscription.renewed` will be triggered whenever the subscription amount is deducted. So yes, this is expected behavior.
</Accordion>

<Accordion title="Q134: How can I test webhooks locally?">
...
<Accordion title="Q135: Why is my webhook not being called after payment?">
...
<Accordion title="Q136: Is it safe to rely on status=success in URL query params?">
A: No. While it's a quick win, URL query params can be tampered with.

If you're using `return_url?status=success`:
...
<Accordion title="Q137: Can I integrate Dodo Payments with my existing website or platform?">
...
<Accordion title="Q138: Why are my webhooks always failing?">
  A: Common causes:
...
<Accordion title="Q139: Where do I find my API key?">
...
<Accordion title="Q140: What are the base URLs for the Dodo Payments API?">
...
<Accordion title="Q141: Can I test subscriptions in Test Mode?">
A: Yes — subscriptions can be tested fully in Test Mode.

Just don't use real card numbers.
</Accordion>

<Accordion title="Q142: How do I switch between Test and Live Mode?">
A:
...
<Accordion title="Q143: Can I use real customer data in Test Mode?">
...
<Accordion title="Q144: Can I advance the next billing date in test mode?">
...
<Accordion title="Q145: Why don't I receive emails in Test Mode?">
...
<Accordion title="Q146: How do I contact support?">
...
<Accordion title="Q147: My support messages are getting no response. What should I do?">
...
<Accordion title="Q148: How do I unsubscribe a customer from Dodo Digest emails?">
  A: Share the customer's email address with us via Intercom and we'll unsubscribe them from our end.
</Accordion>

## Security & Fraud Prevention

<Accordion title="Q149: How does Dodo Payments protect my business from fraud?">
...
<Accordion title="Q150: Is my customer data secure with Dodo Payments?">
...
<Accordion title="Q151: I found a security vulnerability in Dodo Payments. How should I report it?">
A: Please report any discovered vulnerabilities directly to our security team at pt-team@dodopayments.com
</Accordion>

<Accordion title="Q152: Does Dodo Payments offer bug bounties?">
A: We currently do not offer monetary bounties, but we do provide Dodo Payments merchandise as a token of appreciation.
</Accordion>

<Accordion title="Q153: How are customers protected from fraud on Dodo Payments?">
...
<Accordion title="Q154: Why is my payment status showing 'Not Initiated'?">
  A: This means the payment link was created, but the user never opened the payment page or attempted the payment.
</Accordion>

<Accordion title="Q155: Why is my payment status showing 'Requires Payment Method'?">
...
<Accordion title="Q156: Why am I getting an 'Invalid authorization details' error?">
  A: This usually happens when the 3D Secure (3DS) authentication fails or is not completed by the user.
</Accordion>


<Accordion title="Q157: Why am I getting an 'Authentication failure' error for payments?">
...
<Accordion title="Q158: I'm seeing a 500 INTERNAL_SERVER_ERROR (&quot;Internal Server Error&quot;) in test mode. How do I fix it?">
  ...
  Refer to the [test card numbers documentation](https://docs.dodopayments.com/miscellaneous/testing-process#test-cards).
</Accordion>

<Accordion title="Q159: Why am I getting PAYMENT_METHOD_UNSUPPORTED (&quot;mode not enabled for merchant&quot;) in test mode?">
...
<Accordion title="Q160: What does the requires_customer_action status mean?">
...
<Accordion title="Q161: Why am I getting a SUBSCRIPTION_NOT_ACTIVE error?">
...
<Accordion title="Q162: Why am I getting a payment failure with UNKNOWN_ERROR?">
...
<Accordion title="Q163: I'm getting: &quot;You must provide a mandate for off-session card payments&quot;">
  A: It means:
...
<Accordion title="Q164: Is the customer portal tied to a specific customer?">
...
<Accordion title="Q167: Can I integrate Dodo Payments in an Android or mobile app?">
  A: Yes. Refer to the [mobile integration guide](/developer-resources/mobile-integration).

You can also use the [Sentra VS Code extension](/developer-resources/sentra) for integration assistance.
</Accordion>

<Accordion title="Q168: How do I find my live publishable key (pk_live_...)?">
...
<Accordion title="Q169: Does Dodo Payments support WooCommerce blocks (WordPress block editor)?">
...
<Accordion title="Q170: Why can't I see the upgrade/downgrade button in my customer portal?">
  A: There are two things you need to do:
  - Go to **Settings → Subscriptions → Allow Subscription Updates** and turn this toggle on.
  - Add the products you want customers to be able to upgrade or downgrade to a **Product Collection**.

  Once this is enabled and the products are added to the collection, customers will be able to upgrade or downgrade their subscription through the customer portal.
</Accordion>

<Accordion title="Q171: How many discount codes can be applied in one checkout?">
  A: Up to 20 codes can be stacked via the `discount_codes` array (max 20 entries). The older singular `discount_code` field is deprecated and cannot be used together with `discount_codes`.
</Accordion>

<Accordion title="Q172: How do I change my login email?">
  A: You can change your login email from your account settings. The new email will be updated across all your businesses.
...
<Accordion title="Q173: Why am I getting the business_id as my brand_id?">
  A: When a business is created, a primary brand is automatically created for it. For this primary brand, the `brand_id` is the same as the `business_id`.

  If you create additional brands under the same business, each of them will have a unique `brand_id`.
</Accordion>

<Accordion title="Q174: What are the limitations when adding Notion content?">
  A: Notion pages cannot be added if they exceed the supported file size limit. We also currently do not support Notion databases.
</Accordion>